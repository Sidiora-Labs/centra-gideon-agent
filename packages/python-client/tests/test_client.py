...
class TestAuthCheck:
    ...
    def test_remote_with_token_ok(self):
        client = GideonClient(base_url="http://example.com:7777", token="test")
        client._check_auth()
...
class TestMessageValidation:
    ...
    async def test_rejects_long_message(self):
        client = GideonClient(message_length_limit=100)
        with pytest.raises(GideonError) as exc_info:
            await client.send_message("session-1", "x" * 101)
    ...
    async def test_rejects_long_notification(self):
        client = GideonClient(message_length_limit=100)
        with pytest.raises(GideonError) as exc_info:
            await client.send_notification("x" * 101)
        assert exc_info.value.code == ErrorCode.VALIDATION_ERROR
...
class TestMcpValidation:
    ...
    async def test_rejects_empty_name(self):
        client = GideonClient()
        with pytest.raises(GideonError) as exc_info:
            await client.register_mcp_server("", "node server.js")
        assert exc_info.value.code == ErrorCode.VALIDATION_ERROR
    ...
    async def test_rejects_empty_command(self):
        client = GideonClient()
        with pytest.raises(GideonError) as exc_info:
            await client.register_mcp_server("my-server", "")
        assert exc_info.value.code == ErrorCode.VALIDATION_ERROR
...
class TestContextBuffer:
    ...
    async def test_null_session_buffers_locally(self):
        client = GideonClient()
        await client.inject_context(None, "test content")
        assert client.pending_context_count == 1

    @pytest.mark.asyncio
    async def test_buffer_limit_fifo(self):
        client = GideonClient()
        for i in range(60):
            await client.inject_context(None, f"entry-{i}")
        assert client.pending_context_count == 50

    @pytest.mark.asyncio
    async def test_flush_preserves_failed_entries(self):
        client = GideonClient(max_retries=0)
        await client.inject_context(None, "a")
        await client.inject_context(None, "b")
        assert client.pending_context_count == 2
        with pytest.raises(Exception):
            await client.flush_pending_context("session-1")
...
class TestAppDataDir:
    def test_contains_app_name(self):
        client = GideonClient(app_name="my-tool")
        d = client.get_app_data_dir()
