...
An async Python client for the Gideon Gateway.
...
```sh
pip install ./packages/python-client
# or
uv pip install ./packages/python-client
```
...
async with GideonClient(app_name="my-app") as client:
    ok = await client.ping()
    status = await client.get_status()
    await client.send_message("session-1", "hello")
```
