...
class TestTypedStore:
    def test_create_typed_note(self, store):
        nid = store.create_typed_item(
            item_type="note", title="Idea", content="a b c", tags=["x"]
        )
    ...
    def test_create_bookmark_records_url(self, store):
        bid = store.create_typed_item(
            item_type="bookmark", title="X", url="https://e.com"
        )
    ...
    def test_tags_serialize_as_array(self, store):
        nid = store.create_typed_item(
            item_type="note", title="T", content="", tags=["a", "b"]
        )
    ...
    def test_update_typed_fields(self, store):
        ...
        item = store.get_item(nid)
        assert (
            item["is_pinned"] is True
            and item["tags"] == ["z"]
            and item["url"] == "https://u"
        )
...
class TestHandlers:
        ...
        resp = _run(
            H.create_item(
                _req(
                    store,
                    "POST",
                    {"type": "note", "title": "N", "content": "hi", "tags": ["t"]},
                )
            )
        )
    ...
    def test_create_bookmark_requires_url(self, store):
        ...
        resp = _run(
            H.create_item(_req(store, "POST", {"type": "bookmark", "title": "X"}))
        )
    ...
    def test_create_bookmark_rejects_non_http_scheme(self, store):
        ...
        for bad in (
            ...
            resp = _run(
                H.create_item(_req(store, "POST", {"type": "bookmark", "url": bad}))
            )
            assert resp.status == 400, f"{bad!r} should be rejected"
        ok = _run(
            H.create_item(
                _req(store, "POST", {"type": "bookmark", "url": "https://example.com"})
            )
        )
    ...
    def test_create_rejects_whitespace_only_content(self, store):
        ...
        resp = _run(
            H.create_item(_req(store, "POST", {"type": "note", "content": "   \n  "}))
        )
        assert resp.status == 400
    ...
    def test_bookmark_dedup_returns_existing(self, store):
        ...
        r1 = _run(
            H.create_item(
                _req(
                    store, "POST", {"type": "bookmark", "url": "https://example.com/x"}
                )
            )
        )
        ...
        assert json.loads(r2.body)["id"] == first_id
        r3 = _run(
            H.create_item(
                _req(
                    store, "POST", {"type": "bookmark", "url": "https://example.com/y"}
                )
            )
        )
    ...
    def test_create_rejects_unknown_type(self, store):
        ...
        resp = _run(
            H.create_item(
                _req(store, "POST", {"type": "nonsense", "title": "X", "content": "y"})
            )
        )
        assert resp.status == 400
    ...
    def test_create_rejects_media_type_via_json(self, store):
        ...
        ok = _run(
            H.create_item(
                _req(store, "POST", {"type": "note", "title": "n", "content": "c"})
            )
        )
        assert ok.status == 201
    ...
    def test_journal_editable_same_day(self, store):
        ...
        jid = store.create_typed_item(
            item_type="journal", title="J", content="today's entry"
        )
        resp = _run(
            H.update_item(
                _req(store, "PATCH", {"content": "edited same day"}, {"id": jid})
            )
        )
    ...
    def test_journal_immutable_after_creation_day(self, store):
        ...
        jid = store.create_typed_item(
            item_type="journal", title="J", content="yesterday's entry"
        )
        # Backdate creation to a prior day.
        store.db.execute(
            "UPDATE items SET created_at = '2026-01-01T08:00:00' WHERE id = ?", (jid,)
        )
        store.db.commit()
        resp = _run(
            H.update_item(
                _req(store, "PATCH", {"content": "rewrite history"}, {"id": jid})
            )
        )
    ...
    def test_note_always_editable(self, store):
        ...
        store.db.execute(
            "UPDATE items SET created_at = '2026-01-01T08:00:00' WHERE id = ?", (nid,)
        )
        store.db.commit()
        resp = _run(
            H.update_item(
                _req(store, "PATCH", {"content": "edited later"}, {"id": nid})
            )
        )
        assert resp.status == 200
    ...
    def test_cannot_change_text_item_to_media_type(self, store):
        ...
        bad = _run(
            H.update_item(
                _req(
                    store, "PATCH", {"type": "image", "item_type": "image"}, {"id": nid}
                )
            )
        )
        assert bad.status == 400 and store.get_item(nid)["type"] == "note"
        bad2 = _run(
            H.update_item(_req(store, "PATCH", {"item_type": "bookmark"}, {"id": nid}))
        )
        assert bad2.status == 400
        ok = _run(
            H.update_item(_req(store, "PATCH", {"item_type": "gist"}, {"id": nid}))
        )
    ...
    def test_content_edit_reenriches(self, store):
        ...
        resp = _run(
            H.update_item(
                _req(store, "PATCH", {"content": "new body"}, {"id": nid}, enqueued=enq)
            )
        )
        ...
        resp2 = _run(
            H.update_item(
                _req(store, "PATCH", {"tags": ["x"]}, {"id": nid}, enqueued=enq2)
            )
        )
    ...
    def test_content_edit_reingest_false_skips_enrich(self, store):
        ...
        assert enq == []
        assert store.get_item(nid)["content"] == "fixed typo"
    ...
    def test_list_filters_by_provider(self, store):
        ...
        assert [i["title"] for i in native["items"]] == ["native one"] and native[
            "total"
        ] == 1
        notion = self._list(store, "provider=notion")
        assert [i["title"] for i in notion["items"]] == ["from notion"] and notion[
            "total"
        ] == 1
    ...
    def test_list_truncates_large_content(self, store):
        ...
        full = json.loads(
            _run(H.get_item(_req(store, "GET", match_info={"id": iid}))).body
        )
    ...
    def test_list_entities_parses_aliases_and_redacts_description(self, store):
        ...
        assert ent["aliases"] == ["db", "deployer"]
        assert (
            "AKIAIOSFODNN7EXAMPLE" not in ent["description"]
            and "REDACTED" in ent["description"]
        )
...
class TestKnowledgeTools:
    def test_create_search_get_round_trip(self, tmp_path):
        ...
        with (
            ...
            patch(
                "gideon.cognition.knowledge.knowledge_db_path",
                lambda: os.path.join(tmp_path, "k.db"),
            ),
        ):
            r = _run(
                prov.invoke(
    ...
    def test_create_untitled_journal_gets_date_title(self, tmp_path):
        ...
        with (
            ...
            r = _run(
                prov.invoke(
                    "knowledge_create",
                    {"type": "journal", "content": "Daily reflection."},
                )
            )
            assert r.success
            iid = r.output.rsplit(" ", 1)[-1]
    ...
    def test_update_sets_gist_language_only_for_gists(self, tmp_path):
        ...
        with (
            ...
            gid = store.create_typed_item(
                item_type="gist", title="snip", content="SELECT 1"
            )
            nid = store.create_typed_item(item_type="note", title="n", content="hello")
            store.db.commit()

            r = _run(
                prov.invoke("knowledge_update", {"id": gid, "gist_language": "sql"})
            )
            assert r.success
            assert store.get_item(gid)["gist_language"] == "sql"

            r2 = _run(
                prov.invoke("knowledge_update", {"id": nid, "gist_language": "sql"})
            )
    ...
    def test_update_enforces_journal_immutability(self, tmp_path):
        ...
        with (
            ...
            r = _run(
                prov.invoke(
                    "knowledge_update", {"id": jid, "content": "rewrite history"}
                )
            )
            ...
            jt = store.create_typed_item(
                item_type="journal", title="today", content="x"
            )
            store.db.commit()
    ...
    def test_get_signals_pending_enrichment(self, tmp_path):
        ...
        with (
            ...
            assert (
                "still enriching"
                not in _run(prov.invoke("knowledge_get", {"id": iid})).output
            )
    ...
    def test_update_round_trip(self, tmp_path):
        ...
        with (
            ...
            r = _run(
                prov.invoke(
                    "knowledge_create",
                    {"type": "note", "title": "Old", "content": "old body"},
                )
            )
            item_id = r.output.rsplit(" ", 1)[-1]
            u = _run(
                prov.invoke(
                    "knowledge_update", {"id": item_id, "title": "New", "tags": ["x"]}
                )
            )
    ...
    def test_update_requires_a_field(self, tmp_path):
        ...
        with (
            ...
            r = _run(
                prov.invoke(
                    "knowledge_create", {"type": "note", "title": "T", "content": "c"}
                )
            )
            item_id = r.output.rsplit(" ", 1)[-1]
...
class TestInsightsExtractor:
    def test_parses_category_keyed_blob(self):
        ...
        out = _run(
            InsightsExtractor(pool=_FakePool(resp)).extract("boil water in a kettle")
        )
    ...
    def test_empty_on_unparseable(self):
        ...
        assert (
            _run(InsightsExtractor(pool=_FakePool("not json at all")).extract("x"))
            == {}
        )
...
class TestMediaClassify:
    def test_classify_and_binary(self):
        ...
        assert (
            media.classify("scan.tiff") == "image"
            and media.classify("scan.tif") == "image"
        )
    ...
    def test_classify_routes_source_code_to_gist(self):
        ...
        assert (
            media.classify("algo.py") == "gist"
            and media.code_language("algo.py") == "python"
        )
        assert (
            media.classify("app.ts") == "gist"
            and media.code_language("app.ts") == "typescript"
        )
        assert (
            media.classify("main.go") == "gist"
            and media.code_language("main.go") == "go"
        )
        assert (
            media.classify("schema.sql") == "gist"
            and media.code_language("schema.sql") == "sql"
        )
        assert (
            media.classify("lib.rs") == "gist"
            and media.code_language("lib.rs") == "rust"
        )
        assert (
            media.classify("readme.md") == "document"
            and media.code_language("readme.md") is None
        )
    ...
    def test_classify_uses_mime_to_disambiguate_webm(self):
        ...
        assert media.classify("x.webm") == "video"
...
class TestMediaItems:
    def _files_dir(self, tmp_path, monkeypatch):
        ...
        d.mkdir()
        monkeypatch.setattr(
            "gideon.cognition.knowledge.knowledge_files_dir", lambda: str(d)
        )
        return d
    ...
    def test_store_file_item_document_is_one_logical_doc(
        self, store, tmp_path, monkeypatch
    ):
    ...
    def test_serve_path_guard_rejects_outside_root(self, store, tmp_path, monkeypatch):
        ...
        iid = store.create_typed_item(
            item_type="image", title="x", extra={"file_path": str(evil)}
        )
    ...
    def test_delete_removes_media_files(self, store, tmp_path, monkeypatch):
        ...
        req = make_mocked_request(
            "DELETE", "/", app=_app(store), match_info={"id": iid}
        )
    ...
    def test_delete_removes_derived_media_artifacts(self, store, tmp_path, monkeypatch):
        ...
        iid = store.create_typed_item(
            item_type="video", title="v", extra={"file_path": str(src)}
        )
        derived = [
            d / f"{iid}.audio.wav",
        ...
        assert resp.status == 200
        assert not src.exists()
        assert all(not p.exists() for p in derived)
        assert other.exists()
...
class TestSkillSynthesisParsing:
    ...
    def test_parses_clean_delimited_response(self):
        from gideon.interfaces.dashboard.handlers.knowledge import _parse_skill_sections
    ...
    def test_bold_headers_and_no_header_fallback(self):
        ...
        assert (
            _parse_skill_sections("just do the thing")["procedure"]
            == "just do the thing"
        )

    def test_slugify_intent(self):
        from gideon.interfaces.dashboard.handlers.knowledge import _slugify_intent
...
class TestItemGraphShape:
    ...
    def test_image_graph_has_parallel_nodes_and_terminals(self, store):
        ...
        assert {"exif", "ocr", "vision", "consolidate"} <= types
        assert {
            "insights",
            "entities",
            "intents",
    ...
    def test_video_graph_edges_deduped(self, store):
        ...
        body = json.loads(
            _run(H.get_item_graph(_req(store, "GET", match_info={"id": iid}))).body
        )
...
class TestEntityRelated:
    ...
    def test_related_entities_with_direction(self, store):
        ...
        store.db.commit()
        resp = _run(
            H.get_entity_related(_req(store, "GET", match_info={"name": "GraphQL"}))
        )
        ...
        resp2 = _run(
            H.get_entity_related(
                _req(store, "GET", match_info={"name": "FeebasService"})
            )
        )
    ...
    def test_related_empty_for_unknown_entity(self, store):
        ...
        resp = _run(
            H.get_entity_related(_req(store, "GET", match_info={"name": "Nope"}))
        )
...
class TestArchivedHiddenFromRetrieval:
    ...
    def test_related_items_excludes_archived(self, store):
        ...
        store.db.commit()
        rel = json.loads(
            _run(H.get_related_items(_req(store, "GET", match_info={"id": a}))).body
        )
    ...
    def test_entity_items_excludes_archived(self, store):
        ...
        iid = store.create_typed_item(
            item_type="note", title="MentionsZeta", content="about Zeta"
        )
        ...
        items = json.loads(
            _run(
                H.get_entity_items(_req(store, "GET", match_info={"name": "Zeta"}))
            ).body
        )
    ...
    def test_entity_items_is_mentions_based_not_text_match(self, store):
        ...
        store.create_typed_item(
            item_type="note",
            title="Aside",
            content="unrelated note that name-drops DynamoDB once",
        )
        store.db.commit()
        items = json.loads(
            _run(
                H.get_entity_items(_req(store, "GET", match_info={"name": "DynamoDB"}))
            ).body
        )
        ...
        assert (
            json.loads(
                _run(
                    H.get_entity_items(
                        _req(store, "GET", match_info={"name": "Nonexistent"})
                    )
                ).body
            )
            == []
...
class TestStaleEmbeddingCount:
    ...
    def test_counts_only_dimension_mismatches(self, store):
        ...
        store.create_typed_item(item_type="note", title="Unembedded", content="z")
        store.db.commit()
        self._embed(store, cur, 384)
        self._embed(store, old, 768)
        ...
        assert H._stale_embedding_count(store, _Emb()) == 1
