...
def test_doctor_warns_when_requested_auth_mode_falls_back(
    requested, monkeypatch, capsys
):
    _clear_auth_env(monkeypatch)
    ...
    assert f"requested={requested} actual=local_token authorable=false" in out, out
    assert "requested auth mode is not authorable" in out, out
