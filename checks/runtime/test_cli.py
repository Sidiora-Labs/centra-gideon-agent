...
class TestConfigSet:
    def test_refuses_to_replace_an_unreadable_config(
        ...
        cfg_file = tmp_path / "config.json"
        original = '{"providers": {"anthropic": {"api_key": "sk-do-not-lose"}}, BROKEN'
        cfg_file.write_text(original, encoding="utf-8")
        monkeypatch.setattr(
            "gideon.interfaces.cli.config.config_path", lambda: cfg_file
        )
    ...
    def test_refuses_a_non_object_config(self, tmp_path, monkeypatch, capsys):
        from gideon.interfaces.cli.config import _config_cmd

        cfg_file = tmp_path / "config.json"
        cfg_file.write_text('["not", "a", "config"]', encoding="utf-8")
        monkeypatch.setattr(
            "gideon.interfaces.cli.config.config_path", lambda: cfg_file
        )
