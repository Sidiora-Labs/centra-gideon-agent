...
import gideon.automation.schedule_trigger as st
...
def test_already_running(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        mc, "_post", lambda path, body=None: {"ok": False, "running": True}
    )
    ok, msg = st.trigger_schedule_job("abc123")
    assert ok is False
...
def test_gateway_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        mc, "_post", lambda path, body=None: {"error": "connection refused"}
    )
    ok, msg = st.trigger_schedule_job("abc123")
    assert ok is False
...
def test_the_immediate_fire_tool_is_registered() -> None:
    ...
    """
    from gideon.integrations.mcp_automation import _list_tools
...
def test_the_immediate_fire_tool_posts_to_the_gateway(
    ...
    TriggerStore(base_dir=tmp_path).upsert(
        Trigger(
            ...
            workflow={
                "inline": {"provider": "run-prompt", "config": {"message": "go"}}
            },
        )
    )
    posted: list[str] = []
...
def test_an_unknown_id_is_refused_before_any_post(
    ...
    monkeypatch.setattr(
        mcp_automation,
        "_http_runner",
        lambda payload: posted.append("posted") or {"status": "ok"},
    )
