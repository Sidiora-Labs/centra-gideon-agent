"""Tests for ConversationDirectory thread tracking (set_thread / get_thread).
...
class TestSetThread:
    ...
    async def test_stores_thread_for_existing_session(self, config) -> None:
        session_manager = ConversationDirectory(
            config, provider_factory=_mock_provider_factory()
        )
...
class TestChannelLink:
    ...
    async def test_set_and_get_channel_link(self, config) -> None:
        sm = ConversationDirectory(config, provider_factory=_mock_provider_factory())
...
class TestSessionMapLoad:
    ...
    @pytest.mark.asyncio
    async def test_load_skips_non_dict_entries(
    ...
    @pytest.mark.asyncio
    async def test_load_skips_corrupt_entries(
        self, config, tmp_path, monkeypatch
    ) -> None:
        import json
...
class TestSetChannelLinkUpdate:
    ...
    @pytest.mark.asyncio
    async def test_idempotent_set_same_link(
        self, config, tmp_path, monkeypatch
    ) -> None:
        monkeypatch.setattr("gideon.engine.session_map.config_dir", lambda: tmp_path)
        sm = ConversationDirectory(config, provider_factory=_mock_provider_factory())
...
class TestChannelAccessors:
    ...
    @pytest.mark.asyncio
    async def test_set_channel_updates_existing_link(
        self, config, tmp_path, monkeypatch
    ) -> None:
        monkeypatch.setattr("gideon.engine.session_map.config_dir", lambda: tmp_path)
        sm = ConversationDirectory(config, provider_factory=_mock_provider_factory())
    ...
    @pytest.mark.asyncio
    async def test_set_channel_stores_without_thread(
        self, config, tmp_path, monkeypatch
    ) -> None:
        monkeypatch.setattr("gideon.engine.session_map.config_dir", lambda: tmp_path)
        sm = ConversationDirectory(config, provider_factory=_mock_provider_factory())
