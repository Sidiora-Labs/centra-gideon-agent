...
the recent-N limit. Uses a real ConsoleState with mocked sessions/crons.
"""
...
from gideon.automation.schedule import ScheduleJob, make_agent_action
...
def _job(**kw) -> ScheduleJob:
    return ScheduleJob(
        ...
        action=make_agent_action(
            message=kw.get("message", "do it"), agent=kw.get("agent_id", "")
        ),
    )
...
def test_hydrates_history_on_first_open_only() -> None:
    ...
    assert "🔧 ran something" not in contents
...
def test_deleted_job_rebuilds_from_history() -> None:
    ...
    state = _make_state()
    job = ScheduleJob(
        id="gone99", name="cron-gone99", action=make_agent_action(message="")
    )
