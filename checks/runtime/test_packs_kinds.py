...
* ``test_tampered_resource_refuses_before_any_gideon_exists`` — one-link's per-resource hash.
...
def test_round_trip_on_a_fresh_home_lands_the_trigger_disabled(
    ...
    """
    archive = pack_bundled.build_bundled(pack_name, tmp_path / f"{pack_name}.gideon")
    home = fresh_home()
...
def test_a_malformed_one_link_document_fails_closed(
    ...
    mutate(doc)
    with pytest.raises(onelink.OneLinkError) as excinfo:
        onelink.materialize(doc, tmp_path / "out.gideon")
    assert fragment in str(excinfo.value)
...
def test_a_url_backed_resource_is_fetched_and_verified(tmp_path, build_home):
    ...
    out = onelink.materialize(doc, tmp_path / "fetched.gideon", fetch=_fetch)
