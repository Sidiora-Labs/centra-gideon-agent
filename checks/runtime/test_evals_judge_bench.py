    ...
    return json.dumps(
        {
            ...
            "evidence_refs": ["checks/runtime/test_http_retry.py"],
...
class StubJudge:
    ...
    async def __call__(self, prompt: str, *, use_case: str) -> jb.JudgeCall:
        ...
        return jb.JudgeCall(
            text=text, elapsed_secs=0.01, cost_usd=cost, model=f"Stub:{use_case}"
        )
...
def test_position_axis_reorders_the_rendered_candidates():
    ...
    measures nothing."""
    strong = _fixture(
        id="s", known_verdict=jb.VERDICT_PASS, artifact="STRONG-BODY: ran the suite"
    )
    null = _fixture(
        id="n",
...
def test_the_matrix_axes_are_the_shared_expansion():
    ...
    paired, unpaired = jb.build_specs(
        fixture_set, tiers=("fast",), sample_counts=(1, 3)
    )
    assert len(expand_cells(paired)) == 8 * 1 * 2 * 2
...
def test_a_pair_collapse_is_reported_once_not_once_per_slot():
    ...
    for position in jb.POSITIONS:
        obs.append(
            _obs(
                fixture_id="s", counterpart_id="n", position=position, quality_score=3.0
            )
        )
        obs.append(
            _obs(
...
def test_an_unmeasured_flip_rate_is_inadequate_not_zero():
    ...
    assert not rows[0].adequate
    assert any(
        "position-swap flip rate was never measured" in r
        for r in rows[0].inadequate_reasons
    )
...
def test_a_row_with_no_verdict_at_all_reports_nothing_measured():
    rows = jb.build_table(
        [_obs(outcome=VERIFIER_ABSENT, verdict="", direction="", note="x")]
    )
...
def test_the_cheapest_adequate_row_is_the_recommendation():
    rows = jb.build_table(
        _adequate_group("reasoning", 5, 0.50) + _adequate_group("fast", 1, 0.01)
    )
...
def test_an_unpriced_adequate_tier_is_never_called_cheapest():
    ...
    unpriced = [
        jb.Observation(
            **{**o.to_dict(), "cost_usd": None, "quality_score": o.quality_score}
        )
        for o in group
    ]
...
def test_the_table_is_byte_identical_across_renders():
    ...
    observations = _adequate_group("fast", 1, 0.01) + _adequate_group(
        "reasoning", 3, 0.2
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "name": "x",
                "fixtures": [{"id": "a", "kind": "nope", "known_verdict": "PASS"}],
            },
            "is not one of",
        ),
        (
...
def test_the_tiers_are_the_engines_own_tier_table():
    ...
    assert [jb.use_case_for_tier(t) for t in jb.TIERS] == [
        DEFAULT_MODEL_TIERS[t] for t in jb.TIERS
    ]
...
def test_the_top_sample_column_is_the_engines_ceiling():
    ...
    from gideon.automation.workflows.engine import MAX_JUDGE_SAMPLES
...
def test_the_direction_vocabulary_is_the_live_divergence_records():
    from gideon.automation.workflows.judge_calibration import DivergenceRecord
...
def test_a_full_run_persists_the_table_the_pin_and_a_ledger_row(bench_home):
    ...
    def answer(i, prompt, use_case):
        fixture = next(
            (
                ...
                if f.artifact in prompt and f.kind != jb.FIXTURE_NULL
            ),
    ...
    assert len(result.observations) == 8 * 2 + 4
    assert result.table
    ...
    assert jb.render_table_tsv(jb.build_table(reread)) == jb.render_table_tsv(
        result.table
    )
...
def test_an_incomplete_pin_refuses_the_run_before_spending(tmp_path, monkeypatch):
    ...
    with pytest.raises(store.PinRequiredError, match="model_fingerprint"):
        asyncio.run(
            jb.run_judge_bench(
                "starter", caller=stub, tiers=("fast",), sample_counts=(1,)
            )
        )
    assert stub.calls == []
...
def test_the_bench_verdict_is_fail_when_no_tier_is_adequate():
    ...
    assert (
        jb._bench_verdict(
            [jb.Recommendation(rubric_class="c", verdict=jb.REC_RECOMMENDED)]
        )
        == "pass"
    )
    assert (
        jb._bench_verdict(
            [jb.Recommendation(rubric_class="c", verdict=jb.REC_NO_ADEQUATE_TIER)]
        )
        == "fail"
    )
