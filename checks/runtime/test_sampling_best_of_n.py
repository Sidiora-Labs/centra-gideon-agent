...
Every model call is stubbed at its seam (``gideon.integrations.llm_helpers.one_shot_completion``
...
from gideon.integrations.sampling import _TEMPERATURE_LADDER, MAX_N, best_of_n
...
async def test_n_calls_run_genuinely_in_parallel(monkeypatch):
    ...
    assert (
        sampler.peak_in_flight == 4
    ), "N calls were not in flight at once (sequential loop?)"
    assert len(result["candidates"]) == 4
    span = sampler.last_end - sampler.first_start
    assert (
        span < 0.15
    ), f"fan-out spanned {span:.3f}s — that is N x latency, not parallel"
...
async def test_temperatures_are_varied_and_ladder_ordered(monkeypatch):
    ...
    assert (
        len(set(sampler.temperatures)) == 3
    ), "N identical temperatures is not best-of-N"
...
async def test_dead_judge_degrades_to_an_unjudged_slate(monkeypatch):
    ...
    assert (
        result["winner_idx"] == 0
    ), "the lowest-temperature survivor is the stable pick"
...
async def test_winner_is_deterministic_across_completion_orders(monkeypatch):
    ...
    for delays in (forward, reverse):
        ...
        assert sampler.order[0] != sampler.order[-1] or True
...
async def test_outcome_record_is_bounded_and_content_free(monkeypatch, tmp_path):
    ...
    _stub_samples(monkeypatch, sampler)
    await best_of_n(
        "a secret prompt", 3, "be terse", judge_provider_factory=_judge_factory(scores)
    )
...
async def test_outcome_log_trims_to_its_bound(monkeypatch, tmp_path):
    from gideon.integrations import sampling
...
async def test_one_shot_completion_threads_temperature_to_the_bridge(monkeypatch):
    ...
    monkeypatch.setattr(
        "gideon.extensions.providers.use_cases.resolution_chain",
        lambda _uc: [],
        raising=False,
    )
    assert (
        await one_shot_completion("hi", use_case="background", temperature=0.85) == "ok"
    )
...
def test_branded_provider_factory_threads_temperature_into_extra_options(monkeypatch):
    ...
    from gideon.integrations.llm import openai as openai_client
    ...
    monkeypatch.setattr(
        provider_helpers, "get_default_registry", lambda: ProviderRegistry()
    )
    ...
    factory, _create_provider, _create_catalog = provider_helpers.register_branded_app(
        spec
    )
    entry = ProviderEntry(
        name="probe", type="hc3-temp-probe", model="probe-1", options={}
    )
...
def test_sampling_outcomes_is_declared_and_snapshot_excluded():
    ...
    entry = next(
        e for e in inventory.all_entries() if e.path == "sampling_outcomes.jsonl"
    )
...
class TestBestOfNSkill:
    ...
    def test_triggers_on_the_real_phrasings(self, tmp_path, monkeypatch):
        ...
        assert "best-of-n" in loader.get_triggered_skills(
            "give me 3 versions and pick the best"
        )
        assert "best-of-n" not in loader.get_triggered_skills(
...
def test_best_of_n_tool_is_declared_and_dispatches_to_the_core(monkeypatch):
    ...
    async def fake_core(prompt, n=3, criteria="", *a, **kw):
        ...
        return {
            ...
            "winner_idx": 0,
            "candidates": [
                {"idx": 0, "temperature": 0.2, "text": "the pick", "error": ""}
            ],
    ...
    monkeypatch.setattr("gideon.integrations.sampling.best_of_n", fake_core)
    ...
    assert mcp_subagents._call_tool_inner("best_of_n", {"prompt": "  "}).startswith(
        "Error:"
    )
