...
class TestEligibleCandidates:
    def test_only_agents_with_metadata_and_not_reserved(self):
        cfg = _cfg(
            {
                "researcher": _profile(
                    specialty="deep web research", route_hints="research this"
                ),
                "bare": _profile(),
                "gideon-lite": _profile(specialty="x", route_hints="y"),
            }
        )
    ...
    def test_keyword_hit_needs_3word_phrase_and_margin(self):
        cands = [
            (
                "dba",
                "database expert",
                "optimize this slow sql query, fix the database index",
            ),
...
class TestSuppressionStore:
    ...
    def test_dismiss_cooldown_then_mute(self):
        ...
        assert not routing.is_suppressed(
            "dba", now=now + 25 * 3600, cooldown_hours=24.0
        )
        routing.record_dismiss("dba", now=now)
...
class TestSuggestForSend:
    ...
    def _session(self, *, agent="", memory_mode="persistent", user_turns=1):
        ...
        return SimpleNamespace(
            key="s1", agent=agent, memory_mode=memory_mode, messages=msgs
        )
    ...
    def test_suggests_in_default_chat(self, monkeypatch):
        cfg = _cfg(
            {
                "dba": _profile(
                    "database expert", "optimize slow sql query, fix db index"
                )
            }
        )
        ...
        r = routing.suggest_for_send(
            state, self._session(), "please optimize slow sql query now"
        )
    ...
    def test_disabled_config_no_suggestion(self, monkeypatch):
        ...
        assert (
            routing.suggest_for_send(
                SimpleNamespace(), self._session(), "optimize slow sql query"
            )
            is None
        )
    ...
    def test_explicit_agent_session_no_suggestion(self, monkeypatch):
        ...
        assert (
            routing.suggest_for_send(SimpleNamespace(), sess, "optimize slow sql query")
            is None
        )
