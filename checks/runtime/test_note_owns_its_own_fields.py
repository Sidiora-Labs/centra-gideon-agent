...
def test_an_attempted_override_is_reported_rather_than_silently_dropped(caplog):
    """req 97 ac_2 — the drop is VISIBLE.

    Refusing a spoofed field and saying nothing leaves an honest emitter (one whose meta
    grew a key that later became platform-owned) unable to tell why its value vanished, and
    leaves an operator with no trace of a caller that tried. The report names every reserved
    key that arrived and nothing else — an ordinary key must not be reported, or the line
    becomes noise on the normal path.
    """
    import logging

    st = _fresh()
    with caplog.at_level(logging.DEBUG, logger="gideon.interfaces.dashboard.state"):
        note = _deliver(
            st, "chat.error", {"acked": True, "mode": "never", "item_id": "abc"}
        )

    assert "acked" not in note and note["mode"] != "never"
    assert note["item_id"] == "abc"
    reports = [
        r.getMessage()
        for r in caplog.records
        if "ignoring platform-owned key" in r.getMessage()
    ]
    assert len(reports) == 1, f"expected exactly one override report, got {reports}"
    assert "acked" in reports[0] and "mode" in reports[0]
    assert (
        "chat.error" in reports[0]
    ), "the report must name the notification it came from"
    assert "item_id" not in reports[0], "ordinary meta must not be reported as smuggled"


def test_an_honest_note_reports_nothing(caplog):
    """Vacuity floor for the report: a note whose meta touches no platform field is
    silent, so the line above means "someone tried" rather than "notify ran"."""
    import logging

    st = _fresh()
    with caplog.at_level(logging.DEBUG, logger="gideon.interfaces.dashboard.state"):
        _deliver(st, "info", {"item_id": "abc", "count": 7})

    assert not [
        r for r in caplog.records if "ignoring platform-owned key" in r.getMessage()
    ]
