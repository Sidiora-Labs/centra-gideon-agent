...
class TestSaveSessionToHistory:
    def test_temporary_slot_still_saved(self):
        ...
        with patch(
            "gideon.interfaces.dashboard.chat_persistence.persisted_history_key",
...
class TestIsRestrictedSession:
    ...
    def test_dashboard_temporary_slot(self):
        ...
        state._sessions = {
            "chat-1-abc": _ChatSession(key="chat-1-abc", memory_mode="temporary")
        }

        assert (
            _is_restricted_session(state, self._mock_request("dashboard:chat-1-abc"))
            is True
        )
    ...
    def test_dashboard_normal_slot(self):
        ...
        assert (
            _is_restricted_session(state, self._mock_request("dashboard:chat-1-def"))
            is False
        )
    ...
    def test_dashboard_restricted_key_set(self):
        ...
        assert (
            _is_restricted_session(state, self._mock_request("dashboard:chat-1-eph"))
            is True
        )

    def test_channel_temporary_thread(self):
        from gideon.engine import session_restrictions as sr
        from gideon.interfaces.dashboard.handlers import _is_restricted_session
        ...
        assert (
            _is_restricted_session(state, self._mock_request("slack:C123-456")) is True
        )
    ...
    def test_dashboard_ui_key_not_restricted(self):
        ...
        state._restricted_keys = set()
        assert (
            _is_restricted_session(state, self._mock_request("dashboard:ui")) is False
        )
...
class TestMemoryRecallGuard:
    ...
    def _state_with(self, session):
        from gideon.interfaces.dashboard.state import _ChatSession  # noqa: F401
    ...
    async def test_recall_blocked_for_temporary_session(self):
        ...
        from gideon.interfaces.dashboard.handlers.memory import api_memory_recall
        from gideon.interfaces.dashboard.state import _ChatSession
        ...
        with patch(
            ...
            side_effect=AssertionError(
                "recall must not reach the service for a temporary session"
            ),
        ):
            resp = await api_memory_recall(
                self._mock_request(state, "dashboard:chat-1-tmp")
            )
        import json
    ...
    async def test_recall_allowed_for_normal_session(self):
        ...
        with patch(
            "gideon.interfaces.dashboard.handlers.memory._get_service", return_value=svc
        ):
            resp = await api_memory_recall(
                self._mock_request(state, "dashboard:chat-1-normal")
            )
        import json
...
class TestMcpSessionKeyPlumbing:
    ...
    @patch("gideon.integrations.mcp_memory._post")
    def test_learn_add_no_session_key_in_body(self, mock_post):
        ...
        from gideon.integrations.mcp_memory import _call_tool_inner
    ...
    @patch.dict("os.environ", {"GIDEON_SESSION_KEY": "dashboard:chat-1-tmp"})
    @patch("gideon.integrations.mcp_memory._delete")
    ...
    @patch.dict("os.environ", {"GIDEON_SESSION_KEY": "dashboard:chat-1-tmp"})
    @patch("gideon.integrations.mcp_memory._get")
...
class TestAfterTurnReviewRestrictedGuard:
    def _run(self, session):
        ...
        state = MagicMock()
        with (
            patch("gideon.cognition.memory_service.service_for") as service_for,
            patch(
                "gideon.cognition.after_turn_review.capture_preference_facet",
                return_value=None,
            ) as capture,
        ):
    ...
    def test_skill_ladder_skips_restricted(self):
        from gideon.interfaces.dashboard.chat_runner import _maybe_skill_ladder_review
        from gideon.interfaces.dashboard.state import _ChatSession

        session = _ChatSession(key="inc-2", memory_mode="incognito")
        state = MagicMock()
        with patch("gideon.cognition.after_turn_review.is_correction_signal") as signal:
...
class TestRestrictedSessionDiscovery:
    ...
    def test_sync_bridge_resume_list_excludes_incognito(self, tmp_path):
        from gideon.integrations.sync_bridge import list_recent_sessions
