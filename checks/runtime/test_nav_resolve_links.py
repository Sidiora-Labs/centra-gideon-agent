...
async def test_resolves_batch_with_single_model_call() -> None:
    ...
    try:
        with patch("gideon.integrations.llm_helpers.one_shot_completion", mock):
...
async def test_caps_link_count_before_model_call() -> None:
    ...
    try:
        with patch("gideon.integrations.llm_helpers.one_shot_completion", _capture):
            resp = await client.post(
                "/api/chat/nav/resolve-links",
                json={
                    "links": [
            ...
            assert resp.status == 200
            body = await resp.json()
            assert len(body["summaries"]) == _NAV_MAX_LINKS
    finally:
        await client.close()
