...
from gideon.learning.surfacing import Candidate, Tier, allocate, count_tokens
from gideon.memory import MemoryStore
from gideon.skills.allocation import (
    CONTEXT_TIERS,
    SkillDecision,
    SkillLoadState,
    SkillRequest,
...
def _usage_store_stays_in_tmp(tmp_path, monkeypatch):
    ...
    counter_home.mkdir()
    monkeypatch.setattr("gideon.skills.usage.skills_dir", lambda: counter_home)
...
class TestTheCapsArePerSkillAndInAggregate:
    ...
    def test_every_bundled_skill_fits_the_cap_its_declared_tier_grants(self):
        ...
        """
        from gideon.skills.allocation import full_block
        from gideon.skills.loader import parse_frontmatter

        bundled = Path(__file__).resolve().parents[1] / "src/gideon/skills/bundled"
        over: list[str] = []
...
class TestContinuedCostIsReEvaluated:
    def test_a_skill_admitted_on_one_turn_is_re_fitted_on_the_next(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("gideon.skills.allocation.AGGREGATE_CAP_TOKENS", 200)
