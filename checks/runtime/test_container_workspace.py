...
class TestProvisionArgv:
    ...
    async def test_a_snapshot_ref_wins_over_the_manifest_image(self, monkeypatch):
        ...
        calls, _ = await self._argv_for(monkeypatch, manifest, from_snapshot="gideon/run-r1:cp-2")
        (argv,) = calls
...
class TestProvisioningChain:
    async def test_a_declared_container_provisions_and_reads_clean(self, home, monkeypatch):
        ...
        assert result.container_id == "gideon-run-r1" and result.container_backend == "fakectl"
    ...
    async def test_container_fields_ride_the_run_record_dict(self, home, monkeypatch):
        ...
        d = result.to_dict()
        assert d["container_id"] == "gideon-run-r5" and d["container_backend"] == "fakectl"
...
class TestForkAnchor:
    def test_checkpoint_round_trips_the_anchor_and_tolerates_old_rows(self, home):
        cp = CP.Checkpoint(
            id="cp-1", run_id="r1", spec_version=1, workspace_snapshot="gideon/run-r1:cp-1"
        )
        assert CP.Checkpoint.from_dict(cp.to_dict()).workspace_snapshot == "gideon/run-r1:cp-1"
