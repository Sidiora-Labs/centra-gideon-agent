...
from gideon.guardrails import model_call as mc
from gideon.guardrails.audit import AttemptRecord, read_recent
from gideon.guardrails.model_call import ModelCallGuard, _joined_content
...
class TestFailOpen:
    ...
    async def test_broken_classifier_leaves_empty_class_not_a_crash(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("gideon.routing.classifier.classify_query", boom)
        guard = ModelCallGuard(FakeProvider(), use_case="chat", provider_name="P", model="m")
        await guard.start()
