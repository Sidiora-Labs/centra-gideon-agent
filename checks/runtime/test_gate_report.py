...
``tooling/scripts/gate_report.py`` runs the six §3/§6 drift/inert gates (config-baseline,
...
``checks/catalogs/configuration.json`` / ``checks/catalogs/inert-surfaces.json`` / ``docs-lint-baseline.json`` are
never mutated.
"""
...
def _seed_config_drift(monkeypatch, tmp_path: Path) -> None:
    ...
    stale = tmp_path / "configuration.json"
    stale.write_text("[]\n", encoding="utf-8")
...
def _seed_inert_drift(monkeypatch, tmp_path: Path) -> None:
    ...
    baseline = _write_json(
        tmp_path / "inert-surfaces.json",
        inert_gen.encode_catalog(
            {
                "generated_from": "tooling/scripts/generate_inert_surface_baseline.py",
                "per_file": {
                    "runtime/gideon/synthetic.py": {"inert": 0, "surfaces": []}
                },
                "totals": {"inert": 0, "by_kind": {}},
            }
        ),
    )
...
def test_three_independent_failures_all_surface_in_one_run(monkeypatch, tmp_path):
    ...
    assert (
        "checks/catalogs/configuration.json is stale"
        in by_name["config-baseline"].failures[0]
    )
...
def test_a_raising_gate_is_captured_and_does_not_stop_the_others(monkeypatch):
    ...
    assert any(
        "raised RuntimeError" in line for line in by_name["inert-surface"].failures
    )
    ...
    for name in _ALL_GATE_NAMES:
        if name == "inert-surface":
            continue
        assert not any("raised" in line for line in by_name[name].failures), by_name[
            name
        ]
    assert [r.name for r in results] == _ALL_GATE_NAMES
...
def test_report_renders_every_failure_in_one_table():
    ...
    results = [
        GateResult(
            "config-baseline",
            False,
            ["checks/catalogs/configuration.json is stale — run ..."],
        ),
        GateResult(
            "inert-surface",
            False,
            ["runtime/gideon/x.py: inert surfaces rose 0 -> 1; new ..."],
        ),
        GateResult(
            "docs-lint", False, ["docs/y.md: docs-lint findings rose 0 -> 1; new ..."]
        ),
    ]
    ...
    assert "checks/catalogs/configuration.json is stale" in report
