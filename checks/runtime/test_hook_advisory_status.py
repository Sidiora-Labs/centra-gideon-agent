...
from gideon.automation.triggers.history import HOOK_STATUS_TO_OUTCOME, hook_to_record
...
def _store(
    tmp_path, event: str = HOOK_EVENT_PRE_TOOL_USE
) -> tuple[ScriptHookStore, ScriptHook]:
...
def test_fire_tool_hooks_seam_records_advisory(tmp_path):
    ...
    store, hook = _store(tmp_path)
    asyncio.run(
        fire_tool_hooks(store, "Running: write_file", '{"path": "/tmp/hooked1.txt"}')
    )
    assert hook.last_status == "advisory"
...
def test_gating_fire_records_blocked(tmp_path):
    ...
    store, hook = _store(tmp_path)
    asyncio.run(
        store.fire_for_ids(HOOK_EVENT_PRE_TOOL_USE, [hook.id], tool_name="write_file")
    )
    assert hook.last_status == "blocked"
...
def test_only_gating_callers_exist():
    ...
    callers = {
        ...
        if ".fire_for_ids(" in p.read_text(encoding="utf-8")
    }
...
def test_advisory_is_mapped_and_projects_as_ran_with_a_reason(tmp_path, caplog):
    ...
    assert record.outcome == Outcome.RAN.value
...
def test_the_three_states_are_distinguishable(tmp_path):
    ...
    def _row(status: str, run_count: int):
        hook = ScriptHook(
            id="h1", name="policy", event=HOOK_EVENT_PRE_TOOL_USE, provider="bash"
        )
