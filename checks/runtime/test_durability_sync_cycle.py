...
class TestOneCycle:
    ...
    def test_transport_failure_is_contained(self, tmp_path):
        ...
        assert report.ok is False and "network down" in report.error
...
class TestCriterion4Convergence:
    ...
    def test_two_machines_converge_on_tasks(self, tmp_path):
        ...
        run_sync_cycle(store, a_home, self_id="A", now="t1")
        run_sync_cycle(store, b_home, self_id="B", now="t2")
        run_sync_cycle(store, a_home, self_id="A", now="t3")
