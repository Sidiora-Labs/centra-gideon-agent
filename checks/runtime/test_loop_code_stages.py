...
def _code(**over):
    base = dict(
        id="", name="C", kind="code", task="add oauth login to the app", kind_config={}
    )
    base.update(over)
...
class TestStageAdvance:
    def test_no_criteria_advances_on_work_happened(self):
        ...
        s, ctx = kinds.get("code"), _Ctx()
            ...
            s.on_new_cycle(
                store.get(c.id), [{"cycle": 1, "stage": "design", "summary": "x"}], ctx
            )
        )
        ...
        s, ctx = kinds.get("code"), _Ctx()
            ...
            s.on_new_cycle(
                ...
                [
                    {
                        "cycle": 1,
                        "stage": "1 — Scaffold & Engine",
                        "summary": "scaffolded + engine",
                    }
                ],
                ctx,
            )
    ...
    def test_strip_stage_ordinal_forms(self):
        ...
        assert strip("implementation") == "implementation"
    ...
    def test_trailing_decoration_attributes_to_titled_stage(self):
        ...
        findings = [
            ...
            {
                "cycle": 4,
                "stage": "Build All Modules — service.py",
                "summary": "service",
            },
        ...
        assert decomp == [
            1,
            2,
        ], f"decomposition should own only its own findings, got {decomp}"
        assert impl == [
            3,
            4,
    ...
    def test_trailing_decoration_prefers_most_specific_stage(self):
        ...
        loop = store.get(c.id)
        assert sorted(f["cycle"] for f in s._findings_for_stage(loop, 0, findings)) == [
            2
        ]
        assert sorted(f["cycle"] for f in s._findings_for_stage(loop, 1, findings)) == [
            1
        ]
    ...
    def test_unrecognized_stage_label_falls_to_active_stage(self):
        ...
        findings = [
            ...
            {
                "cycle": 2,
                "stage": "test_suite_and_verification",
                "summary": "&→and drift",
            },
            {"cycle": 3, "stage": "totally unrelated label", "summary": "arbitrary"},
        ]
        verif = sorted(
            f["cycle"] for f in s._findings_for_stage(store.get(c.id), 1, findings)
        )
        assert verif == [
            2,
            3,
        ...
        impl = sorted(
            f["cycle"] for f in s._findings_for_stage(store.get(c.id), 0, findings)
        )
        assert impl == [
            1
        ], f"non-active stage must only match findings naming it, got {impl}"
    ...
    def test_flaky_judge_does_not_block_a_stage_whose_command_gate_passed(
        self, monkeypatch
    ):
        import gideon.automation.loop.kinds.sdlc as sdlc_mod
        from gideon.automation.loop import gates
        from gideon.automation.loop import kinds as _k

        monkeypatch.setattr(sdlc_mod, "_command_runnable_here", lambda *_a, **_k: True)
        monkeypatch.setattr(gates, "run_verify_command", lambda *_a, **_k: _async(True))
        c = _code(
            plan=[
                {"stage": "implementation", "title": "I", "exit_criteria": ["builds"]}
            ],
            phase_status={"implementation": "active"},
        ...
        ctx2 = _Ctx()
        monkeypatch.setattr(
            gates, "judge_verdict", lambda *_a, **_k2: _async("FAIL: not really")
        )
    ...
    def test_stuck_stage_escalates_to_blocked_once(self, monkeypatch):
        ...
        findings = [
            {"cycle": i, "stage": "implementation", "summary": f"wip {i}"}
            for i in range(1, 6)
        ]
        assert _run(s.on_new_cycle(store.get(c.id), findings, ctx)) is False
        ...
        _run(
            s.on_new_cycle(
                store.get(c.id),
                findings
                + [{"cycle": 6, "stage": "implementation", "summary": "wip 6"}],
                ctx,
            )
        )
    ...
    def test_stage_advance_clears_stall_flag(self, monkeypatch):
        ...
        monkeypatch.setattr(gates, "judge_verdict", lambda *_a, **_k2: _async("FAIL"))
        findings = [
            {"cycle": i, "stage": "design", "summary": f"wip {i}"} for i in range(1, 6)
        ]
        _run(s.on_new_cycle(store.get(c.id), findings, ctx))
        ...
        _run(s.on_new_cycle(store.get(c.id), findings, ctx))
        assert f"{c.id}:design" not in s._stall_notified
    ...
    def test_failing_verify_command_blocks_gate(self, monkeypatch):
        ...
        assert (
            _run(
                s.on_new_cycle(
                    store.get(c.id),
                    [{"cycle": 1, "stage": "implementation", "summary": "x"}],
                    ctx,
                )
            )
            is False
    ...
    def test_planning_stage_verify_command_skipped_when_not_buildable(
        ...
        ws = tmp_path / "ws"
        ws.mkdir()
        (ws / "PLAN.md").write_text("# the plan")
        c = _code(
            plan=[
                {
                    "stage": "design",
        ...
        monkeypatch.setattr(gates, "judge_verdict", lambda *_a, **_k2: _async("PASS"))
            ...
            s.on_new_cycle(
                store.get(c.id),
                [{"cycle": 1, "stage": "design", "summary": "wrote PLAN.md"}],
                ctx,
            )
        )
        assert done is False
        assert (
            called["n"] == 0
        ), "verify_command must be SKIPPED on a non-buildable planning stage"
        assert store.get(c.id).phase_status["design"] == "done"
    ...
    def test_planning_stage_blocks_when_deliverable_missing(
        self, monkeypatch, tmp_path
    ):
        ...
        assert done is False
        assert store.get(c.id).phase_status["design"] == "active"
        assert any(
            e[1] == "gate_check"
            and e[2].get("label") == "deliverable"
            and e[2].get("ok") is False
            for e in ctx.events
        )
    ...
    def test_implementation_stage_advances_with_subdir_deliverable(
        self, monkeypatch, tmp_path
    ):
    ...
    def test_slice_b_feeds_deliverable_content_to_judge(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gates, "judge_verdict", _capture)
            ...
            s.on_new_cycle(
                store.get(c.id),
                [{"cycle": 1, "stage": "design", "summary": "wrote the plan"}],
                ctx,
            )
        )
        assert done is False
    ...
    def test_verify_command_gates_once_project_is_buildable(
        self, monkeypatch, tmp_path
    ):
        ...
        monkeypatch.setattr(gates, "run_verify_command", _fail)
        done = _run(
            s.on_new_cycle(
                store.get(c.id),
                [{"cycle": 1, "stage": "implementation", "summary": "x"}],
...
class TestAutopilotQueue:
    ...
    def test_queues_active_stage_and_holds_gate_until_done(self):
        ...
        assert (
            _run(
                s.on_new_cycle(
                    store.get(c.id),
                    [{"cycle": 2, "stage": "implementation", "summary": "y"}],
                    ctx,
                )
            )
            is True
    ...
    def test_one_by_one_mode_does_not_autoqueue(self):
        ...
        c = _code(
            autopilot=False,
            plan=[
                {"stage": "implementation", "title": "Impl", "tasks": [{"title": "A"}]}
            ],
            phase_status={"implementation": "active"},
        )
        tasks_link.provision(c.id)
        ...
        assert store.get(c.id).kind_config.get("queued_task_ids", []) == []
    ...
    def test_stage_advance_force_closes_the_stage_tasks(self):
        ...
        _run(
            s.on_new_cycle(
                store.get(c.id),
                [{"cycle": 1, "stage": "design", "summary": "sketched"}],
                ctx,
            )
        )
        assert store.get(c.id).phase_status["design"] == "done"
        tasks, _ = _run(registry.list_all_tasks(task_list_id=list_id, limit=50))
        assert tasks and all(t.status.value == "done" for t in tasks)
...
class TestParallelScheduler:
    ...
    class _SchedSvc:
        ...
        def get_by_session(self, session_name):
            return next(
                (lp for lp in self._loops.values() if lp.session_name == session_name),
                None,
            )
    ...
    def test_spawns_task_workers_in_worktrees(self, tmp_path):
        ...
        assert len(q) == 2
        assert all(
            svc.get_by_session(task_session_key(c.id, tid)) is not None for tid in q
        )
...
class TestNoStagePlan:
    def test_completes_once_a_finding_lands(self):
        ...
        assert (
            _run(
                s.on_new_cycle(store.get(c.id), [{"cycle": 1, "summary": "fixed"}], ctx)
            )
            is True
        )
    ...
    def test_failing_verify_holds_completion(self):
        ...
        s, ctx = kinds.get("code"), _Ctx()
        assert (
            _run(s.on_new_cycle(store.get(c.id), [{"cycle": 1, "summary": "x"}], ctx))
            is False
        )
...
class TestP6TickDecision:
    ...
    def test_metric_regression_rolls_back_to_prior_stage(self):
        ...
        c = self._two_stages_metric_second()
        d = kinds.get("code")._tick_decide(
...
class TestP6EndToEnd:
    ...
    def test_metric_gated_stage_holds_below_bar_and_persists_metric(self, monkeypatch):
        ...
        self._stub_scored_judge(monkeypatch, 2.5)
        c = _code(
            plan=[
                {
                    "stage": "verification",
        ...
        s, ctx = kinds.get("code"), _Ctx()
            ...
            s.on_new_cycle(
                store.get(c.id),
                [{"cycle": 1, "stage": "verification", "summary": "x"}],
                ctx,
            )
        )
        assert done is False
    ...
    def test_metric_gated_stage_completes_above_bar(self, monkeypatch):
        self._stub_structural_gate_pass(monkeypatch)
        self._stub_scored_judge(monkeypatch, 4.0)
        c = _code(
            plan=[
                {
                    "stage": "verification",
    ...
    def test_regression_rolls_back_to_prior_stage_end_to_end(self, monkeypatch):
        self._stub_structural_gate_pass(monkeypatch)
        self._stub_scored_judge(monkeypatch, 1.0)
        c = _code(
            plan=[
                {"stage": "implementation", "title": "I", "metric_pass": 3.0},
                {
        ...
        assert ps.get("verification") == "pending"
        assert ps.get("implementation") == "active"
        assert (store.get(c.id).kind_config or {}).get("rollbacks_on_stage", {}).get(
            "1"
        ) == 1
        assert "rolled_back" in [e[1] for e in ctx.events]

    def test_persistent_metric_hold_escalates_stall_with_metric_cause(
        self, monkeypatch
    ):
        from gideon.automation.loop import kinds as _k
        ...
        ctx = _Ctx()
        findings = [
            {"cycle": i, "stage": "verification", "summary": f"refine {i}"}
            for i in range(1, 6)
        ]
        assert _run(s.on_new_cycle(store.get(c.id), findings, ctx)) is False
        assert store.get(c.id).status == "blocked"
