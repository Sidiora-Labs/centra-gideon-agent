"""Tests for ConsoleState WebSocket subscriber methods (activity viewer)."""
...
class TestSubagentSubscribers:
    def test_subscribe_and_unsubscribe(self, state: ConsoleState) -> None:
        ws = MagicMock()
    ...
    def test_unsubscribe_idempotent(self, state: ConsoleState) -> None:
        ws = MagicMock()
        state.unsubscribe_subagents(ws)

    def test_broadcast_sends_to_subscribed_only(self, state: ConsoleState) -> None:
    ...
    def test_broadcast_noop_when_empty(self, state: ConsoleState) -> None:
        state.broadcast_ws_subagent_subscribers("subagent_chunk", {"id": "a1"})

    def test_broadcast_ws_sends_to_all(self, state: ConsoleState) -> None:
    ...
    def test_broken_subscriber_removed(self, state: ConsoleState) -> None:
    ...
    def test_closed_ws_removed_on_broadcast(self, state: ConsoleState) -> None:
...
class TestSessionModel:
    def test_model_in_to_dict(self, state: ConsoleState) -> None:
        session = state.get_or_create_session("test-1", model="claude-opus-4.5")
        assert session.to_dict()["model"] == "claude-opus-4.5"

    def test_model_defaults_empty(self, state: ConsoleState) -> None:
...
class TestCompactCallbackWiring:
    """Tests for ConsoleState.wire_session_compact_callback.

    Covers the async closure that fires after ConversationDirectory recycles a
    ...
    def _captured_callback(self, state: ConsoleState):
    ...
    def test_wire_installs_callback_on_sessions(self, state: ConsoleState) -> None:
        state.wire_session_compact_callback()
        state.sessions.set_compact_callback.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_callback_ignores_non_dashboard_keys(
    ...
    @pytest.mark.asyncio
    async def test_callback_noop_when_session_missing(
        self, state: ConsoleState
    ) -> None:
        cb = self._captured_callback(state)
    ...
    @pytest.mark.asyncio
    async def test_callback_appends_assistant_notice(self, state: ConsoleState) -> None:
        session = state.get_or_create_session("chat-1")
    ...
    @pytest.mark.asyncio
    async def test_callback_rounds_pct_in_notice(self, state: ConsoleState) -> None:
    ...
    async def test_callback_broadcast_runs_even_if_append_fails(
        self, state: ConsoleState, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from gideon.interfaces.dashboard.state import _ChatSession
