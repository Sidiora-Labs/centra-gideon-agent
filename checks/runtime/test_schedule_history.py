"""Unit tests for the schedule run history (`ExecutionRecord` + `ExecutionJournal`).
...
`test_triggers_facade_store.py` covers the badge. `ExecutionJournal` is unchanged, which is what
remains here.
"""
...
async def test_rotation_caps_per_job(tmp_path: Path) -> None:
    ...
    for i in range(_MAX_RECORDS_PER_JOB + 25):
        await store.append(ExecutionRecord(job_id="rot", summary=f"run {i}"))
...
async def test_lock_free_read_tolerates_partial_line(tmp_path: Path) -> None:
    store = ExecutionJournal(base_dir=tmp_path)
    await store.append(ExecutionRecord(job_id="p", summary="good"))
    job_file = tmp_path / "cron-history" / "p.jsonl"
    with job_file.open("a", encoding="utf-8") as fh:
        fh.write('{"run_id": "partial", "job_id": "p"')
...
async def test_delete_for_job(tmp_path: Path) -> None:
    store = ExecutionJournal(base_dir=tmp_path)
    await store.append(ExecutionRecord(job_id="d1", summary="a"))
    await store.append(ExecutionRecord(job_id="d2", summary="b"))
...
# and `test_triggers_facade_store.py`'s badge assertions. `ExecutionJournal` itself is unchanged,
...
async def test_count_since_counts_only_the_window(tmp_path: Path) -> None:
    ...
    for offset in (-10, -100, -3000, -7200):
        await store.append(
            ExecutionRecord(job_id="j", trigger="scheduled", started_at=now + offset)
        )
...
async def test_a_MANUAL_fire_is_excluded_by_default(tmp_path: Path) -> None:
    ...
    now = 1_700_000_000.0
    await store.append(
        ExecutionRecord(job_id="j", trigger="scheduled", started_at=now - 10)
    )
    await store.append(
        ExecutionRecord(job_id="j", trigger="manual", started_at=now - 20)
    )
...
async def test_an_unknown_job_counts_zero_rather_than_raising(tmp_path: Path) -> None:
    ...
    assert await ExecutionJournal(tmp_path).count_since("never-ran", 0) == 0
...
async def test_a_REAL_fire_and_a_FAILED_one_both_still_count(tmp_path: Path):
    ...
    await store.append(
        ExecutionRecord(
            run_id="r1", job_id="j", trigger="ok", started_at=now, status="success"
        )
    )
    await store.append(
        ExecutionRecord(
            run_id="r2", job_id="j", trigger="failed", started_at=now, status="failure"
        )
    )
...
async def test_the_MANUAL_exclusion_still_holds_alongside(tmp_path: Path):
    ...
    await store.append(
        ExecutionRecord(
            run_id="a", job_id="j", trigger="ok", started_at=now, status="success"
        )
    )
    await store.append(
        ExecutionRecord(
            run_id="b", job_id="j", trigger="manual", started_at=now, status="success"
        )
    )
    await store.append(
        ExecutionRecord(
...
async def test_the_exclusion_reads_the_SHARED_inert_set(tmp_path: Path):
    ...
    assert (
        await store.count_since("j", now - 10) == 0
    ), "every inert outcome must be excluded"
...
async def test_a_job_that_NEVER_suppresses_keeps_its_FULL_window(tmp_path: Path):
    ...
    assert (
        total > _MAX_RECORDS_PER_JOB * 0.9
    ), f"a work-only job must keep its window, got {total}"
...
async def test_the_index_stays_NEWEST_FIRST(tmp_path: Path):
    ...
    assert stamps == sorted(
        stamps, reverse=True
    ), "index rotation must not reorder the file"


def _write_legacy(store: ExecutionJournal, job_id: str, rows: list[dict]) -> None:
...
@pytest.mark.asyncio
async def test_BOOT_rotation_does_not_evict_what_APPEND_rotation_protects(
    tmp_path: Path,
):
    """🔴 THE DEFECT. `_compact_history` carried its own inlined `rows[-_MAX_RECORDS_PER_JOB:]` — the
    ...
    rows += [
        _row(f"skip-{i}", "clock:m", "skipped_gate", now + i * 60)
...
async def test_BOOT_rotation_still_caps_a_WORK_only_file(tmp_path: Path):
    ...
    now = 1_800_000_000.0
    _write_legacy(
        store, "work", [_row(f"r{i}", "work", "success", now + i) for i in range(150)]
    )
    await store.rotate_all()
...
async def test_BOOT_rotation_visits_EVERY_job_file(tmp_path: Path):
    ...
    for job in ("alpha", "beta"):
        _write_legacy(
            store, job, [_row(f"{job}{i}", job, "success", now + i) for i in range(140)]
        )
    await store.rotate_all()
    for job in ("alpha", "beta"):
...
async def test_BOOT_rotation_delegates_rather_than_DUPLICATING(tmp_path: Path):
    ...
    from gideon.automation import schedule_history

    source = inspect.getsource(schedule_history.ExecutionJournal._compact_history)
    assert "self._compact_job(" in source
    body = source.split('"""')[-1]
    assert (
        "_MAX_RECORDS_PER_JOB" not in body
    ), "the trim must not be re-implemented here"
    assert "_rewrite" not in body, "boot rotation must not write job files itself"
