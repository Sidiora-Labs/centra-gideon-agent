...
class TestWriteTombstone:
    ...
    def test_extra_fields_included(self, agent_root):
        ...
        write_tombstone(
            "t2", cause="reaped", recovery_action="delivered", pid=999, turns=12
        )
...
class TestSpawnCreatesFolder:
    """Verify DelegationSupervisor.spawn() creates an agent folder on disk."""
    ...
    async def test_rejected_spawn_no_folder(self, agent_root):
        ...
        manager = DelegationSupervisor(sessions=MagicMock(), ctx_builder=None)
    ...
    async def test_queued_spawn_no_folder(self, agent_root):
        ...
        manager = DelegationSupervisor(
            sessions=sessions, ctx_builder=ctx, max_concurrent=1
        )
...
class TestTombstoneOnAbnormalExit:
    ...
    async def test_tombstone_on_timeout(self, agent_root):
        ...
        from gideon.engine.subagent_persistence import create_agent_folder
        ...
        info = SubagentInfo(
            id="timeout1", task="t", parent_session_key="dashboard:default"
        )
    ...
    async def test_timeout_skipped_when_already_reaped(self, agent_root):
        ...
        info = SubagentInfo(
            id="reaped_timeout", task="t", parent_session_key="dashboard:default"
        )
        info.reaped = True
        info.error = "reaped by reaper"
        create_agent_folder("reaped_timeout", task="t")
        write_tombstone(
            "reaped_timeout", cause="reaped", recovery_action="notification_pending"
        )
    ...
    async def test_tombstone_on_turn_limit(self, agent_root):
        ...
        manager = DelegationSupervisor(
            sessions=sessions, ctx_builder=ctx, default_turn_limit=2
        )

        with patch("gideon.engine.subagent.Stats"), patch("gideon.engine.subagent.sel"):
            info = manager.spawn(
                "turn limit test", parent_session_key="dashboard:default"
            )
            await manager._tasks[info.id]
    ...
    async def test_tombstone_on_error(self, agent_root):
        ...
        manager = DelegationSupervisor(sessions=MagicMock(), ctx_builder=MagicMock())

        info = SubagentInfo(
            id="error1", task="t", parent_session_key="dashboard:default"
        )
...
class TestFolderCleanupOnSuccess:
    ...
    async def test_folder_kept_on_delivery_failure(self, agent_root):
        ...
        manager = DelegationSupervisor(
            sessions=sessions, ctx_builder=ctx, on_done=_slow_on_done
        )

        with (
            patch("gideon.engine.subagent._ON_DONE_TIMEOUT", 0.01),
            patch("gideon.engine.subagent.Stats"),
            patch("gideon.engine.subagent.sel"),
        ):
            info = manager.spawn(
                "delivery failure test", parent_session_key="dashboard:default"
            )
            await manager._tasks[info.id]
            await manager.flush_deliveries()
...
class TestOrphanReconciliation:
    ...
    async def test_dead_pid_with_result_tombstoned_as_delivered(self, agent_root):
        ...
        create_agent_folder(
            "orphan1", task="old task", parent_session="dashboard:default"
        )
        write_result_chunk("orphan1", "some result")
        from gideon.engine.subagent_persistence import update_state

        update_state("orphan1", pid=99999)
    ...
    async def test_dead_pid_no_result_tombstoned_as_notified(self, agent_root):
        ...
        from gideon.engine.subagent import DelegationSupervisor
        from gideon.engine.subagent_persistence import create_agent_folder, update_state
    ...
    async def test_reconcile_uses_pid_recorded_at(self, agent_root):
        ...
        with (
            patch.object(manager, "_is_pid_alive", return_value=True),
            patch.object(
                manager, "_is_orphan_process", return_value=True
            ) as mock_check,
...
class TestOrphanNotification:
    ...
    async def test_notification_called_for_orphan_with_result(self, agent_root):
        ...
        create_agent_folder(
            "notif1", task="important task", parent_session="dashboard:default"
        )
        ...
        with (
            ...
            patch.object(
                manager, "_notify_orphan", new_callable=AsyncMock
            ) as mock_notify,
        ):
            await manager._reconcile_orphans()
        ...
        assert call_args[0][0] == "notif1"
        assert call_args[0][2] == "result_available"
    ...
    async def test_notification_called_for_orphan_without_result(self, agent_root):
        ...
        mock_notify.assert_awaited_once()
        call_args = mock_notify.call_args
        assert call_args[0][2] == "notification_pending"
    ...
    async def test_slack_dm_fallback_called(self, agent_root):
        ...
        create_agent_folder(
            "notif3", task="fallback task", parent_session="dashboard:default"
        )
        ...
        with (
            ...
            patch.object(
                manager, "_send_orphan_channel_dm", new_callable=AsyncMock
            ) as mock_dm,
        ):
    ...
    async def test_msg_redacted_before_injection_path(self, agent_root):
        ...
        with (
            patch.object(
                manager, "_try_inject_orphan_notification", side_effect=_capture_inject
            ),
            patch(
                "gideon.engine.subagent._redact", side_effect=lambda m: f"[REDACTED]{m}"
            ) as mock_redact,
        ):
...
class TestSpawnStatusReadsFromAgentFolder:
    ...
    def test_read_state_for_orphaned_agent(self, agent_root):
        ...
        write_tombstone(
            "orphan_status", cause="gateway_restart", recovery_action="delivered"
        )
    ...
    async def test_api_spawn_status_fallback_to_disk(self, agent_root):
        ...
        from gideon.interfaces.dashboard.handlers.messaging import api_spawn_status

        create_agent_folder("disk_agent", task="disk task")
        write_result_chunk("disk_agent", "disk result")
        write_tombstone(
            "disk_agent", cause="gateway_restart", recovery_action="delivered"
        )
...
class TestPathTraversal:
    def test_dot_agent_id_rejected(self, agent_root):
        from gideon.engine.subagent_persistence import _agent_dir
