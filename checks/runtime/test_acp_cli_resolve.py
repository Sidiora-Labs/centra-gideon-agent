...
def test_node_argv_for_script_passes_through_native():
    assert node_argv_for_script("/usr/local/bin/test-cli") == [
        "/usr/local/bin/test-cli"
    ]
...
def test_env_override_single_path(monkeypatch, tmp_path):
    ...
    assert resolve_acp_cli(
        env_var="MY_ACP_BIN", bin_names=["unused"], npm_pkg="pkg"
    ) == [str(target)]
...
def test_path_lookup(monkeypatch, tmp_path):
    ...
    argv = resolve_acp_cli(
        env_var="MY_ACP_BIN", bin_names=["fake-cli-acp"], npm_pkg=None
    )
...
def test_none_when_unresolved_and_no_pkg(monkeypatch):
    ...
    assert (
        resolve_acp_cli(
            env_var="MY_ACP_BIN", bin_names=["definitely-absent-xyz"], npm_pkg=None
        )
        is None
    )
...
def test_node_manager_glob(monkeypatch, tmp_path):
    ...
    argv = resolve_acp_cli(
        env_var="MY_ACP_BIN", bin_names=["globbed-acp"], npm_pkg=None
    )
    assert argv == [str(target)]
...
def test_resolve_node_ge_picks_new_enough(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("PATH", "")
    import gideon.integrations.acp.cli_resolve as cli_resolve
...
def test_provision_disabled_returns_none(monkeypatch, tmp_path):
    ...
    assert (
        provision_acp_adapter("@scope/never-installed-xyz", ["never-installed-xyz"])
        is None
    )


def test_override_and_path_subcommand_contract(monkeypatch, tmp_path):
    binary = tmp_path / "adapter"
    _make_exec(binary)
    monkeypatch.setenv("PATH", str(tmp_path))
    monkeypatch.delenv("LOCAL_ACP_EXECUTABLE", raising=False)
    assert resolve_acp_cli(
        env_var="LOCAL_ACP_EXECUTABLE", bin_names=["adapter"], subcommand=["acp"]
    ) == [str(binary), "acp"]
    monkeypatch.setenv("LOCAL_ACP_EXECUTABLE", str(binary))
    assert resolve_acp_cli(
        env_var="LOCAL_ACP_EXECUTABLE", bin_names=["adapter"], subcommand=["acp"]
    ) == [str(binary)]


def test_preinstalled_managed_adapter_is_readable_when_provisioning_disabled(
    monkeypatch, tmp_path
):
    monkeypatch.setenv("GIDEON_HOME", str(tmp_path))
    monkeypatch.setenv("GIDEON_ACP_NO_PROVISION", "1")
    binary = tmp_path / "acp-adapters" / "node_modules" / ".bin" / "test-adapter"
    binary.parent.mkdir(parents=True)
    _make_exec(binary)
    assert provision_acp_adapter("test-package", ["test-adapter"]) == str(binary)
