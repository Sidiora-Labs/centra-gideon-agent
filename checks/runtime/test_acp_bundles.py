...
def _isolate_registry():
    ...
    gideon.llm`` is already cached and won't re-run their registration.
...
def test_claude_registers_entry_with_explicit_dialect(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("GIDEON_CC_ISOLATE", "0")  # don't touch ~/.claude
...
def test_claude_dialect_explicit_even_when_command_is_npx(monkeypatch, tmp_path):
    ...
    # Isolate HOME + GIDEON_HOME to an empty dir so neither the node-manager
    # globs NOR the managed adapter prefix (~/.gideon/acp-adapters) — both
...
def test_npx_fallback_without_node_ge_is_clean_not_found(monkeypatch, tmp_path):
    ...
    # No Node >= 20 anywhere.
    monkeypatch.setattr("gideon.acp.cli_resolve.resolve_node_ge", lambda *a, **k: None)
    cls = get_agent_provider_class("acp:codex")
...
def test_bundle_options_flow_into_client_dialect(monkeypatch, tmp_path):
    ...
    from gideon.acp.dialect import ClaudeCodeDialect
