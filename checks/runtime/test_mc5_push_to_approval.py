...
``GIDEON_HOME`` is the lever, never a ``config_dir`` monkeypatch: ``config/__init__``
...
def test_the_ntfy_body_is_the_ids_json_and_nothing_else(
    ...
    monkeypatch.setattr(push, "ntfy_topic_url", lambda: "https://ntfy.example/gideon")
...
def test_ntfy_refuses_a_plaintext_topic_url(
    ...
    monkeypatch.setattr(push, "push_backend", lambda: "ntfy")
    monkeypatch.setattr(push, "ntfy_topic_url", lambda: "http://ntfy.example/gideon")
...
async def test_the_whole_push_to_approval_chain_advances_a_paused_run(
    ...
    docstring. `gideon push test` exists so the owner can drive the missing leg by
    hand.
