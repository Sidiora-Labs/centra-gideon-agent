...
def bound_home(pack_home):
    ...
    with patch("gideon.packs.build.config_dir", return_value=pack_home):
        with patch.dict(os.environ, {"GIDEON_HOME": str(pack_home)}):
            yield pack_home


# ── done_when 1: build_pack writes a schema_version=1 .gideon ZIP ──
...
def test_template_missing_agent_edge_becomes_requirement(tmp_path):
    ...
    with patch("gideon.packs.build.config_dir", return_value=home):
...
def test_denied_store_reader_returns_none(bound_home):
    ...
    from gideon.packs.build import _read_denied_safe
...
def test_component_with_planted_credential_is_blocked(tmp_path):
    ...
    assert not (home / "x.gideon").exists()  # nothing written on a refused build
