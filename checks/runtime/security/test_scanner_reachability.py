...
_INERT_APP_JSON = json.dumps(
    {
        "name": "demo",
        "version": "1.0.0",
        "provider": {"implementation": "provider:create_provider"},
    }
)
...
def _bundle(
    tmp_path: Path, files: dict[str, str], *, manifest: str = _INERT_APP_JSON
) -> Path:
...
def _by_rule(report: Any, rule: str) -> Finding:
    ...
    assert (
        hits
    ), f"expected {rule} to fire; got {sorted(f.rule for f in report.findings)}"
    return hits[0]
...
class TestTheBlockedShapesBecomeConsentable:
    ...
    def test_a_literal_attack_string_in_a_test_is_data(
        ...
        root = _bundle(
            tmp_path, {"provider.py": _INERT_PROVIDER, "test_provider.py": body}
        )
        report = _scan(root)
    ...
    def test_the_rescored_finding_keeps_its_whole_disclosure(
        self, tmp_path: Path
    ) -> None:
        ...
        root = _bundle(
            tmp_path, {"provider.py": _INERT_PROVIDER, "test_provider.py": _OPS_SHAPE}
        )
        ...
        assert (
            unscored.severity is Verdict.DANGEROUS
            and scored.severity is Verdict.WARNING
        )
    ...
    def test_the_reason_is_readable_enough_to_check(self, tmp_path: Path) -> None:
        ...
        assert (
            "cannot execute a string" in reason
            and "nothing in the bundle reaches it" in reason
        )
...
class TestCommentaryIsNotCode:
    ...
    def test_the_sample_is_a_real_match_to_begin_with(self, rule: str) -> None:
        ...
        assert pattern.search(
            f"{_SAMPLES[rule]} "
        ), f"{rule}: the sample no longer matches"
    ...
    def test_a_comment_mentioning_the_pattern_is_not_terminal(
        ...
        root = _bundle(
            tmp_path,
            {
                ...
                "test_notes.py": _commented(_SAMPLES[rule]),
            },
        )
        report = _scan(root)
        finding = _by_rule(report, rule)
        assert (
            finding.reachability is Reachability.COMMENTARY
    ...
    def test_a_docstring_mentioning_the_pattern_is_not_terminal(
        ...
        root = _bundle(
            ...
            {
                "provider.py": _INERT_PROVIDER,
                "test_notes.py": _docstringed(_SAMPLES[rule]),
            },
        )
        report = _scan(root)
        finding = _by_rule(report, rule)
    ...
    def test_the_identical_text_in_a_live_call_is_still_terminal(
        ...
        root = _bundle(
            ...
            {
                "provider.py": _INERT_PROVIDER,
                "boot.py": _in_a_live_call(_SAMPLES[rule]),
            },
        )
        report = _scan(root)
        finding = _by_rule(report, rule)
        ...
        ), f"{rule}: the live call was let through"
    ...
    def test_a_comment_is_not_terminal_at_the_builtin_tier_either(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_the_file_is_still_scanned_and_the_finding_still_disclosed(
        ...
        root = _bundle(
            tmp_path, {"provider.py": _INERT_PROVIDER, "test_notes.py": body}
        )
        scored = _by_rule(_scan(root), "destructive_root")
        unscored = _by_rule(
            default_scanner.scan_text(body, surface="script"), "destructive_root"
        )
        assert (scored.rule, scored.surface, scored.evidence) == (
            unscored.rule,
            unscored.surface,
    ...
    def test_commentary_and_unreachable_stay_distinguishable(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_a_payload_dressed_as_a_comment_is_still_terminal(
        self, tmp_path: Path
    ) -> None:
        ...
        assert finding.reachability is Reachability.REACHABLE
        assert (
            "(L0)" not in finding.reachability_reason
            and "(L2)" in finding.reachability_reason
        )

    def test_a_comment_beside_a_live_call_does_not_clear_the_call(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_a_newly_reachable_alternative_is_scoped_like_every_other(
        self, tmp_path: Path
    ) -> None:
        ...
        assert (
            scoped.reachability is Reachability.COMMENTARY
        ), scoped.reachability_reason
        ...
        assert (
            _scan(live).verdict is Verdict.DANGEROUS
        ), "a live raw-disk redirect was let in"
    ...
    def test_the_line_table_the_span_check_stands_on_is_not_str_splitlines(
        self,
    ) -> None:
        ...
        assert len(supply_chain._offset_table(text)) - 1 == 3
...
_ATTACKS: dict[str, tuple[dict[str, str], str]] = {
    "payload in a shell script": (
        {
            "provider.py": _INERT_PROVIDER,
            "tooling/scripts/setup.sh": "rm -rf / --no-preserve-root\n",
        },
        "L1",
    ),
...
def _assert_attack_refused(root: Path, label: str, clause: str) -> None:
    ...
    scoped = [
        f for f in report.findings if f.surface == "script" and f.reachability_reason
    ]
...
class TestAttacksOnTheRule:
    ...
    def test_a_declared_entry_point_is_never_treated_as_unreached(
        self, tmp_path: Path
    ) -> None:
        ...
        root = _bundle(
            ...
            manifest=json.dumps(
                {
                    "name": "demo",
                    "provider": {"implementation": "payload:create_provider"},
                }
            ),
        )
        _assert_attack_refused(root, "declared entry point", "L3")

    def test_an_unreadable_manifest_denies_rather_than_assumes(
        self, tmp_path: Path
    ) -> None:
        root = _bundle(
            tmp_path, {"payload.py": f'P = "{_PAYLOAD}"\n'}, manifest="{ not json"
        )
        _assert_attack_refused(root, "unreadable manifest", "L3")

    def test_an_invisible_character_finding_is_never_rescoped(
        self, tmp_path: Path
    ) -> None:
        ...
        assert (
            _by_rule(report, "bidi_override").reachability
            is not Reachability.UNREACHABLE
        )
...
class TestAnUnparseableFileIsItsOwnState:
    ...
    def test_it_does_not_pass(self, tmp_path: Path) -> None:
        root = _bundle(
            tmp_path, {"provider.py": _INERT_PROVIDER, "test_broken.py": self._BROKEN}
        )
        report = _scan(root)
        assert report.verdict is Verdict.DANGEROUS
    ...
    def test_the_same_file_would_have_been_rescored_had_it_parsed(
        self, tmp_path: Path
    ) -> None:
        ...
        root = _bundle(
            tmp_path, {"provider.py": _INERT_PROVIDER, "test_broken.py": fixed}
        )
        assert (
            _by_rule(_scan(root), "destructive_root").reachability
...
def _catalog_source() -> str:
    return "".join(
        pattern.pattern for _, pattern in (*_DANGEROUS_SCRIPT, *_WARNING_SCRIPT)
    )
...
class TestTheSinkSetTracksThePatternCatalog:
    ...
    def test_every_execution_primitive_the_catalog_names_is_a_sink(self) -> None:
        ...
        for token, present in (
            ...
            assert (
                token in src
            ), f"{token} left the pattern catalog — re-check the sink set"
            assert (
                present
            ), f"the catalog flags {token} but the sink set does not know it"
...
def _corpus_cases() -> Any:
    for path in sorted(_CORPUS.glob("*.json")):
        ...
        for i, variant in enumerate(case.get("variants", [])):
            yield pytest.param(
                {"tooling/scripts/setup.sh": variant}, id=f"{path.stem}-{i}"
            )
...
class TestTheEvasionCorpusStillRefuses:
    """``checks/runtime/security/corpus/verdict-evasion`` is the existing statement of what must
    ...
    def test_case_is_unchanged(self, tmp_path: Path, files: dict[str, str]) -> None:
        ...
        assert (
            report.verdict is Verdict.DANGEROUS
        ), "the scoping let a corpus case through"
        assert not any(
            f.reachability is Reachability.UNREACHABLE for f in report.findings
        )
...
class TestRedsOnAWeakenedCheck:
    ...
    def test_neutering_the_whole_pass_reds_the_two_blocked_shapes(
        ...
        monkeypatch.setattr(
            supply_chain, "_scope_by_reachability", lambda findings, **kw: findings
        )
        _expect_rail_red(
            rail, tmp_path / "weakened", "ops", _OPS_SHAPE, "destructive_root"
        )
    ...
    def test_answering_l0_on_the_first_span_alone_reds_the_comment_beside_a_call(
        ...
        monkeypatch.setattr(
            supply_chain, "_rule_spans", lambda text, rule: real(text, rule)[:1]
        )
        root, label, clause = self._attack(tmp_path / "weakened", label)
        _expect_rail_red(_assert_attack_refused, root, label, clause)
