...
The recorder half lives in core (`gideon.assurance.trace_recorder`); the metrics/baseline half
...
def test_order_violation_detected() -> None:
    events = [_ev(seq=0), _ev(seq=1), _ev(seq=0)]
    m = replay.compute_metrics(events)
    assert m.order_violation_count == 1


def test_reconnect_loss_detected() -> None:
    events = [_ev(seq=0), _ev(seq=1), _ev(seq=4)]
    m = replay.compute_metrics(events)
        ...
        json.dumps(
            {
                "scenarios": {
                    "loose": {
                        "metrics": {},
                        "thresholds": {"duplicate_event_rate_max": 0.9},
                    }
                }
            }
        ),
