...
from gideon.extensions.packs import fingerprint as fp
from gideon.extensions.packs import update as pack_update
...
def terraform_workspace(tmp_path):
    ...
    (ws / "node_modules" / "junk" / "vendored.tf").write_text(
        "# decoy\n", encoding="utf-8"
    )
    return ws
...
def project(terraform_workspace):
    ...
    the real :class:`gideon.engine.tasks.models.Project` needs a whole store to exist, and the
...
def test_the_scanner_makes_no_model_call(home, project, monkeypatch):
    ...
    monkeypatch.setattr("gideon.security.guardrails.audit.record_attempt", _explode)
    monkeypatch.setattr(
        "gideon.integrations.llm_helpers.one_shot_completion",
        lambda *a, **k: (_ for _ in ()).throw(
            AssertionError("scan called the one-shot bridge")
        ),
    )
...
def test_the_scanner_imports_no_model_seam():
    ...
    banned = (
        "gideon.integrations.sampling",
        "gideon.extensions.providers",
        "gideon.integrations.prompt_providers",
        "gideon.security.guardrails.model_call",
        "gideon.model",
        "anthropic",
        "openai",
    )
    ...
    offenders = [
        m for m in imported if any(m == b or m.startswith(b + ".") for b in banned)
    ]
...
def test_a_scan_needs_one_of_two_reasons(home, project):
    ...
    for reason in sorted(fp.SCAN_REASONS):
        fp.scan_project(project, reason=reason, with_inspect=False)
...
def test_surfacing_writes_nothing_but_its_own_audit_row(home, project):
    ...
    assert set(new) <= set(
        _SEL_FILES
    ), f"a propose-only scan wrote entity state: {sorted(new)}"
    ...
    assert "pack_inspect" in (home / "security_events.jsonl").read_text(
        encoding="utf-8"
    )
...
def test_a_rejection_is_remembered_and_never_re_nags(home, project):
    ...
    """
    assert [p.pack for p in fp.scan_project(project, reason=fp.SCAN_REASON_CREATE)] == [
        "infra-ops"
    ]
...
def test_a_rejection_is_scoped_to_its_project(home, project):
    ...
    assert [p.pack for p in fp.scan_project(other, reason=fp.SCAN_REASON_CREATE)] == [
        "infra-ops"
    ]
...
def test_fingerprint_enabled_false_stops_scanning_entirely(home, project):
    ...
    assert [
        p.pack
        for p in fp.scan_project(project, reason=fp.SCAN_REASON_CREATE, config=on)
    ] == ["infra-ops"]
...
def test_confidence_is_the_declared_ceiling_scaled_by_coverage():
    ...
    rule = fp.Fingerprint(
        label="x", globs=("*.tf", "*.tfvars"), signals=("a", "b"), confidence=0.9
    )
    assert (
        fp._score(rule, glob_hits=2, signal_hits=2) == 0.9
    )  # full coverage → the ceiling
    assert (
        fp._score(rule, glob_hits=1, signal_hits=2) == 0.68
    )  # 0.9 * ((0.5+1.0)/2) = 0.675
    assert fp._score(rule, glob_hits=1, signal_hits=0) == 0.23
...
def test_the_card_carries_the_arithmetic_behind_its_score(home, project):
    ...
    match = fp.scan_project(project, reason=fp.SCAN_REASON_CREATE, with_inspect=False)[
        0
    ].matches[0]
...
def test_a_partial_match_scores_below_the_ceiling(home, tmp_path):
    ...
    proposal = fp.scan_project(proj, reason=fp.SCAN_REASON_CREATE, with_inspect=False)[
        0
    ]
...
def test_signals_alone_never_propose(tmp_path):
    ...
    rule = fp.Fingerprint(
        label="tf", globs=("*.tf",), signals=("terraform {",), confidence=0.9
    )
...
def test_a_signals_only_rule_is_dropped_at_parse():
    ...
    assert (
        fp.parse_fingerprints([{"label": "x", "signals": ["a"], "confidence": 1.0}])
        == []
    )
...
def test_no_workspace_means_no_scan(home):
    ...
    assert (
        fp.scan_project(SimpleNamespace(id="p", workspace_dir=""), reason="on-demand")
        == []
    )
    assert (
        fp.scan_project(
            SimpleNamespace(id="p", workspace_dir="/nope/nowhere"), reason="on-demand"
        )
        == []
    )
...
def test_a_sensitive_workspace_is_refused(home, monkeypatch, terraform_workspace):
    ...
    """
    monkeypatch.setattr("gideon.security.security.is_sensitive_path", lambda p: True)
...
def test_an_installed_pack_is_never_proposed(home, project):
    ...
    from gideon.extensions.packs.installed import InstalledPack, record_install
...
def installed_infra(home, tmp_path):
    ...
    archive = pack_bundled.build_bundled("infra-ops", tmp_path / "infra-ops.gideon")
...
def test_a_user_edited_component_is_skipped_with_a_drift_note(home, installed_infra):
    ...
    applied = pack_update.apply_update(
        "infra-ops", installed_infra, tier=TrustTier.BUILTIN
    )
    assert applied.applied is True
    assert applied.skipped == ["skill:infra-plan-review"]
    assert (
        edited.read_text(encoding="utf-8") == mine
    ), "the update clobbered a user edit"
...
def test_a_skipped_component_keeps_its_original_lock(home, installed_infra):
    ...
    second = pack_update.plan_update(
        "infra-ops", installed_infra, tier=TrustTier.BUILTIN
    )
...
def test_a_non_pack_owned_component_is_never_touched(home, tmp_path, monkeypatch):
    ...
    manifest["pack_owned"] = ["skills/infra-*"]
    ...
    archive = pack_bundled.build_bundled("infra-ops", tmp_path / "narrow.gideon")
    import_pack(archive, tier=TrustTier.BUILTIN)
...
def test_an_unlockable_component_is_never_clobbered(home, installed_infra):
    ...
    from gideon.extensions.packs.installed import load_installed, record_install
    from gideon.security.supply_chain import TrustTier
...
def test_an_update_replaces_in_place_rather_than_forking(home, installed_infra):
    ...
    assert [p.name for p in (home / "workflows" / "defs").iterdir()] == [
        "infra-change-review"
    ]
...
def test_updating_an_uninstalled_pack_is_refused(home, tmp_path):
    ...
    archive = pack_bundled.build_bundled("infra-ops", tmp_path / "p.gideon")
...
def test_an_update_must_be_the_same_pack(home, installed_infra, tmp_path):
    ...
    other = pack_bundled.build_bundled("health-os", tmp_path / "health.gideon")
...
def test_component_digest_moves_on_any_content_change(tmp_path):
    ...
    (d / ".gideon-lock.json").write_text('{"installed_at": 1}', encoding="utf-8")
    assert (
        pack_update.component_digest(d) == base
    ), "the lock file is provenance, not content"
...
def test_infra_ops_round_trips_onto_a_second_home(tmp_path, monkeypatch):
    ...
    locked = {p.parent.name for p in (fresh / "skills").rglob(".gideon-lock.json")}
    assert {"infra-plan-review", "infra-drift-audit"} <= locked
    assert (fresh / "agents" / "infra-reviewer" / "agent.json").is_file()
    assert (
        fresh / "workflows" / "defs" / "infra-change-review" / "workflow.json"
    ).is_file()
...
def test_pack_owned_patterns_cover_a_directory_component():
    ...
    assert pack_update.is_pack_owned(
        "skills/infra-plan-review/SKILL.md", ["skills/infra-*"]
    )
