...
(``SemanticArchive.write_lesson``) and read at the REAL injection seam
...
@pytest.fixture
def vs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> SemanticArchive:
    ...
    lc.reset_store()
    store = SemanticArchive(db_path=tmp_path / "memory.db")
    store.init()
    yield store
    lc.reset_store()


def _live_rules(store: SemanticArchive) -> list[str]:
    return [json.loads(row["value_json"]) for row in store.get_lessons()]


def _prompt(store: SemanticArchive) -> str:
...
def _standing(store: SemanticArchive, rule: str) -> lc.LessonVerdict:
...
def test_the_shipped_config_default_matches_the_derived_floor():
    ...
    assert LearningConfig().min_lesson_confidence == pytest.approx(
        lc.DEFAULT_MIN_CONFIDENCE
    )
...
def test_a_single_unrepeated_observation_does_not_reach_a_prompt(vs):
    ...
    assert (
        vs.write_lesson(RULE, "tool", source=AGENT) is True
    ), "premise: the row was written"
...
def test_the_prompt_seam_can_inject_at_all(vs):
    ...
    """
    assert (
        vs.write_lesson("always sign off commits with a DCO trailer", "process") is True
    )
...
def test_a_retained_lesson_is_never_deleted_by_the_gate(vs):
    vs.write_lesson(RULE, "tool", source=AGENT)
    _prompt(vs)
    assert _live_rules(vs) == [
        RULE
    ], "the gate must not evict what it declines to inject"
...
def test_a_refuted_lessons_corroboration_does_not_transfer_to_its_refuter(vs):
    ...
    for _ in range(4):
        vs.write_lesson("deploy on friday is fine", "tool", source=AGENT)
    assert "deploy on friday is fine" in _prompt(
        vs
    ), "premise: the old rule was injected"

    vs.write_lesson("deploy on weekend is risky", "tool", source=AGENT)
    assert "deploy on weekend is risky" not in _prompt(vs), "one observation of its own"
    assert "deploy on friday is fine" not in _prompt(
        vs
    ), "refuted, so no longer injected"
...
def test_a_supersession_carries_evidence_forward_to_the_survivor(vs):
    ...
    vs.write_lesson("use ruff for linting", "tool", source=AGENT)
...
def test_every_declared_evidence_input_moves_the_confidence():
    ...
    assert lc.derive(lc.LessonEvidence(observations=5, contradictions=2)) < lc.derive(
        base
    )
...
def test_a_human_authored_lesson_ages_slower_but_is_not_exempt():
    ...
    assert lc.derive(taught, active_days_idle=100.0) > lc.derive(
        inferred, active_days_idle=100.0
    )


def test_the_reported_standing_matches_what_the_prompt_contains(vs):
    vs.write_lesson("always run make lint before pushing", "process")
    vs.write_lesson(RULE, "tool", source=AGENT)
    prompt = _prompt(vs)
    rows = vs.get_lessons()
    ...
    for row in rows:
        ...
        assert (
            rule in prompt
        ) is verdict.injected, f"{rule!r} disagrees with {verdict.standing}"
...
def test_the_gate_fails_open_rather_than_stripping_every_lesson(vs, monkeypatch):
    ...
    monkeypatch.setattr(SemanticArchive, "_lesson_evidence_store", boom)
    verdicts = vs.lesson_standings(vs.get_lessons())
    assert all(v.injected for v in verdicts.values())
    assert RULE in _prompt(
        vs
    ), "fail-open: an unreadable gate injects rather than drops"
...
def test_a_nonsense_threshold_clamps_instead_of_removing_the_gate(
...
async def test_the_lessons_endpoint_reports_the_standing_and_its_evidence(vs, tmp_path):
    ...
    from gideon.interfaces.dashboard.handlers.schedule import api_lessons
    ...
    builder.memory = mem
    state = ConsoleState(
        sessions=MagicMock(count=0), start_time=0.0, context_builder=builder
    )
    request = MagicMock()
...
def _all_keys(store: SemanticArchive) -> list[str]:
    return [
        str(r["key"])
