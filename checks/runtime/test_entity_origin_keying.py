...
def test_a_project_origin_is_server_owned_not_caller_set(tmp_path):
    """Same contract as a task's, on the other two entity types the plan names.

    ``create_project`` takes no ``origin_harness``: there is no parameter to forge one
    through, which is a stronger guarantee than ignoring the value would be.
    """
    with patch("gideon.engine.tasks.hierarchy.config_dir", return_value=tmp_path):
        store = HierarchyStore()
        with pytest.raises(TypeError):
            store.create_project(name="Forged", origin_harness="somebody-elses-machine")
        project = store.create_project(name="Launch")
    assert project.origin_harness == machine_id(tmp_path)


def test_an_update_cannot_rewrite_a_projects_origin(tmp_path):
    """Write-once, like a task's: re-stamping a merged foreign row would collapse the
    attribution the field exists to preserve."""
    with patch("gideon.engine.tasks.hierarchy.config_dir", return_value=tmp_path):
        store = HierarchyStore()
        project = store.create_project(name="Launch")
        original = project.origin_harness
        updated = store.update_project(
            project.id, name="Relaunch", origin_harness="in-process-forge"
        )
    assert updated is not None
    assert updated.name == "Relaunch"
    assert updated.origin_harness == original
    assert PROJECT_ID_RE.match(updated.id)


def test_a_trigger_origin_is_server_owned_and_not_patchable(tmp_path):
    """The trigger tool surface neither accepts an origin at create nor patches one."""
    store = TriggerStore(base_dir=tmp_path)
    with pytest.raises(TypeError):
        T.create(
            store,
            ...
            spec={"kind": "cron", "expr": "0 9 * * *"},
            message="x",
            origin_harness="somebody-elses-machine",
        )
    assert T.create(
        store,
        name="Daily digest",
    ...
    result = T.update(
        store,
        trigger_id="clock:daily-digest",
        patch={"name": "Daily digest v2", "origin_harness": "in-process-forge"},
    )
    assert result.ok, result.text
    assert "origin_harness" in result.data["rejected"]
    saved = store.get("clock:daily-digest")
    assert saved is not None
    assert saved.trigger.name == "Daily digest v2"
    assert saved.trigger.origin_harness == machine_id(tmp_path)


@pytest.mark.asyncio
async def test_a_provider_served_task_keeps_its_foreign_origin_on_read(tmp_path):
    """A row another harness minted is served back unchanged — ac_2's compatibility half
    for tasks, matching the trigger case above."""
    foreign = Task.from_dict(
        {
            "id": "t-abc12345",
            "title": "minted elsewhere",
            "origin_harness": "alices-laptop",
        }
    )
    assert foreign.origin_harness == "alices-laptop"
    assert foreign.to_dict()["origin_harness"] == "alices-laptop"
    assert TASK_ID_RE.match(foreign.id)
