...
def test_an_unreadable_store_is_a_missing_secret_not_a_crash(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gideon.config.loader.config_dir", lambda: tmp_path)
    with patch("gideon.llm.credentials.CredentialStore", side_effect=OSError("unreadable")):
...
def test_the_workflow_engine_and_the_trigger_path_share_the_credential_store():
    ...
    from gideon.workflows import controller
...
def test_the_lint_never_breaks_a_parse(tmp_path):
    ...
    from gideon.triggers import models as M

    with patch(
        "gideon.workflows.secrets.find_inline_secrets", side_effect=RuntimeError("boom")
    ):
