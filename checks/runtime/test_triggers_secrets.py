...
def test_two_references_in_one_string_both_resolve():
    assert (
        S.resolve("{{secret:MY_KEY}}:{{secret:TOK}}", resolver=_resolver)
        == "sk-abc123:t0k"
    )
...
def test_the_refusal_NAMES_the_key():
    ...
    assert "gideon auth" in str(ei.value), "and says how to fix it"
...
def test_an_unreadable_store_is_a_missing_secret_not_a_crash(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gideon.core.config.loader.config_dir", lambda: tmp_path)
    with patch(
        "gideon.integrations.llm.credentials.CredentialStore",
...
def test_the_stored_config_is_never_mutated():
    ...
    assert (
        stored == "echo {{secret:MY_KEY}}"
    ), "the placeholder must survive on the trigger"
...
def test_the_workflow_engine_and_the_trigger_path_share_the_credential_store(
    ...
    store.save(
        {
            "TRIGGER_WORKFLOW_PARITY": {
                "type": "static_token",
                "value": "local-contract-value",
            },
            "TRIGGER_WORKFLOW_EMPTY": {"type": "static_token"},
        }
    )
    for key, expected in (
        ("TRIGGER_WORKFLOW_PARITY", "local-contract-value"),
        ("TRIGGER_WORKFLOW_EMPTY", ""),
        ("TRIGGER_WORKFLOW_ABSENT", ""),
    ):
        assert controller._secret_resolver(key) == expected
        assert S.default_resolver(key) == expected
...
def test_a_credential_shaped_literal_is_FLAGGED(tmp_path):
    ...
    row = _row(
        tmp_path,
        {"command": "curl -H 'Authorization: Bearer sk-ant-api03-REALTOKEN123456'"},
    )
    assert [
        i.message for i in row.warnings
    ], "a pasted token must be visible on the row"
...
def test_the_SANCTIONED_form_is_not_flagged(tmp_path):
    ...
    row = _row(
        tmp_path, {"command": "curl -H 'Authorization: Bearer {{secret:MY_KEY}}'"}
    )
...
def test_the_lint_never_breaks_a_parse(tmp_path):
    ...
    """
    from gideon.automation.triggers import models as M

    with patch(
        "gideon.automation.workflows.secrets.find_inline_secrets",
        side_effect=RuntimeError("boom"),
    ):
        assert (
            M._inline_credential_issues({"inline": {"config": {"api_key": "x"}}}) == []
        )
