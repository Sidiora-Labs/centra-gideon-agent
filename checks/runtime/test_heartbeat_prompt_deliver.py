...
@pytest.fixture()
def dashboard_state():
    """Minimal ConsoleState mock with session support."""
    state = MagicMock()
...
def orchestrator(dashboard_state):
    """Minimal RuntimeCoordinator with dashboard_state wired up.

    Note: bypasses __init__ via __new__. If RuntimeCoordinator.__init__ adds
...
class TestPromptDashboardDeliver:
    ...
    async def test_prompt_warns_on_missing_session(
        ...
        sel_mock = MagicMock()
        monkeypatch.setattr("gideon.engine.gateway.sel", lambda: sel_mock)
        await orchestrator._deliver_result(
    ...
    async def test_prompt_skips_notify_and_push_when_queued(
        ...
        """
        state, session = dashboard_state
        session.enqueue_or_run_prompt.return_value = False
        await orchestrator._deliver_result(
            "💓 Heartbeat", "CR check", "Fix these", "prompt:dashboard:chat-1"
        )
        session.enqueue_or_run_prompt.assert_called_once()
    ...
    async def test_prompt_fires_notify_and_push_when_run(
        ...
        """
        state, session = dashboard_state
        session.enqueue_or_run_prompt.return_value = True
        await orchestrator._deliver_result(
            "💓 Heartbeat", "CR check", "Fix these", "prompt:dashboard:chat-1"
        )
        state.notify.assert_called_once()
    ...
    async def test_prompt_noop_when_no_dashboard_state(self, orchestrator, caplog):
        ...
        caplog.set_level(logging.DEBUG, logger="gideon.engine.gateway")
    ...
    @pytest.mark.asyncio()
    async def test_prompt_empty_session_name_ignored(
        self, orchestrator, dashboard_state, caplog
    ):
        import logging
    ...
    @pytest.mark.asyncio()
    async def test_prompt_truncates_oversized_ascii(
        self, orchestrator, dashboard_state, caplog
    ):
    ...
    @pytest.mark.asyncio()
    async def test_prompt_truncates_utf8_boundary_safely(
        self, orchestrator, dashboard_state
    ):
        ...
        delivered = session.enqueue_or_run_prompt.call_args.args[0]
        delivered.encode("utf-8").decode("utf-8")
        assert len(delivered.encode("utf-8")) <= MAX_PROMPT_BYTES
        assert "\ufffd" not in delivered

    @pytest.mark.asyncio()
    async def test_prompt_passes_through_at_size_boundary(
        self, orchestrator, dashboard_state
    ):
...
class TestDashboardSessionInjectDeliver:
    ...
    async def test_inject_resolves_via_prefix(self, orchestrator, dashboard_state):
        ...
        state.resolve_session = lambda name: ConsoleState.resolve_session(state, name)
...
class TestLogTaskException:
    ...
    async def test_cancelled_task_short_circuits(self, caplog):
        ...
        from gideon.interfaces.dashboard.state import _log_task_exception
    ...
    async def test_redaction_failure_falls_back_to_raw(self, caplog, monkeypatch):
        ...
        monkeypatch.setattr(
            state_mod,
            "redact_credentials",
            lambda _tb: (_ for _ in ()).throw(ValueError("boom")),
        )
...
class TestSessionPrefixMatch:
    ...
    def test_get_session_is_exact_match_only(self):
        ...
        assert ConsoleState.get_session(state, "chat-2") is None
    ...
    def test_resolve_session_exact_match_preferred(self):
        ...
        state = self._state_with_sessions(sessions)
        assert ConsoleState.resolve_session(state, "chat-2") is exact
    ...
    def test_resolve_session_prefix_fallback(self):
        ...
        assert ConsoleState.resolve_session(state, "chat-2") is timestamped

    def test_resolve_session_no_match_returns_none(self):
        from gideon.interfaces.dashboard.state import ConsoleState

        state = self._state_with_sessions({})
        assert ConsoleState.resolve_session(state, "chat-99") is None
    ...
    def test_resolve_session_tie_break_first_insertion(self):
        ...
        state = self._state_with_sessions(sessions)
        assert ConsoleState.resolve_session(state, "chat-2") is oldest
    ...
    def test_resolve_session_does_not_match_cross_prefix(self):
        ...
        state = self._state_with_sessions(sessions)
        assert ConsoleState.resolve_session(state, "chat-2") is None
    ...
    def test_resolve_session_rejects_non_chat_n_names(self):
        ...
        state = self._state_with_sessions(sessions)
        assert ConsoleState.resolve_session(state, "chat") is None
        # Non-chat namespace → no fallback
        assert ConsoleState.resolve_session(state, "cron-1") is None
        # Empty string → no fallback
        assert ConsoleState.resolve_session(state, "") is None

    @pytest.mark.asyncio()
    async def test_notify_meta_uses_resolved_session_key(
        self, orchestrator, dashboard_state
    ):
    ...
    @pytest.mark.asyncio()
    async def test_heartbeat_deliver_with_bare_prefix(
        self, orchestrator, dashboard_state
    ):
