...
⚠️  Every test here writes credentials, so the home is redirected TWICE — ``GIDEON_HOME`` and
...
real ``~/.gideon/.env`` would be an unacceptable defect, not a flake. ``keyring`` is an
