...
The reader's half then had a hole of its own: the repair covered only the five fields the PUT
boundary can write, while `redact_item` — the same one-pass-every-reader function — also
dereferences `context_summary` and `thread_context`, and the list handler SORTS on
`created_at`. A stored `context_summary: {...}` (an app-emitted item, an older writer, a
hand-edited file) 500'd `GET /api/inbox` exactly as `draft` did, permanently. The repair set is
now derived from the dataclass — every field that HAS a default, and only those.
"""
...
@pytest.mark.asyncio
async def test_an_unknown_item_is_refused_before_its_fields_are_even_read(
    env, monkeypatch
):
    """🪤 The order the boundary promises: resolve, THEN validate. A body that is both
    addressed to a missing item and wrong-typed answers 404 (the item), not 400 (the
    field) — and neither branch may write anything."""
    st, store, inbox_state, tmp_path = env
    signals: list[Any] = []
    monkeypatch.setattr(
        h, "_record_signal", lambda state, item, kind: signals.append((item, kind))
    )
    ...
    status, body = await _put(
        st, {"status": "dismissed", "draft": {"bad": 1}, "favorited": True}, "nope"
    )
    assert (status, body) == (404, {"error": "not found"})
    assert inbox_state.dismissed == set() and signals == []
    assert (tmp_path / "inbox.json").read_text() == before


@pytest.mark.parametrize(
    "field,poison,repaired",
    [
        ("context_summary", {"a": 1}, ""),
        ("thread_context", "not a list", []),
        ("created_at", "not a number", 0.0),
        ("refs", "not an object", {}),
        ("item_kind", 5, "message"),
        ("source", None, "native"),
        ("can_reply", "yes", False),
...
def test_the_reader_repairs_every_field_that_has_a_safe_default(
    tmp_path: Path, field, poison, repaired
):
    """🔑 The repair is not scoped to what the PUT handler writes — it is scoped to what has
    a default. `draft` was simply the first one anybody hit."""
    (tmp_path / "inbox.json").write_text(
        json.dumps(
            {
                "items": [
    ...
    store.load()
    item = store.items[ITEM_ID]
    assert getattr(item, field) == repaired
    assert item.message == "the message must survive"
    assert redact_item(item.to_dict())["message"] == "the message must survive"


@pytest.mark.asyncio
async def test_a_poisoned_context_summary_no_longer_takes_out_the_whole_list(
    env, tmp_path: Path
):
    """🔑 The outage, reproduced on a field the 400 cannot reach: `redact_item` regexes
    `context_summary` for every item in the list, so one poisoned row 500'd `GET /api/inbox`
    for all of them."""
    st, store, _, _ = env
    ...
    poisoned = InboxStore(path=tmp_path / "poisoned.json")
    poisoned.load()
    store.items.update(poisoned.items)

    status, items = await _get(st)
    assert status == 200, "one poisoned row still takes out the list"
    assert {i["id"] for i in items} == {ITEM_ID, "c1_1700000001.1"}
    assert [i["context_summary"] for i in items if i["id"] == "c1_1700000001.1"] == [""]


def test_the_repair_set_is_exactly_the_fields_that_have_a_default():
    """🪤 The safety argument, asserted rather than assumed: derived from the dataclass, so a
    field added later cannot arrive unrepairable, and a REQUIRED field can never be in it —
    there is no safe value to invent for one."""
    import dataclasses

    from gideon.integrations.inbox import _REPAIRABLE_FIELD_TYPES

    defaulted = {
        f.name
        for f in dataclasses.fields(InboxItem)
        if f.default is not dataclasses.MISSING
        or f.default_factory is not dataclasses.MISSING  # type: ignore[misc]
    }
    assert set(_REPAIRABLE_FIELD_TYPES) == defaulted
    assert "message" not in _REPAIRABLE_FIELD_TYPES
    assert "id" not in _REPAIRABLE_FIELD_TYPES


def test_every_writable_field_is_also_repairable():
    """The write contract and the read repair are two maps; a field the boundary refuses but
    the reader keeps would be poisoned once and unreadable forever."""
    from gideon.integrations.inbox import (
        _REPAIRABLE_FIELD_TYPES,
        _UPDATABLE_FIELD_TYPES,
    )

    for name, declared in _UPDATABLE_FIELD_TYPES.items():
        assert declared in _REPAIRABLE_FIELD_TYPES[name]
