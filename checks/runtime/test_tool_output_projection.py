...
def test_csv_projection_head_tail_rows():
    ...
    assert "col1,col2,col3" in p.text
    assert "more rows" in p.text
...
def test_project_and_retain_large_projects_and_retains(tmp_path, monkeypatch):
    ...
    out, meta = project_and_retain(
        big, session_key="sess-op5", content_type="log", cap=2000
    )
...
class TestUserProjectionRules:
    ...
    def test_user_rule_wins_over_heuristic(self):
        ...
        set_user_rules(
            [ProjectionRule(name="acme", match_regex=r"^\[ACME\]", strategy="log")]
        )
    ...
    def test_bad_regex_is_skipped_fail_soft(self):
        ...
        import gideon.integrations.tool_providers.projection as P
    ...
    def test_unknown_strategy_is_skipped(self):
        ...
        set_user_rules(
            [ProjectionRule(name="x", match_regex="foo", strategy="nonsense")]
        )
        import gideon.integrations.tool_providers.projection as P
...
class TestProjectRuleLayer:
    ...
    def test_project_rule_dispatches(self, tmp_path):
        ...
        tok = self._bind(
            tmp_path, [{"name": "svc", "match_regex": r"^\[SVC\]", "strategy": "log"}]
        )
        try:
    ...
    def test_project_beats_user_layer(self, tmp_path):
        ...
        set_user_rules(
            [ProjectionRule(name="u", match_regex=r"^\[SVC\]", strategy="csv")]
        )
        tok = self._bind(
            tmp_path, [{"name": "p", "match_regex": r"^\[SVC\]", "strategy": "log"}]
        )
        try:
    ...
    def test_mtime_cache_reloads_on_change(self, tmp_path):
        ...
        tok = self._bind(
            tmp_path, [{"name": "a", "match_regex": r"^\[A\]", "strategy": "log"}]
        )
        try:
            ...
            f.write_text(
                _json.dumps(
                    [{"name": "b", "match_regex": r"^\[B\]", "strategy": "log"}]
                )
            )
...
class TestRuleOpsV2:
    ...
    def test_skip_filter(self):
        ...
        set_user_rules(
            [
                ProjectionRule(
                    name="s", match_regex=r"^\[SVC\]", strategy="log", skip=r"^DEBUG"
                )
            ]
        )
        text = "[SVC] start\n" + "DEBUG chatter\n" * 4000 + "RESULT: 42\n"
    ...
    def test_ops_output_respects_cap(self):
        ...
        set_user_rules(
            [
                ProjectionRule(
                    name="k", match_regex=r"^\[SVC\]", strategy="log", keep=r"."
                )
            ]
        )
        text = "[SVC] x\n" + "y" * 100_000
    ...
    def test_strategy_only_rule_unaffected_by_ops_path(self):
        ...
        set_user_rules(
            [ProjectionRule(name="v1", match_regex=r"^\[SVC\]", strategy="log")]
        )
        text = "[SVC] go\n" + "noise\n" * 4000 + "ERROR boom\n" + "noise\n" * 100
        p = project_output(text, cap=2000)
        assert p.content_type == "log" and "ERROR boom" in p.text
        assert "[rule" not in p.text
