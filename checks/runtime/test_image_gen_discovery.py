...
from gideon.integrations.image_gen.provider import ImageGenModel, ImageGenProvider
...
class _FakeImg(ImageGenProvider):
    ...
    async def list_models(self):
        return [
            ImageGenModel(
                name="gpt-image-1", description="gen+edit", supports_edit=True
            ),
...
class TestImageGenDiscovery:
    ...
    async def test_skips_unavailable_provider(self, monkeypatch):
        ...
        monkeypatch.setattr(ig, "_ensure_registered", lambda: None)
        monkeypatch.setattr(
            ig, "list_providers", lambda: [_FakeImg(), _UnavailableImg()]
        )
        models = await _discover_image_gen_models()
        assert {m["provider"] for m in models} == {"MyOpenAI"}
