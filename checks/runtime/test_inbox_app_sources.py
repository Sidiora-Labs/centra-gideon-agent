...
def _isolate(tmp_path, monkeypatch):
    ...
    from gideon.integrations.inbox_providers import registry as app_sources
...
_INBOX_PROVIDER_PY = textwrap.dedent('''
    from gideon.integrations.inbox_providers.base import IncomingMessage, MessageSourceProvider
...
async def test_app_inbox_source_resolves_and_a_message_flows(tmp_path):
    ...
    from gideon.integrations.inbox_providers.registry import list_source_names
    ...
    store = InboxStore()
    svc = InboxService(
        state=InboxState(), store=store, provider=provider, user_name="Alex"
    )
...
def test_bundled_filesystem_inbox_is_a_live_consumer_of_this_path():
    ...
    from gideon.integrations.inbox_providers.filesystem_source import create_provider
...
def test_app_instance_beats_entry_point_class(monkeypatch):
    ...
    import gideon.integrations.inbox_providers as ip
    ...
    assert (
        resolved is app_instance
    ), "entry-point class shadowed the app-contributed source"
...
def test_falls_back_to_entry_point_then_native_then_filesystem(monkeypatch):
    ...
    monkeypatch.setattr(
        ip, "_cache", {SOURCE_NAME: _EntryPointFixture, "native": _NativeStub}
    )
    assert isinstance(ip.get_default_provider(SOURCE_NAME), _EntryPointFixture)

    monkeypatch.setattr(
        ip, "_cache", {"native": _NativeStub, "filesystem": _FilesystemStub}
    )
...
def test_handler_register_deregister_round_trips():
    ...
    from gideon.extensions.providers.registry import InboxTypeHandler
