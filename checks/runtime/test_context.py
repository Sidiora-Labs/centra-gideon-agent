...
from gideon.engine.hooks import ContextRule, HookManager, HooksConfig
...
class TestMemoryScopedByCwdProperty:
    ...
    def test_memory_scoped_by_cwd_in_build_session_context(
        ...
        builder = PromptAssembler(
            memory=MemoryJournal(workspace=tmp / "ws"),
            skills=ProcedureLibrary(skills_path=tmp / "skills", install_builtins=False),
        )

        calls: list[str | None] = []
        original_get_memory = PromptAssembler.get_memory_for
        ...
        with patch.object(
            PromptAssembler, "get_memory_for", side_effect=_tracking_get_memory
        ):
...
class TestWorkspaceMemoryUnification:
    ...
    Regression for the confirmed-live gap: PromptAssembler.__init__ registered its
    ...
    def test_workspace_resolves_to_main_store(self, tmp_path, monkeypatch):
        ...
        self._reset_cache()
        main = MemoryJournal(workspace=tmp_path / "mainmem")
        PromptAssembler(
            memory=main,
        ...
        resolved = PromptAssembler.get_memory_for(default_workspace_dir())
    ...
    def test_no_cwd_also_resolves_to_main_store(self, tmp_path, monkeypatch):
        ...
        assert PromptAssembler.get_memory_for(None) is main
    ...
    def test_other_cwd_still_gets_its_own_partition(self, tmp_path, monkeypatch):
        ...
        other = PromptAssembler.get_memory_for(
            str(tmp_path / "a-totally-different-project")
        )
    ...
    def test_config_profile_key_resolves_silently_not_as_provider(
        ...
        with caplog.at_level(logging.WARNING, logger="gideon.cognition.context"):
            store = PromptAssembler.get_memory_for(None, "default")
        ...
        caplog.clear()
        with caplog.at_level(logging.WARNING, logger="gideon.cognition.context"):
            PromptAssembler.get_memory_for(None, "no-such-store-or-profile-xyz")
        assert any(
            "not registered" in r.getMessage() for r in caplog.records
...
class TestContextBuilder:
    ...
    def test_current_date_appended_after_truncation(self, tmp_path, monkeypatch):
        ...
        """
        import gideon.cognition.context as ctx_mod
    ...
    def test_memory_citation_clauses_present_when_episodic_injected(self, tmp_path):
        ...
        def _episodic(self, query_text, *, cap=3000, citations_out=None):
            if citations_out is not None:
                citations_out.append(
                    {"n": 1, "id": "42", "preview": "deployed billing"}
                )
        ...
        assert "cite it inline as `[Memory N]`" in msg
    ...
    def test_hook_modify(self, tmp_path):
        from gideon.engine.hooks import TransformHook
    ...
    def test_history_budget_limits_total_chars(self, tmp_path):
        ...
        from gideon.cognition.context import _HISTORY_BUDGET_CHARS
        from gideon.cognition.history import ConversationLog
        ...
        assert len(history_block) <= _HISTORY_BUDGET_CHARS + 1000
...
class TestCompressAssistantMessage:
    ...
    def test_small_code_block_preserved(self):
        from gideon.cognition.context import _compress_assistant_message
...
class TestCompressThreadHistory:
    ...
    async def test_long_transcript_calls_llm(self, tmp_path, monkeypatch):
        ...
        result = await compress_thread_history(
            conv_log, "t1", "latest q", mock_sessions
        )
        assert result is not None
    ...
    def test_build_session_context_uses_compressed_history(self, tmp_path):
        ...
        ctx = builder.build_session_context(
            "t1",
            compressed_history="COMPRESSED: user asked about color, answer was blue",
        )
...
class TestLoadAgentPrompt:
    ...
    def test_null_prompt_returns_empty(self, tmp_path, monkeypatch):
        ...
        assert PromptAssembler._load_agent_prompt("test") == ""
...
class TestRuntimeDisplayName:
    ...
    def test_runtime_display_name(self, session_key, expected_runtime):
        from gideon.cognition.context import _runtime_display_name
    ...
    def test_agent_identity_injected_with_session_key(self, tmp_path):
        ...
        builder = PromptAssembler(memory=MemoryJournal(workspace=tmp_path))
...
class TestCurrentDateTimezone:
    ...
    def test_current_date_uses_configured_timezone(self, tmp_path):
        ...
        with patch("gideon.core.config.loader.AppConfig.load") as mock_load:
    ...
    def test_current_date_falls_back_to_utc_only_when_nothing_resolves(
        self, tmp_path, monkeypatch
    ):
