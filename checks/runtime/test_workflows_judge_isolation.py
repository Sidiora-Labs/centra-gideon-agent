...
from gideon.automation.workflows.engine import dispatch_gate
from gideon.automation.workflows.models import FailureClass, InstanceState, Node
...
def _gate(cfg: dict) -> Node:
    return Node.from_dict(
        {"kind": "gate", "id": "accept", "config": {"kind": "judge", **cfg}}
    )
...
async def test_a_different_family_judge_runs_and_is_pinned() -> None:
    ...
    node = _gate(
        {"prompt": "accept?", "evidence": _EVIDENCE, "isolation": "cross_model"}
    )
    ...
    assert (
        seen["model"] == "OpenAI:gpt-5"
    ), "the validated different-family model must be pinned"
...
async def test_a_same_family_judge_fails_closed() -> None:
    ...
    assert (
        calls["n"] == 0
    ), "an unsatisfiable isolation guarantee must not call the model at all"
