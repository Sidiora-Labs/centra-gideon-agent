...
from gideon.security.guardrails.audit import AttemptRecord, read_recent, record_attempt
from gideon.security.guardrails.breaker import BreakerState, CircuitBreaker, get_breaker
...
class FakeProvider(ModelProvider):
    ...
    def __init__(
        self, *, text="ok", tokens=(3, 5), raise_exc=None, hang=False, chunk_delay=0.0
    ):
        self._text = text
        self._tokens = tokens
...
async def _drain(provider, message="hi"):
    return "".join(
        [e.text async for e in provider.stream(message) if e.kind == EVENT_TEXT_CHUNK]
    )
...
def test_breaker_half_open_after_recovery(monkeypatch):
    ...
    assert b.is_open(now=120.0)
    assert b.state(now=131.0) is BreakerState.HALF_OPEN
    assert not b.is_open(now=131.0)
...
def test_breaker_half_open_probe_failure_reopens():
    ...
    b.record_failure(now=20.0)
    assert b.state(now=20.0) is BreakerState.OPEN
    assert b.retry_after(now=25.0) == pytest.approx(5.0)
...
def test_wrap_is_idempotent():
    inner = FakeProvider()
    g1 = wrap_model_call_guard(
        inner, use_case="reasoning", provider_name="P", model="m"
    )
...
async def test_one_shot_output_type_retries_then_raises(monkeypatch):
    ...
    with pytest.raises(OutputContractError):
        await helpers.one_shot_completion(
            "give me json", use_case="background", output_type=dict
        )
    assert len(calls) == 2
...
async def test_one_shot_output_type_succeeds_first_try(monkeypatch):
    ...
    out = await helpers.one_shot_completion(
        "json pls", use_case="background", output_type=dict
    )
