...
class _FakeNudge:
    def __init__(self, lid, session_name, first_idle_secs=0):
        self.id, self.session_name, self.active = lid, session_name, True
        self.first_idle_secs = first_idle_secs
...
class _FakeSvc:
    ...
    async def add(
        ...
        *,
        ...
        if self.add_entered is not None:
            self.add_entered.set()
        if self.add_release is not None:
            await self.add_release.wait()
        self._n += 1
        lp = _FakeNudge(f"N{self._n}", session_name, first_idle_secs)
...
class TestLifecycle:
    ...
    def test_start_pause_resume_stop(self, state, svc):
        ...
        assert svc.get_by_session(f"loop-{cid}").first_idle_secs == 5
        assert (
            _body(
                _run(
                    H.api_loop_action(
    ...
    def test_concurrent_start_and_delete_share_one_loop_lock(self, state, svc):
        cid = self._make(state)
        svc.add_entered = asyncio.Event()
        svc.add_release = asyncio.Event()
        ...
        svc.add_release.set()
        first_response, delete_response = _run(asyncio.gather(first, delete))
        assert first_response.status == 200
        assert _body(delete_response)["ok"] is True
