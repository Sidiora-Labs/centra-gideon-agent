...
def fake_config_dir(tmp_path):
    with patch("gideon.engine.session_workspace.config_dir", return_value=tmp_path):
        yield tmp_path
...
def test_write_and_read_result(fake_config_dir):
    from gideon.engine.session_workspace import read_result, write_result
...
def test_append_result(fake_config_dir):
    from gideon.engine.session_workspace import append_result, read_result
...
def test_read_missing_returns_empty(fake_config_dir):
    from gideon.engine.session_workspace import read_result

    assert read_result("s1", "nonexistent") == ""


def test_list_results(fake_config_dir):
    from gideon.engine.session_workspace import list_results, write_result
...
def test_history_roundtrip(fake_config_dir):
    from gideon.engine.session_workspace import append_history, load_history
...
def test_cleanup(fake_config_dir):
    from gideon.engine.session_workspace import cleanup, workspace_dir, write_result
...
def test_invalid_agent_id_rejected(fake_config_dir):
    from gideon.engine.session_workspace import write_result
