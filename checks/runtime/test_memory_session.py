...
def test_working_memory_round_trip(svc):
    svc.write_working_memory(
        "dashboard:sess-1", "User is refactoring the memory layer."
    )
...
def test_seal_session_distills_to_durable_episodic(svc):
    svc.write_working_memory(
        "sess-x", "Decided to use SQLite+FAISS for the record store."
    )
...
def test_seal_sweeps_unpromoted_session_records(svc):
    ...
    assert svc.get_record("pref.session_scratch") is None
...
def test_promote_by_heat_skips_cold(svc):
    ...
    svc._vs.db.execute(
        "UPDATE semantic_memory SET updated_at=? WHERE key='pref.cold'", (old,)
    )
    svc._vs.db.commit()
    svc.promote_by_heat(threshold=1.0)
    assert svc.get_record("pref.cold").scope == MemoryScope.SESSION
