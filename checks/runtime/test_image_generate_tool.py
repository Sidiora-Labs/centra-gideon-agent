...
class _FakeProvider(ImageGenProvider):
    ...
    async def generate(self, prompt, *, model="", size="", n=1, **opts):
        return [
            ImageResult(
                b64=_PNG_B64, mime="image/png", revised_prompt=f"refined: {prompt}"
            )
        ]

    async def edit(
        self, prompt, *, source_image, mask="", model="", size="", n=1, **opts
    ):
...
def _wired(tmp_path, monkeypatch):
    ...
    from gideon.integrations.image_gen import registry as ig_reg
    ...
    monkeypatch.setattr(
        "gideon.integrations.mcp_artifacts._resolve_session_key", lambda: None
    )
    return prov
...
class TestImageGenerateTool:
    ...
    def test_edit_bumps_version_on_source_artifact(self, _wired):
        ...
        out = _call_tool_inner(
            "image_generate", {"prompt": "add steam", "edit_artifact": slug}
        )
    ...
    def test_edit_nonexistent_artifact_errors(self, _wired):
        ...
        out = _call_tool_inner(
            "image_generate", {"prompt": "x", "edit_artifact": "nope"}
        )
    ...
    def test_edit_refuses_non_image_artifact(self, _wired):
        ...
        out = _call_tool_inner(
            "image_generate", {"prompt": "x", "edit_artifact": "doc"}
        )
        assert "not an existing image artifact" in out
...
class TestRegenerateAtSlug:
    ...
    def test_recreates_deleted_artifact_at_same_slug_v1(self, _wired):
        from gideon.integrations.mcp_artifacts import (
            _call_tool_inner,
    ...
    def test_appends_version_when_artifact_still_exists(self, _wired):
        ...
        assert ok is True
        assert _wired.get(slug).version == 2
