...
from gideon.interfaces.dashboard.handlers import api_file_read
...
def home_patch(tmp_path):
    ...
    roots = [
        ("Test", str(tmp_path)),
        ("Workspace", os.path.realpath(str(workspace_root()))),
    ]
    with (
...
class TestFileReadResolve:
    ...
    @pytest.mark.asyncio
    async def test_resolve_ignored_for_absolute_paths(
    ...
    @pytest.mark.asyncio
    async def test_resolve_falls_back_to_workspace(
        self, tmp_path, mock_sel, home_patch
    ):
        from gideon.core.config.loader import workspace_root
    ...
    @pytest.mark.asyncio
    async def test_resolve_prefers_workspace_over_project_dir(
        self, tmp_path, mock_sel, home_patch
    ):
        from gideon.core.config.loader import workspace_root
        ...
        with patch.dict(os.environ, {"GIDEON_PROJECT_DIR": str(proj)}):
            async with TestClient(TestServer(_make_app())) as client:
                resp = await client.get(
                    "/api/file-read?path=../../etc/passwd&resolve=1"
                )
    ...
    @pytest.mark.asyncio
    async def test_head_returns_404_for_missing_file(
        self, tmp_path, mock_sel, home_patch
    ):
        async with TestClient(TestServer(_make_app())) as client:
