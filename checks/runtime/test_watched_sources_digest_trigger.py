...
`checks/runtime/test_watched_sources_digest.py` proves `run_morning_digest` works when something calls
...
def _isolated_home(tmp_path, monkeypatch):
    ...
    gideon.core.config.loader import config_dir`, and the undo restores only the loader
...
def _wire_services(monkeypatch, state):
    ...
    monkeypatch.setattr(
        svc, "_services", svc.ActionServices(state=state, spawn_background=_spawn)
    )
...
async def _fire_from_the_store(trigger_store):
    ...
    ), f"the stored trigger names an unknown provider {provider_name!r}"
...
def test_reconciling_twice_does_not_duplicate_or_reset(trigger_store):
    ...
    assert (
        trigger_store.get(SOURCE_DIGEST_JOB_NAME).trigger.spec["expr"] == "15 6 * * 1-5"
    )
...
def test_the_stored_provider_name_is_dispatchable_and_governed(trigger_store):
    ...
    reconcile_source_digest_cron(trigger_store)
    inline = (trigger_store.get(SOURCE_DIGEST_JOB_NAME).trigger.workflow or {})[
        "inline"
    ]
...
@pytest.mark.asyncio
async def test_the_digest_RUNS_through_the_stored_trigger(
...
@pytest.mark.asyncio
async def test_mute_all_still_suppresses_through_the_trigger(
    trigger_store, tmp_path, monkeypatch
):
    ...
    `ConsoleState.notify` → `notification_allowed()` would deliver here.
    """
    from gideon.extensions.providers import entity_routes
...
async def test_an_unwired_state_refuses_rather_than_bypassing_the_gate(
    ...
    """No `ConsoleState` means no gate. Running anyway would notify past `mute_all`."""
    from gideon.integrations.action_providers import services as svc
...
def test_gateway_boot_calls_the_reconciler():
    ...
    assert code.index("if self.runtime._no_crons:") < code.index(
        "self.reconcile(store)"
    )
    names = [entry for _, entry, _ in RECONCILERS]
    assert names.count("reconcile_source_digest_cron") == 1
    assert "reconcile_usage_recap_cron" in names
    assert "reconcile_source_digest_cron_typo" not in names
    assert "getattr(import_module(module), entry)(store)" in inspect.getsource(
        AutomationBoot.reconcile
    )
