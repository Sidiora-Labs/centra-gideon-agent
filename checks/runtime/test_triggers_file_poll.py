...
from gideon.automation.triggers import file_poll as P
...
def test_a_corrupt_sidecar_degrades_to_unseeded_not_a_crash(home, watched):
    ...
    (watched / "a.md").write_text("x")
    P.save_state(
        "file:notes", P.WatchState(hashes={"stale": "x"}, seeded=True), base_dir=home
    )
...
def test_a_file_trigger_with_no_paths_is_skipped_not_a_cwd_scan(home):
    ...
    store = _store(
        home, Trigger(id="file:bad", name="B", kind="file", enabled=True, spec={})
    )
...
def test_one_failing_watch_does_not_stop_the_others(home, watched, monkeypatch):
    ...
    assert len(calls) == 2
    assert any(f["trigger_id"] == "file:good" for f in fires)
...
def test_file_triggers_are_disjoint_from_the_clock_tick(home, watched):
    ...
    """
    from gideon.automation.triggers import service as S
