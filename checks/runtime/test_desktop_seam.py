...
The Electron half's state machine is unit-tested in ``apps/desktop/test/capabilities.test.js``
...
from gideon.interfaces.dashboard.desktop_registry import (
    CAPABILITIES,
    GRANT_STATES,
    DesktopRegistry,
    normalize_manifest,
)
from gideon.interfaces.dashboard.handlers import desktop as desktop_handlers
...
def test_registry_never_exposes_the_token():
    reg = DesktopRegistry()
    token = reg.register(
        shell={"version": "0.1.0", "platform": "darwin"}, capabilities={}
    )
    assert token
...
def test_update_and_unregister_require_the_token():
    ...
    assert reg.snapshot()["capabilities"]["tray"]["granted"] == "granted"
...
async def _client(tmp_path, monkeypatch, *, local_secret=LOCAL_SECRET):
    ...
    with (
        ...
        app.router.add_post(
            "/api/desktop/register", desktop_handlers.api_desktop_register
        )
        app.router.add_post(
            "/api/desktop/unregister", desktop_handlers.api_desktop_unregister
        )
        app.router.add_get("/api/desktop/state", desktop_handlers.api_desktop_state)
        app.router.add_post(
            "/api/desktop/state", desktop_handlers.api_desktop_state_push
        )
        app.router.add_get(
...
@pytest.mark.asyncio
async def test_register_without_the_local_secret_is_403_and_sel_logged(
    tmp_path, monkeypatch
):
    async with _client(tmp_path, monkeypatch) as client:
        res = await client.post(
            "/api/desktop/register", json={"capabilities": _MANIFEST}
        )
...
@pytest.mark.asyncio
async def test_non_loopback_register_is_refused_before_the_credential(
    tmp_path, monkeypatch
):
...
async def test_state_push_requires_the_shell_token(tmp_path, monkeypatch):
    async with _client(tmp_path, monkeypatch) as client:
        ...
        assert client.desktop.snapshot()["capabilities"]
        ...
        assert (
            client.desktop.snapshot()["capabilities"]["audio_capture"]["granted"]
            == "granted"
        )
        assert "tray" not in client.desktop.snapshot()["capabilities"]
...
async def test_unregister_disconnects_and_needs_the_token(tmp_path, monkeypatch):
    async with _client(tmp_path, monkeypatch) as client:
        ...
        ok = await client.post(
            "/api/desktop/unregister", json={}, headers={"X-Shell-Token": token}
        )
        assert ok.status == 200
...
def test_checker_is_exact_match_and_deny_by_default():
    assert (
        PermissionChecker("a", Permissions()).can_use_desktop("audio_capture") is False
    )
    ...
    assert (
        PermissionChecker("a", Permissions(desktop=["*"])).can_use_desktop("tray")
        is False
    )
    assert (
        PermissionChecker("a", Permissions(desktop=["audio_*"])).can_use_desktop(
            "audio_capture"
        )
        is False
    )
...
async def test_app_with_the_grant_reads_its_capability(tmp_path, monkeypatch):
    async with _client(tmp_path, monkeypatch) as client:
        ...
        res = await client.get(
            "/api/desktop/capabilities/audio_capture",
            headers={"X-Test-App": "voice-app"},
        )
...
async def test_app_without_the_grant_gets_403_and_a_sel_row(tmp_path, monkeypatch):
    async with _client(tmp_path, monkeypatch) as client:
        ...
        row = [
            ...
            if r["operation"] == "desktop.capability_denied"
        ][-1]
...
async def test_app_declaring_one_capability_cannot_read_another(tmp_path, monkeypatch):
    async with _client(tmp_path, monkeypatch) as client:
        ...
        await _register(client)
        res = await client.get(
            "/api/desktop/capabilities/screen_capture",
            headers={"X-Test-App": "voice-app"},
        )
        assert res.status == 403
...
@pytest.mark.asyncio
async def test_app_with_no_desktop_grant_cannot_read_the_whole_state(
    tmp_path, monkeypatch
):
    async with _client(tmp_path, monkeypatch) as client:
        _install(tmp_path, "quiet-app")
        ...
        denied = await client.get(
            "/api/desktop/state", headers={"X-Test-App": "quiet-app"}
        )
        assert denied.status == 403
        allowed = await client.get(
            "/api/desktop/state", headers={"X-Test-App": "voice-app"}
        )
...
async def test_an_unknown_app_identity_is_denied(tmp_path, monkeypatch):
    ...
    async with _client(tmp_path, monkeypatch) as client:
        await _register(client)
        res = await client.get(
            "/api/desktop/state", headers={"X-Test-App": "ghost-app"}
        )
        assert res.status == 403
