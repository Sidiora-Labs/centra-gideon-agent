...
from gideon.operations.durability.registry import (
    REGISTRY_KEY,
    MachineEntry,
    Registry,
    shard_prefix,
...
class TestParseSerialize:
    ...
    def test_round_trip(self):
        r = Registry(
            machines={
                "m1": MachineEntry("m1", seq=3, last_export_at="t", manifest_sha="sha")
            }
        )
    ...
    def test_non_dict_machine_value_skipped(self):
        r = Registry.loads(
            json.dumps({"machines": {"m1": "nope", "m2": {"seq": 2}}}).encode()
        )
...
class TestBump:
    ...
    def test_bump_is_monotonic(self):
        ...
        assert r.machines["m1"].manifest_sha == "s2"
...
class TestAdvancedOver:
    def test_reports_peers_that_moved(self):
        ...
        cur.bump("p1", manifest_sha="s2", now="t2")
        cur.bump("p2", manifest_sha="s", now="t")
