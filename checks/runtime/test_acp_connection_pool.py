...
async def test_claim_returns_none_when_empty():
    pool, _ = _make_pool()
    assert await pool.claim("acp:test-cli") is None
...
async def test_invalidate_shuts_down_and_clears():
    ...
    assert built[0]._alive is False
    assert pool.snapshot("acp:test-cli") is None
...
async def test_health_loop_skips_runtime_within_backoff_window(monkeypatch):
    ...
    import gideon.integrations.acp.connection_pool as cp
    ...
    assert (
        calls["n"] == 1
    ), "health loop re-warmed a runtime still inside its backoff window"
...
async def test_open_session_reuses_shared_connection(monkeypatch):
    ...
    import gideon.integrations.llm.acp_session_provider as prov_mod

    monkeypatch.setattr(prov_mod, "open_acp_session_provider", fake_opener)

    pool, _ = _make_pool()
    p1 = await pool.open_session(
        "acp:demo-cli", cwd="/tmp", command=["demo"], dialect="default"
    )
    p2 = await pool.open_session(
        "acp:demo-cli", cwd="/tmp", command=["demo"], dialect="default"
    )

    assert len(spawns) == 1
    assert shared.opened == 2
    assert p1.session_id == "sess-1" and p2.session_id == "sess-2"
    assert p1._conn is p2._conn is shared


@pytest.mark.asyncio
async def test_pool_shutdown_prevents_waiting_warm_from_acquiring_resources():
    from gideon.integrations.acp.connection_pool import _build_acp_provider

    pool = AcpConnectionPool(
        provider_builder=_build_acp_provider, start_sem=asyncio.Semaphore(1)
    )
    slot = await pool._slot("acp:not-installed")
    await slot.warming.acquire()
    pending = asyncio.create_task(pool.warm("acp:not-installed"))
    await asyncio.sleep(0)
    shutdown = asyncio.create_task(pool.shutdown())
    await asyncio.sleep(0)
    assert pool._closed
    slot.warming.release()
    assert await asyncio.wait_for(pending, 1) is False
    await asyncio.wait_for(shutdown, 1)
    assert slot.provider is None and slot._shared_conn is None


@pytest.mark.asyncio
async def test_pool_shutdown_joins_owned_background_tasks():
    from gideon.integrations.acp.connection_pool import _build_acp_provider

    pool = AcpConnectionPool(
        provider_builder=_build_acp_provider, start_sem=asyncio.Semaphore(1)
    )
    pool._spawn_bg(asyncio.sleep(30))
    await asyncio.sleep(0)
    tasks = tuple(pool._background)
    await pool.shutdown()
    assert tasks and all(task.done() for task in tasks)
    assert not pool._background
