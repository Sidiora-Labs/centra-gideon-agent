...
def git_repo(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda raw: str(repo) if raw == str(repo) else None,
    )
    return repo
...
def test_unborn_branch_reports_real_name_not_HEAD(tmp_path, monkeypatch):
    ...
    assert status == 200
    assert body["branch"] and body["branch"] != "HEAD"
...
def test_non_repo_returns_empty(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        F,
        "_validate_dashboard_path",
        lambda raw: str(plain) if raw == str(plain) else None,
    )
    monkeypatch.setattr(
        F,
        "_sel",
...
def test_git_helper_reaps_child_on_timeout(tmp_path, monkeypatch):
    ...
    async def run():
        ...
        assert res.ok is False and res.out == ""
        assert elapsed >= 0.2, (
...
def test_git_original_kills_and_reaps_a_timed_out_read(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        F, "_validate_dashboard_path", lambda raw: fp if raw == fp else None
    )
...
def test_the_git_invoker_matcher_is_not_vacuous():
    ...
    assert not [
        n for n in ast.walk(other) if _spawns_git(n)
    ], "rg is not a git invocation"
...
def test_git_log_returns_commits(git_repo, monkeypatch):
    ...
    req = make_mocked_request(
        "GET", f"/api/file-git-log?path={quote(str(git_repo))}&limit=10"
    )
...
def test_git_commit_returns_diff(git_repo, monkeypatch):
    ...
    req = make_mocked_request(
        "GET", f"/api/file-git-commit?path={quote(str(git_repo))}&hash={h}"
    )
...
def test_git_original_signals_truncation_for_large_committed_file(
...
def test_git_commit_rejects_non_hex_hash(git_repo, monkeypatch):
    ...
    req = make_mocked_request(
        "GET",
        f"/api/file-git-commit?path={quote(str(git_repo))}&hash={quote('HEAD; rm -rf /')}",
    )
    resp = asyncio.run(F.api_file_git_commit(req))
