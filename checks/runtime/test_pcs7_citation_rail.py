...
def _range_contains(
    rel_path: str, start: int, end: int, token: str, *, root: Path = SRC
) -> bool:
...
def _doc() -> str:
    ...
    assert (
        doc
    ), "cache_hit_pct lost its docstring — the evidence block IS the deliverable"
    return doc
...
class TestTheEvidenceBlockStillResolves:
    def test_every_cited_range_still_contains_what_it_claims(self) -> None:
        ...
        assert (
            not broken
        ), "cache_hit_pct cites code that moved or was renamed:\n" + "\n".join(broken)
    ...
    def test_the_docstring_really_carries_every_tabled_citation(self) -> None:
        ...
        doc = _doc()
        missing = [
            spelling for *_, spelling in _CITATIONS if f"``{spelling}``" not in doc
        ]
    ...
    def test_the_citation_set_is_exactly_the_tabled_one(self) -> None:
        ...
        assert (
            found
        ), "the citation pattern matched nothing — the regex, not the docstring, broke"
