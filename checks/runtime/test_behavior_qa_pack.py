...
FAKE_AWS_KEY = "AKIA" + "QA0000FAKE000TEST"
FAKE_SLACK_TOKEN = "xoxb-" + "0000000000-FAKEQATOKEN"
...
class TestWorkerNeverCertifiesOwnWork:
    ...
    def test_worker_finding_has_no_completion_authority(self):
        from gideon.automation.loop import watchdog
...
class TestFailureRecoveryBehavior:
    def test_repeated_failure_is_bounded(self):
        ...
        for _ in range(BLOCK_THRESHOLD):
            b.record(key, True)
        assert b.count(key) >= BLOCK_THRESHOLD

    def test_success_clears_failure_streak(self):
        from gideon.security.guardrails.loop_breaker import LoopBreaker, params_key
