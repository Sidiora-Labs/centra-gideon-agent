...
subprocess calls in :mod:`gideon.service.linux` and
:mod:`gideon.service.macos` are mocked.
"""
...
from gideon.service.common import LAUNCHD_LABEL, SERVICE_NAME, Platform, current_platform
...
class TestLinuxUnitRendering:
    ...
    def test_install_writes_unit_via_sudo_install_and_invokes_systemctl(
        ...
        # 1) `sudo install -m 0644 -o root -g root <tmp> /etc/systemd/system/gideon.service`
        # 2) `sudo systemctl daemon-reload`
        # 3) `sudo systemctl enable gideon.service`
        # 4) `sudo systemctl restart gideon.service`
    ...
    def test_uninstall_is_idempotent_when_unit_missing(self, tmp_path, monkeypatch):
        ...
        with patch("gideon.service.linux.subprocess.run") as run:
            svc_linux.uninstall()
...
class TestLinuxControlPaths:
    ...
    def test_uninstall_runs_full_teardown_when_unit_exists(self, tmp_path, monkeypatch):
        ...
        unit_path = tmp_path / "gideon.service"
    ...
    def test_current_group_falls_back_to_username_when_id_fails(self, monkeypatch):
        ...
        with patch(
            "gideon.service.linux.subprocess.run",
...
class TestMacOSControlPaths:
    ...
    def test_uninstall_idempotent_when_plist_missing(self, tmp_path, monkeypatch):
        ...
        with patch("gideon.service.macos.subprocess.run") as run:
            svc_macos.uninstall()
        run.assert_not_called()
    ...
    def test_is_active_returns_false_when_launchctl_errors(self):
        ...
        with patch("gideon.service.macos.subprocess.run", return_value=not_loaded):
    ...
    def test_is_active_returns_true_with_pid_in_output(self):
        ...
        with patch("gideon.service.macos.subprocess.run", return_value=loaded):
    ...
    def test_gideon_bin_falls_back_to_argv0(self, monkeypatch):
        ...
        from gideon.service import common as svc_common

        monkeypatch.setattr(sys, "argv", ["/some/path/gideon"])
        with patch("gideon.service.common.shutil.which", return_value=None):
