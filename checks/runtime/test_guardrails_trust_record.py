...
from gideon.security.guardrails import trust_record as tr
...
def test_demotion_marks_record_revoked_with_cause():
    ...
    assert record.rung == au.RUNG_DRAFT_ONLY
...
def test_revoked_record_clamps_even_when_store_diverges():
    ...
    tr.record_demotion(
        KEY, floor=au.RUNG_DRAFT_ONLY, cause="harmful verdict", at="2026-01-01"
    )
    assert au.granted_rung(KEY) == au.RUNG_AUTO_WITH_UNDO
    assert au.resolve_rung(KEY) == au.RUNG_DRAFT_ONLY
...
def test_fresh_grant_clears_revocation():
    au.grant_rung(KEY, au.RUNG_ONE_TAP)
    tr.record_demotion(
        KEY, floor=au.RUNG_DRAFT_ONLY, cause="failed study", at="2026-01-01"
    )
...
def test_unknown_rung_is_refused_on_write():
    tr.record_grant(KEY, "unattended", granted_at="2026-01-01")
