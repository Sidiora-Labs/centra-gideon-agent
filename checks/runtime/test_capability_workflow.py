...
def detail(root, value):
    return source(
        root, "spec/capability-expansion/tasks/sample.01.json", json.dumps(value)
    )
...
def test_owned_files_include_production_and_tests_once(tmp_path):
    root = project(tmp_path)
    prod = source(
        root, "runtime/gideon/workspace/capabilities/sample/store.py", "value=1\n"
    )
    test = source(
        root,
        "checks/runtime/capabilities/sample/test_store.py",
        "def test_value():\n    assert 1 == 1\n",
    )
    ui = source(
        root,
        "apps/console/src/features/capabilities/sample/Page.tsx",
        "export const value = 1\n",
    )
    uitest = source(
        root,
        "apps/console/src/features/capabilities/sample/Page.test.tsx",
        "expect(1).toBe(1)\n",
    )
    handler = source(
        root,
        "runtime/gideon/interfaces/dashboard/handlers/capabilities_sample.py",
        "value=2\n",
    )
...
@pytest.mark.parametrize(
    "path", ["/etc/passwd", "../outside.py", "runtime/../../escape.py"]
)
...
def test_ratio_requires_nonempty_production_and_sufficient_tests(tmp_path):
    ...
    source(
        root,
        "runtime/gideon/workspace/capabilities/sample/store.py",
        "value=1\nother=2\n",
    )
...
def test_extra_native_provider_files_participate_in_ratio(tmp_path):
    root = project(tmp_path)
    provider = source(
        root, "runtime/gideon/extensions/apps/native/sample/provider.py", "value=1\n"
    )
    tests = source(root, "checks/runtime/test_extra.py", "assert 1==1\n")
    detail(
        root,
        {
            "production_files": [str(provider.relative_to(root))],
            "test_files": [str(tests.relative_to(root))],
        },
    )
    result = workflow.inspect_task(root, "sample.01")
...
def test_real_command_records_output_and_exit_code(tmp_path):
    ...
    code = workflow.run_command(
        [sys.executable, "-c", 'import sys; print("actual output"); sys.exit(7)'],
        tmp_path,
        os.environ.copy(),
        log,
    )
...
def test_cli_ready_obeys_dependencies(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, str(SOURCE), "ready", "--root", str(root)],
        capture_output=True,
        text=True,
    )
...
def test_cli_missing_identifier_and_malformed_backlog_fail(tmp_path):
    root = project(tmp_path)
    result = subprocess.run(
        [sys.executable, str(SOURCE), "verify", "--root", str(root)],
        capture_output=True,
        text=True,
    )
...
def test_explicit_task_lists_exclude_prior_lane_files(tmp_path):
    ...
    current = source(
        root, "runtime/gideon/workspace/capabilities/sample/new.py", "new=2\n"
    )
    test = source(
        root, "checks/runtime/capabilities/sample/test_new.py", "assert 2==2\n"
    )
    detail(
        root,
        {
            "production_files": [str(current.relative_to(root))],
            "test_files": [str(test.relative_to(root))],
        },
    )
    result = workflow.inspect_task(root, "sample.01")
...
def test_baseline_counts_only_changed_executable_lines(tmp_path):
    ...
    with pytest.raises(ValueError, match="does not exist"):
        workflow.changed_lines(
            root, current, {"baseline_dir": str(tmp_path / "absent")}
        )
...
def test_focused_gate_does_not_run_unchanged_prior_tests(tmp_path):
    root = project(tmp_path)
    prod = source(
        root, "runtime/gideon/workspace/capabilities/sample/store.py", "value=6\n"
    )
    source(
        root,
        "checks/runtime/capabilities/sample/test_old.py",
        'raise RuntimeError("must not collect")\n',
    )
    test = source(
        root,
        "checks/runtime/capabilities/sample/test_new.py",
        'import os\nimport sys\ndef test_interpreter():\n    assert os.environ["GIDEON_TEST_PYTHON"]==sys.executable\n',
    )
    detail(
        root,
        {
            "production_files": [str(prod.relative_to(root))],
            "test_files": [str(test.relative_to(root))],
...
def test_compile_stage_typechecks_declared_dependency_closure_without_whole_console(
    tmp_path,
):
    root = project(tmp_path)
    repository = SOURCE.parents[1]
    (root / "node_modules").symlink_to(
        repository / "node_modules", target_is_directory=True
    )
    source(
        root,
        "apps/console/tsconfig.json",
        (repository / "apps/console/tsconfig.json").read_text(),
    )
    ambient = source(root, "apps/console/src/vite-env.d.ts", "declare module '*.css'\n")
    source(root, "apps/console/src/feature.css", "")
    feature = source(
        root,
        "apps/console/src/feature.ts",
        "import './feature.css'\nimport type { CSSProperties } from 'react'\nimport { typed } from './dependency'\nexport const style: CSSProperties = {color:'red'}\nexport const result: string = typed\n",
    )
    dependency = source(
        root, "apps/console/src/dependency.ts", "export const typed = 7\n"
    )
    test = source(
        root,
        "apps/console/src/feature.test.ts",
        "import { result } from './feature'\nconst actual: string = result\nvoid actual\n",
    )
    source(
        root, "apps/console/src/unrelated.ts", "const broken: string = 7\nvoid broken\n"
    )
    runtime = source(
        root, "checks/runtime/test_feature.py", "def test_feature():\n    assert True\n"
    )
    detail(
        root,
        {
            "production_files": [str(feature.relative_to(root))],
    ...
    assert [row["name"] for row in evidence["commands"]] == [
        "compile",
        "compile_typescript",
    ]
    ...
    config = json.loads(
        (Path(evidence["evidence_dir"]) / "tsconfig.capability.json").read_text()
    )
...
def test_compile_stage_skips_historical_typescript_test_for_python_only_task(tmp_path):
    ...
    runtime = source(
        root, "checks/runtime/test_closure.py", "def test_value():\n    assert True\n"
    )
    historical = source(
        root,
        "apps/console/src/historical.test.tsx",
    ...
    detail(
        root,
        {
            "production_files": [str(production.relative_to(root))],
            "test_files": [
...
def test_compile_stage_rejects_new_typescript_test_error_without_ts_production(
    tmp_path,
):
    root = project(tmp_path)
    repository = SOURCE.parents[1]
    ...
    log = Path(evidence["commands"][-1]["log"]).read_text()
    assert (
        "new.test.ts(2,7): error TS2559" in log and "Type '7' has no properties" in log
    )
    typed.write_text(
...
def test_ui_config_uses_exact_real_repository_file(tmp_path):
    ...
    assert workflow.ui_config_args(
        root, {"ui_config": "apps/console/vitest.feature.ts"}
    ) == ["--config", str(config)]
    assert config.read_text() == "export default {}\n"


@pytest.mark.parametrize(
    "value", ["../outside.ts", "/tmp/outside.ts", "missing.ts", {}, 4]
)
