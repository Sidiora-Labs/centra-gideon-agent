...
def tmp_config(tmp_path):
    with patch("gideon.cognition.context_management.config_dir", return_value=tmp_path):
        yield tmp_path


def test_cap_result_file_no_truncation(tmp_path):
    from gideon.cognition.context_management import cap_result_file
...
def test_cap_result_file_truncates(tmp_path):
    ...
    assert p.stat().st_size <= RESULT_FILE_MAX_BYTES + 200
    content = p.read_text()
    assert "truncated" in content


def test_cap_streaming_text_short():
    from gideon.cognition.context_management import cap_streaming_text
...
def test_cap_history():
    from gideon.cognition.context_management import HISTORY_MAX_ENTRIES, cap_history
...
def test_check_session_budget_under(tmp_path):
    from gideon.cognition.context_management import check_session_budget
...
def test_evict_completed_agents():
    from gideon.cognition.context_management import evict_completed_agents
