...
def datasets(tmp_path):
    return DatasetStore(
        ...
        ImageService(NativeArtifactProvider(tmp_path / "artifacts")),
    )
...
def body(datasets, request="first", caption="A red image"):
    ...
    artifact = datasets.images.artifacts.create_binary(
        name="Source", data=output.getvalue(), mime="image/png"
    )
    return dict(
        title="Dataset",
        base_model="owner/model",
        request_id=request,
        entries=[dict(artifact_id=artifact.slug, version=1, caption=caption)],
    )
...
def test_dataset_save_replay_revisions_history_and_reopen(datasets):
    ...
    second_body["entries"] = [
        dict(first_body["entries"][0], caption="New caption\nwith detail")
    ]
...
def test_real_zip_export_uses_pinned_pixels_captions_and_no_host_paths(datasets):
    ...
    with zipfile.ZipFile(io.BytesIO(archive_bytes)) as archive:
        ...
        assert metadata == {
            "file_name": "00000.png",
            "text": payload["entries"][0]["caption"],
        }
    ...
    datasets.save(
        dict(
            ...
            request_id="edit",
            revision=1,
            entries=[dict(payload["entries"][0], caption="Different")],
        ),
        saved["id"],
    )
    with zipfile.ZipFile(io.BytesIO(datasets.export(saved["id"], 1))) as archive:
        assert (
            json.loads(archive.read("metadata.jsonl"))["text"]
            == payload["entries"][0]["caption"]
        )
...
async def test_http_dataset_export_history_and_native_tool_same_store(datasets):
    sketches = SketchStore(
        datasets.path.parent / "sketches.sqlite3", datasets.images.artifacts
    )
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.get(
            "/api/capabilities/media/datasets/" + saved["id"] + "/history"
        )
        assert (await response.json())["items"] == [saved]
        response = await client.get(
            "/api/capabilities/media/datasets/" + saved["id"] + "/revisions/1/export"
        )
        ...
        result = await provider.invoke(
            "media_datasets_get", {"dataset_id": saved["id"], "revision": 1}
        )
        ...
        result = await provider.invoke(
            "media_datasets_get", {"dataset_id": saved["id"], "home": "other"}
        )
