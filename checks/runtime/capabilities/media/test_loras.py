...
def tensors(root, name="adapter.safetensors", base="base-model"):
    ...
    header = {
        "__metadata__": {"base_model": base, "trigger_words": "red bird"},
        "layer.lora_A.weight": {
            "dtype": "F32",
            "shape": [1, 2],
            "data_offsets": [0, 8],
...
def test_actual_staged_adapter_is_digest_pinned_and_original_unchanged(
    catalog, tmp_path
):
    path, data, _ = tensors(catalog.root)
    item = catalog.get(path.name, "base-model")
    destination = tmp_path / "staged.safetensors"
    staged = catalog.stage(selection(item, -0.5), "base-model", destination)
    assert staged["path"] == str(destination)
    assert staged["sha256"] == item["sha256"]
    assert staged["scale"] == -0.5
...
def test_empty_absent_and_directory_files_are_distinct(catalog):
    ...
    result = catalog.list()
    assert result["items"] == []
    assert {row["id"] for row in result["invalid"]} == {
...
def test_structurally_invalid_tensor_files_are_reported_without_execution(
    catalog, change
):
    ...
    if change == "dtype":
        ...
        path.write_bytes(
            ...
            if change == "truncated"
                ...
                data + b"X"
                if change == "trailing"
                else struct.pack("<Q", 2**60) + b"{}"
            )
        )
    if change in ("dtype", "offset", "shape", "overlap", "layout", "metadata"):
        encoded = json.dumps(header).encode()
        path.write_bytes(
            struct.pack("<Q", len(encoded)) + encoded + struct.pack("<ffff", 1, 2, 3, 4)
        )
    result = catalog.list()
    assert result["items"] == []
...
def test_duplicate_header_keys_are_not_silently_accepted(catalog):
    ...
    (catalog.root / "duplicates.safetensors").write_bytes(
...
def test_model_lora_support_is_opt_in_and_request_values_are_bounded(catalog):
    ...
    service = ImageService(
        NativeArtifactProvider(catalog.root.parents[1] / "artifacts"),
        selector=lambda: (provider, "model"),
    )
    ...
    with pytest.raises(SketchError):
        service.prepare(
            {"prompt": "A bird", "loras": [dict(selection(item), sha256="wrong")]}
        )
    with pytest.raises(SketchError):
        service.prepare(
            {"prompt": "A bird", "loras": [selection(item), selection(item)]}
        )
...
async def test_actual_http_tool_catalog_shares_scoped_files_and_rejects_scope(catalog):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        result = await provider.invoke(
            "media_loras_list", {"base_model": "caller-selected"}
        )
