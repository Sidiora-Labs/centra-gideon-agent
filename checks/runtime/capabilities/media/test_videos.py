...
def clip(tmp_path, color='red'):
    ...
    subprocess.run(['ffmpeg', '-v', 'error', '-y', '-f', 'lavfi', '-i', f'color=c={color}:s=32x24:r=4:d=1', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', str(path)], check=True, timeout=20)
    return path
...
def image(service, color=(3, 20, 70, 190)):
    ...
    artifact = service.artifacts.create_binary(name='Frame', data=buffer.getvalue(), mime='image/png')
...
def jobs_for(service, tmp_path):
    return MediaJobs(tmp_path / 'jobs.sqlite3', SketchStore(tmp_path / 'sketches.sqlite3', service.artifacts), videos=service)
...
def test_first_and_last_frames_pin_real_canonical_pixels(service, tmp_path):
    ...
    body = request(service, first_frame_artifact_id=first.slug, first_frame_version=1, last_frame_artifact_id=last.slug, last_frame_version=1)
...
def test_continuation_extracts_actual_last_frame_without_rewriting_video(service, tmp_path):
    path = clip(tmp_path)
    raw = path.read_bytes()
    source = service.artifacts.create_binary(name='Clip', data=raw, mime='video/mp4', kind='video')
    body = request(service, continuation_artifact_id=source.slug, continuation_version=1)
...
def test_materialization_decodes_real_video_and_retains_exact_provenance(service, tmp_path):
    path = clip(tmp_path)
    prepared = request(service)
    result = service.materialize(VideoResult(local_path=str(path)), prepared, 'video-output')
    ...
    assert metadata['video_request_sha256'] == hashlib.sha256(json.dumps(prepared, sort_keys=True).encode()).hexdigest()
    assert service.materialize(VideoResult(local_path=str(path)), prepared, 'video-output') == result
    assert len(service.artifacts.list()) == 1
    with pytest.raises(SketchError, match='identity conflicts'):
        service.materialize(VideoResult(local_path=str(path)), dict(prepared, prompt='Changed'), 'video-output')


def test_continuation_output_references_immutable_source(service, tmp_path):
    path = clip(tmp_path)
    source = service.artifacts.create_binary(name='Source', data=path.read_bytes(), mime='video/mp4', kind='video')
    prepared = request(service, continuation_artifact_id=source.slug, continuation_version=1)
    result = service.materialize(VideoResult(local_path=str(path)), prepared, 'continued')
...
@pytest.mark.parametrize('patch', [
    {'prompt': ''}, {'prompt': 4}, {'prompt': 'a'*4001}, {'duration_seconds': True},
    {'duration_seconds': 0}, {'duration_seconds': 61}, {'duration_seconds': float('nan')},
    {'duration_seconds': float('inf')}, {'aspect_ratio': []}, {'home': '/tmp/other'},
    {'controls': {'seed': True}}, {'controls': {'seed': 1.5}}, {'controls': {'motion': -1}},
    {'controls': {'guidance': float('nan')}}, {'controls': {'unknown': 1}},
    {'first_frame_artifact_id': 'missing'}, {'continuation_version': 1},
    {'last_frame_artifact_id': {}, 'last_frame_version': 1}, {'controls': []},
])
...
def test_model_capabilities_reject_unadvertised_controls_and_conditioning(service):
    ...
    for changes in ({'duration_seconds': 10}, {'aspect_ratio': '1:1'}, {'controls': {'seed': 1}}, {'first_frame_artifact_id': 'a'}, {'last_frame_artifact_id': 'a'}, {'continuation_artifact_id': 'a'}):
        with pytest.raises(SketchError):
            service.validate_model(dict(body, **changes), legacy)
    model = VideoGenModel('supported', max_duration_s=8, durations=[4, 8], supported_controls={'seed': VideoControl(0, 10, True)})
...
def test_bad_output_bytes_and_container_declarations_never_publish(service, tmp_path):
    ...
    with pytest.raises(SketchError):
        probe_video(path)
    with pytest.raises(SketchError):
        service.materialize(VideoResult(local_path=str(path)), request(service), 'broken')
    valid = clip(tmp_path)
    with pytest.raises(SketchError):
        service.materialize(VideoResult(local_path=str(valid), mime='video/webm'), request(service), 'wrong-mime')
    assert service.artifacts.list() == []


def test_unconfigured_real_worker_records_failure_and_keeps_retry_history(service, tmp_path):
    jobs = jobs_for(service, tmp_path)
    body = {'operation': 'video_generate', 'request_id': 'absent', 'input': {'prompt': 'scene', 'duration_seconds': 4}}
...
async def test_http_and_native_tools_share_real_queue(service, tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        queued = await client.post('/api/capabilities/media/videos', json={'operation': 'video_generate', 'request_id': 'http', 'input': {'prompt': 'scene', 'duration_seconds': 4}})
        ...
        submitted = await tool.invoke('media_video_submit', {'request_id': 'tool', 'input': {'prompt': 'tool scene', 'duration_seconds': 4}})
        ...
        rejected = await tool.invoke('media_video_submit', {'request_id': 'bad', 'input': {'prompt': 'bad', 'duration_seconds': 4, 'provider': 'other'}})
