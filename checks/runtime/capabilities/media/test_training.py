...
def training_input(jobs):
    ...
    artifact = jobs.sketches.artifacts.create_binary(
        name="Training source", data=output.getvalue(), mime="image/png"
    )
    dataset = jobs.datasets.save(
        {
            "title": "Training set",
            "base_model": "owner/model",
            "request_id": str(uuid4()),
            "entries": [
                {"artifact_id": artifact.slug, "version": 1, "caption": "Blue square"}
            ],
        }
    )
    return dict(
...
def test_real_missing_trainer_is_explicit_and_durable_worker_never_succeeds(jobs):
    ...
    job = jobs.submit(
        {"operation": "lora_train", "request_id": "train", "input": request}
    )
...
def test_operator_admission_is_checked_on_both_submit_and_existing_job_execution(jobs):
    request = training_input(jobs)
    queued = jobs.submit(
        {"operation": "lora_train", "request_id": "before-revoke", "input": request}
    )
    ...
    with pytest.raises(SketchError) as denied:
        restricted.submit(
            {"operation": "lora_train", "request_id": "after-revoke", "input": request}
        )
    assert denied.value.status == 503
    assert len(jobs.list()["items"]) == 1
    MediaWorker(restricted).run_once(
        WorkerContext("gideon-media", "default", WorkerControl())
    )
        ...
        jobs.submit(
            {
                ...
                "request_id": "invalid",
                "input": dict(request, **{key: value}),
            }
        )
    assert jobs.list()["items"] == []


def test_documented_diffusers_command_has_owned_paths_and_preserves_resume(
    jobs, tmp_path
):
    ...
    assert "--resume_from_checkpoint=latest" in jobs.trainer.command(
        request, data, output
    )
...
def test_real_child_process_runner_captures_exit_and_owned_identity(jobs, tmp_path):
    request = training_input(jobs)
    job = jobs.submit(
        {"operation": "lora_train", "request_id": "child-lifecycle", "input": request}
    )
    jobs.claim()
    log = tmp_path / "actual-child.log"
    run_process(
        [sys.executable, "-c", 'print("actual child completed")'],
        log,
        lambda: jobs.get(job["id"])["status"] == "cancel_requested",
        lambda pid: jobs.attach_child(job["id"], pid),
    )
...
def test_real_child_cancellation_terminates_group_and_retains_diagnostics(
    jobs, tmp_path
):
    request = training_input(jobs)
    job = jobs.submit(
        {"operation": "lora_train", "request_id": "cancel-child", "input": request}
    )
    running = jobs.claim()
...
def test_structural_adapter_publication_preserves_payload_and_adds_provenance(
    jobs, tmp_path
):
    job_id = str(uuid4())
    output = jobs.trainer.output(job_id)
    ...
    header = {
        "lora_A.weight": {"dtype": "F32", "shape": [1, 1], "data_offsets": [0, 4]},
        "lora_B.weight": {"dtype": "F32", "shape": [1, 1], "data_offsets": [4, 8]},
    }
