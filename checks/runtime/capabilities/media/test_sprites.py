...
def queued(jobs, body, request_id="compile"):
    return jobs.submit(
        dict(operation="sprite_compile", request_id=request_id, input=body)
    )
...
def test_real_worker_compiles_trimmed_alpha_atlas_and_exact_json_layout(jobs):
    ...
    assert (
        metadata["sprite_request_sha256"]
    ...
    assert (
        jobs.sprites.compile(body, job["id"], lambda: False, lambda value: None)
        == result
    )
...
def test_untrimmed_frame_preserves_full_geometry_and_transparency(jobs):
    ...
    body = request(
        [item], cell_width=8, cell_height=8, columns=1, padding=0, trim=False
    )
    ...
    with Image.open(
        io.BytesIO(jobs.sketches.artifacts.raw_bytes(result["artifact_id"])[0])
    ) as image:
...
def test_fully_transparent_frame_remains_transparent_and_has_valid_rect(jobs):
    ...
    result = jobs.sprites.compile(
        request([item]), "blank", lambda: False, lambda value: None
    )
    manifest = jobs.sprites.manifest(result)
...
def test_duplicate_names_and_mismatched_output_identity_are_rejected(jobs):
    ...
    with pytest.raises(SketchError, match="identity conflict"):
        jobs.sprites.compile(
            dict(first, fps=9), "same", lambda: False, lambda value: None
        )
...
def test_cancellation_before_compile_leaves_original_and_no_output(jobs):
    ...
    with pytest.raises(SketchError, match="cancelled"):
        jobs.sprites.compile(
            request([item]), "cancel", lambda: True, lambda value: None
        )
    assert len(jobs.sketches.artifacts.list()) == 1
    assert jobs.sketches.artifacts.raw_bytes(item["artifact_id"])[0] == raw
    ...
    body = dict(
        ...
        frames=[
            dict(
                ...
                animation="idle",
                direction="down",
                prompt="Pixel character facing down",
            )
        ],
    )
    queued_job = jobs.submit(
        dict(operation="sprite_generate", request_id="generate", input=body)
    )
    assert (
        queued_job["input"]["frames"][0]["input"]["prompt"]
        == body["frames"][0]["prompt"]
    )
...
async def test_real_http_inspection_native_compile_and_pinned_manifest(jobs):
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post(
            "/api/capabilities/media/sprites/inspect",
            json=dict(artifact_id=item["artifact_id"], version=1),
        )
        ...
        result = await tool.invoke(
            "media_sprite_compile", {"request_id": "native", "input": request([item])}
        )
        ...
        assert (await atlas.json())["atlas"]["artifact_id"] == finished["result"][
            "artifact_id"
        ]
        incorrect = await client.get(
            "/api/capabilities/media/jobs/" + job["id"] + "/frames"
        )
        assert incorrect.status == 400
        denied = await client.post(
            "/api/capabilities/media/sprites/inspect?home=other",
            json=dict(artifact_id=item["artifact_id"], version=1),
        )
        assert denied.status == 400
        native = await tool.invoke(
            "media_sprite_inspect", {"artifact_id": item["artifact_id"], "version": 1}
        )
