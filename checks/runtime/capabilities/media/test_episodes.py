...
def clip(jobs, tmp_path, color):
    ...
    subprocess.run(['ffmpeg', '-v', 'error', '-y', '-f', 'lavfi', '-i', f'color=c={color}:s=32x24:r=4:d=1', '-c:v', 'libx264', '-threads', '1', '-pix_fmt', 'yuv420p', str(path)], check=True, timeout=20)
    artifact = jobs.sketches.artifacts.create_binary(name=color, data=path.read_bytes(), mime='video/mp4', kind='video')
    return artifact


def plan(jobs, tmp_path):
    artifacts = [clip(jobs, tmp_path, color) for color in ('red', 'blue')]
    return dict(title='Episode', width=32, height=24, fps=4, aspect_ratio='16:9', scenes=[dict(prompt='Scene '+str(index), duration_seconds=1, mode='reuse', allow_fallback=False, artifact_id=artifact.slug, version=1) for index, artifact in enumerate(artifacts)], request_id='create')


def generated_plan():
    return dict(title='Generated', width=32, height=24, fps=4, aspect_ratio='16:9', scenes=[dict(prompt='Establish', duration_seconds=4, mode='establish', allow_fallback=False)], request_id='generate')


def queue(jobs, document, request_id='render'):
    return jobs.submit(dict(operation='episode_render', request_id=request_id, input=dict(episode_id=document['id'], revision=document['revision'])))
...
def test_episode_revisions_preserve_script_order_and_source_pins(jobs, tmp_path):
    ...
    second = jobs.episodes.save(dict(body, title='Revised', request_id='revision', revision=1), first['id'])
...
def test_real_worker_assembles_reused_clips_with_lineage_and_provenance(jobs, tmp_path):
    body = plan(jobs, tmp_path)
    originals = {scene['artifact_id']: jobs.sketches.artifacts.raw_bytes(scene['artifact_id'])[0] for scene in body['scenes']}
    ...
    assert abs(probe_video(path)['duration_seconds']-2) < .1
    ...
    assert scenes[1]['predecessor'] == dict(artifact_id=body['scenes'][0]['artifact_id'], version=1)
    ...
    assert [event['status'] for event in scenes[0]['events']] == ['running', 'succeeded']
    assert scenes[0].get('fallback', False) is False
    assert jobs.timelines.get(job['result']['timeline_id'])['segments'][1]['artifact_id'] == body['scenes'][1]['artifact_id']
...
async def test_real_checkpoint_resume_keeps_completed_scene_attempt(jobs, tmp_path):
    ...
    with pytest.raises(SketchError, match='cancelled'):
        await jobs.episodes.execute(request, 'resume', lambda: len(jobs.episodes.scenes('resume')['items']) == 1, lambda pid: None)
    ...
    reopened = EpisodeStore(jobs.episodes.path, jobs.videos, jobs.timelines)
    pids, progress = [], []
    result = await reopened.execute(request, 'resume', lambda: False, pids.append, progress.append)
...
@pytest.mark.parametrize('patch', [
    {'title': ''}, {'width': 3}, {'height': 2048}, {'fps': True}, {'fps': 61},
    {'aspect_ratio': []}, {'scenes': []}, {'scenes': {}}, {'provider': 'other'},
    {'request_id': []}, {'revision': True},
])
...
@pytest.mark.parametrize('patch', [
    {'prompt': ''}, {'prompt': 'a'*4001}, {'duration_seconds': float('nan')},
    {'duration_seconds': True}, {'duration_seconds': 0}, {'duration_seconds': 61},
    {'mode': 'continue'}, {'mode': 'invalid'}, {'allow_fallback': 'yes'},
    {'artifact_id': 'override'}, {'path': '/private/file'}, {'mode': 'reuse'},
])
...
def test_partial_reuse_cannot_claim_continuity_from_wrong_final_frame(jobs, tmp_path):
    body = plan(jobs, tmp_path)
    body['scenes'][0]['duration_seconds'] = .5
    body['scenes'][1] = dict(prompt='Continue', duration_seconds=4, mode='continue', allow_fallback=False)
...
def test_scene_events_preserve_explicit_fallback_and_attempt_history(jobs):
    first = jobs.episodes.record('history', 0, 'running', mode='continue', predecessor={'artifact_id': 'source', 'version': 1})
    assert first['attempts'] == 1
    fallback = jobs.episodes.record('history', 0, 'fallback', fallback=True, detail='Policy permits fresh establishment')
    ...
    assert [row['status'] for row in retried['events']] == ['running', 'fallback', 'failed', 'running']
...
@pytest.mark.asyncio
async def test_http_and_native_consumers_share_plans_jobs_and_scene_ledger(jobs, tmp_path):
    app = web.Application()
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        saved = await tool.invoke('media_episodes_save', dict(body, episode_id=document['id'], revision=1, request_id='tool-save', title='Updated'))
        assert saved.success
        assert json.loads(saved.output)['revision'] == 2
        history = await client.get('/api/capabilities/media/episodes/'+document['id']+'/history')
        assert [item['revision'] for item in (await history.json())['items']] == [2, 1]
        queued = await tool.invoke('media_episode_render', {'request_id': 'tool-render', 'input': {'episode_id': document['id'], 'revision': 1}})
        assert queued.success
        job = json.loads(queued.output)
        scenes = await client.get('/api/capabilities/media/jobs/'+job['id']+'/scenes')
        ...
        malformed = await tool.invoke('media_episodes_save', dict(body, provider='other'))
        ...
        stale = await client.put('/api/capabilities/media/episodes/'+document['id'], json=dict(body, revision=1, request_id='stale'))
