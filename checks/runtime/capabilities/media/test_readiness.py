...
from gideon.workspace.capabilities.media.readiness import (
    MediaReadiness,
    describe_model,
    probe,
)
...
async def test_actual_openai_adapter_without_credential_is_unavailable():
    provider = OpenAIImageProvider(
        provider_name="media-no-key", endpoint="http://127.0.0.1:1", api_key=""
    )
...
async def test_actual_adapter_catalog_success_is_not_credential_or_generation_proof():
    ...
    register_media_catalog(
        ...
        MediaCatalog(
            models=(
                MediaModel(
                    "local-catalog-model",
                    extra={"sizes": ["512x512"], "supports_edit": True},
                ),
            )
        ),
    )
    try:
        provider = OpenAIImageProvider(
            provider_name="media-catalog",
            provider_type=kind,
            endpoint="http://127.0.0.1:1",
            api_key="not-a-valid-network-credential",
        )
...
def test_real_image_model_metadata_distinguishes_download_and_management():
    ...
    model = ImageGenModel(
        name="image:v2", downloaded=False, sizes=["256x256"], supports_edit=False
    )
...
async def test_real_http_cached_observation_rejects_caller_scope(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            "/api/capabilities/media/readiness",
            data="{",
...
async def test_native_readiness_tool_reads_same_observation_without_overrides(tmp_path):
    ...
    assert (
        definitions["media_readiness_refresh"].parameters["additionalProperties"]
        is False
    )
...
"""
    result = subprocess.run(
        ...
        env=dict(os.environ, GIDEON_HOME=str(tmp_path)),
        capture_output=True,
        text=True,
        timeout=20,
    )
    ...
    assert {row["capability"] for row in observation["items"]} == {
        "image_gen",
        "video_gen",
    }
