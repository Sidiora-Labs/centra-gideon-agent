...
def source(jobs, image=None):
    ...
    artifact = jobs.sketches.artifacts.create_binary(name='Original image', data=output.getvalue(), mime='image/png')
    return artifact.slug, output.getvalue()


def request(artifact_id, operations):
    return {'source_artifact_id': artifact_id, 'source_version': 1, 'operations': operations}
...
def test_ordered_cleanup_job_pixels_and_canonical_provenance_preserve_original(jobs):
    ...
    operations = [{'op': 'crop', 'x': 2, 'y': 3, 'width': 4, 'height': 5}, {'op': 'flip', 'axis': 'horizontal'}]
    body = {'operation': 'image_cleanup', 'request_id': 'cleanup', 'input': request(artifact_id, operations)}
    ...
    metadata = jobs.sketches.artifacts.get(finished['result']['artifact_id']).events[0].metadata
    ...
    assert metadata['cleanup_request_sha256'] == hashlib.sha256(json.dumps(queued['input'], sort_keys=True).encode()).hexdigest()
...
def test_solid_edge_background_preserves_enclosed_matching_color_and_alpha(jobs):
    ...
    artifact_id, original = source(jobs, image)
    prepared = jobs.cleanup.prepare(request(artifact_id, [{'op': 'solid_background', 'color': '#ffffff', 'tolerance': 0}]))
...
def test_resize_is_real_lanczos_and_rotation_is_counterclockwise(jobs):
    ...
    prepared = jobs.cleanup.prepare(request(artifact_id, [{'op': 'resize', 'width': 4, 'height': 5}]))
    ...
    prepared = jobs.cleanup.prepare(request(rotated_id, [{'op': 'rotate', 'degrees': 90}]))
...
def test_brightness_and_contrast_preserve_source_alpha(jobs):
    artifact_id, _ = source(jobs)
    prepared = jobs.cleanup.prepare(request(artifact_id, [{'op': 'brightness', 'factor': .5}]))
    rendered = output_image(jobs, jobs.cleanup.execute(prepared, 'brightness'))
    assert rendered.getpixel((2, 2)) == (40, 50, 60, 170)
    prepared = jobs.cleanup.prepare(request(artifact_id, [{'op': 'contrast', 'factor': 0}]))
...
def test_sharpen_changes_real_edge_pixels_without_corrupting_transparency(jobs):
    ...
    prepared = jobs.cleanup.prepare(request(artifact_id, [{'op': 'sharpen', 'radius': 1, 'percent': 150, 'threshold': 0}]))
...
def test_exif_orientation_is_applied_to_cleanup_coordinates(jobs):
    ...
    artifact = jobs.sketches.artifacts.create_binary(name='Camera original', data=data.getvalue(), mime='image/jpeg')
    prepared = jobs.cleanup.prepare(request(artifact.slug, [{'op': 'crop', 'x': 4, 'y': 0, 'width': 2, 'height': 4}]))
...
@pytest.mark.parametrize('operation', [
    {'op': 'crop', 'x': 7, 'y': 0, 'width': 2, 'height': 1}, {'op': 'resize', 'width': 4097, 'height': 1},
    {'op': 'resize', 'width': True, 'height': 1}, {'op': 'rotate', 'degrees': 45}, {'op': 'flip', 'axis': 'diagonal'},
    {'op': 'brightness', 'factor': float('nan')}, {'op': 'contrast', 'factor': 4}, {'op': 'sharpen', 'radius': 9, 'percent': 10, 'threshold': 1},
    {'op': 'solid_background', 'color': 'white', 'tolerance': 0}, {'op': 'solid_background', 'color': '#ffffff', 'tolerance': 101},
    {'op': 'semantic_remove_person'}, {'op': 'resize', 'width': 4, 'height': 5, 'home': '/tmp'},
])
def test_invalid_or_unimplemented_transforms_never_queue(jobs, operation):
    artifact_id, _ = source(jobs)
    with pytest.raises(SketchError):
        jobs.submit({'operation': 'image_cleanup', 'request_id': 'invalid', 'input': request(artifact_id, [operation])})
...
def test_pipeline_bounds_are_checked_after_prior_transforms(jobs):
    ...
    with pytest.raises(SketchError, match='Crop'):
        jobs.cleanup.prepare(request(artifact_id, [{'op': 'resize', 'width': 2, 'height': 2}, {'op': 'crop', 'x': 0, 'y': 0, 'width': 3, 'height': 1}]))
    with pytest.raises(SketchError, match='megapixels'):
        jobs.cleanup.prepare(request(artifact_id, [{'op': 'resize', 'width': 4096, 'height': 4096}, {'op': 'solid_background', 'color': '#ffffff', 'tolerance': 0}]))
    for invalid in (None, '', [], {}):
        with pytest.raises(SketchError):
...
async def test_http_and_native_tool_queue_same_cleanup_contract(jobs):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post('/api/capabilities/media/jobs', json={'operation': 'image_cleanup', 'request_id': 'http', 'input': payload})
        ...
        result = await provider.invoke('media_cleanup_submit', {'request_id': 'tool', 'input': payload})
        assert result.success
        assert json.loads(result.output)['input'] == queued['input']
        result = await provider.invoke('media_cleanup_submit', {'request_id': 'escape', 'input': dict(payload, home='other')})
