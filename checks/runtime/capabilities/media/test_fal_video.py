...
def test_native_manifest_uses_real_registry_factory():
    ...
    assert (
        ...
        == "gideon.integrations.video_gen.fal_provider:create_provider"
    )
...
def test_real_fal_payload_keeps_exact_prompt_and_supported_duration():
    provider = FalVideoProvider()
    endpoint, body = provider.payload(
        "A scene", "fal-ai/veo3.1", 6, "9:16", {"seed": 42}
    )
...
def test_real_png_conditioning_becomes_documented_data_uri(tmp_path):
    ...
    continuation_endpoint, continuation = provider.payload(
        "Continue",
        "",
        8,
        "16:9",
        {"continuation_video": "internal-only", "continuation_frame": str(path)},
    )
