...
def provider(tmp_path):
    ...
    return MediaToolProvider(
        SketchStore(tmp_path / "sketches.sqlite3", artifacts), MediaLibrary(artifacts)
    )
...
async def test_manifest_and_discovery_are_executable(provider):
    ...
    manifest = json.loads(
        (
            root / "runtime/gideon/extensions/apps/native/gideon-media/app.json"
        ).read_text()
    )
    assert manifest["name"] == provider.name
    assert (
        manifest["provider"]["implementation"]
        == "gideon.workspace.capabilities.media.tools:create_provider"
    )
...
async def test_tool_sketch_draw_erase_undo_export_and_persistence(provider):
    sketch = await call(
        provider,
        "media_sketch_create",
        {"width": 20, "height": 20, "request_id": "tool"},
    )
    ...
    saved = await call(
        provider,
        "media_sketch_update",
        {"sketch_id": sketch["id"], "revision": 1, "strokes": [stroke]},
    )
    assert saved["revision"] == 2
    assert saved["strokes"] == [stroke]
    assert (
        await call(provider, "media_sketch_get", {"sketch_id": sketch["id"]}) == saved
    )
    assert (await call(provider, "media_sketch_list", {}))["items"] == [saved]
    exported = await call(
        provider, "media_sketch_export", {"sketch_id": sketch["id"], "revision": 2}
    )
    ...
    undone = await call(
        provider,
        "media_sketch_update",
        {"sketch_id": sketch["id"], "revision": 2, "strokes": []},
    )
...
async def test_tool_metadata_changes_existing_canonical_artifact(provider):
    sketch = await call(
        provider,
        "media_sketch_create",
        {"width": 4, "height": 4, "request_id": "library"},
    )
    exported = await call(
        provider, "media_sketch_export", {"sketch_id": sketch["id"], "revision": 1}
    )
    item = await call(
        provider, "media_library_get", {"artifact_id": exported["artifact_id"]}
    )
    updated = await call(
        provider,
    ...
    listed = await call(
        provider, "media_library_list", {"tag": "tool-edited", "limit": 1}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "media_sketch_update",
            {"sketch_id": "none", "revision": 1, "strokes": [{"tool": "bad"}]},
        ),
...
async def test_tools_unknown_missing_and_isolated_homes(provider, tmp_path):
    ...
    sketch = await call(
        provider, "media_sketch_create", {"width": 5, "height": 5, "request_id": "one"}
    )
    other_artifacts = NativeArtifactProvider(tmp_path / "other/artifacts")
    other = MediaToolProvider(
        SketchStore(tmp_path / "other/sketches.sqlite3", other_artifacts),
        MediaLibrary(other_artifacts),
    )
