...
def store(tmp_path):
    return AnnotationStore(
        tmp_path / "annotations.sqlite3", NativeArtifactProvider(tmp_path / "artifacts")
    )
...
def image(store):
    ...
    return store.artifacts.create_binary(
        ...
        mime="image/png",
        source="import",
        event_metadata={"original_filename": "source.png"},
    )
    ...
    return dict(
        ...
        annotations=(
            ...
            if entries is not None
                ...
                dict(
                    id="note", text="Original observation", region=[0.1, 0.2, 0.3, 0.4]
                )
            ]
        ),
        attribution=dict(
...
def test_save_edit_restore_preserves_all_revisions_and_import_attribution(store):
    ...
    edited = body(
        1, "second", [dict(id="note", text="Corrected\nmultiline\tobservation")]
    )
    ...
    assert (
        reopened.get(artifact.slug, 1, 2)["annotations"][0]["text"]
        == "Corrected\nmultiline\tobservation"
    )
    ...
    assert (
        reopened.history(artifact.slug, 1, offset=2, limit=2)["items"][0]["revision"]
        == 1
    )
    assert reopened.artifacts.raw_bytes(artifact.slug)[0] == raw
    assert reopened.artifacts.get(artifact.slug).events[0].metadata == {
        "original_filename": "source.png"
    }


def test_retry_returns_original_accepted_revision_and_conflicts_on_changed_payload(
    store,
):
    artifact = image(store)
...
def test_duplicate_ids_empty_attribution_and_explicit_clears(store):
    ...
    with pytest.raises(SketchError):
        store.save(
            artifact.slug,
            1,
            body(entries=[dict(id="same", text="one"), dict(id="same", text="two")]),
        )
    first = store.save(artifact.slug, 1, body())
...
def test_versions_are_isolated_and_deleted_sources_retain_notes(store):
    ...
    store.artifacts.update_binary(
        artifact.slug,
        data=store.artifacts.raw_bytes(artifact.slug)[0],
        mime="image/png",
    )
...
def test_video_time_marks_are_explicitly_not_duration_verified(store):
    ...
    artifact = store.artifacts.create_binary(
        name="Clip", data=video, mime="video/mp4", kind="video"
    )
    saved = store.save(
        artifact.slug,
        1,
        body(entries=[dict(id="time", text="At opening", time_seconds=0)]),
    )
    ...
    for value in (-1, float("inf"), True, 604801):
        with pytest.raises(SketchError):
            store.save(
                artifact.slug,
                1,
                body(1, "bad", [dict(id="time", text="Invalid", time_seconds=value)]),
            )
    with pytest.raises(SketchError):
...
def test_unknown_versions_and_isolated_homes(store, tmp_path):
    ...
    other = AnnotationStore(
        tmp_path / "other/annotations.sqlite3",
...
async def test_real_annotation_tools_share_http_store_and_history(store):
    ...
    provider = MediaToolProvider(
        SketchStore(store.path.parent / "sketches.sqlite3", store.artifacts),
        MediaLibrary(store.artifacts),
        store,
    )
    ...
    result = await provider.invoke(
        "media_annotations_get", {**arguments, "annotation_revision": 1}
    )
    assert result.success
    assert json.loads(result.output) == saved
    result = await provider.invoke(
        "media_annotations_history", {**arguments, "limit": 1}
    )
    assert result.success
    assert json.loads(result.output)["total"] == 1
    result = await provider.invoke(
        "media_annotations_save", {**arguments, **body(0, "stale")}
    )
    assert not result.success
    assert result.metadata["status"] == 409
    result = await provider.invoke(
        "media_annotations_get", {**arguments, "home": "other"}
    )
    assert not result.success
