...
def service(tmp_path):
    ...
    return ImageService(NativeArtifactProvider(tmp_path / 'artifacts'), selector=lambda: (provider, 'image'))


def image(service, size=(12, 10), color=(10, 20, 30, 255)):
    output = io.BytesIO()
    Image.new('RGBA', size, color).save(output, 'PNG')
    return service.artifacts.create_binary(name='Original', data=output.getvalue(), mime='image/png'), output.getvalue()
...
def test_request_pins_real_source_and_preserves_alpha_and_original(service):
    ...
    prepared = request(service, source_artifact_id=original.slug, source_version=1, mask_artifact_id=mask.slug, mask_version=1, controls={'seed': 42})
...
@pytest.mark.parametrize('body', [
    {}, {'prompt': ''}, {'prompt': ' '}, {'prompt': 1}, {'prompt': 'a'*4001},
    {'prompt': 'a', 'provider': 'other'}, {'prompt': 'a', 'home': '/tmp'},
    {'prompt': 'a', 'controls': {'unknown': 1}}, {'prompt': 'a', 'controls': {'seed': True}},
    {'prompt': 'a', 'controls': {'seed': 1.5}}, {'prompt': 'a', 'controls': {'steps': -1}},
    {'prompt': 'a', 'controls': {'guidance': float('nan')}}, {'prompt': 'a', 'controls': {'strength': float('inf')}},
    {'prompt': 'a', 'controls': []}, {'prompt': 'a', 'source_artifact_id': 'only-id'},
    {'prompt': 'a', 'source_version': 1}, {'prompt': 'a', 'mask_artifact_id': 'mask', 'mask_version': 1},
    {'prompt': 'a', 'size': []},
])
...
def test_mismatched_mask_missing_pinned_version_and_nonimage_fail(service):
    ...
    with pytest.raises(SketchError):
        request(service, source_artifact_id=source.slug, source_version=1, mask_artifact_id=mask.slug, mask_version=1)
    with pytest.raises(SketchError):
...
def test_typed_advertised_controls_are_required_and_enforced(service):
    prepared = request(service, controls={'seed': 42, 'steps': 20, 'guidance': 7.5, 'strength': .6}, size='12x10')
    model = ImageGenModel('image', sizes=['12x10'], supported_controls={
        'seed': ImageControl(0, 100, True), 'steps': ImageControl(1, 30, True),
        'guidance': ImageControl(0, 10), 'strength': ImageControl(0, 1)})
    service.validate_model(prepared, model)
    for key in prepared['controls']:
        unsupported = ImageGenModel('image', sizes=['12x10'], supported_controls={name: control for name, control in model.supported_controls.items() if name != key})
        with pytest.raises(SketchError, match=key):
            service.validate_model(prepared, unsupported)
    for key, value in [('seed', 101), ('steps', 0), ('guidance', 11), ('strength', 1.5)]:
        with pytest.raises(SketchError, match=key):
...
def test_conditioning_and_mask_are_separate_advertised_capabilities(service):
    ...
    prepared = request(service, source_artifact_id=source.slug, source_version=1, mask_artifact_id=mask.slug, mask_version=1)
    ...
    service.validate_model(prepared, ImageGenModel('image', supports_edit=True, supports_mask=True))
...
def test_real_typed_image_result_materializes_png_and_retains_provenance(service):
    ...
    result = ImageResult(b64=base64.b64encode(output.getvalue()).decode(), mime='image/jpeg')
    ...
    assert artifact.events[0].metadata['generation_request_sha256'] == hashlib.sha256(json.dumps(prepared, sort_keys=True).encode()).hexdigest()
    ...
    with pytest.raises(SketchError, match='conflicts'):
        service.materialize(result, dict(prepared, prompt='Changed'), 'actual-image-result')


def test_invalid_or_absent_image_results_never_create_artifact(service):
    prepared = request(service)
    for result in (ImageResult(), ImageResult(b64='notbase64'), ImageResult(b64=base64.b64encode(b'not an image').decode())):
        with pytest.raises(SketchError):
...
def test_actual_worker_failed_image_job_reopens_and_retries_without_output(service, tmp_path):
    ...
    body = dict(operation='image_generate', request_id='queued-image', input={'prompt': 'A real generation request'})
...
async def test_http_and_native_tools_queue_same_typed_requests(service, tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        body = dict(operation='image_generate', request_id='http', input={'prompt': 'Image from API'})
        ...
        response = await client.post('/api/capabilities/media/images?home=other', json=body)
        assert response.status == 400
        response = await client.post('/api/capabilities/media/images', json=dict(body, runtime='other'))
        assert response.status == 400
        provider = MediaToolProvider(sketches, MediaLibrary(service.artifacts))
        provider.jobs = jobs
        result = await provider.invoke('media_image_submit', {'request_id': 'tool', 'input': {'prompt': 'Image from tool'}})
        assert result.success
        assert json.loads(result.output)['input']['prompt'] == 'Image from tool'
        result = await provider.invoke('media_image_submit', {'request_id': 'escape', 'input': {'prompt': 'No', 'source_path': '/tmp'}})
