...
def submit(jobs, request='first'):
    sketch = jobs.sketches.create(dict(width=12, height=12, request_id=request, strokes=[dict(tool='draw', color='#ff0000', width=3, points=[[3, 3], [8, 8]])]))
    return jobs.submit(dict(operation='sketch_export', sketch_id=sketch['id'], revision=sketch['revision'], request_id=request))
...
def test_real_render_and_reopen_preserve_pixels_and_public_metadata(jobs):
    ...
    assert [event['status'] for event in finished['events']] == ['queued', 'running', 'succeeded']
...
def test_replay_conflict_and_unknown_engine(jobs):
    first = submit(jobs)
    args = dict(operation='sketch_export', sketch_id=first['sketch_id'], revision=first['revision'], request_id='first')
...
def test_real_revision_change_fails_then_records_retry_error(jobs):
    first = submit(jobs)
    jobs.sketches.update(first['sketch_id'], {'revision': first['revision'], 'strokes': []})
    MediaWorker(jobs).run_once(context())
...
time.sleep(20)
'''
    child = subprocess.Popen([sys.executable, '-c', code, str(jobs.path.parents[2])], stdout=subprocess.PIPE, text=True)
    try:
...
def test_sdk_native_worker_real_process_and_denied_grant(jobs, tmp_path):
    first = submit(jobs)
    worker = Path(__file__).resolve().parents[4] / 'runtime/gideon/extensions/apps/native/gideon-media/worker.py'
    env = dict(os.environ, GIDEON_HOME=str(jobs.path.parents[2]), GIDEON_APP_NAME='gideon-media', GIDEON_APP_WORKER_GRANT='backgroundTasks')
    child = subprocess.Popen([sys.executable, str(worker)], env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    try:
        deadline = time.monotonic() + 10
        while time.monotonic() < deadline and jobs.get(first['id'])['status'] != 'succeeded' and child.poll() is None:
            time.sleep(.05)
        assert jobs.get(first['id'])['status'] == 'succeeded'
    ...
    denied = subprocess.run([sys.executable, str(worker)], env=env, capture_output=True, text=True, timeout=5)
    ...
    wrong_app = subprocess.run([sys.executable, str(worker)], env=env, capture_output=True, text=True, timeout=5)
...
async def test_http_and_tools_use_same_durable_queue(jobs):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        cancelled = await client.post(base+'/'+first['id']+'/cancel', json=guard(first))
        ...
        for payload in ({}, {'state_revision': True}, dict(guard(item), home='/tmp')):
            response = await client.post(base+'/'+first['id']+'/retry', json=payload)
        ...
        result = await provider.invoke('media_jobs_retry', dict(job_id=first['id'], **guard(item)))
        ...
        result = await provider.invoke('media_jobs_cancel', {'job_id': first['id'], 'state_revision': 1, 'home': '/tmp'})
