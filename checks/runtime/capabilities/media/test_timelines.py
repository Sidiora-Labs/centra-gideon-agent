...
def image(jobs, color):
    ...
    return jobs.sketches.artifacts.create_binary(
        name=color, data=raw.getvalue(), mime="image/png"
    )
...
def project(jobs, colors=("red", "blue")):
    return dict(
        ...
        segments=[
            dict(
                artifact_id=image(jobs, color).slug,
                version=1,
                kind="image",
                start=0,
                duration=1,
...
def render(jobs, document, request_id="render"):
    job = jobs.submit(
        dict(
            ...
            input=dict(timeline_id=document["id"], revision=document["revision"]),
        )
    )
...
def test_real_worker_renders_order_timing_progress_and_canonical_provenance(
    jobs, tmp_path
):
    body = project(jobs)
    originals = {
        entry["artifact_id"]: jobs.sketches.artifacts.raw_bytes(entry["artifact_id"])[0]
    ...
    assert abs(job["result"]["duration_seconds"] - 2) < 0.1
...
def test_overlay_actual_position_and_timed_visibility(jobs, tmp_path):
    ...
    assert pixel(path, 0.25, 6, 6)[2] > 240
...
def test_video_trim_uses_requested_source_interval(jobs, tmp_path):
    ...
    body = dict(
        ...
        request_id="trim",
        segments=[
            dict(artifact_id=source_id, version=1, kind="video", start=1, duration=0.5)
        ],
        overlays=[],
        audio=[],
    )
    ...
    assert pixel(path, 0.1)[2] > 230
    assert abs(probe_video(path)["duration_seconds"] - 0.5) < 0.1
...
def tone(jobs):
    ...
    with wave.open(buffer, "wb") as audio:
        ...
        audio.writeframes(
            b"".join(
                struct.pack(
                    "<h", round(16000 * math.sin(2 * math.pi * 440 * i / 48000))
                )
                for i in range(48000)
            )
        )
    return jobs.sketches.artifacts.create_binary(
        name="Tone", data=buffer.getvalue(), mime="audio/wav", kind="audio"
    )
...
def test_explicit_audio_placement_volume_fades_and_silence(jobs, tmp_path):
    ...
    assert rms(0.05, 0.3) < 20
    assert 3800 < rms(0.85, 1.1) < 4200
    assert rms(0.52, 0.58) < rms(0.85, 1.1) * 0.5
    assert rms(1.42, 1.48) < rms(0.85, 1.1) * 0.5
...
def test_invalid_trims_placements_and_paths_are_rejected(jobs):
    ...
    for patch in ({"x": -1}, {"width": 40}, {"start": 2}, {"duration": -1}):
        ...
        changed["overlays"] = [
            dict(
                artifact_id=body["segments"][0]["artifact_id"],
                version=1,
                start=0,
                duration=1,
                x=0,
...
def test_uploaded_playlist_is_rejected_without_following_references(jobs, tmp_path):
    ...
    artifact = jobs.sketches.artifacts.create_binary(
        name="Mislabelled", data=raw, mime="audio/wav", kind="audio"
    )
    body = project(jobs, ("red",))
    body["audio"] = [
        dict(
...
async def test_real_http_tools_share_immutable_projects_and_jobs(jobs):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        history = await tool.invoke(
            "media_timelines_history", {"timeline_id": document["id"]}
        )
        assert json.loads(history.output)["items"] == [document]
        updated = await client.put(
            "/api/capabilities/media/timelines/" + document["id"],
            json=dict(body, request_id="update", revision=1, title="Updated"),
        )
        assert updated.status == 200
        assert (await updated.json())["revision"] == 2
        stale = await client.put(
            "/api/capabilities/media/timelines/" + document["id"],
            json=dict(body, request_id="stale", revision=1),
        )
        ...
        queued = await tool.invoke(
            ...
            {
                "request_id": "render",
                "input": {"timeline_id": document["id"], "revision": 1},
            },
        )
        ...
        assert (
            await client.get("/api/capabilities/media/timelines/missing")
        ).status == 404
        response = await client.get(
            "/api/capabilities/media/timelines/" + document["id"] + "/history"
        )
...
def test_concurrent_real_connections_allow_only_one_revision_update(jobs):
    ...
    first = jobs.timelines.save(body)
    stores = [
        TimelineStore(jobs.timelines.path, jobs.videos),
        TimelineStore(jobs.timelines.path, jobs.videos),
    ]

    def save(index):
        try:
            return stores[index].save(
                dict(
                    body,
                    revision=1,
                    request_id="race-" + str(index),
                    title="Writer " + str(index),
                ),
                first["id"],
            )["revision"]
...
def test_saved_old_revision_render_survives_later_reordering(jobs, tmp_path):
    ...
    path = output(jobs, job, tmp_path)
    assert pixel(path, 0.25)[0] > 230
    assert pixel(path, 1.25)[2] > 230
    metadata = (
        jobs.sketches.artifacts.get(job["result"]["artifact_id"]).events[0].metadata
    )
    assert metadata["timeline_revision"] == 1
...
def test_queued_cancel_never_publishes_rendered_artifact(jobs):
    ...
    queued = jobs.submit(
        dict(
            operation="timeline_render",
            request_id="cancel",
            input=dict(timeline_id=document["id"], revision=1),
        )
    )
...
def test_real_render_scales_and_letterboxes_without_distortion(jobs, tmp_path):
    ...
    assert max(pixel(path, 0.25, 16, 1)) < 10
    assert pixel(path, 0.25, 16, 16)[0] > 230
    assert max(pixel(path, 0.25, 16, 30)) < 10
