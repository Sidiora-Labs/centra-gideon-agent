...
def test_import_replay_and_conflicting_filename_and_bytes(library):
    ...
    with pytest.raises(SketchError) as conflict:
        library.import_image(
            picture("red"), filename="blue.png", request_id="first", mime="image/png"
        )
    assert conflict.value.status == 409
...
def test_metadata_tags_and_collection_are_canonical_and_keep_bytes(library):
    ...
    cleared = library.update(
        item["id"],
        {"expected_updated_at": updated["updated_at"], "tags": [], "collection": ""},
    )
    assert cleared["tags"] == []
...
def test_existing_canonical_writer_invalidates_library_edit(library):
    ...
    with pytest.raises(SketchError) as conflict:
        library.update(
            item["id"], {"expected_updated_at": item["updated_at"], "tags": ["stale"]}
        )
    assert conflict.value.status == 409
    ...
    def change(pair):
        ...
        try:
            return service.update(
                item["id"], {"expected_updated_at": item["updated_at"], "name": name}
            )
...
def test_facets_are_query_wide_not_page_local(library):
    ...
    library.artifacts.create_binary(
        name="Video", data=video, mime="video/mp4", kind="video", tags=["all"]
    )
    ...
    assert library.list({"q": "no matches"})["facets"] == {
        "kinds": {},
        "tags": {},
        "collections": {},
    }
...
def test_invalid_metadata_does_not_mutate_artifact(library, changes):
    ...
    with pytest.raises(SketchError):
        library.update(
            item["id"], {"expected_updated_at": item["updated_at"], **changes}
        )
...
@pytest.mark.parametrize(
    "filename",
    ["../escape.png", "folder/file.png", "C:\\file.png", "..", "", "line\nname.png"],
)
...
def test_import_supported_encodings_and_mime_mismatch(library):
    for fmt, mime in [("JPEG", "image/jpeg"), ("WEBP", "image/webp")]:
        ...
        item = library.import_image(
            data, filename=f"image.{fmt.lower()}", request_id=fmt, mime=mime
        )
    ...
    with pytest.raises(SketchError):
        library.import_image(
            b"not-image", filename="bad.png", request_id="bad", mime="image/png"
        )
...
def test_binary_update_invalidates_metadata_cas(library):
    ...
    with pytest.raises(SketchError) as conflict:
        library.update(
            item["id"],
            {"expected_updated_at": item["updated_at"], "collection": "stale"},
        )
    assert conflict.value.status == 409
...
async def test_http_import_list_filter_metadata_and_source_bytes(library):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.patch(
            base + "/" + item["id"],
            json={"expected_updated_at": item["updated_at"], "name": "stale"},
        )
...
async def test_http_refuses_scope_injection_and_malformed_mutations(library):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.patch(
            base + "/" + item["id"],
            json={"expected_updated_at": item["updated_at"], "provider": "other"},
        )
        assert response.status == 400
        response = await client.post(base + "/import?runtime=other", data=picture())
        assert response.status == 400
        response = await client.post(
            base + "/import", data=picture(), headers={"Content-Type": "image/png"}
        )
        assert response.status == 400
