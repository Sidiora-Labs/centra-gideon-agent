...
@pytest.fixture
def store(tmp_path):
    return SketchStore(tmp_path / 'sketches.sqlite3', NativeArtifactProvider(tmp_path / 'artifacts'))
...
def drawing(tool='draw', color='#ff0000', width=8, points=None):
    return dict(tool=tool, color=color, width=width, points=points or [[8, 20], [40, 20]])
...
def pixels(store, result):
    raw, mime = store.artifacts.raw_bytes(result['artifact_id'], version=result['version'])
...
def test_original_version_and_overlay_survive_reopen(store):
    ...
    source = store.artifacts.create_binary(name='Original', data=original, mime='image/png')
...
def test_erase_reveals_original_and_later_draw_remains(store):
    source = store.artifacts.create_binary(name='Blue', data=image_bytes(), mime='image/png')
    sketch = blank(store, source_artifact_id=source.slug, source_version=1,
                   strokes=[drawing(), drawing('erase', width=4, points=[[20, 20]]), drawing(color='#00ff00', width=2, points=[[30, 20]])])
...
def test_concurrent_updates_have_one_winner(store):
    ...
    def save(color):
        try:
            return store.update(sketch['id'], {'revision': 1, 'strokes': [drawing(color=color)]})
...
@pytest.mark.parametrize('patch', [
    {'width': 0}, {'width': 4097}, {'height': -1}, {'height': True},
    {'width': 1.5}, {'request_id': '../escape'}, {'source_version': 1},
    {'source_artifact_id': '../escape', 'source_version': 1},
    {'source_artifact_id': 'missing'}, {'provider': 'other'}, {'home': '/tmp/other'},
])
...
@pytest.mark.parametrize('stroke', [
    drawing(tool='fill'), drawing(color='red'), drawing(color='#fff'),
    drawing(color='#ff000000'), drawing(width=0), drawing(width=129),
    drawing(width=True), drawing(points=[[64, 0]]), drawing(points=[[0, 48]]),
    drawing(points=[[-1, 0]]), drawing(points=[[float('nan'), 0]]),
    drawing(points=[[float('inf'), 0]]), drawing(points=[[True, 0]]),
    drawing(points=[[1]]), drawing(points=[[1, 2, 3]]), drawing(points=['bad']),
    {**drawing(), 'points': []}, {**drawing(), 'secret': 'bad'},
])
...
def test_bounds_for_strokes_points_and_source_dimensions(store):
    ...
    source = store.artifacts.create_binary(name='Different', data=image_bytes(size=(2, 2)), mime='image/png')
...
def test_source_pinned_even_after_new_artifact_version(store):
    source = store.artifacts.create_binary(name='Pinned', data=image_bytes(), mime='image/png')
    sketch = blank(store, source_artifact_id=source.slug, source_version=1)
    store.artifacts.update_binary(source.slug, data=image_bytes('yellow'), mime='image/png')
...
def test_missing_wrong_kind_and_corrupt_sources(store):
    ...
    corrupt = store.artifacts.create_binary(name='Corrupt', data=b'not an image', mime='image/png')
...
def test_two_homes_cannot_resolve_other_sources(store, tmp_path):
    other = SketchStore(tmp_path / 'other' / 'sketches.sqlite3', NativeArtifactProvider(tmp_path / 'other' / 'artifacts'))
    source = store.artifacts.create_binary(name='Private', data=image_bytes(), mime='image/png')
    with pytest.raises(SketchError) as missing:
...
def test_export_collision_never_returns_unrelated_bytes(store):
    ...
    intruder = store.artifacts.create_binary(name='Unrelated', slug=slug, data=image_bytes('black'), mime='image/png')
    with pytest.raises(SketchError) as conflict:
        store.export(sketch['id'], {'revision': 1})
    assert conflict.value.status == 409
...
def test_transparent_source_alpha_and_canvas_edges(store):
    ...
    source = store.artifacts.create_binary(name='Transparent', data=raw, mime='image/png')
    sketch = blank(store, source_artifact_id=source.slug, source_version=1,
...
def test_source_deleted_after_save_is_an_explicit_export_failure(store):
    source = store.artifacts.create_binary(name='Temporary', data=image_bytes(), mime='image/png')
    sketch = blank(store, source_artifact_id=source.slug, source_version=1)
...
def test_source_exif_orientation_is_preserved_in_export(store):
    ...
    source = store.artifacts.create_binary(name='Rotated', data=original, mime='image/png')
    sketch = blank(store, source_artifact_id=source.slug, source_version=1)
    result = store.export(sketch['id'], {'revision': 1})
...
def test_unknown_detail_and_invalid_update_envelopes(store):
    ...
    assert missing.value.status == 404
    sketch = blank(store)
    for body in ({}, {'strokes': []}, {'revision': True, 'strokes': []}, {'revision': 1, 'strokes': 'bad'}):
        with pytest.raises(SketchError):
...
async def test_real_http_create_save_reload_export(store):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(prefix, json={'width': 64, 'height': 48, 'request_id': 'http'})
        assert response.status == 201
        sketch = await response.json()
        detail = prefix + '/' + sketch['id']
        response = await client.put(detail, json={'revision': 1, 'strokes': [drawing()]})
        assert response.status == 200
...
async def test_real_http_source_and_invalid_requests(store):
    source = store.artifacts.create_binary(name='HTTP source', data=image_bytes(), mime='image/png')
    sketch = blank(store, source_artifact_id=source.slug, source_version=1)
    app = web.Application(client_max_size=1024 * 1024)
    app[STORE_KEY] = store
    register(app)
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(prefix, data='invalid', headers={'Content-Type': 'application/json'})
        assert response.status == 400
        response = await client.post(prefix, json=[])
        assert response.status == 400
        response = await client.get(prefix + '?home=/tmp/other')
        assert response.status == 400
        response = await client.put(detail, json={'revision': 1, 'strokes': [], 'source_artifact_id': source.slug})
        assert response.status == 400
        response = await client.post(detail + '/export', json={'revision': 1, 'account': 'elsewhere'})
        assert response.status == 400
