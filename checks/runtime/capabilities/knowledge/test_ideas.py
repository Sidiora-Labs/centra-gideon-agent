...
from gideon.workspace.capabilities.knowledge.idea_format import parse, preview, render
...
DOCUMENT = {
    "id": "902ee3d2-8e7e-481b-a23b-5b9741c6ca51",
    "title": "Observatory ideas",
    "category": "Exploration",
    "status": "draft",
    "created": "2025-09-25T12:00:00Z",
    "modified": "2025-09-26T12:00:00Z",
    "tags": ["idea-loom", "astronomy"],
    "prompt": "What could we observe?",
    "help": "Consider accessible equipment.\n1. This numbered line is help.",
    "ideas": ["Observe Saturn", "Photograph the Moon"],
    "extra": {"owner": "Personal notebook"},
}
...
def imported(service, document=None, **changes):
    ...
    return service.import_list(
        {
            "request_id": "idea-import-original",
            "content": content,
...
def sync(service, identity, request):
    return service.sync(
        identity,
        {"request_id": request, "expected_hash": service.get(identity)["hash"]},
    )
...
def test_supported_markdown_roundtrip_preserves_order_fields_and_extra_metadata():
    ...
    block_tags = content.replace(
        'tags: ["idea-loom", "astronomy"]', 'tags:\n  - "idea-loom"\n  - "astronomy"'
    )
    assert parse(block_tags)["tags"] == DOCUMENT["tags"]
    aliases = (
        content.replace("id:", "uuid:", 1)
        .replace("created:", "createdAt:", 1)
        .replace("modified:", "updatedAt:", 1)
    )
...
def test_import_uses_real_collection_and_fleeting_records(ideas):
    ...
    assert all(
        ideas.store.get_item(item["id"])["item_type"] == "fleeting"
    ...
    assert (
        result["collection_link"] == "#/knowledge?collection=" + result["collection_id"]
    )
    assert all(
        item["source_link"] == "#/knowledge/item/" + item["id"]
        for item in result["items"]
    )
...
def test_reviewed_reimport_preserves_identity_and_removes_membership_only(ideas):
    ...
    second = imported(
        ideas, changed, request_id="idea-reimport-reviewed", expected_hash=first["hash"]
    )
    ...
    with pytest.raises(CaptureError) as failure:
        imported(
            ideas, changed, request_id="idea-stale-import", expected_hash=first["hash"]
        )
...
def test_owner_deleted_file_and_deleted_collection_never_resurrect(ideas):
    ...
    assert (
        sync(ideas, first["id"], "collection-delete-sync")["outcome"]
        == "collection_deleted"
    )
    assert not path.exists()
...
def test_bound_home_config_and_projection_paths_refuse_foreign_mutation(
    ideas, monkeypatch
):
    first = imported(ideas)
    ...
    (ideas.home / "config.json").write_text(
        json.dumps({"knowledge": {"vault_mode": "two_way", "vault_path": str(foreign)}})
    )
    assert not ideas.availability()["available"]
    with pytest.raises(CaptureError):
...
def test_real_concurrent_import_retries_create_one_collection(ideas):
    ...
    try:
        ...
        assert (
            ideas.db.execute(
                "SELECT count(*) FROM items WHERE item_type='fleeting'"
            ).fetchone()[0]
            == 2
        )
        assert (
            ideas.db.execute(
                "SELECT count(*) FROM capability_knowledge_idea_requests"
            ).fetchone()[0]
            == 1
        )
    finally:
...
def test_interval_schedule_and_actual_clock_dispatch_journal(ideas):
    ...
    arguments = {
        "request_id": "idea-schedule-first",
    ...
    stamp = datetime.fromisoformat(
        configured["next_fire_at"].replace("Z", "+00:00")
    ).timestamp()
    ...
    async def journey():
        ...
        repeated = await provider.execute(
            ...
            ActionContext(event="clock", payload={"scheduled_for": fire.scheduled_for}),
        )
    ...
    disabled = schedules.save(
        first["id"],
        {
            "request_id": "idea-schedule-disable",
    ...
    assert not asyncio.run(
        provider.execute(trigger.workflow["config"], ActionContext(event="clock"))
    ).success
...
@pytest.mark.parametrize(
    "mutation",
    [
        lambda s: s.replace("1. Observe Saturn", "3. Observe Saturn"),
        lambda s: s.replace('"draft"', '"invalid"'),
        lambda s: s.replace('"idea-loom"', '"other"'),
        lambda s: s.replace(DOCUMENT["id"], "../other"),
        lambda s: s + "\n## Ideas\n9. Invalid",
        lambda s: s.replace(
            'owner: "Personal notebook"', "owner: &anchor value\ncopy: *anchor"
        ),
    ],
)
...
def test_native_idea_consumers_and_session_policy(ideas):
    ...
    try:
        state._sessions["ideas-native"] = _ChatSession(
            "ideas-native", memory_mode="persistent"
        )
        content = render(DOCUMENT)
        reviewed = asyncio.run(
            provider.invoke("knowledge_idea_preview", {"content": content})
        )
        assert reviewed.success
        args = {
            "request_id": "native-idea-import",
            "content": content,
            "preview_id": json.loads(reviewed.output)["preview_id"],
            "expected_hash": "",
        }
        ...
        exported = asyncio.run(
            provider.invoke("knowledge_idea_export", {"id": receipt["id"]})
        )
        assert parse(json.loads(exported.output)["content"]) == DOCUMENT
        assert (
            len(
                json.loads(
                    asyncio.run(provider.invoke("knowledge_idea_list", {})).output
                )["items"]
            )
            == 1
        )
...
def test_actual_http_preview_import_export_sync_and_invalid_selectors(ideas):
    async def journey():
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            response = await client.post(
                root + "/" + receipt["id"] + "/sync",
                json={
                    "request_id": "http-ideas-sync",
            ...
            assert (
                await client.post(
                    root + "/preview", json={"content": content, "model": "other"}
                )
            ).status == 400
...
def test_identical_external_uuid_has_distinct_bound_action_identity(ideas, monkeypatch):
    ...
    configured = original_schedule.save(
        first["id"],
        {
            "request_id": "original-bound-clock",
            "revision": first["revision"],
            "enabled": True,
            "minutes": 5,
        },
    ...
    other_home.mkdir()
    (other_home / "config.json").write_text(
        json.dumps({"knowledge": {"vault_mode": "two_way"}})
    )
    ...
    try:
        ...
        scheduled = schedules.save(
            second["id"],
            {
                "request_id": "second-bound-clock",
        ...
        foreign = asyncio.run(
            provider.execute(
                original_trigger.workflow["config"], ActionContext(event="clock")
            )
        )
        ...
        assert (
            asyncio.run(provider.execute(own.workflow["config"], context)).stdout
            == fired.stdout
        )
        assert not ideas.vault_root.exists()
    finally:
...
def test_bound_projection_symlink_refuses_foreign_file_read(ideas):
    ...
    assert (
        ideas.store.get_item(first["items"][0]["id"])["content"] == DOCUMENT["ideas"][0]
    )
...
def test_schedule_revision_validation_and_disabled_mode_preserve_trigger(ideas):
    ...
    schedules = IdeaSchedules(ideas)
    args = {
        "request_id": "schedule-validation-first",
        "revision": 1,
        "enabled": True,
        "minutes": 15,
    }
    ...
    for change in (
        ...
        invalid = {
            **args,
            "request_id": "schedule-validation-" + str(len(str(change))),
    ...
    (ideas.home / "config.json").write_text(
        json.dumps({"knowledge": {"vault_mode": "off"}})
    )
    with pytest.raises(CaptureError):
        schedules.save(
            first["id"],
            {**args, "request_id": "schedule-now-off", "revision": saved["revision"]},
        )
    disabled = schedules.save(
        first["id"],
        {
            **args,
            "request_id": "schedule-disable-off",
            "revision": saved["revision"],
            "enabled": False,
        },
    )
...
def test_format_bounds_and_unsupported_nested_metadata_are_explicit(ideas):
    ...
    with pytest.raises(CaptureError):
        preview(
            render(DOCUMENT).replace(
                'owner: "Personal notebook"', "owner:\n  nested: unsupported"
            )
        )
    with pytest.raises(CaptureError):
        preview("x" * 262145)
...
def test_empty_list_is_portable_and_existing_items_are_not_deleted_on_reimport(ideas):
    ...
    result = imported(
        ideas, emptied, request_id="empty-reviewed-import", expected_hash=first["hash"]
    )
    assert result["items"] == []
...
def test_refused_mode_sync_recovers_same_durable_request_after_explicit_enable(ideas):
    ...
    assert (
        ideas.db.execute(
            "SELECT count(*) FROM capability_knowledge_idea_requests WHERE receipt IS NULL"
        ).fetchone()[0]
        == 0
    )
