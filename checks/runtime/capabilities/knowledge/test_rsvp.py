...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"word_index": -1, "wpm": 350, "chunk_size": 1, "content_revision": "REV"},
            "word_index",
        ),
        (
            {"word_index": 99, "wpm": 350, "chunk_size": 1, "content_revision": "REV"},
            "word_index",
        ),
        (
            {
        ...
        (
            {"word_index": 0, "wpm": 99, "chunk_size": 1, "content_revision": "REV"},
            "wpm",
        ),
        (
            {"word_index": 0, "wpm": 1001, "chunk_size": 1, "content_revision": "REV"},
            "wpm",
        ),
        (
            {"word_index": 0, "wpm": True, "chunk_size": 1, "content_revision": "REV"},
            "wpm",
        ),
        (
            {"word_index": 0, "wpm": 350, "chunk_size": 3, "content_revision": "REV"},
            "chunk_size",
        ),
    ],
)
...
async def test_http_round_trip_guards_shape_and_persists_bookmark(tmp_path):
    ...
    item_id = store.create_typed_item(
        item_type="article", title="HTTP RSVP", content="one two three four five"
    )
    ...
    try:
        ...
        marked_response = await client.post(
            f"/api/capabilities/knowledge/rsvp/{item_id}/bookmark",
            json={
                "word_index": 2,
                "content_revision": opened["content_revision"],
            },
        ...
        moved_response = await client.put(
            f"/api/capabilities/knowledge/rsvp/{item_id}",
            json={
                "word_index": 4,
                "wpm": 500,
        ...
        restored_response = await client.post(
            f"/api/capabilities/knowledge/rsvp/{item_id}/restore", json={}
        )
        assert restored_response.status == 200
        assert (await restored_response.json())["word_index"] == 2
        assert (
            await client.get(f"/api/capabilities/knowledge/rsvp/{item_id}?home=/tmp")
        ).status == 400
...
@pytest.mark.asyncio
async def test_knowledge_umbrella_mounts_one_rsvp_owner_and_restores_canonical_item(
    tmp_path,
):
    ...
    routes = [
        ...
        if route.resource.canonical.startswith("/api/capabilities/knowledge/rsvp/")
    ]
