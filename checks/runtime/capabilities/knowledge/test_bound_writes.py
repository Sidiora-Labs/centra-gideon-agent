...
def test_capture_all_mutations_refuse_drift_and_retry_original(bound, monkeypatch):
    ...
    payload = {
        ...
        "destination": "note",
        "title": "Reviewed",
        "content": "Routed to the actual original store",
    }
    ...
    assert (
        inbox.db.execute(
            "SELECT pending FROM capability_knowledge_captures WHERE id=?",
            (original["id"],),
        ).fetchone()[0]
        is None
    )
    ...
    assert (
        asyncio.run(inbox.transcribe(voice["id"]))["status"]
...
def test_typed_completed_replay_and_pending_refusal_keep_provenance(bound, monkeypatch):
    ...
    def request(capture):
        fields = {
            "title": "Reviewed " + capture["id"],
            "content": "Canonical personal content",
        }
        preview = typed.preview(
            {"capture_id": capture["id"], "kind": "idea", "fields": fields}
        )
        return {
            key: preview[key]
    ...
    inbox.db.execute(
        ...
        (
            pending["capture_id"],
            pending["request_id"],
            json.dumps(pending, sort_keys=True, ensure_ascii=False),
        ),
    )
    inbox.db.commit()
    monkeypatch.setenv("GIDEON_HOME", str(foreign))
    ...
    try:
        result = asyncio.run(
            provider.invoke(
                ...
                {"request_id": "late-native-write", "text": "Must await original home"},
            )
        )
            ...
            [
                {
                    ...
                    "mapping": {
                        "one": {
                            ...
                            "message": {
                                "author": {"role": "user"},
                                "content": {"parts": ["Original archive text"]},
                            },
                        }
                    },
                }
        ...
        assert preview.success
        payload = archive | {
            "request_id": "late-archive-write",
            "source_digest": json.loads(preview.output)["source_digest"],
            "conversation_ids": ["late-archive"],
        }
