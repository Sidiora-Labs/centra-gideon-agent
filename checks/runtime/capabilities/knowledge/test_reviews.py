...
def preview(service, **changes):
    return service.preview(
        **{"period": "weekly", "date": "2026-09-25", "timezone": "UTC", **changes}
    )


def body(service, **changes):
    return {
        "request_id": "review-save-original",
        "period": "weekly",
        "date": "2026-09-25",
        "timezone": "UTC",
        "preview_id": preview(service)["preview_id"],
        "reflection": "Follow up on the telescope booking.",
        **changes,
    }
...
def schedule(service, **changes):
    return service.save(
        {
            "request_id": "review-schedule-original",
            "period": "daily",
...
def note(service, title, stamp):
    identity = service.store.create_typed_item(
        item_type="note", title=title, content="Original source content"
    )
...
def test_actual_obligations_and_activity_have_exact_citations(reviews):
    ...
    upcoming = asyncio.run(
        tasks.create_task(title="Visit observatory", due="2026-09-27")
    )
    ...
    cancelled = asyncio.run(
        tasks.create_task(title="Cancelled visit", due="2026-09-01")
    )
    cancelled.status = TaskStatus.CANCELLED
    tasks._write_task(cancelled)
    people = PeopleStore(reviews.home / "capabilities/communications")
    missing = people.save(
        {"name": "No recorded history", "ring": "core", "cadence_days": 3}
    )
    ...
    people.record(
        ...
        {
            "source": "manual",
            "external_id": "contact-review-original",
            "occurred_at": "2026-09-01T12:00:00Z",
            "direction": "outbound",
            "summary": "Last actual conversation",
        },
    )
...
def test_timezone_window_is_half_open_and_dst_aware(reviews):
    ...
    result = preview(
        reviews, period="daily", date="2026-03-29", timezone="Europe/Berlin"
    )
    assert result["window_start"] == "2026-03-29T00:00:00+01:00"
    assert result["window_end"] == "2026-03-30T00:00:00+02:00"
    assert [row["source_id"] for row in result["sections"]["recent_notes"]] == [
        included
    ]
    assert excluded not in json.dumps(result["sections"])
    start, end = (
        datetime.fromisoformat(result[key]).timestamp()
...
def test_reflection_snapshot_replay_and_source_changes(reviews):
    ...
    assert (
        "Original title" in reviews.store.get_item(receipt["destination_id"])["content"]
    )
...
def test_bound_reads_and_replay_refuse_new_foreign_writes(reviews, monkeypatch):
    ...
    assert reviews.save({**original, "request_id": "review-foreign-save"})[
        "destination_id"
    ]
...
def test_schedules_are_real_revisioned_clock_triggers(reviews):
    ...
    assert (
        schedules.triggers.get(first["trigger_id"]).trigger.spec["expr"]
        == "30 18 * * 5"
    )
    assert schedule(schedules) == first
    ...
    disabled = schedule(
        schedules,
        request_id="review-disable-edit",
        id=first["id"],
        revision=2,
        enabled=False,
    )
...
def test_concurrent_review_writers_share_one_canonical_receipt(reviews):
    ...
    try:
        ...
        assert (
            reviews.db.execute(
                "SELECT count(*) FROM items WHERE guid=?",
                ("review:" + original["request_id"],),
            ).fetchone()[0]
            == 1
        )
    finally:
...
def test_concurrent_schedule_edits_admit_one_revision(reviews):
    ...
    services = [
        ReviewSchedules(ReviewService(store, reviews.home)) for store in connections
    ]
    barrier = Barrier(2)
    ...
    def save_one(pair):
        ...
        try:
            return schedule(
                service,
                request_id=f"concurrent-schedule-{index}",
                id=first["id"],
                revision=1,
                time=f"0{index + 1}:00",
            )
    ...
    try:
        ...
        assert (
            schedules.triggers.get(first["trigger_id"])
            .trigger.spec["expr"]
            .startswith("0 " + str(int(current["time"][:2])))
        )
    finally:
        for store in connections:
            store.close()
...
def test_invalid_schedules_never_publish_triggers(reviews, changes):
    ...
    assert (
        reviews.db.execute(
            "SELECT count(*) FROM capability_knowledge_review_schedule_requests"
        ).fetchone()[0]
        == 0
    )
...
def test_native_review_consumers_enforce_session_policy(reviews):
    ...
    try:
        state._sessions["review-native"] = _ChatSession(
            "review-native", memory_mode="persistent"
        )
        ...
        assert (
            json.loads(
                asyncio.run(provider.invoke("knowledge_review_list", {})).output
            )["total"]
            == 1
        )
        state._sessions["review-native"].memory_mode = "temporary"
        assert not asyncio.run(
            provider.invoke("knowledge_review_preview", arguments)
        ).success
    ...
    async def journey():
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            for query in (
                "home=/tmp/other",
                "model=x",
                "date=2026-09-25&date=2026-09-24",
            ):
            ...
            assert (await (await client.get(root + "/schedules")).json())["items"][0][
                "id"
            ] == schedule_row["id"]
