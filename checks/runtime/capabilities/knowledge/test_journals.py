...
from gideon.interfaces.dashboard.handlers.capabilities_knowledge_journals import (
    register,
)
...
def body(**changes):
    return {
        "request_id": "journal-first-save",
        "date": "2025-09-25",
        "timezone": "UTC",
        "revision": 0,
        "fingerprint": "",
        "preview_id": "",
        "title": "Day at the observatory",
        "content": "Recorded what actually happened.",
        **changes,
    }
...
def update(service, receipt, **changes):
    ...
    return service.save(
        body(
            request_id="journal-second-save",
...
def test_date_timezone_identity_and_original_date_preserved(journals):
    ...
    other = journals.save(
        body(request_id="journal-second-zone", timezone="Europe/Berlin")
    )
...
def test_canonical_edit_revision_fts_and_immutable_replay(journals):
    ...
    second = update(
        ...
        content="Saturn rings were visible.",
    )
    assert second["id"] == first["id"]
    assert second["revision"] == 2
    assert second["fingerprint"] != first["fingerprint"]
    assert (
        journals.store.get_item(first["id"])["content"] == "Saturn rings were visible."
    )
...
def test_existing_editor_changes_detected_without_overwriting(journals):
    first = journals.save(body())
    journals.store.update_item(
        first["id"],
        title="Changed in canonical editor",
        content="Canonical editor owns this correction.",
    )
    with pytest.raises(CaptureError) as failure:
        journals.save(
            body(
                request_id="journal-external-stale",
                revision=1,
                fingerprint=first["fingerprint"],
                content="Old tab overwrites",
            )
        )
    assert failure.value.status == 409
    ...
    second = journals.save(
        body(
            request_id="journal-after-reload",
            revision=1,
            fingerprint=current["fingerprint"],
    ...
    with pytest.raises(CaptureError) as failure:
        journals.save(
            body(
                request_id="journal-archived-save",
                revision=2,
                fingerprint=current["fingerprint"],
            )
        )
...
def test_reviewed_activity_draft_keeps_actual_citations_and_rejects_changes(journals):
    source = journals.store.create_typed_item(
        item_type="note", title="Actual visit", content="We visited the observatory."
    )
    journals.db.execute(
        "UPDATE items SET created_at=? WHERE id=?", ("2025-09-25T12:00:00Z", source)
    )
    journals.db.commit()
...
def test_store_compare_and_update_uses_real_fts_and_default_contract(journals):
    ...
    assert (
        journals.store.update_item(
            first["id"], expected=expected, content="Neptune observation"
        )
        is True
    )
    assert (
        journals.store.update_item(
            first["id"], expected=expected, content="Stale overwrite"
        )
        is False
    )
    ...
    assert (
        journals.store.update_item(first["id"], title="Normal existing editor") is None
    )
    with pytest.raises(ValueError):
        journals.store.update_item(
            first["id"], expected={"sql expression": "invalid"}, content="Rejected"
        )
    assert journals.store.get_item(first["id"])["title"] == "Normal existing editor"
    assert (
        journals.store.update_item("missing-id", expected=expected, content="Missing")
        is False
    )
...
def test_concurrent_canonical_cas_admits_one_writer(journals):
    ...
    expected = {field: raw[field] for field in COLUMNS}
    connections = [
        KnowledgeStore(str(journals.reviews.home / "knowledge.db")) for _ in range(2)
    ]
    barrier = Barrier(2)
    ...
    def writer(pair):
        ...
        return store.update_item(
            first["id"],
            expected=expected,
            content="Concurrent observation " + str(index),
        )
    ...
    try:
        ...
        assert journals.store.get_item(first["id"])["content"].startswith(
...
def test_concurrent_journal_request_replays_one_destination(journals):
    connections = [
        KnowledgeStore(str(journals.reviews.home / "knowledge.db")) for _ in range(4)
    ]
    ...
    try:
        ...
        assert (
            journals.db.execute(
                "SELECT count(*) FROM items WHERE item_type='journal'"
            ).fetchone()[0]
            == 1
        )
        assert (
            journals.db.execute(
                "SELECT count(*) FROM capability_knowledge_journal_mutations"
            ).fetchone()[0]
            == 1
        )
    finally:
...
def test_native_journal_session_policy_and_real_consumer(journals):
    ...
    try:
        state._sessions["journal-native"] = _ChatSession(
            "journal-native", memory_mode="persistent"
        )
        args = {"date": "2025-09-25", "timezone": "UTC"}
        assert (
            json.loads(
                asyncio.run(provider.invoke("knowledge_journal_get", args)).output
            )["journal"]
            is None
        )
...
def test_real_http_journal_canonical_read_and_selector_rejection(journals):
    async def journey():
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            result = await (
                await client.get(root + "?date=2025-09-25&timezone=UTC")
            ).json()
            assert result["journal"]["id"] == receipt["id"]
            assert result["journal"]["content"] == body()["content"]
            assert (
                await client.get(root + "/draft?date=2025-09-25&timezone=UTC")
            ).status == 200
            for query in (
                "home=/tmp/other",
                "model=x",
            ...
            assert (
                await client.post(root, json={**body(), "account_id": "other"})
            ).status == 400
            secret = "sk-" + "a" * 48
