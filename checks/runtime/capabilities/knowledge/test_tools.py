...
from gideon.workspace.capabilities.knowledge.tools import (
    KnowledgeCapabilityTools,
    create_provider,
)
...
def test_manifest_loads_real_native_provider(runtime):
    path = (
        ...
        / "runtime/gideon/extensions/apps/native/gideon-personal-knowledge/app.json"
    )
...
def test_native_video_review_import_and_exact_transcript(runtime):
    ...
    imported = invoke(
        ...
        {
            "request_id": "native-video-request",
            **body,
...
def test_native_external_vault_register_scan_read_search_and_atomic_write(
    ...
    provider._home = home
    registered = invoke(
        provider, "knowledge_vault_register", {"name": "Native", "path": str(vault)}
    )
    ...
    opened = invoke(
        provider, "knowledge_vault_read", {"id": identity, "path": "Home.md"}
    )
    note = json.loads(opened.output)
    assert note["wikilinks"] == ["Other"]
    searched = invoke(
        provider, "knowledge_vault_search", {"id": identity, "query": "searchable"}
    )
    ...
    saved = invoke(
        ...
        {
            "id": identity,
            "request_id": "native-vault-write",
            "path": "Home.md",
            "content": "Native updated",
            "expected_hash": note["current_hash"],
        },
    )
    assert saved.success
...
def test_native_video_schemas_refuse_incomplete_or_extra_arguments(runtime, name, args):
    ...
    table = runtime.state.knowledge_store.db.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='capability_knowledge_video_jobs'"
    ).fetchone()
    assert (
        table is None
        or runtime.state.knowledge_store.db.execute(
...
def test_native_anniversary_and_exact_source(runtime):
    ...
    identity = store.create_typed_item(
        item_type="journal", title="A past day", content="Actual historical journal"
    )
    store.db.execute(
        "UPDATE items SET created_at=? WHERE id=?", ("2024-09-25T23:30:00Z", identity)
    )
    store.db.commit()
    provider = create_provider()
    result = invoke(
    ...
    source = invoke(
        provider,
        "knowledge_anniversary_source",
        {"source_type": "journal", "source_id": identity},
    )
    ...
    missing = invoke(
        provider,
        "knowledge_anniversary_source",
        {"source_type": "note", "source_id": identity},
    )
...
def test_native_capture_route_replay_and_source_identity(runtime):
    ...
    capture = invoke(
        provider,
        "knowledge_capture_text",
        {"request_id": "native-capture-01", "text": "Actual original capture"},
    )
    ...
    assert (
        runtime.state.knowledge_store.get_item(saved["destination_id"])["content"]
        == "Reviewed original"
    )
    assert (
        runtime.state.knowledge_store.db.execute(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "knowledge_capture_text",
            {"request_id": "native-invalid", "text": "x", "model": "other"},
        ),
...
def test_temporary_session_cannot_read_or_write(runtime):
    ...
    try:
        ...
        write = invoke(
            provider,
            "knowledge_capture_text",
            {"request_id": "temporary-native", "text": "Must not persist"},
        )
...
def test_incognito_session_can_read_but_cannot_write(runtime):
    ...
    try:
        ...
        write = invoke(
            provider,
            "knowledge_capture_text",
            {"request_id": "incognito-native", "text": "Must not persist"},
        )
        assert read.success
...
def test_actual_chat_session_modes_are_enforced(runtime):
    runtime.state._sessions["tool-test"] = _ChatSession(
        "tool-test", memory_mode="temporary"
    )
    provider = create_provider()
    ...
    denied = invoke(
        provider,
        "knowledge_capture_text",
        {"request_id": "chat-mode-native", "text": "No write"},
    )
    ...
    assert invoke(
        provider,
        "knowledge_capture_text",
        {"request_id": "chat-mode-native", "text": "Allowed write"},
    ).success


def test_bound_service_store_survives_registry_and_home_changes(
    runtime, tmp_path, monkeypatch
):
    provider = create_provider()
    ...
    set_action_services(
        ActionServices(state=other_state, spawn_background=asyncio.create_task)
    )
    ...
    result = invoke(
        provider,
        "knowledge_capture_text",
        {"request_id": "bound-store-native", "text": "Stay in the original store"},
    )
    assert not result.success
...
def test_provider_redacts_output_without_rewriting_original(runtime):
    ...
    result = invoke(
        create_provider(),
        "knowledge_capture_text",
        {"request_id": "secret-native-capture", "text": original},
    )
    assert result.success
    assert secret not in result.output
    identity = json.loads(result.output)["id"]
    stored = runtime.state.knowledge_store.db.execute(
        "SELECT original_text FROM capability_knowledge_captures WHERE id=?",
        (identity,),
    ).fetchone()
    assert stored["original_text"] == original


def test_voice_unavailable_result_keeps_original_receipt(
    runtime, tmp_path, monkeypatch
):
    import io
    import wave
