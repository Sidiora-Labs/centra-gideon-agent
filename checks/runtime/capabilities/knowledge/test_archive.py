...
def record(identity="conversation-one"):
    return {
        "id": identity,
        "title": "Observatory conversation",
        ...
        "mapping": {
            ...
            "alternative": node(
                ...
                "user",
                "assistant",
                "Try binoculars under clear skies.",
                1700000120,
            ),
        },
    }
...
def body(records=None):
    return {
        "format": "chatgpt",
        "content": json.dumps(
            records if records is not None else [record()], ensure_ascii=False, indent=2
        ),
    }
...
def payload(service, data=None, key="archive-request-001", selected=None):
    ...
    return {
        **data,
        ...
        or [row["id"] for row in preview["conversations"] if row["valid"]],
    }
...
def test_real_canonical_notes_raw_source_and_search(archive):
    source = record()
    source["mapping"]["answer"]["message"]["content"]["parts"].append(
        {
            "content_type": "image_asset_pointer",
            "asset_pointer": "missing-image-reference",
        }
    )
    ...
    assert target["content"].index("observe Jupiter") < target["content"].index(
        "Use the observatory"
    )
    ...
    assert json.loads(archive.original(original["id"]))[0]["custom_metadata"] == {
        "retained": True
    }
    assert any(
        row["id"] == target["id"] for row in archive.store.search_items_fts("telescope")
    )
...
def test_replay_and_whitespace_archive_deduplicate_sources(archive):
    ...
    reformatted = {
        "format": "chatgpt",
        "content": json.dumps(json.loads(data["content"])),
    }
    ...
    assert (
        archive.db.execute(
            'SELECT count(*) FROM items WHERE item_type="note"'
        ).fetchone()[0]
        == 1
    )
    assert (
        archive.preview(reformatted)["conversations"][0]["existing_id"]
...
def test_updated_source_version_retains_previous_note(archive):
    ...
    revised = record()
    revised["mapping"]["answer"]["message"]["content"]["parts"] = [
        "Use a larger telescope after sunset."
    ]
    ...
    assert json.loads(archive.original(first["source_item_id"]))[0]["mapping"][
        "answer"
    ]["message"]["content"]["parts"] == ["Use the observatory telescope."]
...
def test_reviewed_selection_imports_only_chosen_and_keeps_full_source(archive):
    ...
    assert (
        archive.preview(data)["conversations"][1]["existing_id"]
        == receipt["items"][0]["destination_id"]
    )
    assert (
        archive.db.execute(
            'SELECT count(*) FROM items WHERE item_type="note"'
...
@pytest.mark.parametrize(
    "mutation",
    ["cycle", "missing_parent", "bad_parts", "bad_timestamp", "empty", "no_messages"],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"format": "chatgpt", "content": "{}"},
        {"format": "chatgpt", "content": "[NaN]"},
        {"format": "chatgpt", "content": "["},
        {"format": "chatgpt", "content": "x" * (MAX_BYTES + 1)},
        body([record(), record()]),
    ],
)
...
def test_native_import_invokes_real_store_and_session_policy(archive):
    ...
    try:
        state._sessions["archive-native"] = _ChatSession(
            "archive-native", memory_mode="temporary"
        )
        ...
        request = {
            **body(),
            "request_id": "native-archive-import",
            "source_digest": json.loads(reviewed.output)["source_digest"],
        ...
        assert (
            archive.store.get_item(receipt["items"][0]["destination_id"])["item_type"]
            == "note"
        )
        assert (
            json.loads(
                asyncio.run(provider.invoke("knowledge_archive_list", {})).output
            )["total"]
            == 1
        )
        rejected = asyncio.run(
            provider.invoke("knowledge_archive_commit", request | {"runtime": "other"})
        )
...
def test_http_original_bytes_selection_and_isolation(archive, tmp_path):
    async def journey():
        ...
        async with (
            ...
            request = {
                **body(),
                "request_id": "http-archive-import",
                "source_digest": preview["source_digest"],
                "conversation_ids": ["conversation-one"],
            }
            ...
            assert (
                await isolated.get(root + "/sources/" + receipt["source_item_id"])
            ).status == 404
            ...
            assert (
                await client.post(
                    root + "/commit", json=request | {"model": "override"}
                )
            ).status == 400
            assert (
                await client.get(
                    root + "/sources/" + receipt["source_item_id"] + "?runtime=other"
                )
            ).status == 400
        other_store.close()
