...
def save(service, **changes):
    return service.save({'request_id': 'topic-original-save', 'name': 'Observatory', 'query': 'observatory', 'source_types': list(SOURCE_TYPES), **changes})
...
def test_topic_mutations_are_revisioned_and_idempotent(topics):
    ...
    second = save(topics, request_id='topic-edited-save', id=first['id'], revision=1, name='Telescope visits', query='telescope visit', source_types=['note', 'person'])
    ...
    assert error.value.status == 409
    deleted = topics.delete(first['id'], {'request_id': 'topic-delete-once', 'revision': 2})
    assert deleted == {'id': first['id'], 'deleted': True}
    assert topics.delete(first['id'], {'request_id': 'topic-delete-once', 'revision': 2}) == deleted
...
def test_live_matches_use_all_seven_actual_canonical_sources(topics):
    ...
    for kind in ('note', 'journal', 'fleeting'):
        identities[kind] = topics.store.create_typed_item(item_type=kind, title='Observatory ' + kind, content='Visit the telescope tonight.')
    people = PeopleStore(topics.home / 'capabilities/communications')
    person = people.save({'name': 'Observatory guide', 'notes': 'Call before the visit'})
    identities['person'] = person['id']
    project = BoundHierarchy(topics.home).create_project('Observatory expedition', brief='Arrange a visit')
    identities['project'] = project.id
    task = asyncio.run(BoundTasks(topics.home).create_task(title='Observatory booking', description='Reserve telescope time'))
    identities['task'] = task.id
    archive = SemanticArchive(db_path=topics.home / 'memory.db')
    archive.init()
    try:
        assert archive.write_episodic('We visited the mountain observatory together last winter.')
        ...
        assert {row['source_type']: row['source_id'] for row in result['items']} == identities
        ...
        assert {row['source_type'] for row in refreshed['items']} == {'fleeting', 'person', 'project', 'task'}
...
def test_query_all_terms_casefold_and_scanned_bounds(topics):
    first = topics.store.create_typed_item(item_type='note', title='OBSERVATORY plans', content='Book the telescope.')
    topics.store.create_typed_item(item_type='note', title='Observatory schedule', content='No equipment noted.')
    topics.store.create_typed_item(item_type='note', title='Elsewhere', content='Telescope maintenance.')
...
def test_empty_sources_do_not_create_canonical_domains(topics):
    ...
    assert sorted(path.relative_to(topics.home) for path in topics.home.rglob('*')) == before
...
def test_project_topic_reads_do_not_create_builtin_defaults(topics):
    ...
    assert sorted(path.relative_to(topics.home) for path in topics.home.rglob('*')) == paths
    assert len(BoundHierarchy(topics.home)._all_projects_raw()) == 1


def test_home_drift_reads_stay_bound_and_mutations_fail_without_artifacts(topics, monkeypatch, tmp_path):
    identity = topics.store.create_typed_item(item_type='note', title='Observatory memory', content='Original allocation')
    ...
    assert topics.delete(topic['id'], {'request_id': 'drift-topic-delete', 'revision': 1})['deleted']
    assert topics.store.get_item(identity)['content'] == 'Original allocation'


@pytest.mark.parametrize('change', [{'source_types': []}, {'source_types': ['note', 'note']}, {'source_types': ['runtime']}, {'source_types': [None]}, {'query': '---'}, {'name': ''}, {'query': 'x' * 301}, {'id': 'without-revision'}, {'id': [], 'revision': 1}, {'id': None, 'revision': 1}, {'id': '', 'revision': 1}, {'home': '/tmp/other'}])
def test_invalid_topic_shapes_have_no_persistence(topics, change):
    with pytest.raises(CaptureError):
        save(topics, **change)
    assert topics.list()['total'] == 0
    assert topics.db.execute('SELECT count(*) FROM capability_knowledge_topic_mutations').fetchone()[0] == 0


def test_pagination_and_delete_revision_preserve_sources(topics):
    source = topics.store.create_typed_item(item_type='note', title='Observatory source')
    created = [save(topics, request_id=f'topic-page-{index}', name=f'Topic {index}', source_types=['note']) for index in range(3)]
    ...
    with pytest.raises(CaptureError) as error:
        topics.delete(created[0]['id'], {'request_id': 'stale-topic-delete', 'revision': 2})
    assert error.value.status == 409
...
def test_native_topics_read_and_write_actual_data_with_session_guards(topics):
    source = topics.store.create_typed_item(item_type='note', title='Observatory native source')
    ...
    try:
        arguments = {'request_id': 'native-topic-save', 'name': 'Native topic', 'query': 'observatory', 'source_types': ['note']}
        state._sessions['topic-native'] = _ChatSession('topic-native', memory_mode='incognito')
        ...
        result = asyncio.run(provider.invoke('knowledge_topic_matches', {'id': topic['id']}))
        ...
        state._sessions['topic-native'].memory_mode = 'persistent'
        deleted = asyncio.run(provider.invoke('knowledge_topic_delete', {'id': topic['id'], 'revision': 1, 'request_id': 'native-topic-delete'}))
...
def test_real_http_topic_refresh_isolation_and_delete(topics, tmp_path):
    async def journey():
        ...
        register(other)
        secret = 'sk-' + 'a' * 48
        source = topics.store.create_typed_item(item_type='note', title='Observatory HTTP source', content='Credential ' + secret)
        ...
        async with TestClient(TestServer(app)) as client, TestClient(TestServer(other)) as isolated:
            body = {'request_id': 'http-topic-save', 'name': 'HTTP topic', 'query': 'observatory', 'source_types': ['note', 'memory']}
            ...
            assert (await isolated.get(root + '/' + topic['id'] + '/matches')).status == 404
            topics.store.update_item(source, title='Removed keyword')
            assert (await (await client.get(root + '/' + topic['id'] + '/matches')).json())['total'] == 0
            for query in ('?home=/tmp/other', '?model=x', '?limit=1&limit=2'):
                assert (await client.get(root + query)).status == 400
            assert (await client.post(root, json=body | {'runtime': 'other'})).status == 400
            response = await client.delete(root + '/' + topic['id'], json={'request_id': 'http-topic-delete', 'revision': 1})
            assert response.status == 200
...
def test_source_scan_bound_is_reported_without_false_total(topics):
    for index in range(1001):
        topics.store.create_typed_item(item_type='note', title=f'Observatory record {index}')
    topic = save(topics, source_types=['note'])
