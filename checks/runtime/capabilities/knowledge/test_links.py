...
@pytest.mark.parametrize(
    ...
    [
        ...
        "https://github.com/a/b/c/d/e/f/g/h/i/j/k",
    ],
)
...
def test_bucket_is_real_manual_collection_with_ordered_links(service):
    ...
    assert [row["id"] for row in service.store.resolve_collection(created["id"])] == [
        second["id"],
        first["id"],
    ]
...
def test_link_identity_deduplicates_across_buckets(service):
    ...
    assert {row["id"] for row in service.store.collections_for_item(first["id"])} == {
        one["id"],
        two["id"],
    }
    assert (
        service.db.execute(
            "SELECT count(*) FROM items WHERE guid LIKE 'link:%'"
        ).fetchone()[0]
        == 1
    )
...
def test_link_order_survives_service_reconstruction(service):
    ...
    assert [
        row["id"]
        for row in service.reorder_links(created["id"], [second["id"], first["id"]])
    ] == [second["id"], first["id"]]
    reopened = LinkStudies(service.store, service.home)
    assert [row["id"] for row in reopened.links(created["id"])] == [
        second["id"],
        first["id"],
    ]
...
def test_bucket_order_is_exact_and_durable(service):
    ...
    assert [
        row["id"] for row in LinkStudies(service.store, service.home).buckets()
    ] == [second["id"], first["id"]]
...
def test_deleting_bucket_preserves_canonical_bookmarks(service):
    ...
    assert (
        service.db.execute(
            "SELECT count(*) FROM capability_knowledge_link_order"
        ).fetchone()[0]
        == 0
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"url": "https://example.com", "title": "", "bucket_id": "x"},
        {"url": "https://example.com", "title": "x", "bucket_id": "x", "home": "/tmp"},
    ],
)
...
def test_study_refuses_missing_empty_and_oversized_checkout(tmp_path):
    ...
    huge = tmp_path / "huge"
    huge.mkdir()
    (huge / "large.bin").write_bytes(b"x" * 4194305)
    with pytest.raises(CaptureError, match="no readable"):
...
def test_repository_list_is_newest_first_and_retains_exact_stage(service, tmp_path):
    ...
    service.db.execute(
        "UPDATE capability_knowledge_repo_jobs SET id='repo-job-two',request_id='request-two',created_at='2026-02-01' WHERE id=?",
        (first,),
    )
    service.db.execute(
        "UPDATE capability_knowledge_repo_events SET job_id='repo-job-two' WHERE job_id=?",
        (first,),
    )
    service.db.execute(
        "INSERT INTO capability_knowledge_repo_jobs(id,request_id,payload,bookmark_id,url,status,stage,revision,checkout_path,created_at,updated_at) SELECT 'repo-job-old','request-old',payload,bookmark_id,url,'failed','fetching','', '', '2026-01-01','2026-01-01' FROM capability_knowledge_repo_jobs WHERE id='repo-job-two'"
    )
    service.db.commit()
...
def landed_repo(service, tmp_path):
    ...
    bookmark = service.store.create_typed_item(
        item_type="bookmark",
        title="example/small",
        content="",
        guid="repo:github.com:example/small",
        extra={"url": url},
    )
    identity = "repo-job"
    path = tmp_path / "landed"
    path.mkdir()
    (path / "README.md").write_text("# Landed\nActual files")
    service.db.execute(
        "INSERT INTO capability_knowledge_repo_jobs(id,request_id,payload,bookmark_id,url,status,stage,revision,checkout_path,created_at,updated_at) VALUES (?,?,?,?,?,'scanning','scanning',?,?,?,?)",
        (
            identity,
            "request-landed",
            "{}",
...
def test_repository_report_is_canonical_knowledge_note_and_refreshes(service, tmp_path):
    ...
    assert (
        service.db.execute(
            "SELECT count(*) FROM items WHERE guid LIKE 'repo-report:%'"
        ).fetchone()[0]
        == 1
    )
...
async def test_intake_rejects_invalid_repository_before_job_or_write(service):
    with pytest.raises(CaptureError, match="GitHub or GitLab"):
        await service.intake(
            {"request_id": "bad-repo", "url": "https://example.com/a/b"}, "dashboard:ui"
        )
    assert (
        service.db.execute(
            "SELECT count(*) FROM capability_knowledge_repo_jobs"
        ).fetchone()[0]
        == 0
    )
...
@pytest.mark.asyncio
async def test_intake_request_shape_and_home_binding_fail_closed(
    ...
    with pytest.raises(CaptureError, match="Runtime home changed"):
        await service.intake(
            {"request_id": "bound-repo", "url": "https://github.com/a/b"},
            "dashboard:ui",
        )
    assert (
        service.db.execute(
...
@pytest.mark.asyncio
async def test_http_bucket_link_order_delete_and_repository_guards(
    ...
    try:
        created = await (
            await client.post(
                ...
                json={"name": "HTTP", "icon": "link"},
                headers=headers,
            )
        ).json()
        first = await (
            await client.post(
                "/api/capabilities/knowledge/links/links",
                json={
        ...
        reordered = await (
            await client.post(
                f"/api/capabilities/knowledge/links/buckets/{created['id']}/links-reorder",
        ...
        listing = await (
            await client.get(
                "/api/capabilities/knowledge/links/buckets", headers=headers
            )
        ).json()
        assert listing["buckets"][0]["links"][0]["title"] == "Second"
        deleted = await (
            await client.delete(
                f"/api/capabilities/knowledge/links/buckets/{created['id']}/bucket",
                headers=headers,
            )
        ).json()
        ...
        assert (
            await client.get(
                "/api/capabilities/knowledge/links/buckets?home=/tmp", headers=headers
            )
        ).status == 400
    finally:
        await client.close()
