    ...
    preview = service.preview(
        {
            ...
            "fields": {
                ...
                "content": "Canonical content from reviewed capture",
            },
        }
    )
    return {
...
def test_eight_connections_replay_one_canonical_idea_and_receipt(typed):
    ...
    assert (
        typed.store.get_item(destination)["content"]
        == "Canonical content from reviewed capture"
    )
    rows = typed.db.execute(
        "SELECT id FROM items WHERE guid=?", ("typed_capture:" + payload["capture_id"],)
    ).fetchall()
    assert [row[0] for row in rows] == [destination]
    assert typed.list()["total"] == 1
    assert (
        typed.inbox.get(payload["capture_id"])["text"]
        == "Original immutable idea source"
    )
    assert (
...
def test_legacy_duplicates_remain_readable_without_destructive_migration(typed):
    ...
    typed.db.commit()
    duplicate = typed.store.create_typed_item(
        item_type="fleeting",
        title="Second historical record",
        content="Different retained original content",
        guid=original["guid"],
    )
    ...
    assert (
        reloaded.store.get_item(duplicate)["content"]
        == "Different retained original content"
    )
    assert (
        reloaded.db.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='typed_capture_guid'"
        ).fetchone()
        is None
    )
    ...
    assert (
        reloaded.db.execute(
            "SELECT count(*) FROM capability_knowledge_types"
        ).fetchone()[0]
        == 1
    )
    assert (
        reloaded.inbox.get(pending["capture_id"])["text"]
        == "Original immutable idea source"
    )
...
def test_unique_index_rejects_direct_canonical_duplicate_without_rewriting(typed):
    ...
    try:
        duplicate = second.create_typed_item(
            item_type="fleeting",
            title="Unauthorized duplicate identity",
            content="Must not replace canonical original",
            guid=original["guid"],
        )
        ...
        assert (
            asyncio.run(TypedCapture(second, home=typed.home).commit(payload))
            == receipt
        )
    finally:
        second.close()
...
def test_explicit_legacy_resolution_allows_index_and_original_retry(typed):
    typed.db.execute("DROP INDEX typed_capture_guid")
    typed.db.commit()
    first = typed.store.create_typed_item(
        item_type="fleeting",
        title="Keep this original",
        guid="typed_capture:legacy-pair",
    )
    second = typed.store.create_typed_item(
        item_type="fleeting",
        title="User-selected duplicate",
        guid="typed_capture:legacy-pair",
    )
