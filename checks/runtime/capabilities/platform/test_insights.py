...
def weight(home, value=80, day="2026-09-01", request="weight-a"):
    return MeasurementStore(home).create(
        dict(
            request_id=request,
            kind="body_weight",
...
def goal(home):
    ...
    store.configure(
        ...
        horizon="short_term",
        milestones=[
            dict(id="m1", title="First session", done=True, target_date=None),
            dict(id="m2", title="Second session", done=False, target_date=None),
        ],
        links=[],
        unit="sessions",
        target_value=10,
...
def labs(home):
    ...
    return store.commit(
        {**payload, "preview_id": preview["preview_id"], "request_id": "labs"}
    )
...
def test_goal_metrics_are_actual_human_observations_and_milestones(home):
    ...
    assert result["milestones_complete_ratio"] == 0.5
...
def test_measurement_correction_updates_projection_and_preserves_saved_source(home):
    ...
    saved = insights.save(
        {"fingerprint": initial["fingerprint"], "note": "My observation"}
    )
    store = MeasurementStore(home)
    store.correct(
        second["id"], dict(request_id="correction", revision=1, values={"weight": 79})
    )
    ...
    assert (
        historical["document"]["snapshot"]["rows"][0]["observations"][0]["id"]
        == first["id"]
    )
...
def test_lab_series_separate_units_and_link_immutable_original(home):
    ...
    assert (
        by_unit["ng/mL"]["observations"][0]["artifact"]
...
def test_artifact_versions_conflicts_and_foreign_artifact_protection(home):
    ...
    assert (
        insights.save({"fingerprint": data["fingerprint"], "note": "First"})["slug"]
        == saved["slug"]
    )
...
@pytest.mark.parametrize(
    "payload", [{"note": 22}, {"note": "x" * 8001}, {"unrecognized": 1}, []]
)
...
async def test_real_authenticated_http_and_native_read_same_projection(home):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        headers = {
            "Cookie": "gideon_token_8000=" + generate_token("dashboard:insights")
        }
        ...
        saved = await client.post(
            PREFIX,
            headers=headers,
            json={"fingerprint": data["fingerprint"], "note": "HTTP saved"},
        )
        assert saved.status == 200
        document = await saved.json()
        historical = await client.get(
            PREFIX, headers=headers, params={"slug": document["slug"], "version": 1}
        )
        assert (await historical.json())["document"]["note"] == "HTTP saved"
        stale = await client.post(
            PREFIX, headers=headers, json={"fingerprint": "old", "note": "invalid"}
        )
        assert stale.status == 409
        missing = await client.get(PREFIX, headers=headers, params={"slug": "missing"})
        assert missing.status == 404
        invalid = await client.get(
            PREFIX, headers=headers, params={"slug": document["slug"], "version": "bad"}
        )
        assert invalid.status == 400
    invalid_tool = await create_provider().invoke(
        "platform_personal_scorecard", {"private_traits": True}
    )
