...
def project(tmp_path, monkeypatch):
    ...
    workspace.mkdir()
    (workspace / "original.txt").write_text(
        "Original project remains the source of truth"
    )
    return HierarchyStore().create_project(
        "Maintenance project", workspace_dir=str(workspace)
    )
...
async def definition(name="code-project", wait=False):
    ...
    return await provider.save_def(
        ...
        root={
            ...
            "id": "actual",
            "config": {"duration_secs": 300} if wait else {"expr": "done"},
        },
    )
...
async def test_complete_seven_real_workflows_and_persisted_order(project):
    ...
    try:
        ...
        assert (
            (Path(project.workspace_dir) / "original.txt")
            .read_text()
            .startswith("Original")
        )
...
async def test_real_issue_drain_no_progress_and_resume_after_actual_cancellation(
    ...
    try:
        ...
        assert [item["stage"] for item in row["history"]] == [
            m.AUDITS[0],
            "drain",
            "drain",
            "drain",
...
async def test_validation_missing_workflow_duplicate_and_cancel_before_launch(project):
    ...
    try:
        ...
        for invalid in (
            {},
            body(project, verify_command=""),
            body(project, guard_command=""),
            body(project, extra="value"),
        ):
    ...
    try:
        async with TestClient(TestServer(app)) as client:
            ...
            response = await client.get(
                PREFIX, params={"token": generate_token("maintenance-owner")}
            )
            ...
            try:
                controlled = await provider.invoke(
                    "platform_maintenance_control",
                    {"id": row["id"], "action": "cancel"},
                )
                assert controlled.success
                assert json.loads(controlled.output)["status"] in {
                    "cancelled",
                    "cancelling",
                }
            finally:
            ...
            tool = next(
                ...
                if item.name == "platform_maintenance_control"
            )
            ...
            assert (
                await client.post(
                    PREFIX + "/" + row["id"], json={"action": "unsupported"}
                )
            ).status == 409
            assert (
                await client.post(
