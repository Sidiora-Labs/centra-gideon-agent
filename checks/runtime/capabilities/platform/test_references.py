...
async def authenticate(client):
    response = await client.get(
        PREFIX, params={"token": generate_token("reference-owner")}
    )
...
async def track(client):
    response = await client.post(
        PREFIX, json={"name": "Upstream", "path": "reference", "branch": "main"}
    )
...
async def test_real_git_fetch_check_review_and_restart(repository):
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        stale_review = await client.post(
            f"{PREFIX}/{identifier}/review", json={"head": first}
        )
        assert stale_review.status == 409
        reviewed = await client.post(
            f"{PREFIX}/{identifier}/review", json={"head": second}
        )
    ...
    async with TestClient(TestServer(application())) as restarted:
        ...
        definition = next(
            ...
            if tool.name == "platform_reference_repositories"
        )
...
@pytest.mark.asyncio
async def test_failed_fetch_preserves_timestamped_snapshot_and_prevents_review(
    repository,
):
    workspace, upstream, first = repository
    async with TestClient(TestServer(application())) as client:
...
async def test_invalid_paths_and_refs_do_not_create_records(repository, changes):
    async with TestClient(TestServer(application())) as client:
        ...
        response = await client.post(
            PREFIX,
            json={"name": "Upstream", "path": "reference", "branch": "main", **changes},
        )
...
async def test_duplicate_scope_auth_and_corrupt_state(repository):
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        escaped = await client.post(
            PREFIX, json={"name": "Escape", "path": "escaped", "branch": "main"}
        )
        assert escaped.status == 400
        row = await track(client)
        duplicate = await client.post(
            PREFIX, json={"name": "Again", "path": "reference", "branch": "main"}
        )
...
async def test_git_metadata_escape_is_rejected(repository):
    ...
    async with TestClient(TestServer(application())) as client:
        await authenticate(client)
        response = await client.post(
            PREFIX, json={"name": "Escape", "path": "reference", "branch": "main"}
        )
        assert response.status == 400
