...
async def authenticate(client):
    response = await client.get(
        PREFIX, params={"token": generate_token("harness-owner")}
    )
...
async def test_actual_package_install_handshake_update_remove_and_provenance():
    async with TestClient(TestServer(application())) as client:
        ...
        response = await client.post(
            PREFIX + "/codex", json={"action": "install", "version": "1.13.0"}
        )
        ...
        assert (
            ledger[installed["package"]]["integrity"]
        ...
        try:
            message = {
                ...
                "params": {
                    "protocolVersion": 1,
                    "clientCapabilities": {},
                    "clientInfo": {"name": "gideon-lifecycle-check", "version": "1"},
                },
            }
        ...
        stale = await client.post(
            PREFIX + "/codex",
            json={"action": "update", "version": "1.13.1", "expected_version": "0.0.0"},
        )
        ...
        assert (
            json.loads(runners.adapter_lock_path().read_text())[installed["package"]][
                "version"
            ]
            == "1.13.1"
        )
        get_default_registry().register_entry(
            ProviderEntry(
                ...
                type="acp_agent",
                model="",
                options={"command": [str(binary)]},
            )
        )
        blocked = await client.post(
            PREFIX + "/codex", json={"action": "remove", "expected_version": "1.13.1"}
        )
        ...
        assert installed["package"] not in json.loads(
            runners.adapter_lock_path().read_text()
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "codex",
            {"action": "install", "version": "1.0.0; touch hacked"},
            400,
            "exact",
        ),
        ...
        (
            "codex",
            {"action": "install", "version": "1.13.1", "package": "untrusted"},
            400,
            "Invalid",
        ),
        ("codex", [], 400, "Invalid"),
...
async def test_invalid_operations_never_mutate_adapter_directory(
    identifier, body, status, text
):
    async with TestClient(TestServer(application())) as client:
...
async def test_auth_scope_catalog_truth_and_real_native_inventory():
    async with TestClient(TestServer(application())) as client:
        ...
        app = await client.get(
            PREFIX, params={"token": generate_token("owner", app="external")}
        )
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        tool = next(
            tool for tool in definitions if tool.name == "platform_harness_inventory"
        )
        ...
        invalid = await provider.invoke(
            "platform_harness_inventory", {"command": "install"}
        )
