...
def pair(
    first_home, second_home, enabled=True, second_endpoint="https://second.example"
):
    ...
    second.put(
        first_id["peer_id"], peer_record(first_id, "https://first.example", enabled)
    )
    return first_id, second_id


def test_projection_is_exact_semantic_only_with_tombstones_and_no_derived_state(
    tmp_path,
):
    home = tmp_path / "home"
    ...
    semantic(
        home,
        "project.gideon.status",
        {"phase": "source"},
        updated="2026-09-25T08:05:00+00:00",
        deleted=True,
    )
    store = archive(home)
    try:
        assert (
            store.set_semantic(
                "lesson.private", "do not replicate", 1.0, "user_explicit"
            )
            is None
        )
        assert (
            store.set_semantic(
                "user.persona.operator", "private persona", 1.0, "user_explicit"
            )
            is None
        )
        store.db.execute(
...
async def test_signed_two_home_route_conflict_restore_and_exact_tombstone(tmp_path):
    ...
    async with TestServer(application) as server:
        source_id, target_id = pair(
            source, target, second_endpoint=str(server.make_url("/"))
        )
    ...
    semantic(
        ...
        "project.shared.fact",
        "local",
        updated="2026-09-25T08:10:00+00:00",
        category="local",
    )
    semantic(
        source,
        "project.shared.fact",
        "peer",
        updated="2026-09-25T08:15:00+00:00",
        category="peer",
    )
    semantic(
        source,
    ...
    incoming = ReplicationService(source).export_batch(
        target_id["peer_id"], adapter.SCOPE
    )
    ...
    pending = conflicts.ConflictQueue(target).items(
...
def test_policy_and_complete_preflight_reject_private_authority_and_derived_shapes(
    tmp_path,
):
    ...
    value["data"]["id"] = "user.persona.private"
    value["id"] = value["data"]["id"]
    invalid.append(value)
    value = copy.deepcopy(canonical)
    value["data"]["credential_ref"] = "SECRET"
    invalid.append(value)
    value = copy.deepcopy(canonical)
    value["data"]["value"] = {"access_token": "token-value"}
    invalid.append(value)
    value = copy.deepcopy(canonical)
    value["data"]["value"] = "operator@example.test"
    invalid.append(value)
    value = copy.deepcopy(canonical)
    value["data"]["embedding"] = [0.1]
    invalid.append(value)
    value = copy.deepcopy(canonical)
