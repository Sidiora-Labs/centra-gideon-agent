...
def store_at(home):
    ...
    return VideoStore(music, MusicCatalog(music, NativeArtifactProvider(root=home / "artifacts")))
...
def create_project(home, suffix="one"):
    ...
    audio = store.catalog.artifacts.create_binary(
        name="Actual recording " + suffix, data=wav_bytes(), mime="audio/wav", kind="audio", source="import",
    )
    track = store.catalog.attach(track["id"], {
        "revision": track["revision"],
        "artifact_ref": {"slug": audio.slug, "version": audio.version},
        "source": {"kind": "imported", "label": "Authored session", "license": "Test fixture"},
    })
    images = []
    for color in ("red", "blue"):
        ...
        artifact = store.catalog.artifacts.create_binary(
            name=f"{color} frame {suffix}", data=output.getvalue(), mime="image/png", kind="image", source="manual",
        )
...
def editable(project, **changes):
    result = {
        key: project[key]
        for key in ("title", "track_id", "render_id", "tempo_bpm", "offset_seconds", "revision")
    }
    result["scenes"] = [
...
def test_projection_preserves_pins_and_editable_grid_without_bytes_or_execution_authority(tmp_path):
    source = tmp_path / "source"
    target = tmp_path / "target"
    store, project, track, audio, images = create_project(source, "projection")
    with store._db() as database:
        job = {
            "id": "local-render", "status": "completed", "project_id": project["id"],
            "revision": project["revision"], "snapshot": project,
            "artifact_ref": {"slug": "local-output", "version": 1}, "error": None,
        }
        database.execute("INSERT INTO jobs VALUES(?,?,?)", ("local-render", "private-request", json.dumps(job)))
    ...
    outcome = adapter.apply_rows(
        target, adapter.ENTRY_ID, projected, {}, conflicts.ConflictQueue(target), "2026-09-25T12:00:00+00:00",
    )
    assert (outcome.added, outcome.updated, outcome.removed, outcome.conflicts) == (1, 0, 0, 0)
    ...
    with pytest.raises(DomainError) as unavailable:
        target_store.update(project["id"], editable(project, title="Needs local dependencies"))
    ...
    for image in images:
        assert target_store.catalog.artifacts.get(image["slug"], version=image["version"]) is None
...
def test_two_home_fast_forward_conflict_selected_restore_and_tombstone(tmp_path):
    ...
    initial = adapter.apply_rows(
        second, adapter.ENTRY_ID, initial_rows, {}, conflicts.ConflictQueue(second), "2026-09-25T13:00:00+00:00",
    )
    ancestor = initial.new_ancestors
    assert ancestor == {project["id"]: conflicts.row_sha(initial_rows[0])}
    changed = first_store.update(project["id"], editable(project, title="Local editorial cut"))
    forwarded = adapter.apply_rows(
        second, adapter.ENTRY_ID, rows(first), ancestor, conflicts.ConflictQueue(second), "2026-09-25T13:05:00+00:00",
    )
    ...
    replayed = adapter.apply_rows(
        second, adapter.ENTRY_ID, rows(first), forwarded.new_ancestors, conflicts.ConflictQueue(second),
    ...
    adapter.write_row(second, adapter.ENTRY_ID, {"id": project["id"], "data": remote}, project["id"])
    queue = conflicts.ConflictQueue(first)
    conflicted = adapter.apply_rows(
        first, adapter.ENTRY_ID, rows(second), forwarded.new_ancestors, queue, "2026-09-25T13:15:00+00:00",
    )
    assert (conflicted.added, conflicted.updated, conflicted.removed, conflicted.conflicts) == (0, 0, 0, 1)
    ...
    resolved = adapter.restore_fields(first, pending[0].id, ["title"], "2026-09-25T13:20:00+00:00")
    ...
    copied = adapter.apply_rows(
        tombstone_target, adapter.ENTRY_ID, baseline, {}, conflicts.ConflictQueue(tombstone_target),
        "2026-09-25T13:25:00+00:00",
    )
    adapter.write_row(tombstone_source, adapter.ENTRY_ID, None, doomed["id"])
    removed = adapter.apply_rows(
        tombstone_target, adapter.ENTRY_ID, [], copied.new_ancestors, conflicts.ConflictQueue(tombstone_target),
...
def test_complete_preflight_rejects_bad_pins_grids_private_fields_and_duplicates(tmp_path):
    source = tmp_path / "source"
    target = tmp_path / "target"
    _, _, _, _, _ = create_project(source, "validation")
    canonical = rows(source)[0]
    invalid = []
    value = copy.deepcopy(canonical); value["data"]["audio_ref"]["version"] = 0; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["audio_ref"]["slug"] = ""; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["scenes"][0]["image_ref"]["version"] = True; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["scenes"][0]["start_seconds"] = 1; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["scenes"][1]["duration_seconds"] = 2; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["duration_seconds"] = 2; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["tempo_bpm"] = True; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["tempo_bpm"] = 301; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["offset_seconds"] = float("inf"); invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["scenes"] = []; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["scenes"][1]["id"] = "opening"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["scenes"][0]["beats"] = 0; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["revision"] = 0; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["request_id"] = "private"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["audio_ref"]["credential"] = "secret"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["scenes"][0]["bytes"] = "png"; invalid.append(value)
    for row in invalid:
    ...
    with pytest.raises(ValueError):
        adapter.apply_rows(
            target, adapter.ENTRY_ID, [invalid[0]], {}, conflicts.ConflictQueue(target),
...
def test_writer_only_materializes_current_project_row(tmp_path):
    ...
    with sqlite3.connect(target / "capabilities/music/music_video.sqlite3") as database:
        stored = json.loads(database.execute("SELECT payload FROM projects WHERE id=?", (project["id"],)).fetchone()[0])
