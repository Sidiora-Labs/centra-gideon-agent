...
async def main():
    ...
    PeerStore().put(identity["peer_id"], {"label": "Local integration peer", "endpoint": f"http://127.0.0.1:{port}", "public_key": identity["public_key"], "enabled": True, "send_categories": ["workspace.records"], "receive_categories": ["workspace.records"], "revision": 0})
    HierarchyStore().create_project("Replicated project", brief="Canonical project source")
