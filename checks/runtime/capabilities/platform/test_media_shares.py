...
from gideon.workspace.capabilities.platform.media_shares import (
    MAX_BYTES,
    SCOPE,
    MediaShareError,
    MediaShares,
...
def png(color=(20, 80, 140)):
    target = io.BytesIO()
    Image.new("RGB", (2, 2), color).save(target, "PNG")
...
def pair(tmp_path):
    ...
    sender_identity, receiver_identity = identity(sender_peers), identity(
        receiver_peers
    )
    add_peer(receiver_peers, sender_identity, "http://127.0.0.1:1", receive=[SCOPE])
    return {
        "sender_home": sender_home,
        "receiver_home": receiver_home,
        "sender_peers": sender_peers,
        "receiver_peers": receiver_peers,
        "sender_artifacts": sender_artifacts,
...
def test_receive_imports_exact_canonical_bytes_and_minimal_receipt(pair):
    ...
    assert pair["receiver_artifacts"].raw_bytes(result["artifact_id"], version=1) == (
        data,
        "image/png",
    )
...
def test_receive_rejects_invalid_metadata_before_artifact_write(pair, patch, message):
    with pytest.raises(MediaShareError, match=message):
        pair["receiver"].receive(pair["sender_identity"]["peer_id"], offer(**patch))
...
def test_receive_rejects_encoding_size_and_hash_mismatch(pair):
    ...
    bad = offer()
    bad["artifact"]["data"] = "***"
    with pytest.raises(MediaShareError, match="encoding"):
        pair["receiver"].receive(sender, bad)
    bad = offer()
    bad["artifact"]["bytes"] += 1
    with pytest.raises(MediaShareError, match="bytes do not match"):
        pair["receiver"].receive(sender, bad)
    bad = offer()
...
def test_receive_requires_exact_opt_in_policy(pair):
    ...
    with pytest.raises(MediaShareError, match="denies inbound"):
...
def test_receive_revocation_deletes_only_shared_artifact_and_blocks_replay(pair):
    sender = pair["sender_identity"]["peer_id"]
    original = pair["receiver_artifacts"].create_binary(
        name="Local",
        slug="local-image",
        data=png((5, 5, 5)),
        mime="image/png",
    ...
    result = pair["receiver"].receive(
        sender,
        {"operation": "revoke", "share_id": "a" * 32, "sha256": active["sha256"]},
    )
    ...
    with pytest.raises(MediaShareError, match="cannot be replayed"):
        pair["receiver"].receive(sender, offer())


def test_receive_revoke_rejects_unknown_or_wrong_hash(pair):
    sender = pair["sender_identity"]["peer_id"]
    with pytest.raises(MediaShareError, match="not found"):
        pair["receiver"].receive(
            sender, {"operation": "revoke", "share_id": "a" * 32, "sha256": "0" * 64}
        )
    active = pair["receiver"].receive(sender, offer())
    with pytest.raises(MediaShareError, match="hash does not match"):
        pair["receiver"].receive(
            sender, {"operation": "revoke", "share_id": "a" * 32, "sha256": "0" * 64}
        )
...
async def test_two_real_runtimes_share_verify_replay_and_revoke_over_guarded_http(pair):
    ...
    async with TestServer(app) as server:
        add_peer(
            pair["sender_peers"],
            pair["receiver_identity"],
            str(server.make_url("")),
            send=[SCOPE],
        )
        data = png((200, 40, 60))
        artifact = pair["sender_artifacts"].create_binary(
            name="Exact red image",
            slug="red-image",
            data=data,
            mime="image/png",
            kind="image",
        ...
        body = {
            "request_id": "share-red-v1",
            "peer_id": pair["receiver_identity"]["peer_id"],
        ...
        remote = pair["receiver_artifacts"].raw_bytes(
            shared["remote_artifact_id"], version=1
        )
...
def test_outbound_source_requires_exact_version_kind_size_and_send_policy(pair):
    data = png()
    image = pair["sender_artifacts"].create_binary(
        name="Image", slug="image", data=data, mime="image/png", kind="image"
    )
    body = {
        "request_id": "one",
        "peer_id": pair["receiver_identity"]["peer_id"],
        "artifact_id": image.slug,
        "artifact_version": 1,
    }
    with pytest.raises(MediaShareError, match="denies media sharing"):
    ...
    with pytest.raises(MediaShareError, match="not found"):
        pair["sender"].source({**body, "artifact_version": 2})
    audio = pair["sender_artifacts"].create_binary(
        name="Audio", slug="audio", data=b"ID3audio", mime="audio/mpeg", kind="audio"
    )
    with pytest.raises(MediaShareError, match="not found"):
        pair["sender"].source({**body, "artifact_id": audio.slug})
...
async def test_http_dashboard_requires_auth_but_signed_receive_does_not(pair):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        malformed = await client.post(
            "/api/capabilities/platform/media-shares/receive", json={"payload": {}}
        )
...
def test_receiver_receipt_and_replay_survive_service_restart(pair):
    ...
    reopened = MediaShares(
        pair["receiver_home"], pair["receiver_peers"], pair["receiver_artifacts"]
    )
...
def test_receiver_accepts_supported_video_without_decoding_or_parallel_store(pair):
    ...
    assert pair["receiver_artifacts"].raw_bytes(received["artifact_id"], version=1) == (
        data,
        "video/mp4",
    )
    ...
    with pair["sender"].db() as db:
        ...
        db.execute(
            ...
            (
                "b" * 32,
                "stable-request",
                pair["receiver_identity"]["peer_id"],
                "image",
                1,
                "0" * 64,
                10,
