...
from gideon.integrations.llm.registry import ProviderRegistry, ProviderResolutionError, get_default_registry, set_default_registry, sync_entries_from_config
from gideon.interfaces.dashboard.handlers.capabilities_connections import register
from gideon.interfaces.dashboard.handlers.capabilities_platform import register as register_catalog
...
from gideon.workspace.capabilities.platform.connections import ConnectionError, document, effective_entry, projection


def register_types():
    register_branded_app(BrandedProviderSpec(type="openai-connection", protocol="openai", default_model="alpha"))
    register_branded_app(BrandedProviderSpec(type="declared-catalog", protocol="anthropic", fallback_models=({"id": "alpha"}, {"id": "beta"})))
...
def isolated_connections(tmp_path, monkeypatch):
    ...
    CredentialStore(config_dir()).save({"first": {"type": "api_key", "value": "test-first-private-value"}, "second": {"type": "api_key", "value": "test-second-private-value"}})
    config_path().write_text(json.dumps({"unrelated": {"keep": 7}, "providers": [{"name": "primary", "type": "openai-connection", "model": "alpha", "options": {"base_url": "https://original.invalid/v1"}}, {"name": "catalog", "type": "declared-catalog", "model": "alpha"}]}))
    register_types()
...
def body(revision=0, **changes):
    return {"revision": revision, "label": "Shared account", "base_url": "https://shared.invalid/v1", "credential_ref": "first", "model_access": {"mode": "all", "patterns": []}, **changes}


async def authenticate(client):
    response = await client.get(PREFIX, params={"token": generate_token("connection-owner")})
...
async def test_shared_connection_real_provider_construction_rotation_and_restart():
    async with TestClient(TestServer(application())) as client:
        ...
        for revision, name in [(1, "primary"), (2, "catalog")]:
            response = await client.put(PREFIX + f"/shared/bindings/{name}", json={"revision": revision, "bound": True})
            assert response.status == 200
        registry = get_default_registry()
        primary = registry.get_entry("primary")
        catalog = registry.get_entry("catalog")
        assert primary.options["base_url"] == catalog.options["base_url"] == "https://shared.invalid/v1"
        assert primary.credential == catalog.credential == "first"
        actual = registry.build("primary", credential_store=CredentialStore(config_dir()))
        ...
        updated = await client.put(PREFIX + "/shared", json=body(3, base_url="https://rotated.invalid/v1", credential_ref="second"))
        assert updated.status == 200
        second = registry.build("primary", credential_store=CredentialStore(config_dir()))
...
async def test_entitlement_catalog_scope_and_real_execution_guard():
    async with TestClient(TestServer(application())) as client:
        await authenticate(client)
        assert (await client.put(PREFIX + "/shared", json=body(model_access={"mode": "allow", "patterns": ["alpha"]}))).status == 200
        for revision, name in [(1, "primary"), (2, "catalog")]:
            assert (await client.put(PREFIX + f"/shared/bindings/{name}", json={"revision": revision, "bound": True})).status == 200
        ...
        with pytest.raises(ProviderResolutionError, match="excluded"):
            get_default_registry().build("primary", model="beta", credential_store=CredentialStore(config_dir()))
        assert (await client.put(PREFIX + "/shared", json=body(3, model_access={"mode": "deny", "patterns": ["alpha"]}))).status == 200
        with pytest.raises(ProviderResolutionError, match="excluded"):
            get_default_registry().build("primary", credential_store=CredentialStore(config_dir()))
...
async def test_conflicts_unbind_and_delete_preserve_original_provider():
    async with TestClient(TestServer(application())) as client:
        ...
        assert (await client.put(PREFIX + "/shared/bindings/primary", json={"revision": 1, "bound": True})).status == 200
        assert (await client.delete(PREFIX + "/shared?revision=2")).status == 409
        assert (await client.put(PREFIX + "/shared/bindings/primary", json={"revision": 1, "bound": False})).status == 409
        assert (await client.put(PREFIX + "/shared/bindings/primary", json={"revision": 2, "bound": False})).status == 200
...
@pytest.mark.asyncio
@pytest.mark.parametrize("changes", [{"base_url": "https://user:secret@example.invalid/v1"}, {"base_url": "https://example.invalid/v1?key=secret"}, {"base_url": "file:///etc/passwd"}, {"credential_ref": "missing"}, {"model_access": {"mode": "maybe"}}, {"model_access": {"mode": "allow", "patterns": [""]}}, {"revision": True}, {"label": ""}])
...
async def test_native_manifest_loads_and_real_tools_invoke_contracts():
    ...
    from gideon.integrations.tool_providers.registry import register_provider, unregister_provider, get_provider
    manifest = AppManifest.from_json_file(Path("runtime/gideon/extensions/apps/native/gideon-platform/app.json"))
    ...
    try:
        ...
        assert {tool.name for tool in definitions} == {"platform_api_catalog", "prompt_dependency_usage", "provider_connections_get"}
        ...
        usage = await provider.invoke("prompt_dependency_usage", {"name": "system-chat"})
