...
def archive(
    ...
    *,
    ...
    files.update(
        {
            f"brain/people/{row['id']}/index.json": json.dumps(row).encode()
            for row in records
        }
    )
    ...
    with tarfile.open(fileobj=output, mode="w:gz") as tar:
        payloads = {
            ...
            **{f"{root}/data/{name}": value for name, value in files.items()},
        }
        if mutate:
            mutate(tar, payloads)
        else:
...
def knowledge_archive(memories=None, links=None, *, schema=1):
    ...
    if memories is not None:
        files["brain/memories/index.json"] = json.dumps(
            {
                "schemaVersion": schema,
                "type": "memories",
        ...
        files.update(
            {
                f"brain/memories/{row['id']}/index.json": json.dumps(row).encode()
                for row in memories
            }
        )
    if links is not None:
        files["brain/links/index.json"] = json.dumps(
            {
                "schemaVersion": schema,
                "type": "links",
                "updatedAt": "2026-09-12T00:00:00Z",
        ...
        files.update(
            {
                f"brain/links/{row['id']}/index.json": json.dumps(row).encode()
                for row in links
            }
        )
    manifest = {
    ...
    with tarfile.open(fileobj=output, mode="w:gz") as tar:
        for name, value in {
            "snapshot-k/manifest.json": json.dumps(manifest).encode(),
            **{f"snapshot-k/data/{name}": value for name, value in files.items()},
        }.items():
            info = tarfile.TarInfo(name)
            info.size = len(value)
            tar.addfile(info, io.BytesIO(value))
...
def domains_archive(collections, *, schema=1):
    ...
    with tarfile.open(fileobj=output, mode="w:gz") as tar:
        for name, value in {
            "snapshot-domains/manifest.json": json.dumps(manifest).encode(),
            **{f"snapshot-domains/data/{name}": value for name, value in files.items()},
        }.items():
            info = tarfile.TarInfo(name)
            info.size = len(value)
            tar.addfile(info, io.BytesIO(value))
...
def song_archive(song, binaries=None, *, schema=1):
    ...
    with tarfile.open(fileobj=output, mode="w:gz") as tar:
        for name, value in {
            "snapshot-song/manifest.json": json.dumps(manifest).encode(),
            **{f"snapshot-song/data/{name}": value for name, value in files.items()},
        }.items():
            info = tarfile.TarInfo(name)
            info.size = len(value)
            tar.addfile(info, io.BytesIO(value))
...
def song_record(identity="song-1", attachments=None, **overrides):
    record = {
        ...
        "instrument": "guitar",
        "stage": "learning",
        "tags": ["setlist", "acoustic"],
        "key": "Am",
        "capo": 2,
        "tuning": "E A D G B E",
        "sourceUrl": "https://example.test/song",
        "links": [{"type": "project", "id": "tour-set", "label": "Tour set"}],
        "content": {
            "format": "chordpro",
            "text": "{title: Midnight Train}\n[Am]All aboard",
        },
        "notes": "Keep the bridge quiet",
        "scrollDurationSec": 90,
        "attachments": attachments or [],
        "practice": {
            "ease": 2.7,
            "intervalDays": 6,
            "nextReview": "2026-10-01T12:00:00+00:00",
            "lastReviewed": "2026-09-25T12:00:00+00:00",
            "sessions": 4,
            "lastQuality": 4,
        },
...
def project_task_records(
    project_id="40404040-4040-4040-8040-404040404040", task_id="thread-mixed", refs=None
):
    project = {
        "id": project_id,
        "name": "Mixed restore",
    ...
    thread = {
        "id": task_id,
        "title": "Coordinate restore",
        "status": "open",
        "priority": "high",
        "nextAction": "Verify canonical IDs",
        "notes": "Coordinated task",
        "tags": ["migration"],
        "pinned": False,
        "refs": (
            refs
            if refs is not None
            else [{"kind": "brain.project", "id": project_id, "label": "Mixed restore"}]
        ),
...
def multi_canonical_archive(*, broken_ref=False):
    ...
    first_thread = {
        ...
        "status": "open",
        "priority": "high",
        "nextAction": "Verify links",
        "notes": "Depends on project and admin",
        "tags": [],
        "pinned": False,
        "refs": [
            {
    ...
    source = domains_archive(
        {
            "projects": [project_two, project_one],
            "admin": [admin],
...
def test_preview_verifies_manifest_and_maps_explicit_coverage():
    ...
    assert result["coverage"] == {
        "supported": ["people"],
        "unsupported": "all other snapshot domains",
    }
    assert result["records"] == [
        {"source_id": "person-1", "domain": "people", "name": "Ada"}
    ]
...
def test_two_records_commit_in_one_canonical_transaction(tmp_path):
    ...
    assert [row["name"] for row in PeopleStore(tmp_path).people()] == [
        "Two",
        "One",
    ] or sorted(row["name"] for row in PeopleStore(tmp_path).people()) == ["One", "Two"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"format": FORMAT, "content": base64.b64encode(b"not-tar").decode()},
            "gzip tar",
        ),
    ],
)
...
def test_checksum_mismatch_fails_before_store_write(tmp_path):
    source = archive(
        manifest_patch=lambda value: value["files"].update(
            {"brain/people/person-1/index.json": "0" * 64}
        )
    )
...
def test_unmanifested_file_fails_closed():
    def add(_tar, payloads):
        ...
        for name, value in payloads.items():
            info = tarfile.TarInfo(name)
            info.size = len(value)
            _tar.addfile(info, io.BytesIO(value))
...
def test_link_entry_is_never_extracted_or_followed():
    def link(_tar, payloads):
        for name, value in payloads.items():
            ...
            info.size = len(value)
            _tar.addfile(info, io.BytesIO(value))
        info = tarfile.TarInfo("snapshot-1/data/link")
...
def test_traversal_entry_is_rejected():
    def traversal(_tar, payloads):
        for name, value in payloads.items():
            ...
            _tar.addfile(info, io.BytesIO(value))
        value = b"escape"
        info = tarfile.TarInfo("snapshot-1/../escape")
        info.size = len(value)
        _tar.addfile(info, io.BytesIO(value))
...
def test_record_directory_and_body_identity_must_match():
    ...
    with pytest.raises(MigrationError, match="identity"):
        preview(
            archive(
                [], extra={"brain/people/body-id/index.json": json.dumps(bad).encode()}
            )
        )
...
def test_preexisting_deterministic_target_rolls_back_receipt_and_all_new_rows(tmp_path):
    ...
    target = (
        "migration-"
        + hashlib.sha256(
            (checked["archive_digest"] + ":person-1").encode()
        ).hexdigest()[:32]
    )
...
def test_memory_and_link_preview_and_atomic_canonical_commit(tmp_path):
    ...
    source = knowledge_archive(
        memories=[
            {
                "id": memory_id,
                "title": "A durable evening",
                "content": "Rain and neon",
                "mood": "reflective",
                "tags": ["city", "memory"],
                "source": "conversation-import",
                "sourceRef": "conversation-7.json",
                "sourceCreatedAt": "2025-01-02T03:04:05Z",
                "sourceUpdatedAt": "2025-01-03T03:04:05Z",
                "createdAt": "2026-01-01T00:00:00Z",
                "updatedAt": "2026-01-01T00:00:00Z",
            }
        ],
        links=[
            {
                "id": link_id,
                "url": "https://Example.com/docs/?utm_source=old",
    ...
    assert checked["records"] == [
        {"source_id": link_id, "domain": "links", "name": "Reference docs"},
        {"source_id": memory_id, "domain": "memories", "name": "A durable evening"},
    ]
    ...
    assert (
        ...
        "2025-01-03T03:04:05Z",
    )
    assert (
        memory["provider"] == "legacy-migration"
        and memory["source_id"] == "legacy-archive"
        and memory["guid"] == f"memories:{memory_id}"
    )
    assert memory["tags"] == ["city", "memory"]
    assert memory["file_metadata"]["source_ref"] == "conversation-7.json"
    link = knowledge.get_item(link_id)
    assert (
        link["item_type"] == "bookmark" and link["url"] == "https://example.com/docs/"
    )
    assert link["content"] == "Primary reference\nRead chapter two" and link[
        "tags"
    ] == ["docs"]
    again, created = commit(
        people,
        {
            **source,
            "archive_digest": checked["archive_digest"],
...
def test_knowledge_collision_rolls_back_every_record_and_receipt(tmp_path):
    ...
    knowledge.db.execute("UPDATE items SET id=? WHERE title='Existing'", (second,))
    knowledge.db.commit()
    with pytest.raises(MigrationError, match="target identity"):
        commit(
            people,
...
def test_people_and_knowledge_must_be_separate_atomic_imports():
    ...
    extra = {
        "brain/memories/index.json": json.dumps(
            {"schemaVersion": 1, "type": "memories"}
        ).encode(),
        "brain/memories/"
        + memory_id
...
def test_ideas_and_journals_share_one_atomic_knowledge_commit(tmp_path):
    ...
    source = domains_archive(
        {
            "ideas": [
                {
                    "id": idea_id,
                    "title": "Build a solar kiln",
                    "status": "done",
                    "oneLiner": "Dry timber with sunlight",
                    "notes": "Prototype used reclaimed glass",
                    "tags": ["making"],
            ...
            "journals": [
                {
                    "id": journal_id,
                    "date": journal_id,
                    "content": "Shipped the first prototype.",
                    "segments": [
                        {
                            "text": "Shipped the first prototype.",
    ...
    assert idea[
        "content"
    ] == "Dry timber with sunlight\nPrototype used reclaimed glass" and idea[
        "tags"
    ] == [
        "making"
    ]
    journal = knowledge.get_item(journal_id)
    assert journal["item_type"] == "journal" and journal["guid"].startswith(
        "date_journal:"
    )
...
def test_idea_collision_rolls_back_journal_and_receipt(tmp_path):
    idea_id = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"
    day = "2026-09-21"
    source = domains_archive(
        {
            "ideas": [
    ...
    people = PeopleStore(tmp_path / "people")
    knowledge = KnowledgeStore(str(tmp_path / "knowledge.db"))
    knowledge.create_typed_item(
        item_type="fleeting", title="Existing", content="collision"
    )
    knowledge.db.execute("UPDATE items SET id=? WHERE title='Existing'", (idea_id,))
    knowledge.db.commit()
    with pytest.raises(MigrationError, match="target identity"):
        commit(
            people,
            ...
            "projects": [
                {
                    ...
                    "name": "Solar kiln",
                    "status": "blocked",
                    "nextAction": "Source firebrick",
                    "notes": "Waiting on supplier",
                    "tags": ["woodshop"],
...
def test_project_archive_refuses_multiple_records_and_mixed_store_domains(tmp_path):
    ...
    source = domains_archive(
        {
            "projects": [
                project("cccccccc-cccc-4ccc-8ccc-cccccccccccc", "One"),
                project("dddddddd-dddd-4ddd-8ddd-dddddddddddd", "Two"),
            ]
        }
    )
    checked = preview(source)
    with pytest.raises(MigrationError, match="exactly one project"):
        commit(
            ...
            BoundHierarchy(tmp_path / "home"),
        )
    mixed = domains_archive(
        {
            "projects": [project("eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee", "Mixed")],
            "ideas": [
                {
...
def test_project_and_thread_commit_with_same_archive_reference_and_exact_replay(
    tmp_path,
):
    ...
    assert restored.evidence[0]["references"] == [
        {
            "kind": "brain.project",
            "id": project["id"],
            "label": "Mixed restore",
...
def _crashed_project_task_restore(tmp_path, *, edit_project=False):
    project, thread = project_task_records(
        project_id="41414141-4141-4141-8141-414141414141", task_id="thread-restart"
    )
    source = domains_archive({"projects": [project], "threads": [thread]})
    ...
    receipt = {
        ...
        "committed_at": "2026-09-25T12:00:00+00:00",
        "domains": {"projects": 1, "threads": 1},
        "records": [
            {
...
def test_restart_rolls_forward_project_ready_journal_and_deduplicates_receipt(tmp_path):
    source, receipt, people, knowledge, projects, tasks, path, project, thread = (
        _crashed_project_task_restore(tmp_path)
    )
    assert not tasks._task_path(thread["id"]).exists()
    assert receipts(
        people, knowledge, BoundHierarchy(projects.home), BoundTasks(tasks.home)
    ) == [receipt]
    assert BoundTasks(tasks.home)._task_path(thread["id"]).exists()
    assert json.loads(path.read_text())["status"] == "complete"
    checked = preview(source)
    replay, created = commit(
        people,
        {
            **source,
            "archive_digest": checked["archive_digest"],
...
def test_caught_failure_after_project_publication_compensates_owned_project(tmp_path):
    project, thread = project_task_records(
        refs=[{"kind": "brain.person", "id": "missing-person", "label": "Missing"}]
    )
    source = domains_archive({"projects": [project], "threads": [thread]})
    checked = preview(source)
    home = tmp_path / "home"
    ...
    assert (
        projects.get_project(project["id"]) is None
        and not tasks._task_path(thread["id"]).exists()
    )
...
def test_restart_refuses_compensation_after_owned_project_was_edited(tmp_path):
    _, _, people, knowledge, projects, tasks, path, project, thread = (
        _crashed_project_task_restore(tmp_path, edit_project=True)
    )
    with pytest.raises(MigrationError, match="automatic compensation refused"):
        receipts(people, knowledge, projects, tasks)
    assert (
        projects.get_project(project["id"]).name == "Edited after interrupted restore"
    )
...
def test_completed_coordinator_refuses_task_drift_and_preserves_journal(tmp_path):
    project, thread = project_task_records(
        project_id="46464646-4646-4646-8646-464646464646", task_id="thread-drift"
    )
    source = domains_archive({"projects": [project], "threads": [thread]})
    checked = preview(source)
    home = tmp_path / "home"
    ...
    body = json.loads(task_path.read_text())
    ...
    with pytest.raises(MigrationError, match="canonical migration has identity drift"):
        receipts(people, knowledge, projects, tasks)
    assert (
        json.loads(task_path.read_text())["title"] == "Edited after completion"
        and journal.exists()
    )
    knowledge.close()
...
def test_standalone_legacy_markers_without_fingerprint_replay_when_body_is_exact(
    tmp_path,
):
    ...
    task_source = domains_archive(
        {
            "admin": [
                {
                    "id": task_id,
                    "title": "Legacy task marker",
                    "status": "open",
                    "nextAction": "Replay",
                    "notes": "",
                    "createdAt": "2026-01-01T00:00:00Z",
    ...
    task_checked = preview(task_source)
    tasks = BoundTasks(tmp_path / "home")
    task_payload = {
        **task_source,
        "archive_digest": task_checked["archive_digest"],
        "review_token": task_checked["review_token"],
    }
    ...
    assert commit(people, task_payload, knowledge, projects, tasks) == (
        task_receipt,
        False,
    )
    knowledge.close()
...
def test_common_coordinator_imports_multiple_projects_admin_and_ordered_threads(
    tmp_path,
):
    ...
    assert [projects.get_project(row["id"]).name for row in project_rows] == [
        "Alpha restore",
        "Beta restore",
    ]
    ...
    assert [ref["canonical_id"] for ref in first.evidence[0]["references"]] == [
        project_rows[0]["id"],
        admin["id"],
    ]
    assert [ref["canonical_id"] for ref in final.evidence[0]["references"]] == [
        project_rows[1]["id"],
        first_thread["id"],
    ]
    journal_path = (
    ...
    assert [step["key"] for step in journal["steps"]] == [
        f"projects:{project_rows[0]['id']}",
        f"projects:{project_rows[1]['id']}",
        f"admin:{admin['id']}",
        f"threads:{first_thread['id']}",
        f"threads:{final_thread['id']}",
    ]
    assert all(step["state"] == "applied" for step in journal["steps"])
    replay, replay_created = commit(
        people, payload, knowledge, BoundHierarchy(home), BoundTasks(home)
    )
    assert replay_created is False and replay == receipt
    assert receipts(people, knowledge, BoundHierarchy(home), BoundTasks(home)) == [
        receipt
    ]
    knowledge.close()
...
def test_common_coordinator_restart_resumes_each_pending_step_in_order(tmp_path):
    ...
    assert receipts(people, knowledge, BoundHierarchy(home), reopened_tasks) == [
        receipt
    ]
...
def test_common_coordinator_restart_adopts_owned_record_written_before_step_checkpoint(
    tmp_path,
):
...
def test_common_coordinator_compensates_all_owned_records_in_reverse_on_late_failure(
    tmp_path,
):
...
def test_common_coordinator_preserves_all_records_and_journal_when_one_applied_target_drifts(
    tmp_path,
):
    source, project_rows, task_rows = multi_canonical_archive()
    checked = preview(source)
    ...
    changed["title"] = "User changed title"
    first_path.write_text(json.dumps(changed))
    migration._write_journal(path, journal)
    ...
    assert (
        tasks._task_path(task_rows[0]["id"]).exists()
...
def test_common_coordinator_rejects_cycles_and_unrelated_cross_store_families():
    ...
    data = {
        ...
        "threads": [
            {
                **tasks[1],
                "refs": [{"kind": "cos.task", "id": tasks[2]["id"], "label": "Beta"}],
            },
            {
                **tasks[2],
                "refs": [{"kind": "cos.task", "id": tasks[1]["id"], "label": "Alpha"}],
            },
        ],
    }
...
def test_common_coordinator_supports_multiple_task_families_without_a_project(tmp_path):
    ...
    admin, first_thread, final_thread = task_rows
    source = domains_archive(
        {"admin": [admin], "threads": [first_thread, final_thread]}
    )
    checked = preview(source)
    home = tmp_path / "home"
    people = PeopleStore(home / "capabilities" / "communications")
    ...
    no_project_refs = [
        {
            **first_thread,
            "refs": [{"kind": "brain.admin", "id": admin["id"], "label": "Admin"}],
        },
        {
            **final_thread,
            "refs": [{"kind": "cos.task", "id": first_thread["id"], "label": "First"}],
        },
    ]
    ...
    receipt, created = commit(
        ...
        {
            **valid,
            "archive_digest": reviewed["archive_digest"],
    ...
    assert (
        tasks._read_task(tasks._task_path(final_thread["id"])).evidence[0][
            "references"
        ][0]["canonical_id"]
        == first_thread["id"]
    )
...
def test_common_coordinator_preflights_duplicate_project_names_before_journaling(
    tmp_path,
):
    source, project_rows, task_rows = multi_canonical_archive()
    duplicate = {**project_rows[1], "name": project_rows[0]["name"]}
    collision = domains_archive(
        {"projects": [project_rows[0], duplicate], "admin": [task_rows[0]]}
    )
    checked = preview(collision)
    home = tmp_path / "home"
    people = PeopleStore(home / "people")
    knowledge = KnowledgeStore(str(home / "knowledge.db"))
    projects, tasks = BoundHierarchy(home), BoundTasks(home)
    with pytest.raises(MigrationError, match="target identity already exists"):
        commit(
...
def test_common_coordinator_rejects_duplicate_targets_and_more_than_one_hundred_records(
    tmp_path,
):
    ...
    home = tmp_path / "home"
    assert (
        list(migration._canonical_coordinator_root(BoundTasks(home)).glob("*.json"))
        == []
    )
...
def test_common_coordinator_journal_rejects_tampered_step_and_oversized_bytes(tmp_path):
        ...
        json.dumps(
            {
                ...
                "receipt_hash": hashlib.sha256(
                    json.dumps(bad_receipt, sort_keys=True).encode()
                ).hexdigest(),
            }
        )
    )
    ...
    bad.unlink()
    huge = root / ("e" * 64 + ".json")
    huge.write_bytes(b" " * (2 * 1024 * 1024 + 1))
    with pytest.raises(MigrationError, match="exceeds 2 MiB"):
        receipts(people, knowledge, projects, tasks)
    assert huge.exists()
    knowledge.close()
...
def test_buckets_and_links_import_as_canonical_collection_membership(tmp_path):
    ...
    collection = dict(
        knowledge.db.execute(
            "SELECT * FROM collections WHERE id=?", (bucket_id,)
        ).fetchone()
    )
    assert (
        collection["name"],
        ...
        collection["icon"],
        collection["position"],
    ) == ("Research", "manual", "books", 3)
    assert tuple(
        knowledge.db.execute(
            "SELECT collection_id,item_id FROM collection_items"
        ).fetchone()
    ) == (bucket_id, link_id)
...
def test_missing_bucket_reference_rolls_back_link_and_receipt(tmp_path):
            ...
            "links": [
                {
                    ...
                    "bucketId": "17171717-1717-4717-8717-171717171717",
                    "bucketOrder": 0,
                    "createdAt": "2025-01-01T00:00:00Z",
                    "updatedAt": "2025-01-01T00:00:00Z",
                }
    ...
    assert (
        knowledge.db.execute("SELECT count(*) FROM collection_items").fetchone()[0] == 0
    )
    assert receipts(people, knowledge) == []
...
def test_inbox_state_and_history_import_with_same_transaction_destination(tmp_path):
    ...
    source = domains_archive(
        {
            ...
            "inbox": [
                {
                    "id": capture_id,
                    "capturedText": "Build a solar collector",
                    ...
                    "correction": {
                        "correctedAt": "2025-01-02T00:00:00Z",
                        "previousDestination": "projects",
                        "newDestination": "ideas",
                    },
                }
            ],
        }
    ...
    capture = dict(
        knowledge.db.execute(
            "SELECT * FROM capability_knowledge_captures WHERE id=?", (capture_id,)
        ).fetchone()
    )
    assert (capture["original_text"], capture["status"], capture["destination_id"]) == (
        "Build a solar collector",
        "routed",
        idea_id,
    )
    event = knowledge.db.execute(
        "SELECT event,payload,happened_at FROM capability_knowledge_capture_events WHERE capture_id=?",
        (capture_id,),
    ).fetchone()
    assert (
        event["event"] == "migration_snapshot"
        and json.loads(event["payload"])["creative"] is True
    )
...
def test_inbox_rejects_unverifiable_route_and_collision_rolls_back(tmp_path):
    ...
    bad = domains_archive(
        {
            "inbox": [
                {
                    "id": capture_id,
                    "capturedText": "Call Ada",
                    "capturedAt": "2025-01-01T00:00:00Z",
                    "source": "brain_ui",
                    "status": "filed",
                    "filed": {
                        "destination": "people",
                        "destinationId": "21212121-2121-4121-8121-212121212121",
                    },
                }
            ]
        }
    ...
    knowledge.db.execute(
        "INSERT INTO capability_knowledge_captures (id,request_id,input_origin,original_text,captured_at,status,revision) VALUES (?,?,?,?,?,?,1)",
        (
            capture_id,
            "existing-request",
            "text",
    ...
    assert (
        knowledge.db.execute(
            "SELECT count(*) FROM capability_knowledge_capture_events"
        ).fetchone()[0]
        == 0
    )
    assert receipts(people, knowledge) == []
...
def test_admin_import_is_one_crash_safe_canonical_task_with_embedded_receipt(tmp_path):
            ...
            "admin": [
                {
                    ...
                    "nextAction": "Ask broker for revised terms",
                    "notes": "Policy expires soon",
    ...
    assert checked["records"] == [
        {"source_id": identity, "domain": "admin", "name": "Renew insurance"}
    ]
    people = PeopleStore(tmp_path / "people")
    knowledge = KnowledgeStore(str(tmp_path / "knowledge.db"))
    projects = BoundHierarchy(tmp_path / "home")
    ...
    assert (
        task.id == identity
        and task.title == "Renew insurance"
        and task.status.value == "blocked"
    )
    assert (
        task.description == "Policy expires soon" and task.due == "2026-10-01T09:00:00Z"
    )
    assert (
        task.action_plan[0]["content"] == "Ask broker for revised terms"
        and not task.action_plan[0]["completed"]
    )
    marker = task.evidence[0]
    assert (
        marker["type"] == "platform_migration"
        and marker["archive_digest"] == checked["archive_digest"]
    )
...
def test_thread_import_preserves_work_state_and_resolves_real_references(tmp_path):
    ...
    commit(
        ...
        {
            **idea_source,
            "archive_digest": idea_preview["archive_digest"],
            "review_token": idea_preview["review_token"],
        },
        knowledge,
        projects,
        tasks,
    ...
    source = domains_archive(
        {
            "threads": [
                {
                    "id": thread_id,
                    "title": "Finish workshop plan",
                    "status": "someday",
                    "priority": "urgent",
                    "nextAction": "Price firebrick",
                    "notes": "Resume after the roof repair",
                    "waitingOn": "Dry weather",
                    "dueAt": "2026-12-01T00:00:00Z",
                    "tags": ["workshop", "planning"],
                    "pinned": True,
                    "refs": [
                        {"kind": "brain.idea", "id": idea_id, "label": "Solar kiln"},
                        {
                            "kind": "url",
                            "id": "https://example.com/firebrick",
                            "label": "Supplier",
                        },
                    ],
                    "source": {"kind": "github.issue", "key": "example/workshop#1"},
                    "externalState": "open",
    ...
    assert task.notes == [
        {"content": "Waiting on: Dry weather", "timestamp": "2026-03-01T00:00:00Z"}
    ]
    assert (
        task.action_plan[0]["content"] == "Price firebrick"
        and task.due == "2026-12-01T00:00:00Z"
    )
    ...
    assert refs == [
        ...
        },
        {
            ...
            "id": "https://example.com/firebrick",
            "label": "Supplier",
            "canonical_id": "https://example.com/firebrick",
        },
    ]
    assert task.evidence[0]["source_state"] == {
        "source": {"kind": "github.issue", "key": "example/workshop#1"},
        "externalState": "open",
    }
...
def test_thread_person_reference_resolves_prior_migration_identity(tmp_path):
    ...
    person_receipt, _ = commit(
        people,
        {
            **person_source,
            "archive_digest": person_preview["archive_digest"],
            "review_token": person_preview["review_token"],
        },
        knowledge,
        projects,
        tasks,
            ...
            "threads": [
                {
                    ...
                    "status": "open",
                    "priority": "normal",
                    "nextAction": "Send questions",
                    "notes": "",
                    "tags": [],
                    "pinned": False,
                    "refs": [
                        {"kind": "brain.person", "id": source_person, "label": "Ada"}
                    ],
                    "createdAt": "2026-02-01T00:00:00Z",
                    "updatedAt": "2026-02-01T00:00:00Z",
                }
...
def test_task_archives_reject_multiple_records_and_cross_store_mixes(tmp_path):
    ...
    source = domains_archive(
        {
            "admin": [
                admin("29292929-2929-4929-8929-292929292929", "One"),
                admin("30303030-3030-4030-8030-303030303030", "Two"),
            ]
        }
    )
    checked = preview(source)
    with pytest.raises(MigrationError, match="exactly one record"):
        commit(
            ...
            KnowledgeStore(str(tmp_path / "knowledge.db")),
            BoundHierarchy(tmp_path / "home"),
            BoundTasks(tmp_path / "home"),
        )
    mixed = domains_archive(
        {
            "admin": [admin("31313131-3131-4131-8131-313131313131", "Mixed")],
            "ideas": [
                {
...
def test_threads_fail_closed_for_unknown_refs_invalid_urls_and_future_fields():
    ...
    for ref in (
        {"kind": "brain.song", "id": "song", "label": "Unsupported"},
...
def test_song_restore_materializes_real_artifacts_and_survives_restart(tmp_path):
    ...
    source = song_archive(
        song_record(attachments=attachments), {"score.pdf": score, "chart.md": chart}
    )
    checked = preview(source)
    assert checked["coverage"]["supported"] == ["songs"]
    assert checked["records"] == [
        {"source_id": "song-1", "domain": "songs", "name": "Midnight Train"}
    ]
    ...
    receipt, created = commit(
        people, payload, repertoire=repertoire, artifacts=artifacts
    )
    assert created is True and receipt["domains"] == {"songs": 1}
    item = repertoire.get("song-1")
    assert {
        key: item[key]
        for key in ("title", "artist", "instrument", "key", "capo", "tuning")
    } == {
        "title": "Midnight Train",
        "artist": "The Testers",
        "instrument": "guitar",
    ...
    assert item["notation"] == {
        "format": "chordpro",
        "text": "{title: Midnight Train}\n[Am]All aboard",
    }
    assert item["body"] == "Keep the bridge quiet" and item["tags"] == [
        "setlist",
        "acoustic",
    ]
    assert (
    ...
    replayed, replay_created = commit(
        people, payload, repertoire=repertoire, artifacts=artifacts
    )
    assert replay_created is False and replayed == receipt
    reopened_artifacts = NativeArtifactProvider(home / "artifacts")
    reopened_repertoire = RepertoireStore(
        home / "capabilities" / "music", reopened_artifacts
    )
    assert (
        reopened_repertoire.get("song-1")["attachment_refs"] == item["attachment_refs"]
    )
    assert receipts(
        people, repertoire=reopened_repertoire, artifacts=reopened_artifacts
    ) == [receipt]
...
def test_song_preview_rejects_missing_or_mismatched_attachment_material(
    mutation, message
):
    raw = b"%PDF-1.7\nsource"
    meta = {
...
def test_song_preview_rejects_duplicate_attachment_identity():
    ...
    with pytest.raises(MigrationError, match="filenames must be unique"):
        preview(
            song_archive(
                song_record(attachments=[metadata, dict(metadata)]), {"same.pdf": raw}
            )
        )


def test_song_commit_rolls_back_new_artifacts_when_canonical_identity_collides(
    tmp_path,
):
    raw = b"%PDF collision"
    metadata = {
    ...
    assert (
        list((home / "capabilities" / "platform" / "migration-journals").glob("*.json"))
        == []
    )
...
def test_song_restart_recovers_incomplete_import_and_owned_attachment(tmp_path):
    ...
    repertoire.import_song(
        import_id=import_id,
        source_fingerprint=token,
        item_id="song-recovery",
        created_at="2026-01-01T00:00:00+00:00",
        updated_at="2026-01-02T00:00:00+00:00",
        data={
            "title": "Interrupted",
            "attachment_refs": [{"slug": slug, "version": 1}],
        },
        schedule={
            "stage": "new",
            "ease": 2.5,
    ...
    assert (
        artifacts.get(slug) is None and not (journal_root / f"{digest}.json").exists()
    )
...
def test_song_restart_refuses_recovery_after_user_edit(tmp_path):
    ...
    journal = {
        ...
        "review_token": token,
        "import_id": import_id,
        "song_id": "changed-song",
        "artifacts": [],
    }
    (journal_root / f"{digest}.json").write_text(json.dumps(journal))
    with pytest.raises(MigrationError, match="changed and cannot be rolled back"):
        receipts(
            PeopleStore(home / "people"), repertoire=repertoire, artifacts=artifacts
        )
...
def test_song_restart_fails_closed_on_malformed_recovery_journal(tmp_path):
    ...
    path.write_text(
        json.dumps(
            {"schema": 1, "status": "artifacts_ready", "archive_digest": "e" * 64}
        )
    )
...
def test_song_archives_reject_multiple_records_and_cross_store_mixes(tmp_path):
    ...
    with pytest.raises(MigrationError, match="exactly one song"):
        commit(
            ...
            repertoire=RepertoireStore(tmp_path / "music", artifacts),
            artifacts=artifacts,
        )
    mixed = domains_archive(
        {
...
def test_supported_domains_restore_into_a_fresh_home_and_survive_reopen(tmp_path):
    ...
    knowledge_source = domains_archive(
        {
            "memories": [
                {
                    "id": memory_id,
    ...
    knowledge_receipt, _ = commit(
        people,
        {
            **knowledge_source,
            "archive_digest": knowledge_preview["archive_digest"],
            "review_token": knowledge_preview["review_token"],
        },
        knowledge,
        projects,
        tasks,
    ...
    project_source = domains_archive(
        {
            "projects": [
                {
                    "id": project_id,
    ...
    project_receipt, _ = commit(
        people,
        {
            **project_source,
            "archive_digest": project_preview["archive_digest"],
            "review_token": project_preview["review_token"],
        },
        knowledge,
        projects,
        tasks,
    ...
    task_receipt, _ = commit(
        people,
        {
            **task_source,
            "archive_digest": task_preview["archive_digest"],
            "review_token": task_preview["review_token"],
        },
        knowledge,
        projects,
        tasks,
    ...
    assert (
        reopened_tasks._read_task(reopened_tasks._task_path(task_id)).title
        == "Fresh action"
    )
    restored_receipts = receipts(
        reopened_people, reopened_knowledge, reopened_projects, reopened_tasks
    )
    assert {row["archive_digest"] for row in restored_receipts} == {
        person_receipt["archive_digest"],
        knowledge_receipt["archive_digest"],
        project_receipt["archive_digest"],
        task_receipt["archive_digest"],
    }
