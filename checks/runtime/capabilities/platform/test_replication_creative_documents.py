...
def test_owner_revision_import_rejects_noncanonical_current_and_reference_state(tmp_path):
    source, target = tmp_path / "source", tmp_path / "target"
    _, work, _, _ = _source(source)
    _replicate_current_work(source, target)
    rows = adapter.read_rows(source, adapter.VERSIONS_ENTRY)
    ...
    assert store.import_revision(valid) == valid
    with sqlite3.connect(target / "capabilities/creative/catalog.sqlite3") as database:
        assert database.execute("SELECT count(*) FROM work_revisions WHERE id=? AND revision=1",
                                (work["id"],)).fetchone() == (1,)
...
def test_complete_coverage_rejects_order_gaps_duplicates_and_private_fields(tmp_path):
    ...
    with pytest.raises(ValueError, match="complete ordered coverage"):
        adapter.validate_entries(list(reversed(entries)))
    with pytest.raises(ValueError, match="complete ordered coverage"):
        adapter.validate_entries(entries[:1])

    extra_entry_field = copy.deepcopy(entries)
    extra_entry_field[0]["private"] = "request-receipt"
    with pytest.raises(ValueError, match="complete ordered coverage"):
        adapter.validate_entries(extra_entry_field)
    ...
    with pytest.raises(ValueError, match="Duplicate creative document row"):
        adapter.validate_entries(duplicate_version)

    discontinuous = copy.deepcopy(entries)
    discontinuous[0]["rows"] = [discontinuous[0]["rows"][0], discontinuous[0]["rows"][2]]
    with pytest.raises(ValueError, match="complete and contiguous"):
        adapter.validate_entries(discontinuous)
    ...
    with pytest.raises(ValueError, match="Invalid work version data"):
        adapter.validate_entries(private_version)
...
def test_version_dependency_preflight_is_atomic_before_any_history_write(tmp_path):
    ...
    _, work, _, _ = _source(source)
    _replicate_current_work(source, target)
    rows = copy.deepcopy(adapter.read_rows(source, adapter.VERSIONS_ENTRY))
    last = rows[-1]["data"]
    last["record"]["universe_ref"] = {"id": "absent-universe", "revision": 7}
    last["record_sha256"] = adapter._sha(last["record"])
    rows[-1]["id"] = adapter._version_id(last["work_id"], last["revision"])

    with pytest.raises(ValueError, match="pinned context is unavailable"):
        adapter.apply_rows(target, adapter.VERSIONS_ENTRY, rows, {}, conflicts.ConflictQueue(target),
                           "2026-09-25T13:00:00+00:00")
    with sqlite3.connect(target / "capabilities/creative/catalog.sqlite3") as database:
        assert database.execute("SELECT count(*) FROM work_revisions WHERE id=?", (work["id"],)).fetchone() == (0,)
        assert database.execute("SELECT count(*) FROM work_drafts WHERE work_id=?", (work["id"],)).fetchone() == (0,)


def test_owner_draft_import_is_idempotent_and_refuses_content_or_provenance_drift(tmp_path):
    source, target = tmp_path / "source", tmp_path / "target"
    _, work, _, _ = _source(source)
    _replicate_current_work(source, target)
    row = adapter.read_rows(source, adapter.DRAFTS_ENTRY)[0]
    data = row["data"]
    store = WorkStore(target)

    assert store.import_draft(data["work_id"], data["draft"], data["text"],
                              data["artifact_description"], data["content_sha256"]) == data["draft"]
    assert store.import_draft(data["work_id"], data["draft"], data["text"],
                              data["artifact_description"], data["content_sha256"]) == data["draft"]
    with sqlite3.connect(target / "capabilities/creative/catalog.sqlite3") as database:
        assert database.execute("SELECT count(*) FROM work_drafts WHERE id=?", (row["id"],)).fetchone() == (1,)
    assert store.read_draft(work["id"], row["id"])["text"] == data["text"]

    altered = ("X" if data["text"][0] != "X" else "Y") + data["text"][1:]
    with pytest.raises(ValueError, match="hash does not match"):
        store.import_draft(data["work_id"], data["draft"], altered,
                           data["artifact_description"], data["content_sha256"])
    with pytest.raises(ValueError, match="provenance"):
        store.import_draft(data["work_id"], data["draft"], data["text"], "not-a-digest",
                           data["content_sha256"])
...
def test_draft_preflight_refuses_artifact_collision_without_database_mutation(tmp_path):
    ...
    _, work, _, _ = _source(source)
    _replicate_current_work(source, target)
    rows = adapter.read_rows(source, adapter.DRAFTS_ENTRY)
    first = rows[0]["data"]
    target_store = WorkStore(target)
    target_store.artifacts.create(name="Collision", slug=first["draft"]["artifact_id"], kind="markdown",
        content="Locally different content", description=first["artifact_description"], readonly=True)

    with pytest.raises(ValueError, match="artifact conflicts"):
        adapter.apply_rows(target, adapter.DRAFTS_ENTRY, rows, {}, conflicts.ConflictQueue(target),
...
def test_draft_divergence_queues_conflict_and_missing_artifact_fails_export(tmp_path):
    ...
    _replicate_current_work(source, target)
    versions = adapter.read_rows(source, adapter.VERSIONS_ENTRY)
    drafts = adapter.read_rows(source, adapter.DRAFTS_ENTRY)
    adapter.apply_rows(target, adapter.VERSIONS_ENTRY, versions, {}, conflicts.ConflictQueue(target),
                       "2026-09-25T13:02:00+00:00")
    baseline = adapter.apply_rows(target, adapter.DRAFTS_ENTRY, drafts, {}, conflicts.ConflictQueue(target),
                                  "2026-09-25T13:02:00+00:00")
    with sqlite3.connect(target / "capabilities/creative/catalog.sqlite3") as database:
        stored = database.execute("SELECT record FROM work_drafts WHERE id=?", (first_draft["id"],)).fetchone()
        changed = json.loads(stored[0])
        changed["note"] = "local immutable divergence"
        database.execute("UPDATE work_drafts SET record=? WHERE id=?",
                         (json.dumps(changed, sort_keys=True), first_draft["id"]))
    queue = conflicts.ConflictQueue(target)
    held = adapter.apply_rows(target, adapter.DRAFTS_ENTRY, drafts, baseline.new_ancestors, queue,
                              "2026-09-25T13:03:00+00:00")
    assert (held.added, held.updated, held.removed, held.conflicts) == (0, 0, 0, 1)
    pending = queue.items(status=conflicts.STATUS_NEEDS_REVIEW, entry_id=adapter.DRAFTS_ENTRY)
    assert len(pending) == 1 and pending[0].local_row["data"]["draft"]["note"] == "local immutable divergence"
    with sqlite3.connect(target / "capabilities/creative/catalog.sqlite3") as database:
        assert json.loads(database.execute("SELECT record FROM work_drafts WHERE id=?",
                                           (first_draft["id"],)).fetchone()[0])["note"] == "local immutable divergence"

    artifact = WorkStore(source).artifacts.get(first_draft["artifact_id"], version=1)
    assert artifact is not None
    version_path = source / "artifacts" / first_draft["artifact_id"] / "versions" / "v1.html"
    version_path.unlink()
    with pytest.raises(ValueError, match="artifact is unavailable"):
        adapter.read_rows(source, adapter.DRAFTS_ENTRY)
