...
def peer_record(identity, endpoint, scope, send=True, receive=True):
    return {
        ...
        "enabled": True,
        "send_categories": [scope] if send else [],
        "receive_categories": [scope] if receive else [],
        "revision": 0,
    }
...
def pair(first_home, second_home, scope, second_endpoint="https://second.example"):
    ...
    second.put(
        first_id["peer_id"], peer_record(first_id, "https://first.example", scope)
    )
...
def commission_record():
    return {
        ...
        "brief": {
            "intent": "Develop the pinned source.",
            "genre": "mystery",
            "category": "treatment",
            "style": "Precise.",
            "constraints": {},
...
def prepare(adapter, source, target):
    if adapter is replication_identity_stories:
        StoryStore(source / "capabilities/identity/stories.sqlite3").create(
            ...
            theme="Origins",
            text="The harbor lights marked the route.",
        ...
        try:
            ...
            collection_id = source_store.create_collection(
                name="Research shelf", icon="archive"
            )
        ...
        adapter.write_row(
            source, adapter.ENTRY_ID, {"id": value["id"], "data": value}, value["id"]
        )
    return [
        {"entry_id": entry_id, "rows": adapter.read_rows(source, entry_id)}
...
def edit_first(adapter, home, title):
    ...
    field = (
        "theme"
        if adapter is replication_identity_stories
        else "title" if adapter is replication_experience_stories else "name"
    )
    ...
    if adapter is replication_identity_stories or adapter is replication_commissions:
        row["data"]["updated_at"] = (
            ...
            else "2026-09-25T12:11:00+00:00"
        )
...
def test_canonical_domains_are_declared_and_default_denied(tmp_path, adapter):
    ...
    first_store.put(
        second_id["peer_id"],
        peer_record(second_id, "https://second.example", adapter.SCOPE, send=False),
    )
...
async def test_signed_scope_dispatches_complete_canonical_batch(tmp_path, adapter):
    ...
    async with TestServer(app) as server:
        first_id, second_id = pair(
            first, second, adapter.SCOPE, str(server.make_url("/"))
        )
        malformed = ReplicationService(first).export_batch(
            second_id["peer_id"], adapter.SCOPE
        )
        ...
        response = await ReplicationService(first).push(
            second_id["peer_id"], adapter.SCOPE
        )
...
@pytest.mark.parametrize("adapter", ADAPTERS)
def test_central_conflict_restore_dispatch_preserves_unselected_fields(
    tmp_path, adapter
):
    first, second = tmp_path / "first", tmp_path / "second"
    prepare(adapter, first, second)
    first_id, second_id = pair(first, second, adapter.SCOPE)
    first_service, second_service = ReplicationService(first), ReplicationService(
        second
    )
    ...
    current = next(
        row
        for row in adapter.read_rows(first, pending[0].entry_id)
        if row["id"] == identity
    )
...
def test_knowledge_collection_membership_failure_is_atomic_at_central_boundary(
    tmp_path,
):
    first, second = tmp_path / "first", tmp_path / "second"
    ...
    batch = ReplicationService(first).export_batch(
        second_id["peer_id"], replication_knowledge_collections.SCOPE
    )
    ...
    assert (
        replication_knowledge_collections.read_rows(
            second, replication_knowledge_collections.COLLECTION_ENTRY
        )
        == []
    )
    assert (
        replication_knowledge_collections.read_rows(
            second, replication_knowledge_collections.MEMBERSHIP_ENTRY
        )
        == []
    )
