...
def test_migrated_song_artifact_and_complete_journal_restore_into_fresh_home(tmp_path, monkeypatch):
    ...
    metadata = {
        "filename": "score.pdf", "label": "Archive score", "mime": "application/pdf",
        "size": len(score), "sha256": hashlib.sha256(score).hexdigest(),
    }
    source = song_archive(song_record("durable-song", attachments=[metadata]), {"score.pdf": score})
    checked = preview(source)
    reviewed = {**source, "archive_digest": checked["archive_digest"], "review_token": checked["review_token"]}
    receipt, created = commit(PeopleStore(source_home / "capabilities" / "communications"), reviewed,
                              repertoire=repertoire, artifacts=artifacts)
    ...
    journal = source_home / "capabilities" / "platform" / "migration-journals" / f'{checked["archive_digest"]}.json'
    ...
    assert snapshot_module.restore_apply(archive, "replace", None) == {
        "ok": True, "mode": "replace", "snapshot": archive.name,
    }

    restored_artifacts = NativeArtifactProvider(restored_home / "artifacts")
    restored_repertoire = RepertoireStore(restored_home / "capabilities" / "music", restored_artifacts)
    ...
    assert restored_artifacts.raw_bytes(ref["slug"], version=1) == (score, "application/pdf")
    restored_journal = restored_home / "capabilities" / "platform" / "migration-journals" / journal.name
    assert restored_journal.read_bytes() == journal_bytes
    restored_people = PeopleStore(restored_home / "capabilities" / "communications")
    assert receipts(restored_people, repertoire=restored_repertoire, artifacts=restored_artifacts) == [receipt]
    replay, replay_created = commit(restored_people, reviewed, repertoire=restored_repertoire,
