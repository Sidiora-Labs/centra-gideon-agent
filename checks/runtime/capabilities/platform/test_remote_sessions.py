...
class ProtocolRuntime:
    ...
    async def tool(self, request):
        self.requests.append(
            ("tool", request.headers.get("Authorization"), await request.json())
        )
                            ...
                            "messages": [
                                {
                                    "id": "m1",
                                    "role": "user",
                                    "content": [{"text": "real history"}],
                                    "createdAt": "2026-09-25T00:00:00Z",
                                },
                                {
                                    "id": "m2",
                                    "role": "assistant",
                                    "content": "retained remotely",
                                },
                            ]
                        }
                    },
    ...
    async def malformed(self, request):
        self.requests.append(
            ("malformed", request.headers.get("Authorization"), await request.json())
        )
        return web.Response(text="not-json", content_type="text/plain")

    async def unavailable(self, request):
        self.requests.append(
            ("unavailable", request.headers.get("Authorization"), await request.json())
        )
        return web.json_response(
            {"error": {"message": "downstream stopped"}}, status=503
        )
...
class RemoteSessionProtocol(unittest.IsolatedAsyncioTestCase):
        ...
        self.store.save_connection(
            {
                ...
                "credential_ref": "openclaw-token",
                "agent_id": "main",
            }
        )
        self.bridge = RemoteSessionBridge(
            self.home, store=self.store, credentials=credentials
        )
    ...
    async def test_actual_http_protocol_lists_history_and_retains_provenance(self):
        ...
        self.assertEqual(
            ...
            ["real history", "retained remotely"],
        )
        self.assertEqual(
            history["messages"][0]["source"],
            {"connection_id": "primary", "session_id": "remote-1", "kind": "remote"},
        )
    ...
    async def test_real_sse_request_carries_session_attachment_and_closes_transport(
        self,
    ):
        chunks = bytearray()
        async for chunk in self.bridge.stream(
            "primary",
            "remote-1",
            {
                "message": "inspect",
    ...
    async def test_credentials_validation_and_protocol_fail_closed(self):
        ...
        with self.assertRaises(ValueError):
            async for _ in self.bridge.stream(
                "primary", "remote-1", {"message": "", "attachments": []}
            ):
                pass

    async def test_actual_unauthorized_malformed_and_unavailable_responses_are_distinct(
        self,
    ):
        credentials = CredentialStore(self.home)
        credentials.save(
        ...
        self.store.save_connection(
            {
                ...
                "label": "Denied",
                "base_url": self.origin,
                "credential_ref": "wrong-token",
            }
        )
        denied = RemoteSessionBridge(
            self.home, store=self.store, credentials=credentials
        )
        ...
        self.store.save_connection(
            {
                "id": "malformed",
                "label": "Malformed",
                "base_url": self.origin + "/malformed",
                "credential_ref": "openclaw-token",
            }
        )
        with self.assertRaises(RemoteSessionError) as caught:
        ...
        self.store.save_connection(
            {
                "id": "down",
                "label": "Down",
                "base_url": self.origin + "/unavailable",
                "credential_ref": "openclaw-token",
            }
        )
        with self.assertRaises(RemoteSessionError) as caught:
    ...
    async def test_attachment_contract_rejects_ambiguous_or_oversized_payloads_before_network(
        self,
    ):
        ...
        for payload in invalid:
            with (
                self.subTest(payload_type=type(payload["attachments"]).__name__),
    ...
    def test_remote_message_content_normalization_preserves_order_without_raw_objects(
        self,
    ):
        self.assertEqual(_text("plain"), "plain")
        self.assertEqual(_text({"text": "nested"}), "nested")
        self.assertEqual(
            _text({"content": [{"text": "first"}, {"body": "second"}]}),
            "first\n\nsecond",
        )
...
class OwnerHTTPAndNative(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        credentials = CredentialStore(self.home)
        credentials.save(
            {"remote-token": {"type": "static_token", "value": "actual-secret"}}
        )
        ...
        self.client = ClientSession(
            cookie_jar=CookieJar(unsafe=True), headers={"x-owner": "yes"}
        )

    async def asyncTearDown(self):
        await self.client.close()
        await self.runner.cleanup()
        await self.remote_runner.cleanup()
        self.temp.cleanup()
    ...
    async def test_owner_routes_persist_metadata_and_project_protocol_results(self):
        created = await self.client.post(
            ...
            json={
                "id": "remote",
                "label": "Remote",
                "base_url": self.remote_origin,
                "credential_ref": "remote-token",
                "agent_id": "ops",
            },
        )
        ...
        history = await self.client.get(
            self.url + "/remote/sessions/remote-1/history?limit=4"
        )
        self.assertEqual(history.status, 200)
        self.assertEqual(
            (await history.json())["messages"][0]["source"]["kind"], "remote"
        )
        streamed = await self.client.post(
            self.url + "/remote/sessions/remote-1/messages/stream",
            json={"message": "hello"},
        )
    ...
    async def test_owner_auth_and_query_refusals(self):
        ...
        app_denied = await self.client.get(
            self.url, headers={"x-owner": "yes", "x-app": "external"}
        )
    ...
    async def test_native_manifest_and_approval_contract(self):
        from gideon.workspace.capabilities.platform.remote_sessions_tools import (
            create_provider,
        )
        ...
        manifest = json.loads(
            (
                ...
                / "runtime/gideon/extensions/apps/native/remote-agent-sessions/app.json"
            ).read_text()
        )
        self.assertEqual(
            manifest["provider"]["implementation"],
            "gideon.workspace.capabilities.platform.remote_sessions_tools:create_provider",
        )
