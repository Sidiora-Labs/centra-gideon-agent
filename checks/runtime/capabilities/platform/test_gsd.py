...
def planning(tmp_path, monkeypatch):
    ...
    (phase / "01-SUMMARY.md").write_text(
    ...
    project = HierarchyStore().create_project(
        "GSD project", workspace_dir=str(workspace)
    )
    return project, root
...
async def test_real_planning_read_edit_sha_conflict_and_restart(planning):
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        detail = await client.get(
            PREFIX + "/" + project.id, params={"document": "STATE.md"}
        )
        ...
        assert value["phases"] == [
            {
                "id": "01-editor",
                "plans": ["phases/01-editor/01-PLAN.md"],
                "summaries": ["phases/01-editor/01-SUMMARY.md"],
            }
        ]
        ...
        changed = await client.put(
            ...
            json={
                ...
                "content": "Phase 01 is ready for an explicit request.\n",
            },
        )
        ...
        assert (
            HierarchyStore().get_project(project.id).agent_instructions_template == ""
        )
...
@pytest.mark.asyncio
async def test_phase_actions_create_real_open_native_tasks_once_and_native_consumer(
    planning,
):
    project, root = planning
    async with TestClient(TestServer(application())) as client:
        ...
        for action in ("plan", "execute", "verify"):
            result = await client.post(
                f"{PREFIX}/{project.id}/phase",
                json={"phase": "01-editor", "action": action},
            )
        ...
        document = await provider.invoke(
            "platform_gsd_project", {"project_id": project.id, "document": "PROJECT.md"}
        )
        assert document.success
        assert (
            json.loads(document.output)["document"]["content"]
            == (root / "PROJECT.md").read_text()
        )
        definitions = await provider.list_tools()
        tool = next(
            tool for tool in definitions if tool.name == "platform_gsd_phase_task"
        )
        ...
        try:
            native = await provider.invoke(
                tool.name,
                {"project_id": project.id, "phase": "01-editor", "action": "verify"},
            )
...
async def test_unsupported_paths_are_rejected_for_read_and_write(planning, document):
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        read = await client.get(
            PREFIX + "/" + project.id, params={"document": document}
        )
        assert read.status == 400
        write = await client.put(
            PREFIX + "/" + project.id,
            json={"document": document, "content": "overwrite", "sha256": "x"},
        )
...
async def test_symlink_escape_size_auth_and_unknown_phase(planning):
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        response = await client.get(
            PREFIX, params={"token": generate_token("owner", app="other")}
        )
        assert response.status in {401, 403}
    async with TestClient(TestServer(application())) as client:
        await authenticate(client)
        unknown = await client.post(
            f"{PREFIX}/{project.id}/phase", json={"phase": "missing", "action": "plan"}
        )
        assert unknown.status == 404
        bad_action = await client.post(
            f"{PREFIX}/{project.id}/phase",
            json={"phase": "01-editor", "action": "arbitrary"},
        )
        assert bad_action.status == 400
        oversized = await client.put(
            PREFIX + "/" + project.id,
            json={"document": "STATE.md", "content": "x" * 262145, "sha256": "x"},
        )
        ...
        escaped = await client.get(
            PREFIX + "/" + project.id, params={"document": "CONCERNS.md"}
        )
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        try:
            denied = await provider.invoke(
                ...
                {"project_id": project.id, "phase": "01-editor", "action": "plan"},
            )
