...
from gideon.workspace.capabilities.platform.migration import FORMAT, MigrationError, commit, preview
...
def mixed_archive():
    ...
    song = {"id": "song_mixed", "title": "Mixed Song", "artist": "Ada", "instrument": "guitar",
            "stage": "learning", "tags": ["mixed"], "key": "C", "capo": 0, "tuning": "EADGBE",
            "sourceUrl": "", "links": [], "content": {"format": "plain", "text": "C F G"}, "notes": "",
            "scrollDurationSec": None, "attachments": [{"filename": "score.pdf", "label": "Score",
            "mime": "application/pdf", "size": len(attachment), "sha256": hashlib.sha256(attachment).hexdigest()}],
            "createdAt": now, "updatedAt": now}
    person = {"id": "mixed-person", "name": "Ada", "context": "Grouped import", "followUps": [], "tags": [],
              "createdAt": now, "updatedAt": now}
    project = {"id": PROJECT, "name": "Mixed restore", "status": "active", "nextAction": "Practice",
               "createdAt": now, "updatedAt": now}
    files = {
        "brain/people/index.json": json.dumps({"schemaVersion": 1, "type": "people"}).encode(),
        "brain/people/mixed-person/index.json": json.dumps(person).encode(),
        "brain/projects/index.json": json.dumps({"schemaVersion": 1, "type": "projects"}).encode(),
        f"brain/projects/{PROJECT}/index.json": json.dumps(project).encode(),
        "brain/songs/index.json": json.dumps({"schemaVersion": 1, "type": "songs"}).encode(),
    ...
    with tarfile.open(fileobj=stream, mode="w:gz") as archive:
        for name, value in {"snapshot/manifest.json": json.dumps(manifest).encode(),
                            **{f"snapshot/data/{name}": value for name, value in files.items()}}.items():
            member = tarfile.TarInfo(name)
            member.size = len(value)
            archive.addfile(member, io.BytesIO(value))
    return {"format": FORMAT, "content": base64.b64encode(stream.getvalue()).decode()}, attachment


def opened(home):
    artifacts = NativeArtifactProvider(home / "artifacts")
    return (PeopleStore(home / "capabilities" / "communications"), KnowledgeStore(str(home / "knowledge.db")),
            BoundHierarchy(home), BoundTasks(home), RepertoireStore(home / "capabilities" / "music", artifacts), artifacts)
...
def test_mixed_song_canonical_groups_commit_restart_and_exact_replay(tmp_path):
    ...
    assert inspected["commit_groups"] == [{"id": "canonical", "domains": ["people", "projects"]},
...
def test_song_failure_reports_partial_and_exact_retry_finishes_only_pending_group(tmp_path, monkeypatch):
    source, _ = mixed_archive()
    home = tmp_path / "home"
    people, knowledge, projects, tasks, repertoire, artifacts = opened(home)
    original = migration._commit_song
    monkeypatch.setattr(migration, "_commit_song", lambda *args: (_ for _ in ()).throw(MigrationError("GPU unavailable", 503)))
    partial, created = commit(people, reviewed(source), knowledge, projects, tasks, repertoire, artifacts)
    assert created is True and partial["status"] == "partial" and partial["domains"] == {"people": 1, "projects": 1}
    ...
    try:
        ...
        assert retry_created is True and complete["status"] == "complete" and complete["domains"]["songs"] == 1
        assert (restarted[2]._projects_dir() / PROJECT / "project.json").read_bytes() == project_bytes
        assert restarted[0].get(person["id"]) == person
    finally:
        restarted[1].close()


def test_completed_canonical_group_drift_refuses_song_retry_without_erasing_records(tmp_path, monkeypatch):
    source, _ = mixed_archive()
    home = tmp_path / "home"
    people, knowledge, projects, tasks, repertoire, artifacts = opened(home)
    original = migration._commit_song
    monkeypatch.setattr(migration, "_commit_song", lambda *args: (_ for _ in ()).throw(MigrationError("stopped", 503)))
    partial, _ = commit(people, reviewed(source), knowledge, projects, tasks, repertoire, artifacts)
    person_id = next(row["person_id"] for row in partial["records"] if row["domain"] == "people")
    people.record(person_id, {"source": "email", "external_id": "after-partial", "occurred_at": "2026-09-25T11:00:00Z",
                              "direction": "inbound", "summary": "User changed canonical state"})
    monkeypatch.setattr(migration, "_commit_song", original)
...
def test_full_plan_collision_rejects_before_people_or_project_mutation(tmp_path):
    ...
    artifacts.create_binary(name="Collision", data=b"different", mime="application/pdf", kind="pdf", slug=slug)
