...
def create_item(home, title):
    ...
    try:
        identity = store.create_typed_item(
            item_type="bookmark",
            title=title,
            content="Source-grounded note for " + title,
...
def create_collections(home, item_ids):
    ...
    try:
        ...
        smart = store.create_collection(
            name="Unread notes", kind="smart", query="status:unread", icon="search"
        )
...
def entries(home):
    return [
        {
            "entry_id": entry_id,
            "rows": replication_knowledge_collections.read_rows(home, entry_id),
        }
        for entry_id in replication_knowledge_collections.ENTRIES
    ]


def empty_ancestors():
    return {entry_id: {} for entry_id in replication_knowledge_collections.ENTRIES}


def result_ancestors(results):
    return {
        entry_id: results[entry_id].new_ancestors
        for entry_id in replication_knowledge_collections.ENTRIES
    }
...
def test_two_home_current_collections_and_memberships_preserve_exact_canonical_rows(
    tmp_path,
):
    ...
    assert collections["entry_id"] == replication_knowledge_collections.COLLECTION_ENTRY
    assert memberships["entry_id"] == replication_knowledge_collections.MEMBERSHIP_ENTRY
    assert {row["id"] for row in collections["rows"]} == {manual, smart}
    assert len(memberships["rows"]) == 2
    assert all(
        set(row["data"]) == {"collection_id", "item_id", "added_at"}
        for row in memberships["rows"]
    )
    assert all("item_count" not in row["data"] for row in collections["rows"])
    replication_knowledge_collections.validate_entries(projected, source)

    results = replication_knowledge_collections.apply_entries(
        target,
        projected,
        empty_ancestors(),
        conflicts.ConflictQueue(target),
    ...
    assert (
        results[replication_knowledge_collections.COLLECTION_ENTRY].added,
        results[replication_knowledge_collections.MEMBERSHIP_ENTRY].added,
    ) == (2, 2)
    assert entries(target) == projected
    target_store = knowledge(target)
    try:
        assert [row["id"] for row in target_store.resolve_collection(manual)] == list(
            reversed(item_ids)
        )
        assert target_store.get_collection(smart)["query"] == "status:unread"
        assert target_store.db.execute("SELECT count(*) FROM items_fts").fetchone()[
            0
        ] == len(item_rows)
    finally:
...
def test_fast_forward_exact_replay_and_membership_tombstone(tmp_path):
    ...
    initial = replication_knowledge_collections.apply_entries(
        target,
        baseline,
        empty_ancestors(),
        conflicts.ConflictQueue(target),
    ...
    try:
        assert source_store.update_collection(
            manual, name="Field research", icon="archive"
        )
        assert source_store.remove_from_collection(manual, item_ids[0])
    finally:
        source_store.db.close()
    changed = entries(source)
    forwarded = replication_knowledge_collections.apply_entries(
        target,
        changed,
        ancestors,
        conflicts.ConflictQueue(target),
    ...
    assert forwarded[replication_knowledge_collections.COLLECTION_ENTRY].updated == 1
    assert forwarded[replication_knowledge_collections.MEMBERSHIP_ENTRY].removed == 1
    assert collection(target, manual)["name"] == "Field research"
    target_store = knowledge(target)
    try:
        assert [row["id"] for row in target_store.resolve_collection(manual)] == [
            item_ids[1]
        ]
    finally:
        target_store.db.close()

    replayed = replication_knowledge_collections.apply_entries(
        target,
        changed,
        result_ancestors(forwarded),
        conflicts.ConflictQueue(target),
...
def test_collection_conflict_selected_restore_keeps_local_fields(tmp_path):
    ...
    try:
        assert source_store.update_collection(
            manual, name="Remote title", icon="remote"
        )
    ...
    held = replication_knowledge_collections.apply_entries(
        target,
        entries(source),
        ancestors,
        queue,
    ...
    assert held[replication_knowledge_collections.COLLECTION_ENTRY].conflicts == 1
    assert collection(target, manual)["name"] == "Local title"
    pending = queue.items(status=conflicts.STATUS_NEEDS_REVIEW)
    assert (
        len(pending) == 1
        and pending[0].entry_id == replication_knowledge_collections.COLLECTION_ENTRY
    )
    restored = replication_knowledge_collections.restore_fields(
        target,
        pending[0].id,
        ["name"],
    ...
    assert replication_knowledge_collections._timestamp(
        merged["updated_at"], "updated_at"
    ) > replication_knowledge_collections._timestamp(merged["created_at"], "created_at")
    assert (
...
def test_collection_tombstone_removes_memberships_but_preserves_items(tmp_path):
    ...
    try:
        ...
        source_store.db.close()
    removed = replication_knowledge_collections.apply_entries(
        target,
        entries(source),
        result_ancestors(initial),
        conflicts.ConflictQueue(target),
        "2026-09-25T14:30:00+00:00",
    )
    assert removed[replication_knowledge_collections.COLLECTION_ENTRY].removed == 2
    assert removed[replication_knowledge_collections.MEMBERSHIP_ENTRY].removed == 2
    target_store = knowledge(target)
    try:
        assert target_store.list_collections() == []
        assert all(target_store.get_item(item_id) is not None for item_id in item_ids)
        assert (
            target_store.db.execute("SELECT count(*) FROM collection_items").fetchone()[
                0
            ]
            == 0
        )
...
def test_missing_item_and_missing_collection_fail_before_any_mutation(tmp_path):
    ...
    with pytest.raises(ValueError, match="missing canonical item"):
        replication_knowledge_collections.apply_entries(
            target,
            canonical,
            empty_ancestors(),
            conflicts.ConflictQueue(target),
    ...
    assert (
        replication_knowledge_collections.read_rows(
            target, replication_knowledge_collections.COLLECTION_ENTRY
        )
        == []
    )
    assert (
        replication_knowledge_collections.read_rows(
            target, replication_knowledge_collections.MEMBERSHIP_ENTRY
        )
        == []
    )
...
def test_exact_schema_hash_identity_duplicate_names_and_authority_fields_are_rejected(
    tmp_path,
):
    source, target = tmp_path / "source", tmp_path / "target"
    ...
    for payload in invalid:
        with pytest.raises(ValueError):
            replication_knowledge_collections.validate_entries(payload, source)
        with pytest.raises(ValueError):
            replication_knowledge_collections.apply_entries(
                target,
                payload,
    ...
    with pytest.raises(ValueError, match="coverage"):
        replication_knowledge_collections.validate_entries([])
    with pytest.raises(ValueError, match="coverage"):
        replication_knowledge_collections.validate_entries(list(reversed(canonical)))
...
def test_membership_conflict_is_held_without_overwriting_added_at(tmp_path):
    ...
    ancestors = result_ancestors(initial)
    source_membership = replication_knowledge_collections.read_rows(
        source, replication_knowledge_collections.MEMBERSHIP_ENTRY
    )[0]
    target_membership = replication_knowledge_collections.read_rows(
        target, replication_knowledge_collections.MEMBERSHIP_ENTRY
    )[0]
    source_store, target_store = knowledge(source), knowledge(target)
    try:
        source_store.db.execute(
            ...
            (
                ...
                source_membership["data"]["collection_id"],
                source_membership["data"]["item_id"],
            ),
        )
        ...
        target_store.db.execute(
            "UPDATE collection_items SET added_at=? WHERE collection_id=? AND item_id=?",
            (
                "2026-09-25T15:05:00+00:00",
                target_membership["data"]["collection_id"],
                target_membership["data"]["item_id"],
            ),
        )
    ...
    queue = conflicts.ConflictQueue(target)
    held = replication_knowledge_collections.apply_entries(
        target, entries(source), ancestors, queue, "2026-09-25T15:10:00+00:00"
    )
    assert held[replication_knowledge_collections.MEMBERSHIP_ENTRY].conflicts == 1
    actual = {
        row["id"]: row
        for row in replication_knowledge_collections.read_rows(
            target, replication_knowledge_collections.MEMBERSHIP_ENTRY
        )
    }
    assert (
        actual[target_membership["id"]]["data"]["added_at"]
        == "2026-09-25T15:05:00+00:00"
    )
    pending = [
        row
        for row in queue.items(status=conflicts.STATUS_NEEDS_REVIEW)
        if row.entry_id == replication_knowledge_collections.MEMBERSHIP_ENTRY
    ]
