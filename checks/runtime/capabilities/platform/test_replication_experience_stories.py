...
from gideon.workspace.capabilities.platform import (
    replication_experience_stories as adapter,
)
...
def test_fast_forward_conflict_restore_and_exact_graph_validation(tmp_path):
    ...
    remote = source_store.save(
        {**graph("Remote title"), "revision": original["revision"]}, original["id"]
    )
    ...
    local = local_store.save(
        {**graph("Local title"), "revision": remote["revision"]}, original["id"]
    )
    changed = copy.deepcopy(graph("Remote title"))
    changed["nodes"][1]["text"] = "The source orchard is in bloom."
    remote = source_store.save(
        {**changed, "revision": remote["revision"]}, original["id"]
    )
...
def test_tombstone_preserves_local_session_and_its_pinned_story_revision(tmp_path):
    ...
    with sqlite3.connect(target / "capabilities/experience.sqlite3") as database:
        assert database.execute(
            "SELECT deleted FROM stories WHERE id=?", (story["id"],)
        ).fetchone() == (1,)
...
def test_rejects_dangling_graphs_private_fields_and_identity_mismatch_before_write(
    tmp_path,
):
    source, target = tmp_path / "source", tmp_path / "target"
