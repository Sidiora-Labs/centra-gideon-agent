...
from gideon.workspace.capabilities.platform import replication_creative_direction as adapter


def plan(extra=False):
    values = [
        {"id": "verify", "title": "Verify source", "operation": "source.verify", "depends_on": []},
        {"id": "publish", "title": "Publish treatment", "operation": "treatment.snapshot", "depends_on": ["verify"]},
    ]
    if extra:
        values.append({"id": "release", "title": "Verify release", "operation": "source.verify", "depends_on": ["publish"]})
    return values
...
def prepare_work(home, suffix="one"):
    ...
    work = works.create({
        "request_id": "work-" + suffix, "title": "Signal " + suffix, "kind": "work", "prompt": "",
        "author_ref": None, "universe_ref": None, "active_draft_id": None,
    })
    result = works.draft(work["id"], {
        "request_id": "draft-" + suffix, "revision": 1,
        "text": "The lighthouse answered " + suffix + ".", "note": "approved",
    })
...
def test_projection_preserves_current_pins_outputs_and_excludes_private_state(tmp_path):
    ...
    assert [step["status"] for step in complete_row["data"]["steps"]] == ["done", "done"]
    verify_result = complete_row["data"]["steps"][0]["result"]
    assert verify_result["verified_sources"] == [{"kind": "work", "id": work["id"], "revision": 2}]
    ...
    assert pin["content_hash"] == hashlib.sha256(b"The lighthouse answered complete.").hexdigest()
    assert pending_row["data"]["sources"][0]["chapters"][0]["work_id"] == pending_work["id"]
    assert pending_row["data"]["sources"][0]["chapters"][0]["draft_id"] == pending_draft["id"]
    ...
    with sqlite3.connect(source / "capabilities/creative/direction.sqlite3") as database:
        assert database.execute("SELECT count(*) FROM direction_requests").fetchone()[0] == 2
    outcome = adapter.apply_rows(target, adapter.ENTRY_ID, projected, {}, conflicts.ConflictQueue(target), "2026-09-25T08:00:00+00:00")
    ...
    with sqlite3.connect(target / "capabilities/creative/direction.sqlite3") as database:
        assert database.execute("SELECT count(*) FROM direction_requests").fetchone()[0] == 0
    assert WorkStore(target).list()["items"] == []
    assert DirectionStore(target).artifacts.get(output["artifact_id"], version=1) is None
    assert DirectionStore(target).artifacts.get(pin["artifact_id"], version=1) is None
    started = DirectionStore(target).control(draft_project["id"], {"revision": 1, "action": "start"})
...
def test_two_home_conflict_selected_restore_and_tombstone_converge(tmp_path):
    ...
    initial = rows(first)
    received = adapter.apply_rows(second, adapter.ENTRY_ID, initial, {}, conflicts.ConflictQueue(second), "2026-09-25T09:00:00+00:00")
    ancestor = received.new_ancestors
    assert ancestor == {project["id"]: conflicts.row_sha(initial[0])}
    local = DirectionStore(first).control(project["id"], {"revision": 1, "action": "start"})
    remote = DirectionStore(second).replace_plan(project["id"], {"revision": 1, "steps": plan(extra=True)})
    ...
    conflicted = adapter.apply_rows(first, adapter.ENTRY_ID, rows(second), ancestor, queue, "2026-09-25T09:05:00+00:00")
    ...
    resolved = adapter.restore_fields(first, conflict.id, ["steps"], "2026-09-25T09:10:00+00:00")
    ...
    copied = adapter.apply_rows(replica, adapter.ENTRY_ID, baseline, {}, conflicts.ConflictQueue(replica), "2026-09-25T10:00:00+00:00")
    assert DirectionStore(replica).get(doomed["id"]) == doomed
    adapter.write_row(source, adapter.ENTRY_ID, None, doomed["id"])
    assert rows(source) == []
    removed = adapter.apply_rows(replica, adapter.ENTRY_ID, [], copied.new_ancestors, conflicts.ConflictQueue(replica), "2026-09-25T10:05:00+00:00")
...
def test_two_home_fast_forward_and_replay_preserve_canonical_revision(tmp_path):
    ...
    initial = adapter.apply_rows(
        replica, adapter.ENTRY_ID, initial_rows, {}, conflicts.ConflictQueue(replica),
    ...
    changed = DirectionStore(source).control(project["id"], {"revision": 1, "action": "start"})
    remote_rows = rows(source)
    forwarded = adapter.apply_rows(
        replica, adapter.ENTRY_ID, remote_rows, initial.new_ancestors, conflicts.ConflictQueue(replica),
    ...
    replayed = adapter.apply_rows(
        replica, adapter.ENTRY_ID, remote_rows, forwarded.new_ancestors, conflicts.ConflictQueue(replica),
...
def test_complete_batch_preflight_rejects_invalid_pins_outputs_private_fields_and_duplicates(tmp_path):
    source, target = tmp_path / "source", tmp_path / "target"
    completed, _, _ = create_project(source, "validate", complete=True)
    canonical = rows(source)[0]

    invalid = []
    value = copy.deepcopy(canonical); value["data"]["sources"][0]["chapters"][0]["artifact_version"] = 0; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["sources"][0]["chapters"][0]["content_hash"] = "bad"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["sources"][0]["chapters"][0]["chapter_id"] = "unexpected"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["sources"][0]["revision"] = True; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["sources"][0]["kind"] = "prompt"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["sources"] = []; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][0]["depends_on"] = ["later"]; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][0]["operation"] = "shell.execute"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][0]["result"]["verified_sources"] = []; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][0]["result"]["verified_at"] = "yesterday"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][1]["result"]["artifact_version"] = 0; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][1]["result"]["content_hash"] = "0" * 63; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][1]["result"]["path"] = "/etc/passwd"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][1]["attempts"] = True; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["steps"][1]["status"] = "claimed"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["status"] = "executing"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["revision"] = 0; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["updated_at"] = "2020-01-01T00:00:00+00:00"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["request_id"] = "private-ledger"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["sources"][0]["credential_ref"] = "secret"; invalid.append(value)
    for row in invalid:
    ...
    with pytest.raises(ValueError, match="unique"):
        adapter.validate_entries([{"entry_id": adapter.ENTRY_ID, "rows": [canonical, canonical]}])
    with pytest.raises(ValueError):
        adapter.apply_rows(target, adapter.ENTRY_ID, [invalid[0]], {}, conflicts.ConflictQueue(target), "2026-09-25T11:00:00+00:00")
    assert rows(target) == []
...
def test_writer_uses_direction_authority_and_never_materializes_dependencies_or_ledgers(tmp_path):
    source, target = tmp_path / "source", tmp_path / "target"
    ...
    with sqlite3.connect(target / "capabilities/creative/direction.sqlite3") as database:
        stored = json.loads(database.execute("SELECT record FROM direction_projects WHERE id=?", (project["id"],)).fetchone()[0])
