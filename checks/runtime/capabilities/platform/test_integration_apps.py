...
from gideon.interfaces.dashboard.handlers.capabilities_integration_apps import PREFIX, register
...
from gideon.workspace.capabilities.platform.integration_apps.contracts import MUTATIONS, OPERATIONS, configuration, plan
from gideon.workspace.capabilities.platform.integration_apps.store import IntegrationApps, jira_report
from gideon.workspace.capabilities.platform.integration_apps.tools import IntegrationAppTools
...
def connection(kind="jira", **changes):
    values = {
        "kind": kind,
        "label": kind.title(),
        "endpoint": {"jira": "https://team.atlassian.net", "datadog": "https://api.datadoghq.eu", "github": "https://api.github.com"}[kind],
...
def test_jira_read_plans_match_documented_cloud_routes():
    ...
    assert auth == {"operation": "jira_auth", "method": "GET", "path": "/rest/api/3/myself", "params": {}, "body": None, "mutates": False}
    ...
    assert all(not row["mutates"] for row in (auth, projects, boards, sprints, search, issue, transitions))


def test_jira_write_plans_are_exact_and_marked_mutating():
    create = plan("jira", "jira_create", {"project": "OPS", "issue_type": "10001", "summary": "Release", "description": "Ship safely"})
    update = plan("jira", "jira_update", {"issue": "OPS-4", "summary": "Release now", "description": "Reviewed", "labels": ["release", "approved"]})
    comment = plan("jira", "jira_comment", {"issue": "OPS-4", "comment": "Owner reviewed"})
    transition = plan("jira", "jira_transition", {"issue": "OPS-4", "transition_id": "31"})
    delete = plan("jira", "jira_delete", {"issue": "OPS-4"})
    sprint = plan("jira", "jira_sprint_assign", {"sprint_id": 9, "issues": ["OPS-4", "OPS-5"]})
    ...
    assert all(row["mutates"] for row in (create, update, comment, transition, delete, sprint))


def test_datadog_log_search_is_bounded_and_literal_safe():
    request = plan("datadog", "datadog_errors", {"service": "gateway", "environment": "production", "from": "2026-09-01T00:00:00Z", "to": "2026-09-02T00:00:00Z", "cursor": "page-2"})
    assert request["method"] == "POST"
    assert request["path"] == "/api/v2/logs/events/search"
    assert request["body"]["filter"]["query"] == 'status:error service:"gateway" env:"production"'
    ...
    for service in ('bad"value', "bad\\value", "bad\nvalue"):
        with pytest.raises(DomainError):
            plan("datadog", "datadog_errors", {"service": service, "environment": "prod", "from": "2026-09-01T00:00:00Z", "to": "2026-09-02T00:00:00Z"})
    with pytest.raises(DomainError):
        plan("datadog", "datadog_errors", {"service": "api", "environment": "prod", "from": "2026-01-01T00:00:00Z", "to": "2026-09-02T00:00:00Z"})


def test_github_repository_and_actions_secret_plans():
    repositories = plan("github", "github_repos", {"page": 3})
    secrets = plan("github", "github_secrets", {"repository": "Sidiora-Labs/centra", "page": 1})
    archive = plan("github", "github_archive", {"repository": "Sidiora-Labs/centra", "archived": True})
    sync = plan("github", "github_secret_sync", {"repository": "Sidiora-Labs/centra", "name": "DEPLOY_TOKEN", "credential_ref": "production-deploy"})
    ...
    assert "production-deploy" not in json.dumps(headers(connection("github"), lambda name: "token"))
    with pytest.raises(DomainError):
        plan("github", "github_secret_sync", {"repository": "Sidiora-Labs/centra", "name": "GITHUB_TOKEN", "credential_ref": "reserved"})
    with pytest.raises(DomainError):
...
def test_every_declared_operation_belongs_to_one_provider_and_mutation_set_is_complete():
    ...
    for operation, provider in OPERATIONS.items():
        wrong = next(value for value in ("jira", "datadog", "github") if value != provider)
        with pytest.raises(DomainError):
...
def test_named_credential_headers_use_real_store_and_never_enter_persistence(tmp_path):
    ...
    resolve = lambda name: credentials.resolve(name).secret
    jira = headers(connection("jira"), resolve)
    datadog = headers(connection("datadog"), resolve)
    github = headers(connection("github"), resolve)
    assert base64.b64decode(jira["Authorization"].removeprefix("Basic ")).decode() == "owner@example.com:jira-private-token"
...
def test_missing_named_credential_fails_before_network(store):
    ...
    run = store.prepare(saved["id"], {"request_id": "missing-credential", "operation": "github_archive", "input": {"repository": "Sidiora-Labs/centra", "archived": True}})
    result = __import__("asyncio").run(store.execute(run["id"], {"revision": run["revision"], "confirm": True}))
...
def test_store_persists_connections_review_plans_and_idempotency(store):
    ...
    request = {"request_id": "review-1", "operation": "jira_create", "input": {"project": "OPS", "issue_type": "10001", "summary": "Release", "description": "Ship"}}
    ...
    with pytest.raises(DomainError) as duplicate:
        store.prepare(jira["id"], {**request, "input": {**request["input"], "summary": "Different"}})
...
def test_store_requires_exact_owner_confirmation_and_connection_revision(store):
    jira = store.save_connection(connection("jira"))
    run = store.prepare(jira["id"], {"request_id": "review-2", "operation": "jira_delete", "input": {"issue": "OPS-8"}})
    with pytest.raises(DomainError) as unconfirmed:
        __import__("asyncio").run(store.execute(run["id"], {"revision": 1, "confirm": False}))
    assert unconfirmed.value.status == 409
    with pytest.raises(DomainError):
        __import__("asyncio").run(store.execute(run["id"], {"revision": 2, "confirm": True}))
    assert store.get(run["id"])["status"] == "prepared"
    updated = store.save_connection({**connection("jira", label="Renamed"), "revision": 1}, jira["id"])
    assert updated["revision"] == 2
    with pytest.raises(DomainError) as changed:
        __import__("asyncio").run(store.execute(run["id"], {"revision": 1, "confirm": True}))
...
def test_repository_inventory_retains_local_annotations_across_remote_refresh(store):
    ...
    store.ingest_repos(github["id"], [{"full_name": "Sidiora-Labs/centra", "archived": False, "private": True}])
    ...
    annotated = store.annotate(github["id"], rows[0]["name"], {"revision": 1, "flags": {"managed": True, "deploy": False}, "secret_refs": {"DEPLOY_TOKEN": "production-deploy"}})
    ...
    store.ingest_repos(github["id"], [{"full_name": "Sidiora-Labs/centra", "archived": True, "private": True}])
    ...
    with pytest.raises(DomainError):
        store.annotate(github["id"], refreshed["name"], {"revision": 2, "flags": {}, "secret_refs": {}})


def test_jira_report_is_explicitly_partial_and_counts_observed_rows():
    result = jira_report({"issues": [
        {"fields": {"status": {"name": "Open"}, "assignee": {"displayName": "Ada"}}},
...
def test_native_tools_require_canonical_approval_for_execution(store):
    ...
    definitions = {tool.name: tool for tool in __import__("asyncio").run(tools.list_tools())}
    ...
    prepared = __import__("asyncio").run(tools.invoke("integration_apps_prepare", {"connection_id": github["id"], "request_id": "native-1", "operation": "github_archive", "input": {"repository": "Sidiora-Labs/centra", "archived": True}}))
    ...
    executed = __import__("asyncio").run(tools.invoke("integration_apps_execute", {"run_id": receipt["id"], "revision": receipt["revision"]}))
    ...
    invalid = __import__("asyncio").run(tools.invoke("integration_apps_execute", {"run_id": receipt["id"]}))
...
async def test_owner_http_review_flow_persists_without_external_mutation(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        prepared_response = await client.post(PREFIX + f"/connections/{jira['id']}/prepare", json={"request_id": "http-1", "operation": "jira_comment", "input": {"issue": "OPS-4", "comment": "Reviewed by owner"}})
        ...
        rejected = await client.post(PREFIX + f"/runs/{prepared['id']}/execute", json={"revision": 1, "confirm": False})
        assert rejected.status == 409
        assert (await rejected.json())["error"] == "Explicit reviewed execution confirmation required"
        executed = await client.post(PREFIX + f"/runs/{prepared['id']}/execute", json={"revision": 1, "confirm": True})
...
def test_manifest_declares_three_aliases_and_unique_native_provider():
    path = Path("runtime/gideon/extensions/apps/native/gideon-integration-apps/app.json")
    manifest = json.loads(path.read_text())
    assert manifest["name"] == "gideon-integration-apps"
    assert manifest["native"] is True
    assert manifest["provider"]["type"] == "tool"
    assert manifest["provider"]["implementation"] == "gideon.workspace.capabilities.platform.integration_apps.tools:create_provider"
