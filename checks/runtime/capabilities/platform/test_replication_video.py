...
def image(store, color, slug):
    ...
    return store.sketches.artifacts.create_binary(name=color, slug=slug, data=raw.getvalue(), mime="image/png")
...
def seed(home, suffix="one", second=False):
    ...
    body = {
        ...
        "segments": [
            {"artifact_id": red.slug, "version": red.version, "kind": "image", "start": 0, "duration": 1},
            {"artifact_id": blue.slug, "version": blue.version, "kind": "image", "start": 0, "duration": 1},
        ],
        "overlays": [{"artifact_id": red.slug, "version": red.version, "start": 0, "duration": 1, "x": 0, "y": 0, "width": 8, "height": 8}],
        "audio": [],
    }
    timeline = store.timelines.save(body)
    if second:
        timeline = store.timelines.save({**body, "title": "Sequence revised " + suffix, "revision": 1, "request_id": "timeline-revised-" + suffix}, timeline["id"])
    return store, timeline, body


def materialize_pins(home, row):
    store = jobs(home)
    identities = {entry["artifact_id"] for key in ("segments", "overlays") for entry in row["data"][key]}
...
def test_projection_is_current_only_with_exact_pins_and_local_authority(tmp_path):
    ...
    assert "request_id" not in encoded and "private" not in encoded and "events" not in encoded
    ...
    applied = adapter.apply_rows(target, adapter.ENTRY_ID, projected, {}, conflicts.ConflictQueue(target), "2026-09-25T12:00:00+00:00")
    assert applied.added == 1 and applied.updated == applied.removed == applied.conflicts == 0
    assert TimelineStore(target / "capabilities/media/timelines.sqlite3", store.videos).get(current["id"]) == current
...
def test_two_home_actual_edit_conflict_restore_and_tombstone(tmp_path):
    ...
    baseline = rows(first)
    initial = adapter.apply_rows(second, adapter.ENTRY_ID, baseline, {}, conflicts.ConflictQueue(second), "2026-09-25T12:05:00+00:00")
    second_jobs = materialize_pins(second, baseline[0])
    local = first_jobs.timelines.save({**body, "title": "Local title", "segments": list(reversed(body["segments"])), "revision": 1, "request_id": "local-edit"}, project["id"])
    peer = second_jobs.timelines.save({**body, "title": "Peer title", "revision": 1, "request_id": "peer-edit"}, project["id"])
    queue = conflicts.ConflictQueue(first)
    outcome = adapter.apply_rows(first, adapter.ENTRY_ID, rows(second), initial.new_ancestors, queue, "2026-09-25T12:10:00+00:00")
    ...
    assert merged["title"] == "Peer title" and merged["segments"] == local["segments"] and merged["revision"] == 3
    for entry in merged["segments"]:
        assert first_jobs.sketches.artifacts.raw_bytes(entry["artifact_id"], version=entry["version"])

    source, replica = tmp_path / "delete-source", tmp_path / "delete-replica"
    source_jobs, doomed, _ = seed(source, "doomed")
    copied = adapter.apply_rows(replica, adapter.ENTRY_ID, rows(source), {}, conflicts.ConflictQueue(replica), "2026-09-25T12:20:00+00:00")
    with source_jobs.timelines.db() as database:
        database.execute("DELETE FROM timelines WHERE id=?", (doomed["id"],))
    removed = adapter.apply_rows(replica, adapter.ENTRY_ID, [], copied.new_ancestors, conflicts.ConflictQueue(replica), "2026-09-25T12:25:00+00:00")
    assert removed.removed == 1 and rows(replica) == []


def test_complete_batch_rejects_private_malformed_and_cross_project_dependencies(tmp_path):
    source, target = tmp_path / "source", tmp_path / "target"
    _, project, _ = seed(source, "validate")
    canonical = rows(source)[0]
    invalid = []
    value = copy.deepcopy(canonical); value["data"]["request_id"] = "private-request"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["segments"][0]["version"] = 0; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["segments"][0]["kind"] = "document"; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["duration"] = 99; invalid.append(value)
    value = copy.deepcopy(canonical); value["data"]["overlays"][0]["artifact_id"] = "../secret"; invalid.append(value)
    ...
    other["id"] = "second-project"; other["data"]["id"] = "second-project"
    ...
    with pytest.raises(ValueError, match="project identities as artifact dependencies"):
        adapter.validate_entries([{"entry_id": adapter.ENTRY_ID, "rows": [canonical, other]}])
    with pytest.raises(ValueError, match="complete entry coverage"):
        adapter.validate_entries([])
    with pytest.raises(ValueError):
        adapter.apply_rows(target, adapter.ENTRY_ID, [invalid[1]], {}, conflicts.ConflictQueue(target), "2026-09-25T12:30:00+00:00")
    assert rows(target) == []
...
def test_writer_preserves_pins_without_materializing_jobs_requests_or_bytes(tmp_path):
    ...
    source_jobs.submit({"operation": "timeline_render", "request_id": "private-render", "input": {"timeline_id": project["id"], "revision": project["revision"]}})
    row = rows(source)[0]
    adapter.write_row(target, adapter.ENTRY_ID, row, project["id"])
    reopened = TimelineStore(target / "capabilities/media/timelines.sqlite3", source_jobs.videos)
    assert reopened.get(project["id"]) == project
    for key in ("segments", "overlays", "audio"):
        for ref in project[key]:
            assert NativeArtifactProvider(target / "artifacts").raw_bytes(ref["artifact_id"], version=ref["version"]) is None
    with sqlite3.connect(target / "capabilities/media/timelines.sqlite3") as database:
        assert database.execute("SELECT count(*) FROM requests").fetchone()[0] == 0
    assert not (target / "capabilities/media/jobs.sqlite3").exists()
