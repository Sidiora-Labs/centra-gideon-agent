...
def select(identifier):
    return s.set_composition(
        identifier, {"revision": s.composition_state()["revision"], "select": True}
    )
...
def test_canonical_store_default_presets_and_order_size_membership(home):
    ...
    result = s.set_composition(
        view.id,
        patch(
            [
                {"ref": "core:tasks", "size": "full"},
                {"ref": "core:system-health", "size": "s"},
            ]
        ),
    )
    assert result["revision"] == 1
    stored = s.get_view(view.id)
    assert [(tile.ref, tile.size, tile.order) for tile in stored.tiles] == [
        ("core:tasks", "full", 0),
        ("core:system-health", "s", 1),
    ]
    ...
    reordered = s.set_composition(
        view.id,
        patch(
            [
                {"ref": "core:system-health", "size": "l"},
                {"ref": "core:tasks", "size": "m"},
            ]
        ),
    )
    assert reordered["selected_view"] == view.id
    assert [tile.ref for tile in s.get_view(view.id).tiles] == [
        "core:system-health",
        "core:tasks",
    ]
...
@pytest.mark.parametrize(
    "tiles",
    [
        [{"ref": "core:unknown", "size": "m"}],
        [{"ref": "core:tasks", "size": "x"}],
        [{"ref": "artifact:sample", "size": "m"}],
        [{"ref": "core:tasks", "size": "m", "x": 1}],
        [{"ref": "core:tasks", "size": "m"}, {"ref": "core:tasks", "size": "s"}],
    ],
)
...
async def test_signed_real_composition_and_existing_view_http_paths(home):
    ...
    app.router.add_post(
        ...
        views.api_dashboard_view_tile_resolve,
    )
    async with TestClient(TestServer(app)) as client:
        ...
        assert response.status == 200
        view = next(
            row for row in (await response.json())["views"] if not row["preset"]
        )
        response = await client.put(
            PREFIX + "/" + view["id"],
            json={"revision": 0, "tiles": [{"ref": "core:schedule", "size": "full"}]},
        )
        assert response.status == 200
        assert (await response.json())["revision"] == 1
        response = await client.put(
            PREFIX + "/" + view["id"], json={"revision": 1, "select": True}
        )
        assert response.status == 200
        ...
        stored = next(
            row for row in (await response.json())["views"] if row["id"] == view["id"]
        )
        assert [tile["ref"] for tile in stored["tiles"]] == [
            "core:schedule",
            "artifact:real-overlay",
        ]
        response = await client.post(
            "/api/dashboard/views/" + view["id"] + "/tiles/resolve",
            json={"ref": "artifact:real-overlay", "keep": False},
        )
        assert response.status == 200
        assert [tile["ref"] for tile in (await response.json())["view"]["tiles"]] == [
            "core:schedule"
        ]
        assert (
            await client.put(PREFIX + "/overview", json={"revision": 2, "tiles": []})
        ).status == 409
...
async def test_native_selection_uses_canonical_view_and_actor(home):
    ...
    try:
        changed = await provider.invoke(
            "platform_dashboard_select", {"view_id": view.id, "revision": 0}
        )
    ...
    try:
        denied = await provider.invoke(
            "platform_dashboard_select", {"view_id": "overview", "revision": 1}
        )
    ...
    tool = next(
        ...
        if item.name == "platform_dashboard_select"
    )
