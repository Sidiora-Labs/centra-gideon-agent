...
CASES = (
    ...
    (
        replication_music_video.SCOPE,
        replication_music_video,
        {
            "id": "music-video-project",
            "revision": 1,
            "title": "Beat cut",
            "track_id": "track-one",
            ...
            "scenes": [
                {
                    "id": "opening",
                    "image_ref": {"slug": "scene-image", "version": 1},
                    "beats": 2,
                    "start_seconds": 0.0,
            ...
            "duration_seconds": 1.0,
            "audio_ref": {"slug": "source-audio", "version": 1},
        },
    ),
)
...
def test_video_domains_are_declared_and_default_denied(tmp_path, scope, adapter, data):
    ...
    first_store.put(
        second_id["peer_id"],
        peer_record(second_id, "https://second.example", scope, send=False),
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("scope,adapter,data", CASES)
async def test_signed_video_domain_route_dispatches_canonical_current_rows(
    tmp_path, scope, adapter, data
):
    first, second = tmp_path / "first", tmp_path / "second"
    write(adapter, first, data)
    ...
    assert adapter.read_rows(second, adapter.ENTRY_ID) == [
        {"id": data["id"], "data": data}
    ]
    assert NativeArtifactProvider(second / "artifacts").list() == []
    database_path = second / (
        "capabilities/media/timelines.sqlite3"
        if scope == replication_video.SCOPE
        else "capabilities/music/music_video.sqlite3"
    )
    authority_table = "requests" if scope == replication_video.SCOPE else "jobs"
    with sqlite3.connect(database_path) as database:
        assert (
            database.execute(f"SELECT count(*) FROM {authority_table}").fetchone()[0]
            == 0
        )


@pytest.mark.parametrize("scope,adapter,data", CASES)
def test_central_video_conflicts_restore_selected_fields(
    tmp_path, scope, adapter, data
):
    first, second = tmp_path / "first", tmp_path / "second"
