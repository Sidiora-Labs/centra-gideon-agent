...
async def authenticate(client):
    response = await client.get(
        PREFIX, params={"token": generate_token("comparison-owner")}
    )
...
def imported(**changes):
    return {
        "model": "documented-model-a",
        "corpus": "public-corpus-v1",
        "metric": "accuracy-percent",
        "score": 75.5,
        "source": "https://example.org/report",
        "observed_at": "2026-09-01",
        "methodology": "User supplied source record; values are test data, not a model evaluation.",
        **changes,
    }
...
async def test_two_attributed_models_persist_compare_and_delete_without_changing_eval_artifacts():
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        second = await client.post(
            PREFIX, json=imported(model="documented-model-b", score=81)
        )
        assert second.status == 200
        pair = (await second.json())["imports"]
        assert {row["model"] for row in pair} == {
            "documented-model-a",
            "documented-model-b",
        }
...
async def test_recorded_table_history_selection_and_unknown_metrics_native_tool():
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        definition = next(
            tool for tool in definitions if tool.name == "platform_model_comparisons"
        )
...
async def test_corrupt_store_fails_closed_and_auth_rejects_app_scope():
    async with TestClient(TestServer(application())) as client:
        ...
        app = await client.get(
            PREFIX, params={"token": generate_token("owner", app="third-party")}
        )
