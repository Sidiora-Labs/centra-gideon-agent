...
@pytest.mark.asyncio
async def test_creative_work_documents_signed_two_home_round_trip(tmp_path, monkeypatch):
    source, target = tmp_path / "source", tmp_path / "target"
    source.mkdir(); target.mkdir()
    source_store = WorkStore(source)
    work = source_store.create({
        "request_id":"work-central", "title":"Canonical manuscript", "kind":"work",
        "prompt":"Write the ending.", "author_ref":None, "universe_ref":None, "active_draft_id":None,
    })
    first = source_store.draft(work["id"], {
        "request_id":"draft-central-one", "revision":work["revision"],
        "text":"First immutable manuscript.\n", "note":"first",
    })
    second = source_store.draft(work["id"], {
        "request_id":"draft-central-two", "revision":first["work"]["revision"],
    ...
    replicated = replication_adapters.apply_rows(
        target, "creative.works", current, {}, conflicts.ConflictQueue(target),
        "2026-09-25T12:00:00+00:00",
    )
    assert replicated.added == 1

    source_peers, target_peers = PeerStore(source), PeerStore(target)
    source_id, target_id = source_peers.snapshot()["self"], target_peers.snapshot()["self"]
    source_peers.put(target_id["peer_id"], peer_record(target_id, "https://target.example", [adapter.SCOPE]))
    target_peers.put(source_id["peer_id"], peer_record(source_id, "https://source.example", [adapter.SCOPE]))
    sender = ReplicationService(source)
    batch = sender.export_batch(target_id["peer_id"], adapter.SCOPE)
    assert [item["entry_id"] for item in batch["entries"]] == list(adapter.ENTRIES)
    wire = json.dumps(batch, sort_keys=True)
    for forbidden in ("request_id", "work_requests", "work_draft_requests", "credential", "private_key"):
        assert forbidden not in wire
    envelope = {"proof":source_peers.create_proof(target_id["peer_id"], adapter.SCOPE), "payload":batch}
    ...
    async with TestClient(TestServer(application())) as client:
        ...
        accepted = await client.post(
            ...
            headers={"Content-Type":"application/json",
                     "Cookie":"gideon_token_8014=" + generate_token("dashboard:creative-documents")},
        )
        assert accepted.status == 200
        response = await accepted.json()
        assert response["entries"] == [
            {"entry_id":adapter.VERSIONS_ENTRY, "added":3, "updated":0, "removed":0, "conflicts":0},
            {"entry_id":adapter.DRAFTS_ENTRY, "added":2, "updated":0, "removed":0, "conflicts":0},
        ]

    target_store = WorkStore(target)
    assert target_store.revisions(work["id"]) == source_store.revisions(work["id"])
    assert target_store.read_draft(work["id"], first["draft"]["id"])["text"] == "First immutable manuscript.\n"
    assert target_store.read_draft(work["id"], second["draft"]["id"])["text"] == "Second immutable manuscript.\n"
    with sqlite3.connect(target / "capabilities/creative/catalog.sqlite3") as database:
        ...
        assert database.execute("SELECT count(*) FROM work_draft_requests").fetchone() == (0,)
...
def test_creative_work_documents_policy_is_default_denied(tmp_path):
    ...
    source_store.put(target_id["peer_id"], peer_record(target_id, "https://target.example", []))
