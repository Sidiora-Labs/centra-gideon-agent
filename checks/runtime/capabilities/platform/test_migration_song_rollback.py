...
def source_archive():
    ...
    return (
        song_archive(
            song_record(identity="song-rollback", attachments=[attachment]),
            {"score.pdf": raw},
        ),
        raw,
    )
...
def interrupted(tmp_path, *, fingerprint=True):
    ...
    artifacts.create_binary(
        ...
        description="Restored song attachment: score.pdf",
        event_metadata={
            "archive_digest": checked["archive_digest"],
    ...
    repertoire.import_song(
        import_id=import_id,
        source_fingerprint=checked["review_token"],
        item_id="song-rollback",
        ...
        schedule={
            ...
            "last_practiced_at": "2026-09-25T12:00:00+00:00",
            "last_grade": 4,
        },
    )
    plan = {
...
def test_version_and_event_drift_preserves_song_artifact_history_and_journal(tmp_path):
    ...
    assert (
        artifacts.get(slug).version == 2 and artifacts.get(slug, version=1) is not None
    )
    assert artifacts.get(slug).updated_at != original.updated_at
    assert (
        repertoire.get("song-rollback")["title"] == "Midnight Train" and path.exists()
    )
...
def test_metadata_drift_preserves_song_artifact_and_journal(tmp_path):
    ...
    assert (
        artifact.name == "User renamed score"
        and artifact.description == "Retain this history"
    )
    assert (
        repertoire.get("song-rollback")["title"] == "Midnight Train" and path.exists()
    )
...
def test_legacy_ambiguous_ownership_fails_closed_but_complete_replay_stays_exact(
    tmp_path,
):
    home, artifacts, repertoire, ambiguous_path, slug, token, import_id = interrupted(
        tmp_path, fingerprint=False
    )
    ...
    assert all(
        plan.get("ownership_fingerprint") for plan in body["artifacts"] if plan["owned"]
    )
...
def test_prepared_missing_fingerprint_refuses_existing_symlink_before_song_mutation(
    tmp_path,
):
    home, artifacts, repertoire, path, slug, _, _ = interrupted(
        tmp_path, fingerprint=False
    )
    body = json.loads(path.read_text())
    ...
    with repertoire._db() as database:
        assert (
            database.execute(
                "SELECT count(*) FROM items WHERE id='song-rollback'"
            ).fetchone()[0]
            == 1
        )
