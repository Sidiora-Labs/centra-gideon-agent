...
async def main():
    CredentialStore(config_dir()).put(
        "peer-key",
        {"type": "static_token", "value": "qualification-listener-peer-secret"},
    )
    get_default_registry().register_entry(
        ProviderEntry(
            name="Unavailable inference",
            type="uninstalled-model-type",
            model="declared-model",
        )
    )
    register_runner(
        SidecarRunner(
            app="Unavailable GPU runtime",
            worker=config_dir() / "missing-runtime-worker.py",
