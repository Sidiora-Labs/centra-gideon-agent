...
def command(project, request='request-one', revision=0, action='claim', feature='editor'):
    return {'project_id': project.id, 'feature': feature, 'action': action, 'revision': revision, 'request_id': request}


@pytest.mark.asyncio
async def test_actual_project_ownership_persists_replays_and_owner_only_release(project):
    async with TestClient(TestServer(application())) as alice, TestClient(TestServer(application())) as bob:
        ...
        assert row == {'project_id': project.id, 'feature': 'editor', 'owner': 'user:alice', 'revision': 1}
        ...
        forbidden_release = await bob.post(PREFIX, json=command(project, 'bob-release', 1, 'release'))
        assert forbidden_release.status == 403
        stale_release = await alice.post(PREFIX, json=command(project, 'stale-release', 0, 'release'))
        assert stale_release.status == 409
        released = await alice.post(PREFIX, json=command(project, 'alice-release', 1, 'release'))
    ...
    async with TestClient(TestServer(application())) as restarted:
        ...
        history = await restarted.get(PREFIX, params={'project_id': project.id, 'feature': 'editor'})
        events = (await history.json())['history']
        assert [event['action'] for event in events] == ['claim', 'release', 'claim']
        assert [event['actor'] for event in events] == ['user:bob', 'user:alice', 'user:alice']
...
async def test_concurrent_real_http_claims_have_one_winner(project):
    async with TestClient(TestServer(application())) as alice, TestClient(TestServer(application())) as bob:
        ...
        await authenticate(bob, 'bob')
        results = await asyncio.gather(alice.post(PREFIX, json=command(project, 'alice-request')), bob.post(PREFIX, json=command(project, 'bob-request')))
...
async def test_native_session_context_claim_release_and_manifest(project):
    ...
    try:
        ...
        exact_replay = await provider.invoke(claim.name, command(project, 'native-request'))
        ...
        released = await provider.invoke(claim.name, command(project, 'native-release', 1, 'release'))
        assert released.success
        assert json.loads(released.output)['owner'] is None
        spoofed = await provider.invoke(claim.name, {**command(project, 'spoof-request', 2), 'actor': 'administrator'})
...
@pytest.mark.parametrize('changes', [
    {'feature': ''}, {'feature': 'bad feature'}, {'feature': 'x' * 101}, {'revision': True}, {'revision': -1},
    {'revision': '0'}, {'action': 'transfer'}, {'request_id': 'x'}, {'request_id': '../outside'}, {'owner': 'other-user'},
])
...
async def test_missing_project_replay_mismatch_scope_and_immutable_history(project):
    async with TestClient(TestServer(application())) as client:
        ...
        app = await client.post(PREFIX, params={'token': generate_token('owner', app='third-party')}, json=command(project))
        assert app.status in {401, 403}
    async with TestClient(TestServer(application())) as client:
        await authenticate(client)
        missing = await client.post(PREFIX, json={**command(project), 'project_id': 'p-does-not-exist'})
        assert missing.status == 404
        accepted = await client.post(PREFIX, json=command(project))
        assert accepted.status == 200
        altered = await client.post(PREFIX, json=command(project, feature='other-feature'))
        assert altered.status == 409
        assert len(view()['ownership']) == 1
        connection = sqlite3.connect(config_dir() / 'capabilities/platform/feature_ownership.sqlite3')
        try:
