...
async def main():
    ...
    pr = {'state': 'open', 'draft': False, 'head': {'sha': 'a' * 40}, 'changed_files': 1, 'commits': 1, 'title': 'Parser change'}
    source = p.snapshot(pr, [{'filename': 'parser.py', 'patch': '-old\n+new'}], [{'sha': 'a' * 40, 'commit': {'message': 'Parser change'}}])
