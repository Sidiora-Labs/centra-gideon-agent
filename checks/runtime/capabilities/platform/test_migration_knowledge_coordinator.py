...
def records_archive(collections):
    ...
    with tarfile.open(fileobj=output, mode="w:gz") as archive:
        payloads = {
            "snapshot-knowledge/manifest.json": json.dumps(manifest).encode(),
            **{
                f"snapshot-knowledge/data/{name}": value
...
def mixed_source(*, broken=False, inbox=False):
    ...
    domains = {
        ...
        "ideas": [
            {
                "id": IDEA,
                "title": "Coordinate imports",
                "status": "active",
                "oneLiner": "One recovery plan",
                "notes": "Across canonical stores",
                "tags": ["migration"],
                "createdAt": now,
                "updatedAt": now,
            }
        ...
        "journals": [
            {
                "id": "2026-09-25",
                "date": "2026-09-25",
                "content": "Coordinator verified",
                "segments": [
                    {"text": "Coordinator verified", "at": now, "source": "text"}
                ],
                "createdAt": now,
                "updatedAt": now,
            }
        ...
        "memories": [
            {
                "id": MEMORY,
                "title": "Recovery rule",
                "content": "Freeze all stores on drift",
                "mood": "focused",
                "tags": ["durability"],
                "createdAt": now,
                "updatedAt": now,
        ...
        "admin": [
            {
                ...
                "status": "open",
                "nextAction": "Verify references",
                "notes": "Coordinator task",
                "createdAt": now,
                "updatedAt": now,
            }
        ],
        "threads": [
            {
                "id": THREAD,
                "title": "Review knowledge migration",
                "status": "open",
                "priority": "high",
                "nextAction": "Inspect journal",
                "notes": "Final task",
                "tags": [],
                "pinned": False,
                "refs": (
...
def test_five_knowledge_families_share_schema_two_journal_refs_restart_and_replay(
    tmp_path,
):
    source = mixed_source()
    ...
    try:
        ...
        assert (
            ...
            and replay == receipt
            and journal_path.read_bytes() == before
        )
...
def test_late_task_failure_reverse_compensates_knowledge_items_membership_and_bucket(
    tmp_path,
):
    ...
    assert (
        knowledge.db.execute(
            ...
            (IDEA, MEMORY, LINK, "2026-09-25"),
        ).fetchone()[0]
        == 0
    )
    assert (
        knowledge.db.execute(
            "SELECT count(*) FROM collections WHERE id=?", (BUCKET,)
        ).fetchone()[0]
        == 0
    )
    assert (
        knowledge.db.execute(
            "SELECT count(*) FROM collection_items WHERE collection_id=?", (BUCKET,)
        ).fetchone()[0]
        == 0
    )
    assert (
...
def test_knowledge_edit_is_drift_and_freezes_incomplete_plan(tmp_path):
    ...
    knowledge.db.execute(
        "UPDATE items SET content='user edit',updated_at='2026-09-25T09:00:00Z' WHERE id=?",
        (MEMORY,),
    )
    knowledge.db.commit()
    path = (
    ...
    assert (
        knowledge.get_item(MEMORY)["content"] == "user edit"
        and knowledge.get_item(IDEA) is not None
    )
...
def test_mixed_inbox_preview_defers_writes_to_independent_groups(tmp_path):
    ...
    assert [group["id"] for group in inspected["commit_groups"]] == [
        "canonical",
        "inbox",
    ]
