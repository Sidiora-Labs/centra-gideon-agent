    ...
    return (
        {
            ...
            "body": "Review the changed parser.",
            "changed_files": 1,
            "commits": 1,
        },
        [{"filename": "parser.py", "patch": "-old\n+new", "status": "modified"}],
        [{"sha": "a" * 40, "commit": {"message": "Improve parser"}}],
    )
...
def home(tmp_path, monkeypatch):
    ...
    CredentialStore(tmp_path).save(
        {"github-review": {"type": "static_token", "value_env": "GIDEON_TEST_PR_UNSET"}}
    )
...
def proposed(row, event="COMMENT"):
    row.update(
        ...
        screen=p.parse_decision(
            '{"safe":true,"reason":"No instruction attack identified"}', screening=True
        ),
        proposal=p.parse_decision(
            json.dumps(
                {
                    "eligible": True,
                    "event": event,
                    "body": "Please add a regression case.",
...
@pytest.mark.parametrize(
    "change", ["draft", "closed", "sha", "files", "commits", "binary", "oversize"]
)
...
@pytest.mark.parametrize(
    "text,screening",
    [
        ('{"safe":"true","reason":"x"}', True),
        ('{"safe":true,"reason":"x","command":"run"}', True),
        ("not JSON", True),
        ('{"eligible":true,"event":"APPROVE","body":"ok","reason":"x"}', False),
        ('{"eligible":true,"event":"MERGE","body":"ok","reason":"x"}', False),
        ('{"eligible":true,"event":"COMMENT","body":"","reason":"x"}', False),
    ],
)
def test_model_decisions_are_strict_and_cannot_authorize_other_actions(
    home, text, screening
):
    with pytest.raises(ValueError):
...
def test_authorization_exact_actor_revision_state_and_payload(home):
    ...
    for actor, revision, states in [
        ("user:other", 1, {"awaiting_authorization"}),
        ("user:owner", 0, {"awaiting_authorization"}),
        ("user:owner", True, {"awaiting_authorization"}),
        ("user:owner", 1, {"captured"}),
        ("", 1, {"awaiting_authorization"}),
    ]:
        with pytest.raises(ValueError):
...
def test_exact_review_reconciliation_rejects_wrong_actor_sha_body_and_duplicates(home):
    ...
    row.update(
        status="submitting", submitter="reviewer", submission=p.review_payload(row)
    )
    review = {
        "id": 19,
    ...
    wrong = []
    for key, value in [
        ("body", "same proposal without marker"),
        ("commit_id", "c" * 40),
        ("state", "APPROVED"),
        ("user", {"login": "impostor"}),
    ]:
...
@pytest.mark.asyncio
async def test_missing_actual_credential_and_model_fail_closed_without_forge_write(
    home,
):
...
async def test_signed_http_source_read_and_real_failure_mutations(home):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            PREFIX + "/" + row["id"], json={"action": "screen", "revision": 1}
        )
        assert response.status == 200
        ...
        for payload in [
            {"action": "authorize", "revision": 2},
            {"action": "screen", "revision": 1},
            {"action": "merge", "revision": 2},
            {"action": "screen", "revision": 2, "actor": "other"},
        ]:
