...
from test_remote_sessions import ProtocolRuntime


async def main():
    home = Path(os.environ["GIDEON_HOME"])
    credentials = CredentialStore(home)
    credentials.save({"browser-token": {"type": "static_token", "value": "actual-secret"}})
    ...
    remote_origin = f"http://127.0.0.1:{remote_site._server.sockets[0].getsockname()[1]}"
    bridge = RemoteSessionBridge(home, credentials=credentials)
    bridge.store.save_connection({"id": "browser", "label": "Browser runtime", "base_url": remote_origin, "credential_ref": "browser-token", "agent_id": "main"})
    ...
    await site.start()
    print(json.dumps({"origin": f"http://127.0.0.1:{site._server.sockets[0].getsockname()[1]}", "remote_origin": remote_origin}), flush=True)
    try:
        await asyncio.Event().wait()
    finally:
        await runner.cleanup(); await remote_runner.cleanup()
