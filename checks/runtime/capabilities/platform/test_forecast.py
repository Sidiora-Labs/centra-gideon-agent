...
def add(identifier="clock-one", spec=None, **values):
    trigger = Trigger(
        ...
        name=identifier,
        kind="clock",
        spec=spec or {"kind": "interval", "interval_secs": 60},
...
async def test_actual_clock_times_and_no_schedule_or_claim_mutations(home):
    ...
    assert [event["at"] for event in result["events"]] == [
        NOW + 30,
        NOW + 90,
        NOW + 150,
    ]
    ...
    assert all(
        event["edit_url"] == "#/triggers?open=clock-one" for event in result["events"]
    )
...
async def test_cron_timezone_skip_dates_once_and_expiry_use_real_clock_rules(home):
    cron = add(
        "cron", {"kind": "cron", "expr": "0 15 * * *", "timezone": "Europe/Berlin"}
    )
    skipped = add(
        "skipped",
        {
    ...
    assert [row["at"] for row in result["events"] if row["trigger_id"] == once.id] == [
        NOW + 120
    ]
...
@pytest.mark.asyncio
async def test_actual_registered_store_and_native_rows_merge_without_writes(
    home, tmp_path
):
    add("native")
    external = TriggerStore(tmp_path / "other-store")
    provider_trigger = Trigger(
        id="provider:clock",
        name="Provider clock",
        kind="clock",
        spec={"kind": "interval", "interval_secs": 90},
        workflow={"ref": "audit-workflow"},
    )
    ...
    try:
        result = await forecast.view(now=NOW, horizon=180)
        assert {row["trigger_id"] for row in result["events"]} == {
            "native",
            "provider:clock",
        }
        assert [
            ...
            if row["trigger_id"] == "provider:clock"
        ] == [NOW + 90, NOW + 180]
        assert [
            row for row in result["events"] if row["trigger_id"] == "provider:clock"
        ][0]["edit_url"] == "#/triggers?open=provider%3Aclock"
...
async def test_disabled_event_and_parked_current_admission_and_expired_retry(home):
    ...
    event = Trigger(
        id="manual", name="Manual", kind="manual", workflow={"ref": "audit-workflow"}
    )
    ...
    assert (
        next(row for row in result["excluded"] if row["trigger_id"] == event.id)[
            "next_at"
        ]
        is None
    )
...
async def test_actual_rate_gate_and_cadence_are_reflected_without_running(home):
    ...
    assert all(
        row["cadence_reason"] == "low_execution_success" for row in result["events"]
    )
...
async def test_real_maintenance_dependency_times_stay_unknown(home, tmp_path):
    ...
    project = HierarchyStore().create_project(
        "Dependency project", workspace_dir=str(workspace)
    )
    provider = NativeWorkflowDefProvider()
    defs.register_provider(provider)
    await provider.save_def(
        name="code-project",
        root={"kind": "transform", "id": "actual", "config": {"expr": "done"}},
    )
    watchdog = WorkflowWatchdog()
    try:
        row = await maintenance.create(
            {
                "project_id": project.id,
                "verify_command": "python -m pytest",
                "guard_command": "git diff --check",
            },
            "user:owner",
            watchdog,
        )
        issue = await NativeTaskProvider().create_task(
            title="Pending maintenance finding", task_list_id=row["task_list_id"]
        )
...
async def test_signed_http_and_native_forecast_are_read_only(home):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        tool = next(
            ...
            if item.name == "platform_schedule_forecast"
        )
