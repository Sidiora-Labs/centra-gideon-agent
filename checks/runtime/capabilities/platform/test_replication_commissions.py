...
def source(home, suffix):
    ...
    work = works.draft(
        work["id"],
        {
            ...
            "text": "Canonical manuscript " + suffix + ".",
            "note": "approved",
        },
    )["work"]
    ...
    record = store.create(
        {
            "request_id": "commission-" + suffix,
            "name": "Commission " + suffix,
            "target_ability": "series",
            "brief": {
                "intent": "Develop a pinned treatment.",
                "genre": "mystery",
                "category": "treatment",
                "style": "Precise.",
...
def raw(home, identity):
    with sqlite3.connect(
        home / "capabilities/creative/commissions.sqlite3"
    ) as database:
        return json.loads(
            database.execute(
                "SELECT record FROM creative_commissions WHERE id=?", (identity,)
            ).fetchone()[0]
        )


def test_two_home_projection_preserves_definition_pins_and_materializes_no_execution_authority(
    tmp_path,
):
    ...
    assert (received.added, received.updated, received.removed, received.conflicts) == (
        1,
        0,
        0,
        0,
    ...
    assert (
        stored["enabled"] is False
        and stored["mode"] == "planning"
        and stored["dispatch"] is None
    )
    assert (
        stored["max_attempts"] == 1
        and stored["schedule_state"] == "disabled"
        and stored["next_fire_at"] == ""
    )
    ...
    with sqlite3.connect(
        second / "capabilities/creative/commissions.sqlite3"
    ) as database:
        for table in (
            "creative_commission_requests",
            "creative_commission_runs",
...
def test_fast_forward_conflict_selected_restore_and_deletion_preserve_receiver_authority(
    tmp_path,
):
    first, second = tmp_path / "first", tmp_path / "second"
    ...
    source_changed = first_store.update(
        commission["id"], {"revision": commission["revision"], "name": "Remote title"}
    )
    ...
    assert (
        raw(second, commission["id"])["enabled"] is False
        and TriggerStore(base_dir=second).get(TRIGGER_PREFIX + commission["id"]) is None
    )

    second_store = CommissionStore(second)
    local = second_store.update(
        commission["id"],
        {
            "revision": source_changed["revision"],
            "brief": {**source_changed["brief"], "intent": "Receiver-authored intent."},
        },
    )
    remote = first_store.update(
        commission["id"],
        {"revision": source_changed["revision"], "name": "Second remote title"},
    )
    ...
    assert (
        raw(second, commission["id"])["brief"]["intent"] == "Receiver-authored intent."
    )
    ...
    with pytest.raises(ValueError, match="field restoration"):
        adapter.restore_fields(
            second, conflict.id, ["dispatch"], "2026-09-25T12:08:00+00:00"
        )
    result = adapter.restore_fields(
        second, conflict.id, ["name"], "2026-09-25T12:08:00+00:00"
    )
    assert result["fields"] == ["name"]
    merged = raw(second, commission["id"])
    assert (
        merged["name"] == remote["name"]
        and merged["brief"] == local["brief"]
        and merged["revision"] == local["revision"] + 1
    )
    assert merged["enabled"] is False and merged["dispatch"] is None
    delete_source, delete_target = (
        tmp_path / "delete-source",
        tmp_path / "delete-target",
    )
    _, _, doomed = source(delete_source, "delete")
    initial = rows(delete_source)
    replica = adapter.apply_rows(
        delete_target,
        adapter.ENTRY_ID,
        initial,
        {},
        conflicts.ConflictQueue(delete_target),
    ...
    assert (
        rows(delete_target) == []
        and TriggerStore(base_dir=delete_target).get(TRIGGER_PREFIX + doomed["id"])
        is None
    )


def test_missing_dependencies_private_shapes_and_unrelated_local_records_fail_closed(
    tmp_path,
):
    first, second = tmp_path / "first", tmp_path / "second"
    ...
    bad[0]["data"]["dispatch"] = {"provider": "external"}
    with pytest.raises(ValueError, match="definition"):
        adapter.validate_entries([{"entry_id": adapter.ENTRY_ID, "rows": bad}])
    bad = copy.deepcopy(projected)
    bad[0]["data"]["steps"][1]["depends_on"] = ["missing"]
    with pytest.raises(ValueError, match="earlier"):
        adapter.validate_entries([{"entry_id": adapter.ENTRY_ID, "rows": bad}])
    local = copy.deepcopy(projected)
    local[0]["data"]["name"] = "Unrelated local definition"
    adapter.write_row(second, adapter.ENTRY_ID, local[0], commission["id"])
    queue = conflicts.ConflictQueue(second)
    result = adapter.apply_rows(
        second, adapter.ENTRY_ID, projected, {}, queue, "2026-09-25T12:12:00+00:00"
    )
    assert (
        result.conflicts == 1
        and raw(second, commission["id"])["name"] == "Unrelated local definition"
    )
    assert queue.items(status=conflicts.STATUS_NEEDS_REVIEW)[0].ancestor_sha == ""
    assert (
        adapter.missing_dependencies(second, projected[0])
        == projected[0]["data"]["sources"]
    )
