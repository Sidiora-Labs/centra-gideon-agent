...
def trigger(identifier='audit-one', interval=60):
    value = Trigger(id=identifier, name=identifier, kind='clock', spec={'kind': 'interval', 'interval_secs': interval}, workflow={'ref': 'audit-workflow'})
...
def opt(value, task_class='structural_audit', enabled=True):
    return c.mutate(value.id, {'revision': c.document()['revision'], 'enabled': enabled, 'task_class': task_class})


async def evidence(value, successes, *, now, offset=0, error=None):
    journal = ExecutionJournal(c._root())
    for index, success in enumerate(successes):
        outcome = FireResult.read(ToolResult(success=success, error='' if success else 'Task verification failed'), error)
        stamp = now - len(successes) + index - offset
        await journal.append(ExecutionRecord(run_id=f'{value.id}-{offset}-{index}', job_id=value.id, trigger=outcome.exit_type, started_at=stamp, finished_at=stamp, status='success' if outcome.exit_type == 'ok' else 'failure', error=outcome.exception_text))
...
async def test_real_typed_task_failures_slow_actual_interval_reschedule(home):
    ...
    assert next_after_completion(value, completed_at=now, now=now, base_dir=home) == now + 240
    tick = TickPass(TriggerStore(home), now, True, False, home)
    tick.advance(value)
    assert to_epoch(TriggerStore(home).get(value.id).trigger.next_fire_at) == pytest.approx(now + 240)
...
async def test_actual_transport_error_classification_and_unknown_manual_exclusion(home):
    ...
    await evidence(value, [False] * 4, now=now, offset=20, error=ConnectionError('transport unavailable'))
    journal = ExecutionJournal(home)
    for index, kind in enumerate(('auth_unavailable', 'config_error', 'manual', 'scheduled', 'deferred', 'skipped_budget', 'unknown')):
        await journal.append(ExecutionRecord(run_id=f'excluded-{index}', job_id=value.id, trigger=kind, status='failure', started_at=now - 1, finished_at=now - 1))
    result = c.projection(value, now=now)
...
def test_policy_validation_revisions_and_unrelated_manual_trigger(home):
    ...
    manual = Trigger(id='manual-one', name='Manual action', kind='manual', workflow={'ref': 'audit-workflow'})
...
async def test_signed_http_and_actual_native_policy_mutations(home):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.put(PREFIX + '/' + value.id, json={'revision': 0, 'enabled': True, 'task_class': 'audit'})
        assert response.status == 200
        assert (await response.json())['triggers'][0]['task_class'] == 'audit'
        assert (await client.put(PREFIX + '/' + value.id, json={'revision': 0, 'enabled': False, 'task_class': ''})).status == 409
        ...
        try:
            result = await provider.invoke('platform_task_cadence_set', {'trigger_id': value.id, 'revision': 1, 'enabled': False, 'task_class': ''})
        ...
        try:
            denied = await provider.invoke('platform_task_cadence_set', {'trigger_id': value.id, 'revision': 2, 'enabled': True, 'task_class': 'audit'})
