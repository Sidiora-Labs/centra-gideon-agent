...
from checks.runtime.capabilities.platform.test_migration_grouped_song import (
    mixed_archive,
    opened,
    reviewed,
)
...
def event_count(knowledge):
    return knowledge.db.execute(
        "SELECT count(*) FROM capability_knowledge_capture_events WHERE capture_id=?",
        (CAPTURE,),
    ).fetchone()[0]
...
def test_song_failure_retains_completed_inbox_and_retry_does_not_duplicate_event(
    ...
    monkeypatch.setattr(
        migration,
        "_commit_song",
        lambda *args: (_ for _ in ()).throw(MigrationError("song paused", 503)),
    )
    ...
    complete, retry_created = commit(
        people, reviewed(source), knowledge, projects, tasks, repertoire, artifacts
    )
    assert (
        retry_created is True
        and complete["status"] == "complete"
        and event_count(knowledge) == 1
    )
    knowledge.close()


def test_completed_inbox_drift_blocks_pending_song_without_deleting_event(
    tmp_path, monkeypatch
):
    source, _ = mixed_archive(include_inbox=True)
    home = tmp_path / "home"
    ...
    knowledge.db.execute(
        "UPDATE capability_knowledge_captures SET status='error',revision=2 WHERE id=?",
        (CAPTURE,),
    )
    knowledge.db.commit()
    monkeypatch.setattr(migration, "_commit_song", original)
    with pytest.raises(
        MigrationError, match="inbox migration group has canonical identity drift"
    ):
        commit(
            people, reviewed(source), knowledge, projects, tasks, repertoire, artifacts
        )
