...
from gideon.operations.durability.conflicts import STATUS_NEEDS_REVIEW, ConflictQueue
...
def pair(
    a_home, b_home, a_endpoint="https://a.example", b_endpoint="https://b.example"
):
...
def pair_for(a_home, b_home, scope):
    ...
    a.put(
        bid["peer_id"],
        peer_record(bid, "https://b.example", send=[scope], receive=[scope]),
    )
    b.put(
        aid["peer_id"],
        peer_record(aid, "https://a.example", send=[scope], receive=[scope]),
    )
    return aid, bid
    ...
    if scope == "knowledge.records":
        ...
        try:
            identity = store.create_typed_item(
                item_type="note", title=title, content=content, tags=["shared"]
            )
            store.db.commit()
            return identity
        finally:
            store.db.close()
    return IngredientStore(home).create(
        {
            "request_id": "create-" + title.replace(" ", "-"),
            "type": "concept",
            "title": title,
            "body": content,
            "tags": ["shared"],
...
def get_domain_record(home, scope, identity):
    if scope == "knowledge.records":
        store = knowledge_store(home)
        try:
            return store.get_item(identity)
        finally:
            store.db.close()
    try:
        return IngredientStore(home).get(identity)
    except Exception:
        return None
...
def update_domain_record(home, scope, identity, title, content):
    if scope == "knowledge.records":
        store = knowledge_store(home)
        try:
            store.update_item(identity, title=title, content=content)
            store.db.commit()
        finally:
            store.db.close()
    else:
        old = IngredientStore(home).get(identity)
        IngredientStore(home).update(
            identity, {"revision": old["revision"], "title": title, "body": content}
        )
...
def test_two_home_create_edit_delete_and_offline_restart(home, tmp_path):
    ...
    assert (
        ReplicationService(b_home).apply_batch(aid["peer_id"], second)["sequence"] == 2
    )
    ...
    marker = next(
        row
        for row in third["entries"][0]["rows"]
        if row["id"] == created.id + "/project"
    )
    ...
    with sqlite3.connect(b.path) as connection:
        assert (
            connection.execute(
                "SELECT sequence FROM cursors WHERE peer_id=? AND domain=?",
                (aid["peer_id"], SCOPE),
            ).fetchone()[0]
            == 3
        )
...
def test_replay_sequence_gap_mixed_version_coverage_and_sender_denials(home):
    ...
    changed["entries"][0]["rows"][0]["data"]["brief"] = "tampered"
...
def test_disabled_and_directional_policy_deny_before_export_or_apply(home, tmp_path):
    ...
    PeerStore(home).put(
        remote_id["peer_id"],
        peer_record(remote_id, local_peer["endpoint"], revision=1, send=[]),
    )
    ...
    PeerStore(remote_home).put(
        local_id["peer_id"],
        peer_record(local_id, remote_peer["endpoint"], revision=1, receive=[]),
    )
    batch = {
        "schema_version": 1,
        "domain": SCOPE,
        "sender": local_id["peer_id"],
        "sequence": 1,
        "batch_id": "a" * 32,
        "entries": [
            {"entry_id": "projects", "rows": []},
            {"entry_id": "tasks", "rows": []},
        ],
    }
...
async def test_real_signed_guarded_push_between_two_homes(home):
    ...
    with sqlite3.connect(ReplicationService(a_home).path) as connection:
        assert (
            connection.execute(
                "SELECT count(*) FROM ancestors WHERE peer_id=?", (bid["peer_id"],)
            ).fetchone()[0]
            >= 1
        )
...
async def test_signed_creative_direction_scope_uses_canonical_owner_adapter(home):
    ...
    with pytest.raises(ReplicationError, match="policy denies"):
        ReplicationService(denied_a).export_batch(
            denied_peer["peer_id"], "creative.direction"
        )
    ...
    async with TestServer(app) as server:
        ...
        draft = works.draft(
            work["id"],
            {
                "request_id": "direction-draft",
                "revision": 1,
                "text": "A lighthouse answers.",
                "note": "approved",
            },
        )
        project = DirectionStore(a_home).create(
            {
                "request_id": "direction-project",
                "name": "Harbor treatment",
                "treatment": "Cold blue light crosses the harbor.",
                "sources": [
                    {
                        "kind": "work",
                        "id": draft["work"]["id"],
        ...
        result = await ReplicationService(a_home).push(
            bid["peer_id"], "creative.direction"
        )
    assert (
        result["accepted"] is True
        and result["entries"][0]["entry_id"] == "creative.direction_projects"
    )
    ...
    assert (
        NativeArtifactProvider(b_home / "artifacts").get(
            source_pin["artifact_id"], version=source_pin["artifact_version"]
        )
        is None
    )
    with sqlite3.connect(
        b_home / "capabilities/creative/direction.sqlite3"
    ) as database:
        assert (
...
async def test_http_boundaries_status_native_and_signed_receive(home, tmp_path):
    ...
    remote_batch = ReplicationService(remote_home).export_batch(
        local_id["peer_id"], SCOPE
    )
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        headers = {
            "Cookie": "gideon_token_8000=" + generate_token("dashboard:replication")
        }
        status = await client.get(PREFIX, headers=headers)
        assert status.status == 200
        assert status.headers["Cache-Control"] == "no-store"
        denied = await client.post(
            f"{PREFIX}/receive", json={"proof": proof, "payload": remote_batch}
        )
        assert denied.status == 403
        accepted = await client.post(
            f"{PREFIX}/receive",
            headers=headers,
            json={"proof": proof, "payload": remote_batch},
        )
    ...
    assert (
        not (home / "capabilities/platform/replication.sqlite3-wal").exists()
        or (home / "capabilities/platform/replication.sqlite3").exists()
    )
...
def test_status_reports_declared_coverage_cursor_and_conflict_only(home, tmp_path):
    ...
    assert value["domains"] == [
        ...
        {"scope": "knowledge.records", "entries": ["knowledge.items"]},
        {
            ...
            "entries": ["knowledge.collections", "knowledge.collection_items"],
        },
        {
            "scope": "creative.catalog",
            "entries": [
        ...
        {"scope": "creative.direction", "entries": ["creative.direction_projects"]},
        {
            ...
            "entries": ["creative.work_versions", "creative.work_drafts"],
        },
        {
            ...
            "entries": [
                ...
                "identity.sessions",
                "identity.goal_plans",
                "identity.goal_checkins",
            ],
        },
        {
            "scope": "identity.profile",
            "entries": [
                "identity.progress_profile",
                "identity.twin_profile",
                "identity.twin_documents",
            ],
        },
        ...
        {
            "scope": "communications.contacts",
            "entries": ["communications.people", "communications.touchpoints"],
        },
        {
            "scope": "music.library",
            "entries": [
        ...
        {"scope": "music.video", "entries": ["music.video_projects"]},
        {
            ...
            "entries": [
                "wellbeing.epigenetic_results",
                "wellbeing.eye_prescriptions",
                "wellbeing.lifestyle_profiles",
                "wellbeing.body_composition",
            ],
        },
        {
            "scope": "wellbeing.health",
            "entries": [
        ...
        {
            "scope": "wellbeing.routines",
            "entries": [
                "wellbeing.substance_entries",
                "wellbeing.substance_presets",
                "wellbeing.intervention_plans",
                "wellbeing.intervention_records",
            ],
        },
        {
            "scope": "wellbeing.genome",
            "entries": ["wellbeing.genome_sources", "wellbeing.genome_variants"],
        },
        {
            "scope": "wellbeing.practice",
            "entries": ["wellbeing.cognitive_sessions", "wellbeing.memory_cards"],
        },
        {
            "scope": "wellbeing.life_calendar",
            "entries": ["wellbeing.life_config", "wellbeing.life_events"],
        },
    ]
...
def test_sqlite_domain_two_home_create_edit_delete_restart_conflict_restore_and_disabled(
    scope, home
):
    a_home, b_home = home / "a", home / "b"
    ...
    assert (
        merged["content"] if scope == "knowledge.records" else merged["body"]
    ) == "A body kept"
    ...
    PeerStore(a_home).put(
        bid["peer_id"],
        peer_record(bid, peer["endpoint"], revision=peer["revision"], send=[]),
    )
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(bid["peer_id"], scope)


def test_creative_catalog_covers_every_declared_authoritative_current_record_table(
    home,
):
    a_home, b_home = home / "a", home / "b"
    ...
    with sqlite3.connect(path) as db:
        for number, (entry_id, table) in enumerate(CREATIVE_TABLES.items(), 1):
            ...
            db.execute(
                f"INSERT INTO {table}(id,record) VALUES(?,?)",
                (identity, json.dumps(record)),
            )
    ...
    deletion = ReplicationService(a_home).export_batch(
        bid["peer_id"], "creative.catalog"
    )
...
@pytest.mark.asyncio
async def test_field_restore_http_requires_dashboard_auth_and_refuses_unsafe_fields(
    home, tmp_path
):
    a_home, b_home = home / "a", home / "b"
    aid, bid = pair_for(a_home, b_home, "creative.catalog")
    ...
    try:
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            headers = {
                "Cookie": "gideon_token_8001=" + generate_token("dashboard:restore")
            }
            unsafe = await client.post(route, headers=headers, json={"fields": ["id"]})
            assert unsafe.status == 409
            accepted = await client.post(
                route, headers=headers, json={"fields": ["title"]}
            )
            assert accepted.status == 200
...
def test_identity_goals_two_home_full_rows_conflict_restore_delete_restart_and_policy(
    home,
):
    a_home, b_home = home / "a", home / "b"
    ...
    goal = goals_a.save_goal(
        title="Shared goal", description="base", request_id="goal-create"
    )
    session = goals_a.save_session(
        goal_id=goal["id"],
        title="Focus",
    ...
    plans_a.configure(
        ...
        target_value=100,
        expected_revision=0,
        request_id="plan-create",
    )
    checkin = plans_a.checkin(
        goal_id=goal["id"],
        value=10,
        observed_at="2026-10-01T12:00:00+00:00",
        notes="Started",
        request_id="checkin-create",
    )
    a, b = ReplicationService(a_home), ReplicationService(b_home)
    ...
    a._record_sent(bid["peer_id"], baseline)
    remote = GoalPlanStore(b_home / "capabilities/identity/goals.sqlite3").get(
        goal["id"]
    )
    ...
    conflict = next(
        item
        for item in ConflictQueue(a_home).items(status=STATUS_NEEDS_REVIEW)
        if item.entry_id == "identity.goals"
    )
    ...
    with sqlite3.connect(c_path) as db:
        db.execute("DELETE FROM goals WHERE id=?", (doomed["id"],))
    removal = ReplicationService(c_home).export_batch(did["peer_id"], "identity.goals")
    assert (
        ReplicationService(d_home).apply_batch(cid["peer_id"], removal)["entries"][0][
            "removed"
        ]
        == 1
    )
    with pytest.raises(KeyError):
        GoalStore(d_home / "capabilities/identity/goals.sqlite3").get_goal(doomed["id"])
    peer = PeerStore(a_home).get(bid["peer_id"])
    PeerStore(a_home).put(
        bid["peer_id"],
        peer_record(bid, peer["endpoint"], revision=peer["revision"], send=[]),
    ...
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(bid["peer_id"], "identity.goals")


def test_identity_profile_explicit_scope_excludes_private_and_syncs_public_conflicts_and_delete(
    home,
):
    a_home, b_home = home / "a", home / "b"
    aid, bid = pair_for(a_home, b_home, "identity.profile")
    twin_a = TwinStore(a_home / "capabilities/identity/twin.sqlite3")
    public = twin_a.save_document(
        title="Public identity", text="shareable", expected_revision=0
    )
    public_id = public["documents"][0]["id"]
    twin_a.save_document(
    ...
    ProgressStore(a_home / "capabilities/identity/progress.sqlite3").configure(
    ...
    assert [item["title"] for item in twin_b.snapshot()["documents"]] == [
        "Public identity"
    ]
    assert (
        ProgressStore(b_home / "capabilities/identity/progress.sqlite3").profile()[
            "birth_date"
        ]
        == "1990-01-01"
    )
    ...
    twin_a.save_document(
        id=public_id, title="Local identity", text="local kept", expected_revision=3
    )
    twin_b.save_document(
        id=public_id,
        title="Peer identity",
        text="peer text",
        expected_revision=twin_b.snapshot()["revision"],
    )
    incoming = ReplicationService(b_home).export_batch(
        aid["peer_id"], "identity.profile"
    )
    ...
    conflict = next(
        item
        for item in ConflictQueue(a_home).items(status=STATUS_NEEDS_REVIEW)
        if item.entry_id == "identity.twin_documents"
    )
    ReplicationService(a_home).restore_fields(conflict.id, ["title"])
    ...
    assert any(
        item["private"] and item["text"] == "never-export-this"
    ...
    created = twin_c.save_document(
        title="Temporary", text="delete", expected_revision=0
    )
    ...
    deletion = ReplicationService(c_home).export_batch(
        did["peer_id"], "identity.profile"
    )
    assert (
        ReplicationService(d_home).apply_batch(cid["peer_id"], deletion)["entries"][2][
            "removed"
        ]
        == 1
    )
    assert (
        TwinStore(d_home / "capabilities/identity/twin.sqlite3").snapshot()["documents"]
        == []
    )
    peer = PeerStore(a_home).get(bid["peer_id"])
    PeerStore(a_home).put(
    ...
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(bid["peer_id"], "identity.profile")


def test_contacts_two_home_refs_conflict_restore_tombstones_credentials_and_policy(
    home,
):
    a_home, b_home = home / "a", home / "b"
    aid, bid = pair_for(a_home, b_home, "communications.contacts")
    people_a = PeopleStore(a_home / "capabilities/communications")
    person = people_a.save(
        {
            "name": "Ada",
    ...
    touchpoint, created = people_a.record(
        person["id"],
        {
            "source": "mail",
    ...
    social.save(
        people_a,
        {
            ...
            "credential_ref": "CONTACT_SOCIAL_TOKEN",
            "person_id": person["id"],
            "status": "active",
            "notes": "local only",
            "request_key": "social-create",
        },
    )
    a, b = ReplicationService(a_home), ReplicationService(b_home)
    ...
    assert people_b.get(person["id"])["identities"] == [
    ...
    with people_b.connect() as db:
        assert (
            db.execute(
                "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='social_accounts'"
            ).fetchone()[0]
            == 0
        )
    ...
    incoming = ReplicationService(b_home).export_batch(
        aid["peer_id"], "communications.contacts"
    )
    outcome = ReplicationService(a_home).apply_batch(bid["peer_id"], incoming)
    ...
    conflict = next(
        item
        for item in ConflictQueue(a_home).items(status=STATUS_NEEDS_REVIEW)
        if item.entry_id == "communications.people"
    )
    ...
    doomed = people_c.save(
        {"name": "Temporary", "identities": [{"kind": "handle", "value": "temporary"}]}
    )
    people_c.record(
        doomed["id"],
        {
            "source": "chat",
            "external_id": "temporary-1",
            "occurred_at": "2026-09-25T11:00:00+00:00",
            "direction": "inbound",
            "summary": "Temporary",
        },
    )
    first = ReplicationService(c_home).export_batch(
        did["peer_id"], "communications.contacts"
    )
    ReplicationService(d_home).apply_batch(cid["peer_id"], first)
    ReplicationService(c_home)._record_sent(did["peer_id"], first)
    ...
    removal = ReplicationService(c_home).export_batch(
        did["peer_id"], "communications.contacts"
    )
    ...
    with pytest.raises(PeopleError):
        PeopleStore(d_home / "capabilities/communications").get(doomed["id"])
    ...
    malformed = ReplicationService(bad_home).export_batch(
        target_id["peer_id"], "communications.contacts"
    )
    ...
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(
            bid["peer_id"], "communications.contacts"
        )
...
def seed_music(home):
    ...
    track = catalog.create(
        "tracks",
        {"title": "Signal Song", "artist_id": artist["id"], "notes": "metadata only"},
    )
    album = catalog.create(
        "albums",
        {
            "title": "Signal Album",
            "artist_id": artist["id"],
    ...
    song = RepertoireStore(root, artifacts).create(
        {
            "title": "Practice Song",
            "instrument": "guitar",
            "body": "C G Am F",
                ...
                "playlists": [
                    {
                        ...
                        "items": [
                            {
                                "track": {
                                    "trackName": "Signal Song",
                                    "artistName": "Signal Artist",
                                    "albumName": "Signal Album",
                                    "trackUri": "spotify:track:signal",
                                }
                            }
                        ],
                    }
    ...
    listening.import_data(
        {
            "request_id": "playlist-import",
            "account_label": "library",
            "format": "spotify_playlists",
            "artifact_ref": {"slug": source.slug, "version": source.version},
        }
    )
    deck = DeckStore(home, artifacts).create(
        {"name": "Reference Deck", "kind": "playing"}
    )
    bridge = SpotifyBridge(
        home, listening, credential_resolver=lambda _: "never-export-token"
    )
    bridge.configure(
        {
            "enabled": True,
            "credential_name": "SPOTIFY_PRIVATE",
            "account_label": "private account",
            "revision": 0,
        }
    )
    return {
        ...
        "album": album,
        "song": song,
        "playlist": listening.playlists()[0],
        "deck": deck,
    }


def test_music_library_two_home_refs_conflict_restore_tombstones_credentials_and_policy(
    home,
):
    a_home, b_home = home / "a", home / "b"
    ...
    wire = json.dumps(baseline)
    assert (
        "SPOTIFY_PRIVATE" not in wire
        and "never-export-token" not in wire
        and "credential_name" not in wire
    )
    ...
    assert catalog_b.get("albums", records["album"]["id"])["track_ids"] == [
        records["track"]["id"]
    ]
    assert (
        catalog_b.get("tracks", records["track"]["id"])["artist_id"]
        == records["artist"]["id"]
    )
    assert (
        RepertoireStore(b_home / "capabilities/music", artifacts_b).get(
            records["song"]["id"]
        )["attachment_refs"][0]["version"]
        == 1
    )
    assert (
        ListeningStore(b_home, artifacts_b).playlists()[0]["playlist_id"]
        == records["playlist"]["playlist_id"]
    )
    assert (
        DeckStore(b_home, artifacts_b).get(records["deck"]["id"])["name"]
        == "Reference Deck"
    )
    assert (
        artifacts_b.get(records["song"]["attachment_refs"][0]["slug"], version=1)
        is None
    )

    catalog_a = MusicCatalog(
        a_home / "capabilities/music", NativeArtifactProvider(a_home / "artifacts")
    )
    catalog_a.update(
        "artists",
        records["artist"]["id"],
        {"revision": 1, "name": "Local Artist", "bio": "preserve local"},
    )
    catalog_b.update(
        "artists",
        records["artist"]["id"],
        {"revision": 1, "name": "Peer Artist", "bio": "peer bio"},
    )
    ...
    conflict = next(
        item
        for item in ConflictQueue(a_home).items(status=STATUS_NEEDS_REVIEW)
        if item.entry_id == "music.artists"
    )
    ReplicationService(a_home).restore_fields(conflict.id, ["name"])
    merged = catalog_a.get("artists", records["artist"]["id"])
    assert (
        merged["name"] == "Peer Artist"
        and merged["bio"] == "preserve local"
    ...
    with sqlite3.connect(root / "music_catalog.sqlite3") as db:
        db.execute("DELETE FROM catalog")
    with sqlite3.connect(root / "repertoire.sqlite3") as db:
        db.execute("DELETE FROM items")
    with sqlite3.connect(root / "listening.sqlite3") as db:
        db.execute("DELETE FROM snapshots")
    with sqlite3.connect(root / "decks.sqlite3") as db:
    ...
    assert all(
        not item["rows"]
        for item in ReplicationService(d_home).export_batch(
            cid["peer_id"], "music.library"
        )["entries"]
    )
    ...
    seed_music(bad_home)
    malformed = ReplicationService(bad_home).export_batch(
        target_id["peer_id"], "music.library"
    )
    malformed["entries"][2]["rows"][0]["data"]["track_ids"] = ["missing-track"]
    with pytest.raises(
        ReplicationError, match="missing or repeated tracks"
    ) as rejected:
        ReplicationService(target_home).apply_batch(bad_id["peer_id"], malformed)
    assert rejected.value.status == 422
    assert (
        MusicCatalog(
            target_home / "capabilities/music",
            NativeArtifactProvider(target_home / "artifacts"),
        ).list("artists")
        == []
    )
    peer = PeerStore(a_home).get(bid["peer_id"])
    ...
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(bid["peer_id"], "music.library")


def seed_media(home):
    artifacts = NativeArtifactProvider(home / "artifacts")
    image = artifacts.create_binary(
        ...
        source="import",
        tags=["reference"],
        event_metadata={"original_filename": "reference.png", "sha256": "a" * 64},
    )
    video = artifacts.create_binary(
        ...
        tags=["motion"],
        event_metadata={
            "source_artifact_id": image.slug,
            "source_version": image.version,
        },
    )
    artifacts.update(
        image.slug, collection="Campaign", expect_updated_at=image.updated_at
    )
    artifacts.create(
        name="Private request note",
        content="credential_ref=MEDIA_PRIVATE",
        kind="markdown",
    )
...
def test_media_assets_current_metadata_conflict_tombstone_bytes_and_policy(home):
    ...
    assert [item["entry_id"] for item in baseline["entries"]] == [
        "media.library_metadata"
    ]
    assert [row["id"] for row in baseline["entries"][0]["rows"]] == [image_id, video_id]
    wire = json.dumps(baseline)
    assert (
        "private-image-bytes" not in wire
        and "private-video-bytes" not in wire
        and "MEDIA_PRIVATE" not in wire
    )
    assert (
        "raw_url" not in wire and "source_path" not in wire and '"events"' not in wire
    )
    assert b.apply_batch(aid["peer_id"], baseline)["accepted"] is True
    a._record_sent(bid["peer_id"], baseline)
    artifacts_b = NativeArtifactProvider(b_home / "artifacts")
    ...
    assert library_b.get(video_id)["provenance"] == {
        "source_artifact_id": image_id,
        "source_version": 1,
    }
    assert (
        artifacts_b.raw_bytes(image_id) is None
        and artifacts_b.raw_bytes(video_id) is None
    )
    ...
    library_b.update(
        image_id,
        {
            "expected_updated_at": peer["updated_at"],
            "name": "Peer image",
            "tags": ["peer-tag"],
        },
    )
    ...
    conflict = next(
        item
        for item in ConflictQueue(a_home).items(status=STATUS_NEEDS_REVIEW)
        if item.entry_id == "media.library_metadata"
    )
    ReplicationService(a_home).restore_fields(conflict.id, ["name"])
    merged = library_a.get(image_id)
    assert merged["name"] == "Peer image" and merged["tags"] == ["keep-local"]
    assert (
        NativeArtifactProvider(a_home / "artifacts").raw_bytes(image_id)[0]
    ...
    assert (
        ReplicationService(d_home).apply_batch(cid["peer_id"], deletion)["entries"][0][
            "removed"
        ]
        == 1
    )
    ...
    seed_media(bad_home)
    malformed = ReplicationService(bad_home).export_batch(
        target_id["peer_id"], "media.assets"
    )
    ...
    with pytest.raises(ReplicationError, match="exact non-binary") as rejected:
        ReplicationService(target_home).apply_batch(bad_id["peer_id"], malformed)
    assert (
        rejected.value.status == 422
        and NativeArtifactProvider(target_home / "artifacts").list() == []
    )
    configured = PeerStore(a_home).get(bid["peer_id"])
    PeerStore(a_home).put(
        bid["peer_id"],
        peer_record(
            bid, configured["endpoint"], revision=configured["revision"], send=[]
        ),
    )
    with pytest.raises(ReplicationError, match="export"):
...
def seed_wellbeing(home):
    measurement = MeasurementStore(home).create(
        {
            "request_id": "measure",
            "kind": "body_weight",
            "observed_at": "2026-09-25T08:00:00+02:00",
            "unit": "kg",
            "values": {"weight": 80},
            "source": "manual",
            "notes": "baseline",
    ...
    lab_input = {
        ...
        "source": "clinic export",
    }
    lab = labs.commit(
        {
            **lab_input,
            "preview_id": labs.preview(lab_input)["preview_id"],
            "request_id": "lab-import",
        }
    )["records"][0]
    ...
    apple.commit(
        {
            **apple_input,
            "preview_id": apple.preview(apple_input)["preview_id"],
            "request_id": "apple-import",
        }
    )
    metric = apple.list_metrics()[0]
    habits = ConsumptionStore(home)
    preset = habits.create_preset(
        {
            "request_id": "preset",
            "kind": "alcohol",
            "name": "Recorded serving",
            "details": {"volume_ml": 330, "abv_percent": 5},
        }
    )
    entry = habits.create_entry(
        {
            "request_id": "entry",
            "preset_id": preset["id"],
            "count": 1,
            "observed_at": "2026-09-25T18:00:00+02:00",
            "source": "manual",
            "notes": "recorded",
        }
    )
    interventions = InterventionStore(home)
    plan = interventions.create_plan(
        {
            "request_id": "plan",
            "name": "Evening walk",
            "kind": "activity",
    ...
    observation = interventions.record(
        plan["id"],
        {
    ...
    with sqlite3.connect(home / "capabilities/wellbeing.sqlite3") as db:
        db.execute(
            ...
            (
                "private-ledger",
                '{"access_token":"health-private-token","client_secret":"never-share","native_helper":"local-only"}',
                "{}",
            ),
        )
    return {
...
def test_wellbeing_opt_in_scopes_current_refs_conflicts_tombstones_and_private_exclusions(
    home,
):
    ...
    with pytest.raises(ReplicationError, match="policy denies"):
        ReplicationService(none_a).export_batch(
            none_peer["peer_id"], "wellbeing.health"
        )
    ...
    assert [item["entry_id"] for item in batch["entries"]] == [
        "wellbeing.measurements",
        "wellbeing.labs",
        "wellbeing.metrics",
    ]
    ...
    assert (
        "health-private-token" not in wire
        and "never-share" not in wire
        and "native_helper" not in wire
        and "requests" not in wire
    )
    assert b.apply_batch(aid["peer_id"], batch)["accepted"] is True
    a._record_sent(bid["peer_id"], batch)
    assert MeasurementStore(b_home).get(records["measurement"]["id"])["values"] == {
        "weight": 80.0
    }
    assert LabStore(b_home).get(records["lab"]["id"])["analyte"] == "Glucose"
    assert (
        AppleHealthStore(b_home).list_metrics()[0]["metric"]
        == "HKQuantityTypeIdentifierStepCount"
    )
    assert (
        LabStore(b_home).artifacts.get(records["lab"]["artifact"]["slug"], version=1)
        is None
    )

    MeasurementStore(a_home).correct(
        records["measurement"]["id"],
        {
            "request_id": "local-health",
            "revision": 1,
            "values": {"weight": 79},
            "notes": "keep local value",
        },
    )
    MeasurementStore(b_home).correct(
        records["measurement"]["id"],
        {"request_id": "peer-health", "revision": 1, "notes": "peer note"},
    )
    incoming = ReplicationService(b_home).export_batch(
        aid["peer_id"], "wellbeing.health"
    )
    result = ReplicationService(a_home).apply_batch(bid["peer_id"], incoming)
    assert result["entries"][0]["conflicts"] == 1
    conflict = next(
        item
        for item in ConflictQueue(a_home).items(status=STATUS_NEEDS_REVIEW)
        if item.entry_id == "wellbeing.measurements"
    )
    ReplicationService(a_home).restore_fields(conflict.id, ["notes"])
    merged = MeasurementStore(a_home).get(records["measurement"]["id"])
    assert (
        merged["notes"] == "peer note"
        and merged["values"] == {"weight": 79.0}
        and merged["revision"] == 3
    )
    with sqlite3.connect(a_home / "capabilities/wellbeing.sqlite3") as db:
        db.execute("DELETE FROM lab_revisions WHERE id=?", (records["lab"]["id"],))
    deletion = ReplicationService(a_home).export_batch(
        bid["peer_id"], "wellbeing.health"
    )
    assert (
        ReplicationService(b_home).apply_batch(aid["peer_id"], deletion)["entries"][1][
            "removed"
        ]
        == 1
    )
    ...
    initial = ReplicationService(c_home).export_batch(
        did["peer_id"], "wellbeing.routines"
    )
    assert (
        ReplicationService(d_home).apply_batch(cid["peer_id"], initial)["accepted"]
        is True
    )
    ReplicationService(c_home)._record_sent(did["peer_id"], initial)
    assert (
        ConsumptionStore(d_home).get_entry(routines["entry"]["id"])["preset_id"]
        == routines["preset"]["id"]
    )
    assert (
        InterventionStore(d_home).get_record(routines["observation"]["id"])["plan_id"]
        == routines["plan"]["id"]
    )
    ConsumptionStore(c_home).delete_entry(
        routines["entry"]["id"], {"request_id": "delete-entry", "revision": 1}
    )
    tombstone = ReplicationService(c_home).export_batch(
        did["peer_id"], "wellbeing.routines"
    )
    assert (
        ReplicationService(d_home).apply_batch(cid["peer_id"], tombstone)["entries"][0][
            "removed"
        ]
        == 1
    )
    ...
    malformed = ReplicationService(bad_home).export_batch(
        target_id["peer_id"], "wellbeing.routines"
    )
    malformed["entries"][3]["rows"][0]["data"]["plan_id"] = "missing-plan"
    with pytest.raises(ReplicationError, match="missing plan") as rejected:
        ReplicationService(target_home).apply_batch(bad_id["peer_id"], malformed)
    assert (
        rejected.value.status == 422
        and InterventionStore(target_home).list_plans(include_archived=True) == []
    )
    peer = PeerStore(c_home).get(did["peer_id"])
    PeerStore(c_home).put(
        did["peer_id"],
        peer_record(did, peer["endpoint"], revision=peer["revision"], send=[]),
    )
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(c_home).export_batch(did["peer_id"], "wellbeing.routines")
...
def seed_genome(home):
    ...
    receipt = store.commit(
        {
            **payload,
            "preview_id": store.preview(payload)["preview_id"],
            "request_id": "genome-import",
        }
    )
    with sqlite3.connect(home / "capabilities/wellbeing.sqlite3") as db:
        db.execute(
            ...
            (
                "genome-private",
                '{"private_key":"genome-private-key","access_grant":"local-only"}',
                "{}",
            ),
        )
...
def test_genome_scope_two_home_refs_conflict_restore_tombstones_and_privacy(home):
    ...
    with pytest.raises(ReplicationError, match="policy denies"):
        ReplicationService(none_a).export_batch(
            none_peer["peer_id"], "wellbeing.genome"
        )
    ...
    assert [item["entry_id"] for item in baseline["entries"]] == [
        "wellbeing.genome_sources",
        "wellbeing.genome_variants",
    ]
    assert [len(item["rows"]) for item in baseline["entries"]] == [1, 2]
    wire = json.dumps(baseline)
    assert (
        "genome-private-key" not in wire
        and "access_grant" not in wire
        and "requests" not in wire
    )
    assert b.apply_batch(aid["peer_id"], baseline)["accepted"] is True
    a._record_sent(bid["peer_id"], baseline)
    ...
    assert [row["rsid"] for row in genome_b.list_variants(source["id"])] == [
        "rs1",
        "rs2",
    ]
    assert genome_b.artifacts.get(source["artifact"]["slug"], version=1) is None

    GenomeStore(a_home).annotate(
        variants[0]["id"],
        {
            "request_id": "local-note",
            "revision": 1,
            "annotation": "local note",
            "annotation_source": "local review",
        },
    )
    genome_b.annotate(
        variants[0]["id"],
        {
            "request_id": "peer-note",
            "revision": 1,
            "annotation": "peer note",
            "annotation_source": "peer review",
        },
    )
    incoming = ReplicationService(b_home).export_batch(
        aid["peer_id"], "wellbeing.genome"
    )
    outcome = ReplicationService(a_home).apply_batch(bid["peer_id"], incoming)
    ...
    with sqlite3.connect(c_home / "capabilities/wellbeing.sqlite3") as db:
        ...
        db.execute(
            "DELETE FROM genome_variants WHERE source_id=?", (doomed_source["id"],)
        )
    tombstones = ReplicationService(c_home).export_batch(
        did["peer_id"], "wellbeing.genome"
    )
    ...
    malformed = ReplicationService(bad_home).export_batch(
        target_id["peer_id"], "wellbeing.genome"
    )
    malformed["entries"][1]["rows"][0]["data"]["source_id"] = "missing-source"
    with pytest.raises(ReplicationError, match="missing source") as rejected:
        ReplicationService(target_home).apply_batch(bad_id["peer_id"], malformed)
    assert (
        rejected.value.status == 422 and GenomeStore(target_home).list_sources() == []
    )
    peer = PeerStore(a_home).get(bid["peer_id"])
    PeerStore(a_home).put(
        bid["peer_id"],
    ...
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(bid["peer_id"], "wellbeing.genome")


def seed_practice(home):
    cognition = CognitiveStore(home)
    active = cognition.start(
        {
            "request_id": "cognitive-start",
            "kind": "arithmetic",
            "planned_trials": 1,
    ...
    session = cognition.answer(
        active["id"],
        {"request_id": "cognitive-answer", "revision": 1, "answer": answer},
    )
    ...
    with sqlite3.connect(home / "capabilities/wellbeing.sqlite3") as db:
        db.execute(
            ...
            (
                "practice-private",
                '{"password":"practice-private-password","native_helper":"local-only"}',
                "{}",
            ),
        )
    return session, card
...
def test_practice_scope_terminal_results_memory_conflicts_tombstones_and_privacy(home):
    ...
    with pytest.raises(ReplicationError, match="policy denies"):
        ReplicationService(none_a).export_batch(
            none_peer["peer_id"], "wellbeing.practice"
        )
    ...
    assert [item["entry_id"] for item in baseline["entries"]] == [
        "wellbeing.cognitive_sessions",
        "wellbeing.memory_cards",
    ]
    wire = json.dumps(baseline)
    assert (
        '"expected"' not in wire
        and "practice-private-password" not in wire
        and "native_helper" not in wire
    )
    assert b.apply_batch(aid["peer_id"], baseline)["accepted"] is True
    a._record_sent(bid["peer_id"], baseline)
    ...
    assert (
        received_session["status"] == "completed"
        and received_session["score"]["accuracy"] == 1
    )
    memory_b = MemoryPracticeStore(b_home)
    assert memory_b.artifacts.get(card["artifact"]["slug"], version=1) is None
    with memory_b.connection() as db:
        received_card = json.loads(
            db.execute(
                "SELECT data FROM memory_card_revisions WHERE id=?", (card["id"],)
            ).fetchone()[0]
        )
    assert (
        received_card["artifact"] == card["artifact"] and "front" not in received_card
    )
    source_artifact = MemoryPracticeStore(a_home).artifacts.get(
        card["artifact"]["slug"], version=1
    )
    memory_b.artifacts.create(
        name="Capital of France?",
        content=source_artifact.content,
        kind="document",
        source="local",
        slug=card["artifact"]["slug"],
        readonly=True,
    )

    practiced = MemoryPracticeStore(a_home).practice(
        card["id"], {"request_id": "local-practice", "revision": 1, "grade": "good"}
    )
    memory_b.update(
        card["id"], {"request_id": "peer-archive", "revision": 1, "archived": True}
    )
    incoming = ReplicationService(b_home).export_batch(
        aid["peer_id"], "wellbeing.practice"
    )
    outcome = ReplicationService(a_home).apply_batch(bid["peer_id"], incoming)
    assert outcome["entries"][1]["conflicts"] == 1
    conflict = next(
    ...
    first = ReplicationService(c_home).export_batch(
        did["peer_id"], "wellbeing.practice"
    )
    ReplicationService(d_home).apply_batch(cid["peer_id"], first)
    ReplicationService(c_home)._record_sent(did["peer_id"], first)
    with sqlite3.connect(c_home / "capabilities/wellbeing.sqlite3") as db:
    ...
    assert [
        item["removed"]
        for item in ReplicationService(d_home).apply_batch(cid["peer_id"], tombstones)[
            "entries"
        ]
    ] == [1, 1]
    ...
    malformed = ReplicationService(bad_home).export_batch(
        target_id["peer_id"], "wellbeing.practice"
    )
    malformed["entries"][0]["rows"][0]["data"]["status"] = "active"
    with pytest.raises(ReplicationError, match="terminal results") as rejected:
        ReplicationService(target_home).apply_batch(bad_id["peer_id"], malformed)
    assert (
        rejected.value.status == 422
        and CognitiveStore(target_home).list_sessions() == []
    )
    peer = PeerStore(a_home).get(bid["peer_id"])
    PeerStore(a_home).put(
        bid["peer_id"],
    ...
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(bid["peer_id"], "wellbeing.practice")
...
def seed_life_calendar(home):
    ...
    config = store.configure(
        {
            "request_id": "calendar-config",
            "revision": 0,
            "birth_date": "1990-01-01",
            "horizon_years": 90,
            "sleep_hours": 8,
            "timezone": "Europe/Berlin",
            "budgets": [{"name": "creative work", "hours_per_week": 20}],
    ...
    with sqlite3.connect(home / "capabilities/wellbeing.sqlite3") as db:
        db.execute(
            ...
            (
                "calendar-private",
                '{"secret":"calendar-private-secret","access_grant":"local-only"}',
                "{}",
            ),
        )
        db.execute(
            "INSERT INTO life_reminder_claims VALUES(?,?,?)",
            ("2026-09-25", "Europe/Berlin", "private-inbox-item"),
        )
    return config, event
...
def test_life_calendar_scope_config_events_conflict_tombstone_and_private_state(home):
    ...
    with pytest.raises(ReplicationError, match="policy denies"):
        ReplicationService(none_a).export_batch(
            none_peer["peer_id"], "wellbeing.life_calendar"
        )
    ...
    assert [item["entry_id"] for item in baseline["entries"]] == [
        "wellbeing.life_config",
        "wellbeing.life_events",
    ]
    wire = json.dumps(baseline)
    assert (
        "trigger_id" not in wire
        and "calendar-private-secret" not in wire
        and "private-inbox-item" not in wire
        and "life_reminder_claims" not in wire
    )
    assert b.apply_batch(aid["peer_id"], baseline)["accepted"] is True
    a._record_sent(bid["peer_id"], baseline)
    calendar_b = LifeCalendarStore(b_home)
    assert (
        calendar_b.get_config()["birth_date"] == config["birth_date"]
        and "trigger_id" not in calendar_b.get_config()
    )
    assert calendar_b.list_events()[0]["id"] == event["id"]
    assert (
        calendar_b.projection("2026-09-25T12:00:00Z")["events"][0]["title"]
        == "Health review"
    )

    local = LifeCalendarStore(a_home).update_event(
        event["id"],
        {
            "request_id": "local-event",
            "revision": 1,
            "title": "Local health review",
            "notes": "keep local notes",
        },
    )
    calendar_b.update_event(
        event["id"],
        {
            "request_id": "peer-event",
            "revision": 1,
            "title": "Peer health review",
            "notes": "peer notes",
        },
    )
    incoming = ReplicationService(b_home).export_batch(
        aid["peer_id"], "wellbeing.life_calendar"
    )
    outcome = ReplicationService(a_home).apply_batch(bid["peer_id"], incoming)
    assert outcome["entries"][1]["conflicts"] == 1
    conflict = next(
        item
        for item in ConflictQueue(a_home).items(status=STATUS_NEEDS_REVIEW)
        if item.entry_id == "wellbeing.life_events"
    )
    ReplicationService(a_home).restore_fields(conflict.id, ["title"])
    merged = LifeCalendarStore(a_home).list_events()[0]
    assert (
        merged["title"] == "Peer health review"
        and merged["notes"] == local["notes"]
        and merged["revision"] == 3
    )
    ...
    first = ReplicationService(c_home).export_batch(
        did["peer_id"], "wellbeing.life_calendar"
    )
    ReplicationService(d_home).apply_batch(cid["peer_id"], first)
    ReplicationService(c_home)._record_sent(did["peer_id"], first)
    LifeCalendarStore(c_home).update_event(
        doomed["id"], {"request_id": "delete-event", "revision": 1, "deleted": True}
    )
    tombstone = ReplicationService(c_home).export_batch(
        did["peer_id"], "wellbeing.life_calendar"
    )
    assert (
        ReplicationService(d_home).apply_batch(cid["peer_id"], tombstone)["entries"][1][
            "removed"
        ]
        == 1
    )
    ...
    malformed = ReplicationService(bad_home).export_batch(
        target_id["peer_id"], "wellbeing.life_calendar"
    )
    malformed["entries"][0]["rows"][0]["data"]["trigger_id"] = "remote-trigger"
    with pytest.raises(ReplicationError, match="local automation state") as rejected:
        ReplicationService(target_home).apply_batch(bad_id["peer_id"], malformed)
    assert (
        rejected.value.status == 422
        and LifeCalendarStore(target_home).get_config() is None
    )
    peer = PeerStore(a_home).get(bid["peer_id"])
    PeerStore(a_home).put(
        bid["peer_id"],
    ...
    with pytest.raises(ReplicationError, match="export"):
        ReplicationService(a_home).export_batch(
            bid["peer_id"], "wellbeing.life_calendar"
        )
