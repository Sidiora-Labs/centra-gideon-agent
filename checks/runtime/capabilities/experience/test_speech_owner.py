...
from gideon.workspace.capabilities.experience.proactive_speech import (
    latest_digest,
    start_digest,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("release", []),
        ("unknown", {}),
        ("claim", {"owner": "browser", "home": "other"}),
        ("enable", {"enabled": True, "provider": "external"}),
    ],
)
...
async def test_proactive_requires_opt_in_and_actual_installed_digest(owner, tmp_path):
    ...
    with pytest.raises(ValueError):
        start_digest(
            jobs, owner, {**credentials(lease), "text": "Untrusted proactive prose"}
        )
    await jobs.close()
...
async def test_actual_http_owner_and_proactive_routes(owner):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        enable = await client.post(
            base + "/speech-owner/enable", json={"enabled": True}
        )
        assert enable.status == 200
        assert "token" not in (await enable.json())["owner"]
        claim = await client.post(
            base + "/speech-owner/claim", json={"owner": "browser"}
        )
        assert claim.status == 200
        lease = (await claim.json())["owner"]
        second = await client.post(
            base + "/speech-owner/claim", json={"owner": "second"}
        )
        assert second.status == 409
        renewed = await client.post(
            base + "/speech-owner/renew", json=credentials(lease)
        )
        ...
        bad = await client.post(
            base + "/proactive-speech", json={**credentials(lease), "text": "arbitrary"}
        )
        assert bad.status == 400
        released = await client.post(
            base + "/speech-owner/release", json=credentials(lease)
        )
        assert released.status == 200
        assert (
            await client.post(base + "/proactive-speech", json=credentials(lease))
        ).status == 409
...
def persist_digest(home, body="Two tasks need your review."):
    ...
    run = store.create(
        WorkflowRun(id="", workflow_name="morning-triage", status=RunStatus.COMPLETE)
    )
    store.write_spec(
        run.id, {"root": {"id": "triage", "kind": "action", "provider": "triage"}}
    )
    store.write_state(
        run.id, {"root": NodeInstance(path="root", state=InstanceState.DONE)}
    )
    store.write_output(
        run.id,
        "root",
        json.dumps(
            {"digest_title": "Morning review", "digest_body": body, "collected": 2}
        ),
    )
    return run
...
async def test_real_persisted_digest_source_and_no_provider_boundary(owner, tmp_path):
    ...
    assert (
        result["narration"]["source_description"]
        == f"Proactive digest run {run.id}; source {source['source_hash']}"
    )
    assert result["narration"]["source_hash"] == source["source_hash"]
    assert (
        start_digest(jobs, owner, credentials(lease))["narration"]["id"]
        == result["narration"]["id"]
    )
    ...
    store.write_output(
        ...
        json.dumps(
            {
                "digest_title": "Morning review",
                "digest_body": "An updated recorded digest.",
            }
        ),
    )
...
def test_notification_mute_and_actual_quiet_window_defer_source(owner, tmp_path):
    ...
    assert latest_digest(tmp_path)["state"] == "deferred"
    _save_entity_settings(
        "notifications", {"mute_all": False, "quiet_hours_enabled": False}
    )
...
async def test_explicit_retry_preserves_previous_failed_receipt(owner, tmp_path):
    ...
    assert (
        start_digest(jobs, owner, credentials(lease))["narration"]["id"] == first["id"]
    )
