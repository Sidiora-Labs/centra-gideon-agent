...
from checks.runtime.capabilities.experience.test_worlds import world as _world
...
@pytest.mark.asyncio
async def test_real_peer_projection_reports_policy_and_authenticated_probe_without_secrets(
    tmp_path,
):
    ...
    async with TestServer(app) as server:
        local.put(
            ...
            peer_record(remote_identity, str(server.make_url("/")), send=(scope,)),
        )
        remote.put(
            local_identity["peer_id"],
            peer_record(local_identity, "https://local.invalid", receive=(scope,)),
        )
        assert await local.post_signed(
            remote_identity["peer_id"], scope, "/probe", {"probe": True}
        ) == {"verified": True}

    disabled_identity = PeerStore(disabled_home).snapshot()["self"]
    local.put(
        disabled_identity["peer_id"],
        peer_record(
            disabled_identity, "https://disabled.invalid", enabled=False, send=(scope,)
        ),
    )
    ...
    assert (
        rows[remote_identity["peer_id"]]["status"]
        == "configured; probe succeeded at " + probed["last_probe"]
    )
...
async def test_real_process_projection_does_not_take_owner_or_expose_command(tmp_path):
    ...
    try:
        row = await registry.start(
            {
                "project_id": "Actual operation",
                "workspace": str(workspace),
                "command": "printf private_output; sleep 20",
                "request_id": "projection_process",
            }
        )
        ...
        source = next(
            item for item in projection["sources"] if item["kind"] == "operations"
        )
        ...
        source = next(
            item
            for item in extended_sources(tmp_path)["sources"]
...
async def test_actual_bound_memory_projects_and_deleted_source_stays_explicit(
    ...
    removed = await world.mutate(
        ...
        "project",
        {
            ...
            "request_id": "removed_source",
        },
    )
    assert removed["complete"]
    actual = await world.call("memory_projection")
    assert (
        list(actual["state"]["entities"].values())[0]["comp"]["gideon_source"]["status"]
        == "not_in_current_source_preview"
    )
    prior = get_action_services()
    set_action_services(None)
    try:
