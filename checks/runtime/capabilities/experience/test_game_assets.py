...
def game(tmp_path, monkeypatch):
    ...
    (source / "app.json").write_text(json.dumps({"name":"real-game-app", "version":"1.0.0", "displayName":"Real game", "description":"Consumes compiled game assets", "permissions":{"storage":True}}))
    installed = app_manager.install(source, confirm=True)
    assert installed.ok
    store = ExperienceStore(tmp_path)
    foundation_service = WorldFoundations(store)
    foundation = foundation_service.record({"title":"Arcade foundation", "controller":{"kind":"ambient_beacon", "position":[0,0,0]}, "style":{"private":"local"}})
    ...
    refs = {}
    values = {"sprite":("Hero", png("red"), "image", "image/png"), "artwork":("Backdrop", png("blue"), "image", "image/png"),
              "music":("Theme", wav(), "audio", "audio/wav"), "model":("Prop", glb(), "model", "model/gltf-binary")}
    for role, (name, data, kind, mime) in values.items():
        artifact = artifacts.create_binary(name=name, data=data, kind=kind, mime=mime, source="manual")
        refs[role] = {"slug":artifact.slug, "version":artifact.version}
    service = GameAssets(store, artifacts)
    project = service.create({"title":"Runnable arcade", "app_id":"real-game-app", "foundation_id":foundation["id"]})
    for role in ("sprite", "artwork", "music", "model"):
        project = service.bind(project["id"], {"revision":project["revision"], "role":role, "artifact_ref":refs[role], "label":role.title()})
    return service, project, refs, artifacts, foundation, tmp_path


def test_compile_runnable_export_reuses_inputs_and_publishes_verified_app_destination(game):
    ...
    export = artifacts.get(compiled["export_ref"]["slug"], version=compiled["export_ref"]["version"])
    ...
    reopened = GameAssets(ExperienceStore(tmp_path), NativeArtifactProvider(tmp_path / "artifacts"))
...
def test_new_immutable_asset_version_creates_new_compile_version(game):
    ...
    updated = artifacts.update_binary(refs["sprite"]["slug"], data=png("green"), mime="image/png", expect_version=1)
    assert updated.version == 2
    current = service.bind(current["id"], {"revision":current["revision"], "role":"sprite", "artifact_ref":{"slug":updated.slug, "version":updated.version}, "label":"Hero green"})
...
def test_missing_or_corrupted_exact_bytes_refuse_compilation_and_publication(game):
    ...
    compiled = service.compile(project["id"], project["revision"])
    current = service.get(project["id"])
    export_path = artifacts._artifact_dir(compiled["export_ref"]["slug"]) / "versions" / "v1.html"
...
async def test_http_and_native_operations_use_canonical_compiler(game):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        compiled = await client.post(f"/api/capabilities/experience/game-assets/projects/{project['id']}/compile", json={"revision":project["revision"]})
        assert compiled.status == 200
        assert (await compiled.json())["compile"]["created"] is True
        current = service.get(project["id"])
        stale = await client.post(f"/api/capabilities/experience/game-assets/projects/{project['id']}/publish", json={"revision":project["revision"]})
        assert stale.status == 409
        published = await client.post(f"/api/capabilities/experience/game-assets/projects/{project['id']}/publish", json={"revision":current["revision"]})
    ...
    assert json.loads(result.output)["projects"][0]["publication"]["state"] == "verified"
    refused = await provider.invoke("experience_game_assets_compile", {"id":project["id"]})
...
def test_validation_foundation_and_managed_app_boundaries(game):
    ...
    for body in (None, [], {}, {"title":"x", "app_id":"../bad", "foundation_id":foundation["id"]},
    ...
    with pytest.raises(Conflict, match="revision"):
        service.bind(project["id"], {"revision":1, "role":"sprite", "artifact_ref":refs["sprite"], "label":"stale"})
    wrong = artifacts.create_binary(name="Wrong kind", data=png("white"), kind="image", mime="image/png", source="manual")
    with pytest.raises(Conflict, match="music"):
        service.bind(project["id"], {"revision":project["revision"], "role":"music", "artifact_ref":{"slug":wrong.slug,"version":1}, "label":"wrong"})
    incomplete = service.create({"title":"Incomplete", "app_id":"real-game-app", "foundation_id":foundation["id"]})
