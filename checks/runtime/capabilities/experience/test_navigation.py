...
from gideon.workspace.capabilities.experience.navigation import NavigationReceipts, route
...
def command(**overrides):
    return dict(request_id='open', command='Open Projects', origin='chat', target='projects', input_origin='typed', **overrides)
...
def test_requested_receipt_is_not_browser_success_and_reloads(receipts, tmp_path):
    ...
    applied = restarted.acknowledge(requested['id'], {'revision': 1, 'observed_route': 'projects'})
    assert applied['status'] == 'applied'
    assert applied['observed_route'] == 'projects'
    assert applied['revision'] == 2
    assert datetime.fromisoformat(applied['applied_at']) >= datetime.fromisoformat(applied['created_at'])
    assert receipts.get(requested['id']) == applied
    assert receipts.request(command()) == applied
    assert receipts.acknowledge(requested['id'], {'revision': 1, 'observed_route': 'projects'}) == applied


def test_wrong_acknowledgement_and_request_reuse_preserve_requested(receipts):
    requested = receipts.request(command())
    for body in [{'revision': 1, 'observed_route': 'chat'}, {'revision': 2, 'observed_route': 'projects'}]:
...
@pytest.mark.parametrize('value', ['', '/projects', 'https://example.com', '../settings', 'app//one', 'app/', '#projects', 'a'*201, None, 12])
def test_route_refuses_external_or_malformed_destinations(value):
    with pytest.raises(ValueError):
        route(value)


@pytest.mark.parametrize('patch', [{'input_origin':'provider'}, {'command':''}, {'command':'x'*401}, {'request_id':'../oops'}, {'provider':'managed'}, {'home':'/tmp/elsewhere'}])
...
@pytest.mark.parametrize('body', [None, [], {}, {'revision':True, 'observed_route':'projects'}, {'revision':1, 'observed_route':'projects', 'status':'applied'}])
...
async def test_native_tools_only_read_browser_receipts(receipts):
    ...
    rejected = await provider.invoke('experience_navigation_acknowledge', {'id':requested['id']})
...
async def test_real_http_request_acknowledgement_and_errors(receipts):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        wrong = await client.post(detail+'/acknowledge', json={'revision':1,'observed_route':'chat'})
        assert wrong.status == 409
        assert 'acknowledged' in (await wrong.json())['error']
        ack = await client.post(detail+'/acknowledge', json={'revision':1,'observed_route':'projects'})
        ...
        assert (await client.post(detail+'/acknowledge', json={'revision':1})).status == 400
        assert (await client.post(prefix, json={**command(), 'target':'settings'})).status == 409
