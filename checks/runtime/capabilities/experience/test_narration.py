...
from gideon.workspace.capabilities.experience.narration import NarrationJobs, audio_mime, digest, voice_digest
...
def session_home(tmp_path, monkeypatch):
    ...
    story = store.save({"title": "A quiet path", "start_node": "start", "nodes": [
        {"id": "start", "text": "You walk into the woods.", "kind": "scene", "choices": [{"id": "walk", "label": "Continue", "target": "end"}]},
        {"id": "end", "text": "You have reached the clearing.", "kind": "ending", "choices": []},
    ]})
    session = store.start({"story_id": story["id"], "story_revision": 1, "request_id": "start"})["session"]
...
async def test_real_unconfigured_voice_reports_unavailable_without_audio(session_home):
    ...
    expected = digest({"story_id": story["id"], "story_revision": 1, "node_id": "start", "text": "You walk into the woods."})
...
@pytest.mark.asyncio
async def test_narration_retries_resolve_same_job_and_conflicting_reuse_fails(session_home):
...
@pytest.mark.asyncio
async def test_cancel_before_execution_prevents_provider_work_and_is_idempotent(session_home):
    store, _, session = session_home
    jobs = NarrationJobs(store)
    ...
    assert jobs.start(session["id"], {"revision": 1, "request_id": "cancel"}) == cancelled
...
@pytest.mark.asyncio
async def test_source_snapshot_survives_edit_and_choice_and_new_node_has_new_key(session_home):
    store, story, session = session_home
    jobs = NarrationJobs(store)
    ...
    view = store.choose(session["id"], {"choice_id": "walk", "request_id": "walk", "revision": 1})
    ...
    assert jobs.start(session["id"], {"revision": 1, "request_id": "first"})["id"] == first["id"]
...
async def test_bounded_active_jobs_and_shutdown(session_home):
    store, _, session = session_home
    jobs = NarrationJobs(store)
    pending = [jobs.start(session["id"], {"revision": 1, "request_id": f"job-{i}"}) for i in range(4)]
    with pytest.raises(Conflict, match="four"):
        jobs.start(session["id"], {"revision": 1, "request_id": "fifth"})
    assert jobs.start(session["id"], {"revision": 1, "request_id": "job-0"}) == pending[0]
    await jobs.close()
...
asyncio.run(main())
"""
    completed = subprocess.run([sys.executable, "-c", script, str(store.path.parent.parent), session["id"]], capture_output=True, text=True, check=True)
    queued = json.loads(completed.stdout)
    assert queued["status"] == "queued"
    recovered = NarrationJobs(ExperienceStore(store.path.parent.parent)).get(queued["id"])
...
def test_voice_key_excludes_credentials_and_detects_voice_and_reference_changes(tmp_path):
    provider = OpenAITtsProvider(provider_name="speech", api_key="not-a-live-key")
    params = {"provider": provider, "voice": "model", "speech_voice": "alloy", "speed": 1.0}
    first = voice_digest(params)
    assert len(first) == 64
    assert voice_digest({**params, "api_key": "another", "endpoint": "https://example.invalid"}) == first
    ...
    reference.write_bytes(b"reference content for fingerprint testing, not generated speech")
...
@pytest.mark.parametrize("payload", [None, [], {}, {"revision": True, "request_id": "x"}, {"revision": 1, "request_id": "../x"}, {"revision": 1, "request_id": "x", "text": "override"}, {"revision": 1, "request_id": "x", "provider": "override"}])
def test_narration_rejects_untrusted_payload_and_never_creates_job(session_home, payload):
    store, _, session = session_home
    jobs = NarrationJobs(store)
...
@pytest.mark.parametrize("data", [b"", b"plain text", b"<html>failure</html>", b"RIFF", b"x" * (16 * 1024 * 1024 + 1)])
...
async def test_canonical_cache_missing_and_tampered_records_fail_closed(session_home):
    ...
    jobs.update(job["id"], status="ready", artifact_slug="missing", artifact_version=1, audio_hash="wrong")
    with pytest.raises(NotFound, match="missing or changed"):
        jobs.audio(job["id"])
    record = jobs.artifacts.create(name="A text record", content="Not speech", kind="text", source="manual")
...
async def test_http_narration_uses_source_and_returns_unavailable_state(session_home):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(PREFIX + f"/narrations/{job['id']}/cancel", json={})
        assert (await response.json())["narration"]["status"] == "unavailable"
        response = await client.post(path, json={"revision": 1, "request_id": "http", "voice": "override"})
        ...
        assert response.status == 404
        response = await client.post(PREFIX + f"/narrations/{job['id']}/cancel", json={"home": "elsewhere"})
        assert response.status == 400
