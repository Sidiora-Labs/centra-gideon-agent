...
async def test_game_assets_route_and_native_provider_dispatch_from_shared_registration(
    tmp_path,
):
    app = web.Application()
    ...
    refused = await provider.invoke(
        "experience_game_assets_compile", {"id": "missing", "revision": 1}
    )
...
async def test_moltworld_route_and_secondary_native_provider_use_shared_experience_contract(
    ...
    record = RegisteredProvider(
        name=manifest.name, manifest=manifest, provider_config=config
    )
