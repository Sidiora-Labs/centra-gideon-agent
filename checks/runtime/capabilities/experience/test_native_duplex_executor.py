...
from gideon.workspace.capabilities.experience.native_duplex_helper import DuplexExecutorError,NativeDuplexExecutor,register_native_duplex_executor
...
def capture(**patch):
    return {'session_id':SESSION,'operation_id':OPERATION,'capture_seconds':8,**patch}


def wave():
    return b'RIFF'+(36).to_bytes(4,'little')+b'WAVEfmt '+(16).to_bytes(4,'little')+b'\x01\x00\x01\x00'+(16000).to_bytes(4,'little')+(32000).to_bytes(4,'little')+b'\x02\x00\x10\x00data'+(4).to_bytes(4,'little')+b'\x00\x00\x00\x00'


def playback(**patch):
    audio=wave()
    return {'session_id':SESSION,'operation_id':OPERATION,'mime':'audio/wav','audio':base64.b64encode(audio).decode(),'sha256':hashlib.sha256(audio).hexdigest(),**patch}
...
def test_readiness_is_truthful_and_source_is_fixed_avfoundation(executor):
    ...
    assert report['errors']==['Paired duplex execution requires macOS','Apple xcrun is unavailable']
    assert report['permission']=='microphone requested by AVFoundation'
    source=executor.source.read_text()
    for value in ['AVCaptureDevice.requestAccess(for: .audio)','AVAudioRecorder','AVAudioPlayer','AVSampleRateKey: 16000.0','AVNumberOfChannelsKey: 1']:
...
@pytest.mark.parametrize('body',[
    {},
    {'session_id':SESSION,'operation_id':OPERATION,'capture_seconds':8,'command':'/bin/sh'},
...
async def test_actual_linux_dispatch_fails_without_claiming_capture(executor):
    ...
    with sqlite3.connect(executor.db) as database:
        row=database.execute('SELECT kind,status,result FROM operations WHERE id=?',(OPERATION,)).fetchone()
...
def test_restart_marks_running_device_operation_uncertain(tmp_path):
    ...
    with sqlite3.connect(reopened.db) as database:
        status,result=database.execute('SELECT status,result FROM operations WHERE id=?',(OPERATION,)).fetchone()
...
async def test_authenticated_http_protocol_and_fixed_routes(executor):
    ...
    async def authenticate(request,handler):
        if not hmac.compare_digest(request.headers.get('Authorization',''),'Bearer '+TOKEN):
            return web.json_response({'error':'Native authentication required'},status=401)
    ...
    async with TestClient(TestServer(app)) as client:
        assert (await client.post('/v1/voice/duplex/capture',json=capture())).status==401
        headers={'Authorization':'Bearer '+TOKEN}
        response=await client.post('/v1/voice/duplex/capture',json=capture(),headers=headers)
        assert response.status==503
        assert 'requires macOS' in (await response.json())['error']
        wrong=await client.post('/v1/voice/duplex/shell',json={'command':'touch forbidden'},headers=headers)
        assert wrong.status==404
        invalid=await client.post('/v1/voice/duplex/play',json={**playback(operation_id='second'),'path':'/tmp/audio'},headers=headers)
...
def test_capture_and_playback_replay_rules_survive_restart(tmp_path):
    ...
    first.finish(capture_body['operation_id'],'completed_no_replay',{'sha256':'1'*64})
    ...
    reopened=NativeDuplexExecutor(root=tmp_path,platform='linux',xcrun=None)
    with pytest.raises(DuplexExecutorError,match='cannot be replayed safely') as capture_replay:
