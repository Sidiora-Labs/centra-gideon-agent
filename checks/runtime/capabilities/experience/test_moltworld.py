...
from gideon.workspace.capabilities.experience.moltworld import (
    API_BASE,
    PROTOCOL,
    Moltworld,
    RemoteError,
...
async def moltworld(tmp_path):
        ...
        return web.json_response(
            {
                ...
                "hp": 99,
                "energy": 80,
                "inventory": [{"type": "berry", "quantity": 2}],
                "walletAddress": "private-remote-field",
            }
        )
    ...
    async def action(request):
        ...
        return web.json_response(
            {
                "success": True,
                "message": "Action queued for tick 121",
                "queuedForTick": 121,
                "currentTick": 120,
            },
            status=202,
    ...
    service = Moltworld(
        ...
        credential_resolver=lambda name: (
            "mw_test_secret" if name == "moltworld-key" else None
        ),
        base_url=str(server.make_url("")),
    )
    service.configure(
        {"enabled": True, "credential_name": "moltworld-key", "revision": 0}
    )
...
@pytest.mark.asyncio
async def test_current_v1_status_and_public_observation_use_actual_http_contract(
    moltworld,
):
    ...
    assert readiness == {
        "protocol": PROTOCOL,
        "base_url": API_BASE,
        "config": {"enabled": True, "credential_name": "moltworld-key", "revision": 1},
...
@pytest.mark.asyncio
async def test_explicitly_approved_action_is_queued_not_completed_and_idempotent(
    moltworld,
):
    service, seen, _server, _tmp_path = moltworld
    ...
    assert len(seen["actions"]) == 1
    reopened = Moltworld(
        ExperienceStore(_tmp_path),
        credential_resolver=lambda _name: "mw_test_secret",
...
@pytest.mark.asyncio
async def test_external_refusal_and_transport_uncertainty_are_durable_and_not_retried(
    moltworld,
):
    ...
    assert len(seen["actions"]) == 2
    assert [row["state"] for row in reopened.history()] == [
        "outcome_unknown",
        "refused_remote",
    ]
...
async def test_approval_configuration_secret_and_validation_boundaries(moltworld):
    ...
    with pytest.raises(Conflict, match="approval"):
        await service.action(
            {
                "request_id": "not_approved",
                "action": "move",
                "params": {"direction": "n"},
                "approval": {"approved": False, "source": "user"},
            }
        )
    assert seen["actions"] == []
    ...
    for area in (
        {"x1": 0, "y1": 0, "x2": 20, "y2": 19},
        {"x1": 5, "y1": 5, "x2": 4, "y2": 6},
        {"x1": 0, "y1": 0, "x2": 1},
        {"x1": 0, "y1": 0, "x2": 1, "y2": 100},
    ):
        with pytest.raises(ValueError):
            await service.observe(area)
    with pytest.raises(Conflict, match="revision"):
        service.configure(
            {"enabled": False, "credential_name": "moltworld-key", "revision": 0}
        )
    config_bytes = service.store.path.read_bytes()
    assert b"mw_test_secret" not in config_bytes
    disabled = service.configure(
        {"enabled": False, "credential_name": "moltworld-key", "revision": 1}
    )
...
async def test_http_and_native_provider_preserve_verified_vs_attested_states(moltworld):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        queued = await client.post(
            "/api/capabilities/experience/moltworld/actions",
            json=approved("http_move", "move", {"direction": "e"}),
        )
...
async def test_incompatible_current_protocol_responses_fail_closed(tmp_path):
    async def bad_status(_request):
        return web.json_response({"id": "agent"})

    async def bad_action(_request):
        return web.json_response({"success": True, "queuedForTick": 3}, status=200)
...
def test_current_primary_action_shapes_are_explicit_and_bounded(tmp_path):
    service = Moltworld(
        ExperienceStore(tmp_path), credential_resolver=lambda _name: "secret"
    )
    valid = {
        "look": {},
        "move": {"direction": "w"},
