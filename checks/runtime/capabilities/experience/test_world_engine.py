...
from gideon.extensions.apps.backend_runtime import (
    BackendSupervisor,
    get_backend_supervisor,
)
...
@pytest.mark.asyncio
async def test_uninstalled_engine_and_strict_operator_boundary(
...
async def test_real_engine_app_lifecycle_http_websocket_and_persistence(engine):
    ...
    async with aiohttp.ClientSession() as direct:
        ...
        async with direct.get(
            backend.base_url + "/version", headers={"X-Gideon-Proxy": "1:" + "0" * 64}
        ) as denied:
            assert denied.status == 403
    ...
    async with TestClient(TestServer(app)) as client:
        version = await client.get(
            PREFIX + "/host/version",
            headers={
                "Authorization": "untrusted-owner-token",
                "X-Gideon-Proxy": "bad-caller-signature",
            },
        )
        ...
        await socket.send_json(
            {
                "type": "verb",
                "verb": "say",
                "args": {"text": "Persistent actual world message"},
            }
        )
...
def test_actual_pinned_dependencies_and_bun_launch_contract():
    pins = {
        "worlds": "bf9e0231795d55297f976948e2a79de3e1320e9d",
        "video": "2d152b9ae4e12e9ec152ea088850e377c319d4d4",
    }
    for name, expected in pins.items():
        actual = subprocess.check_output(
            ["git", "-C", str(DEPS / name), "rev-parse", "HEAD"], text=True
        ).strip()
        assert actual == expected
        assert (DEPS / name / "LICENSE").is_file()
    bun = subprocess.check_output(
        [str(DEPS / "bun-linux-x64/bun"), "--version"], text=True
    ).strip()
    assert bun == "1.4.2"
    assert BackendSupervisor._launch_cmd("bun", Path("/app/launch.js")) == [
        "bun",
        "/app/launch.js",
    ]
    assert BackendSupervisor._launch_cmd("node", Path("/app/launch.js")) == [
        "node",
        "/app/launch.js",
    ]
...
async def test_actual_launcher_refuses_wrong_pinned_dependency(engine):
    data = install_engine()
    (data / "engine.json").write_text(
        json.dumps({"worlds": str(DEPS / "video"), "video": str(DEPS / "video")})
    )
