...
from gideon.workspace.capabilities.experience import Conflict, ExperienceStore
...
from gideon.workspace.capabilities.experience.world_foundations_provider import (
    WorldFoundationTools,
)
    ...
    return service.record(
        {
            ...
            "controller": {"kind": "ambient_beacon", "position": [2, 0, -3]},
...
async def test_real_controller_lifecycle_survives_engine_stop_and_restart(foundations):
    ...
    controller = await service.install_controller(
        {"foundation_id": foundation["id"], "world": "controller_lounge"}
    )
    assert (
        controller["state"] == "installed" and controller["desired_state"] == "stopped"
    )
    ...
    entity = (await worlds.call("controller_lounge"))["state"]["entities"][
        controller["entity_id"]
    ]
    assert entity["pos"] == [2, 0, -3]
    assert entity["comp"]["gideon_controller"] == {
    ...
    restarted = WorldFoundations(
        ExperienceStore(service.store.path.parent.parent), worlds
    )
    ...
    assert (
        canonical["state"]["entities"][controller["entity_id"]]["comp"][
            "gideon_controller"
        ]["controller_id"]
        == controller["id"]
    )
...
async def test_candidate_inherit_adopt_withdraw_and_private_style_boundary(foundations):
    ...
    assert (
        "private_notes" not in encoded
        and "violet" not in encoded
        and "style" not in encoded
    )
    ...
    assert inherited["provenance"] == {
        "kind": "inherited",
        "origin_instance": service.instance_id,
        "foundation_id": foundation["id"],
    }
    ...
    with pytest.raises(Conflict, match="promoted or adopted"):
        await receiver.install_controller(
            {"foundation_id": received["id"], "world": "controller_lounge"}
        )
...
async def test_http_and_native_permissions_operate_real_controller(foundations):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        installed = await client.post(
            "/api/capabilities/experience/world-foundations/controllers/install",
            json={"foundation_id": foundation["id"], "world": "http_world"},
        )
        assert installed.status == 200
        controller = (await installed.json())["controller"]
        armed = await client.post(
            f"/api/capabilities/experience/world-foundations/controllers/{controller['id']}/arm",
            json={"revision": controller["revision"]},
        )
        assert armed.status == 200
        controller = (await armed.json())["controller"]
        assert controller["last_receipt"]["complete"]
        stale = await client.post(
            f"/api/capabilities/experience/world-foundations/controllers/{controller['id']}/stop",
            json={"revision": 1},
        )
        assert stale.status == 409
        invalid = await client.post(
            "/api/capabilities/experience/world-foundations/record",
            json={
                "title": "Unsafe",
                "controller": {"kind": "arbitrary_script", "position": [0, 0, 0]},
                "style": {},
            },
        )
    ...
    installed = await provider.invoke(
        "experience_controller_install",
        {"foundation_id": foundation["id"], "world": "native_world"},
    )
    assert installed.success
    controller = json.loads(installed.output)["controller"]
    armed = await provider.invoke(
    ...
    refused = await provider.invoke(
        "experience_controller_control",
        {"id": controller["id"], "operation": "exec", "revision": 2},
    )
...
async def test_validation_revision_and_lifecycle_fail_closed(foundations):
    ...
    for body in (
        ...
        {
            "title": "x",
            "controller": {"kind": "other", "position": [0, 0, 0]},
            "style": {},
        },
        {
            "title": "x",
            "controller": {"kind": "ambient_beacon", "position": [0, 0]},
            "style": {},
        },
        {
            "title": "x",
            "controller": {"kind": "ambient_beacon", "position": [True, 0, 0]},
            "style": {},
        },
    ):
    ...
    with pytest.raises(Conflict, match="promoted or adopted"):
        await service.install_controller(
            {"foundation_id": record["id"], "world": "closed"}
        )
    record = service.package(record["id"], record["revision"])
...
async def test_native_provider_rejects_undeclared_and_malformed_operations(foundations):
    ...
    missing_world = await provider.invoke(
        "experience_controller_install", {"foundation_id": "missing"}
    )
    assert not missing_world.success
    malformed_control = await provider.invoke(
        "experience_controller_control", {"id": "missing", "operation": "arm"}
    )
