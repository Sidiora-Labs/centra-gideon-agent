...
async def test_duplex_routes_and_capability_are_absent_without_explicit_opt_in(
    tmp_path,
):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert (
            await client.post(
                "/v1/voice/duplex/capture",
                headers=AUTH,
                json={"session_id": "one", "operation_id": "one", "capture_seconds": 1},
            )
        ).status == 404
        assert (
            await client.post("/v1/voice/duplex/play", headers=AUTH, json={})
        ).status == 404
...
async def test_enabled_helper_advertises_and_authenticates_fixed_routes(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert await identity.json() == {
            "device_id": "paired-mac",
            "protocol": 1,
            "capabilities": ["terminal.read", "voice.duplex"],
        }
        denied_capture = await client.post(
            "/v1/voice/duplex/capture",
            json={
                "session_id": "session",
        ...
        shell = await client.post(
            "/v1/voice/duplex/shell", headers=AUTH, json={"command": "touch forbidden"}
        )
...
async def test_enabled_playback_rejects_bad_digest_before_device_dispatch(tmp_path):
    executor = NativeDuplexExecutor(
        root=tmp_path, platform="darwin", xcrun="/usr/bin/xcrun"
    )
    app = create_app("paired-mac", TOKEN, duplex_audio=True, duplex_executor=executor)
    audio = b"RIFF" + b"\x00" * 4 + b"WAVE" + b"\x00" * 40
    body = {
        "session_id": "session",
        "operation_id": "play-one",
        "mime": "audio/wav",
