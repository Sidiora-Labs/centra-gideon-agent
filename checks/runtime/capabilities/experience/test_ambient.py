...
def test_preferences_persist_revision_and_reject_stale_changes(ambient,tmp_path):
    ...
    assert first=={'revision':1,'show_clock':True,'font_scale':1,'idle_seconds':30}
    saved=ambient.save({**first,'font_scale':2,'show_clock':False,'idle_seconds':60})
...
@pytest.mark.parametrize('patch',[{'show_clock':1},{'font_scale':True},{'font_scale':0},{'font_scale':4},{'idle_seconds':9},{'idle_seconds':301},{'revision':True},{'home':'/other'},{'metric':'invented'}])
...
@pytest.mark.asyncio
async def test_empty_sources_are_not_invented_vitals_and_reads_do_not_create_other_domain_stores(ambient,tmp_path):
...
async def test_actual_schedule_and_native_task_changes_reach_display(ambient,tmp_path):
    ...
    trigger=Trigger(id='daily',name='Daily review',kind='clock',spec={'kind':'cron','expr':'0 8 * * *'},next_fire_at='2030-01-01T08:00:00+00:00')
...
@pytest.mark.asyncio
async def test_actual_goals_sessions_measurements_and_progress_are_projected_without_notes(ambient,tmp_path):
    goals=GoalStore(tmp_path/'capabilities/identity/goals.sqlite3')
    goal=goals.save_goal(title='Learn piano',request_id='goal')
    session=goals.save_session(goal_id=goal['id'],title='Scales',start_at='2026-01-01T09:00:00+00:00',end_at='2026-01-01T09:30:00+00:00',request_id='session',status='completed')
    progress=ProgressStore(tmp_path/'capabilities/identity/progress.sqlite3')
    progress.configure(birth_date=None,timezone='UTC',tracked_task_ids=[],expected_revision=0,request_id='progress')
    health=MeasurementStore(tmp_path)
    measurement=health.create({'request_id':'weight','kind':'body_weight','observed_at':'2026-01-01T08:00:00+00:00','unit':'kg','values':{'weight':72},'source':'manual','notes':'Private note not needed on ambient screen'})
    result=await ambient.snapshot()
    assert result['cards']['goals']['rows']==[{'id':goal['id'],'title':'Learn piano','status':'active'}]
...
@pytest.mark.asyncio
async def test_scope_change_refuses_other_home_projections(ambient,tmp_path,monkeypatch):
    monkeypatch.setenv('GIDEON_HOME',str(tmp_path/'other'))
    with pytest.raises(Conflict,match='scope'):
        await ambient.snapshot()
    assert AmbientDisplay(ExperienceStore(tmp_path/'other')).preferences()['revision']==1
...
async def test_native_tools_and_http_share_preferences_and_source(ambient):
    ...
    saved=await provider.invoke('experience_ambient_update',{**preferences,'show_clock':False})
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        changed=await client.put(base,json={**result['preferences'],'font_scale':3})
        ...
        assert (await client.put(base,json={**preferences,'source_text':'invented'})).status==400


@pytest.mark.asyncio
async def test_completed_sessions_without_progress_profile_do_not_turn_into_zero_effort(ambient,tmp_path):
    goals=GoalStore(tmp_path/'capabilities/identity/goals.sqlite3')
    goal=goals.save_goal(title='Practice',request_id='goal')
    goals.save_session(goal_id=goal['id'],title='Completed practice',start_at='2026-01-01T09:00:00+00:00',end_at='2026-01-01T10:00:00+00:00',request_id='session',status='completed')
    result=await ambient.snapshot()
