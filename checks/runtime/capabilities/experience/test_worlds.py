...
from gideon.workspace.capabilities.experience.world_engine import (
    APP_ID,
    PREFIX,
    WorldEngine,
)
from gideon.workspace.capabilities.experience.worlds import (
    Worlds,
    get_worlds,
    identifier,
)
...
async def test_real_canonical_objects_revisions_receipts_restart(world):
    ...
    assert all(
        set(person) <= {"id", "avatar", "pose", "agent"}
        for person in snapshot["present"]
    )
    ...
    with pytest.raises(Conflict, match="changed"):
        await world.mutate(
            "workshop", "objects", {**create, "request_id": "stale", "id": "second"}
        )
    with pytest.raises(Conflict, match="exists"):
        await world.mutate(
            "workshop",
            "objects",
            {**create, "request_id": "duplicate", "expected_seq": after["seq"]},
        )
    placed = await world.mutate(
        "workshop",
        "objects",
        {
            "operation": "place",
    ...
    assert await world.mutate("workshop", "objects", create) == receipt
    removed = await world.mutate(
        "workshop",
        "objects",
        {
            "operation": "remove",
            "id": "desk",
            "expected_seq": restored["seq"],
            "request_id": "remove",
        },
    )
...
async def test_projection_reads_real_sources_preserves_edit_and_retries(world):
    ...
    goals.save_goal(
        ...
        status="completed",
        request_id="update_garden",
    )
    refreshed = await world.mutate(
        "garden",
        "project",
        {
            "kinds": ["goals"],
            "expected_seq": move["seq"],
            "request_id": "refresh_garden",
        },
    )
    ...
    assert (
        final["state"]["entities"][key]["comp"]["gideon_source"]["title"]
        == "Garden completed"
    )
    assert (
        final["state"]["entities"][key]["comp"]["gideon_source"]["status"]
        == "completed"
    )
    assert goals.get_goal(goal["id"])["title"] == "Garden completed"
    assert not any(
        row.get("kind") == "health" for row in final["state"]["entities"].values()
    )
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        async with client.ws_connect(PREFIX + "/host/ws") as visitor:
            ...
            await visitor.send_json(
                {
                    "type": "pose",
                    "pose": {"p": [3, 0, 3], "yaw": 1, "speed": 0, "clip": "idle"},
                }
            )
            for _ in range(30):
        ...
        assert (
            await client.post(base + "/objects", json={"operation": "exec"})
        ).status == 400
    ...
    result = await provider.invoke(
        "experience_world_objects",
        {
            ...
            "body": {
                ...
                "id": "table",
                "position": [4, 0, 4],
                "expected_seq": canonical["seq"],
                "request_id": "native_move",
            },
        },
    )
    assert result.success
    assert json.loads(result.output)["complete"]
    assert (await world.call("lounge"))["state"]["entities"]["table"]["pos"] == [
        4,
        0,
        4,
    ]
...
@pytest.mark.asyncio
async def test_validation_scope_and_absence_are_not_world_success(
    ...
    invalid = [
        ...
        False,
        {**base, "home": "other"},
        {**base, "expected_seq": True},
        {**base, "expected_seq": -2},
        {**base, "request_id": "../x"},
        {**base, "id": "/x"},
        {**base, "operation": []},
        {**base, "position": [0, 0]},
        {**base, "position": [float("nan"), 0, 0]},
        {**base, "position": [True, 0, 0]},
        {**base, "position": [10001, 0, 0]},
    ]
    for body in invalid:
        with pytest.raises(ValueError):
