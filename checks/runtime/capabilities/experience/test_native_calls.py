...
from gideon.workspace.capabilities.experience.native_calls import (
    NativeCalls,
    get_native_calls,
)
...
def configure(calls, value):
    (calls.home / "capabilities" / "native_call_config.json").write_text(
        json.dumps(value)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [],
        {},
        {"target_handle": "+1234567"},
        {"target_handle": "x;open()", "target_name": "Person"},
        {"target_handle": "+1234567", "target_name": "\nPerson"},
        {"target_handle": True, "target_name": "Person"},
        {"target_handle": "+1234567", "target_name": ""},
        {"target_handle": "+1234567", "target_name": "Person", "helper": "/tmp/other"},
    ],
)
...
async def test_concurrent_retries_have_one_durable_request(calls):
    rows = await asyncio.gather(
        *(calls.command({"command": "probe", "request_id": "same"}) for _ in range(12))
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [],
        {},
        {"command": [], "request_id": "a"},
        {"command": "other", "request_id": "a"},
        {"command": "call", "request_id": ""},
        {"command": "call", "request_id": "a/b"},
        {"command": "call", "request_id": "a", "target_handle": "+1234567"},
    ],
)
@pytest.mark.asyncio
...
async def test_real_http_transcript_handoff_is_supplied_not_captured(calls):
    state = ConsoleState(
        ...
        time.time(),
        conversation_log=ConversationLog(calls.home / "history"),
    )
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            prefix, json={"command": "call", "request_id": "attempt"}
        )
        row = await response.json()
        assert row["state"] == "unavailable"
        body = {
            "conversation": "existing",
            "messages": [{"role": "user", "text": "User-provided conversation notes"}],
            "request_id": "handoff",
        }
        ...
        assert (
            await client.post(route, json={**body, "conversation": "missing"})
        ).status == 404
        assert (await client.post(route, json={**body, "messages": []})).status == 400
        assert (
            await client.post(prefix, json={"command": {}, "request_id": "x"})
        ).status == 400
        state.get_or_create_session("private", app="another-app")
        assert (
            await client.post(route, json={**body, "conversation": "private"})
        ).status == 404
        state.get_or_create_session("temporary", memory_mode="temporary")
        assert (
            await client.post(route, json={**body, "conversation": "temporary"})
        ).status == 409
...
@pytest.mark.asyncio
async def test_handoff_refuses_home_change_ephemeral_and_malformed_records(
    calls, monkeypatch, tmp_path
):
    state = ConsoleState(
        ConversationDirectory(AppConfig()),
        time.time(),
    ...
    body = {
        "conversation": "existing",
        "messages": [{"role": "user", "text": "Supplied text"}],
        "request_id": "record",
    }
    async with TestClient(TestServer(app)) as client:
        ...
        for messages in [
            ...
            [{"role": "system", "text": "x"}],
            [{"role": "user", "text": ""}],
            [{"role": "user", "text": "x", "native": True}],
        ]:
            assert (
                await client.post(route, json={**body, "messages": messages})
            ).status == 400
