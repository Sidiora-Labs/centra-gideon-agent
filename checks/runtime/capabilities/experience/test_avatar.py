...
def mutate_glb(data,change):
    ...
    return struct.pack('<III',0x46546c67,2,20+len(body)+len(remaining))+struct.pack('<II',len(body),0x4e4f534a)+body+remaining
...
def test_original_asset_is_reproducible_actual_geometry_with_real_clips():
    data=build_robot()
    path=Path(__file__).resolve().parents[4]/'runtime/gideon/workspace/capabilities/experience/assets/robot.glb'
...
@pytest.mark.parametrize('change',[
    ...
    lambda doc:doc['animations'][0]['channels'][0]['target'].update(path='imagined'),
...
def test_bundled_installs_once_persists_selection_and_pins_actual_bytes(avatars,tmp_path):
    assert avatars.list()==[]
    ...
    assert avatars.raw(installed['artifact_slug'],installed['artifact_version'])==build_robot()
    selected=avatars.select({'revision':1,'avatar_id':installed['id'],'entity_id':'session:dashboard:ui'})
...
def test_only_animated_actual_model_versions_enter_candidate_list(avatars):
    ...
    avatars.artifacts.create_binary(name='Static model',data=static,mime='model/gltf-binary',kind='model',source='manual')
    invalid=avatars.artifacts.create_binary(name='Broken model',data=b'not glb',mime='model/gltf-binary',kind='model',source='manual')
    assert avatars.models()==[]
    with pytest.raises(ValueError):
        avatars.publish({'title':'Broken','artifact_slug':invalid.slug,'artifact_version':1,'clips':{'idle':'idle'}})
    installed=avatars.bundled()
...
def test_clip_mapping_and_immutable_version_references(avatars):
    installed=avatars.bundled()
    body={'title':'Quiet robot','artifact_slug':installed['artifact_slug'],'artifact_version':1,'clips':{'idle':'idle'}}
    ...
    changed=mutate_glb(build_robot(),lambda doc:doc['nodes'][0].update(name='Revised robot'))
    avatars.artifacts.update_binary(installed['artifact_slug'],data=changed,mime='model/gltf-binary',expect_version=1)
    assert avatars.raw(installed['artifact_slug'],1)==build_robot()
    assert avatars.artifacts.raw_bytes(installed['artifact_slug'],version=2)[0]==changed
...
def test_title_spoof_cannot_replace_original_bundled_asset(avatars):
    alternate=mutate_glb(build_robot(),lambda doc:doc['nodes'][0].update(name='Alternate author'))
    art=avatars.artifacts.create_binary(name='Other model',data=alternate,mime='model/gltf-binary',kind='model',source='manual')
    other=avatars.publish({'title':'Gideon robot','artifact_slug':art.slug,'artifact_version':art.version,'clips':{name:name for name in STATES}})
    installed=avatars.bundled()
    assert installed['id']!=other['id']
    assert installed['source_hash']==hashlib.sha256(build_robot()).hexdigest()


def test_missing_artifact_disables_variant_and_other_home_has_no_source(avatars,tmp_path):
    installed=avatars.bundled()
...
@pytest.mark.parametrize('body',[None,[],{}, {'revision':1,'avatar_id':None,'entity_id':'x'*301},{'revision':1,'avatar_id':None,'entity_id':'','home':'other'}])
...
async def test_actual_http_and_native_provider_share_avatar_selection(avatars):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        select=await provider.invoke('experience_avatar_select',{'revision':1,'avatar_id':installed['id'],'entity_id':'loop:actual'})
        ...
        raw=await client.get(base+f"/avatar-assets/{installed['artifact_slug']}/1/raw")
        assert raw.status==200
        assert raw.headers['Content-Type']=='model/gltf-binary'
        assert await raw.read()==build_robot()
        assert (await client.get(base+f"/avatar-assets/{installed['artifact_slug']}/999/raw")).status==404
        assert (await client.post(base+'/avatars/bundled',json={'provider':'imagined'})).status==400
        assert (await client.put(base+'/avatar-selection',json={'revision':1,'avatar_id':None,'entity_id':''})).status==409
...
def test_concurrent_identical_publication_has_one_durable_variant(avatars):
    ...
    body={'title':'Shared variant','artifact_slug':original['artifact_slug'],'artifact_version':1,'clips':{'idle':'idle','working':'working'}}
...
def test_tampered_pinned_source_cannot_remain_ready(avatars):
    ...
    altered=mutate_glb(build_robot(),lambda doc:doc['nodes'][0].update(name='Tampered after publication'))
...
@pytest.mark.parametrize('patch',[{'title':''},{'artifact_version':True},{'artifact_version':0},{'home':'/other'},{'clips':[]},{'clips':{'idle':'imagined'}}])
def test_malformed_publication_is_not_registered(avatars,patch):
    original=avatars.bundled()
    before=avatars.list()
    body={'title':'Variant','artifact_slug':original['artifact_slug'],'artifact_version':1,'clips':{'idle':'idle'}}
    with pytest.raises(ValueError):
...
async def test_native_publication_and_selection_use_authoritative_registry(avatars):
    ...
    published=await provider.invoke('experience_avatar_publish',{'title':'Agent variant','artifact_slug':original['artifact_slug'],'artifact_version':1,'clips':{'idle':'idle','error':'error'}})
    assert published.success
    variant=json.loads(published.output)['avatar']
    selected=await provider.invoke('experience_avatar_select',{'revision':1,'avatar_id':variant['id'],'entity_id':'session:live'})
    ...
    missing=await provider.invoke('experience_avatar_select',{'revision':2,'avatar_id':'missing','entity_id':''})
...
def test_concurrent_selection_requires_current_revision(avatars):
    ...
    second=avatars.publish({'title':'Alternative','artifact_slug':first['artifact_slug'],'artifact_version':1,'clips':{'idle':'idle'}})
    def select(avatar):
        try:
            return avatars.select({'revision':1,'avatar_id':avatar['id'],'entity_id':''})
        except Conflict:
            return None
