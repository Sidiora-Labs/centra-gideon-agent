...
from gideon.workspace.capabilities.experience.store import Conflict, ExperienceStore, NotFound
...
def store(tmp_path):
    ...
    with value.connection() as db:
        db.execute("CREATE TABLE native_call_requests(id TEXT PRIMARY KEY,request_id TEXT UNIQUE,command TEXT,state TEXT,result TEXT,created_at REAL)")
        db.execute("INSERT INTO native_call_requests VALUES(?,?,?,?,?,?)", ("a" * 32, "call-1", "call", "requested", "requested", 1))
        db.execute("INSERT INTO native_call_requests VALUES(?,?,?,?,?,?)", ("b" * 32, "probe-1", "probe", "idle", "idle", 1))
        db.execute("INSERT INTO native_call_requests VALUES(?,?,?,?,?,?)", ("c" * 32, "failed-1", "call", "failed", "failure", 1))
    return value
...
def start_body(**patch):
    return {"call_id": "a" * 32, "request_id": "duplex-1", "max_turns": 10, "capture_seconds": 8, **patch}
...
def test_mac_command_is_fixed_to_bundled_source_and_bounded_arguments(store, tmp_path):
    ...
    assert service.command("capture", target, 8) == ["/usr/bin/xcrun", "swift", str(service.source), "capture", str(target), "8"]
    assert service.command("play", target) == ["/usr/bin/xcrun", "swift", str(service.source), "play", str(target)]
...
def test_start_rejects_unbounded_values(duplex, patch, message):
    with pytest.raises(ValueError, match=message): duplex.start(start_body(**patch))
    assert duplex.list() == []


def test_start_rejects_unknown_fields_and_missing_fields(duplex):
    for body in ({}, {"call_id": "a" * 32}, {**start_body(), "command": "/bin/sh"}):
        with pytest.raises(ValueError, match="Expected call_id"): duplex.start(body)
    assert duplex.list() == []


def test_start_requires_existing_nonterminal_nonprobe_call(duplex):
    with pytest.raises(NotFound, match="call request not found"): duplex.start(start_body(call_id="d" * 32))
    with pytest.raises(Conflict, match="never initiates"): duplex.start(start_body(call_id="b" * 32))
    with pytest.raises(Conflict, match="never initiates"): duplex.start(start_body(call_id="c" * 32))
    assert duplex.list() == []
...
def test_get_rejects_invalid_or_missing_identifiers(duplex):
    with pytest.raises(ValueError, match="identifier"): duplex.get("../escape")
    with pytest.raises(NotFound, match="not found"): duplex.get("d" * 32)
...
def test_compare_and_swap_tracks_state_and_refuses_stale_updates(store):
    ...
    with pytest.raises(Conflict, match="revision changed"): service.stop(session["id"], session["revision"])
...
def test_stop_refuses_active_device_operation(store):
    ...
    with pytest.raises(Conflict, match="interrupt the worker"): service.stop(active["id"], active["revision"])
...
async def test_capture_refuses_unavailable_session_without_touching_device(duplex):
    ...
    with pytest.raises(Conflict, match="not ready"): await duplex.capture(session["id"], session["revision"])
...
async def test_speak_requires_a_captured_turn_and_bounded_reply(store):
    service = NativeDuplex(store, platform="darwin", xcrun="/usr/bin/xcrun")
    session = service.start(start_body())
    with pytest.raises(ValueError, match="Reply text"): await service.speak(session["id"], session["revision"], "")
    with pytest.raises(ValueError, match="Reply text"): await service.speak(session["id"], session["revision"], "x" * 10001)
    with pytest.raises(Conflict, match="no captured turn"): await service.speak(session["id"], session["revision"], "hello")
...
async def test_http_exposes_readiness_durable_attach_and_fixed_actions(store):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        created = await client.post("/api/capabilities/experience/native-duplex", json=start_body())
        assert created.status == 200
        session = await created.json()
        loaded = await client.get("/api/capabilities/experience/native-duplex/" + session["id"])
        assert loaded.status == 200
        assert (await loaded.json())["call_id"] == "a" * 32
        capture = await client.post(f"/api/capabilities/experience/native-duplex/{session['id']}/capture", json={"revision": session["revision"]})
        assert capture.status == 409
        unknown = await client.post(f"/api/capabilities/experience/native-duplex/{session['id']}/shell", json={"revision": session["revision"]})
        assert unknown.status == 404


def test_no_host_command_or_external_call_fields_are_stored(duplex):
    session = duplex.start(start_body())
    assert set(session) == {"id", "request_id", "call_id", "state", "revision", "max_turns", "capture_seconds", "turns", "error", "created_at", "updated_at"}
