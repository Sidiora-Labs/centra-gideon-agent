...
def test_hierarchy_milestones_replay_and_canonical_goals_unchanged(tmp_path):
    ...
    milestones = [
        ...
        {"id": "mount", "title": "Build mount", "done": False, "target_date": None},
    ]
    plan = configure(store, child, parent_id=parent["id"], milestones=milestones)
    assert plan["revision"] == 1
    assert plan["parent_id"] == parent["id"]
    assert (
        configure(store, child, parent_id=parent["id"], milestones=milestones) == plan
    )
...
def test_real_task_loop_and_session_links_projection_and_missing_source(tmp_path):
    ...
    session = store.goals.save_session(
        goal_id=parent["id"],
    ...
    links = [
        {"kind": "task", "id": sources["task"]},
        {"kind": "loop", "id": sources["loop"]},
...
def test_observations_normalize_sort_deduplicate_and_derive_real_delta(tmp_path):
    ...
    later = store.checkin(
        goal_id=goal["id"],
        value=30,
        observed_at="2026-10-03T12:00:00+02:00",
        notes="Read chapters",
        request_id="later",
    )
    ...
    assert projection["velocity"] == {
        "value_per_day": 10,
        "unit": "pages",
        "from": earlier["observed_at"],
        "to": later["observed_at"],
    }
    ...
    with pytest.raises(ConflictError, match="unit cannot change"):
        configure(
            store, goal, unit="hours", expected_revision=1, request_id="unit-change"
        )
...
async def test_actual_http_hierarchy_checkins_and_conflicts(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert plan["revision"] == 1
        assert (await (await client.get(PREFIX + "/" + parent["id"])).json())[
            "children"
        ] == [child["id"]]
        for request_id, value, observed in [
            ("first", 2, "2026-10-01T00:00:00Z"),
            ("second", 8, "2026-10-03T00:00:00Z"),
        ]:
        ...
        assert (
            await client.post(
                PREFIX + "/configure", json={**body, "request_id": "stale"}
            )
        ).status == 409
        assert (
            await client.post(PREFIX + "/configure", json={**body, "home": "/other"})
        ).status == 400
...
async def test_native_goal_planning_uses_real_authoritative_store(tmp_path):
    ...
    try:
        ...
        assert result.success
        result = await provider.invoke(
            "identity_goal_plan_get", {"goal_id": child["id"]}
        )
