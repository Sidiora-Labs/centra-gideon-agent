...
from gideon.interfaces.dashboard.handlers.capabilities_identity_recipes import (
    PREFIX,
    register,
)
...
def save(store, **changes):
    steps = [
        {"id": "goals", "tool": "identity_goals_list_goals", "arguments": {}},
        {
            "id": "detail",
            "tool": "identity_goals_get_goal",
            "arguments": {"id": {"$ref": "goals#/0/id"}},
        },
    ]
    return store.save(
        **{
            "title": "Inspect human goal",
            "steps": steps,
            "request_id": "recipe",
            **changes,
        }
...
def test_recipe_versions_restore_replay_and_restart(tmp_path):
    ...
    second = save(
        ...
        expected_revision=1,
        request_id="edit",
        title="Inspect changed",
    )
    assert second["revision"] == 2
    assert store.history(first["id"]) == [first, second]
    restored = store.restore(
        id=first["id"], revision=1, expected_revision=2, request_id="restore"
    )
    ...
    with pytest.raises(KeyError):
        store.restore(
            id=first["id"], revision=999, expected_revision=3, request_id="bad-restore"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            {
                "id": "one",
                "tool": "identity_goals_list_goals",
                "arguments": {},
                "provider": "other",
            }
        ],
...
@pytest.mark.asyncio
async def test_actual_guarded_reads_bind_outputs_and_retries_do_not_repeat_steps(
    tmp_path,
):
    ...
    try:
        ...
        assert (
            store.begin(recipe_id=recipe["id"], revision=1, request_id="run") == first
        )
...
@pytest.mark.asyncio
async def test_cancellation_and_session_revocation_are_actual_dispatch_boundaries(
    tmp_path,
):
    store, provider, _ = setup(tmp_path)
    recipe = save(store)
...
async def test_concurrent_step_claim_has_one_actual_outcome(tmp_path):
    ...
    try:
        results = await asyncio.gather(
            store.advance(run["id"], 0, provider), store.advance(run["id"], 0, provider)
        )
...
@pytest.mark.asyncio
async def test_actual_binding_and_tool_schema_failures_preserve_partial_results(
    tmp_path,
):
    store, provider, goal = setup(tmp_path)
    recipe = save(
...
async def test_actual_http_author_restore_and_dispatch_bound_provider(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert await (
            await client.get(PREFIX + "/" + recipe["id"] + "/history")
        ).json() == [recipe]
        run = await (
            await client.post(
                PREFIX + "/begin",
                json=dict(recipe_id=recipe["id"], revision=1, request_id="run"),
            )
        ).json()
        assert run["status"] == "ready"
        response = await client.post(
            PREFIX + "/advance", json=dict(run_id=run["id"], expected_index=0)
        )
        assert response.status == 200
        ...
        assert (
            await (await client.get(PREFIX + "/runs/" + run["id"])).json() == completed
        )
        assert await (await client.get(PREFIX + "/runs")).json() == [completed]
        assert (
            await client.post(
                PREFIX + "/advance",
                json=dict(run_id=run["id"], expected_index=0, provider="other"),
            )
        ).status == 400
...
async def test_native_recipe_authoring_and_actual_nested_read_delegate(tmp_path):
    ...
    try:
        ...
        result = await provider.invoke(
            ...
            {"recipe_id": recipe["id"], "revision": 1, "request_id": "run"},
        )
        assert result.success
        run = json.loads(result.output)
        result = await provider.invoke(
            "identity_recipe_advance", {"run_id": run["id"], "expected_index": 0}
        )
        assert result.success
