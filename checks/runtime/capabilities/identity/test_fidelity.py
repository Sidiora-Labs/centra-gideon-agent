...
from gideon.interfaces.dashboard.handlers.capabilities_identity_fidelity import register, PREFIX


def setup_sources(path):
    twin = TwinStore(path / "twin.sqlite3")
    state = twin.save_document(title="Values", text="I value curiosity and honesty.", expected_revision=0)
    return twin, state["documents"][0], FidelityStore(path / "fidelity.sqlite3")


def case(store, source, **changes):
    return store.save_case(prompt="What matters to me?", source_ids=[source["id"]], request_id="case",
                           rules=changes.pop("rules", [{"type": "contains", "value": "curiosity"}]), **changes)
...
def test_supplied_observation_has_explicit_non_provider_provenance(tmp_path):
    ...
    run = store.record_observation(case_id=record["id"], answer="Curiosity matters.", request_id="one")
    ...
    assert run["source_snapshot"] == [{"id": source["id"], "title": source["title"], "text": source["text"]}]
...
@pytest.mark.parametrize("rule,answer,expected", [
    ({"type": "contains", "value": "Curiosity"}, "CURIOSITY", True),
    ({"type": "contains", "value": "Curiosity", "case_sensitive": True}, "curiosity", False),
...
def test_actual_existing_assertion_rules(rule, answer, expected, tmp_path):
    ...
    run = store.record_observation(case_id=record["id"], answer=answer, request_id="observation")
...
def test_all_rules_must_pass_and_full_evidence_remains(tmp_path):
    _, source, store = setup_sources(tmp_path)
    rules = [{"type": "contains", "value": "curiosity"}, {"type": "not_contains", "value": "honesty"}]
    record = case(store, source, rules=rules)
    run = store.record_observation(case_id=record["id"], answer="Curiosity and honesty.", request_id="one")
...
def test_snapshots_survive_case_and_source_edits(tmp_path):
    ...
    run = store.record_observation(case_id=original["id"], answer="Curiosity", request_id="one")
    changed = store.save_case(id=original["id"], prompt=original["prompt"], source_ids=original["source_ids"],
                              rules=[{"type": "equals", "value": "honesty"}], expected_revision=1)
    ...
    assert store.get_case(original["id"])["rules"] == [{"type": "equals", "value": "honesty"}]
...
def test_observation_replay_conflicts_without_duplicates(tmp_path):
    _, source, store = setup_sources(tmp_path)
    record = case(store, source)
    run = store.record_observation(case_id=record["id"], answer="Curiosity", request_id="one")
    assert store.record_observation(case_id=record["id"], answer="Curiosity", request_id="one") == run
    with pytest.raises(ConflictError, match="different evaluation"):
        store.record_observation(case_id=record["id"], answer="Honesty", request_id="one")
...
@pytest.mark.parametrize("rules", [[], [{"type": "judge", "value": "Looks right"}],
    [{"type": "regex", "value": "(a+)+$"}], [{"type": "contains", "value": ""}],
    [{"type": "equals", "value": "a", "extra": True}], [{"type": "equals", "value": "a", "case_sensitive": 1}]])
...
def test_private_disabled_foreign_and_missing_sources_refused(tmp_path):
    ...
    for ids in [[], [source["id"], source["id"]], [foreign["id"]], ["missing"], [None]]:
        with pytest.raises(ValueError):
            store.save_case(prompt="Question", source_ids=ids, rules=[{"type": "contains", "value": "x"}])
    record = case(store, source)
    twin.save_document(**{**source, "private": True}, expected_revision=1)
    with pytest.raises(ValueError, match="non-private"):
        store.record_observation(case_id=record["id"], answer="Curiosity", request_id="one")
...
def test_case_creation_replay_survives_restart_and_refuses_changed_payload(tmp_path):
    ...
    with pytest.raises(ValueError, match="request_id"):
        store.save_case(prompt="Question", source_ids=[source["id"]], rules=original["rules"])
    assert reopened.list_cases() == [original]
...
async def test_actual_http_cases_observations_runs_and_conflicts(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post(PREFIX + "/cases", json={"request_id": "http-case", "prompt": "What matters?", "source_ids": [source["id"]], "rules": [{"type": "contains", "value": "curiosity"}]})
        assert response.status == 200
        record = await response.json()
        response = await client.get(PREFIX + "/cases/" + record["id"])
        assert await response.json() == record
        body = {"case_id": record["id"], "answer": "I value curiosity.", "request_id": "http"}
        ...
        assert await response.json() == [run]
        response = await client.post(PREFIX + "/observations", json={**body, "answer": "Different"})
        assert response.status == 409
        response = await client.post(PREFIX + "/cases", json={"prompt": "Question", "source_ids": [source["id"]], "rules": [{"type": "judge", "value": "anything"}]})
        assert response.status == 400
        response = await client.post(PREFIX + "/run", json={"case_id": record["id"], "request_id": "http", "model": "external"})
        assert response.status == 400
...
async def test_native_fidelity_observation_and_revoked_private_source(tmp_path):
    ...
    try:
        result = await provider.invoke("identity_fidelity_save_case", {"request_id": "tool-case", "prompt": "Values?", "source_ids": [source["id"]], "rules": [{"type": "contains", "value": "curiosity"}]})
        assert result.success, result.error
        record = json.loads(result.output)
        result = await provider.invoke("identity_fidelity_observe", {"case_id": record["id"], "answer": "Curiosity", "request_id": "tool"})
        assert result.success, result.error
        run = json.loads(result.output)
        assert run["origin"] == "supplied_observation"
        assert run["model"] is None
        assert run["passed"]
        assert json.loads((await provider.invoke("identity_fidelity_get_case", {"id": record["id"]})).output) == record
