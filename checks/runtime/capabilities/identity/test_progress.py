...
from gideon.interfaces.dashboard.handlers.capabilities_identity_progress import (
    PREFIX,
    register,
)
...
@pytest.mark.parametrize(
    "birthday,as_of,age",
    [
        ("2000-10-02", "2026-10-01", 25),
        ("2000-10-02", "2026-10-02", 26),
        ("2000-02-29", "2025-02-28", 24),
        ("2000-02-29", "2025-03-01", 25),
        (None, "2026-10-01", None),
        ("2000-01-01", "1999-12-31", None),
    ],
)
...
def test_profile_replay_optimistic_edit_and_retry_conflicts(tmp_path):
    ...
    second = configure(
        store, timezone="Asia/Tokyo", expected_revision=1, request_id="edit"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"birth_date": "20000101"},
        {"timezone": "Not/AZone"},
        {"tracked_task_ids": ["../escape"]},
        {"tracked_task_ids": ["same", "same"]},
        {"tracked_task_ids": [False]},
...
def test_session_projection_tracks_current_sources_and_local_date_without_duplicates(
    tmp_path,
):
    store = store_for(tmp_path)
...
def test_real_native_task_provider_contributes_once_across_reads_and_status_changes(
    tmp_path,
):
...
"""
    environment = {
        **os.environ,
        ...
        "PYTHONPATH": str(Path(__file__).resolve().parents[4] / "runtime"),
    }
    ...
    assert sheet["tasks"] == [
        {"id": task["id"], "title": "Study optics", "status": "done"}
    ]
    ...
    subprocess.run(
        [sys.executable, "-c", script, task["id"]],
        env=environment,
        capture_output=True,
        text=True,
        check=True,
...
async def test_http_profile_projection_errors_and_isolated_homes(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert (
            await client.put(PREFIX, json={**body, "request_id": "stale"})
        ).status == 409
...
async def test_native_progress_configuration_and_read_actual_core(tmp_path):
    ...
    try:
        response = await provider.invoke(
            "identity_progress_sheet", {"as_of": "2026-10-01"}
        )
        ...
        invalid = await provider.invoke(
            "identity_progress_sheet", {"home": str(tmp_path)}
        )
