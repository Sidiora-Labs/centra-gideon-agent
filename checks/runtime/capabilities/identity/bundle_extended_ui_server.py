...
async def main():
    ...
    twin.save_document(title='Source private story', text='Private formative event', private=True, expected_revision=0)
    stories = StoryStore(home / 'source/capabilities/identity/stories.sqlite3')
    story = stories.create(prompt='Origin?', theme='past', text='Original answer', request_id='origin')
    stories.update(story['id'], expected_revision=1, prompt='Origin?', theme='past', text='Edited answer')
    runners, endpoints = [], []
    for name in ('source', 'destination'):
        ...
        endpoints.append(f'http://127.0.0.1:{site._server.sockets[0].getsockname()[1]}/api/capabilities/identity/bundles')
