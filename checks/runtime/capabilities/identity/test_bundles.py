...
from gideon.workspace.capabilities.identity.bundles import FORMAT, BundleService
...
def source_bundle(home):
    ...
    bundle = service.export_bundle(
        groups=["persona", "self_notes"], passphrase=PASSPHRASE
    )
...
def preview(service, bundle, groups=None):
    return service.preview(
        bundle=bundle, passphrase=PASSPHRASE, groups=groups or ["persona", "self_notes"]
    )
...
def apply(service, bundle, plan, **changes):
    return service.apply_bundle(
        **dict(
            ...
            groups=[row["id"] for row in plan["groups"]],
...
def test_existing_authenticated_envelope_omits_excluded_state_and_randomizes(tmp_path):
    ...
    source.continuity.configure(
        heartbeat_paused=True, expected_revision=0, request_id="pause"
    )
    ...
    second = source.export_bundle(
        groups=["persona", "self_notes"], passphrase=PASSPHRASE
    )
    assert second["ciphertext"] != bundle["ciphertext"]
    assert decrypt_archive(
        base64.b64decode(second["ciphertext"]), PASSPHRASE
    ) == decrypt_archive(base64.b64decode(bundle["ciphertext"]), PASSPHRASE)
    inventory = source.inventory()
    assert inventory["groups"] == [
        {"id": "persona", "count": 1, "cap_chars": 400},
        {"id": "self_notes", "count": 1, "cap_chars": 500},
    ]
...
def test_two_home_selective_import_replay_and_destination_policy_unchanged(tmp_path):
    ...
    destination.continuity.configure(
        heartbeat_paused=True, expected_revision=0, request_id="pause"
    )
    ...
    assert result == {
        "status": "applied",
        "applied": [{"slot": "persona", "count": 1}],
        "skipped": [{"slot": "persona", "count": 0}],
        "errors": [],
    }
    ...
    assert (
        source.continuity.status()["slots"]["self_notes"][0]["text"]
        == "Retain telescope calibration."
    )
...
def test_preview_duplicates_human_tombstones_and_no_resurrection(tmp_path):
    ...
    destination.continuity.append_anchor(
        slot="self_notes", text="Retain telescope calibration."
    )
    destination.continuity.remove_anchor(
        slot="self_notes", text="Retain telescope calibration."
    )
    ...
    assert (
        destination.continuity.status()["slots"]["self_notes"][0]["tombstoned"] is True
    )
    exported = destination.export_bundle(groups=["self_notes"], passphrase=PASSPHRASE)
    assert (
        json.loads(
            decrypt_archive(base64.b64decode(exported["ciphertext"]), PASSPHRASE)
        )["groups"]["self_notes"]
        == []
    )
...
def test_wrong_passphrase_and_tampering_do_not_create_destination_memory(tmp_path):
    ...
    with pytest.raises(ValueError, match="authentication"):
        destination.preview(
            bundle=bundle, passphrase="wrong passphrase value", groups=["persona"]
        )
...
def test_real_database_write_failure_reports_partial_and_does_not_replay(tmp_path):
    ...
    with sqlite3.connect(destination.home / "memory.db") as db:
        db.execute(
            "CREATE TRIGGER refuse_self_notes BEFORE INSERT ON semantic_memory WHEN NEW.key='slot.self_notes' BEGIN SELECT RAISE(ABORT,'write refused'); END"
        )
    plan = preview(destination, bundle)
    result = apply(destination, bundle, plan)
    assert result["status"] == "partial"
    assert result["applied"] == [
        {"slot": "persona", "count": 1},
        {"slot": "self_notes", "count": 0},
    ]
...
def test_receipt_failure_retains_honest_applying_state(tmp_path):
    ...
    with sqlite3.connect(destination.path) as db:
        db.execute(
            "CREATE TRIGGER refuse_receipt BEFORE UPDATE ON receipts BEGIN SELECT RAISE(ABORT,'receipt unavailable'); END"
        )
...
async def test_actual_http_encryption_preview_import_and_native_inventory(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            PREFIX + "/export", json={"groups": ["persona"], "passphrase": PASSPHRASE}
        )
        ...
        assert (
            await client.post(
                PREFIX + "/preview", json={**body, "passphrase": "incorrect passphrase"}
            )
        ).status == 400
        assert (
            await client.post(
                PREFIX + "/apply",
                json={**body, "preview_token": "stale", "request_id": "new"},
            )
        ).status == 409
    ...
    try:
        ...
        refused = await provider.invoke(
            "identity_bundle_export", {"passphrase": PASSPHRASE}
        )
