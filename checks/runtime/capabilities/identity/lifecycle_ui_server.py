...
from gideon.interfaces.dashboard.handlers.capabilities_identity_lifecycle import (
    register,
)
...
async def main():
    ...
    await site.start()
    print(
        f"http://127.0.0.1:{site._server.sockets[0].getsockname()[1]}/api/capabilities/identity/lifecycle#{loop.id}",
        flush=True,
    )
    try:
