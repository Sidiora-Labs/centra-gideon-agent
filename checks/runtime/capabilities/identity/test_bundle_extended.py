...
from gideon.workspace.capabilities.identity.bundle_extended import ExtendedBundleService, FORMAT, MAX_ENCODED
from gideon.workspace.capabilities.identity.bundle_inventory import BundleInventory, GROUPS, digest
...
def seed(home):
    ...
    twin.save_document(title='Private origin', text='Unshared formative experience', expected_revision=0, private=True)
    twin.configure(expected_revision=1, enabled=True, traits={'patience': 8}, personas=[{'id': 'calm', 'name': 'Calm', 'instructions': 'Consider carefully', 'trait_adjustments': {'patience': 9}}], active_persona_id='calm')
    stories = StoryStore(home / 'capabilities/identity/stories.sqlite3')
    parent = stories.create(prompt='Origin?', theme='origin', text='First answer', request_id='parent')
    stories.update(parent['id'], expected_revision=1, prompt='Origin?', theme='origin', text='Revised answer')
    child = stories.create(prompt='What next?', theme='future', text='Follow-up answer', parent_id=parent['id'], request_id='child')
...
def apply(service, bundle, groups, plan, key='apply'):
    return service.apply_bundle(bundle=bundle, groups=groups, passphrase=PASSPHRASE, preview_token=plan['preview_token'], request_id=key)


def envelope(groups):
    return {'format': FORMAT, 'ciphertext': base64.b64encode(encrypt_archive(json.dumps({'format': FORMAT, 'groups': groups}).encode(), PASSPHRASE)).decode()}


def test_inventory_projects_real_canonical_groups_without_provider_configuration(tmp_path):
...
def test_two_home_human_group_roundtrip_preserves_history_privacy_and_disables_context(tmp_path):
    ...
    assert [row['text'] for row in restored.history(parent['id'])] == ['First answer', 'Revised answer']
    ...
    restored.update(parent['id'], expected_revision=2, prompt='Origin?', theme='origin', text='Destination revision')
...
def test_conflict_preview_stale_token_and_request_mismatch_preserve_existing_sources(tmp_path):
    ...
    twin.save_document(title='Destination', text='Do not overwrite', expected_revision=0)
    ...
    assert receipt['status'] == 'applied'
    assert len(TwinStore(fresh.home / 'capabilities/identity/twin.sqlite3').snapshot()['documents']) == 1


def test_agent_definition_whitelist_preserves_destination_credentials_and_authority(tmp_path):
    source = ExtendedBundleService(tmp_path / 'source')
    source_config = {'agents': {'researcher': asdict(AgentProfile(description='Research carefully', system_prompt='Cite evidence', voice='Calm', natural_voice=True, provider='acp:other', provider_agent='private-account', model='secret-provider:model', default_dir='/private', tools=['*'], triggers=['auto'], approval_mode='auto'))}, 'provider': {'api_key': 'SOURCE-SECRET'}}
    (source.home / 'config.json').write_text(json.dumps(source_config))
    bundle = export(source, ['agent_definitions'])
    payload = json.loads(decrypt_archive(base64.b64decode(bundle['ciphertext']), PASSPHRASE))
    row = payload['groups']['agent_definitions'][0]
    assert set(row) == {'name', 'description', 'system_prompt', 'voice', 'natural_voice', 'specialty', 'route_hints'}
    ...
    destination = ExtendedBundleService(tmp_path / 'destination')
    destination_config = {'provider': {'api_key': 'DESTINATION-SECRET'}, 'default_agent': 'kept', 'agents': {'kept': asdict(AgentProfile(description='Keep existing', approval_mode='auto'))}, 'opaque': {'preserve': True}}
    ...
    assert preview(destination, bundle, ['agent_definitions'])['groups'][0]['duplicates'] == ['researcher']
...
def test_model_selectors_require_destination_existing_bindings_and_agents(tmp_path):
    source = ExtendedBundleService(tmp_path / 'source')
    (source.home / 'config.json').write_text(json.dumps({'agents': {'researcher': {'model': 'local:existing-model'}}}))
    bundle = export(source, ['model_policy'])
    destination = ExtendedBundleService(tmp_path / 'destination')
    first = preview(destination, bundle, ['model_policy'])
    assert not first['can_apply']
    assert any('agent definition first' in item for item in first['groups'][0]['conflicts'])
    (destination.home / 'config.json').write_text(json.dumps({'agents': {'researcher': asdict(AgentProfile())}}))
    blocked = preview(destination, bundle, ['model_policy'])
    assert not blocked['can_apply']
    assert any('already bound' in item for item in blocked['groups'][0]['conflicts'])
    (destination.home / 'active_models.json').write_text(json.dumps({'loops': ['local:existing-model']}))
    ...
    assert json.loads((destination.home / 'active_models.json').read_text()) == {'loops': ['local:existing-model']}
    assert preview(destination, bundle, ['model_policy'])['groups'][0]['duplicates'] == ['researcher']
    assert source.inventory()['groups'][-2]['id'] == 'model_policy'


def test_actual_avatar_binary_clip_provenance_rebind_without_activity_selection(tmp_path):
    source = ExtendedBundleService(tmp_path / 'source')
    original = AvatarStore(ExperienceStore(source.home))
    record = original.bundled()
    assert record['availability'] == 'ready'
    original.select({'revision': 1, 'avatar_id': record['id'], 'entity_id': 'loop:source-private'})
    ...
    assert apply(destination, bundle, ['avatars'], duplicate, 'duplicate')['applied'][0]['count'] == 0
...
def test_version_one_slot_export_and_import_still_use_existing_envelope(tmp_path):
    ...
    assert receipt['status'] == 'applied'
    assert destination.continuity.status()['slots']['persona'][0]['text'] == 'Careful observer'
    assert apply(destination, bundle, ['persona'], plan) == receipt
    assert preview(destination, bundle, ['persona'])['groups'][0]['duplicates'] == ['Careful observer']


def test_group_partial_failure_and_durable_receipt_do_not_invent_cross_store_atomicity(tmp_path):
    source, _, _, _, _ = seed(tmp_path / 'source')
    destination = ExtendedBundleService(tmp_path / 'destination')
    groups = ['human_twin', 'autobiography']
    bundle = export(source, groups)
    ...
    with sqlite3.connect(twin.path) as db:
        db.execute("CREATE TRIGGER deny_import BEFORE UPDATE ON twin BEGIN SELECT RAISE(FAIL,'blocked by real database'); END")
    receipt = apply(destination, bundle, groups, plan)
    ...
    assert len(StoryStore(destination.home / 'capabilities/identity/stories.sqlite3').list()) == 2
    assert apply(destination, bundle, groups, plan) == receipt
    assert ExtendedBundleService(destination.home).apply_bundle(bundle=bundle, groups=groups, passphrase=PASSPHRASE, preview_token=plan['preview_token'], request_id='apply') == receipt
    with sqlite3.connect(twin.path) as db:
    ...
    assert len(StoryStore(destination.home / "capabilities/identity/stories.sqlite3").export()["history"]) == 3
...
def test_strict_snapshot_validation_preserves_live_revision_invariants(tmp_path):
    ...
    old['stories'][0] = next(row for row in old['history'] if row['id'] == parent['id'] and row['revision'] == 1)
    with pytest.raises(ValueError, match='latest'):
        inventory.validate('autobiography', old)
    broken = copy.deepcopy(data)
    broken['history'] = [row for row in broken['history'] if not (row['id'] == parent['id'] and row['revision'] == 1)]
...
@pytest.mark.parametrize('group,data', [
    ('unknown', []), ('human_twin', {}), ('human_twin', []), ('autobiography', {}),
    ('agent_definitions', [{'name': '../escape'}]), ('agent_definitions', [{'name': 'safe', 'provider': 'external'}]),
    ('model_policy', [{'name': 'safe', 'model': 'local:model', 'api_key': 'secret'}]),
    ('model_policy', [{'name': '../escape', 'model': 'local:model'}]),
    ('avatars', [{'title': 'Bad', 'data': 'not-binary'}]),
])
def test_invalid_portable_group_rejected_without_destination_changes(tmp_path, group, data):
    inventory = BundleInventory(tmp_path)
...
def test_authenticated_invalid_payload_and_wrong_passphrase_refuse_before_writes(tmp_path):
    destination = ExtendedBundleService(tmp_path)
    bad = envelope({'model_policy': [{'name': 'valid', 'model': 'local:bound', 'provider': 'injected'}]})
    ...
    with pytest.raises(ValueError, match='authentication'):
        destination.preview(bundle=valid, groups=['model_policy'], passphrase='wrong-passphrase-long')
    with pytest.raises(ValueError, match='envelope'):
        preview(destination, {**valid, 'ciphertext': 'A' * (MAX_ENCODED + 1)}, ['model_policy'])
    assert not (tmp_path / 'config.json').exists()
...
async def test_actual_http_extended_records_import_and_override_refusal(tmp_path):
    ...
    async with TestClient(TestServer(source_app)) as first, TestClient(TestServer(destination_app)) as second:
        response = await first.get(PREFIX)
        assert response.status == 200
        assert len((await response.json())['groups']) == 7
        response = await first.post(PREFIX + '/export', json={'groups': ['autobiography'], 'passphrase': PASSPHRASE})
        assert response.status == 200
        ...
        assert plan['can_apply']
        response = await second.post(PREFIX + '/apply', json={**body, 'preview_token': plan['preview_token'], 'request_id': 'http'})
        assert response.status == 200
        assert (await response.json())['status'] == 'applied'
        assert response.headers['Cache-Control'] == 'no-store'
        assert StoryStore(tmp_path / 'destination/capabilities/identity/stories.sqlite3').export() == stories.export()
        for field in ('home', 'provider', 'approval_mode'):
            response = await second.post(PREFIX + '/export', json={'groups': ['autobiography'], 'passphrase': PASSPHRASE, field: 'override'})
            assert response.status == 400
        response = await second.post(PREFIX + '/preview', json={**body, 'passphrase': 'incorrect-long-passphrase'})
        assert response.status == 400
