...
def home(tmp_path):
    keys = [
        "GIDEON_HOME",
        "GIDEON_CREDENTIAL_BACKEND",
        "TG_TEST_WEBHOOK_78DA",
        "TG_TEST_BOT_78DA",
    ]
...
def config(store, enabled=True, automatic=False):
    return telegram.configure(
        ...
        {
            "enabled": enabled,
            "automatic_replies": automatic,
            "bot_credential_ref": "TG_TEST_BOT_78DA",
            "webhook_credential_ref": "TG_TEST_WEBHOOK_78DA",
            "allowed_chat_ids": [12345, -999],
            "allowed_user_ids": [12345],
            "revision": telegram.config(store)["revision"],
        },
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"allowed_chat_ids": [0]},
        {"allowed_chat_ids": [2**53]},
        {"allowed_user_ids": list(range(1, 52))},
        {"bot_credential_ref": "raw-token:value"},
...
def test_actual_command_projection_uses_people_and_care(home):
    ...
    assert (
        telegram.command(store, "/care")["text"]
        == "No overdue or missing relationship touchpoints."
    )
...
def test_real_webhook_secret_allowlists_receipt_and_retry(home):
    ...
    assert telegram.receive(PeopleStore(), payload, "local-test-secret") == (
        receipt,
        False,
    )
...
def test_replayed_command_keeps_original_result_after_people_change(home):
    ...
    assert telegram.receive(store, update(command="/people"), "local-test-secret") == (
        receipt,
        False,
    )
...
def test_actual_notification_sdk_factory_and_registry_uses_allowlist(home):
    ...
    manifest = json.loads(
        (
            root / "runtime/gideon/extensions/apps/native/gideon-telegram-ops/app.json"
        ).read_text()
    )
    ...
    try:
        notification = {
            "id": "source-notice",
            "addressee": "telegram:12345",
            "title": "Actual notification",
            "body": "Queued from SDK",
        }
...
def test_native_telegram_tools_use_real_store_and_approval_metadata(home):
    ...
    async def scenario():
        ...
        result = await provider.invoke(
            ...
            {"request_key": "native", "chat_id": 12345, "text": "Native notification"},
        )
        assert result.success
        row = json.loads(result.output)["delivery"]
        result = await provider.invoke(
            "people_telegram_send", {"delivery_id": row["id"], "confirm_send": True}
        )
        ...
        result = await provider.invoke(
            "people_telegram_send", {"delivery_id": row["id"], "confirm_send": False}
        )
        assert not result.success
        result = await provider.invoke(
            "people_telegram_configure", {**settings, "enabled": False}
        )
        ...
        assert (
            definitions["people_telegram_configure"].risk_level.value == "destructive"
        )
        ...
        try:
            async with ClientSession() as client:
                ...
                async with client.post(
                    base + "/command", json={"command": "/people"}
                ) as response:
                ...
                async with client.post(
                    base + "/webhook", json=update(), headers=headers
                ) as response:
                    assert response.status == 200
                ...
                async with client.post(
                    base + "/deliveries",
                    json={
                        "request_key": "http-notice",
                        "chat_id": 12345,
                        "text": "HTTP notification",
                    },
                ) as response:
            ...
            async with ClientSession() as client:
                for method, path in (
                    ...
                    async with getattr(client, method)(
                        base + path, json={} if method == "post" else None
                    ) as response:
                ...
                async with client.post(
                    ...
                    assert response.status == 403
                async with client.post(
                    base + "/webhook", json=update(103, chat=777), headers=valid
                ) as response:
                    assert response.status == 403
                async with client.post(
                    base + "/webhook", json=update(104, user=777), headers=valid
                ) as response:
                    assert response.status == 403
                async with client.post(
                    base + "/webhook", json=update(105), headers=valid
                ) as response:
                    assert response.status == 200
