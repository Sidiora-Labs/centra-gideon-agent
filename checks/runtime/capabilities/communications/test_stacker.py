...
def registration(store, **extra):
    return social.save(
        store,
        {"platform": "stackernews", "handle": "alice", "request_key": "alice", **extra},
    )[0]
...
def reviewed(store, row, **extra):
    ...
    return stacker.review(
        store, row["id"], draft["id"], {"revision": 1, "confirm_review": True}
    )
...
def test_explicit_review_contains_exact_payload_and_fee_warning(tmp_path):
    ...
    with pytest.raises(PeopleError):
        stacker.review(
            store, row["id"], saved["id"], {"revision": 1, "confirm_review": False}
        )
    result = stacker.review(
        store, row["id"], saved["id"], {"revision": 1, "confirm_review": True}
    )
...
def test_comment_and_zap_review_destinations_and_auth_boundary(tmp_path):
    ...
    zap = reviewed(
        store, row, kind="zap", item_id="123", sats=21, text="", request_key="zap"
    )
    assert zap["sats"] == 21
    ...
    with pytest.raises(PeopleError):
        asyncio.run(
            stacker.submit(
                store, row["id"], zap["id"], {"revision": 2, "confirm_execute": True}
            )
        )
    assert (
        stacker.action(store, row["id"], zap["id"])["external_execution"]
        == "not_attempted"
    )
...
def test_actual_documented_discussion_mutation_variables_are_separate(tmp_path):
    ...
    row = registration(store)
    saved = prepare(
        store, row, title='Quotes " and braces }', text="mutation { not executable }"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"title": ""},
        {"text": ""},
        {"kind": "comment", "item_id": ""},
        {"kind": "comment", "item_id": "../abc"},
        {"kind": "zap", "item_id": "123", "sats": 0},
        {"sats": 1},
        {"sats": True},
        {"kind": "zap", "item_id": "123", "sats": 1000001},
        {"home": "/other"},
    ],
)
...
def test_identity_changes_disable_prior_review_and_account_isolation(tmp_path):
    ...
    with pytest.raises(PeopleError) as error:
        stacker.transition(
            first, row["id"], saved["id"], 2, {"reviewed"}, {"state": "submitting"}
        )
    assert error.value.status == 409
        ...
        with pytest.raises(PeopleError) as error:
            asyncio.run(
                stacker.submit(
                    ...
                    row["id"],
                    saved["id"],
                    {"revision": 2, "confirm_execute": True},
                )
            )
        assert error.value.status == 409
        ...
        with pytest.raises(PeopleError):
            asyncio.run(
                stacker.submit(
                    ...
                    row["id"],
                    saved["id"],
                    {"revision": 2, "confirm_execute": False},
                )
            )
        with pytest.raises(PeopleError):
...
def test_real_transition_claim_excludes_duplicate_submission_and_records_unknown(
    tmp_path,
):
    store = PeopleStore(tmp_path)
    row = registration(store)
    saved = reviewed(store, row)
    claim = stacker.transition(
        store,
        row["id"],
        saved["id"],
        2,
        {"reviewed"},
        {"state": "submitting", "external_execution": "unknown"},
    )
    ...
    assert error.value.status == 409
    current = stacker.finish(
        store, claim, {"state": "unknown", "external_execution": "unknown"}
    )
    ...
    with pytest.raises(PeopleError):
        stacker.transition(
            store, row["id"], saved["id"], 4, {"reviewed"}, {"state": "submitting"}
        )
...
def test_payin_normalizer_preserves_provider_state_without_claiming_execution(
    provider_state,
):
    result = stacker.payin(
        {
            "id": 42,
            "payInState": provider_state,
            "mcost": "21000",
            "item": {"id": "123"},
            "payerPrivates": {"payInFailureReason": None},
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        None,
        {},
        {"id": True, "payInState": "PAID", "mcost": 1},
        {"id": 1, "payInState": None, "mcost": 1},
        {"id": 1, "payInState": "PENDING", "mcost": -1},
        {"id": 1, "payInState": "PENDING", "mcost": 1, "item": []},
    ],
)
...
def test_actual_territory_schema_normalization_fees_cursor_and_page_scope():
    response = {
        ...
        "items": {
            "cursor": "next-page",
            "items": [{"id": "123", "title": "A post", "text": "Body"}],
        },
    }
    ...
    with pytest.raises(PeopleError):
        stacker.normalize_territory(
            {**response, "sub": {**response["sub"], "baseCost": -1}}
        )
    with pytest.raises(PeopleError):
...
def test_loopback_graphql_protocol_covers_authenticated_submit_and_reconcile_without_external_action(
    ...
    with runtime_home(tmp_path):
        ...
        async def scenario():
            ...
            try:
                ...
                reconciled = await stacker.reconcile(
                    store, account["id"], saved["id"], {}
                )
    ...
    with runtime_home(tmp_path):
        ...
        async def scenario():
            ...
            result = await provider.invoke(
                ...
                {"account_id": row["id"], "action_id": saved["id"]},
            )
            assert not result.success
            result = await provider.invoke(
                "people_stacker_actions", {"account_id": row["id"]}
            )
            assert (
                json.loads(result.output)["actions"][0]["external_execution"]
                == "not_attempted"
            )
            result = await provider.invoke("people_stacker_territories", {})
            assert json.loads(result.output)["territories"] == []
            result = await provider.invoke(
                "people_stacker_read", {"name": "../invalid"}
            )
            assert not result.success
            ...
            assert (
                definitions["people_stacker_submit"].risk_level == RiskLevel.DESTRUCTIVE
            )
            ...
            try:
                async with ClientSession() as client:
                    ...
                    async with client.post(
                        base + "/territories", json={"name": "../bad"}
                    ) as response:
                    ...
                    async with client.post(
                        action_path + "/review",
                        json={"revision": 1, "confirm_review": True},
                    ) as response:
                        assert (await response.json())["action"]["state"] == "reviewed"
                    async with client.post(
                        action_path + "/submit",
                        json={"revision": 2, "confirm_execute": True},
                    ) as response:
                        assert response.status == 409
                    async with client.get(path) as response:
                        assert (await response.json())["actions"][0][
                            "external_execution"
                        ] == "not_attempted"
                    async with client.post(
                        action_path + "/reconcile", json={}
                    ) as response:
                        assert response.status == 409
            finally:
                await runner.cleanup()
