...
def source(
    content="name,email,notes\nExample Friend,friend@example.com,Imported note",
    format="csv",
):
    ...
    return {
        **data,
        ...
        "decisions": (
            ...
            else [decision(row["row_id"]) for row in projected["rows"]]
        ),
    }
...
def test_csv_preview_is_read_only_and_normalizes(tmp_path):
    ...
    assert row["candidate"]["identities"] == [
...
def test_explicit_update_unions_identities_preserves_person(tmp_path):
    ...
    projected = preview(store, data)
    assert projected["rows"][0]["matches"] == [
        {"id": original["id"], "name": "Existing Name", "revision": 1}
    ]
    payload = request(
        store, data, [decision(action="update", person_id=original["id"], revision=1)]
    )
    receipt, created = commit(store, payload)
    assert created is True
...
def test_stale_update_rolls_back_new_people_and_receipt(tmp_path):
    ...
    payload = request(
        store,
        data,
        [decision(), decision("2", "update", person_id=original["id"], revision=1)],
    )
    changed = store.save(
        {
            "name": "Changed",
...
def test_ambiguity_is_visible_and_never_merges_people(tmp_path):
    store = PeopleStore(tmp_path)
    left = store.save(
        {"name": "Left", "identities": [{"kind": "email", "value": "left@example.com"}]}
    )
    right = store.save(
        {
            "name": "Right",
            "identities": [{"kind": "email", "value": "right@example.com"}],
        }
    )
    data = source("name,email\nAmbiguous,left@example.com|right@example.com")
    projected = preview(store, data)
    assert {m["id"] for m in projected["rows"][0]["matches"]} == {
        left["id"],
        right["id"],
    }
    for choices in (
        [decision()],
        [decision(action="update", person_id=left["id"], revision=1)],
    ):
        with pytest.raises(PeopleError) as failure:
...
def test_duplicate_source_rows_require_selected_skip(tmp_path):
    ...
    assert store.people() == []
    receipt, _ = commit(
        store, request(store, data, [decision(), decision("2", "skip")])
    )
    assert len(store.people()) == 1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        source("name,secret\none,two"),
        source("name,email\nOne"),
        source("name,email\nOne,a@example.com,extra"),
        source('name\n"Unterminated'),
        source("name"),
        source("x", "json"),
        source("BEGIN:VCARD\nVERSION:2.1\nFN:Old\nEND:VCARD", "vcard"),
        source("BEGIN:VCARD\nFN:No version\nEND:VCARD", "vcard"),
        source("FN:Outside", "vcard"),
        source("BEGIN:VCARD\nVERSION:4.0\nFN:Unfinished", "vcard"),
        source("BEGIN:VCARD\nBEGIN:VCARD", "vcard"),
        source("BEGIN:VCARD\nBad", "vcard"),
        source(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"decisions": []},
        {"decisions": [{}]},
        {"decisions": [decision(action="merge")]},
        {"decisions": [decision("2")]},
        {"decisions": [decision(person_id="injected")]},
        {"root": "/tmp"},
    ],
)
...
def test_update_target_and_identity_bound_checks(tmp_path):
    ...
    data = source("name,email\nNew,new@example.com")
    for target, revision, status in [
        ("missing", 1, 404),
        (original["id"], True, 400),
        (original["id"], 1, 400),
    ]:
        with pytest.raises(PeopleError) as failure:
            commit(
                ...
                request(
                    store,
                    data,
                    [decision(action="update", person_id=target, revision=revision)],
                ),
            )
...
def test_concurrent_same_import_is_one_commit(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        results = list(
            pool.map(lambda _: commit(PeopleStore(tmp_path), payload), range(3))
        )
...
def test_actual_http_preview_commit_and_receipt(tmp_path):
        ...
        try:
            async with ClientSession() as client:
                ...
                async with client.get(
                    origin + "/api/capabilities/communications/people"
                ) as response:
                ...
                person_id = receipt["rows"][0]["person_id"]
                async with client.get(
                    origin + "/api/capabilities/communications/people/" + person_id
                ) as response:
                ...
                async with client.post(
                    base + "/commit", json={**payload, "source_digest": "wrong"}
                ) as response:
                    assert response.status == 409
                async with client.post(
                    base + "/preview", json={**source(), "home": "/elsewhere"}
                ) as response:
    ...
    try:
        asyncio.run(journey())
        assert (
            len(PeopleStore(tmp_path / "capabilities" / "communications").people()) == 1
        )
    finally:
        if previous is None:
