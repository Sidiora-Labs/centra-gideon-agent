...
def mail(identifier='question', sender='friend@example.com', recipient='owner@example.com', reply=None, date=True, attachment=False):
...
def account(store, kind='maildir', **extra):
    return mirrors.save_account(store, {'name': 'Personal', 'kind': kind, 'owner_email': 'OWNER@EXAMPLE.COM', **extra})


def friend(store):
    return store.save({'name': 'Friend', 'identities': [{'kind': 'email', 'value': 'friend@example.com'}]})


def put(store, row, content=None, folder='INBOX'):
    return mirrors.upload(store, row['id'], {'content': content or mail(), 'folder': folder})
...
def test_account_restart_revision_and_no_secrets(tmp_path):
    ...
    updated = mirrors.save_account(store, {**data, 'name': 'Renamed', 'revision': 1}, row['id'])
    ...
    with pytest.raises(PeopleError):
        mirrors.save_account(store, {**data, 'owner_email': 'different@example.com', 'revision': 2}, row['id'])
    assert 'password' not in row


@pytest.mark.parametrize('extra', [{'home': '/tmp'}, {'kind': 'teams'}, {'owner_email': 'invalid'}, {'name': ''}, {'host': 'remote.example'}, {'auth_mode': 'plain'}, {'sent_folder': 'Sent\r\nX'}])
...
def test_reply_and_equal_date_ambiguity(tmp_path):
    ...
    assert {m['direction'] for m in mirrors.messages(store, row['id'])} == {'inbound', 'outbound'}
...
def test_conflicting_message_id_atomic_failure_preserves_prior(tmp_path):
    ...
    put(store, row, original.replace('A real message question', 'A conflicting subject'))
    with pytest.raises(PeopleError) as error:
        mirrors.sync(store, row['id'])
    assert error.value.status == 409
...
def test_real_mbox_replacement_prunes_snapshot(tmp_path):
    ...
    assert mirrors.get_account(store, row['id'])['sync']['scope'] == 'uploaded_or_local_source'
...
def test_missing_mbox_records_failure_then_recovers(tmp_path):
    ...
    assert mirrors.get_account(store, row['id'])['sync']['evidence_status'] == 'recorded'


@pytest.mark.parametrize('content', ['', ' ', None, 1, 'a' * (mirrors.MAX_SOURCE_BYTES + 1)])
    ...
    if level == 'root':
        ...
        if level == 'folder':
            ...
            if level == 'cur':
                ...
                (cur / (hashlib.sha256(raw.encode()).hexdigest() + ':2,S')).symlink_to(outside / 'target')
    with pytest.raises(PeopleError):
        put(store, row, raw)
...
def test_missing_remote_secret_is_honest_failure(tmp_path):
    ...
    row = account(store, 'imap', host='localhost', username='owner', credential_ref=key, alias='gmail', auth_mode='xoauth2')
...
def test_native_mirror_tools_real_runtime(tmp_path):
    ...
    try:
        ...
        async def scenario():
            ...
            row = (await call('people_mirror_create', {'account': {'name': 'Native', 'kind': 'maildir', 'owner_email': 'owner@example.com'}}))['account']
            upload = await call('people_mirror_upload', {'account_id': row['id'], 'content': mail()})
            assert upload['changed'] is True
            ...
            changed = await call('people_mirror_update', {'account_id': row['id'], 'account': update})
            ...
            denied = await provider.invoke('people_mirror_sync', {'account_id': row['id'], 'home': '/outside'})
            assert denied.success is False
            assert denied.metadata['status'] == 400
            missing = await provider.invoke('people_mirror_messages', {'account_id': 'missing'})
    ...
    async def scenario():
        ...
        try:
            async with ClientSession() as client:
                ...
                async with client.post(base + '/accounts', json={'name': 'HTTP', 'kind': 'maildir', 'owner_email': 'owner@example.com'}) as response:
                ...
                async with client.post(path + '/upload', json={'content': mail()}) as response:
                    assert response.status == 200
                ...
                async with client.post(path + '/sync', json={'home': '/outside'}) as response:
