...
def test_native_manifest_resolves_real_tool_provider(isolated_home):
    ...
    manifest = json.loads(
        (
            root / "runtime/gideon/extensions/apps/native/gideon-people/app.json"
        ).read_text()
    )
    ...
    for name, tool in by_name.items():
        ...
        assert tool.risk_level == (
            ...
            else RiskLevel.CAUTION if name in writes else RiskLevel.SAFE
        )
...
def test_actual_tools_create_read_update_and_touchpoint(isolated_home):
    ...
    created = output(
        call(
            provider,
            "people_create",
            {"person": {"name": "Tool Friend", "notes": "Initial"}},
        )
    )["person"]
    ...
    read = output(
        call(
            provider,
            "people_get",
            {"person_id": created["id"], "timezone": "Europe/Berlin"},
        )
    )
    ...
    updated = output(
        call(
            ...
            "people_update",
            {
                ...
                "person": {"name": "Tool Friend", "notes": "Updated", "revision": 1},
            },
        )
    )["person"]
    ...
    stale = call(
        provider,
        "people_update",
        {"person_id": created["id"], "person": {"name": "Stale", "revision": 1}},
    )
    ...
    recorded = output(
        call(
            provider,
            "people_record_touchpoint",
            {"person_id": created["id"], "touchpoint": touch},
        )
    )
...
def test_actual_tools_preview_and_commit_contacts(isolated_home):
    ...
    assert PeopleStore().get(person_id)["identities"] == [
        {"kind": "email", "value": "imported@example.com"}
    ]
...
def test_actual_tools_record_and_report_coverage(isolated_home):
    provider = create_provider()
    person = output(
        call(provider, "people_create", {"person": {"name": "Thread Friend"}})
    )["person"]
    now = datetime.now(timezone.utc)
    start = now - timedelta(hours=2)
    payload = {
        "source": "local-archive",
        "source_account_id": "provider-account",
    ...
    assert (
        output(call(provider, "people_record_thread_evidence", payload))["created"]
        is False
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("missing_tool", {}),
        ("people_get", {}),
        ("people_get", {"person_id": []}),
        ("people_create", {"person": {"name": "Name", "root": "/tmp"}}),
        ("people_list", {"home": "/tmp"}),
        ("people_list", {"timezone": "Bad/Zone"}),
...
def test_provider_configuration_cannot_select_another_home(isolated_home):
    ...
    person = output(
        call(provider, "people_create", {"person": {"name": "Current runtime"}})
    )["person"]
    ...
    assert (
        output(call(provider, "people_get", {"person_id": person["id"]}))["person"][
            "name"
        ]
        == "Current runtime"
    )
