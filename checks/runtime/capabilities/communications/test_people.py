...
def touch(**changes):
    return {
        ...
        "occurred_at": "2026-09-01T10:00:00+02:00",
        "direction": "mutual",
        "summary": "Met for lunch",
        **changes,
    }
...
def test_normalized_identities_and_ambiguity(tmp_path):
    ...
    changed = store.save(
        person(identities=[{"kind": "email", "value": "new@example.com"}], revision=1),
        first["id"],
    )
...
def test_touchpoint_replay_and_source_conflicts(tmp_path):
    ...
    replay, created = PeopleStore(tmp_path).record(
        first["id"], touch(occurred_at="2026-09-01T08:00:00Z")
    )
    assert replay == point
...
def test_reference_checks(tmp_path):
    ...
    for action in (
        ...
        lambda: store.touchpoints("missing"),
        lambda: store.record("missing", touch()),
        lambda: store.save(person(revision=1), "missing"),
    ):
        with pytest.raises(PeopleError) as failure:
            action()
...
def test_care_timezone_boundaries_and_future_exclusion(tmp_path):
    ...
    future, _ = store.record(
        created["id"], touch(external_id="future", occurred_at="2027-01-01T00:00:00Z")
    )
...
def test_concurrent_retry_claim_and_optimistic_edit(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        results = list(
            pool.map(
                lambda _: PeopleStore(tmp_path).record(created["id"], touch()), range(4)
            )
        )
    ...
    def edit(note):
        try:
            return PeopleStore(tmp_path).save(
                person(notes=note, revision=1), created["id"]
            )
...
def test_distinct_homes_and_subprocess_reload(tmp_path):
    ...
    completed = subprocess.run(
        [sys.executable, "-c", script, str(tmp_path / "left")],
        capture_output=True,
        text=True,
        check=True,
    )
        ...
        try:
            async with ClientSession() as client:
                ...
                async with client.put(
                    url, json=person(notes="HTTP edit", revision=1)
                ) as response:
                    assert response.status == 200
                    assert (await response.json())["person"]["revision"] == 2
                async with client.put(
                    url, json=person(notes="stale", revision=1)
                ) as response:
                ...
                async with client.get(base) as response:
                    ...
                    assert (
                        body["people"][0]["care"]["last_contact"]
                        == point["occurred_at"]
                    )
                async with client.get(base + "/missing") as response:
                    assert response.status == 404
                async with client.post(
                    base, data="not-json", headers={"Content-Type": "application/json"}
                ) as response:
                    assert response.status == 400
                for forbidden in ("home", "root", "provider", "model"):
                    async with client.post(
                        base, json=person(**{forbidden: "value"})
                    ) as response:
                        assert response.status == 400
