...
from gideon.workspace.capabilities.communications import calendar
from gideon.workspace.capabilities.communications.tools import create_provider


def source(store, kind='ics', **extra):
    return calendar.save_source(store, {'name': 'Personal calendar', 'kind': kind, **extra})


def event(uid='meeting-one', start='DTSTART:20260925T090000Z', end='DTEND:20260925T100000Z', title='Team meeting', extra=''):
    return f'BEGIN:VEVENT\r\nUID:{uid}\r\n{start}\r\n{end}\r\nSUMMARY:{title}\r\nLOCATION:Office\\, floor 2\r\n{extra}END:VEVENT\r\n'


def ics(*events):
    return 'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//Calendar test export//EN\r\n' + ''.join(events or [event()]) + 'END:VCALENDAR\r\n'


def upload(store, row, content=None):
    return calendar.upload(store, row['id'], {'content': content or ics(), 'revision': row['revision']})
...
def test_calendar_source_restart_revisions_and_configuration_invalidation(tmp_path):
    ...
    updated = calendar.save_source(store, {**data, 'name': 'Renamed', 'revision': 1}, row['id'])
    ...
    with pytest.raises(PeopleError):
        calendar.save_source(store, {**data, 'kind': 'google', 'credential_ref': 'TOKEN', 'revision': 2}, row['id'])


@pytest.mark.parametrize('change', [{'name': ''}, {'kind': 'unsupported'}, {'timezone': 'Invalid/Zone'}, {'home': '/outside'}, {'kind': 'google'}, {'kind': 'outlook', 'credential_ref': 'raw token'}])
...
def test_replacement_and_replay_do_not_resurrect_old_snapshot(tmp_path):
    ...
    assert [event['uid'] for event in calendar.daily(store, '2026-09-25')['events']] == ['replacement']
...
def test_all_day_exclusive_end_and_timezone_independent_date(tmp_path):
    ...
    upload(store, row, ics(event('holiday', 'DTSTART;VALUE=DATE:20260925', 'DTEND;VALUE=DATE:20260927', 'Two day holiday')))
...
def test_timed_midnight_overlap_and_local_day_filter(tmp_path):
    store = PeopleStore(tmp_path)
    row = source(store)
    upload(store, row, ics(event('late', 'DTSTART:20260925T233000Z', 'DTEND:20260926T003000Z', 'Late meeting')))
...
def test_iana_floating_times_and_dst_gap_fold_rejection(tmp_path):
    events, warnings = calendar.parse_ics(ics(event(start='DTSTART;TZID=Europe/Berlin:20260925T090000', end='DTEND;TZID=Europe/Berlin:20260925T100000')), 'UTC')
    assert warnings == []
    assert events[0]['start'] == '2026-09-25T07:00:00+00:00'
    events, _ = calendar.parse_ics(ics(event(start='DTSTART:20260925T090000', end='DTEND:20260925T100000')), 'America/New_York')
...
@pytest.mark.parametrize('text', ['not a calendar', 'BEGIN:VCALENDAR\nEND:VCALENDAR', ics(event() + event()), ics(event(start='DTSTART:invalid')), ics(event(end='DTEND:20260924T090000Z')), ics(event(start='DTSTART;VALUE=DATE:20260925')), ics(event().replace('END:VEVENT', ''))])
...
def test_unknown_empty_timezone_and_stale_coverage_are_distinct(tmp_path):
    ...
    old['sync']['captured_at'] = (datetime.now(timezone.utc) - timedelta(days=2)).isoformat()
...
def test_google_and_outlook_actual_event_shapes_normalize():
    google = {'id': 'google-instance', 'iCalUID': 'stable-uid', 'summary': 'Google event', 'start': {'dateTime': '2026-09-25T09:00:00+02:00'}, 'end': {'dateTime': '2026-09-25T10:00:00+02:00'}, 'location': 'Office'}
    ...
    outlook = {'id': 'outlook-instance', 'iCalUId': 'outlook-uid', 'subject': 'Outlook event', 'start': {'dateTime': '2026-09-25T09:00:00.0000000', 'timeZone': 'UTC'}, 'end': {'dateTime': '2026-09-25T10:00:00.0000000', 'timeZone': 'UTC'}, 'location': {'displayName': 'Remote'}}
    ...
    all_day = calendar.provider_event({**google, 'start': {'date': '2026-09-25'}, 'end': {'date': '2026-09-26'}}, 'google')
...
def test_remote_missing_connection_is_durable_failure_without_erasing_cached_events(tmp_path):
    store = PeopleStore(tmp_path)
    row = source(store, 'google', credential_ref='ABSENT_CALENDAR_TEST_479E')
    parsed, _ = calendar.parse_ics(ics(), 'UTC')
    state = {'state': 'synced', 'coverage': 'available_snapshot', 'scope': 'provider_window', 'start': '2026-09-25T00:00:00+00:00', 'end': '2026-09-26T00:00:00+00:00', 'captured_at': datetime.now(timezone.utc).isoformat()}
    calendar.persist(store, row, parsed, state)
    assert calendar.daily(store, '2026-09-25')['coverage'] == 'available_snapshot'
    assert calendar.daily(store, '2026-09-26')['sources'][0]['review_coverage'] == 'unknown'
    with pytest.raises(PeopleError) as error:
        asyncio.run(calendar.sync_remote(store, row['id'], {'start': state['start'], 'end': state['end']}))
...
def test_native_calendar_tools_actual_source_and_review(tmp_path):
    ...
    async def scenario():
        ...
        result = await provider.invoke('people_calendar_create', {'source': {'name': 'Native calendar', 'kind': 'ics'}})
        assert result.success, result.error
        row = json.loads(result.output)['source']
        result = await provider.invoke('people_calendar_upload', {'source_id': row['id'], 'content': ics(), 'revision': 1})
        assert json.loads(result.output)['sync']['events'] == 1
        result = await provider.invoke('people_calendar_daily', {'date': '2026-09-25', 'timezone': 'UTC'})
        ...
        result = await provider.invoke('people_calendar_update', {'source_id': row['id'], 'source': {'name': 'Updated', 'kind': 'ics', 'revision': 1}})
    ...
    async def scenario():
        ...
        try:
            async with ClientSession() as client:
                async with client.post(base + '/sources', json={'name': 'HTTP calendar', 'kind': 'ics'}) as response:
                    assert response.status == 201
                    row = (await response.json())['source']
                path = base + '/sources/' + row['id']
                async with client.post(path + '/upload', json={'content': ics(), 'revision': 1}) as response:
                    assert response.status == 200
                    assert (await response.json())['sync']['events'] == 1
                async with client.get(base + '/daily', params={'date': '2026-09-25', 'timezone': 'UTC'}) as response:
                    assert (await response.json())['events'][0]['uid'] == 'meeting-one'
                async with client.get(base + '/daily', params={'date': '2026-09-25', 'timezone': 'Invalid/Zone'}) as response:
                    assert response.status == 400
                async with client.put(path, json={'name': 'Edited', 'kind': 'ics', 'revision': 1}) as response:
