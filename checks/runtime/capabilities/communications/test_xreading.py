...
def registration(store, **extra):
    return social.save(
        store, {"platform": "x", "handle": "alice", "request_key": "alice", **extra}
    )[0]


def draft(store, row, **extra):
    return xreading.save_draft(
        store,
        row["id"],
        {"text": "A considered update & question?", "request_key": "draft1", **extra},
    )
...
def test_edit_invalidates_review_and_stale_edit_cannot_overwrite(tmp_path):
    ...
    edited = xreading.save_draft(
        store, row["id"], {"text": "Revised text", "revision": 1}, saved["id"]
    )
    ...
    with pytest.raises(PeopleError) as error:
        xreading.save_draft(
            store, row["id"], {"text": "Stale", "revision": 1}, saved["id"]
        )
    assert error.value.status == 409
    with pytest.raises(PeopleError):
        xreading.review(
    ...
    reviewed = xreading.review(
        store, row["id"], saved["id"], {"revision": 2, "confirm_review": True}
    )
...
def test_registration_revision_changes_invalidate_handoff_until_new_review(tmp_path):
    ...
    assert error.value.status == 409
    updated = xreading.save_draft(
        store, row["id"], {"text": saved["text"], "revision": 1}, saved["id"]
    )
...
def test_real_provider_page_normalization_and_explicit_partial_coverage():
    ...
    page = {
        "data": [
            {"id": "456", "text": "Official post", "created_at": "2026-09-25T09:00:00Z"}
        ],
        "meta": {"next_token": "opaque-next"},
    }
    ...
    partial = xreading.normalize_page(
        user, {**page, "errors": [{"title": "Partial data"}]}
    )
    assert partial["coverage"] == "partial"


def test_actual_read_adapter_uses_named_credential_and_bounded_paginated_api(
            ...
            return web.json_response(
                {
                    "data": [
                        {
                            "id": "456",
                            "text": "Read through the actual adapter",
...
def test_actual_read_adapter_refuses_redirects_and_oversized_responses(
    tmp_path, monkeypatch
):
    async def scenario():
        ...
        async def large(_request):
            return web.Response(
                body=b"x" * (2 * 1024 * 1024 + 1), content_type="application/json"
            )
...
@pytest.mark.parametrize(
    ...
    [
        (None, {}),
        ({"data": []}, {}),
        ({"data": {"id": "bad", "username": "a"}}, {}),
        ({"data": {"id": "1", "username": "a"}}, {"data": {}}),
        ({"data": {"id": "1", "username": "a"}}, {"meta": {"next_token": 2}}),
        (
            {"data": {"id": "1", "username": "a"}},
            {"data": [{"id": "1", "text": "a"}, {"id": "1", "text": "b"}]},
        ),
        ({"data": {"id": "1", "username": "a"}}, {"data": [None]}),
    ],
)
...
def test_missing_real_credential_persists_unknown_failure_and_revision_binding(
    tmp_path,
):
    with runtime_home(tmp_path):
        store = PeopleStore()
...
def test_actual_native_x_draft_review_and_missing_read(tmp_path):
    with runtime_home(tmp_path):
        ...
        async def scenario():
            ...
            result = await provider.invoke(
                "people_x_snapshot", {"account_id": row["id"]}
            )
            ...
            try:
                async with ClientSession() as client:
                    ...
                    async with client.post(
                        base + "/drafts",
                        json={"text": "HTTP draft", "request_key": "http"},
                    ) as response:
                        assert response.status == 200
                        saved = (await response.json())["draft"]
                    path = base + "/drafts/" + saved["id"]
                    async with client.post(
                        path + "/review", json={"revision": 1, "confirm_review": False}
                    ) as response:
                        assert response.status == 400
                    async with client.post(
                        path + "/review", json={"revision": 1, "confirm_review": True}
                    ) as response:
                        reviewed = (await response.json())["draft"]
                        assert reviewed["state"] == "reviewed_handoff"
                        assert reviewed["external_posted"] is False
                    async with client.put(
                        path, json={"text": "Edited HTTP", "revision": 1}
                    ) as response:
