...
def values(**extra):
    return {'platform': 'x', 'handle': '@Alice', 'request_key': 'register-alice', **extra}


def update(row, **extra):
    return {**{key: row[key] for key in social.FIELDS}, 'revision': row['revision'], **extra}
...
def test_person_links_and_atomic_optimistic_history(tmp_path):
    ...
    edited, created = social.save(store, update(row, status='paused', notes='Away'), row['id'])
    assert not created
    ...
    archived, _ = social.save(store, update(edited, status='archived', person_id=None), row['id'])
    ...
    assert [item['account']['status'] for item in history] == ['active', 'paused', 'archived']
...
@pytest.mark.parametrize('change', [
    {'platform': 'unsupported'}, {'handle': ''}, {'handle': '@@@'}, {'handle': 'two names'},
    {'handle': 'https://example.com'}, {'handle': 'a\x00b'}, {'handle': 'a\u202eb'},
    {'profile_url': 'javascript:alert(1)'}, {'profile_url': 'http://example.com'},
    {'profile_url': 'https://name:secret@example.com'}, {'profile_url': 'https://[bad'},
    {'profile_url': 'https://example.com:abc'}, {'profile_url': 'https://example.com:444'},
    {'profile_url': 'https://example.com/a b'}, {'credential_ref': 'secret token'},
    {'status': 'verified'}, {'person_id': ''}, {'home': '/outside'}, {'notes': 'x' * 10001},
])
...
def test_custom_https_is_only_metadata_and_home_isolation(tmp_path):
    ...
    row, _ = social.save(first, values(platform='mastodon', handle='@alice@example.social', profile_url='https://example.social/@alice', credential_ref='SOCIAL_CONNECTION'))
...
def test_native_registry_tools_and_approval_metadata(tmp_path):
    async def scenario():
        ...
        result = await provider.invoke('people_social_update', {'account_id': row['id'], 'account': update(row, status='archived')})
        ...
        result = await provider.invoke('people_social_remove', {'account_id': row['id'], 'revision': 1})
        assert not result.success
        result = await provider.invoke('people_social_remove', {'account_id': row['id'], 'revision': 2})
        assert json.loads(result.output)['external_account_changed'] is False
        result = await provider.invoke('people_social_history', {'account_id': row['id']})
    ...
    async def scenario():
        ...
        try:
            async with ClientSession() as client:
                ...
                async with client.put(path, json=update(row, status='archived')) as response:
                    assert response.status == 200
                    assert (await response.json())['account']['revision'] == 2
                async with client.put(path, json=update(row)) as response:
                    assert response.status == 409
                async with client.delete(path, json={'revision': 2, 'home': 'elsewhere'}) as response:
