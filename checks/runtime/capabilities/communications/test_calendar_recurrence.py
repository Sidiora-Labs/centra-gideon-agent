...
def source(store, timezone_name="America/New_York"):
    return calendar.save_source(
        store, {"name": "Recurring calendar", "kind": "ics", "timezone": timezone_name}
    )
...
def test_daily_recurrence_preserves_wall_time_across_dst_and_applies_exceptions():
    ...
    assert not any(
        "2026-03-09" in row["id"] or "2026-03-10" in row["id"] for row in events
    )
...
def test_weekly_monthly_and_yearly_rules_expand_deterministically():
    ...
    yearly = item(
        ...
        "SUMMARY:Annual",
        "RRULE:FREQ=YEARLY;COUNT=2;BYMONTH=1;BYMONTHDAY=15",
    )
    events, warnings = calendar.parse_ics(
        envelope(weekly, monthly, yearly),
        "UTC",
    ...
    assert [row["start"][:10] for row in monthly_rows] == [
        "2026-01-31",
        "2026-02-28",
        "2026-03-31",
    ]
...
def test_all_day_recurrence_uses_exclusive_end_and_date_exclusions():
    master = item(
        ...
        "EXDATE;VALUE=DATE:20260926",
        "RDATE;VALUE=DATE:20260930",
    )
...
def test_interval_until_and_unmatched_override_are_bounded_truthfully():
    master = item(
        ...
        "RRULE:FREQ=DAILY;INTERVAL=2;UNTIL=20260106T090000Z",
    )
    unmatched = item(
        "bounded",
        "DTSTART:20260110T110000Z",
        "DTEND:20260110T120000Z",
        "RECURRENCE-ID:20260110T090000Z",
        "SUMMARY:Outside rule",
    )
    events, warnings = calendar.parse_ics(
        envelope(master, unmatched),
        "UTC",
        "2026-01-01T00:00:00+00:00",
    ...
    assert warnings == [
        {"uid": "bounded", "unmatched_overrides": ["2026-01-10T09:00:00+00:00"]}
    ]
...
def test_invalid_or_nonstandard_rule_parts_fail_closed(rule, match):
    ...
    with pytest.raises(PeopleError, match=match):
        calendar.parse_ics(
            content, "UTC", "2026-01-01T00:00:00+00:00", "2026-02-01T00:00:00+00:00"
        )
...
def test_subdaily_frequencies_and_time_selectors_expand_in_the_window():
    ...
    events, _ = calendar.parse_ics(
        envelope(hourly, minute, second, selected),
        "UTC",
        "2026-01-01T00:00:00+00:00",
        "2026-01-03T00:00:00+00:00",
    )
    starts = {
        uid: [row["start"] for row in events if row["uid"] == uid]
...
def test_year_position_selectors_and_ordinal_weekdays_are_supported():
    ...
    events, _ = calendar.parse_ics(
        envelope(ordinal, year_day, week_no),
        "UTC",
        "2026-01-01T00:00:00+00:00",
        "2027-01-01T00:00:00+00:00",
    )
...
def test_subdaily_exceptions_and_expansion_cap_fail_closed():
    master = item(
        ...
        "EXDATE:20260101T010000Z",
    )
    moved = item(
        "shift",
        "DTSTART:20260101T023000Z",
        "DTEND:20260101T024000Z",
        "RECURRENCE-ID:20260101T020000Z",
        "SUMMARY:Moved shift",
    )
    events, _ = calendar.parse_ics(
        envelope(master, moved),
        "UTC",
        "2026-01-01T00:00:00+00:00",
        "2026-01-02T00:00:00+00:00",
    )
    ...
    with pytest.raises(PeopleError, match="exceeds 5000"):
        calendar.parse_ics(
            unbounded, "UTC", "2026-01-01T00:00:00+00:00", "2026-01-02T00:00:00+00:00"
        )
...
def test_embedded_vtimezone_expands_forward_and_backward_dst_transitions():
    spring = item(
        "spring-custom",
        "DTSTART;TZID=Custom/Eastern:20260307T090000",
        "DTEND;TZID=Custom/Eastern:20260307T100000",
        "RRULE:FREQ=DAILY;COUNT=3",
    )
    autumn = item(
        "autumn-custom",
        "DTSTART;TZID=Custom/Eastern:20261031T090000",
        "DTEND;TZID=Custom/Eastern:20261031T100000",
        "RRULE:FREQ=DAILY;COUNT=3",
    )
    content = envelope(custom_eastern_timezone(), spring, autumn)
    events, warnings = calendar.parse_ics(
        content, "UTC", "2026-03-01T00:00:00+00:00", "2027-03-01T00:00:00+00:00"
    )
    assert warnings == []
...
def test_thisandfuture_shifts_times_and_durations_with_exact_override_precedence():
    ...
    revived = item(
        "range-series",
        "DTSTART;TZID=America/New_York:20260312T070000",
        None,
        "DURATION:PT20M",
        "SUMMARY:Exact after cancellation",
        "RECURRENCE-ID;TZID=America/New_York:20260312T090000",
    )
    events, warnings = calendar.parse_ics(
        envelope(master, shifted, exact, cancelled, revived),
        "America/New_York",
        "2026-03-01T00:00:00+00:00",
        "2026-04-01T00:00:00+00:00",
    )
...
def test_invalid_vtimezone_and_range_value_fail_closed():
    ...
    master = item(
        "bad-range", "DTSTART:20260101T090000Z", None, "RRULE:FREQ=DAILY;COUNT=2"
    )
    override = item(
        "bad-range",
...
def test_upload_persists_canonical_occurrences_and_daily_window_coverage(tmp_path):
    ...
    sync = calendar.upload(
        store, row["id"], {"content": recurring_calendar(), "revision": 1, **window()}
    )
    ...
    assert (
        calendar.daily(store, "2026-03-12", "America/New_York")["events"][0]["uid"]
        == "standup"
    )
    ...
    with store.connect() as db:
        bodies = [
            json.loads(value)
            for value, in db.execute("SELECT body FROM calendar_events ORDER BY id")
        ]
        assert len(bodies) == 3
        assert len({value["id"] for value in bodies}) == 3


def test_replay_and_changed_exception_set_replace_occurrences_without_resurrection(
    tmp_path,
):
    store = PeopleStore(tmp_path)
    row = source(store)
    ...
    changed = recurring_calendar().replace(
        "EXDATE;TZID=America/New_York:20260309T090000\r\n", ""
    )
    second = calendar.upload(store, row["id"], {**payload, "content": changed})
    assert second["events"] == 4
    assert (
        calendar.daily(store, "2026-03-09", "America/New_York")["events"][0]["title"]
        == "Daily standup"
    )
...
def test_provider_instances_keep_provider_identity_and_cancelled_instances_stay_hidden(
    tmp_path,
):
    store = PeopleStore(tmp_path)
    row = calendar.save_source(
        store, {"name": "Provider", "kind": "google", "credential_ref": "TOKEN"}
    )
    raw = {
        "id": "provider-instance-1",
        "recurringEventId": "series-1",
        "originalStartTime": {"dateTime": "2026-03-08T09:00:00-04:00"},
    ...
    cancelled = calendar.provider_event(
        {**raw, "id": "provider-instance-2", "status": "cancelled"}, "google"
    )
    state = {
        "state": "synced",
...
@pytest.mark.asyncio
async def test_real_http_recurring_upload_and_daily_exception_journey(
    ...
    root = (
        f"http://127.0.0.1:{unused_tcp_port}/api/capabilities/communications/calendar"
    )
    try:
        async with ClientSession() as client:
            ...
            uploaded = await client.post(
                root + "/sources/" + row["id"] + "/upload",
                json={"content": recurring_calendar(), "revision": 1, **window()},
            )
            ...
            moved = await client.get(
                root + "/daily",
                params={"date": "2026-03-08", "timezone": "America/New_York"},
            )
            ...
            excluded = await client.get(
                root + "/daily",
                params={"date": "2026-03-09", "timezone": "America/New_York"},
            )
            assert (await excluded.json())["events"] == []
            invalid = await client.post(
                root + "/sources/" + row["id"] + "/upload",
                json={
                    "content": recurring_calendar(),
                    "revision": 1,
                    "window_start": "2026-01-01T00:00:00+00:00",
                    "window_end": "2027-01-03T00:00:00+00:00",
                },
            )
