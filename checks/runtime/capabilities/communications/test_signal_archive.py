...
from gideon.workspace.capabilities.communications import PeopleError, PeopleStore, signal_archive

KEY = '00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff'
FIXTURE = __file__.replace('test_signal_archive.py', 'fixtures/signal_sqlcipher4.sqlite')
...
def request(account='owned-signal', raw=None, key=KEY):
    return {'source_account_id': account,
            'content_base64': base64.b64encode(raw if raw is not None else encrypted()).decode(),
            'key': key}


def person(store, name='Signal Friend'):
    return store.save({'name': name, 'identities': [{'kind': 'phone', 'value': '+1 (202) 555-0123'}]})
...
def test_actual_sqlcipher4_archive_authenticates_decrypts_and_normalizes(tmp_path):
    ...
    assert row['attachments'] == [{'path': 'attachments/file-1', 'name': 'photo.jpg',
...
def test_commit_persists_only_normalized_history_and_is_exactly_retryable(tmp_path):
    ...
    with store.connect() as db:
        ...
        event = json.loads(db.execute('SELECT body FROM relationship_messages WHERE source="signal"').fetchone()[0])
...
def test_review_binding_and_absent_identity_prevent_unreviewed_linkage(tmp_path):
    ...
    data = {**request(), 'source_digest': preview['source_digest'], 'review_token': preview['review_token']}
...
def test_plaintext_buffer_is_zeroed_after_success_and_schema_failure(tmp_path, monkeypatch):
    store = PeopleStore(tmp_path)
    captured = []
...
@pytest.mark.asyncio
async def test_real_http_preview_commit_list_and_history(tmp_path, monkeypatch, unused_tcp_port):
    ...
    try:
        async with ClientSession() as client:
            ...
            payload = {**request(), 'source_digest': preview['source_digest'], 'review_token': preview['review_token']}
            ...
            history = await (await client.get(root + '/history?source_account_id=owned-signal')).json()
...
def test_attachment_metadata_is_bounded_and_malformed_json_is_safe():
    oversized = {'attachments': [{'path': 'p', 'fileName': 'f', 'contentType': 'x', 'size': 1}] * 150}
