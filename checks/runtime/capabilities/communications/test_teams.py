...
from gideon.workspace.capabilities.communications import PeopleError, PeopleStore, teams
...
def source(store, **changes):
    body = {
        ...
        "credential_ref": "MICROSOFT_GRAPH_WORK",
    }
    body.update(changes)
...
class GraphService:
    ...
    async def handle(self, request):
        ...
        if path.endswith("/me"):
            return web.json_response(
                {"id": "owner-1", "mail": "owner@example.com", "displayName": "Owner"}
            )
        if path.endswith("/me/chats") and not page:
            return web.json_response(
                {
                    "value": [{"id": "chat-a"}],
                    "@odata.nextLink": origin + "/v1.0/me/chats?page=2",
                }
            )
        ...
        if path.endswith("/me/joinedTeams"):
            return web.json_response(
                {"value": [{"id": "team-a", "displayName": "Engineering"}]}
            )
        if path.endswith("/teams/team-a/channels"):
            return web.json_response(
                {"value": [{"id": "channel-a", "displayName": "General"}]}
            )
        if path.endswith("/chats/chat-a/messages") and not page:
            return web.json_response(
                {
                    "value": [
                        self.message("same-id", "friend-1", "<p>Hello <b>owner</b></p>")
                    ],
        ...
        if path.endswith("/chats/chat-b/messages"):
            return web.json_response(
                {
                    "value": [
                        self.message("same-id", "owner-1", "Same ID, different chat")
                    ]
                }
            )
        if path.endswith("/teams/team-a/channels/channel-a/messages"):
            ...
            root["attachments"] = [
                {"id": "file-1", "name": "plan.pdf", "contentType": "reference"}
            ]
    ...
    def message(identifier, sender, body):
        return {
            "id": identifier,
            "etag": identifier + "-v1",
            "createdDateTime": stamp(5),
            "lastModifiedDateTime": stamp(4),
...
def test_source_validation_revision_and_secret_free_persistence(tmp_path):
    ...
    changed = teams.save_source(
        store,
        {
            **{key: row[key] for key in ("name", "owner_email", "credential_ref")},
            "name": "Renamed",
            "revision": 1,
        },
        row["id"],
...
@pytest.mark.asyncio
async def test_real_http_graph_sync_paginates_deduplicates_and_links_people(
    ...
    inbound = next(
        row
        for row in rows
        if row["message_id"] == "same-id" and row["conversation_id"] == "chat-a"
    )
    ...
    assert channel["attachments"] == [
        {"id": "file-1", "name": "plan.pdf", "content_type": "reference"}
    ]
    ...
    async with ClientSession() as session:
        ...
        def revised(identifier, sender, body):
            row = old_message(
                identifier,
                sender,
                "Edited in Teams" if identifier == "edited" else body,
            )
...
@pytest.mark.asyncio
async def test_verified_customer_identity_is_required_before_history(
    tmp_path, graph_server
):
    service, base_url = graph_server
    store = PeopleStore(tmp_path)
    ...
    async with ClientSession() as session:
        with pytest.raises(PeopleError, match="does not match") as error:
            await teams.sync(
                store, configured["id"], teams.GraphClient("token", base_url, session)
            )
    assert error.value.status == 409
    assert [path for path, _ in service.calls] == [
        "/v1.0/me?$select=id,mail,userPrincipalName,displayName"
    ]
...
@pytest.mark.asyncio
async def test_revocation_and_missing_credentials_are_durable_truthful_states(
    tmp_path, graph_server, monkeypatch
):
    service, base_url = graph_server
    store = PeopleStore(tmp_path)
    configured = source(store)
    ...
    async with ClientSession() as session:
        with pytest.raises(PeopleError) as revoked:
            await teams.sync(
                store, configured["id"], teams.GraphClient("expired", base_url, session)
            )
...
@pytest.mark.asyncio
async def test_continuation_cannot_leave_verified_graph_origin(
    ...
    async def first(request):
        ...
        if request.path.endswith("/me/chats"):
            return web.json_response(
                {"value": [], "@odata.nextLink": "https://attacker.invalid/stolen"}
            )
    ...
    try:
        ...
        configured = source(store)
        async with ClientSession() as session:
            client = teams.GraphClient(
                "token", f"http://127.0.0.1:{unused_tcp_port}/v1.0/", session
            )
...
@pytest.mark.asyncio
async def test_dashboard_source_messages_and_sync_failure_journey(
    ...
    try:
        async with ClientSession() as client:
            ...
            detail = await (
                await client.get(root + "/" + configured["id"] + "/messages")
            ).json()
...
def test_message_parser_rejects_invalid_timestamps_and_bounds_metadata(tmp_path):
    ...
    raw["attachments"] = [
        {"id": str(index), "name": "file", "contentType": "reference"}
        for index in range(150)
    ]
    row = teams._normalize(
        raw,
        source_kind="chat",
        conversation_id="chat-a",
        owner_id="owner-1",
