...
def setup(tmp_path):
    ...
    person = store.save(
        {"name": "Friend", "cadence_days": 3, "notes": "Preserve personal context"}
    )
    return store, person
...
@pytest.mark.parametrize(
    "incoming,outgoing", [(False, True), (True, False), (False, False)]
)
def test_partial_coverage_never_claims_unanswered(tmp_path, incoming, outgoing):
    store, person = setup(tmp_path)
    ingest(
        store,
        batch(person["id"], incoming_complete=incoming, outgoing_complete=outgoing),
    )
...
def test_stale_future_and_outside_window_evidence_stays_unknown(tmp_path):
    ...
    assert (
        report(store, now=NOW + timedelta(hours=24))["threads"][0]["state"]
        == "unanswered"
    )
...
def test_message_identity_and_stale_coverage_conflicts_are_atomic(tmp_path):
    ...
    mixed = batch(
        ...
        coverage_end=later.isoformat(),
        messages=[
            message(person["id"], external_id="new-id"),
            message(person["id"], summary="Changed source content"),
        ],
    )
...
def test_sources_and_people_keep_thread_identity_separate(tmp_path):
    ...
    ingest(
        ...
        batch(
            ...
            messages=[message(second["id"], direction="outbound")],
        ),
    )
    ingest(
        store,
        batch(
            first["id"],
            source="other-archive",
            messages=[message(first["id"], thread_id="different")],
        ),
    )
    threads = report(store, now=NOW)["threads"]
    assert len(threads) == 3
    by_key = {
        (t["source"], t["source_account_id"], t["person_id"], t["thread_id"]): t
        for t in threads
    }
    assert (
        by_key[("archive", "source-one", first["id"], "thread-one")]["state"]
        == "unanswered"
    )
    assert (
        by_key[("archive", "source-two", second["id"], "thread-one")]["state"]
        == "answered"
    )
    assert (
        by_key[("other-archive", "source-one", first["id"], "different")]["state"]
        == "unanswered"
    )
...
@pytest.mark.parametrize(
    ...
    [
        {"source": ""},
        {"source_account_id": ""},
        {"source_account_id": "x" * 101},
        {"incoming_complete": 1},
        {"outgoing_complete": None},
        {"messages": None},
        {"messages": [None]},
        {"captured_at": "today"},
        {"coverage_start": "2026-09-26T00:00:00Z"},
        {"coverage_end": "2026-09-26T00:00:00Z"},
        {"home": "/tmp"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"external_id": ""},
        {"person_id": ""},
        {"occurred_at": "2026-09-25T00:00:00"},
...
def test_empty_projection_timezone_and_concurrent_claims(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        outcomes = list(
            pool.map(
                lambda _: ingest(PeopleStore(tmp_path), batch(person["id"])), range(3)
            )
        )
