...
def draft_data(account_id, request_key="privacy-shared"):
    return {
        ...
        "body": "Remove the exact listed profile.",
    }


def test_central_route_and_native_provider_share_canonical_outbound_records(
    ...
    async def exercise():
        ...
        result = await provider.invoke(
            "outbound_email_draft", draft_data(account["id"], "native-privacy")
        )
        assert result.success is True
        native = json.loads(result.output)["draft"]
        listing = json.loads((await provider.invoke("outbound_email_list", {})).output)[
            "drafts"
        ]
        assert native["sender"] == account["owner_email"] and {
            row["id"] for row in listing
        } == {created["id"], native["id"]}
