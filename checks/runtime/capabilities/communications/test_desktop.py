...
def snapshot(source="imessage", messages=None, group=False):
    ...
    messages = (
        ...
        else [("message-1", "Imported desktop hello", "incoming", 1700000000000)]
    )
    if source == "imessage":
        db.executescript(
            "CREATE TABLE message (guid TEXT,date INTEGER,text TEXT,is_from_me INTEGER,handle_id INTEGER); CREATE TABLE handle (id TEXT); CREATE TABLE chat (guid TEXT); CREATE TABLE chat_message_join (message_id INTEGER,chat_id INTEGER);"
        )
        ...
        for identifier, body, kind, when in messages:
            ...
            db.execute(
                ...
                (identifier, apple, body, int(kind == "outgoing")),
            )
            rowid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
            db.execute("INSERT INTO chat_message_join VALUES (?,1)", (rowid,))
    else:
        db.executescript(
            "CREATE TABLE messages (id TEXT,conversationId TEXT,sent_at INTEGER,received_at INTEGER,type TEXT,body TEXT,json TEXT); CREATE TABLE conversations (id TEXT,e164 TEXT,type TEXT,json TEXT);"
        )
        db.execute(
            "INSERT INTO conversations VALUES (?,?,?,?)",
            ("desktop-thread", "+12025550123", "group" if group else "private", "{}"),
        )
        for identifier, body, kind, when in messages:
            db.execute(
                "INSERT INTO messages VALUES (?,?,?,?,?,?,?)",
                (identifier, "desktop-thread", when, when, kind, body, "{}"),
            )
    db.commit()
    raw = db.serialize()
    db.close()
...
def payload(source="imessage", raw=None, account="desktop-account"):
    return {
        ...
        "content_base64": base64.b64encode(raw or snapshot(source)).decode(),
    }
...
def person(store):
    return store.save(
        {
            "name": "Desktop Friend",
            "identities": [{"kind": "phone", "value": "+1 (202) 555-0123"}],
        }
    )
...
def test_real_sqlite_preview_preserves_source_and_matches_people(tmp_path, source):
    ...
    assert (
        ...
        == hashlib.sha256(base64.b64decode(data["content_base64"])).hexdigest()
    )
...
def test_atomic_commit_original_blob_restart_and_exact_retry(tmp_path, source):
    ...
    with store.connect() as db:
        ...
        assert (
            db.execute("SELECT COUNT(*) FROM relationship_messages").fetchone()[0] == 1
        )
...
def test_unmatched_history_is_preserved_without_invented_people(tmp_path):
    ...
    assert report(store)["threads"] == []
    assert (
        desktop.history(store, "signal", "desktop-account")[0]["body"]
        == "Imported desktop hello"
    )
...
def test_changed_source_message_conflict_is_atomic(tmp_path):
    ...
    raw = snapshot(
        "signal",
        [
            ("message-1", "Conflicting text", "incoming", 1700000000000),
            ("message-2", "New message", "outgoing", 1700000001000),
        ],
    )
...
def test_incremental_snapshot_has_stable_message_ids(tmp_path):
    ...
    raw = snapshot(
        "signal",
        [
            ("message-1", "Imported desktop hello", "incoming", 1700000000000),
            ("message-2", "Actual reply", "outgoing", 1700000001000),
        ],
    )
...
def test_message_tombstone_removes_evidence_and_survives_restart_replay(tmp_path):
    ...
    person(store)
    desktop.commit(store, reviewed(store, payload("signal")))
    exclusion, removed, created = desktop.exclude(
        store,
        {
            "source": "signal",
            "source_account_id": "desktop-account",
    ...
    receipt, _ = desktop.commit(
        PeopleStore(tmp_path), reviewed(store, payload("signal", replay))
    )
    assert receipt["excluded"] == 1
    assert receipt["inserted"] == 1
    assert [
        row["external_id"]
        for row in desktop.history(store, "signal", "desktop-account")
    ] == ["message-2"]
    assert desktop.exclusions(PeopleStore(tmp_path), "signal", "desktop-account") == [
        exclusion
    ]
...
def test_identity_block_removes_all_matching_history_and_is_source_scoped(tmp_path):
    ...
    raw = snapshot(
        "imessage",
        [
            ("one", "First", "incoming", 1700000000000),
...
@pytest.mark.parametrize(
    "raw", [b"encrypted SQLCipher bytes", b"", b"SQLite format 3\x00" + b"bad" * 500]
)
...
def test_unexpected_and_view_schemas_rejected(tmp_path):
    ...
    db = sqlite3.connect(":memory:")
    db.executescript(
        "CREATE TABLE unrelated (id TEXT); CREATE VIEW messages AS SELECT id FROM unrelated;"
    )
    raw = db.serialize()
    db.close()
...
def test_timestamp_versions_and_invalid_values():
    ...
    assert (
        desktop.timestamp(721692800000000000, "imessage") == "2023-11-14T22:13:20+00:00"
    )
...
def test_native_snapshot_tools_real_home(tmp_path):
    ...
    async def scenario():
        ...
        result = await provider.invoke(
            ...
            {"source": "signal", "source_account_id": "desktop-account"},
        )
        ...
        result = await provider.invoke(
            "people_desktop_preview", {**data, "root": "/outside"}
        )
    ...
    async def scenario():
        ...
        try:
            async with ClientSession() as client:
                ...
                async with client.post(
                    base + "/commit", json={**request, "source_digest": "wrong"}
                ) as response:
                    assert response.status == 409
                async with client.post(
                    base + "/preview", json={**data, "home": "/outside"}
                ) as response:
...
def test_repacked_snapshot_rowids_do_not_change_message_identity(tmp_path):
    ...
    reordered = snapshot(
        "signal",
        [
            ("new-first", "New row before original", "incoming", 1700000001000),
            ("message-1", "Imported desktop hello", "incoming", 1700000000000),
        ],
    )
    receipt, created = desktop.commit(
        store, reviewed(store, payload("signal", reordered))
    )
    assert created is True
    assert receipt["inserted"] == 1
    assert receipt["linked"] == 1
