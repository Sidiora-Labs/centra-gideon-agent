...
from gideon.core.config.loader import AppConfig, AgentProfile
...
from gideon.workspace.capabilities.communications import social, timeline, calendar, evidence, lifecycle
...
def point(store, row, identity='point1', when='2026-09-25T09:00:00Z', summary='Coffee discussion'):
    return store.record(row['id'], {'source': 'manual', 'external_id': identity, 'occurred_at': when, 'direction': 'mutual', 'summary': summary})[0]
...
def test_canonical_contact_and_message_projection_without_duplicate_mirror_rows(tmp_path):
    store = PeopleStore(tmp_path)
    row = person(store)
    manual = point(store, row)
    batch = {'source': 'imap', 'source_account_id': 'personal', 'captured_at': '2026-09-25T12:00:00Z', 'coverage_start': '2026-09-25T00:00:00Z', 'coverage_end': '2026-09-25T12:00:00Z', 'incoming_complete': False, 'outgoing_complete': False, 'messages': [{'person_id': row['id'], 'thread_id': 'thread1', 'external_id': 'message1', 'occurred_at': '2026-09-25T10:00:00Z', 'direction': 'inbound', 'summary': 'Mail reply'}]}
...
def test_local_day_boundaries_and_dst_intervals(tmp_path):
    ...
    assert [event['summary'] for event in result['events']] == ['Before next midnight', 'Local midnight']
...
def test_stable_seek_pagination_and_query_bound_cursor(tmp_path):
    ...
    assert read(store, limit=2, cursor=first['next_cursor'])['events'] == second['events']
...
def test_registry_and_assignment_history_project_only_safe_summary_fields(tmp_path):
    with runtime_home(tmp_path):
        ...
        store = PeopleStore()
        row = person(store)
        account, _ = social.save(store, {'platform': 'x', 'handle': 'alice', 'request_key': 'account', 'person_id': row['id'], 'credential_ref': 'SECRET_REFERENCE_NOT_FOR_TIMELINE', 'notes': 'Private account note'})
        assignment = lifecycle.create(store, {'account_id': account['id'], 'account_revision': 1, 'agent_id': 'research', 'reason': 'Private operational reason', 'request_key': 'assignment'})
        ...
        assignment_event = next(event for event in result['events'] if event['kind'] == 'assignment')
        assert 'platform_assignment=' + assignment['id'] in assignment_event['href']


@pytest.mark.parametrize('extra', [{'date': 'bad'}, {'date': '9999-12-31'}, {'timezone': 'Bad/Zone'}, {'limit': 0}, {'limit': 101}, {'limit': True}, {'kind': 'imagined'}, {'cursor': 'not-base64'}, {'home': '/another'}, {'person_id': 'missing'}])
...
def test_native_timeline_reads_current_home_without_mutating_sources(tmp_path):
    with runtime_home(tmp_path / 'one'):
        ...
        async def scenario():
            ...
            result = await provider.invoke('people_activity_timeline', {'date': '2026-09-25', 'timezone': 'UTC'})
        ...
        async def scenario():
            ...
            try:
                async with ClientSession() as client:
                    async with client.get(base, params={'date': '2026-09-25', 'person_id': row['id'], 'limit': '1'}) as response:
                        assert response.status == 200
                        result = await response.json()
                        assert result['events'][0]['person_id'] == row['id']
                    async with client.get(base, params={'date': '2026-09-25', 'limit': 'abc'}) as response:
                        assert response.status == 400
                    async with client.get(base, params={'date': '9999-12-31'}) as response:
                        assert response.status == 400
                    async with client.get(base, params={'date': '2026-09-25', 'home': '/other'}) as response:
                        assert response.status == 400
            finally:
