...
def configure(store):
    return beeper.configure(
        store,
        {
            "base_url": "http://127.0.0.1:23373",
            "credential_ref": "GIDEON_ABSENT_BEEPER_TEST_9BC2",
            "revision": beeper.settings(store)["revision"],
        },
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "http://example.com",
        "file:///etc/passwd",
        "http://user:password@localhost:23373",
        "http://localhost:23373/path",
        "http://localhost:23373?key=value",
        "http://localhost:23373#fragment",
        "http://localhost:99999",
        "http://localhost:abc",
        "http://127.0.0.2:23373",
    ],
)
...
@pytest.mark.parametrize(
    "url", ["http://localhost:23373", "http://127.0.0.1:23373/", "http://[::1]:23373"]
)
...
def test_captured_page_normalization_and_persistence(tmp_path):
    ...
    message_page = beeper.persist_page(
        store, messages(), "chat-one", "imported_capture"
    )
...
def test_chat_scope_and_attachment_shape_validation(tmp_path):
    ...
    with pytest.raises(PeopleError, match="2 MiB"):
        beeper.persist_page(
            store, {"items": [{"id": "large", "text": "x" * (2 * 1024 * 1024)}]}
        )
...
def test_connection_change_invalidates_cached_pages_and_stale_fetch(tmp_path):
    ...
    beeper.persist_page(
        store, messages(), "chat-one", expected_revision=configured["revision"]
    )
    queued = draft(store)
    configure(store)
    ...
    with pytest.raises(PeopleError) as error:
        asyncio.run(
            beeper.send(store, queued["id"], {"revision": 1, "confirm_send": True})
        )
    assert error.value.status == 409
...
def test_disconnect_clears_provider_cache_and_invalidates_preserved_drafts(tmp_path):
    ...
    with store.connect() as db, db:
        beeper.schema(db)
        db.execute(
            "INSERT INTO beeper_assets VALUES (?,?,?)",
            ("mxc://example.com/asset-one", b"cached bytes", "digest"),
        )
    queued = draft(store)
    ...
    with pytest.raises(PeopleError) as error:
        asyncio.run(
            beeper.send(
                store,
                queued["id"],
                {"revision": queued["revision"], "confirm_send": True},
            )
        )
    assert error.value.status == 409
    with pytest.raises(PeopleError) as error:
...
def test_draft_persists_idempotently_without_external_effect(tmp_path):
    ...
    retry, created = beeper.draft(
        store,
        {
            "request_key": "request-one",
            "chat_id": "chat-one",
            "text": "A reviewed text",
        },
    )
    assert retry == row
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {"request_key": "key", "chat_id": "", "text": "x"},
        {"request_key": "key", "chat_id": "chat", "text": " "},
        {"request_key": "key", "chat_id": "chat", "text": "x", "send": True},
    ],
)
...
def test_interrupted_send_recovery_never_creates_retry(tmp_path):
    ...
    claimed = beeper.transition(
        store, row["id"], 1, {"draft"}, "sending", delivery="unknown", started_at=old
    )
    ...
    assert (
        beeper.discard(store, row["id"], recovered["revision"])["state"] == "discarded"
    )
...
def test_real_client_unavailable_secret_does_not_claim_send(tmp_path):
    ...
    with pytest.raises(PeopleError, match="confirmation"):
        asyncio.run(
            beeper.send(store, row["id"], {"revision": 1, "confirm_send": False})
        )
    with pytest.raises(PeopleError) as error:
        asyncio.run(
            beeper.send(store, row["id"], {"revision": 1, "confirm_send": True})
        )
    assert error.value.status == 503
...
def test_attachment_provenance_and_unavailable_connection(tmp_path):
    ...
    with pytest.raises(PeopleError) as error:
        asyncio.run(
            beeper.fetch_asset(store, "other-chat", "mxc://example.com/asset-one")
        )
    assert error.value.status == 404
    with pytest.raises(PeopleError) as error:
        asyncio.run(
            beeper.fetch_asset(store, "chat-one", "mxc://example.com/asset-one")
        )
    assert error.value.status == 503
    with pytest.raises(PeopleError) as error:
        beeper.asset(store, "mxc://example.com/asset-one")
...
def test_native_beeper_operations_and_send_policy(tmp_path):
    ...
    async def scenario():
        ...
        assert (
            json.loads(result.output)["settings"]["credential_ref"]
            == config["credential_ref"]
        )
        result = await provider.invoke(
            "people_beeper_draft",
            {
                "request_key": "native-one",
                "chat_id": "native-chat",
                "text": "Native reviewed text",
            },
        )
        assert result.success, result.error
        ...
        result = await provider.invoke(
            "people_beeper_send",
            {"outbox_id": row["id"], "revision": 1, "confirm_send": True},
        )
        ...
        result = await provider.invoke(
            "people_beeper_discard", {"outbox_id": row["id"], "revision": 1}
        )
        assert json.loads(result.output)["item"]["state"] == "discarded"
        result = await provider.invoke(
            "people_beeper_draft",
            {"request_key": "x", "chat_id": "x", "text": "x", "home": "/outside"},
        )
        assert not result.success
    ...
    async def scenario():
        ...
        try:
            async with ClientSession() as client:
                ...
                data = {
                    "request_key": "http-one",
                    "chat_id": "chat-one",
                    "text": "HTTP queued text",
                }
                ...
                async with client.post(
                    path + "/send", json={"revision": 1, "confirm_send": True}
                ) as response:
                ...
                async with client.get(
                    base + "/messages", params={"chat_id": "chat-one"}
                ) as response:
                    assert (await response.json())["items"] == []
                async with client.get(
                    base + "/assets", params={"id": "missing"}
                ) as response:
                    assert response.status == 404
                async with client.post(
                    path + "/discard", json={"revision": 1}
                ) as response:
                ...
                async with client.post(
                    base + "/disconnect", json={"revision": 1}
                ) as response:
                    assert response.status == 200
...
def test_pending_reconciliation_cannot_switch_connections(tmp_path):
    ...
    configure(store)
    row = draft(store)
    pending = beeper.transition(
        store,
        row["id"],
        1,
        {"draft"},
        "pending",
        pending_message_id="recorded-pending-id",
