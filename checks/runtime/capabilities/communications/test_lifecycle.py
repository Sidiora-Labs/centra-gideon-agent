...
from gideon.workspace.capabilities.communications import social, lifecycle
...
def configured():
    cfg = AppConfig.load()
    cfg.agents = {'research': AgentProfile(provider='native'), 'writer': AgentProfile(provider='native')}
    cfg.save()
    return cfg


def registration(store):
    return social.save(store, {'platform': 'x', 'handle': 'alice', 'request_key': 'alice'})[0]


def request(row, **extra):
    return {'account_id': row['id'], 'account_revision': row['revision'], 'agent_id': 'research', 'reason': 'Research identity assignment', 'request_key': 'assign1', **extra}


def change(store, row, state, **extra):
    return lifecycle.change(store, row['id'], {'revision': row['revision'], 'account_revision': row['account_revision'], 'state': state, 'reason': 'Owner reviewed ' + state, **extra})
...
def test_real_configured_agents_and_durable_lifecycle(tmp_path):
    with runtime_home(tmp_path):
        ...
        assert [item['state'] for item in history] == ['requested', 'active', 'paused', 'active', 'revoked']
...
def test_single_active_owner_transfer_requires_pause_and_current_revisions(tmp_path):
    with runtime_home(tmp_path):
        ...
        second = lifecycle.create(store, request(account, agent_id='writer', request_key='assign2'))
        with pytest.raises(PeopleError) as error:
...
def test_current_agent_removal_and_account_change_invalidate_readiness(tmp_path):
    with runtime_home(tmp_path):
        ...
        updated, _ = social.save(store, {**values, 'credential_ref': 'ROTATED_CONNECTION', 'revision': 1}, account['id'])
...
@pytest.mark.parametrize('extra', [{'agent_id': 'unknown'}, {'account_id': 'missing'}, {'account_revision': True}, {'account_revision': 9}, {'reason': ''}, {'root': '/another'}])
...
def test_native_assignment_tools_use_real_agent_config(tmp_path):
    with runtime_home(tmp_path):
        ...
        async def scenario():
            ...
            assert 'research' in {agent['id'] for agent in json.loads(result.output)['agents']}
            result = await provider.invoke('people_platform_assignment_create', request(account))
            assert result.success, result.error
            row = json.loads(result.output)['assignment']
            result = await provider.invoke('people_platform_assignment_change', {'assignment_id': row['id'], 'revision': 1, 'account_revision': 1, 'state': 'active', 'reason': 'Native owner review'})
            assert json.loads(result.output)['assignment']['state'] == 'active'
            result = await provider.invoke('people_platform_assignment_get', {'assignment_id': row['id']})
            ...
            result = await provider.invoke('people_platform_assignment_history', {'assignment_id': row['id']})
            ...
            assert definitions['people_platform_assignment_get'].requires_approval is False
