...
from gideon.interfaces.dashboard.handlers.capabilities_creative_commissions import (
    COMMISSIONS,
    register,
)
from gideon.workspace.capabilities.creative.commission_dispatch import (
    CommissionDispatcher,
    normalize_dispatch,
)
from gideon.workspace.capabilities.creative.commission_tools import (
    SCHEMAS,
    CommissionTools,
)
...
def payload(work, request="commission", cadence=None, attempts=2):
    return {
        ...
        "brief": {
            "intent": "Find a new visual direction without changing the manuscript.",
            "genre": "mystery",
            "category": "treatment",
            "style": "Quiet and precise.",
...
def make_store(tmp_path):
    ...
    return (
        CommissionStore(tmp_path, direction=direction, triggers=triggers),
        direction,
        triggers,
    )
...
def output_ref(run):
    ...
    return {
        key: output[key] for key in ("artifact_id", "artifact_version", "content_hash")
    }
...
def test_create_persists_typed_brief_and_real_trigger(tmp_path):
    ...
    assert commission["cadence"]["spec"] == {
        "kind": "interval",
        "interval_secs": 900,
        "timezone": "UTC",
    }
...
def test_generation_requires_typed_config_and_planning_rejects_dispatch(tmp_path):
    ...
    blank.update(
        target_ability="image", mode="generate", dispatch={"input": {"prompt": ""}}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "video",
            {"input": {"prompt": "x", "duration_seconds": 0}},
            "between 1 and 60",
        ),
        (
            "video",
            {"input": {"prompt": "x", "duration_seconds": 5, "controls": []}},
...
def test_generation_config_matches_canonical_ability_bounds(
    ability, dispatch, message, tmp_path
):
    _, work, _ = source(tmp_path)
    store, _, _ = make_store(tmp_path)
...
def test_update_requires_coherent_mode_transition(tmp_path):
    ...
    with pytest.raises(CatalogError, match="does not accept"):
        store.update(
            commission["id"], {"revision": generated["revision"], "mode": "planning"}
        )


async def test_legacy_record_with_dispatch_is_planning_only_and_never_admits_media(
    tmp_path,
):
    _, work, _ = source(tmp_path)
    ...
    with store.db() as db:
        record = json.loads(
            db.execute(
                "SELECT record FROM creative_commissions WHERE id=?",
                (commission["id"],),
            ).fetchone()[0]
        )
        record.pop("mode")
        record["dispatch"] = {
            "input": {"prompt": "Legacy optional configuration must remain inert."}
        }
        db.execute(
            "UPDATE creative_commissions SET record=? WHERE id=?",
            (json.dumps(record), commission["id"]),
        )
...
async def test_live_claim_blocks_duplicate_attempt_and_stale_direction_claim_requires_retry(
    tmp_path,
):
    _, work, _ = source(tmp_path)
    store, direction, _ = make_store(tmp_path)
    ...
    running = {
        ...
        "claim_id": "live",
        "claim_pid": os.getpid(),
        "claim_identity": process_identity(os.getpid()),
        "claimed_at": stamp,
        "phase": "direction",
        "created_at": stamp,
        "updated_at": stamp,
    ...
    with store.db() as db:
        db.execute(
            "INSERT INTO creative_commission_runs VALUES(?,?,?,?)",
            (run_id, commission["id"], "manual:claimed", json.dumps(running)),
        )
    concurrent = await store.execute(
        commission["id"], "manual:claimed", trigger="manual"
    )
    ...
    with store.db() as db:
        current = json.loads(
            db.execute(
                "SELECT record FROM creative_commission_runs WHERE id=?", (run_id,)
            ).fetchone()[0]
        )
    ...
    interrupted = await store.execute(
        commission["id"], "manual:claimed", trigger="manual"
    )
...
async def test_stale_dispatch_claim_is_quarantined_without_blind_replay(tmp_path):
    ...
    running = {
        ...
        "claim_id": "dead",
        "claim_pid": 999999999,
        "claim_identity": "dead:claim",
        "claimed_at": stamp,
        "phase": "dispatching",
        "dispatch_request_id": "commission-" + run_id[:48] + "-1",
        "created_at": stamp,
        "updated_at": stamp,
    }
    with store.db() as db:
    ...
    uncertain = await store.execute(
        commission["id"], "manual:uncertain", trigger="manual"
    )
...
@pytest.mark.parametrize(
    "ability", ["video", "image", "music", "music-video", "series"]
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"kind": "daily", "at": "09:30", "timezone": "UTC", "weekdays_only": True},
            "cron",
        ),
...
def test_supported_cadences_map_to_shared_clock_specs(cadence, kind, tmp_path):
    ...
    commission = store.create(
        payload(work, request="cadence-" + cadence["kind"], cadence=cadence)
    )
...
async def test_real_scheduler_due_fire_executes_exactly_one_linked_project(tmp_path):
    ...
    result = await provider.execute(
        config, ActionContext(event="trigger.fired", payload=fire)
    )
...
async def test_disable_before_stale_tick_prevents_dispatch_and_removes_trigger(
    tmp_path,
):
    _, work, _ = source(tmp_path)
    store, direction, triggers = make_store(tmp_path)
    commission = store.create(payload(work))
    config = triggers.get(TRIGGER_PREFIX + commission["id"]).trigger.workflow["inline"][
        "config"
    ]
    ...
    result = await CommissionActionProvider(store).execute(
        config, ActionContext(event="trigger.fired", payload={"scheduled_for": 1200})
    )
    assert result.success
...
async def test_changed_cadence_rejects_stale_armed_action(tmp_path):
    ...
    store, direction, triggers = make_store(tmp_path)
    commission = store.create(payload(work))
    old = triggers.get(TRIGGER_PREFIX + commission["id"]).trigger.workflow["inline"][
        "config"
    ]
    changed = store.update(
        commission["id"],
        {"revision": 1, "cadence": {"kind": "interval", "seconds": 1800}},
    )
    assert changed["schedule_revision"] == 2
    result = await CommissionActionProvider(store).execute(
        old, ActionContext(event="trigger.fired", payload={"scheduled_for": 1200})
    )
    assert result.success
    ...
    assert failed["attempts"] == [
        {
            "number": 1,
            "status": "failed",
            "started_at": failed["attempts"][0]["started_at"],
            "finished_at": failed["attempts"][0]["finished_at"],
            "error": "execution-failed:CatalogError",
        }
    ]
    assert TEXT not in failed["attempts"][0]["error"]
    restored = works.artifacts.create(
        name="Commission source draft",
        slug=draft["artifact_id"],
        kind="markdown",
        content=TEXT,
        description="restored exact source",
        readonly=True,
    )
...
async def test_retry_budget_exhaustion_preserves_each_failed_attempt(tmp_path):
    ...
    assert all(
        row["error"] == "execution-failed:CatalogError" for row in second["attempts"]
    )
...
async def test_restart_preserves_schedule_run_history_output_and_project(tmp_path):
    ...
    assert (
        persisted["outputs"][0]["content_hash"]
        == hashlib.sha256(
            direction.artifacts.get(
                persisted["outputs"][0]["artifact_id"], version=1
            ).content.encode()
        ).hexdigest()
    )
...
async def test_feedback_is_output_linked_author_isolated_revisioned_and_tombstoned(
    tmp_path,
):
    _, work, _ = source(tmp_path)
    store, _, _ = make_store(tmp_path)
    ...
    assert (
        next(
            row
            for row in store.feedback(commission["id"])["items"]
            if row["id"] == two["id"]
        )["rating"]
        == "disliked"
    )
    removed = store.remove_reaction(
        commission["id"], updated["id"], {"revision": 2, "author": "alice"}
    )
...
async def test_next_run_consumes_only_live_bounded_feedback(tmp_path):
    ...
    reaction = store.react(
        commission["id"],
        {
            "run_id": first["id"],
            "author": "alice",
            "output": output_ref(first),
            "rating": "liked",
            "note": "Use the quiet rhythm.",
            "tags": ["rhythm"],
        },
    )
    ...
    store.remove_reaction(
        commission["id"], reaction["id"], {"revision": 1, "author": "alice"}
    )
...
async def test_feedback_rejects_foreign_output_author_and_stale_revision(tmp_path):
    ...
    with pytest.raises(CatalogError) as wrong_author:
        store.remove_reaction(
            commission["id"], row["id"], {"revision": 1, "author": "bob"}
        )
...
async def test_http_create_manual_run_feedback_disable_and_restart(tmp_path):
    ...
    triggers = TriggerStore(base_dir=tmp_path)
    app = web.Application()
    register(app, tmp_path, direction=direction, triggers=triggers)
    async with TestClient(TestServer(app)) as client:
        response = await client.post(
            "/api/capabilities/creative/commissions", json=payload(work, request="http")
        )
        assert response.status == 200
        commission = await response.json()
        response = await client.post(
            f'/api/capabilities/creative/commissions/{commission["id"]}/run',
            json={"request_id": "http-run"},
        )
        assert response.status == 200
        run = await response.json()
        assert run["status"] == "planned"
        response = await client.post(
            f'/api/capabilities/creative/commissions/{commission["id"]}/feedback',
            json={
                "run_id": run["id"],
                "author": "http-owner",
                "output": output_ref(run),
        ...
        detail = await (
            await client.get(
                f'/api/capabilities/creative/commissions/{commission["id"]}'
            )
        ).json()
...
async def test_native_tools_use_same_store_and_closed_top_level_schemas(tmp_path):
    ...
    created = await provider.invoke(
        "creative_commission_create", {"payload": payload(work, request="native")}
    )
    assert created.success
    commission = json.loads(created.output)
    run_result = await provider.invoke(
        "creative_commission_run", {"id": commission["id"], "request_id": "native-run"}
    )
    assert run_result.success
    run = json.loads(run_result.output)
    reaction = await provider.invoke(
    ...
    assert (
        detail["runs"][0]["outputs"][0]["artifact_id"]
        == run["outputs"][0]["artifact_id"]
    )
...
async def test_action_provider_rejects_missing_occurrence_and_unknown_commission(
    tmp_path,
):
    store, _, _ = make_store(tmp_path)
    provider = CommissionActionProvider(store)
    missing = await provider.execute(
        {"commission_id": "missing", "schedule_revision": 1, "cadence_hash": "0" * 64},
        ActionContext(event="trigger.fired", payload={"scheduled_for": 1000}),
    )
    assert not missing.success
    assert "not found" in missing.error
    absent_occurrence = await provider.execute(
        {"commission_id": "missing", "schedule_revision": 1, "cadence_hash": "0" * 64},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        },
        {
            ...
            "style": "",
            "constraints": {},
            "seed_refs": list(map(str, range(51))),
        },
    ],
)
...
def test_update_is_optimistic_and_brief_edit_does_not_change_schedule_identity(
    tmp_path,
):
    _, work, _ = source(tmp_path)
    store, _, triggers = make_store(tmp_path)
    commission = store.create(payload(work))
    before = triggers.get(TRIGGER_PREFIX + commission["id"]).trigger.workflow["inline"][
        "config"
    ]
    ...
    after = triggers.get(TRIGGER_PREFIX + commission["id"]).trigger.workflow["inline"][
        "config"
    ]
    assert updated["revision"] == 2
...
def test_trigger_write_failure_is_persisted_without_claiming_schedule_success(tmp_path):
    ...
    reopened = CommissionStore(
        tmp_path, direction=DirectionStore(tmp_path), triggers=triggers
    )
    assert reopened.get(commission["id"])["schedule_error"] == updated["schedule_error"]


async def test_manual_occurrence_idempotency_prevents_duplicate_attempt_and_project(
    tmp_path,
):
    _, work, _ = source(tmp_path)
    store, direction, _ = make_store(tmp_path)
...
async def test_output_reference_resolves_exact_readonly_bytes(tmp_path):
    ...
    output = run["outputs"][0]
    artifact = direction.artifacts.get(
...
async def test_feedback_rejects_unknown_run_invalid_rating_and_excess_tags(tmp_path):
    ...
    store, _, _ = make_store(tmp_path)
    commission = store.create(payload(work))
    run = await store.execute(
        commission["id"], "manual:feedback-errors", trigger="manual"
    )
    output = output_ref(run)
    with pytest.raises(CatalogError) as missing:
        store.react(
    ...
    with pytest.raises(CatalogError, match="tags"):
        store.react(
            commission["id"],
            {
                ...
                "rating": "liked",
                "note": "",
                "tags": [str(index) for index in range(21)],
            },
        )
...
async def test_removed_reaction_can_be_reactivated_with_lineage(tmp_path):
    ...
    removed = store.remove_reaction(
        commission["id"], first["id"], {"revision": 1, "author": "alice"}
    )
    active = store.react(
        commission["id"],
        {
...
async def test_http_rejects_query_fields_and_foreign_feedback_output(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        assert (
            await client.get("/api/capabilities/creative/commissions?home=/tmp/other")
        ).status == 400
        commission = await (
            await client.post(
                "/api/capabilities/creative/commissions",
                json=payload(work, request="http-guards"),
            )
        ).json()
        run = await (
            await client.post(
                f'/api/capabilities/creative/commissions/{commission["id"]}/run',
                json={"request_id": "guard-run"},
            )
        ).json()
...
def test_daily_recurrence_preserves_local_wall_time_across_dst():
    ...
    assert (
        datetime.fromtimestamp(second, timezone.utc).isoformat()
        == "2026-03-08T13:00:00+00:00"
    )
    assert (
        datetime.fromtimestamp(third, timezone.utc).isoformat()
...
def test_monthly_last_weekday_and_exclusions_use_dateutil_calendar_semantics():
    cadence = recurrence(
        rule="FREQ=MONTHLY;COUNT=4;BYDAY=MO,TU,WE,TH,FR;BYSETPOS=-1",
...
async def test_real_recurrence_due_fire_rearms_next_occurrence_after_dispatch(tmp_path):
    ...
    store, direction, triggers = make_store(tmp_path)
    commission = store.create(
        payload(work, cadence=recurrence(rule="FREQ=DAILY;COUNT=4"))
    )
    ...
    assert len(due.fires) == 1
    result = await CommissionActionProvider(store).execute(
        trigger.workflow["inline"]["config"],
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "input": {
                    "prompt": "A brass key on a quiet platform.",
                    "size": "",
                    "controls": {},
                    "loras": [],
                }
...
async def test_unconfigured_media_dispatch_is_explicit_and_queues_no_job(
    ability, dispatch, tmp_path
):
    ...
    body.update(target_ability=ability, mode="generate", dispatch=dispatch)
    commission = store.create(body)
    config = store.triggers.get(TRIGGER_PREFIX + commission["id"]).trigger.workflow[
        "inline"
    ]["config"]
    admitted = await CommissionActionProvider(store).execute(
        config, ActionContext(event="trigger.fired", payload={"scheduled_for": 1200})
    )
...
async def test_retry_keeps_occurrence_mode_and_dispatch_snapshot_after_commission_update(
    tmp_path,
):
    _, work, _ = source(tmp_path)
    store, _, _ = make_store(tmp_path)
    ...
    body.update(
        ...
        dispatch={
            "input": {
                "prompt": "Pinned generation request.",
                "size": "",
                "controls": {},
                "loras": [],
            }
    ...
    first = await store.execute(
        commission["id"], "manual:pinned-generation", trigger="manual"
    )
    assert first["status"] == "failed"
    assert first["mode"] == "generate"
    changed = store.update(
        commission["id"],
        {"revision": commission["revision"], "mode": "planning", "dispatch": {}},
    )
    ...
    assert [row["status"] for row in retried["dispatch_receipts"]] == [
        "external_unavailable",
        "external_unavailable",
    ]


async def test_unconfigured_music_dispatch_persists_receipts_and_retry_identity(
    tmp_path,
):
    from gideon.workspace.artifacts.native import NativeArtifactProvider
    ...
    body.update(
        target_ability="music",
        mode="generate",
        dispatch={
            "track_id": track["id"],
            "track_revision": track["revision"],
            "prompt": "A restrained mystery score.",
            "music_length_ms": 3000,
            "force_instrumental": True,
...
async def test_music_video_dispatch_uses_real_pinned_ffmpeg_project(tmp_path):
    ...
    audio = catalog.artifacts.create_binary(
        name="Pinned score",
        data=_wav_bytes(),
        mime="audio/wav",
        kind="audio",
    ...
    track = catalog.attach(
        track["id"],
        {
            ...
            "source": {
                "kind": "imported",
                "label": "Commission fixture recording",
                "license": "Test-owned recording",
            },
        },
    )
    render = track["renders"][0]
    picture = io.BytesIO()
    Image.new("RGB", (64, 64), "navy").save(picture, format="PNG")
    image = catalog.artifacts.create_binary(
        name="Pinned scene",
    ...
    project = video.create(
        {
            ...
            "scenes": [
                {
                    "id": "scene",
                    "image_ref": {"slug": image.slug, "version": image.version},
                    "beats": 1,
                }
            ],
        }
    ...
    body.update(
        target_ability="music-video",
        mode="generate",
        dispatch={"project_id": project["id"], "revision": project["revision"]},
    )
    commission = store.create(body)
    ...
    assert (
        catalog.artifacts.get(
            finished["artifact_ref"]["slug"],
            version=finished["artifact_ref"]["version"],
        )
        is not None
    )
...
async def test_series_dispatch_enters_real_approval_gated_production(tmp_path):
    ...
    series = production.series.create(
        {
            "request_id": "commission-series",
            "title": "Commission series",
            "synopsis": "A key reveals a route.",
            "volumes": [
                {
                    "id": "volume",
                    "title": "Volume",
    ...
    work = production.series.prepare(
        series["id"], "chapter", {"revision": series["revision"]}
    )
    await production.series.draft(
        series["id"],
        "chapter",
        {
            "request_id": "source-draft",
    ...
    body.update(
        mode="generate",
        sources=[
            {"kind": "series", "id": series["id"], "revision": series["revision"]}
        ],
        dispatch={
            "series_id": series["id"],
...
def test_dispatch_routing_overrides_fail_before_persistence(
    ability, dispatch, tmp_path
):
    _, work, _ = source(tmp_path)
    store, _, _ = make_store(tmp_path)
