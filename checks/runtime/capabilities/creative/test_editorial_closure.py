...
def test_multi_issue_anchored_repair_undo_restores_continuity_source(tmp_path):
    ...
    parent = series.create({
        ...
        "volumes": [{
            ...
            "title": "Volume one",
            "chapters": [
                {"id": "issue-one", "title": "Station", "prompt": "Open at the station."},
                {"id": "issue-two", "title": "Observatory", "prompt": "Continue at the observatory."},
            ],
        }],
        "arcs": [{
            "id": "key-arc",
    ...
    first = series.works.draft(first["id"], {
        "request_id": "closure-first-draft", "revision": first["revision"], "text": FIRST,
    })["work"]
    second = series.works.draft(second["id"], {
        "request_id": "closure-second-draft", "revision": second["revision"], "text": SECOND,
    })["work"]

    continuity = ContinuityStore(series.works)
    mara_start = FIRST.index("Mara")
    proposal = asyncio.run(continuity.propose(first["id"], {
    ...
    accepted = continuity.accept(first["id"], proposal["id"], {
        "revision": 0, "work_revision": first["revision"],
    })
    ...
    source_artifact = series.works.artifacts.get(run["artifact_id"], version=run["artifact_version"])
    assert source_artifact is not None
    assert source_artifact.content == FIRST

    repair = asyncio.run(editorial.repair(first["id"], run["id"], finding["id"], {
    ...
    promoted = editorial.polishing.promote(first["id"], candidate["id"], {"revision": first["revision"]})
    changed = series.works.get(first["id"])
    assert changed["revision"] == first["revision"] + 1
    assert changed["text"] == FIRST.replace("Time stood still", "The station clock stopped")
    ...
    restored = series.works.restore(first["id"], {
        "revision": changed["revision"], "target_revision": first["revision"],
    })
    ...
    assert series.works.artifacts.get(run["artifact_id"], version=run["artifact_version"]).content == FIRST
