...
def data(**changes):
    return {
        ...
        "title": "Aster universe",
        "canon": [
            {
                "id": "night",
                "title": "Permanent night",
                "body": "The city has no sunrise.",
            },
            {"id": "moon", "title": "Moon", "body": "One red moon."},
        ],
        "visual_identity": {"colors": ["#AABBCC"], "style_notes": "Copper and glass."},
        **changes,
    }
...
def linked(home):
    ...
    ingredient = ingredients.create(
        {"request_id": "city", "type": "place", "title": "City"}
    )
...
def test_universe_canon_identity_order_restore_and_reopen(tmp_path):
    ...
    second = store.update(
        ...
        {
            "revision": 1,
            "canon": list(reversed(first["canon"])),
            "visual_identity": {
                "colors": ["#112233", "#abcdef"],
                "style_notes": "Glass only.",
            },
        },
    )
    ...
    assert reopened.get(first["id"])["ingredient_status"] == [
        {"id": ingredient["id"], "missing": False}
    ]
    assert reopened.get(first["id"])["board_status"] == [
        {"id": board["id"], "revision": 1, "missing": False}
    ]
...
def test_reference_pin_stays_at_selected_revision_and_missing_is_explicit(tmp_path):
    ...
    second = store.update(
        first["id"], {"revision": 1, "ingredient_ids": [], "board_refs": []}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"canon": "invalid"},
        {"canon": [{}] * 101},
        {"visual_identity": {"colors": ["red"]}},
        {"visual_identity": {"colors": [123]}},
...
def test_native_universe_operations_use_authoritative_state(tmp_path):
    async def run():
        ...
        edited = await call(
            "update",
            {"id": first["id"], "payload": {"revision": 1, "title": "Native edit"}},
        )
        assert (await call("list", {"q": "Native"}))["items"] == [edited]
        assert (await call("revisions", {"id": first["id"], "limit": 1, "offset": 1}))[
            "items"
        ] == [first]
        assert await call("export", {"id": first["id"], "revision": 1}) == first
        restored = await call(
            "restore",
            {"id": first["id"], "payload": {"revision": 2, "target_revision": 1}},
        )
    ...
    async def run():
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            changed = await client.patch(
                f"{BASE}/{id}", json={"revision": 1, "title": "HTTP edit"}
            )
            assert changed.status == 200
            assert (await changed.json())["revision"] == 2
            stale = await client.patch(
                f"{BASE}/{id}", json={"revision": 1, "title": "Bad"}
            )
            ...
            restored = await client.post(
                f"{BASE}/{id}/restore", json={"revision": 2, "target_revision": 1}
            )
            ...
            override = await client.patch(
                f"{BASE}/{id}", json={"revision": 3, "home": "/tmp/other"}
            )
