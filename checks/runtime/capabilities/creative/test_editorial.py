...
from gideon.workspace.capabilities.creative.prose_checks import CATALOG, SUPPORTED, scan
...
@pytest.mark.parametrize(
    ...
    [
        (
            "prose.filter-words",
            "She saw the door and began to run.",
            ["saw", "began to"],
        ),
        ...
        (
            "prose.filter-words",
            "She could see the door; he found himself running.",
            ["could see", "found himself"],
        ),
        (
            "prose.hedge-words",
            "It was as if he knew. Part of him almost felt it. I kind of think so, no doubt.",
            ["as if", "Part of him", "almost", "kind of", "no doubt"],
        ),
        ("prose.hedge-words", "The almshouse stood empty.", []),
        (
            "prose.crutch-words",
            "It was just really very that thing.",
            ["just", "really", "very"],
        ),
        ("prose.crutch-words", "He left in order to escape.", ["in order to"]),
        (
            "prose.adverbs",
            "She quickly entered the family home only briefly.",
            ["quickly", "briefly"],
        ),
        ("prose.adverbs", "She walked slowly.", ["slowly"]),
        (
            "prose.cliches",
            "And then TIME STOOD STILL as the door opened.",
            ["TIME STOOD STILL"],
        ),
        (
            "prose.cliches",
        ...
        (
            "prose.cliches",
            "little did they know. Later, all hell broke loose, and little did they know again.",
            ["little did they know", "all hell broke loose"],
        ),
        (
            "prose.modifier-stacking",
            "It was a big red shiny new car.",
            ["big red shiny new"],
        ),
        ("prose.modifier-stacking", "a big red car", []),
        (
            "prose.word-echoes",
            "The obsidian blade gleamed. She raised the obsidian high.",
            ["obsidian"],
        ),
        ...
        (
            ...
            "She froze. *He knows I lied to him.* Then she ran.",
        ...
        (
            "prose.italic-thoughts",
            "She would *never* do that. He told her to _run_.",
            [],
        ),
        ...
        (
            "prose.italic-thoughts",
            "*I have to get out.* ... later ... *I have to get out.*",
            ["*I have to get out.*"],
        ),
        (
            "dialogue.said-bookisms",
            '"I disagree," expostulated Marlon.',
            ["expostulated"],
        ),
...
def test_adverb_dialogue_manner_emotion_and_passive_intent():
    ...
    assert (
        scan("prose.passive-voice", "The letter was written by Mara.")[0]["quote"]
        == "was written"
    )
...
def test_rhythm_burstiness_gestures_and_stock_patterns():
    ...
    assert (
        scan("prose.repeated-gestures", "She nodded. He nodded. They nodded.")[0][
            "occurrences"
        ]
        == 3
    )
    assert (
        scan("prose.ai-tells", "She couldn't help but smile.")[0]["quote"]
        == "couldn't help but"
    )
    assert scan("prose.ai-tells", "Her eyes widened.")[0]["quote"] == "eyes widened"
    assert (
        scan("prose.structural-tics", "Go. Run. Stop.")[0]["quote"] == "Go. Run. Stop."
    )
    assert (
        scan("prose.structural-tics", "It was not just rain but a storm.")[0]["quote"]
        == "not just rain but a storm."
    )
    assert scan("prose.burstiness", "One—two—three.")[0]["quote"] == "—"
    paragraphs = "\n\n".join(["one two three four five."] * 4)
    assert any(
        h["problem"] == "Uniform paragraph lengths"
        for h in scan("prose.burstiness", paragraphs)
    )
...
def fixture(
    home,
    source="😀 Time stood still. She saw the obsidian door. The obsidian door opened.",
):
...
def payload(source, **changes):
    return {
        ...
        "start": 0,
        "end": len(source),
        "check_ids": ["prose.cliches", "prose.filter-words", "prose.word-echoes"],
        **changes,
    }
...
def test_registry_all_eighty_and_missing_context_are_explicit(tmp_path):
    ...
    assert (
        next(c for c in catalog if c["id"] == "chekhov.setups-payoffs")["label"]
        != "Max findings per run"
    )
    assert (
        next(c for c in catalog if c["id"] == "prose.cliches")["availability"]
        == "available"
    )
    assert (
        next(c for c in catalog if c["id"] == "visual.eyeline-match")["availability"]
        == "requires_context"
    )
    assert (
        next(c for c in catalog if c["id"] == "visual.eyeline-match")["context_family"]
        == "scene"
    )
    run = editorial.run(
        work["id"], payload(source, check_ids=["prose.cliches", "visual.eyeline-match"])
    )
    assert run["kind"] == "mixed"
    assert run["results"][0]["status"] == "completed"
    assert run["results"][1] == {
        "check_id": "visual.eyeline-match",
        "status": "skipped",
        "reason": "scene_context_missing",
    }
    ...
    assert EditorialStore(WorkStore(tmp_path)).get(work["id"]) == editorial.get(
        work["id"]
    )
...
def test_run_replay_partial_coverage_empty_clear_and_caps(tmp_path):
    ...
    partial = editorial.run(
        work["id"], payload(source, start=20, end=25, check_ids=["prose.cliches"])
    )
    ...
    clean = editorial.run(
        work["id"], payload(source, check_ids=["prose.italic-thoughts"])
    )
    assert clean["readiness"] == "selected_checks_clear"
    missing_context = editorial.run(
        work["id"], payload(source, check_ids=["pov.head-hopping"])
    )
    assert missing_context["readiness"] == "incomplete"
    assert missing_context["findings"] == []
    many = works.draft(
        work["id"], {"request_id": "many", "revision": 2, "text": "really " * 150}
    )["work"]
    capped = editorial.run(
        work["id"],
        payload(
            "really " * 150,
            work_revision=many["revision"],
            check_ids=["prose.crutch-words"],
        ),
    )
...
async def test_exact_repair_review_promote_restore_and_replay(tmp_path):
    ...
    assert works.get(work["id"])["text"] == source.replace(
        "Time stood still", "The clock stopped"
    )
    ...
    with pytest.raises(CatalogError) as replay:
        await editorial.repair(
            work["id"], run["id"], finding["id"], {**data, "replacement": "Other"}
        )
    assert replay.value.status == 409
    assert editorial.get(work["id"])["runs"][0]["stale"]
    restored = works.restore(
        work["id"], {"revision": promoted["work"]["revision"], "target_revision": 2}
    )
...
async def test_actual_http_native_registry_run_repair_and_home_isolation(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert (
            await client.get("/api/capabilities/creative/works/missing/editorial")
        ).status == 404
    ...
    invoked = await provider.invoke(
        "creative_work_editorial_run", {"id": work["id"], "payload": payload(source)}
    )
    ...
    repaired = await provider.invoke(
        "creative_work_editorial_repair",
        {
            ...
            "finding_id": native_run["findings"][0]["id"],
            "payload": {
                "request_id": "native",
                "work_revision": 2,
                "replacement": "The clock stopped",
