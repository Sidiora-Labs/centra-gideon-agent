...
def create(store, **changes):
    return store.create({'request_id': str(uuid4()), 'title': 'Night journey', 'kind': 'exercise', 'prompt': 'Describe a train station.', **changes})


def draft_payload(revision=1, **changes):
    return {'request_id': str(uuid4()), 'revision': revision, 'text': '  The last train arrived.\n\n', 'note': 'First attempt', **changes}
...
def test_manuscript_drafts_are_real_readonly_artifacts_with_exact_text(tmp_path):
    ...
    with pytest.raises(PermissionError):
        store.artifacts.update(artifact.slug, content='Overwrite forbidden', snapshot=True)
    second = store.draft(work['id'], draft_payload(2, text='A revised ending.', note='Second attempt'))
    ...
    with store.connection() as db:
        persisted = ' '.join(str(row) for row in db.execute('SELECT record FROM work_revisions'))
    assert request['text'].strip() not in persisted


def test_pinned_author_universe_context_remains_at_selected_revision(tmp_path):
    store = WorkStore(tmp_path)
    author = store.authors.create({'request_id': 'author', 'title': 'Mira', 'voice': {'tone': 'Quiet'}})
    universe = store.universes.create({'request_id': 'universe', 'title': 'City', 'canon': [{'id': 'law', 'title': 'Night'}]})
    work = create(store, author_ref={'id': author['id'], 'revision': 1}, universe_ref={'id': universe['id'], 'revision': 1})
...
def test_retry_recovers_real_artifact_after_sqlite_rollback(tmp_path):
    ...
    request = draft_payload()
    with store.connection() as db:
        db.execute("CREATE TRIGGER reject_work_revision BEFORE INSERT ON work_revisions BEGIN SELECT RAISE(ABORT, 'storage unavailable'); END")
...
def test_deterministic_artifact_collision_is_rejected_without_deleting_it(tmp_path):
    ...
    work = create(store)
    request = draft_payload()
    draft_id = hashlib.sha256((work['id'] + ':' + request['request_id']).encode()).hexdigest()
    existing = store.artifacts.create(name='Existing source', slug='creative-draft-' + draft_id, content='Keep me', kind='markdown')
...
@pytest.mark.parametrize('changes', [{'text': ''}, {'text': '   '}, {'text': 123}, {'text': 'x' * 1000001}, {'note': 'x' * 2001}, {'home': 'other'}, {'model': 'override'}, {'session_id': 'private'}, {'revision': True}])
...
def test_cross_home_work_context_and_draft_access_is_rejected(tmp_path):
    ...
    for action in (lambda: two.get(work['id']), lambda: two.context(work['id']),
                   lambda: two.drafts(work['id']), lambda: two.read_draft(other_work['id'], manuscript['draft']['id'])):
        with pytest.raises(CatalogError) as error:
            action()
    ...
    with pytest.raises(CatalogError):
        two.update(other_work['id'], {'revision': 1, 'active_draft_id': manuscript['draft']['id']})
...
def test_actual_http_and_all_native_work_operations(tmp_path):
    async def run():
        ...
        async with TestClient(TestServer(app)) as client:
            created = await client.post(base, json={'request_id': 'http', 'title': 'HTTP exercise', 'kind': 'exercise'})
            ...
            invalid = await client.post(path + '/drafts', json=draft_payload(2, provider='override'))
        ...
        native = await call('create', {'payload': {'request_id': 'native', 'title': 'Native work'}})
        result = await call('draft', {'id': native['id'], 'payload': draft_payload()})
        assert (await call('get', {'id': native['id']}))['active_draft_id'] == result['draft']['id']
        assert (await call('read_draft', {'id': native['id'], 'draft_id': result['draft']['id']}))['text'].startswith('  The')
        assert (await call('drafts', {'id': native['id']}))['items'] == [result['draft']]
        assert (await call('context', {'id': native['id']}))['author'] is None
        updated = await call('update', {'id': native['id'], 'payload': {'revision': 2, 'title': 'Updated work'}})
        ...
        restored = await call('restore', {'id': native['id'], 'payload': {'revision': 3, 'target_revision': 1}})
