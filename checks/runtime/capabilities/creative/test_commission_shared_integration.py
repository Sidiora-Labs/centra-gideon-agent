...
MANIFEST = (
    ROOT / "runtime/gideon/extensions/apps/native/gideon-creative-commissions/app.json"
)
...
@pytest.mark.asyncio
async def test_parent_http_and_native_registry_dispatch_the_real_due_trigger(
    ...
    work = works.draft(
        work["id"],
        {
            "request_id": "shared-draft",
            "revision": work["revision"],
            "text": "Exact scheduled source.",
            "note": "canonical",
        },
    )["work"]
    ...
    try:
        ...
        due = await action.provider.execute(
            action.config,
            ActionContext(event="trigger.fired", payload={"scheduled_for": 1730000000}),
        )
        ...
        try:
            ...
            app.router.add_get(
                "/session", lambda request: web.json_response({"authenticated": True})
            )
            register(app)
            async with TestClient(
                ...
                assert (
                    await client.get("/api/capabilities/creative/commissions")
                ).status in (401, 403)
                assert (
                    await client.get(
                        "/session?token=" + generate_token("commission-owner")
                    )
                ).status == 200
