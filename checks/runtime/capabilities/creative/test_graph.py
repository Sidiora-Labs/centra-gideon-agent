...
def universes(home):
    ...
    target = store.create({'request_id': 'target', 'title': 'Target', 'canon': [
        {'id': 'law', 'title': 'Law', 'body': 'No sunrise.'}, {'id': 'target-only', 'title': 'Target only'}],
        'visual_identity': {'colors': ['#112233'], 'style_notes': 'Copper'}})
    source = store.create({'request_id': 'source', 'title': 'Source', 'canon': [
        {'id': 'law', 'title': 'Law', 'body': 'Sunrise allowed.'}, {'id': 'source-only', 'title': 'Source only'}],
...
def payload(target, source, **changes):
    return {'request_id': str(uuid4()), 'source_id': source['id'], 'source_revision': source['revision'],
            'target_revision': target['revision'], 'canon_choices': {'law': 'source'}, 'identity_choice': 'target', **changes}


def test_graph_projects_real_relations_members_external_and_missing(tmp_path):
    catalog = IngredientStore(tmp_path)
    other = catalog.create({'request_id': 'other', 'title': 'Outside city', 'type': 'place'})
    member = catalog.create({'request_id': 'member', 'title': 'Character', 'type': 'character',
                             'relations': [{'kind': 'related', 'target_id': other['id']}]})
    store = UniverseStore(tmp_path)
    target = store.create({'request_id': 'target', 'title': 'Universe', 'ingredient_ids': [member['id']]})
    graph = UniverseGraph(store)
    projection = graph.graph(target['id'])
    assert projection['revision'] == 1
    assert projection['merges'] == []
    assert projection['nodes'] == [
        {'id': member['id'], 'title': 'Character', 'type': 'character', 'missing': False, 'external': False},
        {'id': other['id'], 'title': 'Outside city', 'type': 'place', 'missing': False, 'external': True}]
    assert projection['edges'] == [{'source': member['id'], 'target': other['id'], 'kind': 'related'}]
    store.update(target['id'], {'revision': 1, 'ingredient_ids': [member['id'], other['id']]})
...
def test_merge_preview_resolves_conflicts_and_preserves_source_and_history(tmp_path):
    ...
    assert preview['canon_conflicts'] == [{'id': 'law', 'target': target['canon'][0], 'source': source['canon'][0]}]
    ...
    assert [entry['id'] for entry in result['canon']] == ['law', 'target-only', 'source-only']
...
def test_merge_opposite_choices_union_links_and_no_conflict_defaults(tmp_path):
    ...
    source = store.update(source['id'], {'revision': 1, 'ingredient_ids': [one['id'], two['id']], 'board_refs': [{'id': board['id'], 'revision': 1}]})
    result = graph.merge(target['id'], payload(target, source, canon_choices={'law': 'target'}, identity_choice='source'))['universe']
    ...
    empty = store.create({'request_id': 'empty', 'title': 'Empty', 'visual_identity': result['visual_identity']})
...
@pytest.mark.parametrize('changes', [
    {'canon_choices': {}}, {'canon_choices': {'law': 'unknown'}}, {'canon_choices': {'law': 'source', 'extra': 'target'}},
    {'identity_choice': ''}, {'identity_choice': 'unknown'}, {'source_revision': 2}, {'target_revision': 2},
    {'home': '/tmp/escape'}, {'provider': 'override'}, {'session_id': 'other'}, {'source_id': '../escape'},
])
...
def test_merge_collection_limit_rolls_back_event_and_snapshot(tmp_path):
    store, graph, target, source = universes(tmp_path)
    target = store.update(target['id'], {'revision': 1, 'canon': [{'id': f't-{i}', 'title': f'T{i}'} for i in range(100)]})
...
def test_real_http_and_native_merge_surface(tmp_path):
    async def run():
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            preview = await client.post(base + '/merge-preview', json={'source_id': source['id']})
            ...
            missing = await client.post(base + '/merge-preview', json={'source_id': 'missing'})
            assert missing.status == 404
        provider = CreativeToolProvider(tmp_path)
        native_graph = await provider.invoke('creative_universe_graph', {'id': target['id']})
        assert native_graph.success
        assert len(json.loads(native_graph.output)['merges']) == 1
        native_preview = await provider.invoke('creative_universe_merge_preview', {'id': target['id'], 'payload': {'source_id': source['id']}})
        assert native_preview.success
        viewed = json.loads(native_preview.output)
        request = payload(viewed['target'], source, canon_choices={})
        native_merge = await provider.invoke('creative_universe_merge', {'id': target['id'], 'payload': request})
