...
async def fixture(home):
    ...
    commission_payload = {
        "request_id": "consumer-commission",
        "name": "Consumer commission",
        "target_ability": "image",
        "brief": {
            "intent": "Plan a still image.",
            "genre": "",
            "category": "",
            "style": "",
            "constraints": {},
    ...
    register_peer_feedback(
        receiver_app,
        remote,
        commissions=remote_store,
        direction=remote_direction,
        peers=remote_peers,
    )
    server = TestServer(receiver_app)
    await server.start_server()
    remote_row = connect(
        owner_peers, remote_peers, str(server.make_url("/")), send=[SCOPE]
    )
    connect(remote_peers, owner_peers, "http://127.0.0.1:9", receive=[SCOPE])
    sender = PeerFeedbackStore(
        home, commissions=store, direction=direction, peers=owner_peers
    )
    app[PEER_FEEDBACK] = sender
    return app, server, store, sender, commission, reaction, remote_row["id"], calls


async def test_owner_and_native_consumers_require_exact_revision_approval_and_durable_receipt(
    tmp_path,
):
    app, receiver, store, sender, commission, reaction, peer_id, calls = await fixture(
        tmp_path
    )
    ...
    approval = {
        ...
        "reaction_revision": reaction["revision"],
    }
    try:
        async with TestClient(TestServer(app)) as client:
            response = await client.get(root + "/peer-feedback/peers")
            assert await response.json() == {
                "items": [{"id": peer_id, "label": "Review partner"}]
            }
            ...
            response = await client.post(
                path, json={"peer_id": peer_id, "approval": changed}
            )
            assert response.status == 409
            assert calls == []
            response = await client.post(
                path, json={"peer_id": peer_id, "approval": approval}
            )
        ...
        assert (
            definitions["creative_commission_peer_feedback_peers"].requires_approval
            is False
        )
        assert (
            definitions["creative_commission_peer_feedback_deliver"].requires_approval
            is True
        )
        listed = await tools.invoke("creative_commission_peer_feedback_peers", {})
        assert json.loads(listed.output)["items"][0]["id"] == peer_id
        delivered = await tools.invoke(
...
async def test_owner_consumer_routes_remain_dashboard_token_protected(tmp_path):
    app = web.Application(
        middlewares=[token_auth_middleware(port=47992, local_only=True)]
    )
    register(app, tmp_path)
    async with TestClient(TestServer(app)) as client:
        peers = await client.get(
            "/api/capabilities/creative/commissions/peer-feedback/peers"
        )
        delivery = await client.post(
            "/api/capabilities/creative/commissions/c/feedback/r/deliver", json={}
        )
