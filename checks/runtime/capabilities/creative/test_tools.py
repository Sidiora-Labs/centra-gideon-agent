...
from gideon.workspace.capabilities.creative.production_tools import (
    SCHEMAS as PRODUCTION_SCHEMAS,
)
from gideon.workspace.capabilities.creative.store import CatalogError, IngredientStore
from gideon.workspace.capabilities.creative.tools import (
    SCHEMAS,
    WRITES,
...
def test_manifest_factory_and_actual_tool_registry_dispatches_completed_workflows(
    ...
    manifests = [
        AppManifest.from_json_file(
            root / "runtime/gideon/extensions/apps/native" / name / "app.json"
        )
        for name in (
            "gideon-creative",
    ...
    try:
        ...
        assert invoke(registered, "production_list", series_id=series["id"]) == {
            "items": []
        }
        exports = registry.get_provider("gideon-creative-exports")
        direction = registry.get_provider("gideon-creative-direction")
        assert json.loads(
            asyncio.run(exports.invoke("creative_manuscript_exports", {})).output
        ) == {"items": []}
        assert json.loads(
            asyncio.run(direction.invoke("creative_direction_projects", {})).output
        ) == {"items": []}
    finally:
...
def test_tool_definitions_complete_strict_and_approval_bearing(tmp_path):
    ...
    for definition in definitions:
        ...
        mutates = (
            definition.name.split("_", 2)[2] in WRITES
            or definition.name in PRODUCTION_SCHEMAS
            and definition.name
        ...
        assert definition.risk_level == (
            RiskLevel.CAUTION if mutates else RiskLevel.SAFE
        )
        assert definition.description
        assert (
            not {"home", "session_id", "account", "provider"}
...
def test_ingredient_lifecycle_shared_store_and_revision_pages(tmp_path):
    ...
    edited = invoke(
        ...
        "ingredient_update",
        id=id,
        payload={"revision": 1, "title": "Aster II"},
    )
    ...
    restored = invoke(
        ...
        "ingredient_restore",
        id=id,
        payload={"revision": 2, "target_revision": 1},
    )
...
def test_board_lifecycle_real_artifact_provenance_and_export(tmp_path):
    ...
    artifact = artifact_provider.create(
        name="Reference letter", content="Blue evening", kind="markdown"
    )
    ...
    changed = invoke(
        ...
        "board_update",
        id=id,
        payload={"revision": 1, "title": "Night references"},
    )
    ...
    restored = invoke(
        provider, "board_restore", id=id, payload={"revision": 2, "target_revision": 1}
    )
...
def test_conflict_and_unknown_record_are_structured_errors(tmp_path, entity):
    ...
    invoke(
        ...
        entity + "_update",
        id=record["id"],
        payload={"revision": 1, "title": "Changed"},
    )
    result = asyncio.run(
        provider.invoke(
            "creative_" + entity + "_update",
    ...
    missing = asyncio.run(
        provider.invoke("creative_" + entity + "_get", {"id": "missing"})
    )
...
@pytest.mark.parametrize(
    "field", ["home", "account", "provider", "session_id", "model", "credential"]
)
def test_unknown_scope_fields_fail_without_side_effects(tmp_path, field):
    provider = CreativeToolProvider(tmp_path)
    result = asyncio.run(
        provider.invoke("creative_ingredient_list", {field: "override"})
    )
    assert not result.success
...
def test_runtime_homes_cannot_read_each_others_records_or_sources(tmp_path):
    ...
    for entity, value in [("ingredient", record), ("board", board)]:
        result = asyncio.run(
            two.invoke("creative_" + entity + "_get", {"id": value["id"]})
        )
        assert not result.success
