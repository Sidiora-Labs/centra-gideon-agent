...
def fixture(home, content='The train arived late.\nAn unchanged second line.\n'):
    ...
    work = works.draft(work['id'], {'request_id': 'draft', 'revision': 1, 'text': content})['work']
    return works, PolishingStore(works), work


def request(**changes):
    return {'request_id': str(uuid4()), 'revision': 2, 'start': 0, 'end': 22, 'mode': 'authored',
            'replacement': 'The train arrived late.', 'instruction': 'Correct spelling.', **changes}
...
@pytest.mark.parametrize('changes', [
    {'start': -1}, {'start': True}, {'end': 0}, {'end': 999}, {'revision': 1},
    {'mode': 'fake-model'}, {'replacement': 'x' * 8001}, {'instruction': 'x' * 2001},
    {'replacement': 'The train arived late.'}, {'home': 'other'}, {'provider': 'override'},
    {'model': 'override'}, {'session_id': 'private'}, {'credential': 'override'},
...
def test_no_active_draft_and_invalid_promotion_payload_fail_closed(tmp_path):
    ...
    work = works.draft(work['id'], {'request_id': 'text', 'revision': 1, 'text': 'The train arived late.'})['work']
    proposal = propose(polish, work['id'], request())
...
def test_actual_http_and_native_candidate_workflow(tmp_path):
    async def run():
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            promoted = await client.post(base + '/' + proposal['id'] + '/promote', json={'revision': 2})
        ...
        proposal = await call('propose', {'id': work['id'], 'payload': request(revision=3, end=23, replacement='The train arrived early.')})
        assert (await call('get', {'id': work['id'], 'proposal_id': proposal['id']}))['promotion'] is None
        assert len((await call('list', {'id': work['id']}))['items']) == 2
        result = await call('promote', {'id': work['id'], 'proposal_id': proposal['id'], 'payload': {'revision': 3}})
...
def test_real_configured_model_attempt_records_inference_outcome(tmp_path):
    ...
    report = {'task': 'creative.07', 'actual_provider_attempt': True, 'inference_qualified': False}
    try:
        ...
        report.update(inference_qualified=True, candidate_id=candidate['id'], summary=candidate['summary'])
