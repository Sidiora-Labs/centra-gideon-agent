...
from gideon.workspace.capabilities.creative.production_tools import (
    SCHEMAS,
    CreativeProductionToolProvider,
)
...
def create_series(store, prepared=True, chapters=1):
    plans = [
        {
            ...
            "prompt": f"Write chapter {index}.",
        }
        for index in range(1, chapters + 1)
    ]
    record = store.series.create(
        {
            "request_id": f"series-{chapters}-{prepared}",
            "title": "Production series",
            "synopsis": "Mara follows a key.",
    ...
    if prepared:
        for chapter in plans:
            works.append(
                store.series.prepare(
                    record["id"], chapter["id"], {"revision": record["revision"]}
                )
            )
    return record, works
...
def test_series_revision_change_pauses_without_generation(tmp_path):
    ...
    series, _ = create_series(store)
    run = start(store, series)
    updated = store.series.update(
        series["id"], {"revision": series["revision"], "synopsis": "Changed synopsis."}
    )
    paused = asyncio.run(store.advance(series["id"], run["id"]))
    assert paused["status"] == "paused"
...
def test_candidate_submit_validates_state_revision_and_shape(tmp_path):
    ...
    with pytest.raises(CatalogError, match="not currently requested"):
        store.submit(
            series["id"],
            run["id"],
            {"request_id": "early", "work_revision": 1, "text": CLEAN, "note": ""},
        )
    asyncio.run(store.advance(series["id"], run["id"]))
    with pytest.raises(CatalogError) as stale:
        store.submit(
            series["id"],
            run["id"],
            {"request_id": "stale", "work_revision": 2, "text": CLEAN, "note": ""},
        )
    ...
    with pytest.raises(CatalogError, match="text"):
        store.submit(
            ...
            {
                "request_id": "empty",
                "work_revision": works[0]["revision"],
                "text": "",
                "note": "",
            },
        )
...
def test_approval_promotes_only_the_pinned_candidate_into_canonical_work(tmp_path):
    ...
    assert (
        applied["source_pins"][0]["draft_id"]
        == applied["candidate"]["applied_draft_id"]
    )
    assert applied["source_pins"][0]["artifact_version"] == 1
    assert (
        applied["source_pins"][0]["content_hash"]
...
def test_promotion_pin_survives_restart_and_resolves_to_exact_canonical_source(
    tmp_path,
):
    store = SeriesProductionStore(tmp_path)
    series, works = create_series(store)
    ...
    assert state["sources"][0] == {
        **state["source_pins"][0],
        "missing": False,
        "content_hash": state["source_pins"][0]["content_hash"],
    }


def test_promotion_failure_rolls_back_canonical_work_and_source_pin(
    ...
    assert (
        recovered["source_pins"][0]["content_hash"]
        == hashlib.sha256(CLEAN.encode()).hexdigest()
    )
...
def test_clean_generated_draft_is_editorially_pinned_then_kept_by_explicit_approval(
    tmp_path,
):
    store = SeriesProductionStore(tmp_path)
    series, works = create_series(store)
...
def test_generated_draft_with_findings_can_be_rolled_back_without_orphan_success(
    tmp_path,
):
    store = SeriesProductionStore(tmp_path)
    series, works = create_series(store)
...
def test_existing_authored_draft_gets_review_approval_without_generated_candidate(
    tmp_path,
):
    store = SeriesProductionStore(tmp_path)
    series, works = create_series(store)
    drafted = store.series.works.draft(
        works[0]["id"], {"request_id": "existing", "revision": 1, "text": CLEAN}
    )["work"]
    run = start(store, series)
...
def test_model_mode_uses_configured_adapter_and_never_invents_content(tmp_path):
    ...
    if result["status"] == "exhausted":
        ...
        artifact = store.series.works.artifacts.get(
            result["candidate"]["artifact_id"], version=1
        )
        assert artifact is not None
...
def test_list_is_series_isolated_and_unknown_records_are_404(tmp_path):
    ...
    two = store.series.create(
        {
            "request_id": "second",
            "title": "Second",
            "synopsis": "",
            "volumes": [{"id": "v2", "title": "V2", "chapters": []}],
            "arcs": [],
        }
    )
    run = start(store, one)
...
async def test_real_http_start_advance_submit_approve_and_controls(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert (
            await client.post(root + f'/{run["id"]}/advance', json={})
        ).status == 200
        response = await client.post(
            root + f'/{run["id"]}/submit',
            json={
                "request_id": "http-candidate",
                "work_revision": works[0]["revision"],
                "text": CLEAN,
                "note": "HTTP",
            },
        ...
        response = await client.post(
            root + f'/{run["id"]}/approve', json={"decision": "apply"}
        )
        assert response.status == 200
        ...
        response = await client.post(
            root + f'/{run["id"]}/approve', json={"decision": "keep"}
        )
        assert response.status == 200
...
async def test_native_provider_uses_same_durable_store_and_closed_schemas(tmp_path):
    ...
    advanced = await provider.invoke(
        "creative_production_advance", {"series_id": series["id"], "run_id": run["id"]}
    )
    assert advanced.success
    submitted = await provider.invoke(
        "creative_production_submit",
        {
            "series_id": series["id"],
            "run_id": run["id"],
            "payload": {
                "request_id": "native-candidate",
                "work_revision": works[0]["revision"],
                "text": CLEAN,
                "note": "Native",
            },
    ...
    approved = await provider.invoke(
...
async def test_native_rejects_unknown_tool_foreign_series_and_extra_arguments(tmp_path):
    ...
    foreign = await provider.invoke(
        "creative_production_get", {"series_id": series["id"], "run_id": "missing"}
    )
    assert not foreign.success
    assert foreign.metadata["status"] == 404
    extra = await provider.invoke(
        "creative_production_list", {"series_id": series["id"], "provider": "override"}
    )
