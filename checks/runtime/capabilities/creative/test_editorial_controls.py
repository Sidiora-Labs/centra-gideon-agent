...
from gideon.workspace.capabilities.creative.editorial_controls import (
    EditorialControlsStore,
)
...
from gideon.workspace.capabilities.creative.tools import SCHEMAS, CreativeToolProvider
...
def standalone(home, source=SOURCE_ONE):
    ...
    work = works.draft(
        work["id"], {"request_id": "standalone-draft", "revision": 1, "text": source}
    )["work"]
...
def series_fixture(home):
    ...
    first = series.prepare(
        record["id"], "chapter-one", {"revision": record["revision"]}
    )
    second = series.prepare(
        record["id"], "chapter-two", {"revision": record["revision"]}
    )
    first = series.works.draft(
        first["id"], {"request_id": "draft-one", "revision": 1, "text": SOURCE_ONE}
    )["work"]
    second = series.works.draft(
        second["id"], {"request_id": "draft-two", "revision": 1, "text": SOURCE_TWO}
    )["work"]
...
def configure(editorial, work, **changes):
    payload = {
        ...
        "policy_revision": changes.pop(
            "policy_revision",
            editorial.controls.state(work["id"])["policy"]["revision"],
        ),
        "readiness_gate": changes.pop("readiness_gate", "block_any"),
        "checks": changes.pop("checks", {}),
        **changes,
    }
    return editorial.controls.configure(
        work["id"], payload, {row["id"] for row in editorial.catalog(work["id"])}
    )
...
def run_payload(work, request="run", checks=None):
    return {
        ...
        "start": 0,
        "end": len(SOURCE_ONE),
        "check_ids": checks or ["prose.cliches"],
    }
...
def test_standalone_policy_has_explicit_scope_and_exact_replay(tmp_path):
    ...
    with pytest.raises(CatalogError) as conflict:
        editorial.controls.configure(
            work["id"], {**payload, "readiness_gate": "block_any"}, known
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"checks": {"prose.cliches": {"enabled": "yes", "severity": "low"}}},
            "requires",
        ),
        (
            {"checks": {"prose.cliches": {"enabled": True, "severity": "critical"}}},
            "requires",
        ),
        ({"provider": "override"}, "Unexpected"),
    ],
)
def test_policy_rejects_unbounded_or_routing_inputs_without_writes(
...
def test_custom_check_create_update_delete_preserves_revision_and_scope(tmp_path):
    _, editorial, series, first, second = series_fixture(tmp_path)
    created = editorial.controls.custom(
        first["id"], custom_payload(first, request_id="custom-create")
    )
    ...
    updated = editorial.controls.custom(
        second["id"],
        custom_payload(
            second,
            "update",
            request_id="custom-update",
            id=created["id"],
            revision=1,
            label="Promise audit",
            prompt="Quote an exact unresolved promise.",
            severity="medium",
        ),
    )
    ...
    deleted = editorial.controls.custom(
        first["id"],
        custom_payload(
            first,
            "delete",
            request_id="custom-delete",
            id=created["id"],
            revision=2,
            label=None,
            prompt=None,
...
def test_custom_request_replay_conflict_and_optimistic_revision(tmp_path):
    ...
    assert conflict.value.status == 409
    with pytest.raises(CatalogError) as stale:
        editorial.controls.custom(
            work["id"], custom_payload(work, "update", id=first["id"], revision=2)
        )
    assert stale.value.status == 409
...
def test_custom_checks_reject_invalid_definitions_and_model_overrides(
    tmp_path, changes, message
):
    _, editorial, work = standalone(tmp_path)
...
def test_disabled_checks_are_explicit_and_readiness_uses_effective_severity(tmp_path):
    ...
    assert disabled["results"] == [
        {
            "check_id": "prose.cliches",
            "status": "skipped",
            "reason": "disabled_by_editorial_policy",
        }
    ]
...
def test_readiness_gates_are_transparent(gate, severities, expected, tmp_path):
    ...
    assert (
        controls.readiness([{"severity": severity} for severity in severities], gate)
        == expected
    )
...
def test_typed_judge_validation_requires_exact_source_anchors(tmp_path):
    ...
    sources = [
        {"work_id": work["id"], "revision": work["revision"], "text": SOURCE_ONE}
    ]
    valid = {
        "score": 72,
        "verdict": "Revise",
        "concerns": [
            {
                "work_id": work["id"],
                "quote": "Time stood still",
                "reason": "Familiar phrase",
            },
        ],
    }
    assert EditorialControlsStore._validate_review("judge", valid, sources) == valid
    with pytest.raises(ValueError, match="anchor"):
        EditorialControlsStore._validate_review(
            "judge",
            {
                **valid,
                "concerns": [
...
def test_typed_panel_validation_requires_three_personas_and_grounded_quotes(tmp_path):
    ...
    valid = {
        "responses": [
            {"persona": "genre reader", "verdict": "clear", "concern_quote": ""},
            {
                "persona": "line reader",
                "verdict": "familiar",
                "concern_quote": "Time stood still",
            },
    ...
    with pytest.raises(ValueError, match="shape"):
        EditorialControlsStore._validate_review(
            "panel", {**valid, "responses": valid["responses"][:2]}, sources
        )
...
def test_typed_rank_validation_requires_each_candidate_exactly_once(tmp_path):
    ...
    with pytest.raises(ValueError, match="identity"):
        EditorialControlsStore._validate_review(
            "rank", {**valid, "ranking": valid["ranking"][:1]}, sources
        )
    with pytest.raises(ValueError, match="identity"):
        EditorialControlsStore._validate_review(
            "rank", {**valid, "weakest": ["foreign"]}, sources
        )
...
def test_reviewed_cut_preview_apply_and_undo_preserve_unrelated_text(tmp_path):
    ...
    applied = editorial.controls.apply_cut(
        work["id"], cut["id"], {"revision": work["revision"]}
    )
    ...
    undone = editorial.controls.undo_cut(
        work["id"], cut["id"], {"revision": shortened["revision"]}
    )
...
def test_cut_preview_is_idempotent_and_rejects_foreign_or_stale_sources(tmp_path):
    ...
    first = asyncio.run(
        editorial.controls.cut(editorial, work["id"], run["id"], finding["id"], payload)
    )
    assert (
        asyncio.run(
            editorial.controls.cut(
                editorial, work["id"], run["id"], finding["id"], payload
            )
        )
        == first
    )
...
def test_cut_apply_and_undo_enforce_lifecycle_and_revisions(tmp_path):
    ...
    with pytest.raises(CatalogError) as early:
        editorial.controls.undo_cut(
            work["id"], cut["id"], {"revision": work["revision"]}
        )
    assert early.value.status == 409
    with pytest.raises(CatalogError) as stale:
        editorial.controls.apply_cut(
            work["id"], cut["id"], {"revision": work["revision"] + 1}
        )
    assert stale.value.status == 409
    with pytest.raises(CatalogError) as missing:
        editorial.controls.apply_cut(
            work["id"], "missing", {"revision": work["revision"]}
        )
    assert missing.value.status == 404
...
async def test_real_http_policy_custom_review_and_cut_routes(tmp_path):
    ...
    work = works.draft(
        work["id"], {"request_id": "http-draft", "revision": 1, "text": SOURCE_ONE}
    )["work"]
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.patch(
            root + "/policy",
            json={
                "request_id": "http-policy",
                "work_revision": work["revision"],
                "policy_revision": 0,
                "readiness_gate": "block_high",
                "checks": {"prose.cliches": {"enabled": True, "severity": "low"}},
            },
        )
        ...
        response = await client.post(
            root + "/custom-checks",
            json={
                "request_id": "http-custom",
                "work_revision": work["revision"],
                "label": "Promise audit",
                "prompt": "Find broken promises.",
                "scope": "series",
                "severity": "medium",
            },
        )
        ...
        finding = run["findings"][0]
        response = await client.post(
            root + f'/runs/{run["id"]}/findings/{finding["id"]}/cut',
            json={"request_id": "http-cut", "work_revision": work["revision"]},
        )
        assert response.status == 200
        cut = await response.json()
        assert cut["status"] == "previewed"
        response = await client.post(
            root + f'/cuts/{cut["id"]}/apply', json={"revision": work["revision"]}
        )
        assert response.status == 200
        applied = await response.json()
        assert applied["status"] == "applied"
        response = await client.post(
            root + f'/cuts/{cut["id"]}/undo',
            json={"revision": applied["applied_revision"]},
        )
        assert response.status == 200
...
async def test_native_policy_custom_and_cut_use_same_authoritative_state(tmp_path):
    ...
    work = provider.works.create(
        {"request_id": "native-work", "title": "Native editorial"}
    )
    work = provider.works.draft(
        work["id"], {"request_id": "native-draft", "revision": 1, "text": SOURCE_ONE}
    )["work"]
    ...
    custom = await provider.invoke(
        "creative_work_editorial_custom_create",
        {
            "id": work["id"],
            "payload": {
                "request_id": "native-custom",
                "work_revision": work["revision"],
                "label": "Promise audit",
                "prompt": "Find broken promises.",
                "scope": "work",
    ...
    run_result = await provider.invoke(
        ...
        {"id": work["id"], "payload": run_payload(work, "native-run")},
    )
    run = json.loads(run_result.output)
    cut_result = await provider.invoke(
    ...
    undone = await provider.invoke(
        "creative_work_editorial_cut_undo",
        {
            "id": work["id"],
            "cut_id": cut["id"],
            "payload": {"revision": applied_record["applied_revision"]},
        },
    )
