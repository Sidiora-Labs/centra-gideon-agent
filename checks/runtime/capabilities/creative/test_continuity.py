...
def payload(source=SOURCE, **changes):
    ...
    return {'request_id': str(uuid4()), 'work_revision': 2, 'start': 0, 'end': len(source), 'mode': 'authored',
            'outline': [{**anchor, 'summary': 'Noor carries a key'}],
            'facts': [{**anchor, 'subject': 'Noor', 'predicate': 'key color', 'value': 'red'}], **changes}
...
def test_reviewed_canonical_evidence_reopens_with_immutable_export(tmp_path):
    ...
    assert proposal['coverage'] == {'start': 0, 'end': len(SOURCE), 'total_characters': len(SOURCE)}
    ...
    receipt = store.accept(work['id'], proposal['id'], {'revision': 0, 'work_revision': 2})
...
def test_reverse_outline_source_order_and_conflicting_values_preserve_both(tmp_path):
    ...
    second = propose(store, work['id'], payload(outline=[{**later, 'summary': 'Later blue key'}], facts=[{**later, 'subject': 'Noor', 'predicate': 'key color', 'value': 'blue'}]))
    ...
    assert [i['summary'] for i in result['outline']] == ['Noor carries a key', 'Later blue key']
    assert [i['accepted_revision'] for i in result['outline']] == [2, 1]
    assert result['conflicts'] == [{'subject': 'Noor', 'predicate': 'key color', 'values': ['blue', 'red'], 'proposal_ids': sorted([first['id'], second['id']])}]
...
def test_exact_replay_and_changed_request_rejected_after_source_advances(tmp_path):
    ...
    works.draft(work['id'], {'request_id': 'next', 'revision': 2, 'text': 'Changed text'})
    assert propose(store, work['id'], data) == item
    assert store.accept(work['id'], item['id'], {'revision': 0, 'work_revision': 2}) == receipt
...
def test_unaccepted_stale_source_and_metadata_change_refused(tmp_path):
    ...
    for expected in (2, 3):
        with pytest.raises(CatalogError) as stale:
            store.accept(work['id'], item['id'], {'revision': 0, 'work_revision': expected})
        assert stale.value.status == 409
    assert store.get(work['id'])['revision'] == 0
    works.draft(work['id'], {'request_id': 'next', 'revision': 3, 'text': 'New manuscript'})
...
@pytest.mark.parametrize('change', [
    {'start': -1}, {'start': True}, {'end': 0}, {'end': 999}, {'work_revision': 1},
    {'mode': 'guess'}, {'home': 'foreign'}, {'provider': 'foreign'}, {'session_id': 'other'},
    {'outline': 'wrong'}, {'facts': [None]}, {'outline': [], 'facts': []},
    {'outline': [{'summary': 'invented', 'start': 0, 'end': 2, 'quote': 'XX'}]},
    {'facts': [{'subject': '', 'predicate': 'x', 'value': 'x', 'start': 0, 'end': 1, 'quote': '😀'}]},
    {'mode': 'model'},
])
...
def test_partial_coverage_limits_and_outside_evidence(tmp_path):
    ...
    with pytest.raises(CatalogError):
        propose(store, work['id'], {**data, 'request_id': 'outside', 'start': 0, 'end': 1})
    assert len(store.get(work['id'])['proposals']) == 1
...
def test_concurrent_acceptance_has_one_winner_and_retry_can_follow(tmp_path):
    ...
    def accept(item):
        try:
            return ContinuityStore(WorkStore(tmp_path)).accept(work['id'], item['id'], {'revision': 0, 'work_revision': 2})
...
def test_real_database_failure_rolls_back_acceptance(tmp_path):
    ...
    with works.connection() as db:
        db.execute("CREATE TRIGGER reject_continuity BEFORE INSERT ON continuity_acceptances BEGIN SELECT RAISE(ABORT,'blocked'); END")
    ...
    assert store.accept(work['id'], item['id'], {'revision': 0, 'work_revision': 2})['revision'] == 1
...
async def test_real_http_and_native_current_home_contract(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(root + '/proposals/' + item['id'] + '/accept', json={'revision': 0, 'work_revision': 2})
        assert response.status == 200
        ...
        assert (await client.get('/api/capabilities/creative/works/missing/continuity')).status == 404
        assert (await client.post(root + '/proposals', json={'home': 'other'})).status == 400
    provider = CreativeToolProvider(tmp_path)
    result = await provider.invoke('creative_work_continuity_export', {'id': work['id']})
    assert result.success
    assert json.loads(result.output) == exported
    second = await provider.invoke('creative_work_continuity_propose', {'id': work['id'], 'payload': payload()})
    assert second.success
    accepted = await provider.invoke('creative_work_continuity_accept', {'id': work['id'], 'proposal_id': json.loads(second.output)['id'], 'payload': {'revision': 1, 'work_revision': 2}})
    ...
    denied = await provider.invoke('creative_work_continuity_get', {'id': work['id'], 'home': str(tmp_path)})
    assert not denied.success


async def test_actual_model_attempt_records_unavailable_or_grounded_real_result(tmp_path):
    works, store, work = fixture(tmp_path)
    data = {'request_id': 'real-model', 'work_revision': 2, 'start': 0, 'end': len(SOURCE), 'mode': 'model', 'instruction': 'Identify key color with exact evidence.'}
    observation = {'task': 'creative.10', 'actual_provider_attempt': True, 'inference_qualified': False}
    try:
    ...
    Path('/tmp/gideon-creative-10-inference-oss.json').write_text(json.dumps(observation, indent=2))
    assert works.get(work['id'])['text'] == SOURCE
