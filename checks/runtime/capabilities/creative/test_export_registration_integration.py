...
@pytest.mark.asyncio
async def test_creative_handler_registers_real_authenticated_manuscript_export(tmp_path):
    works = WorkStore(tmp_path)
    work = works.create({'request_id': 'integration-work', 'title': 'Integrated manuscript', 'kind': 'work', 'prompt': 'Write it', 'author_ref': None, 'universe_ref': None, 'active_draft_id': None})
    drafted = works.draft(work['id'], {'request_id': 'integration-draft', 'revision': work['revision'], 'text': '# Chapter\n\nAuthenticated export body.', 'note': 'canonical'})['work']
    series = SeriesStore(tmp_path).create({'request_id': 'integration-series', 'title': 'Integrated series', 'synopsis': '', 'volumes': [{'id': 'volume', 'title': 'Volume', 'chapters': [{'id': 'chapter', 'title': 'Chapter', 'prompt': 'Draft'}]}], 'arcs': []})
    ...
    try:
        ...
        async with TestClient(TestServer(app), cookie_jar=CookieJar(unsafe=True)) as client:
            ...
            authorized = await client.get('/api/capabilities/creative/exports?token=' + generate_token('creative-export-owner'))
            assert authorized.status == 200
            assert await authorized.json() == {'items': []}
            created = await client.post('/api/capabilities/creative/exports', json={
                'request_id': 'integration-export', 'source_kind': 'work', 'source_id': drafted['id'],
                'source_revision': drafted['revision'], 'title': 'Published manuscript', 'creator': 'Owner',
                'language': 'en', 'identifier': 'urn:gideon:integration-export',
            })
            ...
            production = await client.post(f"/api/capabilities/creative/series/{series['id']}/production", json={'request_id': 'integration-production', 'series_revision': series['revision'], 'mode': 'authored', 'max_attempts': 2})
            assert production.status == 200, await production.text()
            assert (await production.json())['series_id'] == series['id']
            direction = await client.post('/api/capabilities/creative/direction', json={'request_id': 'integration-direction', 'name': 'Release direction', 'treatment': 'Keep the authenticated manuscript pinned.', 'sources': [{'kind': 'work', 'id': drafted['id'], 'revision': drafted['revision']}], 'steps': [{'id': 'verify', 'title': 'Verify source', 'operation': 'source.verify', 'depends_on': []}]})
