...
async def main():
    ...
    work = works.draft(
        work["id"],
        {
            "request_id": "ui-draft",
            "revision": 1,
            "text": "A real canonical manuscript source.",
            "note": "UI fixture",
        },
    )["work"]
    app = web.Application()
    ...
    remote = home / "fixture-peer"
    remote.mkdir()
    owner_peers, remote_peers = PeerStore(home), PeerStore(remote)
    remote_identity, owner_identity = (
        remote_peers.snapshot()["self"],
        owner_peers.snapshot()["self"],
    )
    ...
    app[PEER_FEEDBACK] = PeerFeedbackStore(
        home, commissions=app[COMMISSIONS], direction=direction, peers=owner_peers
    )
    ...
    async def deliveries(request):
        with app[PEER_FEEDBACK].db() as db:
            count = db.execute(
                "SELECT COUNT(*) FROM creative_feedback_sends"
            ).fetchone()[0]
