...
from gideon.workspace.capabilities.creative.peer_feedback import PeerFeedbackStore, RECEIVE_PATH, SCOPE, _hash
...
async def owner_home(home):
    ...
    work = works.create({'request_id': 'peer-work', 'title': 'Peer feedback source', 'kind': 'work',
                         'prompt': 'A quiet station.', 'author_ref': None, 'universe_ref': None, 'active_draft_id': None})
    drafted = works.draft(work['id'], {'request_id': 'peer-draft', 'revision': 1,
        'text': 'A lantern moves across the empty station.', 'note': 'Pinned source'})
    direction = DirectionStore(home)
    store = CommissionStore(home, direction=direction, triggers=TriggerStore(base_dir=home))
    commission = store.create({'request_id': 'peer-commission', 'name': 'Shared taste', 'target_ability': 'image',
        'brief': {'intent': 'Develop a quiet visual treatment.', 'genre': 'mystery', 'category': 'treatment',
                  'style': 'restrained', 'constraints': {'rating': 'PG'}, 'seed_refs': []},
        'cadence': {'kind': 'interval', 'seconds': 900},
        'sources': [{'kind': 'work', 'id': work['id'], 'revision': drafted['work']['revision']}],
...
def connect_peer(store, remote, endpoint, send=(), receive=()):
    ...
    return store.put(identity['peer_id'], {'label': 'isolated peer', 'endpoint': endpoint,
...
def reaction_body(revision=None, author='remote-owner', rating='liked', note='Keep the stillness.'):
...
def read_feedback(store, reaction_id):
    with store.db() as db:
        row = db.execute('SELECT record FROM creative_commission_feedback WHERE id=?', (reaction_id,)).fetchone()
...
def receive_app(adapter, calls):
    ...
    async def receive(request):
        calls.append(1)
        try: return web.json_response(adapter.receive_signed(await request.json()))
        except CatalogError as exc: return web.json_response({'error': str(exc)}, status=exc.status)
...
async def connected_pair(tmp_path):
    ...
    owner_adapter = PeerFeedbackStore(owner, commissions=store, direction=direction, peers=owner_peers)
    target = owner_adapter.target(commission['id'], run['id'], output)
    return owner, remote, store, direction, commission, run, output, target, owner_peers, remote_peers, owner_adapter


async def test_real_two_home_remote_author_signed_transport_rerating_tombstone_restart(tmp_path):
    (owner, remote, store, _, _, _, _, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    calls = []
    async with TestClient(TestServer(receive_app(owner_adapter, calls))) as client:
        ...
        first_payload = remote_adapter.prepare(owner_id, target, reaction_body(), target_approval(owner_id, target))
        ...
        second_payload = remote_adapter.prepare(owner_id, target,
            reaction_body(revision=1, rating='disliked', note='Increase movement.'), target_approval(owner_id, target))
        ...
        tombstone = remote_adapter.revoke_prepared(owner_id, reaction_id, 2, target_approval(owner_id, target))
        third = await remote_adapter.push_prepared(owner_id, tombstone)
        assert third['revision'] == 3
        assert read_feedback(store, reaction_id)['deleted'] is True

        restarted = PeerFeedbackStore(owner, commissions=CommissionStore(owner),
                                      direction=DirectionStore(owner), peers=PeerStore(owner))
        new_proof_result = await remote_peers.post_signed(owner_id, SCOPE, RECEIVE_PATH, tombstone)
        assert new_proof_result == third
        assert restarted.path.exists()


async def test_target_is_resolved_from_owner_records_and_wire_excludes_private_state(tmp_path):
    (*_, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    ...
    assert set(payload) == {'schema_version', 'delivery_id', 'reaction', 'lineage', 'binding'}
    assert payload['lineage'] == target
    assert payload['binding']['payload_hash'] == _hash({k: v for k, v in payload.items() if k != 'binding'})
    assert payload['binding']['recipient'] == owner_id
    assert payload['binding']['scope'] == SCOPE
    serialized = json.dumps(payload, sort_keys=True)
    for private in ('cadence', 'brief', 'schedule_revision', 'dispatch', 'attempts', 'treatment'):
        assert f'"{private}"' not in serialized
    assert owner_adapter.target(target['commission_id'], target['run_id'], target['output']) == target
...
async def test_approval_and_directional_policy_fail_before_outbox_write(tmp_path):
    ...
    remote = tmp_path / 'remote'; owner_id = owner_peers.snapshot()['self']['peer_id']
    ...
    with pytest.raises(CatalogError, match='approval') as denied:
        adapter.prepare(owner_id, target, reaction_body(), target_approval(owner_id, target, 'denied'))
    assert denied.value.status == 403
    with pytest.raises(CatalogError, match='policy') as policy:
        adapter.prepare(owner_id, target, reaction_body(), target_approval(owner_id, target))
    assert policy.value.status == 403
    with adapter.db() as db:
        assert db.execute('SELECT COUNT(*) FROM creative_feedback_outbox').fetchone()[0] == 0


async def test_payload_signature_binds_exact_feedback_and_all_lineage_pins(tmp_path):
    (owner, remote, store, direction, _, _, _, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']; remote_id = remote_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
    connect_peer(owner_peers, remote_peers, 'http://127.0.0.1:9', receive=[SCOPE])
    payload = PeerFeedbackStore(remote, peers=remote_peers).prepare(
        owner_id, target, reaction_body(), target_approval(owner_id, target))
    for field, mutate in (
        ...
        ('output', lambda p: p['lineage']['output'].__setitem__('content_hash', '0' * 64)),
    ...
    assert direction.get(target['project']['id'])['revision'] == target['project']['revision']


async def test_signed_scope_nonce_and_signature_guards_precede_merge(tmp_path):
    (_, remote, store, _, _, _, _, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']; remote_id = remote_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
    connect_peer(owner_peers, remote_peers, 'http://127.0.0.1:9', receive=[SCOPE])
    payload = PeerFeedbackStore(remote, peers=remote_peers).prepare(
    ...
    with pytest.raises(CatalogError, match='scope'):
        owner_adapter.receive_signed({'proof': {'scope': 'creative.catalog'}, 'payload': payload})
    bad_signature = copy.deepcopy(payload); bad_signature['binding']['signature'] = 'A' * 86
...
@pytest.mark.parametrize('mutation', ['commission', 'run', 'project', 'revision', 'sources', 'output'])
async def test_receiver_rejects_orphan_or_mismatched_owner_lineage_before_write(tmp_path, mutation):
    (_, remote, store, _, _, _, _, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']; remote_id = remote_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
    connect_peer(owner_peers, remote_peers, 'http://127.0.0.1:9', receive=[SCOPE])
    ...
    if mutation == 'commission': altered['commission_id'] = 'foreign-commission'
    elif mutation == 'run': altered['run_id'] = 'foreign-run'
    elif mutation == 'project': altered['project']['id'] = 'foreign-project'
    elif mutation == 'revision': altered['project']['revision'] += 1
    elif mutation == 'sources': altered['sources'][0]['revision'] += 1
    else: altered['output']['content_hash'] = '0' * 64
    payload = remote_adapter.prepare(owner_id, altered, reaction_body(), target_approval(owner_id, altered))
...
async def test_author_isolation_revision_conflict_and_tombstone_are_durable(tmp_path):
    ...
    connect_peer(owner_peers, remote_peers, 'http://127.0.0.1:9', receive=[SCOPE])
    adapter = PeerFeedbackStore(remote, peers=remote_peers)
    alice = adapter.prepare(owner_id, target, reaction_body(author='alice'), target_approval(owner_id, target))
    bob = adapter.prepare(owner_id, target, reaction_body(author='bob'), target_approval(owner_id, target))
    assert alice['reaction']['id'] != bob['reaction']['id']
    owner_adapter.receive(remote_id, alice); owner_adapter.receive(remote_id, bob)
    conflicting = copy.deepcopy(alice); conflicting['reaction']['note'] = 'different'
    ...
    import base64
    conflicting['binding']['signature'] = base64.urlsafe_b64encode(private.sign(
        json.dumps(signed, sort_keys=True, separators=(',', ':')).encode())).decode().rstrip('=')
    ...
    reopened = PeerFeedbackStore(owner, commissions=CommissionStore(owner), direction=DirectionStore(owner), peers=PeerStore(owner))
...
def test_feedback_scope_is_independent_and_default_denied(tmp_path):
    ...
    with pytest.raises(PeerError): peers.create_proof(row['id'], SCOPE)


async def test_real_dashboard_middleware_delegates_only_exact_signed_receive_post(tmp_path):
    (owner, remote, store, direction, _, _, _, target, owner_peers, remote_peers, _) = await connected_pair(tmp_path)
    app = web.Application(middlewares=[token_auth_middleware(port=47991, local_only=True)])
    register_peer_feedback(app, owner, commissions=store, direction=direction, peers=owner_peers)
    async def nearby_handler(request):
        return web.json_response({'unsafe': True})
    app.router.add_post('/api/capabilities/creative/commission-feedback/receive-nearby', nearby_handler)
    async with TestClient(TestServer(app)) as client:
        connect_peer(remote_peers, owner_peers, str(client.make_url('/')), send=[SCOPE])
        connect_peer(owner_peers, remote_peers, 'http://127.0.0.1:9', receive=[SCOPE])
        owner_id = owner_peers.snapshot()['self']['peer_id']
        remote_adapter = PeerFeedbackStore(remote, peers=remote_peers)
        payload = remote_adapter.prepare(owner_id, target, reaction_body(), target_approval(owner_id, target))
        ...
        nearby = await client.post('/api/capabilities/creative/commission-feedback/receive-nearby', json={})
...
def test_creative_registrar_mounts_the_exact_peer_feedback_route(tmp_path):
    ...
    matching = [route for route in app.router.routes()
                if route.method == 'POST' and route.resource.canonical == RECEIVE_PATH]
    assert len(matching) == 1


async def test_prepared_outbox_survives_restart_and_requires_optimistic_revision(tmp_path):
    (_, remote, _, _, _, _, _, target, owner_peers, remote_peers, _) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
    adapter = PeerFeedbackStore(remote, peers=remote_peers)
    first = adapter.prepare(owner_id, target, reaction_body(), target_approval(owner_id, target))
    reopened = PeerFeedbackStore(remote, peers=PeerStore(remote))
    with reopened.db() as db:
        stored = db.execute('SELECT peer_id,record,lineage FROM creative_feedback_outbox').fetchone()
    ...
    with pytest.raises(CatalogError, match='reload') as stale:
        reopened.prepare(owner_id, target, reaction_body(revision=9), target_approval(owner_id, target))
    assert stale.value.status == 409
    second = reopened.prepare(owner_id, target, reaction_body(revision=1, rating='disliked'),
...
@pytest.mark.parametrize('body,error', [
    ...
    ({'author': 'alice', 'rating': 'liked', 'note': '', 'tags': ['x', 'x']}, 'tags'),
    ({'author': 'alice', 'rating': 'liked', 'note': '', 'tags': ['x'] * 21}, 'tags'),
    ({'author': 'alice', 'rating': 'liked', 'note': 'x' * 2001, 'tags': []}, 'length'),
    ({'author': 'alice', 'rating': 'liked', 'note': '', 'tags': [], 'extra': True}, 'fields'),
])
async def test_prepare_validates_author_rating_note_tags_and_exact_fields(tmp_path, body, error):
    (_, remote, _, _, _, _, _, target, owner_peers, remote_peers, _) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
    adapter = PeerFeedbackStore(remote, peers=remote_peers)
...
@pytest.mark.parametrize('approval', [
    None,
    {},
    {'decision': 'approved', 'peer_id': 'peer-' + '0' * 32, 'target_hash': '0' * 64},
    {'decision': 'denied', 'peer_id': 'placeholder', 'target_hash': '0' * 64},
    {'decision': 'approved', 'peer_id': 'placeholder', 'target_hash': '0' * 64, 'extra': True},
])
...
async def test_receiver_policy_denial_happens_before_payload_binding_or_store_access(tmp_path):
    (_, remote, store, _, _, _, _, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']; remote_id = remote_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
...
async def test_prepared_delivery_refuses_another_recipient_and_invalid_receipt(tmp_path):
    (owner, remote, _, _, _, _, _, target, owner_peers, remote_peers, _) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']
    calls = []
    app = web.Application()
    ...
    async with TestClient(TestServer(app)) as client:
        connect_peer(remote_peers, owner_peers, str(client.make_url('/')), send=[SCOPE])
        adapter = PeerFeedbackStore(remote, peers=remote_peers)
        payload = adapter.prepare(owner_id, target, reaction_body(), target_approval(owner_id, target))
        with pytest.raises(CatalogError, match='different peer') as wrong:
            await adapter.push_prepared(remote_peers.snapshot()['self']['peer_id'], payload)
        ...
        assert len(calls) == 1
        with adapter.db() as db:
            assert db.execute('SELECT COUNT(*) FROM creative_feedback_sends').fetchone()[0] == 0
    assert owner != remote


async def test_lower_unseen_revision_is_receipted_stale_without_overwriting_newer_owner_record(tmp_path):
    (_, remote, store, _, _, _, _, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']; remote_id = remote_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
    connect_peer(owner_peers, remote_peers, 'http://127.0.0.1:9', receive=[SCOPE])
    adapter = PeerFeedbackStore(remote, peers=remote_peers)
    first = adapter.prepare(owner_id, target, reaction_body(), target_approval(owner_id, target))
    second = adapter.prepare(owner_id, target, reaction_body(revision=1, rating='disliked'),
                             target_approval(owner_id, target))
...
async def test_exact_replay_returns_original_receipt_timestamp_across_receiver_restart(tmp_path):
    (owner, remote, store, direction, _, _, _, target, owner_peers, remote_peers, owner_adapter) = await connected_pair(tmp_path)
    owner_id = owner_peers.snapshot()['self']['peer_id']; remote_id = remote_peers.snapshot()['self']['peer_id']
    connect_peer(remote_peers, owner_peers, 'http://127.0.0.1:9', send=[SCOPE])
    connect_peer(owner_peers, remote_peers, 'http://127.0.0.1:9', receive=[SCOPE])
    ...
    reopened = PeerFeedbackStore(owner, commissions=CommissionStore(owner),
                                  direction=DirectionStore(owner), peers=PeerStore(owner))
...
async def test_tombstoning_one_remote_author_preserves_another_author(tmp_path):
    ...
    bob = adapter.prepare(owner_id, target, reaction_body(author='bob'), target_approval(owner_id, target))
    owner_adapter.receive(remote_id, alice); owner_adapter.receive(remote_id, bob)
    tombstone = adapter.revoke_prepared(owner_id, alice['reaction']['id'], 1, target_approval(owner_id, target))
...
async def test_owner_target_accepts_canonical_output_with_extra_local_fields(tmp_path):
    ...
    with owner_adapter.commissions.db() as db:
        row = db.execute('SELECT record FROM creative_commission_runs WHERE id=?', (target['run_id'],)).fetchone()
    ...
    resolved = owner_adapter.target(target['commission_id'], target['run_id'], target['output'])
