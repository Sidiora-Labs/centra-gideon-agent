...
from gideon.workspace.capabilities.creative.editorial_context import (
    FAMILIES,
    family_for,
    validate_context,
)
...
SOURCE = (
    "Mara entered the observatory. The brass key hung at her belt. "
...
VALID = {
    ...
    "arc": {
        "arcs": [{"id": "main", "name": "The key"}],
        "themes": ["trust"],
        "ticking_clock": {"deadline": "midnight", "consequence": "the gate closes"},
        "reader_map": [],
    },
    "world": {
        "rules": ["Magic requires a spoken price."],
        "places": [{"id": "observatory", "name": "Observatory"}],
    },
    "research": {
        "claims": [{"id": "rain", "claim": "Rain falls.", "source": "weather notes"}]
    },
    "comic": {
        "pages": [
            {
                "number": 1,
                "panels": [{"speaker": "Mara", "dialogue": "We leave tonight"}],
            }
        ]
    },
}
...
def test_every_typed_family_binds_an_immutable_versioned_json_artifact(
    tmp_path, family
):
...
def test_rebinding_creates_a_new_canonical_revision_without_mutating_history(tmp_path):
    ...
    assert (
        json.loads(works.artifacts.get(first["artifact_id"], version=1).content)["data"]
        == VALID["world"]
    )
    assert editorial.context.current(work["id"], "world")["data"] == changed
    listed = editorial.context.list(work["id"])
    assert (
        next(row for row in listed["families"] if row["family"] == "world")["revision"]
        == 2
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "canon",
            {"characters": [{"id": "mara"}], "objects": [], "rules": []},
            "requires name",
        ),
        ...
        (
            "pov",
            {"policy": "", "allowed": [], "transitions": [], "extra": True},
            "Unexpected fields",
        ),
        (
            "arc",
...
def test_malformed_context_is_rejected_before_artifact_creation(
    tmp_path, family, data, message
):
    works, editorial, work = fixture(tmp_path)
...
def test_unknown_schema_family_and_stale_work_revision_are_rejected(tmp_path):
    ...
    with pytest.raises(CatalogError, match="Unsupported"):
        editorial.context.bind(
            work["id"], {**base, "request_id": "schema", "schema_version": 2}
        )
    with pytest.raises(CatalogError) as stale:
        editorial.context.bind(
            work["id"], {**base, "request_id": "stale", "work_revision": 1}
        )
...
def test_missing_and_deleted_context_are_truthful_not_clear(tmp_path):
    ...
    assert missing["results"] == [
        {
            "check_id": "naming.dissimilar-names",
            "status": "skipped",
            "reason": "canon_context_missing",
        }
    ]
    ...
    assert (
        editorial.context.current(work["id"], "canon")["reason"]
...
def test_contextual_deterministic_checks_emit_exact_manuscript_anchors(tmp_path):
    ...
    assert (
        next(f for f in run["findings"] if f["check_id"] == "naming.dissimilar-names")[
            "quote"
        ]
        == "Mara"
    )
...
def test_model_checks_do_not_accept_provider_routing_or_credentials(tmp_path):
    ...
    for forbidden in ("provider", "model", "credentials", "api_key", "home", "runtime"):
        with pytest.raises(CatalogError, match="Unexpected fields"):
            editorial.run(
                work["id"], {**base, "request_id": forbidden, forbidden: "override"}
            )
            ...
            "characters": [
                {
                    "id": "nobody",
                    "name": "Absent",
                    "relationships": [{"target": "missing"}],
                }
            ],
            "objects": [{"id": "lost", "name": "Unmentioned", "significant": True}],
            "rules": [],
        },
    }
...
async def test_http_context_binding_run_and_staleness_use_real_server(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            root + "/runs",
            json=run_payload(work, ["naming.dissimilar-names"], request="http-run"),
        )
        assert response.status == 200
        ...
        assert (
            state["runs"][0]["context_bindings"][0]["artifact_id"]
            == binding["artifact_id"]
        )
        assert (
            await client.post(
                root + "/context",
                json={
                    "request_id": "http-invalid",
                    "work_revision": work["revision"],
                    "family": "comic",
                    "schema_version": 1,
                    "data": {"pages": [{"number": 0, "panels": []}]},
                },
            )
        ).status == 400
...
def test_context_state_survives_store_reopen(tmp_path):
    works, editorial, work = fixture(tmp_path)
    records = {
        family: bind(editorial, work, family, VALID[family]) for family in FAMILIES
    }
    reopened = EditorialStore(WorkStore(tmp_path))
    state = reopened.get(work["id"])
    assert {
        row["family"] for row in state["contexts"] if row["status"] == "available"
    } == set(FAMILIES)
...
def test_catalog_availability_tracks_each_bound_family(tmp_path):
    ...
    assert (
        before["prose.telling-emotion"]["availability"] == "configured_model_required"
    )
...
def test_contextual_findings_respect_requested_coverage(tmp_path):
    ...
    late = editorial.run(
        work["id"],
        {
            "request_id": "late-coverage",
            "work_revision": work["revision"],
            "start": 40,
            "end": len(SOURCE),
            "check_ids": ["naming.dissimilar-names"],
        },
    )
...
def test_multiple_family_run_pins_each_exact_binding_once(tmp_path):
    ...
    assert [row["family"] for row in run["context_bindings"]] == [
        "canon",
        "scene",
        "comic",
    ]
...
def test_each_context_family_has_a_distinct_truthful_missing_result(
    tmp_path, check_id, family
):
    _, editorial, work = fixture(tmp_path)
...
async def test_native_context_binding_and_async_run_share_authoritative_store(tmp_path):
    ...
    run_result = await provider.invoke(
        "creative_work_editorial_run",
        {
            "id": work["id"],
            "payload": run_payload(
                work, ["naming.dissimilar-names"], request="native-context-run"
            ),
        },
    )
...
async def test_native_binding_rejects_unknown_arguments_and_foreign_home(tmp_path):
    ...
    provider = CreativeToolProvider(tmp_path)
    extra = await provider.invoke(
        "creative_work_editorial_context_bind",
        {
            "id": work["id"],
            "payload": {
                "request_id": "native-extra",
                "work_revision": work["revision"],
                "family": "canon",
                "schema_version": 1,
                "data": VALID["canon"],
...
def test_native_schema_closes_routing_and_payload_shapes():
    ...
    assert payload_schema["properties"]["schema_version"] == {
        "type": "integer",
        "const": 1,
    }
