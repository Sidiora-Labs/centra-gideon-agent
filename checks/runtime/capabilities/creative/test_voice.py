...
from gideon.workspace.capabilities.creative.voice import (
    METRICS,
    VoiceStore,
    fingerprint,
)
...
LONG = (
    "The traveler carried a heavy iron key along the narrow road beside the silent river. "
    * 5
)
...
def fixture(home, texts=(SHORT, LONG), sample=EXEMPLAR):
    ...
    artifact = series.works.artifacts.create(
        name="Chosen voice", kind="markdown", content=sample
    )
    author = series.works.authors.create(
        {
            "request_id": "author",
            "title": "Literary author",
            "sample_refs": [{"artifact_id": artifact.slug, "artifact_version": 1}],
        }
    )
    ...
    for i, source in enumerate(texts):
        ...
        if source:
            work = series.works.draft(
                work["id"], {"request_id": f"draft{i}", "revision": 1, "text": source}
            )["work"]
        works.append(work)
...
def test_exact_sentence_paragraph_rhythm_fixture():
    ...
    assert (
        metrics["dialogue_pct"]
        == metrics["emdash_rate"]
        == metrics["abstract_rate"]
        == metrics["simile_rate"]
        == 0
    )
...
def test_actual_canonical_sources_drafted_matrix_and_config_history(tmp_path):
    ...
    assert report["samples"] == [
        {
            "artifact_id": sample.slug,
            "artifact_version": 1,
            "missing": False,
            "title": "Chosen voice",
        }
    ]
    assert report["baseline"]["sentence_mean"] == {
        "drafted_mean": 9.5,
        "center": 9.5,
        "std": 5.5,
    }
    assert report["findings"] == []
    config = voice.configure(
        record["id"],
        {
            "revision": 0,
            "series_revision": 1,
            "z_threshold": 0.5,
            "wells": {"Metal": ["KEY", "iron", "key"]},
        },
    )
...
def test_exemplar_and_blended_centers_keep_population_draft_spread(tmp_path):
    series, voice, record, works, sample = fixture(tmp_path)
    voice.configure(
        record["id"], {"revision": 0, "series_revision": 1, "baseline": "exemplars"}
    )
    exemplar = voice.report(record["id"])
    assert exemplar["baseline"]["sentence_mean"] == {
        "drafted_mean": 9.5,
        "center": 2,
        "std": 5.5,
    }
    ...
    assert any(
        f["kind"] == "uniform_corpus_difference" and f["metric"] == "dialogue_pct"
        for f in exemplar["findings"]
    )
    voice.configure(
        record["id"], {"revision": 1, "series_revision": 1, "baseline": "blended"}
    )
    blended = voice.report(record["id"])
    assert blended["baseline"]["sentence_mean"] == {
        "drafted_mean": 9.5,
        "center": 5.75,
        "std": 5.5,
    }
...
def test_pin_exemplar_version_after_source_update_and_author_profile_change(tmp_path):
    ...
    series.works.authors.update(
        author["id"],
        {
            "revision": 1,
            "sample_refs": [{"artifact_id": sample.slug, "artifact_version": 2}],
        },
    )
...
def test_missing_and_thin_sources_are_visible_and_never_silent_fallback(tmp_path):
    series, voice, record, works, sample = fixture(
        tmp_path, texts=(SHORT, "Small.", None), sample="Tiny sample."
    )
    voice.configure(
        record["id"], {"revision": 0, "series_revision": 1, "baseline": "exemplars"}
    )
    ...
    assert empty["baseline"]["sentence_mean"] == {
        "drafted_mean": None,
        "center": None,
        "std": None,
    }
...
def test_updated_current_draft_changes_metrics_and_previous_draft_stays_immutable(
    tmp_path,
):
    series, voice, record, works, sample = fixture(tmp_path)
    before = voice.report(record["id"])
    newer = series.works.draft(
        works[0]["id"], {"request_id": "rewrite", "revision": 2, "text": LONG}
    )["work"]
    ...
    assert (
        series.works.read_draft(works[0]["id"], works[0]["active_draft_id"])["text"]
        == SHORT
    )
...
def test_real_concurrent_config_writes_require_current_revision(tmp_path):
    ...
    def update(threshold):
        try:
            return VoiceStore(SeriesStore(tmp_path)).configure(
                record["id"],
                {"revision": 0, "series_revision": 1, "z_threshold": threshold},
            )
...
async def test_actual_http_and_native_share_config_and_private_source_report(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.patch(
            root, json={"revision": 0, "series_revision": 1, "baseline": "blended"}
        )
        assert response.status == 200
        ...
        assert (
            await client.patch(
                root, json={"revision": 1, "series_revision": 1, "provider": "foreign"}
            )
        ).status == 400
        assert (
            await client.get("/api/capabilities/creative/series/missing/voice")
        ).status == 404
    ...
    configured = await provider.invoke(
        ...
        {
            "id": record["id"],
            "payload": {"revision": 1, "series_revision": 1, "baseline": "drafted"},
        },
    )
    assert configured.success
    assert json.loads(configured.output)["revision"] == 2
    exported_tool = await provider.invoke(
        "creative_series_voice_export", {"id": record["id"]}
    )
