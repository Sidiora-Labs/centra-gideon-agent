...
def ready(store, **changes):
    ...
    store.update(
        first["id"],
        {
            "revision": 1,
            "stage": "development",
            "protagonist_goal": "Get home",
            "conflict": "No trains",
            "stakes": "Family waiting",
            "ending": "Walks home",
        },
    )
    store.update(
        first["id"],
        {
            ...
            "stage": "outline",
            "beats": [
                {"id": "arrival", "title": "Arrival", "summary": "Arrives too late."},
                {"id": "walk", "title": "Walk", "summary": "Finds another way."},
            ],
        },
    )
...
def test_guided_readiness_stage_order_and_restore(tmp_path):
    ...
    development = store.update(
        first["id"], {"revision": 1, "premise": "Missing train", "stage": "development"}
    )
    ...
    with pytest.raises(CatalogError):
        store.update(
            first["id"],
            {
                "revision": 3,
                "stage": "ready",
                "beats": [{"id": "one", "title": "Empty summary"}],
            },
        )
    completed = store.update(
        first["id"],
        {
            "revision": 3,
            "stage": "ready",
            "beats": [{"id": "one", "title": "Walk", "summary": "Walks home"}],
        },
    )
...
def test_authored_guidance_review_adoption_and_conflicts(tmp_path):
    ...
    payload = {
        ...
        "revision": 1,
        "mode": "authored",
        "patch": {"premise": "A traveler must walk home.", "genre": "Literary"},
    }
    ...
    with pytest.raises(CatalogError) as replay:
        asyncio.run(
            store.suggest(story["id"], {**payload, "patch": {"premise": "Other"}})
        )
...
def test_ready_story_creates_one_real_work_atomically_and_preserves_pins(tmp_path):
    ...
    universe = store.works.universes.create(
        {"request_id": "universe", "title": "Night city"}
    )
    story = ready(
        store,
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(
            pool.map(lambda _: store.create_work(story["id"], payload), range(2))
        )
    work = results[0]
    ...
    assert store.get(story["id"])["work_links"] == [
        {"work_id": work["id"], "story_revision": 4, "missing": False}
    ]
    assert store.export(story["id"]) == story
...
def test_real_sqlite_failure_rolls_back_work_and_link(tmp_path):
    ...
    with store.connection() as db:
        db.execute(
            "CREATE TRIGGER reject_story_link BEFORE INSERT ON story_work_links BEGIN SELECT RAISE(ABORT, 'link unavailable'); END"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"beats": [{"id": "../bad", "title": "Bad"}]},
        {"home": "other"},
        {"provider": "override"},
        {"author_ref": {"id": "foreign", "revision": 1}},
        {"universe_ref": {"id": "foreign", "revision": True}},
    ],
)
...
def test_missing_context_and_foreign_story_suggestion_access(tmp_path):
    ...
    for action in (
        lambda: two.get(story["id"]),
        lambda: two.suggestions(story["id"]),
        lambda: two.create_work(story["id"], {"request_id": "cross", "revision": 1}),
    ):
        with pytest.raises(CatalogError):
            action()
    ...
    async def run():
        ...
        async with TestClient(TestServer(app)) as client:
            response = await client.post(
                base, json={"request_id": "http", "title": "HTTP story"}
            )
        ...
        item = await call(
            "create", {"payload": {"request_id": "native", "title": "Native story"}}
        )
        proposal = await call(
            "suggest",
            {
        ...
        adopted = await call(
            ...
            {
                "id": item["id"],
                "payload": {"revision": 1, "suggestion_id": proposal["id"]},
            },
        )
        ...
        await call(
            "restore",
            {"id": item["id"], "payload": {"revision": 2, "target_revision": 1}},
        )
        ...
        work = await call(
            ...
            {
                "id": completed["id"],
                "payload": {"request_id": "native-work", "revision": 4},
            },
        )
...
def test_actual_model_guidance_outcome_is_recorded_separately(tmp_path):
    ...
    try:
        proposal = asyncio.run(
            store.suggest(
                story["id"],
                {
                    "request_id": "model",
                    "revision": 1,
                    "instruction": "Create a one-sentence premise about a train journey.",
                },
            )
        )
        ...
        adopted = store.adopt(
            story["id"], {"revision": 1, "suggestion_id": proposal["id"]}
        )
        assert adopted["revision"] == 2
        report.update(inference_qualified=True, suggestion_id=proposal["id"])
    Path("/tmp/gideon-creative-08-inference-oss.json").write_text(
        json.dumps(report, indent=2) + "\n"
    )


def test_stage_specific_authored_outline_review_preserves_order_and_refuses_cross_stage(
    tmp_path,
):
    store = StoryStore(tmp_path)
    ...
    beats = [
        {"id": "one", "title": "Arrival", "summary": "Late"},
        {"id": "two", "title": "Choice", "summary": "Walks"},
    ]
    outline = asyncio.run(
        store.suggest(
            story["id"],
...
def test_oversized_ready_outline_refuses_work_without_truncation(tmp_path):
    ...
    story = ready(store)
    beats = [
        {"id": f"beat-{index}", "title": f"Beat {index}", "summary": "x" * 2000}
