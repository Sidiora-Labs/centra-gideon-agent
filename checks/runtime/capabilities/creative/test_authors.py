...
def test_author_lifecycle_revisions_restore_and_deterministic_brief(tmp_path):
    ...
    second = store.update(
        first["id"], {"revision": 1, "voice": {**first["voice"], "tone": "Urgent"}}
    )
    ...
    assert reopened.get(first["id"])["sample_status"] == [
        {**ref, "missing": False, "title": artifact.name}
    ]
...
def test_pinned_samples_use_actual_version_and_bounded_excerpt(tmp_path):
    ...
    store = AuthorStore(tmp_path)
    author = store.create(
        payload(sample_refs=[{"artifact_id": artifact.slug, "artifact_version": 1}])
    )
    ...
    restored = store.restore(
        author["id"], {"revision": updated["revision"], "target_revision": 1}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "sample_refs": [
                {"artifact_id": "missing", "artifact_version": 1, "home": "override"}
            ]
        },
        {"provider": "override"},
        {"model": "override"},
...
def test_sample_rejection_two_homes_and_nontext_sources(tmp_path):
    ...
    binary = artifacts.create_binary(
        name="Binary image", data=b"\x89PNG\r\n\x1a\n", mime="image/png"
    )
    with pytest.raises(CatalogError):
        one.create(
            payload(sample_refs=[{"artifact_id": binary.slug, "artifact_version": 1}])
        )
    assert [entry["id"] for entry in one.sources()["items"]] == [artifact.slug]
    with pytest.raises(CatalogError):
        one.create(
            payload(
                sample_refs=[{"artifact_id": artifact.slug, "artifact_version": 999}]
            )
        )
    ...
    async def run():
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            edited = await client.patch(
                path, json={"revision": 1, "title": "HTTP author"}
            )
        ...
        assert (await call("sources", {"q": "writing"}))["items"][0][
            "id"
        ] == artifact.slug
        edited = await call(
            "update",
            {"id": native["id"], "payload": {"revision": 1, "title": "Native updated"}},
        )
        ...
        restored = await call(
            "restore",
            {"id": native["id"], "payload": {"revision": 2, "target_revision": 1}},
        )
        assert (await call("brief", {"id": native["id"]}))["revision"] == restored[
            "revision"
        ]
