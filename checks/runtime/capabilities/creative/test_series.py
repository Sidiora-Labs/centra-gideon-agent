...
def payload(**changes):
    return {'request_id': str(uuid4()), 'title': 'Night journeys', 'synopsis': 'A traveler searches for home.',
            'volumes': [{'id': 'volume-one', 'title': 'Departure', 'chapters': [
                {'id': 'chapter-one', 'title': 'Station', 'prompt': 'Begin at a station.'},
                {'id': 'chapter-two', 'title': 'Walk', 'prompt': 'Continue along the tracks.'}]}],
            'arcs': [{'id': 'home', 'title': 'Homecoming', 'summary': 'Learn where home is.', 'chapter_ids': ['chapter-one', 'chapter-two']}], **changes}


def write(store, series, chapter='chapter-one', work_revision=1, **changes):
    data = {'request_id': str(uuid4()), 'revision': series['revision'], 'work_revision': work_revision,
            'mode': 'authored', 'text': 'The last train arrived.\n', **changes}
...
def test_staged_authored_drafts_review_unlock_and_upstream_invalidation(tmp_path):
    ...
    reviewed = store.review(series['id'], 'chapter-one', {'revision': 1, 'work_revision': 2})
    ...
    second = write(store, series, chapter='chapter-two', text='She followed the tracks.')
    assert second['work']['id'] == second_work['id']
    store.review(series['id'], 'chapter-two', {'revision': 1, 'work_revision': 2})
    assert [s['stage'] for s in store.get(series['id'])['chapter_status']] == ['reviewed', 'reviewed']
    ...
    assert store.works.read_draft(first_work['id'], first['draft']['id'])['text'] == 'The last train arrived.\n'
...
def test_missing_draft_invalidates_review_and_prevents_next_stage(tmp_path):
    ...
    store.review(series['id'], 'chapter-one', {'revision': 1, 'work_revision': 2})
    version = tmp_path / 'artifacts' / result['draft']['artifact_id'] / 'versions' / 'v1.html'
    version.unlink()
    status = store.get(series['id'])['chapter_status']
...
def test_draft_request_replay_before_current_revision_and_concurrent_calls(tmp_path):
    ...
    series = store.create(payload())
    store.prepare(series['id'], 'chapter-one', {'revision': 1})
    request = {'request_id': 'retry', 'revision': 1, 'work_revision': 1, 'mode': 'authored', 'text': 'First prose'}
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(lambda _: asyncio.run(store.draft(series['id'], 'chapter-one', request)), range(2)))
    ...
    with pytest.raises(CatalogError) as conflict:
        asyncio.run(store.draft(series['id'], 'chapter-one', {**request, 'text': 'Different'}))
...
def test_real_prepare_and_draft_sqlite_failure_rollback(tmp_path):
    ...
    with store.connection() as db:
        db.execute("CREATE TRIGGER stop_prepare BEFORE INSERT ON series_chapters BEGIN SELECT RAISE(ABORT, 'prepare fail'); END")
    ...
    with store.connection() as db:
        db.execute("CREATE TRIGGER stop_receipt BEFORE INSERT ON series_draft_requests BEGIN SELECT RAISE(ABORT, 'receipt fail'); END")
    request = {'request_id': 'recover', 'revision': 1, 'work_revision': 1, 'mode': 'authored', 'text': 'Recovered text'}
    with pytest.raises(sqlite3.IntegrityError):
...
@pytest.mark.parametrize('changes', [{'volumes': [{}] * 21}, {'arcs': [{}] * 101}, {'title': ''}, {'synopsis': 'x' * 8001}, {'home': 'other'}, {'provider': 'override'}, {'arcs': [{'id': 'arc', 'title': 'Bad', 'chapter_ids': ['missing']}]}])
...
def test_real_http_and_native_series_paths(tmp_path):
    async def run():
        ...
        async with TestClient(TestServer(app)) as client:
            response = await client.post('/api/capabilities/creative/series', json=payload())
            assert response.status == 201
            series = await response.json()
            path = f"/api/capabilities/creative/series/{series['id']}/chapters/chapter-one"
            prepared = await client.post(path + '/prepare', json={'revision': 1})
            assert prepared.status == 200
            drafted = await client.post(path + '/draft', json={'request_id': 'http', 'revision': 1, 'work_revision': 1, 'mode': 'authored', 'text': 'HTTP manuscript'})
            assert drafted.status == 200
            reviewed = await client.post(path + '/review', json={'revision': 1, 'work_revision': 2})
        ...
        await call('prepare', {'id': series['id'], 'chapter_id': 'chapter-one', 'payload': {'revision': 1}})
        result = await call('draft', {'id': series['id'], 'chapter_id': 'chapter-one', 'payload': {'request_id': 'native', 'revision': 1, 'work_revision': 1, 'mode': 'authored', 'text': 'Native manuscript'}})
        assert result['work']['revision'] == 2
        await call('review', {'id': series['id'], 'chapter_id': 'chapter-one', 'payload': {'revision': 1, 'work_revision': 2}})
        changed = await call('update', {'id': series['id'], 'payload': {'revision': 1, 'title': 'Updated series'}})
        ...
        assert (await call('restore', {'id': series['id'], 'payload': {'revision': 2, 'target_revision': 1}}))['title'] == series['title']
    asyncio.run(run())
...
def test_actual_model_drafting_outcome_separate_from_authored_evidence(tmp_path):
    ...
    series = store.create(payload())
    work = store.prepare(series['id'], 'chapter-one', {'revision': 1})
    report = {'task': 'creative.09', 'actual_provider_attempt': True, 'inference_qualified': False}
    try:
        result = asyncio.run(store.draft(series['id'], 'chapter-one', {'request_id': 'model', 'revision': 1, 'work_revision': 1, 'mode': 'model', 'instruction': 'Write a short opening paragraph.'}))
    ...
    Path('/tmp/gideon-creative-09-inference-oss.json').write_text(json.dumps(report, indent=2) + '\n')


def test_prior_context_is_bounded_to_three_actual_reviewed_chapters(tmp_path):
    store = SeriesStore(tmp_path)
    chapters = [{'id': f'c-{index}', 'title': f'Chapter {index}', 'prompt': f'Prompt {index}'} for index in range(5)]
    series = store.create(payload(volumes=[{'id': 'volume', 'title': 'Book', 'chapters': chapters}], arcs=[]))
    for index in range(5):
        store.prepare(series['id'], f'c-{index}', {'revision': 1})
    for index in range(4):
        result = write(store, series, chapter=f'c-{index}', text=f'Canonical chapter {index}')
        assert result['work']['revision'] == 2
    ...
    assert [item['chapter_id'] for item in context['prior_reviewed']] == ['c-1', 'c-2', 'c-3']
    assert [item['text'] for item in context['prior_reviewed']] == ['Canonical chapter 1', 'Canonical chapter 2', 'Canonical chapter 3']
...
def test_series_drafting_validation_rejects_before_any_manuscript_mutation(tmp_path):
    ...
    series = store.create(payload())
    work = store.prepare(series['id'], 'chapter-one', {'revision': 1})
    base = {'request_id': 'request', 'revision': 1, 'work_revision': 1, 'mode': 'authored', 'text': 'Valid text'}
    for changes in ({'home': 'other'}, {'provider': 'override'}, {'model': 'override'}, {'mode': 'invalid'},
                    {'mode': 'model', 'text': 'Pretend model text'}, {'text': ''}, {'work_revision': True}, {'revision': 2}):
        with pytest.raises(CatalogError):
            asyncio.run(store.draft(series['id'], 'chapter-one', {**base, **changes}))
        assert store.works.get(work['id'])['revision'] == 1
        assert store.works.drafts(work['id'])['items'] == []
    assert store.works.artifacts.list() == []
    for action in (lambda: store.prepare(series['id'], 'missing', {'revision': 1}),
                   lambda: store.review(series['id'], 'chapter-one', {'revision': 1, 'work_revision': 1}),
                   lambda: store.prepare(series['id'], 'chapter-one', {'revision': 1, 'session_id': 'private'})):
        with pytest.raises(CatalogError):
            action()
