...
from gideon.workspace.capabilities.wellbeing.privacy_broker_whitepages import (
    RECIPIENT,
    WhitepagesCaseAdapter,
)
...
def setup(tmp_path, smtp):
    ...
    case = store.apply_adapter_scan(
        case["id"], {"request_id": "scan", "revision": case["revision"]}, found
    )
    ...
    transport = SMTPTransport(
        host="127.0.0.1", port=smtp.port, starttls=False, authenticate=False
    )
    ...
    return (
        ...
        account,
        case,
        WhitepagesCaseAdapter(store, communications, outbound),
    )
...
def prepare(service, account, case, request="prepare"):
    return service.prepare(
        case["id"],
        {
            ...
            "profile_url": "https://www.whitepages.com/name/Jane-Doe/Oakland-CA/abc",
            "jurisdiction": "US-CA",
        },
    )
...
def approve(service, result):
    return service.approve(
        result["case"]["id"],
        {
            ...
            "content_sha256": result["draft"]["content_sha256"],
            "confirm_exact": True,
        },
    )
...
@pytest.fixture
def smtp():
    value = SMTPContract().start()
    yield value
    value.close()


def test_prepare_creates_exact_canonical_draft_without_network_and_binds_case(
    tmp_path, smtp
):
    store, _, account, case, service = setup(tmp_path, smtp)
    result = prepare(service, account, case)
    assert (
        result["case"]["state"] == "optout_in_progress"
        and result["case"]["revision"] == 3
    )
    draft = result["draft"]
    assert draft["state"] == "draft" and draft["sender"] == "owner@example.com"
    assert (
        draft["to"] == [RECIPIENT] and draft["provider_acceptance"] == "not_submitted"
    )
    assert (
        "Jane Doe" in draft["body"]
        and "https://www.whitepages.com/name/Jane-Doe/Oakland-CA/abc" in draft["body"]
    )
    assert "CCPA section 1798.105" in draft["body"]
    assert result["plan"] == {
        "recipient": RECIPIENT,
        "requires_exact_content_approval": True,
        "message_sent": False,
        "external_verification_required": True,
    }
    assert smtp.messages == []
    persisted = store.get_case(case["id"])
    assert persisted["evidence"].startswith(
        "outbound-email:" + draft["id"] + ":sha256:"
    )
    assert "owner@example.com" not in json.dumps(store.events(case["id"]))


def test_exact_approval_and_separate_send_use_real_smtp_and_keep_delivery_uncertain(
    tmp_path, smtp
):
    ...
    assert (
        sent["draft"]["state"] == "accepted"
        and sent["draft"]["provider_acceptance"] == "accepted"
    )
    ...
    assert "Whitepages removal and deletion request for Jane Doe" in encoded
    assert "opt it out of sale or sharing" in encoded
    replay = send(service, sent, "send")
    assert replay["case"]["state"] == "submitted" and len(smtp.messages) == 1


def test_account_identity_profile_and_case_provider_fail_closed_before_email(
    tmp_path, smtp
):
    store, communications, account, case, service = setup(tmp_path, smtp)
    for changes, match in [
        ({"contact_email": "other@example.com"}, "match the selected account"),
        ({"profile_url": "https://attacker.example/jane"}, "Whitepages listing"),
    ]:
        payload = {
            "request_id": "bad-" + match[:3],
            "revision": case["revision"],
            "account_id": account["id"],
            "full_name": "Jane Doe",
            "contact_email": "owner@example.com",
            "profile_url": "https://www.whitepages.com/name/jane",
            "jurisdiction": "US-OTHER",
        }
        payload.update(changes)
        with pytest.raises(MeasurementError, match=match):
            service.prepare(case["id"], payload)
    local = mirrors.save_account(
        communications,
        {
            "name": "Local",
            "kind": "maildir",
    ...
    other_case = store.create_case(
        store.get_case(case["id"])["subject_id"],
        {"request_id": "other-case", "broker_id": other["id"]},
    )
    with pytest.raises(MeasurementError, match="Whitepages cases only"):
        service.prepare(
            other_case["id"],
            {
                "request_id": "wrong",
                "revision": other_case["revision"],
                "account_id": account["id"],
                "full_name": "Jane Doe",
                "contact_email": "owner@example.com",
                "profile_url": "https://www.whitepages.com/name/jane",
...
def test_uncertain_transport_is_preserved_and_never_claims_delivery_or_removal(
    tmp_path,
):
    smtp = SMTPContract(drop=True).start()
    try:
        _, _, account, case, service = setup(tmp_path, smtp)
        result = approve(service, prepare(service, account, case))
        sent = send(service, result)
        assert (
            sent["draft"]["state"] == "uncertain"
            and sent["draft"]["provider_acceptance"] == "uncertain"
        )
        assert (
            sent["draft"]["delivery"] == "uncertain"
            and sent["case"]["state"] == "submitted"
        )
        assert sent["case"]["state"] != "confirmed_removed"
        replay = send(service, sent)
        assert replay["draft"]["state"] == "uncertain" and len(smtp.messages) == 1
    finally:
        smtp.close()
...
def test_correlation_matches_manual_code_from_canonical_reply_and_never_opens_link(
    tmp_path, smtp, monkeypatch
):
    _, communications, account, case, service = setup(tmp_path, smtp)
    sent = send(service, approve(service, prepare(service, account, case)))
    inbound = ingest_reply(
        communications,
        account,
        sent["draft"],
        "Your verification code is 482913. Review https://untrusted.example/continue",
    )
    opened = []
    monkeypatch.setattr(
        "urllib.request.urlopen", lambda *args, **kwargs: opened.append(args)
    )
    with pytest.raises(MeasurementError, match="does not match"):
        service.correlate(
            case["id"], {"revision": sent["case"]["revision"], "manual_code": "1111"}
        )
    result = service.correlate(
        case["id"], {"revision": sent["case"]["revision"], "manual_code": "482913"}
    )
    assert result["verification"] == {
        "reply_correlated": True,
        "manual_code_matched": True,
        "removal_confirmed": False,
        "links_opened": False,
    }
    assert (
        result["draft"]["verification"]["message_external_id"] == inbound["external_id"]
    )
    assert result["draft"]["verification"]["links"] == [
        {"url": "https://untrusted.example/continue", "opened": False}
    ]
    assert result["case"]["state"] == "submitted" and opened == []


@pytest.mark.asyncio
async def test_real_http_handler_drives_prepare_approve_send_and_correlation(
    tmp_path, smtp
):
    _, communications, account, case, service = setup(tmp_path, smtp)
    app = web.Application()
    register(app, adapter=service)
    base = f'/api/capabilities/wellbeing/privacy/broker-cases/{case["id"]}/providers/whitepages'
    async with TestClient(TestServer(app)) as client:
        response = await client.post(
            base + "/prepare",
            json={
                "request_id": "http-prepare",
                "revision": case["revision"],
                "account_id": account["id"],
                "full_name": "Jane Doe",
                "contact_email": "owner@example.com",
        ...
        response = await client.post(
            ...
            json={
                ...
                "revision": prepared["case"]["revision"],
                "draft_revision": prepared["draft"]["revision"],
                "content_sha256": prepared["draft"]["content_sha256"],
                "confirm_send": True,
            },
        )
        ...
        response = await client.post(
            base + "/send",
            json={
                "request_id": "http-send",
                "revision": approved["case"]["revision"],
                "draft_revision": approved["draft"]["revision"],
                "content_sha256": approved["draft"]["content_sha256"],
                "confirm_send": True,
            },
        )
        ...
        assert (
            response.status == 200
            and sent["draft"]["delivery"] == "uncertain"
            and len(smtp.messages) == 1
        )
        ingest_reply(
            communications,
            account,
            sent["draft"],
            "Code 7654. https://untrusted.example/x",
        )
        response = await client.post(
            base + "/correlate",
            json={"revision": sent["case"]["revision"], "manual_code": "7654"},
        )
        correlated = await response.json()
        assert (
            response.status == 200
            and correlated["verification"]["manual_code_matched"] is True
        )
