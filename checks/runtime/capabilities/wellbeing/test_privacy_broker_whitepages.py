...
def setup(tmp_path, smtp):
    ...
    broker=store.create_broker({'request_id':'broker','name':'Whitepages','website':'https://www.whitepages.com','optout_url':'https://www.whitepages.com/suppression_requests','source':'curated donor protocol'})
    ...
    return store,communications,account,case,WhitepagesCaseAdapter(store,communications,outbound)


def prepare(service, account, case, request='prepare'):
    return service.prepare(case['id'],{'request_id':request,'revision':case['revision'],'account_id':account['id'],'full_name':'Jane Doe','contact_email':'owner@example.com','profile_url':'https://www.whitepages.com/name/Jane-Doe/Oakland-CA/abc','jurisdiction':'US-CA'})
...
def test_prepare_creates_exact_canonical_draft_without_network_and_binds_case(tmp_path,smtp):
    ...
    assert 'Jane Doe' in draft['body'] and 'https://www.whitepages.com/name/Jane-Doe/Oakland-CA/abc' in draft['body']
    assert 'CCPA section 1798.105' in draft['body']
    assert result['plan']=={'recipient':RECIPIENT,'requires_exact_content_approval':True,'message_sent':False,'external_verification_required':True}
...
def test_account_identity_profile_and_case_provider_fail_closed_before_email(tmp_path,smtp):
    ...
    for changes,match in [({'contact_email':'other@example.com'},'match the selected account'),({'profile_url':'https://attacker.example/jane'},'Whitepages listing')]:
        payload={'request_id':'bad-'+match[:3],'revision':case['revision'],'account_id':account['id'],'full_name':'Jane Doe','contact_email':'owner@example.com','profile_url':'https://www.whitepages.com/name/jane','jurisdiction':'US-OTHER'}; payload.update(changes)
    ...
    with pytest.raises(MeasurementError,match='credential-bound'):
        service.prepare(case['id'],{'request_id':'local','revision':case['revision'],'account_id':local['id'],'full_name':'Jane Doe','contact_email':'owner@example.com','profile_url':'https://www.whitepages.com/name/jane','jurisdiction':'US-OTHER'})
    ...
    with pytest.raises(MeasurementError,match='Whitepages cases only'):
        service.prepare(other_case['id'],{'request_id':'wrong','revision':other_case['revision'],'account_id':account['id'],'full_name':'Jane Doe','contact_email':'owner@example.com','profile_url':'https://www.whitepages.com/name/jane','jurisdiction':'US-OTHER'})
    assert smtp.messages==[]
...
async def test_real_http_handler_drives_prepare_approve_send_and_correlation(tmp_path,smtp):
    ...
    async with TestClient(TestServer(app)) as client:
        response=await client.post(base+'/prepare',json={'request_id':'http-prepare','revision':case['revision'],'account_id':account['id'],'full_name':'Jane Doe','contact_email':'owner@example.com','profile_url':'https://www.whitepages.com/name/jane','jurisdiction':'US-OTHER'})
