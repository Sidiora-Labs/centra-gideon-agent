...
def setup_records(home):
    ...
    privacy.consent(
        person["id"],
        {
            ...
            "scope": "vault",
            "granted": True,
            "method": "console confirmation",
        },
    )
    old = privacy.create_fact(
        person["id"],
        {
            "request_id": "fact",
            "type": "email",
...
def create_org(store, subject, request="org", name="Example Bank", **changes):
    payload = {
        ...
        "name": name,
        "category": "finance",
        "website": "https://bank.example/account",
        "contact": "privacy@bank.example",
        "source": "owner supplied",
    }
...
def test_revisioned_organizations_require_vault_consent_and_validate_metadata(tmp_path):
    ...
    with pytest.raises(MeasurementError) as stale:
        store.update_org(
            row["id"], {"request_id": "stale", "revision": 1, "name": "Stale"}
        )
    assert stale.value.status == 409
    for request, changes in [
        ("bad-scheme", {"website": "file:///private"}),
        ("credentials", {"website": "https://user:pass@example.test"}),
        ("empty-name", {"name": ""}),
        ("bad-archive", {"archived": "yes"}),
        ("extra", {"unexpected": True}),
    ]:
        with pytest.raises(MeasurementError):
            store.update_org(
                row["id"], {"request_id": request, "revision": 2, **changes}
            )
...
def test_holdings_are_subject_scoped_revisioned_and_never_reveal_values(tmp_path):
    ...
    foreign = privacy.create_fact(
        other["id"],
        {
            "request_id": "foreign-fact",
            "type": "phone",
            "label": "Phone",
            "value": "+15555551212",
            "passphrase": PASSWORD,
    ...
    archived_org = store.update_org(
        org["id"], {"request_id": "archive-org", "revision": 1, "archived": True}
    )
...
def test_change_atomically_propagates_to_every_eligible_holding_and_derives_progress(
    tmp_path,
):
    privacy, store, person, old, current = setup_records(tmp_path)
    ...
    second_holding = hold(
        store, second["id"], old, request="holding-two", status="unknown"
    )
    third_holding = hold(
        store, third["id"], old, request="holding-three", status="removed"
    )
    ...
    assert [row["org_id"] for row in change["targets"]] == sorted(
        [first["id"], second["id"]]
    )
    assert all(
        row["attestation"] == "user_reported" and row["evidence"] == ""
        for row in change["targets"]
    )
...
def test_user_attested_dispositions_preserve_evidence_and_histories(tmp_path):
    privacy, store, person, old, current = setup_records(tmp_path)
    updated_org = create_org(
        store, person["id"], request="org-update", name="Updated Bank"
    )
    removed_org = create_org(
        store, person["id"], request="org-remove", name="Removed Utility"
    )
    ...
    update = store.settle(
        ...
        updated_org["id"],
        {
            "request_id": "settle-update",
            "revision": 1,
            "status": "updated",
            "evidence": "Owner received confirmation number 812",
        },
    )
    assert update["progress"] == {"pending": 1, "updated": 1, "removed": 0, "total": 2}
    update_target = next(
        row for row in update["targets"] if row["org_id"] == updated_org["id"]
    )
    ...
    assert [row["status"] for row in store.history_holding(update_holding["id"])] == [
        "held",
        "update_pending",
        "held",
    ]
    ...
    removed = store.settle(
        change["id"],
        removed_org["id"],
        {
            "request_id": "settle-remove",
            "revision": 1,
            "status": "removed",
            "evidence": "Owner verified account deletion",
        },
    )
    ...
    assert [row["status"] for row in store.history_holding(remove_holding["id"])] == [
        "held",
        "update_pending",
        "removed",
    ]
    assert [
        row["status"] for row in store.history_target(change["id"], updated_org["id"])
    ] == ["pending", "updated"]
    assert [
        row["status"] for row in store.history_target(change["id"], removed_org["id"])
    ] == ["pending", "removed"]
    assert (
        store.settle(
            change["id"],
    ...
    events = [
        row
        for row in privacy.audit(person["id"])
        if row["operation"] == "change_disposition"
    ]
...
@pytest.mark.parametrize(
    "start,end", [(0, 2), (1, 1), (2, 1), (1, 3), (True, 2), (1, "2")]
)
...
def test_concurrent_change_declaration_has_one_receipt_and_one_propagation(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        rows = list(
            pool.map(
                lambda _: OrgHoldingsStore(tmp_path).declare_change(
                    person["id"],
                    {
                        "request_id": "same-change",
                        "fact_id": old["id"],
                        "from_revision": 1,
                        "to_revision": 2,
                        "source": "owner",
    ...
    assert [row["status"] for row in store.history_holding(holding["id"])] == [
        "held",
        "update_pending",
    ]
...
@pytest.mark.asyncio
async def test_native_tools_read_metadata_only_and_cannot_attest_mutate_or_reveal(
    tmp_path,
):
    _, store, person, old, _ = setup_records(tmp_path)
    org = create_org(store, person["id"])
    ...
    operations = (await provider.list_tools())[0].parameters["properties"]["operation"][
        "enum"
    ]
    allowed = [
        ("privacy_organizations", person["id"]),
        ("privacy_organization", org["id"]),
        ("privacy_organization_history", org["id"]),
        ("privacy_holdings", org["id"]),
        ("privacy_holding_history", holding["id"]),
        ("privacy_changes", person["id"]),
        ("privacy_change", change["id"]),
    ]
    for operation, identity in allowed:
        assert operation in operations
        result = await provider.invoke(
            "wellbeing_records", {"operation": operation, "id": identity}
        )
    ...
    for operation in (
        "privacy_organization_create",
        "privacy_holding_set",
        "privacy_change_declare",
        "privacy_change_settle",
        "privacy_change_draft",
    ):
        result = await provider.invoke(
            "wellbeing_records",
            {
                "operation": operation,
                "id": change["id"],
                "payload": {"passphrase": PASSWORD, "value": NEW_VALUE},
            },
        )
...
@pytest.mark.asyncio
async def test_real_http_workflow_is_home_scoped_no_store_and_has_no_send_route(
    tmp_path,
):
    ...
    async with (
        ...
        holding = await (
            await client.post(
                base + "/organizations/" + org["id"] + "/holdings",
                json={
                    "request_id": "holding",
                    "revision": 0,
                    "fact_id": first["id"],
                    "fact_revision": first["revision"],
                    "status": "held",
                    "source": "owner",
                },
            )
        ...
        response = await client.post(
            base + "/changes/" + change["id"] + "/organizations/" + org["id"],
            json={
                "request_id": "settle",
                "revision": 1,
                "status": "updated",
                "evidence": "Owner saw confirmation",
            },
        )
        assert response.status == 200
        assert response.headers["Cache-Control"] == "no-store"
        ...
        assert (
            len(
                (
                    await (
                        await client.get(
                            base + "/organizations/" + org["id"] + "/history"
                        )
                    ).json()
                )["history"]
            )
        ...
        assert (
            len(
                (
                    await (
                        await client.get(
                            base + "/holdings/" + holding["id"] + "/history"
                        )
                    ).json()
                )["history"]
            )
