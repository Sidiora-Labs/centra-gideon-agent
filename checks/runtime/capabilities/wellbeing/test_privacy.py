...
from gideon.automation.workflows.project_archive import is_encrypted, decrypt_archive
from gideon.interfaces.dashboard.handlers.capabilities_wellbeing_privacy import register
from gideon.workspace.capabilities.wellbeing.privacy import PrivacyStore, SCOPES
...
def subject(store, request='subject', **changes):
    return store.create_subject(dict(request_id=request, alias='Household alias', relationship='household', source='owner declaration', **changes))


def grant(store, identity, scope='vault', granted=True, revision=0, request=None):
    return store.consent(identity, dict(request_id=request or scope + str(revision), revision=revision, scope=scope, granted=granted, method='explicit local consent'))


def fact(**changes):
    value = dict(request_id='fact', type='email', label='Personal email', value=VALUE, passphrase=PASSWORD, source='directly supplied', use_for_scans=False)
...
def test_subject_scoped_consent_required_revoked_and_audited(tmp_path):
    ...
    with pytest.raises(MeasurementError, match='consent'):
        store.create_fact(person['id'], fact())
    with pytest.raises(MeasurementError, match='consent'):
        store.correct_fact(row['id'], dict(request_id='correct', revision=1, value='new', passphrase=PASSWORD))
    ...
    assert [event['operation'] for event in events] == ['subject_created', 'consent_recorded', 'fact_written', 'consent_recorded']
    assert events[-1]['granted'] is False
    assert VALUE not in json.dumps(events)
    assert PASSWORD not in json.dumps(events)


def test_real_authenticated_encryption_masks_every_read_and_plaintext_never_persists(tmp_path):
    ...
    with store.connection() as db:
        ...
        kind, receipt, result = db.execute("SELECT kind,payload,result FROM privacy_requests WHERE id='fact'").fetchone()
...
def test_corrections_preserve_encrypted_history_require_old_key_and_archive(tmp_path):
    ...
    row = store.create_fact(person['id'], fact())
    grant(store, person['id'], 'reveal')
    payload = dict(request_id='correction', revision=1, value='new-private-value@example.test', passphrase=PASSWORD, label='Updated email', use_for_scans=True)
    with pytest.raises(MeasurementError, match='authenticated'):
        store.correct_fact(row['id'], dict(payload, passphrase='different valid long password'))
    ...
    assert store.reveal(row['id'], dict(passphrase=PASSWORD, reason='Verify correction'))['value'] == payload['value']
    with pytest.raises(MeasurementError, match='changed'):
        store.correct_fact(row['id'], dict(payload, request_id='stale'))
    archived = store.correct_fact(row['id'], dict(request_id='archive', revision=2, value=payload['value'], passphrase=PASSWORD, archived=True))
    assert archived['archived'] is True
    with pytest.raises(MeasurementError, match='Archived'):
        store.reveal(row['id'], dict(passphrase=PASSWORD, reason='Should refuse archive'))
...
def test_ciphertext_tamper_and_swapping_across_facts_are_detected(tmp_path):
    ...
    second = store.create_fact(person['id'], fact(request_id='second', value='other-private@example.test'))
    grant(store, person['id'], 'reveal')
    with store.connection() as db:
        original = bytes(db.execute('SELECT cipher FROM privacy_facts WHERE id=?', (first['id'],)).fetchone()[0])
        other = bytes(db.execute('SELECT cipher FROM privacy_facts WHERE id=?', (second['id'],)).fetchone()[0])
    ...
    with store.connection() as db:
        ...
        db.execute('UPDATE privacy_facts SET cipher=? WHERE id=?', (altered, first['id']))
    with pytest.raises(MeasurementError, match='authenticated'):
        store.reveal(first['id'], dict(passphrase=PASSWORD, reason='Integrity check'))
    assert store.audit(person['id'])[-1]['outcome'] == 'denied'
    assert store.reveal(second['id'], dict(passphrase=PASSWORD, reason='Unaffected record'))['value'] == 'other-private@example.test'
...
def test_subject_home_isolation_and_no_health_export_or_artifact_projection(tmp_path):
    ...
    for secret in (VALUE, PASSWORD, person['id'], person['alias'], row['id'], row['label']):
        assert secret.encode() not in raw
    assert all(count == 0 for count in metadata['counts'].values())
    assert metadata['attachment_count'] == 0


@pytest.mark.parametrize('change', [dict(type='unknown'), dict(type=[]), dict(type='tax_id', use_for_scans=True), dict(type='passport', use_for_scans=True), dict(use_for_scans='yes'), dict(label=''), dict(value=''), dict(source=''), dict(passphrase='short'), dict(value={'bad': True}), dict(home='/elsewhere')])
...
@pytest.mark.parametrize('scope,granted,revision', [('unknown', True, 0), ([], True, 0), ('vault', 'yes', 0), ('vault', True, True), ('vault', True, -1)])
def test_invalid_consent_is_not_granted(tmp_path, scope, granted, revision):
    store = PrivacyStore(tmp_path)
    person = subject(store)
    with pytest.raises(MeasurementError):
        store.consent(person['id'], dict(request_id='bad', scope=scope, granted=granted, revision=revision, method='owner'))
    assert store.consents(person['id']) == []


def test_private_request_conflict_is_authenticated_and_concurrent_retry_single_revision(tmp_path):
    store, person = ready(tmp_path)
    with ThreadPoolExecutor(max_workers=3) as pool:
        rows = list(pool.map(lambda _: PrivacyStore(tmp_path).create_fact(person['id'], fact()), range(3)))
    ...
    with pytest.raises(MeasurementError, match='already used'):
        store.create_subject(dict(request_id='fact', alias='other', relationship='self', source='owner'))
    assert VALUE.encode() not in store.path.read_bytes()
...
async def test_agent_tools_are_metadata_only_without_secret_parameters(tmp_path):
    ...
    for operation, identity in [('privacy_subjects', None), ('privacy_subject', person['id']), ('privacy_consents', person['id']), ('privacy_facts', person['id']), ('privacy_fact', row['id']), ('privacy_fact_history', row['id']), ('privacy_audit', person['id'])]:
        result = await provider.invoke('wellbeing_records', dict(operation=operation, id=identity))
        assert result.success
        assert VALUE not in result.output
        assert PASSWORD not in result.output
        assert 'cipher' not in result.output
    for operation in ('privacy_reveal', 'privacy_create_fact', 'privacy_fact'):
        result = await provider.invoke('wellbeing_records', dict(operation=operation, id=row['id'], payload={'passphrase': PASSWORD, 'value': VALUE}))
...
async def test_real_http_owner_consent_create_correct_reveal_and_wrong_home(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client, TestClient(TestServer(other)) as isolated:
        response = await client.post(base + '/subjects', json=dict(request_id='subject', alias='Self alias', relationship='self', source='owner'))
        ...
        assert (await client.post(subject_url + '/consents', json=dict(request_id='vault', revision=0, scope='vault', granted=True, method='local consent'))).status == 200
        response = await client.post(subject_url + '/facts', json=fact())
        assert response.status == 200
        row = await response.json()
        assert VALUE not in json.dumps(row)
        fact_url = base + '/facts/' + row['id']
        assert (await client.post(fact_url + '/reveal', json=dict(passphrase=PASSWORD, reason='check'))).status == 403
        assert (await client.post(subject_url + '/consents', json=dict(request_id='reveal', revision=0, scope='reveal', granted=True, method='local consent'))).status == 200
        response = await client.post(fact_url + '/reveal', json=dict(passphrase=PASSWORD, reason='check'))
        assert response.status == 200
        assert response.headers['Cache-Control'] == 'no-store'
        assert (await response.json())['value'] == VALUE
        response = await client.put(fact_url, json=dict(request_id='correct', revision=1, value='replacement@example.test', passphrase=PASSWORD))
        assert response.status == 200
        assert (await response.json())['revision'] == 2
        assert len((await (await client.get(fact_url + '/history')).json())['history']) == 2
        ...
        assert (await isolated.post(fact_url + '/reveal', json=dict(passphrase=PASSWORD, reason='wrong home'))).status == 404
        assert (await isolated.get(subject_url)).status == 404
        assert (await client.post(base + '/subjects', data='{', headers={'Content-Type': 'application/json'})).status == 400
        assert (await client.post(fact_url + '/reveal', json=dict(passphrase=PASSWORD, reason='check', home=str(tmp_path / 'b')))).status == 400
