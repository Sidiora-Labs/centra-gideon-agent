...
from gideon.workspace.capabilities.wellbeing.intervention import InterventionStore, calendar_date
...
def plan(**changes):
    value = dict(request_id='plan-1', name='Evening walk', kind='activity', instructions='My planned walk', source='personal plan', timezone='Europe/Berlin', start_date='2026-09-20', end_date=None, weekdays=[0, 1, 2, 3, 4, 5, 6])
    value.update(changes)
    return value


def record(**changes):
    value = dict(request_id='record-1', date='2026-09-24', status='completed', observed_at='2026-09-24T20:00:00+02:00', notes='logged observation')
    value.update(changes)
    return value
...
def test_plan_persistence_history_and_immutable_schedule(tmp_path):
    ...
    updated = store.update_plan(row['id'], dict(request_id='edit', revision=1, name='My evening walk', instructions='Updated personal instructions'))
    ...
    for key, value in [('source', 'changed'), ('timezone', 'UTC'), ('start_date', '2026-09-22'), ('kind', 'other'), ('weekdays', [1])]:
        with pytest.raises(MeasurementError, match='immutable'):
            store.update_plan(row['id'], dict(request_id=key, revision=2, **{key: value}))
...
def test_observation_corrections_retry_day_identity_and_provenance(tmp_path):
    ...
    correction = dict(request_id='correct', revision=1, status='skipped', notes='corrected observation')
    ...
    with pytest.raises(MeasurementError, match='immutable'):
        store.correct_record(row['id'], dict(request_id='date', revision=2, date='2026-09-25'))
...
def test_summary_preserves_unrecorded_skipped_and_excludes_future_observation(tmp_path):
    ...
    skipped = store.record(parent['id'], record(request_id='skip', date='2026-09-23', status='skipped'))
    future = store.record(parent['id'], record(request_id='future', date='2026-09-25', observed_at='2026-09-25T20:00:00+02:00'))
...
def test_timezone_schedule_boundaries_dst_and_archive_corrections(tmp_path):
    store = InterventionStore(tmp_path)
    parent = store.create_plan(plan(start_date='2026-10-24', end_date='2026-10-26', weekdays=[6]))
    row = store.record(parent['id'], record(date='2026-10-25', observed_at='2026-10-24T23:30:00Z'))
    result = store.summary(parent['id'], days=3, as_of='2026-10-26T12:00:00Z')
    assert [day['date'] for day in result['days']] == ['2026-10-24', '2026-10-25', '2026-10-26']
    ...
    with pytest.raises(MeasurementError, match='outside'):
        store.record(parent['id'], record(request_id='offday', date='2026-10-26', observed_at='2026-10-26T12:00:00Z'))
    archived = store.update_plan(parent['id'], dict(request_id='archive', revision=1, archived=True))
    ...
    corrected = store.correct_record(row['id'], dict(request_id='after-archive', revision=1, status='skipped'))
    assert corrected['status'] == 'skipped'
    assert store.summary(parent['id'], days=3, as_of='2026-10-26T12:00:00Z')['skipped_days'] == 1
    unarchived = store.update_plan(parent['id'], dict(request_id='unarchive', revision=2, archived=False))
    assert store.list_plans() == [unarchived]
    assert len(store.history_plan(parent['id'])) == 3


@pytest.mark.parametrize('changes', [dict(kind='unknown'), dict(name=''), dict(instructions=''), dict(source=''), dict(timezone='Unknown/Zone'), dict(timezone=None), dict(start_date='20260920'), dict(start_date='2026-02-30'), dict(end_date='2026-09-19'), dict(weekdays=[]), dict(weekdays=[1, 1]), dict(weekdays=[True]), dict(weekdays=[7]), dict(weekdays='daily'), dict(extra=1)])
...
@pytest.mark.parametrize('changes', [dict(date='2026-09-19'), dict(date='invalid'), dict(status='unknown'), dict(observed_at='2026-09-24T20:00:00'), dict(observed_at='2026-09-23T20:00:00Z'), dict(notes=None), dict(notes='x' * 4001), dict(request_id=''), dict(source='changed')])
...
def test_real_concurrency_cross_home_and_input_errors(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        plans = list(pool.map(lambda _: InterventionStore(tmp_path / 'one').create_plan(plan()), range(4)))
    assert plans.count(plans[0]) == 4
    with ThreadPoolExecutor(max_workers=4) as pool:
        rows = list(pool.map(lambda _: InterventionStore(tmp_path / 'one').record(plans[0]['id'], record()), range(4)))
    ...
    for method, identity in [(other.get_plan, plans[0]['id']), (other.history_plan, plans[0]['id']), (other.list_records, plans[0]['id']), (other.get_record, rows[0]['id']), (other.history_record, rows[0]['id']), (other.summary, plans[0]['id'])]:
    ...
    with pytest.raises(MeasurementError, match='archive'):
        store.update_plan(plans[0]['id'], dict(request_id='bad-archive', revision=1, archived='true'))
...
async def test_real_http_lifecycle_and_validation(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.put(item, json=dict(request_id='correct', revision=1, status='skipped'))
        assert response.status == 200
        ...
        response = await client.get(path + '/summary', params={'days': 3, 'as_of': '2026-09-25T12:00:00+02:00'})
        summary = await response.json()
        assert summary['skipped_days'] == 1
        assert summary['unrecorded_days'] == 2
        response = await client.put(path, json=dict(request_id='archive', revision=1, archived=True))
        assert response.status == 200
        ...
        for suffix in ['/plans?include_archived=invalid', '/plans/' + parent['id'] + '/summary?days=invalid']:
            assert (await client.get(base + suffix)).status == 400
        assert (await client.post(base + '/plans', data='broken', headers={'Content-Type': 'application/json'})).status == 400
...
async def test_native_operations_share_ledger_and_revisions(tmp_path):
    ...
    async def invoke(name, identity=None, data=None):
        result = await provider.invoke('wellbeing_records', dict(operation='intervention_' + name, id=identity, payload=data or {}))
    ...
    updated = await invoke('update_plan', parent['id'], dict(request_id='rename', revision=1, name='Walk'))
    ...
    corrected = await invoke('correct_record', row['id'], dict(request_id='correction', revision=1, status='skipped'))
    assert await invoke('history_record', row['id']) == [row, corrected]
    result = await invoke('summary', parent['id'], dict(days=3, as_of='2026-09-25T12:00:00Z'))
    assert result['skipped_days'] == 1
    ...
    failed = await provider.invoke('wellbeing_records', dict(operation='intervention_missing'))
    assert not failed.success
    assert 'Unknown' in failed.error
    assert 'intervention_record' in (await provider.list_tools())[0].parameters['properties']['operation']['enum']
