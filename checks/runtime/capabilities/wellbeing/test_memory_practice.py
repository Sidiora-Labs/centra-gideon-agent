...
from gideon.workspace.capabilities.wellbeing.memory_practice import (
    MemoryPracticeStore,
    next_schedule,
)
...
def test_canonical_artifact_content_ledger_reference_and_reopen(tmp_path):
    ...
    assert json.loads(artifact.content) == dict(
        front=row["front"], back="Paris", tags=["geography"]
    )
    assert (
    ...
    with store.connection() as db:
        data = json.loads(
            db.execute("SELECT data FROM memory_card_revisions").fetchone()[0]
        )
...
def test_content_revisions_remain_pinned_and_source_immutable(tmp_path):
    ...
    payload = dict(
        request_id="edit",
        revision=1,
        front="Capital city of France?",
        tags=["geography", "review"],
    )
    ...
    assert (
        json.loads(
            store.artifacts.get(original["artifact"]["slug"], version=1).content
        )["front"]
        == original["front"]
    )
    ...
    with pytest.raises(MeasurementError, match="immutable"):
        store.update(
            original["id"], dict(request_id="source", revision=2, source="different")
        )
...
def test_practice_progression_resets_and_due_projection(tmp_path):
    ...
    for index, (value, interval) in enumerate(
        ...
        assert due - observed == (
            timedelta(minutes=10) if value == "again" else timedelta(days=interval)
        )
    ...
    assert (
        store.list_cards(
            due_only=True, as_of=(due - timedelta(microseconds=1)).isoformat()
        )
        == []
    )
...
def test_archive_preserves_history_but_prevents_new_practice(tmp_path):
    ...
    archived = store.update(
        row["id"], dict(request_id="archive", revision=1, archived=True)
    )
    ...
    restored = store.update(
        row["id"], dict(request_id="restore", revision=2, archived=False)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        dict(tags=[{}]),
        dict(tags="tag"),
        dict(tags=[str(n) for n in range(21)]),
        dict(extra=True),
    ],
)
...
def test_invalid_practice_updates_and_filters_preserve_history(tmp_path):
    ...
    for payload in [
        dict(request_id="bool", revision=True, archived=True),
        dict(request_id="archived", revision=1, archived="true"),
        dict(request_id="empty", revision=1, front=""),
    ]:
        with pytest.raises(MeasurementError):
...
def test_concurrent_receipts_artifact_integrity_and_isolation(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        rows = list(
            pool.map(
                lambda _: MemoryPracticeStore(tmp_path / "one").create(card()), range(4)
            )
        )
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        practiced = list(
            pool.map(
                lambda _: MemoryPracticeStore(tmp_path / "one").practice(
                    rows[0]["id"], grade(rows[0])
                ),
                range(4),
            )
        )
...
async def test_real_http_card_edit_grade_filter_and_archive(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.put(
            path, json=dict(request_id="edit", revision=2, back="Paris, France")
        )
        edited = await response.json()
        assert edited["back"] == "Paris, France"
        assert (await (await client.get(path + "/history")).json())["history"] == [
            row,
            practiced,
            edited,
        ]
        assert (
            await client.put(
                path, json=dict(request_id="stale", revision=1, back="stale")
            )
        ).status == 409
        ...
        assert (
            await client.post(
                base, data="broken", headers={"Content-Type": "application/json"}
            )
        ).status == 400
...
async def test_native_provider_consumes_canonical_artifact_and_schedule(tmp_path):
    ...
    async def invoke(name, identity=None, payload=None):
        result = await provider.invoke(
            "wellbeing_records",
            dict(operation="memory_" + name, id=identity, payload=payload or {}),
        )
    ...
    updated = await invoke(
        "update", row["id"], dict(request_id="edit", revision=1, tags=["review"])
    )
