...
MANIFEST = (
    ROOT / "runtime/gideon/extensions/apps/native/gideon-privacy-broker-spokeo/app.json"
)
WHITEPAGES_MANIFEST = (
    ROOT
    / "runtime/gideon/extensions/apps/native/gideon-privacy-broker-whitepages/app.json"
)


@pytest.mark.asyncio
async def test_parent_signed_route_and_native_provider_are_wired_without_broker_writes(
    ...
    try:
        ...
        try:
            ...
            app.router.add_get(
                "/session", lambda request: web.json_response({"ok": True})
            )
            ...
            async with TestClient(
                ...
                assert (
                    await client.get(
                        "/session?token=" + generate_token("privacy-owner")
                    )
                ).status == 200
        ...
        registry.deregister("gideon-privacy-broker-spokeo")


@pytest.mark.asyncio
async def test_parent_authenticated_whitepages_route_and_native_provider_stop_before_send(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("GIDEON_HOME", str(tmp_path))
    registry = ProviderRegistry()
    ...
    try:
        ...
        assert all(tool.requires_approval for tool in definitions.values())
        assert (
            definitions["privacy_broker_whitepages_send"].risk_level.value
            == "destructive"
        )
        missing = await provider.invoke(
            "privacy_broker_whitepages_prepare",
            {
                ...
                "profile_url": "https://www.whitepages.com/name/Jane-Doe",
                "jurisdiction": "US-CA",
            },
        )
        assert missing.success is False and missing.metadata["status"] == 404
        ...
        registry.deregister("gideon-privacy-broker-whitepages")
