...
from gideon.workspace.capabilities.wellbeing.shared_health import SharedHealthStore, SCHEMA
...
def row(**changes):
    value = dict(id=str(uuid4()), revision=1, kind='body_weight', observed_at='2026-09-25T09:00:00+02:00', unit='lb', values={'weight': 160}, source='native scale', notes='recorded by user')
    value.update(changes)
    return value


def document(rows=None, **changes):
    value = dict(schema=SCHEMA, version=1, store_id=str(uuid4()), records=rows if rows is not None else [row()])
    value.update(changes)
    return value
...
def commit(store, value, request='import'):
    ...
    return store.commit(dict(content=content, preview_id=preview['preview_id'], request_id=request))


def measurement(store, request='local'):
    return store.create(dict(request_id=request, kind='blood_pressure', observed_at='2026-09-25T09:00:00Z', unit='mmHg', values={'systolic': 120, 'diastolic': 80}, source='home cuff', notes='original'))


def test_actual_native_file_round_trip_canonical_corrections_and_normalization(tmp_path):
    ...
    assert store.list() == []
    result = store.commit_file(dict(preview_id=preview['preview_id'], request_id='native'))
    ...
    assert store.commit_file(dict(preview_id=preview['preview_id'], request_id='native')) == result
    ...
    raw = (tmp_path / 'artifacts' / reference['slug'] / 'versions' / reference['filename']).read_bytes()
    assert raw == original
...
def test_remote_revision_conflicts_and_local_changes_reject_atomic_batch(tmp_path):
    store = SharedHealthStore(tmp_path)
    value = document([row(), row(kind='blood_pressure', unit='mmHg', values={'systolic': 120, 'diastolic': 80})])
    ...
    store.correct(local['id'], dict(request_id='local-edit', revision=1, notes='Changed locally'))
...
def test_publication_compare_and_swap_download_and_external_edit(tmp_path):
    ...
    result = store.commit_file(dict(request_id='native-correction', preview_id=preview['preview_id']))
    assert result['records'][0]['id'] == local['id']
    assert store.get(local['id'])['notes'] == 'Native client correction'
    second = store.publish(dict(request_id='publish-again', expected_sha256=store.status()['sha256']))
...
def test_cross_home_document_transport_preserves_identity_mapping(tmp_path):
    ...
    sender.correct(original['id'], dict(request_id='correct', revision=1, values={'systolic': 121, 'diastolic': 81}))
    sender.publish(dict(request_id='publish2', expected_sha256=sender.status()['sha256']))
...
def test_file_changes_after_preview_and_unknown_fields_stop_import(tmp_path):
    ...
    with pytest.raises(MeasurementError, match='only'):
        store.commit_file(dict(request_id='changed', preview_id=preview['preview_id'], path='/elsewhere'))
    with pytest.raises(MeasurementError):
        store.preview({'content': json.dumps(value), 'home': '/elsewhere'})


@pytest.mark.parametrize('mutation', ['schema', 'version', 'store_id', 'duplicate', 'id', 'revision', 'kind', 'unit', 'values', 'observed_at', 'source', 'notes', 'extra'])
def test_invalid_typed_shared_document_rejected_before_any_writes(tmp_path, mutation):
    ...
    if mutation in ('schema', 'version', 'store_id'):
        value[mutation] = {'schema': 'other', 'version': True, 'store_id': 'invalid'}[mutation]
    elif mutation == 'duplicate':
        value['records'].append(value['records'][0].copy())
    else:
        value['records'][0][mutation] = {'id': '../escape', 'revision': 0, 'kind': 'unknown', 'unit': 'stones', 'values': {'weight': True}, 'observed_at': '2026-09-25', 'source': '', 'notes': 6, 'extra': 'unsupported'}[mutation]
    store = SharedHealthStore(tmp_path)
    with pytest.raises(MeasurementError):
...
def test_import_retry_concurrency_source_scope_and_request_conflict(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        receipts = list(pool.map(lambda _: commit(SharedHealthStore(tmp_path), value), range(4)))
...
def test_export_includes_shared_provenance_and_exact_original(tmp_path):
    ...
    assert base64.b64decode(snapshot['attachments'][0]['content_base64']) == json.dumps(value).encode()
...
async def test_native_provider_real_file_operations_and_rejected_overrides(tmp_path):
    ...
    preview = await provider.invoke('wellbeing_records', {'operation': 'shared_preview', 'payload': {'content': content}})
    assert preview.success
    result = await provider.invoke('wellbeing_records', {'operation': 'shared_commit', 'payload': {'content': content, 'preview_id': json.loads(preview.output)['preview_id'], 'request_id': 'native'}})
    assert result.success
    result = await provider.invoke('wellbeing_records', {'operation': 'shared_publish', 'payload': {'request_id': 'publish', 'expected_sha256': None}})
    assert result.success
    result = await provider.invoke('wellbeing_records', {'operation': 'shared_preview_file'})
    assert json.loads(result.output)['counts']['unchanged'] == 1
    result = await provider.invoke('wellbeing_records', {'operation': 'shared_download'})
    assert base64.b64decode(json.loads(result.output)['content_base64']) == SharedHealthStore(tmp_path).download()
    result = await provider.invoke('wellbeing_records', {'operation': 'shared_status', 'payload': {'path': '/tmp/elsewhere'}})
...
@pytest.mark.asyncio
async def test_actual_http_native_document_import_publish_isolation_and_errors(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client, TestClient(TestServer(other)) as isolated:
        ...
        content = json.dumps(document())
        preview = await (await client.post(base + '/preview', json={'content': content})).json()
        response = await client.post(base + '/commit', json=dict(content=content, preview_id=preview['preview_id'], request_id='http'))
        assert response.status == 200
        receipt = await response.json()
        assert receipt['counts']['create'] == 1
        response = await client.post(base + '/publish', json=dict(request_id='publish', expected_sha256=None))
        assert response.status == 200
        ...
        result = await (await client.post(base + '/file/commit', json=dict(request_id='file', preview_id=file_preview['preview_id']))).json()
        assert result['counts']['unchanged'] == 1
        assert (await (await isolated.get(base)).json())['state'] == 'missing'
        assert (await isolated.get(base + '/download')).status == 404
        assert (await client.post(base + '/file/preview', json={'path': str(tmp_path / 'two')})).status == 400
        assert (await client.post(base + '/preview', data='{', headers={'Content-Type': 'application/json'})).status == 400
