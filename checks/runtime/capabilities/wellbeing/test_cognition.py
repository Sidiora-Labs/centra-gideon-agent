...
def start(**changes):
    value = dict(request_id='start', kind='arithmetic', planned_trials=2, time_limit_seconds=60)
...
def expected(row):
    ...
    return str(stimulus['left'] + stimulus['right'] if stimulus['operator'] == '+' else stimulus['left'] - stimulus['right'])


def answer(row, request='answer', value=None):
    return dict(request_id=request, revision=row['revision'], answer=expected(row) if value is None else value)
...
def test_start_actual_challenge_persistence_and_no_expected_answer_leak(tmp_path):
    ...
    assert (datetime.fromisoformat(row['deadline']) - datetime.fromisoformat(row['started_at'])).total_seconds() == 60
...
def test_scoring_real_submission_times_retry_and_completion(tmp_path):
    ...
    elapsed = (datetime.fromisoformat(first['trials'][0]['answered_at']) - datetime.fromisoformat(row['current_trial']['presented_at'])).total_seconds() * 1000
...
def test_color_word_matches_ink_and_case_whitespace(tmp_path):
    ...
    completed = store.answer(row['id'], answer(row, value=' ' + stimulus['color'].upper() + ' '))
    assert completed['status'] == 'completed'
    assert completed['score']['correct'] == 1
    assert completed['trials'][0]['stimulus'] == stimulus
    assert completed['score']['accuracy'] == 1
    wrong = store.start(start(request_id='second-start', kind='color_word', planned_trials=1))
    alternatives = [color for color in ('red', 'blue', 'green', 'yellow') if color != wrong['current_trial']['stimulus']['color']]
...
@pytest.mark.parametrize('changes', [dict(kind='reaction'), dict(planned_trials=0), dict(planned_trials=21), dict(planned_trials=True), dict(planned_trials=1.5), dict(time_limit_seconds=0), dict(time_limit_seconds=601), dict(time_limit_seconds=True), dict(request_id=''), dict(extra=1)])
...
@pytest.mark.parametrize('changes', [dict(answer=''), dict(answer=None), dict(answer=10), dict(answer='a' * 129), dict(revision=True), dict(revision='1'), dict(request_id=''), dict(correct=True)])
...
def test_concurrent_receipts_home_isolation_and_limits(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        rows = list(pool.map(lambda _: CognitiveStore(tmp_path / 'one').start(start()), range(4)))
    assert rows.count(rows[0]) == 4
    payload = answer(rows[0])
    with ThreadPoolExecutor(max_workers=4) as pool:
        answered = list(pool.map(lambda _: CognitiveStore(tmp_path / 'one').answer(rows[0]['id'], payload), range(4)))
...
async def test_actual_http_start_answer_reload_cancel_and_errors(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(base + '/' + row['id'] + '/answers', json=answer(row))
        assert response.status == 200
        answered = await response.json()
        assert answered['score']['correct'] == 1
        response = await client.post(base + '/' + row['id'] + '/cancel', json=dict(request_id='cancel', revision=2))
        assert response.status == 200
        ...
        assert (await client.post(base, data='bad', headers={'Content-Type': 'application/json'})).status == 400
        assert (await client.post(base + '/' + row['id'] + '/answers', json=answer(row, 'stale'))).status == 409
...
async def test_native_provider_uses_real_shared_sessions(tmp_path):
    ...
    async def invoke(operation, identity=None, payload=None):
        result = await provider.invoke('wellbeing_records', dict(operation='cognition_' + operation, id=identity, payload=payload or {}))
    ...
    assert cancelled['status'] == 'cancelled'
    assert CognitiveStore(tmp_path).get(row['id']) == cancelled
    failure = await provider.invoke('wellbeing_records', {'operation': 'cognition_missing'})
    assert not failure.success
    assert 'Unknown' in failure.error
    assert 'cognition_start' in (await provider.list_tools())[0].parameters['properties']['operation']['enum']
