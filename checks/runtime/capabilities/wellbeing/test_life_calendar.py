...
def config(**changes):
    value = dict(request_id='config', revision=0, birth_date='2000-02-29', horizon_years=80, sleep_hours=8, timezone='UTC', budgets=[dict(name='Study', hours_per_week=14)], source='my planning assumptions', reminder=dict(enabled=False, time='00:00'))
    value.update(changes)
    return value


def event(**changes):
    value = dict(request_id='event', date='2026-09-25', title='Completed course', notes='Personal milestone', kind='recorded', source='journal')
    value.update(changes)
    return value
...
def test_config_revision_trigger_and_projection_arithmetic(tmp_path):
    ...
    assert projection['total_days'] == (datetime(2080, 2, 29) - datetime(2000, 2, 29)).days
    ...
    updated = store.configure(config(request_id='updated', revision=1, horizon_years=81, timezone='America/New_York'))
...
def test_event_corrections_tombstones_and_history(tmp_path):
    ...
    corrected = store.update_event(original['id'], dict(request_id='edit', revision=1, date='2026-09-26', title='Course milestone', kind='planned'))
    ...
    with pytest.raises(MeasurementError, match='changed'):
        store.update_event(original['id'], dict(request_id='stale', revision=1, title='stale'))
    with pytest.raises(MeasurementError, match='immutable'):
        store.update_event(original['id'], dict(request_id='source', revision=2, source='changed'))
    deleted = store.update_event(original['id'], dict(request_id='delete', revision=2, deleted=True))
    assert store.list_events() == []
    assert store.history_event(original['id']) == [original, corrected, deleted]
    with pytest.raises(MeasurementError, match='deleted'):
        store.update_event(original['id'], dict(request_id='restore', revision=3, deleted=False))
...
def test_local_reminder_real_inbox_claims_restart_and_completion(tmp_path):
    ...
    session = cognitive.start(dict(request_id='cognitive', kind='color_word', planned_trials=1, time_limit_seconds=60))
    completed = cognitive.answer(session['id'], dict(request_id='complete', revision=1, answer=session['current_trial']['stimulus']['color']))
...
def test_reminder_timezone_boundaries_concurrency_and_home_guard(tmp_path):
    store = LifeCalendarStore(tmp_path)
    store.configure(config(timezone='Europe/Berlin', reminder=dict(enabled=True, time='09:00')))
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        results = list(pool.map(lambda _: LifeCalendarStore(tmp_path).check_reminder('2026-10-26T09:00:00Z'), range(4)))
    ...
    with store.connection() as db:
        assert db.execute('SELECT COUNT(*) FROM life_reminder_claims').fetchone()[0] == 2


@pytest.mark.parametrize('changes', [dict(birth_date='2099-01-01'), dict(birth_date='2000-02-30'), dict(horizon_years=0), dict(horizon_years=True), dict(horizon_years=121), dict(sleep_hours=-1), dict(sleep_hours=25), dict(sleep_hours=float('nan')), dict(timezone='Unknown/Zone'), dict(budgets=[dict(name='Work', hours_per_week=120)]), dict(budgets=[dict(name='x', hours_per_week=1), dict(name='x', hours_per_week=2)]), dict(budgets=[dict(name='x', hours_per_week=True)]), dict(reminder=dict(enabled='true', time='09:00')), dict(reminder=dict(enabled=True, time='24:00')), dict(source='')])
...
def test_isolation_invalid_events_and_config_receipts(tmp_path):
    ...
    assert LifeCalendarStore(tmp_path / 'two').check_reminder() == {'status': 'disabled'}
    with pytest.raises(MeasurementError, match='Request ID'):
        store.configure(config(source='different'))
    for changes in [dict(title=''), dict(date='bad'), dict(kind='unknown'), dict(notes=None), dict(source='')]:
        with pytest.raises(MeasurementError):
...
async def test_actual_clock_dispatch_journal_and_persisted_inbox(tmp_path, monkeypatch):
    ...
    assert item.refs['local_date'] == datetime.fromtimestamp(scheduled, timezone.utc).date().isoformat()
    repeated = await provider.execute({}, ActionContext(event='trigger.fired', payload={'scheduled_for': scheduled}))
    assert repeated.success
    assert json.loads(repeated.stdout)['status'] == 'already_sent'
    forbidden = await provider.execute({'home': '/other'}, ActionContext(event='trigger.fired'))
...
async def test_actual_http_projection_milestones_and_inbox(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert await (await client.get(base + '/projection')).json() == {'configured': False}
        response = await client.put(base + '/config', json=config(reminder=dict(enabled=True, time='00:00')))
        ...
        response = await client.put(base + '/events/' + row['id'], json=dict(request_id='edit', revision=1, title='Revised milestone'))
        assert response.status == 200
        response = await client.get(base + '/events/' + row['id'] + '/history')
        assert len((await response.json())['history']) == 2
        response = await client.get(base + '/projection', params={'as_of': '2026-09-25T12:00:00Z'})
        ...
        assert (await client.post(base + '/reminder/check', json={'as_of': '2099-01-01T00:00:00Z'})).status == 400
        assert (await client.put(base + '/config', data='bad', headers={'Content-Type': 'application/json'})).status == 400
...
async def test_native_life_calendar_operations(tmp_path):
    ...
    async def invoke(operation, identity=None, payload=None):
        result = await provider.invoke('wellbeing_records', dict(operation='life_' + operation, id=identity, payload=payload or {}))
    ...
    updated = await invoke('event_update', milestone['id'], dict(request_id='edit', revision=1, notes='new note'))
    ...
    forbidden = await provider.invoke('wellbeing_records', dict(operation='life_reminder_check', payload={'as_of': '2099-01-01T00:00:00Z'}))
    assert not forbidden.success
