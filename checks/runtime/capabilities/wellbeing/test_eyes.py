...
from gideon.workspace.capabilities.wellbeing.eyes import SCHEMA, EyePrescriptionStore
from gideon.workspace.capabilities.wellbeing.eyes_http import register
from gideon.workspace.capabilities.wellbeing.eyes_provider import (
    EyePrescriptionTools,
    create_provider,
)
    ...
    async def journey():
        ...
        try:
            ...
            status, conflict = await call(
                first, "POST", BASE, prescription(notes="changed replay")
            )
            assert status == 409
            ...
            status, listing = await call(
                first, "GET", BASE + "?from=2026-09-24&to=2026-09-24"
            )
            assert status == 200
            ...
            correction = {
                "request_id": "eyes-correct-1",
                "revision": 1,
                "observed_date": "2026-09-25",
                "notes": "Cylinder transcription corrected",
            ...
            assert not any(
                key in exported
                for key in ("diagnosis", "interpretation", "recommendation")
            )
            ...
            with sqlite3.connect(database_path) as database:
                assert database.execute(
                    "SELECT count(*) FROM eye_prescription_revisions"
                ).fetchone() == (2,)
                assert database.execute(
...
def test_corrections_keep_source_immutable_and_queries_are_bounded(tmp_path):
    async def journey():
        ...
        try:
            ...
            changes = {
                "request_id": "source-change",
                "revision": 1,
                "source": "another clinic",
            }
...
def test_native_manifest_provider_reads_same_store_and_gates_writes(tmp_path):
    async def journey():
        ...
        assert all(
            definitions[name].requires_approval is (name in writes)
            for name in definitions
        )
        assert all(
            definitions[name].risk_level.value
            == ("caution" if name in writes else "safe")
            for name in definitions
        )

        created_result = await provider.invoke(
            "wellbeing_eye_prescriptions_create", {"payload": prescription()}
        )
        ...
        read = await provider.invoke(
            "wellbeing_eye_prescriptions_get", {"id": created["id"]}
        )
        assert json.loads(read.output) == created
        correction = {
            "request_id": "native-correction",
            "revision": 1,
            "notes": "Native correction",
        }
        corrected_result = await provider.invoke(
            "wellbeing_eye_prescriptions_correct",
            {"id": created["id"], "payload": correction},
        )
        assert corrected_result.success
        corrected = json.loads(corrected_result.output)
        assert corrected["revision"] == 2
        history = await provider.invoke(
            "wellbeing_eye_prescriptions_history", {"id": created["id"]}
        )
        ...
        refused = await provider.invoke(
            "wellbeing_eye_prescriptions_correct",
            {"id": created["id"], "payload": {**correction, "notes": "changed replay"}},
        )
...
def test_native_manifest_is_unique_and_points_to_provider():
    ...
    path = (
        ...
        / "runtime/gideon/extensions/apps/native/gideon-wellbeing-eyes/app.json"
    )
...
def test_direct_store_orders_dates_pages_records_and_exports_every_authored_revision(
    tmp_path,
):
    store = EyePrescriptionStore(tmp_path)
    older = store.create(
        prescription(request_id="older", observed_date="2024-01-02", notes="Older")
    )
    newest = store.create(
        prescription(request_id="newest", observed_date="2026-09-25", notes="Newest")
    )
    middle = store.create(
        prescription(request_id="middle", observed_date="2025-06-10", notes="Middle")
    )
    ...
    assert [
        row["revision"] for row in exported["history"] if row["id"] == middle["id"]
    ] == [1, 2]
...
def test_native_provider_rejects_undeclared_shapes_without_writing(tmp_path):
    async def journey():
        ...
        cases = [
            ...
            (
                "wellbeing_eye_prescriptions_create",
                {"payload": prescription(), "extra": True},
            ),
