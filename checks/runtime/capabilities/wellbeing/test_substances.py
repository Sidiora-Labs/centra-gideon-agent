...
from gideon.interfaces.dashboard.handlers.capabilities_wellbeing_substances import (
    register,
)
...
def test_correction_replay_stale_write_and_tombstone_history(tmp_path):
    ...
    assert (
        store.delete_entry(original["id"], dict(request_id="delete", revision=2))
        == deleted
    )
    assert store.create_entry(entry()) == original
    with pytest.raises(MeasurementError):
        store.correct_entry(
            original["id"], dict(request_id="after-delete", revision=3, count=1)
        )
...
def test_presets_snapshot_details_and_survive_preset_edit_delete(tmp_path):
    ...
    snapshot = store.create_entry(
        dict(
            request_id="from-preset",
            preset_id=product["id"],
            count=1,
            observed_at="2026-09-25T12:00:00Z",
            source="manual preset",
            notes="original preset",
        )
    )
    ...
    changed = store.update_preset(
        product["id"],
        dict(
            request_id="preset-edit",
            revision=1,
            details={"volume_ml": 500, "abv_percent": 6},
            name="Larger product",
        ),
    )
    ...
    deleted = store.delete_preset(
        product["id"], dict(request_id="preset-delete", revision=2)
    )
    assert deleted["deleted"]
...
def test_calendar_summary_distinguishes_unknown_from_recorded_zero(tmp_path):
    ...
    assert [day["date"] for day in empty["days"]] == [
        "2026-09-23",
        "2026-09-24",
        "2026-09-25",
    ]
    zero = store.create_entry(
        entry(
            "zero",
            details={"volume_ml": 330, "abv_percent": 0},
            observed_at="2026-09-24T23:30:00Z",
        )
    )
    ...
    assert (
        store.summary("Europe/Berlin", days=3, as_of="2026-09-25T12:00:00Z")["totals"][
            "ethanol_g"
        ]
        is None
    )
...
def test_summary_uses_local_days_excludes_future_and_deleted(tmp_path):
    ...
    first = store.create_entry(
        entry("day-one", count=1, observed_at="2026-09-23T23:00:00Z")
    )
    second = store.create_entry(
        entry("day-two", count=2, observed_at="2026-09-24T23:00:00Z")
    )
    ...
    assert summary["totals"]["ethanol_g"] == pytest.approx(
        first["ethanol_g"] + second["ethanol_g"]
    )
    assert summary["logged_days"]["alcohol"] == 2
    assert summary["averages_per_logged_day"]["ethanol_g"] == pytest.approx(
        (first["ethanol_g"] + second["ethanol_g"]) / 2
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"request_id": ""},
        {"details": {}},
        {"details": {"volume_ml": 0, "abv_percent": 5}},
...
def test_query_validation_and_isolation(tmp_path):
    ...
    original = store.create_entry(entry())
    assert store.list_entries(
        from_date="2026-09-25T07:00:00Z", to_date="2026-09-25T07:00:00.000000Z"
    ) == [original]
    for query in (
        {"kind": "other"},
        {"limit": 0},
...
def test_concurrent_retries_and_optimistic_conflicts(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        rows = list(
            pool.map(
                lambda _: ConsumptionStore(tmp_path).create_entry(entry()), range(3)
            )
        )
    ...
    def edit(number):
        try:
            return ConsumptionStore(tmp_path).correct_entry(
                rows[0]["id"],
                dict(request_id=f"edit-{number}", revision=1, count=number + 1),
            )
...
async def test_real_http_presets_entries_corrections_and_summary(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert await (await client.get(base + "/presets")).json() == {
            "presets": [product]
        }
        ...
        response = await client.put(
            path, json=dict(request_id="correct", revision=1, count=2)
        )
        assert response.status == 200
        ...
        response = await client.put(
            base + "/presets/" + product["id"],
            json=dict(request_id="preset-edit", revision=1, name="Renamed"),
        )
        assert response.status == 200
        assert (await response.json())["name"] == "Renamed"
        response = await client.delete(
            base + "/presets/" + product["id"],
            json=dict(request_id="preset-delete", revision=2),
        )
        assert response.status == 200
...
async def test_native_operations_share_http_ledger(tmp_path):
    ...
    assert (
        "substances_summary" in definition.parameters["properties"]["operation"]["enum"]
    )
    result = await provider.invoke(
        tool, dict(operation="substances_preset_create", payload=preset())
    )
    ...
    result = await provider.invoke(
        tool, dict(operation="substances_entry_create", payload=entry())
    )
    original = json.loads(result.output)
    result = await provider.invoke(
        tool, dict(operation="substances_entry_get", id=original["id"])
    )
    assert json.loads(result.output) == original
    result = await provider.invoke(
        tool,
        dict(
            operation="substances_entry_correct",
            id=original["id"],
            payload=dict(request_id="tool-correct", revision=1, count=1),
        ),
    )
    changed = json.loads(result.output)
    result = await provider.invoke(
        tool, dict(operation="substances_entry_history", id=original["id"])
    )
    ...
    result = await provider.invoke(
        tool,
        dict(
            operation="substances_summary",
            payload=dict(days=1, as_of="2026-09-25T12:00:00Z"),
        ),
    )
    ...
    result = await provider.invoke(
        tool,
        dict(
            operation="substances_entry_delete",
            id=original["id"],
            payload=dict(request_id="tool-delete", revision=2),
        ),
    )
    ...
    result = await provider.invoke(
        tool,
        dict(
            operation="substances_preset_update",
            id=product["id"],
            payload=dict(request_id="product-edit", revision=1, name="Changed"),
        ),
    )
    ...
    result = await provider.invoke(
        tool,
        dict(
            operation="substances_preset_delete",
            id=product["id"],
            payload=dict(request_id="product-delete", revision=2),
        ),
    )
    assert json.loads(result.output)["deleted"]
