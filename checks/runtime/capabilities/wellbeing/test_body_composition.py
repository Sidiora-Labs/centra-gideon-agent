...
from gideon.workspace.capabilities.wellbeing.body_composition_provider import (
...
def test_store_retains_originals_normalizes_units_and_preserves_history(tmp_path):
    ...
    corrected = store.correct(
        record["id"],
        {
            "request_id": "body-correction",
            "revision": 1,
            "observed_at": record["observed_at"],
            "notes": "Corrected from paper record",
            "values": {
                "muscle_percent": 40.9,
                "fat_percent": 18.7,
    ...
    assert exported == {
        "schema_version": 1,
        "body_composition": [corrected],
        "history": [record, corrected],
    }
    with pytest.raises(MeasurementError, match="changed"):
        store.correct(
            record["id"], {"request_id": "stale", "revision": 1, "notes": "stale"}
        )
    ...
    with sqlite3.connect(tmp_path / "capabilities/wellbeing.sqlite3") as database:
        assert (
            database.execute(
                "SELECT count(*) FROM body_composition_revisions"
            ).fetchone()[0]
            == 2
        )
...
async def test_source_http_two_home_crud_history_export_and_reopen(tmp_path):
    ...
    try:
        ...
        correction = {
            "request_id": "http-correct",
            "revision": 1,
            "notes": "Paper correction",
            "values": {
                "muscle_percent": 42,
                "fat_percent": 18,
                "bone_mass": {"value": 3, "unit": "kg"},
        ...
        assert (
            updated["observed_at"] == created["observed_at"]
            and updated["source"] == created["source"]
        )
...
def test_native_tools_use_actual_store_and_mark_writes_for_approval(tmp_path):
    async def journey():
        ...
        created = await provider.invoke(
            ...
            {"operation": "create", "payload": authored(request_id="native-create")},
        )
        assert created.success is True
        record = json.loads(created.output)
        read = await provider.invoke(
            "body_composition_read", {"operation": "get", "id": record["id"]}
        )
        ...
        assert (
            corrected.success is True and json.loads(corrected.output)["revision"] == 2
        )
        history = await provider.invoke(
            "body_composition_read", {"operation": "history", "id": record["id"]}
        )
        assert [row["revision"] for row in json.loads(history.output)] == [1, 2]
        exported = await provider.invoke(
            "body_composition_read", {"operation": "export"}
        )
        assert len(json.loads(exported.output)["history"]) == 2
        refused = await provider.invoke(
            "body_composition_write", {"operation": "dispatch", "payload": {}}
        )
...
def test_native_manifest_points_to_the_approval_aware_provider():
    path = Path(
        "runtime/gideon/extensions/apps/native/gideon-body-composition/app.json"
    )
