    ...
    broker = store.create_broker(
        {
            ...
            "name": "Spokeo",
            "website": origin,
            "optout_url": origin + "/optout",
            "source": "curated protocol",
        }
    )
...
async def contract_server(*, listed=True, form="valid"):
    ...
    async def search(request):
        ...
        return web.Response(
            text="<html><article>Jane Doe lives in Oakland, CA</article></html>"
        )
    ...
    async def optout(request):
        ...
        if form == "captcha":
            return web.Response(
                text='<form action="/optout/submit" method="post"><div class="g-recaptcha"></div><input name="url"><input name="email"></form>'
            )
        if form == "changed":
            return web.Response(
                text='<form action="/new" method="get"><input name="profile"></form>'
            )
        return web.Response(
            text='<form action="/optout/submit" method="post"><input type="hidden" name="csrf" value="contract-token"><input name="url"><input name="email"></form>'
        )
    ...
    async def submit(request):
        ...
        return web.Response(
            text="<html>Request received. Check your email for the confirmation email.</html>"
        )
...
@pytest.mark.asyncio
async def test_real_spokeo_html_protocol_drives_scan_prepare_submit_and_verified_rescan(
    tmp_path,
):
    ...
    try:
        ...
        prepared = await service.prepare(
            ...
            payload(
                "prepare",
                2,
                profile_url=origin + "/Jane-Doe/CA/record-1",
        ...
        assert contract["posts"][0] == {
            "csrf": "contract-token",
            "url": origin + "/Jane-Doe/CA/record-1",
            "email": "jane@example.test",
        }

        present = await service.verify(
            row["id"], payload("verify-present", 4, **identity())
        )
        ...
        removed = await service.verify(
            row["id"], payload("verify-removed", 5, **identity())
        )
        ...
        assert [event["operation"] for event in store.events(row["id"])] == [
            "created",
            "provider_scan",
            "provider_optout_prepared",
            "provider_optout_submitted",
            "provider_verified",
            "provider_verified",
        ]
...
@pytest.mark.asyncio
async def test_http_provider_routes_use_same_protocol_and_never_submit_during_prepare(
    tmp_path,
):
    ...
    base = (
        f'/api/capabilities/wellbeing/privacy/broker-cases/{row["id"]}/providers/spokeo'
    )
    try:
        async with TestClient(TestServer(app)) as client:
            response = await client.post(
                base + "/scan", json=payload("http-scan", 1, **identity())
            )
            ...
            response = await client.post(
                ...
                json=payload(
                    "http-prepare",
                    2,
                    profile_url=origin + "/Jane-Doe/CA/id",
...
@pytest.mark.asyncio
async def test_consent_provider_identity_and_revision_are_checked_before_network(
    tmp_path,
):
    server, contract = await contract_server()
    origin = str(server.make_url("")).rstrip("/")
    try:
        ...
        with pytest.raises(MeasurementError, match="broker_scan"):
            await service.scan(
                row["id"], payload("revoked", scanned["revision"], **identity())
            )
        assert contract["posts"] == []
        assert contract["requests"] == [("GET", "/Jane-Doe/CA")]
        ...
        other_case = store.create_case(
            subject["id"], {"request_id": "other-case", "broker_id": other["id"]}
        )
        with pytest.raises(MeasurementError, match="Spokeo cases only"):
            await service.scan(
                other_case["id"], payload("wrong-provider", 1, **identity())
            )
    finally:
        await server.close()
...
@pytest.mark.asyncio
async def test_protocol_rejects_captcha_changed_form_cross_origin_and_live_local_origin(
    tmp_path,
):
    ...
    try:
        ...
        with pytest.raises(MeasurementError, match="human challenge") as captcha:
            await protocol.prepare(
                captcha_origin + "/Jane-Doe/CA/id", "owner@example.test"
            )
    ...
    try:
        ...
        with pytest.raises(MeasurementError, match="protocol changed") as changed:
            await protocol.prepare(
                changed_origin + "/Jane-Doe/CA/id", "owner@example.test"
            )
        assert changed.value.status == 502
        with pytest.raises(MeasurementError, match="configured Spokeo origin"):
            await protocol.prepare(
                "https://attacker.example/profile", "owner@example.test"
            )
...
@pytest.mark.parametrize(
    ("listed", "expected"), [(True, "found"), (False, "not_found")]
)
...
def test_store_adapter_boundaries_reject_untyped_results_and_invalid_states(tmp_path):
    ...
    found = BrokerAdapterResult(
        "spokeo-html-v1", "found", checked_at, "sha256:" + "a" * 64
    )
    ...
    with pytest.raises(
        MeasurementError, match="not ready for opt-out submission"
    ) as premature:
        store.apply_adapter_submission(
            row["id"],
            {"request_id": "premature", "revision": 2},
            BrokerAdapterResult(
                "spokeo-html-v1", "submitted", checked_at, "sha256:" + "b" * 64
            ),
        )
    assert premature.value.status == 409
    with pytest.raises(
        MeasurementError, match="not ready for provider verification"
    ) as premature_verify:
        store.apply_adapter_verification(
            row["id"],
            {"request_id": "premature-verify", "revision": 2},
            BrokerAdapterResult(
                "spokeo-html-v1", "not_found", checked_at, "sha256:" + "c" * 64
            ),
        )
...
@pytest.mark.asyncio
async def test_protocol_validates_identity_before_request_and_live_gate_before_form(
    ...
    try:
        ...
        with pytest.raises(MeasurementError, match="two-letter"):
            await protocol.scan(
                {"first_name": "Jane", "last_name": "Doe", "state": "California"}
            )
        ...
        with pytest.raises(
            MeasurementError, match="Live Spokeo submission is disabled"
        ) as disabled:
            await live.submit(
                origin + "/Jane-Doe/CA/id", "owner@example.test", APPROVAL
            )
