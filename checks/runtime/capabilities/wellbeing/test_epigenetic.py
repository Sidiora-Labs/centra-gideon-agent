...
from gideon.workspace.capabilities.wellbeing.epigenetic_provider import (
    EpigeneticProvider,
    create_provider,
)
...
def payload(**changes):
    value = {
        "request_id": "epi-create-1",
        "source_report_id": "TruAge-2026-09",
        "observed_at": "2026-09-20",
        "source": "Owner-uploaded TruAge report",
        ...
        "organ_scores": {
            ...
            "liver": {"value": 72, "scale": "percentile"},
            "brain": None,
        },
...
def test_create_correct_history_export_reopen_and_home_isolation(tmp_path):
    async def journey():
        use_ephemeral_secret()
        first, other = await client(tmp_path / "a", "epigenetic-a"), await client(
            tmp_path / "b", "epigenetic-b"
        )
        try:
            status, created = await call(first, "POST", BASE, payload())
            assert status == 201
            assert (
                created["evidence_basis"] == "source_reported"
                and created["revision"] == 1
            )
            assert created["biological_age"] == {"value": 38.4, "unit": "years"}
            assert created["pace_of_aging"] == {"value": 0.91, "scale": "years/year"}
            ...
            correction = {
                "request_id": "epi-correct-1",
                "revision": 1,
                "biological_age": {"value": 37.9, "unit": "years"},
                "notes": "Corrected transcription",
            }
            status, corrected = await call(
                first, "PUT", BASE + "/" + created["id"], correction
            )
            assert (
                status == 200
                and corrected["revision"] == 2
                and corrected["source"] == created["source"]
            )
            assert (
                corrected["organ_scores"] == created["organ_scores"]
                and corrected["biological_age"]["value"] == 37.9
            )
            status, conflict = await call(
                first,
                "PUT",
                BASE + "/" + created["id"],
                {**correction, "request_id": "stale"},
            )
            assert status == 409 and conflict["code"] == "conflict"
            status, history = await call(
                first, "GET", BASE + "/" + created["id"] + "/history"
            )
            assert status == 200 and history["history"] == [created, corrected]
            status, exported = await call(first, "GET", BASE + "/export")
            assert (
                status == 200
                and exported["record_family"] == "source_reported_epigenetic_results"
            )
            assert exported["records"] == [corrected] and exported["history"] == [
                created,
                corrected,
            ]
            await first.close()
            first = await client(tmp_path / "a", "epigenetic-reopen")
...
@pytest.mark.parametrize(
    "bad",
    [
        payload(observed_at="09/20/2026"),
        payload(biological_age=38),
        payload(pace_of_aging={"value": 0.9, "unit": "ratio", "scale": "years/year"}),
        payload(organ_scores={"heart": {"value": float("inf"), "unit": "years"}}),
        payload(organ_scores={"heart": {"value": 30}}),
        payload(diagnosis="younger"),
    ],
)
...
@pytest.mark.asyncio
async def test_native_tools_use_real_store_and_require_approval_only_for_mutations(
    tmp_path,
):
    ...
    assert (
        tools["wellbeing_epigenetic_create"].requires_approval
        and tools["wellbeing_epigenetic_correct"].requires_approval
    )
    assert (
        not tools["wellbeing_epigenetic_list"].requires_approval
        and not tools["wellbeing_epigenetic_export"].requires_approval
    )
    result = await provider.invoke(
        "wellbeing_epigenetic_create", {"payload": payload(request_id="native-create")}
    )
    ...
    history = await provider.invoke(
        "wellbeing_epigenetic_history", {"id": created["id"]}
    )
    ...
    unknown = await provider.invoke(
        "wellbeing_epigenetic_diagnose", {"id": created["id"]}
    )
