...
def case(store, subject, broker_row, request="case"):
    return store.create_case(
        subject["id"], {"request_id": request, "broker_id": broker_row["id"]}
    )


def observe(store, row, outcome, evidence, request):
    return store.record_user_observation(
        row["id"],
        {
            "request_id": request,
...
def test_broker_catalog_case_and_owner_observation_are_durable_and_subject_scoped(
    tmp_path,
):
    ...
    found = observe(
        store, created, "found", "Owner saw a matching name and address", "found"
    )
    ...
    assert [row["state"] for row in reopened.history(created["id"])] == [
        "unscanned",
        "found",
    ]
    assert [row["operation"] for row in reopened.events(created["id"])] == [
        "created",
        "owner_observation",
    ]
    ...
    assert [
        row["operation"] for row in audit if row["operation"].startswith("broker_case_")
    ] == ["broker_case_created", "broker_case_owner_observation"]
    assert audit[-1]["evidence_basis"] == "user_attested"
    assert "matching name" not in json.dumps(audit)


def test_manual_optout_lifecycle_requires_submit_consent_and_never_confirms_removal(
    tmp_path,
):
    privacy, store, person = setup_store(tmp_path)
    row = observe(
        store,
        case(store, person, broker(store)),
        "found",
        "Owner found listing",
        "seen",
    )
    ...
    row = move(
        store, row, "submitted", "submitted", "Owner reports submitting the broker form"
    )
    ...
    row = move(
        ...
        "pending",
        "Owner reports the broker acknowledged receipt",
    )
    ...
    row = move(
        ...
        "awaiting_processing",
        "processing",
        "Owner reports the broker requested processing time",
    )
    ...
    denied = observe(
        denied_store,
        case(denied_store, denied_person, broker(denied_store)),
        "found",
        "Owner saw listing",
        "seen",
    )
...
def test_verified_scanner_boundary_is_the_only_confirmed_removed_path(tmp_path):
    ...
    row = observe(
        store, case(store, person, broker(store)), "found", "Owner saw listing", "seen"
    )
    row = move(store, row, "optout_in_progress", "start")
    ...
    with pytest.raises(MeasurementError, match="offset timestamp"):
        store.apply_verified_observation(
            row["id"],
            VerifiedBrokerObservation("not_found", "scanner", "yesterday", "receipt"),
        )
    ...
    with pytest.raises(MeasurementError, match="does not match"):
        store.apply_verified_observation(
            removed["id"], verified("not_found", "scanner://run/43")
        )
    reappeared = store.apply_verified_observation(
        removed["id"], verified("found", "scanner://run/44")
    )
    assert reappeared["state"] == "reappeared"
    assert reappeared["evidence_basis"] == "verified_rescan"
    assert reappeared["allowed_transitions"] == [
        "optout_in_progress",
        "human_task_queued",
    ]
...
def test_each_owner_scan_outcome_is_explicitly_user_attested(
    tmp_path, initial, outcome
):
    _, store, person = setup_store(tmp_path)
    ...
    assert (
        observed["next_recheck_at"] > observed["updated_at"] or outcome == "unscanned"
    )
...
def test_owner_rescans_settled_observation_but_cannot_interrupt_submission(tmp_path):
    _, store, person = setup_store(tmp_path)
    row = observe(
        store, case(store, person, broker(store)), "not_found", "No result seen", "none"
    )
    ...
    changed = observe(
        store, changed, "blocked", "Challenge blocked owner check", "blocked"
    )
    assert changed["state"] == "blocked"
    with pytest.raises(MeasurementError, match="Invalid manual"):
        move(store, changed, "submitted", "skip-start")
    queued = move(
        store, changed, "human_task_queued", "queue", "Owner needs browser help"
    )
...
def test_recheck_is_revisioned_idempotent_and_preserves_state(tmp_path):
    ...
    with pytest.raises(MeasurementError, match="changed; reload"):
        store.request_recheck(
            row["id"], {"request_id": "stale", "revision": row["revision"]}
        )
    with pytest.raises(MeasurementError, match="Request ID already used"):
        store.request_recheck(
            row["id"], {"request_id": "recheck", "revision": due["revision"]}
        )
...
def test_broker_validation_duplicate_case_and_concurrent_idempotency(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        rows = list(
            pool.map(
                lambda _: PrivacyBrokerStore(tmp_path).record_user_observation(
                    created["id"],
                    {
                        "request_id": "concurrent-observation",
                        "revision": 1,
                        "outcome": "found",
                        "evidence": "Same owner observation",
                    },
                ),
                range(3),
            )
...
@pytest.mark.asyncio
async def test_http_owner_journey_has_no_verified_or_send_route_and_is_home_scoped(
    tmp_path,
):
    ...
    async with (
        ...
        response = await client.post(
            cases_url, json={"request_id": "case", "broker_id": catalog["id"]}
        )
        assert response.status == 200
        created = await response.json()
        response = await client.post(
            f"{base}/broker-cases/{created['id']}/observe",
            json={
                "request_id": "observe",
                "revision": 1,
                "outcome": "found",
                "evidence": "Owner saw HTTP result",
            },
        )
        ...
        assert observed["evidence_basis"] == "user_attested"
        response = await client.post(
            f"{base}/broker-cases/{created['id']}/transition",
            json={
                "request_id": "start",
                "revision": 2,
                "state": "optout_in_progress",
                "reason": "Owner started elsewhere",
            },
        )
        progressed = await response.json()
        assert progressed["state"] == "optout_in_progress"
        response = await client.post(
            f"{base}/broker-cases/{created['id']}/recheck",
            json={"request_id": "recheck", "revision": 3},
        )
        ...
        assert (
            len(
                (
                    await (
                        await client.get(f"{base}/broker-cases/{created['id']}/history")
                    ).json()
                )["history"]
            )
            == 4
        ...
        assert (
            len(
                (
                    await (
                        await client.get(f"{base}/broker-cases/{created['id']}/events")
                    ).json()
                )["events"]
            )
            == 4
        ...
        assert (
            await isolated.get(f"{base}/broker-cases/{created['id']}")
        ).status == 404
        assert (
            await client.post(f"{base}/broker-cases/{created['id']}/verified", json={})
        ).status == 404
        assert (
            await client.post(f"{base}/broker-cases/{created['id']}/send", json={})
        ).status == 404
...
async def test_http_reports_consent_conflict_stale_revision_and_unknown_case(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            f"{base}/subjects/{person['id']}/broker-cases",
            json={"request_id": "case", "broker_id": catalog["id"]},
        )
        ...
        created = await (
            await client.post(
                f"{base}/subjects/{person['id']}/broker-cases",
                json={"request_id": "case-two", "broker_id": catalog["id"]},
            )
        ).json()
        observed = await (
            await client.post(
        ...
        response = await client.post(
            f"{base}/broker-cases/{observed['id']}/transition",
            json={
                "request_id": "submit-denied",
                "revision": 2,
                "state": "optout_in_progress",
                "reason": "",
            },
