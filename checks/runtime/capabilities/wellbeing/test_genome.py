...
def payload(**changes):
    result = dict(filename='sample.tsv', format='tsv', content=TSV, source='personal export', assembly='GRCh37')
...
def commit(store, data=None, request='import-1'):
    ...
    return store.commit(dict(data, preview_id=preview['preview_id'], request_id=request))


def annotation(**changes):
    result = dict(request_id='note-1', revision=1, annotation='Reviewed original call', annotation_source='personal review')
    result.update(changes)
    return result
...
def test_import_receipts_assembly_identity_and_source_replay(tmp_path):
    ...
    assert changed['source']['artifact']['sha256'] == first['source']['artifact']['sha256']
    ...
    with pytest.raises(MeasurementError, match='Preview changed'):
        store.commit(dict(payload(source='changed'), preview_id=preview['preview_id'], request_id='changed'))
    assert len(store.list_sources()) == 3
...
def test_vcf_samples_alleles_phase_missing_calls_and_haploid(tmp_path):
    ...
    assert store.preview(payload(format='vcf', content=missing))['variants'][0]['genotype'] == '.'
    deletion = haploid.replace('\tA\tG,T', '\tAC\tA,ACT')
    assert store.preview(payload(format='vcf', content=deletion))['variants'][0]['genotype'] == 'ACT'
...
def test_manual_annotations_preserve_source_and_history(tmp_path):
    ...
    cleared = store.annotate(row['id'], annotation(request_id='clear', revision=2, annotation=''))
...
def test_filter_pagination_isolation_and_missing_records(tmp_path):
    ...
    for operation, identity in [(other.get_source, source['id']), (other.list_variants, source['id']), (other.original, source['id']), (other.get_variant, rows[0]['id']), (other.history, rows[0]['id'])]:
        with pytest.raises(MeasurementError) as caught:
            operation(identity)
        assert caught.value.status == 404
    for params in [dict(limit=0), dict(limit=501), dict(offset=-1), dict(offset=100001), dict(limit=True), dict(offset=False)]:
        with pytest.raises(MeasurementError, match='pagination'):
            store.list_variants(source['id'], **params)


@pytest.mark.parametrize('changes', [dict(assembly='hg19'), dict(assembly=None), dict(format='json'), dict(source=''), dict(filename=''), dict(content=''), dict(content='rs1\t1\t123\tAG'), dict(content=TSV.replace('123', '0')), dict(content=TSV.replace('123', '-1')), dict(content=TSV.replace('123', '1000000001')), dict(content=TSV.replace('\t1\t', '\t23\t')), dict(content=TSV.replace('AG', 'ZZ')), dict(content=TSV.replace('AG', 'AG\textra')), dict(content='a' * (8 * 1024 * 1024 + 1)), dict(sample=42), dict(extra=True)])
...
@pytest.mark.parametrize('changes', [dict(sample='Nobody'), dict(sample=''), dict(content=VCF.replace('8:1|2', '8:3/0')), dict(content=VCF.replace('8:1|2', '8:x/0')), dict(content=VCF.replace('DP:GT', 'DP:XX')), dict(content=VCF.replace('\tG,T\t', '\t<DEL>\t')), dict(content=VCF.replace('\tBob', '\tAlice')), dict(content=VCF.replace('8:1|2', '8'))])
...
@pytest.mark.parametrize('changes', [dict(revision=True), dict(revision='1'), dict(annotation=None), dict(annotation='x' * 10001), dict(annotation_source=''), dict(request_id=''), dict(genotype='GG')])
...
async def test_real_http_import_annotate_source_and_failure(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(base + '/commit', json=dict(payload(), preview_id=preview['preview_id'], request_id='http'))
        assert response.status == 200
        ...
        response = await client.put(base + f'/variants/{row["id"]}/annotation', json=annotation())
        assert response.status == 200
        ...
        response = await client.put(base + f'/variants/{row["id"]}/annotation', json=annotation(request_id='stale'))
        assert response.status == 409
        response = await client.get(base + f'/sources/{identity}/variants?limit=invalid')
        assert response.status == 400
        response = await client.post(base + '/preview', data='{broken', headers={'Content-Type': 'application/json'})
        assert response.status == 400
...
async def test_native_provider_uses_same_home_and_all_genome_operations(tmp_path):
    ...
    async def invoke(name, identity=None, data=None):
        result = await provider.invoke('wellbeing_records', dict(operation='genome_' + name, id=identity, payload=data or {}))
        assert result.success, result.error
        return json.loads(result.output)
    preview = await invoke('preview', data=payload())
    result = await invoke('commit', data=dict(payload(), preview_id=preview['preview_id'], request_id='native'))
    source = result['source']
    assert await invoke('sources') == [source]
    assert await invoke('source', source['id']) == source
    assert base64.b64decode((await invoke('original', source['id']))['content_base64']) == TSV.encode()
    ...
    unknown = await provider.invoke('wellbeing_records', {'operation': 'genome_missing'})
