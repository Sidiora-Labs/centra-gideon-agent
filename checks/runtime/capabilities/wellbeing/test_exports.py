...
from gideon.workspace.capabilities.wellbeing.exports import TABLES, ExportStore
...
def commit_import(store, payload, request):
    ...
    return store.commit(
        dict(payload, preview_id=preview["preview_id"], request_id=request)
    )
...
def seed(home):
    ...
    measurements.correct(
        row["id"], dict(request_id="weight-edit", revision=1, values={"weight": 73})
    )
    lab_text = (
        "analyte,observed_at,value,unit\r\nGlucose,2026-09-25T09:00:00Z,92,mg/dL\r\n"
    )
    commit_import(
        LabStore(home),
        dict(
    ...
    memory.update(
        card["id"], dict(request_id="memory-edit", revision=1, back="Revised answer")
    )
    cognition = CognitiveStore(home)
    session = cognition.start(
        dict(
            request_id="session",
            kind="arithmetic",
            planned_trials=2,
    ...
    entry = substances.create_entry(
        dict(
            request_id="substance",
            kind="nicotine",
            name="Labeled gum",
    ...
    substances.delete_entry(
        entry["id"], dict(request_id="substance-delete", revision=1)
    )
...
def test_all_domain_history_originals_and_canonical_archive(tmp_path):
    ...
    assert document["sections"]["measurements"]["history"][0]["values"] == {
        "weight": 72
    }
    assert document["sections"]["measurements"]["history"][1]["values"] == {
        "weight": 73
    }
    ...
    assert (
        document["sections"]["life"]["history"][0]["assumption"]
        == "user_declared_horizon"
    )
    ...
    embedded = [
        base64.b64decode(item["content_base64"]) for item in document["attachments"]
    ]
    assert all(original in embedded for original in originals)
    for item in document["attachments"]:
        assert (
            hashlib.sha256(base64.b64decode(item["content_base64"])).hexdigest()
            == item["reference"]["sha256"]
        )
        assert (
            hashlib.sha256(item["descriptor"].encode()).hexdigest()
...
def test_snapshot_is_immutable_and_receipts_do_not_reexport(tmp_path):
    ...
    assert (
        "wellbeing_exports" not in json.loads(store.download(second["id"]))["sections"]
    )
...
def test_concurrent_retry_single_durable_export(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        rows = list(
            pool.map(
                lambda _: ExportStore(tmp_path).create({"request_id": "same"}), range(4)
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"request_id": ""},
        {"request_id": 5},
        {"request_id": "a", "home": "/elsewhere"},
        {"request_id": "a", "domains": ["measurements"]},
    ],
)
...
def test_pagination_never_truncates_export_and_large_archive_is_companion(tmp_path):
    ...
    assert (
        len(
            json.loads(exports.download(row["id"]))["sections"]["measurements"][
                "history"
            ]
        )
        == 301
    ...
    assert (
        len(exports.artifacts.get(row["artifact"]["slug"], version=1).content)
        < 1024 * 1024
    )
...
def test_original_hash_corruption_prevents_partial_export(tmp_path):
    seed(tmp_path)
    original = next(
        (tmp_path / "artifacts").glob("genome-source-*/versions/original@*")
    )
...
async def test_native_agent_archive_preview_download_and_isolation(tmp_path):
    ...
    result = await provider.invoke(
        "wellbeing_records", {"operation": "exports_preview"}
    )
    assert result.success
    assert json.loads(result.output)["counts"]["measurements"] == 1
    result = await provider.invoke(
        "wellbeing_records",
        {"operation": "exports_create", "payload": {"request_id": "agent"}},
    )
    assert result.success
    row = json.loads(result.output)
    result = await provider.invoke(
        "wellbeing_records", {"operation": "exports_download", "id": row["id"]}
    )
    assert result.success
    assert base64.b64decode(json.loads(result.output)["content_base64"]) == ExportStore(
        tmp_path
    ).download(row["id"])
    result = await provider.invoke(
        "wellbeing_records", {"operation": "exports_get", "id": row["id"]}
    )
    ...
    result = await provider.invoke(
        "wellbeing_records",
        {"operation": "exports_preview", "payload": {"home": str(tmp_path)}},
    )
    assert not result.success
    result = await provider.invoke(
        "wellbeing_records", {"operation": "exports_invalid"}
    )
    assert not result.success
    other = WellbeingProvider(tmp_path / "other")
    result = await other.invoke(
        "wellbeing_records", {"operation": "exports_get", "id": row["id"]}
    )
    assert not result.success
...
async def test_actual_http_preview_create_reopen_download_and_home_isolation(tmp_path):
    ...
    async with (
        ...
        TestClient(TestServer(isolated)) as second,
    ):
        ...
        assert (
            await (await client.post(base, json={"request_id": "http"})).json() == row
        )
        ...
        assert (
            await client.post(base, json={"request_id": "bad", "home": str(other)})
        ).status == 400
        assert (
            await client.post(
                base, data="{", headers={"Content-Type": "application/json"}
            )
        ).status == 400
