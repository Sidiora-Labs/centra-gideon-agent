...
from gideon.workspace.capabilities.wellbeing.privacy_broker_whitepages_provider import (
    WhitepagesPrivacyBrokerProvider,
    create_provider,
)
...
async def test_native_tools_declare_exact_canonical_email_operations_and_approval_boundaries():
    service = Service()
    provider = WhitepagesPrivacyBrokerProvider(service)
    ...
    assert tools["privacy_broker_whitepages_approve"].parameters["properties"][
        "confirm_exact"
    ] == {"const": True}
    assert tools["privacy_broker_whitepages_send"].parameters["properties"][
        "confirm_send"
    ] == {"const": True}
    assert (
        tools["privacy_broker_whitepages_prepare"].parameters["additionalProperties"]
        is False
    )
...
async def test_native_provider_routes_send_and_correlation_without_transport_overrides():
    ...
    unknown = await provider.invoke(
        "privacy_broker_whitepages_delete", {"case_id": "case-1"}
    )
