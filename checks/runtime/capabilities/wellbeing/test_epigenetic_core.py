...
def test_authored_unit_and_scale_are_preserved_without_normalization(tmp_path):
    ...
    assert created["organ_scores"]["inflammation"] == {
        "value": -1.4,
        "scale": "z-score",
    }
...
def test_request_replay_and_collision_are_atomic(tmp_path):
    ...
    corrected = store.correct(
        original["id"],
        {
            ...
            "revision": 1,
            "pace_of_aging": {"value": 0.88, "scale": "years/year"},
        },
    )
    assert (
        store.correct(
    ...
    with pytest.raises(MeasurementError, match="Request ID already used"):
        store.correct(
            original["id"],
            {
                "request_id": "correct-once",
                "revision": 2,
                "pace_of_aging": {"value": 0.9, "scale": "years/year"},
            },
        )
...
def test_concurrent_corrections_allow_only_one_revision_winner(tmp_path):
    ...
    def correct(index):
        try:
            return EpigeneticStore(tmp_path).correct(
                original["id"],
                {
...
def test_list_orders_dates_and_applies_bounded_pagination(tmp_path):
    ...
    assert [row["source_report_id"] for row in store.list()] == [
        "report-3",
        "report-2",
        "report-1",
        "report-0",
...
def test_export_has_current_records_and_complete_immutable_history(tmp_path):
    ...
    second = store.create(
        report("create-2", source_report_id="report-2", observed_at="2026-09-22")
    )
    corrected = store.correct(
        first["id"],
        {
            "request_id": "correct-export",
            "revision": 1,
    ...
    assert sorted(exported["records"], key=lambda row: row["id"]) == sorted(
        [corrected, second], key=lambda row: row["id"]
    )
    assert exported["history"] == sorted(
        [first, corrected, second], key=lambda row: (row["id"], row["revision"])
    )
    assert all(
        row["evidence_basis"] == "source_reported" for row in exported["history"]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("pace_of_aging", {"value": 0.9}, "exactly one"),
        (
            "pace_of_aging",
            {"value": 0.9, "unit": "ratio", "scale": "ratio"},
            "exactly one",
        ),
    ],
)
def test_measure_validation_rejects_unusable_source_values(
...
def test_missing_records_and_stale_revisions_are_explicit(tmp_path):
    ...
    with pytest.raises(MeasurementError) as stale:
        store.correct(
            original["id"], {"request_id": "stale", "revision": 0, "notes": "wrong"}
        )
