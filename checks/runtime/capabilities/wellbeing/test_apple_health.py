...
from gideon.workspace.capabilities.wellbeing.apple_health import (
    MAX_BYTES,
    MAX_RECORDS,
    AppleHealthStore,
)
...
def observation(**changes):
    value = dict(
        ...
        effectiveDateTime="2026-09-25T09:00:00+02:00",
...
def payload(raw=XML, format_="xml", **changes):
    value = dict(
        filename="export." + format_,
        format=format_,
        content_base64=base64.b64encode(raw).decode(),
        source="User health export",
    )
    value.update(changes)
    return value
...
def commit(store, data=None, request="import"):
    ...
    return store.commit(
        dict(data, preview_id=store.preview(data)["preview_id"], request_id=request)
    )
...
def test_auto_export_json_qty_and_heart_rate_components(tmp_path):
        ...
        "data": {
            "metrics": [
                ...
                },
                {
                    ...
                    "units": "x",
                    "data": [
                        {"date": "2026-09-25T09:00:00+02:00", "text": "unsupported"}
                    ],
                },
            ]
        }
    ...
    assert [row["metric"] for row in preview["metrics"]] == [
        "step_count",
        "heart_rate:Min",
        "heart_rate:Max",
        "heart_rate:Avg",
...
def test_fhir_bundle_uses_existing_lab_history_and_original(tmp_path):
    bundle = {
        ...
        "entry": [
            ...
            {"resource": observation(id="cancelled", status="cancelled")},
            {"resource": {"resourceType": "Patient", "id": "ignored"}},
            {"resource": observation(id="nonnumeric", valueQuantity=None)},
        ],
    }
    ...
    assert preview["skipped"] == {
        "nonactive_or_nonlaboratory_fhir": 2,
        "nonnumeric_fhir_observation": 1,
    }
    ...
    changed = labs.correct(
        record["id"], dict(request_id="correction", revision=1, value=93)
    )
...
def test_zip_import_commits_metrics_and_clinical_labs_together(tmp_path):
    raw = archive(
        {
            ...
            "apple_health_export/clinical_records/lab.json": json.dumps(observation()),
            "apple_health_export/notes.txt": "ignored",
        }
    )
    ...
    assert result["labs_added"] == 1
    assert (
        LabStore(tmp_path).list()[0]["artifact"] == store.list_metrics()[0]["artifact"]
    )
...
def test_cross_domain_identity_conflict_rolls_back_metrics(tmp_path):
    ...
    raw = archive(
        {
            "export.xml": XML,
            "clinical_records/lab.json": json.dumps(
                observation(valueQuantity={"value": 99, "unit": "mg/dL"})
            ),
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        payload(source=""),
        payload(filename="x" * 201),
        payload(home="/other"),
        payload(b"<broken>"),
        payload(b"<HealthData/>"),
        payload(b'<!DOCTYPE HealthData SYSTEM "file:///etc/passwd"><HealthData/>'),
        payload(
            b'<!DOCTYPE HealthData [<!ENTITY x "repeat">]><HealthData>&x;</HealthData>'
        ),
        payload("<HealthData/>".encode("utf-16")),
        payload(b"[]", "json"),
        payload(b'{"data":{"metrics":{}}}', "json"),
        payload(b"{bad", "json"),
        payload(b'{"data":{"metrics":[{"data":[1]}]}}', "json"),
        payload(b"not zip", "zip"),
        payload(archive({"../export.xml": XML}), "zip"),
        payload(archive({"/export.xml": XML}), "zip"),
        payload(archive({"notes.txt": "empty"}), "zip"),
        payload(
            json.dumps(
                {
                    "resourceType": "Bundle",
...
@pytest.mark.parametrize(
    "replacement",
    [b'value="NaN"', b'value="Infinity"', b'value="wrong"', b'value="1e20"'],
)
    ...
    bundle = {
        ...
        "entry": [
            {"resource": observation()},
            {
                "resource": observation(
                    id="threshold",
                    valueQuantity={"value": 5, "unit": "mg/dL", "comparator": "<"},
                )
            },
        ],
    }
...
def test_limits_and_preview_pagination_are_explicit(tmp_path):
    ...
    with pytest.raises(MeasurementError):
        store.preview(
            payload(b"<HealthData>" + record * (MAX_RECORDS + 1) + b"</HealthData>")
        )
    with pytest.raises(MeasurementError):
        store.preview(
            payload(archive({"export.xml": b" " * (33 * 1024 * 1024)}), "zip")
        )
    assert store.list_metrics() == []
...
def test_metric_date_and_unit_filtering(tmp_path):
    ...
    assert store.list_metrics(limit=1, offset=0) != store.list_metrics(
        limit=1, offset=1
    )
    for query in (
        {"limit": 0},
...
def test_parallel_import_reuses_one_durable_receipt(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        receipts = list(
            pool.map(
                lambda n: commit(AppleHealthStore(tmp_path), request=f"request-{n}"),
                range(3),
            )
        )
...
async def test_real_http_preview_commit_original_and_isolation(tmp_path):
    ...
    async with (
        ...
        response = await client.post(
            base + "/import/commit",
            json=dict(data, preview_id=preview["preview_id"], request_id="http"),
        )
        assert response.status == 200
        receipt = await response.json()
        assert receipt["metrics_added"] == 2
        response = await client.get(
            base + "/metrics", params={"unit": "count", "from": "2026-09-25T07:00:00Z"}
        )
        ...
        assert (
            await isolated.get(base + "/metrics/" + records[0]["id"] + "/source")
        ).status == 404
        assert await (await isolated.get(base + "/metrics")).json() == {"metrics": []}
        assert (
            await client.post(
                base + "/import/preview", json=payload(content_base64="broken")
            )
        ).status == 400
        assert (
            await client.get(base + "/metrics", params={"limit": "bad"})
        ).status == 400
        response = await client.post(
            base + "/import/preview",
            data="bad",
...
async def test_native_tool_imports_without_secondary_store(tmp_path):
    ...
    result = await provider.invoke(
        "wellbeing_records", dict(operation="apple_preview", payload=data)
    )
    ...
    result = await provider.invoke(
        "wellbeing_records",
        dict(
            operation="apple_commit",
            payload=dict(data, preview_id=preview["preview_id"], request_id="agent"),
        ),
    )
    assert result.success
    assert json.loads(result.output)["metrics_added"] == 2
    result = await provider.invoke(
        "wellbeing_records", dict(operation="apple_metrics", payload={"unit": "count"})
    )
