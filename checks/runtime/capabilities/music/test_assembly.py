...
from gideon.workspace.capabilities.music.assembly_geometry import (
    compile_model,
    mesh,
    source_module,
)
    ...
    return {
        ...
        "parts": [
            ...
            part(
                ...
                position=[0, 2, 0],
                parent_id="base",
                joint={"axis": "z", "min_degrees": -45, "max_degrees": 45},
            ),
        ],
        "clips": [
            {
...
def test_actual_primitive_geometry_bounds_triangles_and_normals():
    for shape, triangles in (("box", 12), ("sphere", 168), ("cylinder", 48)):
        ...
        for index in range(0, len(normals), 3):
            assert sum(
                value * value for value in normals[index : index + 3]
            ) == pytest.approx(1)
...
def test_compiled_glb_has_hierarchy_materials_and_real_rotation_channels():
    ...
    assert document["materials"][0]["pbrMetallicRoughness"][
        "baseColorFactor"
    ] == pytest.approx([85 / 255, 153 / 255, 204 / 255, 1])
    ...
    assert rotations[2] == pytest.approx(-math.sqrt(0.5))
    assert rotations[6] == pytest.approx(0)
    assert rotations[10] == pytest.approx(math.sqrt(0.5))
    assert diagnostics["triangle_count"] == 60
    assert diagnostics["mesh_count"] == 2
    assert diagnostics["volume_estimate"] == pytest.approx(
        8 + math.pi * 0.15 * 0.15 * 2
    )
    assert diagnostics["bounds"] == {"min": [-1, -1, -1], "max": [1, 3, 1]}
    assert {row["code"] for row in diagnostics["findings"]} == {
        "clip_outside_joint",
        "off_ground",
    }
...
def test_bounds_include_actual_parent_rotation_and_translation():
    schema = {
        "title": "Transforms",
        "parts": [
            part(position=[3, 0, 0], rotation=[0, 0, math.pi / 2]),
            part("child", position=[2, 0, 0], parent_id="base"),
        ],
        "clips": [],
    }
...
def test_thin_and_overlap_findings_are_explicit_geometric_approximations():
    schema = {
        "title": "Thin overlap",
        "parts": [part(), part("thin", size=[0.01, 2, 2])],
        "clips": [],
    }
    _, diagnostics = compile_model(schema)
...
def test_refinement_grounds_roots_clamps_clip_limits_and_keeps_original(tmp_path):
    store = store_at(tmp_path)
    item = store.create(spec())
    refined = store.refine(
        item["id"], {"revision": 1, "operations": ["ground", "clamp_clips"]}
    )
...
def test_export_receipt_is_durable_and_old_geometry_survives_edit(tmp_path):
    ...
    changed = store.update(
        item["id"], {**spec(), "title": "Edited assembly", "revision": 1}
    )
...
def test_concurrent_export_requests_share_persisted_receipt(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(
            pool.map(
                lambda _: store_at(tmp_path).export(item["id"], {"revision": 1}),
                range(2),
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"size": [1, 2]},
        {"size": [float("nan"), 1, 1]},
        {"position": [0, float("inf"), 0]},
        {"rotation": [0, 0, 10]},
        {"color": "javascript:alert(1)"},
        {"id": "bad.name"},
        {"parent_id": "missing"},
        {"parent_id": "base"},
        {"joint": {"axis": "x", "min_degrees": 30, "max_degrees": -30}},
        {"path": "/tmp"},
    ],
)
def test_invalid_parts_refused_without_persistence(tmp_path, changes):
    store = store_at(tmp_path)
    schema = {"title": "Bad part", "parts": [part(**changes)], "clips": []}
    with pytest.raises(DomainError):
        store.create(schema)
    assert store.list() == []
...
def test_cycles_duplicate_ids_and_clips_without_joint_refused(tmp_path):
    ...
    schemas = [
        {
            "title": "Cycle",
            "parts": [part(parent_id="two"), part("two", parent_id="base")],
            "clips": [],
        },
    ...
    no_joint = spec()
    no_joint["parts"][1]["joint"] = None
    schemas.append(no_joint)
    wrong_axis = spec()
    wrong_axis["clips"][0]["axis"] = "x"
    schemas.append(wrong_axis)
...
def test_untrusted_model_paths_and_unknown_refinements_refused(tmp_path):
    ...
    item = store.create(spec())
    with pytest.raises(DomainError):
        store.create({**spec(), "home": "/tmp"})
    for data in (
        {"revision": 1, "operations": []},
        {"revision": 1, "operations": ["run_script"]},
        {"revision": 1, "operations": ["ground"], "provider": "remote"},
    ):
        with pytest.raises(DomainError):
            store.refine(item["id"], data)
...
def test_foreign_home_cannot_read_refine_or_export(tmp_path):
    ...
    item = one.create(spec())
    for action in (
        lambda: two.get(item["id"]),
        lambda: two.refine(item["id"], {"revision": 1, "operations": ["ground"]}),
        lambda: two.export(item["id"], {"revision": 1}),
    ):
...
async def test_http_real_author_refine_history_export_and_pinned_source(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post(prefix, json=spec())
        assert response.status == 201
        item = (await response.json())["item"]
        refined = await client.post(
            prefix + "/" + item["id"] + "/refine",
            json={"revision": 1, "operations": ["ground", "clamp_clips"]},
        )
        ...
        source = await client.get(
            prefix + f"/artifacts/{ref['slug']}/{ref['version']}/source"
        )
        ...
        stale = await client.patch(
            prefix + "/" + item["id"], json={**spec(), "revision": 1}
        )
...
async def test_native_all_operations_use_real_assembly_store(tmp_path):
    store = store_at(tmp_path)
    tools = AssemblyTools(store)
    assert len(await tools.list_tools()) == 7
    result = await tools.invoke("music_assemblies_create", {"data": spec()})
    assert result.success
    item = json.loads(result.output)
    fetched = await tools.invoke("music_assemblies_get", {"id": item["id"]})
    assert json.loads(fetched.output) == item
    updated = await tools.invoke(
        "music_assemblies_update",
        {
            "id": item["id"],
            "data": {**spec(), "title": "Native updated", "revision": 1},
        },
    )
    assert updated.success
    refined = await tools.invoke(
        "music_assemblies_refine",
        {"id": item["id"], "data": {"revision": 2, "operations": ["ground"]}},
    )
    assert refined.success
    exported = await tools.invoke(
        "music_assemblies_export", {"id": item["id"], "data": {"revision": 3}}
    )
    assert exported.success
    history = await tools.invoke("music_assemblies_history", {"id": item["id"]})
    assert len(json.loads(history.output)) == 3
    listing = await tools.invoke("music_assemblies_list", {})
    assert len(json.loads(listing.output)) == 1
    denied = await tools.invoke(
        "music_assemblies_get", {"id": item["id"], "home": "/tmp"}
    )
