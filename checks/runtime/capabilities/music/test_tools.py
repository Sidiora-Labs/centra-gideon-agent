...
from checks.runtime.capabilities.music.test_repertoire import attempt, store_at
...
async def test_native_manifest_resolves_factory_and_exposes_complete_tools():
    ...
    manifest = json.loads(
        (
            root / "runtime/gideon/extensions/apps/native/gideon-music-tools/app.json"
        ).read_text()
    )
...
async def test_native_repertoire_author_practice_replay_and_edit(tmp_path):
    ...
    practiced = await provider.invoke(
        "music_repertoire_practice", {"id": item["id"], "data": payload}
    )
    assert practiced.success
    receipt = json.loads(practiced.output)
    assert receipt["item"]["stage"] == "learning"
    replay = await provider.invoke(
        "music_repertoire_practice", {"id": item["id"], "data": payload}
    )
    assert json.loads(replay.output) == {**receipt, "replayed": True}
    edited = await provider.invoke(
        "music_repertoire_update",
        {"id": item["id"], "data": {"revision": 2, "body": "New fingering"}},
    )
    assert edited.success
    reloaded = await provider_at(tmp_path).invoke(
        "music_repertoire_get", {"id": item["id"]}
    )
    assert json.loads(reloaded.output)["body"] == "New fingering"
    assert (
        json.loads(reloaded.output)["practice_history"]
        == receipt["item"]["practice_history"]
    )
...
async def test_native_catalog_operations_reach_same_actual_stores(tmp_path):
    provider = provider_at(tmp_path)
    artist = await provider.invoke(
        "music_catalog_create", {"kind": "artists", "data": {"name": "Native artist"}}
    )
    assert artist.success
    artist_id = json.loads(artist.output)["id"]
    created = await provider.invoke(
        "music_catalog_create",
        {"kind": "tracks", "data": {"title": "Native track", "artist_id": artist_id}},
    )
    track = json.loads(created.output)
    audio = artifact(provider.catalog)
    attached = await provider.invoke(
        "music_catalog_attach", {"id": track["id"], "data": attach_body(track, audio)}
    )
    ...
    selected = await provider.invoke(
        ...
        {"id": track["id"], "data": {"revision": 2, "render_id": render_id}},
    )
    assert selected.success
    assert json.loads(selected.output)["selected_render_id"] == render_id
    album = await provider.invoke(
        "music_catalog_create",
        {
            "kind": "albums",
            "data": {"title": "Native album", "track_ids": [track["id"]]},
        },
    )
    assert json.loads(album.output)["track_ids"] == [track["id"]]
    archived = await provider.invoke(
        "music_catalog_update",
        {
            "kind": "tracks",
            "id": track["id"],
            "data": {"revision": 3, "archived": True},
        },
    )
    assert archived.success
    listing = await provider.invoke(
        "music_catalog_list", {"kind": "tracks", "archived": True, "q": "Native"}
    )
    assert json.loads(listing.output) == [json.loads(archived.output)]
    fetched = await provider_at(tmp_path).invoke(
        "music_catalog_get", {"kind": "tracks", "id": track["id"]}
    )
...
async def test_invocation_rejects_home_override_and_unknown_tools(tmp_path):
    ...
    missing_args = await provider.invoke(
        "music_catalog_create", {"data": {"title": "Missing kind"}}
    )
...
async def test_isolated_tool_providers_cannot_read_other_home(tmp_path):
    ...
    created = await first.invoke(
        "music_repertoire_create", {"data": {"title": "First home"}}
    )
    item = json.loads(created.output)
