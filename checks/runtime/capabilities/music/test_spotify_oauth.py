...
from gideon.workspace.capabilities.music.spotify_oauth import SpotifyOAuth,SCOPES
from gideon.workspace.capabilities.music.store import DomainError
from test_listening import store_at


def write_credentials(home,index):
    store=CredentialStore(home)
    for number in range(8):store.put(f'worker-{index}-{number}',{'type':'static_token','value':f'value-{index}-{number}'})
...
def test_atomic_distinct_process_updates_preserve_unrelated(tmp_path):
    ...
    processes=[context.Process(target=write_credentials,args=(tmp_path,index)) for index in range(4)]
    ...
    for index in range(4):
        ...
        for number in range(1,8):assert store.resolve(f'worker-{index}-{number}').secret==f'value-{index}-{number}'
...
def test_put_remove_validation_and_reopen(tmp_path):
    ...
    with pytest.raises(ValueError):store.put('',{})
    with pytest.raises(ValueError):store.put('name',None)
    with pytest.raises(ValueError):store.remove('')
    assert final.resolve('two').secret=='second'


def oauth_at(home):
    bridge=SpotifyBridge(home,store_at(home))
    bridge.configure({'enabled':True,'credential_name':'spotify-own','account_label':'Own','revision':0})
    return SpotifyOAuth(bridge)


def test_actual_pkce_challenge_private_verifier_and_reopen(tmp_path):
    oauth=oauth_at(tmp_path)
    result=oauth.begin({'client_id':'registered-client','redirect_uri':'http://127.0.0.1:8765/callback'})
...
@pytest.mark.parametrize('redirect',['http://example.com/callback','http://localhost/callback','https://user@example.com/callback','https://example.com/#fragment','file:///tmp/callback','https:///missing'])
def test_pkce_rejects_unsafe_redirects(tmp_path,redirect):
    oauth=oauth_at(tmp_path)
    with pytest.raises(DomainError):oauth.begin({'client_id':'client','redirect_uri':redirect})
    assert oauth.credentials.list()==[]


def test_expired_and_unknown_state_cannot_exchange(tmp_path):
    oauth=oauth_at(tmp_path)
    result=oauth.begin({'client_id':'client','redirect_uri':'https://example.com/callback'})
    pending=oauth.pending(result['state']);pending['expires_at']=0
    oauth.credentials.put('spotify-pkce-'+result['state'],{'type':'oauth2','value':json.dumps(pending)})
    with pytest.raises(DomainError) as error:oauth.pending(result['state'])
    assert error.value.code=='oauth_state'
    assert not oauth.credentials.has('spotify-pkce-'+result['state'])
    with pytest.raises(DomainError):oauth.pending('unknown')
...
async def test_saved_token_bundle_refresh_metadata_and_raw_named_token(tmp_path):
    ...
    oauth._save('spotify-own','client',{'access_token':'issued-access','refresh_token':'issued-refresh','expires_in':3600})
    ...
    oauth._save('spotify-own','client',{'access_token':'rotated-access','expires_in':3600},'issued-refresh')
    ...
    oauth.credentials.put('direct',{'type':'oauth2','value':'external-managed-access'})
    ...
    with pytest.raises(DomainError) as error:await oauth.access('expired')
    assert error.value.code=='oauth_expired'
    with pytest.raises(DomainError):await oauth.complete({'state':'unknown','code':'authorization-code'})
