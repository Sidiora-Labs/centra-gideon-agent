...
from gideon.interfaces.dashboard.handlers.capabilities_music_models3d import register as models_register
from gideon.workspace.capabilities.music.image3d import Image3DStore
from test_assembly import store_at

async def main():
    with tempfile.TemporaryDirectory(prefix='gideon-assembly-ui-') as directory:
        home=Path(directory);store=store_at(home)
        app=web.Application();register(app,store);models_register(app,Image3DStore(home,store.artifacts))
