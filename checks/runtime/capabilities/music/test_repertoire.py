...
def create(store, title="Canon in D"):
    return store.create(
        {"title": title, "instrument": "Piano", "body": "D A Bm F#m\nG D G A"}
    )
...
def attempt(
    item,
    grade=4,
    instant="2026-03-28T10:00:00+01:00",
    key="take-1",
    zone="Europe/Berlin",
):
    return {
...
def test_songbook_metadata_notation_and_attachment_availability(tmp_path):
    store = store_at(tmp_path)
    score = store.artifacts.create(
        name="Public-domain chart",
        kind="document",
        content="immutable score",
        source="import",
    )
    ...
    item = store.create(
        {
            ...
            "instrument": "GUITAR",
            "body": "Practice the transition slowly.",
            "tags": ["folk", "public-domain", "folk"],
            "key": "Am",
            "capo": 3,
    ...
    assert missing["attachment_availability"] == [
        {**ref, "available": False, "name": "", "kind": "", "mime": "", "source": ""}
    ]
...
def test_legacy_row_get_update_and_practice_receive_additive_defaults(tmp_path):
    ...
    legacy = {
        ...
        "repetitions": 0,
        "revision": 1,
        "rules_version": "sm2-v1",
    }
    with sqlite3.connect(store.path) as connection:
        connection.execute(
            "INSERT INTO items VALUES (?,?)", (legacy["id"], json.dumps(legacy))
        )
...
def test_stable_archive_import_is_idempotent_and_supports_guarded_rollback(tmp_path):
    ...
    arguments = dict(
        ...
        schedule={
            ...
            "last_practiced_at": "2026-09-17T09:30:00+00:00",
            "last_grade": 4,
        },
    )
    ...
    with pytest.raises(DomainError) as err:
        store.import_song(
            **{**arguments, "data": {**arguments["data"], "title": "Changed"}}
        )
...
def import_arguments(store, *, import_id="archive:piece-1", item_id="piece-1"):
    chart = store.artifacts.create(
        name="Imported score", kind="document", content="Dm A", source="archive"
    )
    return {
        "import_id": import_id,
...
def test_import_validation_is_atomic_and_rejects_existing_stable_id(tmp_path):
    ...
    cases.append(
        {**base, "schedule": {**base["schedule"], "last_practiced_at": "2026-09-20"}}
    )
    ...
    cases.append(
        {
            **base,
            "schedule": {
                key: value
                for key, value in base["schedule"].items()
                if key != "last_grade"
            },
        }
    )
...
def test_concurrent_import_replay_and_cross_connection_schedule_continuity(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        receipts = list(
            pool.map(lambda _: store_at(tmp_path).import_song(**arguments), range(2))
        )
    ...
    assert all(
        receipt["source_fingerprint"] == arguments["source_fingerprint"]
    ...
    practiced = reopened.practice(
        "piece-1", attempt(imported, grade=4, instant="2026-10-20T12:00:00+00:00")
    )
...
def test_import_rollback_authenticates_fingerprint_and_is_transactional(tmp_path):
    ...
    deleted = store_at(tmp_path).rollback_import(
        arguments["import_id"], arguments["source_fingerprint"]
    )
    assert deleted == {
...
def test_grade_sequence_retains_clock_time_across_daylight_saving(tmp_path):
    ...
    second = store.practice(
        item["id"],
        attempt(
            first["item"], grade=5, instant="2026-03-29T10:00:00+02:00", key="take-2"
        ),
    )
    assert second["item"]["due_at"] == "2026-04-04T08:00:00+00:00"
    assert second["item"]["interval"] == 6
    assert second["item"]["stage"] == "review"
    third = store.practice(
        item["id"],
        attempt(
            second["item"], grade=4, instant="2026-04-04T10:00:00+02:00", key="take-3"
        ),
    )
...
def test_failure_resets_repetitions_but_keeps_history(tmp_path):
    ...
    assert [row["attempt_id"] for row in failed["practice_history"]] == [
        "0",
        "1",
        "failed",
    ]
    retried = store.practice(item["id"], attempt(failed, grade=3, key="relearn"))[
        "item"
    ]
...
@pytest.mark.parametrize(
    "grade,ease", [(0, 1.7), (1, 1.96), (2, 2.18), (3, 2.36), (4, 2.5), (5, 2.6)]
)
...
def test_ease_floor_and_restart_idempotency(tmp_path):
    ...
    item = store.practice(
        item["id"], attempt(first["item"], grade=0, key="failed-again")
    )["item"]
...
def test_cross_connection_replay_is_atomic(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        receipts = list(
            pool.map(lambda repository: repository.practice(item["id"], body), stores)
        )
...
def test_edit_preserves_practice_and_detects_conflicts(tmp_path):
    ...
    edited = store.update(
        original["id"], {"revision": 2, "title": "Updated title", "body": ""}
    )
...
def test_canonical_attachment_is_validated_and_not_copied(tmp_path):
    ...
    art = store.artifacts.create(
        name="Score", kind="document", content="C D E F", source="manual"
    )
    ...
    with pytest.raises(DomainError) as err:
        store.update(
            item["id"],
            {"revision": 1, "attachment_refs": [{"slug": art.slug, "version": 900}]},
        )
...
def test_two_homes_never_share_records_or_attachments(tmp_path):
    ...
    art = first.artifacts.create(
        name="Private score", kind="document", content="Private melody"
    )
    item = create(first)
    assert second.list() == []
    with pytest.raises(DomainError) as err:
    ...
    with pytest.raises(DomainError) as err:
        second.create(
            {
                "title": "Cross home",
                "attachment_refs": [{"slug": art.slug, "version": art.version}],
            }
        )
    assert err.value.code == "attachment_not_found"
    assert second.list() == []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"title": 8},
        {"title": "x" * 301},
        {"title": "Piece", "instrument": False},
        {"title": "Piece", "body": None},
        {"title": "Piece", "revision": 1},
        {"title": "Piece", "stage": "review"},
        {"title": "Piece", "practice_history": []},
        {"title": "Piece", "attachment_refs": {}},
        ...
        {
            "title": "Piece",
            "attachment_refs": [{"slug": "x", "version": 1, "path": "/tmp/a"}],
        },
        {"title": "Piece", "instrument": "kazoo"},
        {"title": "Piece", "capo": 13},
        {"title": "Piece", "tags": [""]},
        {"title": "Piece", "notation": {"format": "midi", "text": "x"}},
        {"title": "Piece", "notation": {"format": "tab"}},
        {"title": "Piece", "source_url": "file:///tmp/chart"},
        {"title": "Piece", "links": [{"type": "round", "id": "one"}]},
        {"title": "Piece", "links": [{"type": "../round", "id": "one", "label": ""}]},
        {"title": "Piece", "scroll_duration_seconds": 14},
        {"title": "Piece", "scroll_duration_seconds": True},
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("attempt_id", ""),
        ("attempt_id", "x" * 101),
        ("occurred_at", "2026-03-28T10:00:00"),
...
async def test_real_http_create_edit_practice_retry_and_reopen(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post(
            "/api/capabilities/music/items",
            json={"title": "HTTP piece", "body": "Am F C G"},
        )
        assert response.status == 201
        item = (await response.json())["item"]
        body = attempt(item, grade=2)
        response = await client.post(
            f'/api/capabilities/music/items/{item["id"]}/practice', json=body
        )
        ...
        response = await client.patch(
            f'/api/capabilities/music/items/{item["id"]}',
            json={"revision": 2, "instrument": "Guitar"},
        )
        assert response.status == 200
...
@pytest.mark.asyncio
async def test_http_songbook_metadata_partial_edit_practice_and_attachment_loss(
    tmp_path,
):
    store = store_at(tmp_path)
    score = store.artifacts.create(
        name="Imported chart", kind="document", content="C G Am F", source="import"
    )
    app = web.Application()
    register(app, store)
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert (
            edited["artist"] == payload["artist"]
            and edited["tuning"] == payload["tuning"]
        )
        assert edited["attachment_refs"] == payload["attachment_refs"]

        response = await client.post(
            prefix + "/" + item["id"] + "/practice",
            json=attempt(edited, key="metadata-practice"),
        )
        practiced = (await response.json())["item"]
        assert (
            practiced["artist"] == payload["artist"]
            and practiced["notation"] == edited["notation"]
        )
...
async def test_http_rejects_malformed_missing_and_conflicting_requests(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        invalid = await client.post(
            prefix, data="not-json", headers={"Content-Type": "application/json"}
        )
        ...
        stale = await client.patch(
            prefix + "/" + item["id"], json={"revision": 9, "title": "Stale"}
        )
        assert stale.status == 409
        assert (await stale.json())["error"] == "revision_conflict"
        unknown = await client.post(
            prefix + "/" + item["id"] + "/practice",
            json={**attempt(item), "path": "/tmp"},
        )
...
def test_sqlite_schema_and_failed_write_are_transactional(tmp_path):
    ...
    with sqlite3.connect(store.path) as connection:
        ...
        connection.execute(
            "CREATE TRIGGER reject_attempt BEFORE INSERT ON attempts BEGIN SELECT RAISE(ABORT, 'disk constraint'); END"
        )
