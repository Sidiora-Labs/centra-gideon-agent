...
from gideon.workspace.capabilities.music.midi import MidiStore, transcribe_pcm, midi_bytes
...
from test_catalog import catalog_at, attach_body


def recording(sequence=((69,.5),(None,.15),(72,.5)), rate=16000, harmonic=False, channels=1, width=2):
    pieces=[]
    for pitch, duration in sequence:
        ...
        signal=.35*np.sin(2*np.pi*frequency*t)
        if harmonic:
            signal+=.2*np.sin(4*np.pi*frequency*t)
        pieces.append(signal)
    samples=np.concatenate(pieces)
    output=io.BytesIO()
    with wave.open(output,'wb') as audio:
        audio.setnchannels(channels); audio.setsampwidth(width); audio.setframerate(rate)
        body=(samples*32767).astype('<i2').tobytes() if width==2 else bytes([128])*len(samples)
...
def seeded(home, raw=None):
    ...
    artifact=catalog.artifacts.create_binary(name='Real pitched PCM',data=raw or recording(),mime='audio/wav',kind='audio',source='import')
    attached=catalog.attach(track['id'],attach_body(track,artifact))
    data={'request_id':'transcribe-one','track_id':track['id'],'render_id':attached['renders'][0]['id'],'title':'Detected phrase','tempo_bpm':120}
    return store,data
...
def read_midi(raw):
    ...
    body=io.BytesIO(raw[22:]); events=[]; tick=0
    while body.tell()<length:
        delta=0
        while True:
            octet=body.read(1)[0]; delta=(delta<<7)|(octet&127)
            if not octet&128:
                break
        tick+=delta
...
def test_actual_sinusoidal_pcm_pitch_is_measured(pitch):
    duration, notes=transcribe_pcm(recording(((pitch,.6),)))
    assert duration==pytest.approx(.6)
    assert len(notes)==1
    assert notes[0]['pitch']==pitch
    assert notes[0]['start_seconds']==pytest.approx(0,abs=.03)
    assert notes[0]['duration_seconds']==pytest.approx(.6,abs=.03)
...
def test_phrase_pitch_silence_and_timing_at_supported_rates(rate):
    ...
    assert notes[0]['start_seconds']==pytest.approx(0,abs=.04)
    assert notes[0]['duration_seconds']==pytest.approx(.5,abs=.06)
    assert notes[1]['start_seconds']==pytest.approx(.65,abs=.06)
    assert notes[1]['duration_seconds']==pytest.approx(.5,abs=.06)


def test_harmonic_recording_retains_fundamental_and_silence_stays_empty():
    _,notes=transcribe_pcm(recording(((57,.7),),harmonic=True))
    assert [note['pitch'] for note in notes]==[57]
    assert transcribe_pcm(recording(((None,.6),)))[1]==[]


@pytest.mark.parametrize('raw',[b'not wav', recording(channels=2),recording(width=1),recording(rate=4000),recording(((60,60.01),)),recording()[:-10]])
...
def test_concurrent_exact_retries_return_one_item(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results=list(pool.map(lambda _:store_at(tmp_path).transcribe(data),range(2)))
    assert results[0]==results[1]
    assert store.list()==[results[0]]


@pytest.mark.parametrize('changes',[{'pitch':35},{'pitch':97},{'pitch':True},{'velocity':0},{'velocity':128},{'start_seconds':-1},{'start_seconds':math.inf},{'duration_seconds':math.nan},{'duration_seconds':0},{'duration_seconds':2},{'id':''},{'home':'/tmp'}])
...
def test_duplicate_ids_stale_revision_and_unknown_fields_refused(tmp_path):
    store,data=seeded(tmp_path)
    item=store.transcribe(data)
    for payload in ({'revision':1,'title':'Duplicate','notes':[item['notes'][0]]*2},{'revision':2,'title':'Stale','notes':[]},{'revision':1,'title':'Injected','notes':[],'model':'other'}):
        with pytest.raises(DomainError):
...
def test_export_has_actual_midi_tempo_pitch_timing_and_canonical_identity(tmp_path):
    store,data=seeded(tmp_path)
    item=store.transcribe(data)
    edited=[{'id':'note-a','pitch':60,'start_seconds':.1,'duration_seconds':.4,'velocity':91},
            {'id':'note-b','pitch':64,'start_seconds':.5,'duration_seconds':.3,'velocity':87}]
    item=store.update(item['id'],{'revision':1,'title':'Edited MIDI','notes':edited})
    ...
    assert len(read_midi(store.catalog.artifacts.raw_bytes(newer['artifact_ref']['slug'],version=1)[0]))==2


def test_midi_variable_length_deltas_and_nondefault_tempo():
    raw=midi_bytes([{'pitch':72,'start_seconds':30,'duration_seconds':2,'velocity':127}],90)
    events=read_midi(raw)
...
async def test_real_http_transcribe_edit_export_reload_and_conflict(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        edited=await client.patch(prefix+'/'+item['id'],json={'revision':1,'title':'HTTP edit','notes':[]})
        assert edited.status==200
        assert (await edited.json())['item']['revision']==2
        stale=await client.post(prefix+'/'+item['id']+'/export',json={'revision':1})
        assert stale.status==409
        export=await client.post(prefix+'/'+item['id']+'/export',json={'revision':2})
        assert export.status==200
        ref=(await export.json())['artifact_ref']
        assert store.catalog.artifacts.raw_bytes(ref['slug'],version=ref['version'])[0].startswith(b'MThd')
...
async def test_real_native_operations_and_no_caller_home(tmp_path):
    ...
    changed=await tools.invoke('music_midi_update',{'id':item['id'],'data':{'revision':1,'title':'Native edit','notes':[]}})
    assert changed.success
    exported=await tools.invoke('music_midi_export',{'id':item['id'],'data':{'revision':2}})
