...
from gideon.workspace.capabilities.music.generation import MusicGeneration, ENDPOINT, MODELS
...
from test_catalog import catalog_at, wav_bytes, attach_body
...
def request_for(service):
    ...
    return {'request_id': 'composition-request', 'track_id': track['id'], 'track_revision': 1,
            'prompt': 'Gentle instrumental with acoustic guitar', 'music_length_ms': 5000,
            'force_instrumental': True, 'license': 'Account license to be verified'}


def seed_job(service, request, status='running'):
    job = {'id': request['request_id'], 'status': status, 'request': request, 'model': 'music_v1',
...
def test_default_engine_is_explicitly_disabled_and_unverified(tmp_path):
    ...
    assert ready['config'] == {'enabled': False, 'model': 'music_v1', 'credential_name': '', 'revision': 0}
...
def test_configuration_reopens_and_secrets_stay_in_canonical_store(tmp_path):
    ...
    credentials.save({'composition-key': {'type': 'api_key', 'value': 'fixture-never-sent-to-provider'}})
    service = service_at(tmp_path)
    config = service.configure({'enabled': True, 'model': 'music_v2', 'credential_name': 'composition-key', 'revision': 0})
    ...
    assert credentials.resolve('composition-key').secret == 'fixture-never-sent-to-provider'
...
def test_credential_rotation_and_removal_change_local_readiness(tmp_path):
    service = service_at(tmp_path)
    service.configure({'enabled': True, 'model': 'music_v1', 'credential_name': 'key', 'revision': 0})
...
def test_configuration_conflict_preserves_prior_value(tmp_path):
    service = service_at(tmp_path)
    first = service.configure({'enabled': False, 'model': 'music_v2_5', 'credential_name': 'key', 'revision': 0})
    with pytest.raises(DomainError) as err:
        service.configure({'enabled': True, 'model': 'music_v1', 'credential_name': 'other', 'revision': 0})
...
@pytest.mark.parametrize('changes', [
    {'enabled': 'true'}, {'enabled': 1}, {'model': 'invented-model'},
    {'credential_name': 12}, {'credential_name': 'x' * 101}, {'revision': -1},
    {'api_key': 'must-not-be-persisted'}, {'endpoint': 'https://example.com'},
    {'home': '/tmp/another'}, {'provider': 'unreviewed'},
])
...
async def test_missing_credential_refuses_before_creating_job_or_artifact(tmp_path):
    ...
    service.configure({'enabled': True, 'model': 'music_v1', 'credential_name': 'missing', 'revision': 0})
    with pytest.raises(DomainError) as err:
...
async def test_disabled_engine_refuses_even_with_local_credential(tmp_path):
    CredentialStore(tmp_path).save({'key': {'type': 'api_key', 'value': 'not-a-live-key'}})
    service = service_at(tmp_path)
    service.configure({'enabled': False, 'model': 'music_v1', 'credential_name': 'key', 'revision': 0})
    with pytest.raises(DomainError) as err:
...
@pytest.mark.parametrize('field,value', [
    ('request_id', ''), ('prompt', ''), ('prompt', 'x' * 4101),
    ('music_length_ms', 2999), ('music_length_ms', 600001), ('music_length_ms', True),
    ('force_instrumental', 'true'), ('track_revision', 0), ('license', ''),
    ('track_id', False), ('api_key', 'forbidden'), ('model', 'caller-override'),
])
async def test_generation_request_validation_precedes_submission(tmp_path, field, value):
    service = service_at(tmp_path)
    request = request_for(service)
    with pytest.raises(DomainError) as err:
...
def test_homes_isolate_configuration_credentials_and_job_history(tmp_path):
    ...
    CredentialStore(tmp_path / 'one').save({'key': {'type': 'api_key', 'value': 'one-home-only'}})
    first.configure({'enabled': True, 'model': 'music_v1', 'credential_name': 'key', 'revision': 0})
...
def test_actual_mp3_encoding_is_measured_from_canonical_bytes(tmp_path):
    ...
    subprocess.run(['ffmpeg', '-v', 'error', '-y', '-i', str(original), str(encoded)], check=True, timeout=20)
    art = service.catalog.artifacts.create_binary(name='Encoded recorded fixture', kind='audio', mime='audio/mpeg', data=encoded.read_bytes(), source='import')
    ...
    assert service.catalog.artifacts.raw_bytes(art.slug, version=1)[0] == encoded.read_bytes()
...
async def test_actual_http_config_readiness_refusal_and_job_recovery(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.patch(prefix + '/config', json={'enabled': True, 'model': 'music_v2', 'credential_name': 'missing', 'revision': 0})
        assert response.status == 200
...
async def test_native_controls_preserve_provider_absence_and_secret_boundary(tmp_path):
    ...
    configured = await provider.invoke('music_generation_configure', {'data': {'enabled': True, 'model': 'music_v1', 'credential_name': 'missing', 'revision': 0}})
    assert configured.success
    refusal = await provider.invoke('music_generation_submit', {'data': request_for(service)})
    assert refusal.success is False
    assert refusal.metadata['code'] == 'engine_unavailable'
    invalid = await provider.invoke('music_generation_readiness', {'home': '/tmp/another'})
