...
from checks.runtime.capabilities.music.test_catalog import catalog_at
from checks.runtime.capabilities.music.test_midi import recording
from checks.runtime.capabilities.music.test_midi import seeded as audio_seeded
...
def seeded(home):
    ...
    for color in ("red", "blue"):
        out = io.BytesIO()
        Image.new("RGB", (96, 64), color).save(out, format="PNG")
        artifact = audio.catalog.artifacts.create_binary(
            name=color + " scene",
            data=out.getvalue(),
            mime="image/png",
            kind="image",
    ...
    payload = {
        "title": "Beat-cut video",
        "track_id": data["track_id"],
        "render_id": data["render_id"],
        "tempo_bpm": 120,
        "offset_seconds": 0.25,
        "scenes": [
            {"id": "red", "image_ref": refs[0], "beats": 1},
            {"id": "blue", "image_ref": refs[1], "beats": 1},
        ],
    }
...
def edit(item, **changes):
    return (
        ...
        | {
            "scenes": [
                {key: row[key] for key in ("id", "image_ref", "beats")}
                for row in item["scenes"]
            ]
        }
        | changes
    )
...
def test_project_quantizes_authored_beats_and_preserves_canonical_refs(tmp_path):
    ...
    assert item["offset_seconds"] == 0.25
    assert [row["start_seconds"] for row in item["scenes"]] == [0, 0.5]
    assert [row["duration_seconds"] for row in item["scenes"]] == [0.5, 0.5]
    assert [row["image_ref"] for row in item["scenes"]] == [
        row["image_ref"] for row in payload["scenes"]
    ]
...
def test_edit_retimes_scenes_and_refuses_stale_revision(tmp_path):
    ...
    assert updated["scenes"][1]["start_seconds"] == pytest.approx(0.6)
...
async def test_actual_ffmpeg_render_has_audio_video_and_correct_scene_colors(tmp_path):
    ...
    assert float(metadata["format"]["duration"]) == pytest.approx(1, abs=0.08)
    for timestamp, color in ((0.2, 0), (0.8, 2)):
...
async def test_http_and_native_share_owned_process_cancellation(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            "/api/capabilities/music/videos/jobs", json=render_request(item)
        )
        ...
        bad = await client.post(
            "/api/capabilities/music/videos/jobs/" + job["id"] + "/cancel",
            json={"home": "/tmp"},
        )
...
async def test_native_real_create_get_update_list_job_and_invalid_arguments(tmp_path):
    ...
    denied = await tools.invoke(
        "music_video_render", {"data": render_request(current), "home": "/tmp"}
    )
...
"""
    response = subprocess.run(
        ...
        env={**os.environ, "GIDEON_HOME": str(tmp_path / "native-home")},
        capture_output=True,
        text=True,
        check=True,
    )
