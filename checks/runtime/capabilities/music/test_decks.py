...
from gideon.workspace.capabilities.music.deck_layout import MAJORS, compose, roster
...
def store_at(home):
    return DeckStore(home, NativeArtifactProvider(root=home / "artifacts"))


def picture(store, color="red"):
    data = io.BytesIO()
    Image.new("RGB", (80, 120), color).save(data, "PNG")
    artifact = store.artifacts.create_binary(
        name="Authored color art",
        data=data.getvalue(),
        mime="image/png",
        kind="image",
        source="import",
...
def test_revision_history_styles_and_archive(tmp_path):
    store = store_at(tmp_path)
    item = store.create({"name": "First", "kind": "playing"})
    sample = picture(store)
    context = store.artifacts.create(
        name="Authored setting",
        content='{"setting":"Garden"}',
        kind="json",
        source="import",
    )
    changed = store.update(
        item["id"],
        {
            "revision": 1,
            "name": "Garden",
            "style_notes": "Botanical ink",
            "negative_prompt": "Blur",
            "sample_refs": [sample],
            "context_ref": {"slug": context.slug, "version": context.version},
            "orientation": "one_way",
            "archived": True,
        },
    )
    ...
    with pytest.raises(DomainError) as error:
        store.update(item["id"], {"revision": 1, "name": "Lost update"})
...
def test_card_edits_pinned_art_and_clear(tmp_path):
    store = store_at(tmp_path)
    item = store.create({"name": "Deck", "kind": "playing"})
    ref = picture(store)
    item = store.card(
        item["id"],
        "spades-A",
        card_data(
            item,
            name="Garden Ace",
            prompt="A flowering tree",
            negative_prompt="No letters",
            artifact_ref=ref,
        ),
    )
    ...
    with pytest.raises(DomainError):
        store.card(item["id"], "unknown", card_data(item))
    with pytest.raises(DomainError):
        store.card(item["id"], "spades-A", {**card_data(item), "rank": "K"})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"bleed_mm": -1},
        {"safe_mm": 1},
        {"width_mm": 30, "safe_mm": 15},
        {"orientation": "diagonal"},
        {"archived": "yes"},
        {"sample_refs": [None]},
        {"context_ref": {"slug": "missing", "version": 1}},
        {"provider": "private"},
        {"name": ""},
    ],
)
def test_invalid_update_preserves_revision(tmp_path, fields):
    ...
    item = store.create({"name": "Deck", "kind": "playing"})
    with pytest.raises(DomainError):
        store.update(item["id"], {"revision": 1, **fields})
...
def test_foreign_missing_wrong_kind_and_corrupt_art(tmp_path):
    ...
    item = store.create({"name": "Deck", "kind": "playing"})
    with pytest.raises(DomainError):
        store.card(item["id"], "spades-A", card_data(item, artifact_ref=ref))
    json_art = store.artifacts.create(
        name="JSON", content="{}", kind="json", source="import"
    )
    with pytest.raises(DomainError):
        store.image({"slug": json_art.slug, "version": json_art.version})
    bad = store.artifacts.create_binary(
        name="Invalid PNG",
        data=b"not a PNG",
        mime="image/png",
        kind="image",
    ...
    with pytest.raises(DomainError):
        store.image({"slug": bad.slug, "version": bad.version})
...
def test_pdf_actual_dimensions_pages_text_images_and_cached_receipt(tmp_path):
    store = store_at(tmp_path)
    item = store.create({"name": "Print deck", "kind": "playing"})
    ref = picture(store)
    ...
    raw, mime = store.artifacts.raw_bytes(
        receipt["pdf_ref"]["slug"], version=receipt["pdf_ref"]["version"]
    )
    assert mime == "application/pdf" and raw.startswith(b"%PDF-")
    with pdfplumber.open(io.BytesIO(raw)) as pdf:
        assert len(pdf.pages) == 55
        page = pdf.pages[0]
        assert page.width == pytest.approx((63.5 + 6) * 72 / 25.4, abs=0.001)
        assert page.height == pytest.approx((88.9 + 6) * 72 / 25.4, abs=0.001)
    ...
    manifest = store.artifacts.get(
        receipt["manifest_ref"]["slug"], version=receipt["manifest_ref"]["version"]
    )
...
def test_actual_prompt_composition_and_durable_job_submission(tmp_path):
    ...
    item = store.update(
        item["id"],
        {"revision": 1, "style_notes": "Watercolor", "negative_prompt": "Blur"},
    )
    item = store.card(
        item["id"],
        "spades-A",
        card_data(item, prompt="An oak tree", negative_prompt="Text"),
    )
    ...
    with pytest.raises(DomainError):
        store.generate(item["id"], {**data, "card_keys": ["back"]})
    with pytest.raises(DomainError) as error:
        store.adopt(item["id"], "spades-A", {"revision": 3, "job_id": job["id"]})
    assert error.value.code == "job_incomplete"
    with pytest.raises(DomainError) as error:
        store.adopt(item["id"], "back", {"revision": 3, "job_id": job["id"]})
...
def test_linked_success_adopts_verified_art_and_failed_or_corrupt_output_does_not(
    tmp_path,
):
    store = store_at(tmp_path)
    item = store.create({"name": "Deck", "kind": "playing"})
    ref = picture(store, "blue")
    receipt = store.generate(
        item["id"],
        {
            "revision": 1,
    ...
    finished = store.jobs.finish(
        job["id"], result={"artifact_id": ref["slug"], "version": ref["version"]}
    )
    ...
    failed_job = store.jobs.claim()
    store.jobs.finish(failed_job["id"], error="Provider rejected the request")
    assert failed_job["id"] == failed_receipt["jobs"][0]["job_id"]
    with pytest.raises(DomainError) as error:
        store.adopt(item["id"], "hearts-A", {"revision": 2, "job_id": failed_job["id"]})
    assert error.value.code == "job_incomplete"
    ...
    corrupt_receipt = store.generate(
        item["id"],
        {
            "revision": 2,
            "request_id": "corrupt",
            "card_keys": ["diamonds-A"],
            "size": "",
            "controls": {},
        },
    )
    corrupt_job = store.jobs.claim()
    store.jobs.finish(
        corrupt_job["id"], result={"artifact_id": bad.slug, "version": bad.version}
    )
    assert corrupt_job["id"] == corrupt_receipt["jobs"][0]["job_id"]
    with pytest.raises(DomainError):
        store.adopt(
            item["id"], "diamonds-A", {"revision": 2, "job_id": corrupt_job["id"]}
        )
    assert store.get(item["id"]) == item
...
def test_concurrent_revisions_only_one_wins(tmp_path):
    ...
    def change(index):
        try:
            return store_at(tmp_path).update(
                item["id"], {"revision": 1, "name": str(index)}
            )["revision"]
        except DomainError:
            return "conflict"
...
async def test_http_native_actual_print_and_pinned_download(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post(
            base, json={"name": "HTTP cards", "kind": "playing"}
        )
        assert response.status == 200
        item = (await response.json())["item"]
        changed = await client.patch(
            base + "/" + item["id"] + "/cards/back",
            json=card_data(item, "back", prompt="Woven branches"),
        )
        assert changed.status == 200
        item = (await changed.json())["item"]
        result = await (
            await client.post(base + "/" + item["id"] + "/export", json={"revision": 2})
        ).json()
        ref = result["pdf_ref"]
        response = await client.get(
            base + f"/artifacts/{ref['slug']}/{ref['version']}/raw"
        )
        assert response.status == 200 and response.content_type == "application/pdf"
        assert (await response.read()).startswith(b"%PDF-")
        assert (
            len(
                (await (await client.get(base + "/" + item["id"] + "/history")).json())[
                    "items"
                ]
            )
            == 2
        ...
        assert (
            await client.patch(
                base + "/" + item["id"], json={"revision": 1, "name": "stale"}
            )
        ).status == 409
    ...
    } <= {tool.name for tool in await tools.list_tools()}
    for action, args in [
        ("get", {"id": item["id"]}),
        ("list", {}),
        ("history", {"id": item["id"]}),
        ("export", {"id": item["id"], "data": {"revision": 2}}),
    ]:
    ...
    assert not (
        await tools.invoke(
            "music_decks_adopt",
            {
                "id": item["id"],
                "key": "back",
                "data": {"revision": 2, "job_id": "unknown"},
            },
        )
    ).success
