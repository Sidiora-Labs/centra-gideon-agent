...
from gideon.workspace.capabilities.music.deck_assist import (
    DeckAssist,
    apply_values,
    parse_result,
)
...
def universe_at(assist):
    return assist.universes.create(
        {
            "request_id": "garden",
            "title": "Garden",
            "canon": [
                {
                    "id": "hero",
                    "title": "Willow",
                    "body": "A patient gardener who protects the river.",
                },
                {
                    "id": "tower",
                    "title": "Stone tower",
                    "body": "An abandoned lookout above the orchard.",
                },
            ],
            "visual_identity": {
                "colors": ["#445566"],
                "style_notes": "Botanical copper engravings",
            },
            "ingredient_ids": [],
            "board_refs": [],
        }
...
def prompt_request(item, universe=None, **changes):
    return {
        ...
        "card_keys": ["spades-A"],
        "universe_ref": (
            {"id": universe["id"], "revision": universe["revision"]}
            if universe
            else None
        ),
        **changes,
...
def output(**changes):
    return {
        "cards": [
            {
                "key": "spades-A",
                "prompt": "Willow tending an oak sapling beside the river.",
...
def test_pinned_universe_grounding_survives_canon_edits(tmp_path):
    assist = assist_at(tmp_path)
    item = assist.decks.create({"name": "Garden deck", "kind": "playing"})
    ...
    assist.universes.update(
        universe["id"],
        {
            "revision": 1,
            "title": "Updated setting",
            "canon": [{"id": "other", "title": "Someone else", "body": "New canon"}],
        },
    )
...
def test_actual_image_message_bytes_are_pinned_and_bounded(tmp_path):
    ...
    assert messages[1]["content"][0] == {
        "type": "text",
        "text": "Describe this actual image.",
    }
    ...
    assert assist.model.messages("Only text.")[1]["content"] == [
        {"type": "text", "text": "Only text."}
    ]
    with pytest.raises(DomainError):
        assist.model.messages("x" * 60001)
    with pytest.raises(DomainError):
        assist.model.messages("", ref)
    with pytest.raises(DomainError):
        assist.model.messages("Missing", {"slug": "unknown", "version": 1})


def test_analysis_preparation_uses_canonical_sample(tmp_path):
    assist = assist_at(tmp_path)
    item = assist.decks.create({"name": "Sample", "kind": "tarot"})
    ref = picture(assist.decks)
    data = {
        "revision": 1,
        "request_id": "sample",
        "provider_name": "not-configured",
...
def test_strict_prompt_parser_and_real_canon_mapping(tmp_path):
    assist = assist_at(tmp_path)
    item = assist.decks.create({"name": "Garden", "kind": "playing"})
    universe = universe_at(assist)
    parsed = parse_result(
        "prompts", json.dumps(output()), ["spades-A"], {"hero", "tower"}
    )
    ...
    assert changed["cards"][0]["canon_ref"] == {
    ...
    without_cast = parse_result(
        "prompts", json.dumps(output(canon_id=None)), ["spades-A"], set()
    )
    proposal["result"] = without_cast
    proposal["universe_ref"] = None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        output(canon_id={}),
        {"cards": []},
        {"cards": [output()["cards"][0], output()["cards"][0]]},
        {"cards": [{"key": "spades-A", "prompt": "Tree", "negative_prompt": ""}]},
        {"cards": [output()["cards"][0]], "extra": True},
        [],
    ],
)
def test_rejects_bad_generated_casting_without_mutation(tmp_path, document):
    assist = assist_at(tmp_path)
    item = assist.decks.create({"name": "Deck", "kind": "playing"})
    with pytest.raises(DomainError):
        parse_result("prompts", json.dumps(document), ["spades-A"], {"hero"})
...
def test_analysis_validation_and_reference_limit(tmp_path):
    ...
    ref = picture(assist.decks)
    analysis = {
        "style_notes": "Blue ink on cream paper",
        "layout_prompt": "Framed portrait",
        "negative_prompt": "Heavy blur",
    }
    ...
    with pytest.raises(DomainError):
        apply_values(changed, proposal)
    for value in (
        "broken",
        "[]",
        "{}",
        json.dumps({**analysis, "extra": 1}),
        json.dumps({**analysis, "style_notes": ""}),
    ):
        with pytest.raises(DomainError):
            parse_result("analyze", value, [], set())
    with pytest.raises(DomainError):
        parse_result("analyze", "x" * 65537, [], set())


@pytest.mark.asyncio
async def test_absent_actual_provider_persists_failed_receipt_and_idempotent_retry(
    tmp_path,
):
    assist = assist_at(tmp_path)
    item = assist.decks.create({"name": "Deck", "kind": "playing"})
    data = prompt_request(item)
    ...
    with pytest.raises(DomainError) as error:
        await assist.propose(item["id"], "prompts", {**data, "card_keys": ["back"]})
    assert error.value.status == 409
    with pytest.raises(DomainError) as error:
        assist.apply(item["id"], result["id"], {"revision": 1})
...
async def test_missing_sample_or_foreign_universe_never_calls_model(tmp_path):
    assist = assist_at(tmp_path / "one")
    item = assist.decks.create({"name": "Deck", "kind": "playing"})
    other = assist_at(tmp_path / "other")
    foreign = universe_at(other)
    with pytest.raises(ValueError):
        await assist.propose(item["id"], "prompts", prompt_request(item, foreign))
    with pytest.raises(DomainError):
        await assist.propose(
            item["id"],
            "analyze",
            {
                "revision": 1,
                ...
                "sample_ref": picture(other.decks),
            },
        )
...
def test_request_validation(tmp_path, changes):
    ...
    item = assist.decks.create({"name": "Deck", "kind": "playing"})
    with pytest.raises(ValueError):
        assist.prepare(item["id"], "prompts", prompt_request(item, **changes))
    assert assist.list(item["id"]) == []
...
async def test_real_http_and_native_refusal_isolation_and_persisted_receipts(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        assert (await (await client.get(base + "/assist/providers")).json()) == {
            "items": []
        }
        response = await client.post(
            base + "/" + item["id"] + "/assist/prompts", json=prompt_request(item)
        )
        assert response.status == 200
        proposal = (await response.json())["item"]
        assert proposal["status"] == "failed"
        assert (
            await client.get(base + "/" + second["id"] + "/assist/" + proposal["id"])
        ).status == 404
        rows = await (await client.get(base + "/" + item["id"] + "/assist")).json()
        assert len(rows["items"]) == 1
        assert (
            await client.post(
                base + "/" + item["id"] + "/assist/" + proposal["id"] + "/apply",
                json={"revision": 1},
            )
        ).status == 409
        assert (
            await client.post(
                base + "/" + item["id"] + "/assist/prompts",
                json={**prompt_request(item), "model": "override"},
            )
        ).status == 409
    ...
    receipt = await tools.invoke(
        "music_decks_assist_get", {"id": item["id"], "proposal_id": proposal["id"]}
    )
    assert json.loads(receipt.output)["status"] == "failed"
    assert not (
        await tools.invoke(
            "music_decks_assist_apply",
            {"id": item["id"], "proposal_id": proposal["id"], "data": {"revision": 1}},
        )
    ).success
    assert not (
        await tools.invoke("music_decks_assist_providers", {"home": "foreign"})
    ).success
