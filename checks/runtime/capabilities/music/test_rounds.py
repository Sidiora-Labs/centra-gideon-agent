...
from test_catalog import catalog_at, artifact, attach_body
...
def part(part_id='voice-a', entry=0):
    return {'id': part_id, 'name': part_id, 'entry_beats': entry, 'notation': 'D E F# G', 'catalog_ref': None}


def create(store, title='Evening round', **values):
    return store.create({'title': title, 'tempo_bpm': 120, 'meter_beats': 4, 'parts': [part(), part('voice-b', 4)], **values})


def practice(item, request_id='take-one', **values):
    return {'request_id': request_id, 'round_revision': item['revision'], 'part_ids': [item['parts'][0]['id']],
            'occurred_at': '2026-09-25T12:00:00+02:00', 'grade': 4, 'notes': 'Entry was together', **values}
...
def test_practice_is_idempotent_and_snapshot_survives_part_edits(tmp_path):
    ...
    changed = store.update(item['id'], {'revision': 1, 'parts': [{**part('new-voice'), 'notation': 'A B C'}]})
    ...
    with sqlite3.connect(store.path) as db:
        original = json.loads(db.execute('SELECT payload FROM versions WHERE id=? AND revision=1', (item['id'],)).fetchone()[0])
...
def test_parallel_retry_records_one_practice(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(lambda _: store_at(tmp_path).practice(item['id'], request), range(2)))
...
def test_grade_zero_is_recorded_as_failure_not_missing(tmp_path):
    store = store_at(tmp_path)
    item = create(store)
    result = store.practice(item['id'], practice(item, grade=0, part_ids=['voice-a', 'voice-b']))
...
@pytest.mark.parametrize('values', [
    {'title': ''}, {'tempo_bpm': 19}, {'tempo_bpm': 301}, {'tempo_bpm': True},
    {'meter_beats': 0}, {'meter_beats': 17}, {'parts': []}, {'parts': [part()] * 17},
    {'parts': [part(), part()]}, {'parts': [{**part(), 'entry_beats': -1}]},
    {'parts': [{**part(), 'entry_beats': float('nan')}]}, {'parts': [{**part(), 'entry_beats': float('inf')}]},
    {'parts': [{**part(), 'entry_beats': True}]}, {'parts': [{**part(), 'path': '/tmp'}]},
    {'parts': [{**part(), 'catalog_ref': {'artifact_ref': 'unbound'}}]},
    {'partner_ids': ['missing']}, {'partner_ids': [False]}, {'revision': 9}, {'history': []},
])
...
@pytest.mark.parametrize('values', [
    {'grade': -1}, {'grade': 6}, {'grade': True}, {'part_ids': []}, {'part_ids': ['missing']},
    {'part_ids': ['voice-a', 'voice-a']}, {'part_ids': [None]},
    {'occurred_at': '2026-09-25T12:00:00'}, {'occurred_at': 'yesterday'},
    {'request_id': ''}, {'round_revision': 0}, {'notes': None}, {'home': '/tmp'},
])
...
async def test_actual_http_round_author_edit_practice_replay_and_history(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post(prefix, json={'title': 'HTTP canon', 'parts': [part()]})
        assert response.status == 201
        item = (await response.json())['item']
        response = await client.post(prefix + '/' + item['id'] + '/practice', json=practice(item))
        assert response.status == 200
        receipt = await response.json()
        retry = await client.post(prefix + '/' + item['id'] + '/practice', json=practice(item))
        ...
        changed = await client.patch(prefix + '/' + item['id'], json={'revision': 1, 'notes': 'Edited notes'})
...
async def test_native_round_tools_use_actual_persisted_domain(tmp_path):
    ...
    created = await tools.invoke('music_rounds_create', {'data': {'title': 'Native canon', 'parts': [part()]}})
    assert created.success
    item = json.loads(created.output)
    result = await tools.invoke('music_rounds_practice', {'id': item['id'], 'data': practice(item)})
    ...
    changed = await tools.invoke('music_rounds_update', {'id': item['id'], 'data': {'revision': 1, 'notes': 'Native edit'}})
