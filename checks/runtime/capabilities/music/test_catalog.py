...
def artifact(store, frames=8000):
    return store.artifacts.create_binary(
        name="Recorded practice fixture",
        kind="audio",
        mime="audio/wav",
...
def attach_body(track, art):
    return {
        ...
        "source": {
            "kind": "imported",
            "label": "Locally authored test recording",
...
def test_artist_album_track_relationships_and_order_survive_reopen(tmp_path):
    ...
    first = store.create(
        "tracks", {"title": "First", "artist_id": artist["id"], "notes": "Take notes"}
    )
    ...
    changed = store.update(
        "albums", album["id"], {"revision": 1, "track_ids": [second["id"], first["id"]]}
    )
...
def test_measured_duration_checksum_and_source_are_retained(tmp_path):
    ...
    assert store.artifacts.raw_bytes(art.slug, version=1) == (
        wav_bytes(12000),
        "audio/wav",
    )
...
def test_additional_renders_selection_and_metadata_edits_do_not_rewrite_audio(tmp_path):
    ...
    selected = store.select(
        track["id"],
        {"revision": second["revision"], "render_id": second["renders"][1]["id"]},
    )
...
def test_simultaneous_attachment_retry_uses_one_render(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(
            pool.map(lambda current: current.attach(track["id"], body), repositories)
        )
...
def test_archive_filter_and_restore_preserve_album_links(tmp_path):
    ...
    album = store.create(
        "albums", {"title": "Archive collection", "track_ids": [track["id"]]}
    )
...
def test_duplicate_album_tracks_and_cross_collection_ids_rejected(tmp_path):
    ...
    with pytest.raises(DomainError):
        store.create(
            "albums", {"title": "Repeated", "track_ids": [track["id"], track["id"]]}
        )
    with pytest.raises(DomainError):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tracks", {"title": "x", "renders": []}),
        ("tracks", {"title": "x", "selected_render_id": "fake"}),
        ("artists", {"name": "x", "archived": "false"}),
        ("artists", {"name": "x", "revision": 1}),
        ("tracks", {"title": "x", "duration_seconds": 90}),
        ("tracks", {"title": "x", "path": "/tmp/elsewhere"}),
    ],
)
def test_invalid_metadata_cannot_create_partial_rows(tmp_path, kind, data):
    ...
    assert (
        sum(
            len(store.list(collection))
            for collection in ("artists", "albums", "tracks")
        )
        == 0
    )
...
def test_imported_source_cannot_smuggle_model_duration_or_job_claims(tmp_path):
    ...
    for key in ("model", "job_id", "duration_seconds"):
        with pytest.raises(DomainError):
            store.attach(
                track["id"], {**body, "source": {**body["source"], key: "claimed"}}
            )
...
def test_invalid_truncated_and_empty_audio_rejected(tmp_path, data):
    ...
    art = store.artifacts.create_binary(
        name="Invalid fixture", data=data, kind="audio", mime="audio/wav"
    )
    with pytest.raises(DomainError) as err:
...
def test_non_audio_and_missing_versions_cannot_be_attached(tmp_path):
    ...
    art = store.artifacts.create(
        name="Notes", kind="document", content="Text is not audio"
    )
    with pytest.raises(DomainError):
        store.attach(track["id"], attach_body(track, art))
...
def test_pagination_filters_before_slicing(tmp_path):
    store = catalog_at(tmp_path)
    expected = [
        store.create("artists", {"name": "Match " + str(index)}) for index in range(5)
    ]
    ...
    assert len(last) == 1
    assert {row["id"] for row in first + second + last} == {
...
def test_canonical_audio_bytes_version_roundtrip(tmp_path):
    ...
    initial = provider.create_binary(
        name="Versioned audio", kind="audio", mime="audio/wav", data=wav_bytes(4000)
    )
...
async def test_actual_http_catalog_journey(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        response = await client.post(
            prefix + "/tracks",
            json={"title": "HTTP track", "artist_id": artist_row["id"]},
        )
        track = (await response.json())["item"]
        response = await client.post(
            prefix + f'/tracks/{track["id"]}/renders',
            json=attach_body(track, artifact(store)),
        )
        assert response.status == 200
        attached = (await response.json())["item"]
        assert attached["renders"][0]["duration_seconds"] == 1
        response = await client.patch(
            prefix + f'/tracks/{track["id"]}', json={"revision": 2, "archived": True}
        )
        assert response.status == 200
        filtered = await client.get(prefix + "/tracks?archived=true&q=HTTP")
        assert len((await filtered.json())["items"]) == 1
        stale = await client.patch(
            prefix + f'/tracks/{track["id"]}', json={"revision": 1, "title": "Stale"}
        )
    ...
    assert (
        catalog_at(tmp_path).get("tracks", track["id"])["renders"]
        == attached["renders"]
    )
