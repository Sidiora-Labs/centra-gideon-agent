...
from gideon.workspace.capabilities.music.listening import (
    ListeningStore,
    instant,
    normalize_api,
    normalize_history,
...
def store_at(home):
    return ListeningStore(home, NativeArtifactProvider(root=home / "artifacts"))
...
def extended(**changes):
    return {
        ...
        "ms_played": 12500,
        "master_metadata_track_name": "Measured song",
        "master_metadata_album_artist_name": "Actual artist",
...
def legacy(**changes):
    return {
        "endTime": "2026-01-02 03:04",
        "msPlayed": 12500,
        "trackName": "Measured song",
        "artistName": "Actual artist",
        **changes,
    }


def api_track(**changes):
    return {
        "name": "API song",
...
def payload(
    store, document, format="spotify_extended", request_id="first", account="owner"
):
    art = store.artifacts.create(
        name="Actual export", content=json.dumps(document), kind="json", source="import"
    )
    return {
...
def test_extended_pinned_facts_and_reopen(tmp_path):
    ...
    data = payload(
        ...
        [
            extended(),
            extended(ts="2026-01-02T04:04:05+01:00"),
            extended(master_metadata_track_name=None),
        ],
    )
...
def test_api_duration_unknown_and_time_semantics_separate(tmp_path):
    ...
    document = {
        "recent": [
            {
                "played_at": "2026-01-02T03:04:05Z",
                "track": api_track(
                    name="Measured song", artists=[{"name": "Actual artist"}]
                ),
            }
        ],
        "playlists": [],
...
def test_playlist_order_duplicates_snapshots_and_source(tmp_path):
    ...
    second["playlists"][0]["items"].insert(
        0, {"track": {"trackName": "Two", "artistName": "Second"}}
    )
    ...
    assert [row["title"] for row in detail["current"]["tracks"]] == [
        "Two",
        "One",
        "One",
    ]
...
def test_import_identity_conflict_and_account_isolation(tmp_path):
    ...
    with pytest.raises(DomainError) as error:
        store.import_data({**first, "account_label": "different"})
...
def test_search_sort_pagination_stats(tmp_path):
    ...
    assert [row["title"] for row in page["items"]] == [
        f"Song {i}" for i in range(19, 9, -1)
    ]
    ...
    assert summary["top_artists"] == [
        {"artist": "Actual artist", "event_count": 30, "ms_played": 435}
    ]
...
def test_invalid_queries(tmp_path, query):
    with pytest.raises(DomainError):
        store_at(tmp_path).history(query)
...
@pytest.mark.parametrize(
    ...
    [
        extended(ts="invalid"),
        extended(ts="2026-01-02T00:00:00"),
        extended(ms_played=-1),
        extended(ms_played=True),
        extended(ms_played=86400001),
        extended(master_metadata_track_name=123),
    ],
)
def test_invalid_late_record_rolls_back_all(tmp_path, record):
    store = store_at(tmp_path)
    with pytest.raises(DomainError):
        store.import_data(payload(store, [extended(), record]))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "recent": [
                {"played_at": "2026-01-02T00:00:00Z", "track": api_track(album=7)}
            ],
            "playlists": [],
        },
...
def test_malformed_api_snapshot_domain_error(tmp_path, document):
    store = store_at(tmp_path)
    with pytest.raises(DomainError):
        store.import_data(payload(store, document, "spotify_api"))
        ...
        "playlists": [
            {
                ...
                "name": "API list",
                "snapshot_id": "one",
                "items": [{"item": api_track()}, {"item": None}, {"item": api_track()}],
            }
        ],
    }
    result = store.import_data(payload(store, document, "spotify_api"))
...
def test_concurrent_same_request_exactly_once(tmp_path):
    ...
    with ThreadPoolExecutor(6) as pool:
        results = list(pool.map(lambda service: service.import_data(data), stores))
...
async def test_actual_http_and_native_consumers(tmp_path):
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert (await (await client.get(prefix + "/history?limit=1")).json())[
            "total"
        ] == 1
        assert (await (await client.get(prefix + "/stats")).json())[
            "ms_played"
        ] == 12500
        assert len((await (await client.get(prefix + "/imports")).json())["items"]) == 1
        malformed = payload(
            store, {"recent": [], "playlists": [{}]}, "spotify_api", "malformed"
        )
        ...
        assert (
            await client.post(
                prefix + "/spotify/sync",
                json={"request_id": "sync", "playlist_ids": []},
            )
        ).status == 503
    ...
    assert not (
        await tools.invoke(
            ...
            {"data": {"request_id": "native", "playlist_ids": []}},
        )
    ).success
