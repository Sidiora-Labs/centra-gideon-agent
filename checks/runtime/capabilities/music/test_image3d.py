...
from gideon.workspace.artifacts.models import kind_for_mime,mime_for_ext,ext_for_mime
from gideon.workspace.artifacts.native import NativeArtifactProvider
from gideon.workspace.capabilities.music.image3d import Image3DStore,validate_glb,asset_url,ENDPOINT,MODELS
...
def glb(document=None):
    ...
    document=document if document is not None else {'asset':{'version':'2.0'},'scene':0,'scenes':[{'nodes':[0]}],'nodes':[{'mesh':0}],
    ...
    body=struct.pack('<II',len(encoded),0x4e4f534a)+encoded+struct.pack('<II',len(positions),0x004e4942)+positions
...
def source(service):
    output=io.BytesIO();Image.new('RGB',(32,32),'green').save(output,format='PNG')
    artifact=service.artifacts.create_binary(name='Authored input image',data=output.getvalue(),kind='image',mime='image/png',source='manual')
    return {'request_id':'asset-request','title':'Reconstructed object','image_ref':{'slug':artifact.slug,'version':artifact.version},'target_polycount':30000,'should_texture':True,'license':'Operator to verify provider account rights'}


def seed_interrupted(service,request):
    job={'id':request['request_id'],'status':'interrupted','request':request,'model':'meshy-6','credential_name':'missing','provider_task_id':None,'progress':0,'artifact_ref':None,'error':'Prior runtime ended during submission'}
    with sqlite3.connect(service.path) as db:
        db.execute('INSERT INTO jobs VALUES (?,?,?)',(job['id'],json.dumps(request,sort_keys=True),json.dumps(job)))
    return job
...
def test_disabled_default_configuration_never_claims_remote_availability(tmp_path):
    ...
    assert ready['config']=={'enabled':False,'credential_name':'','model':'meshy-6','revision':0}
...
def test_canonical_named_credential_readiness_reopens_without_copying_secret(tmp_path):
    ...
    credentials.save({'geometry-key':{'type':'api_key','value':'local-test-secret-never-sent'}})
    service=service_at(tmp_path)
    config=service.configure({'enabled':True,'credential_name':'geometry-key','model':'meshy-6','revision':0})
...
def test_configuration_conflict_preserves_saved_model_and_alias(tmp_path):
    service=service_at(tmp_path)
    first=service.configure({'enabled':False,'credential_name':'first','model':'meshy-6-lite','revision':0})
    with pytest.raises(DomainError) as err:
        service.configure({'enabled':True,'credential_name':'second','model':'meshy-7.1','revision':0})
...
@pytest.mark.parametrize('changes',[{'enabled':'true'},{'enabled':1},{'model':'caller-model'},{'model':'latest'},{'credential_name':None},{'revision':True},{'revision':-1},{'api_key':'secret'},{'endpoint':'http://localhost'},{'home':'/tmp'}])
def test_untrusted_configuration_fields_refused(tmp_path,changes):
    service=service_at(tmp_path)
    data={'enabled':False,'credential_name':'missing','model':'meshy-6','revision':0}
...
@pytest.mark.parametrize('changes',[{'request_id':''},{'title':''},{'license':''},{'target_polycount':99},{'target_polycount':300001},{'target_polycount':True},{'should_texture':1},{'image_ref':{}},{'image_ref':{'slug':'missing','version':1}},{'image_ref':{'slug':'missing','version':0}},{'image_ref':{'slug':'missing','version':1,'path':'/tmp'}},{'endpoint':'https://other'},{'provider':'other'},{'model':'other'}])
def test_invalid_job_requests_never_create_jobs(tmp_path,changes):
    service=service_at(tmp_path);request=source(service)
    with pytest.raises(DomainError):
...
def test_image_source_version_is_pinned_and_foreign_home_is_refused(tmp_path):
    one=service_at(tmp_path/'one');request=source(one)
...
async def test_missing_credential_refuses_submission_without_network_or_job(tmp_path):
    ...
    for enabled in (False,True):
        service.configure({'enabled':enabled,'credential_name':'missing','model':'meshy-6','revision':service.config()['revision']})
        with pytest.raises(DomainError) as err:
...
def test_glb_actual_triangle_roundtrips_canonical_model_kind_and_version(tmp_path):
    ...
    artifact=service.artifacts.create_binary(name='Original authored triangle',data=raw,mime='model/gltf-binary',kind='model',source='manual')
    ...
    assert service.artifacts.raw_bytes(artifact.slug,version=1)==(raw,'model/gltf-binary')
...
@pytest.mark.parametrize('raw',[b'',b'not GLB',glb()[:-1],b'glTF'+struct.pack('<II',1,20)+b' '*8,glb({'asset':None}),glb({'asset':{'version':'2.0'},'meshes':[]}),glb({'asset':{'version':'1.0'},'meshes':[{}]}),glb({'asset':{'version':'2.0'},'meshes':[{}],'buffers':[None]}),glb({'asset':{'version':'2.0'},'meshes':[{}],'images':[{'uri':123}]})])
...
@pytest.mark.parametrize('uri',['https://evil.example/image','file:///etc/passwd','../image.png','//example.test/x'])
...
def test_invalid_chunk_length_and_duplicate_json_are_refused():
    ...
    original=glb();json_size=struct.unpack_from('<I',original,12)[0]
...
@pytest.mark.parametrize('url',['http://assets.meshy.ai/model.glb','https://localhost/model.glb','https://assets.meshy.ai.evil.test/model.glb','file:///model.glb','https://name:secret@assets.meshy.ai/model.glb','https://assets.meshy.ai:9443/model.glb','https://assets.meshy.ai/model.glb#fragment'])
def test_provider_download_url_has_fixed_https_origin(url):
    with pytest.raises(DomainError):
        asset_url(url)
    assert asset_url('https://assets.meshy.ai/tasks/a/model.glb?Expires=123')=='https://assets.meshy.ai/tasks/a/model.glb?Expires=123'


@pytest.mark.asyncio
async def test_real_http_settings_refusal_jobs_and_pinned_raw_model(tmp_path):
    service=service_at(tmp_path);request=source(service)
    model=service.artifacts.create_binary(name='Authored HTTP triangle',data=glb(),mime='model/gltf-binary',kind='model',source='manual')
    app=web.Application();register(app,service)
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        saved=await client.patch(prefix+'/config',json={'enabled':True,'credential_name':'missing','model':'meshy-6','revision':0})
        ...
        wrong=await client.get(prefix+f"/artifacts/{request['image_ref']['slug']}/1/raw")
        assert wrong.status==404
        seed_interrupted(service,request)
        stopped=await client.post(prefix+'/jobs/'+request['request_id']+'/stop',json={})
        assert (await stopped.json())['job']['status']=='stopped'
        bad=await client.post(prefix+'/jobs/'+request['request_id']+'/refresh',json={'home':'/tmp'})
...
async def test_real_native_operations_keep_provider_unavailable_honest(tmp_path):
    service=service_at(tmp_path);request=source(service);tools=Image3DTools(service)
    ...
    saved=await tools.invoke('music_models3d_configure',{'data':{'enabled':True,'credential_name':'missing','model':'meshy-6','revision':0}})
    ...
    refreshed=await tools.invoke('music_models3d_refresh',{'id':request['request_id']})
