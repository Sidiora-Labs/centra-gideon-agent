...
from gideon.workspace.artifacts.registry import get_provider, register_provider
...
def png():
    stream = io.BytesIO()
    Image.new("RGB", (12, 9), "orange").save(stream, format="PNG")
...
class ProviderTerminal(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        for key in list(os.environ):
            if any(
                term in key.upper()
                for term in ("TOKEN", "KEY", "SECRET", "CODEX", "OPENAI")
            ):
                os.environ.pop(key, None)
        os.environ.update(
            GIDEON_HOME=str(self.root),
            HOME=str(self.root),
            CODEX_HOME=str(self.root / "codex"),
        )
        (self.root / "codex").mkdir(mode=0o700)
        self.repo = repository(self.root / "repo")
        self.artifacts = get_provider()
        register_provider(NativeArtifactProvider(self.root / "artifacts"))
        self.registry = get_default_registry()
        self.registry.register_entry(
            ProviderEntry(
                name="workspace-codex-test",
                type="acp_agent",
                model="",
                options={"dialect": "codex", "requires_executable": {"path": CODEX}},
            )
        )
        ...
        await self.client.get(
            ...
            + generate_token("provider-owner")
        )

    async def asyncTearDown(self):
        for session in list(self.state._terminal_sessions.values()):
            if session:
                await self.terminal._kill_session(session)
        self.terminal._pending_provider_argv.clear()
        self.terminal._pending_cwd.clear()
    ...
    async def test_profiles_native_tool_and_real_executable(self):
        ...
        self.assertTrue(row["image_supported"])
        ...
        self.registry.register_entry(
            ProviderEntry(
                ...
                type="acp_agent",
                model="",
                options={"dialect": "unknown", "requires_executable": {"path": CODEX}},
            )
        )
        row = next(p for p in profiles() if p["id"] == "unsupported-test")
        self.assertFalse(row["available"])
        with self.assertRaises(ValueError):
            prepare("a" * 12, "unsupported-test")
        with self.assertRaises(ValueError):
            prepare("a" * 12, "not-configured")
    ...
    async def test_canonical_artifact_bytes_and_session_owned_copy(self):
        data = png()
        record = upload(data, "image-owner")
        ...
        self.assertTrue(Path(other[2]).exists())
        self.assertIsNotNone(get_provider().get(record["artifact_id"]))
        cleanup("b" * 12)
        with self.assertRaises(ValueError):
            prepare("../escape", "workspace-codex-test", image)
    ...
    async def test_decode_and_version_validation(self):
        ...
        for image in (
            {},
            {"artifact_id": "x", "version": 0},
            {"artifact_id": "x", "version": True},
            {"artifact_id": "x", "version": 1, "path": "/etc/passwd"},
        ):
            with self.subTest(image=image):
                with self.assertRaises(ValueError):
                    prepare("a" * 12, "workspace-codex-test", image)
        with self.assertRaises(FileNotFoundError):
            prepare(
                "a" * 12,
                "workspace-codex-test",
                {"artifact_id": "missing", "version": 1},
            )
        response = await self.client.post(
            self.url + "/api/capabilities/workspace/provider-terminals/images",
            data=b"not an image",
        )
        self.assertEqual(response.status, 400)
        response = await self.client.post(
            self.url + "/api/terminal/sessions",
            json={"image": {"artifact_id": "x", "version": 1}},
        )
        self.assertEqual(response.status, 400)
    ...
    async def test_pending_session_cancel_and_capacity(self):
        ...
        for _ in range(3):
            response = await self.client.post(
                self.url + "/api/terminal/sessions",
                json={"provider_id": "workspace-codex-test", "cwd": str(self.repo)},
            )
            self.assertEqual(response.status, 200)
            sessions.append((await response.json())["session_id"])
        response = await self.client.post(
            self.url + "/api/terminal/sessions",
            json={"provider_id": "workspace-codex-test"},
        )
        self.assertEqual(response.status, 429)
        response = await self.client.delete(
            self.url + "/api/terminal/sessions/" + sessions[0]
        )
        self.assertEqual(response.status, 200)
    ...
    async def test_real_cli_receives_image_in_existing_websocket_pty(self):
        ...
        response = await self.client.post(
            self.url + "/api/terminal/sessions",
            json={
                "provider_id": "workspace-codex-test",
                "cwd": str(self.repo),
                "image": {key: image[key] for key in ("artifact_id", "version")},
            },
        )
        self.assertEqual(response.status, 200)
        identity = (await response.json())["session_id"]
        ...
        try:
            ...
            for _ in range(40):
                ...
                if message.type == WSMsgType.BINARY:
                    output += message.data
                    if b"\x1b[6n" in message.data:
                        await ws.send_bytes(b"\x1b[1;1R")
                    if any(
                        marker in output
                        for marker in (
                            b"Welcome to",
            ...
            self.assertTrue(
                any(
                    marker in output
                    for marker in (b"Welcome to", b"Sign in", b"sign in", b"ChatGPT")
                ),
                output[-3000:],
            )
            ...
            image_path = (
                self.root
                / "capabilities/workspace/terminal-images"
                / identity
                / "image.png"
            )
            self.assertEqual(image_path.read_bytes(), png())
            response = await self.client.delete(
                self.url + "/api/terminal/sessions/" + identity
            )
            self.assertEqual(response.status, 200)
