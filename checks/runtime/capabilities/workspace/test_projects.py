...
class Projects(unittest.TestCase):
    def setUp(self):
        ...
        self.service = ProjectService(
            self.root / "state", allowed_roots=[self.workspace]
        )
        self.payload = {
            "name": "Actual project",
            "parent": str(self.workspace),
            "directory": "new-app",
    ...
    def test_register_reuses_canonical_project_source_and_reload(self):
        ...
        (repo / "package.json").write_text(
            json.dumps(
                {"scripts": {"dev": "vite --port 6025", "build": "node build.js"}}
            )
        )
        result = self.service.register(
            {"name": "Existing", "workspace": str(repo), "request_id": "register"}
        )
        row = result["project"]
        self.assertTrue(row["id"].startswith("p-"))
        self.assertEqual(
            HierarchyStore().get_project(row["id"]).workspace_dir, str(repo)
        )
    ...
    def test_detection_never_executes_declared_commands(self):
        ...
        (self.workspace / "package.json").write_text(
            json.dumps(
                {
                    "scripts": {
                        "prepare": f"touch {marker}",
                        "start": "node main.js --port=65535",
                    }
                }
            )
        )
        ...
        self.assertEqual(
            detected["types"], ["node", "python", "rust", "swift", "xcode"]
        )
        self.assertEqual(detected["ports"], [65535])
        self.assertEqual(
            set(detected["source_files"]),
            {
                "package.json",
                "pyproject.toml",
                "Cargo.toml",
    ...
    def test_scaffold_is_runnable_and_receipt_is_durable(self):
        ...
        with sqlite3.connect(self.service.db_path) as db:
            self.assertEqual(
                db.execute("SELECT project_id FROM operations").fetchone()[0],
                result["project"]["id"],
            )
        self.assertEqual(self.service.db_path.stat().st_mode & 0o777, 0o600)
        self.assertEqual(
            HierarchyStore().get_project(result["project"]["id"]).name, "Actual project"
        )
    ...
    def test_existing_directory_and_name_conflicts_preserve_files(self):
        ...
        elsewhere.mkdir()
        existing = HierarchyStore().create_project(
            "Actual project", workspace_dir=str(elsewhere)
        )
        with self.assertRaises(ConflictError):
            self.service.scaffold(
                {
                    **self.payload,
                    "directory": "different",
                    "request_id": "name-conflict",
                }
            )
        self.assertFalse((self.workspace / "different").exists())
        self.assertEqual(
            HierarchyStore().get_project(existing.id).workspace_dir, str(elsewhere)
        )
    ...
    def test_concurrent_retry_has_single_canonical_project(self):
        with ThreadPoolExecutor(max_workers=4) as pool:
            rows = list(
                pool.map(lambda _: self.service.scaffold(self.payload), range(4))
            )
    ...
    async def test_generated_python_and_node_serve_real_http(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertEqual(
                    {x["id"] for x in catalog}, {"python-http", "node-http"}
                )
                for template in catalog:
                    ...
                    proc = await asyncio.create_subprocess_exec(
                        template["requires"],
                        template["entrypoint"],
                        cwd=result["project"]["workspace_dir"],
...
class ProjectHttp(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        response = await self.client.get(
            self.url + "?token=" + generate_token("project-owner")
        )
    ...
    async def test_http_and_native_share_canonical_project(self):
        response = await self.client.post(
            self.url + "/detect", json={"workspace": str(self.repo)}
        )
        self.assertEqual(response.status, 200)
        ...
        self.assertEqual(
            HierarchyStore().get_project(result["project"]["id"]).workspace_dir,
            str(self.root / "tool-app"),
        )
    ...
    async def test_auth_and_no_overwrite_errors(self):
        ...
        response = await self.client.post(
            self.url + "/detect", json={"workspace": "/etc"}
        )
