...
from gideon.workspace.capabilities.workspace.iterm import ExternalTerminalMirror,pane_id
...
class MirrorValidation(unittest.IsolatedAsyncioTestCase):
    def test_identity_bounds_and_no_command_or_endpoint_syntax(self):
        ...
        for value in ('','a'*257,'../pane','http://localhost','$(id)','pane\n',None,1,{},[]):
            with self.subTest(value=value):
                with self.assertRaises(ValueError):pane_id(value)
    ...
    async def test_actual_platform_refuses_native_observation(self):
        ...
        with self.assertRaisesRegex(OSError,'macOS'):await mirror.inventory()
        with self.assertRaisesRegex(OSError,'macOS'):await mirror.screen('real-pane')
        with self.assertRaises(ValueError):await mirror.screen('../escape')
        proc=await asyncio.create_subprocess_exec(sys.executable,str(Path(__file__).parents[4]/'runtime/gideon/workspace/capabilities/workspace/iterm_worker.py'),'pane',stdout=asyncio.subprocess.PIPE,stderr=asyncio.subprocess.PIPE)
...
@unittest.skipIf(sys.platform=='darwin','Exercises real unavailable platform boundary')
class MirrorHttp(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        response=await self.client.get(self.origin+'/api/capabilities/workspace?token='+generate_token('mirror-owner'))
    ...
    async def test_no_input_resize_or_lifecycle_routes(self):
        for method in ('post','put','delete','patch'):
            response=await getattr(self.client,method)(self.base+'/pane',json={'command':'id','columns':80})
            self.assertEqual(response.status,405)
        response=await self.client.get(self.base+'/availability?endpoint=http://localhost')
    ...
    async def test_real_native_tools_share_read_only_failure(self):
        ...
        for name in ('workspace_external_terminals','workspace_external_terminal_screen'):
        ...
        result=await provider.invoke('workspace_external_terminal_screen',{'id':'pane'})
        self.assertFalse(result.success)
        result=await provider.invoke('workspace_external_terminals',{'endpoint':'http://localhost'})
        self.assertFalse(result.success)


@unittest.skipUnless(sys.platform=='darwin' and importlib.util.find_spec('iterm2') and os.environ.get('GIDEON_TEST_NATIVE_PANE') and os.environ.get('GIDEON_TEST_NATIVE_TEXT'),'Needs an existing authorized native Mac pane; no fixture substitute')
class NativeMirror(unittest.IsolatedAsyncioTestCase):
    async def test_existing_native_pane_survives_readers_and_retains_native_grid(self):
        ...
        with self.assertRaises(FileNotFoundError):await second.screen('gideon-missing-native-pane')
