...
class Processes(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_natural_exit_and_nonzero_exit(self):
        good = await self.registry.start(
            {**self.payload, "command": "printf completed"}
        )
        ...
        self.assertEqual(
            await self.registry.stop(bad["id"], failed["revision"]), failed
        )
    ...
    async def test_start_retry_and_conflicting_request_id(self):
        ...
        self.assertEqual(
            self.registry.get(one["id"])["command"], self.payload["command"]
        )
    ...
    async def test_workspace_escape_missing_and_symlink(self):
        ...
        with self.assertRaises(FileNotFoundError):
            await self.registry.start(
                {**self.payload, "workspace": str(self.root / "missing")}
            )
        self.assertEqual(self.registry.list(), [])
    ...
    async def test_close_terminates_owned_process_group(self):
        row = await self.registry.start(
            {
                **self.payload,
                "command": 'sleep 60 & child=$!; printf "%s" "$child"; wait',
            }
        )
        for _ in range(100):
            text = self.registry.logs(row["id"])["text"]
    ...
    async def test_history_pagination(self):
        ...
        for index in range(3):
            row = await self.registry.start(
                {**self.payload, "command": "true", "request_id": f"page-{index}"}
            )
            await settled(self.registry, row["id"])
            ids.append(row["id"])
        self.assertEqual(
            [x["id"] for x in self.registry.list(limit=2)], list(reversed(ids))[:2]
        )
...
class ProcessHttp(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        reply = await self.client.get(
            self.url + "?token=" + generate_token("process-owner")
        )
        self.assertEqual(reply.status, 200, await reply.text())
        self.payload = {
            "project_id": "http-process",
            "workspace": str(self.repo),
            "command": "printf http-ready; exec sleep 60",
            "request_id": uuid4().hex,
        }
    ...
    async def test_http_lifecycle_and_auth(self):
        ...
        response = await self.client.post(
            self.url + "/" + row["id"] + "/stop", json={"revision": 0}
        )
        self.assertEqual(response.status, 409)
        response = await self.client.post(
            self.url + "/" + row["id"] + "/stop", json={"revision": row["revision"]}
        )
    ...
    async def test_actual_native_tool_consumer_and_metadata(self):
        ...
        self.assertEqual(
            definitions["workspace_process_start"].risk_level.value, "destructive"
        )
        ...
        stopped = await provider.invoke(
            "workspace_process_stop", {"id": row["id"], "revision": row["revision"]}
        )
        self.assertEqual(json.loads(stopped.output)["status"], "stopped")
        denied = await provider.invoke(
            "workspace_process_start", {**self.payload, "env": {}}
        )
        ...
        fetched = await provider.invoke(
            "workspace_snapshot_get", {"id": snapshot["id"]}
        )
        self.assertEqual(json.loads(fetched.output), snapshot)
        deleted = await provider.invoke(
            "workspace_snapshot_delete", {"id": snapshot["id"], "revision": 1}
        )
