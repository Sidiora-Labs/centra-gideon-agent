...
from gideon.workspace.capabilities.workspace.desktop import DesktopRegistry,get_desktop_registry,close_desktop_registry
...
class Desktops(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.old=os.environ.get('GIDEON_HOME');os.environ['GIDEON_HOME']=str(self.root)
        self.repo=repository(self.root/'repo')
        self.project=HierarchyStore().create_project(name='Desktop source',workspace_dir=str(self.repo))
    ...
    async def asyncTearDown(self):
        ...
        if self.old is None:os.environ.pop('GIDEON_HOME',None)
        else:os.environ['GIDEON_HOME']=self.old
        self.temp.cleanup()

    def payload(self,request_id='desktop'):
        return {'project_id':self.project.id,'request_id':request_id,'width':800,'height':600}
    ...
    async def test_real_frames_terminal_input_and_clean_stop(self):
        ...
        focus=(await self.registry._command(handle,['xdotool','getwindowfocus','getwindowname'],4096)).decode().strip()
        self.assertEqual(focus,'Workspace desktop')
        visible=(await self.registry._command(handle,['xdotool','search','--onlyvisible','--class','XTerm'],1024)).decode().splitlines()
        ...
        position=await self.registry._command(handle,['xdotool','getmouselocation','--shell'])
        self.assertIn(b'X=100',position);self.assertIn(b'Y=100',position)
        await self.terminal(self.registry,record['id'],'printf desktop-proof > desktop-proof.txt')
        self.assertEqual(await self.wait_file(self.repo/'desktop-proof.txt'),'desktop-proof')
        await self.terminal(self.registry,record['id'],'tty > tty-name.txt')
        self.assertEqual((await self.wait_file(self.repo/'tty-name.txt')).strip(),tty_name)
        await self.terminal(self.registry,record['id'],'test -t 0 && printf interactive > interactive.txt')
        self.assertEqual(await self.wait_file(self.repo/'interactive.txt'),'interactive')
        with self.assertRaises(ConflictError):await self.registry.stop(record['id'],1)
        ...
        with self.assertRaises(ConflictError):await self.registry.frame(record['id'])
        with self.assertRaises(ConflictError):await self.registry.input(record['id'],{'kind':'key','key':'Return'})
    ...
    async def test_idempotency_exclusive_owner_and_reopen(self):
        ...
        with self.assertRaises(ConflictError):await self.registry.start({**self.payload(),'width':900})
        with self.assertRaises(ConflictError):DesktopRegistry(self.root/'state',allowed_roots=[self.root])
        await self.registry.close()
        self.registry=DesktopRegistry(self.root/'state',allowed_roots=[self.root])
    ...
    async def test_two_sessions_remain_isolated_and_capacity_refused(self):
        ...
        one=self.registry.handles[first['id']];two=self.registry.handles[second['id']]
        ...
        with self.assertRaises(ConflictError):await self.registry.start(self.payload('third'))
        ...
        with self.assertRaises(FileNotFoundError):await self.registry.stop('other-home-session',2)
    ...
    async def test_input_validation_and_keyboard_editing(self):
        ...
        invalid=[{'kind':'click','x':-1,'y':5},{'kind':'click','x':800,'y':0},{'kind':'click','x':True,'y':0},{'kind':'key','key':'ctrl+alt+F1'},{'kind':'text','text':'\n'},{'kind':'text','text':'x'*2049},{'kind':'text','text':'safe','DISPLAY':':0'},{'kind':'shell','command':'touch escape'},{}]
        for payload in invalid:
            with self.subTest(payload=payload):
                with self.assertRaises(ValueError):await self.registry.input(row['id'],payload)
    ...
    async def test_concurrent_start_reuses_one_live_terminal_and_closes_pty(self):
        results=await asyncio.gather(self.registry.start(self.payload()),self.registry.start(self.payload()))
        ...
        await self.terminal(self.registry,row['id'],'printf "$DISPLAY" > display-name.txt')
        self.assertEqual(await self.wait_file(self.repo/'display-name.txt'),handle['display'])
        await self.terminal(self.registry,row['id'],'printf "$HOME" > private-home.txt')
        self.assertEqual(await self.wait_file(self.repo/'private-home.txt'),str(handle['home']))
        await self.registry.close()
        self.assertEqual(self.registry.handles,{})
    ...
    async def test_missing_dependencies_are_truthfully_unavailable(self):
        old=os.environ['PATH']
        empty=self.root/'empty-bin';empty.mkdir()
    ...
    async def test_validation_does_not_create_sessions(self):
        for change in ({'width':True},{'width':639},{'height':901},{'request_id':''},{'project_id':'missing'},{'display':':0'},{'command':'bash'}):
            with self.subTest(change=change):
                with self.assertRaises((ValueError,FileNotFoundError)):
                    await self.registry.start({**self.payload(),**change})
        self.assertEqual(self.registry.list(),[])
        self.assertEqual(self.registry.handles,{})
        with self.assertRaises(FileNotFoundError):self.registry.get('missing')
        with self.assertRaises(FileNotFoundError):await self.registry.input('missing',{})
        await self.registry.close()
        with self.assertRaises(ConflictError):await self.registry.start(self.payload())
    ...
    async def test_owned_child_exit_updates_failed_state(self):
        ...
        terminal.terminate()
        for _ in range(100):
            if self.registry.get(row['id'])['status']=='failed' and row['id'] not in self.registry.handles:break
            await asyncio.sleep(.05)
    ...
    async def test_actual_registry_crash_cleans_display_and_recovery_does_not_adopt(self):
        await self.registry.close()
        worker=await asyncio.create_subprocess_exec(sys.executable,str(Path(__file__).with_name('desktop_worker.py')),str(self.root),self.project.id,stdout=asyncio.subprocess.PIPE,stderr=asyncio.subprocess.PIPE)
        try:
            line=await asyncio.wait_for(worker.stdout.readline(),30)
            self.assertTrue(line,await worker.stderr.read() if worker.returncode is not None else 'Worker did not report ready')
            ...
            for _ in range(100):
                try:
                    with socket.socket(socket.AF_UNIX,socket.SOCK_STREAM) as connection:connection.connect(socket_path)
                except OSError:break
                await asyncio.sleep(.05)
            else:self.fail('Owned X server survived registry process crash')
            self.registry=DesktopRegistry(self.root/'state',allowed_roots=[self.root])
            ...
            self.assertEqual(self.registry.handles,{})
            with self.assertRaises(ConflictError):await self.registry.frame(recovered['id'])
        finally:
    ...
    async def test_owner_http_native_png_input_and_authentication(self):
        ...
        try:
            async with ClientSession(cookie_jar=CookieJar(unsafe=True)) as client:
                response=await client.get(base+'?token='+generate_token('desktop-owner'))
                ...
                self.assertEqual(Image.open(io.BytesIO(await response.read())).size,(800,600))
                for payload in ({'kind':'text','text':'printf http-proof > http-proof.txt'},{'kind':'key','key':'Return'}):
                    response=await client.post(base+'/'+row['id']+'/input',json=payload)
                    self.assertEqual(response.status,200,await response.text())
                self.assertEqual(await self.wait_file(self.repo/'http-proof.txt'),'http-proof')
                ...
                self.assertTrue(definitions['workspace_desktop_start'].requires_approval)
                self.assertTrue(definitions['workspace_desktop_stop'].requires_approval)
                self.assertNotIn('workspace_desktop_input',definitions)
                stopped=await provider.invoke('workspace_desktop_stop',{'id':row['id'],'revision':row['revision']})
            ...
            async with ClientSession() as anonymous:
                ...
                response=await anonymous.post(base+'/'+row['id']+'/input',json={'kind':'key','key':'Return'})
                self.assertIn(response.status,(401,403))
        finally:
