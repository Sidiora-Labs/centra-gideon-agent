...
class Snapshots(unittest.TestCase):
    def setUp(self):
        ...
        self.input = {'project_id': 'project-one', 'workspace': str(self.repo), 'terminal_ids': [], 'task_ids': [], 'request_id': uuid4().hex}

    def capture(self, **changes):
        return self.store.capture({**self.input, **changes}, terminal_ids=['shell-a', 'shell-b'], task_ids=['task-a'])
    ...
    def test_persistence_and_real_process_reconciliation(self):
        ...
        saved = self.store.capture(self.input, terminal_ids=[str(one.pid), str(two.pid)], task_ids=[])
    ...
    def test_git_metadata_escape(self):
        ...
        with self.assertRaises(ValueError):
            restricted.capture({**self.input, 'workspace': str(linked)}, terminal_ids=[], task_ids=[])
    ...
    def test_capture_cannot_invent_reference_or_state(self):
        for patch in ({'task_ids':['unknown']}, {'terminal_ids':['unknown']}, {'terminal_ids':['shell-a','shell-a']}, {'task_ids':'task-a'}, {'branch':'invented'}, {'dirty':False}, {'project_id':''}, {'request_id':''}, {'task_ids':[1]}):
    ...
    def test_pagination_and_independent_homes(self):
        ...
        self.assertEqual(self.store.list(offset=2, limit=2), list(reversed(records))[2:])
...
class HttpJourney(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.repo = repository(self.root / 'repo')
        (self.root / 'config.json').write_text(json.dumps({'dashboard': {'terminal': {'enabled': False}}}))
        ...
        response = await self.client.get(self.url + '/api/capabilities/workspace?token=' + self.token)
        self.assertEqual(response.status, 200, await response.text())
        self.body = {'project_id':'real-project', 'workspace':str(self.repo), 'request_id':uuid4().hex, 'terminal_ids':[], 'task_ids':[]}
    ...
    async def test_authenticated_capture_compare_delete(self):
        response = await self.client.post(self.url + '/api/capabilities/workspace', json=self.body)
        self.assertEqual(response.status, 200, await response.text())
    ...
    async def test_http_bad_inputs_and_owner_boundary(self):
        ...
        response = await self.client.post(self.url + '/api/capabilities/workspace', json={**self.body, 'workspace':'/etc'})
        self.assertEqual(response.status, 400)
        response = await self.client.post(self.url + '/api/capabilities/workspace', json={**self.body, 'terminal_ids':['invented']})
        self.assertEqual(response.status, 400)
        response = await self.client.get(self.url + '/api/capabilities/workspace?limit=1000')
        self.assertEqual(response.status, 400)
        response = await self.client.post(self.url + '/api/capabilities/workspace', json=[])
        self.assertEqual(response.status, 400)
        response = await self.client.get(self.url + '/api/capabilities/workspace')
        self.assertEqual(await response.json(), [])
