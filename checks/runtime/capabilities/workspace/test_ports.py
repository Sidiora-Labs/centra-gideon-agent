...
from gideon.workspace.capabilities.workspace.ports import PortRegistry, get_port_registry, close_port_registry
...
class Reservations(unittest.TestCase):
    def setUp(self):
        ...
        self.payload = {'project_id':'real-app','request_id':uuid4().hex,'port':self.ports[0]}
    ...
    def test_listener_conflict_no_side_effects(self):
        with socket.socket() as service:
            ...
            self.assertEqual(service.getsockopt(socket.SOL_SOCKET, socket.SO_ACCEPTCONN), 1)
    ...
    def test_concurrent_reservations_and_exhaustion(self):
        def reserve(index):
            return self.registry.reserve({'project_id':f'p-{index}','request_id':f'r-{index}'})
        ...
        with self.assertRaises(ConflictError):
            reserve(4)
        self.assertTrue(all(not x['available'] for x in self.registry.inventory()['ports']))
    ...
    def test_concurrent_same_request_has_one_socket(self):
        with ThreadPoolExecutor(max_workers=4) as workers:
            rows = list(workers.map(lambda _:self.registry.reserve(self.payload), range(8)))
    ...
    def test_validation_and_config_bounds(self):
        for patch in ({'host':'0.0.0.0'}, {'port':80}, {'port':True}, {'port':'6000'}, {'request_id':''}, {'project_id':None}, {'port':self.ports[0]+65536}):
    ...
    def test_other_home_has_no_ownership(self):
        ...
        try:
            ...
            entry = next(x for x in other.inventory()['ports'] if x['port'] == record['port'])
    ...
    def test_bounded_listing_and_inventory_identity(self):
        ...
        self.assertTrue(all(set(x) == {'port','available','reservation_id'} for x in inventory['ports']))
...
class PortHttp(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        response = await self.client.get(self.url+'?token='+generate_token('ports-owner'))
    ...
    async def test_real_http_and_native_release_share_socket(self):
        ...
        response = await self.client.post(self.url,json={'project_id':'http-app','request_id':uuid4().hex})
        self.assertEqual(response.status,200,await response.text())
        ...
        self.assertTrue(any(x['reservation_id']==row['id'] for x in json.loads(inventory.output)['ports']))
        released = await provider.invoke('workspace_port_release',{'id':row['id'],'revision':row['revision']})
        self.assertTrue(released.success,released.error)
        self.assertEqual(json.loads(released.output)['status'],'released')
        with socket.socket() as outsider:
            outsider.bind(('127.0.0.1',row['port']))
        started = await provider.invoke('workspace_port_reserve',{'project_id':'tool-app','request_id':uuid4().hex})
        self.assertTrue(started.success,started.error)
        second = json.loads(started.output)
        response = await self.client.post(self.url+'/'+second['id']+'/release',json={'revision':second['revision']})
    ...
    async def test_auth_validation_and_conflict_envelopes(self):
        ...
        response = await self.client.post(self.url,json={'project_id':'a','request_id':'same'})
        row = await response.json()
        response = await self.client.post(self.url,json={'project_id':'b','request_id':'same'})
        self.assertEqual(response.status,409)
        response = await self.client.post(self.url+'/'+row['id']+'/release',json={'revision':0})
        self.assertEqual(response.status,409)
        response = await self.client.post(self.url,json={'project_id':'b','request_id':'bad','host':'0.0.0.0'})
        self.assertEqual(response.status,400)
        response = await self.client.get(self.url+'/missing')
        self.assertEqual(response.status,404)
        from gideon.workspace.capabilities.workspace.tools import create_provider
        denied = await create_provider().invoke('workspace_port_reserve',{'project_id':'bad','request_id':'type','port':True})
    ...
    async def test_actual_crash_releases_socket_and_marks_history(self):
        ...
        worker = await asyncio.create_subprocess_exec(sys.executable,str(script),str(folder),str(port),stdout=asyncio.subprocess.PIPE)
        try:
