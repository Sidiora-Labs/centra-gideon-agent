...
from gideon.workspace.capabilities.workspace.processes import ProcessRegistry,get_registry,close_registry
from test_processes import Processes,settled,output
...
class LogWindows(unittest.IsolatedAsyncioTestCase):
    ...
    async def launch(self,source,request='log'):
        path=self.repo/(request+'.py');path.write_text(source)
        return await self.registry.start({**self.payload,'command':shlex.quote(sys.executable)+' '+shlex.quote(str(path)),'request_id':request})

    async def test_incremental_cursor_and_real_delayed_stdout_stderr(self):
        row=await self.launch("import sys,time\nprint('first',flush=True)\ntime.sleep(.3)\nprint('second',file=sys.stderr,flush=True)\ntime.sleep(.3)\nprint('third',flush=True)")
        ...
        self.assertEqual(self.registry.logs(row['id'])['text'],first['text']+second['text'])
        await self.registry.close()
        self.registry=ProcessRegistry(self.root/'state',allowed_roots=[self.root])
        self.assertEqual(self.registry.log_window(row['id'],after=first['next']),second)
    ...
    async def test_real_output_retention_gap_and_paging(self):
        ...
        retained=page['text'];cursor=page['next']
        while cursor<page['end']:
            ...
            cursor=part['next'];retained+=part['text']
        ...
        with self.assertRaises(ValueError):self.registry.log_window(row['id'],after=cursor+1)

    async def test_split_utf8_is_preserved_and_incomplete_final_byte_is_replaced(self):
        row=await self.launch("import os,time\nos.write(1,b'\\xe2')\ntime.sleep(.1)\nos.write(1,b'\\x82')\ntime.sleep(.1)\nos.write(1,b'\\xac\\n\\xe2')")
        await settled(self.registry,row['id'])
    ...
    async def test_existing_database_tail_migrates_without_losing_records(self):
        ...
        with sqlite3.connect(legacy/'processes.sqlite3') as db:
            db.execute('CREATE TABLE processes(id TEXT PRIMARY KEY,request_id TEXT UNIQUE,input TEXT NOT NULL,payload TEXT NOT NULL,log TEXT NOT NULL DEFAULT "")')
            db.execute('INSERT INTO processes VALUES(?,?,?,?,?)',('old','old','{}',json.dumps(payload),'saved€'))
        ...
        try:self.assertEqual(reopened.log_window('old'),window)
        finally:await reopened.close()

    async def test_invalid_windows_and_wrong_home_are_refused(self):
        row=await self.launch("print('private')")
        await settled(self.registry,row['id'])
        for args in ({'after':-1},{'after':True},{'limit':0},{'limit':65537},{'limit':False},{'after':'0'}):
            with self.subTest(args=args):
                with self.assertRaises(ValueError):self.registry.log_window(row['id'],**args)
        with self.assertRaises(FileNotFoundError):self.registry.log_window('../processes.sqlite3')
        other=ProcessRegistry(self.root/'other',allowed_roots=[self.root])
        try:
            with self.assertRaises(FileNotFoundError):other.log_window(row['id'])
            self.assertEqual(other.list(),[])
        finally:await other.close()

    async def test_owner_http_and_native_cursor_use_same_registry(self):
        old=os.environ.get('GIDEON_HOME');os.environ['GIDEON_HOME']=str(self.root)
        ...
        try:
            async with ClientSession(cookie_jar=CookieJar(unsafe=True)) as client:
                ...
                response=await client.get(base+'/processes/'+row['id']+'/log-window?after=1&limit=2')
                self.assertEqual(response.status,200)
                value=await response.json()
                self.assertEqual(value['text'],'el');self.assertEqual(value['next'],3)
                tool=await create_provider().invoke('workspace_process_log_window',{'id':row['id'],'after':1,'limit':2})
                self.assertTrue(tool.success);self.assertEqual(json.loads(tool.output),value)
                response=await client.get(base+'/processes/'+row['id']+'/log-window?after=-1')
                self.assertEqual(response.status,400)
                response=await client.get(base+'/processes/missing/log-window')
                self.assertEqual(response.status,404)
            async with ClientSession() as anonymous:
                response=await anonymous.get(base+'/processes/'+row['id']+'/log-window')
                self.assertNotEqual(response.status,200)
                self.assertNotIn('hello',await response.text())
        finally:
            await runner.cleanup();await close_registry(self.root/'capabilities/workspace')
            if old is None:os.environ.pop('GIDEON_HOME',None)
            else:os.environ['GIDEON_HOME']=old
