...
async def main():
    with tempfile.TemporaryDirectory() as directory:
        ...
        (root / 'config.json').write_text(json.dumps({'dashboard': {'terminal': {'enabled': False}}}))
        ...
        print(json.dumps({'url':f'http://127.0.0.1:{port}', 'repo':str(repo), 'token':generate_token('workspace-browser')}), flush=True)
        try:
