...
class GitOperations(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.project = self.hierarchy.create_project(
            name="Git project", workspace_dir=str(self.repo)
        )
        self.service = GitService(
            self.root / "state", allowed_roots=[self.root], hierarchy=self.hierarchy
        )
    ...
    def payload(
        self, operation="create_branch", target="feature/local", request_id="operation"
    ):
        return {
    ...
    async def test_canonical_inspect_create_switch_and_reload_receipt(self):
        ...
        switched = await reloaded.mutate(
            self.payload("switch_branch", "main", "switch")
        )
    ...
    async def test_invalid_and_unknown_inputs(self):
        ...
        for payload in (
            {},
            {**self.payload(), "argv": ["push"]},
            {**self.payload(), "target": True},
        ):
            with self.assertRaises(ValueError):
    ...
    async def test_fsmonitor_hooks_and_inherited_git_environment_do_not_execute(self):
        ...
        (hooks / "post-checkout").write_text(hook.read_text())
        (hooks / "post-checkout").chmod(0o700)
        old = os.environ.get("GIT_DIR")
        os.environ["GIT_DIR"] = "/nonexistent-host-repo"
        try:
    ...
    async def test_filters_config_includes_and_nested_projects_refused(self):
        ...
        project = self.hierarchy.create_project(
            name="Nested", workspace_dir=str(nested)
        )
    ...
    async def test_concurrent_operations_do_not_corrupt_history(self):
        results = await asyncio.gather(
            self.service.mutate(self.payload()),
            self.service.mutate(self.payload(target="competing", request_id="second")),
        ...
        self.assertEqual(
            (await self.service.inspect(self.project.id))["head"], self.head
        )
    ...
    def submodule(self):
        ...
        git(self.repo, "add", ".gitmodules", "vendor/library")
        git(self.repo, "commit", "-m", "Pin library")
        self.head = git(self.repo, "rev-parse", "HEAD")
    ...
    async def test_initialized_submodule_restores_pinned_cached_commit(self):
        ...
        receipt = await self.service.mutate(
            self.payload("submodule_update", "vendor/library")
        )
        self.assertEqual(receipt["status"], "succeeded")
    ...
    async def test_dirty_unknown_and_uninitialized_submodules_refused(self):
        ...
        self.assertFalse(
            (await self.service.inspect(self.project.id))["submodules"][0][
                "initialized"
            ]
        )
        ...
        try:
            async with ClientSession(cookie_jar=CookieJar(unsafe=True)) as client:
                ...
                result = await provider.invoke(
                    "workspace_git_history", {"project_id": self.project.id}
                )
                ...
                result = await provider.invoke(
                    ...
                    self.payload("switch_branch", "main", "native"),
                )
                self.assertTrue(result.success, result.error)
                self.assertEqual(git(self.repo, "branch", "--show-current"), "main")
                response = await client.get(
                    base + "/" + self.project.id + "/operations"
                )
            ...
            async with ClientSession() as anonymous:
                response = await anonymous.post(
                    base + "/operations", json=self.payload()
                )
