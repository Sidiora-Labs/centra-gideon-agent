...
class TestAppConfig:
    ...
    def test_load_from_file(self, tmp_path, monkeypatch):
        ...
        assert not hasattr(cfg.agent, "streaming")
...
class TestObserveSizing:
    ...
    def test_top_level_keys_win(self, tmp_path, monkeypatch):
        ...
        cfg_file.parent.mkdir(parents=True)
        cfg_file.write_text(
            json.dumps({"observe_max_messages": 55, "observe_ttl_hours": 24.0})
        )
        monkeypatch.setattr("gideon.core.config.loader.config_path", lambda: cfg_file)
        cfg = AppConfig.load()
    ...
    def test_legacy_slack_observe_keys_are_ignored(self, tmp_path, monkeypatch):
        ...
        cfg_file.write_text(
            json.dumps(
                {"slack": {"observe_max_messages": 77, "observe_ttl_hours": 12.0}}
            )
        )
        monkeypatch.setattr("gideon.core.config.loader.config_path", lambda: cfg_file)
        cfg = AppConfig.load()
    ...
    def test_save_preserves_legacy_slack_block(self, tmp_path, monkeypatch):
        ...
        cfg_file.parent.mkdir(parents=True)
        legacy = {
            "slack": {"tracking_channels": [{"channel_id": "C1"}], "command": "pc"}
        }
    ...
    def test_save_serializes_observe_top_level(self, tmp_path, monkeypatch):
        ...
        cfg_file.parent.mkdir(parents=True)
        cfg_file.write_text(
            json.dumps({"observe_max_messages": 77, "observe_ttl_hours": 12.0})
        )
        monkeypatch.setattr("gideon.core.config.loader.config_path", lambda: cfg_file)
        cfg = AppConfig.load()
        cfg.save()
        ...
        assert "slack" not in after

    def test_load_does_not_warn_on_legacy_slack_key(
