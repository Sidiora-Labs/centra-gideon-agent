...
from gideon.automation.workflows.loop_middleware import DEFAULT_LADDER
...
from gideon.security.guardrails.policy import HEADLESS, INTERACTIVE, SafetyProfile
...
LOOP_ATTENDED = _field_default(Loop, "attended")
LOOP_MAX_CYCLES = _field_default(Loop, "max_cycles")
LOOP_IDLE_SECS = _field_default(Loop, "idle_secs")
LOOP_TRUST_TTL = _field_default(LoopsConfig, "trust_ttl_secs")

BUNDLED_LOOP_KINDS = ("goal", "code", "design", "research", "general")


def _today_for(
    *, profile: SafetyProfile, attended: bool, max_cycles: int
) -> dict[str, Any]:
    ...
    return {
        "RunBudget": profile.budget.max_tokens,
...
class _Run:
    ...
    def __init__(
        self, name: str, policy: SupervisorPolicy, today: dict[str, Any]
    ) -> None:
        self.name = name
        self.policy = policy
        self.today = today
    ...
    for kind in BUNDLED_LOOP_KINDS:
        runs.append(
            _Run(
                ...
                _today_for(
                    profile=HEADLESS, attended=LOOP_ATTENDED, max_cycles=LOOP_MAX_CYCLES
                ),
            )
        )
...
def test_the_map_covers_the_fourteen_knobs_and_every_field_resolves():
    ...
    for m in POLICY_KNOB_MAP:
        resolve_field(probe, m.field_path)
    heads = [m.field_path.split(".")[0] for m in POLICY_KNOB_MAP]
    assert (
        heads.count("hitl_posture") == 3
    ), "require_hitl + confirmation + attended → one field"
    assert (
        heads.count("autonomy") >= 3
    ), "RunBudget + gate approval + SafetyProfile → one object"
...
def test_the_matrix_is_not_vacuous():
    ...
    assert approvals == {
        "hook_based",
        "ask",
    }, "both profiles' approval postures are exercised"
    assert any(
        r.today["max_cycles"] == LOOP_MAX_CYCLES for r in runs
    ), "a loop cap (30) is seen"
    assert any(
        r.today["max_cycles"] == 0 for r in runs
    ), "an uncapped workflow run is seen"
    assert any(
        r.today["runtime_hints.execution.single_active_feature"] for r in runs
    ), "the one WIP=1 template is seen"
    assert any(
        r.today["loop attended"] == Attention.HITL for r in runs
    ), "an attended run is seen"
...
def test_tightest_wins_a_profile_cannot_widen_the_ceiling():
    ...
    loose = consolidate(
        profile=SafetyProfile(
            name="loose", approval="auto", budget=Budget(max_tokens=10_000_000)
        )
    )
    ...
    assert (
        composed.autonomy.approval != "auto"
    ), "the loose profile value must NOT survive"
    assert (
        composed.autonomy.budget.max_tokens == 1000
    ), "a tighter ceiling token cap must win"
