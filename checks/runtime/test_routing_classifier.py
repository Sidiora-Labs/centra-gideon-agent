...
from gideon.engine.routing import classifier as c
...
class TestCode:
    ...
    def test_code_keyword_signal(self):
        assert (
            c.classify_query("class Foo: def bar(self): return SELECT * FROM t")
            == "code"
        )

    def test_prose_about_a_function_is_not_code(self):
        assert (
            c.classify_query("what is the function of the mitochondria in a cell?")
            != "code"
        )
...
class TestLongReasoning:
    def test_long_text(self):
        assert c.classify_query("x " * 1500) == "long_reasoning"

    def test_reasoning_use_case(self):
        assert (
            c.classify_query("which is better?", use_case="reasoning")
            == "long_reasoning"
        )
...
class TestPurity:
    def test_deterministic(self):
        ...
        assert c.classify_query(prompt, "reasoning") == c.classify_query(
            prompt, "reasoning"
        )
