...
from gideon.automation.loop.watchdog import (
    active_runtime_secs,
    deadline_reached,
    default_stop_reason,
)
...
def test_done_reason_round_trips_and_stamps_with_completed_at(loop_row) -> None:
    ...
    out = store.update_status(
        loop_row.id, LoopStatus.COMPLETE, stop_reason=LoopStopReason.DONE
    )
...
def test_user_stop_reason_via_manager_stop_path(loop_row) -> None:
    store.update_status(loop_row.id, LoopStatus.RUNNING)
    out = store.update_status(
        loop_row.id, LoopStatus.STOPPED, stop_reason=LoopStopReason.USER
    )
...
def test_deadline_reason_round_trips(loop_row) -> None:
    store.update_status(loop_row.id, LoopStatus.RUNNING)
    out = store.update_status(
        loop_row.id, LoopStatus.COMPLETE, stop_reason=LoopStopReason.DEADLINE
    )
...
def test_legacy_row_migrates_to_uncapped_defaults(tmp_path, monkeypatch) -> None:
    ...
    assert got is not None
    assert (
        got.max_cost_usd == 0.0 and got.deadline_secs == 0.0 and got.stop_reason == ""
    )
