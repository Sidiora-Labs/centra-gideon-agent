...
def frozen_clock(monkeypatch):
    """Freeze ``gideon.cognition.context.datetime`` so the ``[CURRENT DATE] … HH:MM …``
    ...
    """
    import gideon.cognition.context as _ctx
...
def test_default_assemble_matches_build_message(builder, frozen_clock):
    ...
    assembled = assemble_context(
        builder, text, is_new_session=True, session_key="chat-1"
    )
...
def test_followup_turn_injects_nothing(builder):
    text = "and then?"
    assembled = assemble_context(
        builder, text, is_new_session=False, session_key="chat-1"
    )
    assert assembled.injected_chars == 0
