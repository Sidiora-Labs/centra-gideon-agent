...
def row(archive,identifier):
    value=archive.db.execute('SELECT * FROM semantic_memory WHERE key=?',(identifier,)).fetchone()
...
def test_exact_active_tombstone_restore_and_idempotent_owner_lifecycle(archive):
    ...
    assert (created.accepted,created.code,created.message)==(True,'import_create','')
    ...
    assert archive.db.execute('SELECT event_type FROM memory_events').fetchall()==events
    ...
    restored=record(value={'fact':'restored'},updated_at='2026-09-24T13:00:00+00:00')
    ...
    assert loaded.created_at==CREATED and loaded.updated_at=='2026-09-24T13:00:00+00:00'
...
def test_tombstone_without_live_row_and_update_clear_local_derived_state(archive):
    ...
    archive.db.commit()
    changed=record('project.beta.fact',value='new',updated_at='2026-09-24T14:00:00+00:00')
...
def test_policy_rejections_are_explicit_and_leave_no_rows_or_events(archive,candidate,code):
...
def test_protected_local_conflict_is_explicit_and_unchanged(archive):
    ...
    incoming=record(value='peer',source='peer_replication',updated_at='2026-09-24T12:00:00+00:00')
    ...
    assert [item['event_type'] for item in archive.read_events(limit=10)]==['import_create']
