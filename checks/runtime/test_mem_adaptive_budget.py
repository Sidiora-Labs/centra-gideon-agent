...
def test_caps_scale_with_window():
    ...
    assert base["history_cap"] == _MEMORY_HISTORY_CAP
    assert base["prefs_cap"] == _MEMORY_PREFS_CAP
    big = _memory_caps(1_000_000)
    assert big["history_cap"] == _MEMORY_HISTORY_CAP * 5
...
def test_caps_floor_at_baseline_for_small_and_unknown():
    from gideon.cognition.context import _MEMORY_HISTORY_CAP, _memory_caps
