...
from gideon.cognition import bg_compress
...
def _patch_prose(monkeypatch):
    ...
    monkeypatch.setattr(
        "gideon.integrations.tool_providers.prose_compress.compress_prose",
        _fake_prose,
        raising=True,
    )
...
async def test_compress_session_shrinks_and_archives(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "gideon.cognition.history.config_dir", lambda: tmp_path, raising=False
    )
...
async def test_pass_skips_incognito_and_active(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        bg_compress, "_record_savings", lambda *a, **k: None, raising=True
    )
    stats = await bg_compress.run_bg_compression_pass(
        log, embed_fn=None, max_sessions=10
    )
