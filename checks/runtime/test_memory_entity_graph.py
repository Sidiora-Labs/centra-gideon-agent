...
def _seeded(store, graph):
    ...
    project = graph.upsert_entity("Gideon", "project", aliases=["pclaw"])
...
class TestAliasIndex:
    def test_matches_name_and_aliases(self):
        index = AliasIndex()
        index.add_entity(Entity("e1", "Gideon", "project", aliases=("pclaw", "p-claw")))
        for text in ("I use Gideon daily", "pclaw is fast", "the p-claw repo"):
    ...
    def test_determinism(self):
        ...
        index = AliasIndex()
        index.add_entity(Entity("e1", "Gideon", "project", aliases=("pclaw",)))
        index.add_entity(Entity("e2", "Keyur Golani", "person"))
        text = "Keyur Golani shipped pclaw and Gideon"
...
class TestEntities:
    def test_create_and_read_back(self, graph):
        eid = graph.upsert_entity("Gideon", "project", aliases=["pclaw"])
    ...
    def test_upsert_merges_aliases(self, graph):
        graph.upsert_entity("Gideon", "project", aliases=["pclaw"])
...
class TestCascade:
    ...
    def test_project_key_plus_project_entity_yields_same_project(self):
        assert (
            classify_link("project.claw.note", "Gideon", self._mention(), {"e1": "project"})
            == "same_project"
        )
