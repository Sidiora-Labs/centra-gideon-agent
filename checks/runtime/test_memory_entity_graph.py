...
class TestAliasIndex:
    def test_matches_name_and_aliases(self):
        index = AliasIndex()
        index.add_entity(
            Entity("e1", "Gideon", "project", aliases=("gideon", "p-claw"))
        )
        for text in ("I use Gideon daily", "gideon is fast", "the p-claw repo"):
...
class TestEntities:
    ...
    def test_upsert_merges_aliases(self, graph):
        ...
        graph.upsert_entity("Gideon", "project", aliases=["claw"])
        assert graph.entities()[0].aliases == ("claw", "gideon")
...
class TestCascade:
    ...
    def test_project_key_plus_project_entity_yields_same_project(self):
        assert (
            classify_link(
                "project.claw.note", "Gideon", self._mention(), {"e1": "project"}
            )
            == "same_project"
        )
    ...
    def test_project_key_with_a_person_does_not_claim_affiliation(self):
        assert (
            classify_link(
                "project.claw.note", "Keyur", self._mention(), {"e1": "person"}
            )
            != "same_project"
        )
...
class TestBackfill:
    def test_links_existing_records(self, store, graph):
        store.set_semantic("project.claw.a", "gideon notes", 0.9, "user_explicit")
...
class TestServiceSurface:
    ...
    def test_service_backlinks_answer_what_do_i_know(self, store, graph):
        project, _ = _seeded(store, graph)
        store.set_semantic("project.claw.a", "gideon is fast", 0.9, "user_explicit")
...
class TestGraphRecallArm:
    ...
    def test_boost_accumulates_across_named_entities(self, store, graph):
        ...
        _seeded(store, graph)
        store.set_semantic(
            "project.claw.both", "Keyur Golani works on gideon", 0.9, "user_explicit"
        )
