...
class TestKeyValidation:
    def test_valid_keys(self, tmp_path: Path) -> None:
        ...
        assert (
            store.set_semantic("pref.backend.framework", "python", 1.0, "user_explicit")
            is None
        )
    ...
    def test_too_long(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert (
            store.set_semantic("pref." + "a" * 100, "x", 1.0, "user_explicit")
            is not None
        )
...
class TestAllowlist:
    def test_allowlisted_key_accepted(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert (
            store.set_semantic("pref.frontend.framework", "react", 1.0, "user_explicit")
            is None
        )
    ...
    def test_non_allowlisted_rejected(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert (
            store.set_semantic("random.key.here", "val", 1.0, "user_explicit")
            is not None
        )

    def test_custom_prefix(self, tmp_path: Path) -> None:
        store = SemanticArchive(
            db_path=tmp_path / "mem.db", extra_prefixes=["custom.myapp.*"]
        )
        store.init()
        assert (
            store.set_semantic("custom.myapp.setting", "val", 1.0, "user_explicit")
            is None
        )

    def test_reserved_prefix_rejected_from_llm(self, tmp_path: Path) -> None:
        store = SemanticArchive(
            db_path=tmp_path / "mem.db", extra_prefixes=["system.*"]
        )
        store.init()
        assert (
            store.set_semantic("system.override", "val", 0.9, "consolidation:abc")
            is not None
        )
    ...
    def test_reserved_prefix_allowed_from_user(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert (
            store.set_semantic("system.override", "val", 1.0, "user_explicit") is None
        )

    def test_underscore_prefix_rejected_by_key_format(self, tmp_path: Path) -> None:
        store = SemanticArchive(
            db_path=tmp_path / "mem.db", extra_prefixes=["_internal.*"]
        )
        store.init()
...
class TestConfidenceGating:
    def test_low_confidence_rejected(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert (
            store.set_semantic("pref.os", "macos", 0.5, "consolidation:abc") is not None
        )
...
class TestValidateSemantic:
    ...
    def test_value_json_kwarg_skips_serialization(self, tmp_path: Path) -> None:
        ...
        big_json = '"' + "x" * 5000 + '"'
        result = store.validate_semantic(
            "pref.os", None, 1.0, "user_explicit", value_json=big_json
        )
        assert result is not None
        code, _ = result
        assert code.value == "value_size"
...
class TestLogRejectEvent:
    def test_auditable_code_logs_event(self, tmp_path: Path) -> None:
        ...
        with patch.object(store, "_log_event") as mock_log:
            store.log_reject_event(
                SemanticRejectCode.ALLOWLIST, "bad.key", "v", "user_explicit"
            )
    ...
    def test_non_auditable_code_skipped(self, tmp_path: Path) -> None:
        ...
        with patch.object(store, "_log_event") as mock_log:
            store.log_reject_event(
                SemanticRejectCode.KEY_FORMAT, "x", "v", "user_explicit"
            )
...
class TestInjectionDetection:
    ...
    def test_clean_values_accepted(self, tmp_path: Path) -> None:
        ...
        assert (
            store.set_semantic(
                "pref.style.indentation", "4 spaces", 1.0, "user_explicit"
            )
            is None
        )
        assert (
            store.set_semantic("pref.backend.framework", "django", 1.0, "user_explicit")
            is None
        )
...
class TestValueSizeLimit:
    def test_large_value_rejected(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert (
            store.set_semantic("pref.os", "x" * 5000, 1.0, "user_explicit") is not None
        )
...
class TestSchemaInit:
    ...
    def test_file_permissions(self, tmp_path: Path) -> None:
        ...
        store = SemanticArchive(db_path=db_path)
        store.init()
    ...
    def test_idempotent_init(self, tmp_path: Path) -> None:
        ...
        store.close()
        store2 = SemanticArchive(db_path=tmp_path / "mem.db")
        store2.init()
...
class TestEmbeddingDimensionChange:
    ...
    def test_write_survives_a_wider_embedding(self, tmp_path: Path) -> None:
        store = SemanticArchive(db_path=tmp_path / "mem.db", embedding_dim=384)
        store.init()
        wide = [0.01] * 1024
    ...
    def test_write_still_indexes_a_matching_embedding(self, tmp_path: Path) -> None:
        store = SemanticArchive(db_path=tmp_path / "mem.db", embedding_dim=8)
        store.init()
        assert store.write_episodic(
            "A memory with a correctly sized vector", embedding=[0.1] * 8
        )
    ...
    def test_a_narrower_embedding_is_also_skipped(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert store.write_episodic(
            "A memory embedded by a smaller model", embedding=[0.5] * 128
        )
        assert len(store.get_episodic_list()) == 1

    def test_the_skip_is_logged_with_both_dimensions(
    ...
    def test_recall_falls_back_instead_of_raising(self, tmp_path: Path) -> None:
        ...
        store.init()
        store.write_episodic(
            "A memory indexed at the current width", embedding=[0.3] * 8
        )
        got = store.search_episodic(
            query_embedding=[0.3] * 1024, query_text="anything at all"
        )
        assert got == []
    ...
    def test_dedup_on_write_survives_a_width_change(self, tmp_path: Path) -> None:
        ...
        store.init()
        assert store.write_episodic(
            "First memory at the indexed width", embedding=[0.3] * 8
        )
        assert store.write_episodic(
            "Second memory from the new model", embedding=[0.3] * 1024
        )
...
class TestEpisodicCRUD:
    ...
    def test_tag_sanitization(self, tmp_path: Path) -> None:
        ...
        store.init()
        store.write_episodic(
            "Some memory about testing", tags=["  UPPER ", "", "valid"]
        )
        entries = store.get_episodic_list()
        import json
    ...
    def test_importance_clamped(self, tmp_path: Path) -> None:
        ...
        store.init()
        store.write_episodic(
            "Important architectural decision about microservices", importance=5.0
        )
        entries = store.get_episodic_list()
        assert entries[0]["importance"] == 1.0

    def test_episodic_cap_enforcement(self, tmp_path: Path) -> None:
        store = SemanticArchive(db_path=tmp_path / "mem.db", episodic_max=5)
        store.init()
        for i in range(7):
            store.write_episodic(
                f"Memory number {i} about some topic here", importance=0.5
            )
    ...
    def test_episodic_limit_configured(self, tmp_path: Path) -> None:
        ...
        store = SemanticArchive(db_path=tmp_path / "mem.db", episodic_limit=2)
        store.init()
        for i in range(5):
...
class TestEpisodicCitations:
    ...
    def _seed(self, tmp_path: Path) -> SemanticArchive:
        store = SemanticArchive(db_path=tmp_path / "mem.db")
        store.init()
    ...
    def test_citation_markers_and_manifest(self, tmp_path: Path) -> None:
        ...
        block = store.get_episodic_context(
            query_text="deploy production", citations_out=cites
        )
        assert block
