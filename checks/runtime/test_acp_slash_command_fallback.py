...
from gideon.integrations.acp.types import CAP_COMMANDS, METHOD_COMMANDS_EXECUTE
            ...
            [
                {
                    "id": 1,
                    "result": {
                        "agentCapabilities": {**_CAPS_0_60_0, CAP_COMMANDS: True}
                    },
                }
            ]
        )
...
class TestNoFrameWithoutAdvertisement:
    ...
    async def test_session_provider_refuses_the_same_way(self):
        ...
        provider = AcpSessionProvider(
            conn, session, runtime_id="acp:g4", model="m", agent_name="a"
        )
...
class TestDispatchOutcomes:
    ...
    async def test_method_not_found_after_output_refuses_to_duplicate(self):
        ...
        assert [e.text for e in seen] == [
            "PARTIAL"
        ], "partial output is kept, not replayed"
        assert notices == [], "nothing was substituted, so nothing may claim it was"
        assert (
            raised is not None
        ), "a command that failed mid-turn must not pass silently"
    ...
    async def test_a_different_error_is_never_substituted(self):
        ...
        assert (
            provider.prompts == []
        ), "a -32000 failure was SWALLOWED and answered as a prompt"
        assert notices == [] and events == []
        assert raised is not None and not isinstance(
            raised, AcpCommandFailedAfterOutput
        )
    ...
    async def test_an_advertised_command_still_runs_natively(self):
        ...
        provider = _FakeProvider(
            supports=True,
            command_events=[LLMEvent(kind=EVENT_TEXT_CHUNK, text="usage report")],
        )
        events, notices = await _dispatch(provider)
    ...
    def test_the_guard_reports_the_inner_providers_answer(self, tmp_path, monkeypatch):
        ...
        def _wrap(inner):
            return ModelCallGuard(
                inner, use_case="chat", provider_name="acp:test", model="m"
            )
...
class TestTurnLevel:
    ...
    async def test_slash_turn_answers_and_announces_instead_of_erroring(
        ...
        assert (
            len(notices) == 1
        ), "the user must be told the command was not run natively"
