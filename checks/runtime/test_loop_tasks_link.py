...
class TestProvision:
    ...
    def test_provision_nests_under_user_chosen_project_id(self):
        ...
        out = tasks_link.provision(c.id)
        assert out.tasks_project_id == proj.id
        assert h.get_project(proj.id).name == "Website Redesign"
    ...
    def test_two_loops_in_one_project_get_isolated_phase_lists(self):
        ...
        assert (
            out_a.task_list_ids["implementation"]
            != out_b.task_list_ids["implementation"]
        )
        # Seeding A's tasks must not appear in B's list.
        _run(
            tasks_link.decompose_phase(
                a.id, "implementation", [{"title": "A-only task"}]
            )
        )
        ...
        b_tasks, _ = _run(
            registry.list_all_tasks(
                task_list_id=out_b.task_list_ids["implementation"], limit=50
            )
        )
...
class TestReconcileAndTeardown:
    def test_reconcile_closes_open_tasks(self):
        ...
        _run(
            tasks_link.decompose_phase(
                c.id, "implementation", [{"title": "A"}, {"title": "B"}]
            )
        )
    ...
    def test_teardown_deletes_tasks_and_project(self):
        ...
        assert removed >= 1
        assert HierarchyStore().get_project(auto_pid) is None
    ...
    def test_teardown_preserves_user_chosen_shared_project(self):
        ...
        c = _code(
            project_id=proj.id, plan=[_stage("implementation", tasks=[{"title": "A"}])]
        )
        out = tasks_link.provision(c.id)
        assert out.tasks_project_id == proj.id
        ...
        assert h.get_project(proj.id) is not None
