...
from gideon.workspace.snapshot import _tree_ignore_dbs, snapshot_main
...
async def test_all_capability_routes_share_the_actual_authenticated_application(home):
    ...
    required = {
        ...
        "/api/capabilities/knowledge/anniversaries",
        "/api/capabilities/knowledge/captures",
        "/api/capabilities/identity/stories",
        "/api/capabilities/identity/twin",
        "/api/capabilities/wellbeing/measurements",
        "/api/capabilities/wellbeing/labs",
        "/api/capabilities/communications/people",
        "/api/capabilities/media/sketches",
        "/api/capabilities/creative/ingredients",
        "/api/capabilities/music/items",
        "/api/capabilities/music/catalog/{kind}",
        "/api/capabilities/experience/stories",
        "/api/capabilities/platform/catalog",
    }
    ...
    try:
        async with ClientSession(cookie_jar=CookieJar(unsafe=True)) as client:
            ...
            response = await client.get(
                base + "/api/capabilities/identity/stories?token=" + token
            )
            assert response.status == 200, await response.text()
            response = await client.post(
                base + "/api/capabilities/identity/stories",
                json={
                    "prompt": "What remained important?",
                    "theme": "Learning",
                    "text": "Preserving the original source.",
            ...
            response = await client.get(
                base + "/api/capabilities/identity/stories/" + story["id"]
            )
            ...
            response = await client.get(
                base + "/api/capabilities/music/catalog/artists"
            )
            assert response.status == 200, await response.text()
            for path in [
                ...
                "/api/capabilities/knowledge/types",
                "/api/capabilities/knowledge/archives",
                "/api/capabilities/identity/fidelity/cases",
                "/api/capabilities/identity/goals/goals",
                "/api/capabilities/identity/progress",
                "/api/capabilities/identity/continuity",
                "/api/capabilities/wellbeing/apple/metrics",
                "/api/capabilities/wellbeing/substances/entries",
                "/api/capabilities/wellbeing/genome/sources",
                "/api/capabilities/music/rounds",
                "/api/capabilities/music/midi",
                "/api/capabilities/platform/harnesses",
                "/api/capabilities/platform/comparisons",
                "/api/capabilities/platform/references",
            ]:
                response = await client.get(base + path)
                assert response.status == 200, (
                    path,
            ...
            assert (
                StoryStore(home / "capabilities/identity/stories.sqlite3").get(
                    story["id"]
                )
                == story
            )
...
def test_every_capability_database_uses_the_sqlite_backup_inventory():
    entries = [
        e
        for e in inventory.all_entries()
        if e.id.startswith("capability_") and e.kind == inventory.KIND_SQLITE
    ]
...
def test_snapshot_preserves_live_wal_records_without_raw_overwrite(home, tmp_path):
    ...
    first = store.create(
        prompt="Original?", theme="History", text="First answer", request_id="first"
    )
    ...
    with tarfile.open(archives[0]) as archive:
        ...
        cadence_member = next(
            name
            for name in members
            if name.endswith("/capabilities/platform/cadence.json")
        )
        assert archive.extractfile(cadence_member).read() == cadence.read_bytes()
        source = next(
            name
            for name in members
            if name.endswith("/capabilities/identity/stories.sqlite3")
        )
        assert not any(
            name.endswith((".sqlite3-wal", ".sqlite3-shm")) for name in members
        )
        ...
        people_member = next(
            name
            for name in members
            if name.endswith("/capabilities/communications/people.sqlite3")
        )
    ...
    with sqlite3.connect(restored_people) as connection:
        ...
        assert person["id"] in [
            row[0] for row in connection.execute("SELECT id FROM people")
        ]
