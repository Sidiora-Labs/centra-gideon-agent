...
class TestChatMessageType:
    ...
    async def test_a_missing_message_is_NOT_caught_by_the_new_type_guard(self):
        ...
        if resp.status == 400:
            assert "must be a string" not in json.loads(resp.body.decode()).get(
                "error", ""
            )
...
class TestFolderOrderCoercion:
    ...
    async def test_an_unparseable_order_is_ignored_not_a_500(self, bad):
        from gideon.interfaces.dashboard.chat_folders import api_chat_folder_update
        ...
        assert (
            state._folders[0]["order"] == 3
        ), "the prior order must survive a bad value"
    ...
    def test_it_matches_the_sibling_it_was_measured_against(self):
        ...
        from gideon.interfaces.dashboard import chat_folders, chat_tags
...
class TestAgentDetailPatch:
    ...
    def test_the_delete_guard_uses_the_reserved_SET(self):
        ...
        assert (
            len(RESERVED_AGENT_NAMES) >= 5
        ), "the set this now consults must be the real one"
...
class TestEventMetadataStaysJSON:
    ...
    def test_a_non_finite_number_does_not_poison_the_payload(self, bad):
        ...
        from gideon.workspace.artifacts.models import clean_event_metadata

        out = clean_event_metadata({"widget_index": bad})
        assert _strict_json_roundtrip(
            out
        ), f"{bad!r} still serializes to invalid JSON: {out}"
    ...
    def test_the_out_of_range_value_is_RECORDED_not_dropped(self):
        ...
        assert clean_event_metadata({"widget_index": float("inf")}) == {
            "widget_index": "inf"
        }
