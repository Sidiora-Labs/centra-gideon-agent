...
from gideon.extensions.apps import app_manager, manager, prompt_registry
...
def _isolate(tmp_path, monkeypatch):
    ...
    import gideon.integrations.prompt_providers.registry as preg
...
def test_web_extract_renders_byte_identical_to_pre_migration_literal():
    ...
    if not (_APPS_DIR / "web-tools").is_dir():
...
def test_knowledge_intent_match_renders_byte_identical():
    ...
    app_manager.install(
        BUNDLED_DIR / "native-knowledge", origin="builtin", confirm=True
    )
...
def test_knowledge_skill_synthesis_renders_byte_identical():
    ...
    rendered = render_use_case_prompt(
        "knowledge_skill_synthesis", {"goal": goal, "digest": digest}
    )
...
def test_provideless_app_seeds_prompt_and_snippet_and_resolves(tmp_path):
    ...
    assert prov.get_snippet("app-sig") is not None
...
def test_seed_is_non_clobbering_of_user_edits(tmp_path):
    ...
    prov.update_prompt(
        "app-greet",
        PromptTemplate(name="app-greet", kind="user", content="EDITED {{who}}"),
    )
