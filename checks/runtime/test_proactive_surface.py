...
_RUN = {
    "run_id": "run-abc",
    "status": "succeeded",
    "finished_at": "2026-08-25T08:00:01+00:00",
}
...
class TestFourDifferentAnswers:
    ...
    def test_a_ready_digest_reports_ready(self, window: Manifest) -> None:
        view = build_digest_view(
            enabled=True, installed=True, run=_RUN, output=_output()
        )
...
class TestAnUnmeasuredValueIsNotAZero:
    ...
    def test_a_stage_that_ran_and_did_nothing_is_distinguishable_from_one_that_is_off(
        self,
    ) -> None:
        off = build_digest_view(
            enabled=True, installed=True, run=_RUN, output=_output()
        )
    ...
    def test_skipped_ledger_rows_read_as_incomplete_not_as_none(self) -> None:
        view = build_digest_view(
            ...
            installed=True,
            run=_RUN,
            output=_output(dropped=2, ledger_rows=0),
        )
    ...
    def test_a_genuinely_empty_filter_is_complete(self) -> None:
        view = build_digest_view(
            ...
            installed=True,
            run=_RUN,
            output=_output(dropped=0, ledger_rows=0),
        )
...
class TestDeliveryIsNotClaimed:
    ...
    `delivered: True` and the notification list did NOT grow by one. `ConsoleState.notify`
...
class TestPending:
    def test_pending_is_the_deferred_set_when_the_stage_ran(self) -> None:
        view = build_digest_view(
            enabled=True, installed=True, run=_RUN, output=_auto_ran()
        )
    ...
    def test_missing_provenance_is_blank_not_a_placeholder(self) -> None:
        view = build_digest_view(
            enabled=True, installed=True, run=_RUN, output=_output(items=[])
        )
...
class TestReplyIdempotency:
    def test_an_answered_ordinal_is_read_off_the_runs_own_ledger(self) -> None:
        events = [
            {"kind": "triage_reply", "item_ordinal": "1", "verb": "always no", "seq": 7}
        ]
    ...
    def test_the_card_marks_an_answered_proposal_as_answered(self) -> None:
        view = build_digest_view(
            ...
            output=_output(),
            events=[
                {"kind": "triage_reply", "item_ordinal": "1", "verb": "no", "seq": 7}
            ],
        )
        answered = {
            row["ordinal"]: (row["answered"], row["answer"]) for row in view["pending"]
        }
...
class TestTheManifestProjectionIsTheOrdinalContractMadeDurable:
    def test_the_summary_carries_the_ordinal_to_id_map(self, window: Manifest) -> None:
        ...
        assert {row["source_id"] for row in summary["items"]} == {
            "gh_1",
            "gh_2",
            "tg_9",
        }

    def test_the_projection_never_carries_the_fenced_body(
    ...
    def test_a_projection_round_trips_without_renumbering(
        self, window: Manifest
    ) -> None:
        restored = manifest_from_projection(
            window.projection(), window_start=window.window_start
        )
...
class TestTheReplyLedgerKindIsVisible:
    def test_triage_reply_is_in_the_readable_set(self) -> None:
        from gideon.assurance.ledger.kinds import LEDGER_KINDS, TRIAGE_REPLY
