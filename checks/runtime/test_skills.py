...
def _isolate_discovery_paths(monkeypatch):
    """Make the loader hermetic: ProcedureLibrary always merges in the global
    ...
    monkeypatch.setattr(
        "gideon.extensions.skills.marketplace.SKILL_DISCOVERY_PATHS", []
    )
...
class TestSkillsLoader:
    ...
    def test_get_context_empty(self, tmp_path):
        loader = ProcedureLibrary(
            skills_path=tmp_path / "empty", install_builtins=False
        )
...
class TestTriggeredSkills:
    ...
    def _loader_with_skill(self, tmp_path, triggers, monkeypatch=None):
        ...
        if monkeypatch is not None:
            ...
            monkeypatch.setattr("gideon.security.sel.sel", lambda: MagicMock())
        return loader
    ...
    def test_reworded_query_matches(self, tmp_path, monkeypatch):
        ...
        assert "tiny-url" in loader.get_triggered_skills(
            "can you shorten this url please"
        )
...
class TestTriggerMatching:
    ...
    def test_basic_trigger_match(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "gideon.core.config.loader.AppConfig.load", lambda: mock_config
        )
        monkeypatch.setattr("gideon.security.sel.sel", lambda: MagicMock())
    ...
    def test_negative_trigger_excludes(self, tmp_path, monkeypatch):
        ...
        assert "code-search" not in loader.get_triggered_skills(
...
class TestFindSimilar:
    """Tests for ProcedureLibrary.find_similar description overlap dedup."""
    ...
    def test_exclude_self_during_refine(self, tmp_path):
        ...
        assert (
            loader.find_similar("one two three four five keywords", exclude="auto/foo")
            is None
        )
...
class TestCreateAutoSkill:
    """Tests for ProcedureLibrary.create_auto_skill."""
    ...
    def test_rejects_oversized_procedure(self, tmp_path):
        from gideon.extensions.skills import AUTO_SKILL_MAX_PROCEDURE_CHARS
...
class TestUpdateAutoSkill:
    """Tests for ProcedureLibrary.update_auto_skill (refine path)."""
...
class TestListAutoSkills:
    ...
    def test_filters_to_auto_namespace_only(self, tmp_path):
        ...
        loader.create_auto_skill(
            ...
            provenance=(
                __import__(
                    "gideon.extensions.skills", fromlist=["AutoSkillProvenance"]
                ).AutoSkillProvenance(
                    session_key="x",
                    created_at="2026-05-05T11:30:00+00:00",
                )
...
class TestAutoNameFromTitleTruncation:
    ...
    def test_trailing_hyphen_stripped_after_truncation(self):
        from gideon.extensions.skills import _auto_name_from_title
    ...
    def test_normal_title_unaffected(self):
        ...
        assert (
            _auto_name_from_title("Debug Timber logs via SSH")
            == "debug-timber-logs-via-ssh"
        )
...
class TestUpdateAutoSkillPreservesCreatedAt:
    ...
    def test_created_at_preserved_across_refine(self, tmp_path):
        ...
        assert ok is True
        content = (
            tmp_path / "skills" / "auto" / "preserved-ts" / "SKILL.md"
        ).read_text()
