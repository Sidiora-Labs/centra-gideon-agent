...
from gideon.llm.registry import ProviderResolutionError as LLMResolveErr
from gideon.providers.provider_bridge import ProviderResolutionError as BridgeResolveErr
...
async def test_no_provider_returns_fallback_instead_of_raising(state_with_context, exc_cls):
    ...
    with patch(
        "gideon.prompt_providers.runtime.render_use_case_prompt", return_value="PROMPT"
    ):
...
async def test_refresh_logs_no_traceback_and_advances_generated_at(
    ...
    with (
        ...
        caplog.at_level(logging.WARNING, logger="gideon.suggestions"),
    ):
