...
def _age(vs, key, *, updated=None, invalidated=None):
    if updated:
        vs.db.execute(
            "UPDATE semantic_memory SET updated_at=? WHERE key=?", (updated, key)
        )
    if invalidated:
        vs.db.execute(
...
def test_clean_store_has_no_flags(vs):
    vs.set_semantic(
        "pref.x", "a clear, recent fact about the user", 1.0, "user_explicit"
    )
    vs.record_recall(["pref.x"])
...
def test_recalled_fact_not_stale(vs):
    ...
    vs.record_recall(["pref.live"])
    report = lint_memory(vs)
    assert not any(
        f["key"] == "pref.live" for f in report.flags if f["check"] == "stale"
    )


def test_flags_near_duplicates(vs):
    vs.set_semantic(
        "pref.a", "prefers the vim text editor for all coding", 1.0, "user_explicit"
    )
    vs.set_semantic(
        "pref.b",
        "prefers the vim text editor for all coding tasks",
        1.0,
        "user_explicit",
    )
    report = lint_memory(vs)
...
def test_flags_sparse(vs):
    ...
    report = lint_memory(vs)
    assert any(
        f["check"] == "sparse" and f["key"] == "pref.empty" for f in report.flags
    )
...
def test_auto_purges_long_superseded(vs):
    ...
    assert (
        vs.db.execute(
            "SELECT COUNT(*) FROM semantic_memory WHERE key='pref.gone'"
        ).fetchone()[0]
        == 0
    )
