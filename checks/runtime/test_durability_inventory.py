...
class TestGapClosure:
    ...
    def test_auth_tree_is_claimed_as_secret_snapshot_state(self, tmp_path):
        """Doctor claims every present and future auth file without making it portable."""
        home = tmp_path / "home"
        target = home / "auth" / "future_auth_state.json"
        target.parent.mkdir(parents=True)
        target.write_text("{}")

        result = inv.audit_home(home)
        claim = inv.claim_for("auth/future_auth_state.json")
        assert result.ok, f"unclaimed={result.unclaimed} dbs={result.undeclared_dbs}"
        assert claim is not None and claim.path == "auth"
        assert claim.kind == inv.KIND_TREE
        assert claim.secret is True
        assert claim.id in {e.id for e in inv.backup_entries()}
        assert claim.id not in {e.id for e in inv.export_entries()}
