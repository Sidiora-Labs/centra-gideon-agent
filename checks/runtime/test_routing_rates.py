...
from gideon.engine.routing import rates as rates_mod
...
def test_overlay_wins_over_app_default(tmp_path, registered_pricing):
    ...
    save_overlay(
        {"acme:acme-large": {"in_per_mtok": 1.0, "out_per_mtok": 2.0}}, home=tmp_path
    )
...
def test_absent_is_none_not_a_free_model(tmp_path):
    ...
    assert rate is None
    assert (
        cost_for("acme", "totally-unpriced-model", input_tokens=10_000, home=tmp_path)
        is None
    )
...
def test_overlay_wins_over_local_zero(tmp_path):
    ...
    save_overlay(
        {"ollama:qwen3:8b": {"in_per_mtok": 0.5, "out_per_mtok": 0.5}}, home=tmp_path
    )
...
def test_builtin_table_is_the_last_tier_before_absent(tmp_path):
    ...
    assert (
        priced
    ), "model_pricing.json has no rows — the builtin tier can't be exercised"
...
def test_overlay_bare_model_key_is_provider_agnostic(tmp_path):
    ...
    save_overlay(
        {"acme-large": {"in_per_mtok": 4.0, "out_per_mtok": 5.0}}, home=tmp_path
    )
...
def test_colon_bearing_model_ref_round_trips(tmp_path):
    ...
    save_overlay(
        {"vendor:gpt-oss:20b": {"in_per_mtok": 2.0, "out_per_mtok": 3.0}}, home=tmp_path
    )
...
def test_editing_the_overlay_changes_the_answer_with_no_restart(tmp_path):
    ...
    save_overlay(
        {"acme:acme-large": {"in_per_mtok": 1.0, "out_per_mtok": 1.0}}, home=tmp_path
    )
    assert rate_for("acme", "acme-large", home=tmp_path) == ModelRate(1.0, 1.0)

    save_overlay(
        {"acme:acme-large": {"in_per_mtok": 9.0, "out_per_mtok": 9.0}}, home=tmp_path
    )
...
def test_corrupt_overlay_fails_open_to_the_app_default(
    tmp_path, registered_pricing, caplog
):
...
def test_cost_for_applies_the_effective_rate(tmp_path):
    save_overlay(
        {"acme:acme-large": {"in_per_mtok": 3.0, "out_per_mtok": 15.0}}, home=tmp_path
    )

    cost = cost_for(
        "acme", "acme-large", input_tokens=1_000, output_tokens=2_000, home=tmp_path
    )
...
def test_model_rate_source_is_not_part_of_equality():
    assert ModelRate(1.0, 2.0, source="overlay") == ModelRate(
        1.0, 2.0, source="builtin"
    )
