...
def test_transcript_snapshots_paging_and_legacy_recovery(room_home):
    ...
    with path.open("a") as stream:
        stream.write('{"role":"user","content":"Legacy","speaker":"user","ts":"2026-01-01"}\n')
...
@pytest.mark.parametrize("text,expected", [
    ("@everyone review", True), ("@EVERYONE!", True),
    ("email@everyone", False), ("-@everyone", False),
    ("@everyone-else", False), ("@équipe, review", True),
    ("@e\u0301quipe review", True), ("@équipe日", False),
    ("@équipe\u0308", False), ("@équipement", False),
])
...
async def test_durable_deduplication_and_queued_cancellation(room_home):
    store = RoomStore(room_home)
    room = store.create("Quiet", [{"id": "a", "agent": "default", "listen_policy": "none"}])
...
async def test_http_enable_transcript_turns_disconnect_and_validation(room_home):
    ...
    path.write_text(json.dumps({"rooms": {"enabled": False, "round_budget": 2}, "unrelated": {"keep": "value"}}))
    app = web.Application()
    app["state"] = state()
    store = RoomStore(room_home)
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert (await client.post("/api/rooms/enable", json={"round_budget": 100})).status == 400
        ...
        response = await client.post("/api/rooms", json={"name": "Notes", "members": [{"id": "a", "agent": "default", "listen_policy": "none"}]})
        room = (await response.json())["room"]
        base = "/api/rooms/" + room["id"]
        for invalid in ([], None, {"text": ""}, {"text": "x", "request_id": None}, {"text": "x" * 16001, "request_id": "x"}):
            assert (await client.post(base + "/turns", json=invalid)).status == 400
        response = await client.post(base + "/turns", json={"text": "Remember this", "request_id": "request-1"})
        ...
        retry = await client.post(base + "/turns", json={"text": "Remember this", "request_id": "request-1"})
        ...
        previous = await (await client.get(base + "/transcript", params={"before": page["before"], "limit": 2})).json()
        assert previous["messages"][-1]["content"] == "Note 1"
        assert not {row["id"] for row in page["messages"]} & {row["id"] for row in previous["messages"]}
