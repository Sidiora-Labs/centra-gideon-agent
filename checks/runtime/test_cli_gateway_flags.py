"""Tests for `gideon gateway` composable CLI flags.
...
def _ns(**kwargs) -> argparse.Namespace:
    ...
    Mirrors what the CLI parser produces for `gideon gateway`. Tests
...
class TestApprovalYoloSafetyRail:
    ...
    def test_yolo_refused_without_gideon_home(self, monkeypatch, capsys):
    ...
    def test_yolo_refused_when_gideon_home_empty_string(self, monkeypatch, capsys):
        monkeypatch.setenv("GIDEON_HOME", "")
        with pytest.raises(SystemExit) as exc:
            _resolve_gateway_args(_ns(approval="yolo"))
        assert exc.value.code == 2
        captured = capsys.readouterr()
