...
def test_design_inputs_block_skips_blank_refs():
    block = pw.design_inputs_block(
        [{"type": "url", "ref": ""}, {"type": "image", "ref": "x.png"}]
    )
    joined = "\n".join(block)
    assert "x.png" in joined and joined.count("- [") == 1
...
def test_build_plan_to_phases_projection():
    ...
    assert (
        phases[1]["step"] == "document_export"
        and phases[1]["title"] == "Document & export"
    )
...
def test_token_step_contract_emits_token_overrides():
    for k in ("foundations", "palette", "typography"):
        ...
        assert (
            "token_overrides" in c and "partial design-token document" in c.lower()
        ), k
