...
@pytest.mark.asyncio
async def test_store_catalog_build_has_a_deadline(monkeypatch):
    release = asyncio.Event()
    monkeypatch.setattr(model_registry, "_CATALOG_BUILD_TIMEOUT_SECS", 0.01)

    async def _stalled():
        await release.wait()

    with pytest.raises(asyncio.TimeoutError):
        await model_registry._bounded_catalog_build(_stalled)
