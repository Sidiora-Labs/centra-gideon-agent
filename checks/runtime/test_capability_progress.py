...
def write_project(root, statuses):
    ...
    for index, status in enumerate(statuses):
        ...
        if status is not None:
            (directory / "tasks" / f"{identifier}.json").write_text(
                json.dumps({"id": identifier, "status": status})
            )
    (directory / "backlog.json").write_text(
        json.dumps(
            {"version": 1, "product": "oss", "status": "in_progress", "tasks": tasks}
        )
    )
    return directory
...
def test_exact_qualified_states_exclude_partial_external_and_missing_records(tmp_path):
    statuses = [
        "locally_qualified",
        "implemented_locally_qualified",
        "locally_qualified_scoped_paths",
        "implemented",
        "implemented_inference_unqualified",
        "locally_qualified_external_pending",
        "locally_checked_browser_pending",
        "runtime_locally_qualified",
        "blocked",
        "dependency_blocked",
        None,
    ]
...
def test_real_cli_is_read_only_unless_write_requested(tmp_path):
    ...
    assert (
        json.loads((directory / "backlog.json").read_text())["tasks"][0]["status"]
        == "done"
    )
