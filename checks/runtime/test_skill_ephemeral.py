...
def test_drafts_are_session_scoped(home):
    ...
    assert len(ephemeral.list_drafts("sess:2")) == 0
...
def test_redacts_secrets_in_body(home):
    d = ephemeral.remember(
        "sess:1", "creds", "set aws_secret_access_key=AKIAIOSFODNN7EXAMPLE now"
    )
...
def test_promote_to_global(home):
    ...
    assert (
        ProcedureLibrary(install_builtins=False).load_skill("global-skill") is not None
    )
...
def test_promote_to_agent_tier(home):
    ...
    assert (
        ProcedureLibrary(install_builtins=False, agent="researcher").load_skill(
            "agent-skill"
        )
        is not None
    )
    assert ProcedureLibrary(install_builtins=False).load_skill("agent-skill") is None
...
def test_promote_with_edits(home):
    ...
    content = ProcedureLibrary(install_builtins=False).load_skill("edited-name")
...
def test_promoted_skill_marked_taught(home):
    ...
    content = ProcedureLibrary(install_builtins=False).load_skill("marked")
