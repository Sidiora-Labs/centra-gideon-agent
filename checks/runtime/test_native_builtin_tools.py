...
async def test_write_file_rejects_directory_and_parent_file_cleanly(ws):
    p = NativeBuiltinToolProvider(ws)
    d = await p.invoke("write_file", {"path": "sub", "content": "x"})
...
async def test_edit_rejects_ambiguous_match(ws):
    ...
    p = NativeBuiltinToolProvider(ws)
    await p.invoke("read_file", {"path": "dup.txt"})
...
async def test_edit_replace_all(ws):
    ...
    r = await p.invoke(
        "edit_file",
        {"path": "dup.txt", "old_str": "1", "new_str": "9", "replace_all": True},
    )
...
async def test_edit_unique_match_still_works(ws):
    p = NativeBuiltinToolProvider(ws)
    await p.invoke("read_file", {"path": "a.txt"})
    r = await p.invoke(
...
async def test_edit_rejects_empty_old_str(ws):
    ...
    r = await p.invoke(
        "edit_file",
        {"path": "a.txt", "old_str": "", "new_str": "X", "replace_all": True},
    )
...
async def test_edit_rejects_noop_identical(ws):
    ...
    r = await p.invoke(
        "edit_file",
        {"path": "a.txt", "old_str": "hello world", "new_str": "hello world"},
    )
...
async def test_bash_timeout_arg_is_capped(ws):
    ...
    assert seen["timeout"] == BT._BASH_TIMEOUT_MAX
...
async def test_extra_roots_allow_engine_dir_outside_cwd(ws):
    ...
    w = await p.invoke(
        "write_file",
        {"path": str(engine / "findings" / "cycle_1.json"), "content": "{}"},
    )
...
def test_kn_title_falls_back_through_ai_url_title():
    from gideon.engine.agents.native.builtin_tools import _kn_title
...
def test_kn_snippet_prefers_summary_then_link_then_body():
    from gideon.engine.agents.native.builtin_tools import _kn_snippet
...
def test_kn_redact_scrubs_credentials():
    ...
    from gideon.engine.agents.native.builtin_tools import _kn_redact
...
async def test_knowledge_get_redacts_content(ws, monkeypatch, tmp_path):
    ...
    import gideon.cognition.knowledge as kn
...
async def test_knowledge_get_flags_archived_item(ws, monkeypatch, tmp_path):
    ...
    store = KnowledgeStore(str(tmp_path / "k.db"))
    iid = store.create_typed_item(
        item_type="note", title="Old Plan", content="superseded approach"
    )
...
async def test_knowledge_get_flags_unreachable_bookmark(ws, monkeypatch, tmp_path):
    ...
    assert "nope.invalid" in r.output
...
async def test_knowledge_search_uses_embedder_for_hybrid(ws, monkeypatch, tmp_path):
    ...
    import gideon.cognition.knowledge as kn
    import gideon.cognition.knowledge.retrieval as retr
    from gideon.cognition.knowledge.store import KnowledgeStore

    store = KnowledgeStore(str(tmp_path / "k.db"))
    store.create_typed_item(
        item_type="note", title="Vector DB notes", content="pgvector and HNSW"
    )
    store.db.commit()
    monkeypatch.setattr(kn, "get_knowledge_store", lambda: store, raising=False)
...
def test_get_knowledge_embedder_none_safe(monkeypatch):
    ...
    monkeypatch.setattr(kn, "_embedder_spec", object(), raising=False)
    monkeypatch.setattr(
        "gideon.integrations.embedding_providers.registry._active_embedding_spec",
        lambda: ("native", "all-MiniLM-L6-v2"),
    )
    monkeypatch.setattr(
        "gideon.cognition.knowledge.embedder.create_embedder_from_config",
...
def test_get_knowledge_embedder_rebuilds_on_model_switch(monkeypatch):
    ...
    monkeypatch.setattr(
        "gideon.cognition.knowledge.embedder.create_embedder_from_config", _build
    )

    e1 = kn.get_knowledge_embedder()
    e2 = kn.get_knowledge_embedder()
    assert e1 is e2 and len(builds) == 1
    spec["v"] = ("native", "model-b")
    e3 = kn.get_knowledge_embedder()
