...
BRIDGE = (
    ...
    / "runtime/gideon/extensions/providers/provider_bridge.py"
)
...
def home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("gideon.core.config.config_dir", lambda: tmp_path)
    return tmp_path


def _enable(
    monkeypatch: pytest.MonkeyPatch, mode: str = "heuristic", pin: str = ""
) -> None:
...
def _locals_are(monkeypatch: pytest.MonkeyPatch, *keys: str) -> None:
    monkeypatch.setattr(
        policy, "_local_provider_keys", lambda: {policy._norm(k) for k in keys}
    )
...
def test_route_refs_is_a_pure_reorder(
    ...
    assert refs == [
        ...
        CLOUD_B,
        LOCAL_TINY,
    ], "route_refs mutated its input list"


def test_route_refs_preserves_duplicates(
...
def test_master_switch_off_beats_a_use_case_mode(
    ...
    monkeypatch.setattr(
        policy, "_settings_for", lambda _uc: {policy.MODE_KEY: "heuristic"}
    )
    assert policy.routing_active("reasoning") is False


def test_heuristic_orders_local_first(
    home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _enable(monkeypatch)
    _locals_are(monkeypatch, "ollama-models")
    assert policy.route_refs("reasoning", "summarize", [CLOUD_A, LOCAL]) == [
        LOCAL,
        CLOUD_A,
    ]
...
def test_no_size_hint_is_not_treated_as_small(
    home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert (
        policy.route_refs("reasoning", "long_reasoning", [CLOUD_A, unsized])[0]
        == unsized
    )
...
def test_structured_class_prefers_a_declaring_provider(
    ...
    _locals_are(monkeypatch, "ollama-models")
    monkeypatch.setattr(
        policy, "_structured_providers", lambda: {policy._norm("CloudB")}
    )
    out = policy.route_refs(
        "reasoning", "extract_structured", [CLOUD_A, LOCAL, CLOUD_B]
    )
    assert out[0] == CLOUD_B


def test_pin_short_circuits_the_heuristic(
    home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A user pin is mightier than the policy: pin=cloud beats local-first outright."""
    _enable(monkeypatch, pin="cloud")
    _locals_are(monkeypatch, "ollama-models")
    assert policy.route_refs("reasoning", "summarize", [LOCAL, CLOUD_A]) == [
        CLOUD_A,
        LOCAL,
    ]


def test_explicit_ref_pin_hoists_that_ref(
    home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _enable(monkeypatch, pin=CLOUD_B)
    _locals_are(monkeypatch, "ollama-models")
    assert (
        policy.route_refs("reasoning", "code", [LOCAL, CLOUD_A, CLOUD_B])[0] == CLOUD_B
    )
...
def test_recorded_table_order_wins_and_keeps_unlisted_refs(
    ...
    out = policy.route_refs(
        "reasoning", "summarize", [LOCAL, CLOUD_B, CLOUD_A], home=home
    )
...
def test_unknown_mode_reads_as_off(home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(policy, "master_enabled", lambda: True)
    monkeypatch.setattr(
        policy, "_settings_for", lambda _uc: {policy.MODE_KEY: "aggressive"}
    )
...
def test_unknown_provider_is_treated_as_cloud(
    home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_route_refs_is_called_inside_resolve_provider_for_use_case() -> None:
    ...
    assert (
        "route_refs" in called
    ), "resolve_provider_for_use_case does not CALL route_refs"
    assert (
        "routing_active" in called
    ), "the routing seam does not check per-use-case enablement"
...
def test_routed_provenance_fields_exist_and_are_distinct_from_degraded() -> None:
    ...
    """
    from gideon.security.guardrails.audit import AttemptRecord

    rec = AttemptRecord(
        audit_id="a", ts=0.0, use_case="reasoning", provider="p", model="m", attempt=1
    )
    assert (
        rec.routed is False and rec.routed_fallback is False and rec.degraded is False
    )
    row = json.loads(
        AttemptRecord(
            audit_id="a",
...
def test_routing_config_survives_a_malformed_section(
    ...
    (home / "config.json").write_text(
        json.dumps(
            {"routing": {"local_timeout_secs": "soon", "min_samples": 0, "weights": 3}}
        ),
        encoding="utf-8",
    )
    cfg = AppConfig.load()
    assert cfg.routing.local_timeout_secs == 20.0
    assert cfg.routing.min_samples == 1
...
def test_seam_walks_the_routed_order_and_stamps_routed(
    ...
    assert [c["model"] for c in calls] == [
        "qwen3:8b"
    ], "local-first order was not walked"
    assert calls[0]["routed"] is True
    assert (
        calls[0]["routed_fallback"] is False
    ), "the FIRST routed candidate is not a fallback"
...
def test_a_cloud_rescue_of_the_routed_local_bet_stamps_routed_fallback(
    ...
    monkeypatch.setattr(
        "gideon.security.guardrails.breaker.get_breaker",
...
def test_model_override_bypasses_routing_entirely(
    ...
    assert len(calls) == 1
    assert (
        calls[0]["routed"] is False
    ), "an explicit override was stamped as a routing decision"
...
def test_routing_policy_travels_in_a_snapshot() -> None:
    ...
    from gideon.workspace.snapshot import CORE_FILES

    assert "routing_policy.json" in CORE_FILES["config"]
    assert "routing_stats.json" not in {
        f for files in CORE_FILES.values() for f in files
    }
