...
def test_bootstrap_cli_providers_launches_no_backends_or_watchdogs(
    tmp_path, monkeypatch, _only_the_probe_app
):
    """The half of ES-3 the delegation test cannot see: a standalone process must get the
    registration WITHOUT the gateway's long-lived tail.

    The real :func:`bootstrap_cli_providers` runs here (only the native-app seeding and the
    bundled discovery are neutralised, by the shared fixture), so the app module really is
    imported — and every gateway launcher plus ``subprocess.Popen`` is watched. A future
    edit that reaches for ``load_all_extensions`` from the CLI path, or that folds a backend
    launch into the shared registration, fails here rather than in a user's ``gideon
    retrieval-eval`` run that suddenly supervises app backends it will abandon on exit.
    """
    launched: list[str] = []
    for module, name in _GATEWAY_LAUNCHERS:
        monkeypatch.setattr(
            f"{module}.{name}", lambda *a, _n=name, **k: launched.append(_n)
        )
    ...
    class _RecordingPopen(real_popen):  # type: ignore[misc,valid-type]
        def __init__(self, *args, **kwargs):
            spawned.append(args)
            raise AssertionError(f"the CLI bootstrap spawned a process: {args!r}")
    ...
    assert receipt.read_text(encoding="utf-8") == "imported", (
        "the bootstrap must still do its real work — otherwise 'launched nothing' is "
        "vacuously true"
    )
    assert launched == [], f"the CLI bootstrap launched the gateway tail: {launched}"
    assert spawned == []


def test_a_standalone_eval_command_runs_the_bootstrap(monkeypatch, tmp_path):
    """The eval family is the caller ES-3 was filed for: dispatching ``gideon
    retrieval-eval`` must run the provider registration before the command body."""
    from gideon.interfaces.cli import main as cli_main

    order: list[str] = []
    monkeypatch.setattr(
        loader, "bootstrap_cli_providers", lambda: order.append("bootstrap")
    )
    monkeypatch.setattr(
        cli_main, "_retrieval_eval", lambda args: order.append("retrieval-eval")
    )
    monkeypatch.setattr("sys.argv", ["gideon", "retrieval-eval"])

    cli_main.main()

    assert order == ["bootstrap", "retrieval-eval"]


def test_every_provider_dependent_standalone_command_is_gated_onto_the_bootstrap():
    """The gate is a SET, so a command added to the eval family without being added to it
    is the silent regression: it runs in a process whose provider registry holds only
    core's built-ins."""
    from gideon.interfaces.cli.main import _PROVIDER_BOOTSTRAP_COMMANDS
