...
def test_update_state_records_recovery_point_and_lifecycle(
    monkeypatch, tmp_path
) -> None:
    monkeypatch.setenv("GIDEON_HOME", str(tmp_path))

    started = uk.begin_update("pip", "v1.4.0", "v1.5.0")
    assert started["phase"] == "applying"
    assert uk.rollback_snapshot("pip") == {"version": "1.4.0", "ref": ""}
    assert uk.rollback_snapshot("git") == {}

    applied = uk.complete_update()
    assert applied["phase"] == "applied"
    view = uk.update_state_view("pip")
    assert view["update_state"] == "applied"
    assert view["update_target"] == "1.5.0"
    assert view["rollback_available"] is True
    assert view["rollback_version"] == "1.4.0"

    rolling_back = uk.begin_rollback()
    assert rolling_back["phase"] == "rolling_back"
    assert uk.update_state_view("pip")["rollback_available"] is False
    uk.complete_rollback()
    assert uk.update_state_view("pip")["update_state"] == "rolled_back"


def test_git_update_state_requires_a_saved_commit_for_rollback(
    monkeypatch, tmp_path
) -> None:
    monkeypatch.setenv("GIDEON_HOME", str(tmp_path))

    uk.begin_update("git", "0.9.0", "1.0.0")
    assert uk.rollback_snapshot("git") == {}

    uk.begin_update("git", "0.9.0", "1.0.0", rollback_ref="abc123")
    assert uk.rollback_snapshot("git") == {"version": "0.9.0", "ref": "abc123"}
    failed = uk.fail_update("frontend build failed")
    assert failed["phase"] == "failed"
    assert uk.update_state_view("git")["update_error"] == "frontend build failed"
