...
from gideon.workspace.snapshot import _extra_restore_paths_for_test_paths, snapshot_main
...
def test_new_media_database_and_listener_settings_survive_snapshot(
    ...
    with sqlite3.connect(media) as connection:
        ...
        connection.execute(
            "INSERT INTO retained VALUES (?, ?)", ("approved", "sha-bound-atlas")
        )
    assert (
        inventory.claim_for(str(media.relative_to(home))).kind == inventory.KIND_SQLITE
    )
    ...
    with tarfile.open(archives[0]) as archive:
        ...
        video_member = next(
            name
            for name in names
            if name.endswith("/capabilities/knowledge/videos/source/captions.txt")
        )
        assert archive.extractfile(video_member).read() == video.read_bytes()
        database_member = next(
            name
            for name in names
            if name.endswith("/capabilities/media/sprites.sqlite3")
        )
        config_member = next(
            name
            for name in names
            if name.endswith("/capabilities/platform/inference_host.json")
        )
        ...
        assert not any(
            name.endswith(("sprites.sqlite3-wal", "sprites.sqlite3-shm"))
            for name in names
        )
    with sqlite3.connect(restored) as connection:
        assert connection.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
        assert connection.execute("SELECT id,value FROM retained").fetchall() == [
            ("approved", "sha-bound-atlas")
        ]
...
def test_new_platform_state_preserves_committed_wal_in_backup(
    ...
    with sqlite3.connect(database) as writer:
        ...
        with tarfile.open(archives[0]) as archive:
            ...
            assert not any(
                name.endswith((relative + "-wal", relative + "-shm")) for name in names
            )
        with sqlite3.connect(restored) as reader:
            assert reader.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
            assert reader.execute("SELECT id,revision FROM receipts").fetchall() == [
                ("retained-operation", 9)
            ]
