...
This is repo-inner dev infrastructure (it lives beside ``src/``/``checks/runtime/``/``tooling/scripts/``
...
- ``harness.replay`` + ``harness.baselines`` — event-trace replay regression (§2). The
  recorder half lives in core (``gideon.assurance.trace_recorder``); core can't import here.
- ``harness.cli`` — ``python -m checks.harness  validate | explain | run | scan | replay``.
...
``checks/harness/README.md`` for layout and ``AGENT.md`` at the repo root for the machine-facing
