...
statement: >
  ...
  update a rule or scenario spec under `checks/harness/specs/` in the SAME change, so the fixed
  bug becomes a permanent machine-checked invariant rather than a one-time patch.
appliesTo:
  - checks/harness/specs/**
source: >
...
- A new architectural invariant → a **rule** spec (`checks/harness/specs/rules/`), ideally with a
...
- A recurring *diagnosis* (a symptom you've now chased more than once) → a **scenario**
  spec (`checks/harness/specs/scenarios/`) with the probe order and mitigation.
...
`python -m checks.harness run --diff` warns when the diff's commit subject looks like a fix
(`fix`/`bug`/`regression`/`hotfix`) but touches nothing under `checks/harness/specs/`. The warning
