...
1. **Reproduce by replay, not by clicking.** Record the failing stream with
   ...
   replay the captured NDJSON through the pure fold: `python -m checks.harness replay` (backend
...
Traces are redacted at write via `security.redact` before landing in `checks/harness/traces/`.
