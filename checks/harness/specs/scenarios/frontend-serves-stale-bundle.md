...
acceptance:
  ...
  - "`runtime/gideon/static/dist` is a SYMLINK to `apps/console/dist`, not a copied directory."
---
...
1. **Is `static/dist` a symlink?** `runtime/gideon/static/dist` must be a *symlink* to
   `apps/console/dist`. A `cp -R` (or a stray real directory) shadows the symlink and serves a
