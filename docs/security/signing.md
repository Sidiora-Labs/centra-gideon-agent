...
python3 scripts/sign_app.py gen-key --signer Gideon --out-dir ~/gideon-keys

# 2. Ship the PUBLIC half.
cp ~/gideon-keys/Gideon.pub src/gideon/trusted_keys/
...
python3 scripts/sign_app.py sign apps/my-app --seed ~/gideon-keys/Gideon.seed
...
```sh
minisign -Vm apps/my-app/.gideon-signature.sha256 -p src/gideon/trusted_keys/Gideon.pub
```
