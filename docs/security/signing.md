...
> by Gideon", and `tooling/scripts/setup.sh` — the file that actually executes as
...
`checks/runtime/security/test_app_signature.py::test_a_tampered_bundle_never_runs_its_install_hook`
...
`runtime/gideon/trusted_keys/<Signer>.pub`, shipped in the wheel. The **filename stem
...
secret holds a base64 blob anyway. `tooling/scripts/sign_app.py` reads a `<Signer>.seed` file: a
