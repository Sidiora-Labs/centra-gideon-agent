...
- **Zero-width splitting degrades `DANGEROUS` to `WARNING`.** A zero-width space
  ...
  checks that `.gideon-lock.json` records `verdict: warning`, so the override leaves a
...
- **Binary entries are not scanned.** A payload entry carrying `data` (bytes) has no
  scannable text; its provenance is the sha256 in `.gideon-lock.json` and nothing more.
