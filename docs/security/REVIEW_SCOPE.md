...
The [threat model](THREAT_MODEL.md) claims that Gideon's controls are enforced at
chokepoints rather than requested in a prompt, and cites a module for every
"enforced" row. A claim that cites its own code is auditable. It is not yet
...
single mistake converts an *outside* input into *owner* authority. They are the
ones where the blast radius is the whole machine, because the agent that runs
there can run commands on it.

**Scope owner:** the maintainer. The review format and the publication plan below
are binding on the review. The path list is the reviewer's starting point, not a
...
- **An already-compromised host.** Root on the box, a compromised OS account, or
  ...
  package before the process starts. See the threat model's [baseline denylist
  integrity](THREAT_MODEL.md#baseline-denylist-integrity-anti-drift-and-anti-llm-tamper-not-anti-owner)
...
Two of the five paths are also *deliberately* incomplete today. The review should
treat them as design review rather than exploitation targets where the control
has not landed. Each says so in its own section.

## The five high-risk paths

Every module path below is relative to `runtime/gideon/`. Entry points are named
as `module.py::symbol` so they resolve by grep.
...
handler that calls it), plus the trigger surfaces it hands off to (`triggers/`,
`trigger_sources/`).
...
`/api` path outside the app's declared permissions. Both are explicitly in scope
in `SECURITY.md`.
...
moved into place only on a passing verdict (`apps/app_manager.py::install`;
`supply_chain.py` with `SkillScanner`, `Verdict`, `TrustTier`). The `dangerous`
verdict is terminal and non-overridable. The design claim is that there is no
time-of-check / time-of-use gap: the scanned tree *is* the installed tree.
...
**Entry points.** `apps/app_manager.py::install`, `supply_chain.py`, `skills/`,
...
`net/policy.py::egress_policy_for` (composed from `egress_policy_for_tier` and
...
The second is worse, because the audit log will show a decision that was never
enforced.
...
`requests` / `urllib` / `socket` use outside `net/`, in `providers/`,
`search_providers/`, `channel_transports/`, `sync_transports/`, `browse/`,
`local_models/`, `uploads/`.
...
**What it is.** The read-only inbound tool surface: `inbound/mcp_http.py` mounts
`/mcp`, gated by `inbound/auth.py` (`peer_allowed`, `verify_bearer`) and
...
ASI07 row is still `in progress`: broad external access is owned by plans not yet
landed. Reviewing it now is cheaper than reviewing it after it calcifies. The
concrete bad outcome: a non-owner caller drives tools on the owner's machine, or
read-only turns out to be read-mostly.
...
`inbound/auth.py`, `inbound/caps.py`, `inbound/tools.py`, `inbound/audit.py`, and
the mount site in `dashboard/server.py`.
...
- Defeat the caps: exhaust slots to deny the owner service, evade `check_rate`
  with a client key you control, pass a payload that `clamp_items` / `clamp_text`
  mis-measures.
...
Both formats below are credible, and the maintainer picks one (see [Approval and
schedule](#approval-and-schedule)). The finding format, the evidence bar, and the
dispute rule are **identical for both**. That is the point of writing them down
...
- **High**: a boundary the threat model claims `enforced` is crossed, without
  reaching execution. Examples: an app token widening, a policy layer bypassed, a
...
- **Low**: a documented claim is wrong in a way that misleads a user, with no
  boundary crossed. This includes a `THREAT_MODEL.md` citation that no longer
  resolves.
...
- **"Is this in scope?"** resolves against `SECURITY.md`'s scope section, which
  is the contract. Not against this document, and not against the maintainer's
...
- **"Is this control actually enforced?"** resolves by executing code. The
  project treats code as the as-built authority, so a test is the tiebreaker:
  whoever writes the test that runs settles it. A reviewer's claim with a failing
  test beats a maintainer's reading of the source; a maintainer's passing test
  beats a reviewer's reading.
...
- **An unpatched Critical or High finding is withheld entirely** until a fix is
  ...
  no "a fix is coming for an issue we won't describe". That tells an attacker
  where to look without telling a user what to do.
- **Exploit code for a Critical finding is withheld permanently** if the impact
  survives the fix for un-upgraded users. For a self-hosted tool with no
  auto-update guarantee, it does. The *reproduction* is published so a reader can
  verify the fix; a weaponised form is not.
- **Anything that identifies a real deployment** is redacted, always: hostnames,
  paths containing a username, tokens, the contents of a real home directory.
...
4. On that release, the report publishes in full, and
   [`THREAT_MODEL.md`](THREAT_MODEL.md) is updated in the same change. An
...
- **The report**: `docs/security/reviews/<YYYY-MM-DD>-<format>.md`, for example
  `docs/security/reviews/2026-09-01-self-audit.md`. Reports are immutable once
  published; a correction is a new dated note appended to the same file, never an
  edit to a finding.
...
- **The cross-links**: `SECURITY.md` gains a link to the latest report.
  `THREAT_MODEL.md`'s affected rows gain the finding ID that tested them, so a
  reader of a control row can see whether anyone has actually attacked it.
- **Accepted limitations** move into [`LIMITATIONS.md`](LIMITATIONS.md), in that
  document's existing voice, with the finding ID.
...
(`SH-9`, in SECURITY-HARDENING) lists scope approval as an owner task for exactly
this reason.

The honest framing for a reader in the meantime: **Gideon has not been externally
reviewed.** Its threat model cites its code, its limitations are written down, and
this document says how a review would run. None of that is the same as having had
one.
