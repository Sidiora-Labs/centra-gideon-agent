...
[`apps/console/src/shared/theme/motion.ts`](../../apps/console/src/shared/theme/motion.ts). This page is what you need to
pick a transition and know what it is allowed to do. If you finish it and still don't know
which preset to use, that is a bug in this page, so fix the page.
...
| `physics.playful` | the ~3 or 4 *earned* personality moments: a menu opening, a success bloom | the most overshoot | 420 / 14 → 34 |
...
constants in `motion.ts`**, which is what they are for, rather than adding a fifth name or a
...
4. **Two helpers answer with an absence rather than a value**, because for them "less motion"
   ...
   route crossfade is still a crossfade, and a quicker cascade is still a cascade.

**What must not collapse: direct manipulation.** Reduced motion removes decoration, never
capability. A drag still follows the finger 1:1, and `dragElastic()` is deliberately *not*
gated, because with zero elasticity against a zero-width constraint box a swipe-to-dismiss
card cannot move at all and the gesture stops being performable. Only the transition the
gesture *resolves* with is decoration, and that is what goes instant.
...
`stiffness` would let Framer infer a spring again, so the collapse would leak back through the
...
- **`dragSpring()`**: the transition a dragged element settles with, either snapping back
  inside its constraints or landing after a reorder. Stiffer than `snappy`, because a return
  that loses the race with the hand that threw it reads as lag.
...
- **`swipeDismiss(velocity, offset)`**: the verdict *and* the curve. It dismisses on a fast
  ...
  accelerating curve and must not overshoot back into the surface it is leaving; a kept one
  springs home on `dragSpring()`.

Never hardcode a gesture threshold at a call site. Both thresholds are user-tunable tokens in
the Motion group, and a literal `> 80` in a component is a tuning knob nobody can reach.
...
`overlayEnter` and `listItemEnter` are already written this way, so copy them.

**Choreograph, don't scatter.** One considered movement beats ten small ones, and scattered
...
surface can pick its own feel. The step is `expr()`-scaled, about 44ms per region at the
default expressiveness and 20ms when refined. Under `prefers-reduced-motion` it returns
`null`, and both components then render plain `<div>`s with no variants, no hidden initial
state and no transition. There is no "fast cascade" tier, because the setting asks for *less*
motion, not quicker.
...
- **Where you put the group is the whole game.** Put it *above* every data-dependent branch
  ...
  `useCachedData` is what makes the first placement safe, because a same-key revalidation holds
  the last value instead of dropping to `undefined`, so a refresh cannot flip the surface back
...
group costs the entrance, never the content, and the failure a region cannot be allowed to have
...
1. **Travel picks the base; the knob picks the bonus.** Bases are ordered by how far the thing
   travels: a cross-page flight is the softest and a bud off a button the tautest, because a
...
three from the source, and `ui/motion/family.reducedMotion.test.tsx` enforces the last.
...
- `prefers-reduced-motion` means **instant**, and it is self-gated in JS by every member. The
  ...
  *transforms*, so neither one stops a `borderRadius` animation, a projection node, or a
...
- `expressiveness = 0` means **the floor, still moving**. `expr()` keeps `floor` of its input, so
  ...
  default threshold, and passing your own would give the family two definitions of "bold".
...
forgetting the group costs the entrance and nothing else; here, hosting it in the wrong place
costs the *only* thing the primitive does.

Three things the call site owes:

- **Text carries the meaning.** The primitive is `aria-hidden` and `pointer-events-none` by
  contract, like `DotGlow`, so a screen reader and a keyboard never learn it exists. The tile keeps
...
- **`intensity` is a plain number.** The primitive scales the amplitude through `expr()` itself, so
  ...
  13% low at the default expressiveness and 65% low at 0, which is the shape of bug that ships:
...
- **`flushSync` is required, not defensive.** The browser captures the "after" frame as
  ...
  scheduler, so without the flush both snapshots are the *old* frame and the crossfade
  animates nothing.
...
shim with no frame clock, so it will report whatever rate you ask it for. The measurement is
therefore a real browser driven by `tooling/scripts/motion_frame_budget.mjs`, next to
`render_smoke.mjs` for the same reason that one is a standalone driver: it loads the built
artifact and needs a live gateway. Deliberately **not** a spec in `apps/console/e2e/`, which is
the zero-diff gate tier, because a frame-time distribution has no committed baseline and no
machine-independent threshold, so a red/green there would be a claim about hardware. Nothing in
CI runs it (`ci.yml` runs exactly one file out of `apps/console/e2e/`, `a11y.spec.ts`, by
explicit path); it is a driver you point at a gateway and read.
...
surface is measured *during* provoked motion (a route change into it, which is `viewTransition`
plus the arriving `EntranceGroup` cascade; the command palette opening on `physics.playful`; a
...
instrument, because every frame is being asked to fit 8.33ms, twice as demanding as the 60fps
floor, and it means a 16.7ms delta, a full miss at 120Hz, still *meets* the 60fps floor.
`over16_7` is therefore the honest 60fps miss count either way. The two clocks do not rank the
same steps the same, which is the subject of the third table.
...
3. **The loop cockpit's own motion is the cheapest thing measured.** Its route entrance,
   disclosure and hover sweep all sit at the 8.33ms idle floor, so they cost nothing
...
4. **The command palette is the most expensive motion in the app, and it is shell chrome.**
   Its open sustains a 15 to 20ms mean (51 to 66fps) for ~1.3s, and its 22-row list scroll holds
   14 to 18ms; it costs the same on both surfaces because it belongs to neither. Every frame that
   misses 60fps in the ChatPage column is inside those two steps: strip them and ChatPage's own
   motion is 8.33 to 9.81ms. **This is a finding, not a fix**: `app/CommandPalette.tsx` is the
...
8.33ms**, because the resting page does no main-thread work worth throttling, so everything
below is interaction cost:
...
The cockpit degrades gracefully: its own steps stay at 8.4 to 8.75ms and only the palette gets
dear (16.4ms mean, against 41.7ms for the same overlay on ChatPage). ChatPage does not: its route
entrance goes to 26ms mean / 117ms worst, and **composer typing** goes to 21.5ms mean with a
106.7ms worst, where unthrottled it was the cheapest step on the page (8.45ms). A per-keystroke
cost that is invisible at 1× and becomes a 100ms frame at 4× is the shape of bug that ships, and
it is the one to look at first if this page ever feels heavy on a real laptop.
...
- Vsync **exposes** long tasks, and the headless clock spreads them. Every step that was fine
  ...
  disclosure, 93.3ms in the palette open, 81.9ms in the route entrance, which is 6 to 12 dropped
  frames each, exactly the "60fps with one 400ms stall" that §8's whole shape exists to catch.
...
- **The cockpit is not live.** The seeded loop is `status=complete`, so the run stream and its
  live region never animate, and what was measured is the cockpit's arrival, its disclosure and
  its hover states. A running loop is the missing case.
