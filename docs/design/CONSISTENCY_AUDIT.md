...
> This document is the *map*, not the fix. It quantifies where the shipped design system (with `apps/console/DESIGN.md` and `apps/console/PRODUCT.md` as authority) drifts across `apps/console/src`, ranked worst-first, so S2/S3 attack the highest-value targets first. **No code was changed to produce it.**

---

## Headline finding

The token layer is **already mature and near-clean**: the shipped `tokenLint` ratchet has driven hardcoded color, spacing, radius, shadow and duration values down to a **handful of legitimate exceptions**. The dominant, measurable drift is **primitive adoption**: pages built over time render **bespoke chrome** (raw `<button>`, raw `<input>/<textarea>/<select>`) instead of the 33 shell primitives in `apps/console/src/ui/`. This is exactly the drift the plan predicted, and it is concentrated in the two mega-pages.
...
**Interpretation:** the S2 "hardcoded to token" work is nearly *done already* (7 hits, most of them legitimate, see below). The real S2/S3 payload is **primitive consolidation**: replacing bespoke `<button>` and form markup with `Button`/`IconButton` plus a shared form-field primitive, which is a genuine system gap. The absence of any bespoke `<dialog>` or `role="dialog"` outside `ui/` confirms `Modal` is already the single canonical dialog, which is a consistency win to preserve rather than fix.

---

## Ranked worst offenders (weighted score)

Weights: color drift ×5 (it bypasses theming), shadow ×3, other raw-value ×2; raw-dialog ×4, raw-button ×2, raw-input ×1 (bespoke chrome).
...
Every hit was spot-checked against the code. Most are **legitimate exceptions**, which is why the token layer counts as near-clean:
...
| shadow | `pages/notifications/NotificationsPage.tsx:168` | `boxShadow: inset 2px 0 0 0 ${km.tone}` (unread rail) | **Legitimate-ish**: hairline inset accent using a token-derived `km.tone`, and the `2px` is a hairline width. Consider a shared `--shadow-inset-accent` if the pattern repeats (it does, see NotificationBell) |
| shadow | `ui/NotificationBell.tsx:127` | `boxShadow: inset 2px 0 0 0 ${km.tone}` (unread rail) | **Duplicate of the row above**: the same inset-accent pattern in a primitive-adjacent file, so a candidate to consolidate into one helper |
...
| spacing | `ui/widget/WidgetFrame.tsx:204` | `height: 'calc(100% - 36px)'` (expanded widget) | **Candidate**: the `36px` is a header-height magic number and should reference a layout token. This is the sole file currently on the `tokenLint.allowlist.json` |

**S2 raw-value target list (only 3 real fixes):**

1. `WidgetFrame.tsx` `calc(100% - 36px)` → layout token (clears the last allowlist entry). **DEFERRED**: the `36px` is the measured height of a sibling toolbar (layout coupling, not a spacing-scale value), so swapping it for a scale token would be semantically wrong and shift layout. It needs the visual harness, which is blocked on auth, to verify any change. Not a blind fix.
2. The two `transform 200ms` durations → `--motion-*` token (verify reduced-motion honored). **PENDING**: global reduced-motion already neutralizes them, so this is low priority.
3. ~~The duplicated `inset 2px 0 0 0` unread-rail shadow → one shared helper.~~ **DONE (cycle 4)**: `unreadRail()` + `toneChipBg()` in `notificationMeta.ts` now own the unread-rail and icon-chip-tint patterns, and `NotificationsPage.tsx` (3 sites) and `NotificationBell.tsx` (2 sites) call them. Byte-identical output (zero visual change), guarded by `notificationMeta.test.ts`. Shadow drift went 2 → 1 (now the single canonical helper).

The MemoryGraph HSL and the CodeMirror `0.9em` are legitimate, so they should be added to `tokenLint`'s exempt list **with a `reason`** (C1 requires reasons) rather than "fixed".

---

## Primitive-adoption drift (the real S2/S3 payload)

**420 raw `<button>` + 206 raw form elements across 112 files.** The system already has `Button`, `IconButton`, `Toggle`, `Segmented` and `Combobox`, but there is **no shared text-input / textarea / select field primitive**, which is why every form re-rolls raw `<input>`. That is the one **genuine system gap** the plan allows filling *once* (T2.3): a `Field`/`TextInput` primitive plus a `PATTERNS.md` entry.

**Consolidation strategy (S2):**

- **Buttons** → replace raw `<button>` with `Button` (variant/size props) or `IconButton`. Start at the mega-pages (rank 1 and 2) in screenshot-verified increments.
- **Inputs** → introduce one `TextField`/`Field` primitive (T2.3, genuine gap), then migrate. Do not migrate raw inputs before the primitive exists. **DONE (cycle 5):** `apps/console/src/ui/TextField.tsx` (`TextField` + `TextArea`, size/surface/mono/leadingIcon variants) is the canonical extraction of the existing input shape: additive, zero existing-pixel change, documented in `docs/design/PATTERNS.md`, with the class contract pinned by `TextField.test.tsx`. The 206-input migration is now unblocked, but each migration that changes pixels is harness-gated.
- **Dialogs** → already consolidated on `Modal` (0 bespoke), so protect it via the primitive-adoption lint (C1/T3.4) so it can't regress.

### Ratchet: done in cycle 6 (C1/T3.4 third rail)

`apps/console/src/design/primitiveAdoption.test.ts` + `primitiveAdoption.baseline.json` now enforce that bespoke-chrome counts (raw `<button>` 420, raw form elements 206, ad-hoc dialogs 0) may only **shrink**, so a new raw element turns the CI `web` job (vitest) red. It was verified to fail on a synthetic +1 and to pass at baseline. This is live in CI **without** the browser harness, because it is a static vitest scan. As S2 migrations land, ratchet the baseline down in the same commit. Combined with the existing `tokenLint.test.ts` (token-lint-strict), two of C1's three rails are CI-enforced now; the axe rail mounts once the harness auth-seed lands.

---

## a11y / parity scan (S1: static coverage measured, axe-per-route pending harness)

The reporter's `scanA11y()` measures the **static** a11y posture; the axe-per-route dynamic scan needs the Playwright harness (see the blocker below). The headline: **the app is a11y-safe by global default, not per component**, which is itself a strong consistency win.
...
**a11y consistency verdict:** reduced-motion and focus-visible coverage are **structurally complete via global rules**, which is the correct, DRY posture. The S3 ratchet must therefore **protect the two global rules** (a test asserting they exist is already wired into `consistencyAudit.test.ts`) and add the **dynamic axe-per-route** scan, which needs the harness. The 171 `outline-none` are safe and should not be individually "fixed", because that would be churn against a working global net.

### Dynamic axe WCAG AA scan + visual baselines: harness built, baseline capture blocked on auth

The Playwright harness (`apps/console/e2e/`, `apps/console/playwright.config.ts`) is built and parses cleanly (**64 tests**: 16 routes × 2 themes × visual+axe). Chromium is installed and the specs run. **But** capturing *valid* baselines is **blocked**: the built SPA gates its first render on an authenticated identity/config fetch (`src/app/identity.tsx`, where `loaded` gates render, and the gateway needs the owner `gideon_token_<port>` cookie). Driven by a fresh, unauthenticated Playwright context (both against `vite preview` and against the live gateway at `:10000`), the app **renders a blank body**, so a naive capture produced 32 *identical blank* PNGs, which is a **false safety rail** and was therefore **discarded** (not committed).

- **What is proven:** harness infra correct: config, route SoT, theme seeding, axe wiring, npm scripts and README all in place; `playwright test --list` = 64 tests; `npx playwright install chromium` succeeds; the preview server builds and serves the real HTML/JS.
- **What is blocked:** meaningful baselines and a real axe reading need an **authenticated session**. This can't be done autonomously in the sandbox (no owner token).
- **Recommended default (owner / CI, one-time):** seed an auth cookie, then `npm run e2e:update` to capture real baselines. Two supported paths:
  1. **Owner-local:** with the gateway running and logged in, run `PW_NO_SERVER=1 PW_BASE_URL=http://localhost:10000 STORAGE_STATE=<exported cookies> npm run e2e:update`.
  2. **CI:** mint a test-owner token in the CI gateway boot and inject it via `page.context().addCookies(...)` in a global-setup (stub: `e2e/auth.setup.ts`, to add when the CI token minting exists, plan 33 rails).
- Until then, the harness is **wired but dormant**, so S2 fixes that need visual verification must be checked by the owner running `npm run e2e` locally, or the auth-seed default above must land first.
...
4. Bespoke dialogs: confirmed `grep -rn 'role="dialog"' src/pages` returns 0, so `Modal` is canonical.
5. `KnowledgeGraph.tsx:101` / `MemoryGraph.tsx:147`: confirmed identical `transform 200ms ease-out` drag transitions (a duplicated pattern).

---

## S2 priority order (worst-first, from the measured ranking)

1. **Stand up the Playwright harness** (the brief-mandated S2 prerequisite): capture baselines of every touched surface first.
...
4. **Introduce the `TextField`/`Field` primitive** (T2.3, the one genuine gap), then migrate rank 3 and below pages' 206 raw inputs.
5. **Clear the 3 real raw-value fixes** (WidgetFrame calc, drag durations, inset-shadow helper) and add `reason` fields for the 2 legitimate exceptions, then shrink `tokenLint.allowlist.json` to empty.
