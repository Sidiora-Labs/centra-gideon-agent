...
| Key | Default | What it does |
|---|---|---|
| `loops.max_cycles_hard_cap` | `100` | Absolute ceiling on any loop's cycle budget, regardless of the per-loop limit. A safety brake against runaway cost. |
...
(separate from session-end consolidation). Kill switches and tuning:
...
| `workflows.match_threshold` | `0.62` | Cosine gate for a workflow match (0 to 1). |
...
| Key | Default | What it does |
|---|---|---|
| `sandbox.nofile` | `4096` | Soft `RLIMIT_NOFILE` for an agent-influenced child; `0` disables the cap. ACP session hosts are exempt by profile, because they multiplex many MCP pipes. |
| `sandbox.max_pids` | `0` (off) | `RLIMIT_NPROC` ceiling. Off by default because `RLIMIT_NPROC` is a **per-user** limit counting every process you already run, so an absolute cap can break a busy host with "cannot fork". |
| `sandbox.max_rss_mb` | `0` (off) | `RLIMIT_AS` ceiling in MB. Off by default: `RLIMIT_AS` is coarse and breaks memory-mapped toolchains. |
| `sandbox.env_passthrough` | `[]` | Extra environment variable **names** that hook, cron-script and bash-action children inherit from the gateway. Those children do **not** inherit the gateway environment. They get a minimal base (`PATH`, `SHELL`, `PWD`, `TERM`, `PYTHONPATH`, locale/`TZ`, `HOME`/`TMPDIR`/`USER`/`XDG_*`, proxy and CA-bundle settings, and `GIDEON_HOME`/`_WORKSPACE`/`_PORT`) plus the names you list here. Use it when a script legitimately needs one more variable (e.g. `SLACK_BOT_TOKEN` for a notifier, or a language runtime's variable). Names matching the credential floor (`AWS_SECRET*`, `AWS_SESSION*`, `SSH_AUTH_SOCK`, `GNUPGHOME`, `GIT_ASKPASS`) are refused even when declared. Withheld names are listed in the debug log at each spawn, so run the gateway with `--verbose` to see what a script is missing. |

## Top-level

| Key | Default | What it does |
|---|---|---|
| `hooks` | `{}` | Webhook trigger config by hook id, plus `webhook_token` and `auto_approve_sources` (sources whose tool calls are auto-approved). Managed via the Triggers UI/API (`/api/hooks`), and documented here because the raw shape is config-visible. |
...
| `snapshot_dir` | `""` (default dir) | Where `gideon snapshot` writes and reads portability snapshots. |
| `dashboard.url` | `""` | Advertised dashboard origin (host:port), written by `gideon setup` and consumed by the server bind/origin checks. |
...
- `GET /api/config/gideon`: full config as JSON (sensitive defaults are masked in the schema, not here; this endpoint is owner-only).
