...
| `--no-crons` | Skip the cron scheduler, for when another instance handles cron execution. |
...
involved, because a local Ollama needs none, and that is why it is the provider on this
path.
...
* **Read-only by default.** The session's *task mode* is set to `ask`, so every
  non-read-only tool call is denied before the approval gate, which is the same gate the
  native runtime enforces and Trust/YOLO cannot bypass. A tool the classifier cannot read
...
**reuses** that one (minting a token via `.local_secret`, so `run` must share that
gateway's `GIDEON_HOME`) and leaves it running. If none is running, `run` starts a
...
Manage the gateway as a system service: a systemd unit on Linux
(`/etc/systemd/system/`, requires sudo) or a launchd LaunchAgent on macOS
...
| `cron add NAME MESSAGE [--every SECS] [--cron EXPR] [--channel ID] [--approval-mode auto]` | Add a job, by interval (`--every`) or cron expression (`--cron "0 9 * * MON-FRI"`); optionally post results to a channel; `--approval-mode auto` auto-approves the job's tools. |
...
files or directories with relative in-tree paths, so a symlink, hardlink, device or `../`
...
| `skills install ID [--marketplace] [--target DIR] [--force]` | Install a skill (e.g. `vercel-labs/agent-skills/next-js`). Installs are supply-chain scanned; `--force` overrides a WARNING verdict, and a DANGEROUS verdict is never overridable. |
...
| Command | What it does |
|---|---|
| `gideon inbound token create <surface> [--rotate]` | Mint that surface's bearer token, stored in the **credential store** (keychain, else `.env` at `0600`) as `GIDEON_INBOUND_<SURFACE>_TOKEN`. **Printed once**, so copy it into your client immediately. `--rotate` replaces an existing token, which immediately invalidates the old one. |
...
internal secret, or equal to **another surface's** token, because five surfaces sharing
one bearer would collapse five independently revocable credentials into one.
...
immediate kill switch with no restart needed, and `external_access.enabled false` takes
...
parse **fail-closed**, so an unreadable flag reads as *off*, which is the inverse of a
...
| `gideon discover [--timeout SECS] [--json]` | Find Gideon gateways advertising themselves on the local network (mDNS/DNS-SD `_gideon._tcp`). Prints each one's name and base URL. Finding nothing is a normal result and exits 0, because discovery is opt-in, is a no-op on a loopback-only gateway, and many networks filter multicast. See [Companion apps](../guides/COMPANION_APPS.md). |
...
[API overview](API_OVERVIEW.md) · [Getting started](../guides/GETTING_STARTED.md)
