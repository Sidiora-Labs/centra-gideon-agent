...
That file, with **`name` and `description` and nothing else**, is a complete,
valid skill in Gideon. It is also a complete, valid skill in the vanilla
...
Both spellings work, and the list form is folded to the comma-separated form on
read. A phrase prefixed with `!` is a **negative** trigger: if any negative
trigger matches, the skill is excluded regardless of positive matches.
...
`triggers` value means "never auto-surface", and it is never treated as
"matches everything".
...
Declaring them makes them **addressable**, so the agent can pull one by name
...
value is *not* stripped, so put comments on their own line.

`skill_invoke` then returns the skill body **plus a catalog** of those
declarations, one line each, path and description, and **never their contents**.
The agent loads one with `skill_resource(skill, path)`. A skill with no
`resources` block behaves exactly as before.

`resources` is the one **list-of-mappings** block the frontmatter reader
understands (see the parser limits below, where nested mappings are otherwise
skipped). It has its own small reader, so `path` and `description` are the only
keys read inside an item.
...
- **Declared only.** The list is an allowlist. A file that exists in the skill
  directory but is not declared is refused, and so is any path with a `..`
  segment, an absolute path, or a backslash. Declarations that break those rules
  are dropped at parse time, so a bad entry never widens what is loadable.
...
- **Read, never run.** A `tooling/scripts/*` resource is returned as *text*.
  Running it is the ordinary command path's job, with the screening that path
  applies.
...
A loaded skill competes for the prompt with everything else in it, on one
budget, and `context_tier` is how a skill **declares** its share:
...
prompt is the skill's own `description` and its declared `resources:` entry
points, complete, plus the call that loads the full body (`skill_invoke{name}`).
A body cut at a byte boundary is worse than a shorter complete one: the reader
cannot tell it is half, and half a procedure fails at step four.

**A skill that declares neither a `description` nor `resources:` is refused, not
reduced**, because there is nothing to reduce *to*, and manufacturing a summary
from the first N characters of the body is the byte-boundary cut this rule exists
to avoid. What loads instead is a one-line pointer naming the skill and the tool
that loads it. So a long skill should always carry a `description`.
...
`context_tier` is treated as `standard`, so a typo reduces nothing.
...
source's trust tier, then commit the exact scanned bytes. A `dangerous` verdict is
refused with no override, `--force` included. `checks/runtime/test_skill_install_guarded.py`
pins both halves: the conformant skill committing byte-identical, and the floor
holding for that same skill when it ships a destructive script.
...
- block and folded scalars (`|`, `>`, `|-`, `>+`, …), where the indented block is
...
| Block scalars keeping line breaks | Folded to one line, so `\|` does not preserve newlines here. These fields are single-line by contract. |
