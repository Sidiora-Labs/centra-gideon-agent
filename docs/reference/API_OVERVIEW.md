...
| `GET /api/onboarding` | First-run onboarding state: model readiness plus persisted flow progress. |
...
| `GET /api/secrets` · `POST /api/secrets` · `DELETE /api/secrets` | The secrets vault: global, per-project and inherit-from-host rows with presence flags and derived consumer links. Owner-only. **Values are write-only, and no route returns one**, because there is deliberately no per-secret read endpoint. |
