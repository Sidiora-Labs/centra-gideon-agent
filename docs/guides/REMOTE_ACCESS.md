...
home you should stop reading here, because nothing in this guide makes a local install better.
...
1. **A private network (a tailnet).** Your phone and your gateway join the same encrypted overlay
   network and talk directly. Nothing is exposed to the public internet, and there is no TLS
...
2. **A public tunnel.** You deliberately publish the dashboard to the internet behind TLS and a
   password. More moving parts, a larger attack surface, and the right choice only when you cannot
...
3. **Let the gateway listen on the tailnet interface.** By default the gateway binds loopback
   only. Bind it so the tailnet can reach it, and leave token auth on, because the token is the
   front door:
   ```bash
...
**`gideon doctor` does this detection for you.** It finds the tailnet interface and
...
If instead it says `local-only`, your gateway isn't on a tailnet yet, so walk back through the steps
...
risks the public-tunnel path spends effort mitigating, which are cleartext on the wire and a
stranger finding your URL, simply do not arise. There is no public listener to find. The token link
is still your authentication, and everything under *What this does not protect you from* still
applies.
...
> does not open a port for you, so you bring a tunnel. (Tailscale can also do this via *Tailscale
...
Minimum 12 characters. Length beats symbols, so a passphrase you can remember is better than a
...
`trusted_proxies` is the address your tunnel connects **from**, as Gideon sees it. That is usually
`127.0.0.1` for a local tunnel daemon, or the container/bridge address in Docker (for example
`172.18.0.0/16`). Single addresses and CIDR blocks both work.

**Why this list matters.** Your tunnel tells Gideon the real client address via a header, and
...
`public_url` is deliberately **not** editable from the Settings UI, because widening a network
surface should be a deliberate file edit rather than a click.
...
Requiring a code before enrolling a secret would make sign-in impossible, so `gideon auth
...
Losing a code costs nothing, so run the command again. `gideon auth enroll --clear`
...
party. Web push is what a browser or installed PWA uses, and the relay is only for the store app.
...
2. Paste your topic's full **https** URL, for example `https://ntfy.example/gideon`. Plain `http` is
   refused at both the settings field and the sender, because an unencrypted ping would put the item id on
...
deliberate, because the ntfy app shows the raw `{"kind":…,"item_id":…}` body. The alternative is
...
Pick a topic name nobody can guess. An ntfy topic URL is a capability, so anyone who knows it can
publish to it, which means a guessable topic lets anyone make your phone buzz. It still cannot approve
anything, because approving happens on your gateway, behind your session.
...
- **A service worker needs a secure context.** Over a tailnet hostname or a TLS tunnel you are
  ...
  and Settings → Companion apps → *Install & offline* names that as the reason.
...
body that is not exactly the two ids, so it is a dumb pipe by construction rather than a cloud
middle tier.
...
2. In the store app, open the companion screen and tap **Turn on push**. The app
   registers its device token with *your gateway*, which holds it, and the relay sees a token
   only in transit, per ping.

The visible notification is a static "Gideon, Attention needed". Any text worth
varying would have to be composed from the item, which is content the relay must never see.
...
- **Never port-forward your router straight to the gateway.** Opening a port on your home router
  ...
  *and* a public address anyone can scan. Use a tailnet (encrypted, private) or a TLS-terminating
  tunnel, and never a bare forwarded port.
- **`AUTH_MODE=none` cannot be exposed, and that is a feature rather than a limitation.** When auth is
  off, the gateway is *forced* to bind loopback (`127.0.0.1`) no matter what you set, so an
...
- **Never expose over plain http.** Publishing an `http://` (not `https://`) URL to the internet
  ...
  protect you from*. A tailnet sidesteps this entirely, because it is encrypted end to end, and a
  public tunnel must terminate TLS.
...
- **The agent's own reach.** Sign-in controls who reaches the *dashboard*. What the agent may do
  once inside is governed separately, so see [SECURITY.md](../../SECURITY.md), the approval modes,
...
(Both apply to the public-tunnel path, and the tailnet path needs no `public_url`.)
...
**Everything says "not enabled".** Run `gideon auth status`. Enabling sign-in requires a
password first, by design.

**I am locked out and away from home.** Wait out the window (default 15m). If you are truly
stuck, you need local access, and that is the intended floor: nothing remote can override the
lockout.
