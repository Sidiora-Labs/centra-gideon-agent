...
a small, deliberately boring surface: **six read-only tools** over `POST /mcp`. That way the
assistant you are already talking to can ask *your* Gideon what it remembers, what you have
open, and what you asked it to do, without you copy-pasting any of it.

It is off by default. Turning it on takes three commands and a restart.

**Read the next section before you start.** This surface works **on the same machine only**,
and that is not a setting you can change.
...
Here is why. The gateway's CSRF middleware runs before the MCP surface does, and it only
trusts a request with no `Origin` header when the peer is loopback. A browser always sends
`Origin`, and an MCP client is not a browser, so it never does. An off-machine MCP client is
therefore refused **before** the `allow_remote` check ever executes. Setting both fields
correctly, meaning remote allowed, public URL matching the request's `Host` exactly, and a
valid token presented, still returns:
...
That is measured, not theoretical. The security outcome is fine, since remote fails closed,
harder than designed. What is not fine is that the two knobs are a promise the gateway
currently cannot keep.

**So: run your editor on the same machine as the gateway.** If your editor is somewhere else,
put the two machines on a private network and reach the *dashboard* instead. See
[Remote access](REMOTE_ACCESS.md). Do not try to publish `/mcp`.
...
**Read-only by absence, not by a switch.** There is no write tool to disable, so ask for one
and you get a plain "no such tool":
...
- **`POST` only.** There is no SSE stream, and `GET /mcp` answers `405` on purpose.
...
> The last line of that output oversells things: `external_access.public_url` plus `allow_remote`
> do **not** get you off-machine access. See
...
connect.** The `/mcp` route is registered when the gateway starts, and only if the flag and the
token are already in place. Enabling the surface under a *running* gateway changes nothing until
you restart it, and you get a bare `404: Not Found` from the web server, because the route was
...
**`405` is the good answer**, because it means the surface is mounted and telling you it is
POST-only. A `404` here means it is not mounted, so re-check steps 1 to 3.
...
`PASTE_YOUR_TOKEN_HERE` is a placeholder, so put the string from step 1 there.
...
- **`Authorization: Bearer` is correct for `/mcp` and wrong everywhere else.** The dashboard's
  owner token goes in `?token=…` or a `gideon_token_<port>` cookie, and a Bearer header against
  any `/api/…` path gets you `403 {"error": "Token required"}` instead. `/mcp` is the exception:
  it is exempt from the dashboard's cookie auth and carries this bearer credential instead. So if
  you see `Token required`, your client is talking to the wrong path. Also do not try to reuse the
  dashboard secret here, because this surface refuses to accept it as its own token.
...
(`pip install mcp`), which is the same client library your editor is using:
...
One command, and it takes effect on the **next call** with no restart. Either switch does it, and
the master one takes every inbound surface down at once:
...
is per-request, so it is live in both directions. What needs a restart is *mounting the route*,
and that only ever happens at gateway startup (step 3). If the gateway came up while the surface
was disabled or without a valid token, the route does not exist, and no amount of flag-flipping
will create it.
...
|---|---|
| plain-text `404: Not Found` | route never mounted, because the flag or token was not in place when the gateway started. Fix, then **restart** (step 3). |
| JSON `{"error": "not found"}` | route is mounted, but a switch is off: `external_access.enabled` **or** `external_access.mcp.enabled`. Check both, because the master one is easy to forget. It takes effect on the next call, with no restart. |
...
| `405 Method Not Allowed` on a `GET` | correct. The surface is POST-only, and this is the mounted-and-healthy signal. |
...
| `{"error": "rate limited"}` (429) | you exceeded the burst of 20. The SDK drops the whole transport on this, so reconnect and slow down. |
