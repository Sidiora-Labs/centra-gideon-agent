...
This guide covers a self-hosted Docker Compose deployment: ports, volumes, the `.env`
...
`https://127.0.0.1:3443` and accept the self-signed certificate. To replace that
certificate, mount a real one over `/etc/nginx/certs/gideon.{crt,key}`.

## Volumes

State lives in the named volume `gideon_home`, mounted at `/data` inside the gateway
container (`GIDEON_HOME=/data`). It holds config, credentials, memory, knowledge, apps,
and the workspace, which is everything that has to survive a container recreation.
...
State survives `docker compose down && docker compose up -d`, because the volume outlives
the containers. It is **removed** by `docker compose down -v`, so do not run that unless
you mean to wipe state. Take a snapshot first.

## Environment (`.env`)

Compose reads the repo-root `.env` through each service's `env_file`. Copy
`.env.example` and set only what you need; every variable is optional and has a sensible
default. The common ones:
...
install. The in-app Updates panel then shows the correct update instructions (pull and
up) instead of a git/pip update flow.
...
To restore, copy an archive back in and run `gideon restore <path>` inside the container.
Take a snapshot before every upgrade.

## Updates

Container installs update by pulling the new image and recreating. There is no in-place
self-update, and the app's Updates panel shows exactly these commands for a container
install:
...
- **`gideon token` says the gateway isn't running.** Check that `docker compose ps` shows
  `gideon-gateway` healthy. The healthcheck hits `/api/healthz`.
