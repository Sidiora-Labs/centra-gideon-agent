...
rendered on a different screen. The only hard problem is the first thirty seconds, when the
other device has to learn where this machine is and then be allowed in.
...
> [Reaching your dashboard from outside your home network](REMOTE_ACCESS.md). Nothing here
...
1. **Let the gateway off loopback.** By default it binds `127.0.0.1`, which no other device
   can reach. Bind it wider, and leave the token on, because it is your front door:
   ```bash
...
**A durable session instead of a long URL.** A token link is fine once, and for a device you
will use every day, pair it instead. On the machine:
...
is unremarkable, and in a café or an office it tells every other device on that network that this
...
hostname is used. It takes effect immediately, with no restart.
...
it works from a fixed list of four keys and cannot express a fifth, and a test asserts that the
serialized packet contains none of the gateway's credentials. The dashboard shows you the
record verbatim under **Settings → Companion apps**, so what you see there is what your network
receives.
...
- **It is not access control.** Discovery says *where*, and the token or the pairing code decides
  *whether*. Turning discovery on does not make your gateway more reachable, because a gateway bound
...
- **It is unauthenticated in both directions.** Anything on the network can claim to be a
  ...
  a code minted on the real machine, so a lookalike gets you a failed pairing rather than a session.
...
- **It does not cross networks.** Multicast stops at your subnet. From outside your home
  network you want [remote access](REMOTE_ACCESS.md), not discovery.
...
| Settings says *"no local-network address"* | The machine has no LAN address to publish, usually because there is no network, or an interface is up but unaddressed. |
| `gideon discover` finds nothing | Discovery may be off on the other machine, or the network filters multicast. Guest and corporate Wi-Fi very often do, and client isolation blocks device-to-device traffic entirely. Nothing is broken, so type the address. |
...
| `CSRF check failed: request origin not allowed` while redeeming a code | See the rough edge above, and reach the gateway by hostname for that step. |
...
1. **Find a gateway, or accept a typed URL.** Both, always, because discovery is never a
...
3. **Render the served dashboard.** There is no separate companion API, because a companion loads
   the same SPA from the gateway it paired with. Keeping one frontend is why the phone never lags
...
own routing, its own caching and its own reconnect behaviour. The genuinely new state, and the
...
- **`id` is the client's own.** It exists so the shell has a stable key to namespace state
  ...
  it or needs to.
- **`base_url` is an origin to navigate to, not a prefix to prepend.** More on this below, and
...
- **`kind`** is `local` for a gateway on your own network and `remote` for one reached through
  the [remote access](REMOTE_ACCESS.md) path. It changes nothing about authentication, and it
...
- **`label`** defaults to the gateway's own `companion.instance_name` (the same field the
  ...
  itself, because a user who renames "Living room Mac" once should not have to rename it again in
  every wrapper.
...
origin.** This is not a preference, because the served dashboard is structurally incapable of
holding it:
...
2. Load that endpoint's `base_url`. That is not a re-configuration but a navigation to a different
   origin.
...
completion sets the cookie on the response, and a native shell such as Electron or a Capacitor
...
A `remote` endpoint is your own gateway reached through the [remote access](REMOTE_ACCESS.md)
...
- **A WebView shell** loads the served dashboard from `base_url`. The SPA then opens its own
  ...
  it to an API path.
- **A native client**, meaning one that speaks to the gateway directly instead of hosting the SPA,
  has no document and therefore no `location` to be relative to. It has only the registry row, so it
...
endpointSocketUrl('http://gideon.local:10000')  // 'ws://gideon.local:10000/api/ws'
...
without anything having to remember that "remote implies TLS". Nothing consults `kind`, because
that field says whether your shell spawned the gateway, which is a lifecycle fact rather than a
transport one. An unparseable `base_url`, a bare host, or a scheme that is not `http`/`https`
returns `undefined` rather than a guess. Show "this endpoint is misconfigured" on that row,
because a guessed scheme dials a socket somewhere you did not intend.
...
- Degraded UI is whatever the dashboard already renders in that state. It is the same contract
  the rest of the product uses, and a companion is not a special case.
...
- **The drop arrives in one of two shapes, and you must handle both.** The read either returns a
  ...
  shape turns an ordinary tunnel restart into an unhandled error. Neither shape is slow, so if
  neither arrives, you are looking at the hang this contract exists to prevent rather than a quiet link.
- **Your device session survives the drop, so keep it.** Reconnecting does not mean re-pairing. The
...
4. **Load `base_url` as an origin.** Never prepend it to an API path, because there is nothing in the
...
1. **Wrap the served UI.** Load an endpoint's `base_url` as an origin in whatever the platform
   ...
   find yourself designing a view, you are on the wrong side of the line, because that view belongs
...
platform that cannot be brought up without one has found a gap in this contract, so report it as
...
- no platform SDK dependency, and no platform SDK identifier in `runtime/gideon` or
  `apps/console/src` (those two are the surfaces every platform shares, so a native symbol appearing there
...
dead code that names a platform is worse than dead, because it reads as a promise. Between now and
...
its place by passing the eight-item list rather than by being wired in ahead of time.
