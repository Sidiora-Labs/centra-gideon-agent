...
```json
{
  ...
  "metadata": {
    ...
    "steering_examples": [
      ...
      {"event": "mutation", "description": "The audit found three Major findings, so re-run …"}
    ]
  },
...
what driving this looks like, and a `mutation` example of what editing it
...
Four ship. They expand into core nodes **at definition time**, so what is
stored, validated and run is one tree, and you can expand a macro and then
hand-edit the result when you outgrow the pattern.
...
Give each lens its own prompt. N identical prompts catch a flaky answer, and N
different lenses catch a failure mode the others are blind to. The second is
what makes a panel worth its tokens.

`verify_panel` asks the model to **refute**, and defaults to refuted when
...
instead of raising a binding error mid-run, after the classifier has already
spent its tokens.
...
than none, because the next iteration would trust it.
...
and leave it unset for a handful of cheap items. It must be a whole number: `1.5`
...
`subworkflow` runs another workflow as a real child run, with its own journal,
state and history, so it can be rewound and inspected on its own:
