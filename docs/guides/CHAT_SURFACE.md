...
them. If you decide you preferred the old direction, **restore** it. That creates a **new
...
Five rewinds' worth of history are kept per message, and a sixth pushes out the oldest.
...
There is no "are you sure?", on purpose. Branch only ever creates, so it cannot overwrite
anything in the chat you are in. Rewind, which replaces, does ask.

**Temporary** and **incognito** chats do not offer it, because there is no retained transcript
to copy. There is also a ceiling on how many chats can exist at once, and at the ceiling
...
Nothing offers this to you and nothing decides you need it, so a quick task stays quick. When
...
and it is the tool gate that refuses a mutating tool while you are in it. That is the same
gate the task-mode pill uses, checked before any approval you might already have granted.
...
- **Approve & run it** returns the chat to the task mode it was in before and carries the
  plan on *in the same conversation*. It is a continuation, not a fresh start.
...
Sending a message mid-turn queues it, so the current answer finishes and then yours runs. Each
queued card has three controls: **Cancel** (drop it, and the text comes back to the composer),
**Edit** (the same, but reopened for changes), and **Interrupt now**.
...
If there is nothing queued there is nothing to promote, and Interrupt is not offered, because
with an empty queue it would just be Stop under another name.
...
highlights in place. The count reads `3/17`, `Enter` or `↓` moves to the next match, and
`Shift+Enter` or `↑` to the previous one, with the matching turn scrolled into view. `Esc`
closes it, and so does a second `⌘F`, which hands the shortcut back to the browser's own find
...
output is not searched. It never reformats your messages to highlight them, so code blocks
and a reply still streaming in stay exactly as they were.

This searches the conversation you are in. To search *across* conversations, use
**Search chats** in the sessions list, which looks at titles and everything said in every session.

On a phone-width screen the bar spans the column instead of sitting as a pill in the corner,
so it shrinks with the page rather than hanging off the edge of a narrow one.
...
it is. They disappear the moment you do anything else, whether that is starting to type,
sending something, or switching session, so they never move the composer under you.
...
- **Smooth** (the default) reveals whole words at a steady pace, decoupled from however the
  network happens to chunk the reply. Text arriving in one large burst still catches up within
  a few frames, because the pacing never lets the display fall behind the answer.
...
- **On macOS**, the gateway's host uses the system snip: a crosshair, drag, done. No browser
  picker, and no whole-screen share to approve.
- **Everywhere else**, the browser asks which surface to share. Gideon grabs **one
  frame** and stops the capture immediately, so nothing keeps recording, and then you drag a
...
leaves exactly one entry in **Settings → Audit log**, and where a mechanic has two distinct
...
A branch that is *refused* is recorded as well, with the reason. That covers a temporary chat,
a chat an app does not own, and the ceiling on how many chats may exist. A refusal is a
...
text that is still waiting for you. Cancelling records nothing either, since it returns the
chat to the task mode it was in before the gate opened.

The other three, which are find, quote, and the streaming reveal, record nothing, and that is
correct rather than missing: none of them leaves your browser. Find scans the conversation
already loaded in the page, quoting writes into your own composer, and the reveal setting only
paces text the reply had already sent.
