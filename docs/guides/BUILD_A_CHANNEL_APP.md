...
source repository configured by your operator, since no public bundle repository is assumed.
Read it alongside
[the inbox & channels architecture](../architecture/INBOX_CHANNELS.md) and
[the provider boundary](../architecture/PROVIDER_BOUNDARY.md).

Two rules frame everything else:

- **Core never names your vendor.** No string, no enum member, no `if provider == …`, no
...
|---|---|---|
| `ChannelTransportProvider` | ABC, so your class subclasses it | `from gideon.sdk.channel import ChannelTransportProvider` |
| `ChannelDelivery` | `Protocol`, structural, so you do not subclass it | `from gideon.sdk.channel import ChannelDelivery` |
...
vendor's UI. A channel that only ever pushes notifications can ship a transport alone, while
a channel that carries conversations ships both.

Everything a channel app is allowed to import from core comes through
`gideon.sdk.channel`. That facade is the boundary the apps-repo import lint enforces, so
reaching into `gideon.<internal>` will fail the lint, and would freeze a core
internal besides.
...
`runtime/gideon/assurance/testing/channel_conformance.py`) and this table is written from it.
...
three tuples. Their levels above are derived from the ABC's own contract, meaning what core
calls and what the default does if you skip it. Treat them as doctrine, not as something the
kit will catch for you.

### `ChannelDelivery`

All eighteen `Protocol` methods. Every delivery obligation is conditional on shipping a
delivery object at all, and once you do, the kit asserts the MUST and SHOULD rows.

| Method | Purpose | Obligation | Kit |
|---|---|---|---|
| `deliver_text` | a plain reply reaching the user, which is the floor of a conversational channel | **MUST** | yes, clause "delivery" |
...
| `stop_stream` | closes the stream and **force-flushes** the exact final text past the throttle | **SHOULD when `edits` is `True`**, because a throttled-away final update is a stream frozen mid-run | yes, clause 8 |
...
| `resolve_user_name` | user id to display name, for transcripts and attention items | **SHOULD** | **no**, in no kit tuple |
...
three tuples either, with the same caveat as above: doctrine, not an assertion.

### Declare capabilities honestly

`ChannelCapabilities` is the routing input, so a field you set to `True` is a promise.
The fields are `inbound`, `threads`, `attachments`, `reactions`, `edits`, `rich_text`,
`typing_indicator`, and `max_text_len` (`0` means unbounded). Defaults are conservative,
text-out only with no inbound, which is the right starting point. Two clauses of the kit
exist purely to catch dishonesty here: declaring `inbound=True` with no inbound path, and
declaring `edits=False` while `start_stream` hands core a ts to animate.

---

## Transport lifecycle: who calls what, and when
...
- `start_inbound` receives a `GatewayServices` handle: sessions, cron, channel history,
  dashboard state, config, owner. Route inbound messages into chat sessions through it, and
...
- Own your loop. Every shipped channel drives its own receiver from `start_inbound` and
  ...
  which handler that is with `inbound_via="_on_message"`, and it accepts a named async method
...
- `health()` is passive and cheap: are credentials present, is the socket up? `test()` is
  allowed to make one round-trip. If they can disagree, you have two truths and the owner
...
policy so it cannot drift per channel, and your transport's job is to *call* it, on **every**
...
- **Unknown DM sender.** Under the default `dm="pairing"` policy an unpaired sender is
  **denied**, `verdict.canned_reply` is the shared `CANNED_PAIRING_REPLY` (every channel
  says the same thing, so do not write your own), and core raises exactly **one** actionable
  owner attention item carrying Allow/Deny plus the `provider` and `sender_id` the buttons
  need. It is deduped, so a second message from the same stranger does not re-alert. The
  canned reply is rate-limited to once per sender per 24h.
- **Pairing.** The owner runs `gideon pair <provider>` (see
  [the CLI reference](../reference/CLI.md)) and hands over the 8-digit code out of band. The
  sender types it into your channel and you call `redeem_pairing_code`. Single-use and
...
- **Groups.** Default `group="tracked_only"`: an untracked group is denied *silently*
  (`reason == "untracked_channel"`, so no owner spam), and a tracked one is allowed. Use
...
- **Fencing.** Non-owner content comes back as `verdict.fenced_text`, already wrapped by
  ...
  neutralised chat-template-token defences. Never test a fence by substring, because the
...
Linking the channel to the dashboard works in both directions. Build a session link with the
token-auth helpers (`generate_token`, `LINK_WINDOW_SECS`) over `dashboard_origin()`, and give
the owner a way back with `build_thread_link`. Treat any token you print as a credential,
because it is a bearer token with a long TTL.
...
instance**, and the kit never constructs one, so it cannot disagree with your wiring:

```python
# your-channel/test_provider.py
from gideon.sdk.channel import assert_channel_contract
...
Import it from `gideon.sdk.channel`, **not** from `gideon.assurance.testing.…` and not
...
rather than core's `checks/runtime/`, which ships in no wheel, and the module docstring records
that decision.

Run it against an isolated `GIDEON_HOME`. The kit drives the **real** trust seam, so it needs
to see default policies and must never write your actual store. It raises
...
so call it once per config if you have several.
...
| vendor completeness | **never fails**; it warns. See below |

If you cannot supply `min_edit_interval` and `clock`, the streaming clause degrades to
presence-only. The kit refuses to sleep, and it refuses to invent a floor it cannot know.
...
`name` (kebab-case), `version` (semver), `displayName`, and `description`, plus the provider
registrations. The full manifest contract is
[the app-platform architecture doc](../architecture/APP_PLATFORM.md) and the
...
- `provider` (singular) is the canonical registration, and `providers[]` carries the rest.
...
channel leaves the rest of the platform blind to it. So **one vendor app registers every seam
...
*that vendor app's own UI surface*, which is a `ui` page inside the app, and **never** a core
...
repository. One bundle holds a channel transport plus an inbox `MessageSourceProvider` plus a
`TriggerSourceProvider`, each built as an adapter over the transport's *existing* client or
inbound stream, never a second connection. It ships no `ui` block, because the generic
...
the two instances never see each other. The wrong fix is a second connection to the
vendor. All four shipped channel apps use the same shape instead: a small `inbound_tap`
module in the app's own runtime package with `subscribe` / `unsubscribe` / `publish`. The
transport publishes, and the trigger source subscribes in `start(emit)` and unsubscribes in
`stop()`. A module in your package is the only thing the two instances demonstrably share.
...
reach your bot can arm the owner's automations by sending it a message, which is strictly
worse than the session the trust gate already refuses them.
...
owner's triggers to match, and choosing the trigger is choosing the action. Then put every
piece of prose in `SourceEvent.text`, because that is the one field core fences at ingestion.
...
not a gate. It also stays silent when no `app.json` is discoverable, because a bare unit test
or a core fixture has no bundle, and an advisory that fires on fixtures teaches readers to
...
suppressors are independent on purpose, because a shared one would silence a seam nobody had
thought about:
