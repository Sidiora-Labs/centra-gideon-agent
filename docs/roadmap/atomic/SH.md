...
`src/gideon/signing.py` is the verifier: a signed bundle carries `.gideon-signature.sha256`
(canonical `gideon-sig-v1` + sorted `sha256  relpath` for **every** file) and
`.gideon-signature.sha256.minisig` over that file's exact bytes. Verification re-derives the manifest
