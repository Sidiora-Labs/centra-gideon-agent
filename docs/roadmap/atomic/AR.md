...
AGENT-ROOMS is a PROPOSED+DEFERRED direction-holder with no contracts, no task tables, no execution log, and zero done work. The whole plan is gated on an owner un-deferral decision (contingent on the LOOPS-EVOLUTION "council" template shipping and demand being re-confirmed) plus four cross-plan prereqs (WORKFLOWS-V2 core slices, ACP-AGENT-PARITY, AUTONOMY-GUARDRAILS, INBOX-NOTIFICATIONS-UNIFICATION). I produced 9 atoms: the cheap council-template precursor (AR-9) that the plan header says to build first, an un-deferral/design-resolution gate (AR-1) that depends on it and must land before any room work, and 7 implementation seams from the scope sketch (room store/transcript, member model + listen policies, per-member cursors + fenced transcript feed, deterministic arbitration + round budget, per-member safety profiles + human-sole-approver, room-level SEL, UI surface) — each depending on AR-1 and its own cross-plan prereq. AR-9 has SHIPPED (PR #2507), so fan-out is now available to try; everything after it waits on one owner question — whether deliberation rather than fan-out is still the missing capability.

Each atom below executes start-to-finish in one go. If an atom lists dependencies, they must be `done` before it starts — that is the whole point of the split: no atom should ever need pausing to go execute other work.

| Atom | Status | Title | Depends on | Done when |
|---|---|---|---|---|
| `AR-1` | ⬜ | Un-defer the plan: ship the council precursor, re-confirm demand, and resolve the 6 design questions into contracts | `AR-9`, `EXT:WORKFLOWS-V2:core slices own pipeline-shaped multi-agent orchestration; rooms are only the deliberative remainder` | The LOOPS-EVOLUTION council template has shipped, the owner re-confirms deliberation (not fan-out) is the missing capability, and all 6 design questions (transcript persistence format, member context budget policy, rooms x memory writes, ACP capability gates, room-level SEL, UI surface) plus the 3 owner tasks (default round budget + pause UX copy, ACP membership eligibility) are resolved into a Contracts section and task table added to this plan. |
