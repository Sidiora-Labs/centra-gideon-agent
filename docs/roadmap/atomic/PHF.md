...
2. Extract the largest cohesive per-domain sections into sibling modules under
   `src/gideon/config/`, following `src/gideon/agents/native/decision_tool_defs.py` —
...
`real-home rail FAILED: 1 entries under /home/runner/.gideon changed` → `modified config.json`.
...
`import gideon.mcp_artifacts` → `mcp_core._API = _resolve_api_base()` → `AppConfig.load()` →
