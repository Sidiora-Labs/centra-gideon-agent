...
scaffold (`gideon app new research-lab --type tool`) and filled in: five tools over a campaign
...
repo, version-controlled here first" convention) because GideonApps is a separate repo the
owner pushes.
...
**Not yet met, and the flip waits on them:** the GideonApps PR itself and the
`github.com/Gideon/research-lab` repo do not exist (owner push), so the registry's live
