...
| `WF2UNI-14` | ⬜ | Retire legacy planning modules after loop drain (split from WF2UNI-12 per ruling) | `WF2UNI-12`, `EXT:PROMPT-PACKS:PP-16 loop-to-WorkflowRun store seam (the Phase 3->4 drain)` | planning/ (runner.py, session.py), loop/plan_walkthrough.py, loop/*_plan_briefs.py, and loop/classify.py + loop/code_classify.py are deleted as loops drain, with their example intents already seeding the eval fixtures. BLOCKED on the Phase 3->4 drain, whose code seam is `PP-16` (`todo`) — NOT on WF2LOO, which is 18/18 `done` but delivered Phases 1-2 (coexistence + parity). Also carries a SCOPE defect: `planning/session.py` is out of reach regardless of the drain, because `CC-8` (`done`) shipped chat plan mode on top of it and names the module in its own `done_when`. See the atom's `blocked_reason` for the measurements and the owner choice. |
