...
| `EA-7` | ✅ | Sender-trust substrate — channels' inbound-identity half + DM pairing | `EXT:CHANNEL-EXPANSION:channel trust seam (plan 40, precedes EXTERNAL-ACCESS per hard rules)`, `EXT:PROVIDER-BOUNDARY:apps/slack-channel allowlist migration` | Done (multi-PR): the trust seam is `channel_trust.py` — one store `entity_settings/channel_trust.json` (atomic_write; the done_when named `sender_trust.json`, the store landed under the entity-settings convention), `guard_inbound` → `TrustVerdict`, pairing via `gideon channel pair` + Settings, unknown-no-code senders dropped-and-counted, slack allowlist migrated. The chokepoint is STRUCTURAL: the guarded door `services.deliver_channel_inbound` → `channel_inbound.deliver_inbound` + conformance kit (#2451); all four bundled apps migrated (Apps #72, #73); step 3 (#2467) dropped `run_chat` from the sdk facade — the chokepoint test asserts the ABSENCE and the `gideon.sdk.*`-only import boundary makes the door structural. ABC unchanged: new transports inherit trust with zero code. |
...
`guard_inbound`, pairing create/redeem + `gideon channel pair`, slack allowlist migration)
...
conformance kit (core #2451), all four bundled channel apps migrated onto it (GideonApps #72
...
`tests/test_channel_inbound_chokepoint.py` asserting the ABSENCE and the `gideon.sdk.*`-only
