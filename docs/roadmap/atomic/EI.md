...
**Status:** done — merged via core PR #2394 (commit 5b156d829) + GideonApps #64.
...
`gideon-<project>-<run>-<session>` worker (its sole producer has only the reader as a caller), and the
...
`gideon_tool` and `tool_gateway` and **no docker provider**; `provisioning.py:448` answers
