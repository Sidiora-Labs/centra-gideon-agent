...
**Status:** done — core PR #2351 + apps PR GideonApps#60 (the OmniVoice-vs-CosyVoice engine spike was split out as `MI-6`).
...
**Status:** done — merged via core PR #2425 (commit 2ec168d00) + #2436 + GideonApps #70.

MI-2 §2.2 engine-selection remainder, carved out of MI-2 and tracked informally as "MI-2c". Minted as the next free numeric id (MI-6) rather than a letter suffix, because the status-sync id regex is `[A-Z0-9]+-\d+` and `MI-2c` is unrepresentable. Deps: the MI-2 halves — the core capability surface (PR #2351, merged) and the apps/voice-clone-tts sidecar (apps PR GideonApps#60) — so it depends on `MI-2` as the atom that owns both.
