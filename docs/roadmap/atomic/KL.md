...
| `KL-18` | ⬜ | Retrieval passage payload: return the passage that matched, not a neighbor (T02) | — | The retrieval payload text is provably the chunk that scored (id-to-text join covered by a test with distinct sentinel chunks); the off-by-one/neighbor-assembly defect from the draft reproduces then passes; snippet highlighting still aligns. |
