...
- **No new action providers** → `ALLOWED_HOOK_PROVIDERS` (src/gideon/validation.py) is **untouched** — this plan adds no hook-fireable actions. (If a later phase adds a `vault-sync` action for the automation substrate, it follows the app-delivered action rule + allowlist addition; explicitly out of v1.)
