...
- **2026-08-24 — atom `EI-6`: runner lifecycle landed whole; durable sessions landed as the
  ...
  **§5.1: the reattach-not-reap pre-step is built and measured; NOTHING IN CORE SPAWNS A DURABLE
  ...
  the identity-derived `gideon-<project>-<run>-<session>` name, and the probes; `terminal.py` now
  ...
  **NOT BUILT, named rather than faked — the durable-worker SPAWN.** No core code opens a durable
  ...
  `gideon-*` session an operator left on our socket) but not yet a worker core placed there. It is
...
- [2026-08-28][EI-10] **DEVIATION — the grant-to-sandbox toggle is NOT shipped, and the atom stays
  ...
  `none`/`gideon_tool`/`tool_gateway` and **no docker provider**; `provisioning.py:448` answers
...
- **[2026-08-27] OWNER RULING — `EI-7`: DEVIATION accepted; the in-sandbox observation moves to `EI-2`'s
  `done_when`.** SC7 (`gideon-tool memory_recall` succeeding while `ss`/`netstat` show zero listening sockets, no
