...
1. **Apple-Silicon/MPS is first-class** — a documented `device_map="mps"` path plus a
   ...
   TensorRT-LLM; MPS is left to third-party forks. Gideon synthesizes on the user's Mac.
...
- **No first-class Apple-Silicon/MPS path.** The documented runtime is CUDA/nvidia-
  docker/vLLM/TensorRT-LLM. Gideon runs on the user's Mac, so this is disqualifying on the
...
**Reconsider CosyVoice if** Gideon ever grows a server-side or CUDA-backed synthesis tier,
