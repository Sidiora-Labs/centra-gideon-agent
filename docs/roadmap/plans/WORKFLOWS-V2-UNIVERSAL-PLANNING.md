...
| Grounding bundle's action catalog | Regenerated from `action_providers/registry.py` (the 9 allowlisted providers) + the MCP instance store (`~/.gideon/mcp.json` via `providers/mcp_instances.py`). **Invariant:** any NEW action provider this program ships must be added to `ALLOWED_HOOK_PROVIDERS` (`src/gideon/validation.py`) or hook create/update rejects it. (This plan ships none; the scratchpad scan uses existing triggers + `run-workflow`.) |
