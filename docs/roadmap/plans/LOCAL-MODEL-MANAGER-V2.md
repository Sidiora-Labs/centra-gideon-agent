...
1. **App credential** — the PClaw credential store: `save_credential()` → `~/.gideon/.env` (0600, mirrored to `os.environ`). *(Reality correction: omnivoice Fernet-encrypts in SQLite with a machine-id key; PClaw's real seam is the existing `.env` credential store — same file the branded-provider `_factory` credential order already reads. No new encryption machinery.)*
...
- **No new action providers.** This plan adds nothing to `ALLOWED_HOOK_PROVIDERS` (`src/gideon/validation.py`) — stated explicitly so nobody "helpfully" wires a download action into hooks; downloads stay user-initiated HTTP.
