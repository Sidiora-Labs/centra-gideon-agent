...
- **[2026-09-06] 🟡 IMPLEMENTATION LANDED for `PEP-15` (Docs/Slides, suite 4/9) across two branches; the atom
  stays ⬜ for the owner's `dag.json` flip.** The bundle is **GideonApps `feature-pep15-docs-slides`**
  (`docs-slides/`: `app.json`, `provider.py`, `app_cli.py`, `test_provider.py` (39 tests), README, LICENSE —
  scaffold-generated by `gideon app new docs-slides --type tool`, then filled in), and the seam it rides
  is core `feature-pep15-docs-slides` (`src/gideon/sdk/documents.py` + `tests/test_sdk_documents_seam.py`).
  **Core merges first** — without the facade `provider.py` cannot import. Three tools: `deck_from_brief`
  ...
  reads it. **The seam had to be promoted, not reached around** — `gideon.documents` was core-internal, so
  ...
  `apps.app_manager.install(origin="local")` in an isolated `GIDEON_HOME`, then
