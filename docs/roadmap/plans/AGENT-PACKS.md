...
| Scope creep toward a pack registry service | Hard line in §Overview soul guardrail: catalogs are fetched JSON files; publishing is "put the .gideon somewhere" (the known pattern: "publishing is just a git push"); no server component, ever, in this plan |
...
5. An external skill catalog registers as a marketplace; installing one of its skills produces a standard `.gideon-lock.json` and passes `verify_skill_integrity` — proving zero bypass of the existing chokepoint.
