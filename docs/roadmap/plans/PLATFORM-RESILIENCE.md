...
### 1.1 Probe tiers (Gideon three-tier readiness, extended per-capability)
...
Adopted verbatim from Gideon (learning #3) as a written invariant on the framework:
...
- **Action providers:** this plan adds NONE. If a future session exposes remediation as a `run-doctor` action provider (so triggers can fire it), it MUST be added to `ALLOWED_HOOK_PROVIDERS` (`src/gideon/validation.py`) or hook create/update rejects it — restated because §4's engine is where that provider would be born.
...
| Remediation engine = a new complexity center (Gideon deleted their breaker machinery for a dumb cooldown) | The engine is a plan-executor over declared jobs, not a policy brain: deficit inputs are measured counts, ordering is declared `after:` edges, stopping is three plain caps; per-job cooldowns are the "dumb cooldown"; if it misbehaves, disabling it restores today's heartbeat jobs (kept callable) |
