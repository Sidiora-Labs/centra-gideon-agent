...
- **Observer = the existing dependency-free poll pattern**, not watchdog. Recon: `fs_watch.py` is a 3s mtime+size-signature poller and Gideon deliberately avoids the dependency; the amendment's "watchdog observer" is adapted to this reality — `dir-source.poll` diffs the signature map in its cursor (`{path: (mtime, size)}`) at the source's `poll_interval_secs` (default 60s for dirs, not 15 min), detecting create/modify/delete. First pass seeds only (no startup ingestion storm — fs_watch's own rule).
...
- **No watchdog dependency** — the poll-signature pattern Gideon already uses.
...
- **No new action providers ship in this plan.** Downstream actions are the substrate's business. IF a connector pack ever ships one (e.g. a `post-back` action), it follows the `apps/webhook-action` precedent — `type: "action"` app **AND its name added to `ALLOWED_HOOK_PROVIDERS` (src/gideon/validation.py)** or hook/trigger create rejects it — stated here so no pack author trips on it.
