...
- **LV-6 DONE.** `docs/roadmap/research/learning-benchmark-protocol.md` is frozen as **PROTOCOL v1**
  ...
  reasons (`loop-worker` fires only inside a loop; `gideon-api`/`gideon-features` would test doc
...
- [2026-08-25][LV-5] **DEVIATION 1 — provenance lives in the overlay, not SKILL.md frontmatter. This is
  ...
  made the base skill immutable precisely so `verify_skill_integrity` / `.gideon-lock.json` stays valid;
...
- [2026-08-26][LV-5] **OWNER RULING — "provenance frontmatter" does NOT require a literal frontmatter
  ...
  `.gideon-lock.json` stays valid, so stamping `SKILL.md` frontmatter would break a marketplace-locked
