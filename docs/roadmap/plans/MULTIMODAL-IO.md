...
All five mechanisms are verified working code in agenticSeek; Gideon's split is that STT capture lives in the **browser** (FE mic → `POST /api/stt/transcribe`) while TTS is server-rendered and FE-played (`voice_chunk` WS events) — so the pack lands as one shared backend module + one FE hook, not a monolithic loop.
