...
errors, S3 `gideon-api` skill + offline reference, S4 app legibility, S5 UI docs + Discover hub +
...
- **App-shipped skills have a dead field:** `apps/manifest.py:_KNOWN_FIELDS` lists `skills` as a LEGACY stripped field ("no runtime consumer"). The live precedents to copy are `prompts[]` app-owned seeding (`apps/prompt_seed.py`: idempotent, non-clobbering, removal keyed by the app's own files) and the skill install chokepoint (`skills/marketplace.py:install_guarded` — quarantine, scan, `.gideon-lock.json`, SEL audit). §4.1 revives the field through BOTH.
...
- **A bundled self-description skill exists but is prose:** `skills/bundled/gideon-features/SKILL.md` describes capabilities channel-neutrally with NO exact signatures — exactly the "0/5 without exact-signature reference" failure Quarkdown measured. §3 upgrades it.
...
- **Gate/validation errors:** `validation.py:validate_tool_args` failures and hook create/update rejections (including the `ALLOWED_HOOK_PROVIDERS` rejection at `src/gideon/validation.py`) return coded envelopes with `suggestions` = the allowed set — today that rejection is exactly the kind of opaque failure that burns an agent turn.
...
- The existing `skills/bundled/gideon-features/SKILL.md` (prose, channel-neutral, user-facing) stays for "what can you do" questions; `gideon-api` is the operator twin and cross-references it.
...
## 7. Gideon as context provider for external agents

- **Neutral routed-context manifest first, adapters second** (the ai-context-os doctrine: adapters are derived, never canonical). A `context_router.py` assembles, per Gideon-managed project (projects already own `projects/<id>/context/` dirs — `tasks/hierarchy.py`): hard rules/directives at the **top**, scored mid-tier content in the **middle** (relevant memories via the existing recall path, surfaced skills index, knowledge-item pointers), and an L0 one-liner **catalog of what was NOT included** at the bottom with a retrieval affordance — lost-in-the-middle positioning by construction.
- **Per-tool adapters with managed markers:** rendered into `CLAUDE.md` / `AGENTS.md` / `.cursorrules` inside the project's bound `workspace_dir`, fenced by `<!-- GIDEON:START -->`/`<!-- GIDEON:END -->` markers, replaced in place on regeneration (never appended twice, user content outside the fence untouched — the Astryx `agent-docs.mjs` pattern). Regeneration: manual button on the project page + on project-context change; opt-in per project (`legibility.context_adapters` default off — writing files into user project dirs is consent-gated).
...
- **Action provider:** exactly one — `call-app-route` (§4.2) — added to `ALLOWED_HOOK_PROVIDERS` (`src/gideon/validation.py`) in the same commit that registers it, or hook create/update rejects it. Its rejection message is itself a §2 envelope with the allowed set as `suggestions`.
...
- **Session 3 — first-party skill + reference (§3):** `gideon-api` SKILL.md; build-time `docs/agent-reference/` rendering from the manifest generator; `doctor --paths`; the 5-task with/without eval — **session doesn't close below 4/5 + 0 silent misses.**
...
2. The 5-task with/without eval passes the Quarkdown bar: with `gideon-api` SKILL.md + offline reference in context, a fresh context-free agent achieves ≥4/5 first-try success with 0 silent misses (vs a measured without-skill baseline), and the battery is checked in as a regression harness.
...
4. Installing/enabling an app whose `app.json` declares skills results in those skills scanned, locked (`.gideon-lock.json`), and live in the skill index; disabling removes exactly them; a user-edited same-name skill survives untouched; a DANGEROUS-verdict skill is refused with the app still enabling.
