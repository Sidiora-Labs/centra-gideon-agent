...
- **Action providers:** the refiner (§3.1) runs as a trigger-fired workflow via the existing `run-workflow` action provider — already in `ALLOWED_HOOK_PROVIDERS` (src/gideon/validation.py), so NO allowlist change is needed. If any future slice ships a dedicated learning action provider, it must be added to that frozenset or `hook_create` rejects it.
...
- **Refiner agent.** Bundled `refine-template` workflow (trigger-fireable via the `run-workflow` action
  provider) whose stage runs the reserved `gideon-template-refiner` agent, holding only
...
- **Skill sidecar overlays.** An accepted skill *refinement* now writes ONE `skills/.overlays/<name>.json`
  ...
  Revert = delete that one file; the base `SKILL.md` and its `.gideon-lock.json` are never touched, so
