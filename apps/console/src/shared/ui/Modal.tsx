...
export function Modal({ title, icon, onClose, children, layoutId }: {
  ...
  const sheet = (
    <motion.div className="fixed inset-0 z-[var(--z-modal)] flex min-h-0 items-center justify-center overflow-y-auto overscroll-contain p-s sm:p-2xl"
      ...
      <motion.div ref={trapRef} role="dialog" aria-modal="true" aria-labelledby={titleId} layoutId={layoutId}
        className="relative flex min-h-0 min-w-0 max-h-[calc(100dvh-1rem)] w-full max-w-full flex-col overflow-hidden rounded-2xl border border-outline-variant/50 bg-surface shadow-sheet"
        style={{ maxWidth: 'var(--modal-width)' }}
        initial={hidden} animate={resting} exit={hidden} transition={reduced ? spring.effects : physics.fluid}>
        <header className="flex min-w-0 shrink-0 items-start gap-m border-b border-outline-variant/40 bg-surface-high/40 px-s py-m sm:px-l">
        ...
        <div className="min-h-0 min-w-0 flex-1 overflow-x-auto overflow-y-auto overscroll-contain break-words px-s py-m sm:px-l sm:py-l">{children}</div>
      </motion.div>
    </motion.div>
  )
