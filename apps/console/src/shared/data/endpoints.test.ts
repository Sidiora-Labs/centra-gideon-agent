...
// (Measured, not assumed: `apps/desktop/src/application/endpoint-session.js:159` does `page.loadURL(this.gateway.url)` and
...
describe('registry round-trip', () => {
  ...
  it('round-trips through a store under the one shell-global key', () => {
    ...
    expect(store.getItem(REGISTRY_STORAGE_KEY)).toContain('gideon.local')
...
describe('parseRegistry is total', () => {
  ...
  it('duplicate ids collapse to the first occurrence', () => {
    ...
    expect(reg.endpoints[0].label).toBe('Home laptop')
    expect(reg.active).toBe(HOME.id)
  })
...
describe('add / remove / setActive', () => {
  ...
  it('re-adding a known id replaces the row in place and preserves the id', () => {
    ...
    expect(moved.endpoints[0].id).toBe(HOME.id)
    expect(moved.endpoints[0].base_url).toBe('http://100.64.0.2:10000')
    expect(moved.endpoints[1]).toEqual(WORK)
  ...
  it('setActive re-points to a known id and refuses an unknown one', () => {
    ...
    expect(setActive(reg, WORK.id).endpoints).toEqual(reg.endpoints)
...
describe('endpointSocketUrl', () => {
  it('maps https to wss and http to ws, from the endpoint\'s OWN scheme', () => {
    expect(endpointSocketUrl('https://gideon.example.com')).toBe('wss://gideon.example.com/api/ws')
    expect(endpointSocketUrl('http://gideon.local:10000')).toBe('ws://gideon.local:10000/api/ws')
  })
  ...
  it('refuses rather than guesses: empty, unparseable, bare host, and non-http schemes', () => {
    ...
    expect(endpointSocketUrl('   ')).toBeUndefined()
    // A bare host parses as the `gideon.local:` scheme with an opaque path — guessing `https` for it
    // would invent a protocol the owner never declared.
    expect(endpointSocketUrl('gideon.local:10000')).toBeUndefined()
  ...
  it('never carries a credential — the device session rides as the cookie', () => {
    const url = endpointSocketUrl('https://gideon.example.com/?token=sekrit')
    expect(url).toBe('wss://gideon.example.com/api/ws')
  ...
  it('endpointSocket derives from a registry row, and answers undefined for no row', () => {
    ...
    expect(endpointSocket(HOME)).toBe('ws://gideon.local:10000/api/ws')
  ...
  it('a remote row over TLS gets wss WITHOUT anything consulting `kind`', () => {
    ...
    expect(endpointSocket({ ...HOME, kind: 'remote' })).toBe('ws://gideon.local:10000/api/ws')
  })
})
