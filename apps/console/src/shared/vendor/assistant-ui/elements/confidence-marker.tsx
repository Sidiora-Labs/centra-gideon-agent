  ...
  return (
    ...
    >
      <p className="text-[13.5px] leading-relaxed">
        {claims.map((claim) => {
          const Claim = onHover ? "button" : "span";
          return (<Claim
            key={claim.id}
            {...(onHover ? { type: "button" as const } : {})}
            ...
            onBlur={() => onHover?.("")}
            className={cn(
              "focus-visible:ring-foreground/20 inline rounded text-start underline decoration-2 underline-offset-[3px] transition-colors outline-none focus-visible:ring-1",
              onHover && "cursor-help",
