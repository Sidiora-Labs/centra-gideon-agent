...
describe("ToolFallback", () => {
  ...
  it("keeps caller supplied tool labels tied to the actual status", () => {
    const { rerender } = render(
      <ToolFallback.Root>
        <ToolFallback.Trigger toolName="search" label="Ran" cancelledLabel="Stopped" status={{ type: "complete" }} />
      </ToolFallback.Root>,
    );
    expect(screen.getByRole("button", { name: /Ran: search/ })).toBeTruthy();
    rerender(
      <ToolFallback.Root>
        <ToolFallback.Trigger toolName="search" label="Ran" cancelledLabel="Stopped" status={{ type: "incomplete", reason: "cancelled" }} />
      </ToolFallback.Root>,
    );
    expect(screen.getByRole("button", { name: /Stopped: search/ })).toBeTruthy();
  });
  ...
  it("renders nothing for a waiting call without a prompt when the thread cannot answer", () => {
    const view = renderTool({ interrupt: { type: "human", payload: {} } });
    expect(
