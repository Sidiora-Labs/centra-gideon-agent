          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="flex flex-col gap-1 text-on-surface-var text-[0.75rem]">Kind
              <select value={s.kind ?? 'goal'} onChange={(e) => setK('kind', e.target.value as LoopKind)}
                className="h-8 rounded-md bg-surface px-2 text-on-surface text-[0.8125rem] outline-none focus:ring-2 focus:ring-inset focus:ring-primary">
            ...
            <label className="flex flex-col gap-1 text-on-surface-var text-[0.75rem]">Agent (optional)
              <input value={s.agent ?? ''} onChange={(e) => setK('agent', e.target.value)} placeholder="default worker"
                className="h-8 rounded-md bg-surface px-2 font-mono text-on-surface text-[0.8125rem] placeholder:text-on-surface-low outline-none focus:ring-2 focus:ring-inset focus:ring-primary" />
            </label>
