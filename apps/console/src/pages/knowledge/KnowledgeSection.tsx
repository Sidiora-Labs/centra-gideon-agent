...
export function KnowledgeSection({ sub, navigate, query, setQuery, navEpoch }: RouteProps) {
  ...
  return <KnowledgeListPage key={navEpoch} onCreate={() => navigate('knowledge/new')} onOpenItem={(id) => navigate(`knowledge/item/${id}`)} onOpenSources={() => navigate('knowledge/sources')} onOpenReports={() => navigate('knowledge/reports')} query={query} setQuery={setQuery} />
}
