  ...
  return (
    <div className="shrink-0 border-b border-outline-variant/30 px-2xl py-1.5">
      <button type="button" onClick={() => setOpen((v) => !v)} aria-expanded={open} className="flex w-full items-center gap-s text-left min-w-0">
        ...
        <span data-type="caption" className="shrink-0 text-on-surface-low uppercase tracking-wide">Prompt</span>
        {!open && <span data-type="body-s" className="min-w-0 flex-1 truncate text-on-surface-var">{firstLine}</span>}
      </button>
      {open && (
        <div data-type="body-m" className="mt-2 max-h-[40vh] overflow-y-auto text-on-surface">
