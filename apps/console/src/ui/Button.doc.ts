...
const doc: UiDoc = {
  ...
  props: [
    ...
    { name: 'loading', description: 'Cross-fades the label out for a centered spinner while preserving width — use for in-flight async actions. This is also the ONLY prop that publishes `aria-busy`, so it is what tells assistive tech the action is working; a button merely `disabled` by a busy flag announces nothing.' },
    { name: 'loadingLabel', description: 'Optional: WHAT is happening, shown beside the spinner instead of withholding the label. Use it for a slow operation whose progress is worth naming — "Consolidating…", "Syncing…" — especially in a row where several long actions could be running and a bare spinner cannot say which. Omit it for a short, obvious action; `loading` alone then keeps the withheld-label behaviour. The verb should be no longer than the label it replaces: the overlay is sized by the original label and truncates rather than widening the pill.' },
    ...
    { name: 'disabledReason', description: "WHY the button is unavailable, when `disabled` is true. A native disabled button is REMOVED FROM THE TAB ORDER, so a keyboard user cannot reach it to hear anything — measured: 37 of 37 validity-gated submits in the app were disabled with no title and no description. Given a reason, the button switches to aria-disabled (still focusable) with the click suppressed, and the reason rides `title` so AT and sighted users both get it. Omit it to keep the native attribute, which is right when the unavailability is self-evident. Ignored while `loading` — an in-flight button must not be re-clickable, and `loading` is the path that does publish aria-busy. Note the converse, which used to be stated wrongly here: being disabled by a busy flag does NOT announce anything on its own, because aria-busy comes from `loading` and nothing else." },
    { name: 'ariaLabel', description: 'The accessible name, for a button whose children are only an icon. Measured: three <Button>s in the tree wrapped a single icon and nothing else, so they had NO name — axe button-name [critical] on the destructive one (#/knowledge -> Intents delete, both themes). A title alone is not an accessible name in every engine, so this is aria-label. Omit it when the button has visible text: a second string can drift from the one people read.' },
