...
const doc: UiDoc = {
  ...
  description:
    'The Gideon brand mark — the claw silhouette painted with the ACTIVE scheme\'s gradient (reads --grad-1..4, re-tinted per scheme by the appearance store), so the logo tracks Coral/Jade/Lavender/etc. and any custom fork automatically. An optional `blob` halo wraps it in a soft scheme-tinted shape that slowly morphs its border-radius through organic keyframes for the "liquid / alive" thinking feel.',
  props: [
