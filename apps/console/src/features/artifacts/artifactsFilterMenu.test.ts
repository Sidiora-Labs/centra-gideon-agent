...
// sparkline and the brand Gideon mark. Same name, unrelated concepts, nothing to converge. Read both
...
const ARTIFACTS = 'features/artifacts/ArtifactsSection.tsx'
...
describe('the artifacts toolbar renders the canonical Filter & sort pill', () => {
  it('imports the primitive and no longer defines a shadow of it', () => {
    ...
    expect(code, 'the canonical control').toMatch(/import \{ FilterMenu, type FilterSectionDef \} from '\.\.\/\.\.\/shared\/ui\/FilterMenu'/)
    expect(code, 'the page-local shadow is gone, not left beside it').not.toMatch(/function FilterMenu\(/)
    expect(code, 'the hand-rolled trigger/menu imports are gone').not.toMatch(/from '\.\.\/\.\.\/shared\/ui\/Popover'/)
  })
...
describe('the family, so the tenth surface does not hand-roll its own', () => {
  it('nothing outside ui/ defines a component called FilterMenu', () => {
    const offenders = walk(SRC)
      .filter((abs) => !abs.includes(join('src', 'shared', 'ui')))
