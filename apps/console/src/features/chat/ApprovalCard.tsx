...
import { PermissionGrant } from '../../shared/vendor/assistant-ui/elements/permission-grant'
...
export function ApprovalCard({ seg, onAct }: { seg: ApprovalSegment; onAct: (id: string, action: Action, revision?: string) => void }) {
  ...
  return (
    <PermissionGrant
      ...
      onChoice={(choice) => {
        if (choice === 'allow') onAct(seg.id, chosen.action)
        if (choice === 'rejected') onAct(seg.id, 'rejected')
        if (choice === 'revised') onAct(seg.id, 'revised', revision.trim())
      }}
    >
      {!seg.resolved && <div className="flex flex-wrap items-center gap-1.5">
        <span role="alert" data-type="label-s">Permission needed</span>
