...
import { promptProvenance } from './promptMeta'
...
export function selectPromptRows(rows: PromptLibraryRow[], query: string, sort: string, source: string): PromptLibraryRow[] {
  ...
  const comparisons: Record<string, (a: PromptLibraryRow, b: PromptLibraryRow) => number> = {
    ...
    source: (a, b) => promptProvenance(a).localeCompare(promptProvenance(b)),
    vars: (a, b) => (b.variables?.length ?? 0) - (a.variables?.length ?? 0),
  }
  return rows.filter(row => (source === 'all' || promptProvenance(row) === source) && (!needle || [row.name, row.title, row.description, ...(row.tags ?? [])].filter(Boolean).join(' ').toLowerCase().includes(needle))).sort((a, b) => (comparisons[sort]?.(a, b) ?? 0) || compareName(a, b))
}
