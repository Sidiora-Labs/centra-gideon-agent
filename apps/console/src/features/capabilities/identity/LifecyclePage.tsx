...
export default function LifecyclePage({ endpoint = '/api/capabilities/identity/lifecycle' }: { endpoint?: string }) {
  ...
  return <section className="max-w-4xl mx-auto p-4 space-y-4 text-on-surface"><h1 className="text-2xl">Agent lifecycle</h1><p>Bind an existing autonomous loop to this identity. Enable approves its current route; provider readiness remains unknown until actual runtime evidence exists.</p>
    {error && <p role="alert">{error}</p>}{view && <><p>{view.scope}</p><p>Provider readiness: {view.provider_readiness}</p><p role="status">Loop state: {view.loop?.status || 'unbound'}</p>
      <form onSubmit={e => { e.preventDefault(); void perform('/configure', { loop_id: loopId, enabled, expected_revision: view.policy.revision, request_id: requestId }) }}><label className="block" htmlFor="lifecycle-loop">Existing loop ID</label><input id="lifecycle-loop" required value={loopId} disabled={Boolean(view.policy.loop_id)} onChange={e => setLoopId(e.target.value)} /><label className="block"><input type="checkbox" checked={enabled} onChange={e => setEnabled(e.target.checked)} /> Enable bound identity and approve current route</label><Button type="submit" disabled={busy}>Save lifecycle policy</Button></form>
