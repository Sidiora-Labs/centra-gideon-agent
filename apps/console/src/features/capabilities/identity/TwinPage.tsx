  ...
  <section className="mx-auto flex w-full max-w-[72rem] flex-col gap-l px-l py-2xl text-on-surface"><p>Describe yourself in your own words. Private sources stay out of automatic model context.</p>
    ...
    <div className="grid md:grid-cols-2 gap-4">
      <section className="space-y-3 min-w-0"><h2 className="text-xl">Source documents</h2>
        ...
        <form className="space-y-m rounded-lg bg-surface-container p-l" onSubmit={e => { e.preventDefault(); void perform(async () => { const next = await call<Snapshot>('/documents', 'POST', { ...draft, expected_revision: state!.revision }); adopt(next); select(next.documents.find(d => d.id === draft.id) || next.documents[next.documents.length - 1]) }) }}>
          <label className="block" htmlFor="twin-title">Title</label><input className="h-10 w-full min-w-0 rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none transition-colors focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" id="twin-title" required maxLength={200} value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })} />
          <label className="block" htmlFor="twin-text">Source text</label><textarea className="w-full min-w-0 resize-y rounded-md border border-outline-variant/30 bg-surface-container p-m text-on-surface outline-none transition-colors focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" id="twin-text" required rows={6} maxLength={100000} value={draft.text} onChange={e => setDraft({ ...draft, text: e.target.value })} />
          <label className="block"><input type="checkbox" checked={draft.enabled} className="size-4 shrink-0 accent-primary" onChange={e => setDraft({ ...draft, enabled: e.target.checked })} /> Include source</label>
      ...
      <section className="space-y-3 min-w-0"><h2 className="text-xl">Traits and persona overlays</h2>
        <label className="block"><input type="checkbox" checked={enabled} className="size-4 shrink-0 accent-primary" onChange={e => setEnabled(e.target.checked)} /> Use identity in private conversations</label>
        <label className="block" htmlFor="twin-traits">Traits (JSON object)</label><textarea className="w-full min-w-0 resize-y rounded-md border border-outline-variant/30 bg-surface-container p-m text-on-surface outline-none transition-colors focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" id="twin-traits" rows={4} value={traits} onChange={e => setTraits(e.target.value)} />
