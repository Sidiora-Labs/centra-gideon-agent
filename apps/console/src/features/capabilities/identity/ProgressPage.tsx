...
export default function ProgressPage({ endpoint = '/api/capabilities/identity/progress' }: { endpoint?: string }) {
  ...
  return <WorkbenchLayout topBar={<TopBar keepCornerPadding left={<PageTitle>Human progress</PageTitle>} />}>
  <section className="mx-auto flex w-full max-w-[56rem] flex-col gap-l px-l py-2xl text-on-surface">
    ...
    {sheet && <><section aria-label="Progress summary"><h2 data-type="title-l">Current source summary</h2><p>Age: {sheet.age === null ? 'Unknown' : sheet.age}</p><p>Completed goals: {sheet.goals.completed} · Active goals: {sheet.goals.active}</p><p>Completed sessions: {sheet.sessions_completed.length}</p><p>Planned minutes in completed sessions: {sheet.planned_completed_minutes}</p><p>Authored stories: {sheet.authored_story_count}</p><p>Tracked tasks currently done: {sheet.tasks_done}</p><p>{sheet.source_policy}</p></section>
      ...
      <section><h2 data-type="title-l">Tracked native task outcomes</h2>{!sheet.tasks.length && <p>No tracked task outcomes available.</p>}{sheet.tasks.map(row => <p key={row.id}>{row.title} · {row.status} · {row.id}</p>)}{sheet.missing_task_ids.map(id => <p key={id}>Missing source: {id}</p>)}{sheet.invalid_task_ids.map(id => <p key={id}>Unreadable source: {id}</p>)}</section></>}
