...
export default function FidelityPage({ endpoint = '/api/capabilities/identity/fidelity' }: { endpoint?: string }) {
  ...
  return <section className="p-4 max-w-5xl mx-auto space-y-4 text-on-surface">
    <h1 className="text-2xl">Identity fidelity checks</h1>
    ...
    <div className="grid md:grid-cols-2 gap-4">
      <section className="space-y-2"><h2>Cases</h2>{loaded && cases.length === 0 && <p>No fidelity cases yet.</p>}
        ...
        <form className="space-y-2" onSubmit={e => { e.preventDefault(); void perform(async () => {
          ...
          <label className="block" htmlFor="fidelity-question">Question</label><input className="w-full bg-surface-high p-2" id="fidelity-question" required value={prompt} onChange={e => setPrompt(e.target.value)} />
          <label className="block" htmlFor="fidelity-sources">Identity sources</label><select multiple className="w-full bg-surface-high p-2" id="fidelity-sources" required value={sources.split(',').map(x => x.trim())} onChange={e => setSources(Array.from(e.target.selectedOptions, option => option.value).join(','))}>{documents.map(doc => <option key={doc.id} value={doc.id} disabled={doc.private || !doc.enabled}>{doc.title}{doc.private ? ' (private)' : !doc.enabled ? ' (disabled)' : ''}</option>)}</select>
          <label className="block" htmlFor="fidelity-rule">Expectation</label><select id="fidelity-rule" value={rule} onChange={e => setRule(e.target.value)}><option value="contains">Contains</option><option value="not_contains">Does not contain</option><option value="equals">Equals</option></select>
          <label className="block" htmlFor="fidelity-value">Expected text</label><input className="w-full bg-surface-high p-2" id="fidelity-value" required value={value} onChange={e => setValue(e.target.value)} />
          <Button type="submit" disabled={busy}>Save case</Button>
        </form>
      </section>
      <section className="space-y-2"><h2>Observed answer</h2><label className="block" htmlFor="fidelity-answer">Supplied answer</label><textarea className="w-full bg-surface-high p-2" id="fidelity-answer" rows={6} value={answer} onChange={e => setAnswer(e.target.value)} />
    ...
    <section aria-label="Evaluation history"><h2>Evaluation history</h2>{loaded && runs.length === 0 && <p>No evaluations recorded.</p>}
      {runs.map(run => <article key={run.id} className="border-b border-outline py-3"><p>{run.origin === 'supplied_observation' ? 'Supplied observation' : 'Provider run'} · {run.status} · {run.passed === null ? 'Not scored' : run.passed ? 'Literal rules passed' : 'Literal rules failed'}</p>
