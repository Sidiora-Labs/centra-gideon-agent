...
export default function BundlesPage({ endpoint = '/api/capabilities/identity/bundles' }: { endpoint?: string }) {
  ...
  return <section className="p-4 max-w-4xl mx-auto space-y-4 text-on-surface"><h1 className="text-2xl">Encrypted identity bundles</h1>
    ...
    {inventory && <fieldset className="space-x-4"><legend>Groups to transfer</legend>{inventory.groups.map(row => <label key={row.id}><input type="checkbox" disabled={row.available === false} checked={groups.includes(row.id)} onChange={e => { setGroups(e.target.checked ? [...groups, row.id] : groups.filter(value => value !== row.id)); setPreview(null); setReceipt(null) }} /> {row.id} ({row.count} records) {row.unavailable}</label>)}</fieldset>}
