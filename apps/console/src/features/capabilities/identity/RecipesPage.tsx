...
export default function RecipesPage({ endpoint = '/api/capabilities/identity/recipes' }: { endpoint?: string }) {
  ...
  return <section className="p-4 max-w-5xl mx-auto space-y-4 text-on-surface"><h1 className="text-2xl">Bounded read recipes</h1><p>Compose up to five existing identity read operations. Each step rechecks access and the saved revision. Editing or disabling a recipe revokes its pending runs.</p>
    ...
    <form className="space-y-3" onSubmit={e => { e.preventDefault(); void perform(async () => { const saved = await call<Recipe>('', { title: recipe.title, enabled: recipe.enabled, ...(recipe.id ? { id: recipe.id } : {}), expected_revision: recipe.revision || 0, request_id: request, steps: steps.map(step => ({ ...step, arguments: JSON.parse(step.arguments) })) }); setRequest(crypto.randomUUID()); await choose(saved); await load() }) }}>
      <label className="block" htmlFor="recipe-title">Recipe title</label><input className="w-full bg-surface-high p-2" id="recipe-title" required value={recipe.title} onChange={e => setRecipe({ ...recipe, title: e.target.value })} />
      <label><input type="checkbox" checked={recipe.enabled} onChange={e => setRecipe({ ...recipe, enabled: e.target.checked })} /> Recipe enabled</label>
      {steps.map((step, index) => <fieldset key={index} className="border border-outline p-3"><legend>Step {index + 1}</legend>
        <label className="block" htmlFor={'step-id-' + index}>Output name {index + 1}</label><input id={'step-id-' + index} value={step.id} onChange={e => setSteps(steps.map((row, i) => i === index ? { ...row, id: e.target.value } : row))} />
        <label className="block" htmlFor={'step-tool-' + index}>Read operation {index + 1}</label><select id={'step-tool-' + index} value={step.tool} onChange={e => setSteps(steps.map((row, i) => i === index ? { ...row, tool: e.target.value } : row))}>{tools.map(tool => <option key={tool}>{tool}</option>)}</select>
        <label className="block" htmlFor={'step-args-' + index}>Arguments and bindings {index + 1} (JSON)</label><textarea id={'step-args-' + index} className="w-full bg-surface-high p-2" value={step.arguments} onChange={e => setSteps(steps.map((row, i) => i === index ? { ...row, arguments: e.target.value } : row))} />
    ...
    {recipe.id && <><Button disabled={busy || !recipe.enabled} onClick={() => void perform(async () => { await call('/begin', { recipe_id: recipe.id, revision: recipe.revision, request_id: runRequest }); setRunRequest(crypto.randomUUID()); await load() })}>Begin pinned run</Button>
      <section aria-label="Recipe history"><h2>Revision history</h2>{history.map(row => <p key={row.revision}>Revision {row.revision}: {row.title} {row.revision !== recipe.revision && <Button variant="secondary" disabled={busy} onClick={() => void perform(async () => { await call('/restore', { id: row.id, revision: row.revision, expected_revision: recipe.revision, request_id: request }); setRequest(crypto.randomUUID()); await load() })}>Restore revision {row.revision}</Button>}</p>)}</section></>}
    <section aria-label="Recipe runs"><h2>Runs and actual outcomes</h2>{loaded && !runs.length && <p>No runs yet.</p>}{runs.filter(row => !recipe.id || row.recipe_id === recipe.id).map(run => <article className="border-b border-outline py-3" key={run.id}><p>Run {run.id} · {run.status} · revision {run.recipe_revision} · {run.next_index} steps recorded</p>
