...
export default function AmbientDisplay({ baseUrl = '/api/capabilities/experience', onClose }: { baseUrl?: string; onClose: () => void }) {
  ...
  return <main ref={root} aria-label="Ambient display" className="fixed inset-0 z-50 overflow-auto bg-canvas text-on-surface p-4 sm:p-8" onPointerMove={wake} onPointerDown={wake} onFocus={wake} style={{ fontSize: `${snapshot?.preferences.font_scale || 1}rem` }}>
    <div ref={chrome} className="flex flex-wrap gap-3 items-end" style={idle ? { opacity: 0 } : undefined}>
      ...
      {draft && <form onSubmit={event => { event.preventDefault(); void save() }} className="flex flex-wrap gap-3 items-end">
        <label><input type="checkbox" checked={draft.show_clock} onChange={event => setDraft({ ...draft, show_clock: event.target.checked })} /> Show clock</label>
        <label>Text size<select value={draft.font_scale} onChange={event => setDraft({ ...draft, font_scale: Number(event.target.value) })}>{[1, 2, 3].map(value => <option key={value} value={value}>{value}</option>)}</select></label>
        <label>Hide controls after seconds<input type="number" min={10} max={300} value={draft.idle_seconds} onChange={event => setDraft({ ...draft, idle_seconds: Number(event.target.value) })} /></label>
    ...
    {snapshot && <><p>Last observed: <time dateTime={snapshot.observed_at}>{snapshot.observed_at}</time></p><div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
      {(['schedule', 'work'] as const).map(name => <section key={name} aria-label={name === 'schedule' ? 'Scheduled automations' : 'Native tasks'} className="border rounded-xl p-4 min-w-0">
        <h2>{name === 'schedule' ? 'Scheduled automations' : 'Native tasks'}</h2>
