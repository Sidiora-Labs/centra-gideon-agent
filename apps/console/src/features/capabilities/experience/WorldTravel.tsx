...
export default function WorldTravel({ baseUrl, world, open }: { baseUrl: string; world: string; open: boolean }) {
  ...
  if (ticket) return <section aria-label="Guest world travel" className="space-y-3 rounded border p-4">
    <h3>Guest world visit</h3>{busy && <p role="status">Opening scoped guest invitation…</p>}{error && <p role="alert">{error}</p>}
  ...
  return <section aria-label="World travel" className="space-y-3 rounded border p-4">
    <h3>World travel</h3><p>Travel only to enabled canonical peers that explicitly allow scoped world guests.</p>
    ...
    <label>Guest name<input maxLength={64} value={name} onChange={event => setName(event.target.value)} /></label>
    {destinations.map(destination => <Button key={destination.id} disabled={busy || !open || !identifier.test(world) || !name.trim()} onClick={() => void depart(destination.id)}>Visit {destination.label}</Button>)}
