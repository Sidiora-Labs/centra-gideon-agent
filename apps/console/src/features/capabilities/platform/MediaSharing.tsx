...
export default function MediaSharing({ baseUrl = '' }: { baseUrl?: string }) {
  ...
  return <section aria-label="Selective media sharing" className="space-y-m">
    <h2>Selective media sharing</h2>
    ...
    {data && <>
      ...
      <div className="grid gap-m sm:grid-cols-2"><label>Peer ID<input value={peer} onChange={event => setPeer(event.target.value)} /></label><label>Canonical artifact ID<input value={artifact} onChange={event => setArtifact(event.target.value)} /></label><label>Exact artifact version<input type="number" min="1" value={version} onChange={event => setVersion(Number(event.target.value))} /></label></div>
      ...
      <h3>Sent shares</h3>{!data.outbound.length && <p>No media has been shared.</p>}
      ...
      <h3>Received shares</h3>{!data.inbound.length && <p>No peer media received.</p>}
