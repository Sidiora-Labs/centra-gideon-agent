...
export default function Replication({ baseUrl = '' }: { baseUrl?: string }) {
  ...
  return <section aria-label="Domain replication" className="grid gap-l">
    ...
    {data && <><div className="flex flex-wrap gap-m"><label className="grid gap-xs text-sm">Domain<select className="min-h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-sm text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Replication domain" value={domain} onChange={event => { setDomain(event.target.value); setPeer('') }}>{data.domains.map(item => <option key={item.scope}>{item.scope}</option>)}</select></label>
      <label className="grid gap-xs text-sm">Peer<select className="min-h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-sm text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Replication peer" value={peer} onChange={event => setPeer(event.target.value)}><option value="">Choose peer</option>{eligible.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label></div>
