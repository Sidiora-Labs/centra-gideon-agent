...
export default function Comparisons({ baseUrl = '' }: { baseUrl?: string }) {
  ...
  return <section aria-label="Model comparisons" className="grid gap-l">
    ...
    <div className="grid gap-s sm:grid-cols-2">{fields.map(field => <label className="grid gap-xs text-sm" key={field}>{field.replace('_', ' ')}<input className="min-h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-sm outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label={`Observation ${field}`} type={field === 'observed_at' ? 'date' : 'text'} value={draft[field] || ''} onChange={event => setDraft({ ...draft, [field]: event.target.value })} /></label>)}</div>
