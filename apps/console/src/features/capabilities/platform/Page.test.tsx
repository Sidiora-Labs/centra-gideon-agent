...
function show(entry = '/capabilities/platform', origin = baseUrl) {
  return render(<MemoryRouter initialEntries={[entry]}><Page baseUrl={origin} /><Location /></MemoryRouter>)
}
...
describe('API explorer over real dashboard HTTP', () => {
  it('loads registered methods, filters and preserves URL selection', async () => {
    ...
    expect(screen.getByText('4 registered routes; page starts at 1.')).toBeVisible()
    ...
    expect(screen.queryByRole('button', { name: 'HEAD /api/prompts/syntax' })).toBeNull()
    ...
    expect(screen.queryByRole('button', { name: 'GET /api/capabilities/platform/catalog' })).toBeNull()
    fireEvent.click(syntax)
    expect(screen.getByRole('region', { name: 'Route detail' })).toHaveTextContent('api_prompt_syntax')
  ...
  it('disables unapproved methods and renders empty filters', async () => {
    ...
    expect(screen.getByText('No routes match this page.')).toBeVisible()
