...
export default function Cadence({ baseUrl = '' }: { baseUrl?: string }) {
  ...
  return <section aria-label="Adaptive cadence" className="grid gap-l"><h2 data-type="title-m">Task class cadence</h2><p>Opt in interval triggers to share typed execution evidence. Five observations are required; low success slows cadence up to four times. Three recent successes or a day without evidence restore a normal probe.</p>{error && <p role="alert">{error}</p>}
    {view?.triggers.map(row => <article className="grid gap-s rounded-lg border border-outline-variant/20 bg-surface-container p-l" key={row.trigger_id} aria-label={row.name}><h3 data-type="headline-s">{row.name}</h3><p>{row.base_interval}s base → {row.effective_interval}s effective</p><p>{row.successes}/{row.samples} successful · {row.excluded} excluded · {row.confidence} · {row.reason}</p><label className="grid gap-xs text-sm">Task class<input className="min-h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-sm text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label={`Task class ${row.name}`} value={classes[row.trigger_id] ?? row.task_class ?? ''} onChange={event => setClasses({ ...classes, [row.trigger_id]: event.target.value })} /></label><Button disabled={busy} onClick={() => void save(row, true)}>Enable cadence {row.name}</Button><Button disabled={busy || !row.task_class} onClick={() => void save(row, false)}>Disable cadence {row.name}</Button><details><summary>Execution evidence</summary>{row.evidence.map(item => <p key={item.trigger_id + item.run_id}>{item.trigger_id} / {item.run_id}: {item.success ? 'success' : 'task failure'}</p>)}</details></article>)}
  </section>
}
