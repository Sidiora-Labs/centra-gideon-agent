...
export function CoreWidgets({ tiles, route }: { tiles: CoreTile[]; route: RouteProps }) {
  ...
  return <div className="grid grid-cols-1 gap-l lg:grid-cols-2" data-testid="core-composition">{[...tiles].sort((a, b) => a.order - b.order).filter(tile => tile.ref in widgets).map(tile => <section key={tile.ref} data-core-ref={tile.ref} data-core-size={tile.size} className={`min-w-0 ${tile.size === 'full' || tile.size === 'l' ? 'lg:col-span-2' : ''} ${tile.size === 's' ? 'max-w-sm' : tile.size === 'l' ? 'max-w-5xl' : ''}`}><h2 data-type="title-m">{tile.ref.slice(5).replaceAll('-', ' ')}</h2>{widgets[tile.ref as keyof typeof widgets]}</section>)}</div>
}
