...
export function BeenVerifiedBrokerPanel({ brokerCase, onChanged }: { brokerCase: Case; onChanged: (row: Case) => void }) {
  ...
  return <section aria-label="BeenVerified deletion email" className="space-y-3 rounded-lg bg-surface-container p-l">
    ...
    {!draft && <div className="grid gap-m sm:grid-cols-2">
      <label>Sending account<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" value={accountId} onChange={event => { const id=event.target.value; setAccountId(id); setContactEmail(accounts.find(row=>row.id===id)?.owner_email || '') }}><option value="">Select connected account</option>{accounts.map(row=><option key={row.id} value={row.id}>{row.name} — {row.owner_email}</option>)}</select></label>
      ...
      <label>Jurisdiction<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" value={jurisdiction} onChange={event=>setJurisdiction(event.target.value)}><option value="US-OTHER">US other</option><option value="US-CA">California</option></select></label>
    ...
    {draft && <article className="space-y-2"><h5>Exact email</h5><p>From: {draft.sender}</p><p>To: {draft.to.join(', ')}</p><p>Subject: {draft.subject}</p><pre className="whitespace-pre-wrap">{draft.body}</pre><p>Content SHA-256: {draft.content_sha256}</p><p>Draft state: {draft.state}; provider acceptance: {draft.provider_acceptance}; delivery: {draft.delivery}</p>
      {draft.state==='draft' && <><label><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={approve} onChange={event=>setApprove(event.target.checked)} /> I approve this exact BeenVerified deletion recipient and content</label><Button disabled={busy || !approve} onClick={() => void run(async () => await requestJson<Result>(base+'/approve','POST',{revision:brokerCase.revision,draft_revision:draft.revision,content_sha256:draft.content_sha256,confirm_exact:true}))}>Approve exact BeenVerified deletion email</Button></>}
      {draft.state==='approved' && <><label><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={confirmSend} onChange={event=>setConfirmSend(event.target.checked)} /> Dispatch this approved BeenVerified deletion email</label><Button disabled={busy || !confirmSend} onClick={() => void run(async () => await requestJson<Result>(base+'/send','POST',{request_id:crypto.randomUUID(),revision:brokerCase.revision,draft_revision:draft.revision,content_sha256:draft.content_sha256,confirm_send:true}))}>Send approved BeenVerified deletion email</Button></>}
