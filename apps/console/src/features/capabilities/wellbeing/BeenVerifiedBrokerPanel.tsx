...
export function BeenVerifiedBrokerPanel({ brokerCase, onChanged }: { brokerCase: Case; onChanged: (row: Case) => void }) {
  ...
  return <section aria-label="BeenVerified deletion email" className="space-y-3 rounded border p-3">
    ...
    {draft && <article className="space-y-2"><h5>Exact email</h5><p>From: {draft.sender}</p><p>To: {draft.to.join(', ')}</p><p>Subject: {draft.subject}</p><pre className="whitespace-pre-wrap">{draft.body}</pre><p>Content SHA-256: {draft.content_sha256}</p><p>Draft state: {draft.state}; provider acceptance: {draft.provider_acceptance}; delivery: {draft.delivery}</p>
      {draft.state==='draft' && <><label><input type="checkbox" checked={approve} onChange={event=>setApprove(event.target.checked)} /> I approve this exact BeenVerified deletion recipient and content</label><Button disabled={busy || !approve} onClick={() => void run(async () => await requestJson<Result>(base+'/approve','POST',{revision:brokerCase.revision,draft_revision:draft.revision,content_sha256:draft.content_sha256,confirm_exact:true}))}>Approve exact BeenVerified deletion email</Button></>}
      {draft.state==='approved' && <><label><input type="checkbox" checked={confirmSend} onChange={event=>setConfirmSend(event.target.checked)} /> Dispatch this approved BeenVerified deletion email</label><Button disabled={busy || !confirmSend} onClick={() => void run(async () => await requestJson<Result>(base+'/send','POST',{request_id:crypto.randomUUID(),revision:brokerCase.revision,draft_revision:draft.revision,content_sha256:draft.content_sha256,confirm_send:true}))}>Send approved BeenVerified deletion email</Button></>}
