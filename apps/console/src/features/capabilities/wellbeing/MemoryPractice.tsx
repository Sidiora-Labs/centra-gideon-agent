...
function Editor({ card, saved }: { card: Card | null; saved: (id: string) => void }) {
  ...
  return <form onSubmit={submit} className="space-y-3"><h2 data-type="title-m">{card ? 'Edit memory card' : 'New memory card'}</h2><Field label="Memory prompt"><TextArea value={front} onChange={setFront} /></Field><Field label="Memory answer"><TextArea value={back} onChange={setBack} /></Field><Field label="Memory source"><TextInput value={source} onChange={setSource} disabled={!!card} required /></Field><Field label="Tags (comma separated)"><TextInput value={tags} onChange={setTags} /></Field>{card && <label><input type="checkbox" checked={archived} onChange={e => setArchived(e.target.checked)} />Archived memory card</label>}{error && <p role="alert">{error}</p>}<Button type="submit" loading={busy}>Save memory card</Button></form>
}
...
export default function MemoryPractice() {
  ...
  return <main className="h-full overflow-auto p-4 sm:p-6 space-y-6 text-on-surface"><h1 data-type="headline-s">Spaced memory practice</h1><p>Reveal the answer, then report your recall honestly. Version 1 scheduling uses elapsed UTC days and self-reported grades.</p><Button onClick={() => setQuery({ card: null, mode: null })}>New memory card</Button><div className="flex flex-wrap gap-4"><label><input type="checkbox" checked={dueOnly} onChange={e => setDueOnly(e.target.checked)} />Due cards only</label><label><input type="checkbox" checked={includeArchived} onChange={e => setIncludeArchived(e.target.checked)} />Include archived memory cards</label></div>{error && <p role="alert">{error} <Button onClick={() => setGeneration(n => n + 1)}>Reload cards</Button></p>}{loading && <p role="status">Loading memory cards…</p>}
    <section aria-label="Memory card list">{cards.map(card => <button key={card.id} className="block rounded-lg bg-surface-container p-3 my-2 text-left" onClick={() => setQuery({ card: card.id, mode: null })}>{card.front}{card.archived ? ' (archived)' : ''}</button>)}{!loading && !cards.length && <p>No matching memory cards.</p>}</section>
    ...
    {selected && !edit && <section className="space-y-3"><h2 data-type="title-m">Recall prompt</h2><p className="whitespace-pre-wrap break-words">{selected.front}</p><p>Source: {selected.source} · {selected.tags.join(', ')}</p><p>Due: {selected.schedule.due_at}</p><p>Interval: {selected.schedule.interval_days} days · Successful repetitions: {selected.schedule.repetitions} · Lapses: {selected.schedule.lapses}</p><p>Content artifact: {selected.artifact.slug} · version {selected.artifact.version}</p><Button variant="secondary" onClick={() => setQuery({ mode: 'edit' })}>Edit selected card</Button>{selected.archived ? <p>This card is archived.</p> : <><Button onClick={() => setRevealed(true)} disabled={revealed}>Reveal memory answer</Button>{revealed && <section aria-label="Revealed answer"><p className="whitespace-pre-wrap break-words">{selected.back}</p><div className="flex flex-wrap gap-2">{['again', 'hard', 'good', 'easy'].map(value => <Button key={value} disabled={busy} onClick={() => grade(value)}>{value}</Button>)}</div></section>}</>}</section>}
    {!!history.length && <section aria-label="Memory history"><h2 data-type="title-m">Memory history</h2>{history.map(row => <p key={row.revision}>Revision {row.revision}: {row.practice ? `Self-grade ${row.practice.grade}` : 'Content or archive change'} · due {row.schedule.due_at} · content {row.artifact.sha256.slice(0, 12)}</p>)}</section>}
  </main>
}
