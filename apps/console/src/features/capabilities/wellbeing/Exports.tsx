...
export default function Exports() {
  ...
  return <main style={{ maxWidth: 'var(--content-width)' }} className="mx-auto w-full space-y-2xl px-l py-2xl text-on-surface"><h2 data-type="title-m">Versioned wellbeing exports</h2><p>Save a point-in-time JSON archive of records, revision history and referenced original attachments. Existing archives remain unchanged when records change. This export does not restore data or include external calendar triggers and inbox items.</p>{error && <p role="alert">{error}</p>}<Button onClick={() => setGeneration(n => n + 1)} disabled={loading}>Refresh export preview</Button>{loading && <p role="status">Loading export data…</p>}
    {preview && <section aria-label="Export preview" className="space-y-l rounded-lg bg-surface-container p-l"><h2 data-type="title-m">Current snapshot preview</h2><p>Schema {preview.schema} · version {preview.version}</p><dl>{Object.entries(preview.counts).map(([domain, count]) => <div key={domain}><dt>{domain}</dt><dd>{count} history and provenance records</dd></div>)}</dl><p>{preview.attachment_count} source attachments · {preview.attachment_bytes} source bytes · {preview.export_bytes} archive bytes</p><p>Excluded: {preview.omitted.join(', ')}</p><Button onClick={create} disabled={loading} loading={busy}>Create immutable export</Button></section>}
    <section aria-label="Saved exports" className="space-y-l rounded-lg bg-surface-container p-l"><h2 data-type="title-m">Saved exports</h2>{archives.map(archive => <button key={archive.id} className="block rounded-lg border border-outline-variant/20 bg-surface-container p-l my-s text-left" onClick={() => setQuery({ export: archive.id })}>Export {archive.generated_at} · {archive.bytes} bytes</button>)}{!loading && !archives.length && <p>No saved exports.</p>}</section>
    {selected && <section aria-label="Selected export" className="space-y-l rounded-lg bg-surface-container p-l"><h2 data-type="title-m">Selected immutable export</h2><p>Created {selected.generated_at} · schema version {selected.version}</p><p className="break-all">SHA-256: {selected.sha256}</p><p>{selected.attachment_count} attachments · {selected.bytes} bytes</p><a className="underline" href={`${base}/${selected.id}/download`} download={`wellbeing-${selected.id}.json`}>Download selected JSON export</a></section>}
  </main>
}
