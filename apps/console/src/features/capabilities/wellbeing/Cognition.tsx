...
export default function Cognition() {
  ...
  return <main style={{ maxWidth: 'var(--content-width)' }} className="mx-auto w-full space-y-2xl px-l py-2xl text-on-surface"><h2 data-type="title-m">Timed cognitive practice</h2><p>Server-observed response times include network latency. Scores describe this practice session and provide no clinical interpretation.</p><Button onClick={() => { receipt.current = { fingerprint: '', id: '' }; setQuery({ session: null }) }}>New exercise</Button>{error && <p role="alert">{error} <Button onClick={() => setGeneration(value => value + 1)}>Reload exercise</Button></p>}{loading && <p role="status">Loading exercise…</p>}
    {!identity && <form onSubmit={e => { e.preventDefault(); void write('start') }} className="space-y-m"><h2 data-type="title-m">Start a session</h2><label className="block">Exercise<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Exercise kind" value={kind} onChange={e => setKind(e.target.value)}><option value="arithmetic">Arithmetic</option><option value="color_word">Color word</option></select></label><Field label="Number of trials (1–20)"><TextInput value={trials} onChange={setTrials} /></Field><Field label="Time limit in seconds (1–600)"><TextInput value={limit} onChange={setLimit} /></Field><Button type="submit" loading={busy}>Start exercise</Button></form>}
    {selected && <section className="space-y-l"><h2 data-type="title-m">Session: {selected.status}</h2><p>{selected.kind} · {selected.planned_trials} planned trials · Deadline {selected.deadline}</p>{trial && <section className="space-y-m" aria-label="Current trial"><h3 data-type="title-m">Trial {trial.index}</h3>{selected.kind === 'arithmetic' ? <form onSubmit={submit} className="space-y-m"><p aria-label="Arithmetic question" data-left={trial.stimulus.left} data-right={trial.stimulus.right} data-operator={trial.stimulus.operator}>{trial.stimulus.left} {trial.stimulus.operator} {trial.stimulus.right} = ?</p><Field label="Your arithmetic answer"><TextInput value={answer} onChange={setAnswer} required /></Field><Button type="submit" loading={busy}>Submit answer</Button></form> : <><p>Select the ink color, ignoring the written word.</p><p aria-label="Color word stimulus" data-color={trial.stimulus.color} style={{ color: colors[trial.stimulus.color!] }} className="text-4xl font-bold">{trial.stimulus.word}</p><div className="flex gap-s flex-wrap">{Object.keys(colors).map(color => <Button key={color} disabled={busy} onClick={() => write('answer', color)}>{color}</Button>)}</div></>}</section>}{selected.status === 'active' && <Button variant="danger" loading={busy} onClick={() => write('cancel')}>Cancel exercise</Button>}<section aria-label="Session score" className="space-y-l rounded-lg bg-surface-container p-l"><p>Answered: {selected.score.answered} · Correct: {selected.score.correct}</p><p>Accuracy: {selected.score.accuracy === null ? 'Not recorded' : `${Math.round(selected.score.accuracy * 100)}%`}</p><p>Mean response time: {selected.score.mean_elapsed_ms === null ? 'Not recorded' : `${Math.round(selected.score.mean_elapsed_ms)} ms`}</p><p>Total recorded response time: {Math.round(selected.score.total_elapsed_ms)} ms</p></section><section aria-label="Recorded trials" className="space-y-l rounded-lg bg-surface-container p-l">{selected.trials.map(row => <p key={row.index}>Trial {row.index}: {row.answer} · {row.correct ? 'correct' : 'incorrect'} · {Math.round(row.elapsed_ms!)} ms</p>)}</section></section>}
