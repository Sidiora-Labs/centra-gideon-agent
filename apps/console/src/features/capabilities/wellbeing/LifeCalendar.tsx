...
function ConfigEditor({ config, saved }: { config: Config | null; saved: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-3"><h2 data-type="title-m">Projection assumptions</h2><Field label="Birth date"><TextInput value={birth} onChange={setBirth} placeholder="YYYY-MM-DD" required /></Field><Field label="Declared horizon in years"><TextInput value={years} onChange={setYears} required /></Field><Field label="Assumed sleep hours per day"><TextInput value={sleep} onChange={setSleep} required /></Field><Field label="Life calendar timezone"><TextInput value={zone} onChange={setZone} required /></Field><Field label="Assumption source"><TextInput value={source} onChange={setSource} required /></Field>{budgets.map((budget, index) => <div key={index} className="grid gap-2 sm:grid-cols-3"><Field label={`Activity ${index + 1}`}><TextInput value={budget.name} onChange={name => setBudgets(rows => rows.map((row, i) => i === index ? { ...row, name } : row))} /></Field><Field label={`Weekly hours ${index + 1}`}><TextInput value={String(budget.hours_per_week)} onChange={value => setBudgets(rows => rows.map((row, i) => i === index ? { ...row, hours_per_week: Number(value) } : row))} /></Field><Button variant="secondary" onClick={() => setBudgets(rows => rows.filter((_, i) => i !== index))}>Remove activity {index + 1}</Button></div>)}<Button variant="secondary" onClick={() => setBudgets(rows => [...rows, { name: '', hours_per_week: 0 }])}>Add activity budget</Button><label className="block"><input type="checkbox" checked={enabled} onChange={e => setEnabled(e.target.checked)} />Enable daily completion reminder</label><Field label="Reminder local time"><TextInput value={time} onChange={setTime} placeholder="HH:MM" /></Field><p>A local inbox reminder is suppressed when a cognitive practice session is completed that day.</p>{error && <p role="alert">{error}</p>}<Button type="submit" loading={busy}>Save projection assumptions</Button></form>
}
...
function EventEditor({ row, saved }: { row: LifeEvent | null; saved: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-3"><h2 data-type="title-m">{row ? 'Edit life event' : 'New life event'}</h2><Field label="Life event date"><TextInput value={day} onChange={setDay} placeholder="YYYY-MM-DD" required /></Field><Field label="Life event title"><TextInput value={title} onChange={setTitle} required /></Field><Field label="Life event notes"><TextInput value={notes} onChange={setNotes} /></Field><Field label="Life event source"><TextInput value={source} onChange={setSource} disabled={!!row} required /></Field><label className="block">Event kind<select aria-label="Life event kind" value={kind} onChange={e => setKind(e.target.value)}><option value="recorded">Recorded</option><option value="planned">Planned</option></select></label>{row && <label><input type="checkbox" checked={deleted} onChange={e => setDeleted(e.target.checked)} />Delete life event (keep history)</label>}{error && <p role="alert">{error}</p>}<Button type="submit" loading={busy}>Save life event</Button></form>
}
...
export default function LifeCalendar() {
  ...
  return <main className="h-full overflow-auto p-4 sm:p-6 space-y-6 text-on-surface"><h1 data-type="headline-s">Life calendar</h1><p>Your declared horizon is a planning assumption, not a lifespan prediction. Sleep and activity totals are projections, not measured observations.</p>{error && <p role="alert">{error}</p>}{loading && <p role="status">Loading life calendar…</p>}{!loading && <ConfigEditor key={config?.revision ?? 0} config={config} saved={() => setGeneration(n => n + 1)} />}
    ...
    <section className="space-y-3"><h2 data-type="title-m">Life events</h2><Button onClick={() => setQuery({ event: null })}>New life event</Button>{events.map(row => <button key={row.id} className="block rounded-lg bg-surface-container p-3 text-left" onClick={() => setQuery({ event: row.id })}>{row.date}: {row.title} · {row.kind}</button>)}{!loading && !events.length && <p>No life events.</p>}{!loading && (!identity || selected) && <EventEditor key={selected ? `${selected.id}:${selected.revision}` : generation} row={selected} saved={() => { setQuery({ event: null }); setGeneration(n => n + 1) }} />}{!!history.length && <section aria-label="Life event history">{history.map(row => <p key={row.revision}>Revision {row.revision}: {row.title} · {row.date}{row.deleted ? ' · deleted' : ''}</p>)}</section>}</section>
  </main>
}
