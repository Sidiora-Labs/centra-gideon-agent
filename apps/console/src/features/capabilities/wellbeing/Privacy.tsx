...
export default function Privacy() {
  ...
  return <main style={{ maxWidth: 'var(--content-width)' }} className="mx-auto w-full space-y-2xl px-l py-2xl text-on-surface"><h2 data-type="title-m">Private identity facts</h2><p>Facts are encrypted locally. Enter the same passphrase for every correction or reveal; Gideon does not keep it. Aliases, labels and source descriptions are visible metadata: use generic descriptions. Facts are excluded from health exports, agent tools and automatic sharing.</p>{error && <p role="alert">{error}</p>}{loading && <p role="status">Loading private records…</p>}
    <section aria-label="Privacy subjects" className="space-y-l rounded-lg bg-surface-container p-l"><h2 data-type="title-m">Subjects</h2>{subjects.map(row => <Button key={row.id} variant="secondary" onClick={() => setQuery({ subject: row.id, fact: null })}>{row.alias}</Button>)}</section>
    <form onSubmit={event => mutate('subject', event)} className="space-y-m"><h2 data-type="title-m">Create subject</h2><Field label="Subject alias"><TextInput value={alias} onChange={setAlias} required /></Field><label>Relationship<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Relationship" value={relationship} onChange={event => setRelationship(event.target.value)}>{['self', 'household', 'other'].map(item => <option key={item}>{item}</option>)}</select></label><Field label="Subject source"><TextInput value={subjectSource} onChange={setSubjectSource} required /></Field><Button type="submit" disabled={busy}>Create privacy subject</Button></form>
    {query.subject && !loading && <><form onSubmit={event => mutate('consent', event)} className="space-y-m"><h2 data-type="title-m">Explicit scoped consent</h2><label>Consent scope<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Consent scope" value={scope} onChange={event => setScope(event.target.value)}>{['vault', 'reveal', 'broker_scan', 'broker_submit', 'twin_share'].map(item => <option key={item}>{item}</option>)}</select></label><label><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={granted} onChange={event => setGranted(event.target.checked)} />Grant consent</label><Field label="Consent method"><TextInput value={method} onChange={setMethod} required /></Field><Button type="submit" disabled={busy}>Record consent decision</Button><ul>{consents.map(row => <li key={`${row.scope}:${row.revision}`}>{row.scope} revision {row.revision}: {row.granted ? 'granted' : 'revoked'} · {row.method}</li>)}</ul></form>
      <section aria-label="Masked facts" className="space-y-l rounded-lg bg-surface-container p-l"><h2 data-type="title-m">Masked facts</h2><Button onClick={() => setQuery({ fact: null })}>New private fact</Button>{facts.map(row => <button key={row.id} className="block rounded-lg border border-outline-variant/20 bg-surface-container p-l my-s text-left" onClick={() => setQuery({ fact: row.id })}>{row.label} · {row.masked_value} · revision {row.revision}{row.archived ? ' · archived' : ''}</button>)}</section>
      <form onSubmit={event => mutate('fact', event)} className="space-y-m"><h2 data-type="title-m">{selected ? 'Correct selected private fact' : 'Create encrypted fact'}</h2><label>Fact type<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Fact type" value={type} disabled={!!selected} onChange={event => setType(event.target.value)}>{['legal_name', 'email', 'phone', 'address', 'birth_date', 'tax_id', 'passport', 'other'].map(item => <option key={item}>{item}</option>)}</select></label><Field label="Fact label"><TextInput value={label} onChange={setLabel} required /></Field><Field label="Fact source"><TextInput value={source} onChange={setSource} required disabled={!!selected} /></Field><Field label="Private value"><TextInput type="password" value={value} onChange={setValue} required /></Field><Field label="Encryption passphrase"><TextInput type="password" value={password} onChange={setPassword} required /></Field><label><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={scans} onChange={event => setScans(event.target.checked)} />Allow use in explicitly consented scans</label>{selected && <label><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={archived} onChange={event => setArchived(event.target.checked)} />Archive private fact</label>}<Button type="submit" disabled={busy}>Save encrypted fact</Button></form>
      {selected && <form onSubmit={event => mutate('reveal', event)} className="space-y-m"><h2 data-type="title-m">Explicit reveal</h2><Field label="Reveal passphrase"><TextInput type="password" value={revealPassword} onChange={setRevealPassword} required /></Field><Field label="Reveal reason"><TextInput value={reason} onChange={setReason} required /></Field><Button type="submit" disabled={busy || selected.archived}>Reveal selected fact</Button>{revealed && <section aria-label="Revealed private value" className="space-y-l rounded-lg bg-surface-container p-l"><p>{revealed}</p><Button onClick={() => setRevealed('')}>Hide revealed value</Button></section>}</form>}
      {!!history.length && <section aria-label="Private fact history" className="space-y-l rounded-lg bg-surface-container p-l">{history.map(row => <p key={row.revision}>Revision {row.revision}: {row.label} · {row.masked_value}</p>)}</section>}<PrivacyOrganizations subject={query.subject} facts={facts} /><PrivacyBrokers subject={query.subject} /><section aria-label="Privacy audit" className="space-y-l rounded-lg bg-surface-container p-l"><h2 data-type="title-m">Privacy audit</h2>{audit.map(row => <p key={row.id}>{row.operation} {row.outcome ?? ''} · {row.at}</p>)}</section></>}
  </main>
}
