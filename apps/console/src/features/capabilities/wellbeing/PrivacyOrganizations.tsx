...
export default function PrivacyOrganizations({ subject, facts }: { subject: string; facts: PrivateFactMetadata[] }) {
  ...
  return <section aria-label="Organizations and change propagation" className="space-y-2xl">
    ...
    <div className="flex flex-wrap gap-s"><Button variant="secondary" onClick={() => setQuery({ org: null, change: null })}>New organization</Button>{orgs.map(row => <Button key={row.id} variant="secondary" onClick={() => setQuery({ org: row.id, change: null })}>{row.name} · revision {row.revision}</Button>)}</div>
    <form onSubmit={saveOrg} className="grid gap-m sm:grid-cols-2"><h3 className="sm:col-span-2">{selectedOrg ? 'Edit organization' : 'Record organization'}</h3><Field label="Organization name"><TextInput value={name} onChange={setName} required /></Field><Field label="Organization category"><TextInput value={category} onChange={setCategory} required /></Field><Field label="Organization website"><TextInput value={website} onChange={setWebsite} /></Field><Field label="Organization contact"><TextInput value={contact} onChange={setContact} /></Field><Field label="Organization source"><TextInput value={source} onChange={setSource} required disabled={!!selectedOrg} /></Field>{selectedOrg && <label><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={archived} onChange={event => setArchived(event.target.checked)} />Archive organization</label>}<Button type="submit" disabled={busy}>Save organization</Button></form>
    {!!orgHistory.length && <section aria-label="Organization history" className="space-y-l rounded-lg bg-surface-container p-l">{orgHistory.map(row => <p key={row.revision}>Revision {row.revision}: {row.name} · {row.archived ? 'archived' : 'active'}</p>)}</section>}
    {selectedOrg && <form onSubmit={saveHolding} className="space-y-m"><h3>Record a holding</h3><label>Private fact<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Holding private fact" value={factId} onChange={event => setFactId(event.target.value)}>{facts.filter(row => !row.archived).map(row => <option key={row.id} value={row.id}>{row.label} · {row.masked_value} · revision {row.revision}</option>)}</select></label><label>Holding status<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Holding status" value={holdingStatus} onChange={event => setHoldingStatus(event.target.value)}><option value="held">held</option><option value="unknown">unknown</option><option value="removed">removed</option></select></label><Button type="submit" disabled={busy || !factId}>Save holding</Button><ul>{holdings.map(row => <li key={row.id}>{facts.find(fact => fact.id === row.fact_id)?.label ?? row.fact_id} · revision {row.revision} · {row.status} <Button variant="secondary" onClick={() => void showHoldingHistory(row.id)}>Holding history</Button></li>)}</ul></form>}
    <form onSubmit={declare} className="space-y-m"><h3>Declare a changed fact</h3><label>Changed private fact<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Changed private fact" value={factId} onChange={event => setFactId(event.target.value)}>{facts.filter(row => !row.archived).map(row => <option key={row.id} value={row.id}>{row.label} · current revision {row.revision}</option>)}</select></label><Field label="Previous fact revision"><TextInput value={fromRevision} onChange={setFromRevision} required /></Field><Button type="submit" disabled={busy || !factId}>Start change checklist</Button></form>
    <div className="flex flex-wrap gap-s">{changes.map(row => <Button key={row.id} variant="secondary" onClick={() => setQuery({ change: row.id })}>Revision {row.from_revision} → {row.to_revision}: {row.progress.pending}/{row.progress.total} pending</Button>)}</div>
    {selectedChange && <section aria-label="Change progress" className="space-y-m"><h3>Derived progress</h3><p>{selectedChange.progress.pending} pending · {selectedChange.progress.updated} updated · {selectedChange.progress.removed} removed · {selectedChange.progress.total} total</p><Field label="Disposition evidence"><TextInput value={evidence} onChange={setEvidence} maxLength={2000} /></Field>{selectedChange.targets.map(target => <article key={target.org_id}><p>{orgs.find(row => row.id === target.org_id)?.name ?? target.org_id} · {target.status} · {target.attestation}{target.evidence ? ` · ${target.evidence}` : ''}</p>{target.status === 'pending' && <div className="flex gap-s"><Button onClick={() => settle(target.org_id, 'updated', target.revision)} disabled={busy}>Attest updated</Button><Button variant="secondary" onClick={() => settle(target.org_id, 'removed', target.revision)} disabled={busy}>Attest removed</Button></div>}<Button variant="secondary" onClick={() => void showTargetHistory(selectedChange.id, target.org_id)}>Disposition history</Button></article>)}</section>}
    {!!itemHistory.length && <section aria-label="Holding and disposition history" className="space-y-l rounded-lg bg-surface-container p-l">{itemHistory.map((row, index) => <p key={`${row.revision}:${index}`}>Revision {row.revision} · {row.status}{'evidence' in row && row.evidence ? ` · ${row.evidence}` : ''}</p>)}</section>}
  </section>
}
