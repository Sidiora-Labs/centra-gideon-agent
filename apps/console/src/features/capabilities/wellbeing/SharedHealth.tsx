...
export default function SharedHealth() {
  ...
  return <main className="h-full overflow-auto p-4 sm:p-6 space-y-6 text-on-surface"><h1 data-type="headline-s">Native shared health file</h1><p>Exchange version 1 body weight and blood pressure records with a native client. Gideon keeps canonical records and correction history. The fixed file is shared/wellbeing.json in this runtime home; cloud synchronization is not configured here.</p>{error && <p role="alert">{error}</p>}{result && <p role="status">{result}</p>}
    <section aria-label="Shared file status"><h2 data-type="title-m">Local file</h2><p>State: {status?.state ?? 'loading'}</p>{status?.error && <p>{status.error}</p>}{status?.sha256 && <p className="break-all">SHA-256: {status.sha256}</p>}{status?.state === 'ready' && <p>{status.records} shared records · {status.bytes} bytes</p>}<div className="flex flex-wrap gap-3"><Button disabled={busy} onClick={() => act('refresh')}>Refresh shared file</Button><Button disabled={busy || status?.state !== 'ready'} onClick={() => act('file/preview')}>Preview local shared file</Button><Button disabled={busy || !status || !['missing', 'ready'].includes(status.state)} onClick={() => act('publish')}>Publish canonical measurements</Button>{status?.state === 'ready' && <a className="underline" href={base + '/download'} download="wellbeing-shared.json">Download shared JSON</a>}</div></section>
    <section className="space-y-3"><h2 data-type="title-m">Import native document</h2><label>Native health JSON file<input aria-label="Native health JSON file" type="file" accept=".json,application/json" disabled={busy} onChange={async event => { const file = event.target.files?.[0]; if (!file) return; try { setContent(new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(await file.arrayBuffer())); setPreview(null) } catch (err) { setError(String(err)) } }} /></label><Field label="Native document JSON"><TextArea value={content} onChange={value => { setContent(value); setPreview(null) }} /></Field><Button disabled={busy || !content} onClick={() => act('preview')}>Preview uploaded document</Button></section>
    {preview && <section aria-label="Shared import preview"><h2 data-type="title-m">Import preview</h2><p>{preview.counts.create} new · {preview.counts.correct} corrections · {preview.counts.unchanged} unchanged</p><p>Source store: {preview.store_id}</p><p>Missing rows never delete canonical records. Conflicting source revisions or local changes stop the whole import.</p><Button disabled={busy} onClick={() => act(filePreview ? 'file/commit' : 'commit')}>Commit shared import</Button></section>}
  </main>
}
