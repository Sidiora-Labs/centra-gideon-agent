...
function RecordEditor({ plan, row, saved }: { plan: Plan; row: RecordRow | null; saved: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-3"><h2 data-type="title-m">{row ? 'Correct adherence record' : 'Record adherence'}</h2><Field label="Scheduled date"><TextInput type="date" value={day} onChange={setDay} disabled={!!row} required /></Field><label className="block">Status<select aria-label="Recorded status" value={status} onChange={e => setStatus(e.target.value)}><option value="completed">Completed</option><option value="skipped">Skipped</option></select></label><Field label="Observed at (with offset)"><TextInput value={observed} onChange={setObserved} required /></Field><Field label="Adherence notes"><TextInput value={notes} onChange={setNotes} /></Field>{error && <p role="alert">{error}</p>}<Button type="submit" loading={busy}>Save adherence record</Button></form>
}

export default function Interventions() {
  const [params, setParams] = useSearchParams()
  const identity = params.get('plan'), recordId = params.get('record')
