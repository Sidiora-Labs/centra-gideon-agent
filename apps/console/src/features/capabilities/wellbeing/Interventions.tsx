...
function PlanEditor({ row, saved }: { row: Plan | null; saved: (id: string) => void }) {
  ...
  return <form onSubmit={submit} className="space-y-m"><h2 data-type="title-m">{row ? 'Edit intervention' : 'New intervention'}</h2><Field label="Intervention name"><TextInput value={name} onChange={setName} required /></Field><Field label="User supplied instructions"><TextInput value={instructions} onChange={setInstructions} required /></Field>{row ? <><p>Schedule: {row.start_date} to {row.end_date || 'open end'} · {row.timezone} · {row.weekdays.map(day => weekdays[day]).join(', ')}</p><p>Source: {row.source}. Create a new intervention to change its schedule.</p><label><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={archived} onChange={e => setArchived(e.target.checked)} />Archived intervention</label></> : <><label className="block">Kind<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Intervention kind" value={kind} onChange={e => setKind(e.target.value)}>{['activity', 'medication', 'supplement', 'other'].map(value => <option key={value}>{value}</option>)}</select></label><Field label="Intervention source"><TextInput value={source} onChange={setSource} required /></Field><Field label="Intervention timezone"><TextInput value={timezone} onChange={setTimezone} required /></Field><Field label="Start date"><TextInput placeholder="YYYY-MM-DD" value={start} onChange={setStart} required /></Field><Field label="End date (optional)"><TextInput placeholder="YYYY-MM-DD" value={end} onChange={setEnd} /></Field><fieldset><legend>Scheduled weekdays</legend>{weekdays.map((day, index) => <label key={day} className="inline-flex gap-1 mr-3"><input className="size-4 rounded border-outline-variant/40 text-primary focus:ring-primary" type="checkbox" checked={days.includes(index)} onChange={e => setDays(current => e.target.checked ? [...current, index].sort() : current.filter(value => value !== index))} />{day}</label>)}</fieldset></>}{error && <p role="alert">{error}</p>}<Button type="submit" loading={busy}>Save intervention</Button></form>
}
...
function RecordEditor({ plan, row, saved }: { plan: Plan; row: RecordRow | null; saved: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-m"><h2 data-type="title-m">{row ? 'Correct adherence record' : 'Record adherence'}</h2><Field label="Scheduled date"><TextInput placeholder="YYYY-MM-DD" value={day} onChange={setDay} disabled={!!row} required /></Field><label className="block">Status<select className="h-10 w-full rounded-md border border-outline-variant/30 bg-surface-container px-m text-on-surface outline-none focus:border-primary/40 focus:ring-2 focus:ring-inset focus:ring-primary" aria-label="Recorded status" value={status} onChange={e => setStatus(e.target.value)}><option value="completed">Completed</option><option value="skipped">Skipped</option></select></label><Field label="Observed at (with offset)"><TextInput value={observed} onChange={setObserved} required /></Field><Field label="Adherence notes"><TextInput value={notes} onChange={setNotes} /></Field>{error && <p role="alert">{error}</p>}<Button type="submit" loading={busy}>Save adherence record</Button></form>
}
