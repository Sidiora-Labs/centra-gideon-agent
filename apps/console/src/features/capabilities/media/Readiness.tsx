...
export function ReadinessCards({ items }: { items: ReadinessRow[] }) {
  return <div className="space-y-3">{items.map(item => <article key={item.capability} className="rounded-lg bg-surface-container p-l"><h2>{item.capability === 'image_gen' ? 'Image generation' : 'Video generation'}</h2>
...
export default function Readiness() {
  ...
  useEffect(() => { void load() }, [])
  return <NativeMediaPage title="Media readiness" actions={<><a href="#/capabilities/media">Sketches</a><a href="#/settings?tab=models">Models</a><a href="#/settings?tab=providers">Providers</a></>} width="content">
