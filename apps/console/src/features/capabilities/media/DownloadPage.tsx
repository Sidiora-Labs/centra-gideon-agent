...
export default function DownloadPage() {
  ...
  return <NativeMediaPage title="Media source downloader" actions={<a href="#/capabilities/media?view=jobs">Media jobs</a>}><p>Acquire one bounded public YouTube source through the guarded media transport. Every request and redirect is checked against the session egress policy; successful bytes become a canonical artifact with source provenance.</p><fieldset disabled={busy}><label>Source URL<input type="url" maxLength={2000} placeholder="https://www.youtube.com/watch?v=…" value={url} onChange={event => setUrl(event.target.value)} /></label><label>Media kind<select value={kind} onChange={event => setKind(event.target.value)}><option value="video">Video</option><option value="audio">Audio</option></select></label><Button disabled={!url.trim()} onClick={() => void action(async () => { const value = await api('', { operation: 'source_download', request_id: crypto.randomUUID(), input: { url: url.trim(), kind } }); setJob(value); setJobId(value.id) })}>Queue guarded download</Button><label>Download job ID<input value={jobId} onChange={event => setJobId(event.target.value)} /></label><Button disabled={!jobId} onClick={() => void action(() => load(jobId))}>Load job</Button></fieldset>{job && <p role="status">{job.status}{job.error ? ` · ${job.error}` : ''}</p>}{download && <p><a href={download} download>Download canonical {job?.result?.kind}</a> · <a href="#/capabilities/media?view=library">Open media library</a></p>}{error && <p role="alert">{error}</p>}</NativeMediaPage>
}
