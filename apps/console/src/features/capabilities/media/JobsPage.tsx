...
export type MediaJob = { id: string; sketch_id: string; revision: number; status: string; state_revision: number; attempt: number; error: string | null; result: { artifact_id: string; version: number } | null; events: { status: string; at: string; detail: string; attempt?: number; result?: { artifact_id: string; version: number } | null }[] }
...
export function JobCard({ job, act, busy }: { job: MediaJob; act: (job: MediaJob, action: string) => void; busy: boolean }) {
  return <article className="rounded border p-3 space-y-2">
    <h2>Sketch {job.sketch_id} · revision {job.revision}</h2><p role="status">{job.status} · attempt {job.attempt}</p>
