...
export function mediaArtifactRawUrl(result: MediaJob['result'] | undefined): string | null {
  ...
  return result.version !== undefined && Number.isSafeInteger(result.version) && result.version > 0
    ? `${url}?version=${result.version}` : url
}
...
export function JobCard({ job, act, busy }: { job: MediaJob; act: (job: MediaJob, action: string) => void; busy: boolean }) {
  const artifactUrl = mediaArtifactRawUrl(job.result)
  const imagePending = ['queued', 'running', 'cancel_requested'].includes(job.status)
  return <article className="rounded-lg bg-surface-container p-l space-y-s">
    ...
    {job.operation === 'image_generate' && <ImageGeneration prompt={job.input!.prompt!} generating={imagePending} showPrompt={false}
      statusLabel={job.status === 'queued' ? 'Queued' : job.status === 'cancel_requested' ? 'Cancel requested' : imagePending ? 'Generating' : undefined}
      imageUrl={artifactUrl ?? undefined} className="w-full max-w-[20rem]" />}
