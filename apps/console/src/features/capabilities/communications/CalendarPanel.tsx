...
export function CalendarPanel() {
  ...
  return <section aria-label="Calendar daily review" className="space-y-3 rounded border p-4"><h2>Calendar daily review</h2><p>Review mirrored events by local day. Snapshot coverage is separate from live calendar access. ICS recurrence rules are reported without expansion.</p>{error && <p role="alert">{error}</p>}
    ...
    {selected && <div><button disabled={busy} onClick={() => { setEditing({ id: selected.id, revision: selected.revision }); setForm({ name: selected.name, kind: selected.kind, calendar_id: selected.calendar_id, credential_ref: selected.credential_ref, timezone: selected.timezone }) }}>Edit calendar source</button><p>Calendar sync: {selected.sync.state}; {selected.sync.coverage}</p>{selected.sync.error && <p>{selected.sync.error}</p>}{selected.kind === 'ics' ? <><label>ICS export content<textarea value={content} onChange={e => setContent(e.target.value)} /></label><button disabled={busy || !content} onClick={() => void run(async () => { await requestJson(`${base}/sources/${selected.id}/upload`, 'POST', { content, revision: selected.revision }); setContent(''); await reload() })}>Import calendar export</button></> : <button disabled={busy} onClick={() => void run(async () => { try { const start = new Date(day + 'T00:00:00Z'); const end = new Date(start.getTime() + 7 * 86400000); await requestJson(`${base}/sources/${selected.id}/sync`, 'POST', { start: start.toISOString(), end: end.toISOString() }) } finally { await reload() } })}>Sync calendar week</button>}</div>}
