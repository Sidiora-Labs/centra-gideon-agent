...
export function SignalArchivePanel() {
  ...
  return <section className="my-6 space-y-3 rounded border p-4" aria-label="Encrypted Signal archive">
    <h2>Encrypted Signal archive</h2>
    ...
    <label className="block">Encrypted Signal SQLite file<input type="file" accept=".db,.sqlite,.sqlite3" disabled={busy} onChange={event => choose(event.target.files?.[0])} /></label>
    ...
    {preview && <div className="space-y-2"><p>{preview.rows.length} authenticated Signal messages; {preview.rows.filter(row => row.eligible).length} match existing people. Coverage: {preview.coverage}.</p>
      {preview.rows.map(row => <article key={row.external_id}><p>{row.external_id} · {row.direction || 'unsupported event'} · {row.eligible ? 'linked person' : 'history only'}</p><p>{row.body || '(Text unavailable)'}</p>{row.attachments.length > 0 && <p>{row.attachments.length} attachment reference(s)</p>}</article>)}
    ...
    {history.map(row => <article key={row.external_id}><p>Stored: {row.external_id}</p><p>{row.body || '(Text unavailable)'}</p></article>)}
  </section>
}
