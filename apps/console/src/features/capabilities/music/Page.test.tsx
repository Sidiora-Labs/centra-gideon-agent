...
it('authors a piece through real HTTP, practices, and reopens its persisted reader', async () => {
  ...
  fireEvent.change(screen.getByLabelText('Artist'), { target: { value: 'The Placeholders' } })
  ...
  fireEvent.change(screen.getByLabelText('Practice notes'), { target: { value: 'Start slowly' } })
  fireEvent.change(screen.getByLabelText('Source URL'), { target: { value: 'https://example.com/chart' } })
  fireEvent.change(screen.getByLabelText('Related records JSON'), { target: { value: '[{"type":"round","id":"round-1","label":"Opening round"}]' } })
  fireEvent.change(screen.getByLabelText('Scroll duration seconds'), { target: { value: '90' } })
  ...
  expect(within(reader).getByText(/The Placeholders · guitar · Em · capo 2 · Drop D/)).toBeInTheDocument()
  expect(within(reader).getByRole('link', { name: 'Original source' })).toHaveAttribute('href', 'https://example.com/chart')
  expect(within(reader).getByText('Opening round · round')).toBeInTheDocument()
  expect(within(reader).getByText('Scroll target: 90 seconds')).toBeInTheDocument()
  ...
  expect(screen.getByLabelText('Notation')).toHaveValue('Em C G D\nRepeat softly')
  expect(screen.getByLabelText('Practice notes')).toHaveValue('Start slowly')
  expect((screen.getByLabelText('Related records JSON') as HTMLTextAreaElement).value).toContain('round-1')
...
it('shows a real optimistic conflict without discarding the unsaved text', async () => {
  ...
  fireEvent.change(screen.getByLabelText('Practice notes'), { target: { value: 'My unsaved fingering' } })
  fireEvent.click(screen.getByRole('button', { name: 'Save changes' }))
  expect(await screen.findByRole('alert')).toHaveTextContent('Item changed; reload before editing')
  expect(screen.getByLabelText('Practice notes')).toHaveValue('My unsaved fingering')
