...
export default function RoundsPage({apiBase='/api/capabilities/music/rounds',catalogBase='/api/capabilities/music/catalog'}:{apiBase?:string;catalogBase?:string}) {
  ...
  return <section className="mx-auto flex w-full max-w-4xl flex-col gap-4 overflow-auto p-4 text-on-surface">
    <h1>Musical canons and part practice</h1><a className="text-primary" href="#/capabilities/music">Repertoire</a>{error&&<p role="alert">{error}</p>}
    {loading?<p role="status">Loading canons…</p>:<>
      ...
      {(!id||item)&&<form className="flex flex-col gap-3" onSubmit={event=>{event.preventDefault();void save()}}>
        ...
        <label>Arrangement notes<textarea className={cls} value={draft.notes} onChange={event=>edit({notes:event.target.value})}/></label>
        <label>Partner canon IDs (one per line)<textarea className={cls} value={draft.partner_ids.join('\n')} onChange={event=>edit({partner_ids:event.target.value.split('\n').map(value=>value.trim()).filter(Boolean)})}/></label>
        {draft.parts.map((part,index)=><fieldset className="rounded-lg border border-outline p-3" key={part.id}><legend>Part {index+1}</legend>
          ...
          <label>Part {index+1} notation<textarea className={cls} value={part.notation} onChange={event=>editPart(index,{notation:event.target.value})}/></label>
      ...
      {item&&<article aria-label="Part practice" className="flex flex-col gap-3"><h2>{item.title}</h2><p>Practice uses saved arrangement revision {item.revision}.</p>
        {item.parts.map(part=>{const track=tracks.find(row=>row.id===part.catalog_ref?.track_id);const ref=track?.renders.find(row=>row.id===part.catalog_ref?.render_id)?.artifact_ref;return <div key={part.id}><label><input type="checkbox" checked={chosen.includes(part.id)} onChange={event=>setChosen(values=>event.target.checked?[...values,part.id]:values.filter(value=>value!==part.id))}/>{part.name}</label><pre className="whitespace-pre-wrap font-sans">{part.notation}</pre>{ref?<audio aria-label={`Play ${part.name}`} controls src={`/api/artifacts/${ref.slug}/raw?version=${ref.version}`}/>:<p>{part.catalog_ref?'Recording unavailable.':'No recording attached.'}</p>}</div>})}
        ...
        <label>Practice notes<textarea className={cls} value={practiceNotes} onChange={event=>setPracticeNotes(event.target.value)}/></label>
