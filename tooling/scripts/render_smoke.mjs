...
//
//   1. serve apps/console/dist over a throwaway static server (hash routing needs no
...
//      own check).
//
// Run it against the freshly built apps/console/dist (`npm run build` first):
//     npm run smoke:render
// Or point it at a live gateway instead of the static server:
//     GIDEON_SMOKE_URL=http://127.0.0.1:10000 npm run smoke:render
//
// Wired into the pre-push gate (tooling/scripts/run_prepush.sh) and the ci.yml `web`
...
const DIST = path.join(REPO_ROOT, 'apps', 'console', 'dist')
...
async function main() {
  let base = process.env.GIDEON_SMOKE_URL
  let server = null
  if (base) {
    log(`probing live server at ${base}`)
  } else {
    if (!existsSync(path.join(DIST, 'index.html'))) {
      fail('apps/console/dist/index.html missing — run `npm run build` first')
      return
    }
    server = await serveDist()
    base = `http://127.0.0.1:${server.address().port}`
    log(`serving apps/console/dist at ${base}`)
  }
  ...
  try {
    for (const route of ROUTES) {
      ...
      if (problems.length > 0) {
        ...
        try { await page.screenshot({ path: shot, fullPage: true }); problems.push(`screenshot: ${shot}`) } catch {   }
