...
This directory is `examples/registry/` in the Gideon core repo: content prepared
in-tree so it can be reviewed, tested and version-controlled before it exists as a
repository. It is ET-3 in the ECOSYSTEM-TOOLING plan (internal, not in this repo). Nothing in runtime
imports it; `checks/runtime/test_registry_validation.py` verifies it in core.
...
Owner steps to publish:
  ...
  4. Delete `fixtures/` only if you want to; the tests that use it live in core
     (`checks/runtime/test_registry_validation.py`), so the published copy works either way,
