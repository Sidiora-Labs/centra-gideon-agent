...
Lessons are saved via the ``gideon learn`` CLI (called by the LLM via bash)
and loaded into every session's context alongside memory and skills.

Storage: ``~/.gideon/lessons.jsonl`` (one lesson per line; rewritten on
...
_DEFAULT_DIR = _path_home_gideon()
