...
def terminal_session_name(session_id: str) -> str:
    ...
    """
    return "gideon-" + str(session_id).replace(".", "_")
...
def durable_session_name(project_id: str, run_id: str, session_slug: str) -> str:
    ...
    ``gideon-<project>-<run>-<session>``, every component sanitized. Derived purely from
    ...
    """
    return "-".join(
        ("gideon", sanitize(project_id), sanitize(run_id), sanitize(session_slug)),
    )
