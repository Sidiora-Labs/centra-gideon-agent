...
``webapp/`` directory for :func:`~gideon.artifacts.handlers.serve_deployed_artifact`
...
spawn here goes through :func:`gideon.sandbox.create_subprocess_limited`
with :data:`~gideon.sandbox.PROFILE_BUILD` — the same profile the model
...
and the child environment comes from :func:`gideon.sandbox.build_child_env`
...
from gideon.sandbox import PROFILE_BUILD, build_child_env
...
def toolchain_candidates() -> list[Path]:
    ...
    # <repo>/src/gideon/artifacts/build.py → <repo>
...
async def _run_esbuild(
    ...
    *,
    ...
    :func:`gideon.sandbox.create_subprocess_limited` with
    :data:`~gideon.sandbox.PROFILE_BUILD`: bundling arbitrary artifact
