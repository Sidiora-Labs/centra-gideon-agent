...
def discover_providers(
    ...
    for ep in eps:
        try:
            ...
            if cls is None:
                cls_name = (
                    "".join(w.capitalize() for w in ep.name.split("_")) + "Provider"
                )
                cls = getattr(mod, cls_name, None)
            if cls is not None and (
                base_class is object or issubclass(cls, base_class)
            ):
                providers[ep.name] = cls
        except Exception:
            logger.warning(
                "Failed to load %s provider %r", group, ep.name, exc_info=True
            )
    return providers
