...
with the :class:`~gideon.extensions.providers.registry.ProviderRegistry`.
"""
...
def discover_bundled_extensions() -> list[AppManifest]:
    ...
    for entry in sorted(BUNDLED_DIR.iterdir()):
        ...
        try:
            ...
            logger.warning(
                "Failed to parse bundled extension: %s", entry.name, exc_info=True
            )
    return manifests
...
def _load_ext_module(ext: "RegisteredProvider", module_path: str) -> Any:
    ...
    dotted package path (``gideon.engine.tasks.native``) and is imported normally.
...
def _subscription_availability(
    ...
    if not provider_type:
        return None
    try:
        from gideon.integrations.llm.branded_specs import spec_credential_source
...
def _installed_origin(name: str) -> str:
    ...
    try:
        from gideon.extensions.apps.app_manager import _origin_of
...
def _seed_extension_skills(
    manifest: AppManifest, *, enabled: bool, origin: str
) -> None:
    ...
    for app_info in list_apps():
        ...
        try:
            ...
            _seed_extension_skills(
                manifest,
                enabled=True,
                origin=str(app_info.get("origin", "") or "local"),
            )
        except Exception:
            logger.debug(
                "prompt-only app %s: seed failed",
                app_info.get("name", "?"),
                exc_info=True,
            )
...
def register_extension_providers() -> None:
    ...
    try:
        from gideon.extensions.apps.app_manager import seed_builtin_apps
    ...
    try:
        from gideon.integrations.tool_providers.app_routes import (
            register as _register_app_routes,
        )
...
def load_all_extensions() -> None:
    ...
    try:
        ...
        from gideon.extensions.apps.worker_runtime import start_worker_watchdog

        start_worker_watchdog()
        from gideon.integrations.local_models.sidecar import start_sidecar_watchdog
