...
class ProviderRegistry:
    ...
    def register_type_handler(
        self, provider_type: str, handler: "_TypeHandler"
    ) -> None:
    ...
    def _disable_one(self, ext: RegisteredProvider) -> None:
        ...
        ext.enabled = False
        logger.info(
            "Disabled extension %s (type=%s)", ext.name, ext.provider_config.type
        )
...
class TaskTypeHandler(_TypeHandler):
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.engine.tasks.registry import register_provider

        register_provider(instance)

    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.engine.tasks.registry import unregister_provider
...
class WorkflowTypeHandler(_TypeHandler):
    ...
    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.automation.workflows.defs import unregister_provider
...
class SearchTypeHandler(_TypeHandler):
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.search_providers.registry import register_provider

        register_provider(instance)

    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.search_providers.registry import unregister_provider
...
class ActionTypeHandler(_TypeHandler):
    ...
    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        ...
        from gideon.security.guardrails.autonomy import unregister_action_type
...
class DutyGateTypeHandler(_TypeHandler):
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.automation.triggers.calendar import register_duty_gate
    ...
    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.automation.triggers.calendar import _DUTY_GATES
...
class ChannelTypeHandler(_TypeHandler):
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.channel_transports import register_transport

        register_transport(instance)

    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.channel_transports import unregister_transport
...
class SyncTypeHandler(_TypeHandler):
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.sync_transports import register_transport

        register_transport(instance)

    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.sync_transports import unregister_transport

        name = getattr(instance, "name", None) or (
            getattr(ext, "name", "") if ext else ""
        )
        if name:
...
class TriggerSourceTypeHandler(_TypeHandler):
    ...
    **Deregistration PARKS, it does not silently drop** — the difference from every other type
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.automation.trigger_sources import register_source
    ...
    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.automation.trigger_sources import unregister_source
        from gideon.automation.trigger_sources.parking import (
            _default_store,
            park_for_app,
        )
    ...
    def _start(self, instance: Any, name: str) -> None:
        ...
        from gideon.automation.trigger_sources import SourceEvent, emit
    ...
    async def _run_start(instance: Any, name: str, emit_fn: Any) -> None:
        ...
        try:
            ...
            Exception
        ):  # noqa: BLE001 - one app's broken watch must not break the gateway
...
class TriggerTypeHandler(_TypeHandler):
    ...
    **``trigger`` is NOT ``trigger_source``** — they are adjacent in ``PROVIDER_TYPES`` and a reader
    ...
    See ``gideon.automation.triggers.provider`` for the long form.
    ...
    **Rows only, never execution.** The provider is asked for rows and is never handed a fire, a
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.automation.triggers.registry import register_trigger_store
    ...
    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.automation.triggers.registry import unregister_trigger_store
...
class SandboxTypeHandler(_TypeHandler):
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.sandbox_providers import register_provider

        register_provider(instance)

    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.sandbox_providers import unregister_provider
...
class PromptTypeHandler(_TypeHandler):
    ...
    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.prompt_providers.registry import _providers
...
class MemoryTypeHandler(_TypeHandler):
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.memory_providers.registry import register_provider

        provider_name = getattr(instance, "name", ext.name)
        register_provider(provider_name, instance)

    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.memory_providers.registry import unregister_provider
...
class KnowledgeTypeHandler(_TypeHandler):
    ...
    Builds a :class:`~gideon.integrations.knowledge_providers.base.KnowledgeProvider` via
    ...
    needs the ``ConsoleState`` store, so it self-registers via
...
class InboxTypeHandler(_TypeHandler):
    ...
    Builds a :class:`~gideon.integrations.inbox_providers.base.MessageSourceProvider` via
    ...
    **Deregistration is load-bearing.** An unregistered source is a PHANTOM: a
    ...
    def register(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.inbox_providers.registry import register_source

        register_source(instance)

    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        from gideon.integrations.inbox_providers.registry import unregister_source

        name = getattr(instance, "source_name", None) or (
            getattr(ext, "name", "") if ext else ""
        )
        if name:
...
class ModelTypeHandler(_TypeHandler):
        ...
        for provider in providers:
            if "embedding" in caps:
                ...
                if isinstance(provider, _EMB):
                    ...
                    reg_emb(provider)
            if "stt" in caps:
                from gideon.integrations.stt.provider import SttProvider as _STT

                if isinstance(provider, _STT):
                    from gideon.integrations.stt.registry import (
                        register_provider as reg_stt,
                    )

                    reg_stt(provider)
            if "tts" in caps:
                from gideon.integrations.tts.provider import TtsProvider as _TTS

                if isinstance(provider, _TTS):
                    from gideon.integrations.tts.registry import (
                        register_provider as reg_tts,
                    )

                    reg_tts(provider)
            if "diarization" in caps:
                from gideon.integrations.diarization.provider import (
                    DiarizationProvider as _DIA,
                )

                if isinstance(provider, _DIA):
                    from gideon.integrations.diarization.registry import (
                        register_provider as reg_diar,
                    )

                    reg_diar(provider)
            if "image_gen" in caps:
                from gideon.integrations.image_gen.provider import (
                    ImageGenProvider as _IGP,
                )

                if isinstance(provider, _IGP):
                    from gideon.integrations.image_gen.registry import (
                        register_provider as reg_img,
                    )

                    reg_img(provider)
            if "video_gen" in caps:
                from gideon.integrations.video_gen.provider import (
                    VideoGenProvider as _VGP,
                )

                if isinstance(provider, _VGP):
                    from gideon.integrations.video_gen.registry import (
                        register_provider as reg_vid,
                    )
    ...
    def deregister(self, ext: RegisteredProvider, instance: Any) -> None:
        ...
        for provider in providers:
            ...
            if "stt" in caps:
                from gideon.integrations.stt.registry import (
                    unregister_provider as unreg_stt,
                )

                unreg_stt(provider_name)
            if "tts" in caps:
                from gideon.integrations.tts.registry import (
                    unregister_provider as unreg_tts,
                )

                unreg_tts(provider_name)
            if "diarization" in caps:
                from gideon.integrations.diarization.registry import (
                    unregister_provider as unreg_diar,
                )

                unreg_diar(provider_name)
            if "image_gen" in caps:
                from gideon.integrations.image_gen.registry import (
                    unregister_provider as unreg_img,
                )

                unreg_img(provider_name)
            if "video_gen" in caps:
                from gideon.integrations.video_gen.registry import (
                    unregister_provider as unreg_vid,
                )
