...
def _dynamic_media_provider_names() -> set[str]:
    ...
    try:
        ...
        pass
    try:
        from gideon.integrations.local_models.registry import _key_for as _local_key
        from gideon.integrations.local_models.registry import (
            list_providers as _local_list,
        )
...
def _prune_removed_providers(active: dict[str, list[str]]) -> dict[str, list[str]]:
    ...
    for use_case, refs in active.items():
        if not isinstance(refs, list):
            ...
            continue
        kept = [
            r for r in refs if ":" not in str(r) or str(r).split(":", 1)[0] in known
        ]
...
def openai_family_providers() -> list[dict[str, str]]:
    ...
    (gideon.integrations.media_catalogs) instead of host-sniffing.
    """
    from gideon.core.config.loader import config_path
