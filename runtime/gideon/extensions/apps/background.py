...
def _install_signal_handlers(control: WorkerControl) -> None:
    ...
    try:
        signal.signal(
            signal.SIGTERM, lambda *_: control.request_stop(StopReason.SHUTDOWN)
        )
        signal.signal(
            signal.SIGINT, lambda *_: control.request_stop(StopReason.SHUTDOWN)
        )
    ...
    try:
        ...
        while not control.should_stop():
            ...
            try:
                ...
                raise
            interval = max(
                0.0, float(getattr(worker, "poll_interval", DEFAULT_POLL_INTERVAL))
            )
