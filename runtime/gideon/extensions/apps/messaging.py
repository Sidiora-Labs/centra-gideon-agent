...
from gideon.extensions.apps.manager import _validate_app_name
...
def drain_queue(target: str) -> list[dict[str, str]]:
    ...
    try:
        ...
        logger.debug(
            "app_messaging: failed to clear inbox for %s", target, exc_info=True
        )
    return msgs
...
def send_message(
    *, sender: str, target: str, msg_type: str, payload: str
) -> AppMessage:
    ...
    if checker is None or not checker.can_use_app_messaging(target):
        ...
        raise AppMessageError(
            f"app {sender!r} is not permitted to message {target!r}", status=403
        )
    ...
    _sel_message(
        operation="app_message", outcome="success", sender=sender, target=target
    )
    return msg
