...
registers each prompt's use-case into :mod:`gideon.extensions.apps.prompt_registry`
...
def _read_definition(app_dir: Path, rel_path: str) -> dict[str, Any] | None:
    ...
    the app dir."""
    from gideon.integrations.prompt_providers.native_provider import _yaml_loads
...
def seed_app_prompts(manifest, app_dir: str | Path) -> None:
    ...
    ``manifest`` is an :class:`~gideon.extensions.apps.manifest.AppManifest`."""
        ...
        try:
            if entity == _ENTITY_SNIPPET:
                ...
                if not path.exists():
                    ...
                    path.write_text(
                        _yaml_dumps(_snippet_payload(snip)), encoding="utf-8"
                    )
            ...
            logger.debug(
                "failed to seed app %r %s %r", app_name, entity, name, exc_info=True
            )
...
def remove_app_prompts(manifest, app_dir: str | Path) -> None:
    ...
    for rel in prompts:
        ...
        try:
            path = (
                _snippet_path(name) if entity == _ENTITY_SNIPPET else _prompt_path(name)
            )
            if path.exists():
                path.unlink()
            ...
            logger.debug(
                "failed to remove app %r %s %r", app_name, entity, name, exc_info=True
            )
