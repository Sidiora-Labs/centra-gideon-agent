...
def _deregister_from_agent_config(prefix: str) -> None:
    ...
    try:
        from gideon.engine.agent import AGENT_FILENAME, _atomic_json_write
        ...
        for list_key in ("tools", "allowedTools"):
            ...
            if isinstance(lst, list):
                kept = [
                    t
                    for t in lst
                    if not (isinstance(t, str) and t.lstrip("@").startswith(prefix))
                ]
                if len(kept) != len(lst):
                    cfg[list_key] = kept
                    changed = True
        ...
        logger.debug(
            "agent-config MCP deregister skipped for %s*", prefix, exc_info=True
        )
