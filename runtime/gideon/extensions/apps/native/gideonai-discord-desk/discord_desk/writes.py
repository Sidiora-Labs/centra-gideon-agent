"""The DISABLE_LIVE_WRITES kill switch, as this channel transport sees it.

``GIDEON_DISABLE_LIVE_WRITES`` is the platform's process-wide "make no
...
``delete_model``). Posting a Discord message is exactly that class of write —
outward-facing, instantly visible to everyone in the channel, and only reversible
by a second API call that leaves an edit/delete trace — so this transport honors
the same switch.

**Why the flag is re-read here instead of imported.** Core's implementation lives in
``gideon.guardrails.writes``, which is NOT re-exported through ``gideon.sdk.*``; an
app may only import core via the SDK facade (lint-enforced), so reaching for it
directly is not an option. The env var itself is a stable, documented platform
contract, so this module reads it and mirrors core's parse EXACTLY — including the
fail-safe half. Any drift here would be a guard that disagrees with the rest of the
platform about whether writes are on, which is worse than no guard at all, so the
semantics are pinned by test.

**Why this file is duplicated per channel app rather than shared.** Each app bundle
is installed on its own, so there is no sibling module for it to import and no
cross-app package to put one in. The copies are held together by test, not by
import: each app's suite cross-checks THIS parse against core's own symbol, so all
of them agree with core and therefore with each other. A repo rail additionally
asserts every ``provider.type == "channel"`` app carries the honor point, so a new
channel app cannot ship without one.

**The semantics being mirrored** (core ``guardrails/writes.live_writes_disabled``
over ``guardrails/flags.guard_flag``):
...
class SendRefused:
    ...
    #: The channel that refused (``"discord"``) — set so a log line or an aggregated
    #: refusal is attributable when several transports are live.
    channel: str
    #: The channel id the message WOULD have gone to.
    target: str
