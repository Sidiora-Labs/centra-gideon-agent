...
class TestPhaseTransitions:
    ...
    async def test_queued_adds_eyes(self) -> None:
        with fake_clock():
            slack_desk = FakeSlackDesk()
            ctrl = StatusReactionController(slack_desk, _CH, _TS)
            ctrl.set_phase("queued")
            await asyncio.sleep(0)
            assert ("add", _TS, "eyes") in slack_desk.calls
    ...
    async def test_thinking_swaps_eyes_to_thinking_face(self) -> None:
        with fake_clock() as clock:
            ...
            ctrl.set_phase("queued")
            await asyncio.sleep(0)
            slack_desk.calls.clear()

            ctrl.set_phase("thinking")
            await clock.advance(_DEBOUNCE + 0.01)
            assert ("remove", _TS, "eyes") in slack_desk.calls
            assert ("add", _TS, "thinking_face") in slack_desk.calls
    ...
    async def test_finalize_done_is_immediate(self) -> None:
        with fake_clock():
            ...
            await asyncio.sleep(0)
            assert ("remove", _TS, "eyes") in slack_desk.calls
            assert ("add", _TS, "white_check_mark") in slack_desk.calls
    ...
    async def test_finalize_error(self) -> None:
        with fake_clock():
            ...
            await asyncio.sleep(0)
            assert ("remove", _TS, "eyes") in slack_desk.calls
            assert ("add", _TS, "scream") in slack_desk.calls
    ...
    async def test_debounce_suppresses_rapid_transitions(self) -> None:
        with fake_clock() as clock:
            ...
            await clock.advance(_DEBOUNCE + 0.01)
            add_emojis = [e for a, _, e in slack_desk.calls if a == "add"]
...
class TestStallDetection:
    ...
    async def test_stall_hard_replaces_soft(self) -> None:
        with fake_clock() as clock:
            ...
            assert ("remove", _TS, "yawning_face") in slack_desk.calls
            assert ("add", _TS, "fearful") in slack_desk.calls
    ...
    async def test_finalize_cleans_up_stall(self) -> None:
        with fake_clock() as clock:
            ...
            await clock.advance(_SOFT + 0.01)
            assert ("add", _TS, "yawning_face") in slack_desk.calls
            slack_desk.calls.clear()
...
class TestDisabledReactions:
    ...
    async def test_disabled_set_phase_no_ops(self) -> None:
        with fake_clock() as clock:
            slack_desk = FakeSlackDesk()
            ctrl = StatusReactionController(slack_desk, _CH, _TS, enabled=False)
            ctrl.set_phase("queued")
            await clock.advance(0.5)
            assert slack_desk.calls == []
    ...
    async def test_enabled_true_still_works(self) -> None:
        with fake_clock():
            slack_desk = FakeSlackDesk()
            ctrl = StatusReactionController(slack_desk, _CH, _TS, enabled=True)
            ctrl.set_phase("queued")
            await asyncio.sleep(0)
            assert ("add", _TS, "eyes") in slack_desk.calls
...
class TestPhaseSuppression:
    ...
    async def test_suppressed_queued_adds_nothing(
        ...
        with fake_clock():
            ...
            ctrl.set_phase("queued")
            await asyncio.sleep(0)
            add_calls = [c for c in slack_desk.calls if c[0] == "add"]
            assert add_calls == []
