...
def _make_orch(download_content: bytes = b"hello") -> MagicMock:
    ...
    orch.slack_desk.download_file = AsyncMock(side_effect=_download)
    return orch
...
class TestProcessSlackDeskFiles:
    @pytest.mark.asyncio
    ...
    async def test_text_file_too_large_skipped(self):
        ...
        orch.slack_desk.download_file.assert_not_called()
    ...
    async def test_download_failure_cleans_temp(self):
        ...
        orch = MagicMock()
        orch.slack_desk = AsyncMock()
        orch.slack_desk.download_file = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_orch_slack_desk_none_returns_empty(self):
    ...
    async def test_document_too_large_skipped(self):
        ...
        from slack_desk_runtime.files import _MAX_DOC_BYTES
