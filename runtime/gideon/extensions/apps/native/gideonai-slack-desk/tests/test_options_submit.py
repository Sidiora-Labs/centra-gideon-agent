...
def _mock_orch():
    ...
    orch.slack_desk.post_blocks = AsyncMock(return_value="new_ts")
...
class TestHandleOptionsSubmit:
    ...
    async def test_denied_user_returns_early(self, orch, monkeypatch):
        ...
        orch.slack_desk.post_blocks.assert_not_called()
        orch.slack_desk.delete_message.assert_not_called()
    ...
    async def test_sends_combined_selections(self, orch, monkeypatch):
        ...
        with patch.object(interactions, "handle_message", new_callable=AsyncMock) as mock_hm:
            ...
            orch.slack_desk.update_message.assert_called_once()
            call_kwargs = orch.slack_desk.update_message.call_args
    ...
    async def test_single_selection(self, orch, monkeypatch):
        ...
        with patch.object(interactions, "handle_message", new_callable=AsyncMock):
            ...
            call_args = orch.slack_desk.update_message.call_args
    ...
    async def test_update_failure_falls_back_to_post_delete(self, orch, monkeypatch):
        ...
        with patch.object(interactions, "handle_message", new_callable=AsyncMock) as mock_hm:
            ...
            orch.slack_desk.post_blocks.assert_called_once()
            orch.slack_desk.delete_message.assert_called_once_with("CH1", "msg1")
    ...
    async def test_preserves_surrounding_blocks(self, orch, monkeypatch):
        ...
        with patch.object(interactions, "handle_message", new_callable=AsyncMock):
            ...
            orch.slack_desk.update_message.assert_called_once()
            new_blocks = orch.slack_desk.update_message.call_args.kwargs["blocks"]
...
class TestImportThreadToSession:
    ...
    async def test_imports_messages_and_links(self, monkeypatch):
        ...
        slack_desk.fetch_thread_replies = AsyncMock(
            return_value=[
                {"user": "U1", "text": "hello"},
                {"bot_id": "B1", "text": "hi"},
            ]
    ...
    async def test_returns_none_when_no_messages(self, monkeypatch):
        ...
        slack_desk = MagicMock()
        slack_desk.fetch_thread_replies = AsyncMock(return_value=[])
        ds = MagicMock()
        ds.get_linked_session = MagicMock(return_value=None)
    ...
    async def test_skips_link_command_messages(self, monkeypatch):
        ...
        with patch("gideon.sdk.channel.save_session_to_history"):
            await interactions._import_thread_to_session(slack_desk, ds, "C1", "100.0")
