...
from slack_desk_runtime.handler import _append_footer_actions
...
def _make_orch_for_link() -> MagicMock:
    ...
    orch.slack_desk.fetch_thread_replies = AsyncMock(
        return_value=[
            {"user": "U1", "text": "hello"},
...
async def test_link_dashboard_null_session_returns(link_orch: MagicMock) -> None:
    ...
    link_orch.slack_desk.fetch_thread_replies = AsyncMock(return_value=[])
...
async def test_link_dashboard_skips_link_command_messages(link_orch: MagicMock) -> None:
    ...
    link_orch.slack_desk.fetch_thread_replies = AsyncMock(
        return_value=[
