...
_SKIP_ENV = "GIDEON_SKIP_APP_WORKERS"
...
def _manifest_for(app: str) -> AppManifest | None:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 — an unreadable manifest is "no workers", not a crash
        logger.debug(
            "app %s: manifest unreadable for worker supervision", app, exc_info=True
        )
        return None
...
def _notify(title: str, body: str, meta: dict) -> None:
    ...
    ``ConsoleState.notify`` is THE path (rules, quiet hours, mute-all all live behind
...
class WorkerSupervisor:
    ...
    def list_running(self) -> list[SupervisedWorker]:
        with self._lock:
            return [
                ...
                if r.state is WorkerState.RUNNING and r.is_alive()
            ]
    ...
    def _spawn(self, rec: SupervisedWorker) -> bool:
        ...
        if pause_reason:
            ...
            if transitioned:
                logger.info(
                    "app %s worker %s held back: %s", rec.app, rec.worker, pause_reason
                )
        ...
        try:
            ...
            logger.warning(
                "app %s worker %s: child env failed", rec.app, rec.worker, exc_info=True
            )
            return False
    ...
    def _child_env(self, rec: SupervisedWorker) -> dict[str, str]:
        ...
        * **the per-app proxy secret.** ``APP_SECRET_ENV`` exists so a backend can VERIFY
          the ``X-Gideon-Proxy`` signature the gateway attaches on the way in. A
        ...
        """
        from gideon.extensions.apps.backend_runtime import shared_storage_env
        from gideon.extensions.apps.background import WORKER_ID_ENV
    ...
    def _terminate(self, rec: SupervisedWorker) -> bool:
        ...
        try:
            ...
            logger.debug(
                "app %s worker %s stop: process already gone", rec.app, rec.worker
            )
        return True

    def pause(
        self, app: str, worker: str, reason: str, *, auto_resume: bool = True
    ) -> bool:
    ...
    def sweep(self) -> None:
        ...
        for info in list_apps():
            ...
            try:
                ...
                logger.debug(
                    "app %s: worker declaration unreadable", app, exc_info=True
                )
                continue
            if not specs:
                continue
