...
``SKILL.md``). This mirrors :mod:`gideon.extensions.apps.prompt_seed` for prompts, with
...
THROUGH the shared chokepoint (:func:`gideon.extensions.skills.marketplace.install_scanned`
→ quarantine → ``scan_dir`` at the app's trust tier → ``.gideon-lock.json`` with
...
app shipped — keyed by the ``.gideon-lock.json`` provenance (``source ==
...
def _app_source(app_name: str) -> str:
    """The ``.gideon-lock.json`` source tag for an app-seeded skill."""
...
def _tier_string_for_origin(origin: str) -> str:
    ...
    Reuses the single mapping in :mod:`gideon.extensions.apps.app_manager` (imported
    ...
    try:
        from gideon.extensions.apps.app_manager import _tier_for_origin
...
class _AppSkillsMarketplace(SkillsMarketplace):
    ...
    ) -> list[SkillEntry]:  # pragma: no cover - unused
        return []
    ...
    def fetch(self, skill_id: str) -> SkillDetail:
        ...
        for f in sorted(skill_dir.rglob("*")):
            ...
            try:
                ...
                logger.warning(
                    "skipping unreadable file in app skill %s: %s", skill_id, rel
                )
                continue
        return SkillDetail(
            id=skill_id, name=skill_dir.name, files=files, audit_status="pass"
        )
...
def seed_app_skills(manifest, app_dir: str | Path, *, origin: str) -> None:
    ...
    :class:`~gideon.extensions.apps.manifest.AppManifest`."""
    ...
    for sk in skills:
        ...
        try:
            ...
            logger.warning(
                "app %r skill %r refused by scan: %s", app_name, skill_dir.name, exc
            )
...
def remove_app_skills(manifest, app_dir: str | Path) -> None:
    ...
    A declared skill's installed dir is removed only when its ``.gideon-lock.json``
    ...
    for sk in skills:
        ...
        if not dest.is_dir() or not lock.is_file():
            continue
        try:
            recorded = (json.loads(lock.read_text(encoding="utf-8")) or {}).get(
                "source"
            )
