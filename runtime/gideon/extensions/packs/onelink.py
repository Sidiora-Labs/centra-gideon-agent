...
document back into a byte-exact ``.gideon`` and then calls :func:`packs.import_.import_pack` —
...
any mismatch, before a ``.gideon`` exists. That makes a partial/URL-backed fetch verifiable
...
def _safe_member(name: str) -> bool:
    ...
    from gideon.extensions.packs.import_ import _safe_member as zip_safe_member
...
def to_onelink(
    ...
    *,
    ...
    """Serialize an existing ``.gideon`` into a one-link JSON document.
    ...
    try:
        ...
        raise OneLinkError(f"not a readable .gideon archive: {exc}") from exc
    with zf:
...
def _fetch_url(url: str) -> bytes:
    ...
    from gideon.extensions.packs.catalog_marketplace import fetch_catalog_text
...
def materialize(
    ...
    *,
    ...
    """Rebuild the ``.gideon`` a one-link document describes; return its path.
    ...
    for name in sorted(resources):
        ...
        if size > MEMBER_MAX_BYTES:
            raise OneLinkError(
                f"resource {name!r} declares {size} bytes (> {MEMBER_MAX_BYTES})"
            )
        raw = _obtain(name, entry, getter)
        if len(raw) > MEMBER_MAX_BYTES:
            raise OneLinkError(
                f"resource {name!r} is {len(raw)} bytes (> {MEMBER_MAX_BYTES})"
            )
    ...
    with zipfile.ZipFile(
        str(out_path), "w", zipfile.ZIP_DEFLATED, compresslevel=6
    ) as zf:
...
def _obtain(name: str, entry: dict[str, Any], getter: Callable[[str], bytes]) -> bytes:
    ...
    if isinstance(b64, str) and b64:
        try:
            ...
            raise OneLinkError(
                f"resource {name!r} has undecodable base64: {exc}"
            ) from exc
    url = entry.get("url")
    if isinstance(url, str) and url:
        try:
            ...
            raise OneLinkError(
                f"resource {name!r} could not be fetched from {url}: {exc}"
            ) from exc
...
def import_onelink(
    ...
    *,
    ...
    """Import a one-link document through the SAME §3 pipeline as a ``.gideon`` file.
    ...
    staging = Path(tempfile.mkdtemp(prefix="gideon-onelink-"))
    try:
        archive = materialize(doc, staging / "onelink.gideon", fetch=fetch)
        return import_pack(
            archive, consent=consent, tier=tier, connector_choices=connector_choices
        )
    finally:
...
def inspect_onelink(
    ...
    *,
    ...
    from gideon.extensions.packs.import_ import inspect_pack
