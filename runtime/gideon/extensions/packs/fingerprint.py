...
(:attr:`gideon.engine.tasks.models.Project.workspace_dir`) is matched against the file-shape
...
def declared_fingerprints() -> dict[str, list[Fingerprint]]:
    ...
    for pack in bundled_packs():
        try:
            manifest = json.loads(
                (pack.source / "pack.json").read_text(encoding="utf-8")
            )
...
def _walk(workspace: Path) -> tuple[list[str], bool]:
    ...
    for root, dirs, files in os.walk(workspace, followlinks=False):
        dirs[:] = sorted(
            d for d in dirs if d not in _SKIP_DIRS and not d.startswith(".ruff")
        )
        base = Path(root)
...
def _glob_hits(rels: list[str], pattern: str) -> list[str]:
    ...
    hits = [
        r
        for r in rels
        if fnmatch.fnmatch(r, pattern) or fnmatch.fnmatch(Path(r).name, pattern)
    ]
    return hits


def _signal_hits(
    workspace: Path, candidates: list[str], signals: tuple[str, ...]
) -> list[str]:
...
def fingerprinting_enabled(config: Any = None) -> bool:
    ...
    if config is None:
        ...
        try:
            ...
            Exception
        ):  # noqa: BLE001 - an unreadable config must not crash project create
...
def scan_project(
    ...
    *,
    ...
    from gideon.extensions.packs.bundled import get_bundled
    ...
    for pack_name, rules in sorted(declared_fingerprints().items()):
        ...
        if (
            bundled is None
        ):  # pragma: no cover - declared_fingerprints reads bundled_packs
            continue
    ...
    logger.info(
        "fingerprint scan (%s) proposed %d pack(s) for project %s",
        reason,
        len(out),
        project_id,
    )
...
def _inspect_report(pack_name: str) -> tuple[dict[str, Any] | None, str]:
    ...
    from gideon.extensions.packs.bundled import BundledPackError, build_bundled
    from gideon.extensions.packs.import_ import PackImportRefused, inspect_pack
    from gideon.security.supply_chain import TrustTier

    staging = Path(tempfile.mkdtemp(prefix="gideon-fingerprint-"))
    try:
        archive = build_bundled(pack_name, staging / f"{pack_name}.gideon")
