...
if TYPE_CHECKING:  # pragma: no cover - typing only
    from gideon.extensions.packs.import_ import ImportPlan
...
def apply_update(
    ...
    try:
        ...
        if plan.needs_consent and not consent:
            raise PackUpdateError(
                "update archive needs explicit consent — WARNING component(s)"
            )
        ...
        try:
            ...
            journal.rollback()
            logger.error(
                "pack update %s faulted and was rolled back: %s", pack_name, exc
            )
