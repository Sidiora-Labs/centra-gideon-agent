    ...
    def from_dict(cls, raw: dict[str, Any]) -> "CatalogEntry":
        return cls(
            ...
            required_credentials=[
                str(c)
                for c in (raw.get("required_credentials") or [])
                if str(c).strip()
            ],
...
def load_catalog(home: Path | None = None) -> list[CatalogEntry]:
    ...
    if not isinstance(raw, list):
        return list(_SEED_CATALOG)
    return [
        CatalogEntry.from_dict(r) for r in raw if isinstance(r, dict) and r.get("name")
    ]
...
def resolve_connector(
    ...
    * ``skip`` — record the ``connector_missing:<name>`` marker; write nothing.
    ...
    if mode == "substitute":
        ...
        if not sub:
            raise ConnectorResolutionError(
                "substitute mode needs a substitute connector name"
            )
        entry = catalog_lookup(sub, home)
        if entry is None:
            raise ConnectorResolutionError(
                f"substitute {sub!r} is not in the connector catalog"
            )
    ...
    if mode == "configure":
        ...
        if entry is None:
            entry = CatalogEntry(
                ...
                args=[
                    str(a) for a in (declaration.get("args") or []) if str(a).strip()
                ],
                required_credentials=[
                    str(c)
                    for c in (
                        (declaration.get("auth") or {}).get("required_credentials")
                        or []
                    )
                    if str(c).strip()
                ],
