...
1. **The card is FENCED before any model sees it.** :func:`gideon.security.security.fence_untrusted`
...
def build_entity(parsed: dict[str, Any]) -> tuple[str, Any, dict[str, Any]]:
    ...
    if not name:
        raise PromptCardError(
            f"{parsed.get('name')!r} is not a usable name (a-z, 0-9, hyphens)"
        )
    ...
    if target == "agent":
        ...
        if errors:
            raise PromptCardError(
                f"the card produced an invalid agent: {'; '.join(errors)}"
            )
        ...
        children.append(
            {
                "kind": "infer",
                "id": sid,
                "config": {"model_tier": "reasoning", "prompt": text},
            }
        )
    if len(children) < 2:
        raise PromptCardError(
...
async def import_prompt_card(card: str) -> dict[str, Any]:
    ...
    """
    from gideon.cognition.learning.proposals import ChangeManifest, enqueue
    ...
    verdict, proposal = enqueue(
        ...
        change_manifest=ChangeManifest(
            ...
            predicted_fixes=[
                f"{target} {name} becomes runnable and editable in-product"
            ],
        ),
...
def _payload_from_body(body: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise PromptCardError(
            f"the proposal body's JSON payload is unreadable: {exc}"
        ) from exc
    if not isinstance(parsed, dict):
