...
from gideon.extensions.packs import lint as pack_lint
from gideon.extensions.packs import roster as pack_roster
from gideon.extensions.packs.build import SCHEMA_VERSION
...
class PackImportRefused(Exception):
    ...
    def __init__(
        self, reason: str, message: str, plan: "ImportPlan | None" = None
    ) -> None:
        self.reason = reason
        self.plan = plan
...
class PackMarketplace(SkillsMarketplace):
    ...
    def __init__(
        self, files_by_skill: dict[str, list[dict[str, Any]]], tier: str
    ) -> None:
    ...
    for info in zf.infolist():
        ...
        if not _safe_member(info.filename):
            raise PackImportRefused(
                "integrity", f"unsafe pack member path: {info.filename!r}"
            )
        raw = zf.read(info)
...
def _recompute_content_hash(
    components: list[dict[str, Any]], members: dict[str, bytes]
) -> str:
...
def _parse_component(
    kind: str, cid: str, path: str, depends_on: list[str], raw: bytes
) -> _Comp:
    ...
    """
    comp = _Comp(
        kind=kind, id=cid, path=path, depends_on=list(depends_on), target_id=cid
    )
...
def _skill_files_from_members(
    ...
    from gideon.extensions.skills.marketplace import read_skill_file_entry
...
def _build_plan(
    ...
    lint_report = pack_lint.lint_pack(
        ...
        local_resolver=lambda ref: (
            _local_exists(home, *ref.split(":", 1)) if ":" in ref else False
        ),
    )
    ...
    integrity_detail = (
        ""
        if integrity_ok
        else f"content_hash mismatch: manifest={claimed!r} actual={actual!r}"
    )
    ...
    plan = ImportPlan(
        ...
        pack_owned=[
            str(p) for p in (manifest.get("pack_owned") or []) if str(p).strip()
        ],
    )
    return plan, parsed
...
def _setup_skill_files(
    quarantine: Path, members: dict[str, bytes]
...
def _editable_config_keys(raw: bytes | None) -> list[str]:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 — no allowlist reachable ⇒ stage nothing (fail closed)
        return []
    ...
    def rollback(self) -> None:
        ...
        for entry in reversed(self._entries):
            ...
            try:
                ...
                logger.warning(
                    "pack import rollback: could not unwind %s", path, exc_info=True
                )
        self.discard()
    ...
    def discard(self) -> None:
        ...
        try:
            if self._path.is_file():
                self._path.unlink()
            if (
                self._created_installing
                and self._dir.is_dir()
                and not any(self._dir.iterdir())
            ):
                self._dir.rmdir()
            packs_dir = self._home / "packs"
            if (
                self._created_packs
                and packs_dir.is_dir()
                and not any(packs_dir.iterdir())
            ):
                packs_dir.rmdir()
        except OSError:
...
def _commit_file_component(
    comp: _Comp, home: Path, journal: _Journal, stage: str
) -> Path:
...
def _commit_skill(
    comp: _Comp, home: Path, marketplace_name: str, journal: _Journal
) -> Path:
...
def inspect_pack(path: Path | str, *, tier: "Any" = None) -> ImportPlan:
    ...
    try:
        try:
            ...
            raise PackImportRefused(
                "integrity", f"not a readable .gideon archive: {exc}"
            ) from exc
        with zf:
        ...
        _audit(
            ...
            (
                "blocked"
                if plan.blocked
                else ("needs_confirm" if plan.needs_consent else "clean")
            ),
    ...
    try:
        ...
        if not plan.integrity_ok:
            _audit(
                ...
                "refused",
                resources=plan.name,
                error=plan.integrity_detail,
            )
            raise PackImportRefused("integrity", plan.integrity_detail, plan)
        if not plan.lint.ok:
            detail = "; ".join(
                f"{f.code}:{f.ref}: {f.detail}" for f in plan.lint.errors
            )
            _audit(
                "pack_import", "refused", resources=plan.name, error=f"lint: {detail}"
            )
            raise PackImportRefused(
                "lint", f"referential-integrity lint failed: {detail}", plan
            )
        if plan.has_dangerous:
            bad = ", ".join(c.ref for c in plan.components if c.verdict == "dangerous")
            _audit(
                "pack_import", "refused", resources=plan.name, error=f"dangerous: {bad}"
            )
            raise PackImportRefused(
                "dangerous",
                f"refused — DANGEROUS component(s), never overridable: {bad}",
                plan,
            )
        if plan.needs_consent and not consent:
            ...
            _audit(
                ...
                "needs_confirm",
                resources=plan.name,
                error=f"warning: {warn}",
            )
            raise PackImportRefused(
                "needs_consent",
                f"needs explicit consent — WARNING component(s): {warn}",
                plan,
            )
        ...
        skill_files = {
            c.target_id: (c.skill_files or []) for c in parsed if c.kind == "skill"
        }
        ...
        try:
            ...
            if isinstance(exc, PackImportRefused):
                raise
            raise PackImportRefused(
                "fault", f"import faulted and was rolled back: {exc}", plan
            )
        finally:
...
def _resolve_and_record(
    ...
    from gideon.extensions.packs import connectors as pack_connectors
    ...
    resolutions = pack_connectors.resolve_for_import(
        plan.connectors, connector_choices, home=home
    )
...
def _read_manifest(members: dict[str, bytes]) -> dict[str, Any]:
    ...
    try:
        ...
        raise PackImportRefused(
            "integrity", f"pack.json is not valid JSON: {exc}"
        ) from exc
