...
So skill bodies now enter :func:`gideon.cognition.learning.surfacing.allocate` as candidates
...
from gideon.cognition.learning.surfacing import Candidate, allocate, count_tokens
...
def reduced_block(name: str, summary: str, resources: list[SkillResource]) -> str:
    ...
    if not summary and not resources:
        return ""
    lines = [
        f"[Skill: {name} — REDUCED to its declared summary; the full body is over budget]"
    ]
    if summary:
        lines.append(summary)
    if resources:
        lines.append(
            f'Entry points (read with skill_resource(skill="{name}", path="…")):'
        )
        for res in resources[:MAX_ENTRY_POINTS]:
            lines.append(
                f"- {res.path}: {res.description}"
                if res.description
                else f"- {res.path}"
            )
...
def allocate_skills(
    ...
    *,
    ...
    for req in requests:
        body = ProcedureLibrary.strip_frontmatter(req.content).strip()
        if not body:
            continue
