...
half: ``ProcedureLibrary.get_triggered_skills`` matches purely on **keyword word-overlap**
...
def _keyword_score(query_words: set[str], triggers: str) -> tuple[float, bool]:
    ...
    ``ProcedureLibrary.get_triggered_skills`` exactly so the keyword half is unchanged.
    """
    best = 0.0
...
def surface_skills(
    ...
    *skills* is ``ProcedureLibrary.list_skills(with_usage=True)`` output (needs
    ...
    *suppressed* (FS-6) is Feedback-Signal's withholding set — the
    ...
    empty = suppress nothing; the live wiring (``ProcedureLibrary.get_surfaced_skills``)
    ...
    for s in skills:
        ...
        kw_score, negated = (
            _keyword_score(query_words, triggers) if triggers else (0.0, False)
        )
        if negated:
            if explain:
                explained.append(
    ...
    for s in skills:
        ...
        if query_vec is not None:
            ...
            try:
                ...
                mtime = 0.0
            vec = cache.get_or_embed(
                s.get("path", key), embed_text, mtime, model, embed_fn
            )
            if vec is not None:
