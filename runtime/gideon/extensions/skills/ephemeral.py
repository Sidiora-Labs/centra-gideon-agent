...
clean SKILL.md into the chosen tier via ``ProcedureLibrary.create_skill`` (global) or
``ProcedureLibrary(agent=...).create_skill`` (agent-local, from skill-agent-local-tier),
...
def _session_slug(session_key: str) -> str:
    return (
        _SLUG_RE.sub("-", (session_key or "default").strip().lower()).strip("-")[:80]
        or "default"
    )
...
def promote(
    ...
    *,
    ...
    if scope == "agent":
        ...
        loader = ProcedureLibrary(install_builtins=False, agent=agent)
