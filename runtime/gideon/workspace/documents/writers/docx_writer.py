...
def _add_hyperlink(para, run, *, monospace: bool = False, bold: bool = False) -> None:
    ...
    rel_id = para.part.relate_to(
        run.link, RELATIONSHIP_TYPE.HYPERLINK, is_external=True
    )
    ...
    **A named size is applied before orientation**, and it decides the page dimensions
    ...
    for section in doc.sections:
        if width_in and height_in:
            section.page_width, section.page_height = Inches(width_in), Inches(
                height_in
            )
        elif (
            page.orientation == "landscape" and section.page_width < section.page_height
        ):
            ...
            page.orientation == "portrait" and section.page_height < section.page_width
        ):
            section.page_width, section.page_height = (
                section.page_height,
                section.page_width,
...
def _add_page_number_field(paragraph, *, separator: bool) -> None:
    ...
    if separator:
        paragraph.add_run(" ")
    field = paragraph.add_run()._r.makeelement(
        qn("w:fldSimple"), {qn("w:instr"): "PAGE"}
    )
...
def _add_cell_table(doc, rows) -> None:
    ...
    for r, row in enumerate(rows):
        for c in range(width):
            _fill_cell(
                table.cell(r, c), row[c] if c < len(row) else None, header=r == 0
            )
