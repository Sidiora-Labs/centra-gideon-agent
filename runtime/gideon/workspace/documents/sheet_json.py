...
:attr:`~gideon.workspace.documents.model.Sheet.rows` is a derived property and therefore
    ...
    return Sheet(
        ...
        cells=[
            [
                ...
                for col_index, cell in enumerate(
                    _sequence(row, f"{where}.cells[{row_index}]")
                )
            ]
            for row_index, row in enumerate(
                _sequence(data.get("cells"), f"{where}.cells")
            )
        ],
        column_widths=[
        ...
        merges=[
            ...
            for index, ref in enumerate(
                _sequence(data.get("merges"), f"{where}.merges")
            )
        ],
