    ...
    while i < len(text):
        ...
        if ch == "`":
            ...
            if close > i + 1:
                ...
                body = text[i + 1 : close]
                out.append(
                    Run(text=body, bold=bold, italic=italic, code=True, link=link)
                )
                i = close + 1
                continue
        elif ch == "[":
            ...
            if m and m.group(1):
                flush_literal()
                out.extend(
                    _scan(m.group(1), bold=bold, italic=italic, link=m.group(2) or link)
                )
                i = m.end()
                continue
        elif ch in "*_":
...
def document_from_markdown(md: str, *, title: str = "") -> DocumentModel:
    ...
    def flush() -> None:
        ...
        if para:
            model.blocks.append(
                Block(kind="paragraph", runs=parse_inline(" ".join(para)))
            )
            para = []
        if bullets:
