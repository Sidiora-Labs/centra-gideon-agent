...
class LossItem:
    ...
    def __post_init__(self) -> None:
        if self.kind not in LOSS_KINDS:
            raise ValueError(
                f"unknown loss kind {self.kind!r}; expected one of {LOSS_KINDS}"
            )
...
def _unrepresentable(part: _Xml, paragraphs: list) -> list[str]:
    ...
    if len(paragraphs) > 1:
        reasons.append(
            f"it has {len(paragraphs)} non-empty paragraphs and the model holds one"
        )
    drawings = sum(
        1 for node in part._element.iter() if _local(node) in ("drawing", "pict")
    )
    if drawings:
...
class _Parser:
        ...
        for number, section in enumerate(sections):
            for name in ("header", "footer"):
                ...
                if number > 0 and (paragraphs or has_page_field):
                    reasons.append(
                        f"it belongs to section {number}, and the model holds one"
                    )
                if reasons:
                    self.report.add(
                        "header_footer",
    ...
    def _paragraph(self, para: _Xml) -> None:
        ...
        if lowered == "title":
            ...
            self._blocks.append(
                self._text_block("heading", runs, text, style=style, level=1)
            )
            return
        ...
        if heading:
            ...
            self._blocks.append(
                self._text_block("heading", runs, text, style=style, level=level)
            )
            return
        ...
        if lowered not in _CONSUMED_STYLES:
            self.report.add(
                "paragraph_style",
                f"paragraph style {style_name!r} has no model field; kept as a plain "
                "paragraph",
                block_index=index,
                paragraph_ordinal=ordinal,
            )
    ...
    def _list_item(
        ...
        if self._pending_list is None:
            self._pending_list = Block(
                kind=kind, items=[], style=self._style(para, index, ordinal)
            )
        else:
    ...
    def _run(
        self, source: _Xml, index: int, ordinal: int, *, link: str = ""
    ) -> Run | None:
    ...
    def _run_props(self, source: _Xml, index: int, ordinal: int, *, link: str) -> None:
        ...
        for child in properties.iterchildren():
            ...
            if name in _RUN_PROPS:
                described = f"{_RUN_PROPS[name]} ({name}" + (
...
def _page_breaks(element: _Xml) -> int:
    ...
    return sum(
        1
        for node in element.iter()
        if _local(node) == "br" and _attr(node, "type") == "page"
    )
...
def _plain(runs: list[Run], *, ignore_code: bool = False) -> bool:
    ...
    return all(
        ...
        and not run.italic
        and not run.link
        and (ignore_code or not run.code)
        for run in runs
    )
