...
def _audit(event_type: str, resources: str) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            "SEL audit event '%s' failed: %s", event_type, e
        )
...
def _everything_paths(pc: Path) -> list[str]:
    ...
    Derived from :mod:`gideon.operations.durability.inventory` — the single manifest of
...
def _copytree_safe(src: Path, dst: Path, **kwargs) -> None:
    ...
    def _ignore_symlinks(directory, contents):
        skipped = {
            name for name in contents if os.path.islink(os.path.join(directory, name))
        }
        for name in skipped:
            print(
...
def snapshot_main(
    ...
    if pc.is_dir():
        ...
        if total_mb > 500:
            print(
                f"⚠️  ~/.gideon is {total_mb:.0f} MB — snapshot may be large and slow"
            )
...
def _domain_counts(stage: Path) -> dict[str, dict[str, int]]:
    ...
    """
    from gideon.operations.durability import inventory as inv
    from gideon.workspace.portability import domain_of
...
def _print_manifest(snap: Path) -> None:
    ...
    try:
        ...
        _auto_kb = (
            c.get("triggers_json", 0)
            + c.get("event_triggers_json", 0)
            + c.get("crons_json", 0)
        ) // 1024
...
def _both_have(conn: "sqlite3.Connection", table: str, column: str) -> bool:
    ...
    for prefix in ("", "src."):
        try:
            cols = {
                r[1]
                for r in conn.execute(f"PRAGMA {prefix}table_info({table})").fetchall()
            }
...
def _merge_memory(src_db: Path, dst_db: Path) -> None:
    ...
    try:
        ...
        def _with_contributor(base: str, table: str) -> str:
            return (
                f"{base}, contributor"
                if _both_have(conn, table, "contributor")
                else base
            )
        ...
        for table, cols, where in [
            ...
            (
                "knowledge_facts",
                "subject, predicate, object, episode_id, created_at",
                "",
            ),
            (
                "knowledge_edges",
...
def _merge_event_triggers(src_path: Path, dst_path: Path) -> None:
    ...
    total = len(src_rows)
    print(
        f"  Event triggers imported: {imported} (skipped {total - imported} duplicates)"
    )
...
def _merge_json_collection(
    ...
    added = [
        r
        for r in src_rows
        if isinstance(r, dict) and r.get(key) is not None and r[key] not in seen
    ]
    if not added:
        return 0
...
def _merge_sqlite_attach(src_db: Path, dst_db: Path, label: str) -> int:
    ...
    try:
        ...
        Exception
    ) as exc:  # noqa: BLE001 — a corrupt source must not abort the restore
...
def _merge_security_events(snap: Path, pc: Path) -> None:
    ...
    * a LIVE home keeps its own key → the snapshot's rows could never verify under it, so importing
    ...
    if (
        snap_key is None
        or live_key is None
        or not hmac.compare_digest(snap_key, live_key)
    ):
        print(
            "  Security events: skipped (HMAC key differs — imported rows could not verify)"
        )
        return
...
def _merge_run_history(src_dir: Path, dst_dir: Path) -> None:
    ...
    Deliberately does NOT rotate afterwards. `ExecutionJournal.rotate_all()` runs at gateway boot
...
def merge_plan(snap: Path, pc: Path, components: list[str] | None) -> list[dict]:
    ...
    def _add(path: str, strategy: str, detail: str = "") -> None:
        ...
        if not dst.exists():
            ...
            action = "merge"
        rows.append(
            {"path": path, "strategy": strategy, "action": action, "detail": detail}
        )
    ...
    if _want(components, "memory"):
        _add(
            "memory.db",
            inv.MERGE_SQLITE_ATTACH_IGNORE,
            "4-table allowlist, is_deleted=0 only",
        )
    ...
    if _want(components, "security"):
        ...
        _add(
            ...
            inv.MERGE_APPEND_DEDUP,
            "dedup on event_id; HMAC-key gated",
        )
    if _want(components, "workspace"):
...
def restore_main(
    ...
    if args.mode is None and populated:
        print(
            f"🔀 Home holds {len(populated)} existing store(s) — proposing MERGE mode."
        )
        print(
            "   Use --mode replace to overwrite instead (previous state is kept aside)."
        )
    ...
    with tempfile.TemporaryDirectory() as work_str:
        ...
        if args.dry_run:
            ...
            if mode == "merge":
                ...
                print(
                    f"  Current state would be moved to {pc}/pre-restore-<timestamp>/"
                )
            return 0
    ...
    if _want(components, "memory") and (pc / "memory.db").is_file():
        ...
        if result == "ok":
            ...
            _audit(
                ...
                f"reason=integrity_check_failed from={snap_path.name}",
            )
            return 1
