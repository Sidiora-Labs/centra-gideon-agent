...
``ConsoleState.notify`` fails open).
"""
...
def assert_content_free(payload: Any) -> dict[str, str]:
    ...
    if not isinstance(payload, dict):
        raise PushPayloadError(
            f"push payload must be a dict, got {type(payload).__name__}"
        )
    keys = set(payload)
...
def payload_bytes(payload: dict[str, str]) -> bytes:
    ...
    return json.dumps(
        assert_content_free(payload), separators=(",", ":"), sort_keys=True
    ).encode()
...
def load_relay_tokens() -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            "push: relay token store unreadable — treating as empty", exc_info=True
        )
        return {}
...
def encrypt_payload(
    plaintext: bytes, p256dh: str, auth: str, *, salt: bytes = b""
) -> bytes:
    ...
    *salt* is a parameter only so a test can pin the whole body byte-for-byte against the
    ...
    header = (
        salt + _RECORD_SIZE.to_bytes(4, "big") + len(as_public_raw).to_bytes(1, "big")
    )
...
def send_webpush(device_id: str, payload: dict[str, str]) -> bool:
    ...
    try:
        ...
        if exc.code in (404, 410):
            ...
            logger.info(
                "push: endpoint gone for device %s — subscription dropped", device_id
            )
        else:
...
def send_relay(
    payload: dict[str, str], relay_url: str, platform: str, token: str
) -> bool:
    ...
    **This is plan 42's ``push`` target.** The rules engine decides WHETHER to call it
    ...
    if backend == "ntfy":
        ...
        delivered = sum(
            ...
            if send_relay(
                payload, url, str(row.get("platform", "")), str(row.get("token", ""))
            )
        )
    elif backend == "webpush":
        delivered = sum(
            1 for device in list(load_subscriptions()) if send_webpush(device, payload)
        )
    else:
...
def deliver_async(kind: str, item_id: str) -> None:
    ...
    Every call site is a notification chokepoint (``ConsoleState.notify``,
...
def push_cmd(args: Any) -> int:
    ...
    if action == "test":
        ...
        payload = content_free_payload(kind, item_id)
        print(
            f"backend={push_backend()} delivered={sent} payload={json.dumps(payload)}"
        )
        if not sent:
    ...
    print(
        f"VAPID keypair:  {'present' if status['vapid_ready'] else 'MISSING (run push init)'}"
    )
    print(
        f"ntfy topic:     {'configured' if status['ntfy_configured'] else 'not configured'}"
    )
