...
class NativeArtifactProvider(ArtifactProvider):
    ...
    def _try_write_source_path(self, source_path: str, content: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "artifact source_path write failed: %s", resolved, exc_info=True
            )
            return False
    ...
    def _version_content(self, slug: str, version: int) -> str | None:
        return self._read_text(
            self._artifact_dir(slug) / "versions" / f"v{version}.html"
        )
        ...
        *,
        ...
        with self._lock:
            ...
            final_slug = (
                ...
                if (
                    slug
                    and is_valid_slug(base)
                    and not (self._ensure_root() / base).exists()
                )
        ...
        with self._lock:
            ...
            if snapshot:
                ...
                if snap_content is None:
                    snap_content = (
                        self._try_read_source_path(art.source_path)
                        if art.source_path
                        else None
                    )
                    if snap_content is None:
            ...
            if cut_version or content_changed:
                resolved_type = event_type or (
                    "iterated" if actor == "agent" else "edited"
                )
                ev = ArtifactEvent(
                    ts=_now(),
                    type=resolved_type,
