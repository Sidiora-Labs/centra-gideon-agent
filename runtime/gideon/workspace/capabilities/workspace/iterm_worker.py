...
def text(value, limit):
    return "".join(
        c for c in str(value)[:limit] if c == "\t" or ord(c) >= 32 and ord(c) != 127
    )
    ...
    for window in app.windows:
        for tab in window.tabs:
            for session in tab.sessions:
                ...
                if visited > 100:
                    if identity:
                        raise ValueError("Native inventory bound exceeded")
                    return {"panes": panes, "truncated": True}
                metadata = {
                    "id": session.session_id,
                    "window_id": str(window.window_id),
                    "tab_id": str(tab.tab_id),
                    "title": text(session.name, 256),
                ...
                if identity is None:
                    ...
                    return {
                        **metadata,
                        ...
                        "truncated": contents.number_of_lines > 500
                        or session.grid_size.width > 500,
                    }
