        ...
        result.append(
            {
                ...
                "image_supported": bool(available),
                "reason": (
                    None if available else "Configured Codex engine executable required"
                ),
            }
        )
    return result
...
def validate_image(data):
    if not isinstance(data, bytes) or not 0 < len(data) <= 8388608:
...
def upload(data, actor):
    ...
    artifact = get_provider().create_binary(
        ...
        data=data,
        mime=mime,
        source="workspace-provider-terminal",
        actor=actor,
    )
    return {
...
def prepare(session_id, provider_id, image=None):
    ...
    if image is not None:
        if (
            ...
            or set(image) != {"artifact_id", "version"}
            or not isinstance(image["artifact_id"], str)
            or type(image["version"]) is not int
            or image["version"] < 1
        ):
        ...
        if raw is None:
            raise FileNotFoundError("Image artifact bytes unavailable")
        mime = validate_image(raw[0])
        folder = config_dir() / "capabilities/workspace/terminal-images" / session_id
        if not folder.resolve().is_relative_to(config_dir().resolve()):
            raise ValueError("Image cache escapes runtime home")
...
def cleanup(session_id):
    if re.fullmatch(r"[a-f0-9]{12}", session_id):
        shutil.rmtree(
            config_dir() / "capabilities/workspace/terminal-images" / session_id,
            ignore_errors=True,
        )
