...
class PortRegistry:
    def __init__(self, root, *, allowed_ports=range(6000, 6100)):
        ...
        if (
            ...
            or len(self.allowed_ports) > 256
            or any(
                type(p) is not int or not 1024 <= p <= 65535 for p in self.allowed_ports
            )
        ):
        ...
        with sqlite3.connect(self.path) as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS reservations(id TEXT PRIMARY KEY, request_id TEXT UNIQUE, input TEXT NOT NULL, payload TEXT NOT NULL)"
            )
            for record_id, payload in db.execute(
                "SELECT id,payload FROM reservations"
            ).fetchall():
                ...
                if record["status"] == "held":
                    ...
                    db.execute(
                        "UPDATE reservations SET payload=? WHERE id=?",
    ...
    def reserve(self, payload):
        ...
        if requested is not None and (
            type(requested) is not int or requested not in self.allowed_ports
        ):
        ...
        with self.mutex, sqlite3.connect(self.path) as db:
            ...
            previous = db.execute(
                "SELECT input,payload FROM reservations WHERE request_id=?",
                (payload["request_id"],),
            ).fetchone()
            if previous:
                if previous[0] != encoded:
                    raise ConflictError(
                        "Request ID already used for another reservation"
                    )
        ...
        with sqlite3.connect(self.path) as db:
            return [
                ...
                for row in db.execute(
                    "SELECT payload FROM reservations ORDER BY rowid DESC LIMIT ? OFFSET ?",
                    (limit, offset),
                )
            ]
    ...
    def release(self, reservation_id, revision):
        with self.mutex:
            ...
            with sqlite3.connect(self.path) as db:
                db.execute(
                    "UPDATE reservations SET payload=? WHERE id=?",
                    (json.dumps(record), reservation_id),
                )
            handle.close()
