...
class SnapshotStore:
    def __init__(self, root, *, allowed_roots):
        ...
        with self._db() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS snapshots (id TEXT PRIMARY KEY, request_id TEXT UNIQUE, input TEXT NOT NULL, payload TEXT NOT NULL, deleted INTEGER NOT NULL DEFAULT 0)"
            )
    ...
    def _live(self, raw):
        ...
        return {
            "branch": None if head == "HEAD" else head,
            "dirty": bool(
    ...
    def capture(self, payload, *, terminal_ids, task_ids):
        ...
        normalized = {
            **payload,
            "workspace": str(self._workspace(payload.get("workspace"))),
        ...
        with self._db() as db:
            ...
            previous = db.execute(
                "SELECT input,payload,deleted FROM snapshots WHERE request_id=?",
                (request_id,),
            ).fetchone()
            if previous:
                if previous[0] != encoded or previous[2]:
                    raise ConflictError(
                        "Request ID already used for another or deleted snapshot"
                    )
                return json.loads(previous[1])
            if set(normalized["terminal_ids"]) - set(terminal_ids) or set(
                normalized["task_ids"]
            ) - set(task_ids):
            ...
            record = {
                ...
                **self._live(normalized["workspace"]),
                "terminal_ids": normalized["terminal_ids"],
                "task_ids": normalized["task_ids"],
                "captured_at": datetime.now(timezone.utc).isoformat(),
                "revision": 1,
            }
            db.execute(
                "INSERT INTO snapshots(id,request_id,input,payload) VALUES(?,?,?,?)",
        ...
        with self._db() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT payload FROM snapshots WHERE deleted=0 ORDER BY rowid DESC LIMIT ? OFFSET ?",
                    (limit, offset),
                )
            ]
    ...
    def get(self, snapshot_id):
        with self._db() as db:
            row = db.execute(
                "SELECT payload FROM snapshots WHERE id=? AND deleted=0", (snapshot_id,)
            ).fetchone()
        if row is None:
    ...
    def delete(self, snapshot_id, revision):
        ...
        with self._db() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute(
                "SELECT payload,deleted FROM snapshots WHERE id=?", (snapshot_id,)
            ).fetchone()
            if row is None:
                raise FileNotFoundError("Snapshot not found")
    ...
    def reconcile(self, snapshot_id, *, terminal_ids, task_ids):
        ...
        result = {
            ...
            "branch_matches": (
                ...
                else snapshot["branch"] == live["branch"]
            ),
        }
