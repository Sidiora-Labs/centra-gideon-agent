...
class GitService:
    def __init__(self, root, *, allowed_roots, hierarchy=None):
        self.projects = ProjectService(
            root, allowed_roots=allowed_roots, hierarchy=hierarchy
        )
        self.root = Path(root)
        self.db_path = self.root / "git-operations.sqlite3"
        with sqlite3.connect(self.db_path) as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS operations(id TEXT PRIMARY KEY, request_id TEXT UNIQUE, input TEXT, payload TEXT)"
            )
    ...
    async def _git(self, path, *args):
        ...
        try:
            ...
            if proc.returncode:
                raise ValueError(
                    "Git operation failed; check repository state and cached objects"
                )
            return output
        finally:
    ...
    async def _repo(self, project_id):
        ...
        top = Path(
            (await self._git(path, "rev-parse", "--show-toplevel")).strip()
        ).resolve()
        if top != path:
    ...
    async def _check_git_dir(self, path):
        for arg in ("--absolute-git-dir", "--git-common-dir"):
            ...
            if not any(
                directory.is_relative_to(root) for root in self.projects.allowed_roots
            ):
                raise ValueError("Git metadata outside allowed roots")
        names = (
            (await self._git(path, "config", "--local", "--name-only", "--list"))
            .lower()
            .splitlines()
        )
        if any(
            name.startswith(("filter.", "include.", "includeif.")) for name in names
        ):
    ...
    async def inspect(self, project_id):
        ...
        for item in (await self._git(path, "ls-files", "--stage", "-z")).split("\0"):
            ...
            if initialized:
                ...
                childtop = Path(
                    (await self._git(child, "rev-parse", "--show-toplevel")).strip()
                ).resolve()
        ...
        return {
            ...
            "branch": branch,
            "status": status,
            "dirty": bool(status) or any(x["dirty"] for x in submodules),
    ...
    def list(self, project_id):
        ...
        with sqlite3.connect(self.db_path) as db:
            rows = db.execute(
                "SELECT payload FROM operations ORDER BY rowid DESC LIMIT 1000"
            ).fetchall()
        return [
            value
            for row in rows
            if (value := json.loads(row[0]))["project_id"] == project_id
        ][:100]
    ...
    async def mutate(self, payload):
        ...
        if any(
            not isinstance(v, str) or not v or len(v) > 256 or "\x00" in v
        ...
        with (
            (self.root / "git-operations.lock").open("a") as lock,
            ...
            previous = db.execute(
                "SELECT input,payload FROM operations WHERE request_id=?",
                (payload["request_id"],),
            ).fetchone()
            if previous:
                ...
                if record["status"] != "succeeded":
                    raise ConflictError(
                        "Prior attempt incomplete or failed; inspect before a new request"
                    )
                return record
            ...
            if state["head"] != payload["expected_head"] or state["dirty"]:
                raise ConflictError(
                    "Repository revision changed or working tree is dirty"
                )
            target = payload["target"]
            if operation == "submodule_update":
                child = next(
                    (x for x in state["submodules"] if x["path"] == target), None
                )
            ...
            try:
                ...
                record.update(
                    status="succeeded",
                    head_after=(await self._git(path, "rev-parse", "HEAD")).strip(),
                )
