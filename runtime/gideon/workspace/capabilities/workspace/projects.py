...
class ProjectService:
    def __init__(self, root, *, allowed_roots, hierarchy=None):
        ...
        with sqlite3.connect(self.db_path) as db:
            db.execute('CREATE TABLE IF NOT EXISTS operations(request_id TEXT PRIMARY KEY, input TEXT NOT NULL, project_id TEXT)')
    ...
    def detect(self, workspace):
        ...
        result = {'workspace':str(path),'name':path.name,'types':[],'commands':{},'ports':[],
                  'has_git':(path / '.git').exists(),'source_files':[]}
        ...
        if package.exists():
            ...
            for name, command in scripts.items():
                ...
                result['ports'].extend(int(port) for port in re.findall(r'--port[ =](\d{2,5})\b', command) if 0 < int(port) <= 65535)
        for name, kind in [('pyproject.toml','python'),('Cargo.toml','rust'),('Package.swift','swift')]:
            file = path / name
            if file.exists():
        ...
        for entry in sorted(entries[:256], key=lambda p:p.name):
            if not entry.is_symlink() and entry.name.endswith(('.xcodeproj','.xcworkspace')):
    ...
    def templates(self):
        return [{'id':'python-http','name':'Python HTTP service','entrypoint':'app.py','command':'python3 app.py','requires':'python3','available':shutil.which('python3') is not None},
                {'id':'node-http','name':'Node HTTP service','entrypoint':'server.mjs','command':'node server.mjs','requires':'node','available':shutil.which('node') is not None}]

    def get(self, project_id):
        project = self.hierarchy.get_project(project_id)
        if project is None:
            raise FileNotFoundError('Project not found')
        return {'project':project.to_dict(),'detection':self.detect(project.workspace_dir)}
    ...
    def _validate(self, payload, fields):
        ...
        for key, value in payload.items():
            if not isinstance(value, str) or not value.strip() or len(value) > (4096 if key in ('workspace','parent') else 256):
    ...
    def scaffold(self, payload):
        self._validate(payload, ['name','parent','directory','template','request_id'])
    ...
    def _operation(self, payload, workspace):
        ...
        with (self.root / 'project-operations.lock').open('a') as lock, sqlite3.connect(self.db_path) as db:
            fcntl.flock(lock, fcntl.LOCK_EX)
            row = db.execute('SELECT input,project_id FROM operations WHERE request_id=?', (payload['request_id'],)).fetchone()
            if row:
                if row[0] != encoded:
                    raise ConflictError('Request ID already used for another project operation')
                if row[1]:
                    return self.get(row[1])
            else:
                if 'template' in payload and workspace.exists():
                    raise ConflictError('Scaffold target already exists')
                db.execute('INSERT INTO operations VALUES(?,?,NULL)', (payload['request_id'], encoded))
                db.commit()
            ...
            if 'template' in payload:
                ...
                if not workspace.exists():
                    ...
                    with marker.open('x') as out:
                        json.dump({'request_id':payload['request_id'],'template':payload['template']}, out)
                elif workspace.is_symlink() or not marker.is_file() or marker.is_symlink() or json.loads(self._read(marker)).get('request_id') != payload['request_id']:
                    raise ConflictError('Scaffold target is not owned by this operation')
                if payload['template'] == 'python-http':
                    files = {'app.py':PYTHON_APP,'pyproject.toml':'[project]\nname = "local-http-service"\nversion = "0.1.0"\n'}
                else:
                    files = {'server.mjs':NODE_APP,'package.json':json.dumps({'name':payload['directory'].lower(),'version':'0.1.0','private':True,'type':'module','scripts':{'start':'node server.mjs'}})+'\n'}
                for name, text in files.items():
                    file = workspace / name
                    if file.exists() or file.is_symlink():
                        if file.is_symlink() or self._read(file) != text:
                            raise ConflictError('Scaffold file was changed; refusing overwrite')
                    else:
                        with file.open('x') as out:
                            out.write(text)
            project = existing or self.hierarchy.create_project(name=payload['name'],workspace_dir=str(workspace))
            db.execute('UPDATE operations SET project_id=? WHERE request_id=?', (project.id,payload['request_id']))
