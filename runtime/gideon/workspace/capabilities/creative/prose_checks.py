...
PHRASES = {
    "prose.cliches": "CLICHE_PHRASES",
    "prose.filter-words": "FILTER_WORDS",
    "prose.hedge-words": "HEDGE_WORDS",
    "prose.crutch-words": "CRUTCH_WORDS",
    "prose.slop-banned-words": "TIER1_BANNED_WORDS",
}
SUPPORTED = set(PHRASES) | {
...
def forms(word):
    return {
        ...
        word + "ed",
        word + "ing",
        word[:-1] + "ed" if word.endswith("e") else word + word[-1] + "ed",
...
def scan(check_id, source):
    ...
    def add(
        ...
        problem,
        suggestion="Review this passage in context before changing it.",
        **details,
    ):
    ...
    if check_id in PHRASES:
        ...
        for index, token in enumerate(tokens):
            ...
            if word.endswith("ly") and word not in LEX["NON_ADVERB_LY"]:
                tag = (
                    index > 0
                    and tokens[index - 1][0].casefold() in LEX["DIALOGUE_TAGS"]
                )
                kind = (
                    ("reporting" if word in LEX["REPORTING_TAG_ADVERBS"] else "emotion")
                    if tag
                    else None
                )
                add(
                    *token.span(),
                    "Review adverb" + (" after dialogue tag" if tag else ""),
                    dialogue_tag=tag,
                    tag_kind=kind,
                )
        ...
        for i, token in enumerate(tokens[:-1]):
            ...
            for following in tokens[i + 1 : i + 5]:
                ...
                if word.endswith("ed") or word in LEX["IRREGULAR_PARTICIPLES"]:
                    agent = bool(
                        re.match(
                            r"\s+by\s+(?!(?:the\s+)?(?:window|door|river|sea)\b)",
                            source[following.end() :],
                            re.I,
                        )
                    )
                    setting = (
                        i > 0 and tokens[i - 1][0].casefold() in LEX["SETTING_SUBJECTS"]
                    )
                    if agent or (
                        not setting and word not in LEX["STATIVE_PARTICIPLES"]
                    ):
                        add(
                            token.start(),
                            following.end(),
                            "Possible agentive passive voice",
                        )
                break
        ...
        for i, token in enumerate(tokens):
            ...
            if len(word) >= 4 and word not in LEX["STOPWORDS"]:
                if (
                    word in previous
                    and i - previous[word] <= 50
                    and word not in emitted
                ):
        ...
        if (
            check_id == "prose.sentence-rhythm"
            and fp["sentences"] >= 8
            and fp["metrics"]["sentence_cv"] <= 0.35
        ):
            add(
                0,
                min(len(source), 4000),
                "Low sentence-length variation",
                coefficient=fp["metrics"]["sentence_cv"],
            )
        if check_id == "prose.burstiness":
            if source.count("—") >= 2 and fp["metrics"]["emdash_rate"] >= 15:
                ...
                add(
                    ...
                    rate=fp["metrics"]["emdash_rate"],
                )
            if (
                fp["paragraphs"] >= 4
                and fp["metrics"]["paragraph_mean"]
                and fp["metrics"]["paragraph_std"] / fp["metrics"]["paragraph_mean"]
                < 0.2
            ):
                add(0, min(len(source), 4000), "Uniform paragraph lengths")
        if (
            check_id == "style.reading-level"
            and fp["words"] >= 100
            and fp["metrics"]["sentence_mean"] > 30
        ):
            add(
                0,
                min(len(source), 4000),
                "Sustained long sentences increase reading burden",
                sentence_mean=fp["metrics"]["sentence_mean"],
            )
    elif check_id in ("prose.ai-tells", "prose.structural-tics"):
        patterns = (
            [
                r"\bcould(?:n[\'’]?t| not) help but\b",
                r"\beyes?\s+widened\b",
                r"\blet out (?:a|the) breath\b[^.!?\n]{0,60}\b(?:holding|held)\b",
                r"\ba wave of\s+[a-z]+\s+(?:washed|crashed|swept) over\b",
                r"\bheart\s+(?:pounded|hammered|thudded|slammed|raced)\s+(?:in|against)\s+(?:his|her|their|my|its)\s+chest\b",
                r"\ba sense of\s+(?:dread|unease|foreboding|wonder|awe|relief|peace|urgency)\b",
            ]
            if check_id == "prose.ai-tells"
            else [
                r"\bnot\s+(?:just|only)\b[^.!?\n]{1,100}?\bbut\b(?:\s+also)?[^.!?\n]{1,100}[.!?]",
                r"\bI[\'’]?m\s+not\s+saying\b[^.!?\n]{1,100}?\bI[\'’]?m\s+saying\b[^.!?\n]{1,100}[.!?]",
            ]
        )
            ...
            for sentence in sentences:
                ...
                if len(run) == 3:
                    add(
                        run[0].start(),
                        run[-1].end(),
                        "Three consecutive short sentences",
                    )
                    run = []
        ...
        for base in LEX["SAID_BOOKISMS"] + LEX["NON_SPEECH_TAGS"]:
            for token in tokens:
                ...
                if re.search(r'[,?!][”"]\s+(?:[\w’\'-]+\s+)?$', before) or re.match(
                    r'\s*,\s*[“"]', after
                ):
        ...
        tags = list(
            re.finditer(
                r'[,?!][”"]\s+(?:[\w’\'-]+\s+)?(said|asked|replied|whispered|shouted|muttered|answered)\b',
                source,
                re.I,
            )
        )
        if len(tags) >= 5:
            counts = {
                word: [m for m in tags if m[1].casefold() == word]
                for word in {m[1].casefold() for m in tags}
            }
            repeated = max(counts.values(), key=len)
            if len(repeated) / len(tags) >= 0.8:
                add(
                    *repeated[0].span(1),
                    "Dominant dialogue tag",
