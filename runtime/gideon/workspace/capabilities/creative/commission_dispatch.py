...
def _contains_override(value):
    if isinstance(value, dict):
        return any(
            str(key).lower() in FORBIDDEN or _contains_override(item)
...
def normalize_dispatch(ability, value, sources):
    ...
    if ability in ("image", "video"):
        ...
        if ability == "image":
            ...
            if not isinstance(controls, dict) or not isinstance(loras, list):
                raise CatalogError(
                    "Image controls and adapters must be typed collections"
                )
            return {
                "input": {
                    **media_input,
    ...
    if ability == "music":
        ...
        if set(value) != allowed:
            raise CatalogError(
                "Music dispatch requires a pinned track, prompt, duration, mode and license"
            )
        ...
        return {
            "track_id": identifier(value["track_id"]),
            "track_revision": integer(value["track_revision"]),
            "prompt": text(value["prompt"], 4100, True),
            "music_length_ms": duration,
            "force_instrumental": value["force_instrumental"],
            "license": text(value["license"], 500, True),
        }
...
class CommissionDispatcher:
    ...
    def media_jobs(self):
        if self._media_jobs is None:
            ...
            self._media_jobs = MediaJobs(
                self.home / "capabilities/media/jobs.sqlite3", sketches
            )
    ...
    def music_catalog(self):
        ...
        return MusicCatalog(
            self.home / "capabilities/music",
            NativeArtifactProvider(root=self.home / "artifacts"),
        )
    ...
    def music_generation(self):
        if self._music_generation is None:
            try:
                ...
                self._music_generation = MusicGeneration(
                    self.home, self.music_catalog()
                )
            except ImportError as exc:
                raise CatalogError(
                    "Canonical music generation is unavailable", 503
                ) from exc
    ...
    def music_video(self):
        if self._music_video is None:
            try:
                ...
                self._music_video = VideoStore(
                    self.home / "capabilities/music", self.music_catalog()
                )
            except ImportError as exc:
                raise CatalogError(
                    "Canonical music video rendering is unavailable", 503
                ) from exc
    ...
    async def submit(self, ability, config, run_id, attempt):
        ...
        try:
            ...
            if ability == "music":
                job = await self.music_generation().submit(
                    {"request_id": request_id, **config}
                )
                status = (
                    "completed"
            ...
            if ability == "music-video":
                job = await self.music_video().submit(
                    {"request_id": request_id, **config}
                )
                status = (
                    "completed"
            ...
            run = store.start(
                ...
                {
                    "request_id": request_id,
                    "series_revision": config["series_revision"],
                    "mode": config["mode"],
            ...
            unavailable = (
                ...
                or run.get("pause_reason") == "configured_model_failed"
            )
            status = (
                "external_unavailable"
                if unavailable
                else "completed" if run["status"] == "done" else "submitted"
            )
            artifacts = [
                {
                ...
                for row in run.get("source_pins", [])
            ...
            return self._receipt(
                ...
                run["status"],
                attempt,
                error_code=run.get("pause_reason", "") if unavailable else "",
                artifacts=artifacts,
            )
            ...
            if unavailable and not code:
                code = {
                    "image": "image_backend_unavailable",
                    "video": "video_backend_unavailable",
                    "music": "music_backend_unavailable",
                    "music-video": "music_video_backend_unavailable",
                    "series": "series_backend_unavailable",
                }[ability]
            return self._receipt(
                {
                    "image": "media_jobs",
                ...
                request_id,
                "",
                "external_unavailable" if unavailable else "failed",
                "",
                attempt,
