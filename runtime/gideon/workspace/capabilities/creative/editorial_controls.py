...
class EditorialControlsStore:
    ...
    def _scope(self, db, work_id):
        ...
        exists = db.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='series_chapters'"
        ).fetchone()
        if exists:
            rows = [
                row[0]
                for row in db.execute(
                    "SELECT series_id FROM series_chapters WHERE json_extract(record,'$.work_id')=? ORDER BY series_id",
                    (work_id,),
                )
            ]
            if len(rows) > 1:
    ...
    def state(self, work_id):
        with self.works.connection() as db:
            ...
            row = db.execute(
                "SELECT record FROM editorial_policies WHERE scope_id=?", (scope,)
            ).fetchone()
            custom = [
                json.loads(item[0])
                for item in db.execute(
                    "SELECT record FROM editorial_custom_checks WHERE scope_id=? ORDER BY rowid",
                    (scope,),
                )
            ]
            reviews = [
                json.loads(item[0])
                for item in db.execute(
                    "SELECT record FROM editorial_reviews WHERE work_id=? ORDER BY rowid DESC",
                    (work_id,),
                )
            ]
            cuts = [
                json.loads(item[0])
                for item in db.execute(
                    "SELECT record FROM editorial_cuts WHERE work_id=? ORDER BY rowid DESC",
                    (work_id,),
                )
            ]
        policy = (
        ...
        for review in reviews:
            ...
            review["stale"] = bool(missing) or current != review.get(
                "source_revisions", {}
            )
        return {
            "scope_id": scope,
    ...
    def configure(self, work_id, payload, known_ids):
        ...
        digest = hashlib.sha256(
            json.dumps({"work_id": work_id, **payload}, sort_keys=True).encode()
        ).hexdigest()
        with self.works.connection() as db:
            work = self.works._work(db, work_id)
            prior = db.execute(
                "SELECT payload_hash,record FROM editorial_policy_requests WHERE id=?",
                (request,),
            ).fetchone()
            if prior:
                if prior[0] != digest:
            ...
            scope = self._scope(db, work_id)
            current = db.execute(
                "SELECT revision FROM editorial_policies WHERE scope_id=?", (scope,)
            ).fetchone()
            ...
            db.execute(
                "INSERT OR REPLACE INTO editorial_policies VALUES(?,?,?)",
                (scope, revision + 1, json.dumps(record)),
            )
            db.execute(
                "INSERT INTO editorial_policy_requests VALUES(?,?,?)",
                (request, digest, json.dumps(record)),
            )
        return record
    ...
    def custom(self, work_id, payload):
        ...
        with self.works.connection() as db:
            work = self.works._work(db, work_id)
            prior = db.execute(
                "SELECT payload_hash,record FROM editorial_custom_requests WHERE id=?",
                (request,),
            ).fetchone()
            if prior:
                if prior[0] != digest:
            ...
            if operation == "create":
                custom_id = (
                    "custom-"
                    + hashlib.sha256((scope_id + ":" + request).encode()).hexdigest()[
                        :40
                    ]
                )
                revision = 1
                ...
                row = db.execute(
                    "SELECT revision,record FROM editorial_custom_checks WHERE id=? AND scope_id=?",
                    (custom_id, scope_id),
                ).fetchone()
                if row is None:
            ...
            if operation == "delete":
                ...
                db.execute(
                    "DELETE FROM editorial_custom_checks WHERE id=?", (custom_id,)
                )
            else:
                record = {
                    "id": custom_id,
                    "scope_id": scope_id,
                    "revision": revision,
                    "label": text(payload.get("label"), 200, True),
                    "prompt": text(payload.get("prompt"), 8000, True),
                    "scope": payload.get("scope"),
                    "severity": payload.get("severity"),
                    "kind": "llm",
                }
                if (
                    record["scope"] not in ("work", "series")
                    or record["severity"] not in SEVERITIES
                ):
                    raise CatalogError("Invalid custom editorial scope or severity")
                db.execute(
                    "INSERT OR REPLACE INTO editorial_custom_checks VALUES(?,?,?,?)",
                    (custom_id, scope_id, revision, json.dumps(record)),
                )
            db.execute(
                "INSERT INTO editorial_custom_requests VALUES(?,?,?)",
                (request, digest, json.dumps(record)),
            )
        return record
    ...
    def readiness(self, findings, gate):
        blocking = {
            "block_high": {"high"},
            "block_medium": {"high", "medium"},
            "block_any": set(SEVERITIES),
        }[gate]
        return (
            "review_required"
    ...
    def _sources(self, work_id, mode):
        with self.works.connection() as db:
            ...
            if not scope.startswith("work:"):
                ids = [
                    json.loads(row[0])["work_id"]
                    for row in db.execute(
                        "SELECT record FROM series_chapters WHERE series_id=? ORDER BY rowid",
                        (scope,),
                    )
                ]
                ...
                sources.append(
                    {
                        ...
                        "text": current["text"][:20000],
                    }
                )
        if mode == "rank" and len(sources) < 2:
            raise CatalogError(
                "Comparative rank requires at least two drafted series works", 409
            )
    ...
    async def review(self, work_id, payload):
        ...
        with self.works.connection() as db:
            work = self.works._work(db, work_id)
            prior = db.execute(
                "SELECT payload_hash,record FROM editorial_reviews WHERE id=?",
                (review_id,),
            ).fetchone()
            if prior:
                if prior[0] != digest:
        ...
        prompt = (
            "Perform the requested editorial review using only the supplied immutable manuscript candidates. "
        ...
        try:
            ...
            status, output, reason = (
                ...
                None,
                "configured_model_unavailable_or_invalid_output",
            )
        record = {
            "id": review_id,
            "work_id": work_id,
        ...
        with self.works.connection() as db:
            ...
            db.execute(
                ...
                (review_id, work_id, digest, json.dumps(record)),
            )
        return record
    ...
    def _validate_review(mode, value, sources):
        ...
        if mode == "judge":
            if (
                set(value) != {"score", "verdict", "concerns"}
                or type(value["score"]) is not int
                or not 0 <= value["score"] <= 100
                or not isinstance(value["verdict"], str)
                or not isinstance(value["concerns"], list)
            ):
                raise ValueError("judge shape")
            for concern in value["concerns"]:
                if (
                    set(concern) != {"work_id", "quote", "reason"}
                    or concern["work_id"] not in ids
                    or concern["quote"] not in texts[concern["work_id"]]
                ):
                    raise ValueError("judge anchor")
        elif mode == "panel":
            if (
                set(value) != {"responses", "consensus"}
                or not isinstance(value["responses"], list)
                or not 3 <= len(value["responses"]) <= 6
                or not isinstance(value["consensus"], list)
            ):
                raise ValueError("panel shape")
            for row in value["responses"]:
                if (
                    set(row) != {"persona", "verdict", "concern_quote"}
                    or not all(isinstance(row[key], str) for key in row)
                    or (
                        row["concern_quote"]
                        and not any(
                            row["concern_quote"] in body for body in texts.values()
                        )
                    )
                ):
            ...
            if (
                set(value) != {"ranking", "weakest"}
                or not isinstance(value["ranking"], list)
                or not isinstance(value["weakest"], list)
            ):
                raise ValueError("rank shape")
            ranked = [
                row.get("work_id")
                for row in value["ranking"]
                if isinstance(row, dict) and set(row) == {"work_id", "rationale"}
            ]
            if (
                len(ranked) != len(ids)
                or set(ranked) != ids
                or any(item not in ids for item in value["weakest"])
            ):
    ...
    async def cut(self, editorial, work_id, run_id, finding_id, payload):
        ...
        with self.works.connection() as db:
            ...
            finding = next(
                (
                    ...
                    if item["id"] == identifier(finding_id)
                ),
                None,
            )
        if finding is None:
        ...
        with self.works.connection() as db:
            prior = db.execute(
                "SELECT payload_hash,record FROM editorial_cuts WHERE id=?", (cut_id,)
            ).fetchone()
            if prior:
                if prior[0] != digest:
                    raise CatalogError("Editorial cut request conflict", 409)
                return json.loads(prior[1])
        repaired = await editorial.repair(
            work_id, run_id, finding_id, {**payload, "replacement": ""}
        )
        record = {
            "id": cut_id,
            "work_id": work_id,
            "run_id": run_id,
            "finding_id": finding_id,
            "base_revision": integer(payload.get("work_revision")),
            "quote": finding["quote"],
        ...
        with self.works.connection() as db:
            db.execute(
                "INSERT INTO editorial_cuts VALUES(?,?,?,?)",
                (cut_id, work_id, digest, json.dumps(record)),
            )
        return record
    ...
    def apply_cut(self, work_id, cut_id, payload):
        ...
        with self.works.connection() as db:
            row = db.execute(
                "SELECT record FROM editorial_cuts WHERE id=? AND work_id=?",
                (identifier(cut_id), identifier(work_id)),
            ).fetchone()
            if row is None:
    ...
    def _promote(self, work_id, cut, payload):
        ...
        promotion = PolishingStore(self.works).promote(
            work_id, cut["proposal"]["id"], payload
        )
        record = {
            **cut,
            "status": "applied",
            "applied_revision": promotion["work"]["revision"],
            "promotion": promotion,
        }
        with self.works.connection() as db:
            db.execute(
                "UPDATE editorial_cuts SET record=? WHERE id=?",
                (json.dumps(record), cut["id"]),
            )
        return record
    ...
    def undo_cut(self, work_id, cut_id, payload):
        ...
        work = self.works.restore(
            work_id,
            {
                "revision": payload.get("revision"),
                "target_revision": cut["base_revision"],
            },
        )
        record = {
            **cut,
            "status": "undone",
            "undo_revision": work["revision"],
            "undo_work": work,
        }
        with self.works.connection() as db:
            db.execute(
