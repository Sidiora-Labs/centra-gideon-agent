...
class ContinuityStore:
    def __init__(self, works: WorkStore):
        ...
        with works.connection() as db:
            db.execute('CREATE TABLE IF NOT EXISTS continuity_proposals(id TEXT PRIMARY KEY, work_id TEXT, payload_hash TEXT, record TEXT)')
            db.execute('CREATE TABLE IF NOT EXISTS continuity_acceptances(proposal_id TEXT PRIMARY KEY, work_id TEXT, revision INTEGER, payload TEXT, UNIQUE(work_id,revision))')

    def _records(self, db, id):
        self.works._work(db, id)
        return [json.loads(r[0]) for r in db.execute('SELECT record FROM continuity_proposals WHERE work_id=? ORDER BY rowid', (id,))]

    def get(self, id):
        with self.works.connection() as db:
            work = self.works._work(db, id)
            records = self._records(db, id)
            accepted = dict(db.execute('SELECT proposal_id,revision FROM continuity_acceptances WHERE work_id=?', (id,)).fetchall())
        outline, facts = [], []
        for record in records:
            base = self.works.read_draft(id, record['base_draft_id'])
            record.update(accepted_revision=accepted.get(record['id']), missing=base['missing'], stale=work['active_draft_id'] != record['base_draft_id'])
            if record['id'] in accepted:
                common = {'proposal_id': record['id'], 'draft_id': record['base_draft_id'], 'artifact_id': record['artifact_id'], 'artifact_version': record['artifact_version'], 'mode': record['mode'], 'missing': record['missing'], 'stale': record['stale'], 'accepted_revision': accepted[record['id']]}
                outline.extend({**item, **common} for item in record['outline'])
                facts.extend({**item, **common} for item in record['facts'])
        outline.sort(key=lambda item: (item['draft_id'], item['start'], item['end'], item['accepted_revision']))
        groups = {}
        for fact in facts:
            groups.setdefault((fact['subject'], fact['predicate']), []).append(fact)
        conflicts = [{'subject': key[0], 'predicate': key[1], 'values': sorted({f['value'] for f in items}), 'proposal_ids': sorted({f['proposal_id'] for f in items})}
                     for key, items in groups.items() if len({f['value'] for f in items}) > 1]
        return {'work_id': id, 'revision': max(accepted.values(), default=0), 'proposals': records, 'outline': outline, 'facts': facts, 'conflicts': conflicts}
    ...
    def _entries(self, values, fields, source, start, end):
        ...
        for value in values:
            ...
            if quote != source[a:b]:
                raise CatalogError('Evidence quote must exactly match canonical draft codepoint offsets')
            result.append({**{field: text(value.get(field), 2000, required=True) for field in fields}, 'start': a, 'end': b, 'quote': quote})
        return result

    async def propose(self, id, payload):
        keys(payload, {'request_id', 'work_revision', 'start', 'end', 'mode', 'instruction', 'outline', 'facts'})
        ...
        with self.works.connection() as db:
            prior = db.execute('SELECT payload_hash,record FROM continuity_proposals WHERE id=?', (proposal_id,)).fetchone()
            if prior:
                if prior[0] != digest:
        ...
        if mode == 'model':
            ...
            prompt = ('Extract an ordered reverse outline and continuity facts from the supplied manuscript coverage. '
                      ...
                      json.dumps({'instruction': instruction, 'coverage_start': start, 'manuscript': source[start:end]}, ensure_ascii=False))
            try:
                ...
                raise CatalogError('Configured continuity model unavailable', 503) from exc
        elif mode == 'authored':
            values = payload
        else:
            raise CatalogError('Choose authored or model extraction')
        outline = self._entries(values.get('outline', []), ('summary',), source, start, end)
        facts = self._entries(values.get('facts', []), ('subject', 'predicate', 'value'), source, start, end)
        if not outline and not facts:
            raise CatalogError('Provide at least one grounded outline entry or fact')
        record = {'id': proposal_id, 'work_id': id, 'base_work_revision': expected, 'base_draft_id': base['id'],
                  'artifact_id': base['artifact_id'], 'artifact_version': base['artifact_version'],
                  'coverage': {'start': start, 'end': end, 'total_characters': len(source)}, 'mode': mode,
                  'outline': sorted(outline, key=lambda item: item['start']), 'facts': facts,
                  'created_at': datetime.now(timezone.utc).isoformat()}
        with self.works.connection() as db:
            current = self.works._work(db, id)
            if current['revision'] != expected or current['active_draft_id'] != base['id']:
                raise CatalogError('Work changed while extracting', 409)
            artifact = self.works.artifacts.get(base['artifact_id'], version=base['artifact_version'])
            ...
            if prior:
                if prior[0] != digest:
                    raise CatalogError('Proposal request conflict', 409)
                return json.loads(prior[1])
            db.execute('INSERT INTO continuity_proposals VALUES(?,?,?,?)', (proposal_id, id, digest, json.dumps(record)))
        return record
    ...
    def accept(self, id, proposal_id, payload):
        ...
        with self.works.connection() as db:
            work = self.works._work(db, id)
            records = self._records(db, id)
            record = next((r for r in records if r['id'] == identifier(proposal_id)), None)
            if record is None:
                raise CatalogError('Continuity proposal not found', 404)
            prior = db.execute('SELECT revision,payload FROM continuity_acceptances WHERE proposal_id=?', (proposal_id,)).fetchone()
            if prior:
            ...
            revision = db.execute('SELECT COALESCE(MAX(revision),0) FROM continuity_acceptances WHERE work_id=?', (id,)).fetchone()[0]
            if revision != expected or work['revision'] != work_revision or record['base_work_revision'] != work_revision or work['active_draft_id'] != record['base_draft_id']:
            ...
            db.execute('INSERT INTO continuity_acceptances VALUES(?,?,?,?)', (proposal_id, id, revision + 1, json.dumps(payload)))
