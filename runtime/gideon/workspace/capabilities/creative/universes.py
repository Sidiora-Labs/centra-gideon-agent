...
class UniverseStore(IngredientStore):
    ...
    def _universe(self, db, id):
        row = db.execute("SELECT record FROM universes WHERE id=?", (identifier(id),)).fetchone()
        if not row:
    ...
    def _values(self, db, value, history=()):
        ...
        if any(not isinstance(items, list) or len(items) > cap for items, cap in ((canon, 100), (links, 100), (refs, 100))):
        ...
        for entry in canon:
            ...
            seen.add(id)
            entries.append({"id": id, "title": text(entry.get("title"), 200, True), "body": text(entry.get("body", ""), 20000)})
        identity = value.get("visual_identity", {})
        keys(identity, {"colors", "style_notes"})
        colors = identity.get("colors", [])
        if not isinstance(colors, list) or len(colors) > 20 or any(not isinstance(c, str) or not re.fullmatch(r"#[0-9a-fA-F]{6}", c) for c in colors):
            raise CatalogError("Colors must be six-digit hex values")
        old_links = {id for row in history for id in row["ingredient_ids"]}
        old_refs = {(ref["id"], ref["revision"]) for row in history for ref in row["board_refs"]}
        ...
        for ref in refs:
            ...
            if pair not in old_refs and not db.execute("SELECT 1 FROM board_revisions WHERE id=? AND revision=?", pair).fetchone():
                raise CatalogError("Moodboard revision not found", 404)
            item = {"id": pair[0], "revision": pair[1]}
            if item not in normalized:
                normalized.append(item)
        return {"title": text(value.get("title"), 200, True), "canon": entries,
                "visual_identity": {"colors": [c.lower() for c in colors], "style_notes": text(identity.get("style_notes", ""), 20000)},
                "ingredient_ids": links, "board_refs": normalized}

    def _save(self, db, record):
        encoded = json.dumps(record, sort_keys=True)
        db.execute("INSERT INTO universe_revisions VALUES(?,?,?)", (record["id"], record["revision"], encoded))
        db.execute("INSERT OR REPLACE INTO universes VALUES(?,?)", (record["id"], encoded))
        return record
    ...
    def create(self, payload):
        ...
        with self.connection() as db:
            prior = db.execute("SELECT payload,record FROM universe_requests WHERE id=?", (request,)).fetchone()
            if prior:
                if prior[0] != encoded:
            ...
            db.execute("INSERT INTO universe_requests VALUES(?,?,?)", (request, encoded, json.dumps(record)))
            return record
    ...
    def update(self, id, patch):
        ...
        with self.connection() as db:
            ...
            history = [json.loads(r[0]) for r in db.execute("SELECT record FROM universe_revisions WHERE id=?", (id,))]
    ...
    def export(self, id, revision=None):
        with self.connection() as db:
            ...
            if revision is None:
                return current
            integer(revision)
            row = db.execute("SELECT record FROM universe_revisions WHERE id=? AND revision=?", (id, revision)).fetchone()
            if not row:
    ...
    def get(self, id):
        ...
        with self.connection() as db:
            ingredients = [{"id": id, "missing": db.execute("SELECT 1 FROM ingredients WHERE id=?", (id,)).fetchone() is None} for id in record["ingredient_ids"]]
            boards = [{**ref, "missing": db.execute("SELECT 1 FROM board_revisions WHERE id=? AND revision=?", (ref["id"], ref["revision"])).fetchone() is None} for ref in record["board_refs"]]
        return {**record, "ingredient_status": ingredients, "board_status": boards}

    def revisions(self, id):
        with self.connection() as db:
            self._universe(db, id)
            return [json.loads(row[0]) for row in db.execute("SELECT record FROM universe_revisions WHERE id=? ORDER BY revision DESC", (id,))]
    ...
    def list(self, q="", offset=0, limit=25):
        ...
        integer(limit, 1, 100)
        with self.connection() as db:
            rows = [json.loads(r[0]) for r in db.execute("SELECT record FROM universes ORDER BY id")]
