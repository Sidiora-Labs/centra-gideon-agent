...
class PeerFeedbackStore:
    ...
    def export(
        self, peer_id: str, commission_id: str, reaction_id: str, approval: dict
    ) -> dict:
        ...
        if (
            ...
            or approval.get("peer_id") != peer_id
            or approval.get("reaction_id") != reaction_id
        ):
            raise CatalogError(
                "Explicit peer-feedback approval does not match this delivery", 403
            )
        ...
        if not any(
            _output_matches(item, reaction["output"]) for item in run.get("outputs", [])
        ):
    ...
    def target(self, commission_id: str, run_id: str, output: dict) -> dict:
        ...
        run = next(
            (
                row
                for row in self.commissions.runs(commission_id)["items"]
                if row["id"] == run_id
            ),
            None,
        )
    ...
    def prepare(self, peer_id: str, lineage: dict, body: dict, approval: dict) -> dict:
        ...
        if not isinstance(body, dict):
            raise CatalogError("Peer-feedback reaction is required")
        ...
        identity = (
            ...
            + digest(
                ":".join(
                    (
                        lineage["commission_id"],
                        lineage["run_id"],
                        lineage["output"]["artifact_id"],
                        author,
                    )
                )
            )[:48]
        ...
        with self.db() as db:
            row = db.execute(
                "SELECT record FROM creative_feedback_outbox WHERE id=?", (identity,)
            ).fetchone()
            old = json.loads(row[0]) if row else None
            if old and integer(body.get("revision")) != old["revision"]:
                raise CatalogError("Feedback changed; reload", 409)
            if not old and "revision" in body:
                raise CatalogError("New feedback does not accept a revision")
            stamp = now_iso()
            reaction = {
                "id": identity,
                "commission_id": lineage["commission_id"],
            ...
            db.execute(
                "INSERT OR REPLACE INTO creative_feedback_outbox VALUES(?,?,?,?)",
                (
                    identity,
                    peer_id,
                    json.dumps(reaction, sort_keys=True),
    ...
    def revoke_prepared(
        self, peer_id: str, reaction_id: str, revision: int, approval: dict
    ) -> dict:
        peer_id, reaction_id = identifier(peer_id), identifier(reaction_id)
        with self.db() as db:
            row = db.execute(
                "SELECT record,lineage,peer_id FROM creative_feedback_outbox WHERE id=?",
                (reaction_id,),
            ).fetchone()
            if row is None:
                raise CatalogError("Prepared feedback not found", 404)
            ...
            reaction.update(
                revision=reaction["revision"] + 1,
                deleted=True,
                deleted_at=now_iso(),
                updated_at=now_iso(),
            )
            db.execute(
                "UPDATE creative_feedback_outbox SET record=? WHERE id=?",
    ...
    def _approve_target(peer_id: str, lineage: dict, approval: dict) -> None:
        ...
        if (
            approval.get("decision") != "approved"
            or approval.get("peer_id") != peer_id
            or approval.get("target_hash") != _hash(lineage)
        ):
            raise CatalogError(
                "Explicit peer-feedback approval does not match this target", 403
            )
    ...
    def _sign(self, peer_id: str, reaction: dict, lineage: dict) -> dict:
        ...
        payload["delivery_id"] = (
            "peer-feedback-"
            + digest(":".join((peer_id, reaction["id"], str(reaction["revision"]))))[
                :48
            ]
        )
    ...
    async def push_prepared(self, peer_id: str, payload: dict) -> dict:
        ...
        with self.db() as db:
            row = db.execute(
                "SELECT payload_hash,receipt FROM creative_feedback_sends WHERE id=?",
                (payload["delivery_id"],),
            ).fetchone()
        if row:
        ...
        if (
            ...
            or result.get("delivery_id") != payload["delivery_id"]
    ...
    def receive(self, sender: str, payload: dict) -> dict:
        ...
        with self.db() as db:
            prior = db.execute(
                "SELECT payload_hash,receipt FROM creative_feedback_deliveries WHERE id=?",
                (delivery_id,),
            ).fetchone()
            if prior:
                if prior[0] != payload_hash:
                    raise CatalogError(
                        "Feedback delivery was replayed with different content", 409
                    )
        ...
        with self.commissions.db() as db:
            ...
            if (
                current
                and current["revision"] == reaction["revision"]
                and current != reaction
            ):
                raise CatalogError(
                    "Feedback revision conflicts with local content", 409
                )
    ...
    def _verify_binding(self, sender: str, payload: dict) -> str:
        ...
        keys(
            binding,
            {"version", "sender", "recipient", "scope", "payload_hash", "signature"},
        )
        unsigned = {key: value for key, value in payload.items() if key != "binding"}
        _, identity = self.peers._identity()
        if (
            binding.get("version") != 1
            or binding.get("sender") != sender
            or binding.get("recipient") != identity["peer_id"]
            or binding.get("scope") != SCOPE
            or binding.get("payload_hash") != _hash(unsigned)
        ):
            raise CatalogError(
                "Peer-feedback payload binding does not match the signed delivery", 401
            )
        ...
        try:
            ...
            raise CatalogError(
                "Peer-feedback payload signature is invalid", 401
            ) from exc
    ...
    def _verify_lineage(self, reaction: dict, lineage: dict) -> None:
        try:
            ...
            run = next(
                row
                for row in self.commissions.runs(reaction["commission_id"])["items"]
                if row["id"] == reaction["run_id"]
            )
            project = self._direction().get(lineage["project"]["id"])
        except (CatalogError, StopIteration) as exc:
            raise CatalogError(
                "Peer-feedback canonical lineage is unavailable", 409
            ) from exc
        pins = [
            {"kind": item["kind"], "id": item["id"], "revision": item["revision"]}
            for item in project["sources"]
        ...
        if (
            run.get("project_id") != lineage["project"]["id"]
            or project["revision"] != lineage["project"]["revision"]
            or pins != lineage["sources"]
            or not any(
                _output_matches(item, reaction["output"])
                for item in run.get("outputs", [])
            )
        ):
            raise CatalogError(
                "Peer-feedback canonical lineage does not match local records", 409
            )

    @classmethod
    def _validate_lineage(cls, lineage: dict) -> None:
        if not isinstance(lineage, dict):
            raise CatalogError("Peer-feedback lineage is required")
        ...
        keys(project, {"id", "revision"})
        identifier(project.get("id"))
        integer(project.get("revision"), 1)
    ...
    def _validate(payload: dict) -> tuple[dict, dict, str]:
        ...
        keys(
            payload, {"schema_version", "delivery_id", "reaction", "lineage", "binding"}
        )
        ...
        run_id, author = identifier(reaction.get("run_id")), identifier(
            reaction.get("author")
        )
        expected_id = (
            "reaction-"
        ...
        if reaction.get("id") != expected_id or reaction.get("rating") not in {
            "liked",
            "disliked",
        }:
            raise CatalogError("Peer-feedback identity or rating is invalid")
        if (
            type(reaction.get("deleted")) is not bool
            or integer(reaction.get("revision"), 1) < 1
        ):
        ...
        for source in sources:
            ...
            integer(source.get("revision"), 1)
        if (
            lineage.get("commission_id") != commission_id
            or lineage.get("run_id") != run_id
            or lineage.get("output") != normalized_output
        ):
