...
SCHEMAS = {
    ...
    "creative_production_get": obj(
        {"series_id": STRING, "run_id": STRING}, ["series_id", "run_id"]
    ),
    "creative_production_start": obj(
        {
            "series_id": STRING,
            "payload": obj(
                {
                ...
                ["request_id", "series_revision", "mode", "max_attempts"],
            ),
        },
        ["series_id", "payload"],
    ),
    "creative_production_advance": obj(
        {"series_id": STRING, "run_id": STRING}, ["series_id", "run_id"]
    ),
    "creative_production_submit": obj(
        {
            "series_id": STRING,
            "run_id": STRING,
            "payload": obj(
                ...
                ["request_id", "work_revision", "text"],
            ),
        },
        ["series_id", "run_id", "payload"],
    ),
    "creative_production_approve": obj(
        {
            "series_id": STRING,
            "run_id": STRING,
            "payload": obj(
...
for action in ("rollback", "pause", "resume", "cancel"):
    SCHEMAS[f"creative_production_{action}"] = obj(
        {"series_id": STRING, "run_id": STRING}, ["series_id", "run_id"]
    )
    ...
    async def list_tools(self):
        return [
            ToolDefinition(
                ...
                risk_level=(
                    RiskLevel.SAFE
                    if name in ("creative_production_list", "creative_production_get")
