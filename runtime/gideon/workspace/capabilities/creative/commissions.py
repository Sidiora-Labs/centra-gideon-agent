...
def _recurrence_rule(value, anchor):
    ...
    try:
        ...
        raise CatalogError(
            "Recurrence interval and count must be positive integers"
        ) from None
...
def _recurrence_next(cadence, after):
    ...
    for _ in range(101):
        ...
        if candidate is None:
            return 0.0
        if (
            candidate.isoformat() not in excluded
            and candidate.date().isoformat() not in excluded
        ):
...
def _cadence_hash(cadence):
    return (
        digest({key: value for key, value in cadence.items() if key != "spec"})
        if cadence["kind"] == "recurrence"
        else digest(cadence["spec"])
    )
...
def _cadence(value):
    ...
    if kind == "daily":
        ...
        spec = {
            "kind": "cron",
            "expr": f"{minute} {hour} * * "
            + ("1-5" if value.get("weekdays_only") else "*"),
        }
    elif kind == "weekly":
        ...
        spec = {
            "kind": "interval",
            "interval_secs": integer(value.get("seconds"), 900, 31536000),
        }
        ...
        return {
            ...
            "rrule": rule,
            "timezone": zone_name,
            "exdates": list(dict.fromkeys(exclusions)),
            "spec": {"kind": "recurrence"},
        }
    else:
        raise CatalogError(
            "Choose daily, weekly, custom, interval or recurrence cadence"
        )
...
def _brief(value):
    ...
    return {
        "intent": text(value.get("intent"), 4000, True),
        "genre": text(value.get("genre", ""), 100),
        "category": text(value.get("category", ""), 100),
        "style": text(value.get("style", ""), 8000),
        "constraints": {
            text(str(k), 80, True): text(str(v), 500) for k, v in constraints.items()
        },
...
def _plan(value):
    ...
    for row in value:
        ...
        result.append(
            {
                "id": identity,
                "title": text(row.get("title"), 200, True),
...
class CommissionStore:
    ...
    def _save(self, db, record):
        db.execute(
            "INSERT OR REPLACE INTO creative_commissions VALUES(?,?)",
    ...
    def _direction(self):
        if self.direction is None:
            try:
                ...
                raise CatalogError(
                    "Creative direction capability is unavailable", 503
                ) from exc
    ...
    def create(self, payload):
        ...
        dispatch = (
            normalize_dispatch(ability, payload.get("dispatch"), sources)
        ...
        if mode == "planning" and dispatch is not None:
            raise CatalogError(
                "Planning mode does not accept ability dispatch configuration"
            )
        normalized = {
            "name": text(payload.get("name"), 200, True),
            "target_ability": ability,
            "brief": _brief(payload.get("brief")),
            "cadence": _cadence(payload.get("cadence")),
            "sources": sources,
            "steps": _plan(payload.get("steps")),
        ...
        with self.db() as db:
            prior = db.execute(
                "SELECT request_hash,commission_id FROM creative_commission_requests WHERE id=?",
                (request,),
            ).fetchone()
            if prior:
                if prior[0] != fingerprint:
                    raise CatalogError(
                        "Commission request already used with different values", 409
                    )
            ...
            db.execute(
                "INSERT INTO creative_commission_requests VALUES(?,?,?)",
                (request, fingerprint, record["id"]),
            )
    ...
    def list(self):
        with self.db() as db:
            return {
                "items": [
                    self._present(json.loads(row[0]))
                    for row in db.execute(
                        "SELECT record FROM creative_commissions ORDER BY rowid DESC"
                    )
                ]
            }
    ...
    def update(self, identity, payload):
        ...
        with self.db() as db:
            ...
            existing_dispatch = (
                record.get("dispatch")
                if "mode" in record or payload.get("mode") == "generate"
                else None
            )
            dispatch = (
                normalize_dispatch(
                    record["target_ability"], payload["dispatch"], record["sources"]
                )
                if "dispatch" in payload
                else existing_dispatch
            )
            if mode == "generate" and dispatch is None:
                raise CatalogError("Generate mode requires typed ability configuration")
            if mode == "planning" and dispatch is not None:
            ...
            if "name" in payload:
                record["name"] = text(payload["name"], 200, True)
            if "brief" in payload:
                record["brief"] = _brief(payload["brief"])
            record["mode"] = mode
            if dispatch is None:
    ...
    def _trigger(self, record, at=0.0):
        ...
        if record["cadence"]["kind"] == "recurrence":
            ...
            specification = {
                "kind": "at",
                "at": occurrence,
                "timezone": record["cadence"]["timezone"],
                "strict": True,
            }
        trigger = Trigger(
            id=TRIGGER_PREFIX + record["id"],
            name="Creative commission: " + record["name"],
            kind="clock",
            enabled=True,
            created_by="creative-commission",
            spec=specification,
            workflow={
                "inline": {
    ...
    def _sync(self, record, after=0.0):
        ...
        with self.db() as db:
            ...
            latest.update(
                schedule_error=error, schedule_state=state, next_fire_at=next_fire
            )
            return self._save(db, latest)

    def rearm_recurrence(
        self, commission_id, occurrence, schedule_revision, cadence_hash
    ):
        with self.db() as db:
        ...
        if record["cadence"]["kind"] != "recurrence" or not record["enabled"]:
            return record
        if (
            record["schedule_revision"] != schedule_revision
            or _cadence_hash(record["cadence"]) != cadence_hash
        ):
            return record
        try:
    ...
    def runs(self, commission_id):
        ...
        with self.db() as db:
            rows = [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT record FROM creative_commission_runs WHERE commission_id=? ORDER BY rowid DESC",
                    (commission_id,),
                )
            ]
        return {"items": rows}
    ...
    async def execute(
        ...
        with self.db() as db:
            ...
            if (
                ...
                and schedule_revision != commission["schedule_revision"]
            ):
                return {
                    "status": "skipped",
            ...
            if cadence_hash is not None and cadence_hash != _cadence_hash(
                commission["cadence"]
            ):
                return {
                    "status": "skipped",
            ...
            row = db.execute(
                "SELECT record FROM creative_commission_runs WHERE commission_id=? AND occurrence=?",
                (commission_id, occurrence),
            ).fetchone()
            run = json.loads(row[0]) if row else None
            if run and run["status"] in ("planned", "completed", "submitted"):
                return run
            if run and run["status"] == "dispatch_unknown":
                return run
            if run and run["status"] == "running":
                live = (
                    run.get("claim_identity")
                    and process_identity(run.get("claim_pid")) == run["claim_identity"]
                )
                if live:
                ...
                run.update(
                    status=status,
                    updated_at=now_iso(),
                    phase="unknown" if uncertain else "interrupted",
                )
            ...
            if run is None:
                run = {
                    ...
                    "feedback_refs": [],
                    "attempts": [],
                    "mode": commission.get("mode", "planning"),
        ...
        try:
            ...
            if mode == "generate" and not run.get("dispatch"):
                raise CatalogError(
                    "Generate commission has no typed ability configuration", 409
                )
            direction = self._direction()
            treatment = json.dumps(
                {
                    "ability": commission["target_ability"],
            ...
            if project_id:
                ...
                project = direction.create(
                    {
                        "request_id": "commission-" + run["id"][:48],
            ...
            if project["status"] == "draft":
                project = direction.control(
                    project_id, {"revision": project["revision"], "action": "start"}
                )
            for _ in range(len(commission["steps"])):
                if project["status"] == "completed":
                    break
                project = direction.advance(
                    project_id, {"revision": project["revision"]}
                )
            if project["status"] != "completed":
                raise CatalogError(
                    "Direction project did not complete its bounded plan", 409
                )
            outputs = [
                step["result"]
                for step in project["steps"]
                if step.get("result", {}).get("artifact_id")
            ]
            if mode == "generate":
                with self.db() as db:
                    current = json.loads(
                    ...
                    current.update(
                        phase="dispatching",
                        dispatch_request_id="commission-"
                        + run["id"][:48]
                        + "-"
                        + str(attempt),
                        updated_at=now_iso(),
                ...
                receipt = await self.dispatcher.submit(
                    commission["target_ability"], run["dispatch"], run["id"], attempt
                )
                if receipt["status"] in ("failed", "external_unavailable"):
                    raise CatalogError(
                        "Ability dispatch " + receipt["status"],
                        503 if receipt["status"] == "external_unavailable" else 409,
                    )
                outputs.extend(
                    row for row in receipt["artifact_refs"] if row.get("content_hash")
                )
                if receipt["status"] not in ("submitted", "completed"):
                    raise CatalogError(
                        "Ability dispatch did not return a terminal admission state",
                        409,
                    )
            ...
            error = (receipt or {}).get("error_code") or "execution-failed:" + type(
                exc
            ).__name__
        ...
        with self.db() as db:
            ...
            if receipt and not any(
                ...
                for row in current.get("dispatch_receipts", [])
            ):
    ...
    async def retry(self, commission_id, run_id):
        with self.db() as db:
            row = db.execute(
                "SELECT record FROM creative_commission_runs WHERE id=? AND commission_id=?",
                (identifier(run_id), identifier(commission_id)),
            ).fetchone()
            if not row:
                raise CatalogError("Commission run not found", 404)
            run = json.loads(row[0])
            if run["status"] != "failed":
                raise CatalogError("Only a retryable failed run can retry", 409)
        return await self.execute(
            commission_id, run["occurrence"], trigger=run["trigger"], retrying=True
        )
    ...
    def feedback(self, commission_id):
        ...
        with self.db() as db:
            rows = [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT record FROM creative_commission_feedback WHERE commission_id=? ORDER BY rowid DESC",
                    (commission_id,),
                )
            ]
        return {"items": rows}
    ...
    def react(self, commission_id, payload):
        keys(
            payload,
            {"run_id", "author", "output", "rating", "note", "tags", "revision"},
        )
        author = identifier(payload.get("author"))
        run_id = identifier(payload.get("run_id"))
        output = payload.get("output")
        if not isinstance(output, dict):
            raise CatalogError("Feedback output is required")
        keys(output, {"artifact_id", "artifact_version", "content_hash"})
        normalized_output = {
            "artifact_id": identifier(output.get("artifact_id")),
            "artifact_version": integer(output.get("artifact_version")),
            "content_hash": text(output.get("content_hash"), 64, True),
        }
        rating = payload.get("rating")
        if rating not in RATINGS:
            raise CatalogError("Feedback rating must be liked or disliked")
        ...
        identity = (
            ...
            + digest(
                ":".join(
                    (commission_id, run_id, normalized_output["artifact_id"], author)
                )
            )[:48]
        )
        with self.db() as db:
            run_row = db.execute(
                "SELECT record FROM creative_commission_runs WHERE id=? AND commission_id=?",
                (run_id, commission_id),
            ).fetchone()
            if not run_row:
                raise CatalogError("Commission run not found", 404)
            run = json.loads(run_row[0])
            linked = any(
                all(item.get(key) == value for key, value in normalized_output.items())
                for item in run["outputs"]
            )
            if not linked:
                raise CatalogError("Feedback output is not linked to this run", 409)
            prior = db.execute(
                "SELECT record FROM creative_commission_feedback WHERE id=?",
                (identity,),
            ).fetchone()
            old = json.loads(prior[0]) if prior else None
            if old and integer(payload.get("revision")) != old["revision"]:
                raise CatalogError("Feedback changed; reload", 409)
            if not old and "revision" in payload:
                raise CatalogError("New feedback does not accept a revision")
            stamp = now_iso()
            record = {
                "id": identity,
                "commission_id": commission_id,
    ...
    def remove_reaction(self, commission_id, reaction_id, payload):
        ...
        with self.db() as db:
            row = db.execute(
                ...
                (identifier(reaction_id), identifier(commission_id)),
            ).fetchone()
            if not row:
                raise CatalogError("Commission feedback not found", 404)
            record = json.loads(row[0])
            if record["author"] != identifier(payload.get("author")):
                raise CatalogError("Feedback author does not match", 403)
            if integer(payload.get("revision")) != record["revision"]:
                raise CatalogError("Feedback changed; reload", 409)
            record.update(
            ...
            db.execute(
                "UPDATE creative_commission_feedback SET record=? WHERE id=?",
                (json.dumps(record, sort_keys=True), record["id"]),
            )
            return record
...
class CommissionActionProvider(ActionProvider):
    ...
    def display_name(self):
        return "Run creative commission"
    ...
    async def execute(self, action_config, ctx: ActionContext, timeout=30):
        try:
            ...
            occurrence = str(
                ctx.payload.get("scheduled_for") or ctx.payload.get("occurrence") or ""
            )
            result = await self.store.execute(
                identifier(action_config.get("commission_id")),
                occurrence,
                schedule_revision=integer(action_config.get("schedule_revision")),
                cadence_hash=text(action_config.get("cadence_hash"), 64, True),
                trigger="schedule",
            )
            if (
                result.get("reason") not in ("disabled", "stale_schedule")
                and result.get("status") != "in_progress"
            ):
                self.store.rearm_recurrence(
                    action_config["commission_id"],
                    occurrence,
                    action_config["schedule_revision"],
                    action_config["cadence_hash"],
                )
            ...
            return ActionResult(
                ...
                error="" if success else result["attempts"][-1]["error"],
                outcome=(
                    "ran"
                    if result.get("status") in ("planned", "completed", "submitted")
                    else "skipped_noop" if success else "failed"
                ),
            )
...
def create_provider(config=None):
    if config not in (None, {}):
        raise CatalogError("Creative commission provider accepts no configuration")
