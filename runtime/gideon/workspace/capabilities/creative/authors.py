...
class AuthorStore(IngredientStore):
    ...
    def _author(self, db, id):
        row = db.execute("SELECT record FROM authors WHERE id=?", (identifier(id),)).fetchone()
        if not row:
    ...
    def _values(self, db, value, history=()):
        ...
        if perspective not in ("first", "third", "any") or tense not in ("past", "present", "any"):
            raise CatalogError("Invalid voice perspective or tense")
        normalized_voice = {"perspective": perspective, "tense": tense,
                            **{key: text(voice.get(key, ""), 4000) for key in ("tone", "diction", "rhythm", "avoid")}}
        refs = value.get("sample_refs", [])
        if not isinstance(refs, list) or len(refs) > 8:
            raise CatalogError("At most eight writing samples are allowed")
        previous = {(ref["artifact_id"], ref["artifact_version"]) for row in history for ref in row["sample_refs"]}
        normalized = []
        for ref in refs:
            keys(ref, {"artifact_id", "artifact_version"})
            pair = (identifier(ref.get("artifact_id")), integer(ref.get("artifact_version")))
            artifact = self.artifacts.get(pair[0], version=pair[1])
            if pair not in previous and (artifact is None or artifact.kind not in ("markdown", "text")):
                raise CatalogError("Writing sample must reference an existing text artifact version", 404)
            item = {"artifact_id": pair[0], "artifact_version": pair[1]}
            if item not in normalized:
                normalized.append(item)
        return {"title": text(value.get("title"), 200, True), "biography": text(value.get("biography", ""), 20000),
                "voice": normalized_voice, "sample_refs": normalized}

    def _save(self, db, record):
        encoded = json.dumps(record, sort_keys=True)
        db.execute("INSERT INTO author_revisions VALUES(?,?,?)", (record["id"], record["revision"], encoded))
        db.execute("INSERT OR REPLACE INTO authors VALUES(?,?)", (record["id"], encoded))
        return record
    ...
    def create(self, payload):
        ...
        with self.connection() as db:
            prior = db.execute("SELECT payload,record FROM author_requests WHERE id=?", (request,)).fetchone()
            if prior:
                if prior[0] != encoded:
            ...
            db.execute("INSERT INTO author_requests VALUES(?,?,?)", (request, encoded, json.dumps(record)))
            return record
    ...
    def update(self, id, patch):
        ...
        with self.connection() as db:
            ...
            history = [json.loads(r[0]) for r in db.execute("SELECT record FROM author_revisions WHERE id=?", (id,))]
    ...
    def export(self, id, revision=None):
        with self.connection() as db:
            ...
            if revision is None:
                return current
            integer(revision)
            row = db.execute("SELECT record FROM author_revisions WHERE id=? AND revision=?", (id, revision)).fetchone()
            if not row:
    ...
    def get(self, id):
        ...
        for ref in record["sample_refs"]:
            artifact = self.artifacts.get(ref["artifact_id"], version=ref["artifact_version"])
            statuses.append({**ref, "missing": artifact is None, "title": artifact.name if artifact else ref["artifact_id"]})
        return {**record, "sample_status": statuses}

    def sources(self, q=""):
        text(q, 200)
        return {"items": [{"id": item.slug, "title": item.name, "version": item.version}
                          for item in self.artifacts.list(q=q) if item.kind in ("markdown", "text")][:100]}
    ...
    def brief(self, id, revision=None):
        ...
        for ref in author["sample_refs"]:
            ...
            samples.append({**ref, "missing": artifact is None, "title": artifact.name if artifact else ref["artifact_id"],
                            "content": content[:4000], "truncated": len(content) > 4000, "original_characters": len(content)})
        return {"author_id": id, "revision": author["revision"], "title": author["title"], "voice": author["voice"],
                "biography": author["biography"], "samples": samples, "kind": "configured_voice_brief"}

    def revisions(self, id):
        with self.connection() as db:
            self._author(db, id)
            return [json.loads(row[0]) for row in db.execute("SELECT record FROM author_revisions WHERE id=? ORDER BY revision DESC", (id,))]
    ...
    def list(self, q="", offset=0, limit=25):
        ...
        integer(limit, 1, 100)
        with self.connection() as db:
            rows = [json.loads(r[0]) for r in db.execute("SELECT record FROM authors ORDER BY id")]
