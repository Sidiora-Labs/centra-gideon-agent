...
def family_for(check_id):
    return next(
        (
            family
            for prefix, family in CHECK_FAMILY.items()
...
def validate_context(family, value):
    ...
    if family == "canon":
        ...
        for index, page in enumerate(pages):
            if (
                not isinstance(page.get("number"), int)
                or isinstance(page.get("number"), bool)
                or page["number"] < 1
            ):
...
class EditorialContextStore:
    ...
    def bind(self, work_id, payload):
        keys(
            payload, {"request_id", "work_revision", "family", "schema_version", "data"}
        )
        ...
        digest = hashlib.sha256(
            json.dumps(
                {"work_id": identifier(work_id), **payload},
                sort_keys=True,
                ensure_ascii=False,
            ).encode()
        ).hexdigest()
        with self.works.connection() as db:
            ...
            prior = db.execute(
                "SELECT payload_hash,record FROM editorial_context_requests WHERE id=?",
                (request_id,),
            ).fetchone()
            if prior:
                if prior[0] != digest:
            ...
            revision = db.execute(
                "SELECT COALESCE(MAX(revision),0)+1 FROM editorial_context_bindings WHERE work_id=? AND family=?",
                (work_id, family),
            ).fetchone()[0]
        slug = (
            "creative-context-"
            + hashlib.sha256(f"{work_id}:{family}:{revision}".encode()).hexdigest()[:48]
        )
        encoded = json.dumps(
            {"schema_version": 1, "family": family, "data": data},
            sort_keys=True,
            ensure_ascii=False,
        )
        ...
        if artifact is None:
            artifact = self.works.artifacts.create(
                name=f"{family.title()} context revision {revision}",
                slug=slug,
                kind="json",
                content=encoded,
                description=digest,
        ...
        with self.works.connection() as db:
            ...
            if prior:
                ...
                return json.loads(prior[1])
            db.execute(
                "INSERT INTO editorial_context_bindings VALUES(?,?,?,?,?)",
                (work_id, family, revision, digest, json.dumps(record)),
            )
            db.execute(
                "INSERT INTO editorial_context_requests VALUES(?,?,?)",
                (request_id, digest, json.dumps(record)),
            )
        return record
    ...
    def list(self, work_id):
        with self.works.connection() as db:
            ...
            rows = [
                ...
                for row in db.execute(
                    "SELECT record FROM editorial_context_bindings WHERE work_id=? ORDER BY family,revision DESC",
                    (work_id,),
                )
            ]
        latest = {}
        ...
        return {
            "families": [
                {"family": family, **self._state(latest.get(family))}
    ...
    def _state(self, record):
        ...
        try:
            ...
            if (
                value.get("schema_version") != record["schema_version"]
                or value.get("family") != record["family"]
            ):
                raise ValueError()
    ...
    def current(self, work_id, family):
        ...
        with self.works.connection() as db:
            self.works._work(db, work_id)
            row = db.execute(
                "SELECT record FROM editorial_context_bindings WHERE work_id=? AND family=? ORDER BY revision DESC LIMIT 1",
                (work_id, family),
            ).fetchone()
