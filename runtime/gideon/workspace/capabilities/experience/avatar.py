...
def avatar_info(data):
    ...
    for animation in animations:
        ...
        if (
            name in names
            or not isinstance(animation.get("channels"), list)
            or not isinstance(animation.get("samplers"), list)
            or not animation["channels"]
            or not animation["samplers"]
        ):
            raise ValueError(
                "Avatar animation names must be unique and contain channels"
            )
        names.append(name)
        for channel in animation["channels"]:
            if not isinstance(channel, dict) or not isinstance(
                channel.get("target"), dict
            ):
            ...
            if (
                ...
                or not 0 <= index < len(animation["samplers"])
                or type(node) is not int
                or not 0 <= node < len(nodes)
                or path not in ("translation", "rotation", "scale", "weights")
            ):
            ...
            if any(
                type(sampler.get(key)) is not int
                or not 0 <= sampler[key] < len(accessors)
...
class AvatarStore:
    def __init__(self, store):
        ...
        with store.connection() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS avatars(id TEXT PRIMARY KEY,body TEXT)"
            )
            db.execute(
                "CREATE TABLE IF NOT EXISTS avatar_selection(id INTEGER PRIMARY KEY CHECK(id=1),body TEXT)"
            )
            db.execute(
                "INSERT OR IGNORE INTO avatar_selection VALUES(1,?)",
                (json.dumps({"revision": 1, "avatar_id": None, "entity_id": ""}),),
            )
    ...
    def list(self):
        with self.store.connection() as db:
            rows = [
                json.loads(row[0])
                for row in db.execute("SELECT body FROM avatars ORDER BY rowid DESC")
            ]
        for row in rows:
            try:
                ...
                row["availability"] = (
                    "ready"
                    if hashlib.sha256(data).hexdigest() == row["source_hash"]
                    else "missing"
                )
    ...
    def publish(self, body):
        if not isinstance(body, dict) or set(body) != {
            ...
            raise ValueError(
                "Avatar requires title, artifact_slug, artifact_version and clips"
            )
        ...
        if (
            ...
            or "idle" not in clips
            or set(clips) - set(STATES)
            or any(value not in info["animations"] for value in clips.values())
        ):
            raise ValueError(
                "Avatar clip mapping requires idle and only actual supported clips"
            )
        record = {
            "id": uuid4().hex,
            "title": title,
            ...
            "available_clips": info["animations"],
            "clips": clips,
        }
        with self.store.connection() as db:
            ...
            db.execute(
                "INSERT INTO avatars VALUES(?,?)", (record["id"], json.dumps(record))
            )
    ...
    def bundled(self):
        ...
        for record in self.list():
            if (
                ...
                and record["clips"] == {name: name for name in STATES}
                and record["availability"] == "ready"
            ):
                return record
        artifact = self.artifacts.create_binary(
            ...
            source="manual",
            tags=["avatar"],
            description="Original articulated robot with authored motion clips.",
        )
        return self.publish(
            {
                "title": "Gideon robot",
    ...
    def selection(self):
        with self.store.connection() as db:
            return json.loads(
                db.execute("SELECT body FROM avatar_selection WHERE id=1").fetchone()[0]
            )
    ...
    def select(self, body):
        ...
        if body["avatar_id"] is not None:
            ...
            if not any(
                row["id"] == key and row["availability"] == "ready"
        ...
        with self.store.connection() as db:
            ...
            db.execute(
                "UPDATE avatar_selection SET body=? WHERE id=1", (json.dumps(saved),)
            )
            return saved
    ...
    def raw(self, slug, version):
        ...
        if not any(
            row["artifact_slug"] == slug
            and row["artifact_version"] == version
            and row["source_hash"] == hashlib.sha256(data).hexdigest()
            for row in self.list()
        ):
