    ...
    async def guest_proxy(request, travel):
        ...
        if request.method not in ("GET", "HEAD"):
            return web.json_response(
                {"error": "Guest world bridge is read-only"}, status=405
            )
        ...
        headers = {
            ...
            for key in ("Accept", "Content-Type", "Range", "If-None-Match")
            if key in request.headers
        }
        headers[PROXY_SIGNATURE_HEADER] = sign_proxy_request(
            secret, request.method, url.raw_path_qs, body
        )
        async with aiohttp.ClientSession(
            ...
            if request.headers.get("Upgrade", "").lower() == "websocket":
                async with client.ws_connect(
                    url, headers=headers, max_msg_size=16 * 1024 * 1024
                ) as upstream:
                        ...
                        async for message in downstream:
                            if message.type == aiohttp.WSMsgType.TEXT:
                                ...
                                if value.get("type") == "join":
                                    if joined:
                                        raise ValueError("Guest may join only once")
                                    joined = True
                                    value.update(
                                        world=guest["world"],
                                        id="guest_" + guest["visit_id"],
                                        agent=False,
                                    )
                                    ...
                                    await downstream.send_json(
                                        {
                                            "type": "error",
                                            "error": "Guest world access is limited to presence and history",
                                        }
                                    )
                                    continue
            ...
            async with client.request(
                ...
                base = (
                    "/api/capabilities/experience/world-travel/guest/"
                    + request.match_info["ticket"]
                    + "/host"
                )
                if any(
                    kind in content_type for kind in ("html", "javascript", "text/css")
                ):
    ...
    async def handle(request):
        try:
            ...
            if request.match_info.get("ticket") is not None:
                ...
                action = (
                    "admit"
                    if request.path.endswith("/federation/admit")
                    else "depart" if request.path.endswith("/depart") else action
                )
            if request.match_info.get("tail") is not None:
                return await guest_proxy(request, travel)
            if request.method == "GET":
                result = (
                    travel.guest(request.match_info["ticket"])
                    if action == "guest"
                    else await travel.destinations()
                )
            else:
                ...
                if action == "depart":
                    ...
                    if (
                        set(body) != {"proof", "payload"}
                        or not isinstance(body["proof"], dict)
                        or not isinstance(body["payload"], dict)
                    ):
                        raise ValueError(
                            "Signed peer envelope requires proof and payload"
                        )
            ...
            return web.json_response(
                {"error": "Canonical peer travel failed"}, status=502
            )

    prefix = "/api/capabilities/experience/world-travel"
    app.router.add_get(prefix + "/destinations", handle)
    app.router.add_post(
        prefix + "/depart", handle, name="experience-world-travel-depart"
    )
    app.router.add_post(
        prefix + "/federation/admit", handle, name="experience-world-travel-admit"
    )
    app.router.add_get(
        prefix + "/guest/{ticket}", handle, name="experience-world-travel-guest"
    )
    app.router.add_post(
        prefix + "/guest/{ticket}/leave", handle, name="experience-world-travel-leave"
    )
    app.router.add_route(
        "*",
        prefix + "/guest/{ticket}/host/{tail:.*}",
        handle,
        name="experience-world-travel-host",
    )
