...
OPERATIONS = {
    ...
    "world_project": (
        {
            ...
            "body": {
                "type": "object",
                "description": "Explicit kinds, expected_seq and request_id",
            },
        },
        True,
    ...
    "world_objects": (
        {
            ...
            "body": {
                "type": "object",
                "description": "operation spawn/place/remove, id, optional position, expected_seq, request_id",
            },
        },
        True,
    ...
    "avatar_publish": (
        {
            ...
            "clips": {
                "type": "object",
                "description": "idle required; optional working, needs_input, waiting_approval, error, speaking; values must be real asset clip names",
            },
        },
        True,
        "Publish an existing animated canonical model as an avatar variant.",
    ),
    "avatar_selection_get": (
        {},
        False,
    ...
    "avatar_select": (
        {
            ...
            "entity_id": {"type": "string", "maxLength": 300},
        },
        True,
    ...
    "ambient_update": (
        {
            "revision": REVISION,
            "show_clock": {"type": "boolean"},
            "font_scale": {"type": "integer", "minimum": 1, "maximum": 3},
            "idle_seconds": {"type": "integer", "minimum": 10, "maximum": 300},
        },
        True,
    ...
    "session_start": (
        {"story_id": STRING, "story_revision": REVISION, "request_id": STRING},
        True,
        "Start an idempotent playthrough of the given story revision.",
    ),
    "session_choose": (
        {"id": STRING, "choice_id": STRING, "revision": REVISION, "request_id": STRING},
        True,
        "Choose a current scene option with optimistic revision and retry identity.",
    ),
    "narration_start": (
        {"id": STRING, "revision": REVISION, "request_id": STRING},
        True,
...
class ExperienceTools(ToolProvider):
    ...
    async def invoke(self, tool_name, arguments):
        ...
        if not isinstance(arguments, dict) or set(arguments) != set(
            ...
            return ToolResult(
                False, error="arguments must match the declared operation fields"
            )
        args = dict(arguments)
        ...
        try:
            if operation in ("world_get", "world_project", "world_objects"):
                ...
                if operation != "world_get":
                    ...
                    result = await self._worlds.mutate(
                        args["world"], operation.removeprefix("world_"), args["body"]
                    )
            ...
            if operation.startswith("world_engine_"):
                ...
                result = (
                    await engine.status()
                    if operation == "world_engine_get"
                    else await engine.control(
                        operation.removeprefix("world_engine_"), {}
                    )
                )
                return ToolResult(success=True, output=json.dumps(result))
            if operation == "native_calls_get":
                ...
                return ToolResult(
                    ...
                    output=json.dumps(
                        {
                            "readiness": self._native_calls.readiness(),
                            "requests": self._native_calls.list(),
                        }
                    ),
                )
            if operation == "avatar_list":
                result = {"avatars": AvatarStore(self.store).list()}
            elif operation == "avatar_bundled":
                result = {"avatar": AvatarStore(self.store).bundled()}
            elif operation == "avatar_publish":
                result = {"avatar": AvatarStore(self.store).publish(args)}
            elif operation == "avatar_selection_get":
                result = {"selection": AvatarStore(self.store).selection()}
            elif operation == "avatar_select":
                result = {"selection": AvatarStore(self.store).select(args)}
            elif operation == "ambient_get":
                result = await AmbientDisplay(self.store).snapshot()
            elif operation == "ambient_update":
                result = {"preferences": AmbientDisplay(self.store).save(args)}
            elif operation == "speech_state":
                result = {"owner": SpeechOwner(self.store).state()}
            elif operation == "story_list":
                result = {"stories": self.store.stories()}
            elif operation == "story_get":
                result = {"story": self.store.story(key)}
            elif operation == "story_create":
                result = {"story": self.store.save(args["story"])}
            elif operation == "story_update":
                result = {"story": self.store.save(args["story"], key)}
            elif operation == "story_delete":
                self.store.delete(key, args["revision"])
                result = {"deleted": True}
            elif operation == "session_list":
                result = {"sessions": self.store.sessions()}
            elif operation == "session_get":
                result = self.store.session(key)
            elif operation == "session_start":
                result = self.store.start(args)
            elif operation == "session_choose":
                result = self.store.choose(key, args)
            elif operation == "narration_start":
                result = {"narration": self.jobs.start(key, args)}
            elif operation == "narration_get":
                result = {"narration": self.jobs.get(key)}
            elif operation == "navigation_list":
                result = {"receipts": NavigationReceipts(self.store).list()}
            elif operation == "navigation_get":
                result = {"receipt": NavigationReceipts(self.store).get(key)}
            else:
                result = {"narration": await self.jobs.cancel(key)}
