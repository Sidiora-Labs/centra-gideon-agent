...
def voice_digest(params):
    ...
    values["reference"] = (
        hashlib.sha256(Path(reference).read_bytes()).hexdigest() if reference else ""
    )
...
def audio_mime(data):
    ...
    if data[:4] == b"OggS":
        return "audio/ogg"
    if data[:3] == b"ID3" or (
        len(data) > 2 and data[0] == 255 and data[1] & 224 == 224
    ):
        return "audio/mpeg"
...
class NarrationJobs:
    def __init__(self, store):
        ...
        with store.connection() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS narrations(id TEXT PRIMARY KEY, session_id TEXT, request_id TEXT, input TEXT, body TEXT, UNIQUE(session_id,request_id))"
            )
            for key, body in db.execute("SELECT id,body FROM narrations").fetchall():
                ...
                if job["status"] in ("queued", "running"):
                    job.update(
                        status="interrupted",
                        error="Narration was interrupted by a restart; request again.",
                    )
                    db.execute(
                        "UPDATE narrations SET body=? WHERE id=?",
                        (json.dumps(job), key),
                    )
    ...
    def get(self, key):
        with self.store.connection() as db:
            row = db.execute(
                "SELECT body FROM narrations WHERE id=?", (identifier(key),)
            ).fetchone()
            if row is None:
    ...
    def update(self, key, **changes):
        with self.store.connection() as db:
            row = db.execute(
                "SELECT body FROM narrations WHERE id=?", (key,)
            ).fetchone()
            job = json.loads(row[0])
            job.update(changes)
            db.execute(
                "UPDATE narrations SET body=? WHERE id=?", (json.dumps(job), key)
            )
            return job
    ...
    def start(self, session_id, body):
        ...
        expected, request_id = revision(body["revision"]), identifier(
            body["request_id"]
        )
        with self.store.connection() as db:
            session = self.store._session(db, session_id)
            old = db.execute(
                "SELECT input,body FROM narrations WHERE session_id=? AND request_id=?",
                (session_id, request_id),
            ).fetchone()
            if old:
                if old[0] != digest(body):
                    raise Conflict(
                        "narration request identifier was used with different input"
                    )
            ...
            job = {
                ...
                "audio_hash": "",
                "error": "",
                "audio_url": f"/api/capabilities/experience/narrations/{key}/audio",
            }
            db.execute(
                "INSERT INTO narrations VALUES(?,?,?,?,?)",
                (key, session_id, request_id, digest(body), json.dumps(job)),
            )
    ...
    def audio(self, key):
        ...
        result = self.artifacts.raw_bytes(
            job["artifact_slug"], version=job["artifact_version"]
        )
    ...
    async def produce(self, key, prose):
        try:
            ...
            if (
                params is None
                or not params.get("enabled")
                or not await params["provider"].can_synthesize(params.get("voice", ""))
            ):
                self.update(
                    key,
                    status="unavailable",
                    error="No enabled speech provider is ready. Configure Speech settings and request again.",
                )
                return
            ...
            for previous in candidates:
                if previous["status"] == "ready" and (
                    previous["source_hash"],
                    previous["voice_hash"],
                ) == (job["source_hash"], job["voice_hash"]):
                    try:
            ...
            with tempfile.TemporaryDirectory(prefix="gideon-narration-") as temporary:
                ...
                if (
                    ...
                    or not Path(result).resolve().is_relative_to(Path(temporary))
                ):
                ...
                artifact = self.artifacts.create_binary(
                    name=f"{job.get('source_kind', 'Story narration')} {job['node_id']}",
                    data=data,
                    mime=mime,
                    kind="audio",
                    source="manual",
                    tags=[job.get("source_kind", "story-narration")],
                    description=job.get("source_description")
                    or f"Story {job['story_id']} revision {job['story_revision']}; node {job['node_id']}; source {job['source_hash']}",
                )
                self.update(
                    key,
                    status="ready",
            ...
            self.update(
                key,
                status="failed",
                error="Speech generation failed; no audio was published. Check Speech settings and retry.",
            )
