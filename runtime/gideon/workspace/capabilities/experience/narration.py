    ...
    async def produce(self, key, prose):
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="gideon-narration-") as temporary:
                ...
                artifact = self.artifacts.create_binary(name=f"{job.get('source_kind', 'Story narration')} {job['node_id']}", data=data, mime=mime, kind="audio", source="manual", tags=[job.get("source_kind", "story-narration")], description=job.get("source_description") or f"Story {job['story_id']} revision {job['story_revision']}; node {job['node_id']}; source {job['source_hash']}")
