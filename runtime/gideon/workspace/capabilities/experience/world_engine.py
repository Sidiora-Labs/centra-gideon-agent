...
from gideon.sdk.security import sign_proxy_request, PROXY_SIGNATURE_HEADER
...
class WorldEngine:
    ...
    def guard(self):
        active = configuration_home(os.environ.get('GIDEON_HOME'), Path.home() / CONFIG_DIR_NAME, logger).resolve()
        if active != self.home:
    ...
    async def status(self):
        self.guard()
        result = {'state': 'unavailable', 'reason': '', 'app_id': APP_ID, 'version': None, 'engine_url': None}
        if not shutil.which('bun'):
            result['reason'] = 'Bun runtime is unavailable'
            return result
        if _read_installed(APP_ID) is None:
            result['reason'] = 'Operator must install the separately packaged world engine app and its pinned dependencies'
            return result
        ...
        try:
            ...
            return result
        try:
            headers = {PROXY_SIGNATURE_HEADER: sign_proxy_request(secret, 'GET', '/version', b'')}
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=3)) as client:
                async with client.get(target + '/version', headers=headers, allow_redirects=False) as response:
            ...
            result.update(state='running', reason='Actual managed world engine answered its version endpoint', version=version, engine_url=PREFIX + '/host/')
    ...
    async def control(self, operation, body):
        ...
        async with self.lock:
            if operation == 'start':
                ...
                if _read_installed(APP_ID) is not None and not app_manager.disable(APP_ID):
