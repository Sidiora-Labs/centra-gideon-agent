...
class Worlds:
    ...
    async def call(self, world, body=None):
        ...
        headers = {
            PROXY_SIGNATURE_HEADER: sign_proxy_request(secret, method, path, raw),
        ...
        async with aiohttp.ClientSession(
            ...
            async with client.request(
                method, target + path, data=raw, headers=headers, allow_redirects=False
            ) as response:
                ...
                if response.status != 200:
                    raise Conflict(
                        result.get("error", "World engine refused operation")
                    )
                return result
    ...
    async def open(self, world, body):
        ...
        async with self.lock:
            ...
            if len(self.connections) >= 16:
                raise Conflict(
                    "World presence limit reached; restart the display service to release unused worlds"
                )
    ...
    async def sources(self):
                ...
                sources.append(
                    {
                        "kind": kind,
                        "id": row["id"],
                        "title": str(row.get("title") or row.get("kind") or row["id"])[
                            :200
                        ],
                        "status": str(
                            row.get(
                        ...
                        + (
                            "wellbeing"
                            if kind == "health"
                            else "identity" if kind == "goals" else "workspace"
                        ),
                    }
                )
            ...
            sources.append(
                {
                    "kind": "apps",
                    "id": row["name"],
                    "title": row.get("display_name") or row["name"],
        ...
        return {
            ...
            "unavailable": unavailable + extra["unavailable"],
            "limit_per_kind": 10,
        }
    ...
    async def mutate(self, world, action, body):
        ...
        fields = (
            {"expected_seq", "request_id", "kinds"}
            if action == "project"
            else {"expected_seq", "request_id", "operation", "id"}
        )
        allowed = fields if action == "project" else fields | {"position"}
        if (
            not fields <= body.keys()
            or body.keys() - allowed
            or type(body["expected_seq"]) is not int
            or body["expected_seq"] < -1
        ):
        ...
        if action == "project":
            ...
            if len(rows) > 20:
                raise ValueError(
                    "Select fewer categories; at most 20 sources per projection"
                )
            for index, row in enumerate(rows):
                key = (
                    "source_"
                    + hashlib.sha256(
                        (row["kind"] + ":" + row["id"]).encode()
                    ).hexdigest()[:24]
                )
                ...
                operations.append(
                    {
                        "verb": "comp",
                        "args": {"id": key, "type": "gideon_source", "data": row},
                    }
                )
            ...
            for key, entity in snapshot["state"]["entities"].items():
                ...
                if (
                    source.get("kind") in kinds
                    and (source["kind"], source.get("id")) not in available
                ):
                    operations.append(
                        {
                            "verb": "comp",
                            "args": {
                                ...
                                "data": {
                                    **source,
                                    "status": "not_in_current_source_preview",
                                },
                            },
                        }
                    )
            if len(operations) > 40:
                raise ValueError(
                    "Projection change set exceeds 40 operations; select fewer categories"
                )
        else:
            ...
            if operation != "remove":
                ...
                if (
                    ...
                    raise ValueError(
                        "Position requires three finite bounded coordinates"
                    )
                args["pos"] = position
            if operation == "spawn":
