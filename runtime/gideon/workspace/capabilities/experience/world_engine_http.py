...
def rewrite_text(text, content_type, base=PREFIX + "/host"):
    text = re.sub(
        r"""(["'`])/(?!/)([^"'`\s<>]*)""",
        lambda match: match[1] + base + "/" + match[2],
        text,
    )
        ...
        try:
            result = (
                ...
                if request.method == "GET"
                    ...
                    request.path.rsplit("/", 1)[-1], await read_json_body(request)
                )
            )
    ...
    async def proxy(request):
        try:
            ...
            async with aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(total=120), trace_configs=[trace]
            ) as client:
                ...
                async with client.request(
                    ...
                    if any(
                        ...
                        for kind in ("html", "javascript", "text/css")
                    ):
                        raw = rewrite_text(raw.decode("utf-8"), content_type).encode(
                            "utf-8"
                        )
                    returned = {
                    ...
                    return web.Response(
                        body=raw, status=response.status, headers=returned
                    )
        except Conflict as exc:
            return web.json_response({"error": str(exc)}, status=409)
            ...
            return web.json_response(
                {"error": "Managed world engine connection failed: " + str(exc)},
                status=502,
            )
