...
def latest_digest(home):
    ...
    if not state['installed'] or not state['enabled']:
        return {'state': 'unavailable', 'reason': 'Enable and install the existing proactive digest first.'}
    if notification_posture('info') != 'allowed':
        return {'state': 'deferred', 'reason': 'Notification delivery settings currently defer speech.'}
    run, output, events = _latest_digest()
    view = build_digest_view(enabled=True, installed=True, run=run, output=output, events=events)
    if view['state'] != 'ready' or not view.get('body') or view.get('status') != 'complete':
        return {'state': 'waiting', 'reason': 'No completed proactive digest is available.'}
    prose = '\n\n'.join(value for value in [view.get('title', ''), view['body']] if value)
    if len(prose) > 12000:
        return {'state': 'unavailable', 'reason': 'Digest exceeds the speech size limit.'}
    return {'state': 'ready', 'run_id': view['run_id'], 'source_hash': digest({'run_id': view['run_id'], 'text': prose}), 'text': prose}
...
def start_digest(jobs, owner, body, retry=False):
    ...
    with jobs.store.connection() as db:
        previous = db.execute('SELECT body FROM narrations WHERE session_id=? ORDER BY rowid DESC LIMIT 1', (key,)).fetchone()
        if previous:
            prior = json.loads(previous[0])
            if not retry or prior['status'] not in ('unavailable','failed','cancelled','interrupted'):
