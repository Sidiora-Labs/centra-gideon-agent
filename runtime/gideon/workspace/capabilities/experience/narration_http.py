...
async def handle(request):
    ...
    try:
        ...
        if request.match_info.get("session_id"):
            result = jobs.start(request.match_info["session_id"], await read_json_body(request))
            ...
            return web.Response(body=data, content_type=mime, headers={"Cache-Control": "private, no-store"})
        else:
            result = jobs.get(key)
        return web.json_response({"narration": result}, status=202 if request.match_info.get("session_id") else 200)
