...
def _text(value, label, limit=80):
    if not isinstance(value, str) or not value.strip() or len(value.strip()) > limit or any(ord(char) < 32 for char in value):
        raise ValueError(f"{label} must contain 1 to {limit} visible characters")
    return value.strip()


def _position(value):
    if not isinstance(value, list) or len(value) != 3 or any(type(item) not in (int, float) or not math.isfinite(item) or abs(item) > 10000 for item in value):
...
class WorldFoundations:
    def __init__(self, store, worlds=None):
        ...
        self.instance_id = hashlib.sha256(str(store.path.parent.parent.resolve()).encode()).hexdigest()[:24]
    ...
    def _get(self, table, key):
        ...
        if not row:
            raise NotFound("foundation not found" if table == "world_foundations" else "controller not found")
        return json.loads(row[0])

    def _put(self, table, record):
        with self.store.connection() as db:
            db.execute(f"INSERT OR REPLACE INTO {table} VALUES(?,?)", (record["id"], _canonical(record)))
        return record

    def list(self):
        with self.store.connection() as db:
            foundations = [json.loads(row[0]) for row in db.execute("SELECT body FROM world_foundations ORDER BY rowid DESC LIMIT 200")]
            controllers = [json.loads(row[0]) for row in db.execute("SELECT body FROM world_controllers ORDER BY rowid DESC LIMIT 200")]
        return {"schema_version": SCHEMA_VERSION, "instance_id": self.instance_id, "foundations": foundations, "controllers": controllers}
    ...
    def record(self, body):
        ...
        if not isinstance(controller, dict) or set(controller) != {"kind", "position"} or controller["kind"] != CONTROLLER_KIND:
        ...
        record = {"id": uuid4().hex, "revision": 1, "state": "draft", "title": _text(body["title"], "Foundation title"),
                  "controller": {"kind": CONTROLLER_KIND, "position": _position(controller["position"])},
                  "style": style, "provenance": {"kind": "authored", "instance_id": self.instance_id}, "candidate": None, "fingerprint": None}
    ...
    def package(self, key, revision):
        ...
        candidate = {"schema_version": SCHEMA_VERSION, "origin_instance": self.instance_id, "foundation_id": record["id"],
                     "title": record["title"], "controller": record["controller"]}
        record.update(state="packaged", revision=record["revision"] + 1, candidate=candidate, fingerprint=_fingerprint(candidate))
        return self._put("world_foundations", record)

    def promote(self, key, revision):
        record = self._expected(key, revision)
        if record["state"] != "packaged" or _fingerprint(record["candidate"]) != record["fingerprint"]:
    ...
    def envelope(self, key):
        ...
        return {"schema_version": SCHEMA_VERSION, "candidate": record["candidate"], "fingerprint": record["fingerprint"]}

    def inherit(self, envelope):
        if not isinstance(envelope, dict) or set(envelope) != {"schema_version", "candidate", "fingerprint"} or envelope["schema_version"] != SCHEMA_VERSION:
            raise ValueError("Foundation envelope is incompatible")
        candidate, fingerprint = envelope["candidate"], envelope["fingerprint"]
        if not isinstance(candidate, dict) or set(candidate) != {"schema_version", "origin_instance", "foundation_id", "title", "controller"} or _fingerprint(candidate) != fingerprint:
        ...
        record = {"id": uuid4().hex, "revision": 1, "state": "inherited", "title": candidate["title"], "controller": controller,
                  "style": None, "candidate": candidate, "fingerprint": fingerprint,
                  "provenance": {"kind": "inherited", "origin_instance": candidate["origin_instance"], "foundation_id": candidate["foundation_id"]}}
        return self._put("world_foundations", record)
    ...
    def withdraw(self, key, revision):
        ...
        return {"schema_version": SCHEMA_VERSION, "origin_instance": self.instance_id, "foundation_id": record["id"], "fingerprint": record["fingerprint"]}

    def apply_withdrawal(self, withdrawal):
        if not isinstance(withdrawal, dict) or set(withdrawal) != {"schema_version", "origin_instance", "foundation_id", "fingerprint"}:
            raise ValueError("Withdrawal is invalid")
        for record in self.list()["foundations"]:
            provenance = record["provenance"]
            if provenance.get("origin_instance") == withdrawal["origin_instance"] and provenance.get("foundation_id") == withdrawal["foundation_id"]:
                if record["fingerprint"] != withdrawal["fingerprint"]:
                    raise Conflict("Withdrawal fingerprint does not match inherited foundation")
                record.update(state="withdrawn", revision=record["revision"] + 1)
                return self._put("world_foundations", record)
    ...
    async def install_controller(self, body):
        ...
        record = {"id": uuid4().hex, "revision": 1, "foundation_id": foundation["id"], "world": world,
                  "kind": CONTROLLER_KIND, "desired_state": "stopped", "state": "installed", "entity_id": None, "last_receipt": None}
    ...
    async def control(self, key, operation, revision):
        ...
        if operation in ("arm", "restart"):
            ...
            record.update(state="armed", entity_id="controller_" + record["id"][:24], last_receipt=receipt)
        elif operation == "stop":
            receipt = await self._remove(record)
            record.update(desired_state="stopped", state="stopped", entity_id=None, last_receipt=receipt)
        elif operation == "retire":
            receipt = await self._remove(record)
            record.update(desired_state="retired", state="retired", entity_id=None, last_receipt=receipt)
        else:
    ...
    async def reconcile(self):
        ...
        for record in self.list()["controllers"]:
            if record["desired_state"] == "armed":
                record.update(state="armed", entity_id="controller_" + record["id"][:24], last_receipt=await self._apply(record), revision=record["revision"] + 1)
    ...
    async def _apply(self, record):
        ...
        if entity not in snapshot["state"]["entities"]:
            operations.append({"verb": "spawn", "args": {"id": entity, "lib": ASSET, "pos": foundation["controller"]["position"], "yaw": 0}})
        operations.append({"verb": "comp", "args": {"id": entity, "type": "gideon_controller", "data": {"controller_id": record["id"], "kind": CONTROLLER_KIND, "foundation_id": foundation["id"]}}})
        return await self.worlds.call(record["world"], {"expected_seq": snapshot["seq"], "request_id": "controller_" + uuid4().hex, "operations": operations,
                                                        "intent": {"operation": "controller_tick", "controller_id": record["id"]}})

    async def _remove(self, record):
        snapshot = await self.worlds.open(record["world"], {})
        entity = "controller_" + record["id"][:24]
        operations = [] if entity not in snapshot["state"]["entities"] else [{"verb": "remove", "args": {"id": entity}}]
        if not operations:
            return {"complete": True, "seq": snapshot["seq"], "operations": []}
        return await self.worlds.call(record["world"], {"expected_seq": snapshot["seq"], "request_id": "controller_" + uuid4().hex,
                                                        "operations": operations, "intent": {"operation": "controller_stop", "controller_id": record["id"]}})
