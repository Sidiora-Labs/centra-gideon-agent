...
def record(kind, key, title, status, url):
    return {'kind':kind,'id':str(key),'title':(redact_credentials(str(title))[0].strip() or str(key))[:200], 'status':str(status),'url':url}
...
def extended_sources(home):
    ...
    memory = getattr(getattr(state, 'context_builder', None), 'memory', None) or getattr(state, '_standalone_memory', None)
    archive = getattr(memory, 'vector_store', None)
    archive_path = getattr(archive, '_db_path', None)
    if archive is None or archive_path is None or not Path(archive_path).resolve().is_relative_to(home):
        ...
        try:
            for row in MemoryService.over_vector_store(archive).episodic_list(limit=10):
                ...
                sources.append(record('memory', row['id'], title, row.get('created_at', ''), '#/capabilities/knowledge?memory=' + quote(str(row['id']), safe='')))
        except Exception:
            unavailable.append('memory')
    ...
    if not path.is_file():
        ...
        try:
            with sqlite3.connect(path.as_uri() + '?mode=ro', uri=True) as db:
                for (payload,) in db.execute('SELECT payload FROM processes ORDER BY rowid DESC LIMIT 10'):
                    row = json.loads(payload)
                    sources.append(record('operations', row['id'], row['project_id'], 'recorded:' + row['status'], '#/capabilities/workspace?process=' + quote(row['id'], safe='')))
