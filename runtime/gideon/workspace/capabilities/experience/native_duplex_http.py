        ...
        try:
            ...
            if request.method == "GET":
                return web.json_response(
                    {"readiness": service.readiness(), "sessions": service.list()}
                    if not identity
                    else service.get(identity)
                )
            body = await read_json_body(request)
            if not identity:
                result = service.start(body)
            elif action == "capture":
                result = await service.capture(identity, body.get("revision"))
            elif action == "speak":
                result = await service.speak(
                    identity, body.get("revision"), body.get("text")
                )
            elif action == "stop":
                result = service.stop(identity, body.get("revision"))
            else:
                raise NotFound("Native duplex action not found")
