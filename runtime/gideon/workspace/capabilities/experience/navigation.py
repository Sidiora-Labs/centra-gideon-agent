...
def route(value):
    if (
        ...
        or not re.fullmatch(r"[a-z0-9][a-z0-9_/-]{0,199}", value)
        or "//" in value
        or value.endswith("/")
    ):
...
class NavigationReceipts:
    def __init__(self, store):
        ...
        with store.connection() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS navigation_receipts(id TEXT PRIMARY KEY, request_id TEXT UNIQUE, input TEXT, body TEXT)"
            )

    def get(self, key):
        with self.store.connection() as db:
            row = db.execute(
                "SELECT body FROM navigation_receipts WHERE id=?", (identifier(key),)
            ).fetchone()
            if row is None:
    ...
    def list(self):
        with self.store.connection() as db:
            return [
                json.loads(r[0])
                for r in db.execute(
                    "SELECT body FROM navigation_receipts ORDER BY rowid DESC LIMIT 100"
                )
            ]
    ...
    def request(self, body):
        if not isinstance(body, dict) or set(body) != {
            ...
            raise ValueError(
                "navigation requires request_id, command, origin, target and input_origin"
            )
        request_id = identifier(body["request_id"])
        command, origin, target = (
            text(body["command"], 400),
            route(body["origin"]),
            route(body["target"]),
        )
        ...
        with self.store.connection() as db:
            previous = db.execute(
                "SELECT input,body FROM navigation_receipts WHERE request_id=?",
                (request_id,),
            ).fetchone()
            if previous:
                if previous[0] != digest(body):
                    raise Conflict(
                        "navigation request identifier was used with different input"
                    )
            ...
            db.execute(
                "INSERT INTO navigation_receipts VALUES(?,?,?,?)",
                (receipt["id"], request_id, digest(body), json.dumps(receipt)),
            )
            return receipt
    ...
    def acknowledge(self, key, body):
        ...
        with self.store.connection() as db:
            ...
            if expected != 1 or observed != receipt["target"]:
                raise Conflict(
                    "the browser has not acknowledged the requested destination"
                )
            ...
            db.execute(
                "UPDATE navigation_receipts SET body=? WHERE id=?",
