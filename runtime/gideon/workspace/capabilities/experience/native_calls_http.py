...
def handoff(request, record, body):
    ...
    for message in messages:
        if (
            ...
            or not 1 <= len(message["text"].strip()) <= 10000
        ):
    ...
    if (
        ...
        or session._ephemeral
        or ("dashboard:" + session.key) in state._ephemeral_keys
        or session.running
        or not state.conversation_log
    ):
    ...
    marker = {
        "native_call_id": record["id"],
        "request_id": body["request_id"],
        "content_hash": digest,
        "source": "user_supplied_transcript",
    }
    key = persisted_history_key(state.conversation_log, session.key)
    prior = [
        m
        for m in state.conversation_log.read_messages(key) + session.messages
        if m.get("meta", {}).get("native_call_id") == record["id"]
        and m.get("meta", {}).get("request_id") == body["request_id"]
    ]
    if prior and any(m.get("meta", {}).get("content_hash") != digest for m in prior):
        raise Conflict("Handoff request content changed")
    if not prior:
        text = (
            "User-supplied native call transcript. Native observation: "
            + record["state"]
            + ". Audio capture and call completion are not verified.\n\n"
        )
    ...
    if not any(m.get("meta") == marker for m in persisted):
        raise Conflict(
            "Conversation transcript was not durably saved; retry with the same request"
        )
    return {
        ...
        "source": "user_supplied_transcript",
        "state": "saved",
        "agent_continuation": "not_started",
    }
        ...
        try:
            if request.method == "GET":
                return web.json_response(
                    {"readiness": calls.readiness(), "requests": calls.list()}
                )
