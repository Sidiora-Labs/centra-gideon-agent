...
def _identifier(value, limit=96):
    if not isinstance(value, str) or not re.fullmatch(
        r"[A-Za-z0-9_-]{1," + str(limit) + r"}", value
    ):
...
class WorldTravel:
    ...
    def _peer(self, peer_id, direction):
        ...
        peer = next(
            (row for row in projection.get("peers", []) if row.get("id") == peer_id),
            None,
        )
        if (
            not peer
            or peer.get("enabled") is not True
            or not self.peers.allows(peer_id, SCOPE, direction=direction)
        ):
    ...
    async def destinations(self):
        ...
        for peer in projection.get("peers", []):
            if peer.get("enabled") is True and self.peers.allows(
                peer.get("id"), SCOPE, direction="send"
            ):
                rows.append(
                    {
                        "id": _identifier(peer.get("id")),
                        "label": _name(peer.get("label")),
                        "category": SCOPE,
                    }
                )
    ...
    def _replay(self, db, scope, request_id, body):
        ...
        row = db.execute(
            "SELECT input,result FROM world_guest_requests WHERE scope=? AND request_id=?",
            (scope, request_id),
        ).fetchone()
    ...
    def _remember(self, db, scope, request_id, encoded, result):
        db.execute(
            "INSERT INTO world_guest_requests VALUES(?,?,?,?)",
            (scope, request_id, encoded, json.dumps(result)),
        )
        return result
    ...
    async def admit(self, verified_peer_id, body):
        ...
        world, guest_name, request_id = (
            _identifier(body["world"]),
            _name(body["guest_name"]),
        ...
        with self.store.connection() as db:
            ...
            if replay is not None:
                return replay
            active = db.execute(
                "SELECT count(*) FROM world_guest_visits WHERE state IN ('invited','active') AND expires_at>?",
                (self.now(),),
            ).fetchone()[0]
            if active >= 32:
            ...
            db.execute(
                "INSERT INTO world_guest_visits VALUES(?,?,?,?,?,?,?,?,1)",
                (
                    visit_id,
    ...
    async def depart(self, body):
        if not isinstance(body, dict) or set(body) != {
            ...
            raise ValueError(
                "Departure requires peer_id, world, guest_name and request_id"
            )
        peer = self._peer(body["peer_id"], "send")
        request = {
            "world": _identifier(body["world"]),
            "guest_name": _name(body["guest_name"]),
    ...
    def guest(self, ticket):
        ...
        with self.store.connection() as db:
            row = db.execute(
                "SELECT id,peer_id,world,guest_name,state,expires_at,revision FROM world_guest_visits WHERE ticket_hash=?",
                (digest,),
            ).fetchone()
            if not row:
                raise NotFound("Guest invitation not found")
            if row[5] <= now:
                db.execute(
                    "UPDATE world_guest_visits SET state='expired',revision=revision+1 WHERE id=? AND state IN ('invited','active')",
                    (row[0],),
                )
            ...
            if row[4] == "invited":
                db.execute(
                    "UPDATE world_guest_visits SET state='active',revision=revision+1 WHERE id=?",
                    (row[0],),
                )
    ...
    def leave(self, ticket, body):
        if (
            ...
            or set(body) != {"revision", "request_id"}
        ...
        with self.store.connection() as db:
            row = db.execute(
                "SELECT id,state,expires_at,revision FROM world_guest_visits WHERE ticket_hash=?",
                (hashlib.sha256(ticket.encode()).hexdigest(),),
            ).fetchone()
            if not row or row[2] <= self.now():
                raise NotFound("Guest invitation expired")
            encoded, replay = self._replay(
                db, "leave:" + row[0], body["request_id"], body
            )
            if replay is not None:
                return replay
            if row[1] == "left":
            ...
            db.execute(
                "UPDATE world_guest_visits SET state='left',revision=revision+1 WHERE id=?",
                (row[0],),
            )
            return self._remember(
                db,
