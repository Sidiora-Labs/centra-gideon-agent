...
class SpeechOwner:
    def __init__(self, store):
        ...
        with store.connection() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS speech_owner(id INTEGER PRIMARY KEY CHECK(id=1), body TEXT)"
            )
            db.execute(
                "INSERT OR IGNORE INTO speech_owner VALUES(1,?)",
                (
                    json.dumps(
                        {"enabled": False, "owner": "", "token": "", "expires_at": 0}
                    ),
                ),
            )
    ...
    def state(self):
        with self.store.connection() as db:
            state = json.loads(
                db.execute("SELECT body FROM speech_owner WHERE id=1").fetchone()[0]
            )
    ...
    def change(self, action, body):
        ...
        with self.store.connection() as db:
            ...
            if action == "enable":
                ...
                state.update(
                    owner=body["owner"], token=uuid4().hex, expires_at=now + 15
                )
            else:
                if (
                    state["owner"] != body["owner"]
                    or state["token"] != body["token"]
                    or state["expires_at"] <= now
                ):
                    raise Conflict("audible ownership expired or changed")
                (
                    state.update(expires_at=now + 15)
                    if action == "renew"
                    else state.update(owner="", token="", expires_at=0)
                )
            db.execute(
                "UPDATE speech_owner SET body=? WHERE id=1", (json.dumps(state),)
            )
            return (
                state
                if action in ("claim", "renew")
                else {key: value for key, value in state.items() if key != "token"}
            )
