...
def register_game_assets(app, store, artifacts=None):
    ...
    async def handle(request):
        try:
            key, action = request.match_info.get("id"), request.path.rstrip("/").rsplit("/", 1)[-1]
            if request.method == "GET": result = service.get(key) if key else service.list()
            else:
                body = await read_json_body(request)
                if action == "projects": result = {"project": service.create(body)}
                elif action == "bindings": result = {"project": service.bind(key, body)}
                ...
                else: raise ValueError("Unknown game asset operation")
            return web.json_response(result, status=201 if request.method == "POST" and action == "projects" else 200)
