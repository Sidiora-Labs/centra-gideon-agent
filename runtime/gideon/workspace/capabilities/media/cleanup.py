...
def solid_background(image, color, tolerance):
    ...
    eligible = ImageChops.lighter(ImageChops.lighter(red, green), blue).point(
        lambda value: 255 if value <= tolerance else 0
    )
    remaining = eligible.copy()
    width, height = image.size
    for point in [(x, y) for x in range(width) for y in (0, height - 1)] + [
    ...
    image.putalpha(
        ImageChops.subtract(
            image.getchannel("A"), ImageChops.subtract(eligible, remaining)
        )
    )
    return image
...
class CleanupService:
    ...
    def prepare(self, body):
        fields(
            body,
            ("source_artifact_id", "source_version", "operations"),
            ("source_artifact_id", "source_version", "operations"),
        )
        if (
            not isinstance(body["source_artifact_id"], str)
            or not 1 <= len(body["source_artifact_id"]) <= 200
        ):
        ...
        for operation in operations:
            ...
            if name == "crop":
                ...
                if (
                    ...
                    or operation["y"] + operation["height"] > height
                ):
                ...
                width, height = integer(operation["width"], 1, 4096), integer(
                    operation["height"], 1, 4096
                )
            elif name == "rotate":
                fields(operation, ("op", "degrees"), ("op", "degrees"))
                if operation["degrees"] not in (90, 180, 270):
                    raise SketchError(
                        "Rotation must be 90, 180 or 270 degrees counterclockwise"
                    )
                ...
                if not isinstance(operation["color"], str) or not re.fullmatch(
                    "#[0-9a-fA-F]{6}", operation["color"]
                ):
                    raise SketchError("Background color must be #RRGGBB")
                integer(operation["tolerance"], 0, 100)
                if width * height > 4 * 1024 * 1024:
                    raise SketchError(
                        "Solid background removal is limited to 4 megapixels"
                    )
            else:
    ...
    def execute(self, request, job_id):
        validated = self.prepare(
            {
                key: request[key]
                for key in ("source_artifact_id", "source_version", "operations")
            }
        )
        if request != validated:
            raise SketchError(
                "Cleanup request no longer matches its pinned source", 409
            )
        ...
        for operation in request["operations"]:
            ...
            if name == "crop":
                ...
                image = image.crop(
                    (x, y, x + operation["width"], y + operation["height"])
                )
            elif name == "resize":
                image = image.resize(
                ...
                )(image).enhance(operation["factor"])
                ...
                image = solid_background(
                    image, operation["color"], operation["tolerance"]
                )
        output = io.BytesIO()
        image.save(output, "PNG")
        return self.images.materialize(
            ImageResult(b64=base64.b64encode(output.getvalue()).decode()),
            request,
            job_id,
        )
