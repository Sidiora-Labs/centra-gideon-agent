...
async def probe(capability, reference, provider, managed=False):
    row = dict(
        ...
        model=None,
        reason="Select a model in Settings",
    )
    if not reference:
        return row
    if provider is None:
        return dict(
            ...
            reason="The selected provider is not registered",
        )
    if provider.name == "stub":
        return dict(
            row,
            status="preview_only",
            reason="Offline preview is not a generation readiness check",
        )
    try:
        ...
        if not available:
            return dict(
                row,
                status="unavailable",
                reason="Provider availability check failed; check its configuration",
            )
        ...
        return dict(
            row, status="provider_error", reason="Provider readiness check failed"
        )
...
def describe_model(row, capability, reference, models):
    ...
    if model is None:
        return dict(
            ...
            reason="Selected model is absent from the provider catalog",
        )
    metadata = dict(name=model.name, downloaded=bool(model.downloaded))
    if capability == "image_gen":
        metadata.update(
            sizes=list(model.sizes), supports_edit=bool(model.supports_edit)
        )
    else:
        metadata.update(
            aspect_ratios=list(model.aspect_ratios), max_duration_s=model.max_duration_s
        )
    row["model"] = metadata
    if not model.downloaded:
        return dict(
            ...
            reason="Install this model through its existing model provider",
        )
    return dict(
        row,
        status="configured",
        reason="Provider and catalog checks passed; inference has not been verified",
    )
...
class MediaReadiness:
    def __init__(self, path, bindings=None):
        ...
        try:
            with db:
                db.execute(
                    "CREATE TABLE IF NOT EXISTS observation (id INTEGER PRIMARY KEY CHECK(id=1), body TEXT)"
                )
        finally:
            db.close()
    ...
    def get(self):
        ...
        return (
            json.loads(row[0])
            if row
            else {"observed_at": None, "items": [], "inference_verified": False}
        )
    ...
    def selected_bindings(self):
        ...
        bindings = []
        for capability, resolve in [
            ("image_gen", active_image_gen),
            ("video_gen", active_video_gen),
        ]:
            ...
            bindings.append(
                (
                    ...
                    bool(reference and get_provider(reference.split(":", 1)[0])),
                )
            )
        return bindings
    ...
    def record(self, rows):
        ...
        try:
            with db:
                db.execute(
                    "INSERT OR REPLACE INTO observation VALUES (1,?)",
