...
def public(job):
    return {
        ...
        if key not in ("owner_pid", "owner_identity", "child_pid", "child_identity")
    }
...
class MediaJobs:
    def __init__(self, path, sketches, images=None, training_allowed=True, videos=None):
        ...
        self.timelines = TimelineStore(
            self.path.parent / "timelines.sqlite3", self.videos
        )
        self.episodes = EpisodeStore(
            self.path.parent / "episodes.sqlite3", self.videos, self.timelines
        )
        self.datasets = DatasetStore(self.path.parent / "datasets.sqlite3", self.images)
        self.trainer = DiffusersTrainer(
            sketches.artifacts.root.parent, self.datasets, allowed=training_allowed
        )
        self.path.parent.mkdir(parents=True, exist_ok=True)
        with self.db() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS jobs (id TEXT PRIMARY KEY, request TEXT UNIQUE, fingerprint TEXT, status TEXT, body TEXT)"
            )
    ...
    def _write(self, db, job, status, detail):
        job.update(
            status=status, updated_at=now(), state_revision=job["state_revision"] + 1
        )
        job["events"].append(
            dict(
                status=status,
    ...
    def submit(self, body):
        ...
        if body.get("operation") == "source_download":
            ...
            image_request = (
                self.sprites.prepare_generate(body["input"])
                if body["operation"] == "sprite_generate"
                else self.sprites.prepare_compile(body["input"])
            )
        ...
        if (
            not isinstance(body["request_id"], str)
            or not 1 <= len(body["request_id"]) <= 100
        ):
        ...
        with self.db() as db:
            ...
            row = db.execute(
                "SELECT fingerprint,body FROM jobs WHERE request=?",
                (body["request_id"],),
            ).fetchone()
            if row:
            ...
            job = dict(
                ...
                events=[
                    dict(
                        status="queued",
                        at=timestamp,
                        detail="Waiting for the media worker",
                    )
                ],
            )
            db.execute(
                "INSERT INTO jobs VALUES (?,?,?,?,?)",
                (job["id"], body["request_id"], fingerprint, "queued", json.dumps(job)),
            )
        return public(job)
    ...
    def cancel(self, job_id, body):
        with self.db() as db:
            ...
            return self._write(
                db,
                job,
                "cancelled" if job["status"] == "queued" else "cancel_requested",
                "Cancellation requested; completed output is retained",
            )
    ...
    def finish(self, job_id, result=None, error=None):
        with self.db() as db:
            ...
            return self._write(
                ...
                job,
                status,
                error or "Renderer finished; any output artifact remains available",
            )
    ...
    def recover(self):
        ...
        with self.db() as db:
            db.execute("BEGIN IMMEDIATE")
            rows = db.execute(
                "SELECT body FROM jobs WHERE status IN ('running','cancel_requested')"
            ).fetchall()
            for row in rows:
                job = json.loads(row[0])
                if (
                    identity(job["owner_pid"]) != job["owner_identity"]
                    or not job["owner_identity"]
                ):
                    if job.get("child_pid") and identity(job["child_pid"]) == job.get(
                        "child_identity"
                    ):
                        try:
                            ...
                            pass
                    job.update(
                        error="Media worker exited before recording completion",
                        owner_pid=None,
                        owner_identity=None,
                    )
...
class MediaWorker(BackgroundWorker):
    ...
    def run_once(self, ctx):
        ...
        if (
            ctx.should_stop()
            or self.jobs.get(job["id"])["status"] == "cancel_requested"
        ):
            self.jobs.cancel(
                job["id"],
                {"state_revision": self.jobs.get(job["id"])["state_revision"]},
            )
        ...
        try:
            if job["operation"] == "source_download":
                result = asyncio.run(
                    self.jobs.downloads.execute(
                        job["input"],
                        job["id"],
                        lambda: ctx.should_stop()
                        or self.jobs.get(job["id"])["status"] == "cancel_requested",
                    )
                )
            elif job["operation"] == "code_animation_generate":
                result = asyncio.run(
                    self.jobs.animations.execute(job["input"], job["id"])
                )
                ...
                def progress(value):
                    return self.jobs.progress(job["id"], value)

                result = (
                    asyncio.run(
                        self.jobs.sprites.generate(
                            job["input"], job["id"], stopped, progress
                        )
                    )
                    if job["operation"] == "sprite_generate"
                    else self.jobs.sprites.compile(
                        job["input"], job["id"], stopped, progress
                    )
                )
            elif job["operation"] == "episode_render":
                result = asyncio.run(
                    self.jobs.episodes.execute(
                        job["input"],
                        job["id"],
                        lambda: ctx.should_stop()
                        or self.jobs.get(job["id"])["status"] == "cancel_requested",
                        lambda pid: self.jobs.attach_child(job["id"], pid),
                        lambda value: self.jobs.progress(job["id"], value),
                    )
                )
            elif job["operation"] == "timeline_render":
                result = self.jobs.timelines.execute(
                    job["input"],
                    job["id"],
                    lambda: ctx.should_stop()
                    or self.jobs.get(job["id"])["status"] == "cancel_requested",
                ...
                result = self.jobs.trainer.execute(
                    job["input"],
                    job["id"],
                    lambda: ctx.should_stop()
                    or self.jobs.get(job["id"])["status"] == "cancel_requested",
                ...
                result = (
                    asyncio.run(self.jobs.images.execute(job["input"], job["id"]))
                    if job["operation"] == "image_generate"
                    else self.jobs.sketches.export(
                        job["sketch_id"], {"revision": job["revision"]}
                    )
                )
            ...
            self.jobs.finish(
                job["id"],
                error=(
                    "Training failed; inspect local diagnostics"
                    if job["operation"] == "lora_train"
                    and not isinstance(exc, SketchError)
                    else str(exc)[:500]
                ),
            )
