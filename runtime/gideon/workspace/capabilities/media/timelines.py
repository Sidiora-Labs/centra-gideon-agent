...
class TimelineStore:
    ...
    def get(self, timeline_id, revision=None):
        ...
        with self.db() as db:
            if revision is None:
                row = db.execute(
                    "SELECT body FROM timelines WHERE id=? ORDER BY revision DESC LIMIT 1",
                    (timeline_id,),
                ).fetchone()
            else:
                integer(revision, 1, 1000000)
                row = db.execute(
                    "SELECT body FROM timelines WHERE id=? AND revision=?",
    ...
    def list(self):
        with self.db() as db:
            rows = db.execute(
                "SELECT body FROM timelines t WHERE revision=(SELECT MAX(revision) FROM timelines WHERE id=t.id) ORDER BY rowid DESC LIMIT 100"
            ).fetchall()
    ...
    def history(self, timeline_id):
        ...
        with self.db() as db:
            rows = db.execute(
                "SELECT body FROM timelines WHERE id=? ORDER BY revision DESC",
                (timeline_id,),
            ).fetchall()
        return {"items": [json.loads(row[0]) for row in rows]}
    ...
    def source(self, entry, kind, directory, index):
        ...
        raw = (
            self.videos.artifacts.raw_bytes(artifact_id, version=version)
        ...
        if kind == "image":
            CleanupService(self.videos.images).source(artifact_id, version).save(
                path, "PNG"
            )
            return path, None
        ...
        if kind == "video":
            ...
            try:
                ...
                if not data["streams"]:
                    raise ValueError()
                duration = number(float(data["format"]["duration"]), 0.01, 3600)
    ...
    def validate(self, body):
        ...
        for key in ("segments", "overlays", "audio"):
            if (
                ...
                or not (1 if key == "segments" else 0) <= len(body[key]) <= 20
            ):
        ...
        with TemporaryDirectory(prefix="gideon-timeline-validation-") as temporary:
            ...
            for index, entry in enumerate(body["segments"]):
                fields(
                    entry,
                    ("artifact_id", "version", "kind", "start", "duration"),
                    ("artifact_id", "version", "kind", "start", "duration"),
                )
                if entry["kind"] not in ("image", "video"):
                    raise SketchError(
                        "Timeline segments require image or video artifacts"
                    )
                number(entry["start"], 0, 3600)
                total += number(entry["duration"], 0.05, 300)
                _, duration = self.source(entry, entry["kind"], directory, index)
                if (
                    entry["kind"] == "image"
                    and entry["start"] != 0
                    or duration is not None
                    and entry["start"] + entry["duration"] > duration + 0.001
                ):
                    raise SketchError("Segment trim exceeds source duration")
            number(total, 0.05, 300)
            for index, overlay in enumerate(body["overlays"]):
                ...
                number(overlay["duration"], 0.05, total - overlay["start"])
                for key in ("x", "y"):
                    integer(
                        overlay[key], 0, body["width" if key == "x" else "height"] - 1
                    )
            ...
            for index, audio in enumerate(body["audio"]):
                ...
                number(audio["duration"], 0.05, total - audio["start"])
                number(audio["trim"], 0, 3600)
                number(audio["volume"], 0, 2)
                for key in ("fade_in", "fade_out"):
                    number(audio[key], 0, audio["duration"])
                _, duration = self.source(
                    audio, "audio", directory, "audio" + str(index)
                )
                if audio["trim"] + audio["duration"] > duration + 0.001:
    ...
    def save(self, body, timeline_id=None):
        ...
        fingerprint = hashlib.sha256(
            json.dumps([timeline_id, body], sort_keys=True).encode()
        ).hexdigest()
        with self.db() as db:
        ...
        with TemporaryDirectory(prefix="gideon-timeline-render-") as temporary:
            ...
            for index, entry in enumerate(document["segments"]):
                ...
                command += [
                    ...
                    "-an",
                    "-vf",
                    f"scale={document['width']}:{document['height']}:force_original_aspect_ratio=decrease,pad={document['width']}:{document['height']}:(ow-iw)/2:(oh-ih)/2,setsar=1,fps={document['fps']}",
                    "-c:v",
                    "libx264",
                    "-pix_fmt",
                    "yuv420p",
            ...
            for index, overlay in enumerate(document["overlays"]):
                path, _ = self.source(
                    overlay, "image", directory, "overlay" + str(index)
                )
                command += [
                    "-loop",
                    "1",
                ...
                filters += [
                    f"[{index+1}:v]scale={overlay['width']}:{overlay['height']}[o{index}]",
                    f"[{video_label}][o{index}]overlay={overlay['x']}:{overlay['y']}:enable='gte(t,{overlay['start']})*lt(t,{overlay['start']+overlay['duration']})'[v{index}]",
                ]
            ...
            for index, audio in enumerate(document["audio"]):
                ...
                filters.append(
                    f"[{silent+1+index}:a]atrim=start={audio['trim']}:duration={audio['duration']},asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo,volume={audio['volume']},afade=t=in:d={audio['fade_in']},afade=t=out:st={audio['duration']-audio['fade_out']}:d={audio['fade_out']},adelay={delay}|{delay}[a{index}]"
                )
                audio_labels.append(f"[a{index}]")
            filters.append(
                "".join(audio_labels)
                + f"amix=inputs={len(audio_labels)}:normalize=0:duration=first[aout]"
            )
            ...
            command += [
                ...
                ";".join(filters),
                "-map",
                f"[{video_label}]" if filters and video_label != "0:v" else "0:v",
                "-map",
                "[aout]",
                "-t",
                str(document["duration"]),
                "-c:v",
                "libx264",
                "-pix_fmt",
                "yuv420p",
            ...
            render_process(command, log, stopped, attached)
            prepared = dict(
                request, prompt=document["title"], selection="", engine="ffmpeg"
            )
            result = self.videos.materialize(
                VideoResult(local_path=str(output)), prepared, job_id
            )
            progress(1)
            return result
