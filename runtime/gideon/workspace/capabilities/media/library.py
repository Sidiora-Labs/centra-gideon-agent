...
class MediaLibrary:
    ...
    def project(self, artifact):
        ...
        for event in artifact.events:
            if event.type == "created":
                provenance.update(
                    {key: value for key, value in event.metadata.items() if key in keys}
                )
        return dict(
            id=artifact.slug,
            name=artifact.name,
            ...
            raw_url=f"/api/artifacts/{artifact.slug}/raw?version={artifact.version}",
    ...
    def list(self, query):
        ...
        filters = {
            key: text(query.get(key, "")) for key in ("kind", "tag", "collection", "q")
        }
        found = self.artifacts.list(
            **{key: value for key, value in filters.items() if value}
        )
        rows = sorted(
            (item for item in found if item.kind in ("image", "video")),
            key=lambda item: (item.updated_at, item.slug),
            reverse=True,
        )
        facets = dict(
            kinds=dict(Counter(item.kind for item in rows)),
            tags=dict(Counter(tag for item in rows for tag in item.tags)),
            collections=dict(
                Counter(item.collection for item in rows if item.collection)
            ),
        )
        return dict(
            items=[self.project(item) for item in rows[offset : offset + limit]],
            total=len(rows),
            offset=offset,
            limit=limit,
            facets=facets,
    ...
    def update(self, artifact_id, body):
        fields(
            body,
            ("expected_updated_at", "name", "tags", "collection"),
            ("expected_updated_at",),
        )
        ...
        if "tags" in body:
            ...
            patch["tags"] = list(
                dict.fromkeys(text(tag, 64, empty=False) for tag in body["tags"])
            )
        self._artifact(artifact_id)
        try:
            artifact = self.artifacts.update(
                artifact_id, expect_updated_at=expected, **patch
            )
    ...
    def import_image(self, data, *, filename, request_id, name="", mime=""):
        with self.artifacts.mutation_lock:
            return self._import_image(
                data, filename=filename, request_id=request_id, name=name, mime=mime
            )
        ...
        try:
            with Image.open(io.BytesIO(data)) as image:
                ...
                if (
                    ...
                    raise SketchError(
                        "Unsupported image format, animation or dimensions"
                    )
                image.verify()
