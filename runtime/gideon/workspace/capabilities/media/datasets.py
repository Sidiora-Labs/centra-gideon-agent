...
class DatasetStore:
    def __init__(self, path, images):
        ...
        with self.db() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS datasets (id TEXT, revision INTEGER, body TEXT, PRIMARY KEY(id,revision))"
            )
            db.execute(
                "CREATE TABLE IF NOT EXISTS dataset_requests (request TEXT PRIMARY KEY, fingerprint TEXT, body TEXT)"
            )
    ...
    def get(self, dataset_id, revision=None):
        with self.db() as db:
            if revision is None:
                row = db.execute(
                    "SELECT body FROM datasets WHERE id=? ORDER BY revision DESC LIMIT 1",
                    (dataset_id,),
                ).fetchone()
            else:
                integer(revision, 1, 1000000)
                row = db.execute(
                    "SELECT body FROM datasets WHERE id=? AND revision=?",
                    (dataset_id, revision),
                ).fetchone()
        if row is None:
    ...
    def list(self):
        with self.db() as db:
            rows = db.execute(
                "SELECT body FROM datasets d WHERE revision=(SELECT MAX(revision) FROM datasets WHERE id=d.id) ORDER BY rowid DESC LIMIT 100"
            ).fetchall()
    ...
    def history(self, dataset_id):
        ...
        with self.db() as db:
            rows = db.execute(
                "SELECT body FROM datasets WHERE id=? ORDER BY revision DESC",
                (dataset_id,),
            ).fetchall()
        return {"items": [json.loads(row[0]) for row in rows]}
    ...
    def save(self, body, dataset_id=None):
        fields(
            body,
            ("title", "base_model", "entries", "request_id", "revision"),
            ("title", "base_model", "entries", "request_id"),
        )
        ...
        if not isinstance(base, str) or not re.fullmatch(
            r"[A-Za-z0-9][A-Za-z0-9_.-]{0,99}/[A-Za-z0-9][A-Za-z0-9_.-]{0,99}", base
        ):
        ...
        fingerprint = hashlib.sha256(
            json.dumps([dataset_id, body], sort_keys=True).encode()
        ).hexdigest()
        with self.db() as db:
            db.execute("BEGIN IMMEDIATE")
            replay = db.execute(
                "SELECT fingerprint,body FROM dataset_requests WHERE request=?",
                (body["request_id"],),
            ).fetchone()
            if replay:
            ...
            db.execute(
                ...
                (result["id"], result["revision"], serialized),
            )
            db.execute(
                "INSERT INTO dataset_requests VALUES (?,?,?)",
                (body["request_id"], fingerprint, serialized),
            )
        return result
    ...
    def stage(self, dataset_id, revision, directory):
        ...
        for index, entry in enumerate(document["entries"]):
            image = self.images.source(entry["artifact_id"], entry["version"]).convert(
                "RGB"
            )
            buffer = io.BytesIO()
