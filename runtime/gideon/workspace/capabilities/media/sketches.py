...
def fields(body, allowed, required=()):
    if (
        ...
        or set(body) - set(allowed)
        or set(required) - set(body)
    ):
...
def strokes(value, width, height):
    ...
    for stroke in value:
        ...
        if (
            stroke["tool"] not in ("draw", "erase")
            or not isinstance(stroke["color"], str)
            or not re.fullmatch(r"#[0-9a-fA-F]{6}", stroke["color"])
        ):
...
class SketchStore:
    def __init__(self, path: Path, artifacts):
        ...
        with self._db() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS sketches (id TEXT PRIMARY KEY, request_id TEXT UNIQUE, request_hash TEXT, body TEXT NOT NULL)"
            )
            db.execute(
                "CREATE TABLE IF NOT EXISTS exports (id TEXT, revision INTEGER, artifact_id TEXT, PRIMARY KEY(id,revision))"
            )
    ...
    def get(self, sketch_id):
        with self._db() as db:
            row = db.execute(
                "SELECT body FROM sketches WHERE id=?", (sketch_id,)
            ).fetchone()
        if not row:
    ...
    def list(self):
        with self._db() as db:
            rows = db.execute(
                "SELECT body FROM sketches ORDER BY rowid DESC LIMIT 100"
            ).fetchall()
    ...
    def source(self, sketch):
        ...
        art = (
            self.artifacts.get(source, version=sketch["source_version"])
            if source
            else None
        )
        raw = (
            self.artifacts.raw_bytes(source, version=sketch["source_version"])
            if art and art.kind == "image"
            else None
        )
        if not raw:
    ...
    def create(self, body):
        ...
        width, height = integer(body["width"], 1, 4096), integer(
            body["height"], 1, 4096
        )
        ...
        sketch = dict(
            ...
            strokes=strokes(body.get("strokes", []), width, height),
            revision=1,
        ...
        with self._db() as db:
            ...
            previous = db.execute(
                "SELECT request_hash,body FROM sketches WHERE request_id=?",
                (request_id,),
            ).fetchone()
            if previous:
                if previous[0] != fingerprint:
                    raise SketchError(
                        "Request ID already used with different content", 409
                    )
                return json.loads(previous[1])
            db.execute(
                "INSERT INTO sketches VALUES (?,?,?,?)",
                (sketch["id"], request_id, fingerprint, json.dumps(sketch)),
            )
        return sketch
    ...
    def update(self, sketch_id, body):
        ...
        with self._db() as db:
            ...
            sketch.update(
                strokes=strokes(body["strokes"], sketch["width"], sketch["height"]),
                revision=sketch["revision"] + 1,
                updated_at=datetime.now(timezone.utc).isoformat(),
            )
            db.execute(
                "UPDATE sketches SET body=? WHERE id=?", (json.dumps(sketch), sketch_id)
            )
        return sketch
    ...
    def export(self, sketch_id, body):
        ...
        with self._db() as db:
            ...
            if artifact and not any(
                event.metadata.get("sketch_id") == sketch_id
                and event.metadata.get("sketch_revision") == sketch["revision"]
            ...
            if not artifact:
                ...
                for stroke in sketch["strokes"]:
                    ...
                    color = (
                        stroke["color"] if stroke["tool"] == "draw" else (0, 0, 0, 0)
                    )
                ...
                artifact = self.artifacts.create_binary(
                    ...
                    event_metadata={
                        ...
                        "source_artifact_id": sketch["source_artifact_id"] or "",
                        "source_version": sketch["source_version"] or 0,
                    },
                )
            db.execute(
                "INSERT OR REPLACE INTO exports VALUES (?,?,?)",
                (sketch_id, sketch["revision"], artifact.slug),
            )
        return dict(
            ...
            sketch_id=sketch_id,
            sketch_revision=sketch["revision"],
            source_artifact_id=sketch["source_artifact_id"],
            source_version=sketch["source_version"],
        )
