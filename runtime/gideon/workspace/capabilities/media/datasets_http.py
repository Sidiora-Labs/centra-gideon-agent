...
async def dispatch(request):
    try:
        ...
        if request.path.endswith('/training'):
            ...
            return web.Response(body=data, content_type='application/zip', headers={'Content-Disposition': 'attachment; filename="dataset.zip"'})
        elif request.method == 'GET':
            result = jobs.datasets.history(dataset_id) if request.path.endswith('/history') else jobs.datasets.get(dataset_id) if dataset_id else jobs.datasets.list()
        else:
            try:
