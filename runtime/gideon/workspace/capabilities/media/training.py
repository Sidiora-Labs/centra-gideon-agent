        ...
        try:
            ...
            while process.poll() is None:
                if stopped() or time.monotonic() >= deadline:
                    raise SketchError(
                        "Training interrupted; completed checkpoints are retained", 409
                    )
                try:
            ...
            if process.returncode:
                raise SketchError(
                    "Training process failed; inspect its local diagnostics", 503
                )
        finally:
...
class DiffusersTrainer:
    def __init__(self, home, datasets, allowed=True):
        ...
        self.script = (
            self.runtime / "examples/text_to_image/train_text_to_image_lora.py"
        )
    ...
    def readiness(self):
        present = (
            self.python.is_file()
            and os.access(self.python, os.X_OK)
            and self.script.is_file()
        )
        return dict(
            available=bool(self.allowed and present),
            admitted=self.allowed,
            inference_verified=False,
            reason=(
                "Operator training grant unavailable"
                if not self.allowed
                else (
                    "Diffusers trainer installation missing"
                    if not present
                    else "Trainer files present; dependencies, GPU and training not verified"
                )
            ),
        )
    ...
    def command(self, request, data_dir, output_dir):
        command = [
            ...
            str(self.script),
            "--pretrained_model_name_or_path=" + request["base_model"],
            "--train_data_dir=" + str(data_dir),
            "--output_dir=" + str(output_dir),
            "--resolution=512",
            "--center_crop",
            "--train_batch_size=1",
            "--max_train_steps=" + str(request["steps"]),
            "--rank=" + str(request["rank"]),
            "--learning_rate=" + str(request["learning_rate"]),
            "--seed=" + str(request["seed"]),
            "--checkpointing_steps=" + str(min(100, request["steps"])),
            "--report_to=none",
        ]
    ...
    def publish(self, source, job_id, base_model):
        ...
        with source.open("rb") as original:
            ...
            header.setdefault("__metadata__", {}).update(
                base_model=base_model, training_job_id=job_id
            )
    ...
    def execute(self, request, job_id, stopped, attached):
        ...
        if (
            self.script.resolve() != self.script.absolute()
            or self.runtime.resolve() != self.runtime.absolute()
        ):
        ...
        with TemporaryDirectory(prefix="gideon-training-data-") as directory:
            data = Path(directory)
            self.datasets.stage(
                request["dataset_id"], request["dataset_revision"], data
            )
            run_process(
                self.command(request, data, output),
                output / "training.log",
                stopped,
                attached,
            )
        ...
        return dict(
            ...
            checkpoints=self.checkpoints(job_id)["items"],
