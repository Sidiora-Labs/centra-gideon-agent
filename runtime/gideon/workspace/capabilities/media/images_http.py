...
async def dispatch(request):
    try:
        ...
        if request.method == "GET":
            ...
            result = (
                await jobs.images.lora_inventory(request.match_info.get("adapter_id"))
                if "/loras" in request.path
                else await jobs.images.capabilities()
            )
        else:
            try:
        ...
        return web.json_response(
            {"error": "Image provider capability lookup failed"}, status=503
        )
