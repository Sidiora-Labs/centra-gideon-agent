...
class AnnotationStore:
    def __init__(self, path, artifacts):
        ...
        with self.db() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS revisions (artifact TEXT, version INTEGER, revision INTEGER, body TEXT, PRIMARY KEY(artifact,version,revision))"
            )
            db.execute(
                "CREATE TABLE IF NOT EXISTS requests (artifact TEXT, version INTEGER, request TEXT, fingerprint TEXT, revision INTEGER, PRIMARY KEY(artifact,version,request))"
            )
    ...
    def source(self, artifact_id, version):
        if not isinstance(artifact_id, str) or not re.fullmatch(
            r"[A-Za-z0-9_-]{1,200}", artifact_id
        ):
        ...
        raw = (
            ...
            if artifact and artifact.kind in ("image", "video")
            else None
        )
    ...
    def get(self, artifact_id, version, revision=None):
        ...
        with self.db() as db:
            if revision is None:
                row = db.execute(
                    "SELECT body FROM revisions WHERE artifact=? AND version=? ORDER BY revision DESC LIMIT 1",
                    (artifact_id, version),
                ).fetchone()
            else:
                integer(revision, 1, 1000000)
                row = db.execute(
                    "SELECT body FROM revisions WHERE artifact=? AND version=? AND revision=?",
                    (artifact_id, version, revision),
                ).fetchone()
        if row:
        ...
        return dict(
            ...
            updated_at=None,
            annotations=[],
            attribution=dict(creator="", license="", source_url=""),
    ...
    def history(self, artifact_id, version, offset=0, limit=50):
        ...
        with self.db() as db:
            total = db.execute(
                "SELECT COUNT(*) FROM revisions WHERE artifact=? AND version=?",
                (artifact_id, version),
            ).fetchone()[0]
            rows = db.execute(
                "SELECT body FROM revisions WHERE artifact=? AND version=? ORDER BY revision DESC LIMIT ? OFFSET ?",
                (artifact_id, version, limit, offset),
            ).fetchall()
        return {
            "items": [
                dict(
                    ...
                    annotation_count=len(value["annotations"]),
                )
                for value in map(lambda row: json.loads(row[0]), rows)
            ],
            "total": total,
            "offset": offset,
            "limit": limit,
    ...
    def save(self, artifact_id, version, body):
        fields(
            body,
            ("revision", "request_id", "annotations", "attribution"),
            ("revision", "request_id", "annotations", "attribution"),
        )
        ...
        for entry in entries:
            ...
            if "time_seconds" in entry:
                ...
                if (
                    ...
                    or not 0 <= value <= 604800
                ):
                    raise SketchError(
                        "Timestamp requires a video and bounded nonnegative seconds"
                    )
        attribution = body["attribution"]
        fields(
            attribution,
            ("creator", "license", "source_url"),
            ("creator", "license", "source_url"),
        )
        ...
        if url and (
            ...
            raise SketchError(
                "Attribution source must be an HTTP URL without credentials"
            )
        ...
        with self.db() as db:
            ...
            prior = db.execute(
                "SELECT fingerprint,revision FROM requests WHERE artifact=? AND version=? AND request=?",
                (artifact_id, version, body["request_id"]),
            ).fetchone()
            if prior:
            ...
            latest = (
                db.execute(
                    "SELECT MAX(revision) FROM revisions WHERE artifact=? AND version=?",
                    (artifact_id, version),
                ).fetchone()[0]
                or 0
            )
            ...
            db.execute(
                ...
                (artifact_id, version, latest + 1, json.dumps(record)),
            )
            db.execute(
                "INSERT INTO requests VALUES (?,?,?,?,?)",
                (artifact_id, version, body["request_id"], fingerprint, latest + 1),
            )
        return record
