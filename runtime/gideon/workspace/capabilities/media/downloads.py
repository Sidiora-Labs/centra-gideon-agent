...
def capabilities():
    try:
        transport = importlib.util.find_spec("gideon.workspace.capabilities.knowledge.video_fetch") is not None
    except ModuleNotFoundError:
        transport = False
    return {"available": transport and importlib.util.find_spec("yt_dlp") is not None, "transport": "guarded-video-reader" if transport else "unavailable", "max_artifact_bytes": 33554432, "max_duration_seconds": 21600, "kinds": ["video", "audio"]}
...
class SourceDownloader:
    ...
    def prepare(self, body):
        ...
        if not isinstance(url, str) or len(url) > 2000 or not re.fullmatch(
    ...
    async def execute(self, request, job_id, stopped):
        ...
        return await asyncio.to_thread(self.publish, request, job_id, metadata, data, extension, duration)
    ...
    def publish(self, request, job_id, metadata, data, extension, duration):
        ...
        if existing:
            ...
            if event and event.metadata.get("download_sha256") == digest and event.metadata.get("source_request_sha256") == fingerprint:
                return self.result(existing, digest, len(data), duration, request["url"], metadata)
            raise SketchError("Downloaded source artifact identity conflict", 409)
        artifact = self.artifacts.create_binary(
            name=str(metadata.get("title") or "Downloaded media").strip()[:200], slug=slug,
            data=data, mime=mime, kind=kind, source="import", tags=["download", "source", kind],
            description="Guarded public media source download",
            event_metadata={"media_job_id": job_id, "source_url": request["url"], "source_external_id": str(metadata.get("id") or "")[:256], "download_sha256": digest, "source_request_sha256": fingerprint, "duration_seconds": str(duration)},
        )
        return self.result(artifact, digest, len(data), duration, request["url"], metadata)

    @staticmethod
    def result(artifact, digest, size, duration, url, metadata):
        return {"artifact_id": artifact.slug, "version": artifact.version, "kind": artifact.kind, "mime": artifact.mime, "sha256": digest, "bytes": size, "duration_seconds": duration, "source_url": url, "source_external_id": str(metadata.get("id") or "")}
