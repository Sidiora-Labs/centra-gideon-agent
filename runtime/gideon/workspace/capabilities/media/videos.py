...
def media_tools():
    if not shutil.which('ffmpeg') or not shutil.which('ffprobe'):
        raise SketchError('FFmpeg and ffprobe are required for verified video processing', 503)
...
def probe_video(path):
    ...
    try:
        ...
        if not math.isfinite(duration) or not 0 < duration <= 3600 or not 1 <= width <= 4096 or not 1 <= height <= 4096:
            raise ValueError()
        return {'width': width, 'height': height, 'duration_seconds': duration, 'codec': stream['codec_name'], 'format': info['format']['format_name']}
...
class VideoService:
    ...
    def source(self, artifact_id, version):
        ...
        raw = self.artifacts.raw_bytes(artifact_id, version=version) if artifact and artifact.kind == 'video' else None
        if not raw:
    ...
    def prepare(self, body):
        keys = ('prompt', 'duration_seconds', 'aspect_ratio', 'controls', 'first_frame_artifact_id', 'first_frame_version', 'last_frame_artifact_id', 'last_frame_version', 'continuation_artifact_id', 'continuation_version')
        ...
        for key, value in controls.items():
            if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(value) or not 0 <= value <= 4294967295 or key == 'seed' and not isinstance(value, int):
        ...
        for prefix in ('first_frame', 'last_frame', 'continuation'):
            if (prefix+'_artifact_id' in body) != (prefix+'_version' in body):
                raise SketchError('Video conditioning references require an artifact and version')
            if prefix+'_artifact_id' in body:
                ...
                if prefix == 'continuation':
                    ...
                    CleanupService(self.images).source(artifact_id, body[prefix+'_version'])
        ...
        return dict(body, prompt=prompt.strip(), aspect_ratio=aspect, controls=dict(controls), selection=f'{selected[0].name}:{selected[1]}' if selected else '')

    async def capabilities(self):
        selected = self.selector()
        tools_available = bool(shutil.which('ffmpeg') and shutil.which('ffprobe'))
        if selected is None:
            return {'selection': '', 'available': False, 'media_tools_available': tools_available, 'models': []}
        provider, model_id = selected
        models = await asyncio.wait_for(provider.list_models(), timeout=5)
        return dict(selection=f'{provider.name}:{model_id}', available=bool(await asyncio.wait_for(provider.is_available(), timeout=5)), media_tools_available=tools_available,
                    models=[dict(name=model.name, durations=model.durations, aspect_ratios=model.aspect_ratios, max_duration_s=model.max_duration_s, supports_first_frame=model.supports_first_frame, supports_last_frame=model.supports_last_frame, supports_continuation=model.supports_continuation, controls={key: dict(minimum=value.minimum, maximum=value.maximum, integer=value.integer) for key, value in model.supported_controls.items()}) for model in models if model.name == model_id])
    ...
    def validate_model(self, request, model):
        ...
        if request['aspect_ratio'] and request['aspect_ratio'] not in model.aspect_ratios:
            raise SketchError('Selected model does not advertise this aspect ratio')
        for prefix in ('first_frame', 'last_frame', 'continuation'):
            if request.get(prefix+'_artifact_id') and not getattr(model, 'supports_'+prefix):
                raise SketchError('Selected model does not advertise '+prefix+' conditioning')
        for key, value in request['controls'].items():
            control = model.supported_controls.get(key)
            if control is None or not control.minimum <= value <= control.maximum or control.integer and not isinstance(value, int):
                raise SketchError('Selected model does not support this '+key+' control')
    ...
    def stage(self, request, directory):
        ...
        for prefix in ('first_frame', 'last_frame'):
            if request.get(prefix+'_artifact_id'):
                ...
                CleanupService(self.images).source(request[prefix+'_artifact_id'], request[prefix+'_version']).save(path, 'PNG')
                paths[prefix] = str(path)
        if request.get('continuation_artifact_id'):
            source = directory / 'continuation.video'
            source.write_bytes(self.source(request['continuation_artifact_id'], request['continuation_version'])[0])
            probe_video(source)
    ...
    async def execute(self, request, job_id):
        ...
        with TemporaryDirectory(prefix='gideon-video-input-') as directory:
            ...
            try:
                results = await asyncio.wait_for(provider.generate(request['prompt'], model=model_id, duration_seconds=request['duration_seconds'], aspect_ratio=request['aspect_ratio'], **request['controls'], **paths), timeout=1200)
            except Exception as exc:
                raise SketchError('Video provider execution failed; inspect provider diagnostics', 503) from exc
            if not results:
    ...
    def materialize(self, result, request, job_id):
        ...
        if raw is None or len(raw[0]) > 16 * 1024 * 1024 or raw[1] not in ('video/mp4', 'video/webm'):
        ...
        if artifact and (not artifact.events or artifact.events[0].metadata != metadata):
            raise SketchError('Video output identity conflicts', 409)
        if artifact is None:
            artifact = self.artifacts.create_binary(name=request['prompt'][:80], data=raw[0], mime=raw[1], kind='video', source='chat', slug=slug, event_metadata=metadata)
        return dict(artifact_id=artifact.slug, version=artifact.version, **{key: decoded[key] for key in ('width', 'height', 'duration_seconds')})
