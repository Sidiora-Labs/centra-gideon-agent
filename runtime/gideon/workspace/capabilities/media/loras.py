...
DTYPES = {'F64': 8, 'F32': 4, 'F16': 2, 'BF16': 2, 'I64': 8, 'I32': 4, 'I16': 2, 'I8': 1, 'U8': 1, 'BOOL': 1}
...
class LoraCatalog:
    ...
    def _path(self, adapter_id):
        if not isinstance(adapter_id, str) or not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]{0,150}\.safetensors', adapter_id):
    ...
    def get(self, adapter_id, base_model=''):
        ...
        with os.fdopen(fd, 'rb') as stream:
            ...
            for descriptor in header.values():
                ...
                if not width or not isinstance(shape, list) or len(shape) > 8 or any(type(size) is not int or size < 0 for size in shape):
                    raise SketchError('Unsupported tensor shape or dtype')
                if not isinstance(pair, list) or len(pair) != 2 or any(type(value) is not int or value < 0 for value in pair) or pair[1] - pair[0] != math.prod(shape) * width:
                    raise SketchError('Invalid tensor offsets')
                offsets.append(pair)
            end = 0
            for start, stop in sorted(offsets):
                if start != end:
                    raise SketchError('Tensor payload contains overlapping or missing regions')
                end = stop
            if end != before.st_size - 8 - length:
                raise SketchError('Tensor payload is truncated or has trailing data')
            names = list(header)
            if not any('lora_A' in name or 'lora_down' in name for name in names) or not any('lora_B' in name or 'lora_up' in name for name in names):
                raise SketchError('Tensor names do not describe a supported LoRA layout')
        ...
        state = 'unknown' if not declared or not base_model else 'metadata_match' if declared == base_model else 'metadata_mismatch'
        return dict(id=adapter_id, sha256=digest.hexdigest(), bytes=before.st_size, tensor_count=len(header), base_model=declared[:200],
                    compatibility=state, effect_verified=False, trigger_words=metadata.get('trigger_words', '')[:500])

    def list(self, base_model=''):
        if not self.root.exists():
            return {'items': [], 'invalid': [], 'truncated': False, 'effect_verified': False}
        if self.root.is_symlink() or self.root.resolve() != self.root:
            raise SketchError('Adapter root must not contain symbolic links')
        ...
        for path in paths[:32]:
            ...
            if consumed >= MAX_BYTES:
                break
        return {'items': items, 'invalid': invalid, 'truncated': len(items)+len(invalid) < len(paths), 'effect_verified': False}

    def resolve(self, selection, base_model):
        if not isinstance(selection, dict) or set(selection) != {'id', 'sha256', 'scale'}:
        ...
        return {'path': str(self._path(selection['id'])), 'scale': scale, 'sha256': item['sha256']}
    ...
    def stage(self, selection, base_model, destination):
        ...
        return {'path': str(destination), 'scale': resolved['scale'], 'sha256': resolved['sha256']}
