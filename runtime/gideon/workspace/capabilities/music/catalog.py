...
FIELDS = {
    "artists": {"name", "bio"},
    "albums": {"title", "artist_id", "track_ids"},
    "tracks": {"title", "artist_id", "notes"},
}
...
class MusicCatalog:
    def __init__(self, root, artifacts):
        ...
        with self._db() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS catalog (kind TEXT, id TEXT PRIMARY KEY, payload TEXT NOT NULL)"
            )
    ...
    def _read(self, db, kind, entity_id):
        self._kind(kind)
        row = db.execute(
            "SELECT payload FROM catalog WHERE kind=? AND id=?", (kind, entity_id)
        ).fetchone()
        if row is None:
    ...
    def _fields(self, db, kind, data):
        ...
        for key in ("name", "title", "bio", "notes"):
            if key in data:
                clean[key] = text(
                    data[key],
                    key,
                    200 if key in ("name", "title") else 10000,
                    key in ("name", "title"),
                )
        if "archived" in data:
        ...
        if "track_ids" in data:
            ...
            if (
                ...
                or len(tracks) > 1000
                or any(not isinstance(value, str) for value in tracks)
            ):
    ...
    def list(self, kind, *, q="", archived=False, offset=0, limit=50):
        ...
        with self._db() as db:
            rows = db.execute(
                "SELECT payload FROM catalog WHERE kind=? AND json_extract(payload,'$.archived')=? AND instr(lower(COALESCE(json_extract(payload,'$.title'),json_extract(payload,'$.name'))),lower(?))>0 ORDER BY id LIMIT ? OFFSET ?",
                (kind, int(archived), q, limit, offset),
            )
    ...
    def _revision(self, item, data):
        ...
        if item["revision"] != revision:
            raise DomainError(
                "Catalog record changed; reload before editing",
                409,
                "revision_conflict",
            )
        item["revision"] += 1
    ...
    def attach(self, track_id, data):
        ...
        if source["kind"] == "generated":
            raise DomainError(
                "Generated-source registration requires an available verified generation job adapter",
                503,
                "generation_provenance_unavailable",
            )
        if set(source) != {"kind", "label", "license"}:
            raise DomainError(
                "Imported source requires kind, label and license; model and job are not inferred"
            )
        source = {
            "kind": "imported",
            "label": text(source["label"], "source label", 500, True),
            "license": text(source["license"], "license", 500, True),
            "model": None,
            "job_id": None,
    ...
    def _attach_verified(self, track_id, revision, ref, source):
        ...
        if artifact is None or raw is None:
            raise DomainError(
                "Audio artifact version or bytes missing", 404, "artifact_not_found"
            )
        if artifact.kind != "audio" or raw[1] not in ("audio/wav", "audio/mpeg"):
            raise DomainError(
                "Catalog measurement currently supports PCM WAV audio artifacts",
                422,
                "unsupported_audio",
            )
        if raw[1] == "audio/mpeg":
            try:
                ...
                raise DomainError(
                    "MP3 measurement requires ffprobe and valid audio",
                    422,
                    "invalid_audio",
                ) from exc
        else:
            try:
                with wave.open(io.BytesIO(raw[0]), "rb") as audio:
                    ...
                    if (
                        ...
                        or rate <= 0
                        or len(decoded)
                        != frames * audio.getsampwidth() * audio.getnchannels()
                    ):
                        raise ValueError("Truncated or empty WAV")
                    duration = frames / rate
            except (wave.Error, EOFError, ValueError) as exc:
                raise DomainError(
                    "Invalid or truncated PCM WAV", 422, "invalid_audio"
                ) from exc
        with self._db() as db:
            ...
            for prior in track["renders"]:
                if prior["artifact_ref"] == ref:
                    if prior["source"] == source:
                        return track
                    raise DomainError(
                        "This artifact version already has immutable provenance",
                        409,
                        "render_conflict",
                    )
            ...
            render = {
                ...
                "sha256": hashlib.sha256(raw[0]).hexdigest(),
                "source": source,
            }
    ...
    def select(self, track_id, data):
        ...
        with self._db() as db:
            ...
            if render is None:
                raise DomainError(
                    "Render is not attached to this track", 404, "render_not_found"
                )
            ref = render["artifact_ref"]
            if self.artifacts.raw_bytes(ref["slug"], version=ref["version"]) is None:
                raise DomainError(
                    "Selected audio bytes are missing", 404, "artifact_not_found"
                )
            track["selected_render_id"] = render["id"]
            self._write(db, "tracks", track)
            return track
