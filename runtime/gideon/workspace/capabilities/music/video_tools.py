    ...
    async def list_tools(self):
        ...
        for action in (
            ...
            if action in ("create", "update", "render"):
                fields["data"] = {
                    "type": "object",
                    "description": "Project: title,track_id,render_id,tempo_bpm,offset_seconds,scenes[{id,image_ref:{slug,version},beats}]; update adds revision. Render: request_id,project_id,revision.",
                }
            readonly = action in ("list", "get", "jobs", "job")
            tools.append(
                ToolDefinition(
                    name="music_video_" + action,
                    provider=self.name,
                    description=action.capitalize()
                    + " a beat-grid project or actual video render.",
                    parameters={
                        "type": "object",
                        "properties": fields,
