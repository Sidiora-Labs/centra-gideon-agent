...
class RepertoireStore:
    def __init__(self, root: Path, artifacts: ArtifactProvider):
        ...
        with self._db() as db:
            ...
            db.execute(
                "CREATE TABLE IF NOT EXISTS attempts (id TEXT PRIMARY KEY, item_id TEXT NOT NULL, fingerprint TEXT NOT NULL, receipt TEXT NOT NULL)"
            )
            db.execute(
                "CREATE TABLE IF NOT EXISTS imports (id TEXT PRIMARY KEY, fingerprint TEXT NOT NULL, item_id TEXT UNIQUE NOT NULL, receipt TEXT NOT NULL)"
            )
    ...
    def _write(self, db, item):
        stored = {
            ...
            if key != "attachment_availability"
        }
        db.execute(
            "INSERT OR REPLACE INTO items VALUES (?, ?)",
            (stored["id"], json.dumps(stored)),
        )
    ...
    def _fields(self, data):
        ...
        if "notation" in data:
            ...
            out["notation"] = {
                "format": format_name,
                "text": text(notation["text"], "notation", 200000),
            }
        ...
        if "links" in data:
            ...
            for link in data["links"]:
                ...
                if not link_type[0].isalnum() or any(
                ...
                clean = {
                    "type": link_type,
                    "id": text(link["id"], "link ID", 200, True),
        ...
        if "scroll_duration_seconds" in data:
            out["scroll_duration_seconds"] = (
                None
                if data["scroll_duration_seconds"] is None
                else integer(
    ...
    def _schedule(self, data):
        ...
        if (
            ...
            or not isinstance(ease, (int, float))
            or not math.isfinite(ease)
            or not 1.3 <= ease <= 5
        ):
        ...
        result["last_grade"] = (
            None
            if data["last_grade"] is None
            else integer(data["last_grade"], "last_grade", 0, 5)
        )
        return result
    ...
    def create(self, data):
        ...
        item = dict(
            ...
            capo=0,
            tuning="",
            notation={"format": "plain", "text": ""},
            source_url="",
            links=[],
            scroll_duration_seconds=None,
            attachment_refs=[],
            stage="new",
            practice_history=[],
            due_at=None,
    ...
    def import_song(
        ...
        *,
        ...
        with self._db() as db:
            prior = db.execute(
                "SELECT fingerprint,item_id,receipt FROM imports WHERE id=?",
                (import_id,),
            ).fetchone()
            if prior:
                if prior[0] != fingerprint:
                    raise DomainError(
                        "Import ID already used with different input",
                        409,
                        "import_conflict",
                    )
                return {
                    **json.loads(prior[2]),
                    "item": self._read(db, prior[1]),
                    "replayed": True,
                }
            ...
            db.execute(
                ...
                (import_id, fingerprint, item_id, json.dumps(receipt)),
            )
    ...
    def rollback_import(self, import_id, source_fingerprint):
        ...
        with self._db() as db:
            row = db.execute(
                "SELECT item_id,receipt FROM imports WHERE id=?", (import_id,)
            ).fetchone()
            if not row:
            ...
            if receipt["source_fingerprint"] != source_fingerprint:
                raise DomainError(
                    "Import fingerprint does not match", 409, "import_conflict"
                )
            item = self._read(db, row[0])
            if item["revision"] != receipt["revision"]:
                raise DomainError(
                    "Imported song changed; rollback refused", 409, "revision_conflict"
                )
        ...
        with self._db() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT payload FROM items ORDER BY id LIMIT ? OFFSET ?",
                    (limit, offset),
                )
            ]
    ...
    def update(self, item_id, data):
        ...
        with self._db() as db:
            ...
            if item["revision"] != revision:
                raise DomainError(
                    "Item changed; reload before editing", 409, "revision_conflict"
                )
            item.update(fields)
            item["revision"] += 1
            self._write(db, item)
    ...
    def practice(self, item_id, data):
        ...
        try:
            occurred = datetime.fromisoformat(
                text(data["occurred_at"], "occurred_at", 64)
            )
            ...
            raise DomainError(
                "Practice requires an offset timestamp and IANA timezone"
            ) from exc
        fingerprint = json.dumps({**data, "item_id": item_id}, sort_keys=True)
        with self._db() as db:
            prior = db.execute(
                "SELECT fingerprint,receipt FROM attempts WHERE id=?", (attempt_id,)
            ).fetchone()
            if prior:
                if prior[0] != fingerprint:
                    raise DomainError(
                        "Attempt ID already used with different input",
                        409,
                        "attempt_conflict",
                    )
            ...
            if item["revision"] != revision:
                raise DomainError(
                    "Item changed; reload before practicing", 409, "revision_conflict"
                )
            history = item["practice_history"]
            if len(history) >= 10000:
                raise DomainError("Practice history limit reached", 409, "history_full")
            if history and occurred < datetime.fromisoformat(
            ...
            interval = (
                ...
                if repetitions <= 1
                    ...
                    6
                    if repetitions == 2
                    else min(36500, math.floor(item["interval"] * item["ease"] + 0.5))
                )
            )
            ease = round(
                max(
                    1.3, item["ease"] + 0.1 - (5 - grade) * (0.08 + (5 - grade) * 0.02)
                ),
                2,
            )
            try:
                due = (
                    (occurred.astimezone(zone) + timedelta(days=interval))
            ...
            item.update(
                stage="review" if repetitions >= 2 else "learning",
                repetitions=repetitions,
                interval=interval,
                ease=ease,
                due_at=due,
                last_practiced_at=occurred.isoformat(),
                last_grade=grade,
                revision=revision + 1,
            )
            ...
            db.execute(
                ...
                (attempt_id, item_id, fingerprint, json.dumps(receipt)),
            )
            return receipt
