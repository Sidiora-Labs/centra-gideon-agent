...
class Image3DTools(ToolProvider):
    def __init__(self, store=None):
        home = config_dir()
        self.store = store or Image3DStore(
    ...
    async def list_tools(self):
        ...
        for action in (
            ...
            if action in ("configure", "submit"):
                props["data"] = {
                    "type": "object",
                    "description": "Config: enabled,credential_name,model,revision. Submit: request_id,title,image_ref{slug,version},target_polycount,should_texture,license.",
                }
            readonly = action in ("readiness", "config", "list", "get")
            tools.append(
                ToolDefinition(
                    name="music_models3d_" + action,
                    provider=self.name,
                    description=action.capitalize() + " explicit image-to-3D jobs.",
                    parameters={
                        "type": "object",
                        "properties": props,
                        "required": list(props),
    ...
    async def invoke(self, tool_name, arguments):
        ...
        try:
            result = getattr(self.store, tool_name.removeprefix("music_models3d_"))(
