...
def validate(data):
    ...
    for part in parts:
        ...
        if (
            ...
            or not re.fullmatch(r"[A-Za-z][A-Za-z0-9_-]{0,63}", part["id"])
            or part["id"] in ids
        ):
            raise DomainError("Part IDs must be unique identifier names")
        ids.add(part["id"])
        text(part["name"], "part name", 200, True)
        if (
            part["shape"] not in ("box", "sphere", "cylinder")
            or not isinstance(part["color"], str)
            or not re.fullmatch(r"#[0-9a-fA-F]{6}", part["color"])
        ):
        ...
        if joint is not None:
            if (
                ...
                or set(joint) != {"axis", "min_degrees", "max_degrees"}
                or joint["axis"] not in ("x", "y", "z")
            ):
                raise DomainError("Invalid rigid joint declaration")
            number(joint["min_degrees"], -360, 360)
            number(joint["max_degrees"], -360, 360)
            if joint["min_degrees"] >= joint["max_degrees"]:
                raise DomainError("Joint range must increase")
    lookup = {part["id"]: part for part in parts}
    for part in parts:
        parent = part["parent_id"]
        seen = {part["id"]}
        while parent is not None:
            if not isinstance(parent, str) or parent not in ids or parent in seen:
                raise DomainError("Unknown parent or cyclic hierarchy")
            seen.add(parent)
    ...
    for clip in clips:
        ...
        if name in names or clip["part_id"] not in lookup:
            raise DomainError("Duplicate clip name or unknown target")
        names.add(name)
        joint = lookup[clip["part_id"]]["joint"]
        if not joint or joint["axis"] != clip["axis"]:
            raise DomainError("Clip axis must match a declared joint")
        number(clip["from_degrees"], -360, 360)
        number(clip["to_degrees"], -360, 360)
        number(clip["duration_seconds"], 0.1, 60)
...
class AssemblyStore:
    def __init__(self, home, artifacts):
        ...
        with self._db() as db:
            ...
            db.execute(
                "CREATE TABLE IF NOT EXISTS assemblies(id TEXT PRIMARY KEY,payload TEXT)"
            )
            db.execute(
    ...
    def _get(self, db, item_id):
        row = db.execute(
            "SELECT payload FROM assemblies WHERE id=?",
            (text(item_id, "id", 100, True),),
        ).fetchone()
        if not row:
            raise DomainError("Assembly not found", 404, "not_found")
    ...
    def _save(self, db, item):
        ...
        db.execute(
            "INSERT OR REPLACE INTO assemblies VALUES (?,?)", (item["id"], payload)
        )
        db.execute(
    ...
    def create(self, data):
        spec = validate(data)
        _, diagnostics = compile_model(spec)
        with self._db() as db:
            return self._save(
                db,
                {
    ...
    def list(self, offset=0, limit=50):
        ...
        with self._db() as db:
            return [
                self._get(db, row[0])
                for row in db.execute(
                    "SELECT id FROM assemblies ORDER BY rowid LIMIT ? OFFSET ?",
                    (limit, offset),
                ).fetchall()
            ]
    ...
    def history(self, item_id):
        with self._db() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT payload FROM versions WHERE id=? ORDER BY revision",
                    (item_id,),
                )
            ]
    ...
    def _revision(self, item, value):
        ...
        if item["revision"] != value:
            raise DomainError(
                "Assembly changed; reload before editing", 409, "revision_conflict"
            )
    ...
    def refine(self, item_id, data):
        if (
            ...
            or set(data) != {"revision", "operations"}
            or not isinstance(data["operations"], list)
            or not data["operations"]
            or any(
                value not in ("ground", "clamp_clips") for value in data["operations"]
            )
        ):
    ...
    def export(self, item_id, data):
        ...
        with self._db() as db:
            ...
            if prior:
                if not self.artifacts.get(
                    prior["model_ref"]["slug"], version=prior["model_ref"]["version"]
                ) or not self.artifacts.get(
                    prior["source_ref"]["slug"], version=prior["source_ref"]["version"]
                ):
                    raise DomainError(
                        "Export artifacts missing", 404, "artifact_not_found"
                    )
                return prior
            spec = {key: item[key] for key in ("title", "parts", "clips")}
            raw, _ = compile_model(spec)
            model = self.artifacts.create_binary(
                name=item["title"],
                data=raw,
                kind="model",
            ...
            source = self.artifacts.create(
                name=item["title"] + " source",
                content=source_module(spec),
                kind="text",
                source="manual",
            )
            result = {
                "revision": item["revision"],
                "model_ref": {"slug": model.slug, "version": model.version},
                "source_ref": {"slug": source.slug, "version": source.version},
            }
            db.execute(
