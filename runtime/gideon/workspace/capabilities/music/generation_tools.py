...
class MusicGenerationTools(ToolProvider):
    def __init__(self, service=None):
        home = config_dir()
        self.service = service or MusicGeneration(home, MusicCatalog(home / 'capabilities' / 'music', NativeArtifactProvider(root=home / 'artifacts')))
    ...
    async def list_tools(self):
        ...
        for operation in ('readiness', 'configure', 'submit', 'get', 'list', 'cancel'):
            field = 'data' if operation in ('configure', 'submit') else 'id' if operation in ('get', 'cancel') else None
            properties = {field: {'type': 'object' if field == 'data' else 'string'}} if field else {}
            definitions.append(ToolDefinition(name='music_generation_' + operation, provider=self.name,
                description={'configure': 'Set enabled,model,credential_name,revision; credentials remain in the canonical store.',
                             ...
                             'get': 'Read a persisted generation job by id.', 'list': 'List the first50 generation job receipts.',
                             'cancel': 'Cancel local work; remote billing/completion may remain unknown.'}[operation],
                parameters={'type': 'object', 'properties': properties, 'required': [field] if field else [], 'additionalProperties': False},
                requires_approval=operation in ('configure', 'submit', 'cancel'),
                risk_level=RiskLevel.CAUTION if operation in ('configure', 'submit', 'cancel') else RiskLevel.SAFE))
        return definitions

    async def invoke(self, tool_name, arguments):
        schemas = {tool.name: tool.parameters for tool in await self.list_tools()}
        if tool_name not in schemas or not isinstance(arguments, dict) or set(arguments) != set(schemas[tool_name]['required']):
