...
class MidiTools(ToolProvider):
    def __init__(self, store=None):
        home = config_dir()
        self.store = store or MidiStore(home / 'capabilities' / 'music', MusicCatalog(home / 'capabilities' / 'music', NativeArtifactProvider(root=home / 'artifacts')))
    ...
    async def list_tools(self):
        ...
        for action in ('list', 'get', 'transcribe', 'update', 'export'):
            ...
            if action in ('transcribe', 'update', 'export'):
                properties['data'] = {'type': 'object', 'description': 'Transcribe: request_id,track_id,render_id,title,tempo_bpm. Update: revision,title,notes[{id,pitch,start_seconds,duration_seconds,velocity}]. Export: revision.'}
            read_only = action in ('list', 'get')
            tools.append(ToolDefinition(name='music_midi_' + action, provider=self.name, description=action.capitalize() + ' monophonic PCM transcription and MIDI notes.',
    ...
    async def invoke(self, tool_name, arguments):
        ...
        try:
            result = await asyncio.to_thread(getattr(self.store, tool_name.removeprefix('music_midi_')), *args)
