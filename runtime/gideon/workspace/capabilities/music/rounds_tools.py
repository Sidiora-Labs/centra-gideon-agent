...
class RoundTools(ToolProvider):
    def __init__(self, store=None):
        home = config_dir()
        self.store = store or RoundStore(home / 'capabilities' / 'music', MusicCatalog(home / 'capabilities' / 'music', NativeArtifactProvider(root=home / 'artifacts')))
    ...
    async def list_tools(self):
        ...
        for action in ('list', 'get', 'create', 'update', 'practice', 'history'):
            ...
            if action in ('create', 'update', 'practice'):
                properties['data'] = {'type': 'object', 'description': 'Round: title,tempo_bpm,meter_beats,notes,parts[{id,name,entry_beats,notation,catalog_ref}],partner_ids; update adds revision. Practice: request_id,round_revision,part_ids,occurred_at offsetISO,grade0..5,notes.'}
            read_only = action in ('list', 'get', 'history')
            tools.append(ToolDefinition(name='music_rounds_' + action, provider=self.name, description=action.capitalize() + ' authored musical canons and pinned part practice.',
