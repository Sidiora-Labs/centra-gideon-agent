...
class MusicToolProvider(ToolProvider):
    def __init__(self, repertoire=None, catalog=None):
        ...
        self.repertoire = repertoire or RepertoireStore(home / 'capabilities' / 'music', artifacts)
        self.catalog = catalog or MusicCatalog(home / 'capabilities' / 'music', artifacts)
    ...
    async def list_tools(self):
        tools = []
        for domain, actions in (('repertoire', ('list', 'get', 'create', 'update', 'practice')),
                                ('catalog', ('list', 'get', 'create', 'update', 'attach', 'select'))):
            for action in actions:
                properties, required = {}, []
                if domain == 'catalog' and action not in ('attach', 'select'):
                    properties['kind'] = {'type': 'string', 'enum': ['artists', 'albums', 'tracks']}
                    required.append('kind')
                if action not in ('list', 'create'):
                    properties['id'] = {'type': 'string', 'description': 'Existing repertoire item or catalog record ID'}
                    required.append('id')
                if action == 'list':
                    properties.update(offset={'type': 'integer', 'minimum': 0}, limit={'type': 'integer', 'minimum': 1, 'maximum': 100})
                    if domain == 'catalog':
                    ...
                    properties['data'] = {'type': 'object', 'description': descriptions[action]}
                    required.append('data')
                read_only = action in ('list', 'get')
                tools.append(ToolDefinition(name=f'music_{domain}_{action}', provider=self.name,
    ...
    async def invoke(self, tool_name, arguments):
        ...
        if tool_name not in definitions:
            return ToolResult(success=False, error='Unknown music tool', metadata={'code': 'unknown_tool'})
        ...
        if not isinstance(arguments, dict) or set(arguments) - set(schema['properties']) or set(schema['required']) - set(arguments):
            return ToolResult(success=False, error='Invalid music tool arguments', metadata={'code': 'invalid_input', 'status': 400})
        ...
        try:
            ...
            return ToolResult(success=True, output=json.dumps(result), metadata={'domain': domain, 'operation': action})
