...
class DeckTools(ToolProvider):
    ...
    async def list_tools(self):
        ...
        for action in ('providers','list','get','analyze','prompts','apply'):
            keys=[] if action=='providers' else ['id'] if action=='list' else ['id','proposal_id'] if action=='get' else ['id','proposal_id','data'] if action=='apply' else ['id','data']
            readonly=action in ('providers','list','get')
            result.append(ToolDefinition(name='music_decks_assist_'+action,provider=self.name,description=action+' grounded card design proposals.',parameters={'type':'object','properties':{key:{'type':'object' if key=='data' else 'string'} for key in keys},'required':keys,'additionalProperties':False},requires_approval=not readonly,risk_level=RiskLevel.SAFE if readonly else RiskLevel.CAUTION))
        return result
    ...
    async def invoke(self,tool_name,arguments):
        ...
        try:
            if tool_name.startswith('music_decks_assist_'):
                ...
                value=self.assist.propose(arguments['id'],action,arguments['data']) if action in ('analyze','prompts') else getattr(self.assist,action)(*[arguments[key] for key in ('id','proposal_id','data') if key in arguments])
            else:value=getattr(self.store,tool_name.removeprefix('music_decks_'))(*[arguments[key] for key in ('id','key','data') if key in arguments])
            if inspect.isawaitable(value):value=await value
            return ToolResult(success=True,output=json.dumps(value))
        except (DomainError,CatalogError,SketchError,ValueError,TypeError) as exc:return ToolResult(success=False,error=str(exc),metadata={'status':getattr(exc,'status',400)})
