...
class SpotifyOAuth:
    ...
    def _local(self):
        if self.credentials is None:
            raise DomainError(
                "Authorization belongs to the configured connection service",
                409,
                "connection_owned",
            )
    ...
    def begin(self, data):
        self._local()
        if not isinstance(data, dict) or set(data) != {"client_id", "redirect_uri"}:
            raise DomainError("client_id and redirect_uri required")
        client = text(data["client_id"], "client ID", 200, True)
        redirect = text(data["redirect_uri"], "redirect URI", 2000, True)
        ...
        if (
            ...
            raise DomainError("HTTPS or explicit loopback redirect URI required")
        config = self.bridge.config()
        if not config["credential_name"]:
            raise DomainError("Save a named Spotify connection first")
        ...
        self.credentials.put(
            "spotify-pkce-" + state, {"type": "oauth2", "value": json.dumps(pending)}
        )
        return {
            "state": state,
            "expires_in": 600,
    ...
    def pending(self, state):
        self._local()
        text(state, "state", 100, True)
        self.credentials.reload()
        try:
            pending = json.loads(
                self.credentials.resolve("spotify-pkce-" + state).secret or ""
            )
        except (KeyError, ValueError):
            raise DomainError("Unknown or consumed OAuth state", 409, "oauth_state")
    ...
    async def _token(self, fields):
        async with aiohttp.ClientSession(
            ...
            async with session.post(
                TOKEN_URL, data=fields, allow_redirects=False
            ) as response:
                if response.status != 200:
                    raise DomainError(
                        "Spotify token exchange returned HTTP " + str(response.status),
                        502,
                        "oauth_exchange",
                    )
        ...
        if (
            ...
            raise DomainError("Invalid Spotify token response", 502, "oauth_exchange")
        return result

    async def complete(self, data):
        async with _LOCKS.setdefault(str(self.bridge.path.resolve()), asyncio.Lock()):
            return await self._complete(data)

    async def _complete(self, data):
        if not isinstance(data, dict) or set(data) != {"state", "code"}:
            raise DomainError("state and authorization code required")
    ...
    async def access(self, name):
        ...
        if not token.get("refresh_token"):
            raise DomainError(
                "Spotify authorization expired; reconnect", 401, "oauth_expired"
            )
