...
def quaternion(euler):
    sx, sy, sz = [math.sin(value / 2) for value in euler]
    cx, cy, cz = [math.cos(value / 2) for value in euler]
    return [
        sx * cy * cz + cx * sy * sz,
        cx * sy * cz - sx * cy * sz,
        cx * cy * sz + sx * sy * cz,
...
def mesh(part):
    sx, sy, sz = [value / 2 for value in part["size"]]
    triangles = []
    if part["shape"] == "box":
        ...
        def point(ring, segment):
            ...
            return (
                sx * math.sin(latitude) * math.cos(longitude),
        ...
        for segment in range(12):
            ...
            triangles.extend(
                [
                    [bottom, top, next_bottom],
                    [top, next_top, next_bottom],
                    [(0, -sy, 0), bottom, next_bottom],
                    [(0, sy, 0), next_top, top],
                ]
            )
    positions, normals = [], []
...
def compile_model(spec):
    ...
    binary = bytearray()
    parts = spec["parts"]
    indices = {part["id"]: i for i, part in enumerate(parts)}
    bounds = {}
    triangle_count = 0
    ...
    def world(point, part):
        point = rotate(point, quaternion(part["rotation"]))
        point = [point[i] + part["position"][i] for i in range(3)]
        return (
            world(point, parts[indices[part["parent_id"]]])
            if part["parent_id"]
            else point
        )

    for index, part in enumerate(parts):
        positions, normals = mesh(part)
        triangle_count += len(positions) // 9
        transformed = [
            world(positions[i : i + 3], part) for i in range(0, len(positions), 3)
        ]
        bounds[part["id"]] = {
            "min": [min(p[i] for p in transformed) for i in range(3)],
        ...
        document["nodes"].append(
            {
                ...
                "rotation": quaternion(part["rotation"]),
                "extras": {"joint": part["joint"]},
            }
        )
    for index, part in enumerate(parts):
        if part["parent_id"]:
            document["nodes"][indices[part["parent_id"]]].setdefault(
                "children", []
            ).append(index)
        else:
            document["scenes"][0]["nodes"].append(index)
    for clip in spec["clips"]:
        part = parts[indices[clip["part_id"]]]
        quaternions = []
    ...
    raw = (
        b"glTF"
        + struct.pack("<II", 2, 28 + len(encoded) + len(binary))
    ...
    for part in parts:
        ...
        volume += math.prod(size) * (
            {"box": 1, "sphere": math.pi / 6, "cylinder": math.pi / 4}[part["shape"]]
        )
        if min(size) / max(size) < 0.02:
            findings.append(
                {
                    "code": "thin_part",
                    "part_ids": [part["id"]],
                    "message": "A dimension is below two percent of the largest dimension.",
                }
            )
    for one, two in itertools.combinations(parts, 2):
        a, b = bounds[one["id"]], bounds[two["id"]]
        if all(
            min(a["max"][i], b["max"][i]) - max(a["min"][i], b["min"][i]) > 1e-6
            for i in range(3)
        ):
            findings.append(
                {
                    "code": "aabb_overlap",
                    "part_ids": [one["id"], two["id"]],
                    "message": "Part bounding boxes overlap; inspect whether the intersection is intended.",
                }
            )
    for clip in spec["clips"]:
        joint = parts[indices[clip["part_id"]]]["joint"]
        if (
            min(clip["from_degrees"], clip["to_degrees"]) < joint["min_degrees"]
            or max(clip["from_degrees"], clip["to_degrees"]) > joint["max_degrees"]
        ):
            findings.append(
                {
                    "code": "clip_outside_joint",
                    "part_ids": [clip["part_id"]],
                    "message": "Clip "
                    + clip["name"]
                    + " exceeds the declared joint range.",
                }
            )
    total = {
        "min": [min(row["min"][i] for row in bounds.values()) for i in range(3)],
        "max": [max(row["max"][i] for row in bounds.values()) for i in range(3)],
    }
    if abs(total["min"][1]) > 1e-6:
        findings.append(
            {
                "code": "off_ground",
                "part_ids": [part["id"] for part in parts if not part["parent_id"]],
                "message": "Lowest model point is not on the ground plane.",
            }
        )
    return bytes(raw), {
