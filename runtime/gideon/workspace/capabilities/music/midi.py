...
def transcribe_pcm(raw):
    try:
        with wave.open(io.BytesIO(raw), 'rb') as audio:
            ...
            if audio.getnchannels() != 1 or audio.getsampwidth() != 2 or not 8000 <= rate <= 48000 or not 0 < frames <= rate * 60:
                raise ValueError('Requires mono 16-bit PCM WAV, 8–48 kHz, at most 60 seconds')
    ...
    window, hop = int(rate * .08), int(rate * .01)
    size = 1 << (2 * window - 1).bit_length()
    notes, active = [], None
    for center in range(0, frames, hop):
        frame = samples[max(0, center - window // 2):min(frames, center + window // 2)]
        rms = float(np.sqrt(np.mean(frame * frame)))
        pitch = None
        if rms >= .01 and len(frame) >= window // 2:
            frame = frame - np.mean(frame)
            spectrum = np.fft.rfft(frame, n=size)
            correlation = np.fft.irfft(spectrum * spectrum.conjugate(), n=size)[:len(frame)]
            correlation /= np.arange(len(frame), 0, -1)
            if correlation[0] > 0:
                correlation /= correlation[0]
                lo, hi = max(2, int(rate / 2200)), min(len(frame) - 2, int(rate / 60))
                peaks = [lag for lag in range(lo, hi) if correlation[lag] > correlation[lag-1] and correlation[lag] >= correlation[lag+1]]
                if peaks:
                    best = max(correlation[lag] for lag in peaks)
                    peaks = [lag for lag in peaks if correlation[lag] >= max(.8, best * .9)]
                    if peaks:
                        lag = peaks[0]
                        a, b, c = correlation[lag-1:lag+2]
                        refined = lag + (float(.5 * (a-c) / (a-2*b+c)) if a-2*b+c else 0)
        ...
        if active is not None and pitch != active['pitch']:
            ...
            if active['duration_seconds'] >= .05:
                notes.append(active)
            active = None
        if pitch is not None and active is None:
            active = {'id': str(uuid4()), 'pitch': pitch, 'start_seconds': round(timestamp, 6), 'duration_seconds': 0,
...
def midi_bytes(notes, tempo):
    ...
    for note in notes:
        ...
        end = max(start + 1, round((note['start_seconds'] + note['duration_seconds']) * tempo * 8))
        events += [(start, 1, bytes([0x90, note['pitch'], note['velocity']])), (end, 0, bytes([0x80, note['pitch'], 0]))]
...
class MidiStore:
    def __init__(self, root, catalog):
        ...
        with self._db() as db:
            ...
            db.execute('CREATE TABLE IF NOT EXISTS scores(id TEXT PRIMARY KEY,payload TEXT NOT NULL)')
            db.execute('CREATE TABLE IF NOT EXISTS requests(id TEXT PRIMARY KEY,fingerprint TEXT,payload TEXT)')
    ...
    def _get(self, db, item_id):
        row = db.execute('SELECT payload FROM scores WHERE id=?', (text(item_id, 'id', 100, True),)).fetchone()
        if not row:
    ...
    def list(self, offset=0, limit=50):
        ...
        with self._db() as db:
            return [json.loads(row[0]) for row in db.execute('SELECT payload FROM scores ORDER BY rowid LIMIT ? OFFSET ?', (limit, offset))]

    def transcribe(self, data):
        if not isinstance(data, dict) or set(data) != {'request_id', 'track_id', 'render_id', 'title', 'tempo_bpm'}:
            raise DomainError('Transcription requires request_id, track_id, render_id, title and tempo_bpm')
        ...
        with self._db() as db:
            prior = db.execute('SELECT fingerprint,payload FROM requests WHERE id=?', (data['request_id'],)).fetchone()
            if prior:
            ...
            if not raw:
                raise DomainError('Source recording unavailable', 404, 'artifact_not_found')
            if raw[1] != 'audio/wav':
                raise DomainError('Monophonic transcription requires PCM WAV', 422, 'unsupported_audio')
            duration, notes = transcribe_pcm(raw[0])
            item = {'id': str(uuid4()), 'title': data['title'], 'tempo_bpm': data['tempo_bpm'], 'revision': 1,
                    'source_ref': {'track_id': track['id'], 'render_id': render['id'], 'artifact_ref': ref},
                    'duration_seconds': duration, 'method': 'monophonic_autocorrelation_v1', 'notes': notes}
            encoded = json.dumps(item)
            db.execute('INSERT INTO scores VALUES (?,?)', (item['id'], encoded))
            db.execute('INSERT INTO requests VALUES (?,?,?)', (data['request_id'], fingerprint, encoded))
            return item

    def _revision(self, item, revision):
        integer(revision, 'revision', 1, 1000000)
        if revision != item['revision']:
            raise DomainError('Transcription changed; reload before editing', 409, 'revision_conflict')
    ...
    def update(self, item_id, data):
        ...
        with self._db() as db:
            ...
            ids = set()
            for note in notes:
                if not isinstance(note, dict) or set(note) != {'id', 'pitch', 'start_seconds', 'duration_seconds', 'velocity'}:
                    raise DomainError('Invalid note fields')
                note_id = text(note['id'], 'note id', 100, True)
                if note_id in ids:
                    raise DomainError('Duplicate note ID')
                ids.add(note_id)
                integer(note['pitch'], 'pitch', 36, 96); integer(note['velocity'], 'velocity', 1, 127)
                for key in ('start_seconds', 'duration_seconds'):
                    if type(note[key]) not in (float, int) or not math.isfinite(note[key]) or note[key] < (0 if key == 'start_seconds' else .01):
                        raise DomainError('Invalid note timing')
                if note['start_seconds'] + note['duration_seconds'] > item['duration_seconds'] + .000001:
                    raise DomainError('Note extends beyond source recording')
            item.update(title=title, notes=notes, revision=item['revision'] + 1)
            db.execute('UPDATE scores SET payload=? WHERE id=?', (json.dumps(item), item_id))
            return item
    ...
    def export(self, item_id, data):
        ...
        with self._db() as db:
            item = self._get(db, item_id)
            self._revision(item, data['revision'])
            prior = db.execute('SELECT payload FROM exports WHERE id=? AND revision=?', (item_id, item['revision'])).fetchone()
            if prior:
                result = json.loads(prior[0])
                ref = result['artifact_ref']
                if not self.catalog.artifacts.raw_bytes(ref['slug'], version=ref['version']):
                    raise DomainError('Export artifact missing', 404, 'artifact_not_found')
                return result
            raw = midi_bytes(item['notes'], item['tempo_bpm'])
            slug = 'midi-' + item_id + '-' + str(item['revision'])
            artifact = self.catalog.artifacts.get(slug)
            if artifact:
                if self.catalog.artifacts.raw_bytes(slug, version=1) != (raw, 'audio/midi'):
                    raise DomainError('Export artifact collision', 409, 'artifact_conflict')
            else:
                artifact = self.catalog.artifacts.create_binary(name=item['title'], slug=slug, data=raw, mime='audio/midi', kind='audio', source='manual')
            result = {'artifact_ref': {'slug': slug, 'version': 1}, 'revision': item['revision']}
