...
class VideoStore:
    def __init__(self, root, catalog):
        ...
        with self._db() as db:
            ...
            db.execute(
                "CREATE TABLE IF NOT EXISTS projects(id TEXT PRIMARY KEY,payload TEXT)"
            )
            db.execute(
    ...
    def _image(self, ref):
        ...
        raw = (
            self.catalog.artifacts.raw_bytes(ref["slug"], version=ref["version"])
            if artifact
            else None
        )
        if not artifact or artifact.kind != "image" or not raw:
            raise DomainError(
                "Canonical scene image missing", 404, "artifact_not_found"
            )
        try:
    ...
    def _validate(self, data):
        ...
        if not isinstance(data, dict) or set(data) != fields:
            raise DomainError(
                "Project requires title,track_id,render_id,tempo_bpm,offset_seconds,scenes"
            )
        ...
        track = self.catalog.get(
            "tracks", text(data["track_id"], "track_id", 100, True)
        )
        render = next(
            (row for row in track["renders"] if row["id"] == data["render_id"]), None
        )
        if not render:
            raise DomainError("Audio render not attached to track", 404, "not_found")
        raw = self.catalog.artifacts.raw_bytes(
            render["artifact_ref"]["slug"], version=render["artifact_ref"]["version"]
        )
        if not raw or raw[1] not in ("audio/wav", "audio/mpeg"):
            raise DomainError(
                "Source audio missing or unsupported", 422, "unsupported_audio"
            )
        scenes = data["scenes"]
        if not isinstance(scenes, list) or not 1 <= len(scenes) <= 16:
            raise DomainError("One to sixteen scenes required")
        ids, elapsed, cleaned = set(), 0.0, []
        for scene in scenes:
            ...
            ids.add(scene_id)
            integer(scene["beats"], "beats", 1, 64)
            self._image(scene["image_ref"])
            duration = scene["beats"] * 60 / data["tempo_bpm"]
            cleaned.append(
                {**scene, "start_seconds": elapsed, "duration_seconds": duration}
            )
            elapsed += duration
        if elapsed > 60 or offset + elapsed > render["duration_seconds"] + 0.000001:
    ...
    def create(self, data):
        ...
        with self._db() as db:
            db.execute(
                "INSERT INTO projects VALUES (?,?)", (item["id"], json.dumps(item))
            )
        return item
        ...
        with self._db() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT payload FROM projects ORDER BY rowid LIMIT ? OFFSET ?",
                    (limit, offset),
                )
            ]
    ...
    def update(self, item_id, data):
        ...
        with self._db() as db:
            ...
            db.execute(
                "UPDATE projects SET payload=? WHERE id=?", (json.dumps(item), item_id)
            )
            return item
    ...
    def _revision(self, item, revision):
        ...
        if revision != item["revision"]:
            raise DomainError(
                "Video arrangement changed; reload first", 409, "revision_conflict"
            )
    ...
    async def submit(self, data):
        ...
        with self._db() as db:
            prior = db.execute(
                "SELECT fingerprint,payload FROM jobs WHERE id=?", (data["request_id"],)
            ).fetchone()
            if prior:
            ...
            project = self._read(db, "projects", data["project_id"])
            self._revision(project, data["revision"])
            if not shutil.which("ffmpeg"):
                raise DomainError(
                    "Local video renderer unavailable", 503, "renderer_unavailable"
                )
            if (
                db.execute(
                    "SELECT count(*) FROM jobs WHERE json_extract(payload,'$.status') IN ('queued','running')"
                ).fetchone()[0]
                >= 2
            ):
                raise DomainError(
                    "Two renders are already active", 429, "render_capacity"
                )
            job = {
                "id": data["request_id"],
                "status": "queued",
    ...
    async def _run(self, job):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                dir=self.root, prefix="render-"
            ) as directory:
                root = Path(directory)
                entries = []
                for index, scene in enumerate(item["scenes"]):
                    with Image.open(
                        io.BytesIO(self._image(scene["image_ref"]))
                    ) as image:
                        ImageOps.pad(image.convert("RGB"), (640, 360)).save(
                            root / f"{index}.png"
                        )
                    entries.extend(
                        [
                            f"file '{index}.png'",
                            f"duration {scene['duration_seconds']:.9f}",
                        ]
                    )
                entries.append(f"file '{len(item['scenes'])-1}.png'")
                (root / "scenes.txt").write_text("\n".join(entries) + "\n")
                ref = item["audio_ref"]
                audio = self.catalog.artifacts.raw_bytes(
                    ref["slug"], version=ref["version"]
                )
                if not audio:
                    raise DomainError(
                        "Source audio unavailable", 404, "artifact_not_found"
                    )
                (root / "audio").write_bytes(audio[0])
                ...
                if process.returncode or not output.exists():
                    raise DomainError(
                        "Video encoder rejected the source media", 422, "render_failed"
                    )
            ...
            job["error"] = "Local render cancelled"
            self._save(job)
        except Exception as exc:
            job["status"] = "failed"
            job["error"] = (
                ...
                else "Video renderer or storage unavailable"
            )
            self._save(job)
        finally:
    ...
    async def cancel(self, request_id):
        ...
        if task and not task.done():
            ...
            if job["status"] in ("queued", "running"):
                job.update(status="cancelled", error="Cancelled before render start")
                self._save(job)
        elif job["status"] in ("queued", "running"):
            raise DomainError(
                "Render is owned by another active service",
                409,
                "render_owner_unavailable",
            )
        ...
        with self._db() as db:
            for request_id, payload in db.execute(
                ...
                if job["status"] in ("queued", "running"):
                    job.update(
                        status="interrupted",
                        error="Runtime restarted; submit a new render request to retry",
                    )
                    db.execute(
