...
class AssemblyTools(ToolProvider):
    def __init__(self, store=None):
        home = config_dir()
        self.store = store or AssemblyStore(
    ...
    async def list_tools(self):
        ...
        for action in (
            ...
            if action in ("create", "update", "refine", "export"):
                properties["data"] = {
                    "type": "object",
                    "description": "Assembly: title,parts,clips. Update adds revision; refine: revision,operations[ground|clamp_clips]; export: revision.",
                }
            read_only = action in ("list", "get", "history")
            tools.append(
                ToolDefinition(
                    name="music_assemblies_" + action,
                    provider=self.name,
                    description=action.capitalize()
                    + " authored procedural assemblies and canonical exports.",
                    parameters={
                        "type": "object",
    ...
    async def invoke(self, tool_name, arguments):
        ...
        try:
            result = getattr(self.store, tool_name.removeprefix("music_assemblies_"))(
                *args
            )
