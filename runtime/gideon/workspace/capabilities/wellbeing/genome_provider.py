...
async def invoke_genome(home, arguments):
    ...
    methods = {'preview': store.preview, 'commit': store.commit, 'sources': store.list_sources, 'source': store.get_source, 'original': store.original, 'variants': store.list_variants, 'variant': store.get_variant, 'annotate': store.annotate, 'history': store.history}
