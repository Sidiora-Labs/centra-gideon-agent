...
def parse_genome(payload):
    ...
    for line_number, line in enumerate(
        payload["content"].removeprefix("\ufeff").splitlines(), 1
    ):
            ...
            if (
                ...
                if sample not in cells[9:]:
                    raise MeasurementError(
                        "Select an explicit VCF sample from the header"
                    )
                header = cells
                continue
            if line.startswith("#"):
        ...
        try:
            ...
            if format_ == "tsv":
                ...
                genotype = "".join(
                    token if token in ("/", "|", ".") else alleles[int(token)]
    ...
    source_id = digest(
        [payload["source"], assembly, sample, format_, payload["content"]]
    )
...
class GenomeStore(LabStore):
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.executescript(
                """CREATE TABLE IF NOT EXISTS genome_sources(id TEXT PRIMARY KEY,data TEXT NOT NULL);
    ...
    def preview(self, payload):
        ...
        with self.connection() as db:
            exists = db.execute(
                "SELECT 1 FROM genome_sources WHERE id=?", (source_id,)
            ).fetchone()
        return dict(
            ...
            sample=sample,
            row_count=len(rows),
            duplicates=len(rows) if exists else 0,
            variants=rows[:100],
        )
    ...
    def commit(self, payload):
        ...
        with self.connection() as db:
            ...
            prior = db.execute(
                "SELECT data FROM genome_sources WHERE id=?", (source_id,)
            ).fetchone()
            if prior:
                receipt = dict(
                    source=json.loads(prior[0]), added=0, duplicates=len(rows)
                )
            else:
                ...
                db.execute(
                    "INSERT INTO genome_sources VALUES(?,?)",
        ...
        with self.connection() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT data FROM genome_sources ORDER BY id LIMIT 1000"
                )
            ]
    ...
    def get_source(self, identity):
        with self.connection() as db:
            row = db.execute(
                "SELECT data FROM genome_sources WHERE id=?", (identity,)
            ).fetchone()
            if not row:
    ...
    def list_variants(
        self, source_id, *, chromosome=None, rsid=None, limit=100, offset=0
    ):
        ...
        with self.connection() as db:
            return [
                json.loads(row[0])
                for row in db.execute(
                    """SELECT r.data FROM genome_variants r WHERE source_id=? AND revision=(SELECT MAX(s.revision) FROM genome_variants s WHERE s.id=r.id)
                AND (? IS NULL OR chromosome=?) AND (? IS NULL OR rsid=?) ORDER BY chromosome,position,id LIMIT ? OFFSET ?""",
                    (source_id, chromosome, chromosome, rsid, rsid, limit, offset),
                )
            ]

    def _variant(self, db, identity):
        row = db.execute(
            "SELECT data FROM genome_variants WHERE id=? ORDER BY revision DESC LIMIT 1",
            (identity,),
        ).fetchone()
        if not row:
    ...
    def annotate(self, identity, payload):
        if not isinstance(payload, dict) or set(payload) != {
            ...
            raise MeasurementError(
                "Annotation requires request_id, revision, annotation and annotation_source"
            )
        ...
        with self.connection() as db:
            ...
            db.execute(
                "INSERT INTO requests VALUES(?,?,?)",
                (request_id, fingerprint, json.dumps(row)),
            )
            return row
    ...
    def history(self, identity):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT data FROM genome_variants WHERE id=? ORDER BY revision",
                    (identity,),
                )
            ]
