...
async def handle(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if request.path.endswith("/export"):
            return web.json_response(store.export(), headers={"Cache-Control": "no-store"})
        ...
        return web.json_response({"prescriptions": store.list(
        ...
        return web.json_response({"error": "Invalid eye prescription request", "code": "invalid_request"}, status=400)
