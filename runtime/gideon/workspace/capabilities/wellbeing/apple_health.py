...
class ExportParser:
    ...
    def metric(self, name, observed, unit, value, device="", end=None, stage=None):
        row = dict(metric=text(name, "metric", 200), observed_at=timestamp(observed), unit=text(unit, "unit", 64), value=quantity(value), device_source=text(device or "", "device_source", 256, True), end_at=timestamp(end) if end else None, stage=stage)
    ...
    def bound(self):
        if len(self.metrics) + len(self.labs) + sum(self.skipped.values()) > MAX_RECORDS:
    ...
    def xml(self, raw):
        ...
        if "\x00" in decoded or "<!ENTITY" in decoded.upper() or re.search(r"<!DOCTYPE\s+\w+\s+(SYSTEM|PUBLIC)\b", decoded, re.I):
            raise MeasurementError("External XML schemas and entity declarations are unsupported")
        try:
            for _, element in ElementTree.iterparse(io.BytesIO(raw), events=("end",)):
                if element.tag == "Record":
                    ...
                    if kind == "HKCategoryTypeIdentifierSleepAnalysis":
                        start, end = timestamp(data.get("startDate")), timestamp(data.get("endDate"))
                        hours = (datetime.fromisoformat(end) - datetime.fromisoformat(start)).total_seconds() / 3600
                        if hours < 0:
                            raise MeasurementError("Sleep interval ends before it starts")
                        self.metric(kind, start, "h", hours, data.get("sourceName", ""), end, data.get("value"))
                    elif kind.startswith("HKQuantityTypeIdentifier"):
                        self.metric(kind, data.get("startDate"), data.get("unit"), data.get("value"), data.get("sourceName", ""), data.get("endDate"))
                    else:
    ...
    def fhir(self, resource):
        ...
        if not isinstance(categories, list) or any(not isinstance(category, dict) or not isinstance(category.get("coding", []), list) for category in categories):
            raise MeasurementError("FHIR categories must contain coding arrays")
        is_lab = any(isinstance(category, dict) and any(isinstance(code, dict) and code.get("code") == "laboratory" for code in category.get("coding", [])) for category in categories) if isinstance(categories, list) else False
        if resource.get("resourceType") != "Observation" or not is_lab or resource.get("status") in ("cancelled", "entered-in-error"):
            ...
            if not isinstance(coding, dict) or not isinstance(coding.get("coding", []), list):
                raise MeasurementError("FHIR code must contain a coding array")
            code = next((item for item in coding.get("coding", []) if isinstance(item, dict) and isinstance(item.get("system"), str) and "loinc" in item["system"]), {})
            ranges = resource.get("referenceRange", [])
            if not isinstance(ranges, list) or (ranges and not isinstance(ranges[0], dict)):
                raise MeasurementError("FHIR referenceRange must be an array of objects")
            bounds = ranges[0] if ranges else {}
            if any(bounds.get(key) is not None and not isinstance(bounds[key], dict) for key in ("low", "high")):
                raise MeasurementError("FHIR reference bounds must be quantities")
            for key in ("low", "high"):
                bound = bounds.get(key) or {}
                if bound.get("comparator") or any(bound.get(field) and bound[field] != (value.get(field) or value.get("unit") or value.get("code")) for field in ("unit", "code")):
                    raise MeasurementError("FHIR reference bounds require matching units and exact values")
            row = dict(analyte=coding.get("text") or code.get("display") or code.get("code"), observed_at=timestamp(resource.get("effectiveDateTime") or resource.get("issued")), value=value.get("value"), unit=value.get("unit") or value.get("code"), reference_low=(bounds.get("low") or {}).get("value"), reference_high=(bounds.get("high") or {}).get("value"), external_id=str(resource.get("id") or ""), notes="")
    ...
    def json(self, raw, fhir=False):
        ...
        for metric in data["metrics"]:
            ...
            for point in metric["data"]:
                ...
                components = [key for key in ("qty", "Min", "Max", "Avg") if key in point]
                if not components:
                    self.skipped["unsupported_json_point"] += 1
                for key in components:
                    name = metric.get("name") if key == "qty" else f"{metric.get('name')}:{key}"
                    self.metric(name, point.get("date"), point.get("unit") or metric.get("units"), point[key], point.get("source") or point.get("src") or "", point.get("end"))
                self.bound()


def parse_export(payload):
    if not isinstance(payload, dict) or set(payload) - {"filename", "format", "content_base64", "source", "preview_id", "request_id"}:
    ...
    encoded = text(payload.get("content_base64"), "content_base64", (MAX_BYTES + 2) // 3 * 4)
    try:
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(raw)) as archive:
                ...
                if len(entries) > 2000 or sum(entry.file_size for entry in entries) > MAX_EXPANDED:
                    raise MeasurementError("ZIP exceeds 2000 entries or 32 MiB expanded")
                found = False
                for entry in entries:
                    ...
                    if entry.filename in ("export.xml", "apple_health_export/export.xml"):
                        parser.xml(archive.read(entry)); found = True
                    elif "clinical_records" in path.parts and path.suffix == ".json":
                        parser.json(archive.read(entry), fhir=True); found = True
                if not found:
...
class AppleHealthStore(LabStore):
    ...
    def preview(self, payload):
        ...
        identity = digest([payload["filename"], payload["format"], hashlib.sha256(raw).hexdigest(), payload["source"]])
        return {"preview_id": identity, "metric_count": len(parser.metrics), "lab_count": len(parser.labs), "metrics": parser.metrics[:100], "labs": parser.labs[:100], "skipped": dict(parser.skipped)}
    ...
    def commit(self, payload):
        ...
        with self.connection() as db:
            ...
            existing = db.execute("SELECT receipt FROM apple_imports WHERE id=?", (identity,)).fetchone()
            if existing:
                db.execute("INSERT INTO requests VALUES(?,?,?)", (request_id, fingerprint, existing[0]))
                return json.loads(existing[0])
            sha = hashlib.sha256(raw).hexdigest()
            slug = "apple-source-" + identity
            artifact = self.artifacts.get(slug) or self.artifacts.create(name=payload["filename"], kind="json", content=json.dumps({"filename": payload["filename"], "sha256": sha}), source="import", slug=slug, readonly=True)
            reference = {"slug": artifact.slug, "version": 1, "sha256": sha, "filename": f"original@{sha}.{payload['format']}"}
            if not self.artifacts.store_version_file(artifact.slug, reference["filename"], raw):
            ...
            for row in parser.metrics:
                ...
                cursor = db.execute("INSERT OR IGNORE INTO apple_metrics VALUES(?,?,?,?,?)", (key, row["metric"], row["unit"], instant(row["observed_at"]), json.dumps(record)))
                added += cursor.rowcount
                duplicates += 1 - cursor.rowcount
            for index, row in enumerate(parser.labs, 1):
                key, row_hash = self._identity(payload["source"], row)
                prior = db.execute("SELECT fingerprint FROM lab_sources WHERE id=?", (key,)).fetchone()
                if prior:
                    if prior[0] != row_hash:
                        raise MeasurementError("Laboratory source identity conflict", 409, "conflict")
                    duplicates += 1
                    continue
                record = dict(row, id=key, kind="laboratory", source=payload["source"], artifact=reference, row_index=index, created_at=datetime.now(timezone.utc).isoformat(), revision=1)
                self._append(db, record)
                db.execute("INSERT INTO lab_sources VALUES(?,?)", (key, row_hash))
                labs_added += 1
            receipt = dict(import_id=identity, artifact=reference, metrics_added=added, labs_added=labs_added, duplicates=duplicates, skipped=dict(parser.skipped))
    ...
    def list_metrics(self, *, metric=None, unit=None, from_date=None, to_date=None, limit=100, offset=0):
        ...
        with self.connection() as db:
            rows = db.execute("""SELECT data FROM apple_metrics WHERE (? IS NULL OR metric=?) AND (? IS NULL OR unit=?)
                AND (? IS NULL OR observed_utc>=?) AND (? IS NULL OR observed_utc<=?) ORDER BY observed_utc DESC,id LIMIT ? OFFSET ?""", (metric, metric, unit, unit, start, start, end, end, limit, offset))
            return [json.loads(row[0]) for row in rows]

    def original_metric(self, identity):
        with self.connection() as db:
            row = db.execute("SELECT data FROM apple_metrics WHERE id=?", (identity,)).fetchone()
        if row is None:
