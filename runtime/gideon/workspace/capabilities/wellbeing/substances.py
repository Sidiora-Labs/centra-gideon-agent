...
def quantities(kind, details, count=1):
    ...
    if kind == "alcohol":
        ...
        return (
            {"volume_ml": volume, "abv_percent": abv},
            volume * abv / 100 * 0.789 * count,
            None,
        )
...
class ConsumptionStore(MeasurementStore):
    def __init__(self, home):
        ...
        with self.connection() as db:
            for entity in ("entries", "presets"):
                db.execute(
                    f"CREATE TABLE IF NOT EXISTS substance_{entity} (id TEXT NOT NULL,revision INTEGER NOT NULL,kind TEXT NOT NULL,observed_utc TEXT NOT NULL,data TEXT NOT NULL,PRIMARY KEY(id,revision))"
                )

    def _record(self, db, entity, identity):
        row = db.execute(
            f"SELECT data FROM substance_{entity} WHERE id=? ORDER BY revision DESC LIMIT 1",
            (identity,),
        ).fetchone()
        if row is None:
    ...
    def _write(self, entity, payload, identity=None, delete=False):
        ...
        mutable = {"name", "details"} | (
            {"observed_at", "count", "notes"} if entity == "entries" else set()
        )
        allowed = (
            {"request_id", "revision"}
            if delete
            ...
            | (
                ...
                | ({"source", "preset_id"} if entity == "entries" else set())
            )
        )
        ...
        try:
            fingerprint = json.dumps(
                ["substance", entity, identity, delete, payload],
                sort_keys=True,
                allow_nan=False,
            )
        ...
        with self.connection() as db:
            ...
            if identity:
                ...
                if (
                    ...
                    or record["revision"] != payload["revision"]
                ):
                    raise MeasurementError(
                        "Record changed or deleted; reload", 409, "conflict"
                    )
                ...
                record.update(
                    id=str(uuid4()),
                    revision=1,
                    created_at=datetime.now(utc.utc).isoformat(),
                    deleted=False,
                )
                if entity == "entries":
                    record.update(
                    ...
                    if payload.get("preset_id") is not None:
                        if set(payload) & {"name", "kind", "details"}:
                            raise MeasurementError(
                                "Preset entries must not override the preset snapshot"
                            )
                        preset = self._record(db, "presets", payload["preset_id"])
                        if preset["deleted"]:
                            raise MeasurementError("Preset is deleted", 409, "conflict")
                        record.update(
                            {key: preset[key] for key in ("name", "kind", "details")}
                        )
                        record.update(
                            preset_id=preset["id"], preset_revision=preset["revision"]
                        )
            text(record["name"], "name", 200)
            count = amount(record.get("count", 1), "count", high=1000, positive=True)
            details, ethanol, nicotine = quantities(
                record["kind"], record["details"], count
            )
            ...
            db.execute(
                f"INSERT INTO substance_{entity} VALUES(?,?,?,?,?)",
                (record["id"], record["revision"], record["kind"], observed, encoded),
            )
            db.execute(
        ...
        with self.connection() as db:
            ...
            return [
                ...
                for row in db.execute(
                    "SELECT data FROM substance_entries WHERE id=? ORDER BY revision",
                    (identity,),
                )
            ]
    ...
    def _list(self, entity, kind=None, start=None, end=None, limit=100, offset=0):
        ...
        with self.connection() as db:
            rows = db.execute(
                f"""SELECT r.data FROM substance_{entity} r WHERE
                ...
                (kind, kind, start, start, end, end, limit, offset),
            )
            return [json.loads(row[0]) for row in rows]

    def list_entries(
        self, *, kind=None, from_date=None, to_date=None, limit=100, offset=0
    ):
        if (
    ...
    def summary(self, timezone="UTC", days=30, as_of=None):
        ...
        for row in rows:
            day = str(
                datetime.fromisoformat(instant(row["observed_at"]))
                .astimezone(zone)
                .date()
            )
            bucket = buckets[day]
