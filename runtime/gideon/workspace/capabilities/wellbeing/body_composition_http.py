...
async def handle(request):
    ...
    try:
        ...
        if request.method in {"POST", "PUT"}:
            ...
            result = store.create(payload) if request.method == "POST" else store.correct(identity, payload)
        ...
        return web.json_response({"records": store.list(
            from_date=request.query.get("from"), to_date=request.query.get("to"),
        ...
        return web.json_response({"error": "Invalid body-composition request", "code": "invalid_request"}, status=400)


def register(app, home=None):
    app[STORE] = BodyCompositionStore(home or app.get("capability_home") or config_dir())
