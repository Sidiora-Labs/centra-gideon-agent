...
def challenge(kind, index, issued):
    ...
    return dict(index=index, stimulus=stimulus, presented_at=issued.isoformat(), expected=expected)


def public(row, at=None):
    result = json.loads(json.dumps(row))
    at = at or now()
    if result['status'] == 'active' and at >= datetime.fromisoformat(result['deadline']):
    ...
    result['score'] = dict(answered=len(trials), correct=correct, accuracy=correct / len(trials) if trials else None, mean_elapsed_ms=elapsed / len(trials) if trials else None, total_elapsed_ms=elapsed)
    return result
...
class CognitiveStore(MeasurementStore):
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.execute('CREATE TABLE IF NOT EXISTS cognitive_sessions(id TEXT NOT NULL,revision INTEGER NOT NULL,data TEXT NOT NULL,PRIMARY KEY(id,revision))')

    def _get(self, db, identity):
        row = db.execute('SELECT data FROM cognitive_sessions WHERE id=? ORDER BY revision DESC LIMIT 1', (identity,)).fetchone()
        if row is None:
            raise MeasurementError('Exercise session not found', 404, 'not_found')
        return json.loads(row[0])

    def _write(self, operation, payload, identity=None):
        fields = {'request_id', 'kind', 'planned_trials', 'time_limit_seconds'} if operation == 'start' else {'request_id', 'revision', 'answer'} if operation == 'answer' else {'request_id', 'revision'}
        if not isinstance(payload, dict) or set(payload) != fields:
            raise MeasurementError('Exercise operation requires its exact declared fields')
        ...
        if operation == 'start':
            ...
            for field, maximum in [('planned_trials', 20), ('time_limit_seconds', 600)]:
                if type(payload[field]) is not int or not 1 <= payload[field] <= maximum:
                    raise MeasurementError(f'{field} must be an integer in 1..{maximum}')
        else:
        ...
        fingerprint = json.dumps(['cognitive', operation, identity, payload], sort_keys=True)
        with self.connection() as db:
            ...
            observed = now()
            if operation == 'start':
                row = {key: payload[key] for key in ('kind', 'planned_trials', 'time_limit_seconds')}
                row.update(id=str(uuid4()), revision=1, source='local_exercise', rules_version=1, started_at=observed.isoformat(), deadline=(observed + timedelta(seconds=row['time_limit_seconds'])).isoformat(), status='active', trials=[], current_trial=challenge(row['kind'], 1, observed))
            else:
                row = self._get(db, identity)
                if payload['revision'] != row['revision']:
                    raise MeasurementError('Exercise changed; reload', 409, 'conflict')
                if row['status'] != 'active':
                    raise MeasurementError('Exercise is already terminal', 409, 'conflict')
                row['revision'] += 1
                if observed >= datetime.fromisoformat(row['deadline']):
                    ...
                    elapsed = (observed - datetime.fromisoformat(trial['presented_at'])).total_seconds() * 1000
                    if elapsed < 0:
                        raise MeasurementError('Server clock moved backwards; retry after clock recovery', 409, 'conflict')
                    trial.update(answer=payload['answer'], answered_at=observed.isoformat(), correct=payload['answer'].strip().lower() == trial['expected'], elapsed_ms=elapsed)
                    row['trials'].append(trial)
                    if len(row['trials']) == row['planned_trials']:
                        row['status'], row['current_trial'] = 'completed', None
                    else:
                        row['current_trial'] = challenge(row['kind'], len(row['trials']) + 1, observed)
            db.execute('INSERT INTO cognitive_sessions VALUES(?,?,?)', (row['id'], row['revision'], json.dumps(row)))
    ...
    def list_sessions(self, limit=100):
        ...
        with self.connection() as db:
            rows = db.execute('SELECT r.data FROM cognitive_sessions r WHERE revision=(SELECT MAX(s.revision) FROM cognitive_sessions s WHERE s.id=r.id) ORDER BY json_extract(data,\'$.started_at\') DESC,id LIMIT ?', (limit,))
            at = now()
