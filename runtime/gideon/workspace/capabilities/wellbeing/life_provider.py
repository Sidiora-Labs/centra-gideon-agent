...
class WellbeingReminderAction(ActionProvider):
    ...
    async def execute(self, action_config, ctx, timeout=30):
        try:
            if action_config != {}:
                raise MeasurementError('Reminder action accepts no path or content overrides')
            home = self.home if self.home is not None else config_dir()
            scheduled = ctx.payload.get('scheduled_for')
            as_of = datetime.fromtimestamp(float(scheduled), timezone.utc).isoformat() if scheduled is not None else None
            services = get_action_services()
            inbox = self.inbox if self.inbox is not None else live_store(services.state) if services else None
            result = await asyncio.to_thread(LifeCalendarStore(home).check_reminder, as_of, inbox)
            return ActionResult(success=True, stdout=json.dumps(result), outcome='completed')
...
async def invoke_life(home, arguments):
    ...
    methods = {'configure': store.configure, 'config': store.get_config, 'config_history': store.config_history, 'projection': store.projection, 'event_create': store.create_event, 'event_update': store.update_event, 'events': store.list_events, 'event_history': store.history_event, 'reminder_check': store.check_reminder}
    ...
    if operation in ('configure', 'event_create'):
        ...
        result = await asyncio.to_thread(method, inbox=inbox) if operation == 'reminder_check' else await asyncio.to_thread(method)
