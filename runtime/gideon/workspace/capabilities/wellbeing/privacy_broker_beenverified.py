...
class BeenVerifiedCaseAdapter:
    def __init__(self, store, communications_store, outbound=None):
        ...
        with self.store.connection() as db:
            db.execute('CREATE TABLE IF NOT EXISTS privacy_beenverified_links(case_id TEXT PRIMARY KEY,data TEXT NOT NULL)')
    ...
    def _case(self, identity, revision, scope):
        ...
        if not broker or broker['name'].strip().lower() != 'beenverified':
            raise MeasurementError('This adapter supports BeenVerified cases only', 409, 'unsupported_provider')
    ...
    def _link(self, case_id, required=True):
        with self.store.connection() as db:
            found = db.execute('SELECT data FROM privacy_beenverified_links WHERE case_id=?', (case_id,)).fetchone()
        if found is None:
            if required: raise MeasurementError('Prepare the BeenVerified email before this action', 409, 'conflict')
            return None
        return json.loads(found[0])

    def _save_link(self, row):
        with self.store.connection() as db:
            db.execute('INSERT INTO privacy_beenverified_links VALUES(?,?) ON CONFLICT(case_id) DO UPDATE SET data=excluded.data', (row['case_id'], json.dumps(row)))

    @staticmethod
    def _profile(value):
        value = text(value, 'profile_url', 1000)
        parsed = urlsplit(value)
        if parsed.scheme != 'https' or not parsed.hostname or not (parsed.hostname == 'beenverified.com' or parsed.hostname.endswith('.beenverified.com')) or parsed.username or parsed.password:
    ...
    def _body(full_name, contact_email, profile_url, jurisdiction):
        ...
        return (f'Hello BeenVerified Privacy Team,\n\n'
    ...
    def prepare(self, identity, payload):
        ...
        return {'case': case, 'draft': draft, 'plan': {'recipient': RECIPIENT, 'requires_exact_content_approval': True,
                ...
                'primary_suppression_form_completed': False, 'affiliate_rescans_required': True}}
    ...
    def approve(self, identity, payload):
        ...
        if payload['content_sha256'] != link['content_sha256']:
            raise MeasurementError('Approval digest does not match the prepared BeenVerified email', 409, 'conflict')
    ...
    def send(self, identity, payload):
        ...
        link = self._link(identity)
        if payload['content_sha256'] != link['content_sha256']:
            raise MeasurementError('Dispatch digest does not match the prepared BeenVerified email', 409, 'conflict')
