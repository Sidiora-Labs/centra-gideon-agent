...
def next_schedule(previous, grade, at):
    ...
    interval, repetitions, lapses = previous['interval_days'], previous['repetitions'], previous['lapses']
    if grade == 'again':
        ...
        if grade == 'hard':
            ...
            interval = 1 if repetitions == 0 else 3 if repetitions == 1 else interval * 2
        else:
            interval = max(4, interval * 3)
        interval = min(365, round(interval, 3))
        repetitions += 1
        due = at + timedelta(days=interval)
    return dict(due_at=due.isoformat(), interval_days=interval, repetitions=repetitions, lapses=lapses, last_grade=grade, last_practiced_at=at.isoformat(), rules_version=1)
...
class MemoryPracticeStore(MeasurementStore):
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.execute('CREATE TABLE IF NOT EXISTS memory_card_revisions(id TEXT NOT NULL,revision INTEGER NOT NULL,due_at TEXT NOT NULL,data TEXT NOT NULL,PRIMARY KEY(id,revision))')

    def _get(self, db, identity):
        row = db.execute('SELECT data FROM memory_card_revisions WHERE id=? ORDER BY revision DESC LIMIT 1', (identity,)).fetchone()
        if not row:
    ...
    def _public(self, row):
        ...
        if hashlib.sha256(artifact.content.encode()).hexdigest() != reference['sha256']:
            raise MeasurementError('Memory card artifact hash mismatch', 409, 'conflict')
    ...
    def _artifact(self, fields):
        ...
        if artifact is None:
            artifact = self.artifacts.create(name=fields['front'][:100], content=content, kind='document', source='import', slug=slug, readonly=True)
    ...
    def _write(self, operation, payload, identity=None):
        ...
        fields = {'request_id', 'front', 'back', 'source', 'tags'} if operation == 'create' else {'request_id', 'revision', 'front', 'back', 'tags', 'archived'} if operation == 'update' else {'request_id', 'revision', 'grade'}
        if set(payload) - fields:
            raise MeasurementError('Unknown or immutable card fields')
        request_id = text(payload.get('request_id'), 'request_id', 128)
        try:
            fingerprint = json.dumps(['memory-card', operation, identity, payload], sort_keys=True, allow_nan=False)
        ...
        with self.connection() as db:
            ...
            if operation == 'create':
                ...
                content = content_fields(payload.get('front'), payload.get('back'), payload.get('tags', []))
                row = dict(id=str(uuid4()), revision=1, source=payload['source'], created_at=at.isoformat(), archived=False, artifact=self._artifact(content), schedule=dict(due_at=at.isoformat(), interval_days=0, repetitions=0, lapses=0, last_grade=None, last_practiced_at=None, rules_version=1), practice=None)
            else:
                row = self._get(db, identity)
                if type(payload.get('revision')) is not int or payload['revision'] != row['revision']:
                    raise MeasurementError('Memory card changed; reload', 409, 'conflict')
                row['revision'] += 1
                row['practice'] = None
                if operation == 'update':
                    previous = self._public(row)
                    content = content_fields(payload.get('front', previous['front']), payload.get('back', previous['back']), payload.get('tags', previous['tags']))
                    ...
                    if row['archived']:
                        raise MeasurementError('Memory card is archived', 409, 'conflict')
                    old_due = row['schedule']['due_at']
                    row['schedule'] = next_schedule(row['schedule'], payload.get('grade'), at)
                    row['practice'] = dict(grade=payload['grade'], practiced_at=at.isoformat(), previous_due_at=old_due, next_due_at=row['schedule']['due_at'])
            db.execute('INSERT INTO memory_card_revisions VALUES(?,?,?,?)', (row['id'], row['revision'], instant(row['schedule']['due_at']), json.dumps(row)))
    ...
    def list_cards(self, due_only=False, as_of=None, include_archived=False, limit=100):
        if type(due_only) is not bool or type(include_archived) is not bool or type(limit) is not int or not 1 <= limit <= 500:
            raise MeasurementError('Boolean filters and limit1..500 are required')
        at = instant(as_of) if as_of else datetime.now(timezone.utc).isoformat(timespec='microseconds')
        with self.connection() as db:
            rows = db.execute('''SELECT r.data FROM memory_card_revisions r WHERE revision=(SELECT MAX(s.revision) FROM memory_card_revisions s WHERE s.id=r.id)
                AND (? OR json_extract(data,'$.archived')=0) AND (?=0 OR due_at<=?) ORDER BY due_at,id LIMIT ?''', (include_archived, due_only, at, limit))
            return [self._public(json.loads(row[0])) for row in rows]

    def history(self, identity):
        with self.connection() as db:
            self._get(db, identity)
            return [self._public(json.loads(row[0])) for row in db.execute('SELECT data FROM memory_card_revisions WHERE id=? ORDER BY revision', (identity,))]
