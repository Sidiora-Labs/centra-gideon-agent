...
class WhitepagesCaseAdapter:
    def __init__(self, store, communications_store, outbound=None):
        ...
        with self.store.connection() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS privacy_whitepages_links(case_id TEXT PRIMARY KEY,data TEXT NOT NULL)"
            )
    ...
    def _case(self, identity, revision, scope):
        ...
        if not broker or broker["name"].strip().lower() != "whitepages":
            raise MeasurementError(
                "This adapter supports Whitepages cases only",
                409,
    ...
    def _link(self, case_id, required=True):
        with self.store.connection() as db:
            found = db.execute(
                "SELECT data FROM privacy_whitepages_links WHERE case_id=?", (case_id,)
            ).fetchone()
        if found is None:
            if required:
                raise MeasurementError(
                    "Prepare the Whitepages email before this action", 409, "conflict"
                )
            return None
    ...
    def _save_link(self, row):
        with self.store.connection() as db:
            db.execute(
                "INSERT INTO privacy_whitepages_links VALUES(?,?) ON CONFLICT(case_id) DO UPDATE SET data=excluded.data",
    ...
    def _profile(value):
        ...
        if (
            parsed.scheme != "https"
            or parsed.hostname not in ("www.whitepages.com", "whitepages.com")
            or parsed.username
            or parsed.password
        ):
    ...
    def _body(full_name, contact_email, profile_url, jurisdiction):
        ...
        return (
            f"Hello Whitepages Privacy Team,\n\n"
    ...
    def approve(self, identity, payload):
        ...
        if payload["content_sha256"] != link["content_sha256"]:
            raise MeasurementError(
                "Approval digest does not match the prepared Whitepages email",
                409,
                "conflict",
            )
    ...
    def send(self, identity, payload):
        ...
        if payload["content_sha256"] != link["content_sha256"]:
            raise MeasurementError(
                "Dispatch digest does not match the prepared Whitepages email",
                409,
                "conflict",
            )
