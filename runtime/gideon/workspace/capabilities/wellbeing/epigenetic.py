...
def reported(value, field):
    if value is None:
        return None
    if not isinstance(value, dict) or set(value) - {'value', 'unit', 'scale'} or 'value' not in value:
        raise MeasurementError(f'{field} must contain value and one authored unit or scale')
    number = value['value']
    if type(number) not in (int, float) or not math.isfinite(number) or abs(number) > 1e9:
...
def validate_record(payload, *, correction=False):
    mutable = {'observed_at', 'source_report_id', 'biological_age', 'chronological_age', 'pace_of_aging', 'organ_scores', 'notes'}
    required = {'request_id', 'revision'} if correction else {'request_id', 'source_report_id', 'observed_at', 'source'}
    ...
    if 'source_report_id' in payload:
        result['source_report_id'] = text(payload['source_report_id'], 'source_report_id', 256)
    ...
    if 'organ_scores' in payload:
        ...
        if not isinstance(scores, dict) or len(scores) > 100:
            raise MeasurementError('organ_scores must be an object with at most 100 named scores')
        result['organ_scores'] = {text(name, 'organ score name', 120): reported(value, f'organ_scores.{name}') for name, value in scores.items()}
    return result
...
class EpigeneticStore:
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.executescript('''CREATE TABLE IF NOT EXISTS requests(id TEXT PRIMARY KEY,payload TEXT NOT NULL,result TEXT NOT NULL);
    ...
    def _current(self, db, identity):
        row = db.execute('SELECT data FROM epigenetic_results WHERE id=? ORDER BY revision DESC LIMIT 1', (identity,)).fetchone()
        if row is None:
    ...
    def _write(self, kind, identity, payload, execute):
        ...
        try:
            fingerprint = json.dumps([kind, identity, payload], sort_keys=True, allow_nan=False)
        except (TypeError, ValueError) as exc:
            raise MeasurementError('Epigenetic payload must contain finite JSON values') from exc
        with self.connection() as db:
    ...
    def _append(self, db, record):
        db.execute('INSERT INTO epigenetic_results VALUES(?,?,?,?)', (
            record['id'], record['revision'], record['observed_at'] + 'T00:00:00+00:00', json.dumps(record, allow_nan=False)))
        return record
    ...
    def create(self, payload):
        ...
        def execute(db):
            ...
            record = dict(fields, id=str(uuid4()), revision=1, evidence_basis='source_reported', created_at=stamp, updated_at=stamp)
    ...
    def correct(self, identity, payload):
        ...
        def execute(db):
            ...
            if payload['revision'] != record['revision']:
                raise MeasurementError('Epigenetic record changed; reload', 409, 'conflict')
            record.update(fields, revision=record['revision'] + 1, updated_at=datetime.now(timezone.utc).isoformat())
            return self._append(db, record)
    ...
    def history(self, identity):
        with self.connection() as db:
            ...
            return [json.loads(row[0]) for row in db.execute('SELECT data FROM epigenetic_results WHERE id=? ORDER BY revision', (identity,))]
    ...
    def list(self, limit=100, offset=0):
        ...
        with self.connection() as db:
            rows = db.execute('''SELECT r.data FROM epigenetic_results r WHERE r.revision=(SELECT MAX(s.revision) FROM epigenetic_results s WHERE s.id=r.id)
                ORDER BY observed_utc DESC,id LIMIT ? OFFSET ?''', (limit, offset))
            return [json.loads(row[0]) for row in rows]

    def export(self):
        with self.connection() as db:
            history = [json.loads(row[0]) for row in db.execute('SELECT data FROM epigenetic_results ORDER BY id,revision')]
        current = {}
