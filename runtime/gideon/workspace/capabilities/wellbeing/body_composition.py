...
def number(value, field, low, high):
    if (
        ...
        raise MeasurementError(
            f"{field} must be a finite number between {low} and {high}"
        )
    return float(value)
...
def normalize(payload):
    if not isinstance(payload, dict) or set(payload) != {
        ...
        raise MeasurementError(
            "Values require muscle_percent, fat_percent, bone_mass and temperature; weight is recorded separately"
        )
    muscle = number(payload["muscle_percent"], "muscle_percent", 0, 100)
    fat = number(payload["fat_percent"], "fat_percent", 0, 100)
    if muscle + fat > 100:
        raise MeasurementError(
            "Reported muscle and fat percentages cannot exceed 100 in total"
        )
    bone, temperature = payload["bone_mass"], payload["temperature"]
    if (
        not isinstance(bone, dict)
        or set(bone) != {"value", "unit"}
        or bone.get("unit") not in MASS_FACTORS
    ):
    ...
    if (
        ...
        or temperature.get("unit") not in TEMPERATURE_UNITS
    ):
        raise MeasurementError("temperature requires value and unit C, F or K")
    bone_value = number(
        bone["value"], "bone_mass.value", 0, 1000 / MASS_FACTORS[bone["unit"]]
    )
...
class BodyCompositionStore(MeasurementStore):
    ...
    def _get(self, database, identity):
        row = database.execute(
            "SELECT data FROM body_composition_revisions WHERE id=? ORDER BY revision DESC LIMIT 1",
            (identity,),
        ).fetchone()
        if row is None:
            raise MeasurementError(
                "Body-composition observation not found", 404, "not_found"
            )
    ...
    def _write(self, identity, payload):
        ...
        if not isinstance(payload, dict) or set(payload) - (
            correction_fields if identity else create_fields
        ):
        ...
        try:
            ...
            raise MeasurementError(
                "Body-composition payload must contain finite JSON values"
            ) from error
        with self.connection() as database:
            database.execute("BEGIN IMMEDIATE")
            prior = database.execute(
                "SELECT payload,result FROM body_composition_requests WHERE id=?",
                (request_id,),
            ).fetchone()
            if prior:
                if prior[0] != fingerprint:
                    raise MeasurementError(
                        "Request ID already used for another body-composition mutation",
                        409,
                        "conflict",
                    )
            ...
            if identity:
                ...
                if (
                    ...
                    raise MeasurementError(
                        "Body-composition observation changed; reload", 409, "conflict"
                    )
                record.update(
                    {
                        key: value
            ...
            record["original_values"], record["normalized_values"] = normalize(
                record.pop("values", record.get("original_values"))
            )
            encoded = json.dumps(record, sort_keys=True, allow_nan=False)
            database.execute(
                "INSERT INTO body_composition_revisions VALUES(?,?,?,?)",
                (record["id"], record["revision"], observed, encoded),
            )
            database.execute(
                "INSERT INTO body_composition_requests VALUES(?,?,?)",
    ...
    def list(self, *, from_date=None, to_date=None, limit=100, offset=0):
        ...
        with self.connection() as database:
            rows = database.execute(
                """SELECT r.data FROM body_composition_revisions r WHERE
        ...
        with self.connection() as database:
            ...
            return [
                ...
                for row in database.execute(
                    "SELECT data FROM body_composition_revisions WHERE id=? ORDER BY revision",
                    (identity,),
                )
            ]
    ...
    def export(self):
        with self.connection() as database:
            history = [
                json.loads(row[0])
                for row in database.execute(
                    "SELECT data FROM body_composition_revisions ORDER BY id,revision"
                )
            ]
        current = {record["id"]: record for record in history}
        return {
            "schema_version": 1,
            "body_composition": list(current.values()),
            "history": history,
        }
    ...
    def import_current(self, record):
        ...
        try:
            ...
            raise MeasurementError(
                "Invalid canonical body-composition identity or timestamp"
            ) from exc
        if (
            ...
            raise MeasurementError(
                "Invalid canonical body-composition revision or timestamp"
            )
        observed = instant(record["observed_at"])
        text(record["source"], "source", 256)
        text(record["notes"], "notes", 4000, empty=True)
        ...
        if (
            ...
            or normalized != record["normalized_values"]
        ):
            raise MeasurementError("Canonical body-composition values are not exact")
        with self.connection() as database:
            database.execute("BEGIN IMMEDIATE")
            row = database.execute(
                "SELECT revision,data FROM body_composition_revisions WHERE id=? ORDER BY revision DESC LIMIT 1",
                (record["id"],),
            ).fetchone()
            ...
            if row and row[0] >= record["revision"]:
                raise MeasurementError(
                    "Canonical body-composition import conflicts with local history",
                    409,
                    "conflict",
                )
            database.execute(
