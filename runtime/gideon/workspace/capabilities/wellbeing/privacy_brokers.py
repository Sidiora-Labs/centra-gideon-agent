...
class PrivacyBrokerStore(PrivacyStore):
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.executescript(
                """CREATE TABLE IF NOT EXISTS privacy_brokers(id TEXT PRIMARY KEY,data TEXT NOT NULL);
    ...
    def _broker(self, db, identity):
        row = db.execute(
            "SELECT data FROM privacy_brokers WHERE id=?", (identity,)
        ).fetchone()
        if row is None:
    ...
    def _case(self, db, identity):
        row = db.execute(
            "SELECT data FROM privacy_broker_cases WHERE id=? ORDER BY revision DESC LIMIT 1",
            (identity,),
        ).fetchone()
        if row is None:
    ...
    def _decorate(self, row):
        return dict(
            row, allowed_transitions=list(MANUAL_TRANSITIONS.get(row["state"], ()))
        )

    def _append(self, db, row, operation, **details):
        row = dict(row, revision=row["revision"] + 1, updated_at=now())
        db.execute(
            "INSERT INTO privacy_broker_cases VALUES(?,?,?,?,?)",
            (
                row["id"],
                row["revision"],
                row["subject_id"],
        ...
        db.execute(
            "INSERT INTO privacy_broker_events VALUES(?,?,?)",
            (row["id"], row["revision"], json.dumps(event)),
        )
        self._audit(
            db,
            row["subject_id"],
            "broker_case_" + operation,
            case_id=row["id"],
    ...
    def create_broker(self, payload):
        fields(
            payload,
            ("request_id", "name", "website", "source"),
            ("optout_url", "enabled"),
        )
        ...
        def execute(db):
                ...
                if payload.get(key):
                    ...
                    if (
                        ...
                        raise MeasurementError(
                            key + " must be an HTTP(S) address without credentials"
                        )
            ...
            row = dict(
                ...
                optout_url=payload.get("optout_url", ""),
            ...
            db.execute(
                "INSERT INTO privacy_brokers VALUES(?,?)", (row["id"], json.dumps(row))
            )
            return row
        ...
        with self.connection() as db:
            return [
                ...
                for row in db.execute(
                    'SELECT data FROM privacy_brokers ORDER BY json_extract(data,"$.name")'
                )
            ]
    ...
    def create_case(self, subject, payload):
        ...
        def execute(db):
            ...
            existing = db.execute(
                "SELECT id FROM privacy_broker_cases WHERE subject_id=? AND broker_id=? LIMIT 1",
                (subject, broker["id"]),
            ).fetchone()
            if existing:
                raise MeasurementError(
                    "Broker case already exists for subject", 409, "conflict"
                )
            stamp = now()
            row = dict(
                id=str(uuid4()),
    ...
    def list_cases(self, subject):
        with self.connection() as db:
            ...
            rows = db.execute(
                'SELECT c.data,b.data FROM privacy_broker_cases c JOIN privacy_brokers b ON b.id=c.broker_id WHERE c.subject_id=? AND c.revision=(SELECT MAX(s.revision) FROM privacy_broker_cases s WHERE s.id=c.id) ORDER BY json_extract(b.data,"$.name")',
                (subject,),
            ).fetchall()
            return [
                dict(self._decorate(json.loads(case)), broker=json.loads(broker))
                for case, broker in rows
            ]
    ...
    def history(self, identity):
        with self.connection() as db:
            ...
            return [
                self._decorate(json.loads(row[0]))
                for row in db.execute(
                    "SELECT data FROM privacy_broker_cases WHERE id=? ORDER BY revision",
                    (identity,),
                )
            ]
    ...
    def events(self, identity):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT data FROM privacy_broker_events WHERE case_id=? ORDER BY revision",
                    (identity,),
                )
            ]
    ...
    def record_user_observation(self, identity, payload):
        ...
        def execute(db):
            ...
            if row["state"] == "confirmed_removed" and payload["outcome"] == "found":
                ...
                raise MeasurementError(
                    "Case must complete or request verification before a new owner observation",
                    409,
                    "conflict",
                )
            row.update(
                state=state,
                evidence_basis="user_attested",
                evidence=payload["evidence"],
                next_recheck_at=self._next(state),
            )
            return self._append(
                db, row, "owner_observation", outcome=payload["outcome"]
            )

        return self._mutate_case(
            "privacy-broker-observation", identity, payload, execute
        )
    ...
    def transition(self, identity, payload):
        ...
        def execute(db):
            ...
            if payload["state"] not in MANUAL_TRANSITIONS.get(row["state"], ()):
                raise MeasurementError(
                    "Invalid manual broker case transition", 409, "conflict"
                )
            ...
            row.update(
                ...
                evidence_basis="user_attested",
                next_recheck_at=self._next(payload["state"]),
            )
            return self._append(db, row, "manual_transition")

        return self._mutate_case(
            "privacy-broker-transition", identity, payload, execute
        )
    ...
    def apply_verified_observation(self, identity, observation):
        if not isinstance(observation, VerifiedBrokerObservation):
            raise MeasurementError(
                "Verified observations require the scanner adapter boundary",
                403,
        ...
        try:
            ...
            raise MeasurementError(
                "Verifier checked_at must be an offset timestamp"
            ) from exc
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            row = self._case(db, identity)
            self._require(db, row["subject_id"], "broker_scan")
            if observation.outcome == "not_found" and row["state"] in (
                "verification_pending",
                "awaiting_processing",
                "human_task_queued",
            ):
                ...
                raise MeasurementError(
                    "Verified observation does not match the case lifecycle",
                    409,
                    "conflict",
                )
            row.update(
                state=state,
                evidence_basis="verified_rescan",
                evidence=observation.evidence_ref,
                verifier=observation.verifier,
                verified_at=observation.checked_at,
                next_recheck_at=self._next(state),
            )
            return self._append(
                db, row, "verified_observation", outcome=observation.outcome
            )

    def apply_adapter_scan(self, identity, payload, result):
        fields(payload, ("request_id", "revision"))
        if (
            not isinstance(result, BrokerAdapterResult)
            or result.outcome not in OBSERVATIONS
        ):
            raise MeasurementError(
                "Broker scan requires a typed adapter result",
                403,
                "verification_required",
            )
        return self._adapter_mutation(
            "privacy-broker-adapter-scan", identity, payload, result, "broker_scan"
        )
    ...
    def apply_adapter_prepared(self, identity, payload, result):
        ...
        if not isinstance(result, BrokerAdapterResult) or result.outcome != "prepared":
            raise MeasurementError(
                "Opt-out preparation requires a typed adapter result",
                403,
                "verification_required",
            )
        return self._adapter_mutation(
            "privacy-broker-adapter-prepare", identity, payload, result, "broker_submit"
        )
    ...
    def apply_adapter_submission(self, identity, payload, result):
        ...
        if not isinstance(result, BrokerAdapterResult) or result.outcome != "submitted":
            raise MeasurementError(
                "Opt-out submission requires a typed adapter result",
                403,
                "verification_required",
            )
        return self._adapter_mutation(
            "privacy-broker-adapter-submit", identity, payload, result, "broker_submit"
        )

    def apply_adapter_verification(self, identity, payload, result):
        fields(payload, ("request_id", "revision"))
        if not isinstance(result, BrokerAdapterResult) or result.outcome not in (
            "found",
            "not_found",
        ):
            raise MeasurementError(
                "Verification requires a typed adapter result",
                403,
                "verification_required",
            )
        return self._adapter_mutation(
            "privacy-broker-adapter-verify", identity, payload, result, "broker_scan"
        )
    ...
    def _adapter_mutation(self, kind, identity, payload, result, scope):
        ...
        try:
            ...
            raise MeasurementError(
                "Adapter checked_at must be an offset timestamp"
            ) from exc
        ...
        def execute(db):
            ...
            if kind.endswith("-scan"):
                if row["state"] not in (
                    ...
                    raise MeasurementError(
                        "Case is owned by the opt-out workflow", 409, "conflict"
                    )
                state, operation = (
                    "reappeared"
                    if row["state"] == "confirmed_removed" and result.outcome == "found"
                    else result.outcome
                ), "provider_scan"
            elif kind.endswith("-prepare"):
                if row["state"] not in ("found", "indirect_exposure", "reappeared"):
                    raise MeasurementError(
                        "Case is not ready for opt-out preparation", 409, "conflict"
                    )
                state, operation = "optout_in_progress", "provider_optout_prepared"
            elif kind.endswith("-submit"):
                if row["state"] != "optout_in_progress":
                    raise MeasurementError(
                        "Case is not ready for opt-out submission", 409, "conflict"
                    )
                ...
                if row["state"] not in (
                    ...
                    raise MeasurementError(
                        "Case is not ready for provider verification", 409, "conflict"
                    )
                state = (
                    "confirmed_removed"
            ...
            row.update(
                state=state,
                evidence_basis="provider_protocol",
            ...
            return self._append(
                db, row, operation, provider=result.provider, outcome=result.outcome
            )
    ...
    def _next(state):
        return (
            datetime.now(timezone.utc) + timedelta(days=BACKOFF_DAYS[state])
        ).isoformat()
