...
TABLES = {
    "measurements": [
        ...
        ("shared_links", "shared_health_links", None),
        ("shared_identity", "shared_health_meta", None),
    ],
    "laboratory": [
        ("history", "lab_revisions", "data"),
        ("imports", "lab_imports", "receipt"),
        ("source_identities", "lab_sources", None),
    ],
    "apple": [
        ("metrics", "apple_metrics", "data"),
        ("imports", "apple_imports", "receipt"),
    ],
    "substances": [
        ("entries", "substance_entries", "data"),
        ("presets", "substance_presets", "data"),
    ],
    "genome": [
        ("sources", "genome_sources", "data"),
        ("variants", "genome_variants", "data"),
    ],
    "interventions": [
        ("plans", "intervention_plans", "data"),
        ("records", "intervention_records", "data"),
    ],
    "cognition": [("sessions", "cognitive_sessions", "data")],
    "memory": [("cards", "memory_card_revisions", "data")],
    "life": [
        ("history", "life_calendar_revisions", "data"),
        ("reminder_claims", "life_reminder_claims", None),
    ],
...
class ExportStore(MeasurementStore):
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS wellbeing_exports(id TEXT PRIMARY KEY,data TEXT NOT NULL)"
            )
    ...
    def _snapshot(self, db):
                    ...
                    for record in cursor:
                        ...
                        section_bytes += len(
                            (
                                values[column]
                                if column
                                else json.dumps(values, ensure_ascii=True)
                            ).encode()
                        )
                        if section_bytes > MAX_BYTES:
                            raise MeasurementError(
                                "Export exceeds64MiB; no partial export created",
                                413,
                                "too_large",
                            )
        ...
        for reference in references(sections):
            ...
            if (
                ...
                or not re.fullmatch(
                    r"(lab-source|apple-source|genome-source|memory-card|shared-health)-[a-f0-9]{64}",
                    slug,
                )
                or version != 1
            ):
                raise MeasurementError(
                    "Export source artifact reference is unsupported", 409, "conflict"
                )
            ...
            if filename is not None:
                if not isinstance(filename, str) or not re.fullmatch(
                    r"original@[a-f0-9]{64}\.(csv|json|xml|zip|fhir|tsv|vcf)", filename
                ):
                    raise MeasurementError(
                        "Invalid original attachment filename", 409, "conflict"
                    )
                folder = self.path.parent.parent / "artifacts" / slug / "versions"
                path = folder / filename
                if path.resolve().parent != folder.resolve() or path.is_symlink():
                    raise MeasurementError(
                        "Original attachment path is unsafe", 409, "conflict"
                    )
                try:
                    ...
                    raise MeasurementError(
                        "Original attachment unavailable", 404, "not_found"
                    ) from exc
            else:
                raw = descriptor
            if hashlib.sha256(raw).hexdigest() != reference.get("sha256"):
                raise MeasurementError(
                    "Source attachment hash mismatch", 409, "conflict"
                )
            attachment_bytes += len(raw) + (len(descriptor) if filename else 0)
            if attachment_bytes > MAX_BYTES:
                raise MeasurementError(
                    "Export exceeds64MiB; no partial export created", 413, "too_large"
                )
            attachments.append(
                dict(
                    reference=reference,
        ...
        snapshot = dict(
            ...
            omitted=[
                ...
                "private_exercise_expected_answers",
                "external_trigger_definitions",
                "inbox_items",
            ],
        )
        raw = json.dumps(
    ...
    def create(self, payload):
        ...
        with self.connection() as db:
            ...
            metadata = dict(
                ...
                bytes=len(raw),
                counts=snapshot["counts"],
                artifact=dict(slug=slug, version=1, filename=filename),
                attachment_count=len(snapshot["attachments"]),
                omitted=snapshot["omitted"],
            )
            ...
            if artifact.content != descriptor or not self.artifacts.store_version_file(
                ...
                raise MeasurementError(
                    "Export artifact could not be persisted", 503, "unavailable"
                )
            if self._read(metadata) != raw:
                raise MeasurementError(
                    "Export artifact verification failed", 503, "unavailable"
                )
    ...
    def get(self, identity):
        with self.connection() as db:
            row = db.execute(
                "SELECT data FROM wellbeing_exports WHERE id=?", (identity,)
            ).fetchone()
            if not row:
        ...
        with self.connection() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT data FROM wellbeing_exports ORDER BY json_extract(data,'$.generated_at') DESC LIMIT 500"
                )
            ]
    ...
    def _read(self, metadata):
        ...
        try:
            ...
            raise MeasurementError(
                "Export attachment unavailable", 404, "not_found"
            ) from exc
