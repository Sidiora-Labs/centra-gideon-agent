...
class SharedHealthStore(MeasurementStore):
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.executescript(
                """CREATE TABLE IF NOT EXISTS shared_health_meta(id INTEGER PRIMARY KEY,store_id TEXT NOT NULL);
            ...
            db.execute(
                "INSERT OR IGNORE INTO shared_health_meta VALUES(1,?)", (str(uuid4()),)
            )
    ...
    def _parse(self, payload):
        ...
        if not isinstance(content, str) or len(content.encode()) > MAX_BYTES:
            raise MeasurementError(
                "Shared document must be UTF-8 text at most8MiB", 413, "too_large"
            )
        try:
        ...
        if (
            ...
            or set(document) != {"schema", "version", "store_id", "records"}
        ...
        for row in document["records"]:
            ...
            if (
                ...
                or not 1 <= row["revision"] <= 1000000000
            ):
    ...
    def _plan(self, db, document, raw):
        ...
        for row in document["records"]:
            link = db.execute(
                "SELECT remote_revision,fingerprint,record_id,local_revision FROM shared_health_links WHERE store_id=? AND external_id=?",
                (document["store_id"], row["id"]),
            ).fetchone()
            ...
            if link is None:
                ...
                if (
                    row["revision"] < link[0]
                    or row["revision"] == link[0]
                    and fingerprint != link[1]
                ):
                    raise MeasurementError(
                        "Shared revision is stale or has conflicting content",
                        409,
                        "conflict",
                    )
                if row["revision"] == link[0]:
                    ...
                    if current["revision"] != link[3]:
                        raise MeasurementError(
                            "Canonical record changed locally; resolve before importing",
                            409,
                            "conflict",
                        )
                    if (
                        row["kind"] != current["kind"]
                        or row["source"] != current["source"]
                    ):
                        raise MeasurementError(
                            "Shared kind and source cannot change", 409, "conflict"
                        )
                    action = "correct"
            counts[action] += 1
            plan.append(dict(id=row["id"], action=action))
        return dict(
            preview_id=digest(raw),
    ...
    def commit(self, payload):
        ...
        if payload["preview_id"] != sha:
            raise MeasurementError(
                "Shared document changed since preview", 409, "conflict"
            )
        ...
        with self.connection() as db:
            ...
            descriptor = json.dumps(
                dict(schema=SCHEMA, sha256=sha, bytes=len(raw), filename=filename),
                sort_keys=True,
            )
            artifact = self.artifacts.get(slug, version=1)
            if artifact is None:
                artifact = self.artifacts.create(
                    name="Native shared health source",
                    content=descriptor,
                    kind="document",
                    source="import",
                    slug=slug,
            ...
            if artifact.content != descriptor or not self.artifacts.store_version_file(
                slug, filename, raw
            ):
                raise MeasurementError(
                    "Shared original could not be saved", 503, "unavailable"
                )
            results = []
            for row, step in zip(document["records"], plan["records"]):
                link = db.execute(
                    "SELECT record_id,local_revision FROM shared_health_links WHERE store_id=? AND external_id=?",
                    (document["store_id"], row["id"]),
                ).fetchone()
                if step["action"] == "unchanged":
                    ...
                    mutation["request_id"] = "shared-" + digest(
                        encoded([document["store_id"], row])
                    )
                    if link:
                        mutation["revision"] = link[1]
                    result = self._write_in(db, link[0] if link else None, mutation)
                    db.execute(
                        "INSERT OR REPLACE INTO shared_health_links VALUES(?,?,?,?,?,?)",
                        (
                            document["store_id"],
                            row["id"],
                            row["revision"],
            ...
            receipt = dict(
                ...
                counts=plan["counts"],
                records=results,
                artifact=dict(slug=slug, version=1, filename=filename, sha256=sha),
            )
            value = json.dumps(receipt)
            db.execute(
                "INSERT OR IGNORE INTO shared_health_imports VALUES(?,?)", (sha, value)
            )
            db.execute(
                "INSERT INTO requests VALUES(?,?,?)", (request_id, fingerprint, value)
            )
            return receipt
    ...
    def _read_file(self):
        if self.file.parent.is_symlink():
            raise MeasurementError(
                "Shared directory symlinks are unsupported", 409, "conflict"
            )
        if self.file.is_symlink():
            raise MeasurementError(
                "Shared file symlinks are unsupported", 409, "conflict"
            )
        try:
            ...
            raise MeasurementError(
                "Shared file is unavailable or not UTF-8", 503, "unavailable"
            ) from exc
        ...
        try:
            ...
            return dict(
                state=(
                    "missing"
                    if exc.status == 404
                    else "unavailable" if exc.status == 503 else "invalid"
                ),
                error=str(exc),
            )
    ...
    def commit_file(self, payload):
        if not isinstance(payload, dict) or set(payload) != {
            ...
            raise MeasurementError(
                "File commit requires only preview_id and request_id"
            )
    ...
    def publish(self, payload):
        ...
        with self._lock(), self.connection() as db:
            ...
            if status["state"] not in ("missing", "ready"):
                raise MeasurementError(
                    "Present shared file cannot be overwritten: " + status["state"],
                    409,
                    "conflict",
                )
            store_id = db.execute(
                "SELECT store_id FROM shared_health_meta WHERE id=1"
            ).fetchone()[0]
            rows = [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT r.data FROM revisions r WHERE r.revision=(SELECT MAX(s.revision) FROM revisions s WHERE s.id=r.id) ORDER BY r.id"
                )
            ]
            records = [{key: row[key] for key in FIELDS} for row in rows]
            raw = encoded(
                dict(schema=SCHEMA, version=1, store_id=store_id, records=records)
            )
            self._parse({"content": raw.decode()})
            if status.get("sha256") not in (payload["expected_sha256"], digest(raw)):
                raise MeasurementError(
                    "Shared file changed; refresh before publishing", 409, "conflict"
                )
