...
FIELDS = {"observed_at", "source", "reported_sex", "sex_source", "smoking_status", "diet_quality",
          "stress", "reported_bmi", "condition_labels", "reported_daily_alcohol"}
...
def _scaled(value, field):
    if not isinstance(value, dict) or set(value) != {"value", "scale"} or not isinstance(value["scale"], dict) or set(value["scale"]) != {"minimum", "maximum", "label"}:
        raise MeasurementError(f"{field} requires value and a declared minimum, maximum and label")
    minimum = _number(value["scale"]["minimum"], field + ".scale.minimum", -1000000, 1000000)
    maximum = _number(value["scale"]["maximum"], field + ".scale.maximum", -1000000, 1000000)
    if minimum >= maximum:
        raise MeasurementError(f"{field} scale minimum must be below maximum")
    score = _number(value["value"], field + ".value", minimum, maximum)
    return {"value": score, "scale": {"minimum": minimum, "maximum": maximum,
...
def _alcohol(value):
    if value is None:
        return None
    if not isinstance(value, dict) or set(value) != {"value", "unit"} or value["unit"] not in ALCOHOL_UNITS:
        raise MeasurementError("reported_daily_alcohol requires a supported explicit unit")
    return {"value": _number(value["value"], "reported_daily_alcohol.value", 0, 1000), "unit": value["unit"]}


def validate(value):
    if not isinstance(value, dict) or set(value) != FIELDS:
        raise MeasurementError("Lifestyle observation fields are incomplete or unsupported")
    observed_at = text(value["observed_at"], "observed_at", 64); instant(observed_at)
    ...
    if value["smoking_status"] not in SMOKING:
        raise MeasurementError("smoking_status must be never, former, current or unknown")
    ...
    return {"observed_at": observed_at, "source": source, "reported_sex": reported_sex,
            "sex_source": sex_source, "smoking_status": value["smoking_status"],
            ...
            "condition_labels": labels, "reported_daily_alcohol": _alcohol(value["reported_daily_alcohol"])}
...
class LifestyleProfileStore(MeasurementStore):
    ...
    def _get(self, db, identity):
        row = db.execute("SELECT data FROM lifestyle_profile_revisions WHERE id=? ORDER BY revision DESC LIMIT 1", (identity,)).fetchone()
        if row is None:
            raise MeasurementError("Lifestyle observation not found", 404, "not_found")
        return json.loads(row[0])

    def _append(self, db, record):
        db.execute("INSERT INTO lifestyle_profile_revisions VALUES(?,?,?,?)",
                   (record["id"], record["revision"], instant(record["observed_at"]), json.dumps(record, sort_keys=True)))

    def create(self, payload):
        if not isinstance(payload, dict) or set(payload) != FIELDS | {"request_id"}:
            raise MeasurementError("Lifestyle creation requires one exact authored observation")
        ...
        with self.connection() as db:
            ...
            record = {**values, "id": str(uuid4()), "revision": 1, "created_at": stamp, "updated_at": stamp}
            self._append(db, record); encoded = json.dumps(record, sort_keys=True)
            db.execute("INSERT INTO requests VALUES(?,?,?)", (request_id, fingerprint, encoded))
            return record

    def correct(self, identity, payload):
        if not isinstance(payload, dict) or set(payload) - (CORRECTABLE | {"request_id", "revision"}) or not {"request_id", "revision"} <= set(payload):
            raise MeasurementError("Lifestyle correction contains unsupported fields")
        request_id = text(payload["request_id"], "request_id", 128)
        try:
            fingerprint = json.dumps(["lifestyle-correct", identity, payload], sort_keys=True, allow_nan=False)
        except (TypeError, ValueError) as exc:
            raise MeasurementError("Lifestyle correction must contain finite JSON values") from exc
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            prior = db.execute("SELECT payload,result FROM requests WHERE id=?", (request_id,)).fetchone()
            if prior:
            ...
            if type(payload["revision"]) is not int or payload["revision"] != record["revision"]:
                raise MeasurementError("Lifestyle observation changed; reload", 409, "conflict")
            values = {key: record[key] for key in FIELDS}
            values.update({key: value for key, value in payload.items() if key in CORRECTABLE})
            record.update(validate(values), revision=record["revision"] + 1, updated_at=datetime.now(timezone.utc).isoformat())
            self._append(db, record); encoded = json.dumps(record, sort_keys=True)
            db.execute("INSERT INTO requests VALUES(?,?,?)", (request_id, fingerprint, encoded))
            return record
    ...
    def list(self):
        with self.connection() as db:
            return [json.loads(row[0]) for row in db.execute("""SELECT r.data FROM lifestyle_profile_revisions r
    ...
    def history(self, identity):
        with self.connection() as db:
            ...
            return [json.loads(row[0]) for row in db.execute("SELECT data FROM lifestyle_profile_revisions WHERE id=? ORDER BY revision", (identity,))]
