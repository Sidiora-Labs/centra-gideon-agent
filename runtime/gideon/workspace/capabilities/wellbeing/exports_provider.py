...
async def invoke_exports(home, arguments):
    ...
    if operation == "create":
        ...
        result = await asyncio.to_thread(
            store.preview if operation == "preview" else store.list_exports
        )
    elif operation in ("get", "download"):
        result = await asyncio.to_thread(
            store.get if operation == "get" else store.download, arguments.get("id")
        )
        if isinstance(result, bytes):
            result = dict(
                content_base64=base64.b64encode(result).decode(),
