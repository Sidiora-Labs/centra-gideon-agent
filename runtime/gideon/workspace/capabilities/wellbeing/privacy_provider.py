...
async def invoke_privacy(home, arguments):
    ...
    methods = {
        ...
        "fact": store.get_fact,
        "fact_history": store.history_fact,
        "audit": store.audit,
    }
    ...
    if operation in ("subject_create", "consent"):
        result = (
            await asyncio.to_thread(methods[operation], identity, payload)
            if operation == "consent"
            else await asyncio.to_thread(methods[operation], payload)
        )
    else:
        if payload:
            raise MeasurementError("Privacy metadata read accepts no payload")
        result = (
            await asyncio.to_thread(methods[operation], identity)
            if operation != "subjects"
            else await asyncio.to_thread(methods[operation])
        )
