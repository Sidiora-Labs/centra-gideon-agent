...
class BodyCompositionProvider(ToolProvider):
    ...
    async def list_tools(self):
        return [
            ToolDefinition(
                name="body_composition_read",
                provider=self.name,
                description="Read authored body-composition observations, immutable history, or canonical export without medical interpretation.",
                parameters=READ_SCHEMA,
                requires_approval=False,
                risk_level=RiskLevel.SAFE,
            ),
            ToolDefinition(
                name="body_composition_write",
                provider=self.name,
                description="Create or correct an authored body-composition observation. Explicit owner approval is required.",
    ...
    async def invoke(self, tool_name, arguments):
        try:
            ...
            elif tool_name == "body_composition_read" and operation in {
                "get",
                "history",
            }:
