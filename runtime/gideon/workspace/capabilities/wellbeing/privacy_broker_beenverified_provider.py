...
class BeenVerifiedPrivacyBrokerProvider(ToolProvider):
    ...
    def __init__(self, service=None):
        self.service = service or BeenVerifiedCaseAdapter(
    ...
    async def list_tools(self):
        ...
        return [
            ToolDefinition(
                name="privacy_broker_beenverified_" + name,
                provider=self.name,
    ...
    async def invoke(self, tool_name, arguments):
        ...
        if (
            ...
            return ToolResult(
                False,
                error="tool and arguments must match the declared BeenVerified operation",
            )
        try:
            value = method(
                arguments["case_id"],
...
def create_provider(config=None):
    if config:
        raise ValueError(
            "BeenVerified provider does not accept caller storage, transport or credential configuration"
        )
