...
def fields(payload, required, optional=()):
    if (
        ...
        or set(payload) - set(required) - set(optional)
        or set(required) - set(payload)
    ):
...
class PrivacyStore:
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.executescript(
                """CREATE TABLE IF NOT EXISTS privacy_subjects(id TEXT PRIMARY KEY,data TEXT NOT NULL);
    ...
    def _subject(self, db, identity):
        row = db.execute(
            "SELECT data FROM privacy_subjects WHERE id=?", (identity,)
        ).fetchone()
        if row is None:
    ...
    def _fact(self, db, identity):
        row = db.execute(
            "SELECT data,cipher FROM privacy_facts WHERE id=? ORDER BY revision DESC LIMIT 1",
            (identity,),
        ).fetchone()
        if row is None:
    ...
    def _allowed(self, db, subject, scope):
        ...
        row = db.execute(
            "SELECT data FROM privacy_consents WHERE subject_id=? AND scope=? ORDER BY revision DESC LIMIT 1",
            (subject, scope),
        ).fetchone()
    ...
    def _audit(self, db, subject, operation, **details):
        ...
        db.execute(
            "INSERT INTO privacy_audit VALUES(?,?,?)",
            (record["id"], subject, json.dumps(record)),
        )

    def _prior(self, db, request_id, kind, payload, password=None):
        text(request_id, "request_id", 128)
        row = db.execute(
            "SELECT kind,payload,result FROM privacy_requests WHERE id=?", (request_id,)
        ).fetchone()
        if row is None:
            return None
        if row[0] != kind:
        ...
        try:
            original = (
                decrypt_archive(bytes(row[1]), password)
                if password is not None
                else bytes(row[1])
            )
        except ArchiveRefused as exc:
            raise MeasurementError(
                "Private request could not be authenticated",
                403,
    ...
    def _receipt(self, db, request_id, kind, payload, result, password=None):
        encoded = (
            encrypt_archive(data(payload), password)
            if password is not None
            else data(payload)
        )
        db.execute(
    ...
    def create_subject(self, payload):
        ...
        with self.connection() as db:
            ...
            db.execute(
                "INSERT INTO privacy_subjects VALUES(?,?)", (row["id"], json.dumps(row))
            )
        ...
        with self.connection() as db:
            return [
                ...
                for row in db.execute(
                    "SELECT data FROM privacy_subjects ORDER BY rowid"
                )
            ]
    ...
    def consent(self, subject, payload):
        ...
        if (
            ...
            or payload["scope"] not in SCOPES
        ...
        with self.connection() as db:
            ...
            if prior:
                return prior
            revision = db.execute(
                "SELECT COALESCE(MAX(revision),0) FROM privacy_consents WHERE subject_id=? AND scope=?",
                (subject, payload["scope"]),
            ).fetchone()[0]
            if revision != payload["revision"]:
                raise MeasurementError(
                    "Consent changed; reload before recording", 409, "conflict"
                )
            row = dict(
                subject_id=subject,
                scope=payload["scope"],
            ...
            db.execute(
                ...
                (subject, row["scope"], row["revision"], json.dumps(row)),
            )
            self._audit(
                db,
                subject,
    ...
    def consents(self, subject):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT data FROM privacy_consents WHERE subject_id=? ORDER BY rowid",
                    (subject,),
                )
            ]
    ...
    def correct_fact(self, identity, payload):
        fields(
            payload,
            ("request_id", "revision", "value", "passphrase"),
            ("label", "archived", "use_for_scans"),
        )
    ...
    def _write_fact(self, subject, identity, payload):
        ...
        fingerprint = dict(
            subject_id=subject,
            fact_id=identity,
            **{key: value for key, value in payload.items() if key != "passphrase"},
        )
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            if identity:
            ...
            prior = self._prior(
                db, payload["request_id"], "fact", fingerprint, password
            )
            if prior:
                return prior
            if identity:
                if (
                    ...
                    raise MeasurementError(
                        "Private fact changed; reload before correcting",
                        409,
                        "conflict",
                    )
                ...
                row.update(
                    {
                        key: payload[key]
                        for key in ("label", "archived", "use_for_scans")
                        if key in payload
                    }
                )
                row["revision"] += 1
                ...
                row = dict(
                    ...
                    label=payload["label"],
                    source=payload["source"],
                    use_for_scans=payload["use_for_scans"],
                    archived=False,
                    created_at=now(),
                )
            if (
                ...
                or type(row["use_for_scans"]) is not bool
            ):
            ...
            db.execute(
                "INSERT INTO privacy_facts VALUES(?,?,?,?,?)",
                (row["id"], row["revision"], subject, json.dumps(row), cipher),
            )
            self._audit(
                db, subject, "fact_written", fact_id=row["id"], revision=row["revision"]
            )
            return self._receipt(
                db, payload["request_id"], "fact", fingerprint, row, password
            )
    ...
    def _decrypt(self, row, cipher, password):
        try:
            ...
            raise MeasurementError(
                "Private fact could not be authenticated", 403, "authentication_failed"
            ) from exc
        if not isinstance(plain, dict) or any(
            plain.get(key) != row[key] for key in ("id", "subject_id", "revision")
        ):
            raise MeasurementError(
                "Private fact identity authentication failed",
                403,
                "authentication_failed",
            )
        return plain["value"]
    ...
    def list_facts(self, subject):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT f.data FROM privacy_facts f WHERE subject_id=? AND f.revision=(SELECT MAX(s.revision) FROM privacy_facts s WHERE s.id=f.id) ORDER BY f.rowid",
                    (subject,),
                )
            ]
    ...
    def history_fact(self, identity):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT data FROM privacy_facts WHERE id=? ORDER BY revision",
                    (identity,),
                )
            ]
    ...
    def reveal(self, identity, payload):
        ...
        with self.connection() as db:
            ...
            try:
                ...
                if row["archived"]:
                    raise MeasurementError(
                        "Archived private fact cannot be revealed", 409, "conflict"
                    )
            ...
            self._audit(
                ...
                outcome="denied" if error else "revealed",
            )
        if error:
            raise error
    ...
    def audit(self, subject):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT data FROM privacy_audit WHERE subject_id=? ORDER BY rowid",
                    (subject,),
                )
            ]
