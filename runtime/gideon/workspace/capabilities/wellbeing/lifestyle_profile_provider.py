...
class LifestyleProfileProvider(ToolProvider):
    ...
    async def list_tools(self):
        ...
        definitions = {
            ...
            "correct": (
                {**common, "payload": {"type": "object"}},
                ["id", "payload"],
                True,
            ),
            "get": (common, ["id"], False),
            "history": (common, ["id"], False),
            "list": ({}, [], False),
            "export": ({}, [], False),
        }
        descriptions = {
            ...
            "get": "Read one canonical lifestyle observation.",
            "history": "Read immutable lifestyle correction history.",
            "list": "List current lifestyle observations.",
            "export": "Export canonical lifestyle observations and history.",
        }
        return [
            ToolDefinition(
                name="lifestyle_profile_" + name,
                provider=self.name,
                ...
                risk_level=RiskLevel.CAUTION if approval else RiskLevel.SAFE,
            )
    ...
    async def invoke(self, tool_name, arguments):
        ...
        if name not in {
            ...
            return ToolResult(
                False,
                error="tool and arguments must match a declared lifestyle-profile operation",
            )
        try:
            if name == "create":
                value = self.store.create(arguments.get("payload"))
            elif name == "correct":
                value = self.store.correct(
                    arguments.get("id"), arguments.get("payload")
                )
            elif name in ("get", "history"):
                value = getattr(self.store, name)(arguments.get("id"))
            else:
                value = getattr(self.store, name)()
            return ToolResult(
                True, output=json.dumps(value, sort_keys=True, allow_nan=False)
            )
...
def create_provider(config=None):
    if config:
        raise ValueError(
            "Lifestyle-profile provider accepts no caller storage configuration"
        )
