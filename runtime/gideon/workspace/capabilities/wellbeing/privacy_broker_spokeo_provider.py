...
class SpokeoPrivacyBrokerProvider(ToolProvider):
    ...
    def __init__(self, service=None):
        self.service = service or SpokeoCaseAdapter(PrivacyBrokerStore(config_dir()), SpokeoProtocol())

    async def list_tools(self):
        identity = {
            'case_id': {'type': 'string'}, 'request_id': {'type': 'string'},
            'revision': {'type': 'integer', 'minimum': 1}, 'first_name': {'type': 'string'},
            'last_name': {'type': 'string'}, 'state': {'type': 'string', 'pattern': '^[A-Za-z]{2}$'},
            'city': {'type': 'string'},
        }
        optout = {
            'case_id': {'type': 'string'}, 'request_id': {'type': 'string'},
            'revision': {'type': 'integer', 'minimum': 1}, 'profile_url': {'type': 'string'},
            'email': {'type': 'string'},
        }
        return [
            ToolDefinition(
                name='privacy_broker_spokeo_scan', provider=self.name,
                description='Scan Spokeo for an explicitly consented privacy case and save the provider result.',
                parameters=self._schema(identity, ('case_id', 'request_id', 'revision', 'first_name', 'last_name', 'state')),
                requires_approval=True, risk_level=RiskLevel.CAUTION),
            ToolDefinition(
                name='privacy_broker_spokeo_prepare', provider=self.name,
                description='Inspect the current Spokeo opt-out form and save a preparation result without submitting it.',
                parameters=self._schema(optout, tuple(optout)), requires_approval=True, risk_level=RiskLevel.CAUTION),
            ToolDefinition(
                name='privacy_broker_spokeo_submit', provider=self.name,
                ...
                parameters=self._schema({**optout, 'approval': {'type': 'string', 'const': 'SUBMIT SPOKEO OPT-OUT'}}, (*optout, 'approval')),
                requires_approval=True, risk_level=RiskLevel.DESTRUCTIVE),
            ToolDefinition(
                name='privacy_broker_spokeo_verify', provider=self.name,
    ...
    async def invoke(self, tool_name, arguments):
        ...
        if method is None or not isinstance(arguments, dict) or not isinstance(arguments.get('case_id'), str):
            return ToolResult(False, error='tool and arguments must match the declared Spokeo operation')
...
def create_provider(config=None):
    ...
    return SpokeoPrivacyBrokerProvider(SpokeoCaseAdapter(PrivacyBrokerStore(config_dir()), SpokeoProtocol(origin)))
