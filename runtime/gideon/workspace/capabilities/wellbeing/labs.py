...
def validate_row(row):
    fields = {"analyte", "observed_at", "value", "unit", "reference_low", "reference_high", "notes", "external_id"}
    if not isinstance(row, dict) or set(row) - fields:
        raise MeasurementError("Unexpected laboratory fields")
    result = {key: text(row.get(key), key, limit) for key, limit in (("analyte", 120), ("observed_at", 64), ("unit", 64))}
    instant(result["observed_at"])
    result.update(value=number(row.get("value"), "value"), reference_low=number(row.get("reference_low"), "reference_low", True), reference_high=number(row.get("reference_high"), "reference_high", True))
    if result["reference_low"] is not None and result["reference_high"] is not None and result["reference_low"] > result["reference_high"]:
...
def parse(payload):
    if not isinstance(payload, dict) or set(payload) - {"filename", "format", "content", "source", "preview_id", "request_id"}:
        raise MeasurementError("Import must contain only filename, format, content and source")
    ...
    try:
        if format_ == "json":
            ...
            if len(header) != len(set(header)) or not {"analyte", "observed_at", "value", "unit"} <= set(header):
                raise MeasurementError("CSV needs unique analyte, observed_at, value and unit headers")
            rows = list(reader)
                ...
                for key in ("value", "reference_low", "reference_high"):
                    if key in row:
                        try:
                            ...
                            raise MeasurementError(f"Row {index}: {key} must be numeric") from exc
...
class LabStore(MeasurementStore):
    ...
    def _identity(self, source, row):
        identity = digest([source, row["external_id"]]) if row["external_id"] else digest([source, row])
    ...
    def preview(self, payload):
        ...
        with self.connection() as db:
            for row in rows:
                ...
                existing = db.execute("SELECT fingerprint FROM lab_sources WHERE id=?", (key,)).fetchone()
                prior = existing[0] if existing else seen.get(key)
                if prior and prior != fingerprint:
                    raise MeasurementError("External record ID conflicts with a different source row", 409, "conflict")
    ...
    def commit(self, payload):
        ...
        if payload.get("preview_id") != preview["preview_id"]:
            raise MeasurementError("Preview changed; preview this input again", 409, "conflict")
        ...
        with self.connection() as db:
            ...
            existing = db.execute("SELECT receipt FROM lab_imports WHERE id=?", (import_id,)).fetchone()
            if existing:
            ...
            if artifact is None:
                artifact = self.artifacts.create(name=payload["filename"], content=payload["content"], kind=payload["format"], source="import", slug=slug, readonly=True)
            if artifact.content.replace("\r\n", "\n").replace("\r", "\n") != payload["content"].replace("\r\n", "\n").replace("\r", "\n"):
                raise MeasurementError("Original artifact content differs", 409, "conflict")
            reference = {"slug": artifact.slug, "version": 1, "sha256": hashlib.sha256(payload["content"].encode()).hexdigest()}
            reference["filename"] = f"original@{reference['sha256']}.{payload['format']}"
            if not self.artifacts.store_version_file(artifact.slug, reference["filename"], payload["content"].encode()):
            ...
            for index, row in enumerate(preview["rows"], 1):
                ...
                prior = db.execute("SELECT fingerprint FROM lab_sources WHERE id=?", (identity,)).fetchone()
                if prior:
                    if prior[0] != row_fingerprint:
                        raise MeasurementError("Concurrent source record conflict", 409, "conflict")
                    duplicates += 1
                    records.append(self._get(db, identity))
                    continue
                record = dict(row, id=identity, kind="laboratory", source=payload["source"], artifact=reference, row_index=index, created_at=datetime.now(timezone.utc).isoformat(), revision=1)
                self._append(db, record)
                db.execute("INSERT INTO lab_sources VALUES(?,?)", (identity, row_fingerprint))
                records.append(record)
                added += 1
            receipt = {"import_id": import_id, "artifact": reference, "records": records, "added": added, "duplicates": duplicates}
    ...
    def _original(self, reference):
        path = self.path.parent.parent / "artifacts" / reference["slug"] / "versions" / reference["filename"]
        try:
            data = path.read_bytes()
        except OSError as exc:
            raise MeasurementError("Original attachment is unavailable", 404, "not_found") from exc
    ...
    def _append(self, db, record):
        db.execute("INSERT INTO lab_revisions VALUES(?,?,?,?,?,?)", (record["id"], record["revision"], instant(record["observed_at"]), record["analyte"], record["unit"], json.dumps(record)))

    def _get(self, db, identity):
        row = db.execute("SELECT data FROM lab_revisions WHERE id=? ORDER BY revision DESC LIMIT 1", (identity,)).fetchone()
        if row is None:
            raise MeasurementError("Laboratory record not found", 404, "not_found")
        return json.loads(row[0])

    def correct(self, identity, payload):
        if not isinstance(payload, dict) or set(payload) - {"request_id", "revision", "value", "reference_low", "reference_high", "notes"}:
            raise MeasurementError("Only value, reference bounds and notes can be corrected")
        request_id = text(payload.get("request_id"), "request_id", 128)
        try:
            fingerprint = json.dumps(["lab-correct", identity, payload], sort_keys=True, allow_nan=False)
        except (ValueError, TypeError) as exc:
            raise MeasurementError("Correction must contain finite JSON values") from exc
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            prior = db.execute("SELECT payload,result FROM requests WHERE id=?", (request_id,)).fetchone()
            if prior:
            ...
            if type(payload.get("revision")) is not int or payload["revision"] != record["revision"]:
                raise MeasurementError("Laboratory record changed; reload", 409, "conflict")
            record.update({key: value for key, value in payload.items() if key not in ("request_id", "revision")})
            fields = {key: record[key] for key in ("analyte", "observed_at", "value", "unit", "reference_low", "reference_high", "notes", "external_id")}
            record.update(validate_row(fields))
            record["revision"] += 1
            self._append(db, record)
            db.execute("INSERT INTO requests VALUES(?,?,?)", (request_id, fingerprint, json.dumps(record)))
            return record
    ...
    def list(self, *, analyte=None, from_date=None, to_date=None, limit=100, offset=0):
        ...
        with self.connection() as db:
            rows = db.execute("""SELECT r.data FROM lab_revisions r WHERE
                r.revision=(SELECT MAX(s.revision) FROM lab_revisions s WHERE s.id=r.id)
                AND (? IS NULL OR analyte=?) AND (? IS NULL OR observed_utc>=?)
                AND (? IS NULL OR observed_utc<=?) ORDER BY observed_utc DESC,id LIMIT ? OFFSET ?""", (analyte, analyte, start, start, end, end, limit, offset))
            return [json.loads(row[0]) for row in rows]

    def history(self, identity):
        with self.connection() as db:
            self._get(db, identity)
            return [json.loads(row[0]) for row in db.execute("SELECT data FROM lab_revisions WHERE id=? ORDER BY revision", (identity,))]

    def trends(self, analyte, unit):
        text(analyte, "analyte", 120)
        text(unit, "unit", 64)
        with self.connection() as db:
            rows = db.execute("""SELECT r.data FROM lab_revisions r WHERE analyte=? AND unit=?
                AND r.revision=(SELECT MAX(s.revision) FROM lab_revisions s WHERE s.id=r.id)
                ORDER BY observed_utc DESC,id LIMIT 500""", (analyte, unit)).fetchall()
