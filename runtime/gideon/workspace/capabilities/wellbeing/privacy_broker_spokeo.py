...
class SpokeoProtocol:
    def __init__(self, origin="https://www.spokeo.com", contract_mode=False):
        ...
        if not contract_mode and (
            parsed.scheme != "https" or parsed.hostname != "www.spokeo.com"
        ):
            raise MeasurementError(
                "Live Spokeo adapter requires https://www.spokeo.com"
            )
    ...
    async def scan(self, identity):
        ...
        path = (
            "/"
            + quote(
                identity["first_name"].strip() + "-" + identity["last_name"].strip()
            )
            + "/"
            + state
        )
        ...
        if status in (403, 429) or any(
            value in lowered
            for value in ("captcha", "verify you are human", "request blocked")
        ):
            outcome = "blocked"
        elif status >= 500:
            raise MeasurementError(
                "Spokeo scan is temporarily unavailable", 503, "provider_failed"
            )
        elif status == 404 or "no results found" in lowered:
            outcome = "not_found"
        else:
            full_name = (
                (identity["first_name"] + " " + identity["last_name"]).strip().lower()
            )
            name_hit = full_name in lowered
            location_hit = state.lower() in lowered or bool(
                identity.get("city") and identity["city"].strip().lower() in lowered
            )
            outcome = (
                "found"
                if name_hit and location_hit
                else "indirect_exposure" if name_hit else "not_found"
            )
    ...
    async def prepare(self, profile_url, email):
        ...
        return self._result("prepared", form_url, body), {
            ...
            "submission_mode": (
                ...
                else "live" if live_enabled else "disabled"
            ),
    ...
    async def submit(self, profile_url, email, approval):
        ...
        if approval != APPROVAL:
            raise MeasurementError(
                "Exact Spokeo opt-out approval phrase is required",
                403,
                "approval_required",
            )
        if (
            not self.contract_mode
            and os.getenv("GIDEON_ALLOW_LIVE_SPOKEO_SUBMIT") != "1"
        ):
            raise MeasurementError(
                "Live Spokeo submission is disabled; enable it only after owner approval",
                403,
                "approval_required",
            )
        ...
        if status not in (200, 201, 202) or not any(
            value in body.lower()
            for value in ("request received", "confirmation email", "check your email")
        ):
            raise MeasurementError(
                "Spokeo did not acknowledge the opt-out request", 502, "provider_failed"
            )
    ...
    async def _form(self):
        ...
        if status != 200:
            raise MeasurementError(
                "Spokeo opt-out form is unavailable", 502, "provider_failed"
            )
        form = _OptOutForm()
        form.feed(body)
        if form.captcha:
            raise MeasurementError(
                "Spokeo requires a human challenge; no submission was made",
                409,
                "human_required",
            )
        ...
        if (
            form.method != "post"
            or not form.action
            or not {"url", "email"}.issubset(form.inputs)
        ):
            raise MeasurementError(
                "Spokeo opt-out form protocol changed", 502, "provider_changed"
            )
        if self._origin(form_url) != self._origin(self.origin):
            raise MeasurementError(
                "Spokeo opt-out form escaped its approved origin",
                502,
                "provider_changed",
            )
    ...
    async def _request(self, method, url, data=None):
        ...
        try:
            async with ClientSession(
                ...
                headers={"User-Agent": "Gideon-PrivacyBroker/1.0"},
            ) as client:
                async with client.request(
                    ...
                    if response.status in (301, 302, 303, 307, 308):
                        raise MeasurementError(
                            "Spokeo protocol returned an unapproved redirect",
                            502,
                            "provider_changed",
                        )
                    chunks, size = [], 0
                    async for chunk in response.content.iter_chunked(65536):
                        ...
                        if size > 1048576:
                            raise MeasurementError(
                                "Spokeo response exceeds 1 MiB", 502, "provider_failed"
                            )
                        chunks.append(chunk)
                    return (
                        response.status,
            ...
            raise MeasurementError(
                "Spokeo connection failed", 503, "provider_failed"
            ) from None
    ...
    def _profile(self, value):
        ...
        if self._origin(value) != self._origin(self.origin):
            raise MeasurementError(
                "Profile URL must belong to the configured Spokeo origin"
            )
...
class SpokeoCaseAdapter:
    ...
    def _case(self, identity, revision, scope):
        ...
        if not broker or broker["name"].strip().lower() != "spokeo":
            raise MeasurementError(
                "This adapter supports Spokeo cases only", 409, "unsupported_provider"
            )
    ...
    async def scan(self, identity, payload):
        fields(
            payload,
            ("request_id", "revision", "first_name", "last_name", "state"),
            ("city",),
        )
        self._case(identity, payload["revision"], "broker_scan")
        result = await self.protocol.scan(
            {
                key: payload[key]
                for key in ("first_name", "last_name", "state", "city")
                if key in payload
            }
        )
        return self.store.apply_adapter_scan(
            identity, {key: payload[key] for key in ("request_id", "revision")}, result
        )
    ...
    async def prepare(self, identity, payload):
        ...
        result, plan = await self.protocol.prepare(
            payload["profile_url"], payload["email"]
        )
        row = self.store.apply_adapter_prepared(
            identity, {key: payload[key] for key in ("request_id", "revision")}, result
        )
    ...
    async def submit(self, identity, payload):
        ...
        self._case(identity, payload["revision"], "broker_submit")
        result = await self.protocol.submit(
            payload["profile_url"], payload["email"], payload["approval"]
        )
        return self.store.apply_adapter_submission(
            identity, {key: payload[key] for key in ("request_id", "revision")}, result
        )
    ...
    async def verify(self, identity, payload):
        ...
        if result.outcome not in ("found", "not_found"):
            raise MeasurementError(
                "Spokeo verification was inconclusive", 409, "verification_inconclusive"
            )
        return self.store.apply_adapter_verification(
            identity, {key: payload[key] for key in ("request_id", "revision")}, result
        )
