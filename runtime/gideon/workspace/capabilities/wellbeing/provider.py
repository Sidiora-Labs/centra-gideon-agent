...
class WellbeingProvider(ToolProvider):
                    ...
                    "properties": {
                        "operation": {
                            ...
                            "enum": [
                                ...
                                "apple_commit",
                                "apple_metrics",
                                "substances_entry_create",
                                "substances_entry_correct",
                                "substances_entry_delete",
                                "substances_entry_get",
                                "substances_entry_history",
                                "substances_entry_list",
                                "substances_preset_create",
                                "substances_preset_update",
                                "substances_preset_delete",
                                "substances_preset_list",
                                "substances_summary",
                                "genome_preview",
                                "genome_commit",
                                "genome_sources",
                                ...
                                "genome_annotate",
                                "genome_history",
                                "intervention_create_plan",
                                "intervention_update_plan",
                                "intervention_get_plan",
                                "intervention_list_plans",
                                "intervention_history_plan",
                                "intervention_record",
                                "intervention_correct_record",
                                "intervention_get_record",
                                "intervention_list_records",
                                "intervention_history_record",
    ...
    async def invoke(self, tool_name, arguments):
        try:
            if tool_name == "wellbeing_records" and arguments.get("operation") in (
                "privacy_organizations",
                "privacy_organization",
                "privacy_organization_history",
                "privacy_holdings",
                ...
                return ToolResult(
                    success=True, output=await invoke_holdings(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("privacy_"):
                from .privacy_provider import invoke_privacy

                return ToolResult(
                    success=True, output=await invoke_privacy(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("shared_"):
                ...
                return ToolResult(
                    success=True, output=await invoke_shared(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("exports_"):
                ...
                return ToolResult(
                    success=True, output=await invoke_exports(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("life_"):
                ...
                return ToolResult(
                    success=True, output=await invoke_life(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("memory_"):
                ...
                return ToolResult(
                    success=True, output=await invoke_memory(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("cognition_"):
                from .cognition_provider import invoke_cognition

                return ToolResult(
                    success=True, output=await invoke_cognition(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("intervention_"):
                from .intervention_provider import invoke_intervention

                return ToolResult(
                    success=True, output=await invoke_intervention(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("genome_"):
                ...
                return ToolResult(
                    success=True, output=await invoke_genome(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("substances_"):
                from .substances_provider import invoke_substances

                return ToolResult(
                    success=True, output=await invoke_substances(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("apple_"):
                ...
                return ToolResult(
                    success=True, output=await invoke_apple(self.home, arguments)
                )
            if tool_name == "wellbeing_records" and str(
                arguments.get("operation", "")
            ).startswith("labs_"):
                ...
                return ToolResult(
                    success=True, output=await invoke_labs(self.home, arguments)
                )
            if tool_name != "wellbeing_records" or arguments.get("operation") not in (
                "list",
                "get",
                "history",
                "export",
            ...
            if operation == "list":
                ...
                value = await asyncio.to_thread(
                    getattr(store, operation), arguments.get("id")
                )
            elif operation == "correct":
                value = await asyncio.to_thread(
                    store.correct, arguments.get("id"), payload
                )
