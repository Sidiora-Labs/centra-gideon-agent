...
from gideon.workspace.capabilities.platform import replication_usage
...
DOMAINS = {
    ...
    replication_episodic_memory.SCOPE: Domain(replication_episodic_memory.SCOPE, replication_episodic_memory.ENTRIES),
    ...
    replication_clinical.SCOPE: Domain(replication_clinical.SCOPE, replication_clinical.ENTRIES),
        ...
        with self._connect() as connection:
            ...
            for item in pending_entries:
                ...
                if entry_id in replication_identity_stories.ENTRIES:
                    ...
                    try:
                        result = replication_episodic_memory.apply_rows(self.home, entry_id, item["rows"], self._ancestors(connection, peer_id, entry_id), queue, now)
                    except ValueError as error:
                        raise ReplicationError(str(error), 422) from error
                elif entry_id in replication_usage.ENTRIES:
                    try:
                        result = replication_usage.apply_rows(self.home, entry_id, item["rows"], self._ancestors(connection, peer_id, entry_id), queue, now)
                    except ValueError as error:
                        raise ReplicationError(str(error), 422) from error
                    ...
                    try:
                        result = replication_clinical.apply_rows(self.home, entry_id, item["rows"], self._ancestors(connection, peer_id, entry_id), queue, now)
                    except ValueError as error:
                        raise ReplicationError(str(error), 422) from error
    ...
    def restore_fields(self, conflict_id: str, fields: list[str]) -> dict:
        try:
            ...
            if record is not None and record.entry_id in replication_clinical.ENTRIES:
                return replication_clinical.restore_fields(self.home, conflict_id, fields, datetime.now(timezone.utc).isoformat())
