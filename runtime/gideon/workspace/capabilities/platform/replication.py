...
DOMAINS = {
    ...
    replication_knowledge_collections.SCOPE: Domain(
        replication_knowledge_collections.SCOPE,
        replication_knowledge_collections.ENTRIES,
    ),
    "creative.catalog": Domain(
        "creative.catalog", tuple(replication_adapters.CREATIVE_TABLES)
    ),
    replication_commissions.SCOPE: Domain(
        replication_commissions.SCOPE, replication_commissions.ENTRIES
    ),
    replication_creative_direction.SCOPE: Domain(
        replication_creative_direction.SCOPE, replication_creative_direction.ENTRIES
    ),
    replication_creative_documents.SCOPE: Domain(
        replication_creative_documents.SCOPE, replication_creative_documents.ENTRIES
    ),
    "identity.goals": Domain(
        "identity.goals", tuple(replication_adapters.IDENTITY_TABLES)
    ),
    ...
    replication_identity_stories.SCOPE: Domain(
        replication_identity_stories.SCOPE, replication_identity_stories.ENTRIES
    ),
    replication_experience_stories.SCOPE: Domain(
        replication_experience_stories.SCOPE, replication_experience_stories.ENTRIES
    ),
    replication_memory.SCOPE: Domain(
        replication_memory.SCOPE, replication_memory.ENTRIES
    ),
    replication_episodic_memory.SCOPE: Domain(
        replication_episodic_memory.SCOPE, replication_episodic_memory.ENTRIES
    ),
    replication_usage.SCOPE: Domain(replication_usage.SCOPE, replication_usage.ENTRIES),
    "communications.contacts": Domain(
        "communications.contacts", tuple(replication_adapters.COMMUNICATION_TABLES)
    ),
    ...
    replication_music_video.SCOPE: Domain(
        replication_music_video.SCOPE, replication_music_video.ENTRIES
    ),
    replication_clinical.SCOPE: Domain(
        replication_clinical.SCOPE, replication_clinical.ENTRIES
    ),
    "wellbeing.health": Domain(
        "wellbeing.health", replication_adapters.WELLBEING_HEALTH_ENTRIES
    ),
    "wellbeing.routines": Domain(
        "wellbeing.routines", replication_adapters.WELLBEING_ROUTINE_ENTRIES
    ),
    "wellbeing.genome": Domain(
        "wellbeing.genome", replication_adapters.WELLBEING_GENOME_ENTRIES
    ),
    "wellbeing.practice": Domain(
        "wellbeing.practice", replication_adapters.WELLBEING_PRACTICE_ENTRIES
    ),
    "wellbeing.life_calendar": Domain(
        "wellbeing.life_calendar", replication_adapters.WELLBEING_CALENDAR_ENTRIES
    ),
}
...
class ReplicationService:
    ...
    def _entry(entry_id: str):
        ...
        if (
            ...
            or entry.secret
            or entry.derived
            or not reconcile.handles_kind(entry.kind)
        ):
            raise ReplicationError(
                "Replication entry is not an exportable canonical row store", 400
            )
    ...
    def _rows(self, entry_id: str) -> list[dict]:
        ...
        if entry_id in replication_episodic_memory.ENTRIES:
            return replication_episodic_memory.read_rows(self.home, entry_id)
        if entry_id in replication_usage.ENTRIES:
        ...
        if entry_id in replication_clinical.ENTRIES:
            return replication_clinical.read_rows(self.home, entry_id)
    ...
    def export_batch(self, peer_id: str, scope: str) -> dict:
        ...
        entries = [
            {"entry_id": entry_id, "rows": self._rows(entry_id)}
            for entry_id in domain.entries
        ]
        content = {
        ...
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM outbound WHERE domain=?", (scope,)
            ).fetchone()
            if row and row["fingerprint"] == fingerprint:
                return json.loads(row["batch"])
            sequence = (row["sequence"] if row else 0) + 1
            batch_id = hashlib.sha256(
                f"{identity['peer_id']}|{scope}|{sequence}|{fingerprint}".encode()
            ).hexdigest()[:32]
            batch = {**content, "sequence": sequence, "batch_id": batch_id}
            connection.execute(
                "INSERT INTO outbound VALUES(?,?,?,?) ON CONFLICT(domain) DO UPDATE SET sequence=excluded.sequence,fingerprint=excluded.fingerprint,batch=excluded.batch",
                (scope, sequence, fingerprint, canonical_json(batch)),
            )
        return batch

    def _ancestors(self, connection, peer_id: str, entry_id: str) -> dict[str, str]:
        return {
            row["entity_id"]: row["sha"]
            for row in connection.execute(
                "SELECT entity_id,sha FROM ancestors WHERE peer_id=? AND entry_id=?",
                (peer_id, entry_id),
            )
        }
    ...
    def apply_batch(self, peer_id: str, batch: dict) -> dict:
        ...
        if batch["sender"] != peer_id:
            raise ReplicationError(
                "Replication sender does not match verified peer", 403
            )
        ...
        if not isinstance(entries, list) or [
            ...
            raise ReplicationError(
                "Replication coverage does not match the declared domain"
            )
        if any(
            set(item) != {"entry_id", "rows"}
            or not isinstance(item["rows"], list)
            or any(not isinstance(row, dict) for row in item["rows"])
            for item in entries
        ):
        ...
        try:
            ...
            if scope == replication_episodic_memory.SCOPE:
                replication_episodic_memory.validate_entries(entries)
            if scope == replication_usage.SCOPE:
            ...
            if scope == replication_clinical.SCOPE:
                replication_clinical.validate_entries(entries)
        ...
        with self._connect() as connection:
            ...
            cursor = connection.execute(
                "SELECT * FROM cursors WHERE peer_id=? AND domain=?", (peer_id, scope)
            ).fetchone()
            if cursor and sequence == cursor["sequence"]:
                if fingerprint != cursor["fingerprint"]:
                    raise ReplicationError(
                        "Replication sequence was reused for different content", 409
                    )
            ...
            if scope == replication_knowledge_collections.SCOPE:
                try:
                    applied = replication_knowledge_collections.apply_entries(
                        ...
                        entries,
                        {
                            entry_id: self._ancestors(connection, peer_id, entry_id)
                            for entry_id in domain.entries
                        },
                        queue,
                        now,
                ...
                applied_items = []
            pending_entries = (
                () if scope == replication_knowledge_collections.SCOPE else entries
            )
            for item in pending_entries:
                entry_id = item["entry_id"]
                if entry_id in replication_identity_stories.ENTRIES:
                    try:
                        result = replication_identity_stories.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                            queue,
                            now,
                        )
                    except ValueError as error:
                    ...
                    try:
                        result = replication_experience_stories.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                    ...
                    try:
                        result = replication_commissions.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                    ...
                    try:
                        result = replication_memory.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                        ...
                        raise ReplicationError(str(error), 422) from error
                elif entry_id in replication_episodic_memory.ENTRIES:
                    try:
                        result = replication_episodic_memory.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                        ...
                        raise ReplicationError(str(error), 422) from error
                elif entry_id in replication_usage.ENTRIES:
                    try:
                        result = replication_usage.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                    ...
                    try:
                        result = replication_creative_direction.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                    ...
                    try:
                        result = replication_video.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                    ...
                    try:
                        result = replication_music_video.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                        ...
                        raise ReplicationError(str(error), 422) from error
                elif entry_id in replication_clinical.ENTRIES:
                    try:
                        result = replication_clinical.apply_rows(
                            self.home,
                            entry_id,
                            item["rows"],
                            self._ancestors(connection, peer_id, entry_id),
                    ...
                    entry = self._entry(entry_id)
                    result = replication_adapters.apply_inventory_rows(
                        self.home,
                        entry,
                        item["rows"],
                        self._ancestors(connection, peer_id, entry.id),
                        queue,
                        now,
                    )
            ...
            for item, result in applied_items:
                ...
                if result.verdict != "consumed":
                    raise ReplicationError(
                        f"Replication payload for {entry_id} was rejected", 422
                    )
                results.append(
                    {
                        "entry_id": entry_id,
                ...
                ancestor_updates.extend(
                    (entry_id, entity_id, sha)
                    for entity_id, sha in result.new_ancestors.items()
                )
            for entry_id, entity_id, sha in ancestor_updates:
                connection.execute(
                    "INSERT INTO ancestors VALUES(?,?,?,?) ON CONFLICT(peer_id,entry_id,entity_id) DO UPDATE SET sha=excluded.sha",
                    (peer_id, entry_id, entity_id, sha),
                )
            response = {
                "accepted": True,
                "domain": scope,
            ...
            connection.execute(
                "INSERT INTO cursors VALUES(?,?,?,?,?,?) ON CONFLICT(peer_id,domain) DO UPDATE SET sequence=excluded.sequence,fingerprint=excluded.fingerprint,response=excluded.response,updated_at=excluded.updated_at",
                (peer_id, scope, sequence, fingerprint, packed, now),
            )
        return response
    ...
    async def push(self, peer_id: str, scope: str) -> dict:
        ...
        if (
            ...
            raise ReplicationError(
                "Peer did not acknowledge the replication batch", 502
            )
    ...
    def restore_fields(self, conflict_id: str, fields: list[str]) -> dict:
        try:
            ...
            if (
                record is not None
                and record.entry_id in replication_identity_stories.ENTRIES
            ):
                return replication_identity_stories.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
            ...
            if (
                record is not None
                and record.entry_id in replication_knowledge_collections.ENTRIES
            ):
                return replication_knowledge_collections.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
            ...
            if (
                record is not None
                and record.entry_id in replication_experience_stories.ENTRIES
            ):
                return replication_experience_stories.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
            ...
            if (
                record is not None
                and record.entry_id in replication_commissions.ENTRIES
            ):
                return replication_commissions.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
            ...
            if record is not None and record.entry_id in replication_memory.ENTRIES:
                return replication_memory.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
            ...
            if (
                record is not None
                and record.entry_id in replication_creative_direction.ENTRIES
            ):
                return replication_creative_direction.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
            ...
            if record is not None and record.entry_id in replication_video.ENTRIES:
                return replication_video.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
            ...
            if (
                record is not None
                and record.entry_id in replication_music_video.ENTRIES
            ):
                return replication_music_video.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
                )
            if record is not None and record.entry_id in replication_clinical.ENTRIES:
                return replication_clinical.restore_fields(
                    self.home,
                    conflict_id,
                    fields,
                    datetime.now(timezone.utc).isoformat(),
                )
            return replication_adapters.restore_fields(
                self.home, conflict_id, fields, datetime.now(timezone.utc).isoformat()
            )
        except ValueError as error:
    ...
    def status(self) -> dict:
        with self._connect() as connection:
            cursors = [
                dict(row)
                for row in connection.execute(
                    "SELECT peer_id,domain,sequence,updated_at FROM cursors ORDER BY peer_id,domain"
                )
            ]
        pending = [
            row.to_dict()
            for row in conflicts.ConflictQueue(self.home).items(
        ...
        return {
            "version": 1,
            "domains": [
                {"scope": scope, "entries": list(domain.entries)}
                for scope, domain in DOMAINS.items()
            ],
            "cursors": cursors,
            "conflicts": pending,
            "peers": self.peers.snapshot()["peers"],
        }
