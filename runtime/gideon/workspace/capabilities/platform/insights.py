...
def snapshot(home=None):
    ...
    measures, labs = MeasurementStore(home).list(limit=500), LabStore(home).list(
        limit=500
    )
    ...
    for item in goals[:200]:
        ...
        rows.append(
            dict(
                ...
                observations=item["checkins"],
                velocity=item["velocity"],
                milestones_complete_ratio=item["milestones_complete_ratio"],
                sources=item["linked_sources"],
                href="#/capabilities/identity/goal-plans",
            )
        )
    for kind, records, href in [
        ...
        for (label, unit), values in sorted(groups.items()):
            ...
            observations = [
                dict(
                    ...
                    values=row.get("values", {"value": row.get("value")}),
                    source=row["source"],
            ...
            delta = (
                {
                    key: value - first["values"][key]
                    for key, value in last["values"].items()
                }
                if len(values) > 1
                else None
            )
    ...
    for plan in plans[:200]:
        ...
        rows.append(
            dict(
                ...
                source=plan["source"],
                summary=summary,
                observations=interventions.list_records(plan["id"]),
                href="#/capabilities/wellbeing/interventions",
            )
        )
    data = dict(
        schema_version=1,
        ...
        coverage={
            ...
            "laboratory_limit": 500,
            "goal_limit": 200,
            "intervention_limit": 200,
            "possibly_truncated": len(measures) == 500
            or len(labs) == 500
            or len(goals) > 200
            or len(plans) > 200,
        },
        ...
        **snapshot(home),
        "narratives": [
            dict(
                ...
                name=row.name,
                version=row.version,
                href=f"#/artifacts/{row.slug}",
            )
            for row in artifacts(home).list(tag=TAG, kind="json")
        ][:100],
    }
...
def read_narrative(slug, version=None, home=None):
    ...
    return dict(
        ...
        versions=provider.list_versions(slug),
    ...
    with provider.mutation_lock:
        if payload.get("slug"):
            ...
            if (
                ...
                or current["version"] != payload["version"]
            ):
                raise MeasurementError("Narrative changed; reload its version", 409)
            row = provider.update(
                payload["slug"], content=content, snapshot=True, actor="user"
            )
        else:
            slug = (
                "personal-scorecard-"
                + hashlib.sha256(content.encode()).hexdigest()[:40]
            )
