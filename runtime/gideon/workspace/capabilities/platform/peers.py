...
def _canonical(proof: dict) -> bytes:
    return json.dumps(
        {key: proof[key] for key in sorted(_PROOF_FIELDS - {"signature"})},
        separators=(",", ":"),
        sort_keys=True,
    ).encode()
...
class PeerStore:
    ...
    def _identity(self) -> tuple[Ed25519PrivateKey, dict]:
        ...
        if not self.key_path.exists():
            ...
            document = {
                "version": 1,
                "private_key": _b64(raw_private),
                "public_key": _b64(public),
            }
            temporary = self.key_path.with_suffix(
    ...
    def snapshot(self) -> dict:
        ...
        with self._connect() as connection:
            peers = [
                self._row(row)
                for row in connection.execute("SELECT * FROM peers ORDER BY label,id")
            ]
        return {
            "version": 1,
            "self": identity,
    ...
    def put(self, peer_id: str, body: dict) -> dict:
        ...
        if (
            not _ID.fullmatch(peer_id)
            or not isinstance(body, dict)
            or set(body) != fields
        ):
            raise PeerError("Invalid peer record")
        label, endpoint, public_key = (
            body["label"],
            body["endpoint"],
            body["public_key"],
        )
        ...
        for key in ("send_categories", "receive_categories"):
            ...
            if (
                ...
                or any(
                    not isinstance(item, str) or not _CATEGORY.fullmatch(item)
                    for item in value
                )
            ):
        ...
        with self._connect() as connection:
            ...
            connection.execute(
                "INSERT INTO peers VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET label=excluded.label,endpoint=excluded.endpoint,public_key=excluded.public_key,enabled=excluded.enabled,send_categories=excluded.send_categories,receive_categories=excluded.receive_categories,revision=excluded.revision,updated_at=excluded.updated_at",
                (
                    peer_id,
                    label.strip(),
                    endpoint.rstrip("/"),
                    ...
                    *categories,
                    revision,
                    current["last_probe"] if current else None,
                    created,
                    now,
                ),
            )
    ...
    def delete(self, peer_id: str, revision: int) -> None:
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
                "SELECT revision FROM peers WHERE id=?", (peer_id,)
            ).fetchone()
            if row is None:
                raise PeerError("Peer not found", 404)
    ...
    def verify_proof(self, proof: dict, now: int | None = None) -> dict:
        ...
        if (
            proof["recipient"] != identity["peer_id"]
            or not isinstance(proof["expires_at"], int)
            or not stamp < proof["expires_at"] <= stamp + 300
            or not isinstance(proof["nonce"], str)
            or not _NONCE.fullmatch(proof["nonce"])
        ):
        ...
        try:
            ...
            Ed25519PublicKey.from_public_bytes(_unb64(peer["public_key"])).verify(
                _unb64(proof["signature"]), _canonical(proof)
            )
        ...
        with self._connect() as connection:
            ...
            try:
                connection.execute(
                    ...
                    (peer["id"], proof["nonce"], proof["expires_at"]),
                )
    ...
    async def post_signed(
        self, peer_id: str, scope: str, path: str, payload: dict
    ) -> dict:
        peer = self.get(peer_id)
        if (
        ...
        data = json.dumps(
            {"proof": proof, "payload": payload}, separators=(",", ":")
        ).encode()
        response = await fetch(
            urljoin(peer["endpoint"] + "/", path.lstrip("/")),
            policy=sync_egress_policy(peer["endpoint"]),
            method="POST",
        ...
        with self._connect() as connection:
            connection.execute(
                "UPDATE peers SET last_probe=?,updated_at=? WHERE id=?",
                (now, now, peer_id),
            )
        return result
