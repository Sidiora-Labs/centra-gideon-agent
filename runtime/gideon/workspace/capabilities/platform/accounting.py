...
def view(*, days=30, now=None):
    ...
    for row in usage_ledger._iter_rows():
        try:
            ...
            group = groups.setdefault(keys, {'instance_id': keys[0], 'provider_instance': keys[1], 'credential_ref': keys[2], 'subscription_source': keys[3], 'turns': 0, 'input_tokens': 0, 'output_tokens': 0, 'cache_read_tokens': 0, 'cache_creation_tokens': 0, 'recorded_cost_usd': 0.0, 'unpriced_turns': 0, 'imported_turns': 0, 'models': set(), 'sources': set(), 'import_formats': set()})
            group['turns'] += 1
            ...
            if row.get('attribution') == 'historical_cli_import':
                group['imported_turns'] += 1
                group['import_formats'].add(str(row.get('import_format') or 'unknown'))
    ...
    for row in rows:
        row.update(models=sorted(row['models']), sources=sorted(row['sources']), import_formats=sorted(row['import_formats']), recorded_cost_usd=round(row['recorded_cost_usd'], 6), billed_cost_usd=None, quota_remaining=None)
    return {'version': 2, 'days': days, 'since': start.isoformat(), 'until': now.isoformat(), 'rows': rows, 'turns': sum(row['turns'] for row in rows), 'imported_turns': sum(row['imported_turns'] for row in rows), 'invalid_rows': invalid, 'unattributed_turns': sum(row['turns'] for row in rows if row['provider_instance'] is None), 'coverage': 'Canonical per-turn ledger including bounded Claude Code and Codex CLI history imports. Imported usage retains hashed provenance and unknown pricing; no billed cost is inferred. Guarded model-call audit records remain available on the existing usage page. Legacy attribution stays unknown. vendor plan, billing and live quotas are unavailable.'}
