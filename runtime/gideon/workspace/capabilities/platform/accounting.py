...
def view(*, days=30, now=None):
    ...
    for row in usage_ledger._iter_rows():
        try:
            ...
            if (
                any(value < 0 for value in tokens)
            ...
            group = groups.setdefault(
                ...
                {
                    "instance_id": keys[0],
                    "provider_instance": keys[1],
                    "credential_ref": keys[2],
                    "subscription_source": keys[3],
                    "turns": 0,
                    "input_tokens": 0,
                    "output_tokens": 0,
                    "cache_read_tokens": 0,
    ...
    for row in rows:
        row.update(
            ...
            import_formats=sorted(row["import_formats"]),
            recorded_cost_usd=round(row["recorded_cost_usd"], 6),
