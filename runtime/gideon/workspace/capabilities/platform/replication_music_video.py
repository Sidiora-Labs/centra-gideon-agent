...
def _contains_private(value):
    if isinstance(value, dict):
        return bool(_PRIVATE_FIELDS & set(value)) or any(
...
def validate_record(value, entity_id=None):
    if (
        ...
        or set(value) != _PROJECT_FIELDS
        or _contains_private(value)
    ):
...
def read_rows(home, entry_id):
    ...
    with store._db() as database:
        rows = database.execute(
            "SELECT id,payload FROM projects ORDER BY id"
        ).fetchall()
    result = [
        {"id": identity, "data": json.loads(payload)} for identity, payload in rows
    ]
