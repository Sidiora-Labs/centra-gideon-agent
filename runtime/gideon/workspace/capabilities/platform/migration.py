        ...
        for member in members:
            ...
            if not member.isfile() or name in files:
                raise MigrationError(
                    "Archive contains an unsupported or duplicate entry"
                )
            total += member.size
...
def _inspect(data):
    ...
    if set(manifest) != {"generatedAt", "fileCount", "files"} or not isinstance(
        manifest["generatedAt"], str
    ):
        raise MigrationError("Manifest version or shape is unsupported")
    declared = manifest["files"]
    if (
        not isinstance(declared, dict)
        or type(manifest["fileCount"]) is not int
        or manifest["fileCount"] != len(declared)
    ):
    ...
    data_files = {
        name.removeprefix(f"{root}/data/"): content
    ...
    allowed = re.compile(
        r"^brain/(?:people|projects|ideas|journals|memories|links|buckets|inbox|admin|threads|songs)/(?:index\.json|[A-Za-z0-9_-]{1,128}/index\.json)$"
    )
    song_asset = re.compile(r"^brain/songbook/([A-Za-z0-9][A-Za-z0-9._-]{0,299})$")
    unsupported = sorted(
        name
        for name in data_files
        if not allowed.fullmatch(name) and not song_asset.fullmatch(name)
    )
    if unsupported:
        domains = sorted({name.split("/", 1)[0] for name in unsupported})
        raise MigrationError(
            f"Archive contains unsupported domains: {', '.join(domains)}"
        )
    domains = [
        domain
        for domain in (
        ...
        if f"brain/{domain}/index.json" in data_files
    ]
    if not domains:
    ...
    grouped_import = (
        len(domains) > 1
        and bool(set(domains) & {"songs", "inbox"})
        and set(domains) <= coordinated_families | {"songs", "inbox"}
    )
    if (
        any(
            domain in domains
            for domain in ("people", "projects", "admin", "threads", "songs")
        )
        and len(domains) > 1
        and not (coordinated_records or grouped_import)
    ):
        raise MigrationError(
            "People, projects, tasks, songs and knowledge domains require separate atomic imports"
        )
    for domain in domains:
        index = _json(
            data_files[f"brain/{domain}/index.json"],
            f"{domain.title()} collection index",
        )
        if index.get("schemaVersion") != 1 or index.get("type") != domain:
            raise MigrationError(
                f"{domain.title()} collection schema version is unsupported"
            )
    records = []
    assets = {
        match.group(1): data
        for name, data in data_files.items()
        if (match := song_asset.fullmatch(name))
    }
    for name in sorted(data_files):
        ...
        if record.get("id") != record_id or record.get("_deleted"):
            raise MigrationError(
                f"{domain.title()} record identity or tombstone is unsupported"
            )
        if domain == "people":
            if set(record) - {
                "id",
            ...
            details = []
            for field, label, separator in (
                ("followUps", "Follow-ups", "; "),
                ("tags", "Tags", ", "),
            ):
                if record.get(field):
                    if not isinstance(record[field], list) or not all(
                        isinstance(v, str) for v in record[field]
                    ):
            ...
            candidate = person_values(
                {
                    "name": record.get("name"),
                    "notes": "\n".join(filter(None, [notes, *details])),
                    "identities": [],
                }
            )
            records.append(
                {"domain": domain, "source_id": record_id, "values": candidate}
            )
            if len(records) > MAX_PEOPLE:
                raise MigrationError("Migration is limited to 500 people records")
            continue
        if domain == "projects":
            if not UUID.fullmatch(record_id) or set(record) - {
                "id",
                "name",
                "status",
                "nextAction",
                ...
                raise MigrationError(
                    "Projects record identity or fields are unsupported"
                )
            ...
            if (
                ...
                or len(record["name"]) > 200
                or not isinstance(record.get("nextAction"), str)
                or not record["nextAction"].strip()
                or not isinstance(tags, list)
                or not all(isinstance(v, str) and len(v) <= 50 for v in tags)
            ):
            ...
            for value in (record.get("createdAt"), record.get("updatedAt")):
                try:
                    ...
                    raise MigrationError(
                        "Projects record timestamps are invalid"
                    ) from None
            details = [
                record.get("notes", ""),
            ...
            records.append(
                {
                    ...
                    "values": {
                        ...
                        "status": (
                            "archived" if record["status"] == "done" else "active"
                        ),
        ...
        if domain in ("admin", "threads"):
            ...
            if (
                domain == "admin"
                and not UUID.fullmatch(record_id)
                or domain == "threads"
                and not re.fullmatch(r"[A-Za-z0-9_-]{1,128}", record_id)
                or set(record) - fields
            ):
                raise MigrationError(
                    f"{domain.title()} record identity or fields are unsupported"
                )
            title, notes, next_action = (
                record.get("title"),
                record.get("notes", ""),
            ...
            statuses = (
                ("open", "waiting", "done")
                if domain == "admin"
                else ("open", "waiting", "someday", "done", "archived")
            )
            if (
                ...
                or len(notes) > (5000 if domain == "admin" else 20000)
                or not isinstance(next_action, str)
                or len(next_action) > 500
            ):
            ...
            for value in (
                created,
                updated,
                record.get("dueDate") or record.get("dueAt"),
                record.get("closedAt"),
            ):
                if value is None:
                    continue
                try:
                    ...
                    raise MigrationError(
                        f"{domain.title()} record timestamps are invalid"
                    ) from None
            ...
            if (
                ...
                or len(tags) > 50
                or any(
                    not isinstance(value, str) or not 0 < len(value) <= 50
                    for value in tags
                )
            ...
            for ref in refs:
                if (
                    ...
                    or set(ref) - {"kind", "id", "label"}
                    or ref.get("kind") not in accepted_refs
                    or not isinstance(ref.get("id"), str)
                    or not 0 < len(ref["id"]) <= 2000
                    or not isinstance(ref.get("label", ""), str)
                    or len(ref.get("label", "")) > 300
                ):
                ...
                if ref["kind"] in (
                    ...
                    "jira.issue",
                    "url",
                ) and not re.fullmatch(r"https?://[^\s]+", ref["id"]):
            ...
            }.get(record.get("priority", "normal"))
            if (
                priority is None
                or "pinned" in record
                and type(record["pinned"]) is not bool
            ):
            ...
            if source_state is not None and (
                ...
                or set(source_state) != {"kind", "key"}
                or source_state.get("kind") != "github.issue"
                or not isinstance(source_state.get("key"), str)
                or not 0 < len(source_state["key"]) <= 2000
            ):
                raise MigrationError("Threads source provenance is invalid")
            if record.get("externalState", "unknown") not in (
                "unknown",
                "open",
                "closed",
            ):
            ...
            labels = (
                tags
                + (["someday"] if record["status"] == "someday" else [])
            ...
            records.append(
                {
                    ...
                    "values": {
                        ...
                        "due": record.get("dueDate") or record.get("dueAt") or "",
                        "action_plan": (
                            [
                                {
                                    "content": next_action,
                                    "completed": mapped_status
                        ...
                        "notes": (
                            [
                                {
                                    "content": "Waiting on: " + record["waitingOn"],
                                    "timestamp": updated,
                                }
                            ]
                            if record.get("waitingOn")
                            else []
                        ),
                        "blocked_reason_kind": (
                            "external" if mapped_status == TaskStatus.BLOCKED else ""
                        ),
                        "created_at": created,
                        "updated_at": updated,
                        "refs": refs,
                        "source_state": {
                            key: record[key]
                            for key in ("source", "externalState", "closedAt")
                            if key in record
                        },
                    },
                }
        ...
        if domain == "songs":
            ...
            if (
                not re.fullmatch(r"[A-Za-z0-9_-]{1,128}", record_id)
                or set(record) - fields
            ):
            ...
            if (
                ...
                or not 0 <= capo <= 12
                or not isinstance(tuning, str)
                or len(tuning) > 40
                or not isinstance(source_url, str)
                or len(source_url) > 2000
                or source_url
                and not re.fullmatch(r"https?://[^\s]+", source_url)
                or not isinstance(notes, str)
                or len(notes) > 5000
            ):
                raise MigrationError("Songs record musical fields are invalid")
            parsed_source = urlsplit(source_url) if source_url else None
            if parsed_source and (
                not parsed_source.hostname
            ...
            if (
                ...
                or any(
                    ...
                    or set(link) - {"type", "id", "label"}
                    or not isinstance(link.get("type"), str)
                    or not re.fullmatch(r"[a-z0-9][a-z0-9_-]{0,31}", link["type"])
                    or not isinstance(link.get("id"), str)
                    or not 0 < len(link["id"]) <= 200
                    or not isinstance(link.get("label", ""), str)
                    or len(link.get("label", "")) > 300
                    for link in links
                )
                or scroll is not None
                and (type(scroll) is not int or not 15 <= scroll <= 3600)
            ):
                raise MigrationError("Songs links or scroll duration are invalid")
            attachments = record.get("attachments", [])
            if (
                not isinstance(attachments, list)
                or len(attachments) > MAX_SONG_ATTACHMENTS
            ):
            ...
            for attachment in attachments:
                if (
                    ...
                    or set(attachment)
                    != {"filename", "label", "mime", "size", "sha256"}
                    or not isinstance(attachment.get("filename"), str)
                    or not song_asset.fullmatch(
                        "brain/songbook/" + attachment["filename"]
                    )
                    or not isinstance(attachment.get("label"), str)
                    or len(attachment["label"]) > 300
                    or type(attachment.get("size")) is not int
                    or attachment["size"] < 0
                    or not SHA256.fullmatch(str(attachment.get("sha256", "")))
                ):
                    raise MigrationError("Songs attachment metadata is invalid")
                extension = os.path.splitext(attachment["filename"])[1].lower()
                expected_mime, raw_asset = SONG_MIMES.get(extension), assets.get(
                    attachment["filename"]
                )
                ...
                if raw_asset is None:
                    raise MigrationError(
                        "Songs attachment bytes are missing from this snapshot", 409
                    )
                if (
                    len(raw_asset) != attachment["size"]
                    or hashlib.sha256(raw_asset).hexdigest() != attachment["sha256"]
                ):
                    raise MigrationError(
                        "Songs attachment size or checksum does not match", 409
                    )
                slug = (
                    "legacy-song-"
                    + hashlib.sha256(
                        (record_id + ":" + attachment["filename"]).encode()
                    ).hexdigest()[:32]
                )
                text_kind = {".txt": "text", ".md": "markdown", ".svg": "svg"}.get(
                    extension
                )
                if text_kind:
                    try:
                        ...
                        raise MigrationError(
                            "Text song attachments must be valid UTF-8"
                        ) from None
                planned.append(
                    {
                        **attachment,
                        "slug": slug,
                        "version": 1,
                        "kind": text_kind or kind_for_mime(expected_mime),
                        "bytes": raw_asset,
                    }
                )
            if len({attachment["filename"] for attachment in attachments}) != len(
                attachments
            ):
            ...
            if practice is not None:
                if (
                    ...
                    or type(practice["ease"]) not in (int, float)
                    or not math.isfinite(practice["ease"])
                    or not 1.3 <= practice["ease"] <= 5
                    or type(practice["intervalDays"]) is not int
                    or not 0 <= practice["intervalDays"] <= 36500
                    or type(practice["sessions"]) is not int
                    or practice["sessions"] < 0
                    or type(practice["lastQuality"]) is not int
                    or not 0 <= practice["lastQuality"] <= 5
                ):
                ...
                for value in (practice["nextReview"], practice["lastReviewed"]):
                    try:
                        ...
                        raise MigrationError(
                            "Songs practice timestamps are invalid"
                        ) from None
            for value in (record.get("createdAt"), record.get("updatedAt")):
                try:
                    if (
                    ...
                    raise MigrationError(
                        "Songs record timestamps are invalid"
                    ) from None
            records.append(
                {
                    "domain": domain,
                    ...
                    "values": {
                        ...
                        "notation": content,
                        "notes": notes,
                        "scroll_duration_seconds": scroll,
                        "attachments": planned,
                        "practice": practice,
        ...
        if domain == "buckets":
            ...
            if not UUID.fullmatch(record_id) or set(record) - fields:
                raise MigrationError(
                    "Buckets record identity or fields are unsupported"
                )
            if (
                not isinstance(record.get("name"), str)
                or not record["name"].strip()
                or len(record["name"]) > 100
                or record.get("color")
                not in (
                    None,
                    "accent",
                ...
                or not isinstance(record.get("icon", ""), str)
                or len(record.get("icon", "")) > 50
            ...
            for value in (record.get("createdAt"), record.get("updatedAt")):
                try:
                    ...
                    raise MigrationError(
                        "Buckets record timestamps are invalid"
                    ) from None
            records.append(
                {
                    "domain": domain,
                    ...
                    "values": {
                        "id": record_id,
                        "name": record["name"].strip(),
                        "icon": record.get("icon") or record.get("color") or "",
                        "position": record.get("order", 0),
                        "created_at": record["createdAt"],
                        "updated_at": record["updatedAt"],
                    },
                }
        ...
        if domain == "inbox":
            ...
            if status in ("filed", "corrected", "done"):
                if (
                    not isinstance(filed, dict)
                    or set(filed) != {"destination", "destinationId"}
                    or filed.get("destination") not in ("ideas", "memories", "links")
                    or not UUID.fullmatch(str(filed.get("destinationId", "")))
                ):
                    raise MigrationError(
                        "Routed inbox records require a supported canonical destination"
                    )
            elif filed is not None:
                raise MigrationError(
                    "Unrouted inbox records cannot carry a filed destination"
                )
            error = record.get("error")
            if error is not None and (
                not isinstance(error, dict)
                or set(error) - {"message", "stack"}
            ...
            if ai is not None and (
                ...
                or any(
                    not isinstance(ai.get(key), str)
                    for key in ("providerId", "modelId", "promptTemplateId")
                )
                or "temperature" in ai
                and (
                    type(ai["temperature"]) not in (int, float)
                    or not 0 <= ai["temperature"] <= 2
                )
                or "maxTokens" in ai
                and (type(ai["maxTokens"]) is not int or ai["maxTokens"] <= 0)
            ):
            ...
            if classification is not None and (
                ...
                or classification.get("destination")
                not in (
                    "people",
                    "projects",
                    "ideas",
                ...
                or type(classification.get("confidence")) not in (int, float)
                or not 0 <= classification["confidence"] <= 1
                or not isinstance(classification.get("title"), str)
                or not 0 < len(classification["title"]) <= 200
            ...
            if correction is not None and (
                ...
                or set(correction)
                - {"correctedAt", "previousDestination", "newDestination", "note"}
                or correction.get("previousDestination")
                not in (
                    "people",
                    "projects",
                    "ideas",
                ...
                or correction.get("newDestination")
                not in ("people", "projects", "ideas", "admin", "memories")
            ...
            for value in filter(
                None,
                [
                    correction.get("correctedAt") if correction else None,
                    record.get("sentToCatalogAt"),
                ],
            ):
                try:
                    if (
                        datetime.fromisoformat(value.replace("Z", "+00:00")).utcoffset()
                        is None
                    ):
                        raise ValueError()
            ...
            records.append(
                {
                    ...
                    "values": {
                        ...
                        "status": (
                            "routed"
                            if filed
                            else "error" if status == "error" else "needs_review"
                        ),
        ...
        allowed_fields = common | (
            ...
            if domain == "memories"
                ...
                if domain == "links"
                    ...
                    {"status", "oneLiner", "notes"}
                    if domain == "ideas"
                    else {"date", "content", "segments"}
                )
            )
        )
        if set(record) - allowed_fields:
            raise MigrationError(f"{domain.title()} record contains unsupported fields")
        if domain == "journals" and (
            record_id != record.get("date")
            or not re.fullmatch(r"\d{4}-\d{2}-\d{2}", record_id)
        ):
            raise MigrationError("Journals record identity must match its ISO date")
        title = record.get("title") or (
            f"Journal for {record_id}" if domain == "journals" else None
        )
        content = (
            record.get("content", "")
            if domain in ("memories", "journals")
            else (
                "\n".join(
                    filter(None, [record.get("oneLiner", ""), record.get("notes", "")])
                )
                if domain == "ideas"
                else "\n".join(
                    filter(
                        None, [record.get("description", ""), record.get("note", "")]
                    )
                )
            )
        )
        ...
        if domain == "ideas" and (
            record.get("status") not in ("active", "done")
            or not isinstance(record.get("oneLiner"), str)
            or not record["oneLiner"].strip()
        ):
        ...
        if (
            not isinstance(tags, list)
            or len(tags) > 100
            or not all(isinstance(v, str) and 0 < len(v) <= 50 for v in tags)
        ):
            raise MigrationError(f"{domain.title()} record tags are invalid")
        if domain == "links" and (
            not isinstance(url, str) or not re.fullmatch(r"https?://[^\s]+", url)
        ):
            raise MigrationError("Links record URL is invalid")
        if domain == "links" and (
            record.get("bucketId") is not None
            and not UUID.fullmatch(str(record["bucketId"]))
            or record.get("bucketOrder") is not None
            and type(record["bucketOrder"]) is not int
        ):
        ...
        for value in (created, updated):
            try:
                if (
                    not isinstance(value, str)
                    or datetime.fromisoformat(value.replace("Z", "+00:00")).utcoffset()
                    is None
                ):
                    raise ValueError()
            except (ValueError, OverflowError):
        ...
        if domain == "journals" and (
            ...
            or any(
                ...
                or set(v) - {"text", "at", "source"}
                or not isinstance(v.get("text"), str)
                or not isinstance(v.get("at"), str)
                or v.get("source") not in ("text", "voice", "edit")
                for v in segments
            )
        ):
        ...
        metadata = {
            ...
            "journal_date": record_id if domain == "journals" else None,
            "journal_timezone": "UTC" if domain == "journals" else None,
            "journal_revision": 1 if domain == "journals" else None,
            "segments": segments,
            "bucket_id": record.get("bucketId"),
            "bucket_order": record.get("bucketOrder"),
            "repository": {
                key: record.get(key)
                for key in ("isRepo", "repoHost", "repoOwner", "repoName")
                if key in record
            },
        }
        records.append(
                ...
                "values": {
                    ...
                    "item_type": (
                        ...
                        if domain == "memories"
                            ...
                            "bookmark"
                            if domain == "links"
                            else "fleeting" if domain == "ideas" else "journal"
                        )
                    ),
                    "summary": record.get("description", "")
                    or record.get("oneLiner", ""),
                    "url": normalize_url(url) if url else "",
                    "tags": tags,
                    "provider": "legacy-migration",
                    "source_id": "legacy-archive",
                    "guid": f"{domain}:{record_id}",
                    "file_metadata": metadata,
                    "created_at": created,
                    "updated_at": updated,
                    "is_archived": (
                        1 if domain == "ideas" and record.get("status") == "done" else 0
                    ),
                },
            }
        )
        if domain == "journals":
            records[-1]["values"]["guid"] = "date_journal:" + knowledge_digest(
                [record_id, "UTC"]
            )
    ...
    if coordinated_records or grouped_import:
        canonical_rows = [
            row for row in records if row["domain"] not in ("songs", "inbox")
        ]
        if canonical_rows:
            _ordered_canonical_records(canonical_rows)
        if (
            grouped_import
            and "songs" in domains
            and sum(row["domain"] == "songs" for row in records) != 1
        ):
    ...
    referenced_assets = {
        ...
        for row in records
        if row["domain"] == "songs"
        for attachment in row["values"]["attachments"]
    }
    ...
    token = hashlib.sha256(
        (archive_digest + json.dumps(token_rows, sort_keys=True)).encode()
    ).hexdigest()
...
def preview(data):
        ...
        "records": [
            {
                ...
                "domain": row["domain"],
                "name": row["values"].get("name") or row["values"]["title"],
            }
            for row in records
        ],
    }
    if len(domains) > 1 and set(domains) & {"songs", "inbox"}:
        canonical_domains = [
            domain for domain in domains if domain not in ("songs", "inbox")
        ]
        result["commit_groups"] = (
            (
                [{"id": "canonical", "domains": canonical_domains}]
        ...
        result["completion_policy"] = (
            "Groups commit independently. Completed groups are retained; "
...
def commit(
    ...
    if not isinstance(data, dict) or set(data) != {
        ...
        raise MigrationError(
            "Commit requires the reviewed archive, digest and review token"
        )
    digest, token, records, generated, domains = _inspect(
        {"format": data["format"], "content": data["content"]}
    )
    ...
    if len(domains) > 1 and set(domains) <= coordinated_families:
        if projects is None or tasks is None:
            raise MigrationError(
                "Canonical project and task stores are unavailable", 503
            )
        return _commit_canonical_records(
            tasks, store, knowledge, projects, digest, token, records, generated
        )
    if domains == ["projects"]:
        if projects is None:
    ...
    if domains == ["songs"]:
        if repertoire is None or artifacts is None:
            raise MigrationError(
                "Canonical repertoire and artifact stores are unavailable", 503
            )
    ...
    with closing(store.connect()) as db, db:
        ...
        db.execute(
            "CREATE TABLE IF NOT EXISTS platform_migrations (archive_digest TEXT PRIMARY KEY, review_token TEXT NOT NULL, receipt TEXT NOT NULL)"
        )
        prior = db.execute(
            "SELECT review_token,receipt FROM platform_migrations WHERE archive_digest=?",
            (digest,),
        ).fetchone()
        if prior:
            if prior[0] != token:
        ...
        for row in records:
            ...
            person_id = (
                "migration-"
                + hashlib.sha256((digest + ":" + source_id).encode()).hexdigest()[:32]
            )
            ...
            db.execute(
                "INSERT INTO people VALUES (?,?,?)", (person_id, json.dumps(body), 1)
            )
            imported.append(
                {"source_id": source_id, "person_id": person_id, "revision": 1}
            )
        receipt = {
            "archive_digest": digest,
            "format": FORMAT,
            ...
            "domains": {"people": len(imported)},
            "records": imported,
        }
        db.execute(
            "INSERT INTO platform_migrations VALUES (?,?,?)",
            (digest, token, json.dumps(receipt)),
        )
    return receipt, True
...
def _commit_knowledge(store: KnowledgeStore, digest, token, records, generated):
    ...
    try:
        ...
        for row in (
            ...
            if candidate["domain"] not in ("buckets", "inbox")
        ):
            values = row["values"]
        ...
        for row in (
            candidate for candidate in records if candidate["domain"] == "buckets"
        ):
            values = row["values"]
            if db.execute(
                ...
                raise MigrationError("Migration target identity already exists", 409)
            db.execute(
                "INSERT INTO collections (id,name,kind,query,icon,position,created_at,updated_at) VALUES (?,?,'manual','',?,?,?,?)",
                (
                    values["id"],
                    values["name"],
                    values["icon"],
        ...
        for row in (
            candidate for candidate in records if candidate["domain"] == "links"
        ):
            values = row["values"]
        ...
        for row in (
            candidate for candidate in records if candidate["domain"] == "inbox"
        ):
            values = row["values"]
            if db.execute(
                "SELECT 1 FROM capability_knowledge_captures WHERE id=?",
                (values["id"],),
            ).fetchone():
                raise MigrationError("Migration target identity already exists", 409)
            if (
            ...
            db.execute(
                "INSERT INTO capability_knowledge_captures (id,request_id,input_origin,original_text,captured_at,status,error,revision,destination_id) VALUES (?,?,?,?,?,?,?,?,?)",
                (
                    values["id"],
                    request_id,
                    "text",
            ...
            db.execute(
                "INSERT INTO capability_knowledge_capture_events (capture_id,request_id,event,payload,happened_at) VALUES (?,?,?,?,?)",
                (
                    values["id"],
        ...
        counts = {
            ...
            for domain in sorted(set(row["domain"] for row in records))
        }
        receipt = {
            "archive_digest": digest,
            "format": FORMAT,
...
def _commit_project(
    ...
    if len(records) != 1:
        raise MigrationError(
            "Project archives must contain exactly one project for atomic publication"
        )
    ...
    record_fingerprint = hashlib.sha256(
        json.dumps(project_body, sort_keys=True).encode()
    ).hexdigest()
    if target.exists():
        try:
            ...
            actual_fingerprint = hashlib.sha256(
                json.dumps(
                    json.loads((target / "project.json").read_text()), sort_keys=True
                ).encode()
            ).hexdigest()
        except (OSError, json.JSONDecodeError):
            raise MigrationError(
                "Migration target identity already exists", 409
            ) from None
        marker_fingerprint = prior.get("record_fingerprint")
        if (
            prior.get("archive_digest") != digest
            or prior.get("review_token") != token
            or actual_fingerprint != record_fingerprint
            or marker_fingerprint
            not in (
                (record_fingerprint,)
                if require_fingerprint
                else (None, record_fingerprint)
            )
        ):
            raise MigrationError("Migration target identity already exists", 409)
    ...
    receipt = receipt or {
        ...
        "domains": {"projects": 1},
        "records": [
            {"source_id": row["id"], "project_id": row["id"], "domain": "projects"}
        ],
    }
    ...
    try:
        ...
        with open(
            os.path.join(stage, ".migration-receipt.json"), "w", encoding="utf-8"
        ) as stream:
            json.dump(
                {
                    "archive_digest": digest,
    ...
    for ref in refs:
        ...
        if kind in ("brain.idea", "brain.memory", "brain.link", "brain.journal"):
            if knowledge is None or knowledge.get_item(source_id) is None:
                raise MigrationError(
                    "Thread references a missing canonical knowledge record", 409
                )
        elif kind == "brain.project":
            if projects is None or projects.get_project(source_id) is None:
                raise MigrationError(
                    "Thread references a missing canonical project", 409
                )
            ...
            coordinated = (
                (
                    "migration-"
                    + hashlib.sha256(
                        (archive_digest + ":" + source_id).encode()
                    ).hexdigest()[:32]
                )
                if archive_digest
                else None
            ...
            if len(matches) != 1:
                raise MigrationError(
                    "Thread person reference does not resolve uniquely", 409
                )
            target_id = matches[0]
...
def _commit_task(
    ...
    if len(records) != 1:
        raise MigrationError(
            "Task archives must contain exactly one record for atomic publication"
        )
    ...
    task = Task(
        ...
        notes=values["notes"],
        blocked_reason_kind=values["blocked_reason_kind"],
        evidence=[],
    ...
    if target.exists():
        ...
        actual_fingerprint = hashlib.sha256(
            json.dumps({**body, "evidence": []}, sort_keys=True).encode()
        ).hexdigest()
        marker_fingerprint = marker.get("record_fingerprint")
        if (
            marker.get("archive_digest") != digest
            or marker.get("review_token") != token
            or actual_fingerprint != record_fingerprint
            or marker_fingerprint
            not in (
                (record_fingerprint,)
    ...
    resolved_refs = _resolve_task_refs(
        values["refs"], people, knowledge, projects, tasks, digest
    )
    task.evidence = [
        {
    ...
    descriptor, staging = tempfile.mkstemp(
        prefix=".migration-", suffix=".json", dir=root
    )
    try:
...
def _coordinator_root(tasks):
    root = (
        tasks.home / "capabilities" / "platform" / "migration-journals" / "coordinated"
    )
...
def _read_coordinator_journal(path):
    try:
        if path.stat().st_size > 2 * 1024 * 1024:
            raise MigrationError(
                "A coordinated migration recovery journal exceeds 2 MiB", 409
            )
        journal = json.loads(path.read_text())
    except (OSError, json.JSONDecodeError):
        raise MigrationError(
            "A coordinated migration recovery journal is unreadable", 409
        ) from None
    expected = {
        "schema",
        "kind",
    ...
    if (
        ...
        or journal.get("kind") != "project_task"
        or journal.get("status") not in ("prepared", "project_ready", "complete")
        or not SHA256.fullmatch(str(journal.get("archive_digest", "")))
        or not SHA256.fullmatch(str(journal.get("review_token", "")))
        or not isinstance(journal.get("generated_at"), str)
        ...
        raise MigrationError(
            "A coordinated migration recovery journal has an unsupported shape", 409
        )
    observed_hash = hashlib.sha256(
        json.dumps(journal["records"], sort_keys=True).encode()
    ).hexdigest()
    if observed_hash != journal["records_hash"]:
        raise MigrationError(
            "A coordinated migration recovery journal record plan changed", 409
        )
    domains = [row.get("domain") for row in journal["records"] if isinstance(row, dict)]
    if (
        len(journal["records"]) != 2
        or domains.count("projects") != 1
        or sum(domain in ("admin", "threads") for domain in domains) != 1
        or journal["receipt"].get("archive_digest") != journal["archive_digest"]
    ):
        raise MigrationError(
            "A coordinated migration recovery journal has an unsupported plan", 409
        )
    return journal
...
def _project_state(projects, row, journal):
    ...
    return (
        "owned"
        if (
            body.get("archive_digest") == journal["archive_digest"]
            and body.get("review_token") == journal["review_token"]
            and body.get("record_fingerprint") == actual_hash
        )
        else "drift"
    )
...
def _task_state(tasks, row, journal):
    ...
    return (
        "owned"
        if (
            marker.get("archive_digest") == journal["archive_digest"]
            and marker.get("review_token") == journal["review_token"]
            and marker.get("record_fingerprint") == actual_hash
        )
        else "drift"
    )
...
def _compensate_project_task(journal, tasks, projects):
    ...
    task = next(
        row for row in journal["records"] if row["domain"] in ("admin", "threads")
    )
    task_id, project_id = task["values"]["id"], project["values"]["id"]
    task_state, project_state = _task_state(tasks, task, journal), _project_state(
    ...
    if task_state == "owned":
        if not TaskMutation(tasks).delete(task_id):
            raise MigrationError(
                "Coordinated task compensation could not be completed", 409
            )
    if project_state == "owned":
        if not projects.delete_project(project_id):
            raise MigrationError(
                "Coordinated project compensation could not be completed", 409
            )
...
def _recover_coordinated_journals(tasks, people, knowledge, projects):
    for path in _coordinator_root(tasks).glob("*.json"):
        ...
        if journal["status"] == "complete":
            project = next(
                row for row in journal["records"] if row["domain"] == "projects"
            )
            task = next(
                row
                for row in journal["records"]
                if row["domain"] in ("admin", "threads")
            )
            if (
                _project_state(projects, project, journal) != "owned"
                or _task_state(tasks, task, journal) != "owned"
            ):
                raise MigrationError(
                    "A completed coordinated migration has canonical identity drift",
                    409,
                )
        else:
            _resume_coordinated_project_task(
                path, journal, tasks, people, knowledge, projects
            )
    _recover_canonical_journals(tasks, people, knowledge, projects)


def _commit_coordinated_project_task(
    ...
    if len(project_records) != 1 or len(task_records) != 1 or len(records) != 2:
        raise MigrationError(
            "Coordinated project and task archives require exactly one record from each family"
        )
    ...
    if path.exists():
        ...
        if prior["status"] != "complete":
            _resume_coordinated_project_task(
                path, prior, tasks, people, knowledge, projects
            )
        return prior["receipt"], False
    project, task = project_records[0], task_records[0]
    if (
        projects._projects_dir() / project["values"]["id"]
    ).exists() or projects.get_project_by_name(project["values"]["name"]):
        raise MigrationError("Migration target identity already exists", 409)
...
def _ordered_canonical_records(records):
    if not 2 <= len(records) <= 100:
        raise MigrationError(
            "Coordinated archives require between 2 and 100 canonical records"
        )
    ...
    for row in records:
        ...
        if target in targets or key in keys:
            raise MigrationError(
                "Coordinated archive contains a duplicate canonical identity", 409
            )
        targets.add(target)
        keys.add(key)
    people = sorted(
        (row for row in records if row["domain"] == "people"),
        key=lambda row: row["source_id"],
    )
    projects = sorted(
        (row for row in records if row["domain"] == "projects"),
        key=lambda row: row["source_id"],
    )
    collections = sorted(
        (row for row in records if row["domain"] == "buckets"),
        key=lambda row: row["source_id"],
    )
    knowledge = sorted(
        (
    ...
    if len(people) + len(projects) + len(collections) + len(knowledge) + len(
        pending
    ) != len(records):
        raise MigrationError(
...
def _read_canonical_journal(path):
    try:
        if path.stat().st_size > 2 * 1024 * 1024:
            raise MigrationError(
                "A canonical migration recovery journal exceeds 2 MiB", 409
            )
        journal = json.loads(path.read_text())
    except (OSError, json.JSONDecodeError):
        raise MigrationError(
            "A canonical migration recovery journal is unreadable", 409
        ) from None
    expected = {
        "schema",
        "kind",
    ...
    if (
        not isinstance(journal, dict)
        or set(journal) != expected
        or journal.get("schema") != 2
        or journal.get("kind") != "canonical_records"
        or journal.get("status") not in ("prepared", "applying", "complete")
        or not SHA256.fullmatch(str(journal.get("archive_digest", "")))
        or not SHA256.fullmatch(str(journal.get("review_token", "")))
        or not isinstance(journal.get("steps"), list)
        or not 2 <= len(journal["steps"]) <= 100
        or not isinstance(journal.get("receipt"), dict)
        ...
        raise MigrationError(
            "A canonical migration recovery journal has an unsupported shape", 409
        )
    keys = set()
    for step in journal["steps"]:
        ...
        if (
            ...
            or set(step)
            != {"key", "adapter", "domain", "state", "record", "record_hash"}
            or step.get("adapter")
            not in ("person", "project", "collection", "knowledge", "task")
            or step.get("state") not in ("pending", "applied")
            or step.get("domain") not in adapter_for
            or not isinstance(step.get("record"), dict)
            or step["record"].get("domain") != step["domain"]
            or step["adapter"] != adapter_for[step["domain"]]
            or step["key"]
            != step["domain"] + ":" + str(step["record"].get("source_id", ""))
            or not SHA256.fullmatch(str(step.get("record_hash", "")))
            or hashlib.sha256(
                json.dumps(step["record"], sort_keys=True).encode()
            ).hexdigest()
            != step["record_hash"]
            or step["key"] in keys
        ):
            raise MigrationError(
                "A canonical migration recovery journal has an unsupported step", 409
            )
        keys.add(step["key"])
    return journal
...
def _coordinated_person_id(journal, step):
    return (
        "migration-"
        + hashlib.sha256(
            (journal["archive_digest"] + ":" + step["record"]["source_id"]).encode()
        ).hexdigest()[:32]
    )
...
def _person_state(people, step, journal):
    ...
    with closing(people.connect()) as db:
        ...
        touches = db.execute(
            "SELECT count(*) FROM touchpoints WHERE person_id=?", (person_id,)
        ).fetchone()[0]
    if row is None:
        return "absent"
    try:
...
def _knowledge_state(knowledge, step, journal):
    ...
    row = db.execute(
        ...
        "file_metadata,is_archived,created_at,updated_at FROM items WHERE id=?",
        (values["id"],),
    ).fetchone()
    memberships = [
        tuple(candidate)
        for candidate in db.execute(
            "SELECT collection_id,added_at FROM collection_items WHERE item_id=? ORDER BY collection_id",
            (values["id"],),
        )
    ]
    if row is None:
    ...
    expected = {
        ...
        "is_archived": values.get("is_archived", 0),
        "created_at": values["created_at"],
        "updated_at": values["updated_at"],
    }
    actual = {
        key: (metadata if key == "file_metadata" else row[key]) for key in expected
    }
    tags = [
        tuple(candidate)
        for candidate in db.execute(
    ...
    expected_tags = sorted(
        (name, "user", values["created_at"]) for name in set(values["tags"])
    )
    bucket_id = (
        values["file_metadata"].get("bucket_id") if step["domain"] == "links" else None
    )
    expected_memberships = (
        [] if bucket_id is None else [(bucket_id, values["created_at"])]
    )
    return (
        "owned"
        if actual == expected
        and tags == expected_tags
        and memberships == expected_memberships
        else "drift"
    )
...
def _write_journal(path, body):
    descriptor, temporary = tempfile.mkstemp(
        prefix=".journal-", suffix=".json", dir=path.parent
    )
    try:
        with os.fdopen(descriptor, "w", encoding="utf-8") as stream:
