...
def document():
    ...
    if not isinstance(data, dict) or not isinstance(data.get("provider_connections", {}), dict) or not isinstance(data.get("providers", []), list):
...
def projection(data=None):
    ...
    for key, row in sorted(data.get("provider_connections", {}).items()):
        rows.append({**{field: row[field] for field in ("label", "base_url", "credential_ref", "model_access", "revision")}, "id": key, "bindings": [p["name"] for p in providers if p.get("options", {}).get("connection_id") == key]})
    return {"version": 1, "connections": rows, "providers": [{"name": p["name"], "connection_id": p.get("options", {}).get("connection_id")} for p in providers], "credentials": [credential.name for credential in CredentialStore(config_dir()).list()]}
...
def mutate(identifier, body, *, operation="save", provider=None):
    ...
    bindings = [p for p in data.get("providers", []) if p.get("options", {}).get("connection_id") == identifier]
    if operation == "save":
        ...
        if url.scheme not in {"http", "https"} or not url.hostname or url.username or url.password or url.query or url.fragment:
            raise ConnectionError("Endpoint must be HTTP(S) without credentials or query parameters")
        if not isinstance(label, str) or not 1 <= len(label.strip()) <= 100:
            raise ConnectionError("Label must contain 1..100 characters")
        if reference is not None and (not isinstance(reference, str) or not CredentialStore(config_dir()).has(reference)):
            raise ConnectionError("Credential reference does not exist")
        if not isinstance(policy, dict) or policy.get("mode") not in {"all", "allow", "deny"}:
            raise ConnectionError("Invalid model access mode")
        patterns = policy.get("patterns", [])
        if not isinstance(patterns, list) or len(patterns) > 32 or any(not isinstance(p, str) or not 1 <= len(p) <= 128 for p in patterns):
            raise ConnectionError("Model patterns must be at most 32 strings of 1..128 characters")
        connections[identifier] = {"label": label.strip(), "base_url": endpoint, "credential_ref": reference, "model_access": {"mode": policy["mode"], "patterns": list(dict.fromkeys(patterns))}, "revision": revision + 1}
    elif operation == "delete":
        if not previous:
            raise ConnectionError("Connection not found", 404)
        if bindings:
            raise ConnectionError("Unbind providers before deleting this connection", 409)
        del connections[identifier]
    elif operation == "bind":
        if not previous:
            raise ConnectionError("Connection not found", 404)
        row = next((p for p in data.get("providers", []) if p.get("name") == provider), None)
        if row is None:
    ...
    for row in data.get("providers", []):
        ...
        if entry is not None:
            registry._entries[entry.name] = dataclasses.replace(entry, options=dict(row.get("options", {})))
...
def effective_entry(entry):
    ...
    options = {key: value for key, value in entry.options.items() if key not in {"api_key", "apiKey", "endpoint", "base_url"}}
    options.update(base_url=connection["base_url"], endpoint=connection["base_url"], model_access=connection["model_access"])
    return dataclasses.replace(entry, options=options, credential=connection["credential_ref"])
...
class ScopedCatalog(ModelManager):
    ...
    async def list_models(self):
        return [model for model in await self.full_catalog() if allowed(model.id, self.policy)]

    async def search_catalog(self, query):
        return [model for model in await self.catalog.search_catalog(query) if allowed(model.id, self.policy)]
