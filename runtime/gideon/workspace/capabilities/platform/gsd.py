...
def _document(planning, name):
    ...
    relative = Path(name)
    allowed = name in _DOCUMENTS or (
        len(relative.parts) == 3
        and relative.parts[0] == "phases"
...
def projects():
    return {
        ...
        "projects": [
            ...
            for project in HierarchyStore().list_projects()
    ...
    if phase_root.is_dir():
        ...
        for directory in directories:
            if not directory.resolve().is_relative_to(planning):
                raise GsdError("Phase directory leaves the project")
            documents = sorted(
                str(path.relative_to(planning)) for path in directory.glob("*.md")
            )
            names.extend(documents)
            phases.append(
                {
                    "id": directory.name,
                    "plans": [name for name in documents if name.endswith("PLAN.md")],
                    "summaries": [
                        name for name in documents if name.endswith("SUMMARY.md")
                    ],
                }
            )
    if len(names) > 300:
...
async def edit(identifier, body):
    if (
        ...
        or set(body) != {"document", "sha256", "content"}
...
async def request_phase(identifier, body, actor):
    ...
    if (
        not isinstance(body, dict)
        or set(body) != {"phase", "action"}
        or body["action"] not in {"plan", "execute", "verify"}
    ):
    ...
    label = (
        "gsd-"
        + hashlib.sha256(
            f"{identifier}:{body['phase']}:{body['action']}".encode()
        ).hexdigest()[:20]
    )
    async with _lock:
        ...
        task_list = next(
            (row for row in lists if row.name == "GSD"), None
        ) or store.create_task_list("GSD", project_id=identifier)
        tasks, total = await provider.list_tasks(
            project=current["project"]["name"], limit=500
        )
        if total > 500:
            raise GsdError(
                "Project task list exceeds bounded phase request search", 409
            )
        existing = next(
            (
                task
                ...
                and task.status.value not in {"done", "cancelled"}
            ),
            None,
        )
        task = existing or await provider.create_task(
            title=f"GSD {body['action']}: {body['phase']}",
            description=f"Explicit request to {body['action']} planning phase {body['phase']}. Read the project .planning artifacts before acting. This task records a request, not completed execution.",
        ...
        return {
            ...
            "execution": "not_started_by_this_request",
        }
