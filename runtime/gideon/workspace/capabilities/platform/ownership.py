...
def view(project_id=None, feature=None):
    ...
    connection = _connect()
    try:
        rows = [dict(row) for row in connection.execute('SELECT * FROM ownership ORDER BY project_id,feature')]
        history = []
        if project_id is not None and feature is not None:
            history = [dict(row) for row in connection.execute('SELECT * FROM events WHERE project_id=? AND feature=? ORDER BY id DESC LIMIT 100', (project_id, feature))]
        return {'version': 1, 'projects': [{'id': project.id, 'name': project.name} for project in projects], 'ownership': rows, 'history': history}
    finally:
        connection.close()
...
def mutate(body, actor):
    ...
    if not isinstance(body, dict) or set(body) != fields:
        raise OwnershipError('Project, feature, action, revision and request ID are required')
    project, feature, action, revision, request = (body[key] for key in ('project_id', 'feature', 'action', 'revision', 'request_id'))
    if not isinstance(project, str) or HierarchyStore().get_project(project) is None:
        raise OwnershipError('Unknown project', 404)
    if not isinstance(feature, str) or not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._/-]{0,99}', feature):
    ...
    if not isinstance(request, str) or not re.fullmatch(r'[A-Za-z0-9-]{8,100}', request):
        raise OwnershipError('Invalid request ID')
    fingerprint = hashlib.sha256(json.dumps([actor, body], sort_keys=True).encode()).hexdigest()
    connection = _connect()
    try:
        connection.execute('BEGIN IMMEDIATE')
        replay = connection.execute('SELECT * FROM requests WHERE id=?', (request,)).fetchone()
        if replay:
            if replay['fingerprint'] != fingerprint:
                raise OwnershipError('Request ID was already used for different input or actor', 409)
            return json.loads(replay['response'])
        row = connection.execute('SELECT * FROM ownership WHERE project_id=? AND feature=?', (project, feature)).fetchone()
        ...
        result = {'project_id': project, 'feature': feature, 'owner': actor if action == 'claim' else None, 'revision': revision + 1}
        connection.execute('INSERT INTO ownership VALUES(?,?,?,?) ON CONFLICT(project_id,feature) DO UPDATE SET owner=excluded.owner,revision=excluded.revision', tuple(result.values()))
        connection.execute('INSERT INTO events(project_id,feature,actor,action,revision,at) VALUES(?,?,?,?,?,?)', (project, feature, actor, action, result['revision'], datetime.now(timezone.utc).isoformat()))
        connection.execute('INSERT INTO requests VALUES(?,?,?)', (request, fingerprint, json.dumps(result)))
        connection.commit()
        return result
    except Exception:
        connection.rollback()
