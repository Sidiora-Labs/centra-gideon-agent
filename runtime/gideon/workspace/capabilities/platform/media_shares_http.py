        ...
        try:
            ...
            if request.method == "GET":
                return web.json_response(
                    await asyncio.to_thread(service.list),
                    headers={"Cache-Control": "no-store"},
                )
            return web.json_response(
                await service.share(await read_json_body(request)),
                status=202,
                headers={"Cache-Control": "no-store"},
            )
        except MediaShareError as exc:
    ...
    async def revoke(request):
        try:
            ...
            if set(body) != {"revision"}:
                raise MediaShareError("Revocation requires revision only")
            return web.json_response(
                await service.revoke(request.match_info["id"], body["revision"]),
                headers={"Cache-Control": "no-store"},
            )
        except MediaShareError as exc:
            return web.json_response({"error": str(exc)}, status=exc.status)
    ...
    async def receive(request):
        try:
            ...
            if (
                ...
                raise MediaShareError("Signed media envelope is invalid")
            peer = await asyncio.to_thread(peers.verify_proof, body["proof"])
            return web.json_response(
                await asyncio.to_thread(service.receive, peer["id"], body["payload"]),
                headers={"Cache-Control": "no-store"},
            )
        except (MediaShareError, PeerError) as exc:
            return web.json_response(
