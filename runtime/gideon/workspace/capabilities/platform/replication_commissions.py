...
def _brief(value):
    ...
    for key, item in value["constraints"].items():
        _text(key, "brief constraint key", 80, True)
        _text(item, "brief constraint value", 500)
...
def _sources(values):
    ...
    for source in values:
        ...
        if source["kind"] not in ("work", "series") or not _positive(
            source["revision"]
        ):
            raise ValueError("Invalid commission source pin")
        key = (
            source["kind"],
            _identifier(source["id"], "source identity"),
            source["revision"],
        )
        if key in seen:
...
def _steps(values):
    ...
    for step in values:
        ...
        if (
            identity in seen
            or step["operation"] not in OPERATIONS
            or not isinstance(step["depends_on"], list)
        ):
            raise ValueError("Invalid commission plan step")
        dependencies = [
            _identifier(item, "step dependency") for item in step["depends_on"]
        ]
...
def read_rows(home, entry_id):
    ...
    with sqlite3.connect(_path(home)) as database:
        rows = database.execute(
            "SELECT id,record FROM creative_commissions ORDER BY id"
        ).fetchall()
    result = [
        {"id": identity, "data": _project(json.loads(encoded))}
...
def _safe_authority(record):
    return (
        record.get("cadence") == _SAFE_CADENCE
        and record.get("enabled") is False
        and record.get("max_attempts") == 1
        and record.get("mode") == "planning"
        and record.get("dispatch") is None
        and record.get("schedule_revision") == 1
        and record.get("schedule_error") == ""
        and record.get("schedule_state") == "disabled"
        and record.get("next_fire_at") == ""
    )
...
def _full(home, entity_id):
    with sqlite3.connect(_path(home)) as database:
        row = database.execute(
            "SELECT record FROM creative_commissions WHERE id=?", (entity_id,)
        ).fetchone()
...
def write_row(home, entry_id, row, entity_id):
    ...
    with sqlite3.connect(path) as database:
        if row is None:
            database.execute(
                "DELETE FROM creative_commissions WHERE id=?", (entity_id,)
            )
            return
        _exact(row, ("id", "data"), "row")
        if row["id"] != entity_id:
        ...
        record = (
            _safe_record(row["data"])
            if prior is None
            else {**json.loads(prior[0]), **row["data"]}
        )
        database.execute(
            "INSERT INTO creative_commissions VALUES(?,?) ON CONFLICT(id) DO UPDATE SET record=excluded.record",
...
def apply_rows(home, entry_id, remote_rows, ancestors, queue, now):
    ...
    for entity_id in sorted(set(local) | set(remote) | set(ancestors)):
        ...
        if local_sha == remote_sha:
            continue
        authority_changed = local_row is not None and not _safe_authority(
            _full(home, entity_id)
        )
        divergent = (
            local_row is not None
            and remote_row is not None
            and (not ancestor or (local_sha != ancestor and remote_sha != ancestor))
        )
        unsafe_delete = (
            remote_row is None
            and ancestor
            and local_sha == ancestor
            and authority_changed
        )
        if (
            divergent
            or unsafe_delete
            or (ancestor and local_sha != ancestor and remote_sha != ancestor)
        ):
...
def restore_fields(home, conflict_id, fields, now):
    ...
    if (
        ...
        or any(field not in remote for field in fields)
    ):
