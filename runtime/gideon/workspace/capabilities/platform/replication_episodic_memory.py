...
from gideon.cognition.archive_episode_import import AuthoredEpisodeImport, validate_authored_episode
...
def _rows(rows):
    ...
    for row in rows:
        ...
        if not isinstance(row.get("data"), dict) or row["data"].get("id") != row.get("id"):
    ...
    try:
        ...
        for row in records:
            event = store.db.execute(
                ...
                "AND event_type IN ('create','import') ORDER BY id LIMIT 1", (row["id"],),
            ).fetchone()
            if event is None or event["source"] not in {"user_explicit", "user", "vault_edit"}:
                continue
            latest = store.db.execute(
                "SELECT created_at FROM memory_events WHERE memory_type='episodic' AND memory_key=? "
                "AND event_type IN ('create','import','delete') ORDER BY id DESC LIMIT 1", (row["id"],),
            ).fetchone()
            data = {"id": row["id"], "conversation_id": row["conversation_id"] or "", "text": row["text"],
                    "tags": json.loads(row["tags"] or "[]"), "importance": row["importance"],
                    "source": event["source"], "contributor": row["contributor"] or "",
                    "created_at": row["created_at"], "updated_at": latest["created_at"] if latest else row["created_at"],
...
def apply_rows(home, entry_id, remote_rows, ancestors, queue, now):
    if entry_id != ENTRY_ID or not isinstance(ancestors, dict):
        raise ValueError("Unknown episodic memory replication entry")
    remote, local = _rows(remote_rows), {row["id"]: row for row in read_rows(home, ENTRY_ID)}
    actions, held = {}, []
    added = removed = 0
    for identity in sorted(set(local) | set(remote) | set(ancestors)):
        local_row, remote_row, ancestor = local.get(identity), remote.get(identity), ancestors.get(identity, "")
        ...
        if local_row is not None:
            ...
            if any(before[key] != after[key] for key in immutable) or before["is_deleted"] or not after["is_deleted"]:
                held.append(_conflict(identity, ancestor, local_row, remote_row, now))
                continue
            removed += 1
        else:
...
def restore_fields(home, record_id, fields, now):
    raise ValueError("Authored episodic memories are immutable; resolve the identity collision without field merge")
