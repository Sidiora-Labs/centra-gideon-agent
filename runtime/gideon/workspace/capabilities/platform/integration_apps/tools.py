...
class IntegrationAppTools(ToolProvider):
    ...
    async def list_tools(self):
        ...
        return [
            ToolDefinition(
                name="integration_apps_overview",
                provider=self.name,
                description="Read configured integration names and durable action receipts without secrets.",
                parameters={
                    "type": "object",
                    "properties": {},
            ...
            ToolDefinition(
                name="integration_apps_prepare",
                provider=self.name,
                description="Persist an exact reviewed API request without contacting the provider.",
                parameters={
                    "type": "object",
                    "properties": {
                    ...
                    "required": ["connection_id", "request_id", "operation", "input"],
                    "additionalProperties": False,
                },
                requires_approval=False,
                risk_level=RiskLevel.SAFE,
            ...
            ToolDefinition(
                name="integration_apps_execute",
                provider=self.name,
                description="Execute one previously reviewed API request and retain its remote receipt.",
                parameters={
                    "type": "object",
                    "properties": {
                        "run_id": {"type": "string"},
    ...
    async def invoke(self, tool_name, arguments):
        try:
            if tool_name == "integration_apps_overview" and arguments == {}:
                ...
                tool_name == "integration_apps_prepare"
                and isinstance(arguments, dict)
                and set(arguments)
                == {"connection_id", "request_id", "operation", "input"}
            ):
                result = self.store.prepare(
                    arguments["connection_id"],
                    {
                        key: arguments[key]
                        for key in ("request_id", "operation", "input")
                    },
                )
            elif (
                tool_name == "integration_apps_execute"
                and isinstance(arguments, dict)
                and set(arguments) == {"run_id", "revision"}
            ):
                result = await self.store.execute(
                    arguments["run_id"],
                    {"revision": arguments["revision"], "confirm": True},
                )
            else:
