...
def document(base_dir=None):
    ...
    if not isinstance(value, dict) or not isinstance(value.get('policies'), dict) or type(value.get('revision')) is not int:
        raise ValueError('Cadence policy is unreadable')
    return value


def signature(trigger):
    return hashlib.sha256(json.dumps(trigger.workflow, sort_keys=True).encode()).hexdigest()


def eligible(trigger):
    return trigger.kind == 'clock' and trigger.spec.get('kind') == 'interval' and float(trigger.spec.get('interval_secs', 0)) > 0


def mutate(identifier, body, base_dir=None):
    if not isinstance(body, dict) or set(body) != {'revision', 'enabled', 'task_class'} or type(body['enabled']) is not bool:
    ...
    if body['enabled']:
        if not isinstance(body['task_class'], str) or not re.fullmatch('[a-z][a-z0-9_-]{0,63}', body['task_class']):
            raise ValueError('Task class must be a stable lowercase identifier')
        data['policies'][identifier] = {'task_class': body['task_class'], 'binding_signature': signature(loaded.trigger)}
    else:
...
def outcome(row):
    if row.get('trigger') in {'ok', 'ran', 'ran_late'} and row.get('status') == 'success':
        return True
...
def projection(trigger, *, now=None, base_dir=None):
    ...
    result = {'trigger_id': trigger.id, 'name': trigger.name, 'base_interval': base, 'effective_interval': base, 'multiplier': 1, 'task_class': None, 'samples': 0, 'successes': 0, 'excluded': 0, 'confidence': 'insufficient', 'reason': 'not_opted_in', 'evidence': []}
    try:
        ...
        for item in TriggerStore(_root(base_dir)).list_triggers(include_broken=False):
            ...
            if not peer or peer['task_class'] != policy['task_class'] or peer['binding_signature'] != signature(item) or not eligible(item):
                continue
            ...
            for row in rows:
                ...
                if verdict is None:
                    ...
                    samples.append({'run_id': row['run_id'], 'trigger_id': item.id, 'finished_at': row['finished_at'], 'success': verdict})
        samples = sorted(samples, key=lambda row: (row['finished_at'], row['run_id']), reverse=True)[:20]
        result.update(samples=len(samples), successes=sum(row['success'] for row in samples), evidence=samples, reason='insufficient_samples')
        if len(samples) < 5:
            return result
        ...
        elif result['successes'] / len(samples) < .5:
            effective = max(base, min(base * 4, 86400))
            result.update(reason='low_execution_success', effective_interval=effective, multiplier=effective / base)
        else:
...
def view(*, now=None, base_dir=None):
    data = document(base_dir)
    return {'version': 1, 'revision': data['revision'], 'triggers': [projection(trigger, now=now, base_dir=base_dir) for trigger in TriggerStore(_root(base_dir)).list_triggers(include_broken=False) if eligible(trigger)]}
