...
from gideon.workspace.capabilities.creative.direction import DirectionStore, OPERATIONS
...
_PROJECT_KEYS = {"id", "revision", "name", "treatment", "sources", "steps", "status", "created_at", "updated_at"}
_SOURCE_KEYS = {"kind", "id", "revision", "title", "chapters"}
_CHAPTER_KEYS = {"chapter_id", "title", "work_id", "work_revision", "draft_id", "artifact_id", "artifact_version", "content_hash"}
_STEP_KEYS = {"id", "title", "operation", "depends_on", "status", "result", "attempts"}
_PRIVATE_KEYS = {"request_id", "credential", "credential_ref", "secret", "token", "trigger_id", "import_id", "bytes"}
...
def _contains_private(value: object) -> bool:
    if isinstance(value, dict):
        return bool(_PRIVATE_KEYS & set(value)) or any(_contains_private(item) for item in value.values())
...
def _chapter(value: object) -> dict:
    ...
    if not isinstance(value["content_hash"], str) or not _SHA256.fullmatch(value["content_hash"]):
        raise ValueError("Direction source content hash must be SHA-256")
    return value


def _source(value: object) -> dict:
    if not isinstance(value, dict) or set(value) != _SOURCE_KEYS or value["kind"] not in {"work", "series"}:
    ...
    for chapter in chapters:
        _chapter(chapter)
    if value["kind"] == "work" and (len(chapters) != 1 or chapters[0]["chapter_id"] is not None):
        raise ValueError("A work source requires one direct manuscript pin")
    if value["kind"] == "series" and any(chapter["chapter_id"] is None for chapter in chapters):
...
def _verified_result(value: object, sources: list[dict]) -> None:
    ...
    expected = [{"kind": source["kind"], "id": source["id"], "revision": source["revision"]} for source in sources]
...
def _artifact_result(value: object) -> None:
    if not isinstance(value, dict) or set(value) != {"artifact_id", "artifact_version", "content_hash", "path"}:
...
def _steps(values: object, sources: list[dict]) -> list[dict]:
    ...
    for step in values:
        ...
        if not isinstance(dependencies, list) or len(dependencies) != len(set(dependencies)) or any(identifier(item) not in seen for item in dependencies):
        ...
        if step["status"] == "pending":
            if step["result"] is not None or attempts != 0:
                raise ValueError("Pending direction steps cannot claim execution output")
        elif step["status"] == "skipped":
            if step["result"] is not None:
                raise ValueError("Skipped direction steps cannot claim execution output")
...
def validate_row(row: object) -> dict:
    if not isinstance(row, dict) or set(row) != {"id", "data"} or not isinstance(row["data"], dict):
        raise ValueError("Invalid canonical direction project row")
    identity = identifier(row["id"])
    data = row["data"]
    if set(data) != _PROJECT_KEYS or data.get("id") != identity or _contains_private(data):
    ...
    if data["status"] == "completed" and any(step["status"] not in {"done", "skipped"} for step in steps):
    ...
    if datetime.fromisoformat(data["updated_at"]) < datetime.fromisoformat(data["created_at"]):
...
def validate_entries(entries: object) -> None:
    if not isinstance(entries, list) or len(entries) != 1 or not isinstance(entries[0], dict):
        raise ValueError("Creative direction replication requires complete entry coverage")
    entry = entries[0]
...
def read_rows(home: Path, entry_id: str) -> list[dict]:
    ...
    with store.db() as database:
        rows = database.execute("SELECT id,record FROM direction_projects ORDER BY id").fetchall()
...
def write_row(home: Path, entry_id: str, row: dict | None, entity_id: str) -> None:
    ...
    with store.db() as database:
        ...
        database.execute("INSERT INTO direction_projects(id,record) VALUES(?,?) ON CONFLICT(id) DO UPDATE SET record=excluded.record",
                         (identity, json.dumps(row["data"], sort_keys=True)))


def _conflict(entity_id: str, ancestor: str, local: dict, remote: dict, now: str) -> conflicts.ConflictRecord:
...
def apply_rows(home: Path, entry_id: str, remote_rows: list[dict], ancestors: dict[str, str],
               queue: conflicts.ConflictQueue, now: str) -> ApplyResult:
...
def restore_fields(home: Path, conflict_id: str, fields: list[str], now: str) -> dict:
    ...
    if not fields or len(fields) > 50 or any(not isinstance(field, str) or field in forbidden for field in fields):
