...
def dependents(runner):
    ...
    if runner.adapter:
        binaries = {
            str(
                (
                    runners.managed_adapter_prefix() / "node_modules/.bin" / name
                ).resolve()
            )
            for name in runner.adapter.bin_names
        }
    result = []
        ...
        rows.append(
            {
                ...
                "package": pin.npm_pkg if pin else None,
                "installed": facts or None,
                "latest": None,
...
async def change(identifier, body):
    ...
    async with _lock:
        ...
        if (
            action == "install"
            and facts
            or action in {"update", "remove"}
            and not facts
        ):
            raise HarnessError("Operation does not match installed state", 409)
        if dependents(runner):
            raise HarnessError(
                "Disable the dependent runtime before changing its adapter", 409
            )
        ...
        if action == "remove":
            ...
            if metadata.get("version") != version or not integrity.startswith(
                "sha512-"
            ):
                raise HarnessError(
                    "Registry did not provide exact package integrity", 502
                )
            await _npm(
                "install",
                "--prefix",
            ...
            if not runners.record_provenance(
                pin.npm_pkg,
                pin=runners.AdapterPin(
                    npm_pkg=pin.npm_pkg, version=version, integrity=integrity
                ),
            ):
                raise HarnessError(
                    "Installed package did not match the resolved integrity", 502
                )
        return inventory()
