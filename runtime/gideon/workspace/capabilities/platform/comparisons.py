...
def view(run_id=None):
    ...
    for row in (table or {}).get('rows', []):
        rows.append({key: row.get(key) for key in ('rubric_class', 'tier', 'samples', 'agreement', 'cost_usd', 'wall_secs', 'scored_cells', 'verifier_absent', 'protocol_errors')})
    return {'version': 1, 'imports': _records(), 'runs': runs, 'selected_run': selected,
            'benchmark': {'origin': 'recorded_judge_benchmark', 'rows': rows, 'served_model': None, 'usage_basis': 'not_recorded_in_table'} if table else None}


def add(body):
    keys = ('model', 'corpus', 'metric', 'source', 'observed_at', 'methodology')
    if not isinstance(body, dict) or set(body) != {*keys, 'score'}:
        raise ComparisonError('Model, corpus, metric, score, source, observed date and methodology are required')
    result = {}
    for key in keys:
        value = body[key]
        if not isinstance(value, str) or not value.strip() or len(value) > (2000 if key == 'methodology' else 500):
    ...
    result.update(id=uuid4().hex, score=score, origin='imported', verification='user_attributed', usage_basis='not_provided')
