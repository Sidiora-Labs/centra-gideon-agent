...
class SubscriptionQuotaStore:
    def __init__(self, home):
        ...
        with self.connection() as db:
            db.executescript(
                """CREATE TABLE IF NOT EXISTS quota_plans(id TEXT NOT NULL,revision INTEGER NOT NULL,data TEXT NOT NULL,PRIMARY KEY(id,revision));
    ...
    def _plan(self, db, identity):
        row = db.execute(
            "SELECT data FROM quota_plans WHERE id=? ORDER BY revision DESC LIMIT 1",
            (identity,),
        ).fetchone()
        if row is None:
    ...
    def _reservation(self, db, identity):
        row = db.execute(
            "SELECT data FROM quota_reservations WHERE id=? ORDER BY revision DESC LIMIT 1",
            (identity,),
        ).fetchone()
        if row is None:
    ...
    def _request(self, db, request_id, kind, payload, execute):
        ...
        prior = db.execute(
            "SELECT kind,payload,result FROM quota_requests WHERE id=?", (request_id,)
        ).fetchone()
        if prior:
        ...
        result = execute()
        db.execute(
            "INSERT INTO quota_requests VALUES(?,?,?,?)",
            (request_id, kind, encoded, json.dumps(result)),
        )
        return result
    ...
    def create_plan(self, payload):
        ...
        with self.connection() as db:
            ...
            def execute():
                start, end = _timestamp(
                    payload["cycle_start"], "cycle_start"
                ), _timestamp(payload["cycle_end"], "cycle_end")
                if end <= start:
                    raise QuotaError("cycle_end must be after cycle_start")
                tokens = _amount(
                    payload["token_limit"], "token_limit", integer=True, optional=True
                )
                dollars = _amount(
                    payload["dollar_limit"], "dollar_limit", optional=True
                )
                if tokens is None and dollars is None:
                    raise QuotaError("At least one configured quota limit is required")
                monthly = _amount(
                    payload.get("monthly_cost_usd"), "monthly_cost_usd", optional=True
                )
                stamp = _now()
                row = {
                    "id": str(uuid4()),
                    "revision": 1,
                    "provider": _text(payload["provider"], "provider", 200),
                    "name": _text(payload["name"], "name", 200),
                    "source": _text(payload["source"], "source", 500),
                    ...
                    "archived": False,
                    "quota_basis": "user_configured",
                    "created_at": stamp,
                    "updated_at": stamp,
                }
                db.execute(
                    "INSERT INTO quota_plans VALUES(?,?,?)",
                    (row["id"], 1, json.dumps(row)),
                )
                return row

            return self._request(
                db, payload["request_id"], "quota-plan-create", payload, execute
            )
        ...
        with self.connection() as db:
            ...
            def execute():
                ...
                for key in ("name", "source"):
                    if key in payload:
                        row[key] = _text(
                            payload[key], key, 500 if key == "source" else 200
                        )
                for key, integer in (
                    ("token_limit", True),
                    ("dollar_limit", False),
                    ...
                    if key in payload:
                        row[key] = _amount(
                            payload[key], key, integer=integer, optional=True
                        )
                ...
                db.execute(
                    "INSERT INTO quota_plans VALUES(?,?,?)",
                    (identity, row["revision"], json.dumps(row)),
                )
                return row
        ...
        with self.connection() as db:
            return [
                ...
                for row in db.execute(
                    'SELECT p.data FROM quota_plans p WHERE p.revision=(SELECT MAX(s.revision) FROM quota_plans s WHERE s.id=p.id) ORDER BY json_extract(p.data,"$.name")'
                )
            ]
    ...
    def history(self, identity):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT data FROM quota_plans WHERE id=? ORDER BY revision",
                    (identity,),
                )
            ]
    ...
    def _usage(self, plan):
        ...
        for row in UsageJournal(self.home / "usage" / "turns.jsonl").rows():
            ...
            if (
                ...
                or not plan["cycle_start"] <= str(row.get("ts", "")) < plan["cycle_end"]
            ):
                continue
            tokens += sum(
                max(0, int(row.get(key, 0) or 0))
                for key in (
                    "input_tokens",
                    "output_tokens",
                    "cache_read_tokens",
    ...
    def _latest_reservations(self, db, plan_id):
        return [
            json.loads(row[0])
            for row in db.execute(
                "SELECT r.data FROM quota_reservations r WHERE r.plan_id=? AND r.revision=(SELECT MAX(s.revision) FROM quota_reservations s WHERE s.id=r.id) ORDER BY r.rowid",
                (plan_id,),
            )
        ]
    ...
    def summary(self, identity, at=None):
        ...
        with self.connection() as db:
            ...
            active = [
                ...
                if row["status"] == "held"
                and _timestamp(row["expires_at"], "expires_at") > instant
            ]
            ...
            available_tokens = (
                None
                if plan["token_limit"] is None
                else max(0, plan["token_limit"] - usage["tokens"] - reserved_tokens)
            )
            available_dollars = (
                None
                if plan["dollar_limit"] is None
                else round(
                    max(
                        0.0, plan["dollar_limit"] - usage["dollars"] - reserved_dollars
                    ),
                    6,
                )
            )
            return {
                ...
                "reservations": {
                    "active": len(active),
                    "expired_unreleased": sum(
                        row["status"] == "held" and row not in active
                        for row in reservations
                    ),
                    "reserved_tokens": reserved_tokens,
                    "reserved_dollars": reserved_dollars,
                },
                "available": {"tokens": available_tokens, "dollars": available_dollars},
                "provider_evidence": self._latest_snapshot(db, identity),
                "coverage": "Configured availability uses owner-supplied ceilings, canonical local usage and active planning reservations. It does not grant provider entitlement or runtime admission. Provider evidence is shown only when a provider adapter recorded it; no vendor quota is inferred.",
            }
    ...
    def reserve(self, identity, payload):
        ...
        with self.connection() as db:
            ...
            def execute():
                ...
                if plan["archived"]:
                    raise QuotaError(
                        "Archived quota plans cannot reserve capacity", 409, "conflict"
                    )
                expiry = _timestamp(payload["expires_at"], "expires_at")
                instant = datetime.now(timezone.utc)
                if not instant < expiry <= _timestamp(plan["cycle_end"], "cycle_end"):
                    raise QuotaError(
                        "Reservation expiry must be future and within plan cycle"
                    )
                expected_tokens = (
                    0
                    if payload["expected_tokens"] == 0
                    else _amount(
                        payload["expected_tokens"], "expected_tokens", integer=True
                    )
                )
                expected_dollars = (
                    0.0
                    if payload["expected_dollars"] == 0
                    else _amount(payload["expected_dollars"], "expected_dollars")
                )
                ...
                if (
                    current["available"]["tokens"] is not None
                    and expected_tokens > current["available"]["tokens"]
                ):
                    raise QuotaError(
                        "Configured token quota is exhausted", 409, "quota_exhausted"
                    )
                if (
                    current["available"]["dollars"] is not None
                    and expected_dollars > current["available"]["dollars"]
                ):
                    raise QuotaError(
                        "Configured dollar quota is exhausted", 409, "quota_exhausted"
                    )
                stamp = _now()
                row = {
                    "id": str(uuid4()),
                    "revision": 1,
                    "plan_id": identity,
                    "run_id": _text(payload["run_id"], "run_id", 200),
                    "purpose": _text(payload["purpose"], "purpose", 500),
                    "expected_tokens": expected_tokens,
                    "expected_dollars": expected_dollars,
                    "expires_at": expiry.isoformat(),
                    "status": "held",
                    "evidence_basis": "local_planning_only",
                    "created_at": stamp,
                    "updated_at": stamp,
                }
                db.execute(
                    "INSERT INTO quota_reservations VALUES(?,?,?,?)",
                    (row["id"], 1, identity, json.dumps(row)),
                )
                return row
        ...
        with self.connection() as db:
            ...
            def execute():
                ...
                row.update(
                    ...
                    status="released",
                    release_reason=_text(payload["reason"], "reason", 500),
                    updated_at=_now(),
                )
                db.execute(
                    "INSERT INTO quota_reservations VALUES(?,?,?,?)",
                    (identity, row["revision"], row["plan_id"], json.dumps(row)),
                )
                return row
    ...
    def record_provider_snapshot(self, plan_id, snapshot):
        if not isinstance(snapshot, ProviderQuotaSnapshot):
            raise QuotaError(
                "Provider evidence requires the provider adapter boundary",
                403,
        ...
        with self.connection() as db:
            ...
            if snapshot.provider != plan["provider"]:
                raise QuotaError(
                    "Provider evidence does not match quota plan", 409, "conflict"
                )
            limits = []
            for limit in snapshot.limits:
                ...
                limits.append(
                    {
                        "key": _text(limit["key"], "key", 100),
                        "label": _text(limit["label"], "label", 200),
            ...
            row = {
                ...
                "source": _text(snapshot.source, "source", 500),
                "limits": limits,
                "evidence_basis": "provider_reported",
            }
            db.execute(
                "INSERT INTO quota_snapshots VALUES(?,?,?)",
                (plan_id, captured, json.dumps(row)),
            )
            return row

    def _latest_snapshot(self, db, plan_id):
        row = db.execute(
            "SELECT data FROM quota_snapshots WHERE plan_id=? ORDER BY captured_at DESC LIMIT 1",
            (plan_id,),
        ).fetchone()
    ...
    def provider_evidence(self, plan_id):
        with self.connection() as db:
            ...
            return [
                json.loads(row[0])
                for row in db.execute(
                    "SELECT data FROM quota_snapshots WHERE plan_id=? ORDER BY captured_at DESC",
                    (plan_id,),
                )
            ]
