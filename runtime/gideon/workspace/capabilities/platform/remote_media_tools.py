...
class RemoteMediaTools(ToolProvider):
    ...
    async def list_tools(self):
        return [
            ToolDefinition(
                name="remote_media_list",
                description="List durable remote media executions and eligible direct peers.",
                provider=self.name,
                parameters={
                    "type": "object",
                    "properties": {},
            ...
            ToolDefinition(
                name="remote_media_dispatch",
                description="Submit a bounded image generation request to an authorized direct peer.",
                provider=self.name,
                parameters={
                    "type": "object",
                    "required": ["peer_id", "request_id", "prompt"],
                    "properties": {
                        "peer_id": {"type": "string"},
            ...
            ToolDefinition(
                name="remote_media_cancel",
                description="Cancel an admitted remote media job at its current revision.",
                provider=self.name,
                parameters={
                    "type": "object",
                    "required": ["execution_id", "state_revision"],
                    "properties": {
        ...
        try:
            ...
            if tool_name == "remote_media_list":
                ...
                result = await self.service.cancel(
                    arguments["execution_id"], arguments["state_revision"]
                )
