...
_DRAFT_FIELDS = {"id", "artifact_id", "artifact_version", "note", "created_at", "characters"}
...
def _version_row(row):
    ...
    data = row["data"]
    if not isinstance(data, dict) or set(data) != {"work_id", "revision", "record", "record_sha256"}:
        raise ValueError("Invalid work version data")
    work_id = identifier(data["work_id"])
    revision, record = data["revision"], data["record"]
    if type(revision) is not int or not 1 <= revision <= 2**31 or not isinstance(record, dict):
        raise ValueError("Invalid work version revision")
    if set(record) != _VERSION_FIELDS or record.get("id") != work_id or record.get("revision") != revision:
    ...
    for field in ("author_ref", "universe_ref"):
        ...
        if ref is not None and (not isinstance(ref, dict) or set(ref) != {"id", "revision"}
                or type(ref["revision"]) is not int or not 1 <= ref["revision"] <= 2**31):
    ...
    created, updated = _timestamp(record["created_at"], "work creation timestamp"), _timestamp(record["updated_at"], "work update timestamp")
    if updated < created:
        raise ValueError("Work version update predates creation")
    if row["id"] != _version_id(work_id, revision) or data["record_sha256"] != _sha(record):
...
def _draft_row(row):
    ...
    if not isinstance(data, dict) or set(data) != expected or not isinstance(data["draft"], dict):
        raise ValueError("Invalid authored draft data")
    work_id, draft = identifier(data["work_id"]), data["draft"]
    if set(draft) != _DRAFT_FIELDS or row["id"] != draft.get("id") or _HEX.fullmatch(row["id"]) is None:
        raise ValueError("Invalid authored draft identity")
    if draft["artifact_id"] != "creative-draft-" + row["id"] or draft["artifact_version"] != 1:
...
def validate_entries(entries):
    if (not isinstance(entries, list) or len(entries) != 2
            or [item.get("entry_id") if isinstance(item, dict) else None for item in entries] != list(ENTRIES)
            or any(set(item) != {"entry_id", "rows"} for item in entries)):
        raise ValueError("Creative document replication requires complete ordered coverage")
    ...
    for data in versions.values():
        ...
        if active is not None and (active not in drafts or drafts[active]["work_id"] != data["work_id"]):
...
def read_rows(home, entry_id):
    ...
    with store.connection() as database:
        if entry_id == VERSIONS_ENTRY:
            rows = database.execute("SELECT id,revision,record FROM work_revisions ORDER BY id,revision").fetchall()
            result = []
            for work_id, revision, encoded in rows:
                record = json.loads(encoded)
                data = {"work_id": work_id, "revision": revision, "record": record,
                        "record_sha256": _sha(record)}
                result.append({"id": _version_id(work_id, revision), "data": data})
        elif entry_id == DRAFTS_ENTRY:
            rows = database.execute("SELECT id,work_id,record FROM work_drafts ORDER BY id").fetchall()
            result = []
            for draft_id, work_id, encoded in rows:
                draft = json.loads(encoded)
                artifact = store.artifacts.get(draft["artifact_id"], version=draft["artifact_version"])
                if artifact is None or artifact.content is None or not artifact.readonly or artifact.kind != "markdown":
                    raise ValueError("Canonical authored draft artifact is unavailable")
                data = {"work_id": work_id, "draft": draft, "text": artifact.content,
                        "content_sha256": hashlib.sha256(artifact.content.encode()).hexdigest(),
...
def _preflight(home, entry_id, rows):
    ...
    with store.connection() as database:
        for data in parsed.values():
            work = database.execute("SELECT record FROM works WHERE id=?", (data["work_id"],)).fetchone()
            if not work:
                raise ValueError("Creative document requires its canonical current work")
            if entry_id == VERSIONS_ENTRY:
                ...
                for field, table in (("author_ref", "author_revisions"), ("universe_ref", "universe_revisions")):
                    ...
                    if ref is not None and not database.execute(
                            f"SELECT 1 FROM {table} WHERE id=? AND revision=?", (ref["id"], ref["revision"])).fetchone():
                        raise ValueError("Work version pinned context is unavailable")
            else:
                artifact = store.artifacts.get(data["draft"]["artifact_id"], version=1)
                if artifact is not None and (artifact.content != data["text"] or not artifact.readonly
                        or artifact.kind != "markdown" or artifact.description != data["artifact_description"]):
...
def apply_rows(home, entry_id, remote_rows, ancestors, queue, now):
    ...
    for entity_id in sorted(set(local) | set(remote)):
        ...
        if remote_row is None or (local_row is not None and conflicts.row_sha(local_row) == conflicts.row_sha(remote_row)):
            continue
        if local_row is not None:
            if queue.record(_conflict(entry_id, entity_id, ancestors.get(entity_id, ""), local_row, remote_row, now)):
                conflict_count += 1
            continue
        ...
        try:
            if entry_id == VERSIONS_ENTRY:
                ...
                store.import_draft(data["work_id"], data["draft"], data["text"],
