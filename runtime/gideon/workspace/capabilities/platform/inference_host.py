...
class InferenceHost:
    ...
    def state(self):
        ...
        state.setdefault(
            "runtime",
            {"app": "", "recipe": "", "status": "unconfigured", "evidence": {}},
        )
        return state
    ...
    def view(self):
        ...
        return {
            **state,
            "listening": self.runner is not None,
            "actual_port": self.port,
            "active": len(self.active),
            "queued": 0,
            "runtime_lifecycle": lifecycle,
            "runtime_apps": sorted(row.app for row in runners()),
            "runtime_recipe": RUNTIME_RECIPE,
            "credentials": [row.name for row in CredentialStore(self.home).list()],
    ...
    async def configure(self, body):
        ...
        async with self.lock:
            ...
            if (
                type(body["port"]) is not int
                or not 0 <= body["port"] <= 65535
                or type(body["capacity"]) is not int
                or not 1 <= body["capacity"] <= 8
            ):
            ...
            if any(
                not store.has(ref) or len(store.resolve(ref).secret or "") < 24
                for ref in peers.values()
            ):
                raise HostError(
                    "Peer credentials must resolve to at least 24 characters"
                )
            runtime = body.get(
                "runtime", {key: state["runtime"][key] for key in ("app", "recipe")}
            )
            if (
                not isinstance(runtime, dict)
                or set(runtime) != {"app", "recipe"}
                or any(
                    not isinstance(runtime[key], str) or len(runtime[key]) > 100
                    for key in runtime
                )
            ):
                raise HostError("Runtime binding requires bounded app and recipe names")
            if bool(runtime["app"]) != bool(runtime["recipe"]) or runtime[
                "recipe"
            ] not in ("", RUNTIME_RECIPE):
                raise HostError("Unknown or incomplete runtime recipe")
            runtime = {
                **runtime,
                "status": "configured" if runtime["app"] else "unconfigured",
                "evidence": {},
            }
            self.persist(
                {**state, **body, "runtime": runtime, "revision": state["revision"] + 1}
            )
            return self.view()
    ...
    async def provision(self):
        async with self.lock:
            ...
            if not worker.is_file() or not runner.python_executable().is_file():
                ...
                raise HostError(
                    "Configured inference runtime files are unavailable", 503
                )
            return self._runtime_state(
                "prepared", {"recipe": state["runtime"]["recipe"]}
            )
        ...
        async with self.lock:
            ...
            try:
                result = await runner.acall(
                    "call", {"method": "readiness", "payload": {}}, timeout=10
                )
            except Exception:
            ...
            if not isinstance(result, dict) or result.get("ready") is not True:
                evidence = (
                    {"alive": True, "state": str(result.get("state", "starting"))[:100]}
    ...
    async def _start_runtime(self):
        ...
        runner = self._runtime_runner()
        try:
            loaded = await runner.acall(
                "load", {"recipe": state["runtime"]["recipe"]}, timeout=30
            )
            ...
            runner.stop()
            self._runtime_state(
                "unavailable", {"reason": getattr(exc, "reason", "startup_failed")}
            )
            if isinstance(exc, HostError):
                raise
            raise HostError(
                "Configured inference runtime failed to start", 503
            ) from exc
                ...
                self.persist(
                    {
                        **state,
                        "enabled": False,
                        "revision": state["revision"] + int(state["enabled"]),
                    }
                )
            ...
            if runtime["app"]:
                try:
                    ...
                    if sidecar.is_alive():
                        try:
                            await sidecar.acall(
                                "call", {"method": "stop", "payload": {}}, timeout=15
                            )
                        finally:
    ...
    def peer(self, request):
        ...
        for peer, ref in self.state()["peers"].items():
            if (
                store.has(ref)
                and hmac.compare_digest(token[7:], store.resolve(ref).secret or "")
                and len(token[7:]) >= 24
            ):
                return peer
    ...
    async def complete(self, request):
        ...
        if (
            ...
            or set(body) - {"model", "messages", "stream"}
            or body.get("model") != entry.model
            or body.get("stream", False) is not False
        ):
            raise web.HTTPBadRequest(
                text="Exact configured model and non-streaming request required"
            )
        ...
        if len(self.active) >= state["capacity"]:
            raise web.HTTPTooManyRequests(
                text="Inference admission full", headers={"Retry-After": "1"}
            )
        ...
        try:
            ...
            async with asyncio.timeout(120):
                async for event in provider.complete(messages, tools=[]):
                    if (
                        ...
                        or event.kind == "permission_request"
                    ):
                    ...
                    "choices": [
                        {
                            ...
                            "finish_reason": complete.stop_reason or "stop",
                        }
                    ],
                    "usage": {
                        "prompt_tokens": complete.input_tokens,
                        "completion_tokens": complete.output_tokens,
                        "total_tokens": complete.input_tokens + complete.output_tokens,
                    },
                }
            )
            ...
            raise web.HTTPServiceUnavailable(
                text="Configured inference provider unavailable"
            )
        finally:
            ...
            current["usage"] = (
                ...
                + [
                    dict(
                        ...
                        input_tokens=complete.input_tokens if complete else None,
                        output_tokens=complete.output_tokens if complete else None,
                    )
                ]
            )[-1000:]
            self.persist(current)
