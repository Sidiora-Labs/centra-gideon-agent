...
def _git(path, *args):
    ...
    if result.returncode:
        raise ReferenceError(
            "Git operation failed; check repository and origin access", 409
        )
...
def change(identifier, action, body):
    ...
    if _identity(path, row["branch"]) != row["identity"]:
        raise ReferenceError(
            "Repository origin or branch changed; register a new reference", 409
        )
    if action == "review":
        if (
            not isinstance(body, dict)
            or body.get("head") != (row.get("snapshot") or {}).get("head")
            or not body.get("head")
            or row["stale"]
        ):
    ...
    try:
        ...
        lines = _git(
            path, "log", "-50", "--format=%H%x09%s", revision, "--"
        ).splitlines()
        row["snapshot"] = {
            "head": head,
            "commits": [
                {"sha": line.partition("\t")[0], "subject": line.partition("\t")[2]}
                for line in lines
            ],
            "limit": 50,
        }
