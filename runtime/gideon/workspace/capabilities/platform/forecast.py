...
async def view(*, horizon=3600, now=None):
    ...
    events, exclusions = [], [{'trigger_id': row.trigger.id, 'reason': 'invalid_trigger'} for row in records if not row.ok]
    truncated = False
    for trigger in triggers:
        ...
        if not trigger.enabled or trigger.state != 'active':
            exclusions.append({'trigger_id': trigger.id, 'reason': 'disabled_or_' + trigger.state})
            continue
        if trigger.kind != 'clock':
            exclusions.append({'trigger_id': trigger.id, 'reason': 'event_driven', 'next_at': None})
            continue
        ...
        candidate = arm.CalendarExclusions(predicted).advance(max(now, due)) if due > 0 else arm.next_fire(predicted, now=now)
        count = 0
        while 0 < candidate <= now + horizon and count < 20 and len(events) < 500:
            ...
            if expiry and candidate >= expiry:
                break
            events.append({'trigger_id': trigger.id, 'name': trigger.name, 'at': candidate, 'conditional': True, 'admission_now': admission, 'cadence_reason': cadence['reason'], 'edit_url': '#/triggers?open=' + quote(trigger.id, safe=''), 'duration_seconds': None})
            count += 1
            following = arm.next_fire(predicted, now=candidate + .001, last_fire=candidate)
        ...
        if count >= 20 and candidate <= now + horizon:
            truncated = True
    dependencies = []
    for path in sorted((config_dir() / 'capabilities/platform/maintenance').glob('*.json')):
        ...
        dependencies.append({'maintenance_id': row['id'], 'project_id': row['project_id'], 'status': row['status'], 'child_id': row['child_id'], 'pending_issues': pending, 'issue_count_complete': total <= 1000, 'next_at': None})
    events.sort(key=lambda row: (row['at'], row['trigger_id']))
    return {'version': 1, 'as_of': now, 'horizon': horizon, 'events': events, 'excluded': exclusions, 'dependencies': dependencies, 'truncated': truncated, 'qualification': 'Conditional clock forecast with current admission only; future state and durations are unknown.'}
