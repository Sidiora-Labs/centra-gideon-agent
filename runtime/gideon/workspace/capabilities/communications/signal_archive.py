...
def _decode(data):
    ...
    if not re.fullmatch(r"[0-9a-fA-F]{64}", key):
        raise PeopleError(
            "Signal SQLCipher key must be exactly 64 hexadecimal characters"
        )
    ...
    if raw.startswith(b"SQLite format 3\x00"):
        raise PeopleError(
            "Signal import requires the encrypted SQLCipher database, not plaintext SQLite"
        )
...
def decrypt_sqlcipher4(raw, encryption_key):
    ...
    for page in range(1, len(raw) // PAGE_SIZE + 1):
        ...
        if not hmac.compare_digest(stored, expected):
            raise PeopleError(
                "Signal SQLCipher authentication failed; the key, database, or supported format is wrong"
            )
        decryptor = Cipher(
            algorithms.AES(encryption_key), modes.CBC(raw[iv_start : iv_start + 16])
        ).decryptor()
    ...
    if output[16:18] != b"\x10\x00" or output[20] != RESERVE_BYTES:
        raise PeopleError(
            "Decrypted Signal database does not use the supported SQLCipher-4 page layout"
        )
    return output
...
def _attachment_refs(value):
    ...
    for item in attachments[:100]:
        ...
        result.append(
            {
                "path": text(item.get("path", ""), "attachment path", 2000),
                "name": text(item.get("fileName", ""), "attachment name", 1000),
                "content_type": text(
                    item.get("contentType", ""), "attachment content type", 500
                ),
                "size": (
                    item.get("size")
                    if type(item.get("size")) is int and 0 <= item["size"] <= 10**12
                    else None
                ),
            }
        )
...
def _rows(raw):
        ...
        try:
            ...
            for table, columns in required.items():
                ...
                if not definition or definition[0] != "table" or not columns <= actual:
                    raise PeopleError(
                        "Unsupported Signal Desktop schema: missing required "
                        + table
                        + " fields"
                    )
            values = db.execute(
                """SELECT m.id,m.conversationId,COALESCE(NULLIF(m.sent_at,0),m.received_at),
                m.type,m.body,m.json,CASE WHEN c.type='private' THEN c.e164 ELSE NULL END
                FROM messages m LEFT JOIN conversations c ON c.id=m.conversationId ORDER BY m.ROWID LIMIT 1001"""
            ).fetchall()
        except (sqlite3.Error, TypeError, UnicodeError) as exc:
            raise PeopleError(
                "Unsupported or damaged decrypted Signal database"
            ) from exc
    if len(values) > 1000:
        raise PeopleError(
            "Signal archive contains more than 1000 messages; supply a bounded owned archive"
        )
    rows = []
    seen = set()
    for identifier, conversation, when, kind, body, metadata, handle in values:
        ...
        if provenance in seen:
            raise PeopleError(
                "Signal archive has duplicate conversation message provenance"
            )
        seen.add(provenance)
        try:
            occurred = (
                datetime.fromtimestamp(int(when) / 1000, timezone.utc).isoformat()
                if int(when) > 0
                else None
            )
        ...
        if handle:
            try:
                identity = person_values(
                    {
                        "name": "Signal source",
                        "identities": [{"kind": "phone", "value": handle}],
                    }
                )["identities"][0]
            except PeopleError:
                pass
        ...
        rows.append(
            {
                ...
                "limitations": (
                    [] if direction and occurred else ["unsupported_event_or_timestamp"]
                ),
            }
        )
    return rows
...
def preview(store, data):
    ...
    return {
        ...
        "review_token": review,
        "rows": rows,
        "coverage": "supplied_encrypted_archive",
        "qualification": "authenticated_sqlcipher4",
    }


def schema(db):
    db.execute(
        "CREATE TABLE IF NOT EXISTS signal_imports(account TEXT NOT NULL,digest TEXT NOT NULL,receipt TEXT NOT NULL,UNIQUE(account,digest))"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS signal_messages(account TEXT NOT NULL,external_id TEXT NOT NULL,body TEXT NOT NULL,UNIQUE(account,external_id))"
    )
    evidence.schema(db)
...
def commit(store, data):
    ...
    with closing(store.connect()) as db, db:
        ...
        schema(db)
        old = db.execute(
            "SELECT receipt FROM signal_imports WHERE account=? AND digest=?",
            (account, projected["source_digest"]),
        ).fetchone()
        if old:
        ...
        for row in projected["rows"]:
            canonical = {
                ...
                if key not in ("person_id", "eligible")
            }
            body = json.dumps(canonical, sort_keys=True)
            old_message = db.execute(
                "SELECT body FROM signal_messages WHERE account=? AND external_id=?",
                (account, row["external_id"]),
            ).fetchone()
            if old_message and old_message[0] != body:
                raise PeopleError(
                    "Signal message provenance conflicts with existing history", 409
                )
            if not old_message:
                db.execute(
                    "INSERT INTO signal_messages VALUES (?,?,?)",
                    (account, row["external_id"], body),
                )
                inserted += 1
            if row["eligible"]:
                event = {
                    ...
                    "summary": row["body"][:2000],
                }
                if not db.execute(
                    "SELECT 1 FROM relationship_messages WHERE source=? AND source_account_id=? AND external_id=?",
                    ("signal", account, row["external_id"]),
                ).fetchone():
                    db.execute(
                        "INSERT INTO relationship_messages VALUES (?,?,?,?)",
                        ("signal", account, row["external_id"], json.dumps(event)),
                    )
                    linked += 1
        receipt = {
            "source_account_id": account,
            "source_digest": projected["source_digest"],
            "review_token": projected["review_token"],
        ...
        db.execute(
            "INSERT INTO signal_imports VALUES (?,?,?)",
            (account, projected["source_digest"], json.dumps(receipt)),
        )
    return receipt, True
    ...
    with closing(store.connect()) as db, db:
        ...
        return [
            ...
            for body, in db.execute(
                "SELECT receipt FROM signal_imports ORDER BY rowid DESC"
            )
        ]
...
def history(store, account):
    with closing(store.connect()) as db, db:
        ...
        return [
            json.loads(body)
            for body, in db.execute(
                "SELECT body FROM signal_messages WHERE account=? ORDER BY rowid",
                (account,),
            )
        ]
