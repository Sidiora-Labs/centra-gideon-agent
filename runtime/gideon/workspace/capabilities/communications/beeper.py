...
def schema(db):
    db.execute('CREATE TABLE IF NOT EXISTS beeper_settings (id INTEGER PRIMARY KEY,body TEXT NOT NULL,revision INTEGER NOT NULL)')
    db.execute('CREATE TABLE IF NOT EXISTS beeper_pages (key TEXT PRIMARY KEY,body TEXT NOT NULL)')
    db.execute('CREATE TABLE IF NOT EXISTS beeper_assets (id TEXT PRIMARY KEY,body BLOB NOT NULL,digest TEXT NOT NULL)')
    db.execute('CREATE TABLE IF NOT EXISTS beeper_outbox (id TEXT PRIMARY KEY,request_key TEXT UNIQUE NOT NULL,body TEXT NOT NULL,revision INTEGER NOT NULL)')
...
class Client:
    def __init__(self, config):
        ...
        self.token = get_credential(config['credential_ref']) if config['credential_ref'] else None
        if not self.token:
    ...
    async def request(self, method, path, data=None, binary=False):
        try:
            async with ClientSession(timeout=ClientTimeout(total=20), headers={'Authorization': 'Bearer ' + self.token}) as session:
                async with session.request(method, self.base + path, json=data, allow_redirects=False) as response:
                    limit = 8 * 1024 * 1024 if binary else 2 * 1024 * 1024
                    chunks, size = [], 0
                    async for chunk in response.content.iter_chunked(65536):
                        size += len(chunk)
                        if size > limit:
                            raise PeopleError('Beeper response exceeded its size limit', 502)
                        chunks.append(chunk)
                    if not 200 <= response.status < 300:
                        raise PeopleError('Beeper request failed with status ' + str(response.status), 502)
                    raw = b''.join(chunks)
                    return raw if binary else json.loads(raw)
        except (ClientError, asyncio.TimeoutError, ValueError):
            raise PeopleError('Beeper connection failed or returned invalid data', 503) from None
...
def stored_page(store, chat_id=None):
    ...
    with closing(store.connect()) as db, db:
        ...
        return json.loads(row[0]) if row else {'items': [], 'coverage': 'unknown', 'observed_at': None}


def persist_page(store, page, chat_id=None, provenance='beeper_api', expected_revision=None):
    if not isinstance(page, dict) or not isinstance(page.get('items'), list) or len(page['items']) > 500:
    ...
    body = {**page, 'coverage': 'available_page_only', 'provenance': provenance, 'observed_at': datetime.now(timezone.utc).isoformat()}
    key = 'messages:' + chat_id if chat_id else 'chats'
    with closing(store.connect()) as db, db:
        schema(db)
        if expected_revision is not None:
            current = db.execute('SELECT revision FROM beeper_settings WHERE id=1').fetchone()
            if not current or current[0] != expected_revision:
                raise PeopleError('Beeper connection changed during fetch', 409)
        db.execute('INSERT INTO beeper_pages VALUES (?,?) ON CONFLICT(key) DO UPDATE SET body=excluded.body', (key, json.dumps(body)))
    return body
...
async def refresh(store, chat_id=None, cursor=None):
    ...
    path = '/v1/chats/' + quote(chat_id, safe='') + '/messages' if chat_id else '/v1/chats'
    if cursor:
        path += '?' + urlencode({'cursor': text(cursor, 'cursor', 1000, True)})
    return persist_page(store, await client.request('GET', path), chat_id, expected_revision=config['revision'])


async def fetch_asset(store, chat_id, asset_id):
    chat_id = text(chat_id, 'chat_id', 500, True)
    asset_id = text(asset_id, 'asset_id', 1000, True)
    attachments = [item for message in stored_page(store, chat_id)['items'] for item in message.get('attachments', [])]
    if not any(isinstance(item, dict) and item.get('id') == asset_id for item in attachments):
    ...
    config = settings(store)
    raw = await Client(config).request('GET', '/v1/assets/serve?' + urlencode({'url': asset_id}), binary=True)
    ...
    with closing(store.connect()) as db, db:
        ...
        db.execute('INSERT INTO beeper_assets VALUES (?,?,?) ON CONFLICT(id) DO UPDATE SET body=excluded.body,digest=excluded.digest', (asset_id, raw, digest))
...
def asset(store, asset_id):
    with closing(store.connect()) as db, db:
        schema(db)
        row = db.execute('SELECT body,digest FROM beeper_assets WHERE id=?', (asset_id,)).fetchone()
        if not row:
            raise PeopleError('Attachment has not been fetched', 404)
        return {'id': asset_id, 'content_base64': base64.b64encode(row[0]).decode(), 'digest': row[1], 'bytes': len(row[0])}


def outbox(store):
    with closing(store.connect()) as db, db:
        schema(db)
        return [{**json.loads(row[0]), 'revision': row[1]} for row in db.execute('SELECT body,revision FROM beeper_outbox ORDER BY rowid DESC')]
...
def draft(store, data):
    ...
    request = {key: text(data.get(key), key, 10000 if key == 'text' else 500, True) for key in ('request_key', 'chat_id', 'text')}
    connection_revision = settings(store)['revision']
    with closing(store.connect()) as db, db:
        db.execute('BEGIN IMMEDIATE')
        schema(db)
        previous = db.execute('SELECT body,revision FROM beeper_outbox WHERE request_key=?', (request['request_key'],)).fetchone()
        if previous:
        ...
        row = {**request, 'connection_revision': connection_revision, 'id': uuid4().hex, 'state': 'draft', 'pending_message_id': None, 'created_at': datetime.now(timezone.utc).isoformat(), 'delivery': 'not_sent'}
        db.execute('INSERT INTO beeper_outbox VALUES (?,?,?,1)', (row['id'], request['request_key'], json.dumps(row)))
...
def transition(store, outbox_id, revision, allowed, state, **changes):
    ...
    with closing(store.connect()) as db, db:
        db.execute('BEGIN IMMEDIATE')
        schema(db)
        old = db.execute('SELECT body,revision FROM beeper_outbox WHERE id=?', (outbox_id,)).fetchone()
        if not old:
            raise PeopleError('Outbox item not found', 404)
        ...
        db.execute('UPDATE beeper_outbox SET body=?,revision=? WHERE id=?', (json.dumps(row), revision + 1, outbox_id))
...
async def send(store, outbox_id, data):
    ...
    row = transition(store, outbox_id, data.get('revision'), {'draft'}, 'sending', delivery='unknown', started_at=datetime.now(timezone.utc).isoformat())
    try:
        response = await client.request('POST', '/v1/chats/' + quote(row['chat_id'], safe='') + '/messages', {'text': row['text']})
        pending = text(response.get('pendingMessageID'), 'pending message ID', 500, True)
        if response.get('chatID') != row['chat_id']:
            raise PeopleError('Beeper acknowledgement belongs to another chat', 502)
        return transition(store, outbox_id, row['revision'], {'sending'}, 'pending', pending_message_id=pending, delivery='not_confirmed')
    except (PeopleError, AttributeError):
        return transition(store, outbox_id, row['revision'], {'sending'}, 'unknown', delivery='unknown', error='Send outcome is unknown. Do not resend automatically.')
...
async def reconcile(store, outbox_id, revision):
    ...
    if not row['pending_message_id'] or row['state'] not in ('pending', 'unknown'):
        raise PeopleError('No pending message identity is available for reconciliation', 409)
    config = settings(store)
    if row['connection_revision'] != config['revision']:
        raise PeopleError('Beeper connection changed; reconciliation requires the original connection', 409)
    response = await Client(config).request('GET', '/v1/chats/' + quote(row['chat_id'], safe='') + '/messages/' + quote(row['pending_message_id'], safe=''))
    ...
    if not response.get('id'):
        status = None
    state = {'SUCCESS': 'confirmed', 'PENDING': 'pending', 'FAIL_RETRIABLE': 'failed', 'FAIL_PERMANENT': 'failed'}.get(status, 'unknown')
    return transition(store, outbox_id, revision, {'pending', 'unknown'}, state, delivery='bridge_confirmed' if state == 'confirmed' else 'not_confirmed', remote_message_id=response.get('id'))


def discard(store, outbox_id, revision):
    return transition(store, outbox_id, revision, {'draft', 'pending', 'unknown', 'failed'}, 'discarded', delivery='not_retracted')


def recover(store, outbox_id, revision):
    row = get_outbox(store, outbox_id)
    if row['state'] != 'sending' or (datetime.now(timezone.utc) - datetime.fromisoformat(row['started_at'])).total_seconds() < 60:
        raise PeopleError('Only an interrupted send older than one minute can be marked unknown', 409)
    return transition(store, outbox_id, revision, {'sending'}, 'unknown', delivery='unknown', error='Interrupted send; confirm in Beeper before creating any replacement.')
