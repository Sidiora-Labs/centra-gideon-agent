...
ACCOUNT_FIELDS = {'name', 'kind', 'owner_email', 'alias', 'host', 'username', 'credential_ref', 'auth_mode', 'inbox_folder', 'sent_folder'}
...
def schema(db):
    db.execute('CREATE TABLE IF NOT EXISTS mirror_accounts (id TEXT PRIMARY KEY, body TEXT NOT NULL, revision INTEGER NOT NULL)')
    db.execute('CREATE TABLE IF NOT EXISTS mirror_messages (account_id TEXT NOT NULL, external_id TEXT NOT NULL, body TEXT NOT NULL, UNIQUE(account_id,external_id))')


def account_values(data):
    fields(data, ACCOUNT_FIELDS)
    result = {key: text(data.get(key, default), key, limit, required) for key, default, limit, required in (
        ('name', '', 200, True), ('host', '', 253, False), ('username', '', 320, False),
        ('credential_ref', '', 120, False), ('inbox_folder', 'INBOX', 128, True), ('sent_folder', 'Sent', 128, True))}
    owner = person_values({'name': result['name'], 'identities': [{'kind': 'email', 'value': data.get('owner_email')}]})
    result.update(owner_email=owner['identities'][0]['value'], kind=data.get('kind'), alias=data.get('alias', 'custom'), auth_mode=data.get('auth_mode', 'password'))
    if result['kind'] not in ('maildir', 'mbox', 'imap') or result['alias'] not in ('custom', 'gmail', 'outlook'):
        raise PeopleError('Unsupported mirror kind or provider alias; Teams needs a separate connector')
    if result['auth_mode'] not in ('password', 'xoauth2'):
        raise PeopleError('Unsupported IMAP authentication mode')
    if any('\r' in result[key] or '\n' in result[key] for key in ('username', 'credential_ref', 'inbox_folder', 'sent_folder')):
        raise PeopleError('Account fields cannot contain control lines')
    if result['kind'] == 'imap' and (not re.fullmatch(r'[A-Za-z0-9.-]+', result['host']) or not result['username'] or not re.fullmatch(r'[A-Za-z_][A-Za-z0-9_]*', result['credential_ref'])):
        raise PeopleError('IMAP requires a hostname, username and credential reference')
    if result['kind'] != 'imap' and any(result[k] for k in ('host', 'username', 'credential_ref')):
...
def accounts(store):
    with closing(store.connect()) as db, db:
        schema(db)
        return [{**json.loads(body), 'revision': revision} for body, revision in db.execute('SELECT body,revision FROM mirror_accounts ORDER BY id')]
...
def save_account(store, data, account_id=None):
    ...
    with closing(store.connect()) as db, db:
        ...
        schema(db)
        old = db.execute('SELECT body,revision FROM mirror_accounts WHERE id=?', (account_id,)).fetchone()
        if revision is not None and old is None:
            raise PeopleError('Mirror account not found', 404)
        if old and old[1] != revision:
            raise PeopleError('Mirror account changed; reload', 409)
        if old and any(json.loads(old[0])[key] != values[key] for key in ('kind', 'owner_email')):
            raise PeopleError('Mirror kind and owner identity are immutable; create another account', 409)
        values.update(id=account_id, sync={'state': 'not_synced', 'coverage': 'unknown'})
        next_revision = old[1] + 1 if old else 1
        db.execute('INSERT INTO mirror_accounts VALUES (?,?,?) ON CONFLICT(id) DO UPDATE SET body=excluded.body,revision=excluded.revision',
...
def source_root(store, account):
    ...
    if root.is_symlink() or target.is_symlink() or not re.fullmatch(r'[a-f0-9]{32}', account['id']):
...
def normalize_message(raw, owner, fallback):
    ...
    addresses = lambda header: [address.casefold() for _, address in getaddresses(message.get_all(header, [])) if address]
    ...
    return {'external_id': external_id[:500], 'thread_id': (references[0] if references else external_id)[:100],
            'occurred_at': occurred_at, 'direction': direction, 'subject': str(message.get('Subject', ''))[:1000],
            'body': '\n'.join(bodies)[:100000], 'sender': sender, 'recipients': recipients, 'attachments': attachments,
            ...
            try:
                for key in box.iterkeys():
                    if len(payloads) >= MAX_MESSAGES:
                        ...
                        break
                    for sub in ('cur', 'new'):
                        if (path / sub).is_symlink() or any(p.is_symlink() for p in (path / sub).iterdir()):
...
def imap_messages(account):
    ...
    if not secret:
        raise PeopleError('IMAP credential is unavailable; connect this account first', 503)
    client = None
    try:
        client = imaplib.IMAP4_SSL(account['host'], 993, ssl_context=ssl.create_default_context(), timeout=20)
        if account['auth_mode'] == 'xoauth2':
            client.authenticate('XOAUTH2', lambda _: f"user={account['username']}\x01auth=Bearer {secret}\x01\x01".encode())
        else:
        ...
        for folder in dict.fromkeys((account['inbox_folder'], account['sent_folder'])):
            ...
            for uid in uids:
                ...
                if status != 'OK' or not isinstance(raw, bytes) or len(raw) > MAX_SOURCE_BYTES:
                    raise PeopleError('IMAP message fetch failed or exceeded the size limit', 503)
                if sum(map(len, payloads)) + len(raw) > MAX_SCAN_BYTES:
                    complete = False
                    break
                payloads.append(raw)
        ...
        raise PeopleError('IMAP TLS sync failed; check account connection and folder access', 503) from None
    finally:
    ...
    try:
        ...
        for row in normalized.values():
            if row['occurred_at'] is None or row['occurred_at'] > captured:
                complete = False
                continue
            counterparts = row['recipients'] if row['direction'] == 'outbound' else row['sender']
            matches = [person for person in people if any(i['kind'] == 'email' and i['value'] in counterparts for i in person['identities'])]
            if len(matches) == 1:
                events.append({'person_id': matches[0]['id'], 'thread_id': row['thread_id'], 'external_id': row['external_id'],
                               'occurred_at': row['occurred_at'], 'direction': row['direction'], 'summary': row['subject'][:2000]})
        result = {'state': 'synced', 'seen': len(normalized), 'coverage': 'complete_source_snapshot' if complete else 'partial_source_snapshot',
                  'captured_at': captured, 'scope': 'remote_selected_folders' if account['kind'] == 'imap' else 'uploaded_or_local_source',
                  'evidence_status': 'pending', 'external_delivery': 'not_performed'}
        with closing(store.connect()) as db, db:
            db.execute('BEGIN IMMEDIATE')
            current = db.execute('SELECT revision FROM mirror_accounts WHERE id=?', (account_id,)).fetchone()
            if current[0] != account['revision']:
                raise PeopleError('Account changed during sync; retry with current settings', 409)
            for row in normalized.values():
                db.execute('INSERT INTO mirror_messages VALUES (?,?,?) ON CONFLICT(account_id,external_id) DO UPDATE SET body=excluded.body', (account_id, row['external_id'], json.dumps(row)))
            if complete:
                for external_id, in db.execute('SELECT external_id FROM mirror_messages WHERE account_id=?', (account_id,)).fetchall():
                    if external_id not in normalized:
                        db.execute('DELETE FROM mirror_messages WHERE account_id=? AND external_id=?', (account_id, external_id))
            body = {k: v for k, v in account.items() if k != 'revision'}
            body['sync'] = result
            db.execute('UPDATE mirror_accounts SET body=? WHERE id=?', (json.dumps(body), account_id))
        evidence_batch = {'source': account['kind'], 'source_account_id': account_id, 'captured_at': captured,
                          'coverage_start': min((event['occurred_at'] for event in events), default=captured), 'coverage_end': captured,
                          'incoming_complete': complete, 'outgoing_complete': complete, 'messages': events}
        try:
        ...
        with closing(store.connect()) as db, db:
            body['sync'] = result
            db.execute('UPDATE mirror_accounts SET body=? WHERE id=? AND revision=?', (json.dumps(body), account_id, account['revision']))
        return result
