...
def person_values(data):
    ...
    result = {
        ...
        "ring": data.get("ring", "tribe"),
        "cadence_days": data.get("cadence_days", 30),
    }
    if result["ring"] not in RINGS:
        raise PeopleError("Invalid ring")
    if (
        type(result["cadence_days"]) is not int
        or not 1 <= result["cadence_days"] <= 3650
    ):
    ...
    for identity in identities:
        ...
        if kind == "email":
            ...
            if not re.fullmatch(r"\+[1-9][0-9]{6,14}", value):
                raise PeopleError(
                    "Phone identity requires international +country format"
                )
        elif kind != "handle":
...
def care(person, touchpoints, zone="UTC", now=None):
    ...
    past = [
        datetime.fromisoformat(t["occurred_at"])
        for t in touchpoints
        if datetime.fromisoformat(t["occurred_at"]) <= current
    ]
    last = max(past) if past else None
    days = (
        (current.astimezone(tz).date() - last.astimezone(tz).date()).days
        if last
        else None
    )
    state = (
        "excluded"
        if person["ring"] == "external"
        else (
            "missing"
            if days is None
            else "overdue" if days > person["cadence_days"] else "current"
        )
    )
    return {
        "state": state,
        ...
        "days_since": days,
        "days_overdue": (
            max(0, days - person["cadence_days"]) if days is not None else None
        ),
    }
...
class PeopleStore:
        ...
        with closing(self.connect()) as db:
            return [
                ...
                for body, revision in db.execute(
                    "SELECT body,revision FROM people ORDER BY id"
                )
            ]
    ...
    def save(self, data, person_id=None):
        ...
        with closing(self.connect()) as db, db:
            ...
            old = db.execute(
                "SELECT revision FROM people WHERE id=?", (person_id,)
            ).fetchone()
            ...
            for body, other_id in db.execute("SELECT body,id FROM people"):
                if other_id != person_id and any(
                    ...
                    raise PeopleError(
                        "Identity already belongs to another person; resolve ambiguity explicitly",
                        409,
                    )
    ...
    def record(self, person_id, data):
        ...
        values = {
            "person_id": person_id,
            "source": text(data.get("source"), "source", 80, True),
            "external_id": text(data.get("external_id"), "external_id", 500, True),
            "occurred_at": instant(data.get("occurred_at")),
        ...
        with closing(self.connect()) as db, db:
            ...
            if not db.execute(
                ...
                raise PeopleError("Person not found", 404)
            old = db.execute(
                "SELECT body FROM touchpoints WHERE source=? AND external_id=?",
            ...
            if old:
                ...
                if {k: v for k, v in existing.items() if k != "id"} != values:
                    raise PeopleError(
                        "Source identity already recorded with different content", 409
                    )
