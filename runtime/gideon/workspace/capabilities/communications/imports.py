...
from .store import PeopleError, PeopleStore, fields, person_values, text
...
def csv_rows(content):
    ...
    if len(set(reader.fieldnames)) != len(reader.fieldnames) or set(reader.fieldnames) - {'name', 'email', 'phone', 'handle', 'notes'}:
    ...
    try:
        for row in reader:
            ...
            identities = [{'kind': kind, 'value': value.strip()} for kind in ('email', 'phone', 'handle')
                          for value in row.get(kind, '').split('|') if value.strip()]
            rows.append({'name': row['name'], 'notes': row.get('notes', ''), 'identities': identities})
    ...
    for line in lines:
        ...
        if name == 'BEGIN' and value.upper() == 'VCARD':
            ...
            if 'ENCODING=' in header.upper():
                raise PeopleError('Encoded vCard fields are unsupported; export UTF-8 vCard')
            value = unescape(value)
            if name in ('FN', 'NOTE'):
                card['name' if name == 'FN' else 'notes'] = value
            else:
                card['identities'].append({'kind': 'email' if name == 'EMAIL' else 'phone',
...
def preview(store, data):
    ...
    for index, raw in enumerate(rows):
        row = {'row_id': str(index + 1), 'name': raw['name'], 'candidate': None, 'matches': [], 'error': ''}
        try:
            row['candidate'] = person_values(raw)
            row['matches'] = [{'id': p['id'], 'name': p['name'], 'revision': p['revision']} for p in people
...
def commit(store, data):
    ...
    for choice in decisions:
        ...
        if not isinstance(row_id, str) or row_id in by_row or row_id not in {str(i + 1) for i in range(len(raw_rows))}:
    ...
    with closing(store.connect()) as db, db:
        ...
        db.execute('CREATE TABLE IF NOT EXISTS contact_imports (source_digest TEXT PRIMARY KEY, decision_hash TEXT NOT NULL, receipt TEXT NOT NULL)')
        previous = db.execute('SELECT decision_hash,receipt FROM contact_imports WHERE source_digest=?', (source_digest,)).fetchone()
        if previous:
            if previous[0] != decision_hash:
                raise PeopleError('This source was already committed with different decisions', 409)
            return json.loads(previous[1]), False
        receipts, updated = [], set()
        for index, raw in enumerate(raw_rows):
            choice = by_row[str(index + 1)]
            if choice['action'] == 'skip':
                receipts.append({'row_id': choice['row_id'], 'action': 'skip', 'person_id': None})
                continue
            ...
            if choice['action'] == 'update':
                ...
                if type(revision) is not int or revision < 1 or person_id in updated:
                    raise PeopleError('Updates require a revision and one row per target person')
                found = db.execute('SELECT body,revision FROM people WHERE id=?', (person_id,)).fetchone()
                if not found:
                ...
                values = {**existing, 'identities': existing['identities'] + [i for i in values['identities'] if i not in existing['identities']]}
            ...
            for body, other_id in db.execute('SELECT body,id FROM people'):
                if other_id != person_id and any(i in json.loads(body)['identities'] for i in values['identities']):
                    raise PeopleError(f'Contact row {index + 1} has an identity owned by another person', 409)
            values['id'] = person_id
            ...
            receipts.append({'row_id': choice['row_id'], 'action': choice['action'], 'person_id': person_id, 'revision': revision})
        receipt = {'source_digest': source_digest, 'format': data['format'], 'committed_at': datetime.now(timezone.utc).isoformat(),
                   'rows': receipts, 'source_rows': raw_rows, 'decisions': canonical}
        db.execute('INSERT INTO contact_imports VALUES (?,?,?)', (source_digest, decision_hash, json.dumps(receipt)))
    return receipt, True
