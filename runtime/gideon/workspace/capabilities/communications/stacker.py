...
def schema(db):
    social.schema(db)
    db.execute(
        "CREATE TABLE IF NOT EXISTS stacker_territories(name TEXT PRIMARY KEY,body TEXT NOT NULL)"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS stacker_actions(id TEXT PRIMARY KEY,account_id TEXT NOT NULL,request_key TEXT NOT NULL,original TEXT NOT NULL,body TEXT NOT NULL,UNIQUE(account_id,request_key))"
    )
...
async def _graphql(query, variables, secret=None):
    ...
    async with ClientSession(
        timeout=ClientTimeout(total=20), headers=headers
    ) as client:
        async with client.post(
            ...
            if response.status != 200:
                raise PeopleError(
                    f"Stacker News request failed (HTTP {response.status})", 502
                )
            raw = bytearray()
...
def normalize_territory(data):
    ...
    for item in listing["items"]:
        if (
            ...
            or not item["id"].isdigit()
        ):
            raise PeopleError("Malformed territory item", 502)
        posts.append(
            {
                "id": item["id"],
                "title": text(item.get("title") or "", "title", 1000),
                "text": text(item.get("text") or "", "text", 100000),
    ...
    return {
        "name": territory,
        "description": text(sub.get("desc") or "", "description", 100000),
        "status": text(sub.get("status"), "status", 100, True),
        "base_cost_sats": sub["baseCost"],
        "reply_cost_sats": sub["replyCost"],
        "posts": posts,
        "cursor": cursor,
...
def territories(store):
    with closing(store.connect()) as db, db:
        ...
        return [
            json.loads(row[0])
            for row in db.execute("SELECT body FROM stacker_territories ORDER BY name")
        ]
...
async def read_territory(store, data):
    ...
    try:
        result = {
            **normalize_territory(
                await graphql(TERRITORY, {"name": territory, "cursor": cursor or None})
            ),
            "state": "synced",
        }
        ...
        result = {
            ...
            "error": (
                str(error)
                if isinstance(error, PeopleError)
                else "Stacker News read failed"
            ),
        }
    ...
    with closing(store.connect()) as db, db:
        schema(db)
        db.execute(
            "INSERT OR REPLACE INTO stacker_territories VALUES (?,?)",
            (territory, json.dumps(result)),
        )
    return result
...
def action_values(data):
    ...
    if (
        ...
        raise PeopleError(
            "Zap amount must be 1 to 1000000 sats; other actions use provider fees"
        )
    return {
        "kind": kind,
        "territory": territory,
    ...
    with closing(store.connect()) as db, db:
        ...
        rows = [
            ...
            for row in db.execute(
                "SELECT body FROM stacker_actions WHERE account_id=? ORDER BY rowid",
                (account_id,),
            )
        ]
    for row in rows:
        row["registration_changed"] = (
            row["account_revision"] != current["revision"]
            or current["status"] != "active"
            or current["platform"] != "stackernews"
        )
...
def action(store, account_id, action_id):
    row = next(
        (row for row in actions(store, account_id) if row["id"] == action_id), None
    )
    if not row:
...
def save(store, account_id, data):
    ...
    values = action_values(
        {key: value for key, value in data.items() if key in ACTION_FIELDS}
    )
    ...
    with closing(store.connect()) as db, db:
        ...
        schema(db)
        previous = db.execute(
            "SELECT original,body FROM stacker_actions WHERE account_id=? AND request_key=?",
            (account_id, key),
        ).fetchone()
        if previous:
        ...
        db.execute(
            "INSERT INTO stacker_actions VALUES (?,?,?,?,?)",
            (row["id"], account_id, key, original, json.dumps(row)),
        )
    return row
...
def transition(store, account_id, action_id, revision, allowed, changes):
    ...
    with closing(store.connect()) as db, db:
        ...
        schema(db)
        found = db.execute(
            "SELECT body FROM stacker_actions WHERE id=? AND account_id=?",
        ...
        if (
            not linked
            or linked[1] != row["account_revision"]
            or json.loads(linked[0])["status"] != "active"
        ):
            raise PeopleError(
                "Stacker registration changed; create a new reviewed action", 409
            )
        ...
        db.execute(
            "UPDATE stacker_actions SET body=? WHERE id=?", (json.dumps(row), action_id)
        )
    return row
...
def review(store, account_id, action_id, data):
    ...
    url = (
        "https://stacker.news/~" + row["territory"]
        if row["kind"] == "discussion"
        else "https://stacker.news/items/" + row["item_id"]
    )
    return transition(
        store,
        account_id,
        ...
        {"draft"},
        {
            ...
            "handoff_url": url,
            "reviewed_at": now(),
            "execution_warning": "Submitting a discussion/comment may immediately post and charge provider-determined fees. Zap API keys are unsupported; use the browser and verify the account and amount.",
        },
    )
...
def mutation(row):
    if row["kind"] == "zap":
        raise PeopleError(
            "Stacker News prohibits API-key zaps; use reviewed browser handoff", 409
        )
    if row["kind"] == "discussion":
        return (
            "mutation($title:String!,$text:String!,$subs:[String!]){result:upsertDiscussion(title:$title,text:$text,subNames:$subs){"
            + PAYIN
            + "}}",
            {"title": row["title"], "text": row["text"], "subs": [row["territory"]]},
        )
    return (
        "mutation($text:String!,$parent:ID!){result:upsertComment(text:$text,parentId:$parent){"
        + PAYIN
        + "}}",
        {"text": row["text"], "parent": row["item_id"]},
    )
...
def payin(data):
    if (
        ...
        or data["id"] < 1
        or not isinstance(data.get("payInState"), str)
        or not str(data.get("mcost", "")).isdigit()
    ):
        raise PeopleError("Malformed PayIn receipt; reconcile before any retry", 502)
    item = data.get("item")
    if item is not None and (
        not isinstance(item, dict) or not str(item.get("id", "")).isdigit()
    ):
        raise PeopleError("Malformed PayIn item", 502)
    return {
        "payin_id": data["id"],
        "provider_state": data["payInState"],
        "cost_msats": str(data["mcost"]),
        "external_item_id": str(item["id"]) if item else None,
        "external_execution": "provider_reported",
...
async def submit(store, account_id, action_id, data):
    ...
    if data.get("confirm_execute") is not True:
        raise PeopleError(
            "Explicit execution approval is required; provider may post and charge fees"
        )
    current = account(store, account_id)
    row = action(store, account_id, action_id)
    ...
    if (
        not isinstance(identity.get("me"), dict)
        or not isinstance(identity["me"].get("name"), str)
        or identity["me"]["name"].casefold() != current["handle"]
    ):
    ...
    claimed = transition(
        ...
        data.get("revision"),
        {"reviewed"},
        {"state": "submitting", "external_execution": "unknown", "submitted_at": now()},
    )
    try:
        ...
        result = {
            ...
            "error": "Submission outcome unknown; do not repeat. Inspect Stacker News before reconciling.",
        }
...
def finish(store, claimed, result):
    with closing(store.connect()) as db, db:
        db.execute("BEGIN IMMEDIATE")
        found = db.execute(
            "SELECT body FROM stacker_actions WHERE id=?", (claimed["id"],)
        ).fetchone()
        ...
        db.execute(
            "UPDATE stacker_actions SET body=? WHERE id=?",
            (json.dumps(current), current["id"]),
        )
    return current
...
async def reconcile(store, account_id, action_id, data):
    ...
    if row["registration_changed"] or not row.get("payin_id"):
        raise PeopleError(
            "An unchanged registration and provider PayIn receipt are required", 409
        )
    result = payin(
        (
            await graphql(
                "query($id:Int!){payIn(id:$id){" + PAYIN + "}}",
                {"id": row["payin_id"]},
                credential(current),
            )
        ).get("payIn")
