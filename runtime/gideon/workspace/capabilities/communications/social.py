...
def schema(db):
    db.execute(
        "CREATE TABLE IF NOT EXISTS social_accounts (id TEXT PRIMARY KEY,platform TEXT NOT NULL,handle TEXT NOT NULL,body TEXT NOT NULL,revision INTEGER NOT NULL,UNIQUE(platform,handle))"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS social_requests (key TEXT PRIMARY KEY,body TEXT NOT NULL,account_id TEXT NOT NULL)"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS social_history (id INTEGER PRIMARY KEY,account_id TEXT NOT NULL,event TEXT NOT NULL,at TEXT NOT NULL,body TEXT NOT NULL)"
    )
...
def normalize(data):
    ...
    handle = (
        unicodedata.normalize("NFKC", text(data.get("handle"), "handle", 200, True))
        .lstrip("@")
        .casefold()
    )
    if (
    ...
    if url:
        try:
            ...
            if (
                ...
                or any(
                    c.isspace() or unicodedata.category(c).startswith("C") for c in url
                )
            ):
                raise ValueError()
        except ValueError:
            raise PeopleError(
                "Social profile links must be HTTPS without credentials or invalid characters"
            ) from None
    ...
    return {
        "platform": platform,
        "handle": handle,
        "label": text(data.get("label", handle), "label", 200, True),
        "profile_url": url,
    ...
    with closing(store.connect()) as db, db:
        ...
        return [
            ...
            for row in db.execute(
                "SELECT body,revision FROM social_accounts ORDER BY platform,handle"
            )
        ]
...
def record(db, account_id, event, body):
    db.execute(
        "INSERT INTO social_history(account_id,event,at,body) VALUES (?,?,?,?)",
        (account_id, event, datetime.now(timezone.utc).isoformat(), json.dumps(body)),
    )
    ...
    try:
        with closing(store.connect()) as db, db:
            ...
            if (
                values["person_id"]
                and not db.execute(
                    "SELECT id FROM people WHERE id=?", (values["person_id"],)
                ).fetchone()
            ):
                raise PeopleError("Linked person not found", 404)
            if request_key:
                previous = db.execute(
                    "SELECT body,account_id FROM social_requests WHERE key=?",
                    (request_key,),
                ).fetchone()
                if previous:
                    ...
                    if not old:
                        raise PeopleError(
                            "This registration was removed; use a new request key", 410
                        )
            ...
            body = {
                **values,
                "id": account_id,
                "qualification": "registry_only",
                "created_at": (
                    json.loads(old[0])["created_at"]
                    if old
                    else datetime.now(timezone.utc).isoformat()
                ),
            }
            db.execute(
                "INSERT INTO social_accounts VALUES (?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET platform=excluded.platform,handle=excluded.handle,body=excluded.body,revision=excluded.revision",
                (
                    account_id,
                    values["platform"],
                    values["handle"],
            ...
            if request_key:
                db.execute(
                    "INSERT INTO social_requests VALUES (?,?,?)",
                    (request_key, canonical, account_id),
                )
            record(
                db,
                account_id,
        ...
        raise PeopleError(
            "This platform and normalized handle are already registered", 409
        ) from None
...
def remove(store, account_id, revision):
    ...
    with closing(store.connect()) as db, db:
        ...
        try:
            ...
            raise PeopleError(
                "Social account is referenced by another record; archive it instead",
                409,
            ) from None
...
def history(store, account_id):
    with closing(store.connect()) as db, db:
        schema(db)
        return [
            {"event": row[0], "at": row[1], "account": json.loads(row[2])}
            for row in db.execute(
                "SELECT event,at,body FROM social_history WHERE account_id=? ORDER BY id",
                (account_id,),
            )
        ]
