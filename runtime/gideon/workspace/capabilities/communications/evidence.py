...
def schema(db):
    db.execute(
        "CREATE TABLE IF NOT EXISTS relationship_messages (source TEXT NOT NULL, source_account_id TEXT NOT NULL, external_id TEXT NOT NULL, body TEXT NOT NULL, UNIQUE(source,source_account_id,external_id))"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS relationship_coverage (source TEXT NOT NULL, source_account_id TEXT NOT NULL, body TEXT NOT NULL, UNIQUE(source,source_account_id))"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS relationship_batches (digest TEXT PRIMARY KEY, receipt TEXT NOT NULL)"
    )
...
def normalize(data):
    ...
    result = {
        key: text(data.get(key), key, 100, True)
        for key in ("source", "source_account_id")
    }
    result.update(
        {
            key: instant(data.get(key))
            for key in ("captured_at", "coverage_start", "coverage_end")
        }
    )
    ...
    for raw in messages:
        ...
        row = {
            key: text(raw.get(key), key, 500 if key == "external_id" else 100, True)
            for key in ("person_id", "thread_id", "external_id")
        }
        row.update(
            occurred_at=instant(raw.get("occurred_at")),
...
def ingest(store, data):
    ...
    with closing(store.connect()) as db, db:
        ...
        schema(db)
        previous = db.execute(
            "SELECT receipt FROM relationship_batches WHERE digest=?", (batch_digest,)
        ).fetchone()
        if previous:
            return json.loads(previous[0]), False
        coverage = db.execute(
            "SELECT body FROM relationship_coverage WHERE source=? AND source_account_id=?",
            (source, account),
        ).fetchone()
        if coverage and json.loads(coverage[0])["captured_at"] >= batch["captured_at"]:
            raise PeopleError(
                "Coverage observation is stale or changed at the same timestamp", 409
            )
        inserted = 0
        for row in batch["messages"]:
            ...
            db.execute(
                ...
                (source, account, row["external_id"], json.dumps(row)),
            )
            inserted += 1
        ...
        db.execute(
            "INSERT INTO relationship_batches VALUES (?,?)",
            (batch_digest, json.dumps(receipt)),
        )
    return receipt, True
...
def thread_verdict(messages, coverage, now):
    ...
    if (
        captured > now
        or now - captured > timedelta(hours=24)
        or now - end > timedelta(hours=24)
    ):
    ...
    if any(
        not coverage["coverage_start"] <= row["occurred_at"] <= coverage["coverage_end"]
        for row in messages
    ):
    ...
    directions = {
        row["direction"]
        for row in messages
        if row["occurred_at"] == latest["occurred_at"]
    }
    if len(directions) > 1:
        return "unknown", "Latest message ordering is ambiguous", latest
    return (
        ("unanswered" if latest["direction"] == "inbound" else "answered"),
        "",
        latest,
    )
...
def report(store, zone="UTC", now=None):
    ...
    with closing(store.connect()) as db:
        ...
        messages = (
            list(
                db.execute(
                    "SELECT source,source_account_id,body FROM relationship_messages"
                )
            )
            if "relationship_messages" in tables
            else []
        )
        coverage = (
            {
                (s, a): json.loads(body)
                for s, a, body in db.execute(
                    "SELECT source,source_account_id,body FROM relationship_coverage"
                )
            }
    ...
    for source, account, body in messages:
        row = json.loads(body)
        groups.setdefault(
            (source, account, row["person_id"], row["thread_id"]), []
        ).append(row)
    threads = []
        ...
        threads.append(
            {
                ...
                "state": state,
                "reason": reason,
                "as_of": observation["coverage_end"] if observation else None,
                "latest": latest,
    ...
    for person in store.people():
        ...
        points += [
            ...
            if json.loads(body)["person_id"] == person["id"]
        ]
