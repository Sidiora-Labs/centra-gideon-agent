...
from . import mirrors, desktop, beeper, telegram, calendar, social, xreading, stacker
...
SPECS = {
    ...
    'people_social_update': ('Edit or archive a local social account using its current revision.', obj({'account_id': STRING, 'account': obj({**SOCIAL, 'revision': {'type': 'integer', 'minimum': 1}}, ('platform', 'handle', 'revision'))}, ('account_id', 'account')), True),
    'people_social_remove': ('Remove a local registry entry; no external account deletion.', obj({'account_id': STRING, 'revision': {'type': 'integer', 'minimum': 1}}, ('account_id', 'revision')), True),
    'people_social_history': ('Read local account registration/edit/removal provenance.', obj({'account_id': STRING}, ('account_id',)), False),
...
class PeopleTools(ToolProvider):
    ...
    async def invoke(self, tool_name, arguments):
        try:
            ...
            if tool_name.startswith('people_stacker_'):
                ...
                if action == 'accounts':
                    ...
                    result = social.remove(store, arguments['account_id'], arguments['revision'])
                else:
                    row, created = social.save(store, arguments['account'], arguments.get('account_id'))
