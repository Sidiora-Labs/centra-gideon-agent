...
class ProgressStore:
    def __init__(self, path: Path):
        ...
        with sqlite3.connect(self.path) as db:
            db.executescript(
                "CREATE TABLE IF NOT EXISTS profile(id INTEGER PRIMARY KEY CHECK(id=1),body TEXT NOT NULL);"
    ...
    def configure(
        self, *, birth_date, timezone, tracked_task_ids, expected_revision, request_id
    ):
        try:
            ZoneInfo(timezone)
        ...
        if birth_date is not None:
            if (
                ...
                or date.fromisoformat(birth_date).isoformat() != birth_date
            ):
        ...
        if (
            not isinstance(tracked_task_ids, list)
            or len(tracked_task_ids) > 100
            or any(not isinstance(id, str) for id in tracked_task_ids)
            or len(set(tracked_task_ids)) != len(tracked_task_ids)
        ):
            raise ValueError(
                "tracked_task_ids must be at most 100 unique native task identifiers"
            )
        ...
        fingerprint = json.dumps(
            [birth_date, timezone, tracked_task_ids, expected_revision]
        )
        with sqlite3.connect(self.path) as db:
            ...
            if prior:
                if prior[0] != fingerprint:
                    raise ConflictError(
                        "request_id already names a different progress profile change"
                    )
            ...
            db.execute(
                "INSERT OR REPLACE INTO profile VALUES(1,?)", (json.dumps(result),)
            )
            db.execute(
    ...
    def sheet(self, as_of=None):
        ...
        age = (
            day.year - birth.year - ((day.month, day.day) < (birth.month, birth.day))
            if birth and day >= birth
            else None
        )
        ...
        sessions = (
            [
                row
                for row in goals.list_sessions()
                if row["status"] == "completed"
                and datetime.fromisoformat(row["end_at"]).astimezone(zone).date() <= day
            ]
            if goals
            else []
        )
        completed = [
            {
                ...
                "goal_id": row["goal_id"],
                "title": row["title"],
                "local_date": datetime.fromisoformat(row["end_at"])
                .astimezone(zone)
                .date()
                .isoformat(),
                "planned_minutes": (
        ...
        return {
            ...
            "age": age,
            "goals": {
                status: sum(row["status"] == status for row in goal_rows)
                for status in ("active", "completed", "archived")
            },
            "sessions_completed": completed,
            "planned_completed_minutes": sum(
                row["planned_minutes"] for row in completed
            ),
            "authored_story_count": (
                len(StoryStore(story_path).list()) if story_path.exists() else 0
            ),
            "tasks": tasks,
            "tasks_done": sum(row["status"] == "done" for row in tasks),
            "missing_task_ids": missing,
            "invalid_task_ids": invalid,
            "unknown_metrics": ["health", "skill_level", "measured_effort"],
            "source_policy": "Current source state; as_of bounds session dates and age, not historical task or goal status",
        }
