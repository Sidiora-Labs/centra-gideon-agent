...
STORY = {
    "prompt": {**TEXT, "maxLength": 4000},
    "theme": {**TEXT, "maxLength": 200},
    "text": {**TEXT, "maxLength": 100000},
...
CONFIG = {
    ...
    "enabled": {"type": "boolean"},
    "traits": TRAITS,
    "personas": {"type": "array", "maxItems": 20, "items": PERSONA},
...
CONTRACTS = {
    ...
    "identity_story_create": (
        {**STORY, "request_id": IDENTIFIER},
        ["prompt", "theme", "text", "request_id"],
        "Create an authored answer with a retry identifier",
        True,
    ),
    "identity_story_update": (
        {**STORY, "story_id": IDENTIFIER, "expected_revision": REVISION},
        ["story_id", "expected_revision", "prompt", "theme", "text"],
        "Edit an answer while preserving earlier revisions",
        True,
    ),
    "identity_story_delete": (
        {"story_id": IDENTIFIER, "expected_revision": REVISION},
        ["story_id", "expected_revision"],
    ...
    "identity_twin_save_document": (
        DOCUMENT,
        ["title", "text", "expected_revision"],
        "Save a non-private identity source",
        True,
    ),
    "identity_twin_delete_document": (
        {"id": IDENTIFIER, "expected_revision": {"type": "integer", "minimum": 0}},
...
RULE = {
    ...
    "additionalProperties": False,
    "properties": {
        "type": {"enum": ["equals", "contains", "not_contains"]},
        "value": TEXT,
...
CONTRACTS.update(
    {
        ...
        "identity_fidelity_observe": (
            {"case_id": IDENTIFIER, "answer": TEXT, "request_id": IDENTIFIER},
            ["case_id", "answer", "request_id"],
            "Score a supplied observation; does not establish provider execution",
            True,
        ),
        "identity_fidelity_run": (
            {"case_id": IDENTIFIER, "request_id": IDENTIFIER},
            ["case_id", "request_id"],
...
CONTRACTS.update(
    {
        ...
        "identity_goals_save_session": (
            SESSION,
            ["goal_id", "title", "start_at", "end_at", "request_id"],
...
CONTRACTS.update(
    {
        ...
        "identity_continuity_configure": (
            {
                "heartbeat_paused": {"type": "boolean"},
                "expected_revision": {"type": "integer", "minimum": 0},
                "request_id": IDENTIFIER,
            },
            ["heartbeat_paused", "expected_revision", "request_id"],
            "Pause or resume scheduled heartbeat turns only",
            True,
        ),
        "identity_continuity_append_anchor": (
            {"slot": {"enum": ["persona", "self_notes"]}, "text": TEXT},
            ["slot", "text"],
...
CONTRACTS["identity_bundle_inventory"] = (
    {},
    [],
...
RECIPE_STEP = {
    "type": "object",
    "additionalProperties": False,
    "properties": {"id": IDENTIFIER, "tool": TEXT, "arguments": {"type": "object"}},
    "required": ["id", "tool", "arguments"],
}
RECIPE = {
    "title": TEXT,
    "steps": {"type": "array", "items": RECIPE_STEP, "minItems": 1, "maxItems": 5},
    "enabled": {"type": "boolean"},
    "id": IDENTIFIER,
    "expected_revision": {"type": "integer", "minimum": 0},
    "request_id": IDENTIFIER,
...
CONTRACTS.update(
    {
        ...
        "identity_recipe_restore": (
            ...
            ["id", "revision", "expected_revision", "request_id"],
            "Restore an earlier recipe as a new revision",
            True,
        ),
        "identity_recipe_begin": (
            {"recipe_id": IDENTIFIER, "revision": REVISION, "request_id": IDENTIFIER},
            ["recipe_id", "revision", "request_id"],
        ...
        "identity_recipe_advance": (
            {"run_id": IDENTIFIER, "expected_index": {"type": "integer", "minimum": 0}},
...
MILESTONE = {
    ...
    "required": ["id", "title", "done", "target_date"],
}
GOAL_LINK = {
    "type": "object",
    "additionalProperties": False,
    "properties": {"kind": {"enum": ["task", "loop", "session"]}, "id": IDENTIFIER},
    "required": ["kind", "id"],
}
GOAL_PLAN = {
    "goal_id": IDENTIFIER,
    "parent_id": {"type": ["string", "null"]},
    "horizon": {"enum": ["short_term", "long_term", "lifetime"]},
    "milestones": {"type": "array", "maxItems": 100, "items": MILESTONE},
    "links": {"type": "array", "maxItems": 100, "items": GOAL_LINK},
    "unit": TEXT,
...
CONTRACTS.update(
    {
        ...
        "identity_lifecycle_request_thinking": (
            {"request_id": IDENTIFIER, "preset": {"enum": ["reflect", "review"]}},
            ["request_id", "preset"],
