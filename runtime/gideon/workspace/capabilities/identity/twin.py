...
class TwinStore:
    def __init__(self, path: Path):
        ...
        with sqlite3.connect(self.path) as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS twin (id INTEGER PRIMARY KEY CHECK(id=1), body TEXT NOT NULL)"
            )
            db.execute(
                "INSERT OR IGNORE INTO twin VALUES (1,?)",
                (
                    json.dumps(
                        {
                            "schema_version": 1,
    ...
    def snapshot(self) -> dict:
        with sqlite3.connect(self.path) as db:
            return json.loads(
                db.execute("SELECT body FROM twin WHERE id=1").fetchone()[0]
            )
    ...
    def _change(self, expected_revision, operation):
        ...
        try:
            ...
            current["revision"] += 1
            db.execute(
                "UPDATE twin SET body=? WHERE id=1",
    ...
    def configure(
        self, *, expected_revision, enabled, traits, personas, active_persona_id
    ):
    ...
    def compose(self, *, budget=1000, include_private=False) -> dict:
        ...
        for doc in sorted(
            state["documents"], key=lambda d: (-d["weight"], d["priority"], d["id"])
        ):
