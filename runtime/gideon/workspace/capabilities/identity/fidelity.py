...
class FidelityStore:
    ...
    def _sources(self, ids):
        if (
            ...
            raise ValueError(
                "source_ids must contain 1..10 unique local document identifiers"
            )
        twin = TwinStore(self.path.parent / "twin.sqlite3").snapshot()
        available = {
            doc["id"]: doc
            for doc in twin["documents"]
            if doc["enabled"] and not doc["private"]
        }
        if any(identifier not in available for identifier in ids):
            raise ValueError(
                "Sources must be enabled, non-private local identity documents"
            )
        sources = [
            {key: available[identifier][key] for key in ("id", "title", "text")}
    ...
    def _get(db, table, identifier):
        row = db.execute(
            f"SELECT body FROM {table} WHERE id=?", (identifier,)
        ).fetchone()
        if row is None:
        ...
        *,
        ...
        for rule in rules:
            ...
            if rule.get("type") not in ("equals", "contains", "not_contains"):
                raise ValueError(
                    "Only equals, contains and not_contains rules are supported"
                )
        ...
        with sqlite3.connect(self.path) as db:
            ...
            if id is None:
                prior = db.execute(
                    "SELECT fingerprint,body FROM case_requests WHERE id=?",
                    (request_id,),
                ).fetchone()
                if prior:
            ...
            if id is None:
                db.execute(
                    "INSERT INTO case_requests VALUES (?,?,?)",
                    (request_id, fingerprint, json.dumps(case)),
                )
            return case
    ...
    def list_cases(self):
        with sqlite3.connect(self.path) as db:
            return [
                json.loads(row[0])
                for row in db.execute("SELECT body FROM cases ORDER BY id")
            ]
    ...
    def _begin(self, case_id, request_id, origin, answer=None):
        ...
        with sqlite3.connect(self.path) as db:
            db.execute("BEGIN IMMEDIATE")
            prior = db.execute(
                "SELECT fingerprint,body FROM runs WHERE request_id=?", (request_id,)
            ).fetchone()
            if prior:
                if prior[0] != fingerprint:
                    raise ConflictError(
                        "request_id already names a different evaluation"
                    )
                existing = json.loads(prior[1])
                if existing["status"] == "running":
                    raise ConflictError(
                        "Evaluation is already running; inspect its recorded state"
                    )
                return existing
            ...
            db.execute(
                ...
                (run["id"], request_id, fingerprint, json.dumps(run)),
            )
            return run
        ...
        results = [
            {
                "rule": rule,
                "passed": Assertion(
                    type=AssertionType(rule["type"]),
                    value=rule["value"],
                    case_sensitive=rule.get("case_sensitive", False),
                ).check(answer),
            }
            for rule in run["case_snapshot"]["rules"]
        ]
        run.update(
            answer=answer,
            results=results,
