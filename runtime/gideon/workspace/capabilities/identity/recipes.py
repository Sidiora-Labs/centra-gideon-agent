...
def _walk(value, previous, outputs=None, depth=0):
    ...
    if isinstance(value, dict):
        if "$ref" in value:
            ...
            return result
        return {
            key: _walk(item, previous, outputs, depth + 1)
...
class RecipeStore:
    def __init__(self, path: Path, *, allowed_tools=None, guarded=False):
        ...
        self.allowed_tools = (
            READ_TOOLS if allowed_tools is None else frozenset(allowed_tools)
        )
        self.path = Path(path)
    ...
    def list(self):
        with sqlite3.connect(self.path) as db:
            return [
                json.loads(row[0])
                for row in db.execute("SELECT body FROM recipes ORDER BY id")
            ]
        ...
        with sqlite3.connect(self.path) as db:
            ...
            return [
                ...
                for row in db.execute(
                    "SELECT body FROM versions WHERE id=? ORDER BY revision", (id,)
                )
            ]
    ...
    def save(
        self, *, title, steps, request_id, enabled=True, id=None, expected_revision=0
    ):
        _text(title)
        if (
        ...
        for step in steps:
            ...
            if (
                ...
                or not re.fullmatch(r"[A-Za-z][A-Za-z0-9_]{0,39}", step["id"])
                or step["id"] in previous
            ):
        ...
        fingerprint = json.dumps(
            ["save", title, steps, enabled, id, expected_revision], sort_keys=True
        )
        with sqlite3.connect(self.path) as db:
            ...
            db.execute(
                ...
                (result["id"], result["revision"], json.dumps(result)),
            )
            db.execute(
    ...
    def restore(self, *, id, revision, expected_revision, request_id):
        old = next(
            (row for row in self.history(id) if row["revision"] == revision), None
        )
        if old is None:
        ...
        with sqlite3.connect(self.path) as db:
            ...
            if replay:
                ...
                if run.get("session_key") and not self.guarded:
                    raise ValueError(
                        "Session-bound runs require the guarded recipe adapter"
                    )
                return run
            ...
            db.execute(
                "INSERT INTO requests VALUES(?,?,?)",
                (request_id, fingerprint, json.dumps({"id": run["id"]})),
            )
            return run
    ...
    def list_runs(self):
        with sqlite3.connect(self.path) as db:
            rows = [
                json.loads(row[0])
                for row in db.execute("SELECT body FROM runs ORDER BY id")
            ]
            return (
                rows
                if self.guarded
                else [row for row in rows if not row.get("session_key")]
            )
    ...
    async def advance(self, run_id, expected_index, provider):
        ...
        with sqlite3.connect(self.path) as db:
            ...
            if (
                run["recipe_snapshot"]["steps"][run["next_index"]]["tool"]
                not in self.allowed_tools
            ):
                raise ValueError(
                    "Recipe operation is not admitted by this execution adapter"
                )
            ...
            if current_run["status"] != "cancelled":
                current_run["status"] = (
                    ...
                    if outcome["status"] == "failed"
                        ...
                        "completed"
                        if current_run["next_index"]
                        == len(run["recipe_snapshot"]["steps"])
                        else "ready"
                    )
                )
            db.execute(
                "UPDATE runs SET body=? WHERE id=?", (json.dumps(current_run), run_id)
            )
            return current_run
