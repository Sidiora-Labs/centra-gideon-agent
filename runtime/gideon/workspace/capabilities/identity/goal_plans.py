...
class GoalPlanStore:
    def __init__(self, path: Path):
        ...
        with sqlite3.connect(self.path) as db:
            db.executescript("CREATE TABLE IF NOT EXISTS goal_plans(id TEXT PRIMARY KEY,body TEXT NOT NULL);"
    ...
    def _default(goal_id):
        return dict(goal_id=goal_id, revision=0, parent_id=None, horizon="long_term", milestones=[], links=[], unit="units", target_value=None)

    @staticmethod
    def _plan(db, goal_id):
        row = db.execute("SELECT body FROM goal_plans WHERE id=?", (goal_id,)).fetchone()
    ...
    def _source(self, link):
        ...
        try:
            if kind == "session":
                ...
                return {**link, "title": row["title"], "status": row["status"], "availability": "available"}
            if kind == "task":
                ...
                return {**link, "title": task.title, "status": task.status.value, "availability": "available"}
            ...
            with sqlite3.connect(database.as_uri() + "?mode=ro", uri=True) as db:
                row = db.execute("SELECT name,status,kind FROM loops WHERE id=?", (identifier,)).fetchone()
            if row is None:
                raise KeyError(identifier)
            return {**link, "title": row[0], "status": row[1], "loop_kind": row[2], "availability": "available"}
        except KeyError:
            return {**link, "title": None, "status": None, "availability": "missing"}
        except (UnsafeRecordId, ValueError, TypeError, AttributeError, sqlite3.Error, OSError):
            return {**link, "title": None, "status": None, "availability": "unavailable"}

    @staticmethod
    def _replay(db, request_id, fingerprint):
        _text(request_id, "request_id", 128)
        row = db.execute("SELECT fingerprint,body FROM plan_requests WHERE id=?", (request_id,)).fetchone()
        if row:
            if row[0] != fingerprint:
                raise ConflictError("request_id already names another goal planning change")
            return json.loads(row[1])
        return None

    def configure(self, *, goal_id, parent_id, horizon, milestones, links, unit, target_value, expected_revision, request_id):
        _text(unit, "unit", 40)
        ...
        for item in milestones:
            if not isinstance(item, dict) or set(item) != {"id", "title", "done", "target_date"}:
            ...
            if item["id"] in names or type(item["done"]) is not bool:
                raise ValueError("Milestone identifiers must be unique and done must be boolean")
            names.add(item["id"])
            if item["target_date"] is not None and date.fromisoformat(item["target_date"]).isoformat() != item["target_date"]:
        ...
        seen = set()
        for link in links:
            if not isinstance(link, dict) or set(link) != {"kind", "id"} or link["kind"] not in ("task", "loop", "session"):
        ...
        fields = dict(goal_id=goal_id,parent_id=parent_id,horizon=horizon,milestones=milestones,links=links,unit=unit,target_value=target_value)
        fingerprint = json.dumps(["configure", fields, expected_revision], sort_keys=True)
        with sqlite3.connect(self.path) as db:
            ...
            if old["unit"] != unit and db.execute("SELECT 1 FROM goal_checkins WHERE goal_id=? LIMIT 1", (goal_id,)).fetchone():
                raise ConflictError("Metric unit cannot change after observations exist")
            ...
            db.execute("INSERT OR REPLACE INTO goal_plans VALUES(?,?)", (goal_id, json.dumps(result)))
            db.execute("INSERT INTO plan_requests VALUES(?,?,?)", (request_id, fingerprint, json.dumps(result)))
            return result
    ...
    def checkin(self, *, goal_id, value, observed_at, notes, request_id):
        ...
        with sqlite3.connect(self.path) as db:
            ...
            if replay:
                return replay
            GoalStore._get(db, "goals", goal_id)
            if db.execute("SELECT 1 FROM goal_checkins WHERE goal_id=? AND observed_at=?", (goal_id, observed_at)).fetchone():
                raise ConflictError("A metric observation already exists at this instant")
            result = dict(id=uuid4().hex,goal_id=goal_id,value=value,observed_at=observed_at,notes=notes,unit=self._plan(db, goal_id)["unit"],source="human_reported")
            db.execute("INSERT INTO goal_checkins VALUES(?,?,?,?)", (result["id"],goal_id,observed_at,json.dumps(result)))
            db.execute("INSERT INTO plan_requests VALUES(?,?,?)", (request_id,fingerprint,json.dumps(result)))
            return result
    ...
    def get(self, goal_id):
        with sqlite3.connect(self.path) as db:
            ...
            children = [row["goal_id"] for raw in db.execute("SELECT body FROM goal_plans") if (row := json.loads(raw[0]))["parent_id"] == goal_id]
            checkins = [json.loads(row[0]) for row in db.execute("SELECT body FROM goal_checkins WHERE goal_id=? ORDER BY observed_at,id", (goal_id,))]
        velocity = None
        if len(checkins) >= 2:
            first, last = checkins[0], checkins[-1]
            days = (datetime.fromisoformat(last["observed_at"]) - datetime.fromisoformat(first["observed_at"])).total_seconds() / 86400
            velocity = {"value_per_day": (last["value"] - first["value"]) / days, "unit": plan["unit"], "from": first["observed_at"], "to": last["observed_at"]}
        return {"goal": goal,"plan": plan,"children": sorted(children),"checkins": checkins,"velocity": velocity,
                "linked_sources": [self._source(link) for link in plan["links"]],
                "milestones_complete_ratio": sum(item["done"] for item in plan["milestones"]) / len(plan["milestones"]) if plan["milestones"] else None}
