...
class ContinuityStore:
    ...
    def configure(self, *, heartbeat_paused, expected_revision, request_id):
        if (
            type(heartbeat_paused) is not bool
            or type(expected_revision) is not int
            or expected_revision < 0
        ):
            raise ValueError(
                "Expected heartbeat_paused boolean and nonnegative revision"
            )
        ...
        with sqlite3.connect(self.path) as db:
            ...
            prior = db.execute(
                "SELECT fingerprint,body FROM journal WHERE request_id=?", (request_id,)
            ).fetchone()
            if prior:
                if prior[0] != fingerprint:
                    raise ConflictError(
                        "request_id already names another continuity change"
                    )
            ...
            entry = {
                "policy": policy,
                "action": "pause" if heartbeat_paused else "resume",
                "created_at": datetime.now(timezone.utc).isoformat(),
            }
            db.execute(
                "INSERT OR REPLACE INTO policy VALUES(1,?)", (json.dumps(policy),)
            )
            db.execute(
                "INSERT INTO journal(request_id,fingerprint,body) VALUES(?,?,?)",
                (request_id, fingerprint, json.dumps(entry)),
            )
            return policy
    ...
    def append_anchor(self, *, slot, text, source="user_explicit"):
        ...
        with self._memory() as memory:
            try:
                ...
                raise ValueError(
                    "Slot is full; explicitly remove an existing line first"
                ) from None
    ...
    def remove_anchor(self, *, slot, text):
        self._validate(slot, text)
        with self._memory() as memory:
            memory_slots.tombstone(
                memory, slot, text, actor="human", source="user_explicit"
            )
            return {
                "slot": slot,
                "lines": [line.to_dict() for line in memory_slots.load(memory, slot)],
            }
    ...
    def status(self):
        ...
        if (self.home / "memory.db").exists():
            with self._memory() as memory:
                slots = {
                    name: [line.to_dict() for line in memory_slots.load(memory, name)]
                    for name in SLOTS
                }
                context = memory_slots.render_slots_block(memory, names=list(SLOTS))
                events = [
                    row
                    for row in memory.get_events(limit=100)
                    if row.get("memory_key") in {"slot.persona", "slot.self_notes"}
                ]
        return {
            **self.policy(),
            "journal": journal,
            ...
            "scope": "Scheduled heartbeat turns only; active turns, workflows and maintenance are unchanged",
