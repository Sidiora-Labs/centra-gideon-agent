...
from gideon.workspace.capabilities.identity.bundles import BundleService, FORMAT as SLOT_FORMAT
...
class ExtendedBundleService(BundleService):
    ...
    def _selected(groups):
        if not isinstance(groups, list) or not groups or len(groups) > len(SLOTS + GROUPS) or any(not isinstance(g, str) or g not in SLOTS + GROUPS for g in groups) or len(set(groups)) != len(groups):
    ...
    def inventory(self):
        ...
        for group in GROUPS:
            ...
            count = len(data) if isinstance(data, list) else len(data.get("documents", data.get("stories", [])))
            result["groups"].append({"id": group, "count": count, "cap_chars": None, "available": available, "unavailable": "" if available else "Unavailable under destination policy"})
        result.update(format=FORMAT, exclusions=["credentials", "provider_configuration", "account_identity", "runtime_authority", "activity_bindings"], max_encoded_bytes=MAX_ENCODED)
        return result
    ...
    def export_bundle(self, *, groups, passphrase):
        ...
        for group in groups:
            ...
            data[group] = self._decode(super().export_bundle(groups=[group], passphrase=passphrase), passphrase)["groups"][group] if group in SLOTS else self.inventory_store.snapshot(group)
        ...
        return {"format": FORMAT, "ciphertext": base64.b64encode(encrypt_archive(raw, passphrase)).decode()}

    def _extended(self, bundle, passphrase):
        self._passphrase(passphrase)
        if not isinstance(bundle, dict) or set(bundle) != {"format", "ciphertext"} or bundle["format"] != FORMAT or not isinstance(bundle["ciphertext"], str) or len(bundle["ciphertext"]) > MAX_ENCODED:
            raise ValueError("Invalid extended identity envelope")
        try:
            data = json.loads(decrypt_archive(base64.b64decode(bundle["ciphertext"], validate=True), passphrase))
        except (ArchiveRefused, ValueError, TypeError):
            raise ValueError("Bundle authentication or format failed") from None
        if not isinstance(data, dict) or set(data) != {"format", "groups"} or data["format"] != FORMAT or not isinstance(data["groups"], dict):
            raise ValueError("Invalid extended identity payload")
        self._selected(list(data["groups"]))
        for group, value in data["groups"].items():
            if group not in SLOTS:
                self.inventory_store.validate(group, value)
            elif not isinstance(value, list) or any(not isinstance(text, str) for text in value):
    ...
    def _slot_envelope(self, group, value, passphrase):
        ...
        return {"format": SLOT_FORMAT, "ciphertext": base64.b64encode(encrypt_archive(raw, passphrase)).decode()}
    ...
    def preview(self, *, bundle, passphrase, groups):
        ...
        for group in selected:
            if group in SLOTS:
                ...
                plans.append(super().preview(bundle=slot, passphrase=passphrase, groups=[group])["groups"][0])
                snapshots[group] = self.continuity.status()["slots"][group]
            else:
                plans.append(self.inventory_store.plan(group, data["groups"][group]))
                snapshots[group] = self.inventory_store.snapshot(group) if group_available(group) else None
        return {"preview_token": digest([bundle, selected, snapshots]), "groups": plans, "can_apply": not any(row["blocked"] for row in plans)}

    def apply_bundle(self, *, bundle, passphrase, groups, preview_token, request_id):
        if isinstance(bundle, dict) and bundle.get("format") == SLOT_FORMAT:
            return super().apply_bundle(bundle=bundle, passphrase=passphrase, groups=groups, preview_token=preview_token, request_id=request_id)
        ...
        with sqlite3.connect(self.path) as db:
            ...
            prior = db.execute("SELECT fingerprint,body FROM receipts WHERE id=?", (request_id,)).fetchone()
            if prior:
                if prior[0] != fingerprint:
                    raise ConflictError("Request identifier names another bundle application")
                return json.loads(prior[1])
            if preview_token != preview["preview_token"]:
                raise ConflictError("Destination identity changed; preview again")
            if not preview["can_apply"]:
                raise ValueError("Selected identity groups have conflicts or are unavailable")
            snapshots = {group: digest(self.inventory_store.snapshot(group)) for group in groups if group not in SLOTS}
            receipt = {"status": "applying", "applied": [], "skipped": [], "errors": [], "provenance": {"payload_hash": digest(data), "groups": sorted(groups)}}
            db.execute("INSERT INTO receipts VALUES(?,?,?)", (request_id, fingerprint, json.dumps(receipt)))
        for plan in preview["groups"]:
            ...
            try:
                if group in SLOTS:
                    ...
                    count = self.inventory_store.apply(group, data["groups"][group], snapshots[group])
            except Exception as error:
                receipt["errors"].append({"slot": group, "error": str(error)})
            receipt["applied"].append({"slot": group, "count": count})
            receipt["skipped"].append({"slot": group, "count": len(plan["duplicates"]) + len(plan["tombstoned"])})
            receipt["status"] = "partial" if receipt["errors"] else "applying"
            with sqlite3.connect(self.path) as db:
                db.execute("UPDATE receipts SET body=? WHERE id=?", (json.dumps(receipt), request_id))
