...
def conversation(record):
    ...
    result = {'id': identity, 'title': record.get('title') or 'Untitled conversation', 'created_at': None,
              'updated_at': None, 'message_count': 0, 'branch_count': 0, 'unsupported_parts': 0, 'valid': False, 'error': ''}
    try:
        if not isinstance(result['title'], str) or len(result['title']) > 1000:
            raise CaptureError('Invalid conversation title')
        result['created_at'], result['updated_at'] = instant(record.get('create_time')), instant(record.get('update_time'))
        ...
        for identity, node in mapping.items():
            ...
            while ancestor is not None:
                ...
                if ancestor is not None and (not isinstance(ancestor, str) or ancestor not in mapping):
                    raise CaptureError('Conversation branch references a missing ancestor')
            message = node.get('message')
            if message is None:
                continue
            if not isinstance(message, dict) or not isinstance(message.get('author'), dict) or not isinstance(message.get('content'), dict):
            ...
            result['unsupported_parts'] += sum(not isinstance(part, str) for part in parts)
            if content.get('content_type') not in ('text', None):
                result['unsupported_parts'] += 1
            stamp = instant(message.get('create_time'))
            sections.append((stamp or '', identity, f"## {role} · {stamp or 'undated'}\nNode: {identity}; parent: {parent or 'root'}\n\n" + '\n'.join(texts)))
        result.update(message_count=len(sections), branch_count=sum(count > 1 for parent, count in child_counts.items() if parent is not None), valid=True)
        if not sections:
            raise CaptureError('Conversation has no messages')
        return result, '\n\n'.join(section[2] for section in sorted(sections)), sha(canonical(record))
...
class ConversationArchive:
    ...
    def _parse(self, body):
        if not isinstance(body, dict) or set(body) != {'format', 'content'} or body['format'] != 'chatgpt':
            raise CaptureError('Archive requires format chatgpt and original JSON content only')
        raw = body['content']
        ...
        try:
            records = json.loads(raw, parse_constant=lambda value: (_ for _ in ()).throw(ValueError('Nonfinite archive number')))
    ...
    def preview(self, body):
        ...
        return {'source_digest': source_digest, 'format': body['format'], 'total': len(rows), 'conversations': [
            {**row, 'existing_id': self._existing('archive:conversation:' + fingerprint)} for row, content, fingerprint in rows]}

    def commit(self, body):
        if not isinstance(body, dict) or set(body) != {'request_id', 'source_digest', 'format', 'content', 'conversation_ids'}:
            raise CaptureError('Commit requires original archive, reviewed digest, conversation_ids and request_id only')
        request_key(body['request_id'])
        source_digest, rows = self._parse({key: body[key] for key in ('format', 'content')})
        if source_digest != body['source_digest']:
            raise CaptureError('Archive changed since preview', 409)
        selected = body['conversation_ids']
        if not isinstance(selected, list) or not selected or any(not isinstance(key, str) for key in selected) or len(set(selected)) != len(selected):
        ...
        previous = self.db.execute('SELECT * FROM capability_knowledge_archives WHERE request_id=?', (body['request_id'],)).fetchone()
        if previous:
            if previous['payload_hash'] != payload_hash:
                raise CaptureError('Import request already belongs to different input', 409)
            if previous['receipt']:
                return json.loads(previous['receipt'])
        if runtime_home() != self.runtime_home:
            raise CaptureError('Runtime home changed; restore the bound allocation before importing archives', 409)
        if not previous:
            self.db.execute('INSERT INTO capability_knowledge_archives VALUES (?,?,NULL)', (body['request_id'], payload_hash))
            self.db.commit()
        self.files_root.mkdir(parents=True, exist_ok=True)
        path = self.files_root / ('archive-' + source_digest + '.json')
        if path.exists() and hashlib.sha256(path.read_bytes()).hexdigest() != source_digest:
        ...
        if not source_id:
            source_id = self.store.create_typed_item(item_type='document', title='Conversation archive ' + source_digest[:12], guid='archive:source:' + source_digest,
                extra={'file_path': str(path), 'file_size': path.stat().st_size, 'mime_type': 'application/json', 'file_metadata': {'content_hash': source_digest, 'original_filename': 'conversations.json'}})
        items = []
        for row, content, fingerprint in chosen:
            ...
            if not identity:
                identity = self.store.create_typed_item(item_type='note', title=row['title'], content=content, guid='archive:conversation:' + fingerprint,
                    extra={'file_metadata': {'archive_source_id': source_id, 'conversation_id': row['id'], 'original_at': row['created_at'], 'source_updated_at': row['updated_at'], 'source_record_hash': fingerprint, 'unsupported_parts': row['unsupported_parts']}})
            items.append({'conversation_id': row['id'], 'destination_id': identity, 'source_link': '#/knowledge/item/' + identity, 'status': status})
        receipt = {'request_id': body['request_id'], 'source_digest': source_digest, 'source_item_id': source_id,
                   'source_link': '#/knowledge/item/' + source_id, 'items': items, 'committed_at': datetime.now(timezone.utc).isoformat()}
        self.db.execute('UPDATE capability_knowledge_archives SET receipt=? WHERE request_id=?', (canonical(receipt), body['request_id']))
        self.db.commit()
        return receipt

    def list(self, limit=20, offset=0):
        if type(limit) is not int or type(offset) is not int or not 1 <= limit <= 100 or not 0 <= offset <= 1000000:
            raise CaptureError('Invalid pagination')
        total = self.db.execute('SELECT count(*) FROM capability_knowledge_archives WHERE receipt IS NOT NULL').fetchone()[0]
        items = [json.loads(row[0]) for row in self.db.execute('SELECT receipt FROM capability_knowledge_archives WHERE receipt IS NOT NULL ORDER BY rowid DESC LIMIT ? OFFSET ?', (limit, offset))]
