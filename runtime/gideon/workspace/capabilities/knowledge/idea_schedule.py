...
class IdeaSchedules:
    ...
    def save(self, identity, body):
        if not isinstance(body, dict) or set(body) != {
            ...
            raise CaptureError(
                "Idea schedule requires request_id, revision, enabled and minutes"
            )
        request = request_key(body["request_id"])
        if (
            type(body["minutes"]) is not int
            or not 5 <= body["minutes"] <= 1440
            or type(body["enabled"]) is not bool
            or type(body["revision"]) is not int
        ):
            raise CaptureError(
                "Schedule minutes must be5..1440 with boolean enabled and integer revision"
            )
        ...
        if not prior:
            ...
            if (
                self.ideas.get(identity)["status"]
                in ("owner_deleted", "collection_deleted")
                and body["enabled"]
            ):
            ...
            state.update(
                ...
                trigger_id="knowledge-ideas:" + state["action_id"],
            )
            self.db.execute("BEGIN IMMEDIATE")
            try:
                self.db.execute(
                    "UPDATE capability_knowledge_ideas SET body=? WHERE id=?",
                    (packed(state), identity),
                )
                self.db.execute(
        ...
        trigger = Trigger(
            id=state["trigger_id"],
            name="Idea-list vault exchange",
            kind="clock",
            enabled=state["sync_enabled"],
            created_by="knowledge-ideas",
            spec={"kind": "interval", "interval_secs": state["minutes"] * 60},
            workflow={
                "provider": "knowledge-ideas-sync",
                "config": {"id": state["action_id"]},
            },
            capabilities={"providers": ["knowledge-ideas-sync"]},
            delivery="none",
...
class IdeaSyncActionProvider(ActionProvider):
    ...
    async def execute(self, action_config, ctx, timeout=30):
        try:
            ...
            row = self.schedules.db.execute(
                "SELECT id FROM capability_knowledge_ideas WHERE json_extract(body,'$.action_id')=?",
                (action_config["id"],),
            ).fetchone()
            if row is None:
                raise CaptureError(
                    "Idea action binding belongs to a different allocation", 404
                )
            identity = row[0]
            ...
            prior = self.schedules.db.execute(
                "SELECT payload FROM capability_knowledge_idea_requests WHERE request_id=?",
                (request,),
            ).fetchone()
            expected_hash = (
            ...
            if result["outcome"] in ("conflict", "owner_deleted", "collection_deleted"):
                return ActionResult(
                    success=False,
                    error="Idea exchange requires review: " + result["outcome"],
                    exit_code=1,
                )
            return ActionResult(
