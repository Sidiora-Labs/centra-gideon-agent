...
class DateJournals:
    ...
    def projection(self, row):
        ...
        return {
            ...
            "title": row["title"],
            "content": row["content"],
            "revision": meta.get("journal_revision", 0),
            "fingerprint": digest({key: row[key] for key in COLUMNS}),
            "source_link": "#/knowledge/item/" + row["id"],
        }
    ...
    def get(self, date, timezone="UTC"):
        return {
            "date": date,
            "timezone": timezone,
            "journal": self.projection(self.raw(self.key(date, timezone))),
        }
    ...
    def draft(self, date, timezone="UTC"):
        ...
        sources = [
            row
            for section in ("completed_activity", "recent_notes")
            for row in snapshot["sections"][section]
    ...
    def save(self, body):
        ...
        if not isinstance(body, dict) or set(body) != required:
            raise CaptureError(
                "Journal save requires request_id, date, timezone, revision, fingerprint, title, content and preview_id only"
            )
        key, request = self.key(body["date"], body["timezone"]), request_key(
            body["request_id"]
        )
        if type(body["revision"]) is not int or body["revision"] < 0:
            raise CaptureError("Journal revision must be a nonnegative integer")
        for field, maximum, required_value in (
            ("title", 300, True),
            ("content", 100000, True),
            ("fingerprint", 128, False),
            ("preview_id", 128, False),
        ):
        ...
        try:
            prior = self.db.execute(
                "SELECT * FROM capability_knowledge_journal_mutations WHERE request_id=?",
                (request,),
            ).fetchone()
            if prior:
                if prior["payload"] != payload:
                    raise CaptureError(
                        "Journal request belongs to different input", 409
                    )
                if prior["receipt"]:
                    self.db.commit()
            ...
            initial_pending = (
                ...
                and row
                and body["revision"] == 0
                and not meta.get("journal_revision")
            ...
            if not recovered:
                ...
                if not initial_pending and (
                    (current["revision"] if current else 0) != body["revision"]
                    or (current["fingerprint"] if current else "")
                    != body["fingerprint"]
                ):
                    raise CaptureError(
                        "Journal changed in its canonical editor; reload before saving",
                        409,
                    )
                if row and (
                    row["item_type"] != "journal"
                    or row["status"] != "active"
                    or row["is_archived"]
                ):
                    raise CaptureError(
                        "Restore the canonical journal before editing", 409
                    )
                if body["preview_id"]:
                    approved = self.draft(body["date"], body["timezone"])
                    if approved["preview_id"] != body["preview_id"]:
                        raise CaptureError(
                            "Activity sources changed; review the draft again", 409
                        )
            if not prior:
                pending = self.db.execute(
                    "SELECT request_id FROM capability_knowledge_journal_mutations WHERE journal_key=? AND receipt IS NULL",
                    (key,),
                ).fetchone()
                if pending:
                    raise CaptureError(
                        "An earlier journal write must be retried first", 409
                    )
                self.db.execute(
                    "INSERT INTO capability_knowledge_journal_mutations VALUES (?,?,?,NULL)",
                    (request, payload, key),
                )
            self.db.commit()
        except Exception:
        ...
        if not recovered:
            metadata = {
                **meta,
                "journal_date": body["date"],
                "journal_timezone": body["timezone"],
                "original_at": body["date"],
                "journal_revision": body["revision"] + 1,
                "journal_last_request": request,
            }
            if body["preview_id"]:
                metadata["journal_activity_draft"] = approved
            if row:
                changed = self.store.update_item(
                    row["id"],
                    title=body["title"],
                ...
                if not changed:
                    ...
                    if (
                        not latest
                        or json.loads(latest["file_metadata"] or "{}").get(
                            "journal_last_request"
                        )
                        != request
                    ):
                        self.db.execute(
                            "DELETE FROM capability_knowledge_journal_mutations WHERE request_id=? AND receipt IS NULL",
                            (request,),
                        )
                        self.db.commit()
                        raise CaptureError(
                            "Canonical journal changed during save; original content retained",
                            409,
                        )
            else:
                ...
                if identity is None:
                    ...
                    if (
                        candidate
                        and candidate["title"] == body["title"]
                        and candidate["content"] == body["content"]
                        and not json.loads(candidate["file_metadata"] or "{}").get(
                            "journal_revision"
                        )
                    ):
                        self.store.update_item(
                            candidate["id"],
                            file_metadata=metadata,
                            expected={field: candidate[field] for field in COLUMNS},
                        )
        row = self.raw(key)
        if (
            not row
            or json.loads(row["file_metadata"] or "{}").get("journal_last_request")
            != request
        ):
            raise CaptureError(
                "Canonical journal write is pending; retry the same request", 409
            )
        receipt = {
            "request_id": request,
            "date": body["date"],
        ...
        self.db.execute(
            "UPDATE capability_knowledge_journal_mutations SET receipt=? WHERE request_id=? AND receipt IS NULL",
            (packed(receipt), request),
        )
        self.db.commit()
        return json.loads(
            self.db.execute(
                "SELECT receipt FROM capability_knowledge_journal_mutations WHERE request_id=?",
                (request,),
            ).fetchone()[0]
        )
