...
FIELDS = {'person': ('name', 'notes', 'identities'), 'project': ('name', 'brief'),
          'idea': ('title', 'content'), 'admin': ('title', 'description'), 'memory': ('text',)}
DESTINATIONS = {'person': 'people', 'project': 'projects', 'idea': 'fleeting', 'admin': 'tasks', 'memory': 'episodic'}
...
class TypedCapture:
    ...
    def preview(self, body):
        ...
        if not isinstance(kind, str) or kind not in FIELDS or not isinstance(fields, dict) or len(encoded(fields)) > 100000:
        ...
        if kind == 'person':
            ...
            for index, key in enumerate(FIELDS[kind]):
                mapped[key] = text_field(mapped.get(key, ''), key, 300 if key in ('name', 'title') else 100000, required=index == 0)
        stable = {**body, 'revision': capture['revision'], 'mapped_fields': mapped,
                  'unsupported_fields': sorted(set(fields) - set(FIELDS[kind])), 'destination': DESTINATIONS[kind]}
        available = kind != 'memory' or self.memory is not None
        return {**stable, 'preview_id': digest(stable), 'available': available,
                'unavailable_reason': '' if available else 'The bound memory service is unavailable'}
    ...
    def list(self, limit=20, offset=0):
        ...
        total = self.db.execute('SELECT count(*) FROM capability_knowledge_types WHERE receipt IS NOT NULL').fetchone()[0]
        items = [json.loads(row[0]) for row in self.db.execute('SELECT receipt FROM capability_knowledge_types WHERE receipt IS NOT NULL ORDER BY rowid DESC LIMIT ? OFFSET ?', (limit, offset))]
    ...
    async def commit(self, body):
        if not isinstance(body, dict) or set(body) != {'request_id', 'preview_id', 'capture_id', 'revision', 'kind', 'fields'}:
            raise CaptureError('Commit requires the reviewed preview and request_id only')
        request_key(body['request_id'])
        payload = encoded(body)
        previous = self.db.execute('SELECT * FROM capability_knowledge_types WHERE capture_id=? OR request_id=?', (body['capture_id'], body['request_id'])).fetchone()
        if previous:
        ...
        preview = self.preview({key: body[key] for key in ('capture_id', 'kind', 'fields')})
        if type(body['revision']) is not int or preview['revision'] != body['revision'] or preview['preview_id'] != body['preview_id']:
        ...
        receipt.update(destination=preview['destination'], destination_id=target, source_link=link,
            original_fields=body['fields'], unsupported_fields=preview['unsupported_fields'],
            original_capture=self.inbox.get(body['capture_id']), committed_at=datetime.now(timezone.utc).isoformat())
        self.db.execute('UPDATE capability_knowledge_types SET receipt=? WHERE capture_id=? AND receipt IS NULL', (encoded(receipt), body['capture_id']))
        self.db.commit()
        row = self.db.execute('SELECT receipt FROM capability_knowledge_types WHERE capture_id=?', (body['capture_id'],)).fetchone()
        return json.loads(row[0])

    async def _destination(self, preview):
        kind, fields = preview['kind'], preview['mapped_fields']
        if kind in ('project', 'admin') and configuration_home(os.environ.get('GIDEON_HOME'), Path.home() / CONFIG_DIR_NAME, logging.getLogger(__name__)).resolve() != self.home:
            raise CaptureError('Runtime home changed; restore the bound allocation before importing projects or tasks', 409)
        marker = 'typed_capture:' + preview['capture_id']
        identity = digest({'capture': preview['capture_id'], 'kind': kind})[:24]
        if kind == 'person':
            buffer = io.StringIO()
            writer = csv.DictWriter(buffer, fieldnames=['name', 'notes', 'email', 'phone', 'handle'])
            writer.writeheader()
            writer.writerow({**{key: fields[key] for key in ('name', 'notes')}, **{key: '|'.join(item['value'] for item in fields['identities'] if item['kind'] == key) for key in ('email', 'phone', 'handle')}})
            data = {'format': 'csv', 'content': buffer.getvalue()}
            people = PeopleStore(root=self.home / 'capabilities/communications')
            receipt, _ = imports.commit(people, {**data, 'source_digest': imports.preview(people, data)['source_digest'], 'decisions': [{'row_id': '1', 'action': 'create'}]})
        ...
        if kind == 'project':
            ...
            project = hierarchy.get_project('p-' + identity) or hierarchy.create_project(**fields)
            return project.id, '#/projects/' + project.id
        if kind == 'admin':
            tasks = BoundTasks(self.home, 't-' + identity)
            task = await tasks.get_task('t-' + identity) or await tasks.create_task(**fields)
        ...
        if not rows:
            if not self.memory.write_episodic(fields['text'], source=marker, tags=[marker]):
                raise CaptureError('The memory service declined this import', 409)
            rows = self.memory.episodic_list(limit=2, tag_filter=[marker])
        if len(rows) != 1:
            raise CaptureError('No unique canonical memory destination was recorded', 409)
