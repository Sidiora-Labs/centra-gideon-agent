...
def window(period, date, timezone_name):
    ...
    try:
        start = datetime.combine(
            day - timedelta(days=6 if period == "weekly" else 0), time.min, zone
        )
        end = datetime.combine(day + timedelta(days=1), time.min, zone)
    except OverflowError:
        raise CaptureError(
            "Review date lies outside the supported calendar window"
        ) from None
...
class ReviewService:
    ...
    def assert_scope(self):
        if CaptureInbox._runtime_home() != self.home:
            raise CaptureError(
                "Runtime home changed; restore the bound allocation before saving reviews",
                409,
            )

    def receipt(self, key):
        row = self.db.execute(
            "SELECT receipt FROM capability_knowledge_reviews WHERE request_id=?",
            (key,),
        ).fetchone()
    ...
    def preview(self, period, date, timezone="UTC"):
        ...
        limitations = [
            "Completed activity uses current done status and updated_at, not an immutable completion event.",
            "Naive source timestamps are interpreted in the review timezone.",
        ]
        ...
        tasks = (
            TopicTasks(self.home)._all_tasks() if (self.home / "tasks").is_dir() else []
        )
            ...
            if status == "done":
                if updated and start <= updated < end:
                    add(
                        ...
                        task.id,
                        task.title,
                        "Currently done; updated " + updated.isoformat(),
                        link,
                    )
                continue
        ...
        people = (
            PeopleStore(people_root)
            if (people_root / "people.sqlite3").is_file()
            else None
        )
        for person in rows("contacts", people.people() if people else []):
            ...
            if state["state"] in ("overdue", "missing"):
                section = (
                    ...
                    else "contacts_missing"
                )
                detail = (
                    f"{state['days_overdue']} days beyond contact cadence"
                    if state["state"] == "overdue"
                    else "No recorded contact history"
                )
                add(
                    section,
                    "person",
                    ...
                    person["name"],
                    detail,
                    "#/capabilities/communications?person=" + person["id"],
                )
        notes = self.db.execute(
            "SELECT id,title,created_at,file_metadata FROM items WHERE item_type IN ('note','journal','fleeting') AND status='active' AND COALESCE(is_archived,0)=0 AND (guid IS NULL OR substr(guid,1,7) != 'review:') AND (file_metadata IS NULL OR json_extract(file_metadata,'$.review_snapshot') IS NULL) ORDER BY created_at DESC,id LIMIT ?",
            (LIMIT + 1,),
        ).fetchall()
        for note in rows("notes", notes):
            ...
            if (
                not metadata.get("review_snapshot")
                and created
        ...
        if "capability_knowledge_captures" in tables:
            exclusion = (
                " AND id NOT IN (SELECT capture_id FROM capability_knowledge_types WHERE receipt IS NOT NULL)"
                if "capability_knowledge_types" in tables
                else ""
            )
            captures = self.db.execute(
                "SELECT id,original_text,captured_at FROM capability_knowledge_captures WHERE destination_id IS NULL"
                + exclusion
        ...
        for capture in rows("captures", captures):
            ...
            if captured and captured < end:
                add(
                    ...
                    "capture",
                    capture["id"],
                    capture["original_text"][:150] or "Voice capture",
                    "Captured " + captured.isoformat(),
                    "#/capabilities/knowledge/capture?capture=" + capture["id"],
                )
    ...
    def save(self, body, *, scheduled=False):
        if not isinstance(body, dict) or set(body) != {
            ...
            raise CaptureError(
                "Review save requires request_id, period, date, timezone, preview_id and reflection only"
            )
        ...
        previous = self.db.execute(
            "SELECT * FROM capability_knowledge_reviews WHERE request_id=?", (key,)
        ).fetchone()
        if previous:
            if previous["payload"] != payload:
                raise CaptureError(
                    "Review request already belongs to different input", 409
                )
            if previous["receipt"]:
        ...
        if snapshot["preview_id"] != body["preview_id"]:
            raise CaptureError(
                "Review sources changed; preview again before saving", 409
            )
        if not previous:
            self.db.execute(
                "INSERT OR IGNORE INTO capability_knowledge_reviews VALUES (?,?,NULL)",
                (key, payload),
            )
            self.db.commit()
        ...
        lines = [
            ...
            "",
            "Reflection",
            body["reflection"] or "(No reflection recorded)",
            "",
        ]
        for section, values in snapshot["sections"].items():
            lines.extend(
                [
                    ...
                    *[
                        f"- [{row['title']}]({row['source_link']}) — {row['detail']}"
                        for row in values
                    ],
                    "",
                ]
        ...
        if snapshot["truncated"]:
            lines.append(
                "Source scan limit reached: " + ", ".join(snapshot["truncated"])
            )
        row = self.db.execute(
            "SELECT id FROM items WHERE guid=?", ("review:" + key,)
        ).fetchone()
        identity = (
            row[0]
            if row
        ...
        if identity is None:
            ...
            if recovered is None:
                raise CaptureError(
                    "Canonical review destination was not persisted", 409
                )
        ...
        self.db.execute(
            "UPDATE capability_knowledge_reviews SET receipt=? WHERE request_id=? AND receipt IS NULL",
            (packed(receipt), key),
        )
        self.db.commit()
    ...
    def list(self, limit=20, offset=0):
        ...
        total = self.db.execute(
            "SELECT count(*) FROM capability_knowledge_reviews WHERE receipt IS NOT NULL"
        ).fetchone()[0]
        items = [
            json.loads(row[0])
            for row in self.db.execute(
                "SELECT receipt FROM capability_knowledge_reviews WHERE receipt IS NOT NULL ORDER BY rowid DESC LIMIT ? OFFSET ?",
                (limit, offset),
            )
        ]
        return {
