...
def parse(content):
    ...
    try:
        if any(isinstance(token, (yaml.tokens.AnchorToken, yaml.tokens.AliasToken)) for token in yaml.scan(block)):
            raise ValueError('YAML aliases are not supported')
        metadata = yaml.load(block, Loader=yaml.BaseLoader)
        if not isinstance(metadata, dict) or len(metadata) > 40 or any(not isinstance(key, str) or not re.fullmatch(r'[A-Za-z][A-Za-z0-9_]*', key) for key in metadata):
            raise ValueError('Invalid frontmatter fields')
        if any(not isinstance(value, str) and not (isinstance(value, list) and len(value) <= 50 and all(isinstance(part, str) for part in value)) for value in metadata.values()):
    ...
    except (ValueError, TypeError, AttributeError, yaml.YAMLError, RecursionError) as exc:
        raise CaptureError('Invalid idea-list metadata: ' + str(exc)) from None
    tags = metadata.get('tags', [])
    if not isinstance(tags, list) or not any(tag.casefold().lstrip('#') == 'idea-loom' for tag in tags):
    ...
    for line in lines:
        if re.fullmatch(r'#{1,6}\s+Help\s*:?', line.strip(), re.IGNORECASE):
            ...
            section = 'ideas'
        elif section == 'ideas' or (section != 'help' and re.match(r'^\d+\.\s+', line.strip())):
            if not line.strip():
                continue
            match = re.fullmatch(r'(\d+)\.\s+(.+)', line.strip())
            if not match or int(match[1]) != len(ideas) + 1 or len(ideas) >= 100:
                raise CaptureError('Ideas must be densely numbered from 1, at most100 entries')
    ...
    extra = {key: value for key, value in metadata.items() if key not in (*FIELDS, 'uuid', 'createdAt', 'updatedAt')}
    return {'id': identity, 'title': metadata['title'], 'category': metadata['category'], 'status': metadata['status'], 'created': created, 'modified': modified,
            'tags': tags, 'prompt': prompt, 'help': help_text, 'ideas': ideas, 'extra': extra}


def render(document):
    fields = [(key, document[key]) for key in FIELDS] + list(document.get('extra', {}).items())
    lines = [frontmatter(fields), '', '# ' + document['prompt']]
    if document['help']:
        lines += ['', '## Help', document['help']]
    if document['ideas']:
        lines += ['', '## Ideas', *[f'{index + 1}. {text}' for index, text in enumerate(document['ideas'])]]
    return '\n'.join(lines) + '\n'


def preview(content):
    document = parse(content)
    return {'preview_id': digest(document), 'document': document, 'unsupported_fields': sorted(document['extra'])}
