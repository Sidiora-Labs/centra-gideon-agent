...
def _knowledge_rows(store):
    ...
    for row in rows:
        ...
        original = meta.get("original_at", meta.get("source_created_at", row["created_at"]))
        yield {
            "source_type": row["item_type"], "source_id": row["id"],
            "original_at": original, "title": row["title"], "content": row["content"],
...
def _memory_rows(service):
    ...
    while True:
        ...
        for record in records:
            ...
            yield {
                "source_type": "memory", "source_id": record["id"],
                "original_at": original, "title": content.splitlines()[0][:100] if content else "Memory",
                "content": content,
...
def source_record(knowledge_store, memory_service, source_type, source_id):
    rows = _memory_rows(memory_service) if source_type == "memory" else _knowledge_rows(knowledge_store)
    return next((row for row in rows if row["source_type"] == source_type and row["source_id"] == source_id), None)


def anniversaries(knowledge_store, memory_service=None, *, date=None, timezone="UTC", limit=20, offset=0):
    if isinstance(limit, bool) or isinstance(offset, bool) or not isinstance(limit, int) or not isinstance(offset, int):
    ...
    for row in chain(_knowledge_rows(knowledge_store), _memory_rows(memory_service)):
        try:
            ...
            skipped += 1
            continue
        if original.year >= day.year or (original.month, original.day) != (day.month, day.day):
            continue
        content = row.pop("content")
        row["excerpt"] = " ".join(content.split())[:240]
        row["years_ago"] = day.year - original.year
        matches.append(row)
    matches.sort(key=lambda row: (row["years_ago"], row["source_type"], row["source_id"]))
    total = len(matches)
    return {
        "date": day.isoformat(), "timezone": timezone, "leap_day_policy": "exact",
        "naive_timestamp_policy": "local_wall_clock", "items": matches[offset:offset + limit],
        "total": total, "offset": offset, "limit": limit,
        "next_offset": offset + limit if offset + limit < total else None,
        "skipped_invalid_dates": skipped,
        "sources": {"knowledge": "available", "memory": "available" if memory_service is not None else "unavailable"},
    }
