...
from gideon.workspace.capabilities.knowledge.idea_schedule import (
    IdeaSchedules,
    IdeaSyncActionProvider,
)
...
_TYPE_FIELDS = {
    "capture_id": _ID,
    "kind": {"enum": ["person", "project", "idea", "admin", "memory"]},
    "fields": {"type": "object"},
}
_ARCHIVE_FIELDS = {
    "format": {"enum": ["chatgpt"]},
...
_JOURNAL_FIELDS = {
    key: value for key, value in _REVIEW_FIELDS.items() if key != "period"
}
_IDEA_CONTENT = {"type": "string", "minLength": 1, "maxLength": 262144}
_VIDEO_PREVIEW = {
    "url": {"type": "string", "maxLength": 2048},
    "title": {"type": "string", "minLength": 1, "maxLength": 300},
    "format": {"enum": ["vtt", "srt", "json"]},
...
_TOOLS = {
    ...
    "knowledge_rsvp_save": (
        ...
        True,
        {
            ...
            "wpm": {"type": "integer", "minimum": 100, "maximum": 1000},
            "chunk_size": {"enum": [1, 2]},
            "content_revision": _ID,
        },
        ["id", "word_index", "wpm", "chunk_size", "content_revision"],
    ),
    "knowledge_rsvp_bookmark": (
        ...
        ["id", "word_index", "content_revision"],
    ),
    ...
    "knowledge_vault_read": (
        ...
        False,
        {"id": _ID, "path": {"type": "string", "minLength": 1, "maxLength": 1000}},
        ["id", "path"],
    ),
    "knowledge_vault_write": (
        ...
        ["id", "request_id", "path", "content", "expected_hash"],
    ),
    ...
    "knowledge_video_import": (
        ...
        True,
        {**_VIDEO_PREVIEW, "request_id": _REQUEST, "preview_id": _ID},
        ["request_id", *_VIDEO_PREVIEW, "preview_id"],
    ),
    "knowledge_video_fetch": (
        ...
        {
            "request_id": _REQUEST,
            "url": _VIDEO_PREVIEW["url"],
            "language": _VIDEO_PREVIEW["language"],
            "transcript": {"type": "boolean"},
            "video": {"type": "boolean"},
            "audio": {"type": "boolean"},
        },
        ["request_id", "url", "language", "transcript", "video", "audio"],
    ),
    ...
    "knowledge_idea_import": (
        ...
        ["request_id", "content", "preview_id", "expected_hash"],
    ),
    ...
    "knowledge_idea_sync": (
        ...
        True,
        {"id": _ID, "request_id": _REQUEST, "expected_hash": _ID},
        ["id", "request_id", "expected_hash"],
    ),
    ...
    "knowledge_journal_save": (
        ...
        True,
        {
            **_JOURNAL_FIELDS,
            ...
            "preview_id": {"type": "string", "maxLength": 128},
            "title": {"type": "string", "minLength": 1, "maxLength": 300},
    ...
    "knowledge_review_save": (
        ...
        True,
        {
            **_REVIEW_FIELDS,
            "request_id": _REQUEST,
            "preview_id": _ID,
            "reflection": {"type": "string", "maxLength": 100000},
        },
        [*_REVIEW_FIELDS, "request_id", "preview_id", "reflection"],
    ),
    ...
    "knowledge_review_schedule_save": (
        ...
        ["request_id", "period", "timezone", "time", "weekday", "enabled"],
    ),
    ...
    "knowledge_archive_commit": (
        ...
        [*_ARCHIVE_FIELDS, "request_id", "source_digest", "conversation_ids"],
    ),
    ...
    "knowledge_type_commit": (
        ...
        [*_TYPE_FIELDS, "request_id", "preview_id", "revision"],
    ),
    "knowledge_type_list": (
    ...
    "knowledge_anniversary_source": (
        ...
        False,
        {
            "source_type": {"enum": ["note", "journal", "fleeting", "memory"]},
            "source_id": _ID,
        },
    ...
    "knowledge_capture_route": (
        ...
        True,
        {
            ...
            "request_id": _REQUEST,
            "revision": {"type": "integer", "minimum": 1},
            "destination": {"enum": ["note", "journal", "fleeting"]},
            "title": {"type": "string", "minLength": 1, "maxLength": 300},
            "content": {"type": "string", "minLength": 1, "maxLength": 100000},
        },
...
class KnowledgeCapabilityTools(ToolProvider):
    ...
    async def list_tools(self):
        return [
            ToolDefinition(
                ...
                requires_approval=definition[1],
                risk_level=RiskLevel.CAUTION if definition[1] else RiskLevel.SAFE,
            )
    ...
    def _bound(self, writes):
        if self._services is None:
            ...
            self._store = (
                self._services.state.knowledge_store if self._services else None
            )
        if self._services is None:
            raise CaptureError(
                "Runtime services are unavailable; start the gateway.", 503
            )
        key = get_current_session_key()
        if not key:
            raise CaptureError(
                "An active session is required for personal knowledge tools", 403
            )
        ...
        if session_restrictions.is_temporary(key) or (session and session.blocks_reads):
            raise CaptureError(
                "This temporary session cannot read personal knowledge", 403
            )
        if writes and (
            session_restrictions.is_restricted(key)
            or key in state._restricted_keys
            or (session and session.is_restricted)
        ):
    ...
    async def invoke(self, tool_name, arguments):
        ...
        try:
            ...
            service = (
                MemoryService.over_vector_store(archive)
                if archive is not None
                else None
            )
            if tool_name.startswith("knowledge_rsvp_"):
                if self._rsvp is None:
                    self._rsvp = RsvpStates(self._store)
                identity = arguments["id"]
                if tool_name == "knowledge_rsvp_get":
                    result = self._rsvp.get(identity)
                elif tool_name == "knowledge_rsvp_save":
                    result = self._rsvp.save(
                        identity,
                        {key: value for key, value in arguments.items() if key != "id"},
                    )
                elif tool_name == "knowledge_rsvp_bookmark":
                    result = self._rsvp.bookmark(
                        identity,
                        {key: value for key, value in arguments.items() if key != "id"},
                    )
                else:
                    result = self._rsvp.restore(identity)
            elif tool_name.startswith("knowledge_vault_"):
                if self._vaults is None:
                    self._vaults = ExternalVaults(self._store, self._home)
                identity = arguments.get("id")
                if tool_name == "knowledge_vault_list":
                    result = self._vaults.list()
                elif tool_name == "knowledge_vault_register":
                    result = self._vaults.register(arguments)
                elif tool_name == "knowledge_vault_scan":
                    result = self._vaults.scan(identity)
                elif tool_name == "knowledge_vault_read":
                    result = self._vaults.detail(identity, arguments["path"])
                elif tool_name == "knowledge_vault_write":
                    result = self._vaults.write(
                        identity,
                        {key: value for key, value in arguments.items() if key != "id"},
                    )
                elif tool_name == "knowledge_vault_delete":
                    result = self._vaults.delete(
                        identity,
                        {key: value for key, value in arguments.items() if key != "id"},
                    )
                elif tool_name == "knowledge_vault_search":
                    result = self._vaults.search(identity, arguments["query"])
                else:
                    result = self._vaults.graph(identity)
                ...
                if tool_name == "knowledge_video_list":
                    ...
                    result = self._videos.start_fetch(
                        arguments, get_current_session_key()
                    )
                ...
                if self._ideas is None:
                    ...
                    register_action_provider(
                        IdeaSyncActionProvider(self._idea_schedules)
                    )
                ...
                if self._reviews is None:
                    ...
                    register_action_provider(
                        ReviewActionProvider(self._review_schedules)
                    )
            ...
            unavailable = tool_name == "knowledge_capture_transcribe" and result[
                "status"
            ] in {"transcription_unavailable", "transcription_failed"}
            return ToolResult(
                success=not unavailable,
                output=json.dumps(_public(result), ensure_ascii=False),
                error=result["error"] if unavailable else "",
                metadata={"source": self.name},
                recovery_hints=(
                    ["Configure a speech provider and retry the preserved capture."]
                    if unavailable
                    else []
                ),
            )
...
def create_provider(config=None):
    if config:
        raise ValueError(
            "Personal knowledge tools use bound runtime services, not selector configuration"
        )
