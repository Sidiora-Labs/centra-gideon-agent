...
* :mod:`~gideon.assurance.ledger.kinds` — the shared event vocabulary. ONE registry, so a second
  producer speaks the same words instead of adding a dialect.
* :mod:`~gideon.assurance.ledger.writer` — `LedgerWriter`: sequencing, stamping, redaction, the
  `events.jsonl` mirror, the oversize/binary spill.
* :mod:`~gideon.assurance.ledger.redaction` — what must never reach the file, and what must never sit
  inline in it.
* :mod:`~gideon.assurance.ledger.hashing` — canonical serialization and content hashing.
* :mod:`~gideon.assurance.ledger.reader` — reading it back.
* :mod:`~gideon.assurance.ledger.outcomes` — the outcome record: any producer's bet on what would
  LAND, and the idempotent, `measured`-vs-`inconclusive` resolution that closes it (PP-9).

Nothing here imports from `gideon.automation.workflows`, and nothing here may: the direction of that
dependency is the whole point. A producer supplies its own :class:`~gideon.assurance.ledger.writer
.LedgerStore` and its own typed emitters; `gideon.automation.workflows.journal` is the first such
producer and, for now, the only one.
"""

from __future__ import annotations

from gideon.assurance.ledger.hashing import hash_value, stable_json
...
from gideon.assurance.ledger.outcomes import (
    CONSUMED,
    DORMANCY_CYCLES,
    DORMANT,
    INCONCLUSIVE,
...
from gideon.assurance.ledger.reader import (
    journal_only_kinds,
    read_events,
    read_journal,
    run_totals,
)
from gideon.assurance.ledger.redaction import is_binary_payload, redact
from gideon.assurance.ledger.writer import (
    EVENTS_FILE,
    JOURNAL_FILE,
