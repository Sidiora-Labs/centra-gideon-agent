...
(:class:`gideon.assurance.selfqa.findings.ScenarioFinding.fix_branch`); a human pushes or merges it, or
does neither.
...
:mod:`gideon.automation.loop.worktree`'s discipline — a fixed argv, no shell, a hex-validated ref, a
...
def _git(repo: Path, *args: str) -> tuple[int, str]:
    ...
    Mirrors :func:`gideon.automation.loop.worktree._git`: the ``build`` ceiling is delivered by the
...
def create_fix_branch(repo: Path | str, sha: str, *, enabled: bool) -> FixBranchResult:
    ...
    if not enabled:
        return FixBranchResult(
            created=False, branch=branch, reason="fix_branch_enabled is off"
        )
    ...
    if not git_available():
        return FixBranchResult(
            created=False, branch=branch, reason="git is not available"
        )
