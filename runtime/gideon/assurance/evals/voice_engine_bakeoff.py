...
This is NOT the ES-10 traffic bake-off (:mod:`gideon.assurance.evals.bakeoff`). That one
...
CRITERIA: list[Criterion] = [
    Criterion(
        ...
        0.25,
        "Gideon runs on the user's own Mac; a first-class MPS path is the load-bearing axis.",
    ),
    Criterion(
        "quality_proxy",
...
_OMNIVOICE_HF = Citation(
    "OmniVoice model card", "https://huggingface.co/k2-fsa/OmniVoice"
)
_OMNIVOICE_MLX = Citation(
    "OmniVoice MLX (Apple Silicon) weights",
    "https://huggingface.co/mlx-community/OmniVoice",
)
...
def candidates() -> list[EngineCandidate]:
    ...
    judgment mapping onto Gideon's use case (see the companion plan doc for each rationale).
    """
    omnivoice = EngineCandidate(
        ...
        metrics={
            ...
            "language_coverage": Metric(
                "600+ languages", 1.0, LITERATURE, _OMNIVOICE_GH
            ),
        },
    )
    cosyvoice = EngineCandidate(
        ...
        metrics={
            ...
            "footprint": Metric(
                "0.5B params (CosyVoice2/3-0.5B)", 0.70, LITERATURE, _COSY_GH
            ),
        ...
        if missing:
            cells.append(
                ScoredCell(
                    crit,
                    False,
                    f"unknown for: {', '.join(missing)} — excluded, not guessed",
                )
            )
        else:
...
def run_bakeoff() -> BakeoffReport:
    ...
    rejection_notes = (
        ...
        "Apple-Silicon/MPS path, and Gideon runs on the user's Mac; (2) its install is a "
        ...
        "in. Re-evaluate if Gideon ever grows a server-side or CUDA synthesis tier."
    )
    deferred = [
    ...
    *,
    ...
    if importlib.util.find_spec(module) is None:
        return MeasureResult(
            engine, True, f"engine package '{module}' not installed on this host"
        )
    ...
    if not wavs:
        return MeasureResult(
            engine, True, f"no reference-audio fixtures (*.wav) under {fdir}"
        )
...
def format_report(report: BakeoffReport) -> str:
    ...
    lines: list[str] = [
        "Voice-clone engine bake-off (MI-6) — OmniVoice vs CosyVoice",
        "",
    ]
    ...
    lines.append(
        f"{'WEIGHTED SCORE (kept criteria)':<40}{'':>5}  " + "  ".join(score_cells)
    )
    lines += [
        "",
        "VERDICT",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--fixtures",
        default="fixtures/voice",
        help="dir of reference-audio *.wav fixtures",
    )
