...
`MatrixSpec`/:func:`~gideon.assurance.evals.matrix.expand_cells`/
:func:`~gideon.assurance.evals.matrix.aggregate` and the same `matrices/<id>/` artifact sinks.
...
def use_case_for_tier(tier: str) -> str:
    ...
    if tier not in table:
        raise JudgeBenchError(
            f"unknown judge tier {tier!r} (known: {', '.join(sorted(table))})"
        )
    return table[tier]
...
def parse_fixture_set(data: dict, *, path: str = "") -> FixtureSet:
    ...
    for raw in raw_fixtures:
        if not isinstance(raw, dict):
            raise JudgeBenchError(
                f"{name}: a fixture must be an object, got {type(raw).__name__}"
            )
...
def render_bench_prompt(
    ...
    *,
    ...
    if position not in POSITIONS:
        raise JudgeBenchError(
            f"unknown position {position!r} (known: {', '.join(POSITIONS)})"
        )
...
def _audit_cost_since(started_ts: float, use_case: str) -> tuple[float | None, str]:
    ...
    try:
        ...
        rows = [
            ...
            if float(r.get("ts") or 0.0) >= started_ts
            and str(r.get("use_case") or "") == use_case
        ]
    except (
        Exception
    ):  # noqa: BLE001 - an unreadable audit means unknown cost, not a failed run
...
async def live_judge_caller(prompt: str, *, use_case: str) -> JudgeCall:
    ...
    return JudgeCall(
        text=str(text or ""), elapsed_secs=elapsed, cost_usd=cost, model=model
    )
...
def direction_for(
    judge_verdict: str, known_verdict: str, *, fixture_id: str = ""
) -> str:
...
async def observe_cell(
    ...
    *,
    ...
    if not verdicts:
        reason = (
            "; ".join(call_errors[:2]) if call_errors else "no parseable judge verdict"
        )
        return Observation(
    ...
    return Observation(
        ...
        model=model,
        note=(
            fixture.failure_note
            if final.passed and fixture.kind != FIXTURE_REAL
            else ""
        ),
    )
...
def adequacy(stats: RowStats) -> tuple[bool, list[str]]:
    ...
    if stats.separation is None:
        reasons.append(
            "strong-vs-null separation was never measured (no paired fixture scored)"
        )
    ...
    if stats.flip_rate is None:
        reasons.append(
            "position-swap flip rate was never measured (no fixture judged both ways)"
        )
...
def build_table(observations: list[Observation]) -> list[TableRow]:
    ...
    for (rubric_class, tier, samples), group in groups.items():
        ...
        agreements = [
            1.0 if o.direction == DIRECTION_AGREEMENT else 0.0 for o in scored
        ]
        ...
        forbidden_missed = [
            o
            for o in scored
            if o.kind == FIXTURE_FORBIDDEN and o.verdict == VERDICT_PASS
        ]
        stats = RowStats(
            ...
            false_rejects=sum(
                1 for o in scored if o.direction == DIRECTION_FALSE_REJECT
            ),
        ...
        for obs in absent:
            notes.append(
                f"{obs.fixture_id} at position {obs.position}: absent — {obs.note}"
            )
        spread = {
            tuple(sorted(set(o.sample_verdicts)))
            for o in scored
            if len(o.sample_verdicts) > 1
        }
    ...
    * :data:`REC_RECOMMENDED` — the lowest-cost adequate row, tie-broken by fewer samples
    ...
    for rubric_class in classes:
        ...
        if not adequate:
            ...
            out.append(
                Recommendation(
                    rubric_class=rubric_class, verdict=REC_NO_ADEQUATE_TIER, notes=notes
                )
            )
            continue
        ...
        if not priced:
            out.append(
                Recommendation(
                    ...
                    notes=[
                        f"{r.tier}/samples={r.samples} is adequate but unpriced"
                        for r in adequate
                    ]
                    + [
        ...
        if use_case:
            try:
                ...
                Exception
            ):  # noqa: BLE001 - an unresolvable ref is a blank, not a failure
                logger.debug(
                    "could not resolve a model ref for tier %s",
                    best.tier,
                    exc_info=True,
                )
        notes = [
...
def build_specs(
    ...
    *,
    ...
    :func:`~gideon.assurance.evals.matrix.expand_cells`, so the crossing rule stays one rule.
    """
    for tier in tiers:
        use_case_for_tier(tier)
    paired_ids = [
        f.id for f in fixture_set.fixtures if fixture_set.counterpart(f) is not None
    ]
    unpaired_ids = [
        f.id for f in fixture_set.fixtures if fixture_set.counterpart(f) is None
    ]
    paired = MatrixSpec(
...
async def run_judge_bench(
    ...
    *,
    ...
    :func:`~gideon.assurance.evals.pinning.compute_pin_for_subject` because the subject is a
