...
:mod:`gideon.assurance.evals.studies` is the instrument: pre-registration, the `locked/` check
...
1. **The harvested-suite adapter** (:func:`harvested_study_cases`). §2's corpus is
   ...
   :class:`~gideon.assurance.evals.harvest.EmptyHarvestError` rather than returning ``[]`` — a
...
2. **The arm prompt** (:func:`render_arm_prompt`) — the ONE place a template body becomes
   ...
   :func:`gideon.automation.workflows.bindings.resolve`, never a second interpolation dialect,
...
bound, not an oversight: :func:`gideon.automation.workflows.service.start_run` requires a live
...
from gideon.assurance.evals import harvest, store, studies
from gideon.assurance.evals.studies import (
...
class StudyArmError(StudyError):
    ...
    A subclass of :class:`~gideon.assurance.evals.studies.StudyError` so a caller that already
...
class HarvestedSuite:
    ...
    def low_power(self) -> bool:
        """Below :data:`~gideon.assurance.evals.studies.LOW_POWER_CASES` decided cases.
...
def harvested_study_cases(*, workflow_name: str = "") -> HarvestedSuite:
    """The harvested suite as :class:`~gideon.assurance.evals.studies.StudyCase` rows.
    ...
    (:func:`gideon.assurance.evals.harvest._screen`). Re-screening them here is the documented
    ...
    Catches :class:`~gideon.assurance.evals.harvest.EmptyHarvestError` on purpose. Letting it
    ...
    try:
        ...
        logger.info(
            "study: no harvested population%s",
            f" for {workflow_name!r}" if workflow_name else "",
        )
...
def arm_bodies_for_ops(spec: dict, ops: Sequence[dict]) -> tuple[str, str]:
    ...
    if not parsed:
        raise StudyArmError(
            "a template study needs at least one op to define its NEW arm"
        )
    candidate, issues = mutations.apply_batch(parsed, spec, {})
    if issues:
        raise StudyArmError(
            "the NEW arm could not be built: " + "; ".join(i.code for i in issues)
        )
...
class TemplateArmRunner:
    """The production :data:`~gideon.assurance.evals.studies.ArmRunner`.
    ...
    async def __call__(self, payload: WorkerPayload) -> ArmOutput:
        ...
        try:
            ...
            logger.warning(
                "study arm %s/%s failed", payload.case_id, payload.arm, exc_info=True
            )
            return ArmOutput(
                output="",
                workspace=str(workspace),
...
def _arm_cost_since(started_ts: float, use_case: str) -> tuple[float | None, str]:
    ...
    """
    from gideon.assurance.evals.judge_bench import _audit_cost_since
...
def register_template_study(
    *,
    ...
    resolved = (
        ...
        else harvested_study_cases(workflow_name=workflow_name)
    )
...
async def _live_spec(workflow_name: str) -> dict | None:
    ...
    for name in defs_mod.list_providers():
        ...
        try:
            ...
            logger.debug(
                "study: provider %s could not read %r",
                name,
                workflow_name,
                exc_info=True,
            )
