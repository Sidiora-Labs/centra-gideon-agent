...
:mod:`gideon.assurance.evals.harvest` writes cases built from real runs into the same
...
* ``tiers`` (list[str]) — which evaluation tiers the scenario opted into. ``"gate"`` is
  ES-6's Loop-2 cheap subset (:mod:`gideon.assurance.evals.gate`); the manifest records it so
...
* ``fixture_home`` (str) — the NAME of a ``tests_fixtures/`` seed the run is
  ...
  temp home (see :mod:`gideon.assurance.evals.child`); a scenario naming a fixture
...
def origin_of(path: Path, data: dict) -> str:
    ...
    (:mod:`gideon.assurance.evals.harvest`) rather than from a side list of names: a case whose
...
def install_library() -> dict:
    ...
    for src in sorted(packaged_library_dir().glob("*.json")):
        try:
            ...
            logger.warning(
                "skipping unparseable shipped scenario %s", src, exc_info=True
            )
            continue
        dst = target / src.name
        write = True
    ...
    for path in sorted(target.iterdir()):
        ...
        try:
            ...
            logger.warning(
                "skipping unparseable installed scenario %s", path, exc_info=True
            )
            continue
        installed[path.stem] = {
...
def resolve_fixture_home(path: str | Path) -> str:
    ...
    from gideon.operations.seed import _resolve_fixture  # noqa: PLC0415 - cycle-free
    from gideon.operations.seed import SeedError
