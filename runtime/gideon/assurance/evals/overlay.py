...
class ComponentOverlay:
    ...
    def __post_init__(self) -> None:
        if self.kind not in KINDS:
            raise ValueError(
                f"unknown component kind {self.kind!r} — expected one of {KINDS}"
            )
...
def throwaway_home() -> Path:
    ...
    (:mod:`gideon.assurance.evals.gate`) writes a candidate artifact into the same throwaway home,
...
def patch_child_config(dotted: str, value: object) -> str:
    ...
    :mod:`gideon.assurance.evals.cell_provider` puts ONE ``providers[]`` entry into the cell
...
def apply_in_child(overlay: ComponentOverlay | None) -> list[str]:
    ...
    if overlay.kind == KIND_SKILL:
        ...
        existing = [
            s for s in os.environ.get(SUPPRESSED_SKILLS_ENV, "").split(",") if s
        ]
