...
``results.tsv`` row — :func:`gideon.assurance.evals.store.append_result` refuses a row
...
* ``scenario_sha256`` — canonical-JSON hash of the scenario file (see
  :mod:`gideon.assurance.evals.scenarios`); reformatting does not move it, editing an
  assertion does.
...
class RunPin:
    ...
    def cell_model_fp(self) -> str:
        ...
        * :data:`~gideon.assurance.evals.provenance.UNRECORDED` — nothing declared it;
        * :data:`~gideon.assurance.evals.provenance.NO_MODEL` — declared, and the cells could reach
          none;
        * a 12-hex digest of the cells' fingerprint.
        ...
        return provenance.digest_cell(
            _sha256_text(canonical)[:MODEL_FP_LEN], recorded=True
        )
    ...
    def with_cell_models(self, cell_models: dict[str, str] | None) -> "RunPin":
        ...
        :class:`~gideon.assurance.evals.cell_provider.CellProviderBinding` so this module stays
        ...
        return replace(
            self,
            cell_model_fingerprint=None if cell_models is None else dict(cell_models),
        )
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "cell_model_fingerprint": (
                None
                if self.cell_model_fingerprint is None
                else dict(self.cell_model_fingerprint)
            ),
    ...
    def from_dict(cls, data: dict) -> "RunPin":
        ...
        return cls(
            ...
            fixture_home=str(
                data.get("fixture_home", scenario_lib.DEFAULT_FIXTURE_HOME)
            ),
            library_version=int(
                data.get("library_version", scenario_lib.LIBRARY_VERSION) or 0
            ),
            cell_model_fingerprint=(
                None
                if raw_cell is None
                else {str(k): str(v) for k, v in dict(raw_cell).items()}
            ),
        )
    ...
    def with_model_override(self, model: str | None) -> "RunPin":
        ...
        if not model:
            return self
        overridden = {
            use_case: model for use_case in (self.model_fingerprint or {"chat": ""})
        }
...
def prompt_pack_manifest() -> dict[str, str]:
    ...
    pkg_root = package_path("core", "config")
    ...
    for sub in _PROMPT_DIRS:
        ...
        for name, path in resolved.items():
            try:
                manifest[f"{sub}/{name}"] = hashlib.sha256(
                    path.read_bytes()
                ).hexdigest()
            except OSError:
...
def compute_pin(subject: str) -> RunPin:
    ...
    Raises :class:`~gideon.assurance.evals.scenarios.ScenarioLibraryError` when the
