...
ES-1a is the PURE substrate — the store layout (:mod:`gideon.assurance.evals.store`)
and the experiment-matrix TYPES (:mod:`gideon.assurance.evals.matrix`). It spawns no
process, calls no model, and runs no matrix body — the ``run_matrix`` execution
body and its subprocess isolation are ES-1b.

The input corpus has two directions. :mod:`gideon.assurance.evals.scenarios` installs the
SHIPPED library (authored scenarios, upgraded by a release);
:mod:`gideon.assurance.evals.harvest` grows the corpus in the other direction, turning real
...
:mod:`gideon.assurance.evals.gate` reads a curated SUBSET of that one library — the scenarios
