...
:mod:`gideon.automation.loop.tick`'s ``hypothesis_exhausted`` / ``no_progress`` detectors —
...
class FrozenScopeReport:
    ...
    def violation(self) -> bool:
        ...
        """
        return (
            bool(self.frozen_touched) or not self.report.clean or self.report.incomplete
        )
...
def scope_check(
    ...
    *,
    ...
    touched = tuple(
        sorted(p for p in report.changed if scope_mod.in_scope(p, frozen_norm))
    )
...
def content_digest(roots: Sequence[str | os.PathLike[str]]) -> dict[str, str]:
    ...
    for root in roots:
        ...
        files = (
            [path]
            if path.is_file()
            else (sorted(path.rglob("*")) if path.is_dir() else [])
        )
        for child in files:
            if child.is_file():
...
def digest_drift(
    recorded: dict[str, str], roots: Sequence[str | os.PathLike[str]]
) -> list[str]:
...
class LiveWitness:
    ...
    def assert_unchanged(self, *, context: str = "") -> None:
        ...
        if drifted:
            ...
            raise LiveMutationError(
                f"the live artifact changed during the search{where}: "
                f"{', '.join(drifted[:5])}"
            )
    ...
    def persist(self, sandbox: str | os.PathLike[str]) -> Path:
        ...
        path.write_text(
            json.dumps(
                {"roots": list(self.roots), "files": self.files},
                indent=2,
                sort_keys=True,
            ),
            encoding="utf-8",
...
def budget_exhausted(
    meter: SpendMeter, run_key: str, stops: StopConditions
) -> tuple[bool, str]:
...
def write_experience(
    ...
    index = [
        {**row.to_dict(), "diff_ref": f"{EXPERIENCE_DIR}/{row.iteration:03d}.diff"}
        for row in rows
    ]
...
def propose_winner(outcome: SearchOutcome, *, workflow_name: str) -> dict[str, Any]:
    ...
    if outcome.winner is None:
        return {
            "filed": False,
            "rejected": [
                f"no candidate was admitted (halted: {outcome.halt_reason.value})"
            ],
...
ENV_PAYLOAD_KEYS: dict[str, str] = {
    "GIDEON_OPT_SUBJECT": "subject",
    "GIDEON_OPT_LIVE_TARGET": "live_target",
    "GIDEON_OPT_SANDBOX": "sandbox",
    "GIDEON_OPT_SUITE_THRESHOLD": "suite_threshold",
    "GIDEON_OPT_SCORE": "score",
    "GIDEON_OPT_BEST_EVER": "best_ever",
    "GIDEON_OPT_ROWS_CONSIDERED": "rows_considered",
    "GIDEON_OPT_FIX_FINGERPRINT": "fix_fingerprint",
    "GIDEON_OPT_DIFF_TEXT": "diff_text",
}

ENV_STOP_KEYS: dict[str, str] = {
    "GIDEON_OPT_BUDGET_USD": "budget_usd",
    "GIDEON_OPT_ABANDON_AFTER": "hypothesis_abandon_after",
    "GIDEON_OPT_NO_IMPROVEMENT_HALT": "no_improvement_halt",
    "GIDEON_OPT_MAX_ITERATIONS": "max_iterations",
}

ENV_LIST_KEYS: dict[str, str] = {
    "GIDEON_OPT_FIX_FINGERPRINTS": "fix_fingerprints",
    "GIDEON_OPT_MARKS": "marks",
}


def payload_from_env(env: dict[str, str] | None = None) -> dict[str, Any]:
    """Build a subcommand payload out of the ``GIDEON_OPT_*`` environment.
...
def _cmd_scope_check(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not sandbox:
        raise OptimizeRefusedError(
            "scope-check needs a `sandbox` — it holds the witness"
        )
...
def main(argv: Sequence[str] | None = None, stdin: Any = None) -> int:
    """``python -m gideon.assurance.evals.optimize <subcommand>``.

    The payload comes from stdin as JSON, or — when stdin is empty, which is how the bundled
    template calls it — from the ``GIDEON_OPT_*`` environment (:func:`payload_from_env`).
    ...
    if handler is None:
        print(
            json.dumps(
                {
                    "ok": False,
                    "error": f"unknown subcommand {name!r}",
