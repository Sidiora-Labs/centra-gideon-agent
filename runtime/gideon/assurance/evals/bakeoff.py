...
Structurally this is the judge benchmark's twin (:mod:`gideon.assurance.evals.judge_bench`),
...
cannot see, and score into the shared :class:`~gideon.assurance.evals.matrix.CellResult` /
:func:`~gideon.assurance.evals.matrix.aggregate` shape. It does NOT go through
:func:`~gideon.assurance.evals.runner.run_matrix`: that runner exists to contain scenario
...
def _capture_path(use_case: str) -> Path:
    safe = (
        "".join(c if c.isalnum() or c in "-_" else "_" for c in use_case) or "unknown"
    )
...
class BakeoffResult:
    ...
    def to_matrix_result(self) -> MatrixResult:
        return MatrixResult(
            spec=self.spec, cells=self.cells, aggregates=self.aggregates
        )
    ...
    *,
    ...
    for combo in expand_cells(spec):
        ...
        try:
            ...
            Exception
        ):  # noqa: BLE001 - a provider fault is an absent sample, not a run failure
            cells.append(CellResult(coords=coords, outcome=VERIFIER_ABSENT))
            continue
    ...
    recommendation = recommend(
        use_case, cells, current_model=current_model, margin=margin
    )
...
def recommend(
    ...
    *,
    ...
    scores.sort(
        key=lambda s: (s.mean_score if s.mean_score is not None else -1.0), reverse=True
    )
    ...
    return Recommendation(
        ...
        recommended_score=best.mean_score,
        rationale=(
            f"No candidate beat the current pin by ≥ {margin:.2f}; keep the current model."
        ),
        scores=scores,
    )
