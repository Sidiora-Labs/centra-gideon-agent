...
Nothing else in Gideon can answer "does this judge / hint / stage still pay for itself?".
...
2. :func:`run_ablation` replays that component's benchmark through the ES-1b matrix runner
   ...
   :class:`~gideon.assurance.evals.overlay.ComponentOverlay` applied ONLY inside the spawned
   child (:func:`gideon.assurance.evals.overlay.apply_in_child`).
...
def pick_component(
    components: list[AblationComponent] | None = None,
...
def live_state_unchanged(
    ...
    try:
        ...
        BaseException
    ) as exc:  # noqa: BLE001 - re-raised below; captured for chaining
        failure = exc
        raise
    finally:
        ...
        if drift:
            raise LiveStateMutatedError(
                "ablation run mutated live spec/config (forbidden by §3.1): "
                + ", ".join(drift)
            ) from failure
...
def read_report(matrix_id: str) -> AblationReport | None:
    ...
    return AblationReport(
        **{k: v for k, v in data.items() if k in AblationReport.__annotations__}
    )
...
def validate_component(component: AblationComponent) -> None:
    ...
    apply time (:func:`gideon.assurance.evals.overlay.apply_in_child`) so a hand-built overlay
    cannot bypass it either.
    """
    component.overlay()
    if (
        component.kind == overlay_lib.KIND_CONFIG_FLAG
        and not overlay_lib.config_field_exists(component.target)
    ):
        raise ValueError(
    ...
    if component.kind == overlay_lib.KIND_SURFACING:
        from gideon.cognition.learning.surfacing import ABLATABLE
...
def _matrix_id(component_id: str, now: datetime) -> str:
    ...
    slug = "".join(ch if (ch.isalnum() or ch in "-_") else "-" for ch in component_id)[
        :48
    ]
...
def run_ablation(
    ...
    *,
    ...
    report = AblationReport(
        ...
        arms=arms,
        delta=(
            None if (on is None or off is None) else round(float(on) - float(off), 6)
        ),
        cheap_delta=(
            None
            if (on is None or cheap is None)
            else round(float(on) - float(cheap), 6)
        ),
