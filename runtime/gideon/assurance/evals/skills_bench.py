...
1. **Suppression is verified, not assumed.** A "suppressed" arm whose prompt still carries
   ...
   (:func:`gideon.extensions.skills.allocation.allocate_skills`) for both arms and checks that
...
3. **The replayed artifact is a consulted run, not a hand-picked scenario.** The default
   ...
   :func:`~gideon.assurance.evals.harvest.ref_names_skill` predicate :func:`consulted_runs` uses
...
ALIAS of the shared :mod:`gideon.assurance.evals.overlay` arms, not a second dialect. Minting a
...
from gideon.assurance.evals.matrix import MatrixSpec, aggregate_by
...
def consulted_runs(
    skill_name: str, *, limit: int = DEFAULT_RUN_SCAN
) -> list[dict[str, Any]]:
    """Runs whose ledger says they CONSULTED ``skill_name``.

    Reads the WF2-R13 ``consulted`` event (``gideon.assurance.ledger.kinds.CONSULTED``) written
    by :meth:`gideon.automation.workflows.journal.WorkflowJournal.consulted`, matching on the
    ...
    if not skill_name:
        return []
    from gideon.assurance.ledger.kinds import CONSULTED
...
def replay_population(skill_name: str) -> ReplayPopulation:
    ...
    :func:`gideon.assurance.evals.harvest.installed_harvested_cases`'s ``consulted_ref`` scope, which
    matches the run's frozen ``harvest.consulted_refs`` through the SAME
    :func:`~gideon.assurance.evals.harvest.ref_names_skill` matcher :func:`consulted_runs` uses on the
    ...
    about the harvest, and :class:`~gideon.assurance.evals.harvest.EmptyHarvestError` already says it
    ...
    :class:`~gideon.assurance.evals.matrix.MatrixSpec` carries exactly ONE subject. Scoring the whole
...
def _suppressing(skill_name: str):
    ...
    spawned child (:func:`gideon.assurance.evals.overlay.apply_in_child`); this context manager
    ...
    """
    from gideon.extensions.skills.suppression import SUPPRESSED_SKILLS_ENV
...
def arm_prompt(loader, skill_name: str, *, query: str = "") -> str:
    ...
    Mirrors :mod:`gideon.cognition.context`'s seam exactly — ``load_skill`` for the body, the
    ...
    alloc = allocate_skills(
        loader, [SkillRequest(name=skill_name, content=content)], query=query
    )
...
class SuppressionCheck:
    ...
    def verified(self) -> bool:
        ...
        """
        return (
            bool(self.probe_chars)
            and self.present_surfaced
            and not self.present_suppressed
        )
...
def verify_suppression(loader, skill_name: str, *, query: str = "") -> SuppressionCheck:
    ...
    if not check.present_surfaced:
        check.reason = (
            "the surfaced arm's prompt does not carry the body — nothing to remove"
        )
    elif check.present_suppressed:
        check.reason = (
            "the suppressed arm's prompt STILL carries the body — arms are identical"
        )
    return check
...
def bench_skill(
    ...
    *,
    ...
    if run_matrix is None:  # pragma: no cover - the default wiring
        ...
        run_matrix = _default
    matrix_id = (
        f"skillbench-{skill_name.replace('/', '-')}-{moment.strftime('%Y%m%dT%H%M%SZ')}"
    )
