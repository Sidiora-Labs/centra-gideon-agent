...
scenarios in :mod:`gideon.assurance.evals.scenarios`, the judge fixture sets in
:mod:`gideon.assurance.evals.judge_bench`. That is fine for a smoke test and useless as a regression
...
:data:`~gideon.assurance.ledger.kinds.LEDGER_KINDS`, so the `events.jsonl` mirror never carries them
and :func:`~gideon.assurance.ledger.reader.read_events` returns `[]` for them — which reads exactly
like "the run had no inputs". :data:`JOURNAL_KINDS` and :data:`EVENT_KINDS` name the split, and
:func:`~gideon.assurance.ledger.reader.journal_only_kinds` is asserted against it, so the day a kind
...
from gideon.assurance.evals import scenarios
from gideon.assurance.ledger import hash_value, redact
...
def _screen(value: Any) -> Any:
    ...
    :func:`~gideon.assurance.ledger.redaction.redact`. It is idempotent on its OWN output
...
def _journal_mod() -> Any:
    from gideon.automation.workflows import (  # noqa: PLC0415 - cycle-free, load-deferred
        journal,
    )
...
def _run_store() -> Any:
    from gideon.automation.workflows import (  # noqa: PLC0415 - cycle-free
        store as run_store,
    )
...
def harvestable_runs(
    *, workflow_name: str = "", limit: int = DEFAULT_LIMIT
) -> list[Any]:
    ...
    """
    runs, _total = _run_store().list_runs(
        workflow_name=workflow_name, limit=max(1, limit)
    )
    return list(runs)
...
def case_from_run(
    ...
    scenario: dict[str, Any] = {
        ...
        "harvest": {
            ...
            "consulted_refs": sorted(
                {str(_screen(e.get("ref") or "")) for e in consulted} - {""}
            ),
            "provenance": {
                "run_started_event_id": str(started.get("event_id") or ""),
                "run_finished_event_id": (
                    str(finished.get("event_id") or "") if finished else ""
                ),
                ...
                "turns": [
                    {
                        ...
                        "assertions": [
                            {
                                "type": "judge",
                                "value": _judge_criteria(workflow_name, status),
                            }
                        ],
                    }
                ],
    ...
    for step in steps:
        ...
        if node_path:
            try:
                ...
                OSError,
                ValueError,
            ):  # pragma: no cover - defensive; read_output logs
...
def case_name(workflow_name: str, run_id: str) -> str:
    ...
    """
    slug = "".join(
        c if (c.isalnum() or c == "_") else "_" for c in workflow_name
    ).strip("_")
...
def write_case(scenario: dict[str, Any]) -> tuple[Path, bool]:
    ...
    :func:`~gideon.assurance.evals.scenarios.install_library` uses: the case's content is derived
    purely from the run, so an existing file with the same
    :func:`~gideon.assurance.evals.scenarios.sha256_of_scenario_data` is already correct and `wrote`
    ...
    if path.exists():
        try:
            ...
            if (
                ...
                and scenarios.sha256_of_scenario_data(existing) == digest
            ):
                return path, False
        except (OSError, ValueError):
            logger.warning(
                "harvest: unreadable existing case %s, rewriting", path, exc_info=True
            )
    ...
    for run in candidates:
        ...
        if write:
            try:
                ...
                logger.warning(
                    "harvest: could not write case for run %s", run_id, exc_info=True
                )
        ...
        cases.append(
            HarvestedCase(
                ...
                run_id=run_id,
                workflow_name=str(
                    (scenario.get("harvest") or {}).get("workflow_name") or ""
                ),
                scenario=scenario,
...
def ref_names_skill(ref: str, skill_name: str) -> bool:
    ...
    * :func:`gideon.assurance.evals.skills_bench.consulted_runs` matches the LIVE event as it sits in
      `events.jsonl`;
...
def load_harvested_suite(
    *, workflow_name: str = "", consulted_ref: str = ""
    ...
    """
    cases = installed_harvested_cases(
        workflow_name=workflow_name, consulted_ref=consulted_ref
    )
    if not cases:
        scopes = []
        if workflow_name:
