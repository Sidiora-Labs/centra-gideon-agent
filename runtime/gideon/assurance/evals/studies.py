...
from gideon.assurance.evals.judge_bench import JudgeCall, JudgeCaller, live_judge_caller
...
def parse_locked_check(raw: object) -> LockedCheck:
    ...
    * both shapes at once, or neither.
    ...
    if not isinstance(raw, dict):
        raise StudyError(
            f"a locked check must be a JSON object, got {type(raw).__name__}"
        )
    ...
    if not isinstance(phrases, (list, tuple)):
        raise StudyError(
            f"locked check {check_id!r}: `required_phrases` must be a list"
        )
    ...
    try:
        ...
        raise StudyError(
            f"locked check {check_id!r}: `weight` must be a number"
        ) from exc
    try:
        expect = int(raw.get("expect_exit_code", 0))
    except (TypeError, ValueError) as exc:
        raise StudyError(
            f"locked check {check_id!r}: `expect_exit_code` must be an int"
        ) from exc
    check = LockedCheck(
        id=check_id,
        weight=weight,
...
class StudyRegistration:
    ...
    metric: str = (
        "primary: rubric median (pinned); guard: wall_secs, cost_usd, attention_events"
    )
    decision_rule: str = (
    ...
    def blinding_leak_tokens(self) -> tuple[str, ...]:
        ...
        return tuple(
            sorted({t.strip() for t in raw if len(t.strip()) >= MIN_LEAK_TOKEN_LEN})
        )
...
def registration_from_dict(data: object) -> StudyRegistration:
    ...
    try:
        return StudyRegistration(
            ...
            agreement_floor=(
                0.6
                if data.get("agreement_floor") is None
                else float(data["agreement_floor"])
            ),
...
def register_study(
    *,
    ...
    Raises :class:`~gideon.assurance.evals.store.StudySealedError` for an already-registered
    ...
    reg = StudyRegistration(
        ...
        locked_checks=tuple(
            f"locked/{parse_locked_check(c).id}.json" for c in locked_checks
        ),
...
def _config_defaults() -> tuple[int, float]:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 - an unreadable config must not block a registration
        logger.debug(
            "study defaults: config unreadable, using shipped values", exc_info=True
        )
...
def _model_fingerprint() -> dict:
    try:
        from gideon.assurance.evals.pinning import model_fingerprint
...
def rubric_status(
    reg: StudyRegistration, live_rubric_text: str | None
) -> tuple[str, str]:
    ...
    *,
    ...
    for check in resolved:
        if check.is_command:
            ...
            verdict = await run_verify_command(
                command, str(root), label=f"locked:{check.id}"
            )
            if verdict is None:
                outcome, detail = (
                    VERIFIER_ABSENT,
...
def median_slot_winner(samples: Sequence[str]) -> str:
    ...
    itself (:data:`~gideon.automation.workflows.judge_contract.DEFAULT_JUDGE_SAMPLES`).
...
async def judge_pair(
    ...
    *,
    ...
    direct_winner = _slot_to_arm(
        median_slot_winner(collected[PRESENTATION_DIRECT]), slot_a_arm
    )
    swapped_winner = _slot_to_arm(
        median_slot_winner(collected[PRESENTATION_SWAPPED]), other_arm
    )
    ...
    *,
    ...
    if win_rate is None:
        return replace(
            measured,
            verdict=VERDICT_TIE,
            detail="no case produced a decided win or loss",
        )
    if win_rate > 0.5:
...
def _pin_for(reg: StudyRegistration):
    ...
    """
    from gideon.assurance.evals.pinning import compute_pin_for_subject
        ...
        store.append_result(
            {
                ...
                "score_old": (
                    None if result.win_rate is None else round(1.0 - result.win_rate, 4)
                ),
                "score_new": (
                    None if result.win_rate is None else round(result.win_rate, 4)
                ),
                "k": reg.k,
                "ts": f"{time.time():.0f}",
            },
...
def file_demotion_proposal(reg: StudyRegistration, result: StudyResult) -> str:
    ...
    body = (
        ...
        + (
            f" Revert to version {old_version}."
            if old_version not in (None, "")
            else ""
        )
    )
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 - the filed proposal must survive a revocation failure
...
def file_judge_calibration(reg: StudyRegistration, result: StudyResult) -> str:
    ...
    if not result.judge_below_floor:
        return ""
    flipped = [
        f"{p.case_id}/trial{p.trial}"
        for c in result.cases
        for p in c.pairs
        if p.position_flipped
    ]
    payload = {
...
async def run_study(
    ...
    *,
    ...
    (:data:`gideon.assurance.evals.study_arms.live_arm_runner`) for the same reason ``caller``
    ...
    if arm_runner is None:
        from gideon.assurance.evals.study_arms import live_arm_runner
    ...
    if seal_state != SEAL_OK:
        return persist(
            reg,
            decide(
                reg, cases=[], locked=[], seal_state=seal_state, seal_detail=seal_detail
            ),
        )
    ...
    if state != RUBRIC_OK:
        return persist(
            reg,
            decide(reg, cases=[], locked=[], rubric_state=state, rubric_detail=detail),
        )
    ...
    for case in cases:
        for trial in range(reg.k):
            for arm, body in (
                (ARM_OLD, old_template_body),
                (ARM_NEW, new_template_body),
            ):
                payloads.append(
                    WorkerPayload(
                        study_id=reg.study_id,
