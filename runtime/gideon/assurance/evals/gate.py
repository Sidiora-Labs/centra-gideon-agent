...
It is not a second runner. Every score here comes from :func:`gideon.assurance.evals.runner.run_matrix`
through :mod:`gideon.assurance.evals.child`, one matrix per (arm × scenario) at ``trial_count=1``,
exactly as :mod:`gideon.assurance.evals.skills_bench` and :mod:`gideon.assurance.evals.ablation` do.
...
:func:`gideon.assurance.evals.scenarios.install_library`), and none of them descends: a scenario
...
1. an unbudgeted gate does not run *unbounded* — it does not run at all, and the proposal
   reads ``ungated``. ``0`` means UNLIMITED to :class:`~gideon.security.guardrails.budgets.Budget`,
...
:func:`gideon.cognition.learning.proposals.accept`. The word is the atom's own; the *score cells*
render with the panels' house string ("not measured") rather than a ``0.0``.

And the pin is never invented. :func:`~gideon.assurance.evals.pinning.RunPin.is_complete` is a
...
from gideon.assurance.evals.overlay import OverlayRefusedError, throwaway_home
...
UNGATED_NOT_RUN = (
    "no gate run yet — accept on the evidence above, or run the gate first"
)
UNGATED_EVALS_OFF = (
...
UNGATED_EMPTY_SUBSET = "no installed scenario is tagged for the gate tier, so there was nothing cheap to re-run"
UNGATED_NO_PIN = (
...
class ArtifactArm:
    ...
    def __post_init__(self) -> None:
        if self.label not in ARM_LABELS:
            raise ValueError(
                f"unknown arm label {self.label!r} — expected one of {ARM_LABELS}"
            )
...
def spawn_env_for(base_env: dict, arm: ArtifactArm | None) -> dict:
    ...
    contract as :func:`gideon.assurance.evals.overlay.spawn_env_for`, for the same reason.
    """
    env = dict(base_env)
    if arm is not None:
...
def resolve_in_home(home: Path, relpath: str) -> Path:
    ...
    if target != root and not target.is_relative_to(root):
        raise ArmRefusedError(
            f"a gate arm path {text!r} resolves outside the cell home {root}"
        )
    return target
...
def apply_in_child(arm: ArtifactArm | None) -> list[str]:
    ...
    Refuses via :class:`~gideon.assurance.evals.overlay.OverlayRefusedError` when the process is
...
class GateSubset:
    ...
    def sha256(self) -> str:
        ...
        (:func:`~gideon.assurance.evals.pinning.compute_pin_for_subject`).
        """
        import hashlib
...
def gate_subset(*, max_scenarios: int = GATE_SUBSET_MAX) -> GateSubset:
    ...
    try:
        ...
        logger.warning(
            "gate: the installed scenario library is unreadable", exc_info=True
        )
        return GateSubset()
    ...
    for path in sorted(installed.iterdir()):
        if path.suffix not in scenario_lib.SCENARIO_SUFFIXES:
            continue
        try:
            data = scenario_lib._read_scenario_data(
                path
            )  # noqa: SLF001 - same-package read
        except (OSError, ValueError, scenario_lib.ScenarioLibraryError):
            logger.warning(
                "gate: skipping unparseable scenario %s", path, exc_info=True
            )
            continue
...
def summary(report: dict | None) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": str(
            data.get("reason") or (UNGATED_NOT_RUN if state != GATE_GATED else "")
        ),
...
def cell_spend(matrix_id: str) -> dict[str, Any]:
    ...
    :data:`~gideon.assurance.evals.provenance.UNRECORDED`: the sum of the cells that DID report is not
    ...
    for result_path in sorted(root.glob("cell-*/result.json")):
        ...
        if provenance.is_unrecorded(spend, "tokens_recorded") or not spend.get(
            "tokens_recorded"
        ):
...
def _accumulate(into: dict[str, Any], one: dict[str, Any]) -> None:
    ...
    recorded = bool(into.get("tokens_recorded", True)) and bool(
        one.get("tokens_recorded", True)
    )
...
def run_gate(
    *,
    ...
    if run_matrix is None:  # pragma: no cover - the default wiring
        from gideon.assurance.evals.runner import run_matrix as _default

        run_matrix = _default

    from gideon.assurance.evals.matrix import MatrixSpec
    from gideon.security.guardrails.budgets import Budget, BudgetVerdict
    ...
    for arm in arms:
        ...
        for member in chosen.members:
            ...
            try:
                ...
                logger.warning(
                    "gate: %s scenario %s failed", arm.label, member.name, exc_info=True
                )
            ...
            charged_tokens = (
                0 if one.get("tokens") is None else int(one.get("tokens") or 0)
            )
            meter.charge(
                charged_tokens,
...
def verdict_of(report: GateReport) -> str:
    ...
    :func:`gideon.assurance.evals.runner._matrix_verdict`'s vocabulary rather than minting a
    second word for "no measurement".
    """
    from gideon.assurance.evals.matrix import VERIFIER_ABSENT
...
def _record_ledger_row(report: GateReport, *, pin: pinning.RunPin, k: int) -> None:
    ...
    try:
        ...
        logger.warning(
            "gate: results.tsv row not written for %s", report.run_id, exc_info=True
        )


_CANDIDATE_RENDERERS: dict[str, str] = {
    "skill": "gideon.cognition.learning.skill_promotion:candidate_files",
}
...
def gate_proposal(pid: str, **kwargs: Any) -> GateReport | None:
    ...
    read surface, and :func:`gideon.cognition.learning.proposals.accept` never consults the field —
