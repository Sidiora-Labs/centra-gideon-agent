...
(:func:`gideon.providers.use_cases.active_models_path`), so that file is the seam:
...
def fingerprint_now() -> tuple[dict[str, str], str]:
    ...
    Reuses the ES-2 pin machinery (:func:`gideon.evals.pinning.model_fingerprint` and
    :meth:`~gideon.evals.pinning.RunPin.model_fp`) rather than hashing
...
def _mtime() -> float:
    from gideon.providers.use_cases import active_models_path
