...
* 👍/👎 — FEEDBACK-SIGNAL's ``feedback.jsonl`` current verdicts, attributed by
  ``producer_id == subject`` — the SAME attribution :func:`gideon.guardrails.
...
* approvals/rejections/undos — the SEL approval verdicts through
  :func:`gideon.guardrails.autonomy.approval_outcome_series` (one attribution
...
* gate status (Loop 2) — the newest gate report attached to a proposal targeting the
  subject, projected through the SAME :func:`gideon.evals.gate.summary` the inbox
  row renders.
...
divergent action type loses its OWN standing grant (:func:`gideon.guardrails.ladder.
revoke_scope`), and a divergent template revokes standing grants wholesale
(:func:`~gideon.guardrails.ladder.revoke_granted_scopes`) — the same consequence a
...
def field_trend(goods: Sequence[bool], *, min_signals: int = FIELD_TREND_MIN_SIGNALS) -> str:
    ...
    :func:`gideon.workflows.introspection.attention_trend`'s shape, because the
...
def _thumb_signals(subject: str) -> list[tuple[float, bool]]:
    ...
    :func:`gideon.guardrails.autonomy._feedback_rejections`'s rule, so the same 👎
    that blocks a promotion also moves this row.
    """
    from gideon.feedback import producer_verdict_series
...
def _action_type_signals(key: str) -> tuple[list[tuple[float, bool]], int, int, int]:
    ...
    from gideon.guardrails.autonomy import approval_outcome_series
    from gideon.guardrails.ladder import reversal_records
...
def subject_rows() -> list[SubjectRow]:
    ...
    """
    from gideon.guardrails.autonomy import registered_action_types
...
def sweep_lab_field_divergence(rows: list[SubjectRow] | None = None) -> list[str]:
    ...
    * an ``action_type`` subject loses its OWN grant
      (:func:`gideon.guardrails.ladder.revoke_scope` — floor, cooldown,
      trust-record ``revoked`` flag, SEL audit, one notice);
    * a ``template`` subject revokes standing grants wholesale
      (:func:`~gideon.guardrails.ladder.revoke_granted_scopes`) — the SAME
