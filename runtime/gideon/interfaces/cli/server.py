...
from gideon.operations.service.common import SERVICE_NAME, Platform, current_platform
...
_UPDATE_HANDLED_KINDS: frozenset[str] = frozenset(
    {"git", "pip", "container", "desktop"}
)
...
def _update_desktop() -> None:
    ...
    print("  Ask your Gideon administrator for the current desktop release.")
...
async def _gateway(
    *,
    ...
    if not no_dashboard and ensure_dev_dist_symlink() is None:
        _hint = package_path().parent.parent / "apps" / "console"
        logging.getLogger(__name__).debug(
    ...
    try:
        from gideon.security.auth.credentials import bootstrap_from_env
    ...
    from gideon.security.guardrails.ceiling import GovernanceBootError
...
def _build_consolidator() -> (
    tuple["ConversationDirectory", HistoryConsolidator, ConversationLog]
):
    ...
    memories land) and a ProcedureLibrary (so auto skills get written), driven off
    the active embedding selection.
    """
    cfg = AppConfig.load()
    factory = cfg.create_provider_factory()
    sessions = ConversationDirectory(cfg, provider_factory=factory)  # type: ignore[arg-type]
...
def _service_cmd(args: argparse.Namespace) -> int:
    ...
    Wraps :mod:`gideon.operations.service.controller` so that platform detection
