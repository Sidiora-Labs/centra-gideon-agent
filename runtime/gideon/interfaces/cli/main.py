...
def _is_project_root(d: Path) -> bool:
    ...
    """
    return (d / "pyproject.toml").is_file() and (d / "runtime" / "gideon").is_dir()
...
""",
    ...
    run_parser.add_argument(
        "--cwd", default="", help="Working directory for the turn's tools"
    )
    run_parser.add_argument(
        "--allow",
        action="store_true",
    ...
    setup_parser = sub.add_parser(
        "setup", help="Install agent config and configure credentials"
    )
...
""",
    ...
    snap_parser.add_argument(
        "--keep", type=int, default=7, help="Keep N most recent snapshots"
    )
    snap_parser.add_argument(
        "--list",
        action="store_true",
    ...
    project_parser = sub.add_parser(
        "project", help="Export or import a single project archive"
    )
    project_sub = project_parser.add_subparsers(dest="project_command")
    proj_export = project_sub.add_parser(
        "export", help="Write one project to a manifest ZIP"
    )
    proj_export.add_argument("project", help="Project id or name")
    proj_export.add_argument(
        "-o", "--output", help="Archive path (default: ./<name>.zip)"
    )
    proj_export.add_argument(
        "--passphrase",
        default="",
    ...
    proj_import.add_argument(
        "--dry-run",
        action="store_true",
        help="Plan the import without writing anything",
    )
    proj_import.add_argument(
        "--passphrase", default="", help="Passphrase for an encrypted archive"
    )

    workflow_parser = sub.add_parser(
        "workflow", help="Inspect and replay workflow runs"
    )
    ...
    wf_replay.add_argument(
        "--json",
        action="store_true",
        help="Emit the trajectory diff as JSON instead of text",
    )
    ...
    rest_parser.add_argument(
        "--components", help="Comma-separated components to restore"
    )
    ...
    inbound_parser = sub.add_parser(
        "inbound",
        help="Manage the inbound access surfaces (openai, mcp, a2a, capture, bridge)",
    )
    inbound_sub = inbound_parser.add_subparsers(dest="inbound_command")
    inbound_token = inbound_sub.add_parser(
        "token", help="Create or inspect a surface token"
    )
    ...
    inbound_confirm.add_argument(
        "confirm_token", help="The confirm_token the bridge returned"
    )
    ...
    auth_setpw = auth_sub.add_parser(
        "set-password", help="Set the owner login password"
    )
    auth_setpw.add_argument(
        "--user", default="", help="Login username (defaults to $USER)"
    )
    ...
    auth_totp.add_argument(
        "totp_action", choices=("setup", "disable"), nargs="?", default="setup"
    )
    auth_enroll = auth_sub.add_parser(
        "enroll", help="Print a single-use code to pair a device"
    )
    auth_enroll.add_argument(
        "--label", default="", help="A note for your own reference"
    )
    auth_enroll.add_argument(
    ...
    auth_revoke.add_argument(
        ...
        type=int,
        default=0,
        help="Gateway port (defaults to the configured one)",
    )
    ...
    push_init_p = push_sub.add_parser(
        "init", help="Generate the VAPID keypair (web push)"
    )
    ...
    push_sub.add_parser(
        "status", help="Show the backend, keypair and subscribed devices"
    )
    push_test = push_sub.add_parser(
        "test", help="Send one content-free ping to every device"
    )
    push_test.add_argument(
        "--kind", default="approval", help="Payload kind (default: approval)"
    )
    push_test.add_argument(
        "--item-id", default="test", help="Payload item id (default: test)"
    )
    ...
    backup_export.add_argument(
        ...
        nargs="?",
        default=None,
        help="Shard directory (default: <home>/shards)",
    )
    backup_export.add_argument(
        "--incremental",
        action="store_true",
    ...
    backup_validate = backup_sub.add_parser(
        "validate",
        help="Verify an export: manifest, sizes, row counts, sha256, parseability",
    )
...
""",
    ...
    bench_parser.add_argument(
        "--tiers",
        default="",
        help="Comma-separated judge tiers (default: fast,standard,reasoning)",
    )
    bench_parser.add_argument(
        "--samples",
        default="",
        help="Comma-separated judge_samples counts (default: 1,3,5)",
    )
    bench_parser.add_argument(
...
""",
    ...
    harvest_parser.add_argument(
        ...
        type=int,
        default=0,
        help="How many recent runs to consider (default: 50)",
    )
    harvest_parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    harvest_parser.add_argument(
        ...
        action="store_true",
        dest="list_suite",
        help="List the installed harvested suite",
    )
...
""",
    ...
    study_parser.add_argument(
        "--list", action="store_true", help="List every registered study"
    )
    study_parser.add_argument(
        "--view", default="", help="Print one study's registration + verdict"
    )
    study_parser.add_argument("--run", default="", help="Run this registered study id")
    study_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the spend preflight and call nothing",
    )
    study_parser.add_argument(
    ...
    sec_sub.add_parser(
        "audit", help="Scan conversation history for suspicious tool usage"
    )
    sec_sub.add_parser("deny-list", help="Show active deny patterns")
    sel_parser = sec_sub.add_parser(
        "events", help="Show recent security event log entries"
    )
    sel_parser.add_argument(
        "-n", "--limit", type=int, default=20, help="Number of entries"
    )
...
""",
    ...
    abl_parser.add_argument(
        "--component",
        default="",
        help="Measure this component id instead of the next in rotation",
    )
    abl_parser.add_argument(
        "--skill",
        default="",
        help="Bench one SKILL surfaced-vs-suppressed (§3.3) instead",
    )
    abl_parser.add_argument(
        "--subject",
        default="",
    ...
    abl_parser.add_argument(
        "--trials", type=int, default=3, help="Trials per arm (default: 3)"
    )
    abl_parser.add_argument(
        "--budget", type=float, default=0.0, help="Hard spend cap in USD (0 = no cap)"
    )
...
""",
    ...
    ret_parser.add_argument(
        "--card",
        action="store_true",
        help="Print the hand-labeling card as JSON and stop",
    )
    ret_parser.add_argument(
    ...
    token_parser = sub.add_parser(
        "token", help="Print a dashboard access URL with auth token"
    )

    # logout
    logout_parser = sub.add_parser(
        "logout", help="Revoke all active dashboard sessions"
    )
    ...
    inc_on = incident_sub.add_parser(
        "on", help="Activate incident mode (suspend automation)"
    )
...
""",
    ...
    learn_rm = learn_sub.add_parser(
        "remove", help="Remove lessons matching a substring"
    )
    ...
    agent_create.add_argument(
        "--memory-store", default="default", help="Memory store name"
    )
...
""",
    ...
    skills_search.add_argument(
        "--marketplace",
        default="skills.sh",
        help="Marketplace to search (default: skills.sh)",
    )
    skills_install = skills_sub.add_parser("install", help="Install a skill")
    skills_install.add_argument(
        "id", help="Skill ID, e.g. vercel-labs/agent-skills/next-js"
    )
    skills_install.add_argument(
    ...
    skills_remove = skills_sub.add_parser(
        "remove", help="Remove a locally installed skill"
    )
    skills_remove.add_argument("name", help="Skill directory name to remove")
    skills_curate = skills_sub.add_parser(
        "curate",
        help="Groom the auto/ skill library (age active→stale→archived by last-use)",
    )
    ...
    if args.command == "gateway" and getattr(args, "seed_local_model", False):
        from gideon.operations.seed_local_model import seed_local_model_cmd
    ...
    if args.command == "chat":
        ...
        from gideon.workspace.snapshot import restore_main
...
from gideon.interfaces.cli.config import _config_cmd  # noqa: E402
from gideon.interfaces.cli.doctor import (
    _doctor,
    _doctor_paths,
    _doctor_rebuild_routing_stats,
)
...
from gideon.interfaces.cli.setup import _setup
from gideon.operations.durability.shards import backup_cmd as _backup_cmd  # noqa: E402
from gideon.security.auth.cli import auth_cmd as _auth_cmd  # noqa: E402
from gideon.workspace.push import push_cmd as _push_cmd  # noqa: E402
...
def _workflow_cmd(args) -> int:  # noqa: ANN001
    ...
    from gideon.automation.workflows.replay import ReplayError, replay_run
...
def _handle_skills(args) -> None:  # noqa: ANN001
    ...
    if cmd == "install":
        ...
        try:
            ...
            note = f" (scanned, tier={result.tier.value}" + (
...
def _incident_cmd(args) -> None:  # noqa: ANN001
    ...
    """
    from gideon.security.guardrails import incident as _inc
