...
The deliberate other half of :mod:`gideon.interfaces.dashboard.chat_undo`. ``/undo`` rolls back
...
def _resolve_session(request: web.Request, operation: str):
    ...
    if not session:
        return None, json_error(
            "not_found", message=f"no such session: {name}", status=404
        )
    if request_app and session._app != request_app:
        ...
        return None, json_error(
            "forbidden", message="app does not own this session", status=403
        )
    return session, None
...
def _turn_arg(raw: object) -> tuple[int, web.Response | None]:
    ...
    if isinstance(raw, bool) or not isinstance(raw, (int, str)):
        return 0, json_error(
            "invalid_turn", message="turn must be an integer", status=400
        )
    try:
        turn = int(raw)
    ...
    if turn < 0:
        return 0, json_error(
            "invalid_turn", message="turn must be an integer >= 0", status=400
        )
    return turn, None
