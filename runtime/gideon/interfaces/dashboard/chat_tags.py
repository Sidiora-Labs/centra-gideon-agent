...
def _tag_by_id(state: ConsoleState, tag_id: str) -> dict | None:
    return next((t for t in state._tags if t.get("id") == tag_id), None)


def find_tag_by_name(state: ConsoleState, name: str) -> dict | None:
    """Case-insensitive lookup of a tag definition by display name."""
    needle = (name or "").strip().lower()
    if not needle:
        return None
    return next(
        (t for t in state._tags if str(t.get("name", "")).strip().lower() == needle),
        None,
    )


def create_tag(
    state: ConsoleState, name: str, *, color: str | None = None, status: bool = False
) -> dict | None:
...
def _normalize_column(
    state: ConsoleState, raw: Any, *, existing: dict | None = None
) -> dict | None:
    ...
    if "tag_ids" in raw:
        ...
        if not isinstance(tag_ids, list):
            return None
        cleaned["tag_ids"] = [
            str(t) for t in tag_ids if isinstance(t, str) and t in valid_ids
        ]
    if "mode" in raw:
...
async def api_chat_session_drop(request: web.Request) -> web.Response:
    ...
    *status* tag, strip every status tag from the session and add that one.
    ...
    kept = [
        t for t in session.tags if t in tag_index and not tag_index[t].get("status")
    ]
