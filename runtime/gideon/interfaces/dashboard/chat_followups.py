...
def maybe_offer_check_work(
    state: "ConsoleState", session: "_ChatSession", tool_calls: int
) -> None:
...
def _parse_followups(text: str) -> list[str]:
    ...
    if text.startswith("```"):
        parts = text.split("\n")
        text = "\n".join(
            parts[1:-1] if parts[-1].startswith("```") else parts[1:]
        ).strip()
    try:
...
async def _generate_followups(
    state: "ConsoleState", session: "_ChatSession"
) -> list[str]:
...
async def _maybe_followups(state: "ConsoleState", session: "_ChatSession") -> None:
