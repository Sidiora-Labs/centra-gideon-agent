...
``gideon.cognition.planning.session``: the same ``PlanSession``/``PlanStep`` model and the
...
session to task mode ``plan``; the canonical gate in ``gideon.engine.task_modes`` (which
...
def read(chat_key: str) -> tuple[PlanSession | None, dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            "chat plan sidecar unreadable for %s — treating as absent", chat_key
        )
        return None, {}
    if not isinstance(raw, dict) or not isinstance(raw.get("session"), dict):
        logger.warning(
            "chat plan sidecar malformed for %s — treating as absent", chat_key
        )
        return None, {}
    binding = raw.get("binding")
    return PlanSession.from_dict(raw["session"]), (
        dict(binding) if isinstance(binding, dict) else {}
    )
...
def write(session: PlanSession, binding: dict[str, Any]) -> None:
    ...
    atomic_write(
        f, json.dumps({"session": session.to_dict(), "binding": binding}, indent=2)
    )
...
def activate(
    chat_session: _ChatSession, *, running: bool
) -> tuple[PlanSession, dict[str, Any]]:
    ...
    if sess is None:
        ...
        binding = {}
    binding.setdefault(
        "resume_task_mode", getattr(chat_session, "_task_mode", "agent") or "agent"
    )
...
def maybe_submit_plan_draft(state: ConsoleState, session: _ChatSession) -> bool:
    ...
    try:
        state.broadcast_ws(
            "chat_plan_step", {"session": session.key, "step_id": step.id}
        )
...
async def api_chat_plan_edit(request: web.Request) -> web.Response:
    ...
    if "markdown" not in body:
        return json_error(
            "markdown_required", message="markdown is required", status=400
        )
    sess, binding = read(chat.key)
    if sess is None:
        return json_error(
...
async def api_chat_plan_comment(request: web.Request) -> web.Response:
    ...
    if not text:
        return json_error(
            "comment_text_required", message="Comment text is required", status=400
        )
    sess, binding = read(chat.key)
    if sess is None:
        return json_error(
...
def _dispatch(state: ConsoleState, chat: _ChatSession, prompt: str) -> None:
