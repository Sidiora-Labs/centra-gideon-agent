...
async def api_chat_session_undo(request: web.Request) -> web.Response:
        ...
        if isinstance(body, dict) and body.get("n") is not None:
            ...
            if isinstance(raw, bool) or not isinstance(raw, int) or raw < 1:
                return web.json_response(
                    {"error": "n must be a positive integer"}, status=400
                )
            n = raw

    if session.running:
        return web.json_response(
            {"error": "cannot undo while a turn is running"}, status=409
        )
    ...
    try:
        ...
        logger.warning(
            "undo: failed to persist truncated transcript for %s", name, exc_info=True
        )
