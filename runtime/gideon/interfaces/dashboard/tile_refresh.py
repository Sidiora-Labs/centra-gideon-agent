...
:data:`~gideon.assurance.ledger.kinds.TILE_REFRESHED` row and creates no run directory — the
...
class _TileLedgerStore:
    """The four-call file store :class:`~gideon.assurance.ledger.LedgerStore` wants, over
...
def render_skeleton(skeleton: str, node_outputs: dict[str, Any]) -> str:
    ...
    :func:`gideon.automation.workflows.bindings.resolve` on purpose: a second interpolator would
...
async def _run_data_node(
    ...
    decision = enforce_action(
        node.provider, dict(node.config), ctx, session_key=session_key
    )
    if decision.blocked:
        return None, NodeOutcome(
            id=node.id,
    ...
    try:
        ...
        Exception
    ) as exc:  # noqa: BLE001 — a provider that raises must not kill the band
        return None, NodeOutcome(
            id=node.id,
            provider=node.provider,
            ok=False,
    ...
    *,
    ...
    if not force:
        ...
        if not is_due:
            return RefreshResult(
                refreshed=False, reason="within_ttl", row=last_row(view_id, ref)
            )
    ...
    if sink is None:
        ...
        written = await sink.execute(
            ...
            ActionContext(
                event="tile_refresh", payload={"view_id": view_id, "ref": ref}
            ),
        )
    ...
    return RefreshResult(
        refreshed=True, ok=True, nodes=outcomes, row=row, content=content
    )
