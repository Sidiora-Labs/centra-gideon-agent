...
def _disk_stale_keys(
    state: ConsoleState, *, cutoff: float, resident: set[str], active_session: str
...
def stale_session_keys(
    ...
    *,
    ...
    stale.extend(
        _disk_stale_keys(
            state, cutoff=cutoff, resident=resident, active_session=active_session
        )
    )
    stale.sort()
...
def _archive_on_disk(state: ConsoleState, name: str) -> bool:
