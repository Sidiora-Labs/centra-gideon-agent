...
def _sel_log(
    operation: str, outcome: str, resources: str, request: web.Request
) -> None:
    try:
...
async def api_agent_marketplace_list(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {
            "agents": agents,
            "marketplace": request.rel_url.query.get(
...
async def api_agent_marketplace_activate(request: web.Request) -> web.Response:
    ...
    from gideon.core.config.loader import AgentProfile, AppConfig, config_dir
    ...
    return web.json_response(
        {
            "ok": True,
            "name": name,
            "prompt_path": str(prompt_path) if defn.system_prompt else "",
        }
    )
...
async def api_agent_marketplace_test(request: web.Request) -> web.Response:
    ...
    test_prompt = (
        ...
        or "Hello! Please introduce yourself in one sentence."
    )
    ...
    return web.json_response(
        {"ok": True, "response": response[:4000], "elapsed_ms": elapsed_ms}
    )
