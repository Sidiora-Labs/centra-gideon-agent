...
def register(app, store=None):
    home = config_dir()
    store = store or MidiStore(home / 'capabilities' / 'music', MusicCatalog(home / 'capabilities' / 'music', NativeArtifactProvider(root=home / 'artifacts')))
    async def handle(request):
        try:
            item_id = request.match_info.get('id')
            if request.path.endswith('/export'):
                result = await asyncio.to_thread(store.export, item_id, await read_json_body(request))
                return web.json_response(result)
            if request.method == 'GET':
                result = await asyncio.to_thread(store.get, item_id) if item_id else await asyncio.to_thread(store.list, int(request.query.get('offset', 0)), int(request.query.get('limit', 50)))
                return web.json_response({'item' if item_id else 'items': result})
            data = await read_json_body(request)
            result = await asyncio.to_thread(store.update, item_id, data) if item_id else await asyncio.to_thread(store.transcribe, data)
