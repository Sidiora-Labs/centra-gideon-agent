...
Three routes over :mod:`gideon.engine.session_organize`: read a suggestion, accept it,
...
async def _proposal_from_body(
    ...
    try:
        ...
        return (
            None,
            None,
            json_error("bad_request", message="invalid JSON body", status=400),
        )
    if not isinstance(body, dict):
        return (
            None,
            None,
            json_error("bad_request", message="body must be an object", status=400),
        )
    ...
    if proposal.is_empty:
        return (
            ...
            json_error(
                "bad_request",
                message="proposal must name a folder or at least one tag",
                status=400,
            ),
        )
