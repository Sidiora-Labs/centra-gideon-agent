...
from gideon.engine.routing import usage as usage_fold
        ...
        return web.json_response(
            {
                "error": {
                    "code": "internal",
                    "message": "could not read the usage ledger",
                }
            },
            status=500,
        )
...
async def api_usage_totals(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"since": since, "until": until, "session": session, "totals": totals}
    )
...
async def api_usage(request: web.Request) -> web.Response:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 — a read-only spend view must never 500 on a bad fold
