...
from gideon.workspace.capabilities.platform.connections import ConnectionError, mutate, projection
...
async def connections(request):
    ...
    try:
        if request.method == "GET":
            return web.json_response(projection(), headers={"Cache-Control": "no-store"})
        body = {"revision": int(request.query.get("revision", "-1"))} if request.method == "DELETE" else await read_json_body(request)
        if not isinstance(body, dict):
            raise ConnectionError("JSON object required")
        operation = "delete" if request.method == "DELETE" else "bind" if "provider" in request.match_info else "save"
        async with _get_config_lock():
            result = mutate(request.match_info["id"], body, operation=operation, provider=request.match_info.get("provider"))
