...
async def status(request):
    _authorized(request)
    return web.json_response(await asyncio.to_thread(ReplicationService().status), headers={"Cache-Control": "no-store"})
...
async def push(request):
    ...
    try:
        ...
        return web.json_response(await ReplicationService().push(request.match_info["peer_id"], body["domain"]), headers={"Cache-Control": "no-store"})
...
async def receive(request):
    try:
        ...
        if not isinstance(envelope, dict) or set(envelope) != {"proof", "payload"} or not isinstance(envelope["proof"], dict) or not isinstance(envelope["payload"], dict):
        ...
        result = await asyncio.to_thread(ReplicationService().apply_batch, peer["id"], envelope["payload"])
