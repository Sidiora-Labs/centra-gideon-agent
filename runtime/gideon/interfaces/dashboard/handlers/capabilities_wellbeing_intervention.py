            ...
            if request.method in ("POST", "PUT"):
                ...
                operation = (
                    ...
                    if not identity
                        ...
                        else store.update_plan if plans else store.correct_record
                    )
                )
                result = (
                ...
                result = {
                    "history": await asyncio.to_thread(
                        store.history_plan if plans else store.history_record, identity
                    )
                }
            elif suffix == "plans":
                ...
                result = {
                    "plans": await asyncio.to_thread(
                        store.list_plans, archived == "true"
                    )
                }
            elif suffix == "records":
                result = {
                    "records": await asyncio.to_thread(store.list_records, identity)
                }
            else:
                result = await asyncio.to_thread(
                    store.get_plan if plans else store.get_record, identity
                )
