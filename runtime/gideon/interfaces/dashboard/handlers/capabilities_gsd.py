...
from gideon.workspace.capabilities.platform.gsd import (
    GsdError,
    edit,
    inspect,
    projects,
...
async def gsd(request):
    ...
    try:
        ...
        if request.method == "PUT":
            ...
            result = (
                inspect(identifier, request.query.get("document"))
                if identifier
                else projects()
            )
        ...
        return web.json_response(
            {"error": "Planning artifacts are invalid or unavailable"}, status=400
        )
