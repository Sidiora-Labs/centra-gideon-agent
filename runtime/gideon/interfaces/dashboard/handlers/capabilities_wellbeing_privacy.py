...
def register(app: web.Application, home: Path | None = None):
    ...
    async def handle(request):
        try:
            ...
            if request.method == "GET":
                if scope == "subjects":
                    method, envelope = (
                        (store.list_subjects, "subjects")
                        if not identity
                        else (
                            (store.consents, "consents")
                            if suffix == "consents"
                            else (
                                (store.list_facts, "facts")
                                if suffix == "facts"
                                else (
                                    (store.audit, "audit")
                                    if suffix == "audit"
                                    else (store.get_subject, None)
                                )
                            )
                        )
                    )
                ...
                if scope == "subjects":
                    method = (
                        ...
                        if not identity
                            ...
                            store.consent if suffix == "consents" else store.create_fact
                        )
                    )
                else:
    ...
    app.router.add_get(
        base + "/{scope:subjects}/{id}/{suffix:consents|facts|audit}", handle
    )
