...
async def import_endpoint(request):
    ...
    try:
        ...
        if not isinstance(body, dict) or set(body) != {'format', 'content', 'source_name'}:
            raise ValueError('Format, content and source filename required')
        return web.json_response(import_cli_usage(body['format'], body['content'], body['source_name']), status=201, headers={'Cache-Control': 'no-store'})
    except (ValueError, OSError, UnicodeError):
        return web.json_response({'error': 'CLI usage import unavailable or invalid'}, status=400)
