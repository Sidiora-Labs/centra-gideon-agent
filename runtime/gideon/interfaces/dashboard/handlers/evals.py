...
async def api_evals_study(request: web.Request) -> web.Response:
    ...
    from gideon.assurance.evals.studies import study_view
...
async def api_evals_ablation(request: web.Request) -> web.Response:
    ...
    _audit(
        ...
        "evals_ablation",
        "read",
        f"matrix_id={view['report'].get('matrix_id')}",
    )
    return web.json_response(view)
...
async def api_evals_retrieval_labels(request: web.Request) -> web.Response:
    ...
    if not isinstance(body, dict):
        return json_error(
            "invalid_json", message="Body must be a JSON object.", status=400
        )
...
async def api_evals_field_metrics(request: web.Request) -> web.Response:
    ...
    _audit(
        ...
        "evals_field_metrics",
        "read",
        f"subjects={len(rows)} diverged={diverged}",
    )
