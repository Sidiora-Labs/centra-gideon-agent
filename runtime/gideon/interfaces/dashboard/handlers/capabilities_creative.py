    ...
    try:
        if request.method == "GET":
            ...
            if action == "sources":
                ...
                revision = (
                    int(request.query["revision"])
                    if "revision" in request.query
                    else None
                )
                return web.json_response(
                    store.export(id, revision),
                    headers={
                        "Content-Disposition": f'attachment; filename="moodboard-{id}.json"'
                    },
                )
            ...
            result = (
                store.restore(id, payload)
                if action == "restore"
                else store.update(id, payload) if id else store.create(payload)
            )
        return web.json_response(
            result, status=201 if request.method == "POST" and not id else 200
        )
    except (CatalogError, ValueError, TypeError) as exc:
        return web.json_response(
            {"error": str(exc), "code": "creative_invalid"},
        ...
        if request.method == "GET":
            ...
            if action == "export":
                ...
                return web.json_response(
                    store.export(id, revision),
                    headers={
                        "Content-Disposition": f'attachment; filename="universe-{id}.json"'
                    },
                )
            elif action == "revisions":
                result = {"items": store.revisions(id)}
        ...
        if request.method == "GET":
            ...
            if action == "export":
                ...
                return web.json_response(
                    store.export(id, revision),
                    headers={
                        "Content-Disposition": f'attachment; filename="series-{id}.json"'
                    },
                )
            elif action == "revisions":
                result = {"items": store.revisions(id)}
            ...
            if chapter_id:
                result = (
                    await store.draft(id, chapter_id, payload)
                    if action == "draft"
                    else getattr(store, action)(id, chapter_id, payload)
                )
            else:
                result = (
                    store.restore(id, payload)
        ...
        if request.method == "GET":
            ...
            if action == "suggestions":
                ...
                return web.json_response(
                    store.export(id, revision),
                    headers={
                        "Content-Disposition": f'attachment; filename="story-{id}.json"'
                    },
                )
            elif action == "revisions":
                result = {"items": store.revisions(id)}
        ...
        if request.method == "GET":
            ...
            if request.match_info.get("draft_id"):
                ...
                return web.json_response(
                    store.export(id, revision),
                    headers={
                        "Content-Disposition": f'attachment; filename="work-{id}.json"'
                    },
                )
            elif action == "revisions":
                result = {"items": store.revisions(id)}
        ...
        if request.method == "GET":
            ...
            if action == "sources":
                ...
                return web.json_response(
                    store.export(id, revision),
                    headers={
                        "Content-Disposition": f'attachment; filename="author-{id}.json"'
                    },
                )
            elif action == "revisions":
                result = {"items": store.revisions(id)}
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if id:
                ...
                result["items"] = [
                    {**item, "source_status": store.source_status(item)}
        ...
        return web.json_response(
            {**record, "source_status": store.source_status(record)},
            status=201 if request.method == "POST" and not id else 200,
        )
    except (CatalogError, ValueError, TypeError) as exc:
        return web.json_response(
            {"error": str(exc), "code": "creative_invalid"},
...
async def universe_graph(request):
    try:
        ...
        result = (
            graph.graph(id)
            if action == "graph"
            else getattr(graph, action.replace("-", "_"))(id, await request.json())
        )
        return web.json_response(result)
    except (CatalogError, ValueError, TypeError) as exc:
        return web.json_response(
...
async def polishing(request):
    try:
        ...
        if request.method == "GET":
            ...
            payload = await request.json()
            result = (
                store.promote(id, proposal, payload)
                if proposal
                else await store.propose(id, payload)
            )
        return web.json_response(result)
    except (CatalogError, ValueError, TypeError) as exc:
        return web.json_response(
...
async def continuity(request):
    try:
        ...
        if request.method == "GET":
            result = (
                store.export(id) if request.path.endswith("/export") else store.get(id)
            )
        elif "proposal_id" in request.match_info:
            result = store.accept(
                id, request.match_info["proposal_id"], await request.json()
            )
        else:
...
async def voice(request):
    try:
        ...
        id = request.match_info["id"]
        result = (
            store.configure(id, await request.json())
            if request.method == "PATCH"
            else store.report(id)
        )
        return web.json_response(result)
    except (CatalogError, ValueError, TypeError) as exc:
        return web.json_response(
...
async def editorial(request):
    try:
        ...
        if request.method == "GET":
            ...
            result = await store.repair(
                id,
                request.match_info["run_id"],
                request.match_info["finding_id"],
                await request.json(),
            )
        else:
...
async def editorial_controls(request):
    try:
        ...
        if path.endswith("/policy"):
            result = store.controls.configure(
                id, payload, {row["id"] for row in store.catalog(id)}
            )
        elif "/custom-checks" in path:
            operation = (
                "delete"
                if request.method == "DELETE"
                else "update" if "custom_id" in request.match_info else "create"
            )
            result = store.controls.custom(
                id,
                {
                    **payload,
                    "operation": operation,
                    "id": request.match_info.get("custom_id"),
                },
            )
        elif path.endswith("/reviews"):
            result = await store.controls.review(id, payload)
        elif "finding_id" in request.match_info:
            result = await store.controls.cut(
                store,
                id,
                request.match_info["run_id"],
                request.match_info["finding_id"],
...
def register(app):
    ...
    app.router.add_post(
        "/api/capabilities/creative/series/{id}/chapters/{chapter_id}/prepare", series
    )
    app.router.add_post(
        "/api/capabilities/creative/series/{id}/chapters/{chapter_id}/draft", series
    )
    app.router.add_post(
        "/api/capabilities/creative/series/{id}/chapters/{chapter_id}/review", series
    )
    ...
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/context", editorial
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/runs", editorial
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/runs/{run_id}/findings/{finding_id}/repair",
        editorial,
    )
    app.router.add_patch(
        "/api/capabilities/creative/works/{id}/editorial/policy", editorial_controls
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/custom-checks",
        editorial_controls,
    )
    app.router.add_patch(
        "/api/capabilities/creative/works/{id}/editorial/custom-checks/{custom_id}",
        editorial_controls,
    )
    ...
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/reviews", editorial_controls
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/runs/{run_id}/findings/{finding_id}/cut",
        editorial_controls,
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/cuts/{cut_id}/apply",
        editorial_controls,
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/editorial/cuts/{cut_id}/undo",
        editorial_controls,
    )
    app[CONTINUITY] = ContinuityStore(app[WORKS])
    app.router.add_get("/api/capabilities/creative/works/{id}/continuity", continuity)
    app.router.add_get(
        "/api/capabilities/creative/works/{id}/continuity/export", continuity
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/continuity/proposals", continuity
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/continuity/proposals/{proposal_id}/accept",
        continuity,
    )
    ...
    app.router.add_get(
        "/api/capabilities/creative/works/{id}/polishing/{proposal_id}", polishing
    )
    app.router.add_post(
        "/api/capabilities/creative/works/{id}/polishing/{proposal_id}/promote",
        polishing,
    )
    ...
    app.router.add_get(
        "/api/capabilities/creative/universes/{id}/graph", universe_graph
    )
    app.router.add_post(
        "/api/capabilities/creative/universes/{id}/merge-preview", universe_graph
    )
    app.router.add_post(
        "/api/capabilities/creative/universes/{id}/merge", universe_graph
    )
