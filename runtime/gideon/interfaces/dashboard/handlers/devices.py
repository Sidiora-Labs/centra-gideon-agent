...
async def api_devices_pair_complete(request: web.Request) -> web.Response:
    ...
    if remaining:
        _audit(
            ...
            "denied",
            caller=ip,
            error=f"locked out retry_after={remaining}s",
        )
        return json_error(
    ...
    if not outcome.ok:
        ...
        err = (
            ERR_CODE_EXPIRED
            if outcome.result == pairing.RESULT_EXPIRED
            else ERR_CODE_INVALID
        )
    ...
    device = DeviceInfo(
        ...
        name=name
        or sanitize_device_name(outcome.label)
        or _derive_device_name(request),
        kind=(
            kind
            if kind != "unknown"
            else sanitize_device_kind(_derive_device_kind(request))
        ),
        minted_at=time.time(),
    )
    if not nonce or not attach_device(nonce, device):
        ...
        _audit(
            ...
            "denied",
            caller=ip,
            error="could not persist the device session",
        )
...
def _nonce_of(token: str) -> str:
    ...
    """
    from gideon.interfaces.dashboard.token_auth import _b64url_decode
    ...
    try:
        ...
        logger.warning(
            "could not read the nonce out of a freshly minted token", exc_info=True
        )
        return ""
...
async def pair_page(request: web.Request) -> web.Response:
    ...
    **Why exempting this path from token auth opens nothing.** The document is a CONSTANT: the
    ...
    """
    from gideon.interfaces.dashboard.handlers.auth import has_valid_session
