...
async def endpoint(request):
    ...
    try:
        if request.method == 'POST':
            ...
            value = insights.read_narrative(request.query['slug'], int(request.query['version']) if 'version' in request.query else None)
        else:
            value = insights.view()
        ...
        return web.json_response({'error': 'Invalid or unavailable scorecard'}, status=400)
