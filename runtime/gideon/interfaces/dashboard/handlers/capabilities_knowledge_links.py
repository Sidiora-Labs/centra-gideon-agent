...
async def operation(request):
    try:
        ...
        if _blocks_reads_session(state, request) or (request.method != "GET" and _is_restricted_session(state, request)):
            raise CaptureError("This session cannot access personal link collections", 403)
        service = request.app["capability_link_studies"]
        identity = request.match_info.get("identity")
        action = request.match_info.get("action", "")
        if request.path.endswith("/buckets") and request.method == "GET": result = {"buckets": service.buckets()}
            ...
            if not isinstance(body, dict) or set(body) != {"ids"}: raise CaptureError("Bucket reorder requires ids")
            result = {"buckets": service.reorder_buckets(body["ids"])}
        elif action == "bucket" and request.method == "PATCH": result = service.update_bucket(identity, await read_json_body(request))
        elif action == "bucket" and request.method == "DELETE": result = service.delete_bucket(identity)
        elif request.path.endswith("/links"):
            result = service.add_link(await read_json_body(request))
        elif action == "links-reorder":
            body = await read_json_body(request)
            if not isinstance(body, dict) or set(body) != {"ids"}: raise CaptureError("Link reorder requires ids")
            result = {"links": service.reorder_links(identity, body["ids"])}
        elif request.path.endswith("/repositories") and request.method == "GET": result = service.repos()
        elif request.path.endswith("/repositories"):
            session_key = request.headers.get("X-Session-Key", "")
            if not session_key: raise CaptureError("An active session is required for repository intake", 403)
            result = await service.intake(await read_json_body(request), session_key)
        elif action == "repository": result = service.repo(identity)
        elif action == "study": result = service.restudy(identity)
        elif action == "report": result = service.report(identity)
        else: raise CaptureError("Unknown link operation", 404)
