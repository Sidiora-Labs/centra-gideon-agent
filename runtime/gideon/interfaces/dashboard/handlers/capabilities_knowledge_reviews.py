...
async def operation(request):
    try:
        ...
        allowed = {'period', 'date', 'timezone'} if preview else ({'limit', 'offset'} if request.method == 'GET' and not schedules else set())
        ...
        if preview:
            result = service.preview(request.query.get('period', 'daily'), request.query.get('date'), request.query.get('timezone', 'UTC'))
            for section in result['sections']:
                result['sections'][section] = [{**row, 'title': _redact(row)['title']} for row in result['sections'][section]]
        elif schedules:
            selected = request.app['capability_review_schedules']
            result = selected.list() if request.method == 'GET' else selected.save(await read_json_body(request))
...
def register(app):
    ...
    app['capability_review_service'], app['capability_review_schedules'] = service, schedules
