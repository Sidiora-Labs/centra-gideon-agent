...
async def operation(request):
    try:
        ...
        if request.method == "GET":
            ...
            result = (
                service.matches(request.match_info["id"], **paging)
                if "id" in request.match_info
                else service.list(**paging)
            )
        else:
            body = await read_json_body(request)
            result = (
                service.delete(request.match_info["id"], body)
                if request.method == "DELETE"
                else service.save(body)
            )
...
def register(app):
    state = app["state"]
    memory = (
        state.context_builder.memory
        if state.context_builder
        else getattr(state, "_standalone_memory", None)
    )
    archive = getattr(memory, "vector_store", None)
    app["capability_tracked_topics"] = TrackedTopics(
        state.knowledge_store,
        MemoryService.over_vector_store(archive) if archive is not None else None,
    )
