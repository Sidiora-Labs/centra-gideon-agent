...
async def api_autonudge_list(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"enabled": True, "loops": [_serialize(lp) for lp in svc.list_all()]}
    )
...
async def api_autonudge_get(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"enabled": True, "loop": _serialize(loop) if loop else None}
    )
...
async def api_autonudge_start(request: web.Request) -> web.Response:
    ...
    if not session_name or not message:
        return web.json_response(
            {"error": "session_name and message required"}, status=400
        )
    if session_name not in state._sessions:
        return web.json_response(
            {"error": f"unknown session {session_name}"}, status=404
        )
    if len(message) > 8000:
        return web.json_response(
            {"error": "message too long (max 8000 chars)"}, status=400
        )
...
async def api_autonudge_update(request: web.Request) -> web.Response:
    ...
    sel().log_tool_invocation(
        ...
        metadata={
            "loop_id": loop_id,
            "fields": [
                k for k in ("message", "idle_secs", "max_cycles", "active") if k in body
            ],
            "caller": request.remote or "",
        },
    )
