...
def register(app: web.Application, home: Path | None = None):
    ...
    async def handle(request):
        try:
            ...
            if request.method == 'POST':
                result = await asyncio.to_thread(getattr(store, suffix), await read_json_body(request))
            elif request.method == 'PUT':
                result = await asyncio.to_thread(store.annotate, identity, await read_json_body(request))
            elif suffix == 'original':
                raw = await asyncio.to_thread(store.original, identity)
                return web.Response(body=raw, content_type='application/octet-stream', headers={'Content-Disposition': 'attachment; filename="genome-source.txt"'})
            elif suffix == 'history':
                ...
                result = {'variants': await asyncio.to_thread(store.list_variants, identity, chromosome=request.query.get('chromosome'), rsid=request.query.get('rsid'), limit=int(request.query.get('limit', '100')), offset=int(request.query.get('offset', '0')))}
            else:
                method = store.get_source if '/sources/' in request.path else store.get_variant
    ...
    for path in ('sources', 'sources/{id}', 'sources/{id}/original', 'sources/{id}/variants', 'variants/{id}', 'variants/{id}/history'):
