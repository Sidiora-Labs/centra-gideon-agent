...
async def api_packs_bundled(request: web.Request) -> web.Response:
    ...
    from gideon.extensions.packs.bundled import bundled_packs
...
async def api_pack_bundled_install(request: web.Request) -> web.Response:
    ...
    from gideon.extensions.packs.import_ import PackImportRefused, import_pack
    from gideon.security.supply_chain import TrustTier

    name = request.match_info.get("name", "")
    if get_bundled(name) is None:
        return json_error(
            "pack_not_bundled", message=f"no bundled pack named {name!r}", status=404
        )
...
async def api_pack_roster_deploy(request: web.Request) -> web.Response:
    ...
    """
    from gideon.extensions.packs.installed import load_installed
    from gideon.extensions.packs.roster import deploy_roster, load_roster

    name = request.match_info.get("name", "")
    if not any(p.name == name for p in load_installed()):
        return json_error(
            "pack_not_installed", message=f"pack not installed: {name}", status=404
        )
...
async def api_pack_bindings(request: web.Request) -> web.Response:
    ...
    if not key:
        return json_error(
            "binding_key_required", message="a `key` is required", status=400
        )
    try:
...
async def api_pack_prompt_card(request: web.Request) -> web.Response:
    ...
    from gideon.extensions.packs.prompt_cards import PromptCardError, import_prompt_card
    ...
    try:
        ...
        Exception
    ) as exc:  # noqa: BLE001 — a model/provider failure is the caller's answer
        logger.warning("prompt-card import failed: %s", exc, exc_info=True)
        return json_error(
            "prompt_card_failed", message=f"{type(exc).__name__}: {exc}", status=502
        )
...
async def api_pack_one_link(request: web.Request) -> web.Response:
    ...
    from gideon.extensions.packs.onelink import OneLinkError, import_onelink
    ...
    if not isinstance(doc, dict):
        return json_error(
            "one_link_required", message="a `link` object is required", status=400
        )
    try:
        plan = import_onelink(
            doc,
...
async def api_pack_proposals(request: web.Request) -> web.Response:
    ...
    from gideon.extensions.packs.fingerprint import SCAN_REASON_ON_DEMAND, scan_project

    wanted = str(request.query.get("project_id", "") or "").strip()
    projects = [
        p for p in HierarchyStore().list_projects() if not wanted or p.id == wanted
    ]
    if wanted and not projects:
        return json_error(
            "project_not_found", message=f"no project {wanted!r}", status=404
        )
    out: list[dict] = []
    for project in projects:
        try:
            out.extend(
                p.to_dict() for p in scan_project(project, reason=SCAN_REASON_ON_DEMAND)
            )
        except (
            Exception
        ) as exc:  # noqa: BLE001 - one unscannable workspace must not blank the rest
            logger.warning(
                "fingerprint scan failed for project %s: %s", project.id, exc
            )
...
async def api_pack_proposal_reject(request: web.Request) -> web.Response:
    ...
    from gideon.extensions.packs.fingerprint import reject_proposal
...
async def api_pack_update(request: web.Request) -> web.Response:
    ...
    if get_bundled(name) is None:
        return json_error(
            "pack_not_bundled",
            message=f"no bundled pack named {name!r} to update from",
            status=404,
        )
