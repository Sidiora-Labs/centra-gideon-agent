...
def register(app: web.Application, home: Path | None = None):
    ...
    async def handle(request):
        try:
            ...
            if suffix == '/download':
                return web.Response(body=await asyncio.to_thread(store.download), content_type='application/json', headers={'Content-Disposition': 'attachment; filename="wellbeing-shared.json"'})
            if request.method == 'GET':
                result = await asyncio.to_thread(store.status)
            else:
                payload = await read_json_body(request)
                methods = {'/preview': store.preview, '/commit': store.commit, '/file/commit': store.commit_file, '/publish': store.publish}
