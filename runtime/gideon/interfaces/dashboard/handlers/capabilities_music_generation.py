...
def register(app, service=None):
    home = config_dir()
    service = service or MusicGeneration(home, MusicCatalog(home / 'capabilities' / 'music', NativeArtifactProvider(root=home / 'artifacts')))
    service.recover()
    ...
    async def handle(request):
        try:
            ...
            if job_id:
                result = await service.cancel(job_id) if request.method == 'POST' else service.get(job_id)
                return web.json_response({'job': result})
            if operation == 'config':
                return web.json_response({'config': service.configure(await read_json_body(request)) if request.method == 'PATCH' else service.config()})
            if operation == 'readiness':
                return web.json_response(service.readiness())
            if request.method == 'POST':
                return web.json_response({'job': await service.submit(await read_json_body(request))}, status=202)
            return web.json_response({'jobs': service.list(offset=int(request.query.get('offset', 0)), limit=int(request.query.get('limit', 50)))})
