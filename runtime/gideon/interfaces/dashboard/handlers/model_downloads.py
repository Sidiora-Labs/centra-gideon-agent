...
async def _download_precheck(reg, provider_name: str, model: str):
    ...
    Returns a :class:`gideon.integrations.local_models.fit.DiskPrecheck`. None (skip entirely) when
    ...
    from gideon.integrations.local_models.registry import catalog_for, get_provider
    ...
    try:
        ...
        logger.debug(
            "catalog lookup failed for %s/%s", provider_name, model, exc_info=True
        )
...
async def api_model_download_stream(request: web.Request) -> web.StreamResponse:
    ...
    """
    from gideon.interfaces.dashboard.model_downloads import registry_key
...
async def api_local_model_delete(request: web.Request) -> web.Response:
    ...
    if provider is None:
        return web.json_response(
            {"error": f"Unknown provider {provider_name!r}"}, status=404
        )
    ...
    try:
        ...
        logger.warning(
            "model delete failed for provider %r", provider_name, exc_info=True
        )
...
def _provider_cache_roots() -> list:
    ...
    from gideon.integrations.local_models.registry import list_providers
...
async def api_sidecar_install_delete(request: web.Request) -> web.Response:
    ...
    from gideon.integrations.local_models.sidecar import unregister_runner

    unregister_runner(provider)
...
async def api_models_loaded(request: web.Request) -> web.Response:
    ...
    """
    from gideon.integrations.local_models.residency import residency_snapshot
...
async def api_local_model_search(request: web.Request) -> web.Response:
    ...
    from gideon.integrations.local_models.registry import get_provider, to_local_model
    ...
    try:
        ...
        logger.warning(
            "model search failed for provider %r", provider_name, exc_info=True
        )
        return web.json_response({"models": [], "error": relayed_failure_copy(exc)})
    from gideon.integrations.local_models.registry import capabilities_for
...
def register_model_download_routes(app: web.Application) -> None:
    ...
    app.router.add_get(
        "/api/models/downloads/cleanup-candidates",
        api_model_download_cleanup_candidates,
    )
    ...
    app.router.add_post(
        "/api/models/sidecar/{provider}/install", api_sidecar_install_start
    )
    app.router.add_get(
        "/api/models/sidecar/{provider}/install/status", api_sidecar_install_status
    )
    app.router.add_delete(
        "/api/models/sidecar/{provider}/install", api_sidecar_install_delete
    )
    ...
    app.router.add_delete(
        "/api/models/local/{provider}/{model}", api_local_model_delete
    )
