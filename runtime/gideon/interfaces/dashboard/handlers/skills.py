...
SKILL_FILE_MAX_BYTES = 1_048_576
SKILL_FILES_MAX = 500
...
async def api_skills_list(request: web.Request) -> web.Response:
    ...
    ``intact`` (on-disk hashes match the install-time ``.gideon-lock.json`` baseline),
...
async def api_skills_marketplace_detail(request: web.Request) -> web.Response:
    ...
    try:
        ...
        logger.warning(
            "skills detail fetch failed for %s/%s: %s", marketplace_name, skill_id, exc
        )
        return web.json_response({"error": relayed_failure_copy(exc)}, status=500)
    ...
    frontmatter: dict[str, Any] = dict(
        ProcedureLibrary._parse_frontmatter_text(skill_md)
    )
    body = ProcedureLibrary.strip_frontmatter(skill_md) if frontmatter else skill_md

    file_view = [
        {"path": f.get("path", ""), "binary": "data" in f} for f in detail.files
    ]
    return web.json_response(
        {
            "id": detail.id,
...
async def api_skill_files(request: web.Request) -> web.Response:
    ...
    if not _safe_skill_name(name) or (
        rel and (".." in rel or rel.startswith("/") or "\\" in rel)
    ):
    ...
    if not rel:
        ...
        for f in detail.files:
            ...
            if not p or _is_sensitive(p):
                continue
            files.append(
                {"path": p, "size": len(f.get("contents", "").encode("utf-8"))}
            )
...
async def api_skills_install(request: web.Request) -> web.Response:
    ...
    try:
        ...
        _sel_log(
            "skills.install",
            "denied",
            f"notfound:{marketplace_name}/{skill_id}",
            request,
        )
        return json_error("not_found", message=str(exc), status=404)
...
async def api_skill_verify(request: web.Request) -> web.Response:
    ...
    Compares on-disk file hashes against the install-time ``.gideon-lock.json`` baseline
...
async def api_ephemeral_skill_promote(request: web.Request) -> web.Response:
    ...
    if not slug or scope not in ("agent", "global"):
        return web.json_response(
            {"error": "slug + scope('agent'|'global') required"}, status=400
        )
    try:
...
async def api_skill_proposal_accept(request: web.Request) -> web.Response:
    ...
    try:
        result = proposals.accept(
            ...
            procedure_md=(
                str(body["procedure_md"]) if body.get("procedure_md") else None
            ),
        )
