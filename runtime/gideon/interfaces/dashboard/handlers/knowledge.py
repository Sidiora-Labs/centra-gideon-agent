...
async def create_item(request: web.Request) -> web.Response:
    ...
    if not title and not content.strip() and not url:
        return web.json_response(
            {"error": "title, content, or url required"}, status=400
        )
    if not title:
    ...
    item_id = provider.create_typed(
        ...
        gist_language=(
            str(body.get("gist_language") or "") if item_type == "gist" else ""
        ),
    )
...
async def get_item_file(request: web.Request) -> web.StreamResponse:
    ...
    path, mime = _serve_item_path(
        _store(request), request.match_info["id"], thumbnail=False
    )
    if path is None:
        return web.json_response({"error": "not found"}, status=404)
...
async def get_item_thumbnail(request: web.Request) -> web.StreamResponse:
    ...
    path, mime = _serve_item_path(
        _store(request), request.match_info["id"], thumbnail=True
    )
    if path is None:
        return web.json_response({"error": "not found"}, status=404)
    return web.FileResponse(path, headers={"Content-Type": mime})
...
async def list_source_recipes(request: web.Request) -> web.Response:
    ...
    """
    from gideon.cognition.knowledge.source_recipes import list_recipes, recipes_for_url
...
async def list_entities(request: web.Request) -> web.Response:
    ...
    params.append(limit)
    rows = store.db.execute(
        f"SELECT * FROM entities WHERE {' AND '.join(where)} ORDER BY name LIMIT ?",
        params,
    ).fetchall()  # noqa: S608
...
async def merge_items(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"ok": True, "kept": keep_id, "merged": merge_id, "moved": moved}
    )
...
async def get_related_items(request: web.Request) -> web.Response:
    ...
    if (
        store.db.execute("SELECT 1 FROM items WHERE id = ?", (item_id,)).fetchone()
        is None
    ):
        return web.json_response({"error": "item not found"}, status=404)

    edges: dict[str, dict] = {}
    for edge in store.similar_items(
        item_id, limit=limit * _RELATED_OVERFETCH, min_score=min_score
    ):
...
def _load_project_2d():
    ...
    """
    from gideon.cognition.knowledge.projection import project_2d
...
def _thin_edges(graph, *, min_weight: float, top_k: int) -> tuple[list[dict], int]:
    ...
    kept = [
        {"source": u, "target": v, "type": t or None, "weight": w}
        for w, u, v, t in ordered
    ]
    return kept, len(raw)
...
def _cluster_labels(
    store, graph, comps: list[list[str]]
    ...
    for cid, comp in enumerate(c for c in comps if len(c) >= _GRAPH_CLUSTER_MIN_SIZE):
        ...
        if tally:
            ...
            if types:
                ...
                names = sorted(
                    n for n in (graph.nodes[e].get("name") for e in comp) if n
                )
                label, source = (
                    (names[0], "entity_name") if names else (comp[0], "entity_id")
                )
        clusters.append(
            {"id": cid, "label": label, "label_source": source, "size": len(comp)}
        )
        for eid in comp:
            membership[eid] = cid
...
async def batch_embed_items(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"embedded": embedded, "total": len(rows), "failed": failed}
    )
...
async def search_for_context(request: web.Request) -> web.Response:
    ...
    try:
        max_tokens = max(
            1,
            min(
                int(request.query.get("max_tokens", max_tokens)),
...
def _consolidated_text(store, item: dict) -> str:
    ...
    """
    from gideon.cognition.knowledge.slicing import is_slice_row
...
async def _run_intent_retroactive(app: web.Application, intent_id: str) -> dict:
    ...
    from gideon.cognition.knowledge.intents import IntentStore, match_intent
    ...
    prior = {
        o["item_id"] for o in store.outcomes_for_intent(intent_id) if o.get("item_id")
    }
    ...
    candidates = [
        ...
        if intent.applies_to(it.get("item_type") or it.get("type") or "")
    ]
    ...
    results = await asyncio.gather(
        *(_match(it) for it in candidates), return_exceptions=True
    )
...
async def generate_skill_from_intent(request: web.Request) -> web.Response:
    ...
    for o in outcomes[:30]:
        flds = "; ".join(
            f"{f.get('name')}={f.get('value')}" for f in (o.get("fields") or [])[:6]
        )
    ...
    if not pool:
        return web.json_response(
            {"error": "No model available to synthesize a skill."}, status=503
        )
    try:
    ...
    if not parts.get("procedure"):
        return web.json_response(
            {"error": "Could not synthesize a skill from the gathered outcomes."},
            status=502,
        )
    ...
    name = _loader.create_auto_skill(
        ...
        provenance=AutoSkillProvenance(
            session_key=f"intent:{intent_id}", created_at=now
        ),
    )
    if name is None:
        return web.json_response(
...
async def stream_item_ingest(request: web.Request) -> web.StreamResponse:
    ...
    from gideon.cognition.knowledge.pipeline.runner import progress_feed
...
async def library_home(request: web.Request) -> web.Response:
    ...
    try:
        limit = min(
            _HOME_SHELF_MAX, max(1, int(request.query.get("limit", _HOME_SHELF_LIMIT)))
        )
    except ValueError:
...
async def set_item_read_state(request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {
                "error": {
                    "code": "state_required",
                    "message": "supply state: unread|reading|read",
                }
            },
            status=400,
        )
    ...
    if not ok:
        return web.json_response({"error": "item not found"}, status=404)
    return web.json_response(
        {"ok": True, "read_state": store.get_item(iid)["read_state"]}
    )
...
async def set_item_favorited(request: web.Request) -> web.Response:
    ...
    if not store.set_favorited(iid, bool(body.get("value", True))):
        return web.json_response({"error": "item not found"}, status=404)
    return web.json_response(
        {"ok": True, "favorited": store.get_item(iid)["favorited"]}
    )
...
async def get_item_staleness(request: web.Request) -> web.Response:
    ...
    in :mod:`gideon.cognition.knowledge.staleness`.
    """
    store = _store(request)
    item_id = request.match_info["id"]
    try:
...
async def regenerate_item(request: web.Request) -> web.Response:
    ...
    in :func:`gideon.cognition.knowledge.updates.regenerate_synthesis` — this route is the HTTP
    ...
    * no model produced a synthesis — ``503`` with ``reason: "model_unavailable"``, the same
    ...
    :func:`gideon.cognition.knowledge.updates.propose_update` says so under that key and ``null``
    ...
    try:
        from gideon.cognition.knowledge.updates import (
    ...
    try:
        ...
        return web.json_response(
            {"error": str(exc), "reason": "model_unavailable"}, status=503
        )
    if not isinstance(
        result, dict
    ):  # pragma: no cover — the outcome contract is a dict
        result = {}
...
def _int_param(
    request: web.Request, name: str, default: int, *, low: int, high: int
) -> int:
        ...
        return web.json_response(
            {
                "error": {
                    "code": "into_required",
                    "message": "supply into: <tag id> (an integer)",
                }
            },
            status=400,
        )
        ...
        return web.json_response(
            {
                "error": {
                    "code": (
                        code
                        if code == "smart_collection_immutable"
                        else "invalid_bulk_args"
                    ),
                    "message": code,
                }
            },
...
def _source_providers() -> list:
    ...
    """
    from gideon.integrations.knowledge_providers.base import KnowledgeSourceProvider
    from gideon.integrations.knowledge_providers.registry import list_providers
...
def _kind_descriptor(provider) -> dict:
    ...
    nobody can use.
    """
    from gideon.integrations.knowledge_providers import (
        dir_source,
        feed_source,
        web_source,
    )
        ...
        out.append(
            {
                ...
                "poll_interval_secs": int(
                    getattr(prov, "poll_interval_seconds", 3600) or 3600
                ),
...
def _remediation(source: dict) -> dict:
    ...
    """
    from gideon.integrations.knowledge_providers.base import HEALTH_NEEDS_RENDER
...
async def list_watched_sources(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {
            "sources": [
                _serialize_source(s, enrolled) for s in _store(request).list_sources()
            ],
            "kinds": kinds,
...
async def create_watched_source(request: web.Request) -> web.Response:
    ...
    if provider is None:
        known = (
            ", ".join(sorted(p.name for p in _source_providers())) or "none registered"
        )
        return web.json_response(
            {"error": f"unknown source provider {provider_name!r} (known: {known})"},
            status=400,
        )
    ...
    item_type = (
        str(body.get("item_type") or "").strip() or descriptor["default_item_type"]
    )
    if item_type not in _KNOWLEDGE_TYPES:
        return web.json_response({"error": f"unknown type {item_type!r}"}, status=400)
    if item_type not in _AUTHORABLE_TYPES:
    ...
    _sel_log(
        "sources.create", source_id=sid, provider=provider.name, enrichment=enrichment
    )
    created = store.get_source(sid)
    return web.json_response(
        {
            "source": _serialize_source(
                created or {}, {p.name for p in _source_providers()}
            )
        },
        status=201,
    )
...
async def update_watched_source(request: web.Request) -> web.Response:
    ...
    if "enrichment" in body:
        ...
        if enrichment not in ENRICHMENTS:
            return web.json_response(
                {"error": f"enrichment must be one of {sorted(ENRICHMENTS)}"},
                status=400,
            )
    ...
    if "poll_interval_secs" in body:
        try:
            ...
            return web.json_response(
                {"error": "poll_interval_secs must be an integer"}, status=400
            )
        if interval < 1:
            return web.json_response(
                {"error": "poll_interval_secs must be positive"}, status=400
            )
    ...
    if "spec" in body:
        ...
        provider = next(
            (p for p in _source_providers() if p.name == current["provider"]), None
        )
        if provider is None:
            return web.json_response(
                {"error": f"provider {current['provider']!r} is not registered"},
                status=400,
            )
...
async def _restructure_body(request: web.Request) -> tuple[dict, web.Response | None]:
    ...
    if not isinstance(body, dict):
        return {}, web.json_response(
            {"error": {"code": "bad_body", "message": "body must be an object"}},
            status=400,
        )
    return body, None
...
async def get_item_sections(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"sections": restructure.sections(content), "length": len(content)}
    )
...
async def restructure_item(request: web.Request) -> web.Response:
    ...
    if params is None:
        params = {
            k: v for k, v in body.items() if k not in ("confirm", "token", "relink")
        }
    if not isinstance(params, dict):
        return web.json_response(
            {"error": {"code": "bad_body", "message": "params must be an object"}},
            status=400,
        )
    try:
    ...
    if not body.get("confirm"):
        return web.json_response(
            {"confirmed": False, "token": plan.token, "plan": plan.to_dict()}
        )
        ...
        return web.json_response(
            {
                "error": {
                    "code": "token_required",
                    "message": "supply the token to undo",
                }
            },
            status=400,
        )
...
def setup_knowledge_routes(app: web.Application) -> None:
    ...
    app.router.add_post(
        "/api/knowledge/items/{id}/restructure/{verb}", restructure_item
    )
    ...
    app.router.add_post(
        "/api/knowledge/items/{id}/generate-intelligence", generate_intelligence
    )
    app.router.add_post(
        "/api/knowledge/regenerate-intelligence", regenerate_intelligence
    )
    ...
    app.router.add_post(
        "/api/knowledge/intents/{id}/generate-skill", generate_skill_from_intent
    )
