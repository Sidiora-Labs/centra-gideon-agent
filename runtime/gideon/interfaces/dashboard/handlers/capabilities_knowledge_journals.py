...
async def operation(request):
    try:
        ...
        if request.method == 'POST':
            ...
            arguments = {'date': request.query.get('date'), 'timezone': request.query.get('timezone', 'UTC')}
            result = service.draft(**arguments) if request.path.endswith('/draft') else service.get(**arguments)
