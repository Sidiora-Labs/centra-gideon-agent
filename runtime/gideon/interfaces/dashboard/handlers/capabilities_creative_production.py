...
async def production(request):
    try:
        ...
        return web.json_response({'error': str(exc), 'code': 'creative_production_invalid'}, status=getattr(exc, 'status', 400))
...
def register(app, home=None):
    ...
    app.router.add_post(root + '/{run_id}/{action:advance|submit|approve|rollback|pause|resume|cancel}', production)
