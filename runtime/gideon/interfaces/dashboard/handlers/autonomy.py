...
:func:`~gideon.security.guardrails.autonomy.grant_rung` — registration, ladder membership, the
...
async def api_autonomy(request: web.Request) -> web.Response:
    ...
    from gideon.security.guardrails.rungs import RUNG_HINTS, RUNG_LABELS
...
async def api_autonomy_grant(request: web.Request) -> web.Response:
    ...
    """
    from gideon.security.guardrails import ladder
    from gideon.security.guardrails.autonomy import grant_rung, promotion_eligibility
    ...
    return web.json_response(
        {"ok": True, "key": key, "rung": granted, "evidence": detail}
    )
...
async def api_autonomy_demote(request: web.Request) -> web.Response:
    ...
    """
    from gideon.security.guardrails.autonomy import action_type, demote
    from gideon.security.guardrails.rungs import ensure_core_action_types
    ...
    if result is None:
        return web.json_response(
            {"error": f"{key} is not a registered action type"}, status=404
        )
...
async def api_autonomy_undo(request: web.Request) -> web.Response:
    ...
    """
    from gideon.security.guardrails.ladder import reverse_action
    ...
    return web.json_response(
        payload, status=404 if outcome.code == "unknown_record" else 400
    )
