...
def register(app, store=None):
    home = config_dir()
    store = store or Image3DStore(home, NativeArtifactProvider(root=home / "artifacts"))
    ...
    async def raw(request):
        try:
            ...
            result = (
                store.artifacts.raw_bytes(slug, version=version) if artifact else None
            )
        ...
        try:
                ...
                return web.json_response(
                    {
                        "config": (
                            ...
                            else store.configure(await read_json_body(request))
                        )
                    }
                )
            ...
            if item_id and request.method == "POST":
                ...
                return web.json_response(
                    {
                        "job": (
                            await store.refresh(item_id)
                            if path.endswith("/refresh")
                            else store.stop(item_id)
                        )
                    }
                )
            ...
            return web.json_response(
                {"job": store.get(item_id)} if item_id else {"jobs": store.list()}
            )
    ...
    app.router.add_get(prefix + "/config", handle)
    app.router.add_patch(prefix + "/config", handle)
    app.router.add_get(prefix + "/readiness", handle)
    app.router.add_get(prefix + "/jobs", handle)
    app.router.add_post(prefix + "/jobs", handle)
    app.router.add_get(prefix + "/jobs/{id}", handle)
    app.router.add_post(prefix + "/jobs/{id}/refresh", handle)
    app.router.add_post(prefix + "/jobs/{id}/stop", handle)
