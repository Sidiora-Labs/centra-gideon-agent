...
:mod:`gideon.integrations.browse.mirror`, so the domain (the action provider) can call them without
...
async def api_browse_status(request: web.Request) -> web.Response:
    ...
    from gideon.integrations.browse.handoff import expired_sites
...
async def api_browse_kill_release(request: web.Request) -> web.Response:
    ...
    if not (isinstance(body, dict) and body.get("confirm") is True):
        return json_error(
            ...
            message='release requires {"confirm": true}',
            status=400,
        )
