...
def _build_loop_from_body(body: dict) -> Loop:
    ...
    loop = Loop(
        ...
        plan=[
            p
            for p in _as_list(body.get("plan") or body.get("stage_plan"))
            if isinstance(p, dict)
        ],
        roster=[r for r in _as_list(body.get("roster")) if isinstance(r, dict)],
        strategy_config=(
            _sc if isinstance((_sc := body.get("strategy_config")), dict) else {}
        ),
        ...
        success_criteria=(
            str(body["success_criteria"]) if body.get("success_criteria") else None
        ),
        kind_config=kc,
    )
...
async def _installed_capability_catalogs() -> tuple[list[dict], list[dict]]:
    ...
    workflows: list[dict] = []
    try:
        from gideon.extensions.skills.marketplace import list_local_skills
...
async def api_loop_grill_tree(request: web.Request) -> web.Response:
    ...
    result = await grill.grill(
        goal, shape="tree", ask=_ask, recall=_recall, assess=False
    )
    return web.json_response(
        {"phases": result.phases, "memory_hits": result.memory_hits}
    )
    ...
    try:
        ...
        for pi, phase in enumerate(phases):
            ...
            for si, st in enumerate(phase.get("steps") or []):
                text = str(
                    (st or {}).get("title") or (st or {}).get("prompt") or ""
                ).strip()
                if text:
        ...
        # so the real ConsoleState must be threaded through — passing None here would raise inside
        svc = MemoryService.over_vector_store(_get_provider(state))
...
async def _reap_loop_sessions(state, loop_id: str) -> None:
    ...
    for k in keys:
        try:
            await _remove_session_for_history_key(state, k)
        except Exception:
...
async def api_loop_nudge(request: web.Request) -> web.Response:
    ...
    if proj is None:
        return web.json_response({"error": "Not found"}, status=404)
    from gideon.automation.loop.loop import PRELAUNCH_STATUSES, TERMINAL_STATUSES
...
async def api_loop_queue(request: web.Request) -> web.Response:
    ...
    if action == "queue":
        if LoopStatus(loop.status) in TERMINAL_STATUSES:
            return web.json_response(
                {"error": f"Cannot queue tasks on a loop in '{loop.status}' state"},
                status=409,
            )
    ...
    try:
        request.app["state"].loop_sse().publish(
            registry_key(cid), "queued", {"loop_id": cid}
        )
        request.app["state"].push_refresh("loops")
    except Exception:
...
async def api_loop_autopilot(request: web.Request) -> web.Response:
    ...
    if loop is None:
        return web.json_response({"error": "Not found"}, status=404)
    from gideon.automation.loop.loop import TERMINAL_STATUSES

    if LoopStatus(loop.status) in TERMINAL_STATUSES:
        return web.json_response(
            {"error": f"Cannot change autopilot on a loop in '{loop.status}' state"},
            status=409,
        )
    raw = body.get("on")
...
def _kick_plan_advance(request: web.Request, cid: str) -> web.Response:
    ...
    async def _run() -> None:
        try:
            ...
            try:
                state.loop_sse().publish(
                    registry_key(cid), "plan_step", {"loop_id": cid}
                )
...
async def api_loop_design_tokens(request: web.Request) -> web.Response:
    ...
    overrides = (
        loop.kind_config.get("token_overrides")
