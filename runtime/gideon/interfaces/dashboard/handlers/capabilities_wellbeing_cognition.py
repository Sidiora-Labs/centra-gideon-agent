    ...
    async def handle(request):
        try:
            ...
            if request.method == 'POST':
                ...
                method = store.start if not identity else store.answer if request.path.endswith('/answers') else store.cancel
                ...
                result = {'sessions': await asyncio.to_thread(store.list_sessions, int(request.query.get('limit', '100')))}
