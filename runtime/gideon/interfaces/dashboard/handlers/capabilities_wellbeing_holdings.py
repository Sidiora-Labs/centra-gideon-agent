...
def register(app: web.Application, home: Path | None = None):
    ...
    async def subject_orgs(request):
        try:
            ...
            if request.method == 'POST':
                return await response(store.create_org, subject, await read_json_body(request))
    ...
    async def organization(request):
        try:
            ...
            if suffix == 'holdings':
                if request.method == 'POST':
                    return await response(store.set_holding, identity, await read_json_body(request))
                return await response(store.list_holdings, identity, envelope='holdings')
            if request.method == 'PUT':
                return await response(store.update_org, identity, await read_json_body(request))
    ...
    async def holding_history(request):
        try:
            return await response(store.history_holding, request.match_info['id'], envelope='history')
        except MeasurementError as exc:
            return json_error(exc.code, message=str(exc), status=exc.status)
    ...
    async def subject_changes(request):
        try:
            subject = request.match_info['subject']
            if request.method == 'POST':
                return await response(store.declare_change, subject, await read_json_body(request))
    ...
    async def change(request):
        try:
            ...
            if org and request.match_info.get('suffix') == 'history':
                return await response(store.history_target, identity, org, envelope='history')
            if org:
                return await response(store.settle, identity, org, await read_json_body(request))
    ...
    app.router.add_get(base + '/organizations/{id}/{suffix:history|holdings}', organization)
    ...
    app.router.add_get(base + '/changes/{id}/organizations/{org}/{suffix:history}', change)
