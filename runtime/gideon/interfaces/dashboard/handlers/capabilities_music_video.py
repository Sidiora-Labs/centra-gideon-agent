...
def register(app, store=None):
    ...
    async def handle(request):
        try:
            ...
            if is_job:
                ...
                return web.json_response({'job': store.get_job(item_id)} if item_id else {'jobs': store.jobs()})
            if request.method == 'GET':
                return web.json_response({'item': store.get(item_id)} if item_id else {'items': store.list(int(request.query.get('offset', 0)), int(request.query.get('limit', 50)))})
    ...
    app.router.add_get(prefix+'/jobs', handle); app.router.add_post(prefix+'/jobs', handle)
    app.router.add_get(prefix+'/jobs/{id}', handle); app.router.add_post(prefix+'/jobs/{id}/cancel', handle)
