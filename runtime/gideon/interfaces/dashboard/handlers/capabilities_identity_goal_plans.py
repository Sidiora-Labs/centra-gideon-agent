...
async def handle(request):
    try:
        ...
        if request.method == "GET":
            result = store.get(request.match_info["goal_id"]) if request.match_info.get("goal_id") else store.list()
        else:
            body = await request.json()
            if not isinstance(body, dict):
                raise ValueError("Expected a JSON object")
            result = getattr(store, "checkin" if request.match_info["operation"] == "checkins" else "configure")(**body)
        return web.json_response(result)
    except ConflictError as error:
        return web.json_response({"error": str(error)}, status=409)
    except KeyError:
        return web.json_response({"error": "Local goal or source not found"}, status=404)
    except (TypeError, ValueError) as error:
        return web.json_response({"error": str(error)}, status=400)


def register(app: web.Application, *, home: Path | None = None):
    app[KEY] = GoalPlanStore((home or config_dir()) / "capabilities/identity/goals.sqlite3")
