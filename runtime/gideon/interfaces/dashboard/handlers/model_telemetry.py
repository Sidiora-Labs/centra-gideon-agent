...
async def api_models_telemetry(request: web.Request) -> web.Response:
    ...
    try:
        ...
        from gideon.engine.routing.stats import load_stats
        ...
        return web.json_response(
            {
                "error": {
                    "code": "internal",
                    "message": "could not read routing telemetry",
                }
            },
            status=500,
        )
    return web.json_response(
        {"use_case": use_case, "query_class": query_class, "rows": rows}
    )
...
async def api_routing_policy(request: web.Request) -> web.Response:
    ...
    """
    try:
        from gideon.engine.routing.policy import is_local_ref, master_enabled, table_for
        ...
        for use_case in _ROUTED_USE_CASES:
            ...
            table["candidates"] = [
                {"ref": ref, "local": is_local_ref(ref)}
...
async def api_routing_policy_put(request: web.Request) -> web.Response:
    ...
    **Every lever is validated before any lever is applied.** Interleaving the two (validate mode,
    ...
    goes out.
    """
    from gideon.engine.routing.policy import MODES, set_mode, set_order, set_pin
    ...
    if not isinstance(body, dict):
        return json_error(
            "bad_request", message="a JSON object is required", status=400
        )
    use_case = str(body.get("use_case", "") or "")
    if use_case not in VALID_USE_CASES:
        return json_error(
            "bad_request", message=f"unknown use_case {use_case!r}", status=400
        )
    ...
    if "order" in body:
        ...
        if not isinstance(raw_order, list) or not all(
            isinstance(r, str) for r in raw_order
        ):
            return json_error(
                "bad_request", message="order must be a list of refs", status=400
            )
    ...
    if mode is None and pin is None and order is None:
        return json_error(
            "bad_request",
            message="nothing to change: send mode, pin, and/or order",
            status=400,
        )
        ...
        return web.json_response(
            {
                "error": {
                    "code": "internal",
                    "message": "could not save the routing policy",
                }
            },
            status=500,
        )
...
async def api_routing_proposals(request: web.Request) -> web.Response:
    ...
    """
    try:
        from gideon.engine.routing.proposals import pending
...
async def api_routing_proposal_accept(request: web.Request) -> web.Response:
    ...
    try:
        from gideon.engine.routing.proposals import accept, find
    ...
    if record is None:
        return json_error(
            "not_found", message=f"no routing proposal {proposal_id!r}", status=404
        )
    ...
    return json_error(
        "not_found",
        message=f"routing proposal {proposal_id!r} is not pending",
        status=404,
    )
...
async def api_routing_proposal_reject(request: web.Request) -> web.Response:
    ...
    proposal_id = request.match_info.get("id", "")
    try:
        from gideon.engine.routing.proposals import reject
...
def register_model_telemetry_routes(app: web.Application) -> None:
    ...
    app.router.add_post(
        "/api/models/routing-proposals/{id}/accept", api_routing_proposal_accept
    )
    app.router.add_delete(
        "/api/models/routing-proposals/{id}", api_routing_proposal_reject
    )
