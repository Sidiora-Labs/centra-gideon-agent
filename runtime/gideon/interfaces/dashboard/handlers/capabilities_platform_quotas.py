...
from gideon.workspace.capabilities.platform.quotas import QuotaError, SubscriptionQuotaStore
...
def register(app: web.Application, home: Path | None = None):
    ...
    async def plan(request):
        try:
            identity, action = request.match_info['id'], request.match_info.get('action')
            if request.method == 'PUT':
                return await respond(store.update_plan, identity, await read_json_body(request))
            if action == 'history':
                return await respond(store.history, identity, envelope='history')
            if action == 'summary':
                return await respond(store.summary, identity)
            if action == 'reservations':
                return await respond(store.list_reservations, identity, envelope='reservations')
            if action == 'provider-evidence':
                return await respond(store.provider_evidence, identity, envelope='evidence')
    ...
    async def reservations(request):
        try:
            if request.match_info.get('id'):
                return await respond(store.release, request.match_info['id'], await read_json_body(request))
            return await respond(store.reserve, request.match_info['plan'], await read_json_body(request))
        except QuotaError as exc:
            return json_error(exc.code, message=str(exc), status=exc.status)
        except RequestValidationError as exc:
            return json_error('invalid_request', message=str(exc), status=400)
    ...
    app.router.add_get(base + '/plans/{id}/{action:history|summary|reservations|provider-evidence}', plan)
