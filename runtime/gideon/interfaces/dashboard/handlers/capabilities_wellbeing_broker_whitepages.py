...
def register(app, home=None, adapter=None):
    ...
    service = adapter or WhitepagesCaseAdapter(PrivacyBrokerStore(root), PeopleStore())

    async def action(request):
        try:
            value = getattr(service, request.match_info["action"])(
    ...
    async def status(request):
        try:
            return web.json_response(
                service.status(
                    request.match_info["id"], int(request.query.get("revision", ""))
                ),
            ...
            if isinstance(exc, MeasurementError):
                return json_error(exc.code, message=str(exc), status=exc.status)
            return json_error(
                "invalid_request", message="revision is required", status=400
            )

    app.router.add_get(
        "/api/capabilities/wellbeing/privacy/broker-cases/{id}/providers/whitepages",
        status,
    )
    app.router.add_post(
        "/api/capabilities/wellbeing/privacy/broker-cases/{id}/providers/whitepages/{action:prepare|approve|send|correlate}",
        action,
    )
