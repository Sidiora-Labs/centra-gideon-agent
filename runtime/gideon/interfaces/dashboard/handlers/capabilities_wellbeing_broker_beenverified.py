...
def register(app, home=None, adapter=None):
    ...
    service = adapter or BeenVerifiedCaseAdapter(
        PrivacyBrokerStore(root), PeopleStore()
    )
    ...
    app.router.add_get(
        "/api/capabilities/wellbeing/privacy/broker-cases/{id}/providers/beenverified",
        status,
    )
    app.router.add_post(
        "/api/capabilities/wellbeing/privacy/broker-cases/{id}/providers/beenverified/{action:prepare|approve|send|correlate}",
        action,
    )
