        ...
        try:
            ...
            if path.endswith("/reminder/check"):
                ...
                result = await asyncio.to_thread(
                    store.check_reminder, inbox=live_store(request.app.get("state"))
                )
                ...
                result = await asyncio.to_thread(
                    store.projection, request.query.get("as_of")
                )
            elif path.endswith("/history"):
                result = {
                    "history": (
                        await asyncio.to_thread(store.history_event, identity)
                        if identity
                        else await asyncio.to_thread(store.config_history)
                    )
                }
