...
async def operation(request):
    try:
        if request.query:
            raise CaptureError("Vault operations do not accept query overrides")
        state = request.app["state"]
        if _blocks_reads_session(state, request) or (
            request.method != "GET" and _is_restricted_session(state, request)
        ):
            raise CaptureError("This session cannot access external vaults", 403)
        vaults = request.app["capability_external_vaults"]
        identity = request.match_info.get("identity")
        action = request.match_info.get("action", "")
        if request.path.endswith("/register"):
            result = vaults.register(await read_json_body(request))
        elif not identity:
            result = vaults.list()
        elif request.method == "DELETE" and not action:
            result = vaults.remove(identity)
        elif action == "scan":
            result = vaults.scan(identity)
        elif action == "read":
            body = await read_json_body(request)
            if not isinstance(body, dict) or set(body) != {"path"}:
                raise CaptureError("Vault read requires path")
            result = vaults.detail(identity, body["path"])
        elif action == "write":
            result = vaults.write(identity, await read_json_body(request))
        elif action == "delete":
            result = vaults.delete(identity, await read_json_body(request))
        elif action == "search":
            body = await read_json_body(request)
            if not isinstance(body, dict) or set(body) != {"query"}:
                raise CaptureError("Vault search requires query")
            result = vaults.search(identity, body["query"])
        elif action == "graph":
            result = vaults.graph(identity)
        else:
            raise CaptureError("Unknown vault operation", 404)
...
def register(app):
    ...
    app.router.add_post(root + "/register", operation)
    app.router.add_delete(root + "/{identity}", operation)
    app.router.add_post(
        root + "/{identity}/{action:scan|read|write|delete|search}", operation
    )
    app.router.add_get(root + "/{identity}/{action:graph}", operation)
