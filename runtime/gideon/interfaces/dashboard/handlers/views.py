...
async def api_genui_library(request: web.Request) -> web.Response:
    ...
    """
    from gideon.workspace.genui import library_manifest
    ...
    if request.method == "POST":
        ...
        try:
            view = store.create_view(
                name, icon=(str(body["icon"]) if body.get("icon") else None)
            )
...
async def api_dashboard_view_tile_binding(request: web.Request) -> web.Response:
    ...
    if not ref:
        return web.json_response(
            {"error": {"code": "tile_ref_required", "message": "ref is required"}},
            status=400,
        )
    ...
    try:
        ...
        return web.json_response(
            {"error": {"code": "tile_not_found", "message": "view or tile not found"}},
            status=404,
        )
    from dataclasses import asdict
...
async def api_dashboard_view_tile_refresh(request: web.Request) -> web.Response:
    ...
    result = await tile_refresh.refresh_tile(
        view_id, ref, force=bool(body.get("force", False))
    )
...
async def api_dashboard_view_tile_action(request: web.Request) -> web.Response:
    ...
    """
    from gideon.interfaces.dashboard import tile_actions
