...
from gideon.core.config.credential_migration import (
...
def register_security_credential_routes(app: web.Application) -> None:
    ...
    app.router.add_post(
        "/api/security/credentials/migrate", api_security_credentials_migrate
    )
    app.router.add_post(
        "/api/security/credentials/rollback", api_security_credentials_rollback
    )
