...
async def operation(request):
    try:
        allowed = {'limit', 'offset'} if request.method == 'GET' and 'id' not in request.match_info else set()
        ...
        if 'id' in request.match_info:
            return web.Response(body=service.original(request.match_info['id']), content_type='application/json', headers={'Content-Disposition': 'attachment; filename="conversations.json"'})
        if request.method == 'GET':
            result = service.list(int(request.query.get('limit', '20')), int(request.query.get('offset', '0')))
        else:
            body = await read_json_body(request)
            result = service.commit(body) if request.path.endswith('/commit') else service.preview(body)
...
def register(app):
    app['capability_conversation_archive'] = ConversationArchive(app['state'].knowledge_store)
