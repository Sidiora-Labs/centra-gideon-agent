...
def register(app: web.Application, home: Path | None = None):
    ...
    async def subject_cases(request):
        try:
            ...
            if request.method == 'POST':
                return await respond(store.create_case, subject, await read_json_body(request))
    ...
    async def case(request):
        try:
            ...
            if request.method == 'GET':
                method, envelope = (store.history, 'history') if action == 'history' else (store.events, 'events') if action == 'events' else (store.get_case, None)
                return await respond(method, identity, envelope=envelope)
            payload = await read_json_body(request)
            method = store.record_user_observation if action == 'observe' else store.transition if action == 'transition' else store.request_recheck
    ...
    app.router.add_post(base + '/broker-cases/{id}/{action:observe|transition|recheck}', case)
