...
async def owner(request):
    ...
    try:
        ...
        if request.method == "GET":
            result = (
                await asyncio.to_thread(service.get, execution_id)
                if execution_id
                else await asyncio.to_thread(service.list)
            )
        else:
            ...
            if request.path.endswith("/refresh"):
                ...
                if (
                    ...
                    or set(body) != {"peer_id", "request"}
                    or not isinstance(body["peer_id"], str)
                ):
        ...
        return web.json_response(
            {"error": "Remote media store unavailable"}, status=503
        )
    ...
    try:
        ...
        if action == "jobs":
            ...
            result = await asyncio.to_thread(
                service.cancel_remote, peer, body["payload"]
            )
        else:
            raise RemoteMediaError("Unknown remote media action", 404)
        return web.json_response(
            result,
            status=202 if action == "jobs" else 200,
            headers={"Cache-Control": "no-store"},
        )
    except (RemoteMediaError, PeerError) as error:
        return web.json_response({"error": str(error)}, status=error.status)
