        ...
        try:
            ...
            if request.method == "GET":
                return web.json_response(
                    {"draft": outbound._get(draft_id)}
                    if draft_id
                    else {"drafts": outbound.list()}
                )
            data = await request.json() if request.can_read_body else {}
            if not draft_id:
                row, created = outbound.draft(data)
                return web.json_response(
                    {"draft": row, "created": created}, status=201 if created else 200
                )
            if action == "approve":
                ...
                row = outbound.correlate(draft_id)
            else:
                raise PeopleError("Unknown outbound email operation", 404)
