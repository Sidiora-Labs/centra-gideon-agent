...
def register(app):
    ...
    for lane in HANDLERS:
        module = import_module(
            f"gideon.interfaces.dashboard.handlers.capabilities_{lane}"
        )
        module.register(app)
    additional = (
        ("/api/capabilities/knowledge/links/buckets", "knowledge_links"),
        ("/api/capabilities/knowledge/vaults", "knowledge_vaults"),
        ...
        (
            "/api/capabilities/wellbeing/privacy/subjects/{subject}/organizations",
            "wellbeing_holdings",
        ),
    ...
    for path, module_name in additional:
        if not any(route.resource.canonical == path for route in app.router.routes()):
            import_module(
                f"gideon.interfaces.dashboard.handlers.capabilities_{module_name}"
            ).register(app)
