...
from gideon.interfaces.dashboard.handlers.capabilities_communications_outbound import (
    register as register_outbound_email,
)
    ...
    try:
        ...
        if "/signal-archive/" in request.path:
            ...
            if request.path.endswith("/history"):
                return web.json_response(
                    {
                        "messages": signal_archive.history(
                            store, request.query.get("source_account_id", "")
                        )
                    }
                )
        ...
        if "/teams/" in request.path:
            ...
            if request.path.endswith("/messages"):
                return web.json_response(
                    {
                        "messages": teams.messages(store, source_id),
                        "source": teams.get_source(store, source_id),
                    }
                )
            ...
            if request.method == "GET":
                return web.json_response(
                    {"source": teams.get_source(store, source_id)}
                    if source_id
                    else {"sources": teams.sources(store)}
                )
        ...
        if "/platform-assignments" in request.path:
            ...
            if request.path.endswith("/history"):
                return web.json_response(
                    {"history": lifecycle.history(store, assignment_id)}
                )
            if request.method == "GET":
                return web.json_response(
                    {"assignment": lifecycle.get(store, assignment_id)}
                    if assignment_id
                    else {"assignments": lifecycle.records(store)}
                )
            data = await request.json()
            return web.json_response(
                {
                    "assignment": (
                        lifecycle.change(store, assignment_id, data)
                        if assignment_id
                        else lifecycle.create(store, data)
                    )
                }
            )
        if "/stacker/" in request.path:
            ...
            if request.method == "GET":
                return web.json_response(
                    {"actions": stacker.actions(store, account_id)}
                    if account_id
                    else {"territories": stacker.territories(store)}
                )
            data = await request.json()
            if action == "territories":
                return web.json_response(
                    {"territory": await stacker.read_territory(store, data)}
                )
            if action == "submit":
        ...
        if "/x/accounts/" in request.path:
            ...
            if request.method == "GET":
                return web.json_response(
                    {"snapshot": xreading.snapshot(store, account_id)}
                    if action == "snapshot"
                    else {"drafts": xreading.drafts(store, account_id)}
                )
            data = await request.json()
            if action == "sync":
                return web.json_response(
                    {"snapshot": await xreading.sync(store, account_id, data)}
                )
            row = (
                xreading.review(store, account_id, draft_id, data)
                if action == "review"
                else xreading.save_draft(store, account_id, data, draft_id)
            )
        ...
        if "/social/" in request.path:
            ...
            if request.method == "GET":
                return web.json_response(
                    {"account": social.get(store, account_id)}
                    if account_id
                    else {"accounts": social.accounts(store)}
                )
            data = await request.json()
            if request.method == "DELETE" or request.path.endswith("/remove"):
                social.fields(data, {"revision"})
                return web.json_response(
                    social.remove(store, account_id, data.get("revision"))
                )
            row, created = social.save(store, data, account_id)
            return web.json_response(
                {"account": row, "created": created}, status=201 if created else 200
            )
            ...
            if route == "daily":
                return web.json_response(
                    calendar.daily(
                        ...
                        or AppConfig.load().timezone
                        or "UTC",
                    )
                )
            if route == "upload":
                return web.json_response(
                    {"sync": calendar.upload(store, source_id, await request.json())}
                )
            if route == "sync":
                return web.json_response(
                    {
                        "sync": await calendar.sync_remote(
                            store, source_id, await request.json()
                        )
                    }
                )
            if request.method == "GET":
            ...
            return web.json_response(
                {
                    "source": calendar.save_source(
                        store, await request.json(), source_id
                    )
                },
                status=200 if source_id else 201,
            )
        if "/telegram/" in request.path:
            ...
            if route == "config":
                return web.json_response(
                    {
                        "config": (
                            telegram.config(store)
                            if request.method == "GET"
                            else telegram.configure(store, await request.json())
                        )
                    }
                )
            if route == "command":
            ...
            if route == "deliveries":
                ...
                return web.json_response(
                    {"delivery": row, "created": created},
                    status=201 if created else 200,
                )
            if route == "webhook":
                receipt, created = telegram.receive(
                    store,
                    await request.json(),
                ...
                if receipt["automatic_replies"]:
                    await asyncio.to_thread(
                        telegram.deliver, store, receipt["delivery_id"]
                    )
            ...
            return web.json_response(
                {
                    "delivery": await asyncio.to_thread(
                        telegram.deliver, store, request.match_info["delivery_id"]
                    )
                }
            )
        if "/beeper/" in request.path:
            ...
            if route == "settings":
                return web.json_response(
                    {
                        "settings": (
                            beeper.settings(store)
                            if request.method == "GET"
                            else beeper.configure(store, await request.json())
                        )
                    }
                )
            if route == "disconnect":
                return web.json_response(
                    {"settings": beeper.disconnect(store, await request.json())}
                )
            if route in ("chats", "messages"):
                return web.json_response(
                    beeper.stored_page(
                        store,
                        request.query.get("chat_id") if route == "messages" else None,
                    )
                )
            if route == "refresh":
                data = await request.json()
                ...
                return web.json_response(
                    await beeper.refresh(store, data.get("chat_id"), data.get("cursor"))
                )
            if route == "assets":
                if request.method == "GET":
                    return web.json_response(
                        beeper.asset(store, request.query.get("id", ""))
                    )
                data = await request.json()
                beeper.fields(data, {"chat_id", "asset_id"})
                return web.json_response(
                    await beeper.fetch_asset(
                        store, data.get("chat_id", ""), data.get("asset_id", "")
                    )
                )
            if route == "outbox":
                if request.method == "GET":
                ...
                return web.json_response(
                    {"item": row, "created": created}, status=201 if created else 200
                )
            data = await request.json()
            item_id = request.match_info["outbox_id"]
            if route == "send":
                return web.json_response(
                    {"item": await beeper.send(store, item_id, data)}
                )
            beeper.fields(data, {"revision"})
            operation = getattr(beeper, route)
            row = (
                await operation(store, item_id, data.get("revision"))
                if route == "reconcile"
                else operation(store, item_id, data.get("revision"))
            )
        ...
        if "/desktop/" in request.path:
                ...
                return web.json_response(
                    {
                        "exclusions": desktop.exclusions(
                            ...
                            request.query.get("source_account_id", ""),
                        )
                    }
                )
            if request.path.endswith("/history"):
            ...
            if request.path.endswith("/exclusions"):
                ...
                return web.json_response(
                    {"exclusion": exclusion, "removed": removed, "created": created},
                    status=201 if created else 200,
                )
            if request.path.endswith("/preview"):
        ...
        if "/mirror/" in request.path:
            ...
            if request.path.endswith("/messages"):
                return web.json_response(
                    {
                        "messages": mirrors.messages(store, account_id),
                        "account": mirrors.get_account(store, account_id),
                    }
                )
            if request.path.endswith("/sync"):
                mirrors.fields(await request.json(), set())
                return web.json_response(
                    {"sync": await asyncio.to_thread(mirrors.sync, store, account_id)}
                )
            if request.path.endswith("/upload"):
                return web.json_response(
                    mirrors.upload(store, account_id, await request.json())
                )
            if request.method == "GET":
                return web.json_response(
                    {"account": mirrors.get_account(store, account_id)}
                    if account_id
                    else {"accounts": mirrors.accounts(store)}
                )
            return web.json_response(
                {
                    "account": mirrors.save_account(
                        store, await request.json(), account_id
                    )
                },
                status=200 if account_id else 201,
            )
        ...
        if request.path.endswith("/touchpoints"):
            ...
            return web.json_response(
                {"touchpoint": point, "created": created},
                status=201 if created else 200,
            )
