    ...
    try:
        ...
        if request.method == 'GET':
            if person_id:
                ...
                return web.json_response({'person': person, 'touchpoints': points, 'care': care(person, points, zone), 'timezone': zone})
            people = [{**p, 'care': care(p, store.touchpoints(p['id']), zone)} for p in store.people()]
