    ...
    async def handle(request):
        try:
            ...
            if request.method in ("POST", "PUT", "DELETE"):
                body = {"request_id": request.query.get("request_id"), "revision": int(request.query.get("revision", "0"))} if request.method == "DELETE" and request.query else await read_json_body(request)
                operation = {("POST", False): store.create_entry, ("PUT", False): store.correct_entry, ("DELETE", False): store.delete_entry, ("POST", True): store.create_preset, ("PUT", True): store.update_preset, ("DELETE", True): store.delete_preset}[(request.method, preset)]
                ...
                result = await asyncio.to_thread(store.summary, timezone=request.query.get("timezone", "UTC"), days=int(request.query.get("days", "30")), as_of=request.query.get("as_of"))
            elif request.path.endswith("/history"):
                result = {"history": await asyncio.to_thread(store.history_entry, identity)}
            elif identity:
                result = await asyncio.to_thread(store.get_entry, identity)
            elif preset:
                result = {"presets": await asyncio.to_thread(store.list_presets, request.query.get("kind"))}
            else:
                result = {"entries": await asyncio.to_thread(store.list_entries, kind=request.query.get("kind"), from_date=request.query.get("from"), to_date=request.query.get("to"), limit=int(request.query.get("limit", "100")), offset=int(request.query.get("offset", "0")))}
