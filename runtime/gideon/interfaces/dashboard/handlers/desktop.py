...
:mod:`gideon.interfaces.dashboard.desktop_registry`.
...
def _require_loopback(request: web.Request, operation: str) -> web.Response | None:
    ...
    if not _h.is_loopback(request.remote or ""):
        return _deny(
            request, operation=operation, reason="non-loopback", message="loopback only"
        )
    return None
...
async def api_desktop_state_push(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"ok": True, "capabilities": reg.snapshot()["capabilities"]}
    )
