...
async def api_session_template_update(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {"ok": True, "template": session_templates.get_template(tid)}
    )
...
def _read_transcript(
    state: ConsoleState, name: str, *, what: str
    ...
    if log is None:
        return (
            ...
            {},
            [],
            web.json_response({"error": "history is unavailable"}, status=503),
        )
    ...
    try:
        ...
        return (
            ...
            [],
            web.json_response(
                {"error": "could not read this conversation"}, status=500
            ),
        )
...
async def api_session_share(request: web.Request) -> web.Response:
    ...
    session = state._sessions.get(name)
    memory_mode = str(
        meta.get("memory_mode") or getattr(session, "memory_mode", "") or ""
    )
    ...
    try:
        ...
        return web.json_response(
            {"error": "could not create the shared artifact"}, status=500
        )
...
def register_routes(app: web.Application) -> None:
    ...
    app.router.add_put(
        "/api/chat/sessions/templates/{template}", api_session_template_update
    )
    app.router.add_delete(
        "/api/chat/sessions/templates/{template}", api_session_template_delete
    )
