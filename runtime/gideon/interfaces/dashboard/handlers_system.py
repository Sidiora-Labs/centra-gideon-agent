...
def _safe_surfaces_flag() -> bool:
    ...
    from gideon.workspace.surface_layers import safe_surfaces
...
def _memory_totals() -> tuple[float, float] | None:
    ...
    System memory is owned by :mod:`gideon.integrations.local_models.residency` — the same reader
    ...
    if total_mb <= 0 or snapshot.get("source") == "unavailable":
        return None
    return round(total_mb / 1024, 1), round(
        int(snapshot.get("available_mb") or 0) / 1024, 1
    )
...
def _get_owner_hash(state: ConsoleState) -> str:
...
def _collect_gpu_metrics() -> dict[str, object]:
    ...
    (:func:`gideon.integrations.local_models.fit._probe_gpu`, cached there) rather than detected a
    ...
    Output schema (all optional):
        ...
        vram_total_gb:   float
    """
    from gideon.integrations.local_models.fit import _probe_gpu
...
def _collect_system_metrics() -> dict[str, object]:
    ...
    try:
        ...
        if sys.platform == "darwin":
            ...
            child_pids = (
                [d.name for d in task_dir.iterdir()] if task_dir.exists() else []
            )
...
async def api_onboarding(request: web.Request) -> web.Response:
    ...
    see :mod:`gideon.cognition.onboarding`), which is what lets a mid-flow reload resume.
    ...
    from gideon.cognition.onboarding import load_onboarding_state
