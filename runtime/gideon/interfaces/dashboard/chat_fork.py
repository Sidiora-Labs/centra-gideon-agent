...
async def api_chat_session_fork(request: web.Request) -> web.Response:
    ...
    if request_app:
        if not session._app:
            ...
            return web.json_response(
                {"error": "app cannot fork unscoped sessions"}, status=403
            )
    ...
    if session.memory_mode != "persistent":
        ...
        return web.json_response(
            {"error": "cannot fork a non-persistent session"}, status=400
        )
...
async def api_chat_session_fork_rewound(request: web.Request) -> web.Response:
    ...
    if isinstance(index, bool) or not isinstance(index, int) or index < 0:
        return web.json_response(
            {"error": "index must be a non-negative integer"}, status=400
        )
    ...
    if snapshot_index is not None and (
        ...
        return web.json_response(
            {"error": "snapshot_index must be an integer"}, status=400
        )
    ...
    async with session._fork_lock:
        ...
        if not isinstance(chain, list) or not chain:
            return web.json_response(
                {"error": "no retained tail at this turn"}, status=400
            )
