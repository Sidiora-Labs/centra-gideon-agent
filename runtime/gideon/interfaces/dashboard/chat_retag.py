...
LLM calls go through :func:`gideon.integrations.llm_helpers.one_shot_completion`
...
def _get_job(state: ConsoleState) -> RetagJob | None:
    return getattr(state, "_retag_job", None)


def _get_task(state: ConsoleState) -> "asyncio.Task | None":
    return getattr(state, "_retag_task", None)


def _collect_candidates(state: ConsoleState) -> list[_Candidate]:
    ...
    for s in state._sessions.values():
        ...
        if getattr(s, "memory_mode", "persistent") != "persistent":
            continue
        out.append(
            _Candidate(key=s.key, history_key="", in_memory=True, tags=list(s.tags))
        )
    if state.conversation_log:
        ...
        for d in disk:
            ...
            out.append(
                _Candidate(key=name, history_key=raw_key, in_memory=False, tags=tags)
            )
    return out


def _transcript_for(state: ConsoleState, cand: _Candidate) -> str:
...
def _build_retag_prompt(state: ConsoleState, cand: _Candidate, transcript: str) -> str:
    ...
    current = (
        ", ".join(tag_index[t]["name"] for t in cand.tags if t in tag_index) or "(none)"
    )
    return (
...
def _resolve_tag_ids(state: ConsoleState, names: list[str]) -> list[str]:
...
def _apply_tags(state: ConsoleState, cand: _Candidate, new_ids: list[str]) -> bool:
    ...
    if cand.in_memory:
        ...
        state.conversation_log.update_metadata(
            cand.history_key, {"tags": list(new_ids)}
        )
    return True


async def _retag_one(
    state: ConsoleState, job: RetagJob, cand: _Candidate, sem: asyncio.Semaphore
) -> None:
...
async def _drive(state: ConsoleState, job: RetagJob) -> None:
