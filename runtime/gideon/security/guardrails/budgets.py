...
class SpendMeter:
    ...
    def charge(
        self, tokens: int, dollars: float, *, run_key: str | None = None
    ) -> None:
    ...
    def day_totals(self) -> _ScopeTotal:
        ...
        return _ScopeTotal(
            tokens=int(row.get("tokens", 0)), dollars=float(row.get("dollars", 0.0))
        )
    ...
    def run_totals(self, run_key: str) -> _ScopeTotal:
        with self._lock:
            ...
            return (
                _ScopeTotal(tokens=rt.tokens, dollars=rt.dollars)
                if rt
                else _ScopeTotal()
            )
