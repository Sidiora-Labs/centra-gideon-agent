...
def _glob_match(path: str, pattern: str) -> bool:
    ...
    One matcher, one behaviour: :func:`gideon.security.guardrails.registries.path_glob`
...
def _load_config_rules() -> tuple[list[DenyRule], list[str]]:
    ...
    try:
        ...
        logger.debug(
            "denylist config read failed (fail-open to built-ins)", exc_info=True
        )
        return [], []
...
def check_action(
            ...
            for p in paths:
                if _glob_match(p, pattern):
                    return DenyDecision(
                        ...
                        verdict=(
                            rule.verdict
                            if rule.verdict in ("block", "needs_human")
                            else "block"
                        ),
    ...
    for cmd in commands:
        from gideon.security.guardrails.self_destruct import unattended_host_effect
...
def enforce_action(
    ...
    logger.warning(
        "action denied (%s) for provider %r: %s",
        decision.verdict,
        provider_name,
        decision.reason,
    )
