...
def record_project_trust(cwd: str, trusted: bool) -> dict[str, Any]:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 - audit is best-effort; the decision still persisted
    ...
    try:
        ...
        if state is None:
            try:
                ...
                Exception
            ):  # noqa: BLE001 - headless: the row is best-effort, decision persisted
                state = None
        emit_attention_item(
            state,
        ...
        Exception
    ):  # noqa: BLE001 - the prompt is best-effort; the safe default already applied
