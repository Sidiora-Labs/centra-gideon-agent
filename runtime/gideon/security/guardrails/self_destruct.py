...
:meth:`RuntimeCoordinator._fire_store_trigger` → :func:`gideon.security.guardrails.denylist.
...
process hosting the run. The runner dies mid-flight, so the `ExecutionJournal` row never reaches
...
`.*gideon restart.*`,
`.*gideon gateway restart.*` and `.*\\b(kill|pkill|killall)\\b.*\\bgideon\\b.*`
...
:func:`gideon.security.guardrails.policy.is_unattended_session` — not a second notion of the word,
...
def _norm(word: str) -> str:
    ...
    So `gideon`, `gideon`, `Gideon`, `io.gideon.engine.gateway` and
    `gideon.operations.service` all reduce to a form containing `gideon`. Identity by
...
def _self_identities() -> tuple[str, ...]:
    ...
    or the systemd unit cannot silently narrow the guard. Imported lazily: `gideon.operations.service`
    pulls in the platform controllers, and this module is imported on a dispatch hot path.
    """
    names = ["gideon"]
    try:
        from gideon.operations.service.common import LAUNCHD_LABEL, SERVICE_NAME

        names += [SERVICE_NAME, LAUNCHD_LABEL]
    except (
        Exception
    ):  # pragma: no cover - constants are packaged; the floor still holds
        pass
...
def _drop_wrapper_operands(name: str, args: list[str]) -> list[str]:
    ...
    while rest:
        ...
        if head.startswith("-"):
            rest.pop(0)
            if (
                head in ("-u", "-U", "-g", "-n", "-p", "-k", "-s", "--user", "--group")
                and rest
            ):
                rest.pop(0)
            continue
...
def _classify_manager(
    program: str, args: list[str], evidence: str
) -> HostEffect | None:
    ...
    if any(_is_self_name(t) for t in targets):
        return HostEffect(
            kind, "self", f"`{program} {verb}` on this gateway's service", evidence
        )
...
def _classify_killer(program: str, args: list[str], evidence: str) -> HostEffect | None:
    ...
    if any(_is_self_name(w) for w in words):
        return HostEffect(
            _KIND_STOP, "self", f"`{program}` targeting this gateway", evidence
        )
    ...
    for word in words:
        if word.lstrip("-").isdigit() and abs(int(word)) in ours:
            return HostEffect(
                _KIND_STOP, "self", f"`{program} {word}` (this process)", evidence
            )
...
def _classify_statement(tokens: list[str], depth: int) -> HostEffect | None:
    ...
    for _ in range(6):
        ...
        if head in _SHELLS:
            ...
            if inline is None:
                return (
                    None  # an interactive/stdin shell: unclassifiable by construction
                )
        ...
        if head in _INTERPRETERS or head in _RUNNERS:
            ...
            if any(_UNRESOLVED in w for w in args[1:]):
                return HostEffect(
                    _KIND_UNKNOWN,
                    "unknown",
                    f"`{head}` running an unresolved target",
                    evidence,
                )
            return None
        break
    ...
    if _UNRESOLVED in args[0]:
        if any(w in _LIFECYCLE_WORDS for w in rest) or any(
            _is_self_name(w) for w in rest
        ):
            return HostEffect(
                _KIND_UNKNOWN,
                "unknown",
                "an unresolved program next to a lifecycle verb",
                evidence,
            )
        return None
...
def classify_host_effect(command: str, *, _depth: int = 0) -> HostEffect:
    ...
    if parse_failed:
        ...
        if any(
            ...
            for w in _LIFECYCLE_WORDS
        ) or (_is_self_name(low)):
            return HostEffect(
                _KIND_UNKNOWN,
                "unknown",
...
def unattended_host_effect(command: str, session_key: str = "") -> HostEffect | None:
    ...
    Unattendedness is :func:`gideon.security.guardrails.policy.is_unattended_session` and nothing
