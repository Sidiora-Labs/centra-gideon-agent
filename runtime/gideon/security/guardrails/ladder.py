...
:mod:`~gideon.security.guardrails.autonomy` decides (ladder, declarations, grants, derived
eligibility) and :mod:`~gideon.security.guardrails.rungs` routes a dispatch seam. Both are
...
:func:`~gideon.security.guardrails.autonomy.promotion_eligibility`'s read-path caller.
...
:func:`~gideon.security.guardrails.autonomy.grant_rung`.
...
* The handle is the PROVIDER's (``ActionResult.reversal``) and stays opaque here. Only the
  ...
  :meth:`~gideon.integrations.action_providers.base.ActionProvider.reverse` on the provider the
...
def _load_records() -> list[ReversalRecord]:
    ...
    try:
        ...
        logger.warning(
            "autonomy_reversals.json unreadable — no undo is offered", exc_info=True
        )
        return []
    rows = data.get("records") if isinstance(data, dict) else None
    if not isinstance(rows, list):
        logger.warning(
            "autonomy_reversals.json has no record list — no undo is offered"
        )
        return []
...
def record_reversal_handle(
    *, action_type: str, rung: str, handle: str, label: str = ""
) -> str:
    """Persist one reversible execution and return its record id (``""`` when refused).

    Called from :func:`~gideon.security.guardrails.rungs.record_reversal` — the seam path an
...
def _refuse(
    code: str, reason: str, *, record_id: str, action_type: str = ""
) -> ReversalOutcome:
...
def _reverser_for(action_type: str, kind: str):
    ...
    from gideon.security.guardrails.autonomy import action_type as _spec
...
async def reverse_action(record_id: str) -> ReversalOutcome:
    ...
    try:
        ...
        Exception
    ) as exc:  # noqa: BLE001 — a raising provider is a refusal, not a 500
        return _refuse(
            "provider_failed",
    ...
    record_demotion: Demotion = demote(
        record.action_type, "you undid an action it took on its own"
    )
    _audit(
        "autonomy_reversed",
        outcome="ok",
...
def _authority_sentence(spec, resolved: str, granted: str, state, held: bool) -> str:
    ...
    """
    from gideon.security.guardrails.rungs import rung_label as _label
    ...
    if (
        ...
        and rung_rank(granted) > rung_rank(spec.floor)
    ):
...
def explain_refused_grant(key: str, rung: str) -> str:
    """Why a grant that :func:`~gideon.security.guardrails.autonomy.grant_rung` ALREADY
    refused was refused.
    ...
    """
    from gideon.security.guardrails.autonomy import action_type as _spec
    from gideon.security.guardrails.autonomy import cooldown_date
    ...
    cooldown = max(
        (d.cooldown_until for d in (state.demotions if state else ())), default=""
    )
    if cooldown:
...
def propose_promotions(note_for: Callable[[str], str] | None = None) -> list[str]:
    ...
    Deduped per (type, next rung) by :func:`~gideon.integrations.inbox.emit_attention_item`, so a
    ...
    for spec in registered_action_types():
        try:
            ...
            logger.warning(
                "autonomy: eligibility scan failed for %s", spec.key, exc_info=True
            )
            continue
...
def _file_proposal(key: str, next_rung: str, record: str) -> bool:
    ...
    try:
        ...
        logger.warning(
            "autonomy: could not file a promotion proposal for %s", key, exc_info=True
        )
        return False
...
def revoke_granted_scopes(*, cause: str, evidence_id: str, source: str) -> list[str]:
    ...
    Each demotion goes through :func:`~gideon.security.guardrails.autonomy.demote`, which
    ...
    for spec in registered_action_types():
        ...
        try:
            ...
            logger.warning(
                "autonomy: revocation failed for %s", spec.key, exc_info=True
            )
            continue
        revoked.append(spec.key)
    if not revoked:
        return []
    _file_revocation_notice(
        revoked, cause=cause, evidence_id=evidence_id, source=source
    )
    return revoked
...
def revoke_scope(key: str, *, cause: str, evidence_id: str, source: str) -> bool:
    ...
    sweep), demoted through :func:`~gideon.security.guardrails.autonomy.demote` (floor +
...
def _file_revocation_notice(
    keys: list[str], *, cause: str, evidence_id: str, source: str
) -> None:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 — the demotions already happened; the notice is best-effort
