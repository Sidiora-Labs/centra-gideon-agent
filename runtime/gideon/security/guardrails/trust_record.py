...
inputs :func:`gideon.security.guardrails.autonomy.resolve_rung` already consumes (grant
...
- **One rung dialect.** This module mints NO rung vocabulary. Every rung name it
  stores or validates comes from :data:`gideon.security.guardrails.autonomy.RUNGS`; a
...
def load_record(key: str) -> TrustRecord | None:
    ...
    try:
        ...
        logger.warning(
            "trust record unreadable for %s — treated as absent", key, exc_info=True
        )
        return None
    ...
    try:
        ...
        logger.warning(
            "trust record for %s has malformed fields — treated as absent", key
        )
        return None
