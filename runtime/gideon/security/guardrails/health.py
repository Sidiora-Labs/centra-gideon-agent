...
from gideon.security.guardrails.audit import UNATTRIBUTED, read_recent
...
def _percentile(sorted_vals: list[float], pct: float) -> float:
    ...
    k = max(
        0,
        min(
            len(sorted_vals) - 1, int(round((pct / 100.0) * len(sorted_vals) + 0.5)) - 1
        ),
    )
...
def _caller_rollup(rows: list[dict]) -> list[dict]:
    ...
    for name, crows in by_caller.items():
        latencies = sorted(
            float(r.get("latency_ms", 0) or 0) for r in crows if r.get("passed")
        )
        ...
        out.append(
            {
                ...
                "failure_modes": modes,
                "dollars_est": round(
                    sum(float(r.get("dollars_est", 0) or 0) for r in crows), 4
                ),
            }
        )
...
def provider_health(limit: int = 2000) -> dict:
    ...
    for name in sorted(names):
        ...
        latencies = sorted(
            float(r.get("latency_ms", 0) or 0) for r in prov_rows if r.get("passed")
        )
