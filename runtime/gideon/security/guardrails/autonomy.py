...
does not override :func:`~gideon.security.guardrails.denylist.check_action`, a budget
pause, or :func:`~gideon.security.guardrails.incident.incident_active`.
...
from gideon.security.guardrails import trust_record
...
def register_action_type(spec: ActionTypeSpec) -> None:
    ...
    for name in spec.providers:
        ...
        if held is not None and held != spec.key:
            logger.warning(
                "action provider %r was governed by %s — %s now claims it",
                name,
                held,
                spec.key,
            )
...
def _parse_grant(key: str, raw: object) -> RungGrant | None:
    ...
    if rung_rank(rung) < 0:
        logger.warning(
            "autonomy_rungs.json: %s has unknown rung %r — ignored", key, rung
        )
        return None
...
def _load_store() -> dict[str, RungGrant]:
    ...
    try:
        ...
        logger.warning(
            "autonomy_rungs.json unreadable — no rung grants apply", exc_info=True
        )
        return {}
...
def _config_rule() -> PromotionRule:
    ...
    try:
        ...
        logger.debug(
            "autonomy config read failed (using default thresholds)", exc_info=True
        )
...
def _in_cooldown(grant: RungGrant | None, now: datetime) -> bool:
    ...
    if parsed is None:
        logger.warning(
            "autonomy cooldown timestamp %r unparseable — treated as active", until
        )
        return True
    return now < parsed
...
def approval_outcome_series(key: str, *, window_days: int) -> list[tuple[float, bool]]:
    ...
    return sorted(
        (when.timestamp(), good) for when, good in _outcome_events(key, cutoff)
    )
...
def _sel_evidence(key: str, cutoff: datetime) -> tuple[int, int, float]:
    ...
    span = (
        (max(stamps) - min(stamps)).total_seconds() / 86_400.0
        if len(stamps) > 1
        else 0.0
    )
...
def _feedback_rejections(key: str, window_days: int) -> int:
    ...
    """
    from gideon.cognition.feedback import producer_stats
...
def grant_rung(key: str, rung: str, *, evidence_window: str = "") -> str | None:
    ...
    if spec is None:
        logger.warning(
            "autonomy grant refused: %s is not a registered action type", key
        )
        return None
    if rung_rank(rung) < 0:
    ...
    if rung_rank(rung) <= rung_rank(granted_rung(key)):
        logger.info(
            "autonomy grant is a no-op: %s already resolves at or above %s", key, rung
        )
        return None
    store[key] = RungGrant(
        key=key,
    ...
    _audit(
        "autonomy_granted", key=key, detail=f"rung={rung} evidence={evidence_window}"
    )
...
def demote(key: str, cause: str) -> Demotion:
    ...
    store[key] = RungGrant(
        ...
        granted_at="",
        evidence_window="",
        demotions=((existing.demotions if existing else ()) + (record,))[
            -_MAX_DEMOTIONS:
        ],
    )
    _save_store(store)
    ...
    _audit(
        "autonomy_demoted",
        key=key,
        detail=f"cause={record.cause} until={record.cooldown_until}",
    )
    logger.warning(
        "autonomy: %s demoted to %s (%s)", key, floor, record.cause or "no cause given"
    )
    return record
