    ...
    *,
    ...
    if method.upper() not in ("GET", "HEAD"):
        ...
        if live_writes_disabled():
            ...
            if not is_loopback:
                from gideon.security.guardrails.writes import LiveWriteDisabled
                ...
                if resp.status in (301, 302, 303, 307, 308) and resp.headers.get(
                    ...
                    if not hop_decision.allow:
                        _audit(
                            ...
                            policy,
                            outcome="denied",
                            reason=f"redirect: {hop_decision.reason}",
                        )
    ...
    blocked = GuardDecision(
        ...
        risk_level="caution",
        recovery_hints=[
            "The URL redirects too many times; fetch the final URL directly."
        ],
    )
