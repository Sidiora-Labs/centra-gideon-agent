...
def _print_status() -> int:
    ...
    if st["configured"]:
        print(
            f"  credential:  set for {st['username']!r} ({st['algo']}, {st['updated_at']})"
        )
    else:
    ...
    if cfg.get("login_enabled") and not st["configured"]:
        print()
        print(
            "⚠️  Login is enabled but no credential is set — password login cannot succeed."
        )
        print(
            "   The local token link still works. Set a password, or run `auth disable`."
        )
    if cfg.get("require_totp") and not st["totp_enabled"]:
        print()
        print(
            "⚠️  A 2FA code is required but no TOTP secret is enrolled — login would fail."
        )
...
def auth_cmd(args) -> int:
    ...
    if action == "set-password":
        user = str(getattr(args, "user", "") or "").strip() or os.environ.get(
            "USER", "owner"
        )
    ...
    if action == "disable":
        ...
        print(
            "   Use `gideon auth set-password` to change it, or delete it with --clear."
        )
        return 0
...
def _enroll_cmd(args) -> int:
    ...
    code, _expires_at = enrollment.issue_code(
    ...
    print()
    print(
        "Open the dashboard on the other device and enter this code. It is shown once,"
    )
...
def _totp_cmd(args) -> int:
    ...
    if sub == "disable":
        creds.disable_totp()
        print(
            "✅ 2FA turned off. The secret is kept, so re-enabling needs no re-enrollment."
        )
        return 0
    if sub != "setup":
    ...
    from gideon.security.auth.totp import new_secret, provisioning_uri
