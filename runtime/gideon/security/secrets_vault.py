...
* The only credential-store call in this module's import graph is
  :func:`~gideon.core.config.credentials.credential_names`, which reads key names on both
...
def list_presence(
    ...
    *consumers* is injected rather than derived here so the sync read model stays sync:
    ...
    for name in _host_secret_names():
        if (
            name in vault_names
            or split_project_key(name) is not None
            or is_reserved_key(name)
        ):
            continue
        rows.append(
            SecretPresence(
                name=name, scope=SCOPE_HOST, consumers=consumer_map.get(name, ())
            )
        )
...
def _spec_of(definition: Any) -> Any:
    ...
    if callable(to_dict):
        try:
            ...
            Exception
        ):  # noqa: BLE001 - a provider's serializer is not this module's problem
            logger.debug(
                "workflow def to_dict() failed while deriving consumers", exc_info=True
            )
    return definition
...
async def _workflow_references() -> list[tuple[str, str, list[str]]]:
    ...
    for provider_name in list_providers():
        ...
        try:
            ...
            Exception
        ):  # noqa: BLE001 - one bad provider must not blank the whole vault
            logger.debug(
                "workflow provider %r unreadable", provider_name, exc_info=True
            )
            continue
...
def _trigger_references() -> list[tuple[str, str, list[str]]]:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 - an unreadable trigger store costs links, not the listing
    ...
    for trigger in triggers or []:
        try:
            ...
            Exception
        ):  # noqa: BLE001 - one malformed trigger must not blank the others
            logger.debug(
                "trigger action unreadable while deriving consumers", exc_info=True
            )
            continue
        if not keys:
            continue
