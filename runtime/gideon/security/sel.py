...
def _default_dir() -> Path:
    ...
    if override:
        ...
        if p != Path("/") and p.parts[:2] not in (
            ("/", "usr"),
            ("/", "System"),
            ("/", "etc"),
        ):
...
_UNREDACTED_FIELDS = frozenset(
    {"event_id", "timestamp", "event_type", "prev_hash", "entry_hash"}
)
...
def redact_event(record: dict) -> dict:
    ...
    else, at any nesting depth, goes through :func:`gideon.security.security.redact`. The
    ...
    def _deep(obj: object) -> object:
        ...
        if isinstance(obj, dict):
            return {
                k: (v if k in _UNREDACTED_FIELDS else _deep(v)) for k, v in obj.items()
            }
...
def _audit_matches(data: dict, filters: dict[str, str], since: str, until: str) -> bool:
    ...
    for field_name, needle in filters.items():
        ...
        alternatives = [
            part.strip().lower() for part in needle.split(",") if part.strip()
        ]
        if not alternatives:
            continue
...
class SecurityEventLog:
    ...
    def verify_integrity(
        self, max_entries: int | None = _VERIFY_WINDOW
    ) -> tuple[int, int]:
    ...
    def _prune_plan(self, keep_days: int, max_entries: int) -> tuple[list[str], int]:
        ...
        cutoff_str = (
            datetime.now(tz=timezone.utc) - timedelta(days=keep_days)
        ).isoformat()
