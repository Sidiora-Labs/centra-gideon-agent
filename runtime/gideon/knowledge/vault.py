...
MGAV-6 already ships is imported from :mod:`gideon.memory_vault` rather than
reimplemented:
...
* **the page format and its wikilink projection** — :func:`~gideon.memory_vault.slug`,
  :func:`~gideon.memory_vault.compose_page`,
  :func:`~gideon.memory_vault.split_page`,
  :func:`~gideon.memory_vault.parse_frontmatter`,
  :data:`~gideon.memory_vault.GENERATED_MARKER` and
  :func:`~gideon.memory_vault.extract_edited_value`, so an Obsidian vault of memory
...
* **its verification** — the same ``(check, key, detail)`` flag shape, the same
  ...
  :data:`~gideon.memory_vault.WIKILINK_RE`, so a refusal is visible in the file the
  owner was just editing as well as in a health surface.

**The content hash already excludes the field it writes.**
:func:`~gideon.memory_vault.body_hash` covers the BODY only and ``source_hash`` lives
...
def render_item(
    ...
    *,
    ...
    :func:`~gideon.memory_vault.extract_edited_value` able to hand the owner's version
...
class KnowledgeVault:
    ...
    def _ensure_static(self) -> None:
        ...
        ONE query — :meth:`~gideon.knowledge.store.KnowledgeStore.vault_tag_membership`
