...
1. **COMMUNITY tier, always.** A catalog is untrusted third-party content, so
   ...
   :attr:`~gideon.supply_chain.TrustTier.COMMUNITY` — never derived from config,
...
2. **No install path of its own.** Like every marketplace this is a read-only SOURCE:
   ...
   :meth:`~gideon.skills.marketplace.SkillsRegistry.install_guarded`'s job, which
...
3. **Network reach is profile-bound.** Every byte arrives through
   :func:`gideon.net.fetch` under the CONNECTOR egress profile (layered with the
...
``limit`` :class:`~gideon.skills.marketplace.SkillEntry` rows, and only a chosen
...
def fetch_catalog_text(url: str) -> str:
    ...
    from gideon.net import CONNECTOR, egress_policy_for, fetch
...
class CatalogMarketplace(SkillsMarketplace):
    ...
    Constructed from a :class:`~gideon.config.loader.SkillCatalogConfig` row via
