...
* :func:`request_grant` — one task, one authorization, routed through the SHIPPED fail-closed
  :class:`~gideon.agents.native.approval.ApprovalGate`. No answer in :data:`GRANT_TIMEOUT`
...
* :func:`make_close_check` — closing that tab group is a HARD STOP, OBSERVED not requested. The
  ...
  bound to, so :func:`~gideon.browse.loop.run_browse_loop` parks within one step — the same
  per-step seam BA-5's kill switch uses. **Distinct from** :mod:`gideon.browse.killswitch`:
...
property" reasoning :mod:`gideon.browse.target` uses for the connector — and
...
def make_close_check(grant: BrowserGrant, *, status_reader=None):
    ...
    bound to, so :func:`~gideon.browse.loop.run_browse_loop` parks within one step. Reads the
    live connector through :func:`~gideon.browse.target.connector_status`; ``status_reader``
    injects one for tests.
    """
    from gideon.browse.target import connector_status
