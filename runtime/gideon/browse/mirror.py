...
* **The kill switch broadcast.** :func:`broadcast_kill` relays a kill-switch state change so the
  ...
  switch call it; the switch itself lives in :mod:`gideon.browse.killswitch`.
...
(:mod:`gideon.dashboard.handlers.browse_mirror`) import DOWN into this module instead, which
is the allowed direction.

Everything here is BEST-EFFORT toward the live state: the state is resolved through
:func:`gideon.inbox_providers.native_source.get_dashboard_state`, and when no gateway is up
...
def surface_auth_expired(url: str, *, state: Any = None) -> None:
    ...
    """
    from gideon.browse.handoff import site_slug
