...
The handoff is the *positive* half of the invariant :mod:`gideon.browse.credentials` enforces
...
``durability/inventory.py`` claims ``browse`` in :data:`~gideon.durability.inventory.IGNORED`
...
:class:`~gideon.workflows.needs_input.NeedsInputItem` and a sentence; it does NOT invent a
...
def request_login(
    ...
    *,
    ...
    Goes through :func:`gideon.workflows.needs_input.build_item` so the card is the SAME shape
    ...
    **Nothing in the returned payload can hold a credential.** ``evidence`` carries the screened URL
    ...
    """
    from gideon.browse.credentials import screen_url
    from gideon.workflows.needs_input import build_item
