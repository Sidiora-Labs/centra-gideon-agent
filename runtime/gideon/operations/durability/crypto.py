...
def decrypt_object(obj: SyncObject, master: bytes) -> SyncObject:
    ...
    aad = header + obj.key.encode("utf-8")
    try:
        plain = AESGCM(shard_key(master, obj.key)).decrypt(
            nonce, obj.data[_HEADER_BYTES:], aad
        )
...
class SyncCodec:
    ...
    def encrypt_for_push(
        self, objects: list[SyncObject]
    ) -> tuple[list[SyncObject], SkipReport]:
        ...
        if skipped:
            logger.error(
                "sync encryption: refused to push %d plaintext object(s)", len(skipped)
            )
        return out, skipped

    def decrypt_after_pull(
        self, objects: list[SyncObject]
    ) -> tuple[list[SyncObject], SkipReport]:
...
def codec_for(
    transport: SyncTransportProvider, *, setting: str = "auto"
) -> SyncCodec | None:
