...
def _assert_service_git_dir(gd: Path, *, home: Path | None = None) -> None:
    ...
    try:
        ...
        raise HistoryError(
            f"refusing to operate on a git dir outside {root_dir}: {gd}"
        ) from exc
...
def ensure_repo(root: HistoryRoot, *, home: Path | None = None) -> Path:
    ...
    if not (gd / "HEAD").is_file():
        ...
        if proc.returncode != 0:
            raise HistoryError(
                f"could not init history repo for {root.id}: {proc.stderr.strip()}"
            )
...
def has_head(root: HistoryRoot, *, home: Path | None = None) -> bool:
    ...
    return (
        _git(
            root, "rev-parse", "--verify", "-q", "HEAD", home=home, check=False
        ).returncode
        == 0
    )
...
def timeline(
    ...
    *,
    ...
    proc = _git(
        root, "log", f"--max-count={max(1, limit)}", f"--format={fmt}%x1f", home=home
    )
    entries: list[dict] = []
...
def _resolve(root: HistoryRoot, sha: str, *, home: Path | None = None) -> str:
    ...
    proc = _git(
        root, "rev-parse", "--verify", "-q", f"{sha}^{{commit}}", home=home, check=False
    )
...
def _normalize_paths(paths: Sequence[str | Path] | None) -> list[str]:
    ...
    for raw in paths:
        ...
        if (
            ...
            raise HistoryError(
                f"restore path must be repo-relative, not absolute: {text!r}"
            )
...
def _changed_paths(
    root: HistoryRoot, frm: str, to: str, *, home: Path | None = None
) -> set[str]:
    ...
    """
    out = _git(
        root, "diff", "--name-only", "-z", "--no-renames", frm, to, home=home
    ).stdout
...
def _require_changed(
    ...
    *,
    ...
    if unknown:
        raise HistoryError(
            f"{operation} of {to[:8]} in root {root.id} does not change: "
            + ", ".join(unknown)
        )
...
def _paths_in_tree(
    ...
    if not selected:
        return set()
    out = _git(
        root, "ls-tree", "-r", "-z", "--name-only", tree, "--", *selected, home=home
    ).stdout
    return {p for p in out.split("\0") if p}


def _revert_span(
    root: HistoryRoot, target: str, *, home: Path | None = None
) -> tuple[str, str]:
    ...
    """
    parents = _git(
        root, "rev-list", "--parents", "-n", "1", target, home=home
    ).stdout.split()
    if len(parents) < 2:
        empty = _git(
            root, "hash-object", "-t", "tree", "/dev/null", home=home
        ).stdout.strip()
        return target, empty
...
def preview_rollback(
    ...
    *,
    ...
    if not selected:
        dropped = _git(
            root, "rev-list", "--count", f"{target}..{head}", home=home
        ).stdout.strip()
    return {
        "operation": "rollback",
        "root": root.id,
...
def _rollback_paths(
    ...
    body = (
        ...
        + "".join(f"Restored-path: {p}\n" for p in selected)
    )
    _git(
        root, "commit", "--quiet", "-m", subject, "-m", body, "--", *selected, home=home
    )
    return _git(root, "rev-parse", "HEAD", home=home).stdout.strip()
...
def rollback(
    ...
    *,
    ...
    if selected:
        ...
        head = target
        logger.info(
            "time-travel: rolled %s back to %s (prior HEAD kept at %s)",
            root.id,
            target[:8],
            ref,
        )
...
def _revert_paths(
    ...
    if not combined.strip():
        raise HistoryError(
            f"revert of {target[:8]} in root {root.id} would change nothing"
        )
    applied = _git(
        root, "apply", "--index", "-", home=home, check=False, stdin=combined
    )
...
def commit_memory_roots(
    *, home: Path | None = None, reason: str = "hourly"
) -> list[dict]:
    ...
    with writing_surface(SURFACE_SCHEDULED):
        for root in roots(home):
            ...
            try:
                ...
                logger.warning(
                    "time-travel: hourly commit failed for %s: %s", root.id, exc
                )
                results.append({"root": root.id, "ok": False, "error": str(exc)})
                continue
            results.append(
                {"root": root.id, "ok": True, "sha": sha, "changed": bool(sha)}
            )
    return results
