...
from gideon.operations.durability.conflicts import (
    STATUS_NEEDS_REVIEW,
    ConflictQueue,
    ConflictRecord,
)
...
def pending(
    queue: ConflictQueue, *, limit: int = DEFAULT_LIMIT
) -> list[ConflictRecord]:
...
async def draft_proposals(
    home: Path, *, limit: int = DEFAULT_LIMIT, now: str = ""
) -> DraftReport:
    ...
    for rec in todo:
        prompt = _PROMPT.format(
            ...
            local=json.dumps(
                rec.local_row, indent=2, sort_keys=True, ensure_ascii=False
            ),
            remote=json.dumps(
                rec.remote_row, indent=2, sort_keys=True, ensure_ascii=False
            ),
        )
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 — fail-open: keep the conflict, lose the draft
            logger.warning(
                "conflict merge: no proposal for %s/%s (%s)",
                rec.entry_id,
                rec.entity_id,
                exc,
            )
