...
(:func:`gideon.event_triggers.emit_event`). Core learns no vendor's schema: the app
...
into ``app:<app-name>:<event>`` at ingestion (:func:`gideon.trigger_sources.namespace`).
...
**Fencing happens at INGESTION, at origin.** :func:`gideon.trigger_sources.emit` fences the
...
class TriggerSourceProvider(ABC):
    ...
    def events(self) -> tuple[str, ...]:
        ...
        and is reported by :func:`gideon.trigger_sources.undeclared_events` — the same
