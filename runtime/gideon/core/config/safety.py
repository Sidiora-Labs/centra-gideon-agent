...
@dataclass
class EgressConfig:
    """Operator overrides for the outbound egress guard (``gideon.security.net``).
...
class SecurityConfig:
    ...
    :mod:`gideon.security.security` (always enforced, read-only). ``denied_commands``
...
class SandboxConfig:
    ...
    (``gideon.engine._spawn_exec_shim``): a soft NOFILE cap, a max-child-process count
