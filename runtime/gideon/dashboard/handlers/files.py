...
# distinct from UPLOAD limits, which are per-filetype via gideon.uploads.
...
#
# The subtree list is sourced from gideon.security (the single source of truth
...
from gideon.security import _SYSTEM_SUBTREES as _SYSTEM_ROOTS  # noqa: E402
...
async def api_channel_upload_file(request: web.Request) -> web.Response:
    ...
    from gideon.config.loader import outbox_dir, workspace_root  # noqa: F811
    ...
    if not (resolved.is_relative_to(allowed_outbox) or resolved.is_relative_to(allowed_workspace)):
        ...
        return web.json_response({"error": "file_path must be under ~/.gideon/"}, status=403)
    try:
        raw = safe_read_file_bytes(str(resolved))
    except FileTooLargeError as e:
        _sel().log_tool_invocation(
...
def _upload_check(filename: str, mime: str | None = None, *, size: int | None = None):
    ...
    Returns an :class:`gideon.uploads.UploadCheck` — ``ok`` accepts, else
...
def _dashboard_roots() -> list[tuple[str, str]]:
    ...
    from gideon.config.loader import config_dir, outbox_dir
    ...
    # Uploads + Gideon home roots must follow the ACTIVE home (config_dir()),
    # NOT a hardcoded ~/.gideon: a gateway on a custom GIDEON_HOME (every
    # dev instance) would otherwise browse AND edit the developer's REAL home via the
    # write allowlist (#294). config_dir() re-reads GIDEON_HOME live, and both
    ...
    # Gideon home root — quick access to the whole config/data tree.
    ...
    try:
        from gideon.projects import _store as _project_store
...
def _validate_dashboard_path(raw: str) -> str | None:
    ...
    Two-layer check:
      1. Reject sensitive credential paths via ``gideon.hooks.validate_file_path``
         (e.g. ``~/.ssh``, ``~/.aws``).
...
async def api_config_fs_watch(request: web.Request) -> web.StreamResponse:
    ...
    from gideon.fs_watch import FS_WATCH_FEED
...
async def api_file_read(request: web.Request) -> web.Response:
    ...
    from gideon.validation import (  # noqa: F811
        FILE_READ_SCHEMA,
        ValidationError,
        validate_tool_args,
    )
...
async def api_file_search(request: web.Request) -> web.Response:
    ...
    from gideon.security import is_sensitive_path  # noqa: F811
    ...
    if not search_roots:
        ...
        # GIDEON_WORKSPACE override) plus an explicit project dir.
        proj = os.environ.get("GIDEON_PROJECT_DIR", "")
    ...
    # Dot-prefixed dirs (.gideon, .local, .config) excluded by startswith(".") guard below.
    ...
    from gideon.dashboard.file_index import is_pkg_cache_dir  # noqa: F811
