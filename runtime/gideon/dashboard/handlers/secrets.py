...
:mod:`gideon.secrets_vault` builds the whole read model from key NAMES
...
because that object is a :class:`~gideon.secrets_vault.SecretPresence` and the type has no
...
from gideon.secrets_vault import (
    SCOPE_HOST,
    SecretPresence,
    consumers_for,
    list_presence,
...
async def api_secrets_delete(request: web.Request) -> web.Response:
    ...
    from gideon.config.credentials import delete_credential
