...
:mod:`~gideon.computer_use.service` COMPOSES them into the one dispatch (`DCU-4`) and is
the only dispatchable entry point in the package; :mod:`~gideon.computer_use.tools`
declares the seven-tool surface and is the thin stdio shim that forwards a call to that dispatch,
holding no authority and no OS handle; and :mod:`~gideon.computer_use.driver_host` is the
