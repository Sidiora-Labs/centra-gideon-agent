...
**Why this is not the refusal :mod:`~gideon.computer_use.driver_host` already had.** When
...
**One refusal, two platform modules.** :mod:`~gideon.computer_use.windows_driver` and
:mod:`~gideon.computer_use.linux_driver` exist because ``DRIVER_MODULES`` resolves a driver
...
from gideon.computer_use.types import DriverError
...
def refusal(platform: str, accessibility_api: str, op: str) -> dict[str, Any]:
    ...
    A :class:`~gideon.computer_use.types.DriverError` rather than a dict literal built
