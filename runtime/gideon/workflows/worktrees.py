...
#: Per-run branch prefix for durable persistence. Distinct from `loop/worktree.py`'s `gideon/task-`
...
#: engine's own `.gideon-setup/` markers showed up in the changed-files panel as though the run had
...
def is_infrastructure(path: str, preserved: list[str] | None = None) -> bool:
    ...
    `.gideon-setup/` with a trailing slash, so a prefix check written against the bare name matched
