...
The unit lives at ``/etc/systemd/system/gideon.service`` and is
...
from gideon.service.common import SERVICE_NAME, gideon_bin, service_path
...
def render_unit() -> str:
    ...
    has access to ``$HOME/.gideon`` and the user's other config. The
...
def _write_unit_via_sudo(contents: str) -> subprocess.CompletedProcess[str]:
    ...
    ``daemon-reload``.
    """
    fd, tmp_path = tempfile.mkstemp(prefix="gideon-unit-", suffix=".service")
    try:
...
def install() -> None:
    ...
    No gideon / LLM / agent code runs under sudo: only ``tee`` and
