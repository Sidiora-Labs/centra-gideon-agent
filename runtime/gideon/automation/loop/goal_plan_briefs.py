...
from gideon.cognition.planning.session import PlanStep
...
def default_steps() -> list[dict]:
    ...
    return [
        {"kind": k, "title": titles[k], "objective": desc}
        for k, desc in GOAL_STEP_GUIDE
    ]
...
def build_step_brief(
    goal: str, step: PlanStep, *, approved: list[PlanStep] | None = None
) -> str:
...
def parse_artifact_sentinel(text: str) -> dict | None:
    ...
    from gideon.automation.loop.code_classify import (
        _parse_obj,  # shared JSON-from-prose helper
    )
