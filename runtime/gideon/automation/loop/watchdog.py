...
read from the loop's DECLARED :class:`~gideon.automation.workflows.supervisor_policy.\
SupervisorPolicy` by the one evaluator in :mod:`gideon.automation.loop.supervisor` (a
...
from gideon.automation.loop import instrument, kinds, manager, store, supervisor
from gideon.automation.loop.loop import Loop, LoopStatus, LoopStopReason
...
def _cycle_calls(finding: dict) -> list[str]:
    ...
    """
    from gideon.automation.workflows.loop_middleware import call_fingerprint
...
def _repeats_identically(values: list[Any], window: int, label: str) -> str:
    ...
    from gideon.automation.workflows.resilience import BreakerState, check_breaker
...
def check_stagnation(findings: list[dict], *, window: int | None = None) -> str:
    ...
    if detail := _repeats_identically(
        [_finding_content(f) for f in recent], w, "content"
    ):
...
class LoopWatchdog:
    ...
    async def _complete(
        ...
        *,
        ...
        store.update_status(
            loop_id, LoopStatus.COMPLETE, stop_reason=stop_reason, **fields
        )
        ...
        if genuine:
            try:
                ...
                logger.debug(
                    "reproduce_confirm failed for %s — shipping anyway",
                    loop_id,
                    exc_info=True,
                )
        if ship_ok:
        ...
        try:
            ...
            logger.debug(
                "scratch auto-teardown check failed for %s", loop_id, exc_info=True
            )
        ...
        self._publish(
            loop_id,
            "complete",
            {"loop_id": loop_id, "reason": reason, "genuine": genuine},
        )
    ...
    def _schedule_loop_end_ladder(self, loop_id: str) -> None:
        ...
        try:
            ...
            session = SimpleNamespace(
                key=loop.session_key, is_restricted=False, _ephemeral=False
            )
            ...
            if not decision.allowed or not getattr(cfg, "skill_ladder", True):
                logger.debug(
                    "loop %s: loop-end ladder gated (%s)",
                    loop_id,
            ...
            if tasks is None:
                coro.close()
                logger.debug(
                    "loop %s: loop-end ladder has nowhere to schedule", loop_id
                )
                return
            ...
            logger.debug(
                "loop %s: loop-end ladder scheduling failed", loop_id, exc_info=True
            )
    ...
    async def _run_loop_end_ladder(
        ...
        try:
            ...
            logger.debug(
                "loop %s: loop-end ladder review failed", loop_id, exc_info=True
            )
            return None
    ...
    def _capture_loop_end(self, loop_id: str) -> None:
        ...
        try:
            ...
            from gideon.cognition.learning import loop_end
            from gideon.cognition.learning.gate import Cadence, LearningGate
            ...
            if not decision.allowed:
                logger.debug(
                    "loop %s: loop-end capture gated (%s)",
                    loop_id,
                    decision.reason.value,
                )
                return
    ...
    def _register_deliverable_artifact(self, loop_id: str) -> None:
        ...
        try:
            ...
            name = (
                ...
                if loop.name
                else f"Loop {loop_id} deliverable"
            )
            ...
            logger.debug(
                "deliverable→artifact registration failed for %s",
                loop_id,
                exc_info=True,
            )
    ...
    async def _reconcile_linked_tasks(self, loop_id: str) -> None:
        ...
        try:
            ...
            for tid in loop.linked_task_ids:
                try:
                    ...
                    await registry.update_task(
                        tid, provider_name="native", status="done"
                    )
                    reconciled += 1
                except ValueError:
                    logger.debug(
                        "loop %s: linked task %s not auto-completed (gated)",
                        loop_id,
                        tid,
                    )
                except Exception:
                    logger.debug(
                        "loop %s: linked task %s reconcile failed",
                        loop_id,
                        tid,
                        exc_info=True,
                    )
            if reconciled:
                logger.info(
                    "loop %s complete: marked %d linked task(s) done",
                    loop_id,
                    reconciled,
                )
        except Exception:
    ...
    async def _boot_sweep(self) -> set[str]:
        ...
        **This was `loop/manager.reap_orphaned_loops`, awaited from a gateway startup hook** —
        ...
        decided |= await concurrency.boot_sweep(
            "loop-planning",
            loops,
            survived=_stranded_in_planning,
            decide=self._rekick_planning,
        )
        try:
    ...
    async def _rearm_running(self, loop: Loop) -> bool:
        ...
        try:
            ...
            logger.debug(
                "loop: watcher_reaped emit failed for %s", loop.id, exc_info=True
            )
        strat = kinds.get_or_none(loop.kind)
        ...
        if reason:
            ...
            logger.warning(
                "loop: orphaned %s blocked from re-arm (%s) — paused for the user",
                loop.id,
                reason,
            )
            return True
    ...
    async def _poll_once(self) -> None:
        ...
        running = [
            loop for loop in store.list_all() if loop.status == LoopStatus.RUNNING.value
        ]
        ...
        for loop in running:
            ...
            if count > self._last_count[cid]:
                ...
                if strat is not None:
                    hooked = await kinds.run_cycle_hook(
                        strat, loop, findings, self._cycle_ctx()
                    )
                if hooked is not None:
                    ...
                    try:
                        ...
                        logger.warning(
                            "loop %s: done signal errored", cid, exc_info=True
                        )
                        signal = None
                    if signal is None:
                        if supervisor.has_done_check(loop, policy):
                            ...
                            blind = (
                                bool(
                                    (fresh.kind_config or {}).get("judge_calibrated")
                                    is False
                                )
                                if fresh
                                else False
                ...
                if (
                    cur := store.get(cid)
                ) is not None and cur.status != LoopStatus.RUNNING.value:
                    continue
                ...
                if (
                    session is not None and getattr(session, "running", False)
                ) or reprompt:
    ...
    def _stagnation_disabled(self, loop) -> bool:
        ...
        return not supervisor.stagnation_enabled(
            policy_for_kind(loop.kind, loop.kind_config)
        )
