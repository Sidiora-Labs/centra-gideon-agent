...
def _sibling_loops_block(loop: Loop) -> str:
    ...
    try:
        siblings = [
            lp for lp in store.list_for_project(loop.project_id) if lp.id != loop.id
        ]
        if not siblings:
            return ""
        lines = [
...
async def rearm_nudge_message(svc, loop_id: str) -> None:
    ...
    try:
        ...
        await svc.update(
            nl.id,
            message=_build_nudge_message(strat, loop, loop_files.loop_dir(loop_id)),
        )
    except Exception:
...
async def stop(state, svc, loop_id: str) -> Loop:
    ...
    return store.update_status(
        loop_id, LoopStatus.STOPPED, stop_reason=LoopStopReason.USER
    )
        ...
        if _is_parallel(loop):
            for tid in loop.kind_config.get("queued_task_ids", []) or []:
                try:
                    ...
                    logger.debug(
                        "fan-out steer to task %s failed for %s",
                        tid,
                        loop_id,
                        exc_info=True,
                    )
    loop_files.append_nudge(
        loop_id, text, sent_at_cycle=loop_files.cycles_completed(loop_id)
    )
...
async def spawn_task_worker(
    ...
    try:
        ...
        await registry.update_task(
            task.id, provider_name="native", status="in_progress"
        )
    except Exception:
