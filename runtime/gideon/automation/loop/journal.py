...
:mod:`gideon.assurance.ledger`; this module makes the loop engine the second. Every property is
inherited from :class:`~gideon.assurance.ledger.writer.LedgerWriter` — append-only, `redact()` on
...
The vocabulary is imported from :mod:`gideon.assurance.ledger`, never re-minted: a loop that emitted
...
:class:`~gideon.automation.workflows.judge_contract.JudgeVerdict` shape — the loop no longer has a
private verdict dialect.

Boundary: this is a LOOP-side thing that writes THROUGH the ledger primitive. `gideon.assurance.ledger`
does not, and may not, import `gideon.automation.loop` — the direction is the whole point (the rail
`test_the_ledger_package_does_not_import_the_workflow_engine` guards it). `loop.store` is the
:class:`~gideon.assurance.ledger.writer.LedgerStore` this ledger appends through.
"""
...
from gideon.assurance.ledger import (
    BREAKER_TRIP,
    EVENTS_FILE,
    JOURNAL_FILE,
    JUDGE_VERDICT,
...
def cycles_completed(loop_id: str) -> int:
    ...
    Routed through :func:`gideon.assurance.ledger.reader.run_totals` rather than counting here, so "a
