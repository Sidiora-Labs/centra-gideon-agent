...
task creation), operating on the :class:`gideon.automation.loop.loop.Loop` entity:
...
def _is_done(status) -> bool:
    ...
    (:func:`gideon.automation.workflows.materialize.is_done`). Named here only because callers
...
def _is_resolved(status) -> bool:
    """The loop side's entry to the ONE task-status vocabulary
    (:func:`gideon.automation.workflows.materialize.is_resolved`). This module used to carry its own
...
def ensure_phase_lists(loop: Loop, tasks_project_id: str) -> dict:
    ...
    h = _hierarchy()
    existing_by_name = {
        tl.name: tl.id for tl in h.list_task_lists(project_id=tasks_project_id)
    }
...
async def decompose_sub_goals(loop_id: str) -> list[str]:
    ...
    sub_goals = [
        str(s).strip()
        for s in (loop.kind_config or {}).get("sub_goals", [])
        if str(s).strip()
    ]
    if not sub_goals:
        return []
...
async def resolved_stage_task_count(loop: Loop, phase_key: str) -> int:
    ...
    try:
        ...
        logger.debug(
            "resolved_stage_task_count failed for %s phase %s",
            loop.id,
            phase_key,
            exc_info=True,
        )
    ...
    try:
        ...
        def is_ready(t) -> bool:
            if t.id not in queued:
                return False
            if (
                _is_resolved(t.status)
                or getattr(t.status, "value", t.status) == "in_progress"
            ):
                return False
        ...
        order = {
            ...
            for i, tid in enumerate(
                (loop.kind_config or {}).get("queued_task_ids", []) or []
            )
        }
        ...
        logger.debug(
            "ready_queued_tasks failed for %s phase %s",
            loop.id,
            phase_key,
            exc_info=True,
        )
...
async def seed_phase_tasks(loop_id: str) -> int:
    ...
    for phase in loop.plan or []:
        ...
        try:
            ...
            logger.debug(
                "seed_phase_tasks failed for phase %r of %s",
                key,
                loop_id,
                exc_info=True,
            )
    ...
    for orig_idx, spec in spec_by_orig.items():
        ...
        if dep_ids:
            try:
                ...
                logger.debug(
                    "decompose_phase: failed to set deps for %s",
                    by_orig[orig_idx],
                    exc_info=True,
                )
    return created
        ...
        for t in tasks:
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(
                        "reconcile_phase_done: couldn't force-close %s",
                        t.id,
                        exc_info=True,
                    )
            except Exception:
                logger.debug(
                    "reconcile_phase_done: update failed for %s", t.id, exc_info=True
                )
        return closed
    except Exception:
        logger.warning(
            "reconcile_phase_done failed for %s phase %s",
            loop_id,
            phase_key,
            exc_info=True,
        )
    ...
    try:
        ...
        for list_id in (loop.task_list_ids or {}).values():
            ...
            tasks, _ = await registry.list_all_tasks(
                task_list_id=str(list_id), limit=500
            )
            for t in tasks:
            ...
            for list_id in (loop.task_list_ids or {}).values():
                if list_id:
                    try:
                        ...
                        logger.debug(
                            "teardown: drop task list %s failed", list_id, exc_info=True
                        )
        return removed
    except Exception:
