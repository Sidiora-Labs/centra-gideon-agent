...
from gideon.automation.loop.goal_meta import GOAL_TYPES as _GOAL_TYPES
from gideon.automation.loop.goal_meta import GRANULARITIES as _GRANULARITIES
...
class GoalKind(LoopKindStrategy):
    ...
    label = "Goal"
    description = (
        "Research + action toward a goal — verifiable, open-ended, or monitoring."
    )
    ...
    def turn_directive(self, loop: Loop) -> str:
        ...
        next_exit = (
            str(phase.get("phase_exit", "")).strip() if idx + 1 < len(plan) else ""
        )
    ...
    def validate_config(self, config: dict) -> tuple[list[str], list[str]]:
        ...
        from gideon.automation.loop.tick import validate_step_phase
    ...
    def build_brief(self, loop: Loop, context_dir: str = "") -> str:
        ...
        lines += [f"- {s}" for s in sub_goals] or [
            "- (none — derive your own from the goal)"
        ]
        if loop.linked_task_ids:
            list_id = (loop.task_list_ids or {}).get("sub_goals", "")
            scope_hint = (
                f'`task_list {{task_list_id: "{list_id}"}}`'
                if list_id
                else "`task_list`"
            )
            lines += [
    ...
    def cycle_nudge(self, loop: Loop, loop_dir: str) -> str:
        ...
        rendered = render_snippet_block(
            "loop-goal-cycle-nudge",
            {
                ...
                "is_multi_agent_with_roster": loop.execution == "multi_agent"
                and bool(loop.roster),
            },
        )
        return rendered or "\n".join(lines)
...
_DELIVERABLES = {
    "open_ended": "REPORT.md",
    "monitor": "MONITOR_LOG.md",
    "verifiable": "",
}
...
class _GoalWalkthrough:
    ...
    def default_steps(self) -> list[dict]:
        from gideon.automation.loop import goal_plan_briefs as pw
