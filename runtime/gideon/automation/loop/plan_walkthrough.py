...
over a :class:`gideon.cognition.planning.session.PlanSession`: the planner agent
...
async def run_step_pass(state, svc, loop_id: str, step_id: str) -> PlanStep | None:
    ...
    raw = await _run_pass(
        state, svc, loop, wt, sentinel=ARTIFACT_SENTINEL, brief=base_brief
    )
    ...
    if step.status != PS.StepStatus.RUNNING.value:
        step.status = PS.StepStatus.RUNNING.value
...
async def finalize_plan(loop_id: str) -> bool:
    ...
    if hook is not None:
        try:
            ...
            logger.warning(
                "walkthrough on_finalize failed for %s", loop_id, exc_info=True
            )
    try:
