...
_DESIGN_DIR = package_path("core", "config", "design")
...
def to_css_variables(
    token_overrides: dict | None = None, *, scheme: str = "light"
) -> str:
    ...
    path with dots/slashes → dashes, prefixed ``--gideon-``."""
    ...
    for name in sorted(flat):
        css_name = "--gideon-" + name.replace(".", "-").replace("/", "-")
