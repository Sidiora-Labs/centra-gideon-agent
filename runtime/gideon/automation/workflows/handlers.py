...
def _ok(body: dict[str, Any], *, status: int = 200) -> web.Response:
    return web.json_response(
        {k: v for k, v in body.items() if k != "ok"}, status=status
    )
        ...
        return web.json_response(
            {
                "error": {
                    ...
                    "message": "this session cannot mutate",
                }
            },
            status=403,
        )
...
def _template_run_stats(name: str) -> dict[str, Any]:
    ...
    from gideon.automation.workflows.journal import GATE_REJECTED
...
def _attention_scopes(
    *, per_scope_runs: int = 15, now: float | None = None
    ...
    from gideon.automation.workflows import introspection, journal
...
def nodding_revocation_cause(*, max_runs: int = 120) -> str:
    ...
    try:
        ...
        for name, records in sorted(by_template.items()):
            allowed, reason = judge_calibration.may_become_default(
        ...
        return (
            ...
            "across enough real runs is a check that does not check: "
            + "; ".join(nodding)
            + "."
        )
        ...
        rows = [
            {
                ...
                "created_at": str(
                    spec.get("updated_at") or spec.get("created_at") or ""
                ),
                "note": "",
                "run_ids": [],
                "ops_count": 0,
...
async def api_def_version_diff(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(
            {"error": {"code": "invalid_request", "message": "a/b must be integers"}},
            status=400,
        )
        ...
        return web.json_response(
            {
                "error": {
                    "code": "not_found",
                    "message": f"no version {version} for {name!r}",
                }
            },
            status=404,
        )
...
async def api_run_start(request: web.Request) -> web.Response:
    ...
    return _reply(
        result, status=202 if result.get("ok") and not result.get("blocking") else 200
    )
...
async def api_run_delete(request: web.Request) -> web.Response:
    ...
    result = await service.delete_run(
        run_id, supervisor=_supervisor(request), keep_open=keep_open
    )
    _audit(
        request,
        "workflow_run_delete",
        ...
        return web.json_response(
            {
                "error": {
                    "code": "invalid_request",
                    "message": f"unreadable multipart body: {exc}",
                }
            },
            status=400,
        )
...
async def api_run_node_inspect(request: web.Request) -> web.Response:
    ...
    safe = {
        k: (journal.redact(v) if k in redacted_keys else v)
...
async def _reentry(request: web.Request, operation: str, fn: Any) -> web.Response:
    ...
    if not node_id:
        return web.json_response(
            {"error": {"code": "invalid_request", "message": "'node_id' is required"}},
            status=400,
        )
...
async def api_run_events(request: web.Request) -> web.Response | web.StreamResponse:
    ...
    from gideon.automation.workflows.watchdog import registry_key
    ...
    if registry is None:
        return web.json_response(
            {"error": {"code": "engine_unavailable", "message": "no event registry"}},
            status=503,
        )
