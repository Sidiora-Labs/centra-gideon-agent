...
:mod:`gideon.cognition.review_triage` is deliberately surface-free and engine-free: it parses, anchors,
...
from gideon.automation.workflows import provisioning, service, store
from gideon.cognition import review_triage
...
async def workspace_diff(run: Any) -> tuple[str, str, bool]:
    ...
    for rel in [line for line in untracked.splitlines() if line.strip()][
        ...
        if not (Path(path) / rel).is_file():
            continue
        diff += await _git(
            ["diff", "--no-color", "--no-index", "--", "/dev/null", rel], path
        )
    ...
    for row in rows:
        ...
        out.append(
            Finding(
                ...
                origin_node_id=str(
                    row.get("origin_node_id", "") or row.get("node_id", "") or ""
                ),
...
async def apply_triage(
    run_id: str, raw_decisions: Any, *, dispatch: bool = True
) -> dict[str, Any]:
    ...
    if not dispatch:
        return {
            ...
            "dry_run": True,
            "brief": (
                review_triage.dispatch_brief(result.accepted) if result.accepted else ""
            ),
            **result.to_dict(),
    ...
    calibration = review_triage.calibration_records(
        result, template=str(run.workflow_name or "")
    )
    if calibration:
