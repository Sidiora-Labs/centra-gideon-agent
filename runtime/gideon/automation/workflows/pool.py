...
Gideon already runs concurrent co-tenant sessions and batch `subagent_run` children sharing one task
...
class Lease:
    ...
    def from_dict(cls, d: dict[str, Any]) -> Lease:
        ...
        return cls(
            ...
            ttl_seconds=int(
                d.get("ttl_seconds", DEFAULT_LEASE_SECS) or DEFAULT_LEASE_SECS
            ),
...
def suggest_handoffs(
    ...
    *,
    ...
    out: list[HandOff] = []
    for edge in table.get(def_name, ()):
...
def claim_task(
    ...
    if ttl_seconds is None:
        from gideon.automation.workflows.settings import lease_ttl_secs
