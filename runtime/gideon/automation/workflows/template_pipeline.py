...
def scrub_entities(
    text: str, *, slot_prefix: str = "entity"
...
def parameterize(spec: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if mapping:
        ...
        for slot, original in mapping.items():
            inputs.setdefault(
                slot, {"default": original, "help": f"was {original!r} in the run"}
            )
        out["inputs"] = inputs
    return out, mapping
...
def _scrub_node(node: dict[str, Any], mapping: dict[str, str]) -> dict[str, Any]:
    ...
    for key in ("children", "branches"):
        if isinstance(out.get(key), list):
            out[key] = [
                _scrub_node(c, mapping) if isinstance(c, dict) else c for c in out[key]
            ]
