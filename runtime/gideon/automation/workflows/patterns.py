    ...
    Shape(
        ...
        skeleton={
            ...
            "children": [
                ...
                _stage(
                    "synthesize", "synthesis: reconcile the angles, keep disagreements"
                ),
                {
                    "kind": "gate",
                    "id": "answered",
...
def pick_shape(
    intent_text: str, *, classifier_shape: str = ""
    ...
    if mapped:
        return (
            SHAPES_BY_NAME[mapped],
            f"intent shape `{classifier_shape}` maps to {mapped}",
        )
    ...
    if not scored:
        return (
            None,
            "no shape signal matched — freeform generation with strict validation",
        )
    ...
    if best_score == runner_up:
        ...
        return (
            None,
            f"shape signals tied between {', '.join(tied)} — planner should choose",
        )
