...
def merge_patches(spec: dict[str, Any], patches: list[Patch]) -> MergeResult:
    ...
    if not isinstance(root, dict):
        result.rejected = [
            f"{p.node_id}: the spec has no root to patch" for p in patches
        ]
        return result
    ...
    for patch in patches:
        ...
        if patch.op == "remove":
            if patch.node_id not in existing:
                result.rejected.append(
                    f"{patch.node_id}: cannot remove a node that does not exist"
                )
                continue
...
def resolve_step_ref(root: dict[str, Any], step_ref: str) -> tuple[str, str, str]:
    ...
    if not ref:
        return (
            ...
            "a revise must name the step to change (`step_ref`)",
        )
...
def comment_patch(
    ...
    return Patch(
        op="replace", node_id=node_id, node=node, reason=text, requested_by=requested_by
    )
...
def _containers(
    node: Any, out: list[tuple[dict, list]] | None = None
    ...
    for parent, children in _containers(root):
        for index, child in enumerate(children):
            if isinstance(child, dict) and str(child.get("id", "")) == node_id:
                ...
                return True
        if (
            isinstance(parent.get("body"), dict)
            and str(parent["body"].get("id", "")) == node_id
        ):
...
class SketchStore:
    ...
    def __init__(
        self, *, ttl: float = SKETCH_TTL_SECS, cap: int = MAX_SKETCHES
    ) -> None:
    ...
    def get(
        self, sketch_id: str, *, now: float | None = None
    ) -> tuple[Sketch | None, str]:
        ...
        if sketch is None:
            if sketch_id in self._expired:
                return (
                    None,
                    "sketch expired — re-plan rather than revising a stale draft",
                )
...
def announce_block(
    *,
    ...
    if intent is not None:
        ...
        lines.append(
            f"Risk:      {'; '.join(risk_bits) if risk_bits else 'nothing flagged'}"
        )
        lines.append(
            f"Rigor:     {getattr(getattr(intent, 'rigor', None), 'value', 'standard')}"
        )
...
def estimate_cost(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from gideon.automation.workflows.models import LLM_KINDS, Node, walk
    ...
    for path, node in walk(root):
        if node.kind.value in llm_kinds:
            ...
            for other_path, other in walk(root):
                if other.kind.value in ("foreach", "loop") and path.startswith(
                    other_path + "."
                ):
                    cap = (other.config or {}).get("max_iterations") or (
                        other.config or {}
                    ).get("n")
        ...
        if node.kind.value == "loop":
            cfg = node.config or {}
            if not isinstance(cfg.get("max_iterations"), int) and not isinstance(
                cfg.get("n"), int
            ):
                unbounded_loops += 1
