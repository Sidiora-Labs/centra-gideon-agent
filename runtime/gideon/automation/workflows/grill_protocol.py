...
The existing :mod:`gideon.cognition.grill` pipeline stays exactly as it is — it is the vendor-neutral
...
from gideon.automation.workflows.human_input import Ask, AskField, AskKind
...
def pace(questions: list[Question]) -> list[Round]:
    ...
    if len(load_bearing) >= 3:
        ...
        rounds.extend(
            Round(questions=[q], batched=False, reason="too few to batch")
    ...
    for question in questions:
        ...
        if given and not _is_deferral(given):
            ...
            step.assumptions.append(
                f"{question.text} → {question.recommended} (assumed)"
            )
...
def prohibitions_block(prohibitions: list[str]) -> str:
    ...
    return (
        "HARD PROHIBITIONS — these were set by the user and are not negotiable:\n"
        + lines
    )


def inject_prohibitions(
    spec: dict[str, Any], prohibitions: list[str]
) -> dict[str, Any]:
...
def _inject(node: dict[str, Any], block: str) -> dict[str, Any]:
    ...
    for key in ("children", "branches"):
        if isinstance(out.get(key), list):
            out[key] = [
                _inject(c, block) if isinstance(c, dict) else c for c in out[key]
            ]
...
def deep_triggered(
    intent: Intent, risk_hits: list[Any] | None = None
) -> tuple[bool, str]:
