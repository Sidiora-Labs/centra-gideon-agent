...
from gideon.assurance.ledger import hash_value
...
def run_stats(run_id: str, events: list[dict[str, Any]]) -> RunStats:
    ...
    stats.models = models
    stats.unverified_steps = len(
        [n for n in completed_nodes if n not in verified_nodes]
    )
...
def trajectory_steps(events: list[dict[str, Any]]) -> list[tuple[str, str, str]]:
    ...
    for event in events or []:
        ...
        steps.append(
            (node, lane_by_path.get(path, ""), _trajectory_verdict(kind, event))
        )
    return steps
...
class TrajectorySignature:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "length": self.length,
            "steps": [
                {"node": n, "lane": lane, "verdict": v} for n, lane, v in self.steps
            ],
        }
...
class JudgeStats:
    ...
    def degenerate_warning(self, *, min_runs: int = EDGE_STATS_MIN_RUNS) -> str:
        ...
        if self.total < min_runs:
            return ""
        chosen = [
            (verdict, count) for verdict, count in self.verdicts.items() if count > 0
        ]
        if len(chosen) == 1:
            return (
...
def template_card(
    ...
    """
    card = TemplateCard(
        template=template, runs=len(runs), warnings=list(warnings or [])
    )
    if not runs:
        return card
...
class ProofSection:
    ...
    def coverage(self) -> float:
        return (
            round(self.verified_steps / self.total_steps, 4)
            if self.total_steps
            else 0.0
        )
...
def proof_section(
    stats: RunStats, *, evidence_files: list[str] | None = None
) -> ProofSection:
...
def attention_debt(
    ...
    *,
    now: float,
    half_life_days: float = ATTENTION_DEBT_HALF_LIFE_DAYS,
) -> float:
...
def attention_trend(
    per_run_counts: list[int], *, min_runs: int = ATTENTION_TREND_MIN_RUNS
) -> str:
...
def findings_rail(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for event in events or []:
        if (
            ...
            or str(event.get("kind") or "") != "step_completed"
        ):
            continue
...
def verdict_rail(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for event in events or []:
        if (
            not isinstance(event, dict)
            or str(event.get("kind") or "") != "judge_verdict"
        ):
            continue
        row: dict[str, Any] = {"ts": str(event.get("ts") or "")}
...
class RailTotals:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "tokens": self.tokens,
            "duration_secs": (
                None if self.duration_secs is None else round(self.duration_secs, 3)
            ),
            "verdicts_by_word": dict(self.verdicts_by_word),
            "overall_series": (
                None if self.overall_series is None else list(self.overall_series)
            ),
...
def rail_totals(
    ...
    for key in ("cost_usd", "duration_secs"):
        carried = [
            row[key] for row in findings if isinstance(row.get(key), (int, float))
        ]
        if carried:
            setattr(totals, key, float(sum(carried)))
    carried_tokens = [
        row["tokens"] for row in findings if isinstance(row.get("tokens"), int)
    ]
    if carried_tokens:
