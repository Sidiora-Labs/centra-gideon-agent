...
class JudgeHints:
    ...
    stop_condition: dict[str, int] = field(
        default_factory=lambda: {"consecutive_clean": 2}
    )
    ...
    for enum_field, enum_cls in (
        ...
        if value is not None:
            try:
                ...
                logger.debug(
                    "unknown %s %r — keeping the strict default", enum_field, value
                )

    if isinstance(raw.get("stop_condition"), dict):
        hints.stop_condition = {
            k: int(v)
            for k, v in raw["stop_condition"].items()
...
def meets_ratchet(scores: dict[str, int], hints: JudgeHints) -> tuple[bool, list[str]]:
    ...
    for criterion in hints.rubric:
        ...
        if int(actual) < criterion.clamp_target():
            shortfalls.append(
                f"{criterion.criterion}: {actual} < {criterion.clamp_target()}"
            )
...
def validate_verdict(
    ...
    *,
    ...
    result = JudgeVerdict(
        ...
        model_overall=(
            float(raw["overall"])
            if isinstance(raw.get("overall"), (int, float))
            else None
        ),
    ...
    if evidence_text:
        ...
        result.unanswerable_refs = ungrounded_refs(
            result.evidence_refs, evidence_text, hints
        )
    ...
    if (
        ...
        result.invalid_reason = (
            "PASS cites only evidence not shown to the judge: "
            + "; ".join(r[:80] for r in result.unanswerable_refs[:3])
        )
        result.protocol_error = True
        return result
    ...
    if hints.rubric and not any(
        score_for(c.criterion, scores) is not None for c in hints.rubric
    ):
        result.invalid_reason = (
        ...
        return result
    if not ok:
        result.invalid_reason = "PASS below rubric targets: " + "; ".join(
            shortfalls[:5]
        )
        return result
    ...
    if fallback_result is False:
        result.escalated = True
        result.escalation_reason = f"judge PASS contradicts {hints.fallback_check.value} (deterministic check failed)"
    return result
...
def detect_forbidden_modes(text: str, hints: JudgeHints) -> str:
    ...
    **The match is subject-AND-any-alternative, not all-words.** Measured: requiring
    ...
    for mode in hints.forbidden_success_modes:
        signals = {
            w
            for w in re.findall(r"[a-z]+", mode.lower())
            if len(w) > 2 and w not in _MODE_NOISE
        }
        if not signals:
            continue
...
def judge_instruction(prompt: str, hints: JudgeHints) -> str:
    ...
    if hints.freedom_level is FreedomLevel.LOW:
        lines.append(
            "Judge per-step compliance: did it do what it was told, step by step?"
        )
    ...
    if hints.rubric:
        ...
        for criterion in hints.rubric:
            lines.append(
                f'  "{criterion.criterion}" (target {criterion.clamp_target()})'
            )
        if hints.ratchet is Ratchet.STRICT:
            lines.append(
    ...
    if hints.forbidden_success_modes:
        lines.append("")
        lines.append(
            "These passes are forbidden. Do not PASS if the work did any of them:"
        )
    ...
    if hints.proof_command:
        lines.append("")
        lines.append(
            f"Re-run this yourself and cite its output as proof: {hints.proof_command}"
        )
    lines += [
        "",
