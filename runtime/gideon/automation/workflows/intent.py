...
def classify(text: str) -> Intent:
    ...
    if not lowered.strip():
        return Intent(
            rigor=Rigor.FAST, confidence=0.0, reason="no intent text to classify"
        )

    signals: dict[str, list[str]] = {}
    complexity = _dimension(
        lowered, _COMPLEXITY_HIGH, _COMPLEXITY_LOW, signals, "complexity"
    )
    ...
    uncertainty = _dimension(
        lowered, _UNCERTAINTY_HIGH, _UNCERTAINTY_LOW, signals, "uncertainty"
    )
...
def route_rigor(intent: Intent) -> Rigor:
    ...
    if (
        intent.stakes == Level.LOW
        and intent.uncertainty != Level.HIGH
        and not _is_complex(intent)
    ):
...
def _confidence(signals: dict[str, list[str]], words: int) -> float:
    ...
    """
    dimensions = sum(
        1 for key in ("complexity", "uncertainty", "stakes") if signals.get(key)
    )
