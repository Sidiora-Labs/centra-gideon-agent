...
ALLOWED_TARGETS: dict[Actor, frozenset[TaskStatus]] = {
    Actor.ENGINE: frozenset(TaskStatus),
...
class Check:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.kind is CheckKind.FILE_PHRASE:
            payload.update(
                {"path": self.path, "required_phrases": list(self.required_phrases)}
            )
        else:
            payload.update(
                {"command": self.command, "expect_exit_code": self.expect_exit_code}
            )
        return payload
    ...
    def from_dict(cls, d: dict[str, Any]) -> "Check | None":
        ...
        if kind is CheckKind.FILE_PHRASE:
            ...
            check.required_phrases = [
                str(p) for p in (d.get("required_phrases") or []) if str(p)
            ]
...
class Verdict:
    ...
    def score(self) -> float:
        return (
            round(self.hit_weight / self.total_weight, 4) if self.total_weight else 0.0
        )
...
def parse_criterion(raw: Any) -> tuple[list[Check], list[str]]:
    ...
    if not isinstance(raw, list):
        return [], [
            f"criterion must be a string, an object, or a list; got {type(raw).__name__}"
        ]
    checks: list[Check] = []
    for index, entry in enumerate(raw):
        ...
        if check is None:
            problems.append(
                f"check {index} is unusable and was DROPPED, not treated as passing"
            )
            continue
        checks.append(check)
...
def cascade_blocked(
    ...
    *,
    ...
    """
    reason = (
        f"Node {failed_node} failed: {cause}" if cause else f"Node {failed_node} failed"
    )
    blocked: list[str] = []
...
def sweep(tasks: list[Any], *, now: float) -> list[Finding]:
    ...
    for task in tasks or []:
        ...
        if status is TaskStatus.IN_PROGRESS:
            beat = _epoch(
                getattr(task, "last_heartbeat_at", "")
