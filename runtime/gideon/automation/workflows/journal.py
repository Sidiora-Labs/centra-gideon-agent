...
:mod:`gideon.assurance.ledger`, because none of it is workflow-shaped and a second producer must
...
class Journal(LedgerWriter):
    ...
    def step_skipped(
        self, path: str, node_id: str, *, epoch: int, actor: str = "engine"
    ) -> None:
        ...
        self.write(
            STEP_SKIPPED, instance_path=path, node_id=node_id, epoch=epoch, actor=actor
        )
    ...
    def decision(self, path: str, node_id: str, *, epoch: int, decision: dict) -> None:
        ...
        """
        self.write(
            DECISION, instance_path=path, node_id=node_id, epoch=epoch, **decision
        )
    ...
    def child_run_attach(
        self, parent_run_id: str, child_run_id: str, node_id: str
    ) -> None:
        self.write(
            CHILD_RUN_ATTACH,
...
def journal_records(
    run_id: str, *, kinds: set[str] | None = None
) -> list[dict[str, Any]]:
    ...
    :func:`gideon.assurance.ledger.reader.read_journal`: it exists so a consumer asks the facade
