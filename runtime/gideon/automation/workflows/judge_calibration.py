...
def assess_gate(
    ...
    *,
    ...
    relevant = [
        r
        for r in records
        if (not template or r.template == template)
    ...
    if len(relevant) < max(1, min_runs):
        report.detail = (
            f"only {len(relevant)} verdict(s); need {min_runs} to judge the judge"
        )
        return report
    ...
    if passes == 0:
        ...
        report.detail = (
            f"rejected {rejects}/{rejects} — nothing has ever passed this gate"
        )
        return report
...
def assess_all_gates(
    ...
    return [
        assess_gate(records, template=t, node_id=n, min_runs=min_runs) for t, n in pairs
    ]
    ...
    *,
    ...
    if strong_score is None or null_score is None:
        return CalibrationResult(
            None,
            None,
            "the probe could not produce both scores — deferring, not declaring blind",
        )
    separation = round(float(strong_score) - float(null_score), 4)
    if separation >= min_separation:
        return CalibrationResult(
            True, separation, f"separation {separation} >= {min_separation}"
        )
    return CalibrationResult(
        False,
        separation,
...
MAE_BUCKETS: tuple[tuple[float, float], ...] = (
    (0.0, 0.25),
    (0.25, 0.5),
    (0.5, 0.75),
    (0.75, 1.0),
