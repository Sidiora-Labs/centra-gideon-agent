...
class SurfacingMeta:
    ...
    def from_dict(cls, d: dict[str, Any]) -> SurfacingMeta:
        ...
        return cls(
            ...
            freedom_level=_enum(
                FreedomLevel, d.get("freedom_level"), FreedomLevel.MEDIUM
            ),
            preconditions=[
                p for p in (d.get("preconditions") or []) if isinstance(p, dict)
            ],
...
def lint_metadata(
    meta: SurfacingMeta, *, existing: dict[str, str] | None = None
) -> list[str]:
...
_STEP_MARKERS = (
    re.compile(r"\b(?:step\s*\d|1[.)]\s|first,|then\b|finally,|after that)", re.I),
)
...
def may_suggest(
    ...
    *,
    ...
    reasons = veto_reasons(
        query, meta, named_workflow=named_workflow, available=available
    )
...
def render_suggest(
    meta: SurfacingMeta, *, name: str, inputs: dict[str, Any] | None = None
) -> str:
...
def migrate_sop(
    sop: dict[str, Any], *, existing: dict[str, str] | None = None
) -> MigrationResult:
    ...
    meta = SurfacingMeta(
        ...
        agent_digest=str(
            sop.get("agent_digest", "") or sop.get("description", "") or ""
        ),
        surface_mode=mode,
