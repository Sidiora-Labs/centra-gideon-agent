...
def escalation_decision(state: CadenceState, now: float) -> tuple[bool, str]:
    ...
    if state.escalation is not Escalation.AUTO:
        return (
            False,
            "escalation is manual: the def surfaces in the list but materializes nothing",
        )
    ...
    if (
        ...
        and now - state.last_escalated_at < ESCALATION_INTERVAL_SECS
    ):
...
def dismiss_pack(project_id: str, *pack_names: str) -> set[str]:
    ...
    current = load_dismissals(project_id) | {
        str(n) for n in pack_names if str(n).strip()
    }
    path = dismissals_path(project_id)
...
def effective(entries: Iterable[ScopedDef]) -> dict[str, ScopedDef]:
    ...
    return {
        r.entry.name: r.entry
        for r in resolve_scopes(entries)
        if r.state is ScopeState.EFFECTIVE
    }
...
def adopt_target(entry: ScopedDef) -> tuple[str, str]:
    ...
    return (
        "",
        f"{entry.name} is already at editable scope {entry.scope!r}; edit it in place",
    )
...
def apply_overlay(
    ...
    for stage in base_stages:
        ...
        diff.append(
            f"~ {stage_id} ({', '.join(changed)})" if changed else f"= {stage_id}"
        )
    return out, diff
...
def probe_state(
    ...
    *,
    ...
    try:
        ...
        Exception
    ) as exc:  # noqa: BLE001 - a bundle's probe must not decide the answer by dying
...
def build_prefill(
    ...
    * **`all_filled` is re-validated against the schema**, never taken from the extractor.
    ...
    optional_missing = [
        key
        for key in props
        if key not in required and key not in extracted and key not in skip
    ]
    follow_up = ""
    if missing:
        ...
        if entry.get("shadowed_by"):
            findings.append(
                DoctorFinding(
                    ...
                    detail=(
                        f"shadowed by {entry['shadowed_by']}; it will never be the "
                        "effective def"
                    ),
                )
            )
...
def fixture_gaps(name: str, fixtures: Sequence[TriggerFixture]) -> list[str]:
    ...
    return [
        f"{name}: no {kind} fixture" for kind in FIXTURE_KINDS if kind not in present
    ]


def check_fixtures(
    fixtures: Sequence[TriggerFixture], matcher: Callable[[str], bool]
) -> list[str]:
    ...
    for fixture in fixtures:
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 - a broken matcher is a failure, not a crash
            failures.append(
                f"[{fixture.kind}] matcher raised on {fixture.prompt!r}: {exc}"
            )
            continue
        if fired and not fixture.expect_match:
            failures.append(
                f"[{fixture.kind}] fired on {fixture.prompt!r} but must not"
            )
        elif not fired and fixture.expect_match:
            failures.append(
                f"[{fixture.kind}] did NOT fire on {fixture.prompt!r} but must"
            )
    return failures
...
def meta_from_def(metadata: Any) -> Any:
    ...
    """
    from gideon.automation.workflows.surfacing import SurfaceMode, SurfacingMeta
...
def handoffs_from_def(metadata: Any) -> list[Any]:
    ...
    """
    from gideon.automation.workflows.pool import HandOff
...
def route_from_def(metadata: Any, root: Any) -> Any:
    ...
    """
    from gideon.automation.workflows import models as _models
    from gideon.automation.workflows.pool import route
    ...
    try:
        ...
        return route(
            surface_mode="passive", has_gates=True, max_turns=1, has_schema=False
        )
    return route(
        surface_mode=str(getattr(metadata, "surface_mode", "off") or "off"),
        has_gates=has_gates,
