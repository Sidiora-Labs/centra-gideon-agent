...
def type_attention(
    spec: dict[str, Any], hits: list[RiskHit] | None = None
) -> dict[str, Attention]:
    ...
    destructive_nodes = {
        h.node_id for h in hits if h.level == RiskLevel.DESTRUCTIVE.value
    }
...
def offer_autonomy(
    ...
    *,
    ...
    offer = AutonomyOffer(
        offered=[
            m for m in MODE_ORDER if MODE_ORDER.index(m) <= MODE_ORDER.index(ceiling)
        ],
        ceiling=ceiling,
        floor=floor,
        capped_by=capped_by,
    ...
    offer.offered = [
        m for m in offer.offered if MODE_ORDER.index(m) >= MODE_ORDER.index(floor)
    ]
    ...
    if requested is not None and MODE_ORDER.index(requested) > MODE_ORDER.index(
        ceiling
    ):
        worst = max(
            hits, key=lambda h: h.level == RiskLevel.DESTRUCTIVE.value, default=None
        )
        because = (
            worst.consequence
            if worst is not None
            else "this plan is capped below that mode"
        )
...
def _recommend(
    ...
    *,
    ...
    """
    from gideon.automation.workflows.contracts import derive_contracts
    ...
    if (
        earned is not None
        and MODE_ORDER.index(earned) > MODE_ORDER.index(base)
        and not destructive
    ):
        base = earned
...
def confirmation_policy(
    ...
    if (
        ...
        or confirmation_type == ConfirmationType.DESTRUCTIVE
    ):
...
def _classify_node(
    node: Any, hits: list[RiskHit]
) -> tuple[ConfirmationType, RiskLevel]:
...
def _question_for(
    node_id: str, confirmation_type: ConfirmationType, hits: list[RiskHit]
) -> str:
    if hits:
...
def unattended_interrupts(
    ...
    for request in confirmations:
        stop, which, reason = should_interrupt(
