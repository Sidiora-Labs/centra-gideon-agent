...
* the workflow ledger — ``step_completed`` / ``step_failed`` / ``step_skipped`` / ``effect`` /
  ``decision``, read through :func:`gideon.ledger.reader.read_events`. Written by
...
``denied``    a tool result :func:`gideon.security.is_denial_observation` recognises. A
...
def _facts_from_tool_history(
    ...
    :func:`gideon.security.is_denial_observation` — reused rather than re-derived, because a
