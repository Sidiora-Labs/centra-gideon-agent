...
workspace (:mod:`gideon.proposer.verify`) — never the runner's own claim. A backend that
cannot prove the claimed edits are on disk MUST report ``diff_verified=False``; the service then
refuses the handoff (:mod:`gideon.proposer.service`). This is the plan's stated mitigation
...
if TYPE_CHECKING:
    ...
    from gideon.proposer.verify import DiskBaseline
