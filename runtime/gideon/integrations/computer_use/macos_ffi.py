...
from gideon.integrations.computer_use.types import (
    MAX_DEPTH,
    MAX_ELEMENTS,
    Element,
    WindowWalk,
...
def _load() -> _Frameworks:
    ...
    if platform.system() != "Darwin":
        raise FFIUnavailable(
            f"the macOS accessibility API does not exist on {platform.system()}"
        )
    try:
        ...
        raise FFIUnavailable(
            f"a required macOS framework could not be loaded: {exc}"
        ) from exc
    _LOADED = frameworks
    return frameworks
...
def _attribute(frameworks: _Frameworks, element: Any, name: str) -> Any:
    ...
    if code in _AX_NOT_TRUSTED_ERRORS:
        raise AXPermissionDenied(
            f"reading {name} was refused by the OS (AXError {code})"
        )
    if code != _AX_SUCCESS:
        return None
    return out.value
...
def _text_attribute(frameworks: _Frameworks, element: Any, name: str) -> str:
    ...
    if value is None:
        return ""
    if (
        frameworks.cf.CFGetTypeID(ctypes.c_void_p(value))
        == frameworks.cf.CFStringGetTypeID()
    ):
...
def _bool_attribute(
    frameworks: _Frameworks, element: Any, name: str, default: bool
) -> bool:
    value = _attribute(frameworks, element, name)
    if value is None:
        return default
    if (
        frameworks.cf.CFGetTypeID(ctypes.c_void_p(value))
        == frameworks.cf.CFBooleanGetTypeID()
    ):
...
def _array_items(frameworks: _Frameworks, ref: Any) -> list[Any]:
    if ref is None:
        return []
    if (
        frameworks.cf.CFGetTypeID(ctypes.c_void_p(ref))
        != frameworks.cf.CFArrayGetTypeID()
    ):
        return []
...
def _action_names(frameworks: _Frameworks, element: Any) -> tuple[str, ...]:
    out = ctypes.c_void_p()
    code = frameworks.ax.AXUIElementCopyActionNames(
        ctypes.c_void_p(element), ctypes.byref(out)
    )
    if code in _AX_NOT_TRUSTED_ERRORS:
        raise AXPermissionDenied(
            f"reading actions was refused by the OS (AXError {code})"
        )
    if code != _AX_SUCCESS:
        return ()
    return tuple(
        _from_cfstring(frameworks, item)
        for item in _array_items(frameworks, out.value)
        if item
    )
...
def walk_window(app: str) -> WindowWalk:
    ...
    *same* order on a re-walk, because that is what makes an index from one snapshot mean the
    ...
    Both caps (:data:`~gideon.integrations.computer_use.types.MAX_ELEMENTS`,
    :data:`~gideon.integrations.computer_use.types.MAX_DEPTH`) are enforced here rather than left to
    ...
    while stack:
        ...
        if depth < MAX_DEPTH:
            children = _array_items(
                frameworks, _attribute(frameworks, handle, "AXChildren")
            )
...
def perform_action(handle: Any, action: str) -> None:
    ...
    code = frameworks.ax.AXUIElementPerformAction(
        ctypes.c_void_p(handle), ctypes.c_void_p(name)
    )
...
def type_text(pid: int, text: str) -> None:
    ...
    for down in (True, False):
        ...
        count = len(units) // 2
        buffer = (ctypes.c_uint16 * max(count, 1)).from_buffer_copy(
            units or b"\x00\x00"
        )
        frameworks.cg.CGEventKeyboardSetUnicodeString(
            ctypes.c_void_p(event), count, buffer
        )
