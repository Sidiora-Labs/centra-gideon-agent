...
:mod:`gideon.security.guardrails.ceiling` and deliberately reuses that module's operator-owned
...
from gideon.integrations.computer_use.tools import TOOL_NAMES
from gideon.security.guardrails.ceiling import GOVERNANCE_DIRNAME
    ...
    **Entries are matched by EXACT string equality later**, so this is the one place a malformed
    ...
    for entry in raw:
        if not isinstance(entry, str):
            return refuse(
                f'the enable file\'s "{key}" carries {entry!r}, which is not a string'
            )
        if not entry.strip():
            return refuse(
