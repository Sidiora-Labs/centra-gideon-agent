...
:mod:`~gideon.integrations.computer_use.driver_host` finds this module by importing
``gideon.integrations.computer_use.macos_driver`` and calling ``op_<name>`` — that resolution IS the
...
**No ctypes here.** Every OS call goes through :mod:`~gideon.integrations.computer_use.macos_ffi`, so
...
from gideon.integrations.computer_use import macos_ffi as ffi
...
def _permission_fix(responsible: macos_tcc.Responsible) -> str:
    ...
    to fail — see :mod:`~gideon.integrations.computer_use.macos_tcc`.
...
def op_click(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if method == "auto":
        ...
        ffi.press(handle)
        return {
            "clicked": {"index": element.index, "role": element.role},
            "method": "auto",
        }
...
def op_scroll(request: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "scrolled": {"index": element.index, "direction": direction, "lines": lines}
    }
