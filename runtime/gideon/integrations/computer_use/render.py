...
(:func:`gideon.integrations.computer_use.service.live_snapshots`), the overlay's trail of approved
acting targets (:func:`gideon.integrations.computer_use.overlay.motion_trail`), and the SEL rows
...
survives passes through :func:`gideon.security.security.redact_credentials`, the same one
...
from gideon.integrations.computer_use import enable_state, gate, overlay, service
...
def _feed_rows(limit: int) -> list[dict[str, Any]]:
    ...
    Rows pass through :func:`gideon.security.sel.redact_event` — the single definition of what
    ...
    """
    from gideon.security.sel import SecurityEventLog, redact_event

    rows: list[dict[str, Any]] = []
    for record in SecurityEventLog().recent(_FEED_SCAN):
        if (
            not isinstance(record, dict)
            or record.get("event_type") != gate.SEL_EVENT_TYPE
        ):
            continue
...
def live_view(
    *, feed_limit: int = FEED_LIMIT, trail_limit: int = overlay.MAX_TRAIL
) -> dict:
