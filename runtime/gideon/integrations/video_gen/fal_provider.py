...
class FalVideoProvider(VideoGenProvider):
    ...
    async def list_models(self):
        return [
            VideoGenModel(
                "fal-ai/veo3.1",
                description="Veo 3.1; continuation conditions on the last frame only.",
                aspect_ratios=["16:9", "9:16"],
                max_duration_s=8,
                durations=[4, 6, 8],
                supports_first_frame=True,
                supports_continuation=True,
                supported_controls={"seed": VideoControl(0, 4294967295, True)},
            )
        ]

    def payload(self, prompt, model, duration_seconds, aspect_ratio, opts):
        if (
            model not in ("", "fal-ai/veo3.1")
            or duration_seconds not in (4, 6, 8)
            or isinstance(duration_seconds, bool)
        ):
            raise VideoGenError("Unsupported FAL model or duration")
        if aspect_ratio not in ("", "16:9", "9:16") or set(opts) - {
            "seed",
            "first_frame",
            "continuation_frame",
            "continuation_video",
        ...
        if frame:
            ...
            with Image.open(io.BytesIO(raw)) as image:
                ...
                image.verify()
            body["image_url"] = "data:image/png;base64," + base64.b64encode(raw).decode(
                "ascii"
            )
    ...
    async def generate(
        self, prompt, *, model="", duration_seconds=8, aspect_ratio="", **opts
    ):
        endpoint, body = self.payload(
            prompt, model, duration_seconds, aspect_ratio, opts
        )
        if not self.api_key:
            ...
            async with asyncio.timeout(1100):
                ...
                while True:
                    ...
                    if state.get("status") == "COMPLETED":
                        ...
                        return [
                            VideoResult(
                                url=url, mime="video/mp4", duration_s=duration_seconds
                            )
                        ]
