"""The real CDP page driver behind :class:`~gideon.integrations.browse.loop.PageDriver` (BA-3).

Everything the browse loop does to a page that is NOT navigation, spoken over the same
:class:`~gideon.integrations.browse.cdp.CdpTransport` BA-2 already owns. Navigation is absent by
design and stays with :class:`~gideon.integrations.browse.cdp.GatedCdpSession`, which pre-flights
...
class CdpPageDriver:
    """A :class:`~gideon.integrations.browse.loop.PageDriver` over one CDP page target.
    ...
    async def _act(self, expression: str, what: str) -> None:
        ...
        if value == NOT_FOUND:
            raise PageActionError(
                f"{what}: no matching element is on the page any more"
            )
    ...
    async def fill(self, ref: ElementRef, value: str) -> None:
        ...
        await self._act(
            _js(body, ref=ref, extra={"VALUE": json.dumps(value)}), f"TYPE {ref.ref}"
        )
    ...
    async def screenshot(self) -> str:
        ...
        try:
            ...
            path = (
                ...
                / f"browse-{int(time.time())}-{uuid.uuid4().hex[:8]}.png"
            )
