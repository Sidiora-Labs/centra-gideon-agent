...
class _StructureParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "form":
            ...
            link = _LinkRec(
                href=a.get("href", ""), aria=a.get("aria-label") or a.get("title", "")
            )
            ...
            fr = _FieldRec(
                role=ROLE_FIELD, label=(a.get("name") or a.get("id") or "text")
            )
            if "required" in a:
            ...
            self._current_form().fields.append(
                _FieldRec(role=ROLE_SELECT, label=label, note=note)
            )
    ...
    def _handle_input(self, a: dict[str, str]) -> None:
        ...
        if itype in ("submit", "button", "image", "reset"):
            label = (
                a.get("value")
                or a.get("name")
                or ("submit" if itype == "submit" else "button")
            )
            self._current_form().fields.append(
        ...
        if itype in ("checkbox", "radio"):
            ...
            self._current_form().fields.append(
                _FieldRec(
                    role=ROLE_CHECKBOX, label=label, state=state, note=f"type={itype}"
                )
            )
            return
    ...
    for form_rec in parser.forms:
        ...
        for fr in form_rec.fields:
            ...
            if fr.role in (ROLE_BUTTON,) and fr.text.strip():
                ...
                and fr.text.strip()
                and not state
                and not fr.credential
            ):
                state = _WS_RE.sub(" ", fr.text).strip()
            fields.append(
                ElementRef(
