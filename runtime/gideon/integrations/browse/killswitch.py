...
def engage(reason: str = "") -> BrowseKillState:
    ...
    current = get_kill()
    started = (
        current.started_at if current.active else datetime.now(timezone.utc).isoformat()
    )
    state = BrowseKillState(
        active=True, reason=reason or current.reason, started_at=started
    )
    _write(state)
