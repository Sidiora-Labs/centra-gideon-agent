...
from gideon.security.sel import sel as _sel

_PROMPT_BUSY_RETRIES = 2
_PROMPT_BUSY_DELAY = 1.5
        ...
        try:
            ...
            if "already in progress" not in str(exc) or attempt >= _PROMPT_BUSY_RETRIES:
                if "already in progress" in str(exc):
                    logger.warning(
                        "Prompt busy after %d retries, shutting down provider",
                    ...
                    try:
                        ...
                        logger.debug(
                            "Provider shutdown after busy retries failed", exc_info=True
                        )
...
async def stream_and_collect_json(
    ...
    *,
    ...
    text = await stream_and_collect(
        provider, message, approval_policy=approval_policy, hooks=hooks
    )
...
async def _resolve_permission(
    ...
    from gideon.engine.hooks import TOOL_AUTO_APPROVE, TOOL_DENY
...
def _enforces_json_schema_natively(model_ref: str) -> bool:
    ...
    try:
        ...
        logger.debug(
            "one_shot_completion: no native schema enforcement resolvable for %r",
            model_ref,
        )
        return False
...
async def one_shot_completion(
    ...
    *,
    ...
    if it still fails an :class:`~gideon.security.guardrails.failure.OutputContractError`
    ...
    :func:`gideon.integrations.local_models.budgets.output_budget` for the model it is about to
