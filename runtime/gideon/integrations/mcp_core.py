...
def _api_base() -> str:
    ...
    Delegates to :func:`gideon.engine.gateway_base.resolve_api_base`, which answers from
...
def _current_session_thread_ts() -> str | None:
    ...
    from gideon.engine.hooks import safe_read_file_bytes
...
def _render_resource_catalog(skill_name: str, loader: Any) -> str:
    ...
    for res in resources:
        lines.append(
            f"- {res.path}" + (f" — {res.description}" if res.description else "")
        )
    return "\n".join(lines) + "\n"
...
def _load_skill_resource(args: dict[str, Any]) -> str:
    ...
    All the refusal logic lives in ``ProcedureLibrary.read_resource`` (allowlist,
    ...
    out = [
        f"[Skill resource: {read.skill}/{read.path} — read-only, not executed]",
        body,
    ]
    if read.truncated:
        out.append(
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "skill_search":
        ...
        return (
            "Matching skills (call skill_invoke(name) to load full steps):\n"
            + "\n".join(lines)
        )
    ...
    if name == "wait":
        ...
        from gideon.assurance.validation import WAIT_SCHEMA, validate_tool_args
    ...
    if name == "hook_register":
        ...
        from gideon.assurance.validation import REGISTER_HOOK_SCHEMA, validate_tool_args
    ...
    if name == "notify":
        ...
        if args.get("session") == "channel":
            ...
            if resp.get("channel"):
                return (
                    f"Message sent to channel. ts={ts}"
                    if ts
                    else "Message sent to channel."
                )
    ...
    if name == "refiner_evidence":
        ...
        evidence = refiner_tools.gather_evidence(
            str(args.get("workflow_name", "") or "")
        )
        return json.dumps(
            {"ok": True, **evidence}, indent=2, ensure_ascii=False, default=str
        )
    ...
    if name == "propose_template_diff":
        ...
        return json.dumps(
            {"ok": True, **result}, indent=2, ensure_ascii=False, default=str
        )
...
def _suggest_template(args: dict[str, Any]) -> str:
    ...
    """
    from gideon.assurance.validation import SUGGEST_TEMPLATE_SCHEMA, validate_tool_args
...
def _resolve_review_project_id(explicit: str) -> str:
    ...
    if pid:
        return pid
    try:
        from gideon.engine.agents.native.builtin_tools import current_project_id
...
def _save_template_from_session(args: dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.debug(
            "template_save_from_session: def lookup unavailable", exc_info=True
        )

    outcome = evaluate(
        Candidate(
            run_id=slug, steps=steps, template_surfaced=surfaced, intent=description
        ),
        session_key=_resolve_session_key(),
...
def _project_context_review(args: dict[str, Any]) -> str:
    ...
    """
    from gideon.cognition.learning.project_context_review import (
        ReviewCandidate,
...
def _dashboard_tile_propose(args: dict[str, Any]) -> str:
    ...
    where = (
        "the Overview home"
        if view_id == views_store.PRESET_OVERVIEW_ID
        else f"view '{view_id}'"
    )
    return (
...
def run_mcp_core_server() -> None:
    ...
    from gideon.integrations.mcp_shared import run_mcp_stdio_loop

    run_mcp_stdio_loop(
        "gideon-core", "1.0.0", _aggregated_list_tools, _aggregated_call_tool
    )
