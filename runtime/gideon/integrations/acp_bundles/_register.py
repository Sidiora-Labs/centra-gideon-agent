...
:class:`~gideon.integrations.llm.registry.ProviderEntry` named ``acp:<cli>`` into the
default registry. That entry is the single source of truth an ``acp:<cli>``
agent resolves through:

* :func:`gideon.interfaces.dashboard.handlers.providers.api_agent_providers_list`
...
def register_acp_cli_entry(
    *,
    ...
    :mod:`gideon.integrations.acp.mcp_servers`), so seeding a vendor config file is
    ...
    try:
        ...
        logger.warning(
            "acp:%s bundle: failed to register provider entry", cli, exc_info=True
        )
        return None
