...
from gideon.integrations.mcp_core import _get, _post, _resolve_session_key
...
def _to_leaf(text: str, spec: dict[str, Any], agent: str) -> LeafTask:
    ...
    capability = (
        Capability.MUTATING if raw_capability == "mutating" else Capability.RESEARCH
    )
...
def _best_of_n(args: dict[str, Any]) -> str:
    ...
    A thin wrapper over ``gideon.integrations.sampling.best_of_n``: the fan-out, judging,
    ...
    from gideon.integrations.mcp_artifacts import _run_async
    ...
    if result["winner"] is None:
        return f"No candidate: {result['note']}. Nothing was selected — try again or answer directly."
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "subagent_run":
        ...
        # Fire-and-forget — gateway's DelegationSupervisor queues excess tasks
        ...
        if agent_ids:
            ...
            spawn_lines.append(
                "All tasks queued — results will arrive as completion events."
            )
        return "\n".join(spawn_lines)
    ...
    if name == "subagent_list":
        ...
        if not agents:
            ...
            for a in agents:
                ...
                lines.append(
                    f"{a['id']}  [{status}]{err}{progress}  {_redact(a['task'])[:60]}"
                )
        try:
