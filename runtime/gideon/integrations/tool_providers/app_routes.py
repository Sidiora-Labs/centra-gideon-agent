...
from gideon.extensions.apps.manifest import AppManifest, RouteEntry
...
def _iter_enabled_apps() -> Iterator[tuple[str, AppManifest]]:
    ...
    for app_info in list_apps():
        ...
        try:
            ...
            logger.debug(
                "app-routes: skipping unparseable manifest for %r", app_info.get("name")
            )
...
async def call_app_route(resolution: RouteResolution) -> ToolResult:
    ...
    from gideon.security.net import LOOPBACK_INTERNAL, EgressBlocked, fetch
    ...
    try:
        resp = await fetch(
            url, policy=LOOPBACK_INTERNAL, method=method, headers=headers, data=data
        )
...
def note_proxy_status(app_name: str, op: str, status: int) -> None:
    ...
    try:
        ...
        logger.debug(
            "app-routes: drift notify failed for %s/%s", app_name, op, exc_info=True
        )
            ...
            routes_out.append(
                {
                    ...
                    "tool": (
                        tool_name_for(name, route.op) if route.agentCallable else None
                    ),
                }
            )
        if routes_out:
