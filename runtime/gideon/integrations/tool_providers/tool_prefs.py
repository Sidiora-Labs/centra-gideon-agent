...
def _load() -> dict:
    ...
    try:
        ...
        logger.debug(
            "tool_prefs: unreadable — treating as none disabled", exc_info=True
        )
        return {}
...
def load_disabled_providers() -> set[str]:
    ...
    return {
        str(k) for k in items if isinstance(k, str) and not is_provider_locked(str(k))
    }
...
def is_provider_disabled(
    provider: str, disabled_providers: set[str] | None = None
) -> bool:
    dp = (
        disabled_providers
        if disabled_providers is not None
