...
_NO_INDEX_HINT = (
    "Use `grep` and `read_file` instead — they work regardless of whether the "
    "code index exists."
)
...
def _render_symbol(symbol: str, definitions: list, references: list) -> str:
    ...
    if definitions:
        ...
        lines.append(
            f"No definition of '{symbol}' found (it may be imported or dynamic)."
        )
    ...
    if references:
        ...
        if len(references) >= _MAX_REFERENCES:
            lines.append(
                f"  … reference list capped at {_MAX_REFERENCES}; grep for the full set."
            )
    else:
