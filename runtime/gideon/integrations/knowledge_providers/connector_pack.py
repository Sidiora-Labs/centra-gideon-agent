...
``gideon.sdk.knowledge``, so the class that actually polls is CORE's — a pack cannot
...
declared :class:`~gideon.extensions.apps.manifest.PackSourceEntry`, validates the user's ``args``
...
to the script on stdin via :func:`~gideon.integrations.knowledge_providers.pack_parse.run_parse_script`.
...
:mod:`~gideon.integrations.knowledge_providers.conditional_get` helpers — the same ONE implementation
...
def load_manifest(app_dir: Path) -> Any:
    ...
    try:
        ...
        raise PackConfigError(
            f"connector pack manifest unreadable at {path}: {exc}"
        ) from exc
...
def _coerce_arg(name: str, value: Any, declared: dict[str, Any]) -> str:
    ...
    if kind == "integer":
        if isinstance(value, bool) or not isinstance(value, int):
            raise PackConfigError(
                f"arg {name!r} must be an integer, got {type(value).__name__}"
            )
        return str(value)
    if kind == "boolean":
    ...
    if not isinstance(value, str):
        raise PackConfigError(
            f"arg {name!r} must be a string, got {type(value).__name__}"
        )
    return value
...
def render_fetch(
    ...
    *,
    ...
    """
    from gideon.extensions.apps.manifest import PACK_FETCH_METHODS, PACK_PLACEHOLDER_RE
    ...
    def _sub(raw: str, *, in_url: bool) -> str:
        def _one(match: Any) -> str:
            ...
            raise PackConfigError(
                f"source {entry.name!r} has unknown placeholder {token!r}"
            )
    ...
    if not url.lower().startswith(("http://", "https://")):
        raise PackConfigError(
            f"source {entry.name!r} rendered a non-http(s) url {url[:80]!r}"
        )
...
class ConnectorPackProvider(KnowledgeSourceProvider):
    ...
    async def list_sources(self) -> list[KnowledgeSource]:
        return [
            KnowledgeSource(
                id=s["id"], name=s["name"], source_type=SOURCE_KIND, provider=self.name
            )
    ...
    async def _fetch(
        self, url: str, *, method: str, headers: dict[str, str], policy: Any
    ) -> Any:
        ...
        if self._fetch_fn is not None:
            return await self._fetch_fn(
                url, policy=policy, method=method, headers=headers
            )
    ...
    async def poll(
        ...
        if not ok:
            return SourcePollResult(
                cursor=cursor, error=err, health_status=HEALTH_DEGRADED
            )
        try:
            ...
            url, method, headers = render_fetch(
                entry, args, secret_resolver=self._secret_resolver
            )
        except PackConfigError as exc:
            return SourcePollResult(
                cursor=cursor, error=str(exc), health_status=HEALTH_DEGRADED
            )
        ...
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 — an egress denial is a soft poll failure
            return SourcePollResult(
                cursor=cursor,
        ...
        new_cursor = (
            conditional_get.encode(
                conditional_get.validators_from(getattr(resp, "headers", {}))
            )
            or cursor
        )
        ...
        try:
            ...
            logger.warning(
                "pack %s parse failed (%s): %s", self._name, exc.code, exc.detail
            )
            return SourcePollResult(
                cursor=cursor,
        ...
        try:
            items = [
                SourceItem(**normalize_row(row, i)) for i, row in enumerate(parsed.rows)
            ]
        except ParseFailure as exc:
            return SourcePollResult(
                cursor=cursor,
...
def connector_pack_provider(
    app_ref: str | Path, config: dict | None = None, **kwargs: Any
) -> Any:
    ...
    if timeout not in (None, ""):
        try:
            ...
            logger.warning(
                "connector pack %s: bad parse_timeout_secs %r", app_dir.name, timeout
            )
