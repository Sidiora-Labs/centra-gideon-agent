...
_END_PREFIX = "__GIDEON_PACK_END__"
...
_END_PREFIX = "__GIDEON_PACK_END__"
_DENIED = frozenset(__GIDEON_DENIED_MODULES__)
...
def harness_source() -> str:
    ...
    return _PARSE_HARNESS_SRC.replace("__GIDEON_DENIED_MODULES__", denied)


def _require_script(script: Path) -> None:
    if script.suffix != ".py":
        raise ParseFailure(
            ParseFailure.UNRUNNABLE, f"parse script must be a .py file: {script}"
        )
...
def run_parse_script(
    ...
    *,
    ...
    cfg_fd, cfg_path = tempfile.mkstemp(prefix="gideon-pack-cfg-", suffix=".json")
    harness_fd, harness_path = tempfile.mkstemp(prefix="gideon-pack-run-", suffix=".py")
    out_fd, out_path = tempfile.mkstemp(prefix="gideon-pack-out-", suffix=".jsonl")
    os.close(out_fd)
    ...
    try:
        ...
        if fence_status == "unrunnable":
            raise ParseFailure(
                ParseFailure.UNRUNNABLE, fence_detail or "script unreadable"
            )
        if fence_detail:
            raise ParseFailure(
