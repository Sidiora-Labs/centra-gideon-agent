        ...
        if t == "number":
            if isinstance(raw, bool):
                raise PromptRenderError(
                    f"variable {var.name!r} expects number, got bool"
                )
        ...
        if t == "boolean":
            ...
            if isinstance(raw, str):
                ...
                if low in ("false", "no", "0"):
                    return False
            raise PromptRenderError(
                f"variable {var.name!r} expects boolean, got {raw!r}"
            )
        if t == "select":
            s = str(raw)
...
BUILT_IN_FUNCTIONS: dict[str, Callable[..., Any]] = {
    ...
    "truncate": lambda s, n=80: (
        str(s) if len(str(s)) <= int(n) else str(s)[: int(n)] + "…"
    ),
    ...
    "slice": lambda xs, start, end=None: (
        (list(xs)[int(start) :] if end is None else list(xs)[int(start) : int(end)])
        if xs
        else []
    ),
    ...
    "map": lambda xs, prop: [
        (x.get(prop) if isinstance(x, dict) else None) for x in (xs or [])
    ],
    ...
    if isinstance(coll, (list, tuple)):
        for el in coll:
            ...
            if isinstance(el, dict) and (
                el.get("id") == item or el.get("name") == item
            ):
                return True
        return False
    try:
...
def _eval_expr(expr: str, ctx: dict[str, Any]) -> Any:
    ...
    if m:
        ...
        args = (
            [_eval_expr(a, ctx) for a in _split_args(argstr)] if argstr.strip() else []
        )
        try:
            return fn(*args)
        except PromptRenderError:
...
def _eval_comparison(cond: str, ctx: dict[str, Any]) -> bool:
    ...
    left, op, right = (
        _eval_expr(m.group(1), ctx),
        m.group(2),
        _eval_expr(m.group(3), ctx),
    )
    try:
        if op == "==":
            return left == right
...
def _extract_block(
    text: str, start: int, open_kw: str, close_kw: str
) -> tuple[str, int]:
...
def _render_exprs(text: str, ctx: dict[str, Any]) -> str:
    ...
    def repl(m: re.Match[str]) -> str:
        ...
        if (
            val is None
            and not is_func
            and _lookup(expr, ctx) is None
            and not _is_known(expr, ctx)
        ):
            return m.group(0)
...
_WS_OPEN_RE = re.compile(
    r"[ \t\r\n]+(\{[{%#])-"
)  # whitespace before an opener whose marker is '-'
_WS_CLOSE_RE = re.compile(r"-([}%#]\})[ \t\r\n]+")
_WS_OPEN_BARE_RE = re.compile(r"(\{[{%#])-")
_WS_CLOSE_BARE_RE = re.compile(r"-([}%#]\})")
...
def _build_value_ctx(
    variables: list[PromptVariable], values: dict[str, Any]
) -> dict[str, Any]:
