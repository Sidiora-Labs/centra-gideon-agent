...
class NativePromptProvider(PromptProvider):
    ...
    def update_prompt(self, name: str, prompt: PromptTemplate) -> PromptTemplate:
        ...
        if prompt.name != name:
            raise ValueError(
                "renames are not supported in update_prompt; delete and re-create"
            )
    ...
    def update_snippet(self, name: str, snippet: PromptSnippet) -> PromptSnippet:
        ...
        existing_path.write_text(
            _yaml_dumps(_snippet_payload(snippet)), encoding="utf-8"
        )
        return snippet
...
def seed_bundled_system_prompts() -> None:
    ...
    src_dir = package_path("core", "config") / "prompts"
...
def seed_bundled_app_prompts() -> None:
    ...
    for entry in sorted(BUNDLED_DIR.iterdir()):
        ...
        try:
            ...
            logger.debug(
                "bundled app %s: prompt seed failed", entry.name, exc_info=True
            )
...
def seed_bundled_snippets() -> None:
    ...
    if os.environ.get("GIDEON_SKIP_PROMPT_SEED"):
        return
    src_dir = package_path("core", "config") / "prompt_snippets"
    for entry in BUNDLED_SNIPPETS:
        ...
        try:
            content = (
                (src_dir / entry.filename).read_text(encoding="utf-8").rstrip("\n")
            )
        except OSError:
            continue
        ...
        payload["bundled_sha"] = (
            bundled_sha  # stamp for the pristine-check on next seed
        )
        try:
