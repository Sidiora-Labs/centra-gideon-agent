...
from gideon.integrations.mcp_core import _resolve_session_key
...
def _session_bound_project_id() -> str:
    ...
    **Out-of-process callers only, and the guard is load-bearing.** The in-process native
    ...
    """
    from gideon.integrations.mcp_core import _CURRENT_SESSION_KEY, _get
    ...
    return [
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "content": {
                        "type": "string",
                        "description": "Artifact body (inline)",
                    },
                    "content_file": {
                        "type": "string",
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    "prompt": {
                        "type": "string",
                        "description": "What to generate / how to edit",
                    },
                    "size": {
                        "type": "string",
    ...
    try:
        ...
        if name == "artifact_delete":
            ...
            return (
                ...
                if ok
                else f"Artifact not found: {args['slug']}"
            )
        ...
        if name == "document_formats":
            from gideon.workspace.documents import available_formats

            fmts = available_formats()
            _audit("success")
            return "Available document formats: " + (
                ", ".join(fmts) if fmts else "none"
            )
...
def _image_generate(
    prov: Any, args: dict[str, Any], sk: str | None, _audit: Any
) -> str:
    ...
    try:
        if edit_slug:
            ...
            if src is None or src.kind != "image":
                ...
                return (
                    f"Error: {edit_slug!r} is not an existing image artifact to edit."
                )
            raw = prov.raw_bytes(edit_slug)
            if raw is None:
                return f"Error: could not read source image {edit_slug!r}."
            src_bytes, src_mime = raw
            from gideon.workspace.artifacts.models import ext_for_mime
    ...
    if edit_slug:
        art = prov.update_binary(
            edit_slug, data=data, mime=mime, actor="agent", session_id=sk
        )
        if art is None:
...
def regenerate_image_at_slug(
    ...
    if existing is not None and existing.kind == "image":
        art = prov.update_binary(
            slug, data=data, mime=mime, actor="user", session_id=session_id
        )
    else:
        art = prov.create_binary(
            name=prompt[:60],
...
def _visualize(args: dict[str, Any], _audit: Any) -> str:
    ...
    try:
        ...
        Exception
    ) as e:  # noqa: BLE001 — a model/provider failure is a caller-facing refusal
...
def _bullet(entry: Any) -> Any:
    ...
    """
    from gideon.workspace.documents.model import Bullet

    if isinstance(entry, dict):
        return Bullet(
            text=str(entry.get("text") or ""), level=int(entry.get("level") or 0)
        )
    ...
    if name == "deck_create":
        ...
        if markdown.strip():
            model: Any = deck_from_markdown(
                markdown, title=str(args.get("title") or "")
            )
        ...
        if isinstance(sheets, dict) and sheets:
            model = SheetModel.from_rows(
                {str(k): list(v or []) for k, v in sheets.items()}
            )
