...
own — S90's executor + `DelegationSupervisor.spawn` do. So an immediate run routes through the gateway's
...
def _list_tools() -> list[dict[str, Any]]:
    ...
    trigger_id = {
        "type": "string",
        "description": "The automation id (e.g. 'file:my-notes').",
    }
    return [
        {
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "dry_run": {
                        "type": "boolean",
                        "description": "Observe without executing.",
                    },
                },
                "required": ["id"],
            },
...
def _http_runner(payload: dict[str, Any]) -> Any:
    ...
    try:
        ...
        Exception
    ) as exc:  # noqa: BLE001 - a failed dispatch is a reported outcome, not a crash
        logger.debug(
            "automation_run HTTP dispatch failed for %s", trigger_id, exc_info=True
        )
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "automation_create":
        ...
        result = T.history(
            store, trigger_id=str(args.get("id") or ""), n=int(args.get("n") or 10)
        )
        ...
        result = T.delete_all(
            store, created_by="agent", confirm=bool(args.get("confirm"))
        )
    else:
