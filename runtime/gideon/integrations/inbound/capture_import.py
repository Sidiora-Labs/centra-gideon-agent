...
_READ_TOOLS = frozenset(
    {"read", "grep", "glob", "ls", "notebookread", "readfile", "viewfile"}
)
...
def _paths_from_args(name: str, args: Any) -> tuple[list[str], list[str]]:
    ...
    if not isinstance(args, dict):
        return [], []
    found = [
        str(args[k]) for k in _PATH_KEYS if isinstance(args.get(k), str) and args[k]
    ]
    if not found:
        return [], []
...
def _tool_call(name: str, args: Any, *, ok: bool | None = None) -> dict[str, Any]:
    ...
    if isinstance(args, str):
        ...
        try:
            clipped = _clip(
                json.dumps(args, sort_keys=True, default=str), ARGS_CLIP_CHARS
            )
...
def adapt_jsonl(text: str) -> ParseResult:
    ...
    for lineno, raw in enumerate(text.splitlines(), start=1):
        ...
        if not isinstance(kind, str) or not kind:
            out.skip(
                f"line {lineno}: entry has no 'type' field, cannot tell user from assistant"
            )
            continue
        ...
        if not message:
            out.skip(
                f"line {lineno}: assistant entry has no 'message' object to read a turn from"
            )
            continue
...
def _entries_from_json(doc: Any) -> tuple[list[Any], str | None]:
    ...
    return (
        [],
        f"file is JSON but its top level is {type(doc).__name__}, not a list or object",
    )
...
def _absorb_sse_payload(
    stream: _SSEStream, payload: dict[str, Any], event: str
) -> None:
...
def _remember(
    content_hash: str, entry: dict[str, Any], home: Path | str | None = None
) -> None:
    path = ledger_path(home)
...
def resolve_import_file(
    name: str, home: Path | str | None = None
) -> tuple[Path | None, str]:
...
def _resolve_stage(
    ...
    if stage is not None:
        return stage
    from gideon.integrations.inbound.capture_store import stage_records
    ...
    *,
    ...
    if not parsed.records:
        if not parsed.reasons:
            report["reasons"].append(
                f"no records found in {path.name}: the file is empty or holds no "
                f"{fmt} turns"
            )
        return report
