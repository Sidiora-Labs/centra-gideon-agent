...
def rate_key(surface: str, client_id: str, peer_fallback: str = "") -> str:
    ...
    *before* identity is known, so an unauthenticated flood is still bounded.
    """
    return (
        f"{surface}|client:{client_id}"
        if client_id
        else f"{surface}|peer:{peer_fallback}"
    )
...
def caps_for(client=None) -> Caps:
    ...
    rps, burst, concurrent = (
        DEFAULT_CAPS.rps,
        DEFAULT_CAPS.burst,
        DEFAULT_CAPS.concurrent,
    )
    try:
        ...
        rps, burst, concurrent = (
            float(ea.rate_rps),
            int(ea.rate_burst),
            int(ea.rate_concurrent),
        )
    except Exception:  # noqa: BLE001 — unreadable config keeps the module constants
        logger.debug(
            "inbound: cap config unreadable; using module defaults", exc_info=True
        )
    overrides = (
        dict(getattr(client, "rate_overrides", None) or {})
