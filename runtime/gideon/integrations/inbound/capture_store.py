...
:func:`gideon.security.security.redact_credentials` and (b) wrapped by
:func:`gideon.security.security.fence_untrusted` with ``source="capture:<client_id>"``.
...
from gideon.security.security import fence_untrusted, redact_credentials
...
def session_id_for(client_id: str, body: dict) -> str:
    ...
    safe_client = (
        "".join(c if c.isalnum() or c in "-_" else "-" for c in client_id) or "unknown"
    )
...
def _extract_tool_calls(
    response_body: dict | None, request_body: dict
) -> list[_ToolCall]:
...
def _hash_record(record: dict) -> str:
    ...
    return hashlib.sha256(
        _dumps(
            {k: v for k, v in record.items() if k not in ("ts", "record_hash")}
        ).encode("utf-8")
    ).hexdigest()
...
def _build_record(
    *,
    ...
    for call in calls:
        ...
        tool_rows.append(
            {"name": name_clean, "args_clipped": args_clean, "ok": call.ok}
        )
...
def _stage_capture(
    record: dict, sidecar: dict, *, session_id: str, client_id: str
) -> int:
...
def record_turn(
    *,
    ...
    user's own coding agent failing on a request Gideon merely observed — a strictly
...
def stage_records(records: list[dict], *, source: str) -> dict:
    ...
    for raw in records:
        ...
        if isinstance(raw_request, dict):
            ...
            response_body = (
                {"choices": [{"message": {"content": response}}]} if response else None
            )
            overlay = raw
        ...
        try:
            ...
            record, sidecar = _build_record(
                ...
                tokens=(
                    raw.get("tokens") if isinstance(raw.get("tokens"), dict) else None
                ),
...
def prune(retention_days: int | None = None) -> int:
    ...
    if retention_days is None:
        try:
            ...
            retention_days = int(
                AppConfig.load().external_access.capture.retention_days
            )
        except Exception:
