...
def surfaces() -> tuple[str, ...]:
    ...
    from gideon.core.config.external_access import EXTERNAL_ACCESS_SURFACES
...
def load_surface_token(surface: str) -> str | None:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 — an unreadable store means "no token", i.e. no mount
...
def inbound_cmd(args) -> int:
    ...
    if action == "confirm":
        from gideon.integrations.inbound.bridge import confirm_cli
