...
from gideon.security.net.policy import LISTED, EgressPolicy
...
UPSTREAM_KEY_HEADER = "X-Gideon-Upstream-Key"
UPSTREAM_BASE_HEADER = "X-Gideon-Upstream-Base-Url"
...
def _auth_headers(dialect: str, secret: str, request: web.Request) -> dict[str, str]:
    ...
    holds OUR capture bearer, and forwarding it would hand a Gideon credential to a third
...
def _provider_upstream(name: str) -> tuple[str, str]:
    ...
    if cred is None and spec is not None:
        explicit = str(options.get("api_key", "") or "") or str(
            options.get("apiKey", "") or ""
        )
...
class UpstreamRedirected(Exception):
    ...
    def __init__(self, status: int, location: str) -> None:
        super().__init__(
            f"upstream answered {status} to {location!r}; redirects are not followed"
        )
        self.status = status
...
def _recorder() -> Callable[..., Awaitable[str]] | None:
    ...
    """
    try:
        from gideon.integrations.inbound.capture_store import record_turn_async

        return record_turn_async
    except Exception:  # noqa: BLE001
        logger.debug(
            "capture: capture_store is unavailable; not recording", exc_info=True
        )
        return None
...
async def _record(**kwargs: Any) -> None:
    ...
    if record is None:
        logger.warning(
            "capture: recorded nothing — capture_store.record_turn_async is unavailable"
        )
        return
    try:
        await record(**kwargs)
...
async def _handle(request: web.Request, dialect: str, route: str) -> web.StreamResponse:
    ...
    if upstream is None:
        audit(
            CAPTURE_SURFACE, route=route, status=502, refused=why, client_id=client_id
        )
        return json_error(
            "upstream_unavailable", status=502, error_extra={"reason": why}
        )
    ...
    try:
        ...
        logger.warning(
            "capture: refused to follow upstream redirect to %r", exc.location
        )
        audit(
            CAPTURE_SURFACE,
            route=route,
        ...
        if sent is not None:
            ...
            return sent
        return json_error(
            "upstream_failed", status=502, error_extra={"reason": str(exc)}
        )
    ...
    if stream and sent is not None:
        ...
        result = web.Response(
            ...
            status=reply.status,
            headers={
                k: v for k, v in reply.headers.items() if k.lower() != "content-type"
            },
            content_type=reply.headers.get("Content-Type", "application/json").split(
                ";"
            )[0],
        )
...
async def handle_import(request: web.Request) -> web.StreamResponse:
    ...
    route calls the same :func:`~gideon.integrations.inbound.capture_import.import_capture_file`
    ...
    :func:`~gideon.integrations.inbound.capture_import.imports_dir` and nothing else. See
    ...
    try:
        report = await asyncio.to_thread(
            import_capture_file, path, fmt=fmt, source=source
        )
    except (
        Exception
    ) as exc:  # noqa: BLE001 — a store fault is reported, never swallowed
