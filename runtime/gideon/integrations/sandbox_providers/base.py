...
* the OS-level filesystem sandbox (:func:`gideon.security.sandbox.wrap_argv` — macOS seatbelt /
  Linux user-namespace / none), which HIDES sensitive paths from an agent-influenced child; and
* the post-exec resource ceilings (:class:`gideon.security.sandbox.ResourceCeilings` delivered by
...
The in-core ``none`` provider (:mod:`gideon.integrations.sandbox_providers.none`) composes the two
...
app registering through :class:`gideon.extensions.providers.registry.SandboxTypeHandler`.
"""
...
from gideon.security.sandbox import PROFILE_SESSION_HOST, ResourceCeilings
...
class SandboxSpec:
    ...
    ``mode`` is the :func:`gideon.security.sandbox.wrap_argv` level: ``"auto"``/``"standard"``
