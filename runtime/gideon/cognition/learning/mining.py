...
class MatchSet:
    ...
    ``matches`` is exactly what :func:`gideon.cognition.learning.detectors.similarity_verdict` consumes.
...
def miss_counts(*, days: int = 30) -> dict[str, int]:
    ...
    try:
        ...
        with (
            store._cursor() as cur
        ):  # noqa: SLF001 — same-package read of the store's connection
            rows = cur.execute(
...
def _step_names(run: Any, *, journal: Any = None) -> list[str]:
    ...
    if journal is None:
        return []
    try:
        events = journal.ledger(
            str(getattr(run, "id", "") or ""), kinds={journal.STEP_COMPLETED}
        )
    except Exception:
        logger.debug(
            "mining: ledger read failed for %s", getattr(run, "id", "?"), exc_info=True
        )
        return []
    seen: list[str] = []
    for rec in events:
...
def invert_intent(run: Any, *, journal: Any = None) -> Inversion:
    ...
    steps = _step_names(run, journal=journal)
    name = str(getattr(run, "workflow_name", "") or "").strip()
    did = f"ran {name or 'a workflow'}: " + (
        ", ".join(steps) if steps else "no steps completed"
    )
    synthesized = _synthesize_intent(name, steps)
