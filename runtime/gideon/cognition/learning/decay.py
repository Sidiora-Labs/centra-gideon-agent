...
from gideon.cognition.learning.admission_policy import AgeProfile
...
def effective_lambda(kind: str, importance: float = 0.0) -> float:
    ...
    return AgeProfile.rate(sys.modules[__name__], kind, importance)
...
def strength(
    *,
    ...
    clock exist at all.
    """
    elapsed = max(0.0, float(active_days_since_use))
    exponent = -effective_lambda(kind, importance)
    return math.exp(exponent * elapsed)
...
def reinforcement_weight(secs_since_last: float) -> float:
    ...
    if secs_since_last < 0 or not secs_since_last < REINFORCE_WINDOW_SECS:
        return 1.0
    return REINFORCE_DAMPED_WEIGHT
...
def evaluate(
    *,
    ...
    """
    level = strength(
        kind=kind, active_days_since_use=active_days_since_use, importance=importance
    )
    return AgeProfile.verdict(
...
def active_days_between(
    active_dates: list[str], since: str, until: str | None = None
) -> float:
    ...
    """
    return AgeProfile.active_days(sys.modules[__name__], active_dates, since, until)
...
def _parse_date(raw: str):
    value = raw if "T" in str(raw) else f"{raw}T00:00:00+00:00"
    timestamp = _parse(value)
    return None if not timestamp else timestamp.date()
