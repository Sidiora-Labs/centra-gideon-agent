...
**It never touches** :mod:`gideon.assurance.evals.runner`. That runner is the right tool for the
...
:func:`~gideon.integrations.llm_helpers.one_shot_completion` for the arms and
:class:`~gideon.assurance.eval.judge.LLMJudge` for the score — the same composition
...
:class:`~gideon.security.guardrails.budgets.Budget` into the two ContextVars
:class:`~gideon.security.guardrails.model_call.ModelCallGuard` reads, and the guard — which
...
def neutral_margin() -> float:
    ...
    from gideon.cognition.learning.refiner import MIN_TARGET_IMPROVEMENT
...
def mine_cases(
    *, max_sessions: int = 8, limit: int = MAX_CASES_PER_PROPOSAL
) -> list[ReplayCase]:
    ...
    for path in _session_files(root)[: max(0, max_sessions)]:
        ...
        for record in sorted(
            records, key=lambda r: float(r.get("ts") or 0.0), reverse=True
        ):
...
def unreplayed(reason: str, **extra: Any) -> ReplayReport:
    ...
    """
    report = ReplayReport(
        state=REPLAY_UNREPLAYED, reason=reason, verdict=VERDICT_UNMEASURED
    )
...
def finalize(report: ReplayReport) -> ReplayReport:
    ...
    report.candidate_mean = _mean(
        [c.candidate for c in scored if c.candidate is not None]
    )
...
def summary(report: dict | None) -> dict[str, Any]:
    ...
    state = (
        state if state in (REPLAY_REPLAYED, REPLAY_UNREPLAYED) else REPLAY_UNREPLAYED
    )
    ...
    return {
        ...
        "reason": str(
            data.get("reason")
            or (UNREPLAYED_NOT_RUN if state != REPLAY_REPLAYED else "")
        ),
        ...
        "tool_free": sum(
            1 for c in cases if isinstance(c, dict) and c.get("tool_free")
        ),
...
def criteria_for(prop: Any) -> str:
    ...
    predicted = [
        str(p) for p in (manifest.get("predicted_fixes") or []) if str(p).strip()
    ]
    lines = [
    ...
    if predicted:
        lines.append(
            "The change under test predicts it will: " + "; ".join(predicted[:5]) + "."
        )
...
async def _score(
    judge: Any, *, description: str, criteria: str, prompt: str, answer: str
):
...
async def _run_case(
    ...
    *,
    ...
    description = f"A captured turn from the user's own coding session, replayed to test a proposed {kind}."
    try:
        baseline_answer = await completion(case.prompt, use_case="background")
        candidate_answer = await completion(
            _candidate_prompt(
                case.prompt, str(getattr(prop, "body", "") or ""), kind=kind
            ),
            use_case="background",
        )
    ...
    if not (baseline_answer or "").strip() or not (candidate_answer or "").strip():
        score.rejected = True
        score.reason = (
            "an arm returned an empty completion, so there is nothing to score"
        )
        return score
    ...
    if _is_parse_failure(base_v) or _is_parse_failure(cand_v):
        score.rejected = True
        score.reason = "the judge's response could not be parsed, so the case scores 0 and is rejected"
        return score
...
def replay_budget() -> float:
    ...
    try:
        ...
        return float(
            getattr(AppConfig.load().learning, "replay_max_dollars", 0.0) or 0.0
        )
    except Exception:
        logger.debug(
            "replay: could not read learning.replay_max_dollars", exc_info=True
        )
        return 0.0
...
def replay_enabled() -> bool:
    ...
    try:
        ...
        return bool(getattr(cfg, "enabled", True)) and bool(
            getattr(cfg, "replay_enabled", False)
        )
    except Exception:
...
async def replay_proposal(
    ...
    *,
    ...
    :class:`~gideon.assurance.eval.judge.LLMJudge` over the ``eval_judge`` binding, so a test can
    ...
    try:
        for case in cases[:MAX_CASES_PER_PROPOSAL]:
            try:
                ...
                logger.info(
                    "replay: learning replay budget exhausted; deferring the rest"
                )
...
async def run_pass(*, max_proposals: int = 3) -> dict[str, Any]:
    ...
    for prop in pending:
        try:
            ...
            logger.warning(
                "replay pass: failed for %s", getattr(prop, "id", "?"), exc_info=True
            )
            continue
