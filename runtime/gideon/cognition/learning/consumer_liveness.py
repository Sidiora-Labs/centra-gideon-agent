...
1. :func:`measure_consumption` — the ground truth for a :data:`~gideon.assurance.ledger.outcomes.
...
publishes, runs at least :data:`~gideon.assurance.ledger.outcomes.DORMANCY_CYCLES` times, and whose
...
def measure_consumption(question: outcomes.OutcomeQuestion) -> float | None:
    ...
    :data:`~gideon.assurance.ledger.outcomes.CONSUMED` (1.0) if a consumer touched the artifact after
    the question was opened, :data:`~gideon.assurance.ledger.outcomes.UNCONSUMED` (0.0) if not, and
    ...
    """
    slug = str(question.record.get("slug") or "") or outcomes.slug_from_metric(
        question.metric
    )
    if not slug:
        return None
    ...
    try:
        ...
        logger.debug(
            "consumer-liveness: artifact read failed for %r", slug, exc_info=True
        )
        return None
...
def sweep(
    *, max_runs: int = _MAX_RUNS, cycles: int = outcomes.DORMANCY_CYCLES
) -> dict[str, int]:
    ...
    for run in runs:
        try:
            ...
            logger.debug(
                "consumer-liveness: ledger read failed for %s", run.id, exc_info=True
            )
            continue
        ...
        for event in events:
            ...
            if event.get("kind") == PENDING_OUTCOME:
                if event.get("producer") == outcomes.PRODUCER_PUBLISH:
                    horizons.setdefault(unit, []).append(
                        float(event.get("horizon_secs") or 0.0)
                    )
                continue
    ...
    for unit, resolutions in sorted(by_unit.items()):
        ...
        try:
            ...
            logger.debug(
                "consumer-liveness: proposal failed for %s", unit, exc_info=True
            )
    return report
