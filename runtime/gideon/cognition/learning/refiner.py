...
DIFF_OPS: frozenset[str] = frozenset(
    {"update_node", "insert", "delete", "move", "set_input"}
)
...
def risk_tier(ops: list[dict[str, Any]]) -> str:
    ...
    """
    tiers = [
        _OP_RISK.get(str(op.get("op", "")), RiskTier.MANUAL_ONLY.value)
        for op in ops or []
    ]
    if not tiers:
...
) -> tuple[list[dict[str, Any]], list[Screened]]:
    ...
    for event in events or []:
        ...
        for field_name in UNTRUSTED_EVIDENCE_FIELDS:
            ...
            try:
                ...
                Exception
            ):  # pragma: no cover - the screen is defensive; so is its caller
                continue
            if verdict.blocked:
                blocked = True
        ...
        verdicts.append(
            Screened(event=event, blocked=False, matched_group=worst, fenced=False)
        )
...
def cluster_safely(
    events: list[dict[str, Any]],
) -> tuple[list[Cluster], list[Screened]]:
...
def cluster_failures(events: list[dict[str, Any]]) -> list[Cluster]:
    ...
    for event in events or []:
        ...
        if kind in ("step_failed", "run_abandoned", "gate_rejected"):
            ...
            ops = event.get("ops")
            rendered = (
                ", ".join(sorted(str(o) for o in ops))
...
class CriticScore:
    ...
    def total(self) -> float:
        ...
        return sum(float(self.scores.get(name, 0.0)) for name in CHECK_SCORES) / len(
            CHECK_SCORES
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "scores": {
                name: round(float(self.scores.get(name, 0.0)), 4)
                for name in CHECK_SCORES
            },
...
def evaluate_diff(
    *,
    ...
    if not verdict.accepted:
        return Decision(
            surfaced=False, tier=tier, refusals=[verdict.reason], critic=verdict
        )
