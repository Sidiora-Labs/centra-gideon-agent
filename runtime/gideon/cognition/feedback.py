    ...
    *,
    ...
    try:
        if target_kind not in TARGET_KINDS:
            logger.warning(
                "record_feedback: unknown target_kind %r — dropped", target_kind
            )
            return None
        ...
        if producer_kind and producer_kind not in PRODUCER_KINDS:
            logger.warning(
                "record_feedback: unknown producer_kind %r — dropped", producer_kind
            )
            return None
        rec = FeedbackRecord(
...
def _broadcast(rec: FeedbackRecord, state=None) -> None:
    ...
    ConsoleState the route handler carries; None (SDK/core callers with no
...
def producer_stats(*, window_days: int | None = None) -> dict[tuple[str, str], dict]:
    ...
    for rec in _load_index().values():
        if rec.created_at < cutoff or not rec.producer_kind or not rec.producer_id:
            continue
        row = out.setdefault(
            (rec.producer_kind, rec.producer_id),
            {"ups": 0, "downs": 0, "n": 0, "accuracy": 0.0},
        )
...
def _settings() -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning(
            "feedback settings unreadable — suppressing nothing", exc_info=True
        )
        return {}
...
def suppressed_producers(
    *, threshold: float | None = None, min_n: int | None = None
    ...
    try:
        ...
        logger.warning(
            "suppressed_producers failed — suppressing nothing", exc_info=True
        )
        return set()
