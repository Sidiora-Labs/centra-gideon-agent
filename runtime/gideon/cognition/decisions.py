...
def default_horizon_days() -> int:
    ...
    try:
        ...
        logger.debug(
            "decision journal: could not read the horizon default", exc_info=True
        )
        return 90
...
def log_decision(
    *,
    ...
    if not 0.0 <= conf <= 1.0:
        raise DecisionError(
            f"confidence {conf!r} is out of range — it must be between 0 and 1"
        )
    dom = (domain or "other").strip().lower()
    if dom not in DECISION_DOMAINS:
        raise DecisionError(
            f"unknown domain {dom!r} — one of {', '.join(DECISION_DOMAINS)}"
        )
    horizon = (review_horizon or "").strip() or horizon_from_days(
        default_horizon_days(), now=now
    )
...
def reschedule_review(
    ...
    *,
    ...
    _write_meta(
        ks, item_id, {"review_horizon": review_horizon, "reminder_trigger_id": tid}
    )
    return projection(_require_decision(ks, item_id))


def abandon_decision(
    item_id: str, *, store: Any = None, trigger_store: Any = None
) -> dict:
...
def _lesson_key_for(memory: Any, rule: str) -> str | None:
    ...
    try:
        ...
        logger.debug(
            "decision journal: could not read the lesson key back", exc_info=True
        )
    return None
...
def resolve_decision(
    ...
    *,
    ...
    if g not in RESOLUTION_GRADES:
        raise DecisionError(
            f"unknown grade {g!r} — one of {', '.join(RESOLUTION_GRADES)}"
        )
    ...
    try:
        ...
        logger.warning(
            "decision journal: lesson write failed for %s", item_id, exc_info=True
        )
    if key:
...
def _reingest(item_id: str, enqueue: Any) -> None:
    ...
    gateway's ``ConsoleState``, and a core module that reached for it would either import
    ...
    try:
        ...
        logger.debug(
            "decision journal: re-enqueue skipped for %s", item_id, exc_info=True
        )
