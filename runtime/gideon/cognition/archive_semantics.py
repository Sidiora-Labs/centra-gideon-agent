...
_DENIED_PREFIXES = (
    "lesson.", "slot.", "user.procedural.", "user.commitment.",
    "user.persona.", "user.approval.",
)
_PRIVATE_FIELDS = {
    "credential", "credential_ref", "access_token", "refresh_token",
    "client_secret", "private_key", "password", "secret", "legal_name",
    "full_name", "birth_date", "date_of_birth", "ssn", "passport",
    "government_id", "email", "phone", "address", "biometric", "genome",
}
...
class SemanticImport:
    ...
    def apply(self, record):
        ...
        if record.kind is not MemoryKind.SEMANTIC:
            return self.rejected("kind_rejected", "Only semantic memory records may be imported")
        if (not record.id.startswith(_REPLICATION_PREFIXES)
                or record.id.startswith(_DENIED_PREFIXES)):
            return self.rejected("key_rejected", "Semantic key is outside the replication policy")
        key_parts = set(record.id.split("."))
        if key_parts & _PRIVATE_FIELDS:
            return self.rejected("private_identity", "Private identity memory is not replicable")
        derived = (
            record.embedding is not None or record.recall_count or record.visit_count
            or record.last_accessed_at is not None or record.superseded_by is not None
            or record.invalidated_at is not None or record.extra or record.safe_to_act is not None
            or record.due_window is not None or record.channel is not None
            or record.dismissed_at is not None or record.source_ref is not None
            or record.conversation_id or record.tags
        )
        if derived:
            return self.rejected("derived_state", "Derived or execution memory state is not replicable")
        if record.tier is not MemoryTier.SEMANTIC or record.scope_ref is not None:
            return self.rejected("authority_rejected", "Memory reach authority is not replicable")
        value = record.value if record.value is not None else record.text
        if _private(value) or _private(record.source) or _private(record.category or ""):
            return self.rejected("private_data", "Credential or private identity data is not replicable")
        try:
        ...
        if updated < created:
            return self.rejected("invalid_timestamp", "Semantic update predates creation")
        try:
            encoded = json.dumps(value)
        except (TypeError, ValueError):
            return self.rejected("invalid_value", "Semantic value is not JSON serializable")
        rejection = self.archive.validate_semantic(
            record.id, value, record.confidence, record.source, value_json=encoded,
        )
        ...
        try:
            ...
            previous = self.archive.db.execute(
                "SELECT * FROM semantic_memory WHERE key = ?", (record.id,),
            ).fetchone()
            mutation = SemanticMutation(
                self.archive, record.id, encoded, record.confidence, record.source,
            )
            mutation.previous = previous
            conflict = mutation.conflict()
            if conflict is not None:
                self.archive.db.rollback()
                return self.rejected(self.api.SemanticRejectCode.CONFLICT.value, conflict)
            ...
            event = "import_delete" if record.is_deleted else ("import_update" if previous else "import_create")
            try:
                self.archive.db.execute(
                    ...
                    (record.id, encoded, record.confidence, record.source, record.created_at,
                     record.updated_at, int(record.is_deleted), scope, record.category),
                )
                self.archive.db.execute(
                    "INSERT INTO memory_events(event_type,memory_type,memory_key,old_value,new_value,source,created_at) VALUES(?,?,?,?,?,?,?)",
                    (event, "semantic", record.id, prior_value, None if record.is_deleted else encoded,
                     record.source, self.api._now_iso()),
                )
