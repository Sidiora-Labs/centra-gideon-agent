...
Gideon assembles a turn's context monolithically in ``context.PromptAssembler``
...
- The DEFAULT engine is a thin delegate over the existing ``PromptAssembler`` — so
...
class DefaultContextEngine:
    """The built-in engine: a thin delegate over ``PromptAssembler.build_message``.
...
def active_recall_block(
    builder: "PromptAssembler", text: str, *, cwd: str | None, memory_store: str | None
) -> str:
...
def _recent_turns(builder: "PromptAssembler", session_key: str) -> list[str]:
    ...
    """
    from gideon.cognition.memory_push import WINDOW_TURNS
    ...
    try:
        msgs = log.recent(
            session_key, max_messages=WINDOW_TURNS, roles={"user", "assistant"}
        )
    except Exception:  # noqa: BLE001
        return []
...
def assemble_context(
    builder: "PromptAssembler", text: str, *, is_new_session: bool, **kwargs: Any
) -> AssembledContext:
...
async def check_headroom(
    assembled: AssembledContext, *, model_ref: str = ""
) -> Headroom:
    ...
    """
    try:
        return await check_for_model(
            headroom_components(assembled), model_ref=model_ref
        )
    except Exception:  # noqa: BLE001 — a headroom check never costs a turn
        logger.debug(
            "context headroom check failed; proceeding unmeasured", exc_info=True
        )
