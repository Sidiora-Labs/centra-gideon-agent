...
:mod:`gideon.cognition.learning_summary` answers "what changed lately" (deltas inside a
...
* **facets** — :func:`gideon.cognition.preference_facets.load_facets`, each carrying its
...
* **lessons** — ``MemoryService.over_vector_store(vs).get_lessons()``. (The plan's
  ...
  lesson.*``.)
* **skills** — the ``auto/`` namespace from ``ProcedureLibrary.list_skills``, with use
...
* **proposals** — :func:`gideon.extensions.skills.proposals.list_pending`.
...
item through :func:`gideon.integrations.inbox.emit_attention_item`, which is the only correct
...
:mod:`gideon.integrations.action_providers.identity_report_provider`, which owns only the cron
...
CADENCE_OFF = "off"
IDENTITY_REPORT_CADENCES: tuple[str, ...] = (
    CADENCE_MONTHLY,
    CADENCE_WEEKLY,
    CADENCE_OFF,
)
...
class ReportSection:
    ...
    @classmethod
    def of(
        cls, rows: list[dict[str, Any]], *, cap: int = _MAX_ITEMS
    ) -> "ReportSection":
...
def _gather_facets(vs: Any, now: datetime) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return []
    rows.sort(
        key=lambda r: (str(r.get("updated_at") or ""), str(r.get("text") or "")),
        reverse=True,
    )
    return rows
...
def _gather_lessons(vs: Any) -> list[dict[str, Any]]:
    ...
    handing it a ``SemanticArchive`` yields ``_vs = None`` and ``get_lessons()``
...
def _gather_skills(cutoff: datetime) -> list[dict[str, Any]]:
    ...
    try:
        ...
        rows = ProcedureLibrary(install_builtins=False).list_skills(
...
def compose_identity_report(
    *,
    ...
    ``vs`` is a :class:`~gideon.cognition.vector_memory.SemanticArchive` (facets and
...
def identity_report_payload(
    *,
    ...
    """
    payload = compose_identity_report(
        window_days=window_days, vs=vs, now=now
    ).to_payload()
...
def render_markdown(report: IdentityReport) -> str:
    ...
    out += [
        f"## Preferences I hold ({report.facets.count}){_sample_note(report.facets)}",
        "",
    ]
    ...
    out += [
        f"## Lessons I follow ({report.lessons.count}){_sample_note(report.lessons)}",
        "",
    ]
    ...
    out += [
        f"## Skills I built ({report.skills.count}){_sample_note(report.skills)}",
        "",
    ]
    if report.skills.items:
        for s in report.skills.items:
            recent = (
                "used this period"
                if s.get("used_in_window")
                else "not used this period"
            )
            out.append(
    ...
    out += [
        f"## Waiting on you ({report.proposals.count}){_sample_note(report.proposals)}",
        "",
    ]
...
def _narrative_facts(report: IdentityReport) -> str:
    ...
    for f in report.facets.items:
        lines.append(
            f"preference ({f.get('cls', '')}, {f.get('state', '')}): {f.get('text', '')}"
        )
    for lesson in report.lessons.items:
    ...
    for s in report.skills.items:
        ...
        lines.append(
            f"skill ({s.get('aging_state', '')}, {recent}): {s.get('name', '')}"
        )
    for p in report.proposals.items:
...
async def narrate_identity_report(report: IdentityReport) -> IdentityReport:
    ...
    :func:`gideon.security.security.fence_untrusted`, because a facet's text is user prose
...
def _write_artifact(markdown: str, *, generated_at: str) -> tuple[str, int]:
    ...
    try:
        if existing is None:
            ...
            return str(getattr(art, "slug", "") or ""), int(
                getattr(art, "version", 1) or 1
            )
        ...
        if updated is None:
            return "", 0
        return str(getattr(updated, "slug", "") or ""), int(
            getattr(updated, "version", 1) or 1
        )
    except Exception:
...
def delivery_dedup_key(report: IdentityReport) -> str:
    ...
    bucket = (
        at.strftime("%G-W%V")
        if report.window_days <= MIN_WINDOW_DAYS
        else at.strftime("%Y-%m")
    )
...
async def deliver_identity_report(
    ...
    **The artifact is written before the item is raised.** That ordering is what makes
    ...
    """
    report = await build_identity_report(
        window_days=window_days, vs=vs, now=now, narrate=narrate
    )
