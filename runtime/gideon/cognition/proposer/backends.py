...
* :class:`SubagentProposerBackend` — a fresh Gideon subagent as the second brain. Zero external
...
class RunnerProposerBackend:
    ...
    async def prepare(self, brief: HandoffBrief) -> PreparedInvocation:
        ...
        baseline = brief.baseline or snapshot_workspace(
            brief.workspace, paths=brief.files_touched
        )
    ...
    async def collect(self, ref: InvocationRef) -> ProposerResult:
        ...
        return normalise(
            ...
            raw_ref=ref.handle,
            error=(
                "" if rc == 0 else f"{self._defn.id} exited {rc}: {err.strip()[:400]}"
            ),
        )


class SubagentProposerBackend:
    """A fresh Gideon subagent as the second brain — the zero-dependency degradation path."""
    ...
    def _resolve_manager(self) -> object | None:
        """The live :class:`DelegationSupervisor`, via the action-service accessor.
