    ...
    *,
    ...
    for runner_id, defn in rows.items():
        ...
        if not ok:
            considered.append(
                Candidate(
                    runner_id=runner_id, eligible=False, reason=why, evidence=evidence
                )
            )
            continue
        considered.append(
            Candidate(
                runner_id=runner_id, eligible=True, reason="eligible", evidence=evidence
            )
        )
        eligible.append(defn)

    order = {
        name.strip().lower(): i for i, name in enumerate(binding_order) if name.strip()
    }
    eligible.sort(key=lambda d: (order.get(d.id.lower(), len(order)), d.id))
    target = eligible[0] if eligible else None
    if target is not None and is_same_runner(
        target.id, exclude_runner
    ):  # pragma: no cover
        target = None
    return Selection(
        target=target, considered=tuple(considered), excluded=exclude_runner or ""
    )
