...
* **Transcript excerpts are FENCED.** Verbatim errors and tool output are exactly the untrusted
  ...
  :func:`gideon.security.security.fence_untrusted` before a second model reads them.
...
from gideon.cognition.proposer.contract import CLAIM_MARKER
from gideon.cognition.proposer.verify import DiskBaseline, snapshot_workspace
from gideon.core.atomic_write import atomic_write
from gideon.security.security import fence_untrusted, redact
...
class HandoffBrief:
    ...
    def render(self) -> str:
        ...
        if self.attempts:
            lines += [
                "## What was already tried (verbatim output from the stalled agent)",
                "",
            ]
