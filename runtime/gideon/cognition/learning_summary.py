...
def _gather_skills(cutoff: datetime) -> tuple[list[str], list[str]]:
    ...
    for row in rows:
        ...
        try:
            ...
            logger.debug(
                "learning summary: overlay read failed for %s", name, exc_info=True
            )
            continue
...
def _gather_pending() -> list[str]:
    ...
    for prop in pending:
        ...
        out.append(
            f"{target or slug} (refine)" if kind == "refine" and target else slug
        )
...
def _gather_facts(vs: Any, cutoff: datetime, now: datetime) -> list[str]:
    ...
    out: list[str] = []
    try:
        from gideon.cognition.preference_facets import facet_state, load_facets
    ...
    try:
        # store on `provider.vector_store`, so handing it a SemanticArchive yields
...
def compose_learning_summary(
    *,
    ...
    ``vs`` is a :class:`~gideon.cognition.vector_memory.SemanticArchive` (facets +
