...
_FIELDS = {"id", "conversation_id", "text", "tags", "importance", "source", "contributor",
...
def _safe(value):
    ...
    if isinstance(value, str):
        ...
        return not redactions and screened == value and _AUTH_HEADER.search(value) is None
...
def validate_authored_episode(value):
    ...
    if (not isinstance(tags, list) or len(tags) > 10
            or any(not isinstance(tag, str) or not tag or len(tag) > 50 or not _safe(tag) for tag in tags)
    ...
    if (updated < created or type(value.get("is_deleted")) is not bool
...
class AuthoredEpisodeImport:
    ...
    def apply(self, record, *, commit=True):
        ...
        row = database.execute(
            ...
            "FROM episodic_memories WHERE id=?", (value["id"],),
        ).fetchone()
        if row is not None:
            origin = database.execute(
                "SELECT source FROM memory_events WHERE memory_type='episodic' AND memory_key=? "
                "AND event_type IN ('create','import') ORDER BY id LIMIT 1", (value["id"],),
            ).fetchone()
            latest = database.execute(
                "SELECT created_at FROM memory_events WHERE memory_type='episodic' AND memory_key=? "
                "AND event_type IN ('create','import','delete') ORDER BY id DESC LIMIT 1", (value["id"],),
            ).fetchone()
            if origin is None or latest is None:
                raise ValueError("Episodic memory identity lacks canonical provenance")
            current = {"id": row["id"], "conversation_id": row["conversation_id"] or "", "text": row["text"],
                       "tags": json.loads(row["tags"] or "[]"), "importance": row["importance"],
                       "source": origin["source"], "contributor": row["contributor"] or "",
                       "created_at": row["created_at"], "updated_at": latest["created_at"],
                       "is_deleted": bool(row["is_deleted"])}
            immutable = {key for key in _FIELDS - {"updated_at", "is_deleted"} if current[key] != value[key]}
            if immutable or (current["is_deleted"] and not value["is_deleted"]):
                raise ValueError("Episodic memory identity already has different immutable content")
            if current["is_deleted"] == value["is_deleted"]:
                if current["updated_at"] != value["updated_at"]:
                    raise ValueError("Episodic memory identity already has different lifecycle state")
                return EpisodeImportResult(True, "already_current")
            database.execute("UPDATE episodic_memories SET is_deleted=1,embedding=NULL,last_accessed_at=NULL WHERE id=?",
                             (value["id"],))
            event_type = "delete"
        else:
            database.execute(
                ...
                "VALUES(?,?,?,NULL,?,?,?,NULL,?,?)",
                (value["id"], value["conversation_id"], value["text"], json.dumps(value["tags"]),
                 value["importance"], value["created_at"], int(value["is_deleted"]), value["contributor"]),
            )
            database.execute(
                "INSERT INTO memory_events(event_type,memory_type,memory_key,old_value,new_value,source,created_at) "
                "VALUES(?,?,?,?,?,?,?)",
                ("import", "episodic", value["id"], None, value["text"][:200],
                 value["source"], value["created_at"]),
            )
        ...
        if event_type is not None:
            database.execute(
                "INSERT INTO memory_events(event_type,memory_type,memory_key,old_value,new_value,source,created_at) "
                "VALUES(?,?,?,?,?,?,?)",
                (event_type, "episodic", value["id"], None, value["text"][:200],
                 value["source"], value["updated_at"]),
            )
        if commit:
            database.commit()
        return EpisodeImportResult(True, "tombstoned" if value["is_deleted"] else "imported")
