...
class ChunkVectorIndex:
        ...
        if self._loaded is None:
            ...
            if not cap.available:
                ...
                try:
                    ...
                    Exception
                ) as exc:  # noqa: BLE001 — never raise into a search/write
    ...
    def sync_item(self, item_id: str, rows: list[tuple[str, bytes | None]]) -> None:
        ...
        for chunk_id, blob in rows:
            ...
            try:
                ...
                Exception
            ) as exc:  # noqa: BLE001 — an index write never fails a chunk write
                logger.debug(
                    "chunk vector index: insert failed for %s: %s", chunk_id, exc
                )
    ...
    def drop_item(self, item_id: str) -> None:
        ...
        try:
            ...
            logger.debug(
                "chunk vector index: id lookup failed for %s: %s", item_id, exc
            )
            return
        ...
        for table in self._tables():
            for chunk_id in chunk_ids:
                try:
                    ...
                    logger.debug(
                        "chunk vector index: delete failed for %s: %s", chunk_id, exc
                    )
    ...
    def candidate_chunk_ids(
        ...
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 — a broken index means slow, never broken
    ...
    def ensure_synced(self, dim: int) -> bool:
        ...
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 — reconciliation failure means exact scan
            logger.debug(
                "chunk vector index: reconciliation failed at dim %s: %s", dim, exc
            )
            return False
