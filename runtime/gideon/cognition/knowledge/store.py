...
def _dup_reason(filename_sim: float, cosine: float) -> str:
    ...
    title = (
        "Same title"
        if filename_sim >= 1.0
        else f"Title similarity {_sim2(filename_sim)}"
    )
...
def knowledge_db_path(
    home: "pathlib.Path | None" = None, *, create: bool = True
) -> pathlib.Path:
...
class KnowledgeStore:
    ...
    def _migrate_tags_to_rows(self):
        ...
        try:
            ...
            ids = {
                r["name"]: r["id"]
                for r in self.db.execute("SELECT id, name FROM tags").fetchall()
            }
                ...
                for name in names:
                    self.db.execute(
                        ...
                        (
                            item_id,
                            ids[name],
                            self._migrated_tag_source(item_id, names),
                            now,
                        ),
                    )
    ...
    def _migrated_tag_source(self, item_id: str, names: list[str]) -> str:
        ...
        """
        row = self.db.execute(
            "SELECT insights FROM items WHERE id = ?", (item_id,)
        ).fetchone()
    ...
    def _migrate_sources(self) -> None:
        ...
        item_cols = {
            r[1] for r in self.db.execute("PRAGMA table_info(items)").fetchall()
        }
        ...
        src_cols = {
            r[1] for r in self.db.execute("PRAGMA table_info(sources)").fetchall()
        }
    ...
    def create_typed_item(
        ...
        *,
        ...
        :class:`~gideon.cognition.knowledge.source_engine.SourceEngine` writing a polled
    ...
    def find_item_by_merge_key(
        self, merge_key: str, *, exclude_source_id: str = ""
    ) -> dict | None:
    ...
    def record_also_seen_in(self, item_id: str, *labels: str) -> bool:
        ...
        provider-facing :attr:`~gideon.integrations.knowledge_providers.base.SourceItem.also_seen_in`
        ...
        seen = (
            [str(x) for x in current if str(x).strip()]
    ...
    def forget_source_item(self, source_id: str, guid: str) -> bool:
        ...
        try:
            ...
            self.db.execute(
                "DELETE FROM source_seen WHERE source_id = ? AND guid = ?",
                (source_id, guid),
            )
            self.db.execute("COMMIT")
        except Exception:
        ...
        for start in range(0, len(gated), 400):
            ...
            for row in self.db.execute(
                ...
                try:
                    ...
                    logger.debug(
                        "dup scoring failed for %s", cand.get("id"), exc_info=True
                    )
                    continue
    ...
    def merge_items(
        self, keep_id: str, merge_id: str, *, relink_citations: bool = True
    ) -> dict:
        ...
        for iid in (keep_id, merge_id):
            if (
                self.db.execute("SELECT 1 FROM items WHERE id = ?", (iid,)).fetchone()
                is None
            ):
        ...
        try:
            ...
            cur = self.db.execute(
                "UPDATE collection_items SET item_id = ? WHERE item_id = ?",
                (keep_id, merge_id),
            )
            ...
            cur = self.db.execute(
                "UPDATE item_tags SET item_id = ? WHERE item_id = ?",
                (keep_id, merge_id),
            )
            ...
            cur = self.db.execute(
                "UPDATE annotations SET item_id = ? WHERE item_id = ?",
                (keep_id, merge_id),
            )
            ...
            best_state = max(
                (
                    keep_row["read_state"] or "unread",
                    merge_row["read_state"] or "unread",
                ),
    ...
    def set_item_identity(self, item_id: str, *, kind: str | None = None) -> dict:
        ...
        row = self.db.execute(
            "SELECT title, kind FROM items WHERE id = ?", (item_id,)
        ).fetchone()
        if row is None:
    ...
    def items_linking_to_title(self, title: str) -> list[dict]:
        ...
        pattern = re.compile(
            r"\[\[" + re.escape(needle) + r"(?:\|[^\]]*)?\]\]", re.IGNORECASE
        )
        out: list[dict] = []
        for row in rows:
    ...
    def rewrite_wikilinks(self, old_title: str, new_title: str) -> dict:
        ...
        pattern = re.compile(
            r"\[\[" + re.escape(old) + r"(\|[^\]]*)?\]\]", re.IGNORECASE
        )
        links = 0
        for ref in self.items_linking_to_title(old):
            row = self.db.execute(
                "SELECT content FROM items WHERE id = ?", (ref["id"],)
            ).fetchone()
            if row is None:
                continue
            body = str(row["content"] or "")
            updated, count = pattern.subn(
                lambda m: f"[[{new}{m.group(1) or ''}]]", body
            )
            if not count:
                continue
    ...
    def snapshot_items(self, item_ids: Sequence[str]) -> dict:
        ...
        for table, columns in self._SNAPSHOT_TABLES:
            ...
            children[table] = [
                {k: _snapshot_value(r[k]) for k in r.keys()} for r in found
            ]
        return {
            "ids": ids,
            "existing": existing,
        ...
        try:
            for iid in ids:
                if iid in existing:
                    continue
                if self.db.execute(
                    "SELECT 1 FROM items WHERE id = ?", (iid,)
                ).fetchone():
            ...
            for table, columns in self._SNAPSHOT_TABLES:
                where = " OR ".join(
                    f"{col} IN ({_placeholders(ids)})" for col in columns
                )
                self.db.execute(
                ...
                for child in children.get(table, []):
                    ...
                    try:
                        ...
                        logger.debug(
                            "undo skipped a %s row: %r", table, child, exc_info=True
                        )
                        continue
                    restored += 1
    ...
    def load_undo(self, token: str) -> dict | None:
        ...
        row = self.db.execute(
            "SELECT * FROM restructure_undo WHERE token = ?", (token,)
        ).fetchone()
        if row is None:
            return None
        out = dict(row)
    ...
    def set_item_citations(self, item_id: str, citations: Sequence[Any]) -> int:
        ...
        Accepts :class:`~gideon.cognition.knowledge.citations.Citation` objects or plain dicts --
        ...
        for citation in citations:
            if isinstance(citation, dict):
                ...
                source_id = (
                    citation.get("source_item_id") or citation.get("item_id") or ""
                )
    ...
    def delete_intent_outcomes(self, intent_id: str) -> int:
        ...
        cur = self.db.execute(
            "DELETE FROM intent_outcomes WHERE intent_id = ?", (intent_id,)
        )
        self.db.commit()
        return cur.rowcount
    ...
    def clear_item_intent_outcomes(self, item_id: str) -> int:
        ...
        """
        cur = self.db.execute(
            "DELETE FROM intent_outcomes WHERE item_id = ?", (item_id,)
        )
        self.db.commit()
        return cur.rowcount
    ...
    def _delete_item_cascade(self, item_id):
        ...
        self.db.execute(
            "DELETE FROM entity_relations WHERE source_item_id = ?", (item_id,)
        )
        self.db.execute(
    ...
    def items_missing_mention_sweep(
        self, limit: int, after_id: str | None = None
    ) -> list[dict]:
    ...
    def items_missing_similarity_sweep(
        self, limit: int, after_id: str | None = None
    ) -> list[str]:
    ...
    _VAULT_PROJECTABLE = (
        "COALESCE(v.owner_deleted, 0) = 0 AND COALESCE(v.conflict, '') = ''"
    )

    def items_needing_vault_projection(
        self, limit: int, after_id: str | None = None
    ) -> list[dict]:
    ...
    def vault_projections(
        self, *, limit: int = 0, after_item_id: str = ""
    ) -> list[dict]:
    ...
    def vault_tag_membership(self) -> list[dict]:
        ...
        return [
            {"tag": r["tag"], "id": r["item_id"], "title": r["title"] or ""}
            for r in rows
        ]
    ...
    def mark_vault_page_deleted(self, item_id: str) -> None:
        ...
        self.db.execute(
            "UPDATE vault_projections SET owner_deleted = 1 WHERE item_id = ?",
            (item_id,),
        )
        self.db.commit()
    ...
    def release_similarity_claims(
        self, item_id: str, keep: set[tuple[str, str]]
    ) -> int:
    ...
    def similar_items(
        ...
        for r in rows:
            ...
            out.append(
                {
                    "item_id": (
                        r["target_item_id"] if mine_is_source else r["source_item_id"]
                    ),
                    "score": float(r["score"]),
                    "chunk_index": (
                        r["source_chunk_index"]
                        if mine_is_source
                        else r["target_chunk_index"]
                    ),
                    "neighbour_chunk_index": (
                        r["target_chunk_index"]
                        if mine_is_source
                        else r["source_chunk_index"]
                    ),
                }
            )
        return out
    ...
    def backfill_entity_description(
        self, entity_id: str, description: str | None
    ) -> bool:
    ...
    def merge_entities(self, keep_id, merge_id):
        self.db.execute(
            "UPDATE entity_relations SET source_id = ? WHERE source_id = ?",
            (keep_id, merge_id),
        )
        self.db.execute(
            "UPDATE entity_relations SET target_id = ? WHERE target_id = ?",
            (keep_id, merge_id),
        )
        self.db.execute(
    ...
    def get_entity_subgraph(self, entity_id, depth=2) -> dict:
        ...
        for nid in visited:
            data = self.graph.nodes.get(nid, {})
            nodes.append(
                {"id": nid, "name": data.get("name"), "type": data.get("entity_type")}
            )
        edges = []
    ...
    def set_tag_parent(self, tag_id: int, parent_id: int | None) -> bool:
        ...
        if parent_id is not None:
            ...
            if not self.db.execute(
                "SELECT id FROM tags WHERE id = ?", (parent_id,)
            ).fetchone():
                raise ValueError(f"no such parent tag: {parent_id}")
            if tag_id in self._tag_ancestors(parent_id):
                raise ValueError("tag_cycle")
        self.db.execute(
            "UPDATE tags SET parent_id = ? WHERE id = ?", (parent_id, tag_id)
        )
        self.db.commit()
        return True
    ...
    def _resync_fts(self, snapshot: list[tuple]) -> None:
        ...
        for rowid, title, content, tags in snapshot:
            ...
            fresh = self.db.execute(
                "SELECT title, content, tags FROM items_fts_src WHERE rowid = ?",
                (rowid,),
            ).fetchone()
            if fresh is None:
                continue
    ...
    def create_collection(
        ...
        now = datetime.now().isoformat()
        nxt = self.db.execute(
            "SELECT COALESCE(MAX(position), -1) + 1 AS p FROM collections"
        )
    ...
    def bulk_apply(self, op: str, item_ids: list[str], **args) -> dict:
        ...
        if op not in self.BULK_OPS:
            raise ValueError(
                f"unknown bulk op {op!r}; expected one of {list(self.BULK_OPS)}"
            )
        ...
        for item_id in item_ids:
            ...
            if op == "collect":
                ...
                did = bool(row["favorited"]) != value and self.set_favorited(
                    item_id, value
                )
