...
def effective_budgets() -> dict[str, int]:
    ...
    try:
        ...
        configured = int(
            getattr(AppConfig.load().knowledge, "report_budget_chars", 0) or 0
        )
        if configured > 0:
            budgets["report"] = configured
    except Exception:
        logger.debug(
            "knowledge budget config unreadable — using defaults", exc_info=True
        )
    return budgets
...
def citations_required() -> bool:
    ...
    try:
        ...
        logger.debug(
            "knowledge citation config unreadable — requiring citations", exc_info=True
        )
        return True
...
def _claim_id(statement: str) -> str:
    ...
    return (
        "c-"
        + hashlib.sha256(
            " ".join(statement.split()).casefold().encode("utf-8")
        ).hexdigest()[:12]
    )
...
class PersistCheck:
    ...
    def to_result(self) -> dict[str, Any]:
        return (
            {"success": self.ok, "error": self.error}
            if not self.ok
            else {"success": True}
        )
...
def check_persist(
    *,
    ...
    if not (title or "").strip():
        return PersistCheck(
            False, "a knowledge item needs a title (it is half its identity)"
        )
...
def validate_relation(
    ...
    *,
    ...
    if verb not in RELATION_TYPES:
        return (
            None,
            f"unknown relation {relation_type!r} — one of: {', '.join(RELATION_TYPES)}",
        )
