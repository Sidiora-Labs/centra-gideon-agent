...
``ConsoleState.notify`` → ``notification_allowed()``*. Synthesis is a **background one-shot**
...
``SourceItemIngested`` records carry a monotonic ``seq`` (:mod:`gideon.cognition.knowledge
...
is wrapped by the ONE core fence (:func:`gideon.security.security.fence_untrusted`) with
...
def read_cursor(path: Path | None = None) -> int:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 — a corrupt cursor re-reads the window, never skips it
...
def collect_window(
    *,
    ...
    :mod:`gideon.cognition.knowledge.source_queries`' grammar, so the digest narrows with the same
    ...
    from gideon.cognition.knowledge.source_streams import SOURCE_ITEM_INGESTED
    ...
    for record in records:
        ...
        try:
            ...
            Exception
        ):  # noqa: BLE001 — a missing row is a skipped item, not a failed digest
        ...
        entry = {
            ...
            "source_name": _source_name(
                knowledge_store, str(payload.get("source_id") or "")
            ),
...
async def run_morning_digest(
    *,
    ...
    ``state`` is the :class:`~gideon.interfaces.dashboard.state.ConsoleState` whose ``notify`` is
...
UNSYNTHESISED_BODY = (
    "Digest synthesis was unavailable. The collected items are in your library."
)


async def _synthesise(
    prompt: str, completion_fn: Callable[..., Awaitable[str]] | None
) -> str:
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 — the items are already in the library; say so plainly
        logger.warning(
            "digest synthesis failed; writing an unsynthesised digest", exc_info=True
        )
        return UNSYNTHESISED_BODY
    if not narrative.strip():
        logger.warning(
            "digest synthesis produced nothing; writing an unsynthesised digest"
        )
        return UNSYNTHESISED_BODY
    return narrative
...
def _notify(state: Any, *, title: str, body: str, item_count: int) -> bool:
    """One notification through ``ConsoleState.notify`` → ``notification_allowed()``.
    ...
    try:
        ...
        summary = (
            body.strip().splitlines()[0] if body.strip() else f"{item_count} new items"
        )
        state.notify(
