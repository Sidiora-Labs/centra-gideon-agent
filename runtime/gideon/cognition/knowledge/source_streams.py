...
* :data:`SOURCE_QUERY_MATCHED` ``{query_id, item_id}`` — a saved source query matched a
  newly-ingested item (§6.4, :mod:`gideon.cognition.knowledge.source_queries`).
...
(:func:`gideon.security.security.fence_untrusted`) with ``source=f"source:{source_id}"`` per §8.
...
def spool_path() -> Path:
    ...
    Resolved per call through :func:`gideon.core.config.loader.config_dir` — never captured
