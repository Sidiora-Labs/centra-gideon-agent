...
async def _lexicon_bias_terms(ctx: NodeContext) -> list[str] | None:
    ...
    try:
        from gideon.cognition.lexicon import select_bias_terms
    except Exception:
        return None
    try:
...
class LexiconCorrectionNode:
    ...
    async def run(self, inputs, ctx: NodeContext) -> NodeOutput:
        ...
        try:
            ...
            logger.debug(
                "lexicon_correction failed (non-fatal); passing transcript through",
                exc_info=True,
            )
            return NodeOutput(
                node_type=self.node_type,
...
class DiarizationNode:
    ...
    async def run(self, inputs, ctx: NodeContext) -> NodeOutput:
        ...
        try:
            from gideon.integrations.diarize import diarize_audio
...
def _speaker_turns_from(inputs: dict) -> list[dict]:
    for o in inputs.values():
        if (
            ...
            and isinstance(o.metadata.get("speaker_turns"), list)
        ):
            ...
            seg = {
                **seg,
                "speaker": _speaker_for(
                    seg.get("start", 0.0), seg.get("end", 0.0), turns
                ),
            }
...
def _transcript_from(inputs: dict) -> dict | None:
    ...
    for o in inputs.values():
        if (
            o
            and isinstance(o.metadata, dict)
            and isinstance(o.metadata.get("transcript"), dict)
        ):
...
class FrameExtractNode:
    ...
    async def run(self, inputs, ctx: NodeContext) -> NodeOutput:
        ...
        if dense_regions:
            dense_fps = min(2.0, 0.3 * (2**iteration))
            frames = list(self._existing_frames(work, ctx.item_id))
            for ri, region in enumerate(dense_regions):
                start = float(region.get("start", 0))
                end = float(region.get("end", 0))
                pat = os.path.join(
                    work, f"{ctx.item_id}.dense{iteration}_{ri}_%03d.jpg"
                )
...
class VideoClassifyNode:
    ...
    async def run(self, inputs, ctx: NodeContext) -> NodeOutput:
        ...
        content_cls = (
            "text-heavy"
            if "text" in v
            else "talking-head" if "talking" in v else "visual"
        )
...
def _frames_from(inputs: dict) -> list[str]:
    ...
    for o in inputs.values():
        if o and o.artifacts:
            imgs = [
                a for a in o.artifacts if a.lower().endswith((".jpg", ".jpeg", ".png"))
            ]
            if imgs:
                return imgs
    return []
