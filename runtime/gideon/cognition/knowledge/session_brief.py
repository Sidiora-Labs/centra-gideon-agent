...
def _rank(item: BriefItem) -> tuple[int, int, str]:
    ...
    kind_rank = (
        PRIORITY_KINDS.index(item.kind)
        if item.kind in PRIORITY_KINDS
...
def _invert(stamp: str) -> str:
    ...
    return "".join(
        chr(0x10FFFF - ord(ch)) if ord(ch) < 0x10FFFF else ch for ch in (stamp or "")
    )
...
def load_items(store: Any, *, project_id: str, limit: int = 50) -> list[BriefItem]:
    ...
    try:
        ...
        logger.debug(
            "session brief query failed for project %r", project_id, exc_info=True
        )
        return []
...
def build(
    store: Any, *, project_id: str, max_tokens: int = DEFAULT_MAX_TOKENS
) -> SessionBrief:
