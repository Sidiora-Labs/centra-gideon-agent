...
it subclasses :class:`~gideon.integrations.knowledge_providers.base.KnowledgeSourceProvider` (the
...
:meth:`~gideon.cognition.knowledge.store.KnowledgeStore.create_typed_item`, which folds the
...
it advance the cursor (:meth:`~gideon.cognition.knowledge.store.KnowledgeStore.record_poll`).
...
class SourceEngine:
    ...
    def _queries(self) -> Any:
        ...
        if self._saved_queries is None:
            from gideon.cognition.knowledge.source_queries import SavedQueryStore

            self._saved_queries = SavedQueryStore()
        return self._saved_queries

    def _emit_ingested(
        self, source: dict, item: Any, item_id: str, change: str
    ) -> None:
        ...
        try:
            from gideon.cognition.knowledge import source_queries
            ...
            Exception
        ):  # noqa: BLE001 — a query fault must not lose the ingested item
            logger.debug(
                "saved source query evaluation failed for %s", item_id, exc_info=True
            )
    ...
    def _emit_poll_completed(
        ...
        *,
        ...
        from gideon.cognition.knowledge.source_streams import SOURCE_POLL_COMPLETED
    ...
    def _interval_for(self, source: dict, cfg: Any) -> float:
        ...
        want = int(source.get("poll_interval_secs") or 0) or int(
            cfg.poll_interval_default_secs
        )
    ...
    async def poll_source(self, source: dict, cfg: Any) -> int:
        ...
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 — a provider that raises must not kill the loop
    ...
    def _merge_cross_source(self, source: dict, item: Any) -> bool:
        ...
        (:func:`~gideon.cognition.knowledge.source_identity.merge_key`, which returns no key
        ...
        * the surviving item gains the second source's attribution, APPENDED. Dropping the
        ...
        """
        from gideon.cognition.knowledge.source_identity import merge_key

        key = merge_key(getattr(item, "url", "") or "")
        if not key:
            return False
        existing = self._store.find_item_by_merge_key(
            key, exclude_source_id=source["id"]
        )
        if existing is None:
            return False
    ...
    def _create_new(self, source: dict, item: Any) -> int:
        ...
        from gideon.integrations.knowledge_providers.base import CHANGE_CREATED
    ...
    def _reindex_modified(self, source: dict, item: Any) -> int:
        ...
        self._enqueue(item_id)
        from gideon.integrations.knowledge_providers.base import CHANGE_MODIFIED
    ...
    def _archive_deleted(self, source: dict, item: Any) -> int:
        ...
        store exposes only :meth:`~gideon.cognition.knowledge.store.KnowledgeStore.archive_source_item`
    ...
    def _enqueue(self, item_id: str) -> None:
        try:
            ...
            Exception
        ):  # noqa: BLE001 — a queue hiccup must not lose the written item
    ...
    async def tick(self) -> float:
        ...
        active = [s for s in sources if s["provider"] in enrolled][
            : int(cfg.max_sources)
        ]
    ...
    async def run_forever(
        self, sleep_fn: Callable[[float], Awaitable[None]] | None = None
    ) -> None:
        ...
        while not shutdown_event.is_set():
            ...
            try:
                ...
                Exception
            ):  # noqa: BLE001 — the loop must outlive any single tick failure
