...
def _config_int(field: str, default: int) -> int:
    try:
        ...
        logger.debug(
            "embed batching: %s unreadable; using %d", field, default, exc_info=True
        )
        return default
...
def embed_texts(
    ...
    *,
    ...
    if embed_many is None and embed_one is None:
        logger.debug(
            "embed batching: no embedder available; %d text(s) stay vector-less",
            len(texts),
        )
        return out
    ...
    *,
    ...
    for attempt in range(budget):
        try:
            ...
            Exception
        ) as exc:  # noqa: BLE001 — the whole point is to classify, not propagate
            last_exc = exc
            if _is_terminal(exc):
                logger.warning(
